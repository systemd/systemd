...
def call(*command, **kwargs):
    ...
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, check=False, text=True, stderr=subprocess.DEVNULL, **kwargs).returncode
...
class DevlinkdTests(unittest.TestCase, DevlinkdTestHelpers):
    ...
    def test_param_set(self):
        ...
        self.assertIs(test1_value, False)
...
def skip_when_netdevsim_not_available():
    def skip():
        call_quiet('rmmod netdevsim')
        sys.exit(77)
...
if __name__ == '__main__':
    skip_when_netdevsim_not_available()
    unittest.main()
