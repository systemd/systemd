...
case "$1" in
    setup)
        ...
        if [[ "$(stat --file-system --format %T /)" == "btrfs" ]] && [[ -d /snapshot ]]; then
        ...
        if [[ "$(stat --file-system --format %T /)" == "btrfs" ]]; then
    ...
    finalize)
        ...
        # If the test involves rebooting the machine, this will manifest in the test being killed by a signal
        # so let's assume we're still in progress if we're killed by a signal.
        if [[ "$SERVICE_RESULT" != "signal" ]]; then
            rm -f /inprogress
        fi
        ;;
    *)
