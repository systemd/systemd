...
def enableSystemdResolved(console):
    ...
    console.sendline("mkdir -p /run/systemd/resolved.conf.d")
    console.sendline("echo [Resolve] >/run/systemd/resolved.conf.d/mdns-llmnr.conf")
    console.sendline("echo MulticastDNS=yes >>/run/systemd/resolved.conf.d/mdns-llmnr.conf")
    console.sendline("echo LLMNR=yes >>/run/systemd/resolved.conf.d/mdns-llmnr.conf")
    console.expect("#", 5)
    console.sendline("resolvectl mdns host0 true")
    console.expect("#", 5)
    console.sendline("echo $?")
    console.expect('0\r', 2)
...
def run_test(args):
    ...
    rootdir=rootpath.split('=')[1].replace('/root','')
