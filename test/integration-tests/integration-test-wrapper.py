...
def process_sanitizer_report(args: argparse.Namespace, journal_file: Path) -> bool:
    ...
    # 'Standard' errors:
    #
    # Note: there's currently a bug in LLVM 22 due to which certain systemd
    # units can throw following warning:
    # [ 3366.747202] systemd-oomd[93]: ==93==WARNING: ptrace appears to be blocked (is seccomp enabled?). LeakSanitizer may hang.
