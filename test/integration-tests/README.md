...
`TEST_SKIP_SUBTESTS=subtest`: takes a space separated list of POSIX extended
regexes (matched unanchored, like `$TEST_MATCH_SUBTEST`) naming subtests to
skip.

`TEST_SKIP_TESTCASES=testcase`: takes a space separated list of POSIX extended
regexes (matched unanchored) naming testcases to skip.

`TEST_MATCH_SCENARIO=scenario`: run only conformance-scorecard scenarios
(TEST-89) matching this POSIX extended regex (matched unanchored). An expression
matching no scenario fails that test: it is the only one reading the variable.

`TEST_SKIP_SCENARIOS=scenario`: takes a space separated list of POSIX extended
regexes (matched unanchored) naming conformance-scorecard scenarios to skip.
