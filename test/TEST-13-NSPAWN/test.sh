...
test_append_files() {
    ...
    local container_full="$workspace/usr/share/TEST-13-NSPAWN-container-full-template"

    _install_base_container "$container"
    _install_base_container "$container_full"
    initdir="$container_full" install_systemd
}
