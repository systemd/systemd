...
EOF
    ...
    KERNEL_APPEND="systemd.setenv=TEST_FUNCTION_NAME=${FUNCNAME[0]} ${USER_KERNEL_APPEND:-}"
    # Limit the number of VCPUs, disable KVM, and set a timeout to make sure
    # we trigger the issue
    QEMU_OPTIONS="${qemu_opts[*]} -machine accel=tcg ${USER_QEMU_OPTIONS:-}"
    QEMU_SMP=1 QEMU_KVM=no QEMU_TIMEOUT=180 test_run_one "${1:?}"
}
