...
can_test_pkcs11() {
    ...
    if ! get_bool $(grep -F ' HAVE_P11KIT ' "${BUILD_DIR:?}/config.h"| awk '{ print $3 }'); then
        ddebug "Support for p11-kit is disabled, skipping the PKCS#11 test"
        return 1
    fi
    if ! get_bool $(grep -F ' HAVE_LIBCRYPTSETUP ' "${BUILD_DIR:?}/config.h"| awk '{ print $3 }'); then
        ddebug "Support for libcryptsetup is disabled, skipping the PKCS#11 test"
        return 1
    fi
    if ! get_bool $(grep -F ' HAVE_LIBCRYPTSETUP_PLUGINS ' "${BUILD_DIR:?}/config.h"| awk '{ print $3 }'); then
