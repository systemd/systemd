...
EOF
    ...
    # RSA #####################################################
    pkcs11-tool --module "/usr/lib/pkcs11/libsofthsm2.so" --token-label "TestToken" --pin "env:GNUTLS_PIN" --so-pin "env:GNUTLS_SO_PIN" --keypairgen --key-type "RSA:2048" --label "RSATestKey" --usage-decrypt
    ...
    pkcs11-tool --module "/usr/lib/pkcs11/libsofthsm2.so" --token-label "TestToken" --pin "env:GNUTLS_PIN" --so-pin "env:GNUTLS_SO_PIN" --write-object "/tmp/rsa_test.crt" --type cert --label "RSATestKey"
    rm "/tmp/rsa_test.crt"

    # prime256v1 ##############################################
    pkcs11-tool --module "/usr/lib/pkcs11/libsofthsm2.so" --token-label "TestToken" --pin "env:GNUTLS_PIN" --so-pin "env:GNUTLS_SO_PIN" --keypairgen --key-type "EC:prime256v1" --label "ECTestKey" --usage-derive
    ...
    pkcs11-tool --module "/usr/lib/pkcs11/libsofthsm2.so" --token-label "TestToken" --pin "env:GNUTLS_PIN" --so-pin "env:GNUTLS_SO_PIN" --write-object "/tmp/ec_test.crt" --type cert --label "ECTestKey"
    rm "/tmp/ec_test.crt"
    ...
    inst_libs "/usr/lib/pkcs11/libsofthsm2.so"
    inst_library "/usr/lib/pkcs11/libsofthsm2.so"
    inst_simple "/usr/share/p11-kit/modules/softhsm2.module"
