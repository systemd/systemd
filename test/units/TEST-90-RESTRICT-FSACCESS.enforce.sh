...
# The kernel silently ignores the option if it predates it (Linux 6.12): a write that goes through
# means a kernel too old to test on, which "check" below would otherwise report as a hard failure.
rc=0
"$HELPER" procmem-cow /usr/bin/true || rc=$?
if [[ "$rc" -eq 0 ]]; then
    echo "Kernel does not honour proc_mem.force_override=never (needs Linux 6.12+), skipping"
    exit 77
fi
