...
#
# Drive the same kernel path (dm-verity signature verification →
# security_bdev_setintegrity → our bdev_setintegrity hook → trusted dev_t in
# the map → bprm_check_security on exec) via systemd-run with RootImage=.
...
if [[ -e "$MINIMAL.raw" ]] &&
   [[ -e "$MINIMAL.verity" ]] &&
   [[ -e "$MINIMAL.roothash" ]] &&
   [[ -e "$MINIMAL.roothash.p7s" ]] &&
   machine_supports_verity_keyring; then

    # bash is in minimal-base (mkosi/mkosi.images/minimal-base/mkosi.conf) and
    # standalone (not multicall), so no basename-dispatch concern. --version
    # is a builtin, no sub-exec. systemd-run with RootImage= sets up dm-verity,
    # runs in the image's mount namespace, and tears everything down on exit.
    systemd-run --pipe --wait \
        --property RootImage="$MINIMAL.raw" \
        bash --version >/dev/null
    echo "Execution from signed dm-verity device: OK"
else
    echo "Pre-built signed verity image not available, skipping positive verity test"
fi
