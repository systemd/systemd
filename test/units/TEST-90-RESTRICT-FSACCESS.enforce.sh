...
if [[ ! -x "$HELPER" ]]; then
    echo "test-bpf-restrict-fsaccess helper not found at $HELPER, skipping"
    exit 0
fi
...
at_exit() {
    ...
    if [[ -e /dev/mapper/test-restrict-fsaccess-verity ]]; then
        umount /tmp/verity-mount 2>/dev/null || true
        veritysetup close test-restrict-fsaccess-verity 2>/dev/null || true
    fi
    rm -rf /tmp/verity-mount /tmp/restrict-fsaccess-workdir /tmp/restrict-fsaccess-attach.out
}
trap at_exit EXIT
...
cp /usr/bin/true /tmp/restrict-fsaccess-baseline/true-baseline
chmod +x /tmp/restrict-fsaccess-baseline/true-baseline
if ! /tmp/restrict-fsaccess-baseline/true-baseline 2>/dev/null; then
...
cp /usr/bin/true /tmp/restrict-fsaccess-test/true-on-tmpfs
chmod +x /tmp/restrict-fsaccess-test/true-on-tmpfs

# This should fail with EPERM
if /tmp/restrict-fsaccess-test/true-on-tmpfs 2>/dev/null; then
...
if command -v veritysetup >/dev/null 2>&1 &&
   ...
   machine_supports_verity_keyring; then
    ...
    WORKDIR=/tmp/restrict-fsaccess-workdir
    mkdir -p "$WORKDIR/img"
    ...
    veritysetup open \
        "$WORKDIR/test.raw" test-restrict-fsaccess-verity \
        "$WORKDIR/test.verity" \
    ...
    mount -o ro /dev/mapper/test-restrict-fsaccess-verity /tmp/verity-mount

    /tmp/verity-mount/true-on-verity
    echo "Execution from signed dm-verity device: OK"

    umount /tmp/verity-mount
    veritysetup close test-restrict-fsaccess-verity
...
if [[ -x /tmp/restrict-fsaccess-test/true-on-tmpfs ]]; then
    /tmp/restrict-fsaccess-test/true-on-tmpfs
