...
# or a finalizing runner is about to turn it into a proper skip. The testcase pair keeps the
# same books for run_testcases' own empty-run guard.
_UNMATCHED_TESTS=()
_EXCLUDED_TESTS=()
_UNMATCHED_TESTCASES=()
_EXCLUDED_TESTCASES=()
...
# one helper, so no two call sites can answer that differently -- and under nullglob the array
# is empty, which the :- keeps from tripping set -u. Prints the list, fails when there is none.
_list_subtests() {
...
_subtest_filtered_out() {
    ...
    # read -rd '' rather than an unquoted expansion: the words are EREs, and several
    # legitimate ones (a trailing *, a bracket class) are also globs the shell would otherwise
    # replace with matching filenames -- and unlike plain read -ra, the empty delimiter keeps
    # newline a separator, as the unquoted expansion had it (the wrapper deliberately
    # preserves newlines in these values). read -d '' returns 1 at EOF, hence the || :.
    read -rd '' -a skips <<<"${TEST_SKIP_SUBTESTS:-}" || :
...
# The testcase counterpart, keeping the books run_testcases' empty-run guard and summary read.
_testcase_filtered_out() {
    local testcase="${1:?}"
    local skips=() skip

    if [[ -n "${TEST_MATCH_TESTCASE:-}" ]] && ! [[ "$testcase" =~ $TEST_MATCH_TESTCASE ]]; then
        echo "Skipping $testcase (not matching '$TEST_MATCH_TESTCASE')"
        _UNMATCHED_TESTCASES+=("$testcase")
        return 0
    fi

    read -rd '' -a skips <<<"${TEST_SKIP_TESTCASES:-}" || :
    for skip in "${skips[@]}"; do
        if [[ "$testcase" =~ $skip ]]; then
            echo "Skipping $testcase (matching '$skip')"
            _EXCLUDED_TESTCASES+=("$testcase")
            return 0
        fi
    done
...
run_testcases() {
    ...
    # Per call: a script with two testcase phases reports each one's filtering on its own.
    _UNMATCHED_TESTCASES=()
    _EXCLUDED_TESTCASES=()
    ...
    for testcase in "${testcases[@]}"; do
        if _testcase_filtered_out "$testcase"; then
            continue
        fi
    ...
    if [[ ${#_UNMATCHED_TESTCASES[@]} -gt 0 || ${#_EXCLUDED_TESTCASES[@]} -gt 0 ]]; then
        printf "FILTERED OUT TESTCASES: %3d:\n" "$((${#_UNMATCHED_TESTCASES[@]} + ${#_EXCLUDED_TESTCASES[@]}))"
        printf "        %s\n" "${_UNMATCHED_TESTCASES[@]}" "${_EXCLUDED_TESTCASES[@]}"
    fi
    ...
    if [[ $_EXECUTED_TESTCASES -eq 0 && ( ${#_UNMATCHED_TESTCASES[@]} -gt 0 || ${#_EXCLUDED_TESTCASES[@]} -gt 0 ) ]]; then
        ...
        # in which the requested testcase never executed. A skip list that excluded whatever
        # was selected, by contrast, is a deliberate exclusion: carry on to the subtests when
        # there are any. (No /skipped write here: that file asserts the whole test skipped,
        # and this may be one subtest process among many.)
        if [[ ${#_EXCLUDED_TESTCASES[@]} -eq 0 ]]; then
            echo "TEST_MATCH_TESTCASE='${TEST_MATCH_TESTCASE:-}' matched no testcase; skipping"
            exit "$_SUBTEST_SKIP_RC"
        fi
        local also=""
        [[ ${#_UNMATCHED_TESTCASES[@]} -gt 0 ]] && also=" (and TEST_MATCH_TESTCASE='${TEST_MATCH_TESTCASE:-}')"
        if _list_subtests >/dev/null; then
            echo "TEST_SKIP_TESTCASES='${TEST_SKIP_TESTCASES:-}'$also filtered out every testcase; continuing to this test's subtests"
        else
            echo "TEST_SKIP_TESTCASES='${TEST_SKIP_TESTCASES:-}'$also filtered out every testcase; skipping"
            exit "$_SUBTEST_SKIP_RC"
        fi
    fi
}
