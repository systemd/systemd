...
at_exit() {
    ...
    systemctl stop pty96-input.service
    rm -f /tmp/pty96-input-*
}
...
systemd-run -u pty96-input.service \
    -p StandardInput=broker -p StandardOutput=broker \
    bash -c 'stty raw -echo; exec cat >/tmp/pty96-input-received'
timeout 10 bash -c 'until ptyctl list | grep pty96-input.service >/dev/null; do sleep .5; done'

PTY_NAME=$(varlinkctl --collect call /run/systemd/io.systemd.PTYBroker io.systemd.PTYBroker.ListPty '{}' | jq -r '.[] | select(.description == "pty96-input.service") | .name')
[[ -n "$PTY_NAME" ]]
...
UNIT = 'pty96-input.service'
RECEIVED = '/tmp/pty96-input-received'
...
marker = b'hello-pty96-input-marker-1\n'
send(marker)
expected += marker
...
send(seq[8:])
marker = b'hello-pty96-input-marker-2\n'
send(marker)
expected += seq + marker
assert wait_for_received(marker) == expected
...
time.sleep(0.5)
send(seq[10:])
marker = b'hello-pty96-input-marker-3\n'
send(marker)
expected += marker
assert wait_for_received(marker) == expected
...
send(seq)
marker = b'hello-pty96-input-marker-4\n'
send(marker)
expected += seq + marker
assert wait_for_received(marker) == expected
...
bulk = b''.join(b'pty96-input-bulk-%08d\n' % i for i in range(2 * BUFFER_MAX // 26))
...
send(view)
marker = b'hello-pty96-input-marker-5\n'
send(marker)
...
systemctl stop pty96-input.service
wait_for_pty_deregistration pty96-input.service
