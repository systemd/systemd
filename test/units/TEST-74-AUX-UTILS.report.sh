...
# should be positive and no larger than what /proc/stat shows now with some margin. This would
# catch a bogus tick-to-nanosecond conversion.
cpu_idle_reported="$(basic_number CPUWait '.fields.type == "idle"')"
awk -v hz="$(getconf CLK_TCK)" -v sampled="$cpu_idle_reported" \
    '/^cpu / { exit !(sampled > 0 && sampled <= $5 / hz * 1e9 + hz ) }' /proc/stat
...
for dev in /sys/block/*; do
    name="${dev##*/}"
    [ -e "/sys/devices/virtual/block/$name" ] && continue

    name="${name//\!//}"  # translate e.g. "cciss!c0d0" to "cciss/c0d0" like sd_device_get_sysname()

    read_bytes="$(basic_number DiskReadBytes ".object == \"$name\"")"
    write_bytes="$(basic_number DiskWriteBytes ".object == \"$name\"")"
    test -n "$read_bytes"
    test -n "$write_bytes"
