...
EOF
    ...
    systemctl --reset-failed TEST-82-SOFTREBOOT-trigger.service || :
...
check_device_property() {
    ...
    local expected_action=bind
    local expected_softreboot_count=1
    local count=0
    while read -r action seqnum softreboot_count; do
        test "$seqnum" -gt "$previous_seqnum"
        assert_eq "$action" "$expected_action"
        assert_eq "$softreboot_count" "$expected_softreboot_count"

        previous_seqnum="$seqnum"
        if [[ "$action" == bind ]]; then
            # Triggered by systemd-udev-trigger.service
            expected_action=add
        else
            expected_action=bind
            : $((++expected_softreboot_count))
        fi
        : $((++count))
...
if [ -f /run/TEST-82-SOFTREBOOT.touch3 ]; then
    ...
    # This service triggers a uevent after systemd-udevd has stopped.
    systemd-run \
        --unit=TEST-82-SOFTREBOOT-trigger.service \
        --property Before=systemd-udevd.service \
        --property RemainAfterExit=yes \
