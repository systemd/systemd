...
verify_device_renaming_test() {
    ...
    timeout 30 bash -c 'while [[ "$(systemctl show --property=ActiveState --value /sys/subsystem/net/devices/testif0)" != "inactive" ]]; do sleep .5; done'
    timeout 30 bash -c 'while [[ "$(systemctl show --property=ActiveState --value /sys/devices/virtual/net/testif0)" != "inactive" ]]; do sleep .5; done'
    timeout 30 bash -c 'while [[ "$(systemctl show --property=ActiveState --value /sys/subsystem/net/devices/testrenamed0)" == "active" ]]; do sleep .5; done'
    timeout 30 bash -c 'while [[ "$(systemctl show --property=ActiveState --value /sys/devices/virtual/net/testrenamed0)" == "active" ]]; do sleep .5; done'
