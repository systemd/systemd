...
if grep -qw luo_nboot=1 /proc/cmdline; then
    ...
    # PID 1 should have logged the skip during the previous boot's shutdown/kexec. This is
    # best-effort: the warning is emitted late in the shutdown transition and may land on the
    # console/kmsg after journald has been shut down, so a missing line is not a failure. The
    # deterministic proof that the anti-hijack guard fired is the check-hijack/NFileDescriptorStore
    # assertions above.
    if journalctl -b -1 --no-pager | grep "infringes systemd's namespace" >/dev/null; then
        echo "Found anti-hijack warning in previous boot journal."
    else
        echo "Anti-hijack warning not found in previous boot journal (emitted late in shutdown, not fatal)."
    fi
