...
(
    set +e; \
    systemd-run -q -t --property SystemCallFilter=~open_tree getent passwd definitelynotarealuser; \
    test $? -eq 2
)
(
    set +e; \
    systemd-run -q -t --property SystemCallFilter=~open_tree getent group definitelynotarealgroup; \
    test $? -eq 2
)
set +o pipefail
