...
if systemd-analyze compare-versions "$(uname -r)" ge 6.19 && systemctl is-enabled systemd-coredumpd.service; then
    timeout 30 bash -xec 'until systemctl -q is-active systemd-coredumpd.service; do sleep 1; done'
fi
