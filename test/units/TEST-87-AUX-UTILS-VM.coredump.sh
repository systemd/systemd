...
if ! systemctl start systemd-coredump-kernel.socket ||
        ! systemctl -q is-active systemd-coredump-kernel.socket; then
    echo "The kernel seems to not support coredump kernel socket feature. Skipping the remaining test cases."
    exit 0
fi
