...
systemd-run --wait --unit=two -p ExitType=cgroup -p ExecCondition=true /tmp/test59-exit-cgroup.sh

# false exec condition: systemd-run should exit immediately with status code: 1
! systemd-run --wait --unit=three -p ExitType=cgroup -p ExecCondition=false /tmp/test59-exit-cgroup.sh

# service should exit uncleanly
(sleep 1; systemctl kill --signal 9 four) &
! systemd-run --wait --unit=four -p ExitType=cgroup /tmp/test59-exit-cgroup.sh
...
systemd-run --wait --unit=five -p ExitType=cgroup /tmp/test59-exit-cgroup-parentless.sh

# service should exit uncleanly
(sleep 1; systemctl kill --signal 9 six) &
! systemd-run --wait --unit=six -p ExitType=cgroup /tmp/test59-exit-cgroup-parentless.sh
...
systemd-run --wait --unit=seven -p ExitType=cgroup /tmp/test59-exit-cgroup-clean.sh
...
! systemd-run --wait --unit=eight -p ExitType=cgroup /tmp/test59-exit-cgroup-unclean.sh
