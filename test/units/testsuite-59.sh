...
(sleep 1; systemctl stop one) &
systemd-run --wait --unit=one -p ExitType=cgroup -p ExecCondition=true /tmp/test59-exit-cgroup.sh

# false exec condition: systemd-run should exit immediately with status code: 1
! systemd-run --wait --unit=one -p ExitType=cgroup -p ExecCondition=false /tmp/test59-exit-cgroup.sh
