...
testcase_lvm_basic() {
    ...
    timeout --foreground 30 bash -c "until systemctl -q is-active /tmp/mypart1-mount-point; do sleep .2; done"
    ...
    udevadm wait --settle --timeout="$timeout" "/dev/disk/by-label/mylvpart1"
    timeout --foreground 30 bash -c "until systemctl -q is-active '/dev/$vgroup/mypart1'; do sleep .2; done"
    timeout --foreground 30 bash -c "until systemctl -q is-active /tmp/mypart1-mount-point; do sleep .2; done"
...
EOF
    ...
    systemctl daemon-reload
    mount "$mpoint"
    timeout --foreground 30 bash -c "until systemctl -q is-active '$mpoint'; do sleep .2; done"
    test -e "$mpoint/test"
    umount "$mpoint"
