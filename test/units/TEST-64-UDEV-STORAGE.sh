...
EOF
    ...
    mkdir -p "${rule%/*}"
    cat >"$rule" <<EOF
SUBSYSTEM=="block", KERNEL=="${devices[$((num_disks-2))]##*/}*|${devices[$((num_disks-1))]##*/}*", OPTIONS="link_priority=10"
EOF
    ...
    for ((i = 1; i <= iterations; i++)); do
        ...
        if ((i % 10 <= 1)); then
            ...
            for ((part = 1; part <= num_part; part++)); do
                ...
                target="$(readlink -f "$link")"
                if ((i % 2 == 0)); then
                    expected="${devices[$((num_disks-1))]}$part"
                else
                    expected="${devices[$((num_disks-2))]}$part"
                fi
...
testcase_simultaneous_events() {
    SLOW_TESTBED=0
    . /etc/os-release
    if [[ -v ASAN_OPTIONS || "$(systemd-detect-virt -v)" == "qemu" || "$ID" == "ubuntu" ]]; then
        SLOW_TESTBED=1
    fi
