...
testcase_issue_37823() {
    ...
    for ((i = 1; i <= iterations; i++)); do
        cat >"$script_dir/partscript-$i" <<EOF
$(for ((part = 1; part <= num_part; part++)); do printf 'name="testlabel-%d-%d", size=1M\n' "$part" "$i"; done)
EOF
    done
    ...
    for ((i = 1; i < iterations; i++)); do
        for ((part = 1; part <= num_part; part++)); do
            udevadm wait --settle --timeout="$timeout" --removed "/dev/disk/by-partlabel/testlabel-$part-$i"
        done
    done
    for ((part = 1; part <= num_part; part++)); do
        udevadm wait --settle --timeout="$timeout" "/dev/disk/by-partlabel/testlabel-$part-$iterations"
    done
    echo "## $iterations iterations end: $(date '+%H:%M:%S.%N')"
}
