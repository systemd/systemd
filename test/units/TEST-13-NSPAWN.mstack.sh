...
check "robind@config ro"          " on /config type.*ro"   "$OUT"
...
# Probe for support, but only accept that one specific failure as "unsupported" - treating any non-zero
# exit as a skip would silently turn a genuine rw/ overlay regression into a green run.
PROBE_ERR="$(systemd-nspawn --pipe --mstack "$MSTACK_DIR" true 2>&1 >/dev/null)" && PROBE_RC=0 || PROBE_RC=$?
if [[ $PROBE_RC -ne 0 ]] && ! grep -E 'Failed to (make|bind) \.mstack/ mounts: (Invalid argument|Operation not supported)' <<<"$PROBE_ERR" >/dev/null; then
    echo "FAIL: rw/ probe failed for an unexpected reason (exit $PROBE_RC):"
    echo "$PROBE_ERR"
    FAIL=$((FAIL+1))
    ...
    if systemd-nspawn --pipe --mstack "$MSTACK_DIR" cat /persist-test.txt | grep -F "persistent" >/dev/null; then
...
# Same narrowly-scoped probe as above: only the known unsupported-overlay failure may skip this section.
PROBE_ERR="$(systemd-nspawn --pipe --mstack "$MSTACK_DIR" true 2>&1 >/dev/null)" && PROBE_RC=0 || PROBE_RC=$?
if [[ $PROBE_RC -ne 0 ]] && ! grep -E 'Failed to (make|bind) \.mstack/ mounts: (Invalid argument|Operation not supported)' <<<"$PROBE_ERR" >/dev/null; then
    echo "FAIL: root/+layer@+rw/ probe failed for an unexpected reason (exit $PROBE_RC):"
    echo "$PROBE_ERR"
    FAIL=$((FAIL+1))
elif [[ $PROBE_RC -ne 0 ]]; then
...
#
# Assert those refusals while rw/ is still around, though. Every check below only runs once rw/ has
# been removed, so if the refusals were ever dropped - and rw/ silently ignored, or written into -
# the whole rest of this file would stay green.
echo "=== rw/ is refused in combination with --read-only and --volatile= ==="
if (! systemd-nspawn --pipe --mstack "$MSTACK_DIR" --read-only true); then
    echo "PASS: rw/ with --read-only refused"
    PASS=$((PASS + 1))
else
    echo "FAIL: rw/ with --read-only refused"
    FAIL=$((FAIL + 1))
fi
if (! systemd-nspawn --pipe --mstack "$MSTACK_DIR" --volatile true); then
    echo "PASS: rw/ with --volatile= refused"
    PASS=$((PASS + 1))
else
    echo "FAIL: rw/ with --volatile= refused"
    FAIL=$((FAIL + 1))
fi
...
check "robind@config ro with --read-only" " on /config type.*ro" "$OUT"
...
check "bind@writable rw"              "/writable type.*rw" "$OUT"
check "robind@config ro"              " on /config type.*ro"   "$OUT"
# The nested one is the interesting case here: --volatile=state puts a fresh tmpfs on /var, and the
# entry has to end up layered over it rather than buried underneath.
check "robind@var-config ro over the /var tmpfs" " on /var/config type.*ro" "$OUT"
...
echo "=== Compatibility paths (SYSTEMD_MSTACK_ASSUME_NO_CAPS=1) ==="
# On a kernel that has the newer overlayfs interfaces, the paths written for the ones that don't are
# unreachable - and an unreachable fallback is an untested one, on precisely the partially-backported
# distro kernels that will take it. Re-run the two shapes that build an overlay with the probe forced
# to answer "no", so the string-path 'lowerdir+' variant and the joined 'lowerdir=' fallback get
# exercised here rather than only in the field.
OUT=$(SYSTEMD_MSTACK_ASSUME_NO_CAPS=1 systemd-nspawn --pipe --mstack "$MSTACK_DIR" mount | grep "/ type\|/config\|/writable" || true)
check "no caps: root ro without rw/"  "/ type.*ro"           "$OUT"
check "no caps: robind@config ro"     " on /config type.*ro"     "$OUT"
check "no caps: bind@writable rw"     "/writable type.*rw"   "$OUT"
MARKER=$(SYSTEMD_MSTACK_ASSUME_NO_CAPS=1 systemd-nspawn --pipe --mstack "$MSTACK_DIR" cat /marker.txt || true)
check "no caps: layer@1 marker visible" "layer1"             "$MARKER"

OUT=$(SYSTEMD_MSTACK_ASSUME_NO_CAPS=1 systemd-nspawn --pipe --mstack "$MSTACK_DIR" --volatile=overlay mount | grep "/ type" || true)
check "no caps: root rw with --volatile=overlay" "/ type.*rw" "$OUT"

echo "=== --image= combined with --volatile= ==="
# --directory= and --image= reach the same synthetic-stack code in nspawn.c, but by different routes:
# --directory= hands over a path nspawn bind mounts itself, whereas --image= arrives already dissected
# and mounted. Only the first had coverage, and that code is now the only implementation of --volatile=
# for both. mkfs.ext4 -d populates the image straight from a directory, so this needs no loop device,
# no partition table and no DDI signing.
IMG_DIR="$(mktemp -d)"
IMG="$IMG_DIR/mstack-volatile.raw"
truncate -s 512M "$IMG"
if mkfs.ext4 -q -F -d /usr/share/TEST-13-NSPAWN-container-template "$IMG" 2>/dev/null; then

    img() { systemd-nspawn --pipe --register=no --image="$IMG" "$@" 2>/dev/null; }
    # Absence has to be read off the payload's stderr, which img() drops.
    img_err() { systemd-nspawn --pipe --register=no --image="$IMG" "$@" 2>&1; }

    OUT=$(img --volatile=state mount | grep "/ type\|/var type" || true)
    check "image + volatile=state: root ro"    "/ type.*ro"    "$OUT"
    check "image + volatile=state: /var tmpfs" "/var type.*rw" "$OUT"

    OUT=$(img --volatile=overlay mount | grep "/ type" || true)
    check "image + volatile=overlay: root rw"  "/ type.*rw"    "$OUT"
    img --volatile=overlay touch /volatile-write-probe >/dev/null 2>&1 || true
    OUT=$(img_err --volatile=overlay ls /volatile-write-probe || true)
    check "image + volatile=overlay: writes discarded" "No such file or directory" "$OUT"

    OUT=$(img --volatile=yes mount | grep "/ type" || true)
    check "image + volatile=yes: root rw"      "/ type.*rw"    "$OUT"
    # Something specific out of the image's /usr, so this cannot pass on any non-empty output.
    OUT=$(img --volatile=yes cat /usr/lib/os-release 2>&1 || true)
    check "image + volatile=yes: /usr kept"    "^ID="          "$OUT"
else
    echo "SKIP: could not build an ext4 image, skipping the --image= section"
fi
rm -rf "$IMG_DIR"
