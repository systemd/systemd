...
OUT=$(systemd-nspawn --mstack "$MSTACK_DIR" mount 2>/dev/null | grep "/ type\|/config\|/writable" || true)
...
MARKER=$(systemd-nspawn --mstack "$MSTACK_DIR" cat /marker.txt 2>/dev/null || true)
...
echo "=== --read-only ==="
OUT=$(systemd-nspawn --mstack "$MSTACK_DIR" --read-only mount 2>/dev/null | grep "/ type\|/config\|/writable" || true)
...
MARKER=$(systemd-nspawn --mstack "$MSTACK_DIR" --read-only cat /marker.txt 2>/dev/null || true)
check "layer@1 marker visible with --read-only" "layer1"     "$MARKER"

echo "=== --volatile ==="
OUT=$(systemd-nspawn --mstack "$MSTACK_DIR" --volatile mount 2>/dev/null | grep "/ type\|/config\|/writable" || true)
check "root rw with --volatile"      "/ type.*rw"           "$OUT"
check "bind@writable rw"             "/writable type.*rw"   "$OUT"
check "robind@config ro"             "/config type.*ro"     "$OUT"
check "robind@var-config ro"         "/var/config type.*ro" "$OUT"
# marker.txt is at root, not under /usr — expected absent with --volatile=yes
MARKER=$(systemd-nspawn --mstack "$MSTACK_DIR" --volatile cat /marker.txt 2>/dev/null || true)
check "layer@1 marker absent with --volatile=yes" "" "$MARKER"

echo "=== --volatile=overlay ==="
OUT=$(systemd-nspawn --mstack "$MSTACK_DIR" --volatile=overlay mount 2>/dev/null | grep "/ type\|/config\|/writable" || true)
check "root rw overlay"   "/ type overlay.*rw"   "$OUT"
check "bind@writable rw"  "/writable type.*rw"   "$OUT"
check "robind@config ro"  "/config type.*ro"     "$OUT"
MARKER=$(systemd-nspawn --mstack "$MSTACK_DIR" --volatile=overlay cat /marker.txt 2>/dev/null || true)
check "layer@1 marker visible with --volatile=overlay" "layer1" "$MARKER"

echo "=== --volatile=state ==="
OUT=$(systemd-nspawn --mstack "$MSTACK_DIR" --volatile=state mount 2>/dev/null | grep "/ type\|/config\|/writable" || true)
check "root ro with --volatile=state" "/ type.*ro"         "$OUT"
check "bind@writable rw"              "/writable type.*rw" "$OUT"
check "robind@config ro"              "/config type.*ro"   "$OUT"
MARKER=$(systemd-nspawn --mstack "$MSTACK_DIR" --volatile=state cat /marker.txt 2>/dev/null || true)
