...
if [[ -d /usr/lib/softhsm/tokens ]]; then
    ...
    PIN="1234" systemd-cryptenroll --pkcs11-token-uri="pkcs11:token=TestToken;object=RSATestKey" --unlock-key-file="$IMAGE_EMPTY_KEYFILE" "$IMAGE_EMPTY"
    # Ensure the json dump is captured in the logs, to help with debugging
    JSON_DUMP=$(cryptsetup luksDump --dump-json-metadata "$IMAGE_EMPTY")
    echo "$JSON_DUMP" | jq -e '.tokens | with_entries(select(.value.type == "systemd-pkcs11"))[].["pkcs11-padding"] | select(. == "oaep-sha256" or . == "oaep-sha1")' >/dev/null
    ...
    JSON_DUMP=$(cryptsetup luksDump --dump-json-metadata "$IMAGE_EMPTY")
    echo "$JSON_DUMP" | jq -r '.tokens | to_entries[] | select(.value.type == "systemd-pkcs11") | .key' | head -n1
    TOKEN_ID=$(echo "$JSON_DUMP" | jq -r '.tokens | to_entries[] | select(.value.type == "systemd-pkcs11") | .key' | head -n1)
    ...
    JSON_DUMP=$(cryptsetup luksDump --dump-json-metadata "$IMAGE_EMPTY")
    echo "$JSON_DUMP" | jq -r '.keyslots | keys | map(tonumber) | max'
    LEGACY_SLOT=$(echo "$JSON_DUMP" | jq -r '.keyslots | keys | map(tonumber) | max')
