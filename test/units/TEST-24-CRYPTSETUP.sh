...
cryptsetup_start_and_check() {
    ...
    for volume in "$@"; do
        ...
        if [[ "$check_fs_header" -ne 0 ]]; then
            blkid -p "/dev/mapper/$volume" && blkid_status="succeeded" || blkid_status="failed"
...
# 3) Image with a random key file (auto-implies wipe)
IMAGE_URANDOM="$WORKDIR/urandom.img"
truncate -s 32M "$IMAGE_URANDOM"
...
empty_urandom        $IMAGE_URANDOM  /dev/urandom                    headless=1,plain,size=256
empty_urandom_ro     $IMAGE_URANDOM  /dev/urandom                    headless=1,plain,size=256,read-only
...
detached_wipe_ro     $IMAGE_DETACHED $IMAGE_DETACHED_KEYFILE         headless=1,header=$IMAGE_DETACHED_HEADER,keyfile-offset=32,keyfile-size=16,wipe,read-only
EOF
...
# implied wipe + read-only must not fail the unit
cryptsetup_start_and_check empty_urandom_ro
...
if systemd-analyze --version | grep -F "+BLKID" >/dev/null; then
    # launch only if blkid is compiled in
    cryptsetup_start_and_check -H detached_wipe
fi
# explicit wipe + read-only is rejected before mapping
cryptsetup_start_and_check -f detached_wipe_ro
test ! -e /dev/mapper/detached_wipe_ro
