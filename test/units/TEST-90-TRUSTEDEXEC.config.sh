...
#
# Test TrustedExec= configuration parsing and graceful failure modes.
#
# Runs in a VM WITHOUT dm_verity.require_signatures=1, so enabling TrustedExec
...
# TrustedExec= requires +BPF_FRAMEWORK at compile time
...
enable_trusted_exec() {
    ...
    cat >/run/systemd/system.conf.d/50-trusted-exec.conf <<EOF
[Manager]
TrustedExec=yes
EOF
}

disable_trusted_exec() {
    rm -f /run/systemd/system.conf.d/50-trusted-exec.conf
}
...
# ------ Test case 1: Default (TrustedExec=no) — no log messages ------
...
testcase_default_no_messages() {
    ...
    # TrustedExec= setup) and is back on D-Bus by the time it returns. PID1
    # logs to kmsg synchronously, so messages are already visible.

    # No TrustedExec-related messages should appear
    if dmesg | grep "bpf-trusted-exec" >/dev/null 2>&1; then
        echo "Unexpected TrustedExec log messages with TrustedExec=no"
        return 1
    fi
}

# ------ Test case 2: TrustedExec=yes without BPF LSM — warning ------
...
testcase_no_require_signatures_helper() {
    ...
    # The helper's "check" command runs the same bpf_trusted_exec_supported()
