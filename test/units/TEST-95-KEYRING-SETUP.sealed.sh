...
jq -e '.[0].certificates | any(endswith("/mkosi-certificate.pem")) and any(endswith("/anyeku-certificate.pem"))
    ...
    and (any(endswith("/empty-certificate.pem")) | not)' /tmp/keyring-setup.json
...
# .fs-verity has no keyring_unsealed parameter, its unsealed state is null
"$KEYRING_SETUP" --dry-run --no-pager --json=short .fs-verity | jq -e '.[0].unsealed == null'
