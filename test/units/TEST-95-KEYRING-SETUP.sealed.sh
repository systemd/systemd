...
. /etc/os-release
mkdir -p "/etc/voa/$ID/image/default/x509" "/etc/voa/$ID/trust-anchor-image/default/x509"
...
openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:P-256 -nodes -subj /CN=anyeku -days 1 \
    ...
    -keyout /dev/null -out "/etc/voa/$ID/image/default/x509/anyeku-certificate.pem"
openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:P-256 -nodes -subj /CN=ca -days 1 \
    -addext basicConstraints=critical,CA:TRUE -addext keyUsage=critical,keyCertSign \
    -keyout /dev/null -out "/etc/voa/$ID/trust-anchor-image/default/x509/ca-certificate.pem"
rc=0
...
jq -e '.[0].certificates | any(endswith("/mkosi-certificate.pem")) and any(endswith("/anyeku-certificate.pem"))
    and any(endswith("/trust-anchor-image/default/x509/ca-certificate.pem"))
    and (any(endswith("/tls-certificate.pem")) | not)' /tmp/keyring-setup.json

# The .bpf spec looks up the bpf role
mkdir -p "/etc/voa/$ID/bpf/default/x509"
openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:P-256 -nodes -subj /CN=bpfrole -days 1 \
    -keyout /dev/null -out "/etc/voa/$ID/bpf/default/x509/bpfrole-certificate.pem"
"$KEYRING_SETUP" --dry-run --no-pager --json=short .bpf >/tmp/keyring-setup-bpf.json
jq -e '.[0].certificates | any(endswith("/bpf/default/x509/bpfrole-certificate.pem"))' /tmp/keyring-setup-bpf.json
...
"$KEYRING_SETUP" --dry-run --no-pager --json=short .dm-verity |
    jq -e '.[0] | .exists == true and .unsealed == false and .sealed == false and (.certificates | any(endswith("/mkosi-certificate.pem")))'
