...
timeout 30 systemd-pty-forward --quiet --console=broker-log --pty-name=pty96-flush -- \
    sh -c 'printf hello-pty96-flush-marker'

timeout 30 bash -c 'until journalctl -q -t pty96-flush | grep hello-pty96-flush-marker >/dev/null; do journalctl --sync; sleep 1; done'
