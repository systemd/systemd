...
result=$(varlinkctl call "$MANAGER_SOCKET" io.systemd.Unit.StartTransient \
    '{"context":{"ID":"varlink-transient-cgroup.service","CGroup":{"CPUWeight":200,"StartupCPUWeight":150,"CPUQuotaPerSecUSec":500000,"CPUQuotaPeriodUSec":20000,"MemoryAccounting":true,"MemoryLow":8388608,"StartupMemoryLow":4194304,"MemoryHigh":33554432,"MemoryMax":67108864,"TasksAccounting":true,"TasksMax":16,"IOAccounting":true,"IOWeight":100},"Service":{"Type":"oneshot","RemainAfterExit":true,"ExecStart":[{"path":"/bin/true"}]}}}')
...
systemctl show -P CPUQuotaPeriodUSec varlink-transient-cgroup.service | grep '^20ms$' >/dev/null
...
grep '^CPUQuotaPeriodSec=20ms$'  "$fragment" >/dev/null
grep '^MemoryMax=67108864$'      "$fragment" >/dev/null
grep '^TasksMax=16$'             "$fragment" >/dev/null

# The special values are written as TasksMax=infinity, CPUWeight=idle and an empty
# StartupCPUWeight= (UINT64_MAX resets a weight to the default)
defer_transient_cleanup varlink-transient-cgroup-special.service
varlinkctl call "$MANAGER_SOCKET" io.systemd.Unit.StartTransient \
    '{"context":{"ID":"varlink-transient-cgroup-special.service","CGroup":{"TasksMax":18446744073709551615,"CPUWeight":0,"StartupCPUWeight":18446744073709551615},"Service":{"Type":"oneshot","RemainAfterExit":true,"ExecStart":[{"path":"/bin/true"}]}}}' >/dev/null
timeout 30 bash -c 'until systemctl is-active varlink-transient-cgroup-special.service; do sleep 0.5; done'
systemctl show -P TasksMax varlink-transient-cgroup-special.service | grep '^infinity$' >/dev/null
fragment=$(systemctl show -P FragmentPath varlink-transient-cgroup-special.service)
test -n "$fragment"
grep '^TasksMax=infinity$' "$fragment" >/dev/null
grep '^CPUWeight=idle$'    "$fragment" >/dev/null
grep '^StartupCPUWeight=$' "$fragment" >/dev/null

# Cgroup limits materialize on the live cgroup
defer_transient_cleanup varlink-transient-cgroup-live.service
varlinkctl call "$MANAGER_SOCKET" io.systemd.Unit.StartTransient \
    '{"context":{"ID":"varlink-transient-cgroup-live.service","CGroup":{"MemoryMax":67108864,"TasksMax":16},"Service":{"ExecStart":[{"path":"/usr/bin/sleep","arguments":["/usr/bin/sleep","infinity"]}]}}}' >/dev/null
...
# Relative forms: TasksMaxScale/MemoryMaxScale (floats, 1.0 == 100%) are converted
# internally and written to the fragment as permyriad percentages. An explicit null
# is treated as absent (so MemoryMax=null does not conflict with MemoryMaxScale),
# and a zero scale is accepted for the min-0 memory properties, as over D-Bus
defer_transient_cleanup varlink-transient-cgroup-scale.service
result=$(varlinkctl call "$MANAGER_SOCKET" io.systemd.Unit.StartTransient \
    '{"context":{"ID":"varlink-transient-cgroup-scale.service","CGroup":{"TasksMaxScale":0.5,"MemoryMax":null,"MemoryMaxScale":0.125,"MemoryMinScale":0.0,"TasksAccounting":null},"Service":{"Type":"oneshot","RemainAfterExit":true,"ExecStart":[{"path":"/bin/true"}]}}}')
echo "$result" | jq -e '.context.CGroup.TasksMaxScale == 0.5'
echo "$result" | jq -e '.context.CGroup.TasksMax == null'
timeout 30 bash -c 'until systemctl is-active varlink-transient-cgroup-scale.service; do sleep 0.5; done'
fragment=$(systemctl show -P FragmentPath varlink-transient-cgroup-scale.service)
test -n "$fragment"
grep '^TasksMax=50.00%$'  "$fragment" >/dev/null
grep '^MemoryMax=12.50%$' "$fragment" >/dev/null
grep '^MemoryMin=0.00%$'  "$fragment" >/dev/null
(! grep '^TasksAccounting=' "$fragment")
...
expect_invalid_parameter \
    ...
    "CGroup.CPUWeight"
# IOWeight=0 is rejected, only the CPU weights accept idle
defer_transient_cleanup varlink-transient-bad-ioweight.service
expect_invalid_parameter \
    '{"context":{"ID":"varlink-transient-bad-ioweight.service","CGroup":{"IOWeight":0},"Service":{"Type":"oneshot","ExecStart":[{"path":"/bin/true"}]}}}' \
    "CGroup.IOWeight"
...
expect_invalid_parameter \
    ...
    "CGroup.MemoryMax"
# Out-of-range TasksMaxScale= (>= 100%) is rejected
defer_transient_cleanup varlink-transient-bad-tasksscale.service
expect_invalid_parameter \
    '{"context":{"ID":"varlink-transient-bad-tasksscale.service","CGroup":{"TasksMaxScale":1.5},"Service":{"Type":"oneshot","ExecStart":[{"path":"/bin/true"}]}}}' \
    "CGroup.TasksMaxScale"
# Absolute and relative forms of the same limit may not be combined
defer_transient_cleanup varlink-transient-bad-taskscombo.service
expect_invalid_parameter \
    '{"context":{"ID":"varlink-transient-bad-taskscombo.service","CGroup":{"TasksMax":16,"TasksMaxScale":0.5},"Service":{"Type":"oneshot","ExecStart":[{"path":"/bin/true"}]}}}' \
    "CGroup.TasksMaxScale"
# Zero MemoryMaxScale= is rejected
defer_transient_cleanup varlink-transient-bad-memscale.service
expect_invalid_parameter \
    '{"context":{"ID":"varlink-transient-bad-memscale.service","CGroup":{"MemoryMaxScale":0.0},"Service":{"Type":"oneshot","ExecStart":[{"path":"/bin/true"}]}}}' \
    "CGroup.MemoryMaxScale"
