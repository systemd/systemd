...
# The EXIT trap is skipped if a previous run was terminated abruptly (e.g.
# SIGKILL). Remove any leftover runtime state so this test always exercises the
# intended daemon-reload path.
rm -f \
        /run/systemd/system/"$unit_success" \
        /run/systemd/system/"$unit_timeout" \
        "$script_success" \
