...
run_systemd_sysext "$fake_root" merge
status_json=$(run_systemd_sysext "$fake_root" status --json=pretty)
jq -e 'any(.[]; .hierarchy == "/opt")' >/dev/null <<<"$status_json"
if jq -e 'any(.[]; .extensions != [])' >/dev/null <<<"$status_json"; then
