...
# the mounts for the final system with with --root=/sysroot
...
if run_systemd_sysext "$fake_root" status --json=pretty |  jq -r '.[].extensions' | grep -v '^none$' ; then
  echo >&2 "Extension got loaded for an initd structure passed as --root while the extension does not declare itself compatible with the initrd scope"
  exit 1
fi
