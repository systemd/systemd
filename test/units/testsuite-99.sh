...
# Run a command and pipe its output to both stdout/stderr and a file ($RUN_OUT),
# to make the command still reasonably debug-able while still being able to do
# various checks on its output via the $RUN_OUT file
run() {
    # Don't pollute the test "output" with this function's setup code
    set +x
    trap "set -x" RETURN

    local ec exp_ec=0 rcond=0
    local opt

    # _Always_ reset $OPTIND to 1, since it keeps its value through run() invocations
    OPTIND=1
    while getopts "e:r" opt; do
        case "$opt" in
            e)
                # Change the expected exit code from the default 0
                exp_ec="$OPTARG"
                if ! [[ "$exp_ec" =~ ^[0-9]+$ ]]; then
                    echo >&2 "The expected exit code must be a number, got '$exp_ec'"
                    exit 1
                fi
                ;;
            r)
                # Reverse the "expected exit code" condition, so now we expect any
                # exit code _but_ the "expected" one
                rcond=1
                ;;
            *)
                echo "Usage: $0 [-e EXP_EXIT_CODE] [-r] -- COMMAND"
                exit 1
        esac
    done

    # Drop "our" arguments from argv
    shift $((OPTIND - 1))

    # Run the command & pipe the output to both stdout & the $RUN_OUT file, so
    # the test remains reasonably debug-able while allows us to do further checks
    # on the command output via grep or other means
    : >"$RUN_OUT"
    ec=0
    "$@" |& tee "$RUN_OUT" || ec=$?

    if [[ $rcond -ne 0 ]]; then
        if [[ $ec -eq $exp_ec ]]; then
            echo >&2 "Unexpected exit code: expected anything but $exp_ec but got $exp_ec"
            return 1
        fi
    else
        if [[ $ec -ne $exp_ec ]]; then
            echo >&2 "Unexpected exit code: expected $exp_ec but got $ec"
            return 1
        fi
    fi
}
...
# Sanity check
run -- getent -s resolve hosts ns1.signed.test
grep -qE "^1\.2\.3\.1\s+ns1\.signed\.test" "$RUN_OUT"
...
# FIXME: re-enable once the follow-up issue is resolved
#run -- getent -s myhostname hosts localhost
#grep -qE "^127\.0\.0\.1\s+localhost" "$RUN_OUT"
...
grep -qF "follow14.final.signed.test IN A 1.2.3.14" "$RUN_OUT"
grep -qF "authenticated: yes" "$RUN_OUT"
...
# CNAME loop
run -- dig +dnssec cname-loop.signed.test
grep -qF "status: NXDOMAIN" "$RUN_OUT"
for i in {1..3}; do
    grep -qE "^loop$i\..+CNAME\s+" "$RUN_OUT"
done
run -r -- resolvectl query --cache=no cname-loop.signed.test
grep -qE "^cname-loop\.signed\.test.+ not found" "$RUN_OUT"
...
grep -qF "1.2.3.121" "$RUN_OUT"
run -- resolvectl query untrusted.test
grep -qF "untrusted.test: 1.2.3.121" "$RUN_OUT"
grep -qF "authenticated: no" "$RUN_OUT"
