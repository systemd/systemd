...
PAYLOAD_MAX=50000
# Maximum memory size (in kilobytes) which a process can occupy before warning
# is printed by dfuzzer - default: 512M
MEMORY_MAX=524288
...
for bus in "${BUS_LIST[@]}"; do
    ...
    systemd-run -E HOME=/root --pipe --wait \
                -- dfuzzer -L /dfuzzer-logs -v -m "$MEMORY_MAX" -b "$PAYLOAD_MAX" -n "$bus"
done
