...
# Get current time and add a future offset (e.g., 1 year ahead)
current_time=$(date)
future_time_berlin=$(date -d "$current_time + 1 year" +"%Y-%m-%d %H:%M:%S")

# Set system time to the calculated future time for Europe/Berlin
timedatectl set-time "$future_time_berlin"
