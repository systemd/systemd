...
if systemctl show -P ActiveState initrd-switch-root-no-state.service 2>/dev/null | grep -v inactive >/dev/null; then
    echo >&2 "initrd-switch-root-no-state.service should not be known to the new manager"
    exit 1
fi
