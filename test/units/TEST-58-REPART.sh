...
testcase_varlink_subscribe_devices() {
    local imgs subscribe_log pre_existing_log loop loop2 i mark non_sub_output
    ...
    subscribe_log="$(mktemp "/tmp/test-repart.subscribe-log.XXXXXXXXXX")"
    pre_existing_log="$(mktemp "/tmp/test-repart.pre-log.XXXXXXXXXX")"
    loop=""
    loop2=""
    ...
    trap '
        ...
        rm -rf "$imgs" "$subscribe_log" "$pre_existing_log"
    ' RETURN
