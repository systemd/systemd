...
Type=linux-generic
Label=plain
BtrfsReplace=${btrfs_mntpoint_plain}
EOF
...
Label=encrypted
Encrypt=key-file
BtrfsReplace=${btrfs_mntpoint_encrypted}
VolumeName=btrfs-replace-encrypted
EOF

    loop="$(losetup -P --show --find "${imgs}/img")"
    # shellcheck disable=SC2064
    trap "umount '${btrfs_mntpoint_encrypted}'; losetup -d '${loop_btrfs_encrypted}'; umount '${btrfs_mntpoint_plain}'; losetup -d '${loop_btrfs_plain}'; losetup -d '${loop}'; rm -rf '$defs' '$imgs' '$btrfs_mntpoint_plain' '$btrfs_mntpoint_encrypted'" RETURN
    ...
    # shellcheck disable=SC2064
    trap "umount '${btrfs_mntpoint_encrypted}'; cryptsetup close btrfs-replace-encrypted; losetup -d '${loop_btrfs_encrypted}'; umount '${btrfs_mntpoint_plain}'; losetup -d '${loop_btrfs_plain}'; losetup -d '${loop}'; rm -rf '$defs' '$imgs' '$btrfs_mntpoint_plain' '$btrfs_mntpoint_encrypted'" RETURN
