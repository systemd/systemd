...
# The registration must be gone again now that the container exited. (Poll for
# it, as deregistration happens asynchronously, and keep a failing "ptyctl list"
# fatal rather than mistaking it for the entry being gone.)
timeout 10 bash -c "while ptyctl list | grep '$NAME' >/dev/null; do sleep .5; done"
