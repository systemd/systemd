...
echo "mstack base layer visible" >"$MSTACK/layer@0/usr/mstack-layer-marker"
echo "mstack extra layer visible" >"$MSTACK/layer@1/usr/mstack-extra-layer-marker"
cat >"$MSTACK/layer@0/usr/mstack-test-init" <<'EOF'
#!/bin/sh
set -eu

read -r marker </usr/mstack-layer-marker
test "$marker" = "mstack base layer visible"
