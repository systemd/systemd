...
# Save the current date & time into RTC, so we can restore it later once we're done with out time shenanigans
hwclock --systohc
...
# Bump the system date 1 minute _after the original calendar timer would've expired (without any random
