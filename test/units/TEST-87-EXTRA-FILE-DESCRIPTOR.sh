...
# Open files and assign FD to env variables
exec {TEST_FD1}</tmp/test-extra-fd/1.txt
exec {TEST_FD2}</tmp/test-extra-fd/2.txt

TEST_UNIT="test-87-extra-fd.service"
...
busctl call \
    ...
    "ssa(sv)a(sa(sv))" "$TEST_UNIT" replace 4 \
      ...
      Type "s" oneshot \
      ExtraFileDescriptor "a(hs)" 2 \
        "$TEST_FD1" test \
        "$TEST_FD2" other \
    0

cmp -b <(systemctl show -p ExtraFileDescriptorName "$TEST_UNIT") <<EOF
ExtraFileDescriptorName=test
ExtraFileDescriptorName=other
EOF

# shellcheck disable=SC2016
timeout 10s bash -xec 'while [[ "$(systemctl show -P SubState test-87-extra-fd.service)" != "exited" ]]; do sleep .5; done'
