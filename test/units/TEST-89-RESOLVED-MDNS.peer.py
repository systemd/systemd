    ...
    for section, count in (('an', ancount), ('ns', nscount), ('ar', arcount)):
        for _ in range(count):
            try:
                ...
                # The target may legitimately chase compression pointers outside the RDATA,
                # but it must at least start inside it.
                target = parse_name(buf, offset)[0] if rtype == TYPE_PTR and rdlength >= 1 else None
