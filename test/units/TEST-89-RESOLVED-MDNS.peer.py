...
A minimal, self-contained mDNS peer (RFC 6762/6763, python stdlib only) used to
exercise systemd-resolved from the network side, deterministically and at the
packet level. It implements exactly the slice of mDNS that the
bonjour_reference testcases need:
  ...
  query    Send one crafted query on an interface, then capture like listen
           ...
           section (section 7.1, --known-answer-ptr).
...
def parse_packet(buf):
    """Returns (flags, questions, records): questions as (name, qtype, qclass) tuples,
    records as (section, name, rtype, rclass, ttl, ptr-target-or-None) tuples."""
    if len(buf) < 12:
        raise ValueError('truncated header')
    _, flags, qdcount, ancount, nscount, arcount = struct.unpack_from('>HHHHHH', buf, 0)
    offset = 12
    questions = []
...
def log_packet(buf, dst):
    ...
    try:
        flags, questions, records = parse_packet(buf)
    ...
    if not qr:
        ...
        for name, qtype, _ in questions:
            log(f'Q name={name} qtype={qtype} probe={probe}')
    for section, name, rtype, rclass, ttl, target in records:
        flush = 1 if rclass & CACHE_FLUSH else 0
        suffix = f' ptr={target}' if target is not None else ''
        log(f'R qr={qr} dst={dst} sec={section} name={name} type={rtype} ttl={ttl} flush={flush}{suffix}')
...
class Publisher:
    ...
    def handle_query(self, buf):
        try:
            flags, questions, _ = parse_packet(buf)
        except (ValueError, struct.error) as e:
            log(f'MALFORMED {e}')
            return
        ...
        if answers:
            ...
            log(f'REPLY questions={",".join(q[0] for q in questions)}')
    ...
    def run(self):
        ...
        if self.goodbye_on_exit:
            ...
            # repeated like regular announcements for robustness against loss.
            for i in range(2):
                if i > 0:
                    time.sleep(1)
...
def query(args):
    ...
    packet = (
        struct.pack('>HHHHHH', qid, 0, 1, len(answers), 0, 0)
    ...
    log(
        f'QUERY name={args.qname} qtype={args.qtype} qu={1 if args.qu else 0} '
        f'ka={args.known_answer_ptr or "-"}'
    )
    capture(sock, args.duration)
...
def main():
    ...
    publish.add_argument('--iface', required=True)
    publish.add_argument('--addr', required=True, help='IPv4 address to advertise (A record)')
    publish.add_argument('--instance', required=True, help='service instance name (single label)')
    publish.add_argument('--service', required=True, help='service type, e.g. _ref._udp')
    publish.add_argument('--subtype', help='service subtype label, e.g. _vendor (RFC 6763 section 7.1)')
    publish.add_argument('--port', type=int, default=42089)
    publish.add_argument('--ttl', type=int, default=120)
    publish.add_argument('--hostname', required=True, help='host name to advertise, e.g. refpeer1.local')
    publish.add_argument('--txt', action='append', default=[])
    publish.add_argument('--goodbye-on-exit', action='store_true')
