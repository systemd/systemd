...
conformance scorecard testcase:
  ...
  listen   Passively capture mDNS traffic on one interface and print one
           greppable line per question/record seen, e.g.:
             ...
             12.345 Q name=RefPub89._refpub._udp.local qtype=255 probe=1
           ...
           transaction ID and destination address, e.g.:

             12.344 P qr=0 id=0 dst=224.0.0.251 len=46
  ...
  query    Send one crafted query on an interface, then capture like listen
           ...
           the TC bit and, with --followup-known-answer, sends the known answer
           in a separate follow-up packet, as a querier with more known answers
           than fit in one packet does (section 7.2).
...
def parse_packet(buf):
    ...
    records as (section, name, rtype, rclass, ttl, ptr-target-or-None) tuples."""
    if len(buf) < 12:
    ...
    for section, count in (('an', ancount), ('ns', nscount), ('ar', arcount)):
        for _ in range(count):
            ...
            offset += 10
            target = parse_name(buf, offset)[0] if rtype == TYPE_PTR else None
            offset += rdlength
...
def open_socket(iface, port=MDNS_PORT):
...
def recv_with_dst(sock):
    ...
    data, ancdata, _, _ = sock.recvmsg(9000, socket.CMSG_SPACE(16))
    dst = '?'
...
def log_packet(buf, dst):
    ...
    try:
        pid, flags, questions, records = parse_packet(buf)
    except (ValueError, struct.error) as e:
        log(f'MALFORMED {e}')
        return
    qr = 1 if flags & 0x8000 else 0
    log(f'P qr={qr} id={pid} dst={dst} len={len(buf)}')
    if qr:
        ...
        for name, qtype, _ in questions:
            log(f'Q name={loggable(name)} qtype={qtype} probe={probe}')
...
class Publisher:
    def __init__(self, args):
        self.sock = open_socket(args.iface)
        ...
        self.txt_rdata = b''.join(bytes([len(t)]) + t for t in map(str.encode, args.txt)) or b'\0'
        ...
        self.goodbye_interval = args.goodbye_interval
    ...
    def run(self):
        ...
        if self.goodbye_on_exit:
            ...
            #
            # Deliberately not a second apart. resolved deletes goodbye'd
            # records one second late, arms that timer only when none is
            # already armed, and then decides whether to re-arm by measuring
            # the cache against the timer's *scheduled* time rather than the
            # current one (resolved-mdns.c, mdns_goodbye_callback()). A second
            # goodbye that lands after the deadline but before the callback is
            # dispatched refreshes the record past that horizon, so nothing
            # re-arms and the browser only learns of the removal from the
            # maintenance ladder, a hundred seconds later. The window is as
            # wide as the dispatch lag: measured here at 4 losses in 40 runs
            # with a one-second spacing, none in 45 with this one. Section
            # 10.1 asks for no particular spacing.
            for i in range(2):
                if i > 0:
                    time.sleep(self.goodbye_interval)
...
def query(args):
    ...
    # (RFC 6762 section 7.2), so the known answer moves to that packet.
    flags = 0x0200 if args.tc else 0
    inline = [] if args.followup_known_answer else answers
    ...
    if args.witness_qname:
        ...
        witness = encode_name(tuple(args.witness_qname.split('.'))) + struct.pack(
            '>HH', args.witness_qtype, qclass
        )
    packet = (
    ...
    if args.followup_known_answer and answers:
        time.sleep(0.1)
        sock.sendto(
            struct.pack('>HHHHHH', qid, 0, 0, len(answers), 0, 0) + b''.join(answers),
...
def main():
    ...
    publish_parser.add_argument('--txt', action='append', default=[])
    publish_parser.add_argument(
        '--announce-count',
        type=int,
        default=2,
    ...
    publish_parser.add_argument(
        ...
        type=float,
        default=0.25,
        help='seconds between the repeated goodbyes (see the note in run())',
    )
    publish_parser.add_argument(
    ...
    query_parser.add_argument(
        ...
        action='store_true',
        help='send the known answer in a follow-up packet rather than in the query',
    )
    query_parser.add_argument(
        '--source-port',
        type=int,
    ...
    query_parser.add_argument(
        ...
        type=int,
        default=120,
        help='suppression only applies at >= half the true TTL (section 7.1), so keep this'
        " >= half of the responder's published TTL (resolved's MDNS_DEFAULT_TTL is 120)",
    )
    query_parser.add_argument(
        '--witness-qname',
