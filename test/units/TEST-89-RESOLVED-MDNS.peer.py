...
class Publisher:
    def __init__(self, args):
        ...
        self.enumeration_name = ('_services', '_dns-sd', '_udp', 'local')
    ...
    def handle_query(self, buf):
        ...
        for name, qtype, _ in questions:
            if names_equal(name, '.'.join(self.ptr_name)) and qtype in (TYPE_PTR, TYPE_ANY):
                ...
                additionals += [r['srv'], r['txt'], r['a']]
            elif names_equal(name, '.'.join(self.enumeration_name)) and qtype in (TYPE_PTR, TYPE_ANY):
                # RFC 6763 section 9: the type, not an instance. Answered on request only, so no
                # other scenario's announcements change.
                answers.append(
                    make_record(
                        self.enumeration_name, TYPE_PTR, self.ttl, encode_name(self.ptr_name), flush=False
                    )
                )
        ...
        while not stopping:
            try:
                ...
                for key, _ in selector.select(timeout=0.1):
                    data, _, msg_flags, _ = key.fileobj.recvmsg(9000)
                    if msg_flags & socket.MSG_TRUNC:
                        # A clipped query cannot be answered faithfully, and unanswered
                        # queries are what the absence scenarios score -- the survived-gate
                        # greps this marker, so the run fails as a harness error instead.
                        log('PUBLISHER-ERROR truncated datagram')
                        continue
...
def query(args):
    ...
    packet = (
        struct.pack('>HHHHHH', qid, flags, 2 if witness else 1, len(answers), 0, 0)
...
def main():
    ...
    # A dispatch table rather than an if/elif chain: argparse already rejects unknown
    # subcommands, and this way a future subcommand registered without a handler fails
    # loudly here instead of falling off the end of a chain as a silent no-op.
    {
        'publish': lambda: Publisher(args).run(),
        'listen': lambda: listen(args),
        'query': lambda: query(args),
    }[args.mode]()
    return 0
