...
SYSPATH="/sys$(udevadm info "/sys/class/net/${IFNAME}" --query=property --property DEVPATH)"
ESCAPED=$(systemd-escape -p "${SYSPATH}")
assert_eq "$(systemd-escape -u "${ESCAPED}")" "${SYSPATH}"

systemctl start "${ESCAPED}.device"
assert_eq "$(systemctl show -p Wants --value "${ESCAPED}.device")" "test@${ESCAPED}.service"
