...
run_and_check_services() {
    ...
    # RETURN traps, an explicit return fires them. The stop comes first so the
    # unit is gone before its output files are removed, and is best-effort so
    # that a unit which already exited on its own cannot skip the removal.
...
testcase_browse_ifindex_zero_no_flap() {
    ...
    # link nor the output file leaks into later testcases. This has to be an
    # EXIT trap: run_testcases runs each testcase in its own subshell, and that
    # subshell's EXIT trap fires however the testcase ends, including when
    # errexit aborts it in the setup below -- a RETURN trap would not, as the
    # self-healing link removal above already has to account for. The browse
    # unit may not exist yet, hence the best-effort stop.
    # shellcheck disable=SC2064
    ...
    systemd-run --unit="$unit_name" --service-type=exec -p StandardOutput="file:$out_file" \
        ...
        "{ \"domain\": \"$service_type.local\", \"type\": \"\", \"ifindex\": 0, \"flags\": 16785432 }" || return 1
