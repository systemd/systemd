...
testcase_browse_ifindex_zero_no_flap() {
    ...
    # second (empty) scope that the ifindex=0 reconciliation must combine. Best
    # effort: if the link/scope can't be set up the test simply loses this
    # guarantee (it never yields a false failure).
    ip link add "$dummy" type dummy 2>/dev/null || :
    ip link set "$dummy" up multicast on || :
    ip address add 169.254.171.171/16 dev "$dummy" 2>/dev/null || :
    ...
    # shellcheck disable=SC2064
    trap "trap - RETURN; systemctl stop $unit_name; ip link del $dummy 2>/dev/null || :" RETURN
