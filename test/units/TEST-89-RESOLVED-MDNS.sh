...
run_and_check_services() {
    ...
    #       one, *sigh*
    # Armed before the unit is started, and combined with the explicit return
    # below this covers a failing systemd-run, too: an errexit abort would skip
    # RETURN traps, an explicit return fires them.
    ...
    systemd-run --unit="$unit_name" --service-type=exec -p StandardOutput="file:$out_file" -p StandardError="file:$error_file" \
        varlinkctl call --more /run/systemd/resolve/io.systemd.Resolve io.systemd.Resolve.BrowseServices "$parameters" || return 1
...
# --- Bonjour reference behavior scorecard -----------------------------------
#
# The following testcase runs resolved against a scripted mDNS peer
# (TEST-89-RESOLVED-MDNS.peer.py) and scores resolved's behavior in a set of
# scenarios derived from RFC 6762/6763 and from interoperability reports.
# "Reference behavior" means what the RFCs require, with the de facto
# reference implementation (Bonjour's mDNSResponder) as the tie-breaker where
# they leave room; the peer itself is just a deterministic, packet-exact
# instrument implementing the slice of mDNS the scenarios need.
#
...
ref_scenario_announce() {
    : "One publisher: exactly one added event, none removed while it stays up"
...
ref_scenario_two_interfaces_same_name() {
    : "The same instance name announced on two links yields an added event per link"
...
ref_scenario_goodbye() {
    : "A goodbye (TTL=0) announcement promptly yields a removed event"
...
ref_scenario_goodbye_one_interface() {
    : "A goodbye on one link removes only that link's instance, the other lives on"
...
ref_scenario_expiry_no_goodbye() {
    : "A publisher vanishing without goodbye is removed when its records expire"
...
ref_scenario_expiry_no_goodbye_plain_flags() {
    : "Silent expiry must be reported to clients passing plain flags, too"
...
ref_scenario_case_insensitive_match() {
    ...
    # Match on the instance name only: which case the type field of the event
    # echoes back is presentation, that the instance is found is conformance.
    for ((i = 0; i < 30; i++)); do
        if grep -E "\"updateFlag\":\"added\",\"family\":[0-9]+,\"name\":\"RefCase\"" >/dev/null 2>&1 "$out"; then
            return 0
        fi
        sleep 1
    done
...
ref_scenario_duplicate_question_suppression() {
    : "A second browser for the same question must not add queries on the wire"
...
ref_scenario_browse_subtype() {
    ...
    # Match on the instance name only, like in case_insensitive_match: what the
    # event's type field says for a subtype browse is a separate question.
    for ((i = 0; i < 30; i++)); do
        if grep -E "\"updateFlag\":\"added\",\"family\":[0-9]+,\"name\":\"RefSub\"" >/dev/null 2>&1 "$out"; then
            return 0
        fi
        sleep 1
    done
...
ref_scenario_responder_service_enumeration() {
    ...
    for _ in 1 2; do
        ...
        grep -E "R qr=1 dst=[^ ]+ sec=an name=_services._dns-sd._udp.local type=12 ttl=[1-9][0-9]* flush=[01] ptr=$REF_PUB_SERVICE\.local" \
...
ref_scenario_responder_legacy_unicast() {
    ...
    for _ in 1 2; do
        ...
        # this socket, showing up as an empty log), and it must echo the
        # query's transaction ID (section 6.7) or a real legacy client could
        # never match it to its query -- 35081 is the peer's QUERY_ID, 0x8909.
...
ref_scenario_responder_known_answer_suppression() {
    ...
    local control_log="$REF_TMPDIR/kasup.control" control2_log="$REF_TMPDIR/kasup.control2"
    local q_log="$REF_TMPDIR/kasup.query"
    local answered=0

    # Positive control: a known answer with a TTL below half the true one must
    # still be answered (section 7.1 only forbids answering at >= half). This
    # proves the responder is reachable and answering right now -- without it,
    # silence below would count as an unexpected pass of this known-failing
    # scenario on any lost datagram, failing the testcase.
    for _ in 1 2; do
        ref_query "$control_log" "$REF_PUB_SERVICE.local" \
            --known-answer-ptr "$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local" --known-answer-ttl 10 \
            --duration 5 || return 2
        if grep -E "R qr=1 dst=[^ ]+ .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" >/dev/null "$control_log"; then
            answered=1
            break
        fi
    done
    [[ "$answered" -eq 1 ]] || { cat "$control_log" >&2; return 2; }
    ...
    # Bracket the silence with a second positive control: on its own, the
    # unanswered probe above is indistinguishable from the query or the
    # responder having gone missing -- which would make this known-failing
    # scenario pass unexpectedly and fail the whole testcase.
    answered=0
    for _ in 1 2; do
        ref_query "$control2_log" "$REF_PUB_SERVICE.local" \
            --known-answer-ptr "$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local" --known-answer-ttl 10 \
            --duration 5 || return 2
        if grep -E "R qr=1 dst=[^ ]+ .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" >/dev/null "$control2_log"; then
            answered=1
            break
        fi
    done
    [[ "$answered" -eq 1 ]] || { cat "$control2_log" >&2; return 2; }
}
...
ref_scenario_publisher_probe_announce() {
    : "resolved probes before announcing and announces a new service at least twice"
    ...
    # the instance name carrying the proposed records in the authority section)...
    grep "Q name=$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local .*probe=1" >/dev/null "$listen_log" || { cat "$listen_log" >&2; return 1; }
...
ref_scenario_publisher_unregister_goodbye() {
    : "Unregistering a service sends goodbyes for its records"
...
ref_scenario_publisher_stop_goodbye() {
    : "Stopping resolved sends goodbyes for published services"
...
testcase_bonjour_reference_behavior() {
    : "Score resolved's behavior against reference mDNS/DNS-SD behavior"
    ...
    local -A known_failures=(
        ...
        # https://github.com/OpenPrinting/libcups/issues/81#issuecomment-5122506798
        [two_interfaces_same_name]="https://github.com/OpenPrinting/libcups/issues/81#issuecomment-5122506798"
        [goodbye_one_interface]="https://github.com/OpenPrinting/libcups/issues/81#issuecomment-5122506798"
        ...
        [expiry_no_goodbye_plain_flags]="browse maintenance timer dies without SD_RESOLVED_NO_STALE"
        ...
        [publisher_unregister_goodbye]="dnssd_registered_service_clear_on_reload() does not withdraw"
        # No goodbye packets are sent for published services on daemon stop.
        [publisher_stop_goodbye]="https://github.com/systemd/systemd/issues/30421"
        # mdns_scope_process_query never looks at the query's answer section,
        # so it re-answers queries that already carry the answer.
        [responder_known_answer_suppression]="responder-side known-answer suppression not implemented"
        ...
        [duplicate_question_suppression]="one query engine per subscriber, TODO in commit 8458b7fb91ea"
    )
    ...
    for scenario in "${scenarios[@]}"; do
        : "+++ bonjour-ref scenario $scenario BEGIN +++"
        rc=0
        "ref_scenario_$scenario" || rc=$?
        ref_scenario_cleanup
        results[$scenario]=$rc
        : "+++ bonjour-ref scenario $scenario END (rc=$rc) +++"
    done

    echo "=== Bonjour reference behavior scorecard ==="
    for scenario in "${scenarios[@]}"; do
        rc="${results[$scenario]}"
        printf '%-32s %s%s\n' "$scenario" \
