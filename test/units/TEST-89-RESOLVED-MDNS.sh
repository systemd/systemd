...
testcase_browse_shared_querier() {
    ...
    # fires however the testcase ends — the infinity browse units must never outlive it.
    # shellcheck disable=SC2064
...
EOF
    ...
    # An EXIT trap, not RETURN: set -e aborts skip RETURN traps, and this subshell's EXIT trap
    # fires however the testcase ends — the infinity browse unit must never outlive it.
    # shellcheck disable=SC2064
