...
testcase_mdns_goodbye_on_stop() {
    ...
    # Match the address record itself: the container's name also appears in the PTR, SRV and TXT
    # records of its 200 published services, so a bare name match would survive the very flush
    # this is here to catch.
    resolvectl show-cache | grep -E "$CONTAINER_2\.local IN A\b" >/dev/null
