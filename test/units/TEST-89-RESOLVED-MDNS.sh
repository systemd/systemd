...
testcase_browse_all_interfaces_ifindex_minus_one() {
    : "Test browsing all interfaces with ifindex=-1 (Avahi compatibility)"
    resolvectl flush-caches
    # Using ifindex=-1 (AVAHI_IF_UNSPEC) should also discover services on all mDNS interfaces
    run_and_check_services_with_ifindex 0 check_both -1
}
