...
testcase_mdns_no_goodbye_without_services() {
    ...
    # Both halves of the claim are read off the daemon's own decision lines: the hold is armed
    # only on the path that logs the goodbye transmission, so that line's absence next to the
    # fast path's line is the grace second not happening. A wall-clock bound around the stop
    # would measure the client's bus round trips and the cgroup teardown as much as the daemon.
