...
testcase_browse_shared_querier() {
    ...
    local out1 out2 out3 unit1 unit2 unit3 service_type params n1 n2 n3 since joins
    ...
    # shellcheck disable=SC2064
    trap "systemctl stop $unit1 $unit2 $unit3 2>/dev/null || :; resolvectl log-level info || :; rm -f $out1 $out2 $out3" EXIT

    # Debug logging, so the sharing itself is observable below and not just its symptoms: every
    # assertion on the event streams alone is equally satisfied by one querier per subscriber. The
    # level is daemon-wide state that outlives this subshell, hence the restore in the trap above.
