...
testcase_mdns_goodbye_on_stop() {
    ...
    # The stop has to hold for the grace second, or the second transmission never goes out. The
    # count below sees two passes either way -- an immediate retransmission produces exactly the
    # same journal -- so the spacing is only pinned here, around the stop itself.
    ...
    if [[ "$elapsed_msec" -lt 900 ]]; then
        echo >&2 "Stopping a resolved with published services took only ${elapsed_msec}ms:"
        echo >&2 "it cannot have held the RFC 6762 §8.3 second between the two goodbyes"
...
testcase_mdns_runtime_withdrawal() {
    ...
    local out_file error_file unit_name service_type sub_type selective_name off out
    out_file="$(mktemp)"
    error_file="$(mktemp)"
    ...
    ok=0
    for _ in {0..14}; do
        if resolvectl query -p mdns -t PTR "$selective_name" >/dev/null 2>&1; then
            ok=1
            break
        fi
        sleep 1
    ...
    ok=0
    for _ in {0..14}; do
        # Capture first: a negated pipeline would let any resolvectl failure -- a resolver that is
        # simply gone -- read as "the record was withdrawn".
        out="$(resolvectl query -p mdns -t PTR "$selective_name" 2>&1 || :)"
        if ! grep -F "$selective_name" >/dev/null <<<"$out"; then
            ok=1
            break
        fi
        sleep 1
    ...
    ok=0
    for _ in {0..14}; do
        # Capture first, as above: under 'set -o pipefail' a negated pipeline treats a failing
        # resolvectl as proof of withdrawal.
        out="$(resolvectl query -p mdns -t PTR _services._dns-sd._udp.local 2>&1 || :)"
        if ! grep -F "$service_type.local" >/dev/null <<<"$out"; then
            ok=1
            break
        fi
        sleep 1
