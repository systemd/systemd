...
ref_scenario_querier_known_answers() {
    ...
    # re-query at 64s after browse start even in the worst case).
    ...
    grep "Q name=$REF_SERVICE.local qtype=12" >/dev/null "$listen_log" || { cat "$listen_log" >&2; return 2; }
...
ref_scenario_responder_legacy_unicast() {
    ...
    for _ in 1 2; do
        ...
        # The reply must be unicast to the querier's ephemeral port. Note that a
        # (wrong) multicast-only reply would go to 224.0.0.251:5353 and never
        # reach this socket at all, so that failure mode shows as an empty log.
...
ref_scenario_responder_known_answer_suppression() {
    ...
    local control_log="$REF_TMPDIR/kasup.control" q_log="$REF_TMPDIR/kasup.query"
    local answered=0
...
testcase_bonjour_reference_behavior() {
    ...
    REF_TMPDIR="$(mktemp -d)"
    trap 'trap - RETURN; ref_teardown' RETURN
    ip netns add "$REF_NS"
