...
# (all units come from the base image). This is a regression test for a bug where extensions that
# only overlay data files (no units) would not get an ExtensionImages= entry in the drop-in.
