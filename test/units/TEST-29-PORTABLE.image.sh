...
grep -q -F 'root=signed+growfs-off+squashfs:' /run/systemd/system.attached/minimal-app0-foo.service.d/20-portable.conf
...
# Ensure pinning by policy works
grep -q -F 'RootImagePolicy=root=signed+growfs-off+squashfs:' /run/systemd/system.attached/app0.service.d/20-portable.conf >/dev/null
grep -q -F 'ExtensionImagePolicy=root=signed+growfs-off+squashfs:' /run/systemd/system.attached/app0.service.d/20-portable.conf >/dev/null
