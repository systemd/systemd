...
ngroups_max="$(getconf NGROUPS_MAX 2>/dev/null || echo 0)"
if ((${#truncation_gids[@]} >= 400 && ngroups_max >= 400)); then
    truncation_groups=()
    sysusers_args=()
    ...
    echo "Current environment supports fewer than 400 unused mapped supplementary GIDs, skipping truncation check."
fi
