...
# `systemctl daemon-reexec` returns as soon as it sees ECONNRESET/ETIMEDOUT
# on its D-Bus call, which can be before PID 1 has finished re-establishing
# the bus connection. Wait for the manager to settle before querying it.
