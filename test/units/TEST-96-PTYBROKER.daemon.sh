...
expect_failure_with io.systemd.PTYBroker.NoSuchPty ptyctl monitor no-such-pty-96 </dev/null
expect_failure_with io.systemd.PTYBroker.NoSuchPty ptyctl hangup no-such-pty-96
expect_error ConfigurePty '{"name":"no-such-pty-96","terminalSettings":{"columns":80}}' io.systemd.PTYBroker.NoSuchPty
...
assert 'pty96-enroll' in listing, listing
...
os.write(slave, b'hello-pty96-enroll-marker\r\n')
os.close(slave)
os.close(master)
EOF

timeout 30 bash -c 'until journalctl -q -t pty96-enroll-tag | grep hello-pty96-enroll-marker >/dev/null; do journalctl --sync; sleep 1; done'
wait_for_pty_deregistration pty96-enroll
