...
if grep -E '^[0-9a-f]+ [^ ]+ +[0-9]+ +[^ ]+ +08070000 +[0-9]+ +[0-9]+ +keyring +\.dm-verity: [0-9]+$' /proc/keys >/dev/null; then
    ...
    if openssl x509 -in /tmp/stranger.pem -outform DER | keyctl padd asymmetric '' %:.dm-verity 2>/dev/null; then
    ...
    # With the keyring left open by the kernel and systemd-keyring-setup installed,
    # getting here means the service did not provision it
    if [[ -e /usr/lib/systemd/system/systemd-keyring-setup.service &&
