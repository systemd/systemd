...
testcase_dot_strict_per_link_verify() {
    ...
    /usr/lib/systemd/systemd-networkd-wait-online --timeout=30 --interface=dns3-sdot
    ...
    timeout 10s bash -c "until ss -lnt 'sport = :853' | grep -q ${link_addr}; do sleep 0.2; done"
