...
EOF
    ...
    # restart resolved and enable mdns on interface hoge
    restart_resolved
    resolvectl mdns hoge yes
    ...
    run dig -p 5353 -t SRV "$(hostname -s)._ssh._tcp.local" @192.168.0.12
    grep -qE "$(hostname -s)\._ssh\._tcp\.local\.\s+[0-9]+\s+IN\s+SRV\s+42\s+13\s+22\s+$(hostname -s)\.local\." "$RUN_OUT"

    run dig -p 5353 -t TXT "$(hostname -s)._ssh._tcp.local" @192.168.0.12
    grep -qE "$(hostname -s)\._ssh\._tcp\.local\.\s+[0-9]+\s+IN\s+TXT\s+\"hogehogehoge\"" "$RUN_OUT"

    run resolvectl query "$(hostname -s).local"
    grep -qE "$(hostname -s).local: " "$RUN_OUT"
    grep -qE ".*192\.168\.0\.12\s+-- link: hoge" "$RUN_OUT"

    run resolvectl query -t SRV "$(hostname -s)._ssh._tcp.local"
    grep -qE "$(hostname -s)\._ssh\._tcp\.local IN SRV 42 13 22 $(hostname -s)\.local\s+-- link: hoge" "$RUN_OUT"

    run resolvectl query -t TXT "$(hostname -s)._ssh._tcp.local"
    grep -qE "$(hostname -s)\._ssh\._tcp\.local IN TXT \"hogehogehoge\"\s+-- link: hoge" "$RUN_OUT"

    run resolvectl service "$(hostname -s)._ssh._tcp.local"
    grep -qE "$(hostname -s)\._ssh\._tcp\.local: $(hostname -s)\.local:22 \[priority=42, weight=13\]" "$RUN_OUT"
}
