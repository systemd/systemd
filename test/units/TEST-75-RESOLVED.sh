...
testcase_08_resolved() {
    ...
    grep -qF "Refusing to output unauthenticated OPENPGPKEY records in JSON format." "$RUN_OUT"
    (! run resolvectl query 5a786cdc59c161cdafd818143705026636962198c66ed4c5b3da321e._openpgpkey.untrusted.test --json=short --type=ANY)
    grep -qF "Refusing to output unauthenticated OPENPGPKEY records in JSON format." "$RUN_OUT"
    (! run resolvectl openpgp mr.smith@signed.test --json=short --type=OPENPGPKEY)
    grep -qF -- "--type= may not be combined with openpgp when --json= is specified." "$RUN_OUT"
    (! run resolvectl tlsa signed.test:invalid --json=short)
    grep -qF 'Invalid port "invalid".' "$RUN_OUT"
    (! run resolvectl tlsa signed.test --json=short --type=TLSA)
    grep -qF -- "--type= may not be combined with tlsa when --json= is specified." "$RUN_OUT"
