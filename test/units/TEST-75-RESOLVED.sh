...
testcase_15_wait_online_dns() {
    ...
    timeout 5 bash -c "until [[ -s $tmpfile ]] && grep -qF 'Failed to connect to /run/systemd/resolve/io.systemd.Resolve.Monitor: Connection refused' $tmpfile; do sleep 0.1; done"
    systemctl start systemd-resolved.service

    # Wait until a retry attempt succeeds
    timeout 5 bash -c "until [[ -s $tmpfile ]] && grep -qE 'dns0: No.*DNS server is accessible' $tmpfile; do sleep 0.1; done"
