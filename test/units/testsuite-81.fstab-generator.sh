...
FSTAB_GENERAL=(
    ...
    "/dev/test21    /usr                                reiserfs    defaults 0 1"
    ...
    "/dev/test24    /initrd/mount/deps                  btrfs       x-initrd.mount,x-systemd.before=early.service,x-systemd.after=late.service 0 1"
...
check_fstab_mount_units() {
    ...
    for item in "${fstab_entries[@]}"; do
        ...
        # No links or units should exist for invalid and ignored entries
        if [[ "$what" =~ /(invalid|ignored)[0-9]*$ ]]; then
            (! test -e "$out_dir/$unit")
            (! test -h "$out_dir/$unit")
            (! grep -rF "$what" "$out_dir")
            continue
        fi
