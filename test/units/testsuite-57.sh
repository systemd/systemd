...
[Unit]
Description=TEST-57-RELOADING-RESTART Normal exit
...
cat >/run/systemd/system/testservice-fail-restart-57.service <<EOF
[Unit]
...
cat >/run/systemd/system/testservice-abort-restart-57.service <<EOF
[Unit]
...
wait_on_state_or_fail "testservice-fail-57.service" "failed" "30"

# This service sends a RELOADING=1 message then exits before it sends a
# READY=1. It should automatically restart on failure. Ensure it enters failed
# state and does not linger in reloading state.
systemctl start testservice-fail-restart-57.service
wait_on_state_or_fail "testservice-fail-restart-57.service" "failed" "30"
...
systemctl start testservice-abort-restart-57.service
pid=$(systemctl show testservice-abort-restart-57.service --property=ExecMainPID --value)
kill -6 "$pid"
wait_on_state_or_fail "testservice-abort-restart-57.service" "failed" "30"
