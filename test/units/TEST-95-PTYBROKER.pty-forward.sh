...
timeout 10 bash -c 'while ptyctl list | grep pty95-fwd-basic >/dev/null; do sleep .5; done'
...
MON_PID=
timeout 10 bash -c 'while ptyctl list | grep pty95-fwd-mon >/dev/null; do sleep .5; done'
