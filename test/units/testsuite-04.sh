...
EOF
    ...
    timeout 10 bash -c "while ! SYSTEMD_LOG_LEVEL=debug journalctl --verify --quiet --file /var/log/journal/$ID/system.journal 2>&1 | grep -q -F 'compress=${c}'; do sleep .5; done"
done
