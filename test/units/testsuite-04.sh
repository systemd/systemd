...
function is_xattr_supported() {
    ...
    LOGS=$(journalctl -q -u "text_xattr" -S "$START" -U "$END")
    FAILED_MSG="Failed to set 'user.journald_log_filter_patterns' xattr"
    NOTSUP_MSG="not supported"
    if [[ "$LOGS" == *"$FAILED_MSG"* && "$LOGS" == *"$NOTSUP_MSG"* ]]; then
        echo 0
    else
        echo 1
    fi
...
if [[ $(is_xattr_supported) -eq 1 ]]; then
