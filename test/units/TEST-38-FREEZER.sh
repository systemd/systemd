...
testcase_sigkill_frozen_unit() {
    echo "Test that killing a frozen unit resets FreezerState and allows restart:"

    local unit="sigkill-frozen.service"

    echo -n "  - systemctl kill --signal=KILL clears stuck FreezerState: "
    ...
    echo -n "  - kill -9 main PID clears stuck FreezerState: "
    systemctl freeze "$unit"
    check_freezer_state "$unit" "frozen"
    local pid
    ...
    echo -n "  - dead frozen-by-parent child stays frozen-by-parent until slice thaws: "
