...
test "$(run0 -u testuser --pipe unshare --user varlinkctl --exec call \
        ...
        /run/systemd/userdb/io.systemd.NamespaceResource \
        io.systemd.NamespaceResource.AllocateUserRange \
        '{"name":"test-delegate","size":65536,"userNamespaceFileDescriptor":0,"delegateCount":2,"delegateSize":65536}' \
        -- cat /proc/self/uid_map | wc -l)" -eq 3

# Test that delegateCount > 16 fails with TooManyDelegations error
(! run0 -u testuser --pipe unshare --user varlinkctl call \
        --push-fd=/proc/self/ns/user \
        /run/systemd/userdb/io.systemd.NamespaceResource \
        io.systemd.NamespaceResource.AllocateUserRange \
        '{"name":"test-fail","size":65536,"userNamespaceFileDescriptor":0,"delegateCount":17,"delegateSize":65536}') |&
...
(! run0 -u testuser --pipe unshare --user varlinkctl call \
        ...
        /run/systemd/userdb/io.systemd.NamespaceResource \
        io.systemd.NamespaceResource.AllocateUserRange \
        '{"name":"test-fail2","size":65536,"userNamespaceFileDescriptor":0,"delegateCount":1,"delegateSize":1234}') |&
...
# Test nested delegation with identity mapping
test "$(run0 -u testuser --pipe unshare --user varlinkctl --exec call \
        --push-fd=/proc/self/ns/user \
        /run/systemd/userdb/io.systemd.NamespaceResource \
        io.systemd.NamespaceResource.AllocateUserRange \
        '{"name":"test-delegate2","identity":true,"size":1,"userNamespaceFileDescriptor":0,"delegateAmount":3,"delegateSize":65536}' \
        -- unshare --user varlinkctl --exec call \
            --push-fd=/proc/self/ns/user \
            /run/systemd/userdb/io.systemd.NamespaceResource \
            io.systemd.NamespaceResource.AllocateUserRange \
            '{"name":"test-delegate3","size":65536,"userNamespaceFileDescriptor":0,"delegateAmount":2,"delegateSize":65536}' \
            -- cat /proc/self/uid_map | wc -l)" -eq 3
