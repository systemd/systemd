...
test "$(run0 -u testuser --pipe unshare --user varlinkctl --exec call \
        ...
        /run/systemd/userdb/io.systemd.NamespaceResource \
        io.systemd.NamespaceResource.AllocateUserRange \
        '{"name":"test-delegate2","identity":true,"size":1,"userNamespaceFileDescriptor":0,"delegateCount":3,"delegateSize":65536}' \
        -- unshare --user varlinkctl --exec call \
            --push-fd=/proc/self/ns/user \
            /run/systemd/userdb/io.systemd.NamespaceResource \
            io.systemd.NamespaceResource.AllocateUserRange \
            '{"name":"test-delegate3","size":65536,"userNamespaceFileDescriptor":0,"delegateCount":2,"delegateSize":65536}' \
            -- cat /proc/self/uid_map | wc -l)" -eq 3
