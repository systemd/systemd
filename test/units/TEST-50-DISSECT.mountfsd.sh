...
test "$(run0 -u testuser --pipe unshare --user varlinkctl --exec call \
        ...
        /run/systemd/userdb/io.systemd.NamespaceResource \
        io.systemd.NamespaceResource.AllocateUserRange \
        '{"name":"test-delegate","size":65536,"userNamespaceFileDescriptor":0,"delegateContainerRanges":2, "delegateDynamicUsers": 3}' \
        -- cat /proc/self/uid_map | wc -l)" -eq 6
...
# Test that delegateDynamicUsers > 16 fails
(! run0 -u testuser --pipe unshare --user varlinkctl call \
        --push-fd=/proc/self/ns/user \
        /run/systemd/userdb/io.systemd.NamespaceResource \
        io.systemd.NamespaceResource.AllocateUserRange \
        '{"name":"test-fail2","size":65536,"userNamespaceFileDescriptor":0,"delegateDynamicUsers":17}') |&
            grep "io.systemd.NamespaceResource.TooManyDelegations" >/dev/null
...
test "$(run0 -u testuser --pipe unshare --user varlinkctl --exec call \
        ...
        /run/systemd/userdb/io.systemd.NamespaceResource \
        io.systemd.NamespaceResource.AllocateUserRange \
        '{"name":"test-delegate2","identity":true,"size":1,"userNamespaceFileDescriptor":0,"delegateContainerRanges":3}' \
