...
systemd-pty-forward --quiet --console=broker --pty-name=pty96-winsize -- \
    bash -c 'touch /tmp/pty96-winsize-ready; exec sleep infinity' &
FWD_PID=$!
timeout 10 bash -c 'until test -e /tmp/pty96-winsize-ready; do sleep .2; done'
timeout 10 bash -c 'until ptyctl list | grep pty96-winsize >/dev/null; do sleep .5; done'

BACKEND_PTS=$(varlinkctl call /run/systemd/io.systemd.PTYBroker io.systemd.PTYBroker.ListPty '{"name":"pty96-winsize"}' | jq -r .backendPath)
test -e "$BACKEND_PTS"
...
os.write(bwfd, b'pty96-winsize-marker-1\n')
wait_for_output(b'pty96-winsize-marker-1')
...
os.write(bwfd, b'\x1b]2811;?\x1b\\' + b'pty96-winsize-marker-2\n')
wait_for_output(b'pty96-winsize-marker-2')
set_size(m1, 30, 90)
...
assert b'pty96-winsize-marker-1' in drained and b'pty96-winsize-marker-2' in drained
assert b'2811' not in drained, f'OSC 2811 sequence leaked into monitor output: {drained!r}'

kill_monitor(pid1)
EOF

ptyctl hangup pty96-winsize
