...
# This fails if fsopen()/fsconfig() bpffs is not supported by the kernel.
set +e
/usr/lib/systemd/tests/unit-tests/manual/test-private-bpf
RET_TEST_PRIVATE_BPF=$?
set -e

if (( RET_TEST_PRIVATE_BPF != 0 )); then
