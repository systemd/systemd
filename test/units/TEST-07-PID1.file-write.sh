...
testcase_multiple_features() {
    local squashed_container_image=/usr/share/minimal_0.raw
    ...
    # Draft note: I'd like to eliminate this write to the machine-id file, or at least use uuidgen if it has to presist.
    # I carefully debugged most permutations of 1) DelegateNamespaces=yes, 2) writing to etc/machine-id and 3) systemd.firstboot=no.
    # With 1 and not 2, /etc/machine-id is written, but firstboot runs and fails
    # With 2 and not 1, tmp mount fails, firstboot is skipped
    # When firstboot runs, it fails because it expects console input that it does not recieve.
    # Adding systemd.firstboot=no is no silver bullet:
    #     Trying not 1, not 2, and 3 fails
    #     Trying 1, not 2, and 3 fails
    # These testing iterations are very long, so I thought I'd ask the community before going further down this debugging path.
    echo 777cb2a8882f4a9f851bf132febd44be > "${CONTAINER_ROOT_DIR}/etc/machine-id"
