...
test_deferred_removal() {
    ...
    local volume=testdeferred
    local unit="systemd-clonesetup@$volume.service"

    workdir="$(mktemp -d)"
    read -r loop_src loop_dst loop_meta < <(create_loop_triplet "$workdir")

    clonetab_create "testdeferred" "$loop_src" "$loop_dst" "$loop_meta" "-"
    ...
    # keep the device open so its busy
    exec 9<"/dev/mapper/$volume"
    systemctl stop "$unit"
    udevadm settle --timeout=10
    exec 9<&-
    # Wait for udev to process mapper-node removal after releasing the open fd.
    udevadm settle --timeout=10

    # check if mapper has been removed
    test ! -e "/dev/mapper/$volume"
    ...
    rm -rf "$workdir"
    rm -f /etc/clonetab
    dmsetup remove testdeferred 2>/dev/null || true
}
...
run_main_flow
test_idempotent_stop
# Use a dedicated loop triplet; metadata from earlier cases can make this test fail.
test_region_size
test_escape_coverage
test_invalid_path_rejected
test_invalid_options_rejected
