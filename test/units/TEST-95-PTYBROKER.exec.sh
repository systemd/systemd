...
systemctl stop pty95-exec-long.service
timeout 10 bash -c 'while ptyctl list | grep pty95-exec-long.service >/dev/null; do sleep .5; done'
...
varlinkctl call /run/systemd/io.systemd.Manager io.systemd.Unit.StartTransient '
{
    "context" : {
        ...
        "Service" : {
            "Type" : "oneshot",
            "ExecStart" : [ { "path" : "/bin/sh", "arguments" : [ "-c", "echo hello-pty95-exec-varlink-marker" ] } ]
        }
    },
    "mode" : "replace"
}'
