...
at_exit() {
    set +e
    systemctl stop pty95-exec-long.service pty95-exec-term.service
    rm -f /tmp/pty95-exec-*
}
...
# shellcheck disable=SC2016
systemd-run -u pty95-exec-term.service \
    -p StandardInput=broker -p StandardOutput=broker \
    -p Environment=TERM=xterm \
    bash -xec '[[ "$TERM" == xterm ]]; exec sleep infinity'
timeout 10 bash -c 'until ptyctl list | grep pty95-exec-term.service >/dev/null; do sleep .5; done'
systemctl is-active pty95-exec-term.service
PTY_NAME=$(varlinkctl --more call /run/systemd/io.systemd.PTYBroker io.systemd.PTYBroker.ListPty '{}' | jq -r 'select(.description == "pty95-exec-term.service") | .name')
[[ -n "$PTY_NAME" ]]
...
systemctl stop pty95-exec-term.service
wait_for_pty_deregistration pty95-exec-term.service
