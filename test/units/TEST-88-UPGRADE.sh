...
save_unit_state() {
    ...
    rm -f /tmp/active-units /tmp/preexisting-units /tmp/triggered-units "$unit_files" "$units"
...
unit_removal_expected() {
    ...
    if [[ $operation == downgrade ]]; then
        [[ -e /tmp/preexisting-units ]] && grep -sxqF "$unit" /tmp/preexisting-units
    else
