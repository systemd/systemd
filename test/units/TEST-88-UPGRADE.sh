...
save_distro_units() {
    local package
    local paths=/tmp/distro-package-paths

    rm -f /tmp/distro-units "$paths"
    for package in "$pkgdir"/distro/*."$package_extension"; do
        if [[ $package_extension == deb ]]; then
            if ! dpkg-deb --fsys-tarfile "$package" | tar -tf - >>"$paths"; then
                echo 'Failed to inspect distro packages, removed units will be treated as failures.'
                rm -f "$paths"
                return 0
            fi
    ...
    sed -n -E 's#^(\./|/)?(usr/)?lib/systemd/system/(.*/)?([^/]+)$#\4#p' "$paths" |
        sort -u >/tmp/distro-units
    rm -f "$paths"
}
...
unit_removal_expected() {
    ...
    if [[ $operation == downgrade ]]; then
        [[ -e /tmp/distro-units ]] && ! grep -sxqF "$unit" /tmp/distro-units
    else
        [[ -e /tmp/original-units ]] && ! grep -sxqF "$unit" /tmp/original-units
    fi
}
...
rm -f /tmp/original-units
save_unit_state
save_distro_units
if [[ -e /tmp/preexisting-units ]]; then
    cp /tmp/preexisting-units /tmp/original-units
fi
