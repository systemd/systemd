...
if [ "$(systemctl --failed --output json |jq length)" -gt 0 ]; then
    echo 'Systemd failed units found before the test:'
    systemctl --failed
    systemctl --failed --output json |jq -r '.[].unit' >/tmp/failed-units
fi

check_sd() {
    local unit fail=0
    for unit in $(systemctl --failed --output json |jq -r '.[].unit'); do
        grep -sxqF "$unit" /tmp/failed-units && continue
        fail=1
        systemctl status "$unit"
    done
    [ "$fail" -eq 1 ] && echo 'Systemd units above failed after the test!'
