...
ExecStart=bash -c ' \
    trap '"'"' \
        printf "RELOADING=1\\nMONOTONIC_USEC=\$\${EPOCHREALTIME/[^0-9]/}" | ncat --no-shutdown --unixsock --udp \$\$NOTIFY_SOCKET --source /run/notify; \
        ls /etc | grep marker; \
        cat /usr/lib/os-release; \
        printf "READY=1" | ncat --no-shutdown --unixsock --udp \$\$NOTIFY_SOCKET --source /run/notify; \
    '"'"' SIGHUP; \
    printf "READY=1" | ncat --no-shutdown --unixsock --udp \$\$NOTIFY_SOCKET --source /run/notify; \
    while true; do sleep 1; done; \
'
