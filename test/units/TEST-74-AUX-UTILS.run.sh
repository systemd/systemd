...
if [[ -e /usr/lib/pam.d/systemd-run0 ]] || [[ -e /etc/pam.d/systemd-run0 ]]; then
    ...
    for tu in "" "root" "0" "testuser"; do
        ...
        assert_eq "$(run0 --setenv=SHLVL=10 ${tu:+"--user=$tu"} printenv SHLVL)" "10"
        assert_eq "$(run0 --setenv=SHLVL=10 ${tu:+"--user=$tu"} -i echo '$$SHLVL')" "11"
        assert_eq "$(run0 --setenv=SHLVL=10 ${tu:+"--user=$tu"} --expand-environment=no -i echo '$SHLVL')" "11"
