...
if [[ -e /usr/lib/pam.d/systemd-run0 ]] || [[ -e /etc/pam.d/systemd-run0 ]]; then
    ...
    for arg in "" "root" "0" "testuser"; do
        assert_eq "$(run0 ${arg:+"--user=$arg"} bash -c 'echo $SUDO_USER')" "$USER"
        assert_eq "$(run0 ${arg:+"--user=$arg"} bash -c 'echo $SUDO_UID')" "$(id -u "$USER")"
        assert_eq "$(run0 ${arg:+"--user=$arg"} bash -c 'echo $SUDO_GID')" "$(id -u "$USER")"

        # Validate that we actually went properly through PAM (XDG_SESSION_TYPE is set by pam_systemd)
        assert_eq "$(run0 ${arg:+"--user=$arg"} bash -c 'echo $XDG_SESSION_TYPE')" "unspecified"

        # Validate that $SHELL is set to login shell of target user when cmdline is supplied (not invoking shell)
        TARGET_LOGIN_SHELL="$(getent passwd "$arg" | cut -d: -f7)"
        assert_eq "$(run0 ${arg:+"--user=$arg"} printenv SHELL)" "$TARGET_LOGIN_SHELL"
    done
