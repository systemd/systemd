...
timeout 30 bash -c "
    while ! machinectl list --no-legend 2>/dev/null | grep -q '$MACHINE'; do
...
varlinkctl call /run/systemd/machine/io.systemd.Machine io.systemd.Machine.List "{\"name\":\"$MACHINE\"}" | grep -q varlinkAddress
echo "varlinkAddress exposed in Machine.List"

# Test machinectl pause
machinectl pause "$MACHINE"
echo "pause succeeded"

# Test machinectl resume
machinectl resume "$MACHINE"
echo "resume succeeded"

# Test machinectl poweroff -- this sends ACPI powerdown to QEMU. The guest will not handle it
# (our init is just 'sleep infinity'), but the QMP command should succeed.
machinectl poweroff "$MACHINE"
echo "poweroff succeeded"

# Also exercise the VMControl varlink interface directly via varlinkctl.
# Look up the varlink address from machined.
...
STATUS=$(varlinkctl call "$VARLINK_ADDR" io.systemd.VMControl.QueryStatus '{}')
echo "$STATUS" | jq -e '.running == true'
echo "$STATUS" | jq -e '.status == "running"'
echo "QueryStatus returned running state"

# Pause, verify, resume
varlinkctl call "$VARLINK_ADDR" io.systemd.VMControl.Pause '{}'
STATUS=$(varlinkctl call "$VARLINK_ADDR" io.systemd.VMControl.QueryStatus '{}')
echo "$STATUS" | jq -e '.running == false'
echo "Verified paused state via QueryStatus"

varlinkctl call "$VARLINK_ADDR" io.systemd.VMControl.Resume '{}'
STATUS=$(varlinkctl call "$VARLINK_ADDR" io.systemd.VMControl.QueryStatus '{}')
echo "$STATUS" | jq -e '.running == true'
