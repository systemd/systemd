...
testcase_volatile_link_journal_no_userns() {
    local root machine_id journal_dir
    ...
    # Check ACLs to ensure no invalid GIDs are present If getfacl is available,
    # verify no 4294967295 appears in the ACLs
    if command -v getfacl &> /dev/null; then
        local acl_output
        acl_output="$(getfacl "$journal_dir" 2>/dev/null || true)"
        if [[ -n "$acl_output" ]]; then
            # Ensure the invalid GID doesn't appear in ACL output
