...
testcase_link_journal_image() {
    ...
    trap 'umount "$mnt_dir" &>/dev/null; rm -fr "$mnt_dir"; rm -f "$journal_path"; rm -f "$image"; rm -fr "$root"; rmdir "$persistent_dir" &>/dev/null' EXIT
    ...
    # Verify the symlink points to the persistent directory we created
    # NOT to a temporary /tmp/nspawn-root-* path
    symlink_target="$(readlink -f "$journal_path")"
