...
testcase_link_journal_directory() {
    ...
    trap 'rm -f "$journal_path"; rm -fr "$root"' EXIT
...
testcase_link_journal_image() {
    ...
    mnt_dir="$(mktemp -d)"
    trap 'umount "$mnt_dir" &>/dev/null; rm -fr "$mnt_dir"; rm -f "$journal_path"; rm -f "$image"; rm -fr "$root"' EXIT
    ...
    # Verify the symlink points to the persistent image path
    # NOT to a temporary /tmp/nspawn-root-* path
    symlink_target="$(readlink -f "$journal_path")"
    [[ "$symlink_target" != /tmp/nspawn-root-* ]]
}
...
testcase_link_journal_ephemeral() {
    # Test that journal symlinks point to a persistent snapshot path
    # for --ephemeral.
    local root machine_id journal_path symlink_target
    ...
    # Verify the symlink points to a persistent path (ephemeral snapshot)
    # NOT to a temporary /tmp/nspawn-root-* path
    symlink_target="$(readlink -f "$journal_path")"
    [[ "$symlink_target" != /tmp/nspawn-root-* ]]
}
...
testcase_link_journal_negative() {
    ...
    trap 'rm -fr "$root"' EXIT # journal_path won't be created
