...
cat >"/run/systemd/systemd/$ONSUCCESS_UNIT_NAME" <<EOF
[Service]
Type=oneshot
...
cat <<EOF | cmp - <(systemctl show "$UNIT_NAME" -P SubState -P Result -P NRestarts)
dead
success
1
EOF
...
rm "$TMP_FILE" /run/systemd/system/{"$UNIT_NAME","$ONSUCCESS_UNIT_NAME"}
