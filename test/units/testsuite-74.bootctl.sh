...
basic_tests() {
    ...
    bootctl "${opts[@]}" --help
    bootctl "${opts[@]}" --version

    bootctl "${opts[@]}" install --make-entry-directory=yes
    bootctl "${opts[@]}" remove  --make-entry-directory=yes

    bootctl "${opts[@]}" install --all-architectures
    bootctl "${opts[@]}" remove  --all-architectures

    bootctl "${opts[@]}" install --make-entry-directory=yes --all-architectures
    bootctl "${opts[@]}" remove  --make-entry-directory=yes --all-architectures

    bootctl "${opts[@]}" install
    (! bootctl "${opts[@]}" update)
    bootctl "${opts[@]}" update --graceful

    bootctl "${opts[@]}" is-installed
    bootctl "${opts[@]}" is-installed --graceful
    bootctl "${opts[@]}" random-seed

    bootctl "${opts[@]}"
    bootctl "${opts[@]}" status
    bootctl "${opts[@]}" status --quiet
    bootctl "${opts[@]}" list
    bootctl "${opts[@]}" list --quiet
    bootctl "${opts[@]}" list --json=short
    bootctl "${opts[@]}" list --json=pretty
...
EOF
    ...
    SYSTEMD_RELAX_ESP_CHECKS=yes SYSTEMD_RELAX_XBOOTLDR_CHECKS=yes basic_tests "${IMAGE_DIR}/root"
}
