...
cp /etc/vconsole.conf /etc/otherpath
cp "${LOCALE_PATH}" /etc/otherpath/locale.conf
...
[[ "$(readlink /etc/otherpath/localtime)" = "$(readlink "${ROOT}/etc/otherpath/hostname")" ]]
...
[[ "$(readlink "${ROOT}/etc/otherpath/hostname")" = "../../usr/share/zoneinfo/Europe/Oslo" ]]
