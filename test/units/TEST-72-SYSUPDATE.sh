...
# Test '@W/' as prefix in MatchPattern= for subpaths in SHA256SUMS
...
# A regular-file source where '@W/' descends into sub/ to match against the basename
cat >"$CONFIGDIR/01-basename-dir.transfer" <<EOF
[Source]
Type=regular-file
Path=$WORKDIR/source
MatchPattern=@W/blob-@v.bin
