...
URL_TAR_SOURCE="$WORKDIR/url-tar-source"
mkdir -p "$URL_ROOT/etc/systemd/sysupdate.conf.d" "$URL_ROOT/run/sysupdate.d" \
         ...
         "$URL_SUB_SOURCE/sub" "$URL_TAR_SOURCE/dir-v11"
...
printf 'foo\n' >"$URL_TAR_SOURCE/dir-v11/foo.txt"
printf 'bar\n' >"$URL_TAR_SOURCE/dir-v11/bar.txt"
...
tar --numeric-owner -C "$URL_TAR_SOURCE/dir-v11" -czf "$URL_TAR_SOURCE/dir-v11.tar.gz" .
(cd "$URL_TAR_SOURCE" && sha256sum dir-v11.tar.gz >SHA256SUMS)
...
EOF
"$SYSUPDATE" --root="$URL_ROOT" --verify=no update v11
cmp "$URL_TAR_SOURCE/dir-v11/foo.txt" "$URL_ROOT/var/lib/sysupdate-url-tar/current/foo.txt"
...
rm -rf "$URL_ROOT" "$URL_INVALID_ROOT" "$URL_MAIN_SOURCE" "$URL_DROPIN_SOURCE" \
       "$URL_COMMAND_SOURCE" "$URL_SUB_SOURCE" "$URL_TAR_SOURCE" "$URL_INVALID_SOURCE"
