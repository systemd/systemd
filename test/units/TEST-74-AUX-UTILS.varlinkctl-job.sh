...
job_id=$(varlinkctl call /run/systemd/io.systemd.Manager io.systemd.Job.List '{"unit": "varlink-test-job.service"}' | jq -r '.runtime.Id')
test "$job_id" -gt 0

# Verify context/runtime fields
varlinkctl call /run/systemd/io.systemd.Manager io.systemd.Job.List '{"unit": "varlink-test-job.service"}' | jq -e '.context.Unit == "varlink-test-job.service"'
varlinkctl call /run/systemd/io.systemd.Manager io.systemd.Job.List '{"unit": "varlink-test-job.service"}' | jq -e '.context.JobType == "start"'
varlinkctl call /run/systemd/io.systemd.Manager io.systemd.Job.List '{"unit": "varlink-test-job.service"}' | jq -e '.runtime.State'

# Lookup by job ID should return the same job
varlinkctl call /run/systemd/io.systemd.Manager io.systemd.Job.List "{\"id\": $job_id}" | jq -e ".context.Unit == \"varlink-test-job.service\""

# Lookup by both id and unit (matching) should succeed
varlinkctl call /run/systemd/io.systemd.Manager io.systemd.Job.List "{\"id\": $job_id, \"unit\": \"varlink-test-job.service\"}" | jq -e ".runtime.Id == $job_id"

# Lookup by both id and unit (conflicting) should fail
(! varlinkctl call /run/systemd/io.systemd.Manager io.systemd.Job.List '{"id": 1, "unit": "varlink-test-job.service"}')
...
systemctl start --no-block varlink-test-job.service
varlinkctl call /run/systemd/io.systemd.Manager io.systemd.Job.List '{"unit": "varlink-test-job.service"}' | jq -e '.runtime.Id'
