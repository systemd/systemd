...
testcase_disk_size_max_activation() {
    ...
    local initial_allocated_bytes initial_image_size leave_extra_bytes old_hole_bytes old_unreserved_image_size
    ...
    trap "cleanup_disk_size_max_activation ${user@Q} ${filler@Q} ${create_output@Q}" EXIT
    ...
    initial_image_size="$(stat --format "%s" "$image")"
    ...
    test "$image_size" -gt "$initial_image_size"
