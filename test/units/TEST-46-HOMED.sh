...
testcase_dry_run_rlimit_matching() {
    ...
    output="$(homectl_dry_run create test-rlimit-user \
        --rlimit=NOFILE=1 -A --rlimit=NOFILE=42 --enforce-password-policy=no)"
    jq -e '.resourceLimits.RLIMIT_NOFILE == {"cur":42,"max":42}' <<<"$output"
    jq -e '
        (.perMachine // [])
        | map(select(.resourceLimits.RLIMIT_NOFILE != null))
        | length == 0
    ' <<<"$output"
    ...
    for match in -A -T -N; do
        assert_rlimit_targeted_removal_rejected "$match" --rlimit=NOFILE=
        assert_rlimit_targeted_removal_rejected "$match" --rlimit=
        assert_rlimit_targeted_removal_rejected "$match" --storage=directory --rlimit=NOFILE=
    done

    output="$(homectl_dry_run create test-rlimit-user \
        --match=auto --rlimit=NOFILE=42 --rlimit=NOFILE= --enforce-password-policy=no)"
    jq -e '.resourceLimits == null and ((.perMachine // []) | map(select(has("resourceLimits"))) | length == 0)' <<<"$output"
}
...
testcase_match() {
    ...
    PASSWORD=test homectl update --rlimit=NOFILE=1 matchtest
    jq -e '.resourceLimits.RLIMIT_NOFILE == {"cur":1,"max":1}' \
        <<<"$(homectl inspect --json=short matchtest)"
    PASSWORD=test homectl update -A --rlimit=NOFILE=42 matchtest
    jq -e '
        .resourceLimits.RLIMIT_NOFILE == {"cur":42,"max":42} and
        ((.perMachine // []) | map(select(.resourceLimits.RLIMIT_NOFILE != null)) | length == 0)
    ' \
        <<<"$(homectl inspect --json=short matchtest)"
    if PASSWORD=test homectl update -T --storage=directory --rlimit=NOFILE= matchtest; then
        echo "targeted resource limit removal unexpectedly succeeded" >&2
        return 1
    fi
