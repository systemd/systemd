...
if findmnt -n -o options /tmp | grep -q usrquota ; then
    ...
    props='--property=SetCredential=pam.authtok.systemd-run0:quux -u tmpfsquota'

    run0 $props dd if=/dev/zero of=/dev/shm/quotatestfile1 bs=1024 count=30
    (! run0 $props dd if=/dev/zero of=/dev/shm/quotatestfile2 bs=1024 count=30)
    run0 $props rm /dev/shm/quotatestfile1 /dev/shm/quotatestfile2
    run0 $props dd if=/dev/zero of=/dev/shm/quotatestfile1 bs=1024 count=30
    run0 $props rm /dev/shm/quotatestfile1
