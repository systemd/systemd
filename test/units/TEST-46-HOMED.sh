...
testcase_update_auto_resize_mode_without_space() {
    ...
    cleanup_update_auto_resize_mode_without_space() (
        ...
        local cleanup_create_output="${2:?}"
        local cleanup_filler="${3:?}"

        rm -f "$cleanup_create_output" "$cleanup_filler"
    ...
    trap "cleanup_update_auto_resize_mode_without_space ${user@Q} ${create_output@Q} ${filler@Q}" RETURN ERR EXIT
