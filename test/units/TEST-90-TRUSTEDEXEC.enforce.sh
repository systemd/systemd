...
HELPER_PID=
"$HELPER" attach > /tmp/restrict-exec-attach.out &
HELPER_PID=$!

# Wait for the helper to finish printing (it calls pause() after fflush)
for ((i = 0; i < 600; i++)); do
    if grep -q 'LINK_IDS=' /tmp/restrict-exec-attach.out 2>/dev/null; then
        break
    fi
    sleep 0.1
done
...
PID1_ADDR=$(awk '/r-xp/ { print $1; exit }' /proc/1/maps | cut -d- -f1)
if [[ -n "$PID1_ADDR" ]]; then
