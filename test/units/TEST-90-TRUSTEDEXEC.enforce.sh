...
if command -v bpftool >/dev/null 2>&1 && [[ -n "${VERITY_MAP_ID:-}" ]]; then
    ...
    if [[ -n "${PROG_IDS:-}" ]]; then
        ...
        for pid in "${prog_ids[@]}"; do
            if bpftool prog show id "$pid" 2>/dev/null; then
                echo "ERROR: bpftool should not be able to access protected prog (ID $pid)!" >&2
                exit 1
            fi
        done
...
if ! dd if=/proc/1/mem of=/dev/null bs=1 count=1 seek=0 2>/dev/null; then
