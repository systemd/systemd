...
at_exit() {
    ...
    rm -rf /tmp/verity-mount /tmp/restrict-exec-workdir
}
trap at_exit EXIT
...
if command -v python3 >/dev/null 2>&1; then
    ...
    if python3 -c "
import mmap, os, sys
fd = os.open('/tmp/restrict-exec-test/testfile', os.O_RDONLY)
try:
...
ls -la /sys/fs/bpf/test_restrict_exec_* 2>&1 || echo "(no pins found)"
