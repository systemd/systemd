...
(! busctl call \
    ...
    0 \
    start replace \
    0 2>&1) | grep -F "No units specified" >/dev/null
...
(! busctl call \
    ...
    start replace \
    1 2>&1) | grep -F "Invalid flags parameter" >/dev/null
...
(! busctl call \
    ...
    0 2>&1) | grep -F "Job type not-a-real-job-type invalid" >/dev/null
...
(! busctl call \
    ...
    0 2>&1) | grep -F "Job mode not-a-real-mode invalid" >/dev/null
...
# Give PID1 a moment to settle and then verify the valid unit was not started
...
# Atomicity: neither unit must end up activated by the failed call.
sleep 0.2
