...
test /dev/disk/by-ref/waldo -ef "$LOOP"
...
if [ "$(stat -c '%Hd:%Ld' "${image}.raw")" != '?d:?d' ] ; then
   ...
   test "$(stat -c '/dev/disk/by-inode/%Hd:%Ld-%i' "${image}.raw")" -ef "$LOOP"
fi
...
# Check if the /dev/disk/by-ref/$name symlink really references the right device
test "/dev/disk/by-ref/$name" -ef "$LOOP"

# Detach by the /dev/disk/by-ref symlink
systemd-dissect --detach "/dev/disk/by-ref/$name"

name="$(for _ in {1..63}; do echo -n 'x'; done)"
LOOP="$(systemd-dissect --attach --loop-ref="$name" "${image}.raw")"
udevadm trigger -w "$LOOP"

# Check if the /dev/disk/by-ref/$name symlink does not exist
test ! -e "/dev/disk/by-ref/$name"
