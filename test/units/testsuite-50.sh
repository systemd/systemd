...
find /tmp/test-50-confext
grep . /tmp/test-50-confext/etc/waldo
grep . /tmp/test-50-confext/etc/extension-release.d/*

systemd-repart -C -s /tmp/test-50-confext --certificate=/tmp/test-50-cert.crt --private-key=/tmp/test-50-privkey.key /var/lib/confexts/waldo.confext.raw
rm -rf /tmp/test-50-confext

mkdir -p /run/verity.d
cp /tmp/test-50-cert.crt /run/verity.d/

systemd-dissect --mtree /var/lib/confexts/waldo.confext.raw
...
rm /run/verity.d/test-50-cert.crt /var/lib/confexts/waldo.confext.raw /tmp/test-50-cert.crt /tmp/test-50-privkey.key
