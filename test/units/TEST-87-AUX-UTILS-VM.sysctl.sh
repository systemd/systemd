...
# --save= and --revert=
##########################
(! /usr/lib/systemd/systemd-sysctl --save=)
(! /usr/lib/systemd/systemd-sysctl --save=.)
(! /usr/lib/systemd/systemd-sysctl --save=.conf)
(! /usr/lib/systemd/systemd-sysctl --save=aaa/hoge.conf)
(! /usr/lib/systemd/systemd-sysctl --save=../hoge.conf)
...
(! /usr/lib/systemd/systemd-sysctl --save=foo)

# Do not create an empty conf file.
/usr/lib/systemd/systemd-sysctl \
    --prefix=/net/ipv4/conf/hoge --save=foo --inline "# comment only"
...
/usr/lib/systemd/systemd-sysctl \
    --prefix=/net/ipv4/conf/hoge --save=foo --inline -- \
...
/usr/lib/systemd/systemd-sysctl --prefix=/net/ipv4/conf/hoge --save=foo - <<EOF
...
# Verify that the saved .conf file is loaded.
...
# Check that the saved .conf file can be overridden.
/usr/lib/systemd/systemd-sysctl --prefix=/net/ipv4/conf/hoge --save=foo /run/sysctl.d/foo.conf - <<EOF
...
# Assume that no saved sysctl .conf files modify these variables.
assert_eq "$(cat /proc/sys/net/ipv4/conf/hoge/drop_gratuitous_arp)" "0"
