...
for scenario in cpu-only memory-control startup-control; do
    ...
    reset_properties=(AllowedCPUs=)
    if [[ "$scenario" == startup-control ]]; then
        reset_properties+=(StartupAllowedCPUs=)
    fi
    systemctl set-property --runtime "$unit" "${reset_properties[@]}"

    pid_after=$(systemctl show "$unit" --property=MainPID --value)
    allowed=$(systemctl show "$unit" --property=AllowedCPUs --value)
    startup_allowed=$(systemctl show "$unit" --property=StartupAllowedCPUs --value)
    effective=$(systemctl show "$unit" --property=EffectiveCPUs --value)
    task_cpus=$(
        awk '/^Cpus_allowed_list:/ { print $2 }' "/proc/$pid_after/status" 2>/dev/null ||
            echo "<no process>"
    )
    ...
    if [[ "$pid_before" != "$pid_after" || -n "$allowed" || -n "$startup_allowed" ]] ||
        [[ "$task_cpus" != "$parent_cpus" ]] ||
    ...
    if [[ "$scenario" == memory-control ]]; then
        ...
        if [[ "$memory_reset_supported" == yes ]]; then
            ...
            task_mems=$(
                awk '/^Mems_allowed_list:/ { print $2 }' "/proc/$pid_after/status" 2>/dev/null ||
                    echo "<no process>"
            )
