...
END
    ...
    mkdir -p /run/systemd/coredump.conf.d/
    printf '[Coredump]\nAccessContainer=no' >/run/systemd/coredump.conf.d/99-access-container.conf
    ...
    printf '[Coredump]\nAccessContainer=yes' >/run/systemd/coredump.conf.d/99-access-container.conf
    unshare --pid --fork --mount-proc --mount --uts --ipc --net /bin/bash -c "$MAKE_STACKTRACE_DUMP" || :
    coredumpctl -1 info "$CORE_STACKTRACE_TEST_BIN" | grep -zqE 'baz.*bar.*foo'
else
    echo "libdw doesn't not support setting sysroot, skipping AccessContainer= test"
fi
