...
if [[ "\$MONITOR_UNIT" != "testservice-success-68.service" && "\$MONITOR_UNIT" != "testservice-success-68-additional.service" && "\$MONITOR_UNIT" != "testservice-transient-success-68.service" ]]; then
    echo 'MONITOR_UNIT was "\$MONITOR_UNIT", expected "testservice-success-68{-additional}.service"';
    exit 1;
fi
...
if [[ "\$MONITOR_UNIT" != "testservice-failure-68.service" && "\$MONITOR_UNIT" != "testservice-failure-68-additional.service" && "\$MONITOR_UNIT" != "testservice-transient-failure-68.service" ]]; then
    echo 'MONITOR_UNIT was "\$MONITOR_UNIT", expected "testservice-failure-68{-additional}.service"';
    exit 1;
fi
