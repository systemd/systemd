...
verify_seqnum() {
    ...
    if systemd-detect-virt -vq; then
        # When we are running on qemu, the initrd may come from the host, and its journald may be old.
        return 0
    fi

    (! journalctl -q -o short-monotonic -u systemd-journald.service --grep 'rotating')

    set +x
    previous_seqnum=0
    previous_seqnum_id=
    previous_boot_id=
    journalctl --system -o json | jq -r '[.__SEQNUM, .__SEQNUM_ID, ._BOOT_ID] | @tsv' |
