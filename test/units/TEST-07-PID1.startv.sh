...
systemd-run --unit=marker-hogefoo.service --service-type=notify -p NotifyAccess=all bash -c 'journalctl --follow | grep -m1 marker_hogefoo'
systemd-run --unit=marker-hogebar.service --service-type=notify -p NotifyAccess=all bash -c 'journalctl --follow | grep -m1 marker_hogebar'
