...
[Link]
Property=
Property=LINK_DRIVER=%d LINK_SYSNAME=%k LINK_DEVPATH=%p LINK_VERSION=%v
EOF

udevadm control --reload

output=$(udevadm test --action add /sys/class/net/test1)
assert_in "LINK_DRIVER=dummy" "$output"
assert_in "LINK_SYSNAME=test1" "$output"
assert_in "LINK_DEVPATH=/devices/virtual/net/test1" "$output"
