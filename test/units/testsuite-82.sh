...
if [ -f /run/testsuite82.touch3 ]; then
    ...
    # Check that we got a PrepareForShutdownWithType signal with the right type
    test "$(cat /run/testsuite82.signal | jq .payload.data[1])" = "\"soft-reboot\""
...
EOF
    ...
    # PrepareForShutdownWithType signal and that it says 'soft-reboot'.
    systemd-run --unit busctl.service --property StandardOutput=file:/run/testsuite82.signal \
        busctl monitor --json=pretty --match 'sender=org.freedesktop.login1,path=/org/freedesktop/login1,interface=org.freedesktop.login1.Manager,member=PrepareForShutdownWithType,type=signal'
