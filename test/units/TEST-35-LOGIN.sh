...
testcase_varlink() {
    ...
    echo "$session_out" | jq -e ".context.Id == \"$session\"" >/dev/null
    echo "$session_out" | jq -e ".context.User.UID == $uid" >/dev/null
    echo "$session_out" | jq -e '.context.User.Name == "logind-test-user"' >/dev/null
    ...
    echo "$session_out" | jq -e '.context.CanIdle == true' >/dev/null
    echo "$session_out" | jq -e '.context.CanLock == true' >/dev/null
    ...
    echo "$pid_out" | jq -e ".context.Id == \"$session\"" >/dev/null
    ...
    echo "$ok_out" | jq -e ".context.Id == \"$session\"" >/dev/null
    ...
    varlinkctl call --more "$VARLINK_SOCKET" io.systemd.Login.ListSessions '{}' \
        | jq --seq -e --arg s "$session" 'select(.context.Id == $s)' >/dev/null
