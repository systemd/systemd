...
testcase_varlink() {
    ...
    varlinkctl call --more "$VARLINK_SOCKET" io.systemd.Login.ListSessions '{}' \
        | jq -e --arg s "$session" 'select(.Session.Id == $s)' >/dev/null
    ...
    # default UID should return the caller's user
    caller_uid=$(id -ru)
    default_user_out=$(varlinkctl call "$VARLINK_SOCKET" io.systemd.Login.DescribeUser '{}')
    echo "$default_user_out" | jq -e --argjson u "$caller_uid" '.User.UID == $u' >/dev/null
