...
testcase_varlink() {
    ...
    echo "$user_out" | jq -e ".UID == $uid" >/dev/null
    echo "$user_out" | jq -e '.Name == "logind-test-user"' >/dev/null
    echo "$user_out" | jq -e '.State' >/dev/null
    echo "$user_out" | jq -e '.Linger == false' >/dev/null
    # Sessions is now a flat array of session id strings (no wrapper object).
    echo "$user_out" | jq -e ".Sessions[] | select(. == \"$session\")" >/dev/null
    ...
    pid_user_out=$(varlinkctl call "$VARLINK_SOCKET" io.systemd.Login.ListUsers \
        "{\"PID\":{\"pid\":$leader_pid}}")
    echo "$pid_user_out" | jq -e --argjson u "$uid" '.UID == $u' >/dev/null
    ...
    echo "$ok_user_out" | jq -e --argjson u "$uid" '.UID == $u' >/dev/null
    ...
    echo "$seat_out" | jq -e '.ID == "seat0"' >/dev/null
    echo "$seat_out" | jq -e '.CanTTY == true' >/dev/null
    # Sessions is a flat array of session id strings.
    echo "$seat_out" | jq -e ".Sessions[] | select(. == \"$session\")" >/dev/null
    ...
    echo "$inhibitor_out" | jq --seq -e 'select(.Who == "varlink-test") | .What | type == "array" and contains(["shutdown"])' >/dev/null
    echo "$inhibitor_out" | jq --seq -e 'select(.Who == "varlink-test") | .Mode == "block"' >/dev/null
    echo "$inhibitor_out" | jq --seq -e 'select(.Who == "varlink-test") | .Why == "testing varlink"' >/dev/null
