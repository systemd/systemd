...
testcase_varlink() {
    ...
    : "--- ListSessions ---"
    varlinkctl call --more "$VARLINK_SOCKET" io.systemd.Login.ListSessions '{}' | grep "$session" >/dev/null
    ...
    inhibitor_out=$(varlinkctl call --more --collect "$VARLINK_SOCKET" io.systemd.Login.ListInhibitors '{}')
    echo "$inhibitor_out" | jq '[.[] | select(.Inhibitor.Who == "varlink-test")][0]' | jq -e '.Inhibitor.What == "shutdown"' >/dev/null
    echo "$inhibitor_out" | jq '[.[] | select(.Inhibitor.Who == "varlink-test")][0]' | jq -e '.Inhibitor.Mode == "block"' >/dev/null
    echo "$inhibitor_out" | jq '[.[] | select(.Inhibitor.Who == "varlink-test")][0]' | jq -e '.Inhibitor.Why == "testing varlink"' >/dev/null
    echo "$inhibitor_out" | jq '[.[] | select(.Inhibitor.Who == "varlink-test")][0]' | jq -e '.Inhibitor.Id' >/dev/null
    echo "$inhibitor_out" | jq '[.[] | select(.Inhibitor.Who == "varlink-test")][0]' | jq -e '.Inhibitor.Since' >/dev/null
