...
testcase_varlink() {
    ...
    echo "$inhibitor_out" | jq -s '[.[] | select(.Inhibitor.Who == "varlink-test")][0]' | jq -e '.Inhibitor.What == "shutdown"' >/dev/null
    echo "$inhibitor_out" | jq -s '[.[] | select(.Inhibitor.Who == "varlink-test")][0]' | jq -e '.Inhibitor.Mode == "block"' >/dev/null
    echo "$inhibitor_out" | jq -s '[.[] | select(.Inhibitor.Who == "varlink-test")][0]' | jq -e '.Inhibitor.Why == "testing varlink"' >/dev/null
    echo "$inhibitor_out" | jq -s '[.[] | select(.Inhibitor.Who == "varlink-test")][0]' | jq -e '.Inhibitor.Id' >/dev/null
    echo "$inhibitor_out" | jq -s '[.[] | select(.Inhibitor.Who == "varlink-test")][0]' | jq -e '.Inhibitor.Since' >/dev/null
