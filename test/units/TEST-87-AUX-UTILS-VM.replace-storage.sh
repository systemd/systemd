...
machinectl bind-volume "$MACHINE" \
    "fs:${RUNTIME_VOL}:virtio-blk:create=new,size=32M,template=sparse-file"
echo "Hot-added runtime bind-volume"

# --- Test 1: happy-path replace ---
varlinkctl --push-fd="$WORKDIR/new-backing-1.raw" call "$VARLINK_ADDR" \
...
varlinkctl --push-fd="$WORKDIR/new-backing-2.raw" call "$VARLINK_ADDR" \
    io.systemd.MachineInstance.ReplaceStorage \
    "{\"fileDescriptorIndex\":0,\"name\":\"fs:${RUNTIME_VOL}\"}"
...
# --- Test 5: invalid name (no provider:volume separator) must fail ---
if varlinkctl --push-fd="$WORKDIR/new-backing-1.raw" call "$VARLINK_ADDR" \
        io.systemd.MachineInstance.ReplaceStorage \
        '{"fileDescriptorIndex":0,"name":"not-a-valid-name"}' 2>"$WORKDIR/replace-badname.err"; then
    echo "ERROR: ReplaceStorage with invalid name should have failed"
    cat "$WORKDIR/replace-badname.err"
    exit 1
fi
grep -E "InvalidParameter|invalid" "$WORKDIR/replace-badname.err" >/dev/null
echo "Invalid name correctly rejected"
