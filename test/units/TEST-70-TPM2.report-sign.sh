...
echo "$sig_json" | jq -r '.data["public-key"]' | unhex >"$WORK/ak.pub"
...
for i in $(seq 0 $((n_components - 1))); do
    ...
    echo "$comp" | jq -r '.["attest-info"]' | unhex >"$WORK/attest-$i.bin"
    ...
    case "$type" in
        ...
        nvcpr)
            ...
            # populated.
            [ -n "$(echo "$comp" | jq -r '.["nvpcr-name"] // empty')" ]
            [ -n "$(echo "$comp" | jq -r '.["nv-public"] // empty')" ]
            [ -n "$(echo "$comp" | jq -r '.["authenticated-data"] // empty')" ]
            ;;
        session-audit)
            saw_audit=1
            audit_comp="$comp"
...
# fixed offset.
attest_hex="$(echo "$audit_comp" | jq -r '.["attest-info"]')"
