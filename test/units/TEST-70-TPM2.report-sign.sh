...
test_multiple_keys() {
    if ! tpm2_supports_params ecc_nist_p256 ecdsa-sha256 || ! tpm2_supports_params ecc_nist_p384 ecdsa-sha384; then
        echo "TPM does not support the multi-key test parameters, skipping."
        return 0
    fi

    local digest sig pub voucher pub_p256="" pub_p384=""
    ...
    # signature per key, signed by the same two keys.
    digest="$(generate_signed)"
    mapfile -t sig_files < <(find "$WORK" -maxdepth 1 -name 'report.sig.*' | sort)
    [ "${#sig_files[@]}" -eq 2 ]
