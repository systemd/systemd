...
dd if=/dev/urandom of=/tmp/external-huge bs=1M count=64

# create w/ prepopulated blob dir
NEWPASSWORD=EMJuc3zQaMibJo homectl create test-user \
...
# error out if file is too big
(! PASSWORD=EMJuc3zQaMibJo homectl update blob-user -b huge=/tmp/external-huge )
