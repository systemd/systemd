...
if ! checkNUMA; then
    ...
    # The capture is closed on both paths, so the at_exit dump shows this subtest's window
    # rather than a stale one, and exit "$rc" preserves the wait's own status: at the bound
    # that is timeout's 124, which a bare false would rewrite into a 1.
    rc=0
    waitUnitMainExited "$testUnit" || rc=$?
    stopJournalctl "$testUnit"
    [[ $rc -eq 0 ]] || exit "$rc"
    ...
    # the wait times out with 124, which exit "$rc" preserves. The grep then checks the captured
    # window for the executor's failure line, which has no ", ignoring" suffix unlike PID1's.
    rc=0
    waitUnitMainHandoff "$testUnit" || rc=$?
    stopJournalctl "$testUnit"
    [[ $rc -eq 0 ]] || exit "$rc"
    systemctlCheckNUMAProperties "$testUnit" "preferred"
    grep "Failed to set NUMA memory policy" "$journalLog" >/dev/null && { echo >&2 "unexpected pass"; exit 1; }
    pid1StopUnit "$testUnit"
