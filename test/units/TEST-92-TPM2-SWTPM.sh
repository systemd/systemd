...
assert_swtpm_up() {
    local tpm_device tpmrm_devices

    systemctl is-active systemd-tpm2-swtpm.service
    timeout 30 bash -c 'until compgen -G "/sys/class/tpmrm/tpmrm*" >/dev/null; do sleep 1; done'
    ...
    TPM2_DEVICE="/dev/${tpmrm_devices[0]##*/}"
    tpm_device="/dev/tpm${TPM2_DEVICE#/dev/tpmrm}"

    timeout 30 bash -c "until [[ -c '$TPM2_DEVICE' && -c '$tpm_device' ]]; do sleep 1; done"
