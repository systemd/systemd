...
testcase_unpriv() {
    ...
    # Now verify the behavior with masking. PrivatePIDs=yes should be ignored and we should not be in a new PID namespace.
    if [[ "$HAS_EXISTING_SCSI_MOUNT" == "no" ]]; then
        mount -t tmpfs tmpfs /proc/scsi
    fi

    runas testuser systemd-run --wait --user --pipe --expand-environment=no -p PrivatePIDs=yes bash -c 'test $$ != 1'
    assert_neq "$(runas testuser systemd-run --wait --user --pipe -p PrivatePIDs=yes readlink /proc/self)" "1"
