...
testcase_basic() {
    ...
    # Alpine/postmarketOS build coreutils with --enable-single-binary=symlinks. In that case, coreutils calls
    # prctl(PR_SET_NAME, argv[0]), hence the comm will be the path to the symlink. If the sleep file is a
    # dedicated binary (like most other distributions do), the comm will be the filename, i.e. "sleep".
