...
testcase_basic() {
    # systemd-executor hangs at __lsan::DoLeakCheck on exit with LSAN enabled and tests
    # time out. Temporarily disable these tests under ASAN until we can figure this out
    # or run this as a separate test with LSAN_OPTIONS=detect_leaks=0.
    if [[ -n "${ASAN_OPTIONS:-}" ]]; then
        echo "Sanitizers enabled, skipping basic PrivatePIDs=yes test"
        return 0
    fi
...
testcase_multiple_features() {
    if [[ -n "${ASAN_OPTIONS:-}" ]]; then
        echo "Sanitizers enabled, skipping multiple features PrivatePIDs=yes test"
        return 0
    fi
...
testcase_unpriv() {
    if [[ -n "${ASAN_OPTIONS:-}" ]]; then
        echo "Sanitizers enabled, skipping unprivileged PrivatePIDs=yes test"
        return 0
    fi
