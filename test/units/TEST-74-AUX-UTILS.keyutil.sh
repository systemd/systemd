...
generate_pkcs1() {
    openssl dgst -$1 -sign /tmp/test.key -out /tmp/payload.p1s /tmp/payload
}
...
verify_pkcs7() {
    ...
    eval $not openssl smime -verify -binary -inform der -in /tmp/payload.p7s -content /tmp/payload -noverify > /dev/null
    # Verify using external (original) certificate
    eval $not openssl smime -verify -binary -inform der -in /tmp/payload.p7s -content /tmp/payload -noverify -certfile /tmp/test.crt -nointern > /dev/null
}
    ...
    for hashalg in sha256 sha384 sha512; do
        ...
        for content_param in "" "--content /tmp/payload"; do
            ...
            for hashalg_param in "" "--hash-alg $hashalg"; do
                ...
                if [ -z "$hashalg_param" -a "$hashalg" != "sha256" ]; then
                    verify_pkcs7_fail
                else
                    verify_pkcs7
                fi
