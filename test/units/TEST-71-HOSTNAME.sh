...
testcase_tags() {
    ...
    assert_eq "$(varlinkctl call /run/systemd/io.systemd.Hostname io.systemd.Hostname.Describe '{}' | jq --compact-output '.MachineInformationData[] | select(startswith("TAGS="))')" '"TAGS=good:-invalid"'
