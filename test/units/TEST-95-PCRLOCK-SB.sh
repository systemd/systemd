...
generate_bounded_update_predictions() {
    local authenticated_update="${1:?}" candidate expected_authority="${2:?}" expected_outcomes="${3:?}" selected_authority=
    local -a authority_predictions policy_predictions
    ...
    for candidate in "${authority_predictions[@]}"; do
        if jq --exit-status \
              ...
              --argjson insertion "$current_db_authorities" \
              --argjson records "$((current_authorities + 1))" \
              '(.records | length) == $records and
                  any(.records[$insertion].digests[]?; .hashAlg == "sha256" and .digest == $digest)' \
              "$candidate" >/dev/null; then
            [[ -z "$selected_authority" ]]
            selected_authority="$candidate"
        fi
    done
    [[ -n "$selected_authority" ]]
    cp "$selected_authority" "$STATE/bounded-authority/"
...
case "${REBOOT_COUNT:-0}" in
    0)
        ...
        expected_authority_predictions="$((current_db_authorities * 2 + 1))"
        ...
        expected_predicted_pcr7_outcomes="$((baseline_pcr7_outcomes * 4))"
...
PY
        ...
        generate_bounded_update_predictions \
            "$authenticated_update" "$expected_authority" "$expected_predicted_pcr7_outcomes"
...
PY
        ...
        /usr/lib/systemd/systemd-pcrlock make-policy --pcr=7
        assert_policy_pins_pcr7
        assert_pcr7_outcome_count "$expected_predicted_pcr7_outcomes"
    ...
    1)
        ...
        cp -a "$authority_backup" "$ambiguous_backup"
        if notify_secure_boot_update "{\"variable\":\"dbx\",\"data\":\"$dbx_update\"}"; then
            rm -rf "$ambiguous_backup"
            echo "Interrupted update recovery unexpectedly accepted ambiguous authority backups" >&2
            exit 1
        fi
        rm -rf "$ambiguous_backup"
        diff -r "$STATE/components-before-rollback/policy" "$POLICY_DIR"
        ...
        if notify_secure_boot_update \
                "{\"variable\":\"dbx\",\"data\":\"$dbx_update\",\"regenerate\":false}"; then
            mv "$authority_backup_no_regenerate" "$authority_backup"
            echo "Interrupted update recovery unexpectedly accepted inconsistent recovery modes" >&2
            exit 1
        fi
        diff -r "$STATE/components-before-rollback/policy" "$POLICY_DIR"
        diff -r "$STATE/components-before-rollback/authority" "$AUTHORITY_DIR"
        ...
        test -d "$policy_backup_no_regenerate" "$authority_backup_no_regenerate"
        policy_checksum="$(sha256sum /var/lib/systemd/pcrlock.json)"
        notify_secure_boot_update \
