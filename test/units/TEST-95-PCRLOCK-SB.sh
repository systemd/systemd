...
case "${REBOOT_COUNT:-0}" in
    0)
        ...
        # Collapse separator alternatives so authority prediction always fits the PCR outcome budget.
        install_separator_error_overrides
...
PY
        ...
        python3 - "$STATE/db.auth" "$STATE/db.esl" \
            "$STATE/db-malformed-list.auth" "$STATE/db-short-sha256.auth" <<'PY'
import struct
import sys
