...
PY
        ...
        if varlinkctl call /run/systemd/io.systemd.PCRLock \
                       io.systemd.PCRLock.PrepareSecureBootUpdate \
                       '{"variable":"db","data":"%%%"}'; then
            echo "Invalid Base64 update unexpectedly accepted" >&2
            exit 1
        fi
        ...
        if varlinkctl call /run/systemd/io.systemd.PCRLock \
                       io.systemd.PCRLock.PrepareSecureBootUpdate \
                       "{\"variable\":\"db\",\"data\":\"$truncated_update\"}"; then
            echo "Truncated authenticated update unexpectedly accepted" >&2
            exit 1
        fi
