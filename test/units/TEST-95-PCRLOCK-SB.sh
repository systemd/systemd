...
PY
        ...
        remove_separator_error_overrides
        fallback_outcome="$(varlinkctl --json=short call /run/systemd/io.systemd.PCRLock \
                                      io.systemd.PCRLock.PrepareSecureBootUpdate \
                                      "{\"variable\":\"db\",\"data\":\"$authenticated_update\"}")"
        grep '"policy":"predicted"' <<<"$fallback_outcome" >/dev/null
        grep '"authority":"fallback"' <<<"$fallback_outcome" >/dev/null
        test -f "$POLICY_DIR/generated.pcrlock"
        compgen -G "$POLICY_DIR/generated-prediction-*.pcrlock" >/dev/null
        test ! -e "$AUTHORITY_DIR/generated.pcrlock"
        ...
        # Both active components must have predictions for the next boot.
        test -f "$POLICY_DIR/generated.pcrlock"
        test -f "$AUTHORITY_DIR/generated.pcrlock"
