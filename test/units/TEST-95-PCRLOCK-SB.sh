...
assert_update_not_applied() {
    ...
    output="$(varlinkctl --json=short --graceful=io.systemd.PCRLock.UpdateNotApplied \
                         call /run/systemd/io.systemd.PCRLock \
                         io.systemd.PCRLock.NotifySecureBootUpdate \
                         "$parameters" 2>&1)"
    grep 'returned expected error: io.systemd.PCRLock.UpdateNotApplied' <<<"$output" >/dev/null
}

normalize_pcrlock() {
    jq --sort-keys --compact-output \
       '{records: [.records[] | {pcr: .pcr, digests: [.digests[] | {hashAlg, digest}] | sort_by(.hashAlg)}]}' \
       "${1:?}"
}
...
assert_single_pcr7_outcome() {
    local prediction

    prediction="$(/usr/lib/systemd/systemd-pcrlock predict --pcr=7 --json=short)"
    jq --exit-status '.sha256 | length == 1 and .[0].pcr == 7 and (.[0].values | length == 1)' <<<"$prediction"
}
...
assert_prediction_matches() {
    ...
    jq --exit-status 'any(.records[].digests[]?; true)' "$actual"
...
case "${REBOOT_COUNT:-0}" in
    0)
        ...
        # Keep policy and authority predictions below the TPM's eight-outcome limit. This VM measures only
        # successful separators, so override the synthetic error alternatives for this test.
        install_separator_error_overrides
...
PY
        ...
        if [[ -f "$AUTHORITY_DIR/generated.pcrlock" ]]; then
            ...
            if compgen -G "$AUTHORITY_DIR/generated-prediction-*.pcrlock" >/dev/null; then
                echo "Authority prediction survived a planning fallback" >&2
                exit 1
            fi
...
PY
        ...
        # The policy component must have a prediction for the next boot. Firmware with a direct-hash
        # authority or too many current authority records cannot safely predict insertion of a new
        # certificate and reports fallback.
        test -f "$POLICY_DIR/generated.pcrlock"
