...
install_separator_error_overrides() {
    ...
    touch "$SECUREBOOT_SEPARATOR_DIR/600-0xffffffff.pcrlock"
    touch "$SEPARATOR_DIR/600-0xffffffff.pcrlock"
}
...
apply_efi_update() {
    local auth="${2:?}" attributes immutable=no name="${3:?}" variable="${1:?}"
    ...
    if ! efi-updatevar -a -f "$auth" "$name"; then
