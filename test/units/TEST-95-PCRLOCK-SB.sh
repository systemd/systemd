...
generate_bounded_update_predictions() {
    ...
    for candidate in "${authority_predictions[@]}"; do
        if jq --exit-status \
              ...
              --argjson insertion "$authority_insertion_position" \
        ...
        authority_layout="$(
            /usr/lib/systemd/systemd-pcrlock log --pcr=7 --json=short | \
                jq --slurpfile authority "$AUTHORITY_DIR/generated.pcrlock" '
                    ...
                    (($from_db | index(false)) // ($from_db | length)) as $insertion |
                    {
                        insertion: $insertion,
                        predictions: ($insertion + 1 + ($from_db | map(select(.)) | length))
                    }
                '
        )"
        authority_insertion_position="$(jq --raw-output '.insertion' <<<"$authority_layout")"
