...
case "${REBOOT_COUNT:-0}" in
    0)
        ...
        preserve_current_authority=yes
        expected_reboot_pcr7_outcomes="$expected_predicted_pcr7_outcomes"
        if ((expected_reboot_pcr7_outcomes > 8)); then
            preserve_current_authority=no
            expected_reboot_pcr7_outcomes="$((baseline_pcr7_outcomes * 2 * expected_insertion_predictions))"
        fi
...
PY
        ...
        generate_bounded_update_predictions \
            "$authenticated_update" "$expected_predicted_pcr7_outcomes"
        if [[ "$preserve_current_authority" == no ]]; then
            rm "$AUTHORITY_DIR/generated.pcrlock"
        fi
        /usr/lib/systemd/systemd-pcrlock make-policy --pcr=7
        assert_policy_pins_pcr7
        ...
        if [[ "$preserve_current_authority" == yes ]]; then
            test -f "$AUTHORITY_DIR/generated.pcrlock"
        else
            test ! -e "$AUTHORITY_DIR/generated.pcrlock"
        fi
        mapfile -t authority_predictions < <(compgen -G "$AUTHORITY_DIR/generated-prediction-*.pcrlock")
        ((${#authority_predictions[@]} == expected_insertion_predictions))
        cp "${authority_predictions[@]}" "$STATE/predicted-authority/"

        # If the current path fits alongside every future authority ordering, it must remain unlockable.
        if [[ "$preserve_current_authority" == yes ]]; then
            unlock_volume
        fi
