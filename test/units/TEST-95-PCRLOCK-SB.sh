...
generate_bounded_update_predictions() {
    local authenticated_update="${1:?}" expected_outcomes="${2:?}"
    ...
    cp "${authority_predictions[@]}" "$STATE/bounded-authority/"
...
case "${REBOOT_COUNT:-0}" in
    0)
        ...
        expected_predicted_pcr7_outcomes="$((baseline_pcr7_outcomes * 2 * (expected_authority_predictions + 1)))"
        ((expected_predicted_pcr7_outcomes <= 8))
