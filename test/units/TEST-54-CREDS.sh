...
# Null-key credentials and the systemd.credentials_boot= policy.
#
...
# accepted at decrypt time is governed by systemd.credentials_boot= and the system
...
SYSTEMD_PROC_CMDLINE="systemd.credentials_boot=strict" \
...
(! SYSTEMD_PROC_CMDLINE="systemd.credentials_boot=off" \
...
SYSTEMD_PROC_CMDLINE="systemd.credentials_boot=bogus" SYSTEMD_FIRST_BOOT=1 \
    systemd-creds --name=nullcred decrypt /tmp/cred.enc /tmp/cred.dec
diff /tmp/cred.orig /tmp/cred.dec
rm -f /tmp/cred.dec

# tofu accepts during first boot, independent of TPM2/SecureBoot.
SYSTEMD_PROC_CMDLINE="systemd.credentials_boot=tofu" SYSTEMD_FIRST_BOOT=1 \
    systemd-creds --name=nullcred decrypt /tmp/cred.enc /tmp/cred.dec
diff /tmp/cred.orig /tmp/cred.dec
rm -f /tmp/cred.{enc,dec}
