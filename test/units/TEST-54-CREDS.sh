...
# Null-key credentials and the systemd.credentials-boot= policy.
#
...
# accepted at decrypt time is governed by systemd.credentials-boot= and the system
...
SYSTEMD_PROC_CMDLINE="systemd.credentials-boot=strict" \
...
(! SYSTEMD_PROC_CMDLINE="systemd.credentials-boot=off" \
...
# An invalid policy value falls back to the default (tofu), which accepts during first boot.
SYSTEMD_PROC_CMDLINE="systemd.credentials-boot=bogus" SYSTEMD_FIRST_BOOT=1 \
    systemd-creds --name=nullcred decrypt /tmp/cred.enc /tmp/cred.dec
diff /tmp/cred.orig /tmp/cred.dec
rm -f /tmp/cred.dec

# tofu accepts during first boot, independent of TPM2/SecureBoot.
SYSTEMD_PROC_CMDLINE="systemd.credentials-boot=tofu" SYSTEMD_FIRST_BOOT=1 \
    systemd-creds --name=nullcred decrypt /tmp/cred.enc /tmp/cred.dec
diff /tmp/cred.orig /tmp/cred.dec
rm -f /tmp/cred.{enc,dec}
