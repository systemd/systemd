...
PID="$(systemd-notify --fork -- run0 -u testuser --pipe --service-type=notify systemd-run --user --wait --unit=brummbaer.service -p LoadCredential=brummbaer sleep infinity)"
[[ -d "$TESTUSER_CRED_DIR/brummbaer.service" ]]
[[ -f "$TESTUSER_CRED_DIR/brummbaer.service/brummbaer" ]]

run0 -u testuser --pipe systemd-run --user --wait -p PrivateMounts=yes -p ImportCredential=brummbaer \
    test ! -d "$TESTUSER_CRED_DIR/brummbaer.service"
run0 -u testuser --pipe systemd-run --user --wait -p ImportCredential=brummbaer \
    test -d "$TESTUSER_CRED_DIR/brummbaer.service"
