...
# single unit filter
varlinkctl call --more "$VARLINK_SOCKET" io.systemd.Journalctl.GetEntries "{\"units\": [\"$UNIT_NAME_1\"]}" | grep -q "hello-from-varlink-test-1"
(! varlinkctl call --more "$VARLINK_SOCKET" io.systemd.Journalctl.GetEntries "{\"units\": [\"$UNIT_NAME_1\"]}" | grep "hello-from-varlink-test-2")
# multi unit filter
varlinkctl call --more "$VARLINK_SOCKET" io.systemd.Journalctl.GetEntries "{\"units\": [\"$UNIT_NAME_1\", \"$UNIT_NAME_2\"]}" | grep -q "hello-from-varlink-test-1"
varlinkctl call --more "$VARLINK_SOCKET" io.systemd.Journalctl.GetEntries "{\"units\": [\"$UNIT_NAME_1\", \"$UNIT_NAME_2\"]}" | grep -q "hello-from-varlink-test-2"
