...
mkdir -p "$OUT_DIR"/{main,initrd,standalone}

run_for_cmdline() {
    local cmdline="${1:?}"
    local out_dir="${2:?}"
    local in_initrd=$([ "${3:?}" != "initrd" ]; echo $?)
...
compare_initrd_main() {
    ...
    run_for_cmdline "$cmdline" "$out_dir"/main
    run_for_cmdline "$cmdline" "$out_dir"/initrd initrd
...
run_for_cmdline "ro root=gpt-auto" "$OUT_DIR"/standalone
grep -qE "^ExecStart=(.+,)?read-only(,|$)" "$OUT_DIR/standalone/late/systemd-cryptsetup@root.service"
