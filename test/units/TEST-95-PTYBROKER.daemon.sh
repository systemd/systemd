...
# The broker is a socket-activated singleton: it is running right after we
# talked to it, but is expected to exit on its own once it has been idle for a
# bit, while the socket stays around. (Talk to it right before checking, so that
# the idle timer cannot expire in between on a slow machine.)
ptyctl list >/dev/null
