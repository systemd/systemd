...
def zstd_rle_frame(entries):
    """Return a zstd frame containing entries with 64 MiB messages compressed using run-length encoding."""
    ...
    return header + (raw(entry('')[:-2]) + rle * 512 + raw(b'\n\n')) * entries + b'\x01\x00\x00'
