...
CORE_TEST_BIN="/usr/bin/test-dump"
CORE_TEST_UNPRIV_BIN="/usr/bin/test-usr-dump"
# Unset $PAGER so we don't have to use --no-pager everywhere
export PAGER=

at_exit() {
    rm -fv -- "$CORE_TEST_BIN" "$CORE_TEST_UNPRIV_BIN"
}
...
ln -vf /bin/bash "${CORE_TEST_BIN:?}"
ln -vf /bin/bash "${CORE_TEST_UNPRIV_BIN:?}"
...
"$CORE_TEST_BIN" -c 'kill -TRAP $$' && exit 1
"$CORE_TEST_BIN" -c 'kill -ABRT $$' && exit 1
# In the tests we store the coredumps in journals, so let's generate a couple
# with Storage=external as well
mkdir -p /etc/systemd/coredump.conf.d/
printf '[Coredump]\nStorage=external' >/etc/systemd/coredump.conf.d/99-external.conf
"$CORE_TEST_BIN" -c 'kill -TRAP $$' && exit 1
"$CORE_TEST_BIN" -c 'kill -ABRT $$' && exit 1
rm -fv /etc/systemd/coredump.conf.d/99-external.conf
...
SETPRIV_CMD=(setpriv --reuid=testuser --regid=testuser --init-groups --)
# Trigger a couple of coredumps as an unprivileged user
systemd-run --user -M "testuser@.host" --wait -- "$CORE_TEST_UNPRIV_BIN" -c 'kill -TRAP $$$$' && exit 1
systemd-run --user -M "testuser@.host" --wait -- "$CORE_TEST_UNPRIV_BIN" -c 'kill -ABRT $$$$' && exit 1
# In the tests we store the coredumps in journals, so let's generate a couple
# with Storage=external as well
mkdir -p /etc/systemd/coredump.conf.d/
printf '[Coredump]\nStorage=external' >/etc/systemd/coredump.conf.d/99-external.conf
...
"${SETPRIV_CMD[@]}" coredumpctl
"${SETPRIV_CMD[@]}" coredumpctl info "$CORE_TEST_UNPRIV_BIN"
"${SETPRIV_CMD[@]}" coredumpctl info "${CORE_TEST_UNPRIV_BIN##*/}"
(! "${SETPRIV_CMD[@]}" coredumpctl info --all "$CORE_TEST_BIN")
(! "${SETPRIV_CMD[@]}" coredumpctl info --all "${CORE_TEST_BIN##*/}")

"${SETPRIV_CMD[@]}" coredumpctl debug --debugger=/bin/true "$CORE_TEST_UNPRIV_BIN"
"${SETPRIV_CMD[@]}" coredumpctl debug --debugger=/bin/true --debugger-arguments="-this --does --not 'do anything' -a -t --all" "${CORE_TEST_UNPRIV_BIN##*/}"

"${SETPRIV_CMD[@]}" coredumpctl dump "$CORE_TEST_UNPRIV_BIN" >/tmp/core.redirected
test -s /tmp/core.redirected
"${SETPRIV_CMD[@]}" coredumpctl dump -o /tmp/core.output "${CORE_TEST_UNPRIV_BIN##*/}"
test -s /tmp/core.output
rm -f /tmp/core.{output,redirected}
(! "${SETPRIV_CMD[@]}" coredumpctl dump "$CORE_TEST_BIN" >/dev/null)
