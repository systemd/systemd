...
"$CORE_TEST_BIN" -xc 'sleep 1; kill -TRAP $$' && exit 1
"$CORE_TEST_BIN" -xc 'sleep 1; kill -ABRT $$' && exit 1
...
systemd-run --user -M "testuser@.host" --wait -- "$CORE_TEST_UNPRIV_BIN" -xc 'ulimit -c unlimited; sleep 1; kill -TRAP $$$$' && exit 1
systemd-run --user -M "testuser@.host" --wait -- "$CORE_TEST_UNPRIV_BIN" -xc 'ulimit -c unlimited; sleep 1; kill -ABRT $$$$' && exit 1
# In the tests we store the coredumps in journals, so let's generate a couple
# with Storage=external as well
mkdir -p /etc/systemd/coredump.conf.d/
printf '[Coredump]\nStorage=external' >/etc/systemd/coredump.conf.d/99-external.conf
