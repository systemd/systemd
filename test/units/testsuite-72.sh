...
mkdir -p /var/tmp/72-esp /var/tmp/72-xbootldr
...
verify_version() {
    ...
    ls -A1q /var/tmp/72-esp | grep -q . # Assert that the ESP is empty (https://unix.stackexchange.com/a/202276)
...
rmdir /var/tmp/72-esp
rm -r /var/tmp/72-dirs /var/tmp/72-defs /var/tmp/72-source /var/tmp/72-xbootldr
