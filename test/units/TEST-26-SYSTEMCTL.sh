...
systemctl revert "$UNIT_NAME"
output=$(systemctl limits "$UNIT_NAME")
(! echo "$output" | grep "Memory Max" | grep -v inherited >/dev/null)
(! echo "$output" | grep "Memory High" | grep -v inherited >/dev/null)
(! echo "$output" | grep "CPU Quota" >/dev/null)
(! echo "$output" | grep "Tasks Max" | grep -v inherited >/dev/null)
(! echo "$output" | grep "IO Weight" >/dev/null)
# Error cases
