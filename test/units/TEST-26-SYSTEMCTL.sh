...
result=$(varlinkctl call "$MANAGER_SOCKET" io.systemd.Unit.StartTransient \
    '{"name":"varlink-transient-args.service","service":{"Type":"oneshot","ExecStart":[{"path":"/bin/echo","arguments":["/bin/echo","hello"]}]}}')
echo "$result" | grep -q '"jobId"'
timeout 30 bash -c 'until systemctl show -P ActiveState varlink-transient-args.service | grep -q inactive; do sleep 0.5; done'
systemctl show -P Result varlink-transient-args.service | grep -q success
...
result=$(varlinkctl call "$MANAGER_SOCKET" io.systemd.Unit.StartTransient \
    '{"name":"varlink-transient-noargs.service","service":{"Type":"oneshot","ExecStart":[{"path":"/bin/true"}]}}')
timeout 30 bash -c 'until systemctl show -P ActiveState varlink-transient-noargs.service | grep -q inactive; do sleep 0.5; done'
