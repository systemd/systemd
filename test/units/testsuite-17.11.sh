...
test_style_error 'KERNEL!="", KERNEL=="?*", NAME="a"' 'style: duplicate expressions.'
test_style_error 'KERNEL=="|a|b", KERNEL=="b|a|", NAME="c"' 'style: duplicate expressions.'
# shellcheck disable=SC2016
test_style_error 'ENV{DISKSEQ}=="?*", ENV{DEVTYPE}!="partition", ENV{DISKSEQ}=="?*", ENV{ID_IGNORE_DISKSEQ}!="1", SYMLINK+="disk/by-diskseq/$env{DISKSEQ}"' \
                 'style: duplicate expressions.'
...
test_style_error 'RESULT=="a", PROGRAM="b"' 'style: reordering RESULT check after PROGRAM assignment.'
test_style_error 'RESULT=="a*", PROGRAM="b", RESULT=="*c", PROGRAM="d"' \
                 'style: reordering RESULT check after PROGRAM assignment.'
...
EOF
cat >"${exp}" <<EOF
${rules}:1 style: duplicate expressions.
