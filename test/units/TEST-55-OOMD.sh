...
test_basic() {
    ...
    timeout 1m bash -xec "until oomctl | grep 'Path: $cgroup_path'; do sleep 1; done"
    oomctl
    oomctl | grep -A7 "Path: $cgroup_path" | grep "20.00%"
...
EOF
    ...
    # If systemd is running on sanitizers, stress-ng in testmunch.service relatively rapidly consumes memory,
    # and testbloat.service may not be started yet. In such case, gracefully skip the test.
    if systemd-cgls /TEST.slice/TEST-55.slice/TEST-55-OOMD.slice/TEST-55-OOMD-workload.slice/TEST-55-OOMD-testbloat.service | grep -q -F "stress-ng"; then
        ...
        echo "stress-ng in testbloat.service not started yet, skipping checking if testmunch.service is killed by oomd."
    fi
