...
EOF
    ...
    # no hooks
    systemctl reload systemd-oomd.service
    systemd-run -t --wait -p ManagedOOMMemoryPressure=kill -p MemoryMax=40M stress-ng -t 1M -m 1 --vm-bytes 60M --vm-keep || true

    # one hook
    mkdir -p /run/systemd/oom/prekill/althook
