...
EOF
    ...
    # one hook
    mkdir -p /run/systemd/oomd.hooks/
    ncat --recv-only -kUl /run/systemd/oomd.hooks/althook >/tmp/oomd_event.json &
    ! systemctl start --wait TEST-55-OOMD-testbloat.service || exit 1
    [[ $(jq -r .method </tmp/oomd_event.json) = 'io.systemd.oom.PreKill' ]]

    rm -f /run/systemd/oomd.hooks/* /tmp/oomd_event.json

    # many hooks
    for i in {1..4}; do
        ncat --recv-only -kUl "/run/systemd/oomd.hooks/althook$i" >"/tmp/oomd_event$i.json" &
    done
