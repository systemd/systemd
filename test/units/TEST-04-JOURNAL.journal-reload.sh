...
total_size=$(du -sb "/run/log/journal/$MACHINE_ID" | cut -f1)
echo "[DEBUG] Total size of runtime journal: $total_size"
...
if grep -Fxq "MESSAGE=$TEST_MSG_PREFIX $rand_val" "/run/log/journal/$MACHINE_ID/system.journal"; then
    echo "$rand_val present in runtime journal"
fi
if grep -Fxq "MESSAGE=$TEST_MSG_PREFIX $rand_val" "/var/log/journal/$MACHINE_ID/system.journal"; then
    echo "$rand_val present in system journal"
fi
