...
verify_journals() {
    ...
    assert_rc "$run_expected" journalctl -D "/run/log/journal/$MACHINE_ID/" --since "$since" --grep "$TEST_MSG_PREFIX $msg"
    assert_rc "$var_expected" journalctl -D "/var/log/journal/$MACHINE_ID/" --since "$since" --grep "$TEST_MSG_PREFIX $msg"
}

get_num_archived_journals() {
    find "/var/log/journal/$MACHINE_ID/" -type f -name "*.journal" ! -name "system.journal" | wc -l
}
...
verify_journals "$VAL" "$TS" runtime

: "Flush and confirm that messages are written to system."
journalctl --flush
TS="$(date '+%H:%M:%S')"
VAL=$(write_to_journal)
verify_journals "$VAL" "$TS" persistent
...
if (( total_size < max_size )); then
    echo "ERROR: journal size is smaller than 1M."
    exit 1
fi
...
: "Write 2M data to volatile journal"
set +x
dd if=/dev/urandom bs=1M count=5 | base64 | systemd-cat -t "$SYSLOG_ID"
set -x
journalctl --sync
...
if (( total_size < max_size )); then
    echo "ERROR: volatile journal size does not exceed 2M."
    exit 1
fi
...
num_var_journals=$(get_num_archived_journals)
if (( num_var_journals < limit_var_journals )); then
    echo "ERROR: Number of archived system journal files is ${num_var_journals} < ${limit_var_journals}."
    exit 1
fi
...
num_var_journals=$(get_num_archived_journals)
if (( num_var_journals >= limit_var_journals )); then
    echo "ERROR: Number of system journal files is ${num_var_journals} >= ${limit_var_journals}."
    exit 1
fi

: "Check the size of system journal"
total_size=$(du -sb "/var/log/journal/$MACHINE_ID" | cut -f1)
if (( total_size > max_size )); then
