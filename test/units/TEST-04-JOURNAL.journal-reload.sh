...
write_to_journal() {
    local rand_val=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 14)
...
verify_journals() {
    ...
    verify_journal "$msg" "$system_expected" "$test_name" "var"
    if [ $? != 0 ]; then
        failed=true
    fi
...
# Reload should push existing runtime entries into system journal.
verify_journals "$rand_val1" false true "Persistent->Persistent Reload: "
verify_journals "$rand_val2" false true "Persistent->Persistent Reload: "
...
# Ensure reload without changing conf does not change journal size.
systemctl reload systemd-journald.service
...
# Reload with RuntimeMaxUse=1M (only works when storage != VOLATILE?)
cat <<EOF >/run/systemd/journald.conf.d/reload.conf
[Journal]
Storage=volatile
RuntimeMaxUse=1M
...
# Invoke vacuum using existing conf, which is done in reload.
systemctl reload systemd-journald.service
...
# Confirm that previous message was saved to system journal.
verify_journals "$rand_val" false true "Volatile->Persistent Reload: "
grep "$rand_val" /var/log/journal/$MACHINE_ID/system.journal | wc -l
...
num_var_journals=$(ls /var/log/journal/$MACHINE_ID/ | wc -l);
