...
assert_timesyncd_log() {
    ...
    for _ in {0..29}; do
        journalctl --sync
        journalctl -q -u systemd-timesyncd.service --after-cursor="$(cat "$cursor")" --grep "$match" >/dev/null && return 0
        sleep .5
    done
...
teardown_timesyncd_resolve() {
    ...
    systemctl service-log-level systemd-timesyncd.service info
...
testcase_timesyncd_resolve() {
    ...
    systemctl restart systemd-timesyncd
    systemctl service-log-level systemd-timesyncd.service debug
    ...
    assert_timesyncd_log "$cursor" "Resolved address .* for foo.localhost"
