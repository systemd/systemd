...
usr_root_size="$(du --apparent-size -k "$MINIMAL_IMAGE.raw" | cut -f1)"
usr_verity_size="$(du --apparent-size -k "$MINIMAL_IMAGE.verity" | cut -f1)"
usr_signature_size=4
truncate -s $(((8192+usr_root_size*2+usr_verity_size*2+usr_signature_size*2)*512)) "$MINIMAL_IMAGE.usr.gpt"
if [[ "$usr_root_size" -ge 1024 ]]; then
    usr_root_size="$((usr_root_size/1024 + 1))MiB"
else
    usr_root_size="${usr_root_size}KiB"
fi
usr_verity_size="$((usr_verity_size * 2))KiB"
usr_signature_size="$((usr_signature_size * 2))KiB"
uuid="$(head -c 32 "$MINIMAL_IMAGE.roothash" | sed -r 's/(.{8})(.{4})(.{4})(.{4})(.+)/\1-\2-\3-\4-\5/')"
echo -e "label: gpt\nsize=$usr_root_size, type=$USR_GUID, uuid=$uuid" | sfdisk "$MINIMAL_IMAGE.usr.gpt"
uuid="$(tail -c 32 "$MINIMAL_IMAGE.roothash" | sed -r 's/(.{8})(.{4})(.{4})(.{4})(.+)/\1-\2-\3-\4-\5/')"
