...
networkctl cat @test2 | cmp - <(networkctl cat "$NETWORK_NAME")
...
EOF
    ...
    SYSTEMD_LOG_LEVEL=debug EDITOR='mv new' script -ec 'networkctl edit "$LINK_NAME"' /dev/null
    assert_in 'alias test_alias2' "$(ip link show test2)"
fi
