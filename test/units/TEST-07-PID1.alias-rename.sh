...
EOF
    ...
    test "$new_pid" -eq "$orig_pid"
    test "$(systemctl show -P MainPID rename.service)" -eq "$orig_pid"
    test "$(systemctl show -P ActiveState the-unit-formerly-known-as-rename.service)" = active
    test "$(systemctl show -P ActiveState rename.service)" = active
}
