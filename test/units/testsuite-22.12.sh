...
if stat --format "%w" /tmp/ageby | grep -qv '^[\?\-]$'; then
...
for a in ':1s' '2:1h' 'nope:42h' '"  :7m"' 'm:' ':' '"+r*w-x:2h"' 'b ar:64'; do
...
# Check that "ctime" is ignored for directories.
# Specifying only "C" is the same as default mode.
systemd-tmpfiles --clean - <<-EOF
e /tmp/ageby/ - - - C:2s -
EOF
