...
# a .network may refer to a .netdev given in the same invocation, in any order; a dangling reference is
# only a warning, as in the daemon
output=$("${VERIFY[@]}" "$VERIFY_DIR/10-member.network" 2>&1)
assert_in "could not be found" "$output"
