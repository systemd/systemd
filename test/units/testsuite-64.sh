...
EOF
    ...
    for i in {0..63}; do
        ...
        # Note: the leading '3' is added by scsi_id (see scsi_id(8))
        path="/dev/mapper/3$wwid"
        dmpath="$(readlink -f "$path")"

        lsblk "$path"
        lsblk "/dev/disk/by-id/wwn-0x$wwid"
        multipath -C "$dmpath"
    ...
    mount -t ext4 "${path}-part1" "$mpoint"
    expected=0
