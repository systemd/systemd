...
testcase_mdadm_basic() {
    ...
    for i in {0..9}; do
        ...
        udevadm lock /dev/disk/by-id/ata-foobar_deadbeefmdadm{0..1} \
    ...
    for i in {0..9}; do
        echo "Disassemble - reassemble loop, iteration #$i"
        mdadm -v --stop "$raid_dev"
        udevadm lock /dev/disk/by-id/ata-foobar_deadbeefmdadm{0..2} \
...
EOF
    ...
    for i in {0..9}; do
        echo "Disassemble - reassemble loop, iteration #$i"
        mdadm -v --stop "$raid_dev"
        udevadm lock /dev/disk/by-id/ata-foobar_deadbeefmdadm{0..3} \
