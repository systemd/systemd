...
check_device_unit() {(
    ...
    read -a links < <(udevadm info -q symlink "$syspath" 2>/dev/null)
    ...
    read -a links < <(udevadm info "$syspath" | sed -ne '/SYSTEMD_ALIAS=/ { s/^E: SYSTEMD_ALIAS=//; p }' 2>/dev/null)
    for link in "${links[@]}"; do
