...
testcase_mdadm_basic() {
    ...
    for i in {0..9}; do
        ...
        udevadm lock \
                --device /dev/disk/by-id/ata-foobar_deadbeefmdadm0 \
                --device /dev/disk/by-id/ata-foobar_deadbeefmdadm1 \
                mdadm --assemble "$raid_dev" --name "$raid_name" -v /dev/disk/by-id/ata-foobar_deadbeefmdadm{0..1}
    done
    udevadm wait --settle --timeout=30 "${expected_symlinks[@]}"
    helper_check_device_symlinks
    # Cleanup
    ...
    for i in {0..9}; do
        ...
        udevadm lock \
                --device /dev/disk/by-id/ata-foobar_deadbeefmdadm0 \
                --device /dev/disk/by-id/ata-foobar_deadbeefmdadm1 \
                --device /dev/disk/by-id/ata-foobar_deadbeefmdadm2 \
                mdadm --assemble "$raid_dev" --name "$raid_name" -v /dev/disk/by-id/ata-foobar_deadbeefmdadm{0..2}
    done
    udevadm wait --settle --timeout=30 "${expected_symlinks[@]}"
    helper_check_device_symlinks
    # Cleanup
...
EOF
    ...
    for i in {0..9}; do
        ...
        udevadm lock \
                ...
                --device /dev/disk/by-id/ata-foobar_deadbeefmdadm1 \
                --device /dev/disk/by-id/ata-foobar_deadbeefmdadm2 \
                --device /dev/disk/by-id/ata-foobar_deadbeefmdadm3 \
                mdadm --assemble "$raid_dev" --name "$raid_name" -v /dev/disk/by-id/ata-foobar_deadbeefmdadm{0..3}
    done
    udevadm wait --settle --timeout=30 "${expected_symlinks[@]}"
    helper_check_device_symlinks
    # Cleanup
