...
# Wait for a specific service (or services matching a glob) to become active
# Arguments:
#   $1 - service name (or a glob)
#   $2 - number of retries (default: 10)
helper_wait_for_service() {
    local service_glob="${1:?}"
    local ntries="${2:-10}"
    local service i
    ...
    while read -r service; do
        ...
        if ! systemctl is-active "$service"; then
            echo >&2 "Timeout when waiting for '$service' to start-up"
            return 1
        fi
    done < <(systemctl -q -l --plain --no-pager list-units "$service_glob" | awk '{print $1}')
}
...
testcase_iscsi_lvm() {
    ...
    # Wait for pvscan to finish
    helper_wait_for_service "lvm2-pvscan@*.service"
    udevadm settle
    test -e "/dev/$vgroup/mypart1"
    test -e "/dev/$vgroup/mypart2"
    test -e "/dev/disk/by-label/mylvpart1"
