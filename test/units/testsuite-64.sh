...
testcase_mdadm_basic() {
    ...
    expected_symlinks=(
        ...
        "/dev/disk/by-partlabel/mdpart1"
        "/dev/disk/by-partlabel/mdpart2"
        "/dev/disk/by-partlabel/mdpart3"
        "/dev/disk/by-partuuid/deadbeef-dead-dead-beef-111111111111"
        "/dev/disk/by-partuuid/deadbeef-dead-dead-beef-222222222222"
        "/dev/disk/by-partuuid/deadbeef-dead-dead-beef-333333333333"
    )
...
EOF
    udevadm wait --settle --timeout=30 "/dev/disk/by-partlabel/mdpart2"
    mkfs.ext4 -L "$part_name" "/dev/disk/by-partlabel/mdpart2"
    udevadm wait --settle --timeout=30 "${expected_symlinks[@]}"
    for i in {0..9}; do
        echo "Disassemble - reassemble loop, iteration #$i"
        # Disassemble the array
