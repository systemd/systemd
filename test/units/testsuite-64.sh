...
testcase_mdadm_basic() {
    ...
    echo y | mdadm --create "/dev/md/$raid_name" --name "$raid_name" --uuid "$uuid" /dev/disk/by-id/ata-foobar_deadbeefmdadm{0..1} -v -f --level=1 --raid-devices=2
    udevadm wait --settle --timeout=30 "/dev/md/$raid_name"
    mkfs.ext4 -L mdadm_mirror "/dev/md/$raid_name"
    udevadm wait --settle --timeout=30 "${expected_symlinks[@]}"
    # Disassemble the array
    mdadm -v --stop "/dev/md/$raid_name"
    udevadm settle
    helper_check_device_symlinks
    # Reassemble it and check if all requires symlinks exist
    mdadm --assemble "/dev/md/$raid_name" --name "$raid_name" -v
    udevadm wait --settle --timeout=30 "${expected_symlinks[@]}"
    helper_check_device_symlinks
}
