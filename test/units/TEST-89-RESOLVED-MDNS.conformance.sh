...
ref_scenario_responder_aggregation() {
    : "Answers to a two-question query share one packet (RFC 6762 section 6.4)"
    local q_log="$REF_TMPDIR/aggregate.query"

    ref_ensure_pub_service || return 2

    # Two questions the responder owns an answer for: the shared type PTR and
    # the instance's own SRV. Section 6.4 asks for as few packets as possible.
...
testcase_conformance_scorecard() {
    ...
    local scenarios=(
        ...
        responder_aggregation               # 6762 section 6.4
