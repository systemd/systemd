...
ref_expiry_scenario() {
    ...
    if [[ "$mode" == goodbye ]]; then
        ref_start_peer "$peer_log" "$REF_IF1_PEER" "$REF_IF1_PEER_ADDR" "$instance" --goodbye-on-exit "$@" || return 2
    else
        ref_start_peer "$peer_log" "$REF_IF1_PEER" "$REF_IF1_PEER_ADDR" "$instance" --ttl 20 "$@" || return 2
    fi
