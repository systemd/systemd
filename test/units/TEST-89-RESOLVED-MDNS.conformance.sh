...
ref_scenario_publisher_probe_announce() {
    ...
    local listen_log="$REF_TMPDIR/pubannounce.listen" file="/run/systemd/dnssd/refprobe.dnssd"
...
ref_scenario_conflict_rename() {
    ...
    # answers its probe (section 8.1). That is a conflict, not the section 8.2
    # tiebreak, which only decides between hosts probing at the same time:
    # resolved must pick another name rather than go silent.
...
testcase_conformance_scorecard() {
    ...
    local -A known_failures=(
        ...
        [duplicate_question_suppression]="TODO (RFC 6762 section 7.3 by analogy): one query engine per subscriber, see commit 8458b7fb91ea"
    )
