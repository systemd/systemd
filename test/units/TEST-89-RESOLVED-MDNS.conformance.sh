...
run_conformance_scorecard() {
    ...
    for scenario in "${scenarios[@]}"; do
        ...
        # read -ra rather than an unquoted expansion: the words are EREs, and several
        # legitimate ones (a trailing *, a bracket class) are also globs the shell would
        # otherwise replace with whatever filenames happen to match.
        read -ra skips <<< "${TEST_SKIP_SCENARIOS:-}"
