...
ref_start_listener() {
    ...
    # The log holds the traceback or errno that explains the death, and teardown deletes it
    # -- dump it now or the CI log shows nothing but the harness-error verdict.
    cat "$log_file" >&2
    return 2
}
...
ref_scenario_responder_qu_refresh() {
    ...
    ref_query "$qm_log" "$REF_PUB_SERVICE.local" --duration 3 || return 2
    grep -E "R qr=1 dst=224\.0\.0\.251 .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" \
        >/dev/null "$qm_log" || { cat "$qm_log" >&2; return 2; }
    sleep 31
    ...
    # this capture. Its absence means a lost datagram or a deaf socket, not a verdict.
    grep -E "R qr=1 dst=$REF_IF1_PEER_ADDR_RE .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" \
...
run_conformance_scorecard() {
    ...
    if [[ ${#selected[@]} -eq 0 ]]; then
        # A skip, like every sibling filter's empty run: the filters reach every guest, so
        # this may be aimed elsewhere -- and a typo must surface as a skip, not a red run.
        echo "The scenario filters selected nothing; skipping"
