...
ref_scenario_goodbye_shared_instance() {
    ...
    ip -n "$REF_NS" address add "$REF_IF1_PEER_ADDR_2/24" dev "$REF_IF1_PEER" || return 2
    # shellcheck disable=SC2064
    trap "trap - RETURN; ip -n $REF_NS address del $REF_IF1_PEER_ADDR_2/24 dev $REF_IF1_PEER 2>/dev/null || :" RETURN
