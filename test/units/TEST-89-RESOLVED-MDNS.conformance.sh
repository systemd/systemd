...
# A second machine on link 1, for the scenarios that need two publishers on one link;
# each adds it for its own duration and removes it on return.
REF_IF1_PEER_ADDR_2="${REF_IF1_PEER_ADDR%.*}.3"
...
# That bit is the plain-flags expiry scenario's entire distinguishing input: if the parent's
# constant ever stops carrying it, the derivation collapses into its sibling and the clamp
# regression pin goes vacuous. Fail the harness rather than let that happen silently.
[[ "$REF_BROWSE_FLAGS_PLAIN" -ne "$REF_BROWSE_FLAGS" ]]
...
# The liveness-marker question: sent into a capture and grepped back out, so the send and the
# grep must agree on the spelling -- one constant makes a mismatch (a guaranteed harness error)
# impossible.
REF_MARKER_NAME="refmark._udp.local"
...
# The objects come first and every field is then matched on its own: the key
# order in resolved's reply is an artifact of its serializer, not interface
# contract, so nothing here may depend on adjacency or sequence. Splitting on
# '{...}' assumes browserServiceData entries carry no nested objects, which is
# exactly why it lives in one helper rather than at each site.
ref_match_events() {
    local file="${1:?}"
    local matches
    shift
...
# Count the browse events for one instance, on one link when an ifindex is
# given. IPv6 is disabled on the reference links, hence no per-family
...
# Poll a log for a line, at the given interval for the given number of tries. What is waited
# for is a peer's own readiness or progress marker, which no scenario scores, so missing it is
# a harness error -- and the log, which teardown deletes, is dumped first: it holds the
# traceback or errno that explains the miss.
ref_wait_line() {
    local log_file="${1:?}" pattern="${2:?}" iterations="${3:?}" interval="${4:?}"
    local i

    for ((i = 0; i < iterations; i++)); do
        grep "$pattern" >/dev/null "$log_file" && return 0
        sleep "$interval"
    done
...
# A second address for the first peer link, for the scenarios that need two publishers on it.
# The RETURN trap that undoes it stays with the scenario: armed in here it would fire as soon
# as this function returned.
ref_add_second_peer_address() {
    ip -n "$REF_NS" address add "$REF_IF1_PEER_ADDR_2/24" dev "$REF_IF1_PEER"
}

ref_del_second_peer_address() {
    ip -n "$REF_NS" address del "$REF_IF1_PEER_ADDR_2/24" dev "$REF_IF1_PEER" 2>/dev/null || :
}

ref_remove_subtype_canary() {
    systemd-run -M "$CONTAINER_1" --wait --pipe -- \
        rm -f /etc/systemd/dnssd/subtype-canary.dnssd 2>/dev/null || :
    systemd-run -M "$CONTAINER_1" --wait --pipe -- \
...
ref_start_peer() {
    ...
    ref_wait_line "$log_file" ANNOUNCED 100 .2
}
...
# Query up to twice, until the capture holds a line matching the extended regex: the one
# retry policy of every scenario that scores something *about* an answer rather than whether
# one comes at all. One lost datagram is a retry, its absence after that a harness error --
# not their verdict. Further arguments go to the query.
ref_get_reply() {
    local log_file="${1:?}" qname="${2:?}" pattern="${3:?}"
    shift 3

    for _ in 1 2; do
        ref_query "$log_file" "$qname" "$@" || return 2
        grep -E "$pattern" >/dev/null "$log_file" && return 0
    done
...
# scenarios that go on to score something about that reply share this.
ref_get_legacy_reply() {
    ref_get_reply "${1:?}" "$REF_PUB_SERVICE.local" \
        "R qr=1 dst=$REF_IF1_PEER_ADDR_RE .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" \
        --source-port 0 --duration 5
}
...
ref_marker_canary() {
    ...
    ref_query "$marker_file" "$REF_MARKER_NAME" --duration 1 || return 2
    ref_wait_listener || return 2
    ref_capture_fully_decoded "$listen_log" || return 2
    grep "Q name=$REF_MARKER_NAME" >/dev/null "$listen_log" || { cat "$listen_log" >&2; return 2; }
}

# A verdict that assumes a publisher kept answering must check it survived: the process still
# runs, and its log carries no PUBLISHER-ERROR marker (the peer logs one before dying on a send
# failure). Scored as a harness error -- a dead publisher is an environment problem, not evidence
# about resolved.
ref_publisher_survived() {
    local pid="${1:?}" peer_log="${2:?}"

    # Three ways a publisher stops answering: it died, its send path errored
    # (PUBLISHER-ERROR), or it declined queries it could not parse (MALFORMED) --
    # the last leaves the process alive and quiet, which is exactly the silence
    # the absence verdicts would misread as evidence about resolved.
    if ! kill -0 "$pid" 2>/dev/null || grep -E "PUBLISHER-ERROR|MALFORMED" >/dev/null "$peer_log"; then
        cat "$peer_log" >&2
        return 2
    fi
}
...
# Did the response that carried the record also carry the header field (an extended regex)?
# A packet's P line precedes its record lines, so the header a record rode in is the last P
# line seen -- a header matched on some other response in the window says nothing about the
# one that answered.
ref_answer_header() {
    local log_file="${1:?}" header="${2:?}" record="${3:?}"

    awk -v header="$header" -v record="$record" '
        / P qr=1 / { p = $0 }
        / R qr=1 / && index($0, record) && p ~ header { found = 1; exit }
...
ref_start_listener() {
    ...
    ref_wait_line "$log_file" LISTENING 50 .2
}
...
ref_ensure_pub_service() {
    ...
    ref_write_dnssd_file "$REF_PUB_DNSSD_FILE" "$REF_PUB_INSTANCE" "$REF_PUB_SERVICE" 42089 ref=1 || return 2
...
ref_scenario_reverse_lookup_non_local() {
    ...
    # Bounded: with an unresponsive upstream DNS server the lookup could outlast the
    # listener's window, and the marker canary below would then read as a deaf listener.
    timeout 6 varlinkctl call /run/systemd/resolve/io.systemd.Resolve io.systemd.Resolve.ResolveAddress \
...
ref_expiry_scenario() {
    ...
    # The silent mode's 20s TTL runs from the announcements ref_start_peer just waited out;
    # its floor below anchors here, not at the kill, which can lag by several seconds.
    local announced_at="$EPOCHSECONDS"
    ...
    # In the silent mode the removal must also not come EARLY: section 5.2 holds the records
    # until the TTL runs out, walking the re-confirmation ladder first, so a removal well
    # inside the TTL is the opposite violation -- an expiry clamped early (the shape of the
    # bug the plain-flags variant pins) or a spurious flap, not conformance.
    if [[ "$mode" == silent && $((EPOCHSECONDS - announced_at)) -lt 15 ]]; then
        echo >&2 "Instance removed $((EPOCHSECONDS - announced_at))s after announcement, before its 20s TTL could expire"
        cat "$out" "$peer_log" >&2
        return 1
    fi
}
...
ref_scenario_query_backoff() {
    ...
    # A short sample -- fewer than three captured queries -- is environmental (two lost
    # multicast datagrams suffice), so the capture retries once like the other
    # single-datagram-sensitive helpers, with a fresh browse question each attempt.
    for _ in 1 2; do
        ...
        # through. The tolerances account for what CI actually produces: a lost or late datagram
        # MERGES two doubling intervals (observed gaps of 3.5s then 4.25s -- ratio 1.21 -- from a
        # perfect 1,2,4,8 schedule missing one query), so the growth floor sits well under 1.5; on
        # sub-2s gaps scheduling jitter swamps ratios entirely, so those only need to grow at all,
        # which still catches a fixed-rate repeat. The ceiling of 8 tolerates a single mid-window
        # loss (worst merge is 6x) while an interval ballooning toward the cap still trips it.
        rc=0
        awk -v q="Q name=$REF_SERVICE.local qtype=12" '
            index($0, q) {
                if (prev != "") {
                    gap = $1 - prev
                    if (last != "") {
                        if (last < 2) { if (gap <= last) bad = 1 }
                        else if (gap < last * 1.3 - 0.5 || gap > last * 8) bad = 1
                    }
                    last = gap
                    gaps++
                }
        ...
        case "$rc" in
            ...
            *) ref_cleanup_scenario_state ;;  # short sample: drop this browse, try once more
        esac
    done
...
# One attempt at the scenario below. Returns 3 when the capture missed its anchor, which says
# something about the runner and nothing about resolved, so the scenario tries once more. Each
# attempt gets files of its own: the browse output is appended to, and a second attempt must
# not find the first one's events in it.
ref_ttl_reconfirmation_attempt() {
    local attempt="${1:?}"
    local out="$REF_TMPDIR/reconfirm$attempt.browse" peer_log="$REF_TMPDIR/reconfirm$attempt.peer"
    local listen_log="$REF_TMPDIR/reconfirm$attempt.listen"
    ...
    ref_marker_canary "$REF_TMPDIR/reconfirm$attempt.marker" "$listen_log" || return 2
...
ref_scenario_ttl_reconfirmation() {
    ...
    ref_ttl_reconfirmation_attempt 1 || rc=$?
    if [[ "$rc" -eq 3 ]]; then
        rc=0
        ref_cleanup_scenario_state
        ref_ttl_reconfirmation_attempt 2 || rc=$?
        [[ "$rc" -ne 3 ]] || rc=2
    fi
...
ref_scenario_responder_unicast_qu() {
    ...
    for _ in 1 2; do
        ...
        # The QU step is inside the retry on purpose: its reply is a single datagram, and one
        # loss must not read as a regression (its siblings retry for the same reason).
    ...
    if [[ "$elicited" -eq 0 ]]; then
        cat "$qm_log" >&2
        return 2
    fi
    # Asked twice, never answered by unicast: that is the verdict.
    cat "$qm_log" "$qu_log" >&2
    return 1
}

ref_scenario_responder_qu_refresh() {
    : "A QU answer for a record not multicast recently should be multicast (RFC 6762 section 5.4)"
    local qm_log="$REF_TMPDIR/quref.qm" qu_log="$REF_TMPDIR/quref.qu"

    ref_ensure_pub_service || return 2

    # The other half of section 5.4, which the sibling scenario deliberately steps around:
    # when the record has NOT been multicast within a quarter of its TTL, the answer to a QU
    # question should be multicast anyway, to keep peer caches current. Pin the clock with
    # one multicast answer, then wait out a quarter of the 120s TTL; nothing else queries
    # this record in the meantime.
    ref_get_reply "$qm_log" "$REF_PUB_SERVICE.local" \
        "R qr=1 dst=224\.0\.0\.251 .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" --duration 3 || return 2
    sleep 31

    # Either reply settles it, so a lost one is retried: a repeat a few seconds on is still
    # well past the quarter TTL since that last multicast answer.
    ref_get_reply "$qu_log" "$REF_PUB_SERVICE.local" \
        "R qr=1 dst=[^ ]+ .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" --qu --duration 5 || return 2
    grep -E "R qr=1 dst=224\.0\.0\.251 .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" \
        >/dev/null "$qu_log" && return 0

    # The tracked deviation is "answered, but by unicast" -- so the unicast reply has to be in
    # this capture; anything else (a reply whose destination went unread) is not a verdict.
    grep -E "R qr=1 dst=$REF_IF1_PEER_ADDR_RE .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" \
        >/dev/null "$qu_log" || { cat "$qu_log" >&2; return 2; }
    cat "$qu_log" >&2
    return 1
}
...
# Score that minimum against section 6's window. The floor is the point: a shared record
# answered immediately is the deviation both callers track. The ceiling is loose on purpose --
# section 6.4 lets a responder hold an answer up to 500ms longer to aggregate, and the sample
# carries two process wakeups on a possibly loaded runner -- so it rejects an answer parked on
# a later timer, not the window's far edge.
ref_score_answer_delay() {
    local file_prefix="${1:?}"
    shift
    local min

    min="$(ref_min_answer_delay "$file_prefix" "$@")" || return 2
    if [[ "$min" -lt 20 || "$min" -gt 1000 ]]; then
        echo >&2 "answered after ${min}ms at the fastest"
        cat "$file_prefix".* >&2
        return 1
    fi
}
...
ref_scenario_shared_record_response_delay() {
    ...
    ref_score_answer_delay "$REF_TMPDIR/delay.query"
}
...
ref_scenario_responder_nsec() {
    ...
    ref_get_reply "$q_log.srv" "$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local" \
        "R qr=1 .* name=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local type=33" --qtype 33 --duration 5 || return 2
    ...
    for _ in 1 2; do
        ...
        grep -E "R qr=1 .* name=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local type=47" >/dev/null "$q_log" && return 0
    done
...
ref_scenario_multiquestion_response_delay() {
    ...
    ref_score_answer_delay "$REF_TMPDIR/mqdelay.query" \
        --witness-qname "$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local"
}
...
ref_scenario_responder_aggregation() {
    ...
    # packets as possible.
    # Both have to come back at all before their packing means anything. Pinned to sec=an
    # like the wildcard scenario below: the SRV rides along as an additional record of the
    # PTR answer as a matter of routine, and that is not the second question being answered.
    ref_get_reply "$q_log" "$REF_PUB_SERVICE.local" "R qr=1 .* sec=an name=$REF_PUB_SERVICE\.local type=12" \
    ...
    ref_same_packet "$q_log" "sec=an name=$REF_PUB_SERVICE.local type=12" \
        "sec=an name=$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local type=33" || { cat "$q_log" >&2; return 1; }
}
...
ref_scenario_responder_legacy_ttl_cap() {
    ...
    grep -E "R qr=1 dst=$REF_IF1_PEER_ADDR_RE sec=an name=$REF_PUB_SERVICE\.local type=12 ttl=([1-9]|10) flush=" \
...
ref_scenario_querier_known_answers() {
    ...
    # The line scored below is a record line, and a partial decode drops exactly those --
    # this scenario is a hard PASS expectation, so a parser shortfall must be rc=2, not
    # "regressed".
    ref_capture_fully_decoded "$listen_log" || return 2
    # flush=0 pinned: section 10.2 forbids the cache-flush bit on known answers, and a querier
    # leaking it would have every responder on the link drop its own copies.
    grep -E "R qr=0 dst=[^ ]+ sec=an .* flush=0 ptr=RefKA\.$REF_SERVICE\.local" >/dev/null "$listen_log" || { cat "$listen_log" >&2; return 1; }
}
...
ref_scenario_known_answer_continuation() {
    ...
    # TC set, a non-covering known answer inline, and the covering one in the continuation
    # packet 100ms later: only a responder that deferred (400-500ms), collected the follow-up
    # and merged it with the inline list stays quiet. That separates section 7.2 from 7.1 --
    # an implementation honouring only inline known answers still answers, since the inline
    # one does not cover the instance, and one honouring only continuations answers when the
    # follow-up is lost. Which of several known answers lands in the overflow packet is a
    # querier's arbitrary choice, so the shape stays realistic. The witness question travels
    # in the first packet and proves it arrived.
    ref_known_answer_scenario kacont --known-answer-ptr "Continuation.$REF_PUB_SERVICE.local" \
        --tc --followup-known-answer "$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local"
}
...
ref_scenario_conflict_rename() {
    ...
    ref_write_dnssd_file "$conflict_file" RefConflict _refconf._udp 100 || return 2
...
ref_scenario_publisher_probe_announce() {
    ...
    local announces instance probes
    ...
    for _ in 1 2; do
        ...
        ref_write_dnssd_file "$file" "$instance" "$REF_PUB_SERVICE" 8010 || return 2
...
ref_scenario_conflict_established() {
    ...
    ref_write_dnssd_file "$REF_EST_DNSSD_FILE" "$instance" "$service" 8012 || return 2
    systemctl reload systemd-resolved.service || return 2
    sleep 4
...
ref_scenario_goodbye_shared_instance() {
    ...
    ref_add_second_peer_address || return 2
    trap 'trap - RETURN; ref_del_second_peer_address' RETURN

    ref_start_browse "$out" "$REF_IF1_INDEX" || return 2
    ref_start_peer "$peer1_log" "$REF_IF1_PEER" "$REF_IF1_PEER_ADDR" RefSharedBye || return 2
    local peer1_pid="$REF_LAST_PEER_PID"
    ...
    # instance is shared.
    ref_start_peer "$peer2_log" "$REF_IF1_PEER" "$REF_IF1_PEER_ADDR_2" RefSharedBye \
    ...
    ref_wait_line "$peer2_log" "GOODBYE n=2" 10 1 || return 2
    ...
    sleep 8
    # The verdict assumes the first machine answered the rescue; if it died instead, the
    # missing removal (or a spurious one) says nothing about resolved.
    ref_publisher_survived "$peer1_pid" "$peer1_log" || return 2
...
ref_scenario_publisher_unregister_goodbye() {
    ...
    systemctl reload systemd-resolved.service || return 2
    # As in the other absence-adjacent verdicts: the ttl=0 line scored below is exactly what a
    # partial decode loses, and a deaf listener sees nothing at all -- both must be rc=2.
    ref_marker_canary "$REF_TMPDIR/pubunreg.marker" "$listen_log" || return 2
...
ref_scenario_publisher_stop_goodbye() {
    ...
    # The marker loops back from the peer, so it needs no resolved -- it can prove the
    # listener's decode and liveness even across the stop.
    ref_marker_canary "$REF_TMPDIR/pubstop.marker" "$listen_log" || rc=2
...
ref_scenario_cache_flush_update() {
    ...
    # ...and the old one is gone: the cache-flush bit replaces the cached record set rather
    # than adding to it, so a lookup still listing the stale port is the other half of the
    # failure this scenario exists for.
    if grep '"port":41001' >/dev/null "$out"; then
        cat "$out" "$peer_log.2" >&2
        return 1
    fi
}
...
ref_scenario_responder_authoritative_bit() {
    ...
    ref_ensure_pub_service || return 2
    ref_get_reply "$q_log" "$REF_PUB_SERVICE.local" \
        "R qr=1 .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" --duration 5 || return 2
    ...
    # elsewhere. Read off the packet that carried the answer: another response
    # in the window setting the bit says nothing about this one.
    ref_answer_header "$q_log" " aa=1 " "ptr=$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local" ||
        { cat "$q_log" >&2; return 1; }
}
...
ref_scenario_responder_rcode() {
    ...
    # transmission" in both queries and responses -- checked on the packet
    # that carried the answer, not on any response in the window.
    ref_get_reply "$q_log.zero" "$REF_PUB_SERVICE.local" \
        "R qr=1 .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" --duration 5 || return 2
    ref_answer_header "$q_log.zero" " rcode=0$" "ptr=$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local" ||
        { cat "$q_log.zero" >&2; return 1; }
    ...
    query_pid=$!
    # The marker has to reach a socket already in the group: wait for the querier's
    # readiness line rather than a fixed second a loaded runner can overrun.
    ref_wait_line "$q_log" LISTENING 50 .2 || { wait "$query_pid" || :; return 2; }
    ref_query "$q_log.marker" "$REF_MARKER_NAME" --duration 1 || { wait "$query_pid" || :; return 2; }
    wait "$query_pid" || return 2
    ref_capture_fully_decoded "$q_log" || return 2
    grep "Q name=$REF_MARKER_NAME" >/dev/null "$q_log" || { cat "$q_log" >&2; return 2; }
...
ref_scenario_responder_compressed_question() {
    ...
    ref_get_reply "$q_log" "$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local" \
        "R qr=1 .* sec=an name=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local type=33" --qtype 33 \
        --witness-qname "$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local" --witness-qtype 16 \
        --compress-witness --duration 5 || return 2

    # Pinned to sec=an: the TXT rides along with an SRV answer as an additional
    # record as a matter of routine, which would show nothing about the pointer.
    grep -E "R qr=1 .* sec=an name=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local type=16" >/dev/null "$q_log" ||
        { cat "$q_log" >&2; return 1; }
}
...
ref_scenario_browse_subtype() {
    ...
    local plain_log="$REF_TMPDIR/subtype.plain" q_log="$REF_TMPDIR/subtype.query"

    # Negative control: an instance of the base type published without the subtype, from a
    # second address on the same link (the shared-instance scenario's arrangement). A browser
    # that dropped the _vendor._sub prefix and asked for the base type would list it too, and
    # nothing else here could tell that from the subtype browse working.
    ref_add_second_peer_address || return 2
    trap 'trap - RETURN; ref_del_second_peer_address' RETURN
    ref_start_peer "$plain_log" "$REF_IF1_PEER" "$REF_IF1_PEER_ADDR_2" RefSubPlain --hostname refpeer-plain.local || return 2
    # Its absence below means something only while it answers for the base type.
    ref_query "$q_log" "$REF_SERVICE.local" --duration 3 || return 2
    grep -E "R qr=1 .* ptr=RefSubPlain\.$REF_SERVICE\.local" >/dev/null "$q_log" ||
        { cat "$q_log" "$plain_log" >&2; return 2; }
    ...
    [[ -z "$(ref_match_events "$out" "\"updateFlag\":\"added\"" "\"name\":\"RefSubPlain\"")" ]] ||
        { cat "$out" >&2; return 1; }
}

ref_scenario_publisher_subtype() {
    : "A service published with a subtype is found by a subtype browse (RFC 6763 section 7.1)"
    local out="$REF_TMPDIR/pubsub.browse" other="$REF_TMPDIR/pubsub.other"
...
EOF
    ...
    trap 'trap - RETURN; ref_remove_subtype_canary' RETURN
    ...
    # Negative control: a subtype the canary was not published under. A publisher answering
    # any _sub name with its base-type instances would list the canary here too, and the
    # browse above could not tell that from the _vendor PTR being served.
    ref_start_browse "$other" "$BRIDGE_INDEX" "" "_other._sub._testSubtype._udp.local" || return 2
    sleep 5
    [[ -z "$(ref_match_events "$other" "\"updateFlag\":\"added\"" "\"name\":\"Subtype Canary on $CONTAINER_1\"")" ]] ||
        { cat "$other" >&2; return 1; }
}

ref_scenario_browse_type_enumeration() {
    : "Browsing for service types reports each type found (RFC 6763 section 9)"
    local out="$REF_TMPDIR/enum.browse" peer_log="$REF_TMPDIR/enum.peer"
    local i

    # The peer first: it answers the enumeration question but never announces that record, so
    # only a query sent while it is up can bring the type back.
    ref_start_peer "$peer_log" "$REF_IF1_PEER" "$REF_IF1_PEER_ADDR" RefEnum || return 2
    ref_start_browse "$out" "$REF_IF1_INDEX" "" "_services._dns-sd._udp.local" || return 2

    # The answer's target is the type itself, with no instance in front of it.
    for ((i = 0; i < 30; i++)); do
        [[ -n "$(ref_match_events "$out" "\"updateFlag\":\"added\"" "\"type\":\"$REF_SERVICE\"")" ]] &&
            return 0
        sleep 1
    done

    # Nothing reported at all is the harness; something reported under another type is the
    # finding.
    cat "$out" "$peer_log" >&2
    [[ -n "$(ref_match_events "$out" "\"updateFlag\":\"added\"")" ]] || return 2
    return 1
}
...
ref_scenario_duplicate_question_suppression() {
    ...
    # Let the first browser's re-query schedule back off (the intervals double), so at most
    # one of its queries -- the ~15s slot -- can fall into the window below: it opens at ~12s
    # (clear of the ~7s slot even a few seconds late) and closes at ~25s (clear of ~31s).
    sleep 12

    ref_start_listener "$listen_log" 13 || return 2
    ref_start_browse "$out_b" "$REF_IF1_INDEX" || return 2

    # The canary proves the listener was not deaf for the window the count below scores on. It
    # waits the window out, so it goes first: the marker then always falls inside the window,
    # however long the second subscriber takes.
...
ref_scenario_cross_host_question_suppression() {
    ...
    ref_query "$query_log" "$REF_SERVICE.local" \
        ...
        --witness-qname "$REF_SERVICE.local" --compress-witness --witness-qtype 33 \
    ...
    # below meaningless, and it goes ahead of the control so that it always falls inside
    # the window.
...
ref_scenario_no_cache_lookup() {
    ...
    if ! ref_resolve_ptr "$out.nocache" "$service.local" "$REF_IF1_INDEX" "$REF_BROWSE_FLAGS_NO_CACHE" ||
       ! grep RefNoCache >/dev/null "$out.nocache"; then
        ...
        # The tracked failure is the one the publisher's suppression produces; an empty
        # answer the peer never suppressed anything for is a lost datagram, not the report.
        grep SUPPRESSED >/dev/null "$peer_log" || return 2
        return 1
    fi
}
...
ref_scenario_two_interfaces_same_name() {
    ...
    if ! ref_wait_event "$out" added RefTwoIf "$REF_IF2_INDEX" 10; then
        # Distinguish the tracked conformance failure from a peer that died or went mute.
        ref_publisher_survived "$REF_LAST_PEER_PID" "$peer2_log" || { cat "$out" >&2; return 2; }
        cat "$out" "$peer2_log" >&2
        return 1
    fi
}
...
ref_scenario_goodbye_one_interface() {
    ...
    # A cross-link flap need not be simultaneous with that removal: let the goodbye grace and
    # the re-query it triggers play out before counting, or one arriving a moment later passes.
    sleep 5
    # The premise of the absence below is that the link-1 publisher is still alive and
    # answering -- its 120s record cannot expire inside this window, so a dead peer would
    # pass vacuously and mask exactly the cross-link removal this scenario guards against.
    ref_publisher_survived "$peer1_pid" "$peer1_log" || return 2
...
run_conformance_scorecard() {
    ...
    local -A known_failures=(
        ...
        # mdns_should_reply_using_unicast() answers any all-QU query by unicast, with no test
        # of when the record was last multicast, so peer caches are never refreshed this way.
        [responder_qu_refresh]="TODO, SHOULD (RFC 6762 section 5.4): a QU answer for a record not multicast recently is unicast anyway"
        ...
        # The answer to an enumeration browse names a type, with no instance label. Finding no
        # instance, the event builder drops the type it just parsed and splits the question's
        # own name instead, so every type is reported as "_services" of type "_dns-sd._udp".
        [browse_type_enumeration]="TODO (RFC 6763 section 9): every enumerated type is reported as _dns-sd._udp"
    ...
    local scenarios=(
        ...
        browse_type_enumeration             # 6763 section 9
    ...
    local -A results=()
    # Scenario-level selection, in the family idiom: unanchored POSIX EREs, forwarded by the
    # integration-test wrapper like the subtest and testcase filters. Excluded names never
    # enter results[], and the bookkeeping tripwires below only audit unfiltered runs.
    local selected=() skips=() skip scenario
    local -i excluded=0
    # read -rd '' rather than an unquoted expansion: EREs are often also globs, and the
    # empty delimiter keeps newline a separator like the expansion it replaces (see the
    # sibling in test-control.sh).
    read -rd '' -a skips <<<"${TEST_SKIP_SCENARIOS:-}" || :
    for scenario in "${scenarios[@]}"; do
        if [[ -n "${TEST_MATCH_SCENARIO:-}" ]] && ! [[ "$scenario" =~ $TEST_MATCH_SCENARIO ]]; then
            echo "Skipping scenario $scenario (not matching '$TEST_MATCH_SCENARIO')"
            continue
        fi
        for skip in "${skips[@]}"; do
            if [[ "$scenario" =~ $skip ]]; then
                echo "Skipping scenario $scenario (matching '$skip')"
                excluded+=1
                continue 2
            fi
        done
    ...
    if [[ ${#selected[@]} -eq 0 ]]; then
        # A skip list that emptied the run is a deliberate exclusion, reported as a skip like
        # the sibling filters' empty runs. A positive selector that matched nothing is not:
        # this is the only test reading TEST_MATCH_SCENARIO, so the value cannot be aimed
        # elsewhere, and a typo reported as a skip would let the parent's testcases stamp
        # the run green with no scenario ever having run.
        if [[ $excluded -eq 0 ]]; then
            echo >&2 "TEST_MATCH_SCENARIO='${TEST_MATCH_SCENARIO:-}' matched no scenario"
            exit 1
        fi
        echo "TEST_SKIP_SCENARIOS='${TEST_SKIP_SCENARIOS:-}' filtered out every scenario; skipping"
        exit "$_SUBTEST_SKIP_RC"
    fi
    local i rc verdict=0
    ...
    REF_TMPDIR="$(mktemp -d)"
    # An EXIT trap rather than RETURN: the scorecard runs in an explicit
    # subshell (the invocation at the bottom of this file), and the subshell's
    # EXIT trap also fires when errexit aborts the setup below mid-flight -- a
    # RETURN trap does not, and would leak the tmpdir on an aborted setup (the
    # fixed-name namespace and links are covered by the self-healing removal
    # above either way).
    ...
    for i in 0 1; do
        local host_if="${ref_ifs[i]}" peer_if="${ref_peer_ifs[i]}"
        ...
        ip address add "${peer_addrs[i]%.*}.1/24" dev "$host_if"
        ip -n "$REF_NS" link set "$peer_if" up
        ip -n "$REF_NS" address add "${peer_addrs[i]}/24" dev "$peer_if"
        # resolved may not have processed the new link's rtnl event yet (the restart
        # restore helper retries for the same reason), so flipping the per-link switch
        # gets a bounded retry rather than one errexit-fatal shot.
        local try
        for ((try = 0; try < 20; try++)); do
            resolvectl mdns "$host_if" yes 2>/dev/null || { sleep .5; continue; }
            [[ "$(resolvectl mdns "$host_if")" =~ :\ yes$ ]] && break
            sleep .5
        done
    ...
    for scenario in "${selected[@]}"; do
        ...
        ref_cleanup_scenario_state
        results[$scenario]=$rc
        # One verdict line as each scenario lands: a later scenario hanging into the meson
        # timeout must not cost the evidence of everything that already ran.
        printf 'scorecard so far: %-36s %s\n' "$scenario" \
    ...
    # Scenarios whose verdict rides on scheduling luck: the response-delay pair scores a
    # wall-clock minimum of three samples, which on a loaded runner can all exceed the 20ms
    # floor with resolved unchanged -- and the shared-instance goodbye flap depends on whether
    # a browse re-query let the surviving publisher retake the cache entry before the goodbye
    # landed, so it occasionally fails to reproduce. An unexpected PASS on these is reported
    # but must not fail the run: following its advice would drop the entry and disable the
    # check for good on the next lucky run.
    local -A timing_scenarios=(
        [shared_record_response_delay]=1
        [multiquestion_response_delay]=1
        [goodbye_shared_instance]=1
    )

    # Bookkeeping tripwires. Audited against the static tables, not the run's results, so
    # they hold under scenario filters too -- drift is introduced exactly while iterating
    # with a filter set.
    # The known-failure table is a work list: an entry that no longer names a live scenario
    # is stale -- renamed or removed without updating the table -- and must not linger.
    local key
    ...
    # The timing carve-outs modify known-failure entries, so each must name one.
    for key in "${!timing_scenarios[@]}"; do
        if [[ ! -v "known_failures[$key]" ]]; then
            echo >&2 "timing_scenarios entry '$key' does not name a known failure"
            return 1
        fi
    done

    # And the reverse direction: a ref_scenario_* function that is not listed in
    # scenarios[] never runs and never shows on the scorecard -- written, reviewed, and
    # silently dead.
    local fn
