...
ref_scenario_reverse_lookup_non_local() {
    ...
    #
    # #20267 reports the opposite for both mDNS and LLMNR. This covers the
    # mDNS half of it through the Varlink API, where it holds; if the report
    # is reproduced through another path, that belongs in its own scenario.
    varlinkctl call /run/systemd/resolve/io.systemd.Resolve io.systemd.Resolve.ResolveAddress \
...
ref_scenario_conflict_rename() {
    ...
    # which resolved does, and choose new names, which it does not -- so this
    # is neither the section 8.2 tiebreak (that decides between hosts probing
    # at the same time) nor a section 9 conflict (that needs a record resolved
    # is already authoritative for; this one never left probing).
...
ref_scenario_no_cache_lookup() {
    ...
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefNoCache --service "$service" || return 2
    sleep 4
    ...
    # wire rather than come back empty.
    ref_resolve_ptr "$out.nocache" "$service.local" "$REF_IF1_INDEX" "$REF_BROWSE_FLAGS_NO_CACHE" || return 2
    grep RefNoCache >/dev/null "$out.nocache" || { cat "$out.nocache" >&2; return 1; }
}
...
testcase_conformance_scorecard() {
    ...
    local -A known_failures=(
        ...
        [conflict_rename]="TODO, SHOULD (RFC 6762 section 8.1): a lost probe withdraws the name instead of choosing a new one"
