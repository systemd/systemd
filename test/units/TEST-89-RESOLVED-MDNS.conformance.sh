...
ref_start_peer() {
    ...
    for ((i = 0; i < 100; i++)); do
        grep "ANNOUNCED" >/dev/null 2>&1 "$log_file" && return 0
        sleep .2
    done
...
ref_scenario_query_backoff() {
    : "Repeated queries back off, each interval about twice the last (RFC 6762 section 5.3)"
...
ref_scenario_shared_record_response_delay() {
    ...
    # supposed to be spread over 20-120ms rather than sent immediately. What
    # is measured here also contains the two process wakeups around it, so
    # take the smallest of several samples: one descheduled reader must not
    # read as a deliberate delay.
    for i in 1 2 3; do
        q_log="$REF_TMPDIR/delay.query.$i"
        ...
        # A lost datagram costs a sample, not the run.
        [[ -n "$delay" ]] || continue
    ...
    [[ -n "$min" ]] || { cat "$REF_TMPDIR"/delay.query.* >&2; return 2; }
...
ref_scenario_legacy_question_echo() {
    ...
    # Getting the reply at all is responder_legacy_unicast's business.
    grep -E "R qr=1 dst=$REF_IF1_PEER_ADDR_RE .* ptr=$REF_PUB_INSTANCE\.$REF_PUB_SERVICE\.local" \
        >/dev/null "$q_log" || { cat "$q_log" >&2; return 2; }
...
ref_scenario_conflict_established() {
    ...
    mkdir -p "$(dirname "$REF_EST_DNSSD_FILE")"
    cat >"$REF_EST_DNSSD_FILE" <<EOF
[Service]
Name=$instance
Type=$service
Port=8012
...
testcase_conformance_scorecard() {
    ...
    # Scenarios whose outcome is not stable enough to gate on. They still run
    # and are still reported, but neither outcome fails the test -- only a
    # harness error does. A flaky entry is a defect that has been pinned down
    # and is waiting on its fix, not a scenario nobody understands: when the
    # fix lands the entry goes away and the scenario is gated again.
    local -A flaky=(
        # Withdrawing a service is reported by deleting the record a second
        # after the goodbye, but mdns_goodbye_callback() decides whether to
        # keep looking by measuring the cache against the time the wakeup was
        # scheduled for rather than the time it ran. A publisher repeating its
        # goodbye a second later lands in that gap often enough to matter: the
        # record is refreshed past the horizon, nothing re-arms, and the
        # browser is not told until the maintenance ladder comes round some
        # hundred seconds later. Measured at roughly one run in twenty here,
        # and more on a loaded machine, since the window is the dispatch lag.
        [goodbye]="TODO, MUST (RFC 6762 section 10.1): a repeated goodbye can defer the removal to the maintenance ladder"
        [goodbye_single_announcement]="TODO, MUST (RFC 6762 section 10.1): same deferred removal as goodbye"
        [goodbye_then_reannounce]="TODO, MUST (RFC 6762 section 10.1): same deferred removal as goodbye"
    )
    ...
    local scenarios=(
        ...
        query_backoff                       # 6762 section 5.3
    ...
    for scenario in "${scenarios[@]}"; do
        ...
        printf '%-36s %s%s%s\n' "$scenario" \
            ...
            "${flaky[$scenario]:+ (not gated, flaky: ${flaky[$scenario]})}"
    done

    for scenario in "${scenarios[@]}"; do
        rc="${results[$scenario]}"
        if [[ -n "${flaky[$scenario]:-}" ]]; then
            # Only a broken harness is worth failing on here; both verdicts are
            # expected until the defect named in the table is fixed.
            if [[ "$rc" -gt 1 ]]; then
                echo >&2 "Scenario $scenario failed with a harness error"
                verdict=1
            fi
        elif [[ "$rc" -eq 0 && -n "${known_failures[$scenario]:-}" ]]; then
