...
ref_expiry_scenario() {
    ...
    if [[ "$mode" == goodbye ]]; then
        ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" "$instance" --goodbye-on-exit "$@" || return 2
    else
        ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" "$instance" --ttl 20 "$@" || return 2
    fi
    ...
    if [[ "$mode" == goodbye ]]; then
        ...
        ref_wait_event "$out" removed "$instance" "$REF_IF1_INDEX" 30 || { cat "$out" "$peer_log" >&2; return 1; }
    else
        # Vanish abruptly: no goodbye, and nobody answers re-confirmation queries. The records
        # carried a 20s TTL, so a conforming querier removes the service within ~20s of the last
        # answer; poll generously.
        kill -KILL "$REF_LAST_PEER_PID" || return 2
        ref_wait_event "$out" removed "$instance" "$REF_IF1_INDEX" 60 || { cat "$out" "$peer_log" >&2; return 1; }
    fi
}
...
ref_scenario_goodbye_then_reannounce() {
    ...
    ref_wait_event "$out" added RefRejoin "$REF_IF1_INDEX" 30 2 && return 0
...
# Not run_testcases: this subtest has exactly one testcase, and putting it into the parent's
# TEST_MATCH_TESTCASE / TEST_SKIP_TESTCASES namespace would let a filter aimed at the parent turn
# this file into a silent no-op (or empty the parent's own run). It stays selectable as a whole
# through TEST_MATCH_SUBTEST=conformance. The subshell keeps the scorecard's set -e and traps to
# itself, as run_testcases would have.
( testcase_conformance_scorecard )
