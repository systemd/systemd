...
FIXTURE_SERVICE_NAME_PREFIX="${FIXTURE_SERVICE_NAME_PREFIX:?}"
FIXTURE_SERVICE_TYPE_PREFIX="${FIXTURE_SERVICE_TYPE_PREFIX:?}"
...
# i.e. what the testcases in TEST-89-RESOLVED-MDNS.sh pass.
...
REF_BROWSE_FLAGS_NO_CACHE=16789528
...
ref_count_events() {
    ...
    matches="$(grep -oE '\{[^{}]*\}' "$file" |
        { grep -E "\"updateFlag\":\"$flag\"" || :; } |
        { grep -E "\"name\":\"$instance\"" || :; } |
        { grep -E "\"type\":\"$REF_SERVICE\"" || :; } |
        { grep -E "\"domain\":\"local\"" || :; })"
    if [[ -n "$ifindex" ]]; then
        matches="$(grep -E "\"ifindex\":${ifindex}[,}]" <<<"$matches" || :)"
    fi
...
ref_wait_added_name() {
    ...
    for ((i = 0; i < iterations; i++)); do
        if grep -oE '\{[^{}]*\}' "$file" | grep -E "\"updateFlag\":\"added\"" | grep -E "\"name\":\"$instance\"" >/dev/null; then
            return 0
        fi
        sleep 1
    done
...
ref_scenario_announce() {
    ...
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefAnnounce || return 2
...
ref_scenario_expiry_no_goodbye() {
    ...
    ref_start_browse "$out" "$REF_IF1_INDEX" || return 2
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefExpiry --ttl 20 || return 2
...
ref_scenario_expiry_no_goodbye_plain_flags() {
    ...
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefExpiryPlain --ttl 20 || return 2
...
ref_scenario_query_backoff() {
    ...
    # Three repeats give two gaps to compare; the later one has to be clearly
    # longer rather than the query repeating at a fixed rate.
    awk -v q="Q name=$REF_SERVICE.local qtype=12" '
        index($0, q) {
            if (prev != "") { gap = $1 - prev; if (first == "") first = gap; last = gap; gaps++ }
            prev = $1
        }
        END {
            if (gaps < 2) exit 2
            exit (last >= first * 1.5) ? 0 : 1
        }' "$listen_log"
    case "$?" in
        0) return 0 ;;
...
ref_scenario_shared_record_response_delay() {
    ...
    [[ "$min" -ge 20 ]] || { echo >&2 "answered after ${min}ms at the fastest"; return 1; }
}
...
ref_scenario_responder_aggregation() {
    ...
    # Records are logged under the packet that carried them, so a packet whose
    # records include both answers is one that aggregated them.
    awk -v ptr="name=$REF_PUB_SERVICE.local type=12" \
        -v srv="name=$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local type=33" '
        / P qr=1 / { seen_ptr = 0; seen_srv = 0 }
        / R qr=1 / {
            if (index($0, ptr)) seen_ptr = 1
            if (index($0, srv)) seen_srv = 1
            if (seen_ptr && seen_srv) { together = 1; exit }
        }
        END { exit together ? 0 : 1 }' "$q_log" || { cat "$q_log" >&2; return 1; }
}
...
ref_scenario_responder_legacy_unicast() {
    ...
    for _ in 1 2; do
        ...
        if grep -E "P qr=1 id=35081 dst=$REF_IF1_PEER_ADDR_RE" >/dev/null "$q_log" &&
...
ref_scenario_querier_known_answers() {
    ...
    ref_start_browse "$out" "$REF_IF1_INDEX" || return 2
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefKA || return 2
...
ref_scenario_known_answer_continuation() {
    ...
    ref_query_with_witness "$q_log" --tc --followup-known-answer || return 2
...
ref_scenario_conflict_rename() {
    ...
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefConflict --service _refconf._udp || return 2
    sleep 2
...
ref_scenario_goodbye_then_reannounce() {
    ...
    local out="$REF_TMPDIR/rejoin.browse" peer_log="$REF_TMPDIR/rejoin.peer"

    ref_start_browse "$out" "$REF_IF1_INDEX" || return 2
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefRejoin --goodbye-on-exit || return 2
    ...
    # than treat the name as still known.
    ref_start_peer "$peer_log.2" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefRejoin || return 2
    local i
    for ((i = 0; i < 30; i++)); do
        [[ "$(ref_count_events "$out" added RefRejoin "$REF_IF1_INDEX")" -ge 2 ]] && return 0
        sleep 1
    done
...
ref_scenario_conflict_established() {
    ...
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" "$instance" --service "$service" --port 41999 || return 2
    ref_wait_listener || return 2
...
ref_scenario_goodbye() {
    ...
    ref_start_browse "$out" "$REF_IF1_INDEX" || return 2
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefGoodbye --goodbye-on-exit || return 2
...
ref_scenario_goodbye_single_announcement() {
    ...
    local out="$REF_TMPDIR/gbonce.browse" peer_log="$REF_TMPDIR/gbonce.peer"

    ref_start_browse "$out" "$REF_IF1_INDEX" || return 2
    # Section 10.1 ties the goodbye to the record the browser holds, not to how
    # many times it was announced, so one announcement has to be enough.
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefGbOnce \
        --announce-count 1 --goodbye-on-exit || return 2
    ref_wait_event "$out" added RefGbOnce "$REF_IF1_INDEX" 30 ||
        { cat "$out" "$peer_log" >&2; return 2; }

    kill "$REF_LAST_PEER_PID" || return 2
    ref_wait_event "$out" removed RefGbOnce "$REF_IF1_INDEX" 30 ||
        { cat "$out" "$peer_log" >&2; return 1; }
}
...
ref_scenario_goodbye_shared_instance() {
    ...
    ip -n "$REF_NS" address add "$second_addr/24" dev "${REF_IF1}p" || return 2
    # shellcheck disable=SC2064
    trap "trap - RETURN; ip -n $REF_NS address del $second_addr/24 dev ${REF_IF1}p 2>/dev/null || :" RETURN

    ref_start_browse "$out" "$REF_IF1_INDEX" || return 2
    ref_start_peer "$peer1_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefSharedBye || return 2
    ...
    # instance is shared.
    ref_start_peer "$peer2_log" "${REF_IF1}p" "$second_addr" RefSharedBye \
...
ref_scenario_cache_flush_update() {
    ...
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefFlush --service _refflush._udp --port 41001 || return 2
    sleep 4
    ...
    kill -KILL "$REF_LAST_PEER_PID" || return 2
    ref_start_peer "$peer_log.2" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefFlush --service _refflush._udp --port 41002 || return 2
    sleep 4
...
ref_scenario_case_insensitive_match() {
    ...
    ref_start_browse "$out" "$REF_IF1_INDEX" || return 2
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefCase --service _Ref._UDP || return 2
...
ref_scenario_responder_rcode() {
    ...
    local q_log="$REF_TMPDIR/rcode.query"
    ...
    # silently ignored". The reply to the well-formed query above is what makes
    # this silence meaningful rather than merely a responder that went away.
    ref_query "$q_log" "$REF_PUB_SERVICE.local" --rcode 3 --duration 5 || return 2
...
ref_scenario_resolve_service() {
    ...
    # exists, resolving it is the other half of DNS-SD. Name, type and port
    # come from the parent's exported fixture contract, so an edit to the
    # .dnssd generation cannot silently turn into a bogus "regressed" here.
    varlinkctl call /run/systemd/resolve/io.systemd.Resolve io.systemd.Resolve.ResolveService \
        "{ \"name\": \"$FIXTURE_SERVICE_NAME_PREFIX 0 on $CONTAINER_1\", \"type\": \"${FIXTURE_SERVICE_TYPE_PREFIX}0._udp\", \"domain\": \"local\", \"ifindex\": $BRIDGE_INDEX, \"flags\": $REF_BROWSE_FLAGS }" \
        >"$out" 2>&1 || { cat "$out" >&2; return 1; }
...
ref_scenario_browse_subtype() {
    ...
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefSub --subtype _vendor || return 2
...
ref_scenario_duplicate_question_suppression() {
    ...
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefDup || return 2
...
ref_scenario_cross_host_question_suppression() {
    ...
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefXhqs --suppress-known-answers || return 2
    ...
    # Positive control: the subscriber still learns of the instance -- if only
    # from the answers the peer keeps giving to its own question.
    ref_wait_event "$out" added RefXhqs "$REF_IF1_INDEX" 10 || { cat "$out" "$peer_log" >&2; return 2; }

    # Liveness canary, as in the sibling scenario: the marker must appear in
    # the capture, or the listener was deaf and the count below meaningless.
    ref_query "$REF_TMPDIR/xhqs.marker" "refmark._udp.local" --duration 1 || return 2
    ref_wait_listener || return 2
    grep "Q name=refmark._udp.local" >/dev/null "$listen_log" || { cat "$listen_log" >&2; return 2; }
    ...
    queries="$(awk -v q="Q name=$REF_SERVICE.local qtype=12" -v witness="qtype=33" '
        ...
        /^P / { flush_block(); blk = ($0 ~ /^P qr=0 /) ? $0 : ""; next }
        { if (blk != "") blk = blk "\n" $0 }
...
ref_scenario_multi_interface_ptr_query() {
    ...
    ref_start_peer "$peer1_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefIf1 --service _refmulti._udp || return 2
    ref_start_peer "$peer2_log" "${REF_IF2}p" "$REF_IF2_PEER_ADDR" RefIf2 --service _refmulti._udp || return 2
    sleep 5
...
ref_scenario_no_cache_lookup() {
    ...
    ref_start_peer "$peer_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefNoCache \
...
ref_scenario_two_interfaces_same_name() {
    ...
    ref_start_peer "$peer1_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefTwoIf || return 2
    ...
    ref_start_peer "$peer2_log" "${REF_IF2}p" "$REF_IF2_PEER_ADDR" RefTwoIf || return 2
...
ref_scenario_goodbye_one_interface() {
    ...
    ref_start_browse "$out" 0 || return 2
    ref_start_peer "$peer1_log" "${REF_IF1}p" "$REF_IF1_PEER_ADDR" RefHalfBye || return 2
    ref_wait_event "$out" added RefHalfBye "$REF_IF1_INDEX" 30 || { cat "$out" "$peer1_log" >&2; return 2; }

    ref_start_peer "$peer2_log" "${REF_IF2}p" "$REF_IF2_PEER_ADDR" RefHalfBye --goodbye-on-exit || return 2
    peer2_pid="$REF_LAST_PEER_PID"
...
testcase_conformance_scorecard() {
    ...
    local scenarios=(
        ...
        cross_host_question_suppression     # RFC 6762 section 7.3
    ...
    for i in 1 2; do
        local host_if="${ref_ifs[i - 1]}" peer_if="${ref_ifs[i - 1]}p"
