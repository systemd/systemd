...
ref_scenario_known_answer_continuation() {
    ...
    # TC set, the covering known answer inline, and a further known answer for a different
    # target in a continuation packet 100ms later: the responder is supposed to defer
    # 400-500ms, merge both halves, and then stay quiet on the strength of the inline one.
    # The witness question travels in the first packet, so its answer proves that packet
    # arrived -- and losing only the follow-up leaves the responder answering, which is the
    # tracked failure rather than a spurious pass. An implementation that ignored inline
    # known answers and honoured only continuations would answer too: the continuation's
    # target does not cover the instance.
    ref_known_answer_scenario kacont --tc --followup-known-answer "Continuation.$REF_PUB_SERVICE.local"
}
