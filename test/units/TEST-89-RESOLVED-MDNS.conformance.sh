...
ref_scenario_multi_interface_ptr_query() {
    ...
    grep RefIf1 >/dev/null "$out.0" && grep RefIf2 >/dev/null "$out.0" ||
        { cat "$out.0" >&2; return 1; }
}
...
ref_scenario_publisher_probe_announce() {
    ...
    # Start from unregistered, so the reload below really probes and announces.
...
testcase_conformance_scorecard() {
    ...
    # RFC 6763, then the scenarios that come from interoperability reports.
    # The scenarios do not depend on each other or on this order.
    local scenarios=(
        announce                            # baseline
        ...
        multi_interface_ptr_query           # 6762 section 10, #38380
        ...
        dual_stack_resolution               # 6762 section 6.2, #25855
