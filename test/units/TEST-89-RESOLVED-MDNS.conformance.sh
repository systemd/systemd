...
ref_scenario_multiquestion_response_delay() {
    ...
    # for probe replies does not apply. As in the single-question case, take
    # the smallest of several samples so a descheduled reader cannot read as a
    # deliberate delay.
    for i in 1 2 3; do
        q_log="$REF_TMPDIR/mqdelay.query.$i"
        ref_query "$q_log" "$REF_PUB_SERVICE.local" \
            --witness-qname "$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local" --duration 3 || return 2
        delay="$(awk -v ptr="ptr=$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local" \
            '/ QUERY name=/ { q = $1 }
             / R qr=1 / && index($0, ptr) && q != "" { printf "%d\n", ($1 - q) * 1000; exit }' "$q_log")"
        [[ -n "$delay" ]] || continue
    ...
    [[ -n "$min" ]] || { cat "$REF_TMPDIR"/mqdelay.query.* >&2; return 2; }
    [[ "$min" -ge 20 ]] || { echo >&2 "answered after ${min}ms at the fastest"; return 1; }
}
...
ref_scenario_goodbye_shared_instance() {
    ...
    for ((i = 0; i < 10; i++)); do
        grep "GOODBYE n=2" >/dev/null 2>&1 "$peer2_log" && break
        sleep 1
    done
