...
ref_scenario_reverse_lookup_non_local() {
    ...
    varlinkctl call /run/systemd/resolve/io.systemd.Resolve io.systemd.Resolve.ResolveAddress \
        ...
        >"$out" 2>&1 || :
    # Liveness canary, as in the suppression scenarios: the marker must show up in the capture,
    # or a listener that never joined the group would read as a conforming resolved the moment
    # the #20267 known-failure entry is gone.
    ref_query "$REF_TMPDIR/nonlocal.marker" "refmark._udp.local" --duration 1 || return 2
...
ref_scenario_responder_wildcard_query() {
    ...
    # And in the same response: records are logged under the packet that carried them, so both
    # answers scraped together across the window would not do.
    awk -v srv="sec=an name=$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local type=33" \
        -v txt="sec=an name=$REF_PUB_INSTANCE.$REF_PUB_SERVICE.local type=16" '
        / P qr=1 / { seen_srv = 0; seen_txt = 0 }
        / R qr=1 / {
            if (index($0, srv)) seen_srv = 1
            if (index($0, txt)) seen_txt = 1
            if (seen_srv && seen_txt) { together = 1; exit }
        }
        END { exit together ? 0 : 1 }' "$q_log" || { cat "$q_log" >&2; return 1; }
}
