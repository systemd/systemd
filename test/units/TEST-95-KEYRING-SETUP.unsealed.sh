...
enrolled="$(journalctl --merge -o json -t systemd-keyring-setup |
    jq -rn --arg m "Enrolled '/usr/share/voa/$ID/image/default/x509/mkosi-certificate.pem' into keyring .dm-verity" \
        'first(inputs | select(.MESSAGE | startswith($m)) | .__MONOTONIC_TIMESTAMP)')"
switched="$(journalctl --merge -o json _PID=1 |
    jq -rn 'first(inputs | select(.MESSAGE | startswith("Switching root")) | .__MONOTONIC_TIMESTAMP)')"
