...
# The service enrolled it from the VOA hierarchy, in the initrd
. /etc/os-release
journalctl --merge -o cat -u systemd-keyring-setup.service |
    grep -F "Enrolled '/usr/share/voa/$ID/image/default/x509/mkosi-certificate.pem' into keyring .dm-verity" >/dev/null
