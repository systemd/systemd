...
# Sealed, holding the mkosi certificate under its kernel-derived description and the one
# handed over as a credential
grep -E '^[0-9a-f]+ [A-Za-z-]{7} +[0-9]+ +perm +08070000 +0 +0 +keyring +\.dm-verity: 2$' /proc/keys
...
enrolled="$(journalctl --merge -o json -t systemd-keyring-setup |
    jq -r --arg m "Enrolled '/usr/share/voa/$ID/image/default/x509/mkosi-certificate.pem' into keyring .dm-verity" \
        'select(.MESSAGE | startswith($m)) | .__MONOTONIC_TIMESTAMP' | head -n1)"
switched="$(journalctl --merge -o json _PID=1 |
    jq -r 'select(.MESSAGE | startswith("Switching root")) | .__MONOTONIC_TIMESTAMP' | head -n1)"
...
(! openssl x509 -in /tmp/stranger.pem -outform DER | keyctl padd asymmetric '' %:.dm-verity)
...
grep -E ' +08070000 +0 +0 +keyring +\.dm-verity: 3$' /proc/keys
