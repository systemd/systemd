...
add_service() {
    ...
    SERVICE_LIST+=( "systemd-${name}d.service" )
    BUS_LIST+=( "org.freedesktop.${name}1" )
}
...
add_service home
add_service hostname
add_service import
add_service locale
add_service login
add_service machine
add_service portable
add_service resolve
add_service timedate
...
if ! systemd-detect-virt --container; then
    add_service network
    add_service timesync
elif ip link add dummy-fuzz type dummy; then
    ip link del dummy-fuzz
    add_service network
fi
...
# Unmask and enable services to be tested.
systemctl unmask "${SERVICE_LIST[@]}"
