...
if command -v openssl >/dev/null 2>&1 && command -v fsverity >/dev/null 2>&1 && command -v xxd >/dev/null 2>&1; then
	...
	# fsverity imposes that the filesystem's block size is equival to the kernel's page size
    ...
    if tune2fs -O verity /tmp/verity.ext4; then
        mount -o X-mount.mkdir /tmp/verity.ext4 /etc/systemd/system.attached/
