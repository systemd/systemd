...
journalctl --sync
TEST2_CURSOR=TEST_CURSOR="$(journalctl -q -t "$TEST_TAG" -n 0 --show-cursor | awk '{ print $3; }')"
echo "-= This is a third test message =-" | systemd-cat -t "$TEST_TAG"
journalctl --sync
sleep 1
END_TIMESTAMP="$(date +%s)"
curl -Lfs --header "Accept: application/json" --header "Range: time=$BEFORE_TIMESTAMP::1:1" http://localhost:19531/entries?SYSLOG_IDENTIFIER="$TEST_TAG" | \
    jq -se "length == 1 and select(.[].__CURSOR=\"$TEST2_CURSOR\")"
curl -Lfs --header "Accept: application/json" --header "Range: time=$END_TIMESTAMP::-2:1" http://localhost:19531/entries?SYSLOG_IDENTIFIER="$TEST_TAG" | \
    jq -se "length == 1 and select(.[].__CURSOR=\"$TEST2_CURSOR\")"
