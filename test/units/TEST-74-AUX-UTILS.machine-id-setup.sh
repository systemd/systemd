...
testcase_force_transient() {
    ...
    # every refusal happens before the truncation, so the ID in place must be untouched
...
testcase_force_cmdline_machine_id() {
    ...
    # mount namespace with its own /etc so the real one is untouched.
    #
    # The fixture must not sit on a tmpfs: the temporary-file-system guard runs right after this
    # one, so the two invocations that are expected to succeed would be refused for the wrong
    # reason. /tmp is a tmpfs in the test image, /var/tmp is not.
