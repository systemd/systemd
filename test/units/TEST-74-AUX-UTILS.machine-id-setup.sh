...
testcase_force_cmdline_machine_id() {
    ...
    out="$(unshare --mount --propagation private bash -eux -c '
        ...
        # With no ID to replace --force falls through to the regular initialization, where
        # /run/machine-id is the *first* source consulted on the host — and on a booted clone it
        # holds the identity currently in use, i.e. what we were asked to replace. It must be
        # skipped, and rewritten to the new ID like any other stale copy
        old="$(systemd-id128 new)"
        echo "$old" >/run/machine-id
        : >/etc/machine-id
        systemd-machine-id-setup --print --force
        test "$(cat /etc/machine-id)" != "$old"
        test "$(cat /run/machine-id)" = "$(cat /etc/machine-id)"
    ' bash "$tmp" 2>&1)"
