...
testcase_force_readonly_no_id() {
    ...
    # The fall-through arm skips force_new_check_persistent() entirely, so on a read-only /etc with
    # no usable ID --force must not quietly install a transient overmount and exit 0 — that is the
    # outcome testcase_force_readonly rules out for the valid-ID case
    : >"$root/etc/machine-id"
    mount -o remount,ro "$root"
    mount -t tmpfs tmpfs "$root/run"
...
testcase_force_cmdline_machine_id() {
    ...
    out="$(unshare --mount --propagation private bash -eux -c '
        ...
        mount --bind "$1/dbus" /var/lib/dbus
