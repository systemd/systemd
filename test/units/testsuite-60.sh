...
cleanup_23796() {(
    ...
    rm /run/systemd/system/tmp-hoge.mount
)}


test_issue_23796() {
    trap cleanup_23796 RETURN

    MOUNT=$(command -v mount 2>/dev/null)
    MOUNT_MYTMPFS="$(echo "$MOUNT" | sed -e 's|bin|sbin|')".mytmpfs
    cat >"$MOUNT_MYTMPFS" <<EOF
#!/bin/bash

sleep ".\$RANDOM"
exec -- $MOUNT -t tmpfs tmpfs \$2
EOF
