...
EOF
    ...
    systemd-run --unit="$unit" --service-type=exec \
        systemd-inhibit --what=sleep --mode=delay --signal=SIGUSR1 --who="$who" --why="testing repeated notifications" \
    ...
    for count in 1 2; do
        ...
        systemd-inhibit --list --no-legend --mode=delay | grep -F "$who" >/dev/null
        # shellcheck disable=SC2016
        timeout 30 bash -c 'until [[ -e "$1/signals" ]] && [[ "$(wc -l <"$1/signals")" -ge "$2" ]]; do sleep 0.1; done' bash "$STATE_DIR" "$count"

        # Observe the count after resuming too, to catch a signal incorrectly forwarded for false.
        for _ in {1..10}; do
            assert_eq "$(wc -l <"$STATE_DIR/signals")" "$count"
            sleep 0.1
        done
    done
