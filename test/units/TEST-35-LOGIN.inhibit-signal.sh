...
for signal in SIGTERM SIGABRT; do
    ...
    check_shutdown_forwarding shutdown "$signal" PreparingForShutdown "go down" 1 systemctl poweroff --no-block
done

# Only the operation that actually happens gets the signal forwarded for it
check_shutdown_forwarding sleep:shutdown SIGTERM PreparingForShutdown "go down" 1 systemctl poweroff --no-block
...
test -e /run/suspend.flag
check_shutdown_forwarding shutdown SIGTERM PreparingForShutdown "go down" 0 systemctl poweroff --no-block
