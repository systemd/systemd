...
: "Matching signal death is a failure when no signal has been sent"
...
check_signalling() {
    ...
    journalctl --sync
    journalctl -b --no-pager -u "$unit" | grep -F "Sending $signal to '${command[0]}', because the system is about to $wording." >/dev/null
...
for signal in SIGTERM SIGABRT; do
    check_signalling sleep "$signal" PreparingForSleep sleep 1 systemctl suspend --no-block
    test -e /run/suspend.flag

    check_shutdown_signalling shutdown "$signal" PreparingForShutdown "go down" 1 systemctl poweroff --no-block
done

# Only the operation that actually happens gets the signal sent for it
check_shutdown_signalling sleep:shutdown SIGTERM PreparingForShutdown "go down" 1 systemctl poweroff --no-block

: "A command can handle the notification and exit successfully before the delay expires"
check_signalling sleep SIGTERM PreparingForSleep sleep 0 systemctl suspend --no-block
test -e /run/suspend.flag
check_shutdown_signalling shutdown SIGTERM PreparingForShutdown "go down" 0 systemctl poweroff --no-block

check_repeated_signalling() {
...
EOF
    ...
    for count in 1 2; do
        ...
        # Observe the count after resuming too, to catch a signal incorrectly sent for false.
        for _ in {1..10}; do
...
check_repeated_signalling sleep

: "A shutdown-only lock must not act on sleep notifications"
check_repeated_signalling shutdown
