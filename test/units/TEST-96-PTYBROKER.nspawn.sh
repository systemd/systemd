...
ROOT=/var/lib/machines/pty96-nspawn

at_exit() {
    set +e
    rm -fr "$ROOT" /run/systemd/nspawn/pty96-nspawn.nspawn
}
...
OUT="$(systemd-nspawn -D "$ROOT" --machine=pty96-nspawn --register=no --console=broker-log sh -xc 'echo hello-pty96-nspawn-marker' 2>&1)"
NAME="$(sed -n "s/.*Broker registered console pseudo TTY under name '\([^']*\)'.*/\1/p" <<<"$OUT")"
test -n "$NAME"

timeout 30 bash -c "until journalctl -q -t '$NAME' | grep hello-pty96-nspawn-marker >/dev/null; do journalctl --sync; sleep 1; done"
