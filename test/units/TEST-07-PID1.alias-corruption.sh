...
[Service]
Type=simple
ExecStart=/bin/bash -c 'trap "" TERM; touch /run/hung-stop.ready; while :; do sleep infinity & wait $!; done'
...
EOF
    ...
    # Wait for the bash process to actually install the SIGTERM trap before
    # queueing the stop. Otherwise on slow CI hosts the SIGTERM from the stop
    # job can race in and terminate bash before it runs the 'trap' builtin,
    # which makes the stop job complete immediately instead of staying queued.
    for _ in {1..100}; do
        [[ -e /run/hung-stop.ready ]] && break
        sleep 0.1
    done
    if [[ ! -e /run/hung-stop.ready ]]; then
        echo "ERROR: hung-stop.service main process did not become ready"
        return 1
    fi
    ...
    stop_job=$(systemctl show -P Job "$scope" 2>/dev/null || true)
    if [[ -z "$stop_job" ]] && ! [[ "$(systemctl show -P SubState "$scope")" =~ ^stop- ]]; then
