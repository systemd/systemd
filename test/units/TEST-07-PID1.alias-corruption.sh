...
EOF
    ...
    if [[ "$pending_jobs" == "with_pending_jobs" ]]; then
        ...
        for i in {1..100}; do
            [[ -n "$(systemctl list-jobs --no-legend | grep '^[[:space:]]*[0-9]\+ sus-' || true)" ]] && break
            if (( i == 100 )); then
    ...
    echo "Setup complete: 1 running legit unit, 100 ${pending_jobs:+job-bearing }sus units"
