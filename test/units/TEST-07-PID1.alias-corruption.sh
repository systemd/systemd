...
test_stop_job_preserved() {
    local reload_cmd="${1:?}"
    local hung_pid orphan stop_job
...
EOF
    ...
    # process) is leaked across the reload. Either the job is still in the
    # queue, or the orphan already moved into a stop sub-state.
    stop_job=$(systemctl show -P Job "$orphan" 2>/dev/null || true)
    # Accept any stop-* sub-state: the synthesized orphan inherits the manager's
    # defaults (TimeoutStopSec, SendSIGKILL=yes) rather than hung-stop.service's
    # SendSIGKILL=no, so on slow CI it may progress past stop-sigterm to
    # stop-sigkill (or final) before this assertion runs.
    if [[ -z "$stop_job" ]] && ! [[ "$(systemctl show -P SubState "$orphan")" =~ ^stop- ]]; then
        echo "ERROR: stop job for original hung-stop.service was not preserved on $orphan!"
        echo "Current substate: $(systemctl show -P SubState "$orphan")"
        systemctl list-jobs || true
        return 1
    fi
