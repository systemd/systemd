...
EOF
    ...
    output=$(systemd-repart --definitions="$defs" \
                            ...
                            --size=auto \
                            --json=pretty \
                            "$imgs/verity" | tee proc/self/fd/2)

    roothash=$(echo "$output" | jq -r ".[] | select(.type == 'root-${architecture}-verity') | .roothash")
    ...
    systemd-dissect "$imgs/verity" --root-hash "$roothash" -U "$imgs/mnt"
}
