...
def glibc_violations(path: str, baseline: Version) -> list[tuple[str, str]]:
    ...
    for line in out.splitlines():
        ...
        if not m:
            continue
        # Parenthesised versions like "(GLIBC_2.2.5)" mark non-default aliases —
        # symbols pinned via .symver, or older variants of a name. They don't
        # impose an ABI floor of their own; the default-version line does.
        if f'({m.group(0)})' in line and m.group(0) not in line.split():
            continue
        ver: Version = (int(m.group(1)), int(m.group(2)))
        if ver <= baseline:
            continue
        sym = next((t for t in line.split() if '@' in t), line.strip())
