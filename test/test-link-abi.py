...
# glibc splits libc/libm/etc.; musl bundles everything into a single libc whose
# soname encodes the architecture (e.g. libc.musl-x86_64.so.1, libc.musl-aarch64.so.1).
# libintl is part of glibc but ships as a standalone library on musl; allow it
# either way until gettext use is made optional/dlopen()'d.
LIBC_LIBS: frozenset[str] = frozenset({
    'libc.so.6',
    'libm.so.6',
    'libintl.so.8',
    'libasan.so.8',
...
MUSL_LIBC_PREFIX: str = 'libc.musl-'
...
def is_libc(name: str) -> bool:
    return name in LIBC_LIBS or name.startswith(MUSL_LIBC_PREFIX)
...
def main() -> int:
    ...
    for path in args.paths:
        ...
        if needed_bad:
            print('  links against unexpected libraries (dlopen() them instead):',
                  file=sys.stderr)
    ...
    print(f'OK: {checked} ELF objects checked; all within {baseline_str} '
          'and with only allowed NEEDED entries.')
    return 0
