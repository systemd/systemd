...
def check_output(*command, text=True, **kwargs):
    ...
    return subprocess.run(command, check=True, text=text, capture_output=True, **kwargs).stdout.rstrip()

def call(*command, check=False, text=True, **kwargs):
    # This returns returncode. stdout and stderr are shown in console
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, check=check, text=text, **kwargs).returncode

def call_quiet(*command, check=False, text=True, **kwargs):
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, check=check, text=text, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **kwargs).returncode

def run(*command, check=False, text=True, **kwargs):
    # This returns CompletedProcess instance.
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, check=check, text=text, capture_output=True, **kwargs)
