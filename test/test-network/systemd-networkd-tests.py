...
def dnr_v4_instance_data(adn: str, addrs: list[str | int], prio: int = 1, alpns: tuple[str] = ("dot",), dohpath: str | None = None):
    b = bytes()
...
class NetworkdDHCPClientTests(unittest.TestCase, Utilities):
    ...
    def test_dhcp_client_use_dnr(self):
        ...
        start_networkd()
        self.wait_online(['veth-peer:carrier'])
        dnr = dnr_v4_instance_data(adn = "dns.google", addrs = ["8.8.8.8", "8.8.4.4"])
        masq = lambda bs: ':'.join(f"{b:02x}" for b in bs)
        start_dnsmasq(f'--dhcp-option=162,{masq(dnr)}')
