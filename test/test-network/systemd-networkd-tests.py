...
class NetworkdNetDevTests(unittest.TestCase, Utilities):
    ...
    def test_tuntap(self):
        ...
        print(output)
        self.assertRegex(output, f'testtap99: tap pi (multi_queue |)vnet_hdr persist filter *(0x100|)\n\tAttached to processes:{name}\({pid}\)systemd\(1\)\n*')
        self.assertRegex(output, f'testtun99: tun pi (multi_queue |)vnet_hdr persist filter *(0x100|)\n\tAttached to processes:{name}\({pid}\)systemd\(1\)\n*')
