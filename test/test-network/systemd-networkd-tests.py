...
@unittest.skipUnless(ovs_available(), 'Open vSwitch tools (ovs-vsctl/ovsdb-server/ovs-vswitchd) not installed')
class NetworkdOVSTests(unittest.TestCase, Utilities):
    ...
    def setUp(self):
        ...
        if not any(call_quiet(f'systemctl start {unit}') == 0
    ...
    def wait_ovs(self, command, contains=None, timeout=10):
        ...
        while time.monotonic() < deadline:
            p = subprocess.run(command.split(), check=False, universal_newlines=True,
    ...
    def test_ovs_port_internal(self):
        copy_network_unit('25-ovs-bridge.netdev', '25-ovs-bridge.network',
                          '25-ovs-port-internal.netdev', '25-ovs-port-internal.network')
        start_networkd()
    ...
    def test_ovs_bond(self):
        copy_network_unit('25-ovs-bridge.netdev', '25-ovs-bridge.network',
                          ...
                          '12-dummy.netdev', '25-ovs-bond-member-dummy98.network',
                          '11-dummy.netdev', '25-ovs-bond-member-test1.network')
        start_networkd()
        self.wait_online('br0:degraded')
        ...
        output = self.wait_ovs('ovs-vsctl list-ifaces br0', contains='dummy98')
        print(output)
    ...
    def test_ovs_system_port(self):
        copy_network_unit('25-ovs-bridge.netdev', '25-ovs-bridge.network',
                          '12-dummy.netdev', '25-ovs-bridge-port.network')
        start_networkd()
        self.wait_online('br0:degraded')
