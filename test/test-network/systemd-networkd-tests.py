...
def expectedFailureIfNetdevsimWithSRIOVIsNotAvailable():
    def f(func):
        ...
        supported = os.path.exists('/sys/bus/netdevsim/devices/netdevsim99/sriov_numvfs')
        return finalize(func, supported)
...
class Utilities():
    ...
    def wait_online(self, links_with_operstate, timeout='20s', bool_any=False, ipv4=False, ipv6=False, setup_state='configured', setup_timeout=5):
        ...
        if ipv6:
            args += ['--ipv6']
        try:
            output = check_output(*args, env=wait_online_env)
        except subprocess.CalledProcessError:
            print(output) # show logs only on failure
...
class NetworkdSRIOVTests(unittest.TestCase, Utilities):
    ...
    def test_sriov(self):
        copy_network_unit('25-sriov.network', '90-default-netdevsim.link')
