...
def check_output(*command, text=True, **kwargs):
    # This checks the result and returns stdout on success.
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, check=True, universal_newlines=text, capture_output=True, **kwargs).stdout.rstrip()

def call(*command, check=False, text=True, **kwargs):
    # This returns returncode. stdout and stderr are shown in console
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, check=check, universal_newlines=text, **kwargs).returncode

def call_quiet(*command, check=False, text=True, **kwargs):
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, check=check, universal_newlines=text, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **kwargs).returncode

def run(*command, check=False, text=True, **kwargs):
    # This returns CompletedProcess instance.
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, check=check, universal_newlines=text, capture_output=True, **kwargs)
...
def expectedFailureIfAlternativeNameIsNotAvailable():
    def f(func):
        call_quiet('ip link add dummy98 type dummy')
        supported = (call_quiet('ip link prop add dev dummy98 altname hogehogehogehogehoge') == 0 and
...
def expectedFailureIfNetdevsimWithSRIOVIsNotAvailable():
    def f(func):
        ...
        call_quiet('udevadm info -w10s /sys/devices/netdevsim99/net/eni99np1')
        try:
...
def save_existing_links():
    ...
    print('The following links will be protected: ' + ', '.join(sorted(list(protected_links))))
...
def save_routes():
    ...
    global saved_routes
    ret = run('ip route save table all', text=False)
    if ret.returncode == 0 and len(ret.stdout) > 4:
        saved_routes = ret.stdout
    else:
        print('cannot save routes')
        sys.exit(1)

def restore_routes():
    # Unlike 'ip route restore', this flushes routes before restoring routes
    with open(os.path.join(networkd_ci_temp_dir, 'saved-routes.bin'), mode='w+b') as f:
        f.write(saved_routes)
        f.flush()
        f.seek(0)
        call('ip route flush table all')
        call('ip route restore', text=False, stdin=f)
...
def save_routing_policy_rules():
    ...
    def save(ipv):
        ret = run(f'ip -{ipv} rule save', text=False)
        if ret.returncode == 0 and len(ret.stdout) > 0:
            return ret.stdout
        print(f'cannot save ipv{ipv} routing policy rules')
        sys.exit(1)

    saved_ipv4_rules = save(4)
    saved_ipv6_rules = save(6)

def restore_routing_policy_rules():
    def restore(ipv, rules):
        # Unlike 'ip rule restore', this flushes rules before restoring rules
        call(f'ip -{ipv} rule flush')

        # 'ip rule flush' does not remove rules with priority 0
        while True:
            if call_quiet(f'ip -{ipv} rule del priority 0') != 0:
                break

        call(f'ip -{ipv} rule restore', text=False, input=rules)

    restore(4, saved_ipv4_rules)
    restore(6, saved_ipv6_rules)
...
class Utilities():
    ...
    def wait_online(self, links_with_operstate, timeout='20s', bool_any=False, ipv4=False, ipv6=False, setup_state='configured', setup_timeout=5):
        ...
        if ipv6:
            args += ['--ipv6']
        try:
            call(*args, check=True, env=wait_online_env)
...
class NetworkdSRIOVTests(unittest.TestCase, Utilities):
    ...
    def test_sriov(self):
        ...
        call('udevadm info -w10s /sys/devices/netdevsim99/net/eni99np1')
        with open('/sys/class/net/eni99np1/device/sriov_numvfs', mode='w', encoding='utf-8') as f:
            f.write('3')
