...
class NetworkdRATests(unittest.TestCase, Utilities):
    ...
    def test_ndisc_router_limit(self):
        ...
        # The RAs are delivered and processed in order over a single socket, so once the last (over-limit)
        # sender has been rejected, all of them have been handled and the route table is final.
        self.check_networkd_log(f'Too many routers per link. Ignoring RA from fe80::47:{n_senders - 1:x}.')
        ...
        check_output(f'{test_ndisc_send} --interface veth-peer --type ra --mtu 1400 --source fe80::48:1')  # fmt: skip
        self.check_ndisc_mtu(1400)
