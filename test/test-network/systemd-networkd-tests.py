...
def check_output(*command, text=True, **kwargs):
    ...
    return subprocess.run(command, text=text, capture_output=True, check=True, **kwargs).stdout.rstrip()

def call(*command, text=True, **kwargs):
    # This returns returncode. stdout and stderr are shown in console
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, text=text, **kwargs).returncode

def call_quiet(*command, text=True, **kwargs):
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, text, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **kwargs).returncode

def run(*command, text=True, **kwargs):
    # This returns CompletedProcess instance.
    command = command[0].split() + list(command[1:])
    return subprocess.run(command, text=text, capture_output=True, **kwargs)
