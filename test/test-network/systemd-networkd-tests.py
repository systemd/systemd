...
class NetworkdStateFileTests(unittest.TestCase, Utilities):
    ...
    def test_state_file(self):
        ...
        print(output)
        self.asserIn('IPV4_ADDRESS_STATE=routable', output)
        self.asserIn('IPV6_ADDRESS_STATE=routable', output)
        self.asserIn('ADMIN_STATE=configured', output)
        self.asserIn('OPER_STATE=routable', output)
        self.asserIn('REQUIRED_FOR_ONLINE=yes', output)
        self.asserIn('REQUIRED_OPER_STATE_FOR_ONLINE=routable', output)
        self.asserIn('REQUIRED_FAMILY_FOR_ONLINE=both', output)
        self.asserIn('ACTIVATION_POLICY=up', output)
        self.asserIn('NETWORK_FILE=/run/systemd/network/25-state-file-tests.network', output)
        self.asserIn('DNS=10.10.10.10#aaa.com 10.10.10.11:1111#bbb.com [1111:2222::3333]:1234#ccc.com', output)
        self.asserIn('NTP=0.fedora.pool.ntp.org 1.fedora.pool.ntp.org', output)
        self.asserIn('DOMAINS=hogehoge', output)
        self.asserIn('ROUTE_DOMAINS=foofoo', output)
        self.asserIn('LLMNR=no', output)
        self.asserIn('MDNS=yes', output)
        self.asserIn('DNSSEC=no', output)
