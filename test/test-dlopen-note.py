...
# Function filter: Maps wrapper/sentinel symbols to real symbols or None (ignore).
FUNCTION_FILTER = {
    # Filtered out (internal helpers, verbose loggers, sentinels)
    'dlopen_safe': None,
    'dlopen_verbose': None,
    'dlopen_many_sym_or_warn_sentinel': None,
    'dlopen_dw_has_dwfl_set_sysroot': None,
...
def dlopen_function_append(name: str, function_names: set[str]) -> None:
    ...
    if name in FUNCTION_FILTER:
        filter_val = FUNCTION_FILTER[name]
        if filter_val is not None:
            # Expand multiple symbols (e.g., generic dlopen_tpm2 -> esys/mu/rc/tcti)
            function_names.update(filter_val)
    else:
...
def dlopen_functions_parse(path: Path) -> set[str]:
    ...
    pattern = re.compile(r'^(?:sym_)?(dlopen_[a-zA-Z0-9_]+)$')
        ...
        with path.open('rb') as f:
            ...
            for section in elffile.iter_sections():
                ...
                for symbol in section.iter_symbols():
                    ...
                    # Walrus operator (:=) matches and assigns in one clean step
                    if m := pattern.match(symbol.name):
...
def main():
    ...
    func_static = dlopen_functions_parse(Path(args.static))
    unknown_functions = set()
    features_by_func = set()
    for name in func_static:
    ...
    for feat, item in sorted(note_merged.items()):
        ...
        if feat not in features_by_func:
