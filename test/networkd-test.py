...
'''.format(iface=self.iface, dhcp_mode=dhcp_mode, extra_opts=extra_opts))
        ...
        try:
            ...
            self.assertRegex(out, (r'{}\s+ether\s+[a-z-]+\s+unknown\s+unmanaged'.format(self.if_router)).encode())
            self.assertRegex(out, (r'{}\s+ether\s+routable\s+online\s+configured'.format(self.iface)).encode())
