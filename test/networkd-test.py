...
'''.format(iface=self.iface, dhcp_mode=dhcp_mode, extra_opts=extra_opts))
        ...
        try:
            ...
            self.assertRegex(out, (r'{}\s+ether\s+unknown\s+[a-z-]+\s+unmanaged'.format(self.if_router)).encode())
            self.assertRegex(out, (r'{}\s+ether\s+online\s+routable\s+configured'.format(self.iface)).encode())
