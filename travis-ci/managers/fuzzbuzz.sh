...
apt-get install -y gettext python3-evdev python3-pyparsing libmount-dev python3-setuptools ninja-build quota
pip3 install meson
...
ninja -v -C build
make -C test/TEST-44-LOG-NAMESPACE clean setup run TEST_NO_QEMU=yes NSPAWN_ARGUMENTS=--keep-unit RUN_IN_UNPRIVILEGED_CONTAINER=no
