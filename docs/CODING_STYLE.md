...
- Typically, function parameters fit into four categories: input parameters,
  mutable objects, and call-by-reference return parameters that are initialized
  on success, and call-by-reference return parameters that are initialized on
  ...
  should be prefixed with `ret`, to clarify they are return parameters. The
  name of return parameters that are initialized on failure should be prefixed
  with `reterr_`. (Examples for such parameters are additional error
  information, such as the row/column of parse errors or so). – Conversely,
  please do not prefix parameters that aren't output-only with `ret_` or
  `reterr_`, in particular not mutable parameters that are both input as well
  as output. Example:
...
- Do not write functions that clobber call-by-reference success return
  parameters on failure (i.e. `ret_xyz`, see above), or that clober
