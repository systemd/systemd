...
or user-space programs reading the boot loader configuration should locate
those in the following manner:
...
* On disks with GPT (GUID Partition Table)
  ...
  * Optionally, an Extended Boot Loader Partition (XBOOTLDR partition for
    ...
    location for boot loader entries.
...
and `$BOOT2` will be used to refer to (the root of) the optional second partition.
...
`$BOOT/loader/entries/` and `$BOOT2/loader/entries/` are the directories
...
images are installed in the`$BOOT/EFI/Linux/` and `$BOOT2/EFI/Linux/`
...
A _boot loader_ locates `$BOOT` and `$BOOT2`, then simply reads all the files
`$BOOT/loader/entries/*.conf` and `$BOOT2/loader/entries/*.conf`, and populates
its boot menu. On EFI, it then extends this with any unified kernel images
...
installer_ installs the kernel and initrd images to `$BOOT2` (if present) or
