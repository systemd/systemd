...
creating partitions:
  ...
  * If a boot partition (in case of MBR) or an XBOOTLDR partition already
    exists (in case of GPT) it should be used as `$BOOT` and used as primary
...
done. Such a nested setup complicate a setup via direct `autofs` mounts — as
implemented by `systemd` for example —, as establishing the inner autofs will
trigger the outer one. Mounting the two partitions via `autofs` is a good idea
though, since the simple VFAT file system they typically has weak data
integrity properties and thus should be unmounted whenever possible.)
