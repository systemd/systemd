...
Fuzzers are a type of unit tests that execute code on an externally-suplied
input data. Fuzzers for `src/basic/` and `src/shared` live under `src/fuzz/`,
and fuzzers for other parts of the codebase should be located next to the code
they test.
...
`test/fuzz/` as part of test suite. Secondly, fuzzers may be instrumented with
sanitizers and invoked as part of the test suite (if `-Dfuzz-tests=true` is
configured). Thirdly, fuzzers are executed through fuzzing engines that try to
find new "interesting" inputs through coverage feedback and massive
