...
The system and session manager supports a large number of settings. These can generally
...
From a user's perspective, the third is a wrapper for the second. To implement a new setting,
it is necessary to support all three input methods:

1. `unit files` are parsed in `src/core/load-fragment.c`, with many simple and fixed-type
settings being parsed by common helpers, with the definition in the generator file
...
So that they are exercised by the fuzzing CI, new settings should also be listed in the
