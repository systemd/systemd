...
This document should be read together with [Writing Display Managers](https://systemd.io/WRITING_DISPLAY_MANAGERS) which focuses on the porting work necessary for display managers.
...
- If your session manager handles the special power, suspend, hibernate hardware keys or the laptop lid switch on its own it is welcome to do so,
  ...
  See [Inhibitor Locks](https://systemd.io/INHIBITOR_LOCKS) for further details on this.
- Before rebooting/powering-off/suspending/hibernating and when the operation is triggered by the user by clicking on some UI elements
  ...
  Use logind's ListInhibitors() call to get a list of these inhibitors. See [Inhibitor Locks](https://systemd.io/INHIBITOR_LOCKS) for further details on this.
