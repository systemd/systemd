...
We always aim to maintain backward compatiblity and never break user's systems.
We try to fix reported regressions.

Ocasionally we need to remove some interace or raise the minimum required version of a dependency.
...
We try to always preserve backward compatibility in programatic interfaces
...
Public shared libaries are exempt from this: it is important to us that they compile on non-Linux and even non-Unix platforms, even if they might just become NOPs.
