...
3. On Intel/AMD systems systemd's own reliance on the kernel entropy pool is
   minimal (due to the use of `GRND_INSECURE`).
...
2. Virtualized environments that lack both virtio-rng and have kernels without
   `GRND_INSECURE`, outside of test environments. Tough luck. Talk to your
   hosting provider, and ask them to fix this.
