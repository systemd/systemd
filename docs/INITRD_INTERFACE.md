...
* It's highly recommended that the initrd also mounts `/usr/` (if split off) as
  ...
  problems described in [Booting without /usr is Broken](https://systemd.io/SEPARATE_USR_IS_BROKEN).
...
* The switch-root operation will result in a killing spree of all running
  ...
  on [systemd and Storage Daemons for the Root File System](https://systemd.io/ROOT_STORAGE_DAEMONS).
