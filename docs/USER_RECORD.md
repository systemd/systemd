...
under. These are *alias* names only, the name in `userName` is always he
primary name. Typically, a user record that carries this field shall be
retrieavable and resolveable under every name listed here, pretty much
everywhere where the primary user name is allowed too. If logging in is
attempted via an alias name it should be normalized to the primary name.
