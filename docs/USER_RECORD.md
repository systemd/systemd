...
`selfModifiableFields` → An array of strings, each cooresponding to a field name that can appear
...
`selfModifiableBlobs` → Similar to `selfMoidifiableFields`, but it lists blobs that the user
is allowed to edit.
