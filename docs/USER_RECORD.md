...
`preferredSessionFile` → A string that indicates the user's preferred session desktop
entry file (i.e. `gnome`, `gnome-classic`, `plasma`, `kodi`, or others that appear in
`/usr/share/xsessions/` or `/usr/share/wayland-sessions/`). This should be used by the
display manager to pre-select the correct environment to log into.
...
`autoLogin`, `preferredSessionType`, `preferredSessionFile`, `stopDelayUSec`, `killProcesses`,
