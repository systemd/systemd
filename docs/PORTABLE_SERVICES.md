...
5. For each such units and drop-in files, if the kernel supports it, fsverity
   will be used to provide integrity protection. If a pkcs7 signature of the
   fsverity digest is available next to the unit, for example as
...
provide integrity protection and measurement in the IMA registry. Furthermore,
if the unit files are accompanied by a pkcs7 signature of the fsverity digest of
the unit, they will be automatically used to provide signed fsverity integrity
protection, again if the kernel supports it. For more information on fsverity,
on the required kernel versions, filesystems and kernel configuration, see the
