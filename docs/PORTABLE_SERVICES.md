...
2. The upper extensions images must contain an extension-release file in
...
4. The upper extensions images must contain at least one matching unit file
   each, with the right name prefix and suffix (see above).

5. As with the base/OS image, each upper extensions image must be a plain
