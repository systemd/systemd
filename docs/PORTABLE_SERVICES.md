...
metadata to the journal log entries. The parsed fields will be an ID between `IMAGE_ID`
and `ID` in this order of preference, and a version among `IMAGE_VERSION`, `VERSION_ID`,
and `BUILD_ID` in this order of preference. The ID and version, if any, are concatenated
