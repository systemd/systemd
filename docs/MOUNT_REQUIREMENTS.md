...
This is easy to miss: the accessing process simply sits on the autofs wait queue
while the queued mount job waits for the network, and the boot stalls.

A second, distinct failure mode shows up when the jobs are queued,
before anything ever accesses the path:
an automount below a network mount requires the parent network mount to be established first,
so if the parent mount depends on networking services ordered after `local-fs.target`,
the result is an ordering cycle.
Unlike the hang above, this one is reported,
and systemd breaks it by deleting jobs,
leaving some of the involved units unstarted.
...
Regardless of category, also avoid placing an automount below a network mount,
as that risks the ordering cycle described above
even for paths from category 3/regular.
...
Like the rest of `/usr/`, `/usr/local/` falls into category 1/initrd (see above), and
...
but anything the accessing services expect below the path is missing at that point,
