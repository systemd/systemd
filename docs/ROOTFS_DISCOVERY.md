...
1. A boot loader,
   ...
   boot. It takes care of enumerating all possible boot targets, potentially
   presenting it to the user in a menu, but otherwise picking an item
   automatically, implementing boot counting and automatic rollback if desired.

2. A unified kernel image ("UKI"), i.e. an UEFI PE executable that combines
   ...
   entries to generate for them. Within the UKI runtime the transition from the
   UEFI firmware world into the Linux world takes place, i.e. it encapsulates
   the fundamental `ExitBootServices()` UEFI call, that ends PC firmware
   control, and let's the Linux kernel take over.
...
2. The UKIs may either be placed in the ESP (below the `/EFI/Linux/`
   ...
   `.d/`, i.e. a UKI `foo.efi` has its sidecars in `foo.efi/`.
...
1. Many UEFI firmwares support HTTP(S) network boot (usually requires enabling in
   firmware setup). If this is available it permits downloading a disk image
   ...
   disk, and then processed much likey a regular disk: an ESP is searched for
...
2. UKIs can be placed on the same downloaded disk image, within the ESP. If
   ...
   Loader Specification Type #1 entry files in the ESP instead, which my carry
...
Much as for the the disk-based boot scheme described in the previous section
...
itself was downloaded from. Moreover the `initrd` of the UKI can automatically
downloads the `rootfs` from the same source itself was downloaded from.
...
1. The boot loader is typically authenticated by the firmware before invocation
   ...
   resources available the `systemd-boot` boot loader reads are not
   individually authenticated (i.e. `loader.conf` as well as Type #1 Boot
   Loader Specification entries). Because of this they can typically only be
   used in a very restricted fashion, i.e. configure some UI details as well as
   menu entries. Some options available are ignored if Secure Boot mode is
...
2. The UKIs are also authenticated by the firmware via UEFI Secure Boot, and so
   ...
   the kernel's built-in keyring. System credentials are authenticated via
...
3. Authentication of the `rootfs` and `usrfs` is more variable: depending on
   ...
   the UKI, or authenticated by signature prvoided to kernel, checked against
   the kernel's built-in keyring) or `dm-crypt`+`dm-integrity` (protected by
   TPM or user provided password/FIDO/PKCS#11), or in the network case at
   download time via detached signatures (currently only GPG) or via HTTPS
   certificate validation. Note that by default the automatic discovery
...
undesirable, particularly in a community Linux world. Moreover, because he
certficate list is very large its security value is limited: it mostly acts
more as denylist of known bad software rather than as allowlist of known good.
...
2, 3 described above). `shim` is primarily relevant for two reasons: it's adds
...
The `systemd-boot` boot loader also supports automatic enrollment of alernative
SecureBoot certificates: if the system is booted in the firmware-provided
Secure Boot "Setup Mode", it can automatically enroll certificates placed
inside the ESP into the firmware, replacing the Microsoft provided certificate
list. This mechanism massively enhances the security value of Secure Boot: you
can enroll your own certificates, ensuring that only the software you want
shall be allowed to be run on the system, in a very focussed way. However, do
note that this mechanism is only suitable if the hardware is known to be
compatible with this mechanism, because the Secure Boot certificate list is
also used to authenticate firmware extensions provided by certain extension
boards of PCs (for example graphics cards). Or in other words: replacing the
certficate list with your own might result in unbootable and even bricked
systems. Automatic enrolling of Secure Boot certficates is however a really
good option if the targeted harwdare is well known to be compatible with this,
...
updates. Morevoer, it's more compatible with a TOFU security model ("Trust on
...
Note that Secure Boot and Measured Boot are not exclusive to each other, the
...
* [`mkosi`](https://github.com/systemd/mkosi) is a higher level tool that can combines all of the above to build
  and sign basically all of the above from distribution packages, as needed:
  `initrd` trees, UKIs, USIs, `rootfs`/`usrfs`, whole DDIs, `sysext` images,
  `.tar` files to boot into and more.
