...
* SMBIOS type 1 (system information). The volatile "Wake-up Type" field is masked
  to zero before measuring, since it varies depending on how the machine was
...
plus trailing string set), with the type 1 "Wake-up Type" field masked out as
described above.
