...
request this with `MSTACK_DEFER_MOUNT` and then call `mstack_apply_bind_mounts()` themselves at the
right moment.
...
- **Assembly and attachment.** `mstack_make_mounts()` produces a detached root; `mstack_bind_mounts()`
  and `mstack_apply_bind_mounts()` attach it and the entries that sit on top.
