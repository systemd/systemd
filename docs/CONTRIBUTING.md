...
rules, an case-by-case exceptions might be applied at the discretion of the maintainers. The current set of build time
...
are runtime and build time graceful fallbacks (eg: daemon will not be built, runtime functionality will be skipped with
clear log message).
...
of the maintenance process. But as a generic rule, existing dependencies should not be moved to the bleeding edge without
very strong reasons. At a minimum, the current set of mainstream LTS distributions should be supported by the current main
branch for optional components. For core components, currently maintained LTS distributions versions should be supported.

## Kernel Requirements

Same principles as with other dependencies should be applied. It is fine to require newer kernel versions for additional functionality or optional features, but very strong reasons should be required for breaking compatibility for existing
...
In general, currently maintained LTS branches should keep being supported for existing functionality, especially for core
components.
...
always be avoided apart from the most extreme circumstances. For example, always prefer adding a new interface rather than
modifying the signature of an existing function. It is fine to mark an interface as deprecated to gently nudge users toward
a newer one, but in general support for the old one should be maintained whenever possible.
