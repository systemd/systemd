...
- An internal enum may be exposed as a simple string field instead of as a
  Varlink enum type when the field is output-only—that is, when it is never
  provided or controlled by the user. However, such fields should avoid using
  dashes to prevent breaking changes if they are later converted into enums
  (see below).
