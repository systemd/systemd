...
* The EFI configuration table entry, `LINUX_EFI_RANDOM_SEED_TABLE_GUID`
  ...
  `EfiACPIReclaimMemory`, and the boot loader uses it to pass an entropy seed
  read from the ESP to the OS, which is read and then cleared by the kernel
  before userspace starts. Its contents have the form:
  ```
