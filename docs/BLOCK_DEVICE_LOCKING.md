...
int lock_whole_disk_from_devname(const char *devname, int open_flags, int flock_operation) {
    ...
    if (r < 0) {
        fprintf(stderr, "Failed to create sd_device: %s\n", strerror(-r));
        return r;
    }
    ...
    if (r < 0) {
        fprintf(stderr, "Failed to get the subsystem: %s\n", strerror(-r));
        return r;
    }
    if (strcmp(subsystem, "block") != 0) {
        fprintf(stderr, "Ignore non-block device\n");
        return -EINVAL;
    }

    r = sd_device_get_devtype(dev, &devtype);
    if (r < 0) {
        fprintf(stderr, "Failed to get the devtype: %s\n", strerror(-r));
        return r;
    }
    if (strcmp(devtype, "disk") == 0)
        ...
        if (r < 0) {
            fprintf(stderr, "Failed to get the parent device: %s\n", strerror(-r));
            return r;
        }
    }
    ...
    if (r < 0) {
        fprintf(stderr, "Failed to get the whole disk name: %s\n", strerror(-r));
        return r;
    }
...
int main(int argc, char **argv) {
    ...
    __attribute__((cleanup(closep))) int fd = lock_whole_disk_from_devname(argv[1], O_RDONLY|O_CLOEXEC|O_NONBLOCK|O_NOCTTY, LOCK_EX|LOCK_NB);
