...
image for the host distro by default. Currently, mkosi v18 or newer is required.
First, run `mkosi genkey` to generate a key and certificate to be used for secure
boot and verity signing. After that is done, it is sufficient to type `mkosi` in the
...
$ mkosi -f qemu                   # build and boot up the test image in qemu
