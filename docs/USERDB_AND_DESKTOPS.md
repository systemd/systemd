...
so that `systemd-logind` knows that it is safe to activate a secure lock for this
user. This is done by setting the `$SYSTEMD_CAN_SECURE_LOCK` environment variable,
or alternatively by passing the `can-secure-lock=` argument to the
...
sessions report that they are compatible with secure locking via the
...
This will happen, for instance, if the user switches back to their display manager
and logs into a different account.
...
Desktop environments can manually activate a secure lock by calling
`SecureLock()` on the same interface. This should be done in response to
explicit user request (similar to the "lockdown" mode feature found in Android).
Note that any client calling `SecureLock()` from within the session will be
frozen before the call returns.

Applications running in the session may want to be notified about an imminent
secure lock. This is possible via `Inhibit()` and `PrepareForSecureLock()` on
the same interface. Subscribe to `PrepareForSecureLock()`, then call `Inhibit()`
with `what` set to `SD_LOGIND_USER_INHIBIT_SECURE_LOCK` and `delay` set to true.
When the signal is emitted, applications should wipe their own sensitive data
from memory (e.g. login session tokens, open documents, encryption keys, etc.)
to strengthen the security of the secure lock. Applications should also close
open FDs, if possible, to give the kernel a better chance at wiping the files'
contents out of memory. Applications should also temporarily stop listening for
signals emitted from system DBus services; since the applciation will be frozen
it won't be able to process the incoming events, and the broker may conclude
that the application is unresponsive and disconnect it from the bus. Once the
application has cleaned up its resources, it can drop the inhibitor FD to signal
that it's ready to be locked. Applications are notified that they're free to
re-acquire their inhibitor FDs via the `SecureUnlocked()` signal.
