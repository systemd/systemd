...
def main():
    ...
    for section in pe.sections:
        ...
        if file_addr % 512 != 0:
            print(f"File address of section {name} is not aligned to 512 bytes", file=sys.stderr)
            return 1

        if virt_addr % 512 != 0:
            print(f"Virt address of section {name} is not aligned to 512 bytes", file=sys.stderr)
            return 1
