...
def get_elf_dwarf_section_names(elf_path):
    try:
        readelf = subprocess.run(
            ['readelf', '--wide', '-S', elf_path], capture_output=True, text=True, check=True
        )
    except FileNotFoundError:
        print('readelf not found', file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    for section in pe.sections:
        ...
        if name in elf2efi.DWARF_SECTION_NAMES and not (
            section.Characteristics & elf2efi.IMAGE_SCN_MEM_DISCARDABLE
        ):
