...
# Functions to ignore during validation
...
def split_params(params_str):
    ...
    for part in parts:
        ...
        # Varargs
        if part == '...':
            params.append(Param(type='', name='', varargs=True))
            continue

        # Inline function pointer: type (*name)(...)
        fp_match = re.search(r'\(\s*\*\s*(\w+)\s*\)', part)
        if fp_match:
            pname = fp_match.group(1)
            params.append(Param(type=normalize_type(part), name=pname))
            continue

        # Extract array dimensions from the end
        arr_match = re.search(r'(\[[^\]]*\])+\s*$', part)
        arraydims = ''
        if arr_match:
        ...
        params.append(Param(type=ptype, name=pname, arraydims=arraydims))
...
def parse_headers(root):
    ...
    for header in header_files:
        ...
        text = re.sub(r'_SD_DECLARE_TRIVIAL_REF_UNREF_FUNC\(\s*(\w+)\s*\)',
                      r'\1* \1_ref(\1 *p); \1* \1_unref(\1 *p)', text)
        text = re.sub(r'_SD_DECLARE_TRIVIAL_REF_FUNC\(\s*(\w+)\s*\)',
                      r'\1* \1_ref(\1 *p)', text)
        text = re.sub(r'_SD_DECLARE_TRIVIAL_UNREF_FUNC\(\s*(\w+)\s*\)',
                      r'\1* \1_unref(\1 *p)', text)

        # Expand cleanup pointer macros
        text = re.sub(r'_SD_DEFINE_POINTER_CLEANUP_FUNC\(\s*(\w+)\s*,\s*(\w+)\s*\)',
                      r'void \2p(\1 **p)', text)
        ...
        for stmt in statements:
            ...
            m = re.match(r'^typedef\s+(?P<ret>.+?)\s*\(\s*\*\s*(?P<name>\w+)\s*\)\s*\((?P<params>.*)\)$', stmt)
            if m:
                name = m.group('name')
            ...
            m = re.match(r'^typedef\s+(\w+)\s+(\w+)$', stmt)
            if m:
            ...
            m = re.match(r'^(?P<ret>[A-Za-z_][\w\s\*]*?[\w\*])\s+(?P<name>\w+)\s*\((?P<params>.*)\)$', stmt)
            if m:
                name = m.group('name')
                ret = m.group('ret')
...
def parse_paramdef(pd):
    ...
    for i, param in enumerate(parameters):
        ...
        name_arr_match = re.match(r'^(\w+)(\[[^\]]*\])+$', name_text)
        if name_arr_match:
        ...
        tail = (param.tail or '')
        arr_match = re.match(r'^\s*(?P<arr>(\[[^\]]*\])*)\s*(,(?P<rest>.*))?$', tail, re.DOTALL)
        ...
        params.append(Param(type=ptype, name=name, arraydims=arraydims))
...
def parse_man(page):
    ...
    for fp in root.findall('.//funcprototype'):
        ...
        protos.append(ManProto(name=name, ret=ret, params=params, is_typedef=is_typedef, filename=filename))
...
def compare_params(man_params, header_params):
    ...
    if len(man_params) != len(header_params):
        diffs.append(f"parameter count: man={len(man_params)} header={len(header_params)}")
        return diffs

    for i, (mp, hp) in enumerate(zip(man_params, header_params)):
        if mp.varargs or hp.varargs:
            if not (mp.varargs and hp.varargs):
                diffs.append(f"param {i}: varargs mismatch")
            continue

        if mp.type != hp.type:
            diffs.append(f"param {i}: type mismatch: man='{mp.type}' header='{hp.type}'")
        if mp.name != hp.name:
            diffs.append(f"param {i}: name mismatch: man='{mp.name}' header='{hp.name}'")
        if mp.arraydims != hp.arraydims:
            diffs.append(f"param {i}: array dims mismatch: man='{mp.arraydims}' header='{hp.arraydims}'")
...
def compare_params_with_macro(man_params, macro_param_names):
    ...
    for mp in man_params:
        if mp.varargs:
            ...
            man_names.append(mp.name)

    if len(man_names) != len(macro_param_names):
        diffs.append(f"argument count: man={len(man_names)} macro={len(macro_param_names)}")
        return diffs

    for i, (man_name, macro_name) in enumerate(zip(man_names, macro_param_names)):
        if man_name != macro_name:
            diffs.append(f"argument {i}: name mismatch: man='{man_name}' macro='{macro_name}'")
...
def main():
    ...
    for page in sys.argv[1:]:
        ...
        for mp in man_protos:
            if mp.name in IGNORE_FUNCTIONS:
                continue
            ...
            errors.setdefault(mp.filename, []).append(
                f"{mp.name}: not found in headers or macros"
            )
