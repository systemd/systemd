...
# and populate the directory the script uses to generate seed corpora
rm -rf test/fuzz/fuzz-bcd/
mkdir -p test/fuzz/fuzz-bcd/

# shellcheck disable=SC2016
# Single quotes are used here intentionally to let sh expand $
# when it's run by xargs
find "test/test-bcd" -type f -name '*.zst' -print0 |
xargs -0 -I%%FILE%% sh -x -c 'unzstd %%FILE%% -o "test/fuzz/fuzz-bcd/$(basename %%FILE%% .zst)"'
