...
def construct_path(element):
    ...
    while element is not None:
        ...
        if tag == "varlistentry":
            ...
            path.append(f'{tag}[term="{text}"]')
        elif element.tag.startswith("refsect"):
            text = "".join(element.find("title").itertext())
            path.append(f'{tag}[title="{text}"]')
        elif element.tag == "refentry":
            path.append("")
            break
        elif element.tag == "variablelist":
            varlists = element.getparent().findall(element.tag)
            if len(varlists) > 1:
                path.append(f"{element.tag}[{varlists.index(element)+1}]")
            else:
                path.append(element.tag)
        else:
            assert tag in ["para", "listitem"]
            path.append(tag)
