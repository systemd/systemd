...
def bpf_skel_gen(bpftool_exec, in_file, out_fd):
...
def bpf_build(args):
    ...
    #with tempfile.NamedTemporaryFile(mode='rb', suffix='.o') as clang_out_fd, \
    with open('.'.join([pathlib.Path(args.bpf_src_c).stem, 'o']), mode='w') as clang_out_fd, \
        ...
        bpf_skel_gen(bpftool_exec=args.bpftool_exec,
