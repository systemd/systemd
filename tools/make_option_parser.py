...
#   // help: A help string for --help
#   // help: with a second line if needed
#   // group: <group_name>
#
# The help strings are concatenated (if mode than one help: line is present)
# and wrapped to 80 columns.
#
# The required or optional argument must be specified for all options. (This is
# different then the shorthand notation we use in --help). Thus:
#
    ...
    def new(enum: str,
        ...
        if enum.startswith('OPTION_SHORT_'):
            if len(enum) != len('OPTION_SHORT_X'):
                raise ValueError(f'Bad enum name {enum!r}')
            spec = '-' + enum[-1]
        else:
...
def parse_input(lines: list[str]) -> tuple[list[Option], Globals]:
    ...
    while n < len(lines) or state != InputState.other:
        ...
        if state == InputState.other:
            m = re.match(r'^\s*case (?P<enum>OPTION_.+):(?:\s*{)?$', line)
            if m:
                enum = m.group('enum')
                print(f'// found {enum}')
            ...
            m = re.match(r'\s*// group: (?P<group>.*)', line)
            if m:
