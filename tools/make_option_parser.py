...
class Option:
    ...
    def new(enum: str,
        ...
        spec = ('-' if len(enum) == 1 else '--') + enum.lower().replace('_', '-')
    ...
    def _insert_defaults(self) -> None:
        ...
        if _argtype is None:
            _argtype = ArgType.no_argument

        if not self.help:
            self.help = _help
        if _sn and not self.short_name():
            self.names += [_sn]
            assert self.short_name() == _sn[1:]
        if not self.argtype:
            self.argtype = _argtype
        if _metavar and not self.metavar:
            self.metavar = _metavar
...
def generate_lines(options: list[Option], globals: Globals) -> Generator[str, None, None]:
    ...
    for num, option in enumerate(options, start=0x100):
        sn = option.short_name()
        value = f"'{sn}'" if sn else f"0x{num:x}"
        yield f'\tOPTION_{option.enum:{opt_enums_width}} = {value},'
    yield '};'
    yield ''
...
def parse_input(lines: list[str]) -> tuple[list[Option], Globals]:
    ...
    while n < len(lines) or state != InputState.other:
        ...
        if state == InputState.other:
            m = re.match(r'^\s*case OPTION_(?P<enum>.+):$', line)
            if m:
                enum = m.group('enum')
                print(f'// found OPTION {enum}')
            ...
            m = re.match(r'\s*// option: (?P<specs>.*)', line)
            if m:
            ...
            m = re.match(r'\s*// help: (?P<help>.*)', line)
            if m:
                help += [m.group('help')]
