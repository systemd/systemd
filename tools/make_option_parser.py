...
def generate_lines(globals: Globals, options: list[Option], verbs: list[Verb]) -> Iterator[str]:
    ...
    verb_names_width = max((len(verb.name) for verb in verbs), default=0)
    verb_maxargs_width = max((len(str(verb.maxargs)) for verb in verbs), default=0)
    verb_functions_width = max((len(verb.function) for verb in verbs), default=0)
...
def parse_input(lines: list[str]) -> tuple[Globals, list[Option], list[Verb]]:
    ...
    while n < len(lines) or state != InputState.other:
        ...
        if 'static const Verb verbs' in line:   # do not generate verbs in unconverted files
            skip_verbs = True
        ...
        if state == InputState.other:
            ...
            if m := re.match(fr'\s*// {kw}: (?P<specs>.*)', line):
                ...
                n -= 1  # process this line again looking for directives
            ...
            if m := re.match(r'\s*// help: (?P<help>.*)', line):
                ...
                state = InputState.other
                n -= 1  # process this line again

        n += 1

    return globals, options, ([] if skip_verbs else verbs)
