...
if [ "${2:-}" != "-n" ]; then (
    ...
    curl -A 'CustomUserAgentString' --fail -L -o pnp_id_registry.csv 'https://uefi.org/uefi-pnp-export'
    curl -A 'CustomUserAgentString' --fail -L -o acpi_id_registry.csv 'https://uefi.org/uefi-acpi-export'
) fi
