...
NT_GNU_BUILD_ID = 3
POINTER_TO_RAW_DATA_UNRESOLVED = 0
...
def read_elf_build_id(file: elffile.ELFFile) -> bytes:
    section = file.get_section_by_name('.note.gnu.build-id')
    if section is None:
        raise BadSectionError('ELF file has no .note.gnu.build-id section')

    note = section.data()
    n_namesz, n_descsz, n_type = struct.unpack_from('<III', note)
    owner_offset = struct.calcsize('<III')
    ...
    if n_type != NT_GNU_BUILD_ID or owner.rstrip(b'\0') != b'GNU':
        raise BadSectionError('.note.gnu.build-id is not a GNU build-id note')
    if len(build_id) != 16:
        raise BadSectionError(
            f'ELF build-id is {len(build_id)} bytes, expected 16 (link with --build-id=md5)'
        )
...
def add_codeview_section(opt: PeOptionalHeader, cv_signature: bytes, sections: list[PeSection]) -> PeSection:
    ...
    debug_dir_entry.Type = PE_IMAGE_DEBUG_TYPE_CODEVIEW
    ...
    debug_dir_entry.PointerToRawData = POINTER_TO_RAW_DATA_UNRESOLVED
...
def convert_elf_relocations(
    ...
    opt.SizeOfHeaders = align_to(
        ...
        + sizeof(PeCoffHeader)
        + sizeof(opt)
        + sizeof(PeSection) * max(len(sections) + 2, minimum_sections),
        FILE_ALIGNMENT,
    )
...
def resolve_raw_data_pointers(codeview_section: PeSection) -> None:
    entry = PeDebugDirectoryEntry.from_buffer(codeview_section.data)
    entry.PointerToRawData = codeview_section.PointerToRawData + sizeof(PeDebugDirectoryEntry)


def write_pe_data(file: typing.IO[bytes], sections: list[PeSection], offset: int, trailer: bytes) -> None:
    for pe_s in sections:
    ...
    file.truncate(offset + len(trailer))
...
def elf2efi(args: argparse.Namespace) -> None:
    ...
    copy_sections_by_name(
        file, opt, args.copy_sections.split(',') + DWARF_SECTION_NAMES, sections, coff_string_table
    )
    ...
    offset = write_pe_headers(args.PE, coff, opt, sections)
    resolve_raw_data_pointers(codeview_section)
    if coff_string_table:
        coff.PointerToSymbolTable = offset
        args.PE.seek(PE_OFFSET + len(PE_MAGIC), io.SEEK_SET)
        args.PE.write(coff)
    write_pe_data(args.PE, sections, offset, bytes(coff_string_table))
