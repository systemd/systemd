...
def read_elf_build_id(file: elffile.ELFFile) -> typing.Optional[bytes]:
    ...
    if section is None:
        print('elf2efi.py: no .note.gnu.build-id section, not embedding a CodeView GUID', file=sys.stderr)
        return None
    ...
    if n_type != NT_GNU_BUILD_ID or owner.rstrip(b'\0') != b'GNU':
        print('elf2efi.py: .note.gnu.build-id is not a GNU build-id note, not embedding a CodeView GUID', file=sys.stderr)
        return None
    if len(build_id) != 16:
        print(
            f'elf2efi.py: ELF build-id is {len(build_id)} bytes, expected 16 (link with --build-id=md5), '
            f'not embedding a CodeView GUID',
            file=sys.stderr,
        )
        return None
...
def convert_elf_relocations(
    ...
    opt.SizeOfHeaders = align_to(
        ...
        + sizeof(PeCoffHeader)
        + sizeof(opt)
        + sizeof(PeSection) * (len(sections) + 2 + minimum_sections),
        FILE_ALIGNMENT,
    )
...
def calculate_section_offsets(opt: PeOptionalHeader, sections: list[PeSection]) -> int:
...
def elf2efi(args: argparse.Namespace) -> None:
    ...
    trailer_offset = calculate_section_offsets(opt, sections)
