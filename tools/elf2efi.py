...
def read_elf_build_id(file: elffile.ELFFile) -> typing.Optional[bytes]:
    ...
    if n_type != NT_GNU_BUILD_ID or owner.rstrip(b'\0') != b'GNU':
        print('WARNING: .note.gnu.build-id is not a GNU build-id note', file=sys.stderr)
        return None
    if len(build_id) != 16:
        print(
...
def convert_elf_relocations(
    ...
    n_reserved_sections = len(sections) + 1 + extra_sections + (1 if reserve_codeview_section else 0)
...
def elf2efi(args: argparse.Namespace) -> None:
    ...
    pe_reloc_s = convert_elf_relocations(file, opt, sections, args.extra_sections, build_id is not None)
