...
apt-get install -y tree libpwquality-dev rpm libcurl4-openssl-dev
...
for phase in "${PHASES[@]}"; do
    case "$phase" in
        SETUP)
            ...
            [ -d "$AUTOPKGTEST_DIR" ] || git clone --quiet --branch=test --depth=1 https://salsa.debian.org/bluca/autopkgtest.git "$AUTOPKGTEST_DIR"
