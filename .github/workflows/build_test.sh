...
ARGS=(
    ...
    "--optimization=2"
    "--optimization=s"
    $'--optimization=3 -Db_lto=true -Drust_args="-Clinker-plugin-lto -Copt-level=3"'
...
for args in "${ARGS[@]}"; do
    ...
    if ! AR="$AR" CC="$CC" CXX="$CXX" CFLAGS="-Werror" CXXFLAGS="-Werror" RUSTFLAGS="--deny warnings" meson -Dtests=unsafe -Dslow-tests=true -Dfuzz-tests=true --werror ${RUST} $args build; then
