...
for args in "${ARGS[@]}"; do
    ...
    if ! AR="$AR" CC="$CC" CXX="$CXX" CFLAGS="-Werror" CXXFLAGS="-Werror" meson -Dtests=unsafe -Dslow-tests=true -Dfuzz-tests=true -Ddns-over-tls=openssl --werror $args build; then
