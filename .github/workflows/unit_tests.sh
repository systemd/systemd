...
for phase in "${PHASES[@]}"; do
    case $phase in
        ...
        RUN|RUN_GCC|RUN_GCC_RUST|RUN_CLANG|RUN_CLANG_RUST)
            ...
            ;;
        RUN_ASAN_UBSAN|RUN_GCC_ASAN_UBSAN|RUN_CLANG_ASAN_UBSAN|RUN_CLANG_ASAN_RUST)
            MESON_ARGS=(--optimization=1)

            if [[ "$phase" = "RUN_CLANG_ASAN_UBSAN" ]] || [[ "$phase" = "RUN_CLANG_ASAN_RUST" ]]; then
                export CC=clang
                export CXX=clang++
            ...
            if [[ "$phase" = "RUN_CLANG_ASAN_RUST" ]]; then
                MESON_ARGS=(-Dbuild-rust=true)
            fi
            RUSTFLAGS="--deny warnings -Zsanitizer=address" meson --werror -Dtests=unsafe -Db_sanitize=address,undefined "${MESON_ARGS[@]}" build
            ninja -C build -v
