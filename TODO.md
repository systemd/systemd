...
- Maybe introduce an InodeRef structure inspired by PidRef, which references a
  ...
  only one, once the other and sometimes both. It would make the code a lot
  simpler if we could pass both around at the same time in a simple way, via an
...
- pam_systemd_home: add module parameter to control whether to only accept
  only password or only pkcs11/fido2 auth, and then use this to hook nicely
