...
static int notify(const char *message) {
  ...
  if (message_length == 0) {
    fprintf(stderr, "Message to send to $NOTIFY_SOCKET is empty\n");
    return -EINVAL;
  }
  ...
  if (path_length >= sizeof( socket_addr.sun.sun_path)) {
    fprintf(stderr, "$NOTIFY_SOCKET contains a path that is too long for AF_UNIX\n");
    return -E2BIG;
  }

  memcpy(socket_addr.sun.sun_path, socket_path, path_length);

  /* Support for abstrace socket */
  if (socket_addr.sun.sun_path[0] == '@')
    socket_addr.sun.sun_path[0] = 0;

  fd = socket(AF_UNIX, SOCK_DGRAM|SOCK_CLOEXEC, 0);
  if (fd < 0) {
    fprintf(stderr, "Failed to open $NOTIFY_SOCKET\n");
    return -errno;
  }

  if (connect(fd, &socket_addr.sa, sizeof(socket_addr)) != 0) {
    fprintf(stderr, "Failed to connect to $NOTIFY_SOCKET\n");
    return -errno;
  }

  ssize_t written = write(fd, message, message_length);
  if (written != (ssize_t) message_length) {
    fprintf(stderr, "Failed to write message to $NOTIFY_SOCKET\n");
...
static int notify_reloading() {
  ...
  if (ts.tv_sec < 0 || ts.tv_nsec < 0 ||
    ...
    fprintf(stderr, "Sanity checks on timestamp failed\n");
    return -EINVAL;
  }
  ...
  if (snprintf(reload_message, sizeof(reload_message), "RELOADING=1\nMONOTONIC_USEC=%" PRIu64, now) < 0) {
    fprintf(stderr, "Failed to format reload message\n");
    return -EINVAL;
  }
...
int main(int argc, char **argv) {
  ...
  r = notify_ready();
  if (r < 0) {
    fprintf(stderr, "Failed to notify readiness to $NOTIFY_SOCKET\n");
    return r;
  }
  ...
  while (!terminating) {
    if (reloading) {
      ...
      r = notify_reloading();
      if (r < 0) {
        fprintf(stderr, "Failed to notify reloading to $NOTIFY_SOCKET\n");
        return r;
      }
