 ...
 *
 * Compile with 'cc logcontrol-example.c -lsystemd'
 *
 * To get and set properties via busctl:
 *
 * $ busctl --user get-property org.freedesktop.Example /org/freedesktop/LogControl1 org.freedesktop.LogControl1 SyslogIdentifier
 *   s "example"
 * $ busctl --user get-property org.freedesktop.Example /org/freedesktop/LogControl1 org.freedesktop.LogControl1 LogTarget
 *   s "journal"
 * $ busctl --user get-property org.freedesktop.Example /org/freedesktop/LogControl1 org.freedesktop.LogControl1 LogLevel
 *   s "info"
...
#define check(log_level, x) ({                  \
  int _r = (x);                                 \
  errno = _r < 0 ? -_r : 0;                     \
  sd_journal_print((log_level), #x ": %m", _r); \
  if (_r < 0)                                   \
    return EXIT_FAILURE;                        \
  })
...
int main(int argc, char **argv) {
  /* The bus should be relinquished once finished. The cleanup attribute
   * allows to do it nice and cleanly whenever we exit the block. */
  _cleanup_(sd_bus_flush_close_unrefp) sd_bus *bus = NULL;

  /* Acquire a connection to the bus, letting the library figure out the details.
  ...
  /* Publish an interface on the bus, specifying our well-known object access path
   * and public interface name.
   * https://www.freedesktop.org/software/systemd/man/sd_bus_add_object.html
  ...
  /* By default the service is assigned an ephemeral name. Also add a fixed one,
   * so that clients know who to call.
