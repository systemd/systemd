...
static const char* const secure_boot_variable_table[_SECURE_BOOT_VARIABLE_MAX] = {
        [SECURE_BOOT_VARIABLE_PK]  = "pk",
        [SECURE_BOOT_VARIABLE_KEK] = "kek",
        [SECURE_BOOT_VARIABLE_DB]  = "db",
        [SECURE_BOOT_VARIABLE_DBX] = "dbx",
        [SECURE_BOOT_VARIABLE_DBT] = "dbt",
        [SECURE_BOOT_VARIABLE_DBR] = "dbr",
};
...
static int collect_applied_signature_records(
        ...
        r = validate_signature_list(database, database_size);
        if (r < 0)
                return r;

        r = validate_signature_list(update, update_size);
        if (r < 0)
                return r;
...
static int recover_secure_boot_update_backups(void) {
        _cleanup_closedir_ DIR *dir = NULL;
...
static int plan_secure_boot_update(
        ...
        if (r < 0) {
                ...
                plan->authority.fallback = variable != SECURE_BOOT_VARIABLE_KEK;
        } else {
...
static int vl_method_prepare_secure_boot_update(
        ...
        sd_json_variant *data_parameter;
        ...
        data_parameter = sd_json_variant_by_key(parameters, "data");
        if (IN_SET(p.variable, SECURE_BOOT_VARIABLE_DB, SECURE_BOOT_VARIABLE_DBX) &&
            (!data_parameter || sd_json_variant_is_null(data_parameter)))
                return sd_varlink_error_invalid_parameter_name(link, "data");

        r = recover_secure_boot_update_backups();
        if (r < 0)
                return r;
