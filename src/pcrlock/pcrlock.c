...
#define PCRLOCK_PE_INPUT_MAX                (256U * U64_MB)
...
static int acquire_stdin_pe_fd(void) {
        ...
        if (S_ISREG(st.st_mode)) {
                if (st.st_size < 0 || (uint64_t) st.st_size > PCRLOCK_PE_INPUT_MAX)
                        return log_error_errno(SYNTHETIC_ERRNO(EFBIG),
                                               "PE binary from stdin is larger than the maximum supported size of %s.",
                                               FORMAT_BYTES(PCRLOCK_PE_INPUT_MAX));

                /* Regular stdin is already seekable. Keep it directly to avoid copying large redirected
                 * PE/UKI files before hashing them. */
        ...
        _cleanup_close_ int ro_fd = fd_reopen(fd, O_RDONLY|O_CLOEXEC);
        if (ro_fd < 0)
                return log_error_errno(ro_fd, "Failed to reopen PE binary temporary file: %m");

        return TAKE_FD(ro_fd);
}

static int acquire_pe_fd(int argc, char *argv[]) {
        assert(argv);
