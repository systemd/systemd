...
static int vl_method_lock(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        static const sd_json_dispatch_field dispatch_table[] = {
                ...
                { "unlock",   SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_stdbool, offsetof(MethodLockParameters, unlock),   0                 },
                {}
        };
        ...
        case LOCK_CATEGORY_FIRMWARE_CODE:
                r = p.unlock ? unlock_firmware(/* is_code= */ true) : lock_firmware(/* is_code= */ true);
                break;

        case LOCK_CATEGORY_FIRMWARE_CONFIG:
                r = p.unlock ? unlock_firmware(/* is_code= */ false) : lock_firmware(/* is_code= */ false);
                break;

        case LOCK_CATEGORY_SECUREBOOT_POLICY:
                r = p.unlock ? unlink_pcrlock(PCRLOCK_SECUREBOOT_POLICY_PATH) : lock_secureboot_policy();
                break;

        case LOCK_CATEGORY_SECUREBOOT_AUTHORITY:
                r = p.unlock ? unlink_pcrlock(PCRLOCK_SECUREBOOT_AUTHORITY_PATH) : lock_secureboot_authority();
                break;
