...
static int collect_applied_authority_records(
        ...
        FOREACH_ARRAY(entry, applied, n_applied) {
                bool supported = efi_guid_equal(&entry->type, &x509) ||
                        efi_guid_equal(&entry->type, &sha256);
                ...
                SignatureRecord signature_record = {
                        .type = entry->type,
                        .data = memdup(entry->data, entry->size),
                        .size = entry->size,
                        .typed = true,
                        .from_db = true,
                };
...
static int plan_secure_boot_update(
        ...
        r = efi_signature_list_validate(current.iov_base, current.iov_len);
        if (r < 0)
