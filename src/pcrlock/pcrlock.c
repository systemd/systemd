...
static bool signature_in_database(
        ...
        while (offset < database_size) {
                ...
                if (efi_guid_equal(&list->SignatureType, type) && list_signature_size == signature_size) {
...
static int collect_applied_signature_records(
        ...
        while (offset < update_size) {
                ...
                bool supported = efi_guid_equal(&list->SignatureType, &x509) ||
                        efi_guid_equal(&list->SignatureType, &sha256);
...
static int load_secureboot_authorities(
        ...
        FOREACH_ARRAY(rr, el->records, el->n_records) {
                ...
                if (vdata->variableDataLength == offsetof(EFI_SIGNATURE_DATA, SignatureData) + SHA256_DIGEST_SIZE)
...
typedef enum SecureBootUpdateOutcome {
        SECURE_BOOT_UPDATE_NOT_MANAGED,
        SECURE_BOOT_UPDATE_UNCHANGED,
        SECURE_BOOT_UPDATE_PREDICTED,
        SECURE_BOOT_UPDATE_FALLBACK,
        _SECURE_BOOT_UPDATE_OUTCOME_MAX,
        _SECURE_BOOT_UPDATE_OUTCOME_INVALID = -EINVAL,
} SecureBootUpdateOutcome;

static const char* const secure_boot_update_outcome_table[_SECURE_BOOT_UPDATE_OUTCOME_MAX] = {
        [SECURE_BOOT_UPDATE_NOT_MANAGED] = "notManaged",
        [SECURE_BOOT_UPDATE_UNCHANGED]   = "unchanged",
        [SECURE_BOOT_UPDATE_PREDICTED]   = "predicted",
        [SECURE_BOOT_UPDATE_FALLBACK]    = "fallback",
};

DEFINE_PRIVATE_STRING_TABLE_LOOKUP_TO_STRING(secure_boot_update_outcome, SecureBootUpdateOutcome);
...
static int write_secure_boot_update_predictions(
        ...
        if (plan->authority.active &&
            ...
            !plan->authority.fallback) {
                ...
                plan->authority.predicted = !strv_isempty(state->authority_paths);
        }

        return 0;
}

static SecureBootUpdateOutcome secure_boot_update_component_outcome(const SecureBootUpdateComponentPlan *component) {
        assert(component);

        if (!component->active)
                return SECURE_BOOT_UPDATE_NOT_MANAGED;
        if (component->fallback)
                return SECURE_BOOT_UPDATE_FALLBACK;
        if (component->predicted)
                return SECURE_BOOT_UPDATE_PREDICTED;

        return SECURE_BOOT_UPDATE_UNCHANGED;
}
        ...
        *state = (SecureBootUpdateState) {
        ...
        return sd_varlink_replybo(
                        ...
                        SD_JSON_BUILD_PAIR_STRING(
                                        "policy",
                                        secure_boot_update_outcome_to_string(
                                                        secure_boot_update_component_outcome(&plan.policy))),
                        SD_JSON_BUILD_PAIR_STRING(
                                        "authority",
                                        secure_boot_update_outcome_to_string(
                                                        secure_boot_update_component_outcome(&plan.authority))));
}
