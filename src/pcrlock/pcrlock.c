...
static int verb_lock_raw(int argc, char *argv[], void *userdata) {
        ...
        r = read_full_stream_full(f ?: stdin, NULL, UINT64_MAX, SIZE_MAX, READ_FULL_FILE_IGNORE_READ_FULL_BYTES_MAX, &data, &size);
        if (r < 0)
...
static int verb_lock_kernel_initrd(int argc, char *argv[], void *userdata) {
        ...
        r = read_full_stream_full(f ?: stdin, NULL, UINT64_MAX, SIZE_MAX, READ_FULL_FILE_IGNORE_READ_FULL_BYTES_MAX, (char**) &data, &size);
         if (r < 0)
                return log_error_errno(r, "Failed to read data from %s: %m", f ? argv[1] : "stdin");
