...
static int validate_policy_outcome_count(uint32_t *ret_pcr_mask) {
        ...
        return tpm2_calculate_policy_super_pcr(&prediction, event_log->primary_algorithm, &digest);
}
...
/* Check a non-owning array for an equivalent authority record. */
static bool signature_record_pointer_seen(
                const SignatureRecord *const *records,
                size_t n_records,
                const SignatureRecord *candidate) {
...
static int write_authority_predictions(
        ...
        size_t n_current_records = 0, n_from_db = 0, n_unique = 0;
        bool found_non_db = false;
        int r;
        ...
        for (size_t i = 0; i < n_new_records; i++) {
                if (signature_record_seen(current_records, n_current_records, new_records + i) ||
                    signature_record_pointer_seen(unique, n_unique, new_records + i))
                        continue;
        ...
        /* A newly used db authority may be inserted at any firmware-authority position. */
        for (size_t insertion = 0; insertion <= n_from_db; insertion++) {
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *array = NULL;
        ...
        for (size_t replacement = 0; replacement < n_from_db; replacement++) {
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *array = NULL;
...
static int backup_component_files(
        ...
        if (asprintf(&backup,
                     ...
                     SD_ID128_FORMAT_VAL(id), component, regenerate ? "regenerate" : "no-regenerate") < 0)
                return -ENOMEM;
        ...
        if (remove_source) {
                r = remove_component_files(source);
                if (r < 0)
                        goto restore;
        }
...
restore:
        k = restore_component_files(backup, source);
        if (k < 0)
...
static int remove_component_files_for_update(
        ...
        if (*backup)
                return remove_component_files(source);
...
static int recover_secure_boot_update_backups(bool regenerate) {
        ...
        FOREACH_DIRENT(de, dir, return -errno) {
                ...
                const char *destination, *suffix;
                bool recovered_regenerate = false;
                sd_id128_t id;

                suffix = endswith(de->d_name, ".policy.regenerate");
                if (suffix) {
                        destination = PCRLOCK_SECUREBOOT_POLICY_DIR;
                        recovered_regenerate = true;
                } else if ((suffix = endswith(de->d_name, ".policy.no-regenerate")))
                        destination = PCRLOCK_SECUREBOOT_POLICY_DIR;
                else if ((suffix = endswith(de->d_name, ".authority.regenerate"))) {
                        destination = PCRLOCK_SECUREBOOT_AUTHORITY_DIR;
                        recovered_regenerate = true;
                } else if ((suffix = endswith(de->d_name, ".authority.no-regenerate")))
                        destination = PCRLOCK_SECUREBOOT_AUTHORITY_DIR;
                else if ((suffix = endswith(de->d_name, ".policy")))
                        destination = PCRLOCK_SECUREBOOT_POLICY_DIR;
                else if ((suffix = endswith(de->d_name, ".authority")))
                        destination = PCRLOCK_SECUREBOOT_AUTHORITY_DIR;
                else
                        continue;
                ...
                r = restore_component_files(backup, destination);
                if (r < 0)
        ...
        if (reseal) {
                r = make_policy(
                                ...
                                /* ret_pcr_mask= */ NULL);
                if (r < 0)
                        RET_GATHER(ret, log_error_errno(r, "Failed to reseal the TPM PCR lock policy after recovering an interrupted Secure Boot update: %m"));
        }
...
static int apply_secure_boot_update(
        ...
        for (;;) {
                ...
                if (plan->authority.fallback && !plan->authority.fallback_applied) {
                        r = remove_component_files_for_update(
                                        PCRLOCK_SECUREBOOT_AUTHORITY_DIR,
                                        update_id,
                                        "authority",
                                        state->regenerate,
...
static void rollback_secure_boot_update(SecureBootUpdateState *state) {
        ...
        if (state->regenerate && changed) {
                k = make_policy(
                                ...
                                /* ret_pcr_mask= */ NULL);
                if (k < 0)
                        log_error_errno(k, "Failed to restore the TPM PCR lock policy after a Secure Boot update failure: %m");
        }
