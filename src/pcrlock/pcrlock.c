...
static bool secure_boot_prediction_file_is_owned(const char *name) {
        assert(name);
...
static bool secure_boot_prediction_should_remove(const char *name, void *userdata) {
        ...
        return secure_boot_prediction_file_is_owned(name) &&
...
static bool secure_boot_prediction_matches_transaction(const char *name, void *userdata) {
        ...
        return secure_boot_prediction_file_is_owned(name) && strstr(name, transaction_token);
}
...
static int calculate_policy_prediction(EventLog **ret_event_log, Tpm2PCRPrediction *prediction) {
...
static int validate_policy_outcome_count(void) {
        _cleanup_(event_log_freep) EventLog *event_log = NULL;
        ...
        r = calculate_policy_prediction(&event_log, &prediction);
        if (r < 0)
                return r;
...
static int make_policy(bool force, RecoveryPinMode recovery_pin_mode) {
        ...
        _cleanup_(event_log_freep) EventLog *el = NULL;
        r = calculate_policy_prediction(&el, &new_prediction);
        if (r < 0)
                return r;
...
static int directory_has_file_matching(const char *directory, bool (*predicate)(const char *name)) {
        _cleanup_closedir_ DIR *dir = NULL;
...
static int secure_boot_predictions_exist(const char *directory) {
        return directory_has_file_matching(directory, secure_boot_prediction_file_is_owned);
}
...
static int load_secureboot_authorities(
        ...
        FOREACH_ARRAY(rr, el->records, el->n_records) {
                ...
                vdata = rec->firmware_payload;
                data_offset = offsetof(UEFI_VARIABLE_DATA, unicodeName) + vdata->unicodeNameLength * 2;
                bool direct_hash = vdata->variableDataLength ==
                        offsetof(EFI_SIGNATURE_DATA, SignatureData) + SHA256_DIGEST_SIZE;
                if (direct_hash)
                ...
                SignatureRecord signature_record = {
                        .type = direct_hash ? sha256 : x509,
                        .data = memdup((const uint8_t*) rec->firmware_payload + data_offset, vdata->variableDataLength),
                        .size = vdata->variableDataLength,
                        .from_db = sd_id128_equal(variable_vendor, EFI_VENDOR_DATABASE) &&
...
static bool signature_record_equal(const SignatureRecord *a, const SignatureRecord *b) {
        return efi_guid_equal(&a->type, &b->type) &&
                a->from_db == b->from_db &&
                a->size == b->size &&
...
static int write_authority_predictions(
        ...
        /* A new db entry can replace any current db authority even if the old entry remains enrolled. */
        ...
        if (has_direct_hash || efi_guid_equal(&unique[0]->type, &sha256) || n_unique > 1) {
                *ret_fallback = true;
                *ret_paths = NULL;
                return 0;
        }
        ...
        /* Firmware db authorities precede authorities emitted by later boot stages. */
...
static bool secure_boot_component_file_is_owned(const char *name) {
        ...
        return streq(name, "generated.pcrlock") ||
                secure_boot_prediction_file_is_owned(name);
}

static bool secure_boot_component_file_should_remove(const char *name, void *userdata) {
        return secure_boot_component_file_is_owned(name);
}
...
static int remove_component_files(const char *directory) {
        return unlink_matching_files(directory, secure_boot_component_file_should_remove, NULL);
}
...
static int component_backup_has_files(const char *backup) {
        return directory_has_file_matching(backup, secure_boot_component_file_is_owned);
}
...
static int backup_component_files(
        ...
        if (asprintf(&backup,
                     PCRLOCK_SECUREBOOT_UPDATE_DIR "/" SD_ID128_FORMAT_STR ".%s",
                     SD_ID128_FORMAT_VAL(id), component) < 0)
                return -ENOMEM;
...
static int remove_component_files_for_update(
        ...
        return backup_component_files(source, id, component, /* remove_source= */ true, backup);
}
...
static int recover_secure_boot_update_backups(bool regenerate) {
        ...
        FOREACH_DIRENT(de, dir, return -errno) {
                ...
                suffix = endswith(de->d_name, ".policy");
                if (suffix)
                        destination = PCRLOCK_SECUREBOOT_POLICY_DIR;
                else {
                        suffix = endswith(de->d_name, ".authority");
                        if (!suffix)
                                continue;
...
static int apply_secure_boot_update(
        ...
        for (;;) {
                ...
                r = state->regenerate ?
                        make_policy(/* force= */ false, /* recovery_pin_mode= */ RECOVERY_PIN_HIDE) :
                        validate_policy_outcome_count();
                if (r != -E2BIG)
                        return r;
...
static void rollback_secure_boot_update(SecureBootUpdateState *state) {
        ...
        if (state->regenerate && changed) {
                k = make_policy(/* force= */ false, /* recovery_pin_mode= */ RECOVERY_PIN_HIDE);
                if (k < 0)
