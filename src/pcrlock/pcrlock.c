 ...
 *
 * pre-merge:
 *    0. make signed PCR work together with pcrlock
 *    1. add integration + unit tests
...
static int add_algorithm_columns(
        ...
        for (size_t i = 0; i < el->n_algorithms; i++) {
                ...
                if (FLAGS_SET(arg_json_format_flags, JSON_FORMAT_OFF) &&
                    ...
                    el->algorithms[i] != el->primary_algorithm)
...
static int event_log_component_instance_calculate(
        ...
        FOREACH_ARRAY(rr, instance->records, instance->n_records) {
                ...
                if (arg_location_start && strcmp(component->id, arg_location_start) >= 0) {
                        r = pcr_prediction_add_result(context, result, pcr, path, rr - instance->records);
                        if (r < 0)
                                return r;
                }
        }
...
static int event_log_show_predictions(Tpm2PCRPrediction *context, uint16_t alg) {
        ...
        size_t alg_idx = tpm2_hash_algorithm_index(alg);
        ...
        for (uint32_t pcr = 0; pcr < TPM2_PCRS_MAX; pcr++) {
                ...
                ORDERED_SET_FOREACH(result, context->results[pcr]) {
                        ...
                        h = hexmem(result->hash[alg_idx].buffer, result->hash[alg_idx].size);
                        if (!h)
                                return log_oom();
...
static int verb_make_policy(int argc, char *argv[], void *userdata) {
        ...
        if (have_old_policy) {
                if (arg_nv_index != 0 && old_policy.nv_index != arg_nv_index)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Stored policy references different NV index (0x%x) then specified (0x%x), refusing.", old_policy.nv_index, arg_nv_index);
