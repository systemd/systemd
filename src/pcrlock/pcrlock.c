...
static uint32_t arg_pcrs[TPM2_PCRS_MAX];
static size_t arg_n_pcrs = 0;
...
#define PCRLOCK_SECUREBOOT_PATH             "/var/lib/pcrlock.d/250-secureboot.pcrlock"
...
/* todo:
 *
 *    1. add verbs for generating .pcrlock files from UKI
 *    2. pre-calc sysext + kernel cmdline hashes
 *    3. pre-calc kernel's own initrd + kernl cmdline
 *    4. pre-calc cryptsetup root key
 *    5. calculate resulting hashes for all possible combinations
 *    6. create key pair in tpm, with signature-based access policy on itself, and sign result PCR values
 *    7. lock this against counter
 */
...
struct EventLogRecord {
        ...
        EventLogComponentInstance *component_instance;
...
struct EventLog {
        ...
        /* All records defined by components, indexed by their PCR and primary hash */
        Hashmap *component_records;
};
...
static EventLog* event_log_free(EventLog *el) {
        ...
        hashmap_free(el->component_records);
...
static int event_log_record_extract_firmware_description(EventLogRecord *rec) {
        ...
        case EV_EFI_VARIABLE_BOOT2: {
                ...
                if (rec->firmware_payload_size < sizeof(UEFI_VARIABLE_DATA)) {
                        log_warning_errno(SYNTHETIC_ERRNO(EBADMSG), "EFI variable field too short, ignoring.");
                        goto invalid;
                }
                ...
                if (vdata->unicodeNameLength > (SIZE_MAX - offsetof(UEFI_VARIABLE_DATA, unicodeNameLength)) / 2) {
                        log_error_errno(SYNTHETIC_ERRNO(EBADMSG), "Unicode name length too large, ignoring.");
                        goto invalid;
                }
                ...
                if (vdata->variableDataLength > SIZE_MAX - m) {
                        log_warning_errno(SYNTHETIC_ERRNO(EBADMSG), "Oversize EFI variable data size, ignoring.");
                        goto invalid;
                }

                if (rec->firmware_payload_size != m + vdata->variableDataLength) {
                        log_warning_errno(SYNTHETIC_ERRNO(EBADMSG), "EFI variable data has wrong size, ignoring.");
                        goto invalid;
                }
                ...
                if (!string_is_safe(p)) {
                        log_warning_errno(SYNTHETIC_ERRNO(EBADMSG), "Unsafe EFI variable string in record, ignoring.");
                        goto invalid;
                }

                if (asprintf(&rec->description, "Variable: %s-" SD_ID128_UUID_FORMAT_STR, p, SD_ID128_FORMAT_VAL(efi_guid_to_id128(vdata->variableName))) < 0)
                        return log_oom();
...
catchall:
        ...
        _cleanup_free_ char *s = NULL;
        s = cescape_length(rec->firmware_payload, rec->firmware_payload_size);
        if (!s)
                return log_oom();

        rec->description = strjoin("Raw: ", s);
        if (!rec->description)
                return log_oom();
        return 1;
...
static int event_log_record_verify_firmware_hash(
                EventLogRecord *record,
                EventLogRecordBank *bank,
                const EVP_MD *md) {
...
static int event_log_record_verify_userspace_hash(
                EventLogRecord *record,
                EventLogRecordBank *bank,
                const EVP_MD *md) {
...
static int event_log_calculate_pcrs(EventLog *el) {
        ...
        FOREACH_ARRAY(rr, el->records, el->n_records) {
                ...
                for (size_t i = 0; i < el->n_algorithms; i++) {
                        ...
                        /* Verify contents of the record, if we can */
                        r = event_log_record_verify_firmware_hash(*rr, rec_b, el->mds[i]);
                        if (r < 0)
                                return r;

                        r = event_log_record_verify_userspace_hash(*rr, rec_b, el->mds[i]);
                        if (r < 0)
                                return r;
...
static void event_log_record_hash_func(const EventLogRecord *record, struct siphash *state) {
        EventLogRecordBank *bank;

        assert(record);
        assert(record->event_log);
        assert(record->event_log->primary_algorithm != UINT16_MAX);

        siphash24_compress_safe(&record->pcr, sizeof(record->pcr), state);

        bank = event_log_record_find_bank(record, record->event_log->primary_algorithm);
        if (bank)
                siphash24_compress_safe(bank->hash, bank->hash_size, state);
}

static int event_log_record_compare_func(const EventLogRecord *a, const EventLogRecord *b) {
        ...
        return memcmp_nn(x->hash, x->hash_size, y->hash, y->hash_size);
}

DEFINE_HASH_OPS(event_log_record_hash_ops, EventLogRecord, event_log_record_hash_func, event_log_record_compare_func);
        ...
        *instance = (EventLogComponentInstance) {
        ...
        if (records) {
                ...
                JSON_VARIANT_ARRAY_FOREACH(rj, records) {
                        ...
                        record->component_instance = instance;

                        r = hashmap_ensure_put(&el->component_records, &event_log_record_hash_ops, record, record);
                        if (r < 0)
                                return log_error_errno(r, "Failed to insert component instance into lookup table: %m");
...
static int event_log_add_component_dir(EventLog *el, const char *path, char **base_search) {
        ...
        _cleanup_free_ char **search = NULL;
...
static int event_log_validate_component_order(EventLog *el) {

        for (uint32_t pcr = 0; pcr < ELEMENTSOF(el->registers); pcr++) {
                const EventLogComponentInstance *previous = NULL;
                ...
                FOREACH_ARRAY(rr, el->records, el->n_records) {
                        ...
                        if (!rec->component_instance) {
                                fully_recognized = false;
                                continue;
                        }

                        if (previous && strcmp(previous->component->id, rec->component_instance->component->id) > 0)
                                return log_error_errno(
                                                SYNTHETIC_ERRNO(EINVAL), "Order between components %s and %s on PCR %" PRIu32 " is reversed, refusing.",
                                                previous->component->id, rec->component_instance->component->id, pcr);

                        previous = rec->component_instance;
                }
...
static int event_log_map_components(EventLog *el) {
        ...
        FOREACH_ARRAY(rr, el->records, el->n_records) {
                EventLogRecord *observed_record = *rr, *predicted_record;

                if (observed_record->component_instance)
                        continue;

                predicted_record = hashmap_get(el->component_records, observed_record);
                if (!predicted_record)
                        continue;

                observed_record->component_instance = predicted_record->component_instance;
        }

        r = event_log_validate_component_order(el);
        if (r < 0)
                return r;
...
static int show_log_table(EventLog *el) {
        ...
        FOREACH_ARRAY(rr, el->records, el->n_records) {
                ...
                r = table_add_many(table,
                                   ...
                                   TABLE_STRING, pcr_index_to_string(record->pcr));
                if (r < 0)
                        return table_log_add_error(r);
                ...
                r = table_add_many(table,
                                   ...
                                   TABLE_PATH_BASENAME, record->component_instance ? record->component_instance->component->id : NULL,
...
static bool pcr_checks_out(const EventLog *el, const EventLogRegister *reg) {
        assert(el);
        assert(reg);
...
static int show_pcr_table(EventLog *el) {
        ...
        table = table_new_raw(6 + el->n_algorithms*2);
        if (!table)
                return log_oom();
        ...
        for (uint32_t pcr = 0; pcr < TPM2_PCRS_MAX; pcr++) {
                ...
                bool match = pcr_checks_out(el, el->registers + pcr);

                r = table_add_many(table,
                                   TABLE_BOOLEAN_CHECKMARK, match,
                                   TABLE_SET_COLOR, ansi_highlight_green_red(match),
                                   TABLE_BOOLEAN_CHECKMARK, el->registers[pcr].fully_recognized,
                                   TABLE_SET_COLOR, ansi_highlight_green_red(el->registers[pcr].fully_recognized));
                if (r < 0)
                        return table_log_add_error(r);
                ...
                for (size_t i = 0; i < el->n_algorithms; i++) {
                        ...
                        color = !match ? ANSI_HIGHLIGHT_RED :
...
static int verb_lock_raw(int argc, char *argv[], void *userdata) {
        ...
        for (size_t i = 0; i < arg_n_pcrs; i++) {
                _cleanup_(json_variant_unrefp) JsonVariant *record = NULL;

                r = make_pcrlock_record(arg_pcrs[i], data, size, &record);
                if (r < 0)
                        return r;
...
static int verb_lock_secureboot(int argc, char *argv[], void *userdata) {
        static const struct {
                sd_id128_t id;
                const char *name;
        } variables[] = {
                { EFI_VENDOR_GLOBAL,   "SecureBoot" },
                { EFI_VENDOR_GLOBAL,   "PK"         },
                { EFI_VENDOR_GLOBAL,   "KEK"        },
                { EFI_VENDOR_DATABASE, "db"         },
                { EFI_VENDOR_DATABASE, "dbx"        },
        };
        ...
        FOREACH_ARRAY(vv, variables, ELEMENTSOF(variables)) {
                ...
                if (r < 0) {
                        if (r != -ENOENT || !sd_id128_equal(vv->id, EFI_VENDOR_DATABASE))
                                return log_error_errno(r, "Failed to EFI variable '%s': %m", name);

                        /* If the database entries are not set we don't consider this an error, but measure
                         * an empty database instead. (This appears to be what real-life UEFI implementations
                         * do, including qemu/ovmf.) */
                ...
                *vdata = (UEFI_VARIABLE_DATA) {
                ...
                r = make_pcrlock_record(PCR_SECURE_BOOT_POLICY /* =7 */, vdata, vdata_size, &record);
                if (r < 0)
                        return r;
        ...
        return write_pcrlock(array, PCRLOCK_SECUREBOOT_PATH);
}

static int verb_unlock_secureboot(int argc, char *argv[], void *userdata) {
        return unlink_pcrlock(PCRLOCK_SECUREBOOT_PATH);
}
...
static int verb_lock_gpt(int argc, char *argv[], void *userdata) {
        ...
        r = make_pcrlock_record(PCR_BOOT_LOADER_CONFIG /* =5 */, vdata, vdata_size, &record);
        if (r < 0)
                return r;
...
static int event_log_record_is_calling_efi_app(const EventLogRecord *rec) {
        _cleanup_free_ char *d = NULL;
        int r;
        ...
        if (rec->pcr != PCR_BOOT_LOADER_CODE)
                return false;
...
static int verb_lock_firmware(int argc, char *argv[], void *userdata) {
        ...
        _cleanup_(event_log_freep) EventLog *el = NULL;
        uint32_t always_mask, separator_mask, separator_seen = 0;
        ...
        if (endswith(argv[0], "firmware-code")) {
                always_mask = (UINT32_C(1) << PCR_PLATFORM_CODE) |      /* → 0 */
                        (UINT32_C(1) << PCR_EXTERNAL_CODE);             /* → 2 */

                separator_mask = UINT32_C(1) << PCR_BOOT_LOADER_CODE;   /* → 4 */
                ...
                always_mask = (UINT32_C(1) << PCR_PLATFORM_CONFIG) |    /* → 1 */
                        (UINT32_C(1) << PCR_EXTERNAL_CONFIG);           /* → 3 */

                separator_mask = UINT32_C(1) << PCR_BOOT_LOADER_CONFIG; /* → 5 */
        ...
        FOREACH_ARRAY(rr, el->records, el->n_records) {
                _cleanup_(json_variant_unrefp) JsonVariant *digests = NULL, *record = NULL;
                EventLogRecord *rec = *rr;

                if (!EVENT_LOG_RECORD_IS_FIRMWARE(rec))
                        continue;

                if (!FLAGS_SET(always_mask, UINT32_C(1) << rec->pcr) &&
                    !(FLAGS_SET(separator_mask, UINT32_C(1) << rec->pcr) && !FLAGS_SET(separator_seen, UINT32_C(1) << rec->pcr)))
                        continue;
                ...
                if (rec->firmware_event_type == EV_SEPARATOR) {
                        separator_seen |= (UINT32_C(1) << rec->pcr);
                        continue;
                }

                /* If we hit the special "Calling EFI Application from Boot Option" option we treat this the
                 * same as a separator here, as that's where firmware passes control to boot loader */
                r = event_log_record_is_calling_efi_app(rec);
                if (r < 0)
                ...
                r = json_variant_append_arrayb(
                                separator_seen & (UINT32_C(1) << rec->pcr) ? &array_late : &array_early,
                                JSON_BUILD_OBJECT(
...
static int verb_lock_machine_id(int argc, char *argv[], void *userdata) {
        ...
        r = make_pcrlock_record(PCR_SYSTEM_IDENTITY /* = 15 */, word, SIZE_MAX, &record);
        if (r < 0)
                return r;
...
static int verb_lock_pe(int argc, char *argv[], void *userdata) {
        _cleanup_(json_variant_unrefp) JsonVariant *array = NULL;
        static const uint32_t default_pcr = PCR_BOOT_LOADER_CODE;
        _cleanup_close_ int fd = -EBADF;
        const uint32_t *pcrs;
        size_t n_pcrs;
        int r;
        ...
        for (size_t i = 0; i < n_pcrs; i++) {
                ...
                r = json_variant_append_arrayb(
                                &array,
                                JSON_BUILD_OBJECT(
                                                JSON_BUILD_PAIR("pcr", JSON_BUILD_UNSIGNED(pcrs[i])),
                                                JSON_BUILD_PAIR("digests", JSON_BUILD_VARIANT(digests))));
                if (r < 0)
...
static int verb_lock_uki(int argc, char *argv[], void *userdata) {
        ...
        r = json_variant_append_arrayb(
                        &array,
                        JSON_BUILD_OBJECT(
                                        JSON_BUILD_PAIR("pcr", JSON_BUILD_UNSIGNED(PCR_BOOT_LOADER_CODE)),
        ...
        for (UnifiedSection section = 0; section < _UNIFIED_SECTION_MAX; section++) {
                ...
                r = make_pcrlock_record(PCR_KERNEL_BOOT /* =11 */, unified_sections[section], strlen(unified_sections[section]) + 1, &record);
                if (r < 0)
                        return r;
                ...
                r = json_variant_append_arrayb(
                                &array,
                                JSON_BUILD_OBJECT(
                                                JSON_BUILD_PAIR("pcr", JSON_BUILD_UNSIGNED(PCR_KERNEL_BOOT /* =11 */)),
...
static int help(int argc, char *argv[], void *userdata) {
        ...
        printf("%1$s  [OPTIONS...] COMMAND ...\n"
               ...
               "  lock-secureboot            Generate a .pcrlock file from current SecureBoot policy\n"
               "  unlock-secureboot          Remove .pcrlock file for SecureBoot policy\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0)
                ...
                case ARG_PCR: {
                        ...
                        if (!typesafe_bsearch(&pcr, arg_pcrs, arg_n_pcrs, cmp_uint32)) {
                                assert(arg_n_pcrs < TPM2_PCRS_MAX-1);
                                arg_pcrs[arg_n_pcrs++] = pcr;
                                typesafe_qsort(arg_pcrs, arg_n_pcrs, cmp_uint32);
                        }
...
static int pcrlock_main(int argc, char *argv[]) {
        static const Verb verbs[] = {
                ...
                { "lock-secureboot",        VERB_ANY, 1,        0,            verb_lock_secureboot    },
                { "unlock-secureboot",      VERB_ANY, 1,        0,            verb_unlock_secureboot  },
