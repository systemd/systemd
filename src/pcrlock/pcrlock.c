...
typedef int (*DirectoryFileAction)(const char *directory, const char *name, void *userdata);

static int process_matching_files(
                const char *directory,
                bool (*predicate)(const char *name, void *userdata),
        ...
        FOREACH_DIRENT(de, dir, return -errno) {
                ...
                r = action(directory, de->d_name, userdata);
                if (r < 0)
                        return r;
        }
...
static int unlink_matching_file(const char *directory, const char *name, void *userdata) {
        _cleanup_free_ char *path = NULL;
        int r;
...
static int directory_has_file_matching(
        ...
        return process_matching_files(directory, predicate, /* action= */ NULL, userdata);
}
...
static int secure_boot_authenticated_payload(const struct iovec *input, struct iovec *ret_payload) {
        ...
        r = validate_signature_list(input->iov_base, input->iov_len);
        if (r >= 0) {
...
static int secure_boot_predictions_exist(const char *directory) {
        return directory_has_file_matching(
                        directory,
                        secure_boot_prediction_file_is_owned,
                        NULL);
}
...
static int write_authority_predictions(
        ...
        r = load_secureboot_authorities(&current, &current_records, &n_current_records);
        if (r == -ENOMEM)
                return r;
        if (r < 0) {
...
static int restore_component_files(const char *backup, const char *destination) {
        ...
        FOREACH_DIRENT(de, dir, return -errno) {
                _cleanup_free_ char *source = NULL, *target = NULL;
                ...
                r = copy_file_atomic(source, target, 0600, COPY_REPLACE|COPY_FSYNC);
                if (r < 0)
                        return r;
        }
...
static int recover_secure_boot_update_backups(bool regenerate) {
        _cleanup_closedir_ DIR *dir = NULL;
        _cleanup_strv_free_ char **backups = NULL;
        bool reseal = regenerate, restored = false;
        int r, ret = 0;
        ...
        FOREACH_DIRENT(de, dir, return -errno) {
                ...
                remainder = de->d_name + SD_ID128_STRING_MAX - 1;
                FOREACH_ELEMENT(info, secure_boot_update_components) {
                        const char *mode;

                        mode = startswith(remainder, strjoina(".", info->name, "."));
                        if (!mode)
                                continue;
                ...
                r = remove_secure_boot_predictions_for_transaction(PCRLOCK_SECUREBOOT_POLICY_DIR, id);
                if (r < 0)
                        return log_error_errno(r, "Failed to remove interrupted Secure Boot policy predictions: %m");
                r = remove_secure_boot_predictions_for_transaction(PCRLOCK_SECUREBOOT_AUTHORITY_DIR, id);
                if (r < 0)
                        return log_error_errno(r, "Failed to remove interrupted Secure Boot authority predictions: %m");
                r = remove_secure_boot_predictions_for_transaction(backup, id);
                if (r < 0)
                        return log_error_errno(r, "Failed to remove interrupted Secure Boot predictions from '%s': %m", backup);

                r = restore_component_files(backup, component->directory);
                if (r < 0)
                        return log_error_errno(r, "Failed to recover Secure Boot PCR lock components from '%s': %m", backup);

                r = strv_consume(&backups, TAKE_PTR(backup));
                if (r < 0)
                        return r;

                restored = true;
                reseal = reseal || recovered_regenerate;
        }
...
static int vl_method_notify_secure_boot_update(
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                ...
                { "data",       SD_JSON_VARIANT_STRING,  json_dispatch_unbase64_iovec,  offsetof(MethodNotifySecureBootUpdateParameters, data),       SD_JSON_NULLABLE  },
