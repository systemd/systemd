...
#define PCRLOCK_PCR_POLICY_MAX_ALTERNATIVES 8U
...
static int authority_predictions_fit_pcr7_budget(size_t n_predictions) {
        ...
        FOREACH_ARRAY(component, el->components, el->n_components) {
                ...
                if (n_variants > PCRLOCK_PCR_POLICY_MAX_ALTERNATIVES / combinations)
                        return false;
                combinations *= n_variants;
        }

        if (!authority_found && n_predictions > PCRLOCK_PCR_POLICY_MAX_ALTERNATIVES / combinations)
                return false;
...
static int backup_component_files(
        ...
        if (asprintf(&backup,
                     ...
                     SD_ID128_FORMAT_VAL(id), info->name, regenerate ? "regenerate" : "no-regenerate") < 0)
                return -ENOMEM;
...
static int recover_secure_boot_update_backups(bool regenerate) {
        ...
        FOREACH_DIRENT(de, dir, return -errno) {
                ...
                const char *regenerate_suffix, *suffix = NULL;
                bool recovered_regenerate = false;
                sd_id128_t id;

                if ((regenerate_suffix = endswith(de->d_name, ".regenerate")))
                        recovered_regenerate = true;
                else if (!(regenerate_suffix = endswith(de->d_name, ".no-regenerate")))
                        continue;

                FOREACH_ELEMENT(info, secure_boot_update_components) {
                        size_t n = strlen(info->name);

                        if ((size_t) (regenerate_suffix - de->d_name) <= n)
                                continue;

                        const char *candidate = regenerate_suffix - n;
                        if (candidate[-1] != '.' || memcmp(candidate, info->name, n) != 0)
                                continue;
                ...
                transaction = strndup(de->d_name, suffix - de->d_name);
                if (!transaction)
                        return -ENOMEM;
...
static int apply_secure_boot_update(
        ...
        for (;;) {
                if (plan->policy.fallback && !plan->policy.fallback_applied) {
                        ...
                        if (r < 0)
                                return r;
                        plan->policy.fallback_applied = true;
                }

                if (plan->authority.fallback && !plan->authority.fallback_applied) {
                        r = remove_component_files_for_update(
                                        SECURE_BOOT_UPDATE_COMPONENT_AUTHORITY,
                                        update_id,
                                        state->regenerate,
                        ...
                        if (r < 0)
                                return r;
                        plan->authority.fallback_applied = true;
                }
...
static int vl_method_notify_secure_boot_update(
        ...
        *state = (SecureBootUpdateState) {
        ...
        bool pcr7_unprotected = !state->pcr7_protected &&
                (plan.policy.fallback_applied || plan.authority.fallback_applied);
