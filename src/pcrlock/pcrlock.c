 ...
 *    4. pre-calc cryptsetup root key measurement
 ...
 *    7. lock this against tpm counter
 *    8. reverse match logic: iterate through each component, and find one or more places for it
 *    9. make cryptsetup honour two PCR signatures
 *    10. add services that lock down firmware, machine id, secureboot policy, … on boot, enable some of them by default
 *    11. add kernel-install plugin that automatically creates UKI .pcrlock file when UKI is installed, and removes it when it is removed again
 *    12. write generated pcrlock signature files to the ESP, one for each installed oS
 *    13. pick up generated pcrlock signature file in sd-stub, pass it via initrd to OS
 *    13. generate expected SECURITY_AUTHORITY records also from a PE file when measuring PE or UKI
 */
...
static int event_log_record_verify_hash_firmware(
                EventLogRecord *record,
                EventLogRecordBank *bank,
                const EVP_MD *md) {
...
static int event_log_record_verify_hash_userspace(
                EventLogRecord *record,
                EventLogRecordBank *bank,
                const EVP_MD *md) {
...
static int event_log_verify_record_hashes(EventLog *el) {
        int r;
        ...
        FOREACH_ARRAY(rr, el->records, el->n_records) {
                ...
                LIST_FOREACH(banks, bank, (*rr)->banks) {
                        ...
                        r = event_log_record_verify_hash_firmware(*rr, bank, md);
                        if (r < 0)
                                return r;

                        r = event_log_record_verify_hash_userspace(*rr, bank, md);
                        if (r < 0)
                                return r;
                }
        }
...
static int show_pcr_table(EventLog *el) {
        ...
        for (uint32_t pcr = 0; pcr < TPM2_PCRS_MAX; pcr++) {
                ...
                bool match = event_log_pcr_checks_out(el, el->registers + pcr);
...
static int verb_show_log(int argc, char *argv[], void *userdata) {
        ...
        r = event_log_verify_record_hashes(el);
        if (r < 0)
                return r;
...
static bool event_log_record_is_boot_authority(EventLogRecord *rec) {
        assert(rec);

        if (!EVENT_LOG_RECORD_IS_FIRMWARE(rec))
                return false;

        if (rec->pcr != PCR_SECURE_BOOT_POLICY)
                return  false;
...
static int verb_lock_secureboot_authority(int argc, char *argv[], void *userdata) {
        ...
        r = event_log_pcr_mask_checks_out(el, UINT32_C(1) << PCR_SECURE_BOOT_POLICY);
        if (r < 0)
                return r;
        ...
        FOREACH_ARRAY(rr, el->records, el->n_records) {
                ...
                if (!event_log_record_is_boot_authority(rec))
                        continue;
...
static int verb_lock_firmware(int argc, char *argv[], void *userdata) {
        ...
        _cleanup_(event_log_freep) EventLog *el = NULL;
        uint32_t always_mask, separator_mask, separator_seen_mask = 0;
        ...
        FOREACH_ARRAY(rr, el->records, el->n_records) {
                ...
                if (!FLAGS_SET(always_mask, bit) &&
                    !(FLAGS_SET(separator_mask, bit) && !FLAGS_SET(separator_seen_mask, bit)))
                        continue;
...
static int verb_lock_pe(int argc, char *argv[], void *userdata) {
        ...
        // Maybe also generate a matching EV_EFI_VARIABLE_AUTHORITY records here for each signature that
