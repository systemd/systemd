...
static int event_log_validate_component_order(EventLog *el) {
        ...
        for (uint32_t pcr = 0; pcr < ELEMENTSOF(el->registers); pcr++) {
                const EventLogRecord *previous = NULL;
                ...
                FOREACH_ARRAY(rr, el->records, el->n_records) {
                        ...
                        if (previous)
                                FOREACH_ARRAY(mm, rec->mapped, rec->n_mapped)
                                        FOREACH_ARRAY(nn, previous->mapped, previous->n_mapped)
                                        if (strcmp((*mm)->component->id, (*nn)->component->id) < 0)
                                                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Order between components %s and %s on PCR %" PRIu32 " is reversed, refusing.",
                                                                               (*nn)->component->id, (*mm)->component->id, pcr);
...
static int event_log_match_component_instance(
        ...
        r = event_log_record_compare_func(el->records[i], instance->records[j]);
        if (r != 0)
...
static int event_log_map_components(EventLog *el) {
        ...
        unsigned n_missing = 0, n_skipped = 0;
        int r;
        ...
        FOREACH_ARRAY(cc, el->components, el->n_components) {
                ...
                if (n_matching + n_empty == 0) {
                        log_error("Couldn't find component '%s' in event log.", c->id);
                        n_missing++;
