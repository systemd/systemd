...
static const SecureBootVariableInfo secure_boot_variables[_SECURE_BOOT_VARIABLE_MAX] = {
        [SECURE_BOOT_VARIABLE_PK]  = { "pk",  EFI_VENDOR_GLOBAL,   "PK",  1 },
        [SECURE_BOOT_VARIABLE_KEK] = { "kek", EFI_VENDOR_GLOBAL,   "KEK", 1 },
        [SECURE_BOOT_VARIABLE_DB]  = { "db",  EFI_VENDOR_DATABASE, "db",  1 },
        [SECURE_BOOT_VARIABLE_DBX] = { "dbx", EFI_VENDOR_DATABASE, "dbx", 1 },
        [SECURE_BOOT_VARIABLE_DBT] = { "dbt", EFI_VENDOR_DATABASE, "dbt", -1 },
        [SECURE_BOOT_VARIABLE_DBR] = { "dbr", EFI_VENDOR_DATABASE, "dbr", -1 },
};
...
static int read_secure_boot_variable(SecureBootVariable variable, struct iovec *ret_data) {
        ...
        if (variable < 0 || variable >= _SECURE_BOOT_VARIABLE_MAX)
                return -EINVAL;

        info = secure_boot_variables + variable;
        if (asprintf(&efi_name, "%s-" SD_ID128_UUID_FORMAT_STR,
...
static int build_secureboot_policy_variable(
        ...
        if (asprintf(&name, "%s-" SD_ID128_UUID_FORMAT_STR,
                     info->efi_name, SD_ID128_FORMAT_VAL(info->vendor)) < 0)
                return log_oom();
...
static int event_log_ensure_secureboot_consistency(EventLog *el) {
        ...
        /* Check for existence */
        for (size_t i = 0; i < ELEMENTSOF(table); i++)
                if (table[i]->synthesize_empty >= 0 && !records[i])
...
static int load_secureboot_authorities(
        ...
        CLEANUP_ARRAY(records, n_records, signature_records_done);
...
static void secure_boot_update_plan_done(SecureBootUpdatePlan *plan) {
        ...
        signature_records_done(plan->authority_records, plan->n_authority_records);
}
...
static int plan_secure_boot_update(
        ...
        if (plan->authority.active && variable == SECURE_BOOT_VARIABLE_DBX && any_present)
                plan->authority.fallback = true;
        else if (plan->authority.active && variable == SECURE_BOOT_VARIABLE_DB && has_unsupported && any_present)
                plan->authority.fallback = true;
...
static int vl_method_prepare_secure_boot_update(
        ...
        *state = (SecureBootUpdateState) {
        ...
        r = apply_secure_boot_update(update_id_string, &plan, state);
        if (r < 0)
                return r;
