...
static bool secure_boot_prediction_is_for_transaction(const char *name, sd_id128_t transaction_id) {
        char formatted[SD_ID128_STRING_MAX];
        const char *p;

        assert(!sd_id128_is_null(transaction_id));

        if (!secure_boot_prediction_file_is_owned(name))
                return false;

        p = name + STRLEN(PCRLOCK_SECUREBOOT_PREDICT_PREFIX);
        if (strlen(p) < 2 * (SD_ID128_STRING_MAX - 1) + 2)
                return false;

        p += SD_ID128_STRING_MAX - 1;
        if (*p++ != '-')
                return false;

        sd_id128_to_string(transaction_id, formatted);
        return startswith(p, formatted) && p[SD_ID128_STRING_MAX - 1] == '-';
}
...
static int remove_secure_boot_predictions(const char *directory, const char *keep_prefix) {
        ...
        FOREACH_DIRENT(de, dir, return -errno) {
                ...
                if (!secure_boot_prediction_file_is_owned(de->d_name) ||
...
static int remove_secure_boot_predictions_for_transaction(const char *directory, sd_id128_t transaction_id) {
        ...
        FOREACH_DIRENT(de, dir, return -errno) {
                ...
                if (!secure_boot_prediction_is_for_transaction(de->d_name, transaction_id))
                        continue;
...
static int signature_in_database(
                const void *database,
        ...
        for (;;) {
                ...
                if (efi_guid_equal(&list.header.type, type) && list.header.signature_size == signature_size) {
...
static int event_log_ensure_secureboot_consistency(EventLog *el) {
        const SecureBootVariableInfo *table[1 + _SECURE_BOOT_VARIABLE_MAX] = {
                &secure_boot_flag,
        };
        ...
        FOREACH_ELEMENT(info, secure_boot_variables)
                table[1 + (info - secure_boot_variables)] = info;
...
static int vl_method_prepare_secure_boot_update(
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                ...
                { "data",       SD_JSON_VARIANT_STRING,  json_dispatch_unbase64_iovec,  offsetof(MethodPrepareSecureBootUpdateParameters, data),       SD_JSON_NULLABLE  },
                { "regenerate", SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_tristate,     offsetof(MethodPrepareSecureBootUpdateParameters, regenerate), SD_JSON_NULLABLE  },
                {}
        };
