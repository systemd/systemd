...
static int parse_argv(int argc, char *argv[], char ***ret_args) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION_LONG("pcr", "NR",
                        ...
                        r = tpm2_parse_pcr_argument_to_mask(state.argument, &arg_pcr_mask);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse PCR specification: %s", state.argument);
                        break;
                ...
                OPTION_LONG("nv-index", "NUMBER",
                        ...
                        if (isempty(state.argument))
                                ...
                                r = safe_atou32_full(state.argument, 16, &u);
                                if (r < 0)
                                        return log_error_errno(r, "Failed to parse --nv-index= argument: %s", state.argument);
                ...
                OPTION_LONG("location", "START[:END]",
                        ...
                        e = strchr(state.argument, ':');
                        if (e) {
                                start = strndup(state.argument, e - state.argument);
                                if (!start)
                                        return log_oom();
                                ...
                                start = strdup(state.argument);
                                if (!start)
                                        return log_oom();

                                end = strdup(state.argument);
                                if (!end)
                                        return log_oom();
                        }
                ...
                OPTION_LONG("recovery-pin", "MODE",
                        ...
                        arg_recovery_pin = recovery_pin_mode_from_string(state.argument);
                        if (arg_recovery_pin < 0)
                                return log_error_errno(arg_recovery_pin, "Failed to parse --recovery-pin= mode: %s", state.argument);
                        break;
                ...
                OPTION_LONG("pcrlock", "PATH",
                        ...
                        if (empty_or_dash(state.argument))
                                ...
                                r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_pcrlock_path);
                                if (r < 0)
                                        return r;
                        }
                ...
                OPTION_LONG("policy", "PATH",
                        ...
                        if (empty_or_dash(state.argument))
                                ...
                                r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_policy_path);
                                if (r < 0)
                                        return r;
                        }
