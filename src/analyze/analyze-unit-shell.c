...
int verb_unit_shell(int argc, char *argv[], void *userdata) {
        ...
        r = sd_bus_call_method(
                ...
                unit);
        if (r < 0)
                return log_error_errno(r, "Failed to call GetUnit on '%s': %s", unit, bus_error_message(&error, r));

        const char *object = NULL;
        r = sd_bus_message_read(reply, "o", &object);
        if (r < 0)
                return log_error_errno(r, "Failed to get unit object of service %s, ignoring: %m", unit);
