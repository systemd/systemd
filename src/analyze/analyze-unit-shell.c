...
int verb_unit_shell(int argc, char *argv[], void *userdata) {
        ...
        const char *object = unit_dbus_path_from_name(unit);
        ...
        _cleanup_strv_free_ char **args = argc > 2 ? strv_copy(strv_skip(argv, 2)) : NULL;
        pid_t child;
        r = namespace_fork(
                        "(unit-shell-ns)",
                        "(unit-shell)",
        ...
        if (r == 0) {
                if (args) {
                        ...
                        if (r < 0)
                                return log_error_errno(r, "execvp(%s, [%s...]) failed: %m", args[0], *args);
                }
                else {
                        ...
                        if (!streq(DEFAULT_USER_SHELL, "/bin/bash")) {
                                ...
                                if (r < 0)
                                        return log_error_errno(r, "execl(\"/bin/bash\", \"-bash\") failed: %m");
                        }
                        if (!streq(DEFAULT_USER_SHELL, "/bin/sh")) {
                                r = execl("/bin/sh", "-sh", NULL);
                                if (r < 0)
                                        return log_error_errno(r, "execl(\"/bin/sh\", \"-sh\") failed: %m");
                        }
                }
                _exit(EXIT_FAILURE);
        }
