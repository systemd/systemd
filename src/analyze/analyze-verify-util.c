...
static int verify_manager_diagnostic_callback(const ManagerDiagnostic *record, void *userdata) {
        VerifyDiagnostics *diagnostics = ASSERT_PTR(userdata);
...
static void verify_syntax_callback(const LogSyntaxRecord *record, void *userdata) {
        ...
        if (record->config_file) {
                if (record->config_line > 0)
                        ...
                        r = strdup_to(&message, record->config_file);
                if (r >= 0 && !strextend_with_separator(&message, ": ", record->message))
                        r = -ENOMEM;
        } else if (record->unit) {
                message = strjoin(record->unit, ": ", record->message);
                r = message ? 0 : -ENOMEM;
        } else {
                message = strdup(record->message);
                r = message ? 0 : -ENOMEM;
        }
        if (r < 0) {
                diagnostics->error = -ENOMEM;
                return;
        }

        r = verify_diagnostics_add(
                        diagnostics,
                        record->priority,
                        message,
                        record->unit,
...
static int verify_diagnostics_get_error(const VerifyDiagnostics *diagnostics, Manager *manager) {
        int r;
        ...
        r = manager_get_test_run_diagnostic_error(manager);
        return r < 0 ? r : 0;
}
...
static int verify_units_internal(const VerifyUnitsParameters *parameters, VerifyUnitsResult *ret) {
        ...
        set_log_syntax_callback(verify_syntax_callback, &result.diagnostics);
        manager_set_test_run_diagnostic_callback(m, verify_manager_diagnostic_callback, &result.diagnostics);
        ...
        k = verify_diagnostics_get_error(&result.diagnostics, m);
        if (k < 0)
                return k;
        ...
        FOREACH_ARRAY(i, units, count) {
                ...
                r = verify_diagnostics_get_error(&result.diagnostics, m);
                if (r < 0)
                        return r;
