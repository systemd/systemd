...
static int edid_parse(sd_device *drm_card, char16_t **ret_panel) {
        ...
        r = sd_device_get_sysattr_value_with_size(drm_card, "edid", &edid_content, &edid_size);
        if (r < 0)
                return r;
        if (edid_size == 0)
                return -ENODEV;
...
static int edid_search(char16_t **ret_panel) {
        ...
        FOREACH_DEVICE(e, d) {
                ...
                if (r < 0)
                        return log_device_error_errno(d, r, "Couldn't get syspath from device: %m");
                ...
                strv_push_with_size(&drm_paths, &n, TAKE_PTR(drm_path_copy));
        }
        ...
        log_error("Multiple monitors detected.");
        STRV_FOREACH(s, drm_paths)
                log_info("Hint: use --drm-path=%s", *s);
