...
static int edid_search(char16_t **ret_panel) {
        ...
        _cleanup_strv_free_ char **drm_paths = new0(char *, 1);
        ...
        FOREACH_DEVICE(e, d) {
                ...
                if (ERRNO_IS_DEVICE_ABSENT(r))
                        continue;
                if (r < 0) {
                        log_debug_errno(r, "Failed to parse EDID for DRM device '%s', skipping: %m", drm_path);
                        continue;
                }

                _cleanup_free_ char *drm_path_copy = strdup(drm_path);
                if (!drm_path_copy)
                        return log_oom();

                if (strv_push_with_size(&drm_paths, &n, TAKE_PTR(drm_path_copy)) < 0)
                        return log_oom();
        }
        ...
        if (n == 0)
                return log_error_errno(SYNTHETIC_ERRNO(ENODEV), "No monitors detected, skipping extension EDID CHIDs.");

        log_error("Multiple monitors detected, skipping extension EDID CHIDs.");
