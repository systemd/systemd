...
static int start_and_wait_unit(
        ...
        SET_FLAG(flags, BUS_WAIT_JOBS_LOG_ERROR, true);
        SET_FLAG(flags, BUS_WAIT_JOBS_LOG_SUCCESS, false);
        r = bus_wait_for_jobs_one(w, path, flags, /* extra_args= */ NULL);
        if (r < 0)
                return r;
...
int verb_unit_gdb_start(int argc, char *argv[], void *userdata) {
        ...
        /* Don't interfere with debugger and its handling of DIGINT. */
        ...
        if (r == 0) {
                ...
                b = start_and_wait_unit(bus, "StartUnit", unit, "replace", &error, w);
                if (b < 0)
