...
static int is_exec_status_info_defined(sd_bus_message *m) {
        int r;
...
int verb_unit_gdb_start(int argc, char *argv[], void *userdata) {
        ...
        if (r == 0) {
                ...
                b = sd_bus_get_property(
                                ...
                                "Type", &error, &reply, "s");
                if (b < 0)
                        return log_error_errno(b, "Failed to get the 'Type' of %s: %s", unit, bus_error_message(&error, b));

                const char *service_type;
                b = sd_bus_message_read(reply, "s", &service_type);
                if (b < 0)
                        return log_error_errno(b, "Failed to read the service 'Type' of %s from reply: %m", unit);
                ...
                b = sd_bus_get_property(
                                bus, "org.freedesktop.systemd1",
                                object, "org.freedesktop.systemd1.Service",
                                "ExecStartPre", &error, &reply, "a(sasbttttuii)");
                if (b < 0)
                        return log_error_errno(b, "Failed to get the 'ExecStartPre' of %s: %s", unit, bus_error_message(&error, r));

                b = is_exec_status_info_defined(reply);
                if (b > 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "'ExecStartPre' command type is not allowed: %m");

                b = sd_bus_get_property(
                                bus, "org.freedesktop.systemd1",
                                object, "org.freedesktop.systemd1.Service",
                                "ExecStartPost", &error, &reply, "a(sasbttttuii)");
                if (b < 0)
                        return log_error_errno(b, "Failed to get the 'ExecStartPost' of %s: %s", unit, bus_error_message(&error, r));

                b = is_exec_status_info_defined(reply);
                if (b > 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "'ExecStartPost' command type is not allowed: %m");

                b = sd_bus_get_property(
                                bus, "org.freedesktop.systemd1",
                                object, "org.freedesktop.systemd1.Service",
                                "ExecStopPost", &error, &reply, "a(sasbttttuii)");
                if (b < 0)
                        return log_error_errno(b, "Failed to get the 'ExecStopPost' of %s: %s", unit, bus_error_message(&error, r));

                b = is_exec_status_info_defined(reply);
                if (b > 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "'ExecStopPost' command type is not allowed: %m");

                b = sd_bus_get_property(
                                bus, "org.freedesktop.systemd1",
                                object, "org.freedesktop.systemd1.Service",
                                "ExecCondition", &error, &reply, "a(sasbttttuii)");
                if (b < 0)
                        return log_error_errno(b, "Failed to get the 'ExecCondition' of %s: %s", unit, bus_error_message(&error, r));

                b = is_exec_status_info_defined(reply);
                if (b > 0)
                if (b > 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "'ExecCondition' command type is not allowed: %m");

                b = sd_bus_message_new_method_call(
                                bus, &reply,
                ...
                if (b < 0)
                        return log_error_errno(b, "Failed to append DebugWait: %m");
