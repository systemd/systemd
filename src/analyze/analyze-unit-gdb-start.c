...
int verb_unit_gdb_start(int argc, char *argv[], void *userdata) {
        ...
        r = manager_startup(m, /* serialization= */ NULL, /* fds= */ NULL, arg_root);

        r = verify_set_unit_path(STRV_MAKE(unit));
        if (r < 0)
                return log_error_errno(r, "Failed to set unit load path: %m");

        r = manager_load_startable_unit_or_warn(m, /* name= */ NULL, unit, &u);
        if (r < 0)
                return log_error_errno(r, "Failed to create %s: %m", unit);

        if (SERVICE(u)->type != SERVICE_ONESHOT)
                return log_error_errno(r, "Service must not be of type 'oneshot': %m");

        if (SERVICE(u)->exec_command[SERVICE_EXEC_START_PRE] ||
            SERVICE(u)->exec_command[SERVICE_EXEC_START_POST] ||
            SERVICE(u)->exec_command[SERVICE_EXEC_STOP_POST] ||
            SERVICE(u)->exec_command[SERVICE_EXEC_CONDITION])
                return log_error_errno(
                                SYNTHETIC_ERRNO(EINVAL),
                                "'StartPre', 'StartPost', 'StopPost', and 'ExecCondition' are not allowed");
