...
static SecurityInfo *security_info_free_free(SecurityInfo *info) {
            security_info_free(info);
            free(info);
            return NULL;
}

DEFINE_TRIVIAL_CLEANUP_FUNC(SecurityInfo*, security_info_free_free);
...
static int property_read_restrict_namespaces(
        ...
        r = sd_bus_message_read(m, "t", &size);
        if (r < 0)
                return r;

        info->restrict_namespaces = (unsigned long long) size;
...
static int property_read_umask(
        ...
        info->restrict_namespaces = (mode_t) size;
...
static int analyze_security_one(sd_bus *bus, const char *name, Table *overview_table, AnalyzeSecurityFlags flags, unsigned threshold) {
...
static int get_security_info(Unit *u, ExecContext *c, CGroupContext *g, SecurityInfo **ret_info) {
        ...
        _cleanup_(security_info_free_freep) SecurityInfo *info = new0(SecurityInfo, 1);
        if (!info)
                return log_oom();
        ...
        if (u) {
                ...
                if (u->type == UNIT_SERVICE) {
                        if (notify_access_to_string(SERVICE(u)->notify_access)) {
...
static int offline_security_check(Unit *u, unsigned threshold) {
        ...
        k = get_security_info(u, unit_get_exec_context(u), unit_get_cgroup_context(u), &info);
        if (k < 0 && r == 0)
                r = k;
        else {
                k = assess(info, overview_table, flags, threshold);
                if (k < 0 && r == 0)
                        r = k;
        }
...
int analyze_security(sd_bus *bus, char **units, UnitFileScope scope, bool check_man, bool run_generators, bool offline, unsigned threshold, const char *root, AnalyzeSecurityFlags flags) {
        _cleanup_(table_unrefp) Table *overview_table = NULL;
        int ret = 0, r;
