...
static int analyze_security_one(sd_bus *bus, const char *name, Table *overview_table,
        ...
        _cleanup_(security_info_freep) SecurityInfo *info = new0(SecurityInfo, 1);
        if (!info)
                return log_oom();

        info->default_dependencies = true;
        info->capability_bounding_set = UINT64_MAX;
        info->restrict_namespaces = UINT64_MAX;
        info->_umask = 0002;
