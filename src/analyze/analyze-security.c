...
static int assess(const SecurityInfo *info, Table *overview_table, AnalyzeSecurityFlags flags) {
        ...
        if (exposure > 50 && FLAGS_SET(flags, ANALYZE_SECURITY_EXPOSURE_ABOVE_MEDIUM))
                return -EINVAL;
...
static int get_security_info(Unit *u, ExecContext *c, CGroupContext *g, SecurityInfo **ret_info) {
        ...
        if (u) {
                ...
                info->notify_access = u->type == UNIT_SERVICE ? strdup(notify_access_to_string(SERVICE(u)->notify_access)) : NULL;
        }
