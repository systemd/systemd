...
int analyze_elf(char **filenames, JsonFormatFlags json_flags) {
        ...
        STRV_FOREACH(filename, filenames) {
                ...
                r = parse_elf(fd, abspath, NULL, &package_metadata);
                if (r < 0)
                        return log_error_errno(r, "Parsing \"%s\" as ELF object failed: %m", abspath);

                /* Maybe not an ELF object? Try to parse as a core dump. */
                if (!package_metadata) {
                        r = parse_core(fd, abspath, NULL, &package_metadata);
                        if (r < 0)
                                return log_error_errno(r, "Parsing \"%s\" as core file failed: %m", abspath);
                }
        ...
        return 0;
#else
        return log_error_errno((SYNTHETIC_ERRNO(EOPNOTSUPP), "ELF parsing not supported.");
#endif
}
