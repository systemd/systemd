...
static int get_previous_boot_id(sd_journal *j, sd_id128_t *ret_boot_id) {
        ...
        /* Use journal_find_boot to get the previous boot (-1 offset) */
        r = journal_find_boot(j, SD_ID128_NULL, -1, ret_boot_id);
        if (r < 0)
                return r;
        if (r == 0) {
                /* If no previous boot found, try current boot */
...
static int acquire_shutdown_times(Hashmap **ret) {
        ...
        if (r == 0)
                log_info("No previous boot found, analyzing current boot.");
