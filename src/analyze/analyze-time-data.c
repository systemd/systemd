...
int pretty_boot_time(sd_bus *bus, char **ret) {
        ...
        if (timestamp_is_set(t->shutdown_start_time) &&
            ...
            timestamp_is_set(t->shutdown_binary_finish_time)) {
                ...
                usec_t stopping = usec_sub_unsigned(t->shutdown_finish_time, t->shutdown_start_time);
                usec_t exit_time = usec_sub_unsigned(t->shutdown_binary_start_time, t->shutdown_finish_time);
                usec_t systemd_shutdown = usec_sub_unsigned(t->shutdown_binary_finish_time, t->shutdown_binary_start_time);
