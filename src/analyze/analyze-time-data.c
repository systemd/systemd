...
int pretty_boot_time(sd_bus *bus, char **ret) {
        ...
        if (timestamp_is_set(t->shutdown_start_time) &&
            ...
            timestamp_is_set(t->shutdown_binary_finish_time)) {
                ...
                shutdown_time = usec_sub_unsigned(t->shutdown_binary_finish_time, t->shutdown_start_time);

                if (!strextend(&text, FORMAT_TIMESPAN(usec_sub_unsigned(t->shutdown_finish_time, t->shutdown_start_time), USEC_PER_MSEC), " (stopping) + "))
                        return log_oom();
                if (!strextend(&text, FORMAT_TIMESPAN(usec_sub_unsigned(t->shutdown_binary_start_time, t->shutdown_finish_time), USEC_PER_MSEC), " (exit) + "))
                        return log_oom();
                if (!strextend(&text, FORMAT_TIMESPAN(usec_sub_unsigned(t->shutdown_binary_finish_time, t->shutdown_binary_start_time), USEC_PER_MSEC), " (systemd-shutdown) + "))
                        return log_oom();
        }
