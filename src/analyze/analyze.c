...
static int process_aliases(char *argv[], char *tempdir, char ***ret) {
        ...
        STRV_FOREACH(filename, strv_skip(argv, 1)) {
                _cleanup_free_ char *src = NULL, *dst = NULL, *argname = NULL;

                if (!strchr(*filename, ':')) {
                        r = strv_extend(&filenames, argv[i++]);
                        if (r < 0)
                                return log_oom();
...
static int setup_working_dir(char **ret) {
        ...
        char *t, bid[SD_ID128_STRING_MAX];
        sd_id128_t boot_id;
...
static int do_verify(int argc, char *argv[], void *userdata) {
        ...
        if (r < 0)
                return log_error_errno(r, "Couldn't process aliases");
