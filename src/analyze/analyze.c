...
static int parse_argv(int argc, char *argv[]) {
        ...
        if (sd_json_enabled(arg_json_format_flags) && !STRPTR_IN_SET(argv[optind], "security", "inspect-elf", "plot", "fdstore", "pcrs", "architectures", "capability", "exit-status"))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
        ...
        if ((!arg_legend && !STRPTR_IN_SET(argv[optind], "plot", "architectures")) ||
           (streq_ptr(argv[optind], "plot") && !arg_legend && !arg_table && !sd_json_enabled(arg_json_format_flags)))
        ...
        if (arg_table && sd_json_enabled(arg_json_format_flags))
