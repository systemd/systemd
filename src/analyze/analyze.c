...
static int do_security(int argc, char *argv[], void *userdata) {
        ...
        if (arg_security_policy) {
                ...
                r = search_and_fopen_nulstr("systemd-analyze-security.policy", "re", /*root=*/ NULL, CONF_PATHS_NULSTR("systemd"), &f, /*ret_path=*/ NULL);
                if (r < 0 && r != -ENOENT)
                        return r;
                if (f != NULL) {
                        r = json_parse_file(f, /* path=*/ NULL, /*flags=*/ 0, &policy, &line, &column);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse policy at %u:%u: %m", line, column);
                }
        }
