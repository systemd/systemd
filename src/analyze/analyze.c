...
static ReturnErrorOn arg_return_error_on = RETURN_ERROR_ON_NONE;
...
static int do_verify(int argc, char *argv[], void *userdata) {
        return verify_units(strv_skip(argv, 1), arg_scope, arg_man, arg_generators, arg_return_error_on);
}
...
static int help(int argc, char *argv[], void *userdata) {
        ...
        printf("%s [OPTIONS...] COMMAND ...\n\n"
               ...
               "     --return-error-on=MODE  Show if 'verify' returns errors based on mode\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        static const struct option options[] = {
                ...
                { "return-error-on", required_argument, NULL, ARG_RETURN_ERROR_ON  },
        ...
        while ((c = getopt_long(argc, argv, "hH:M:", options, NULL)) >= 0)
                ...
                case ARG_RETURN_ERROR_ON:
                        if (streq(optarg, "help")) {
                                DUMP_STRING_TABLE(return_error_on, ReturnErrorOn, _RETURN_ERROR_ON_MAX);
                                return 0;
                        }

                        r = return_error_on_from_string(optarg);
                        if (r < 0)
                                return log_error_errno(r, "Unknown mode '%s'.", optarg);
                        arg_return_error_on = r;
                        break;
