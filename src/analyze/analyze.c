...
static bool arg_json_output = false;
...
static int do_security(int argc, char *argv[], void *userdata) {
        ...
        if (arg_security_policy) {
                r = json_parse_file(/*f=*/ NULL, arg_security_policy, /*flags=*/ 0, &policy, /*ret_line=*/ NULL, /*ret_column=*/ NULL);
                if (r < 0)
                        return log_error_errno(r, "Failed to parse '%s': %m", arg_security_policy);
        }
...
static int help(int argc, char *argv[], void *userdata) {
        ...
        printf("%s [OPTIONS...] COMMAND ...\n\n"
               ...
               "     --security-policy=PATH  Obtain a set of custom requirements\n"
               "                             against which to compare the unit file(s)\n"
               "     --json-output=BOOL      Generate JSON output of the security\n"
               "                             analysis table\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        static const struct option options[] = {
                ...
                { "json-output",      required_argument, NULL, ARG_JSON_OUTPUT      },
                {}
        };
        ...
        while ((c = getopt_long(argc, argv, "hH:M:", options, NULL)) >= 0)
                ...
                case ARG_JSON_OUTPUT:
                        r = parse_boolean_argument("--json-output", optarg, &arg_json_output);
                        if (r < 0)
                                return r;
                        break;
        ...
        if (arg_json_output && !streq_ptr(argv[optind], "security"))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "Option --json-output= is only supported for security right now.");
