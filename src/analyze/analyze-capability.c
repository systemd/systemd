...
int verb_capabilities(int argc, char *argv[], void *userdata) {
        ...
        if (arg_capability == ARG_CAP_MASK || (argc == 2 && argv[1][0] == '@')) {
                uint64_t cap_mask;
                ...
                if (argv[1][0] == '@')
                        ++argv[1];
                if (safe_atoux64(argv[1], &cap_mask))
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Capability mask \"%s\" is not valid.", argv[1]);

                for (int c = 0; cap_mask != 0; ) {
                        if (cap_mask & 0b1) {
                                if ((unsigned) c > last_cap)
                                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Capability %d is not known.", c);
