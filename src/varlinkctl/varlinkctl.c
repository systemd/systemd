...
static int verb_info(int argc, char *argv[], void *userdata) {
        ...
        r = varlink_call_and_log(vl, "org.varlink.service.GetInfo", /* parameters= */ NULL, &reply, /* ret_error= */ NULL);
        if (r < 0)
                return r;
...
static int verb_introspect(int argc, char *argv[], void *userdata) {
        ...
        JsonVariant *reply = NULL;
        r = varlink_callb_and_log(vl, "org.varlink.service.GetInterfaceDescription", &reply, /* error= */ NULL, JSON_BUILD_OBJECT(JSON_BUILD_PAIR_STRING("interface", interface)));
        if (r < 0)
                return r;
