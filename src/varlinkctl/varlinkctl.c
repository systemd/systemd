...
static int varlink_call_and_upgrade(const char *url, const char *method, sd_json_variant *parameters, char **exec_cmdline) {
        ...
        /* For bidirectional sockets input_fd and output_fd are the same fd. Dup so that
         * each _cleanup_close_ variable owns a distinct fd, avoiding double-close on error. */
        if (input_fd == output_fd) {
                output_fd = fcntl(input_fd, F_DUPFD_CLOEXEC, 3);
                if (output_fd < 0)
                        return log_error_errno(errno, "Failed to duplicate upgraded connection fd: %m");
        }
