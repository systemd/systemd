...
static int parse_argv(int argc, char *argv[], char ***ret_args) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION_LONG("graceful", "ERROR", "Treat specified Varlink error as success"):
                        r = varlink_idl_qualified_symbol_name_is_valid(state.argument);
                        if (r < 0)
                                return log_error_errno(r, "Failed to validate Varlink error name '%s': %m", state.argument);
                        if (r == 0)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Not a valid Varlink error name: %s", state.argument);

                        if (strv_extend(&arg_graceful, state.argument) < 0)
                                return log_oom();
                        break;
                ...
                OPTION_LONG("timeout", "SECS", "Maximum time to wait for method call completion"):
                        ...
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --timeout= parameter '%s': %m", state.argument);
                ...
                OPTION_LONG("push-fd", "FD", "Pass the specified fd along with method call"): {
                        ...
                        if (STARTSWITH_SET(state.argument, "/", "./")) {
                                 ...
                                 * relative path */
                                add_fd = open(state.argument, O_CLOEXEC|O_RDONLY|O_NOCTTY);
                                if (add_fd < 0)
                                        return log_error_errno(errno, "Failed to open '%s': %m", state.argument);
                        } else {
                                int parsed_fd = parse_fd(state.argument);
                                if (parsed_fd < 0)
                                        return log_error_errno(parsed_fd, "Failed to parse --push-fd= parameter: %s", state.argument);
