...
static ButtonModifierMask button_get_key_modifier_mask(int keycode)
{
        ButtonModifierMask modifier_mask = BUTTON_MODIFIER_NONE;

        FOREACH_ARRAY(modifier_map, keycode_modifier_table, ELEMENTSOF(keycode_modifier_table)) {
                if (keycode == modifier_map->keycode)
                        modifier_mask = modifier_map->modifier_mask;
        }
...
static int button_dispatch(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        if (ev.type == EV_KEY && ev.value > 0) {
                ...
                keycode_modifier_mask = button_get_key_modifier_mask(ev.code);
                switch (ev.code) {
