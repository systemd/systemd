...
static int apply_tmpfs_quota(
        ...
        STRV_FOREACH(p, paths) {
                ...
                if (set_contains(processed, UINT32_TO_PTR(st.st_dev))) {
                ...
                if (set_ensure_put(&processed, /* hash_ops= */ NULL, UINT32_TO_PTR(st.st_dev)) < 0)
                        return log_oom();
                ...
                if (!F_TYPE_EQUAL(sfs.f_type, TMPFS_MAGIC)) {
                        log_debug("Not setting quota on '%s', since not tmpfs.", *p);
                        continue;
                }

                struct dqblk req;
                r = quotactl_fd(fd, QCMD_FIXED(Q_GETQUOTA, USRQUOTA), uid, &req);
                if (r == -ESRCH)
                        zero(req);
                else if (ERRNO_IS_NEG_NOT_SUPPORTED(r)) {
                        log_info_errno(r, "No UID quota support on %s, not setting quota: %m", *p);
                        continue;
                } else if (ERRNO_IS_NEG_PRIVILEGE(r)) {
                        log_info_errno(r, "Lacking privileges to query UID quota on %s, not setting quota: %m", *p);
                        continue;
                } else if (r < 0) {
                ...
                if (FLAGS_SET(req.dqb_valid, QIF_BLIMITS) && v == req.dqb_bhardlimit) {
                        ...
                        log_info("Configured quota on '%s' already matches the intended setting, not updating quota.", *p);
                        continue;
                }
                ...
                r = quotactl_fd(fd, QCMD_FIXED(Q_SETQUOTA, USRQUOTA), uid, &req);
                if (r == -ESRCH) {
                        log_info_errno(r, "Not setting UID quota on %s since UID quota is not supported: %m", *p);
                        continue;
                } else if (ERRNO_IS_NEG_PRIVILEGE(r)) {
                        log_notice_errno(r, "Lacking privileges to set UID quota on %s, skipping: %m", *p);
                        continue;
                } else if (r < 0) {
