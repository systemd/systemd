...
static int apply_tmpfs_quota(
        ...
        STRV_FOREACH(p, paths) {
                ...
                struct dqblk req;
                r = RET_NERRNO(quotactl_fd_with_fallback(fd, QCMD_FIXED(Q_GETQUOTA, USRQUOTA), uid, &req));
                if (r == -ESRCH)
                        zero(req);
                ...
                r = RET_NERRNO(quotactl_fd_with_fallback(fd, QCMD_FIXED(Q_SETQUOTA, USRQUOTA), uid, &req));
                if (r == -ESRCH) {
