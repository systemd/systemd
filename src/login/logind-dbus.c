...
static int method_set_reboot_to_boot_loader_menu(
        ...
        if (use_efi) {
                if (x == UINT64_MAX)
                        r = efi_set_variable(EFI_VARIABLE_LoaderConfigTimeoutOneShot, NULL, 0);
                else {
                        char buf[DECIMAL_STR_MAX(uint64_t) + 1];
                        xsprintf(buf, "%" PRIu64, DIV_ROUND_UP(x, USEC_PER_SEC)); /* second granularity */

                        r = efi_set_variable_string(EFI_VARIABLE_LoaderConfigTimeoutOneShot, buf);
                }
                if (r < 0)
                        return r;
        } else {
...
static int method_set_reboot_to_boot_loader_entry(
        ...
        if (use_efi) {
                if (isempty(v))
                        /* Delete item */
                        r = efi_set_variable(EFI_VARIABLE_LoaderEntryOneShot, NULL, 0);
                else
                        r = efi_set_variable_string(EFI_VARIABLE_LoaderEntryOneShot, v);
                if (r < 0)
                        return r;
        } else {
                if (isempty(v)) {
