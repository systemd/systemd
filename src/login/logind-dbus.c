...
err_no_session:
        return sd_bus_error_setf(
                error, BUS_ERROR_NO_SESSION_FOR_PID, "Caller does not belong to any known session");
}
...
int manager_get_user_from_creds(Manager *m, sd_bus_message *message, uid_t uid, sd_bus_error *error, User **ret) {
        ...
        if (!user)
                return sd_bus_error_setf(error,
                                         ...
                                         "User ID " UID_FMT " is not logged in or lingering",
                                         uid);
...
static int property_get_inhibited(sd_bus *bus,
                                  const char *path,
                                  const char *interface,
                                  const char *property,
                                  sd_bus_message *reply,
...
static int property_get_preparing(sd_bus *bus,
                                  const char *path,
                                  const char *interface,
                                  const char *property,
                                  sd_bus_message *reply,
...
static int property_get_scheduled_shutdown(sd_bus *bus,
                                           const char *path,
                                           const char *interface,
                                           const char *property,
                                           sd_bus_message *reply,
        ...
        if (pid == 0) {
                ...
                if (!session)
                        return sd_bus_error_setf(error,
                                                 ...
                                                 "PID " PID_FMT " does not belong to any known session",
                                                 pid);
        }
...
static int method_list_users(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        HASHMAP_FOREACH(user, m->users, i) {
                ...
                r = sd_bus_message_append(reply, "(uso)", (uint32_t) user->uid, user->name, p);
                if (r < 0)
                        return r;
        }
...
static int method_create_session(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        if (isempty(type))
                ...
                if (t < 0)
                        return sd_bus_error_setf(
                                error, SD_BUS_ERROR_INVALID_ARGS, "Invalid session type %s", type);
        }
        ...
        if (isempty(class))
                ...
                if (c < 0)
                        return sd_bus_error_setf(
                                error, SD_BUS_ERROR_INVALID_ARGS, "Invalid session class %s", class);
        }
        ...
        if (isempty(desktop))
                ...
                if (!string_is_safe(desktop))
                        return sd_bus_error_setf(
                                error, SD_BUS_ERROR_INVALID_ARGS, "Invalid desktop string %s", desktop);
        }
        ...
        if (tty_is_vc(tty)) {
                ...
                if (!seat)
                        ...
                        return sd_bus_error_setf(error,
                                                 ...
                                                 "TTY %s is virtual console but seat %s is not seat0",
                                                 tty,
                                                 seat->id);
                ...
                if (v <= 0)
                        return sd_bus_error_setf(error,
                                                 SD_BUS_ERROR_INVALID_ARGS,
                                                 "Cannot determine VT number from virtual console TTY %s",
                                                 tty);
                ...
                if (vtnr == 0)
                        ...
                        return sd_bus_error_setf(
                                error, SD_BUS_ERROR_INVALID_ARGS, "Specified TTY and VT number do not match");
        ...
        if (seat) {
                if (seat_has_vts(seat)) {
                        if (vtnr <= 0 || vtnr > 63)
                                return sd_bus_error_setf(
                                        error, SD_BUS_ERROR_INVALID_ARGS, "VT number out of range");
                } else {
                        if (vtnr != 0)
                                return sd_bus_error_setf(error,
                                                         SD_BUS_ERROR_INVALID_ARGS,
        ...
        /* Check if we are already in a logind session. Or if we are in user@.service which is a special PAM
         * session that avoids creating a logind session. */
        r = manager_get_user_by_pid(m, leader, NULL);
        if (r < 0)
                return r;
        if (r > 0)
                return sd_bus_error_setf(
                        error, BUS_ERROR_SESSION_BUSY, "Already running in a session or user slice");
         ...
         */
        if (c != SESSION_GREETER && vtnr > 0 && vtnr < m->seat0->position_count &&
            m->seat0->positions[vtnr] && m->seat0->positions[vtnr]->class != SESSION_GREETER)
...
static int method_set_user_linger(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        r = sd_bus_query_sender_creds(
                message, SD_BUS_CREDS_EUID | SD_BUS_CREDS_OWNER_UID | SD_BUS_CREDS_AUGMENT, &creds);
        if (r < 0)
                return r;
...
static int attach_device(Manager *m, const char *seat, const char *sysfs) {
        ...
        if (asprintf(&rule, "TAG==\"seat\", ENV{ID_FOR_SEAT}==\"%s\", ENV{ID_SEAT}=\"%s\"", id_for_seat, seat) <
            0)
                return -ENOMEM;
...
static int have_multiple_sessions(Manager *m, uid_t uid) {
...
static int bus_manager_log_shutdown(Manager *m, const char *unit_name) {
        ...
        return log_struct(LOG_NOTICE, "MESSAGE_ID=" SD_MESSAGE_SHUTDOWN_STR, p, q);
}
...
static int send_prepare_for(Manager *m, InhibitWhat w, bool _active) {
        ...
        static const char *const signal_name[_INHIBIT_WHAT_MAX] = { [INHIBIT_SHUTDOWN] = "PrepareForShutdown",
        ...
        return sd_bus_emit_signal(
                m->bus, "/org/freedesktop/login1", "org.freedesktop.login1.Manager", signal_name[w], "b", active);
}

static int execute_shutdown_or_sleep(Manager *m, InhibitWhat w, const char *unit_name, sd_bus_error *error) {
...
int manager_dispatch_delayed(Manager *manager, bool timeout) {
        ...
        if (manager_is_inhibited(
                    manager, manager->action_what, INHIBIT_DELAY, NULL, false, false, 0, &offending)) {
                ...
                log_notice("Delay lock is active (UID " UID_FMT "/%s, PID " PID_FMT
        ...
        if (r < 0) {
                log_warning(
                        "Error during inhibitor-delayed operation (already returned success to client): %s",
...
static int manager_inhibit_timeout_handler(sd_event_source *s, uint64_t usec, void *userdata) {
...
static int delay_shutdown_or_sleep(Manager *m, InhibitWhat w, const char *unit_name) {
        ...
        if (m->inhibit_timeout_source) {
                ...
                r = sd_event_add_time(m->event,
                                      ...
                                      timeout_val,
                                      0,
                                      manager_inhibit_timeout_handler,
                                      m);
                if (r < 0)
                        return r;
        }
...
int bus_manager_shutdown_or_sleep_now_or_later(Manager *m,
        ...
        if (!streq(load_state, "loaded"))
                return log_notice_errno(
                        SYNTHETIC_ERRNO(EACCES), "Unit %s is %s, refusing operation.", unit_name, load_state);

        /* Tell everybody to prepare for shutdown/sleep */
        (void) send_prepare_for(m, w, true);

        delayed = m->inhibit_delay_max > 0 &&
...
static int verify_shutdown_creds(Manager *m,
        ...
        if (!multiple_sessions && !blocked && action) {
                r = bus_verify_polkit_async(
                        message, CAP_SYS_BOOT, action, NULL, interactive, UID_INVALID, &m->polkit_registry, error);
                if (r < 0)
                        return r;
                if (r == 0)
                        return 1; /* No authorization for now, but the async polkit stuff will call us again when it has it */
...
static int method_do_shutdown_or_sleep(Manager *m,
                                       sd_bus_message *message,
                                       const char *unit_name,
                                       InhibitWhat w,
                                       const char *action,
        ...
        r = verify_shutdown_creds(
                m, message, w, interactive, action, action_multiple_sessions, action_ignore_inhibit, error);
        if (r != 0)
                return r;
...
static int method_poweroff(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        return method_do_shutdown_or_sleep(m,
                                           message,
...
static int nologin_timeout_handler(sd_event_source *s, uint64_t usec, void *userdata) {

        Manager *m = userdata;

        log_info("Creating /run/nologin, blocking further logins...");

        m->unlink_nologin = create_shutdown_run_nologin_or_warn() >= 0;
...
static int manager_scheduled_shutdown_handler(sd_event_source *s, uint64_t usec, void *userdata) {
...
static int method_schedule_shutdown(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        r = verify_shutdown_creds(
                m, message, INHIBIT_SHUTDOWN, false, action, action_multiple_sessions, action_ignore_inhibit, error);
        if (r != 0)
                return r;
        ...
        if (m->scheduled_shutdown_timeout_source) {
                ...
                r = sd_event_add_time(m->event,
                                      &m->scheduled_shutdown_timeout_source,
                                      CLOCK_REALTIME,
                                      elapse,
                                      0,
        ...
        if (r < 0) {
                m->scheduled_shutdown_timeout_source = sd_event_source_unref(
                        m->scheduled_shutdown_timeout_source);
                return log_oom();
        }
        ...
        if (m->nologin_timeout_source) {
                ...
                r = sd_event_add_time(m->event,
                                      &m->nologin_timeout_source,
                                      CLOCK_REALTIME,
                                      elapse - 5 * USEC_PER_MINUTE,
                                      0,
                                      nologin_timeout_handler,
                                      m);
                if (r < 0)
                        return log_error_errno(r, "sd_event_add_time() failed: %m");
        }
        ...
        r = sd_bus_query_sender_creds(
                message, SD_BUS_CREDS_AUGMENT | SD_BUS_CREDS_TTY | SD_BUS_CREDS_UID, &creds);
        if (r >= 0) {
                const char *tty = NULL;
...
static int method_cancel_scheduled_shutdown(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        if (cancelled && m->enable_wall_messages) {
                ...
                utmp_wall("The system shutdown has been cancelled", username, tty, logind_wall_tty_filter, m);
        }

        return sd_bus_reply_method_return(message, "b", cancelled);
}

static int method_can_shutdown_or_sleep(Manager *m,
                                        sd_bus_message *message,
                                        InhibitWhat w,
                                        const char *action,
                                        const char *action_multiple_sessions,
        ...
        if (multiple_sessions) {
                r = bus_test_polkit(
                        message, CAP_SYS_BOOT, action_multiple_sessions, NULL, UID_INVALID, &challenge, error);
                if (r < 0)
                        return r;
        ...
        if (blocked) {
                r = bus_test_polkit(
                        message, CAP_SYS_BOOT, action_ignore_inhibit, NULL, UID_INVALID, &challenge, error);
                if (r < 0)
                        return r;
...
static int method_can_poweroff(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        return method_can_shutdown_or_sleep(m,
                                            message,
                                            INHIBIT_SHUTDOWN,
                                            "org.freedesktop.login1.power-off",
                                            "org.freedesktop.login1.power-off-multiple-sessions",
...
static int property_get_reboot_to_firmware_setup(sd_bus *bus,
                                                 const char *path,
                                                 const char *interface,
                                                 const char *property,
                                                 sd_bus_message *reply,
...
static int method_set_reboot_to_firmware_setup(sd_bus_message *message, void *userdata, sd_bus_error *error) {
...
static int method_can_reboot_to_firmware_setup(sd_bus_message *message, void *userdata, sd_bus_error *error) {
...
static int method_inhibit(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        if (w <= 0)
                return sd_bus_error_setf(
                        error, SD_BUS_ERROR_INVALID_ARGS, "Invalid what specification %s", what);

        mm = inhibit_mode_from_string(mode);
        if (mm < 0)
                return sd_bus_error_setf(
                        error, SD_BUS_ERROR_INVALID_ARGS, "Invalid mode specification %s", mode);
        ...
        r = bus_verify_polkit_async(
                ...
                CAP_SYS_BOOT,
                w == INHIBIT_SHUTDOWN ?
                        (mm == INHIBIT_BLOCK ? "org.freedesktop.login1.inhibit-block-shutdown" :
                                               "org.freedesktop.login1.inhibit-delay-shutdown") :
                        w == INHIBIT_SLEEP ?
                        (mm == INHIBIT_BLOCK ? "org.freedesktop.login1.inhibit-block-sleep" :
...
const sd_bus_vtable manager_vtable[] = {
        ...
        SD_BUS_WRITABLE_PROPERTY(
                "EnableWallMessages", "b", NULL, NULL, offsetof(Manager, enable_wall_messages), 0),
        ...
        SD_BUS_PROPERTY(
                "KillOnlyUsers", "as", NULL, offsetof(Manager, kill_only_users), SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("KillExcludeUsers",
                        "as",
                        NULL,
                        offsetof(Manager, kill_exclude_users),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("KillUserProcesses",
                        "b",
                        NULL,
                        offsetof(Manager, kill_user_processes),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        ...
        SD_BUS_PROPERTY("InhibitDelayMaxUSec",
                        "t",
                        NULL,
                        offsetof(Manager, inhibit_delay_max),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY(
                "UserStopDelayUSec", "t", NULL, offsetof(Manager, user_stop_delay), SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("HandlePowerKey",
                        "s",
                        property_get_handle_action,
                        offsetof(Manager, handle_power_key),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("HandleSuspendKey",
                        "s",
                        property_get_handle_action,
                        offsetof(Manager, handle_suspend_key),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("HandleHibernateKey",
                        "s",
                        property_get_handle_action,
                        offsetof(Manager, handle_hibernate_key),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("HandleLidSwitch",
                        "s",
                        property_get_handle_action,
                        offsetof(Manager, handle_lid_switch),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("HandleLidSwitchExternalPower",
                        "s",
                        property_get_handle_action,
                        offsetof(Manager, handle_lid_switch_ep),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("HandleLidSwitchDocked",
                        "s",
                        property_get_handle_action,
                        offsetof(Manager, handle_lid_switch_docked),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("HoldoffTimeoutUSec",
                        "t",
                        NULL,
                        offsetof(Manager, holdoff_timeout_usec),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("IdleAction",
                        "s",
                        property_get_handle_action,
                        offsetof(Manager, idle_action),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY(
                "IdleActionUSec", "t", NULL, offsetof(Manager, idle_action_usec), SD_BUS_VTABLE_PROPERTY_CONST),
        ...
        SD_BUS_PROPERTY("RemoveIPC",
                        ...
                        offsetof(Manager, remove_ipc),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("RuntimeDirectorySize",
                        "t",
                        NULL,
                        offsetof(Manager, runtime_dir_size),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY(
                "InhibitorsMax", "t", NULL, offsetof(Manager, inhibitors_max), SD_BUS_VTABLE_PROPERTY_CONST),
        ...
        SD_BUS_PROPERTY("UserTasksMax",
                        "t",
                        property_get_compat_user_tasks_max,
                        0,
        ...
        SD_BUS_METHOD(
                "SuspendThenHibernate", "b", NULL, method_suspend_then_hibernate, SD_BUS_VTABLE_UNPRIVILEGED),
        ...
        SD_BUS_METHOD("CanSuspendThenHibernate",
                      NULL,
                      "s",
                      method_can_suspend_then_hibernate,
                      SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_METHOD("ScheduleShutdown", "st", NULL, method_schedule_shutdown, SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_METHOD("CancelScheduledShutdown",
                      NULL,
                      "b",
                      method_cancel_scheduled_shutdown,
                      SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_METHOD("Inhibit", "ssss", "h", method_inhibit, SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_METHOD("CanRebootToFirmwareSetup",
                      NULL,
                      "s",
                      method_can_reboot_to_firmware_setup,
                      SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_METHOD("SetRebootToFirmwareSetup",
                      "b",
                      NULL,
...
static int session_jobs_reply(Session *s, const char *unit, const char *result) {
        ...
        if (result && !streq(result, "done")) {
                ...
                sd_bus_error_setf(
                        &e, BUS_ERROR_JOB_FAILED, "Start job for unit '%s' failed with '%s'", unit, result);
...
int match_job_removed(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        if (user) {
                if (streq_ptr(path, user->service_job)) {
                        ...
                        (void) session_jobs_reply(
                                session,
                                unit,
                                NULL /* don't propagate user service failures to the client */);
...
int manager_send_changed(Manager *manager, const char *property, ...) {
        ...
        return sd_bus_emit_properties_changed_strv(
                manager->bus, "/org/freedesktop/login1", "org.freedesktop.login1.Manager", l);
}
