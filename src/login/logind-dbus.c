...
int manager_load_scheduled_shutdown(Manager *m) {
        ...
        if (r < 0)
                goto out; /* without proper time no point handling the rest */
        ...
        if (handle != _HANDLE_ACTION_INVALID)
                ...
                log_debug("Failed to parse scheduled shutdown type '%s', ignoring.", mode);
                m->scheduled_shutdown_timeout = 0;
                r = -EINVAL;
                goto out;
        }

        /* remaining settings are not crucial, so ignore errors */

        if (warn_wall && parse_boolean(warn_wall) >= 0)
                m->enable_wall_messages = r;

        if(wall_message && cunescape(wall_message, 0, &unescaped) >= 0)
                free_and_strdup_warn(&m->wall_message, empty_to_null(unescaped));

        if (uid)
                parse_uid(uid, &m->scheduled_shutdown_uid);

        free_and_strdup_warn(&m->scheduled_shutdown_tty, empty_to_null(tty));
