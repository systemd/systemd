...
static int send_prepare_for(Manager *m, const HandleActionData *a, bool _active) {
        ...
        if (a->inhibit_what == INHIBIT_SHUTDOWN) {
                k = sd_bus_emit_signal(m->bus,
                                       ...
                                       "PrepareForShutdownWithType",
                                       "bs",
                                       active,
                                       strna(a->log_verb));
                if (k < 0)
                        log_debug_errno(k, "Failed to emit PrepareForShutdownWithType(): %m");
        }
...
static const sd_bus_vtable manager_vtable[] = {
        ...
        SD_BUS_SIGNAL_WITH_ARGS("PrepareForShutdown",
                                SD_BUS_ARGS("b", start),
                                0),
        SD_BUS_SIGNAL_WITH_ARGS("PrepareForShutdownWithType",
                                SD_BUS_ARGS("b", start, "s", type),
                                0),
