...
static int method_do_shutdown_or_sleep(
        ...
        if (with_flags) {
                ...
                if (FLAGS_SET(flags, (SD_LOGIND_REBOOT_VIA_KEXEC|SD_LOGIND_REBOOT_VIA_RENEW)))
                        return sd_bus_error_set(error, SD_BUS_ERROR_INVALID_ARGS,
                                                "Both reboot via kexec and via renew selected, which is not supported");

                if (a->handle != HANDLE_REBOOT) {
                        if (flags & SD_LOGIND_REBOOT_VIA_KEXEC)
                                return sd_bus_error_set(error, SD_BUS_ERROR_INVALID_ARGS,
                                                        "Reboot via kexec is only applicable with reboot operations");
                        if (flags & SD_LOGIND_REBOOT_VIA_RENEW)
                                return sd_bus_error_set(error, SD_BUS_ERROR_INVALID_ARGS,
                                                        "Reboot via renew is only applicable with reboot operations");
                }
        } else {
        ...
        else if ((flags & SD_LOGIND_REBOOT_VIA_RENEW))
                a = handle_action_lookup(HANDLE_RENEW);
...
static int method_schedule_shutdown(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        if (!IN_SET(handle, HANDLE_POWEROFF, HANDLE_REBOOT, HANDLE_RENEW, HANDLE_HALT, HANDLE_KEXEC))
