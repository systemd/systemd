...
static int bus_manager_log_shutdown(
        ...
        const char *message_id = a->message_id;
        const char *message = a->message;
        const char *log_str = a->log_str;

        assert(m);

        message_id = strjoina("MESSAGE_ID=", message_id ? message_id : SD_MESSAGE_SHUTDOWN_STR);
        message = strjoina("MESSAGE=", message ? message : "System is shutting down");
        if (log_str)
                log_str = strjoina("SHUTDOWN=", log_str);

        return log_with_wall_message(m, message_id, message, log_str);
}
...
static int execute_shutdown_or_sleep(
        ...
        _cleanup_(sd_bus_message_unrefp) sd_bus_message *reply = NULL;
        const ActionTableItem *a = manager_item_for_handle(handle);
        const char *p;
        int r;
        ...
        m->delayed_action = handle;
        m->action_what = a->inhibit_what;
...
int manager_dispatch_delayed(Manager *manager, bool timeout) {
        ...
        if (r < 0) {
                ...
                manager->delayed_action = _HANDLE_ACTION_INVALID;
...
int bus_manager_shutdown_or_sleep_now_or_later(
        ...
        if (delayed)
                ...
                r = delay_shutdown_or_sleep(m, handle);
        else
                /* Shutdown is not delayed, execute it
                 * immediately */
                r = execute_shutdown_or_sleep(m, handle, error);
...
static int method_do_shutdown_or_sleep(
        ...
        if (with_flags) {
                ...
                if (handle != HANDLE_REBOOT && (flags & SD_LOGIND_REBOOT_VIA_KEXEC))
        ...
        m->scheduled_shutdown_type = handle;

        (void) setup_wall_message_timer(m, message);

        r = bus_manager_shutdown_or_sleep_now_or_later(m, handle, error);
        if (r < 0)
                return r;
...
static void reset_scheduled_shutdown(Manager *m) {
        ...
        m->scheduled_shutdown_type = _HANDLE_ACTION_INVALID;
...
static int manager_scheduled_shutdown_handler(
        ...
        a = manager_item_for_handle(m->scheduled_shutdown_type);
        assert(a);
...
static int method_schedule_shutdown(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        a = manager_item_for_handle(handle);
        if (!IN_SET(handle, HANDLE_POWEROFF, HANDLE_REBOOT, HANDLE_HALT, HANDLE_KEXEC) || !a || !a->polkit_action)
...
static int method_cancel_scheduled_shutdown(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        cancelled = !IN_SET(m->scheduled_shutdown_type, HANDLE_IGNORE, _HANDLE_ACTION_INVALID);
        if (!cancelled)
                goto done;
...
static int method_can_shutdown_or_sleep(
        ...
        handle = handle_action_from_string(sleep_operation_to_string(a->sleep_operation));
        if (handle >= 0) {
                const char *target;
...
static int method_can_suspend_then_hibernate(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        return method_can_shutdown_or_sleep(
                        m, message, HANDLE_SUSPEND_THEN_HIBERNATE,
                        error);
}
...
int match_job_removed(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        if (m->action_job && streq(m->action_job, path)) {
                ...
                m->delayed_action = _HANDLE_ACTION_INVALID;
                m->action_what = 0;
                return 0;
        }
