...
int manager_load_scheduled_shutdown(Manager *m) {
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "USEC="))) {
                        ...
                        r = free_and_strdup_warn(&m->wall_message, empty_to_null(unescaped));
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "UID="))) {
                        if (parse_uid(val, &m->scheduled_shutdown_uid) < 0)
                                log_notice("Failed to parse UID '%s', ignoring.", val);
                } else if ((val = startswith(l, "TTY="))) {
                        r = free_and_strdup_warn(&m->scheduled_shutdown_tty, empty_to_null(val));
                        if (r < 0)
                                return r;
                }
        }
