...
int manager_load_scheduled_shutdown(Manager *m) {
        ...
        if (r == -ENOENT)
                return 0;
        if (r < 0) {
                log_debug_errno(r, "Failed to parse " SHUTDOWN_SCHEDULE_FILE ": %m");
                goto fail;
        }

        r = deserialize_usec(strnull(usec), &m->scheduled_shutdown_timeout);
        if (r < 0)
                goto fail;

        HandleAction handle = handle_action_from_string(mode);
        if (handle == _HANDLE_ACTION_INVALID) {
                log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to parse scheduled shutdown type '%s'", mode);
                r = -EINVAL;
                goto fail;
        }
        ...
        if (wall_message) {
                ...
                if (r < 0) {
                        log_debug_errno(r, "Failed to parse wall message");
                        goto fail;
                }
                if (free_and_strdup_warn(&m->wall_message, empty_to_null(unescaped)) < 0)
                        goto fail;
        }
        ...
        if (uid) {
                ...
                if (r < 0) {
                        log_debug_errno(r, "Failed to parse wall uid");
                        goto fail;
                }
        }

        if (free_and_strdup_warn(&m->scheduled_shutdown_tty, empty_to_null(tty)) < 0)
                goto fail;

        /* do this last as we'd have to reset on fail and we do not want to repeat any built in default */
        if (warn_wall) {
