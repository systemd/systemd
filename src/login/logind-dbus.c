...
static int create_session(
        ...
        if (pidfd >= 0) {
                session->leader_pidfd = fcntl(pidfd, F_DUPFD_CLOEXEC, 3);
                if (session->leader_pidfd < 0)
                        goto fail;
        } else
                session->leader_pidfd = -EBADF;
...
static int method_create_session(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        return create_session(message, userdata, error,
                              uid, leader, -EBADF /* pidfd= */, service, type, class, desktop, cseat, vtnr, tty, display, remote, remote_user, remote_host);
}
...
static int method_create_session_pidfd(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        r = sd_bus_message_read(message, "uhsssssussbss",
        ...
        return create_session(message, userdata, error,
                              uid, 0 /* pid */, leaderfd, service, type, class, desktop, cseat, vtnr, tty, display, remote, remote_user, remote_host);
}
