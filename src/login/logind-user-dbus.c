...
static void secure_lock_cb(User *u, void *userdata, sd_bus_error *error) {
...
static int bus_user_method_inhibit_auto_secure_lock(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        fd = user_inihibit_auto_secure_lock(u);
        if (fd < 0)
                return fd;
...
int bus_user_method_enable_secure_lock(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        _cleanup_close_ int fd = -EBADF;
        User *u = ASSERT_PTR(userdata);
        int r;
