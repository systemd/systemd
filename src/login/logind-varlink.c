...
static int manager_varlink_get_session_by_name_or_pidref(
         ...
         * argument. If both name and pidref are set they must refer to the same session, otherwise -ESRCH is
         * returned. Returns -ESRCH on "not found". Caller is expected to turn that into a varlink error,
         * typically via sd_varlink_set_sentinel(). Returns negative errno on other failures; a positive
         * return value indicates that a varlink error reply has already been sent. */
