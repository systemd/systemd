...
static int manager_varlink_get_user_by_uid_or_pidref(
        ...
        if (pidref && pidref_is_set(pidref)) {
                ...
                if (r < 0) {
                        /* cg_pidref_get_owner_uid() returns -ENXIO when the PID is not in a
                         * user-N.slice cgroup (e.g. PID 1); translate to -ESRCH so the caller
                         * maps it to the NoSuchUser sentinel. */
