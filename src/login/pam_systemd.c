...
static void session_context_mangle(
        ...
        if (isempty(c->class)) {
                 ...
                 *
                 * - Non-graphical sessions shall be invoked without service manager, and for non-root system
                 *   users TTY sessions, too.
                 *
                ...
                if (IN_SET(user_record_disposition(ur), USER_INTRINSIC, USER_SYSTEM, USER_DYNAMIC)) {
                        if (streq(c->type, "tty") && streq(c->class, "user"))
                                c->class = user_record_is_root(ur) ? "user-early" : "user-light";
