...
static int pam_putenv_and_log(pam_handle_t *handle, const char *e, bool debug) {
        ...
        pam_debug_syslog(handle, "PAM environment variable %s set based on user record.", e);
...
static int apply_user_record_settings(
        ...
        if (ur->umask != MODE_INVALID) {
                umask(ur->umask);
                pam_debug_syslog(handle, "Set user umask to %04o based on user record.", ur->umask);
        }
        ...
        STRV_FOREACH(i, ur->environment) {
                ...
                if (pam_getenv(handle, n)) {
                        pam_debug_syslog(handle, "PAM environment variable $%s already set, not changing based on record.", *i);
                        continue;
                }
        ...
        if (ur->email_address) {
                if (pam_getenv(handle, "EMAIL"))
                        pam_debug_syslog(handle, "PAM environment variable $EMAIL already set, not changing based on user record.");
                else {
        ...
        if (ur->time_zone) {
                if (pam_getenv(handle, "TZ"))
                        pam_debug_syslog(handle, "PAM environment variable $TZ already set, not changing based on user record.");
                else if (!timezone_is_valid(ur->time_zone, LOG_DEBUG))
                        pam_debug_syslog(handle, "Time zone specified in user record is not valid locally, not setting $TZ.");
                else {
                        _cleanup_free_ char *joined = NULL;
        ...
        if (ur->preferred_language) {
                if (pam_getenv(handle, "LANG"))
                        pam_debug_syslog(handle, "PAM environment variable $LANG already set, not changing based on user record.");
                else if (locale_is_installed(ur->preferred_language) <= 0)
                        pam_debug_syslog(handle, "Preferred language specified in user record is not valid or not installed, not setting $LANG.");
                else {
                        _cleanup_free_ char *joined = NULL;
        ...
        if (nice_is_valid(ur->nice_level)) {
                if (nice(ur->nice_level) < 0)
                        ...
                        pam_debug_syslog(handle, "Nice level set to %i, based on user record.", ur->nice_level);
        }
        ...
        for (int rl = 0; rl < _RLIMIT_MAX; rl++) {
                ...
                if (r < 0)
                        ...
                        pam_debug_syslog(handle, "Resource limit %s set, based on user record.", rlimit_to_string(rl));
        }
...
_public_ PAM_EXTERN int pam_sm_open_session(
        ...
        pam_debug_syslog(handle, "pam-systemd initializing");
        ...
        if (seat && !streq(seat, "seat0") && vtnr != 0) {
                pam_debug_syslog(handle, "Ignoring vtnr %"PRIu32" for %s which is not seat0", vtnr, seat);
                vtnr = 0;
        }
        ...
        pam_debug_syslog(handle, "Asking logind to create session: "
        ...
        pam_debug_syslog(handle, "Session limits: "
        ...
        if (r < 0) {
                if (sd_bus_error_has_name(&error, BUS_ERROR_SESSION_BUSY)) {
                        pam_debug_syslog(handle, "Not creating session: %s", bus_error_message(&error, r));
        ...
        pam_debug_syslog(handle, "Reply from logind: "
...
_public_ PAM_EXTERN int pam_sm_close_session(
        ...
        pam_debug_syslog(handle, "pam-systemd shutting down");
