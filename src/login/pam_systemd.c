...
static int export_legacy_dbus_address(
        ...
        if (access(s, F_OK) < 0)  {
                if (errno != ENOENT)
                        pam_syslog_errno(handle, LOG_WARNING, errno, "Failed to check if %s/bus exists, ingoring: %m", runtime);
...
static int update_environment(pam_handle_t *handle, const char *key, const char *value) {
        ...
        /* Updates the environment, and removes environment variables of value is NULL or empty. Also, log
         * about errors */
