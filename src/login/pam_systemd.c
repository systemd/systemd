...
static char **getenv_harder_strv(pam_handle_t *pamh, const char *key, const char *separator) {
        assert(pamh);
        assert(key);
        ...
        if (can_use_varlink(c)) {
                ...
                if (r < 0)
                        ...
                        r = sd_varlink_callbo(
                                        ...
                                        JSON_BUILD_PAIR_CONDITION_STRV(!!c->extra_device_access, "ExtraDeviceAccess", c->extra_device_access));
                        if (r < 0)
...
_public_ PAM_EXTERN int pam_sm_open_session(
        ...
        c.extra_device_access = getenv_harder_strv(pamh, "XDG_SESSION_EXTRA_DEVICE_ACCESS", ":");
