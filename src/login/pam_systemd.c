...
static void session_context_mangle(
        ...
        if (isempty(c->class)) {
                 ...
                 *
                 * - Allow tty logins *before* systemd-user-sessions.service is run, to allow early boot
                 *   logins for root and suchlike to debug things.
                 * - Background sessions shall be invoked without service manager.
                 *
                ...
                if (IN_SET(user_record_disposition(ur), USER_INTRINSIC, USER_SYSTEM, USER_DYNAMIC)) {
                        if (streq(c->type, "tty") && streq(c->class, "user"))
                                c->class = "user-early";
