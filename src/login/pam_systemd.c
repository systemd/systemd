...
static int open_osc_context(pam_handle_t *handle, const char *session_type, UserRecord *ur, bool debug) {
        ...
        if (tty_opath_fd < 0)
                pam_debug_syslog(handle, debug, "Failed to pin TTY, ignoring.");
        else
...
static int close_osc_context(pam_handle_t *handle, bool debug) {
        ...
        if (fd < 0) {
                pam_debug_syslog(handle, debug, "Failed to reopen TTY, ignoring.");
                return PAM_SUCCESS;
        }
        ...
        if (r < 0)
                pam_debug_syslog(handle, debug, "Failed to write OSC sequence to TTY, ignoring.");
