...
static const char* getenv_harder(pam_handle_t *pamh, bool debug, const char *key, const char *fallback) {
        ...
        if (!isempty(v)) {
                pam_debug_syslog(pamh, debug, "Using PAM environment variable %s='%s'", key, v);
                return v;
        }
        ...
        v = secure_getenv(key);
        if (!isempty(v)) {
                pam_debug_syslog(pamh, debug, "Using normal environment variable %s='%s'", key, v);
                return v;
        }

        pam_debug_syslog(pamh, debug, "Using fallback %s='%s'", key, strempty(fallback));
        return fallback;
}
