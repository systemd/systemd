...
static int manager_on_secure_lock_notification(
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                { "uid",    SD_JSON_VARIANT_INTEGER, sd_json_dispatch_uid_gid, offsetof(struct sl_notif_params, uid),    0 },
                { "locked", SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_stdbool, offsetof(struct sl_notif_params, locked), 0 },
                {}
        };
...
static int manager_enumerate_secure_lock_backends(Manager *m) {
        ...
        FOREACH_DIRENT(de, d, return -errno) {
                ...
                r = sd_varlink_bind_reply(link, manager_on_secure_lock_notification);
                if (r < 0) {
                        log_error_errno(r, "%s: Failed to attach notification callback: %m", path);
                        continue;
                }

                r = sd_varlink_observeb(link,
                                     "io.systemd.SecureLockBackend.Subscribe",
                                     SD_JSON_BUILD_OBJECT(
                                        SD_JSON_BUILD_PAIR_STRING("service", de->d_name)));
                if (r < 0) {
