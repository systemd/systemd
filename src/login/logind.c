        ...
        for (int i = 0; i < n; i++) {
                ...
                if (r < 0) {
                        RET_GATHER(ret, log_warning_errno(r,
                                                          "Failed to parse stored fd name '%s', ignoring: %m",
                                                          fdnames[i]));
                        goto fail_skip;
                }
                ...
                if (!s) {
                        ...
                        RET_GATHER(ret, log_debug_errno(SYNTHETIC_ERRNO(ENXIO),
                ...
                if (devno > 0) {
                        ...
                        if (r < 0) {
                                RET_GATHER(ret, log_warning_errno(r,
                                                                  "Failed to create reference to leader of session '%s', ignoring: %m",
                                                                  id));
                                goto fail_skip;
                        }
