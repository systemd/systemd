...
static int session_dispatch_stop_on_idle(sd_event_source *source, uint64_t t, void *userdata) {
        ...
        r = sd_event_now(s->manager->event, CLOCK_MONOTONIC, &now);
        if (r < 0)
                return r;
        ...
        r = sd_event_source_set_time(source,  ts.monotonic + s->manager->stop_idle_session_usec);
        if (r < 0)
...
static int session_setup_stop_on_idle_timer(Session *s) {
        ...
        r = sd_event_source_set_priority(s->stop_on_idle_event_source, SD_EVENT_PRIORITY_IMPORTANT);
        if (r < 0)
                return log_error_errno(r, "Failed to configure stop on idle session event source priority: %m");

        r = sd_event_source_set_enabled(s->stop_on_idle_event_source, SD_EVENT_ONESHOT);
        if (r < 0)
                return log_error_errno(r, "Failed to enable stop on idle session event timer: %m");
