...
static int session_start_scope(Session *s, sd_bus_message *properties, sd_bus_error *error) {
        ...
        r = manager_start_scope(
                        ...
                        s->user->slice,
                        description,
                        /* user-runtime-dir@.service is required. It has StopWhenUnneeded=yes set, so we also
                         * pin it through dep. Also, with job mode 'fail', pid1 would prefer deleting our
                         * start jobs if there're running stop jobs for these two units, as per
                         * transaction_minimize_impact(). Thus Requires= is used to prevent that. */
