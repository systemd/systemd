...
static int chase_and_access(
        ...
        assert(rfd >= 0);
        assert(path);
        assert(!(flags & ~CHASE_NOFOLLOW|CHASE_PROHIBIT_SYMLINKS));

        pfd = chase_symlinks_at_and_open(rfd, path,
                                         CHASE_AT_RESOLVE_IN_ROOT|CHASE_MKDIR_0755|CHASE_PARENT|
                                         CHASE_FILENAME|CHASE_WARN|flags,
                                         O_PATH|O_CLOEXEC,
                                         &f);
        if (pfd < 0)
                return log_error_errno(pfd, "Failed to chase /%s: %m", path);

        r = RET_NERRNO(faccessat(pfd, f, F_OK, AT_SYMLINK_NOFOLLOW));
        if (r < 0 && r != -ENOENT)
                return r;

        if (ret_parent_fd)
                *ret_parent_fd = TAKE_FD(pfd);
        if (ret_filename)
                *ret_filename = TAKE_PTR(f);

        if (r >= 0 && !arg_force) {
                log_debug("Found /%s, assuming it is already configured.", path);
                return 0;
        }
...
static int process_locale(int rfd) {
        ...
        r = chase_and_access(rfd, "/etc/locale.conf", 0, &pfd, &f);
        if (r <= 0)
                return r;
...
static int process_keymap(int rfd) {
        ...
        r = chase_and_access(rfd, "/etc/vconsole.conf", 0, &pfd, &f);
        if (r <= 0)
                return r;
...
static int process_timezone(int rfd) {
        ...
        r = chase_and_access(rfd, "/etc/localtime", CHASE_NOFOLLOW, &pfd, &f);
        if (r <= 0)
                return r;
...
static int process_hostname(int rfd) {
        ...
        r = chase_and_access(rfd, "/etc/hostname", 0, &pfd, &f);
        if (r <= 0)
                return r;
...
static int process_machine_id(int rfd) {
        ...
        r = chase_and_access(rfd, "/etc/machine-id", 0, &pfd, &f);
        if (r <= 0)
                return r;
...
static int process_root_account(int rfd) {
        ...
        FOREACH_STRING(s, "passwd", "shadow") {
                ...
                r = chase_and_access(rfd, p, CHASE_NOFOLLOW, &pfd, NULL);
                if (r < 0)
                        return r;

                k += r;

                r = verify_regular_at(pfd, s);
                if (r == -ENOENT)
                        continue;
                if (IN_SET(r, -EISDIR, -ELOOP, -EBADFD))
                        return log_error_errno(r, "%s is not a regular file", p);
                if (r < 0)
                        return log_error_errno(r, "Failed to check whether %s is a regular file: %m", p);
        }
...
static int process_kernel_cmdline(int rfd) {
        ...
        r = chase_and_access(rfd, "/etc/kernel/cmdline", 0, &pfd, &f);
        if (r <= 0)
                return r;
