...
static int process_locale(int rfd) {
        ...
        r = chase_symlinks_at(rfd, "etc", CHASE_AT_RESOLVE_IN_ROOT|CHASE_NONEXISTENT, &p, NULL);
        if (r < 0)
        ...
        if (pfd < 0)
                return log_error_errno(pfd, "Failed to create %s/%s", FORMAT_FD_PATH(rfd, &buf), p);
...
static int process_kernel_cmdline(int rfd) {
        ...
        r = chase_symlinks_at(rfd, "etc/kernel", CHASE_AT_RESOLVE_IN_ROOT|CHASE_NONEXISTENT, &p, NULL);
        if (r < 0)
