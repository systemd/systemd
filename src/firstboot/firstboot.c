...
static int process_locale(int rfd) {
        ...
        r = chase_symlinks_at(rfd, "etc", CHASE_AT_RESOLVE_IN_ROOT|CHASE_NONEXISTENT, NULL, &pfd);
        if (r < 0)
                return log_error_errno(r, "Failed to resolve %s/etc/: %m", FORMAT_FD_PATH(rfd));
        ...
        if (arg_copy_locale && rfd >= 0) {
                ...
                if (pfd < 0) {
                        pfd = open_mkdir_at(rfd, "etc", O_PATH|O_CLOEXEC|O_EXCL, 0755);
                        if (pfd < 0)
                                return log_error_errno(pfd, "Failed to create %s/etc/", FORMAT_FD_PATH(rfd));
                }
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/locale.conf: %m", FORMAT_FD_PATH(rfd));

        log_info("%s/etc/locale.conf written.", FORMAT_FD_PATH(rfd));
        return 0;
}
...
static int process_keymap(int rfd) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/vconsole.conf: %m", FORMAT_FD_PATH(rfd));

        log_info("%s/etc/vconsole.conf written.", FORMAT_FD_PATH(rfd));
        return 0;
}
...
static int process_timezone(int rfd) {
        ...
        if (arg_copy_timezone && rfd >= 0) {
                ...
                if (r != -ENOENT) {
                        ...
                        r = symlinkat_atomic_full(p, pfd, "localtime", /* make_relative= */ false);
                        if (r < 0)
        ...
        log_info("%s/etc/localtime written", FORMAT_FD_PATH(rfd));
        return 0;
}
...
static int process_hostname(int rfd) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/hostname: %m", FORMAT_FD_PATH(pfd));

        log_info("%s/etc/hostname written.", FORMAT_FD_PATH(rfd));
        return 0;
}
...
static int process_machine_id(int rfd) {
        ...
        r = write_string_file_at(pfd, "machine-id", SD_ID128_TO_STRING(arg_machine_id),
                                 ...
                                 (arg_force ? WRITE_STRING_FILE_ATOMIC : 0));
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/machine id: %m", FORMAT_FD_PATH(rfd));

        log_info("%s/etc/machine-id written.", FORMAT_FD_PATH(rfd));
        return 0;
}
...
static int process_root_account(int rfd) {
        ...
        r = chase_symlinks_at(rfd, "etc",
                              CHASE_AT_RESOLVE_IN_ROOT|CHASE_NONEXISTENT,
                              NULL, &pfd);
        if (r < 0)
                return log_error_errno(r, "Failed to resolve %s/etc: %m", FORMAT_FD_PATH(rfd));
        ...
        if (faccessat(pfd, "passwd", F_OK, AT_SYMLINK_NOFOLLOW) >= 0 &&
            faccessat(pfd, "shadow", F_OK, AT_SYMLINK_NOFOLLOW) >= 0 && !arg_force) {
                log_debug("Found %s/etc/passwd and %s/etc/shadow, assuming root account has been initialized.",
                          FORMAT_FD_PATH(rfd), FORMAT_FD_PATH(rfd));
                return 0;
        }
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/passwd: %m", FORMAT_FD_PATH(rfd));

        log_info("%s/etc/passwd written.", FORMAT_FD_PATH(rfd));

        r = write_root_shadow(pfd, hashed_password);
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/shadow: %m", FORMAT_FD_PATH(rfd));

        log_info("%s/etc/shadow written.", FORMAT_FD_PATH(rfd));
        return 0;
}
...
static int process_kernel_cmdline(int rfd) {
        ...
        r = chase_symlinks_at(rfd, "etc/kernel", CHASE_AT_RESOLVE_IN_ROOT|CHASE_NONEXISTENT, &resolved, &pfd);
        if (r < 0)
                return log_error_errno(r, "Failed to resolve %s/etc/kernel: %m", FORMAT_FD_PATH(rfd));
        ...
        if (pfd < 0) {
                pfd = open_mkdir_at_p(rfd, resolved, O_CLOEXEC|O_PATH|O_EXCL, 0755);
                if (pfd < 0)
                        return pfd;
        }
        ...
        r = write_string_file_at(pfd, "cmdline", arg_kernel_cmdline,
                                 ...
                                 (arg_force ? WRITE_STRING_FILE_ATOMIC : 0));
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/kernel/cmdline: %m", FORMAT_FD_PATH(rfd));

        log_info("%s/etc/kernel/cmdline written.", FORMAT_FD_PATH(rfd));
        return 0;
}
...
static int process_reset(int rfd) {
        ...
        FOREACH_STRING(s,
                ...
                r = chase_symlinks_at(rfd, s, CHASE_AT_RESOLVE_IN_ROOT|CHASE_NONEXISTENT|CHASE_PARENT, NULL,
                                      &pfd);
                if (r == 0)
                        continue;
                if (r < 0)
                        return log_error_errno(r, "Failed to resolve %s/%s: %m", FORMAT_FD_PATH(rfd), s);
                ...
                if (r == -ENOENT)
                        continue;
                if (r < 0)
                        return log_error_errno(errno, "Failed to remove %s/%s: %m", FORMAT_FD_PATH(pfd), bn);

                log_info("Removed %s/%s.", FORMAT_FD_PATH(pfd), bn);
        }
        ...
        r = chase_symlinks_at(esp_fd, "loader",
                              CHASE_AT_RESOLVE_IN_ROOT|CHASE_PROHIBIT_SYMLINKS|CHASE_NONEXISTENT|CHASE_PARENT,
                              NULL, &loader_fd);
        if (r == 0)
                return 0;
        if (r < 0)
                return log_error_errno(r, "Failed to resolve %s/loader: %m", FORMAT_FD_PATH(esp_fd));

        r = RET_NERRNO(unlinkat(loader_fd, "random-seed", 0));
        if (r == -ENOENT)
                return 0;
        if (r < 0)
                return log_error_errno(errno, "Failed to remove %s/random-seed: %m", FORMAT_FD_PATH(loader_fd));

        log_info("Removed %s/random-seed.", FORMAT_FD_PATH(loader_fd));
