...
static int process_locale(int rfd) {
        ...
        pfd = chase_symlinks_at_and_open_mkdir_p(rfd, "etc", CHASE_AT_RESOLVE_IN_ROOT, O_CLOEXEC|O_PATH, 0755, &p);
        if (pfd < 0)
...
static int process_kernel_cmdline(int rfd) {
        ...
        pfd = chase_symlinks_at_and_open_mkdir_p(rfd, "etc/kernel", CHASE_AT_RESOLVE_IN_ROOT, O_CLOEXEC|O_PATH, 0755, &p);
        if (pfd < 0)
