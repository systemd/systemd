...
static int process_locale(int rfd) {
        ...
        r = chase_symlinks_at(rfd, "etc", CHASE_AT_RESOLVE_IN_ROOT|CHASE_NONEXISTENT, &p, &pfd);
        if (r < 0)
                return log_error_errno(r, "Failed to resolve %s/etc/: %m", FORMAT_FD_PATH(rfd, &buf));

        if (pfd >= 0 && faccessat(pfd, "locale.conf", F_OK, AT_SYMLINK_NOFOLLOW) >= 0 && !arg_force) {
                log_debug("Found %s/etc/locale.conf, assuming locale information has been configured.",
                          FORMAT_FD_PATH(rfd, &buf));
                return 0;
        }
        ...
        if (arg_copy_locale && rfd >= 0) {
                ...
                if (pfd < 0) {
                        pfd = open_mkdir_at_p(rfd, p, O_PATH|O_CLOEXEC|O_EXCL, 0755);
                        if (pfd < 0)
                                return log_error_errno(pfd, "Failed to create %s/etc/",
                                                       FORMAT_FD_PATH(rfd, &buf));
                }
        ...
        if (pfd < 0) {
                pfd = open_mkdir_at_p(rfd, p, O_PATH|O_CLOEXEC|O_EXCL, 0755);
                if (pfd < 0)
                        return log_error_errno(pfd, "Failed to create %s/etc/", FORMAT_FD_PATH(rfd, &buf));
        }

        r = write_env_file_at(pfd, "locale.conf", locales);
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/locale.conf: %m", FORMAT_FD_PATH(rfd, &buf));

        log_info("%s/etc/locale.conf written.", FORMAT_FD_PATH(rfd, &buf));
        return 0;
}
...
static int process_keymap(int rfd) {
        ...
        if (pfd >= 0 && faccessat(pfd, "vconsole.conf", F_OK, AT_SYMLINK_NOFOLLOW) >= 0 && !arg_force) {
                log_debug("Found %s/etc/vconsole.conf, assuming console has been configured.",
                          FORMAT_FD_PATH(rfd, &buf));
                return 0;
        }
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/vconsole.conf: %m",
                                       FORMAT_FD_PATH(rfd, &buf));

        log_info("%s/etc/vconsole.conf written.", FORMAT_FD_PATH(rfd, &buf));
        return 0;
}
...
static int process_timezone(int rfd) {
        ...
        if (pfd >= 0 && faccessat(pfd, "localtime", F_OK, AT_SYMLINK_NOFOLLOW) >= 0 && !arg_force) {
                log_debug("Found %s/etc/localtime, assuming timezone has been configured.",
                          FORMAT_FD_PATH(rfd, &buf));
                return 0;
        }
        ...
        if (arg_copy_timezone && rfd >= 0) {
                ...
                if (r != -ENOENT) {
                        ...
                        if (r < 0)
                                return log_error_errno(r, "Failed to create %s/etc/localtime symlink: %m",
                                                       FORMAT_FD_PATH(rfd, &buf));
        ...
        log_info("%s/etc/localtime written", FORMAT_FD_PATH(rfd, &buf));
        return 0;
}
...
static int process_hostname(int rfd) {
        ...
        if (pfd >= 0 && faccessat(pfd, "hostname", F_OK, AT_SYMLINK_NOFOLLOW) >= 0 && !arg_force) {
                log_debug("Found %s/etc/hostname, assuming hostname has been configured.",
                          FORMAT_FD_PATH(rfd, &buf));
                return 0;
        }
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/hostname: %m", FORMAT_FD_PATH(pfd, &buf));

        log_info("%s/etc/hostname written.", FORMAT_FD_PATH(rfd, &buf));
        return 0;
}
...
static int process_machine_id(int rfd) {
        ...
        if (pfd >= 0 && faccessat(pfd, "machine-id", F_OK, AT_SYMLINK_NOFOLLOW) >= 0 && !arg_force) {
                log_debug("Found %s/etc/machine-id, assuming machine-id has been configured.",
                          FORMAT_FD_PATH(rfd, &buf));
                return 0;
        }
        ...
        r = write_string_file_at(pfd, "machine-id", SD_ID128_TO_STRING(arg_machine_id),
                                 ...
                                 (arg_force ? WRITE_STRING_FILE_ATOMIC : 0));
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/machine id: %m", FORMAT_FD_PATH(rfd, &buf));

        log_info("%s/etc/machine-id written.", FORMAT_FD_PATH(rfd, &buf));
        return 0;
}
...
static int write_root_passwd(int efd, const char *password, const char *shell) {
        ...
        r = fopen_temporary_at_label(efd, "passwd", "passwd", &passwd, &passwd_tmp);
        if (r < 0)
                return r;

        r = xfopenat(efd, "passwd", "re", 0, &original);
        if (r < 0 && r != -ENOENT)
                return r;
        ...
        r = renameat_and_apply_smack_floor_label(efd, passwd_tmp, efd, "passwd");
        if (r < 0)
                return r;
...
static int process_root_account(int rfd) {
        ...
        if (pfd >= 0 &&
            faccessat(pfd, "passwd", F_OK, AT_SYMLINK_NOFOLLOW) >= 0 &&
            faccessat(pfd, "shadow", F_OK, AT_SYMLINK_NOFOLLOW) >= 0 && !arg_force) {
                const char *root = FORMAT_FD_PATH(rfd, &buf);
                log_debug("Found %s/etc/passwd and %s/etc/shadow, assuming root account has been initialized.",
                          root, root);
                return 0;
        }
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/passwd: %m", FORMAT_FD_PATH(rfd, &buf));

        log_info("%s/etc/passwd written.", FORMAT_FD_PATH(rfd, &buf));

        r = write_root_shadow(pfd, hashed_password);
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/shadow: %m", FORMAT_FD_PATH(rfd, &buf));

        log_info("%s/etc/shadow written.", FORMAT_FD_PATH(rfd, &buf));
        return 0;
}
...
static int process_kernel_cmdline(int rfd) {
        ...
        r = chase_symlinks_at(rfd, "etc/kernel", CHASE_AT_RESOLVE_IN_ROOT|CHASE_NONEXISTENT, &p, &pfd);
        if (r < 0)
                return log_error_errno(r, "Failed to resolve %s/etc/kernel: %m", FORMAT_FD_PATH(rfd, &buf));

        if (pfd >= 0 && faccessat(pfd, "cmdline", F_OK, AT_SYMLINK_NOFOLLOW) >= 0 && !arg_force) {
                log_debug("Found %s/etc/kernel/cmdline, assuming kernel has been configured.",
                          FORMAT_FD_PATH(rfd, &buf));
                return 0;
        }
        ...
        if (pfd < 0) {
                pfd = open_mkdir_at_p(rfd, p, O_CLOEXEC|O_PATH|O_EXCL, 0755);
                if (pfd < 0)
                        return pfd;
        }
        ...
        r = write_string_file_at(pfd, "cmdline", arg_kernel_cmdline,
                                 ...
                                 (arg_force ? WRITE_STRING_FILE_ATOMIC : 0));
        if (r < 0)
                return log_error_errno(r, "Failed to write %s/etc/kernel/cmdline: %m",
                                       FORMAT_FD_PATH(rfd, &buf));

        log_info("%s/etc/kernel/cmdline written.", FORMAT_FD_PATH(rfd, &buf));
        return 0;
}

static int process_reset(int rfd) {
        _cleanup_free_ char *buf = NULL;
        _cleanup_close_ int esp_fd = -EBADF, loader_fd = -EBADF;
        int r;
        ...
        FOREACH_STRING(s,
                ...
                r = chase_symlinks_at(rfd, s, CHASE_AT_RESOLVE_IN_ROOT|CHASE_PARENT, NULL, &pfd);
                if (r == -ENOENT)
                        continue;
                if (r < 0)
                        return log_error_errno(r, "Failed to resolve %s/%s: %m", FORMAT_FD_PATH(rfd, &buf), s);

                r = path_extract_filename(s, &bn);
                if (r < 0)
                        return r;

                r = RET_NERRNO(unlinkat(pfd, bn, 0));
                if (r == -ENOENT)
                        continue;
                if (r < 0)
                        return log_error_errno(r, "Failed to remove %s/%s: %m", FORMAT_FD_PATH(rfd, &buf), s);

                log_info("Removed %s/%s.", FORMAT_FD_PATH(pfd, &buf), bn);
        }

        r = find_esp_at_and_warn(rfd, NULL, /* unprivileged_mode= */ false, NULL,
                                 &esp_fd, NULL, NULL, NULL, NULL, NULL);
        if (r < 0) {
                if (r == -ENOKEY) {
        ...
        r = chase_symlinks_at(esp_fd, "loader", CHASE_AT_RESOLVE_IN_ROOT|CHASE_PROHIBIT_SYMLINKS,
                              NULL, &loader_fd);
        if (r == -ENOENT)
                return 0;
        if (r < 0)
                return log_error_errno(r, "Failed to resolve %s/loader: %m", FORMAT_FD_PATH(esp_fd, &buf));

        r = RET_NERRNO(unlinkat(loader_fd, "random-seed", 0));
        if (r == -ENOENT)
                return 0;
        if (r < 0)
                return log_error_errno(r, "Failed to remove %s/loader/random-seed: %m",
                                       FORMAT_FD_PATH(esp_fd, &buf));

        log_info("Removed %s/random-seed.", FORMAT_FD_PATH(loader_fd, &buf));
