...
static int prompt_timezone(int rfd) {
        ...
        r = get_timezone(&zone);
        if (r < 0)
                return log_error_errno(r, "Cannot query timezone: %m");
        ...
        r = prompt_loop(rfd, "Please enter the new timezone name or number",
                        zones, zone, 30, timezone_is_ok, &arg_timezone);
        if (r < 0)
                return r;
