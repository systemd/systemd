...
static const struct iovec nul_iovec = IOVEC_MAKE((void*) &zero, sizeof(zero));
...
static int process_vsock(const char *host, const char *port) {
        ...
        if (streq(host, "hypervisor"))
                sa.vm.svm_cid = VMADDR_CID_HYPERVISOR;
        else if (streq(host, "local"))
                sa.vm.svm_cid = VMADDR_CID_LOCAL;
        else if (streq(host, "host"))
                sa.vm.svm_cid = VMADDR_CID_HOST;
        else {
                r = safe_atou(host, &sa.vm.svm_cid);
                if (r < 0)
                        return log_error_errno(r, "Failed to parse vsock cid: %s", host);
        }

        r = safe_atou(port, &sa.vm.svm_port);
        if (r < 0)
        ...
        r = send_one_fd_iov(STDOUT_FILENO, fd, &nul_iovec, /* n_iovec= */ 1, /* flags= */ 0);
        if (r < 0)
