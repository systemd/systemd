...
static int process_vsock_indirect(const char *path, const char *port) {
        int r;
        ...
        int connect_cmd_len = strlen(connect_cmd);
        r = write(fd, connect_cmd, connect_cmd_len);
        if (r != connect_cmd_len)
...
static int run(int argc, char* argv[]) {
        ...
        p = startswith(host, "vsock-indirect/");
        if (p)
                return process_vsock_indirect(p, port);
