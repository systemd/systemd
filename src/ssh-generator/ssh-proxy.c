...
static int skip_ok_port_res(int fd, const char *path, const char *port) {
        ...
        if (setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &newtv, sizeof(newtv)) < 0)
                return log_error_errno(errno, "Failed to set socket receive timeout for %s", path);
                        ...
                        if (bytes_avail > bytes_recv) {
                                /*
                                  ...
                                  XXX: We could have used SO_PEEK_OFF to continously peek more data without changing the socket
                                       receive buffer, but this fucntion breaks since Linux 4.3 due to a kernel bug, which
                                       is fixed in Linux 6.18 commit 7bf3a476ce43 ("af_unix: Read sk_peek_offset() again after sleeping in unix_stream_read_generic().").
                                       It is also not possible to detect whether the kernel is affected by this bug at runtime.
