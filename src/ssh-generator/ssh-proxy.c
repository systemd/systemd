...
static int process_machine(const char *machine, const char *port) {
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                ...
                { "class",    SD_JSON_VARIANT_STRING,   sd_json_dispatch_const_string, voffsetof(p, class),   0 },
        ...
        if (streq(p.class, "container")) {
                ...
                if (!streq(p.service, "systemd-nspawn"))
                ...
                r = RET_NERRNO(access(path, F_OK));
                if (r == -ENOENT)
                        return log_error_errno(r, "%s does not exist, are sshd and systemd-ssh-generator installed and enabled in the container?", path);
                if (r < 0)
                        return log_error_errno(r, "Failed to check if %s exists: %m", path);
