...
/* A small generator binding potentially three AF_VSOCK sockets
 *
 ...
 * logic, but without waiting for networking or suchlike. The latter allows the same for local clients.
 *
 * Note: this does *not* make SSH listen on IP. That's left for users/packagers to enable manually. */

// fixme: also by cred and per kernel cmdline
// fixme: log command line to use for connection explicitly
// fixme: also do machine/ which used machined supplied info and automatically connects to the right place
...
static int write_socket_unit(
        ...
        fprintf(f,
                "[Unit]\n"
                "Description=OpenSSH Server Socket (%s)\n"
        ...
        r = fflush_and_check(f);
        if (r < 0)
                return log_error_errno(r, "Failed to write %s SSH socket unit: %m", af);
...
static int add_vsock_socket(
        ...
        if (vsock_fd < 0)
                return log_error_errno(errno, "Failed to open /dev/vsock: %m");

        unsigned local_cid = 0;
        if (ioctl(vsock_fd, IOCTL_VM_SOCKETS_GET_LOCAL_CID, &local_cid) < 0)
                return log_error_errno(errno, "Failed to query local AF_VSOCK CID: %m");
        ...
        log_info("Binding SSH to AF_VSOCK vsock:%u:22", local_cid);
        return 0;
}
...
static int add_export_unix_socket(
        ...
        if (access("/run/host/", F_OK) < 0) {
                if (errno == ENOENT) {
                        log_debug("Container manager does not provide /run/host mount, not binding AF_UNIX socket there.");
                        return 0;
                }

                return log_debug_errno(errno, "Unable to check if /run/host/ exists: %m");
        }
