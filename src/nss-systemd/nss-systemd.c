...
enum nss_status _nss_systemd_setpwent(int stayopen) {
         ...
         * user database. */
        r = userdb_all(NULL, nss_glue_userdb_flags() | USERDB_DONT_SYNTHESIZE, &getpwent_data.iterator);
...
enum nss_status _nss_systemd_setgrent(int stayopen) {
        ...
        r = groupdb_all(NULL, nss_glue_userdb_flags() | USERDB_DONT_SYNTHESIZE, &getgrent_data.iterator);
        return r < 0 ? NSS_STATUS_UNAVAIL : NSS_STATUS_SUCCESS;
}
...
enum nss_status _nss_systemd_setspent(int stayopen) {
        ...
        /* See _nss_systemd_setpwent() for an explanation why we use USERDB_DONT_SYNTHESIZE here */
        r = userdb_all(NULL, nss_glue_userdb_flags() | USERDB_DONT_SYNTHESIZE, &getspent_data.iterator);
        return r < 0 ? NSS_STATUS_UNAVAIL : NSS_STATUS_SUCCESS;
}
...
enum nss_status _nss_systemd_setsgent(int stayopen) {
        ...
        /* See _nss_systemd_setpwent() for an explanation why we use USERDB_DONT_SYNTHESIZE here */
        r = groupdb_all(NULL, nss_glue_userdb_flags() | USERDB_DONT_SYNTHESIZE, &getsgent_data.iterator);
        return r < 0 ? NSS_STATUS_UNAVAIL : NSS_STATUS_SUCCESS;
}
...
enum nss_status _nss_systemd_initgroups_dyn(
        ...
        for (;;) {
                ...
                r = groupdb_by_name(NULL, group_name, (nss_glue_userdb_flags() & ~USERDB_EXCLUDE_NSS) | USERDB_SUPPRESS_SHADOW, &g);
                if (r == -ESRCH)
                        continue;
                if (r < 0) {
