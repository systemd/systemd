...
static int nvme_subsystem_report(NvmeSubsystem *subsystem, NvmePort *ipv4, NvmePort *ipv6) {
        ...
        if (ipv4) {
                if (strextendf(&ps, " – IPv4 port %" PRIu16, ipv4->portnr) < 0)
                        return log_oom();
        }

        if (ipv6) {
                if (strextendf(&ps, " – IPv6 port %" PRIu16, ipv6->portnr) < 0)
                        return log_oom();
        }

        log_info("NVMe-TCP: %s %s%s", special_glyph(SPECIAL_GLYPH_ARROW_RIGHT), subsystem->name, ps);
        ...
        FOREACH_ARRAY(a, addresses, n_addresses) {
                ...
                log_info("     Try for specific device: nvme connect -t tcp -n '%s' -a %s -s %" PRIu16,
                         subsystem->name,
...
static int nvme_port_report(NvmePort *port) {
        ...
        log_info("NVMe-TCP: %s Listening on %s (port %" PRIu16 ")", special_glyph(SPECIAL_GLYPH_ARROW_RIGHT), af_to_ipv4_ipv6(port->ip_family), port->portnr);

        FOREACH_ARRAY(a, addresses, n_addresses) {
                log_info("     Try for all devices: nvme connect-all -t tcp -a %s -s %" PRIu16,
                         IN_ADDR_TO_STRING(a->family, &a->address),
                         port->portnr);
        }
...
static int run(int argc, char* argv[]) {
        ...
        if (isatty(0) > 0)
                log_notice("Hit Ctrl-C to exit target mode.");
