...
static int reconfigure_repart(void) {
        _cleanup_free_ char *definitions = NULL;

        definitions = strv_join_full(CONF_PATHS_STRV("repart.installer.d"), " ", "--definitions=",
                                     /* escape_separator */ false);
        if (!definitions)
                return log_oom();

        return write_drop_in_format(arg_dest, REPART_SERVICE, 50, "installer-definitions",
                                    "# Automatically generated by systemd-installer-generator\n\n"
                                    "[Service]\n"
                                    "ExecStart=\n"
                                    "ExecStart=" ROOTBINDIR "/systemd-repart --dry-run=no %s\n",
                                    definitions);
}
