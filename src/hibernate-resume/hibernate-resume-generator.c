...
static int parse_efi_hibernate_info(void) {
        ...
        r = efi_get_variable_string(EFI_SYSTEMD_VARIABLE_STR(info_name), &info);
        if (r == -ENOENT)
                return 0;
        if (r < 0)
                return log_full_errno(log_level, r, "Failed to get EFI variable '%s': %m", info_name);

        r = efi_set_variable(EFI_SYSTEMD_VARIABLE_STR(name), NULL, 0);
        if (r < 0)
                log_warning_errno(r, "Failed to clear EFI variable '%s', ignoring: %m", info_name);
        ...
        if (strv_length(info_split) != 2)
                return log_full_errno(log_level, SYNTHETIC_ERRNO(EINVAL), "Failed to parse EFI variable '%s'.", info_name);
...
static int run(const char *dest, const char *dest_early, const char *dest_late) {
        ...
        if (is_efi_boot()) {
                ...
                /* Ensure that generic HibernateInfo will not survive more than one boot
                 * to avoid spurious reuse. */
