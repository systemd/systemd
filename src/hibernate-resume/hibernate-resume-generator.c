...
static int parse_efi_hibernate_info(void) {
        ...
        if (strv_length(info_split) != 2)
                return log_full_errno(log_level, SYNTHETIC_ERRNO(EINVAL), "Failed to parse EFI variable HibernateInfo: %m");
        ...
        if (r < 0)
                return log_full_errno(log_level, r, "Failed to get resume offset through HibernateInfo: %m");
