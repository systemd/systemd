...
static int parse_efi_hibernate_location(void) {
        ...
        if (!streq_ptr(id, location.id) ||
            ...
            !streq_ptr(uts.release, location.kernel_version)) {
        ...
        if (!arg_resume_device) {
                ...
                if (arg_resume_offset != location.offset)
                        log_warning("resume_offset=%" PRIu64 " mismatches with HibernateLocation offset %" PRIu64 ", proceeding anyway with resume_offset=.",
                                    arg_resume_offset, location.offset);
        }
