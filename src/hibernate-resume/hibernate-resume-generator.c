...
static int process_resume(void) {
        ...
        if (!arg_resume_device && !arg_resume_device_efi)
                return 0;

        /* We always use the device from resume= if provided */
        r = unit_name_from_path(arg_resume_device ?: arg_resume_device_efi, ".device", &device_unit);
        if (r < 0)
                return log_error_errno(r, "Failed to generate device unit name from path '%s': %m",
        ...
        /* We don't need to escape arg_resume_device_efi, as it's formatted from UUID above */
