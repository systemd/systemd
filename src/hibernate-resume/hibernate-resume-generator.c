...
static int process_resume(void) {
        _cleanup_free_ char *service_unit = NULL, *device_unit = NULL, *link = NULL;
        int r;
        ...
        link = path_join(arg_dest, SPECIAL_SYSINIT_TARGET ".wants", service_unit);
        if (!link)
                return log_oom();

        (void) mkdir_parents_label(link, 0755);
        if (symlink(SYSTEM_DATA_UNIT_DIR "/systemd-hibernate-resume@.service", link) < 0)
                return log_error_errno(errno, "Failed to create symlink '%s': %m", link);

        if (arg_resume_offset) {
                r = write_drop_in_format(arg_dest, service_unit, 10, "resume-offset",
                                         "# Automatically generated by systemd-hibernate-resume-generator\n\n"
                                         "[Service]\n"
                                         "Environment=SYSTEMD_HIBERNATE_RESUME_OFFSET=%s",
                                         arg_resume_offset);
                if (r < 0)
                        return log_error_errno(r, "Failed to write resume offset drop-in file: %m");
        }
