 ...
 * it, list it and add what the restriction lets through, but not change the mask or the restriction */
 ...
 * anchor that is not a CA certificate is merely warned about. self-signed leaf certificate doubling as its
 * own anchor is common. */
...
static int locate_keyring(Keyring *k, struct kmod_ctx *kmod) {
        ...
        if (k->spec->module && kmod && !arg_dry_run)
         ...
         * a keyring is sealed. */
        if (!FLAGS_SET(perm, KEY_USR_SETATTR)) {
                /* Once SetAttr is dropped the keyring permissions cannot be modified anymore. */
                if (perm == KEYRING_PERM_SEALED)
                        log_debug("Keyring %s is sealed already.", k->spec->name);
                else
                        log_warning("Keyring %s has permission mask 0x%08" PRIx32 " not set by us, treating "
                                    "it as sealed.", k->spec->name, perm);
                k->sealed = true;
...
static void enroll_certificates(Keyring *k) {
        ...
        FOREACH_ARRAY(c, k->certificates, k->n_certificates)
                if (c->serial > 0 && !serial_in(after, n_after, c->serial)) {
                        log_warning("'%s' was replaced by a later certificate with the same subject "
                                    "and key identifier.", c->path);
                        k->data_error = true;
                }
        if (k->n_keys != SIZE_MAX)
                FOREACH_ARRAY(s, k->keys, k->n_keys) {
...
static int seal_keyring(Keyring *k) {
        ...
        /* Sealing only allows certificates signed by a key already in the keyring to be added. Dropping the
         * setattr permission then freezes the restriction and the mask, the read and write permissions
         * remain so that later runs can add what chains to the enrolled certificates. */
        if (!k->sealed) {
                /* Needs the setattr permission that goes away below. If we get -EEXIST it's sealed by
...
static int materialize_credentials(int root_fd, char **os) {
        ...
        FOREACH_ARRAY(i, de->entries, de->n_entries) {
                ...
                if (strv_isempty(os)) {
                        log_warning("Ignoring credential '%s', there is no OS identifier to place it under.",
                                    cn);
                        continue;
                }
...
static int run(int argc, char *argv[]) {
        ...
        if (r < 0)
                data_error = true; /* bad input, but os-release is still there */
        if (r <= 0) {
        ...
        if (arg_dry_run) {
                t = table_new("keyring", "role", "exists", "unsealed", "keys", "sealed", "certificates");
                if (!t)
                        return log_oom();
        ...
        return data_error ? EX_DATAERR : EXIT_SUCCESS;
}
