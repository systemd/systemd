...
typedef struct Keyring {
        ...
        int failed;             /* the kernel refused an operation, but the keyring can still be sealed */
} Keyring;
...
static int locate_keyring(Keyring *k, struct kmod_ctx *kmod) {
         ...
         * a keyring is locked down. */
        if (perm == KEYRING_PERM_LOCKED) {
                log_info("Keyring %s is sealed and locked down already.", k->spec->name);
                k->locked = true;
                return 0;
        }
        if (!FLAGS_SET(perm, KEY_USR_WRITE|KEY_USR_SETATTR))
...
static int check_unsealed(Keyring *k) {
        ...
        if (r < 0) {
                 ...
                 * kernel sealed at boot is then locked down like any other */
...
static void enroll_certificates(Keyring *k) {
        ...
        FOREACH_ARRAY(c, k->certificates, k->n_certificates) {
                ...
                if (r == -EPERM) {
                        /* Restricted by somebody else. Either the kernel at boot, a run that died before
                         * locking down. Nothing more can be added, but it can still be locked down. */
                        const char *hint = "";
...
static int seal_keyring(Keyring *k) {
         ...
         * that makes the state observable by later runs. It stops everything else including removals. */

        /* Needs the setattr permission that goes away below. If we get -EEXIST it's sealed by somebody else already. */
