...
static int locate_keyring(Keyring *k, struct kmod_ctx *kmod) {
        ...
        if (k->spec->spec_id != 0) {
                r = keyring_resolve(k->spec->spec_id, &k->serial);
 ...
 * else's keys */
static int unlink_enrolled(Keyring *k) {
        int r = 0;
        ...
        if (r < 0)
                log_error_errno(r, "Failed to remove the enrolled keys from keyring %s: %m", k->spec->name);
...
static void enroll_certificates(Keyring *k) {
        ...
        for (size_t i = k->n_certificates; i > 0; i--) {
                ...
                if (IN_SET(r, -EACCES, -EDQUOT, -ENOMEM, -EKEYREVOKED, -EKEYEXPIRED)) {
                        /* The keyring itself is the problem, sealing it is all that is left. What went in
                         * so far is the low-priority tail, it must not be sealed in alone. */
                        log_error_errno(r, "Failed to enroll '%s' into keyring %s, giving up: %m",
                                        c->path, k->spec->name);
                        RET_GATHER(k->failed, r);
                        (void) unlink_enrolled(k);
                        break;
                }
                if (r < 0) {
                        if (r == -ENOKEY)
...
static int materialize_credential(
        ...
        dfd = chase_and_open_parent_at(root_fd, root_fd, path,
                                       CHASE_MKDIR_0755|CHASE_SAFE|CHASE_MAX_MODE, &base);
        if (dfd < 0)
