...
static int certificate_key_id(X509 *x, char **ret) {
        ...
        if (serial && id->type == V_ASN1_NEG_INTEGER)
                return -EBADMSG;
...
static int seal_keyring(Keyring *k) {
        ...
        if (!k->sealed) {
                ...
                if (r == -EEXIST) {
                        ...
                        /* If we fail to restrict the keyring we also skip permission changes. For if we were
                         * to remove permissions we would lose the ability to restrict the keyring. Fail
                         * closed: a populated keyring anyone privileged can extend must not stay behind, so
                         * drop what was just enrolled. */
                        if (keyring_clear(k->serial) < 0)
                                return log_error_errno(r, "Failed to restrict keyring %s, leaving it "
                                                       "unsealed and writable with %zu key(s) enrolled: %m",
                                                       k->spec->name, k->n_enrolled);
                        return log_error_errno(r, "Failed to restrict keyring %s, cleared it and left it "
                                               "unsealed: %m", k->spec->name);
                }
