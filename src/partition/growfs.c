...
static int resize_crypt_luks_device(dev_t devno, const char *fstype, dev_t main_devno) {
        ...
        _cleanup_free_ const char *devpath = NULL, *main_devpath = NULL;
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to create main sd-device for block device %u:%u: %m",
                                       major(main_devno), minor(main_devno));

        r = sd_device_get_devpath(main_dev, &main_devpath);
        if (r < 0)
                return log_error_errno(r, "Failed to get main devpath: %m");

        main_devfd = sd_device_open(main_dev, O_RDONLY|O_CLOEXEC);
        if (main_devfd < 0)
                return log_error_errno(main_devfd, "Failed to open block device \"%s\": %m", main_devpath);
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to get devpath: %m");
...
static int maybe_resize_underlying_device(
        ...
        _cleanup_free_ const char *devpath = NULL;
        dev_t devno;
        int r;
...
static int run(int argc, char *argv[]) {
        ...
        if (devfd < 0)
                return log_error_errno(devfd, "Failed to open block device \"%s\": %m", devpath);
