...
typedef struct ErofsMakeFsOptions {
        ...
        int copy_xattrs;                              /* auto: mkfs.erofs' default           | -x */
...
static ErofsMakeFsOptions *erofs_makefs_options_new(void){
        ...
        *eo  = (ErofsExtendedOptions)   {0, 0, 0};     /* i.e. use mkfs.erofs defaults */
        *opt = (ErofsMakeFsOptions)     {
                .max_pcluster_bytes = 0,               /* i.e. use mkfs.erofs defaults */
                .copy_xattrs = -1,                     /* i.e. use mkfs.erofs defaults */
                .drop_timestamp = -1,                  /* i.e. use mkfs.erofs defaults */
                .extended_options = eo,
...
static int append_drop_timestamp(ErofsMakeFsOptions const *opt, char ***ret_options) {
        ...
        r = strv_extend_strv(ret_options, STRV_MAKE("-T0"), /* filter_duplicates = */ false);
        if (r < 0)
                return log_oom();
        return 0;
}
...
int parse_erofs_options(const char* root, const char *path, const char *const *conf_file_dirs, char ***ret_options) {
        ...
        ConfigTableItem erofs_table[] = {
                ...
                { "ErofsOptions", "CopyExtendedAttributes",      config_parse_tristate,                    0, &o->copy_xattrs       },
                { "ErofsOptions", "DropTimestamp",               config_parse_tristate,                    0, &o->drop_timestamp    },
