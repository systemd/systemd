...
static int partition_finalize_fstype(Partition *p, const char *path) {
        ...
        for (PartitionDesignator d = 0; d < _PARTITION_DESIGNATOR_MAX; d++) {
                ...
                if (!gpt_partition_type_has_filesystem(p->type) || p->type.designator != d)
                        continue;

                upper = strdup(partition_designator_to_string(d));
                if (!upper)
                        return log_oom();
                ...
                if (v && !streq(p->format, v)) {
                        log_syntax(NULL, LOG_NOTICE, path, 1, 0,
                                   ...
                                   p->format, partition_designator_to_string(d), v);
