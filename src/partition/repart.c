...
static int context_open_copy_block_paths(
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                if (p->copy_blocks_path) {
                        ...
                        r = sd_device_new_from_path(&dev, opened);
                        if (r < 0)
                                return log_error_errno(r, "Failed to create sd-device object for device '%s': %m", opened);

                        source_fd = sd_device_open(dev, O_RDONLY|O_CLOEXEC|O_NOCTTY);
                        if (source_fd < 0)
