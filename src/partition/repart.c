...
static int derive_uuid(sd_id128_t base, const char *token, sd_id128_t *ret) {
        ...
        hmac_sha256(base.bytes, sizeof(base), token, strlen(token), result.md);
...
static int partition_acquire_uuid(Context *context, Partition *p, sd_id128_t *ret) {
        ...
        hmac_sha256(context->seed.bytes, sizeof(context->seed),
                    &plaintext, k == 0 ? sizeof(sd_id128_t) : sizeof(plaintext),
                    result.md);
