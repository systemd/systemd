...
static int finalize_extra_mkfs_options(const Partition *p, char ***ret) {
        int r;
        char **sv = NULL;
        ...
        if (arg_offline >= 0 && streq(p->format, "btrfs") && p->subvolumes)
...
static int context_mkfs(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                r = finalize_extra_mkfs_options(p, &extra_mkfs_options);
                if (r < 0)
                        return r;
