...
static int context_update_verity_size(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                if (dp->size_max != UINT64_MAX) {
                        ...
                        if (sz > p->size_max)
                                log_warning("SizeMaxBytes=%s of dm-verity hash partition too small for data partition "
                                            "with SizeMaxBytes=%s. Hash partition would require %s bytes for a data "
                                            "partition of specified max size. Consider increasing SizeMaxBytes= of "
                                            "hash partition, or decreasing SizeMaxBytes= of data partition.",
                                            FORMAT_BYTES(p->size_max), FORMAT_BYTES(dp->size_max), FORMAT_BYTES(sz));
