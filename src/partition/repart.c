...
static int context_read_definitions(
        ...
        if (directory) {
                dirs = STRV_MAKE_CONST(directory);
                r = conf_files_list_strv(&files, ".conf", NULL, CONF_FILES_REGULAR|CONF_FILES_FILTER_MASKED, dirs);
        } else {
                dirs = (const char* const*) CONF_PATHS_STRV("repart.d");
                r = conf_files_list_strv(&files, ".conf", root, CONF_FILES_REGULAR|CONF_FILES_FILTER_MASKED, dirs);
        }
