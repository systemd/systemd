...
typedef enum SubvolumeFlags {
        SUBVOLUME_DEFAULT          = 1 << 0,
        SUBVOLUME_RO               = 1 << 1,
        _SUBVOLUME_FLAGS_MASK      = SUBVOLUME_DEFAULT|SUBVOLUME_RO,
...
static char* subvolume_flags_to_string(SubvolumeFlags flags) {
        ...
        if (FLAGS_SET(flags, SUBVOLUME_DEFAULT))
                l[m++] = "default";
...
static int partition_read_definition(Partition *p, const char *path, const char *const *conf_file_dirs) {
        ...
        _cleanup_free_ char *default_subvolume = NULL;
        ConfigTableItem table[] = {
                ...
                { "Partition", "DefaultSubvolume",         config_parse_default_subvolume, 0, &default_subvolume          },
        ...
        if (default_subvolume) {
                subvolume = ordered_hashmap_get(p->subvolumes, default_subvolume);
                if (!p)
                        return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                        "DefaultSubvolume= must be one of the paths in Subvolumes=.");

                subvolume->flags |= SUBVOLUME_DEFAULT;
        }

        bool found = false;
        ORDERED_HASHMAP_FOREACH(subvolume, p->subvolumes) {
                if (!FLAGS_SET(subvolume->flags, SUBVOLUME_DEFAULT))
                        continue;

                if (found)
                        return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                          "Only one default subvolume can be specified.");
...
static int set_default_subvolume(Partition *p, const char *root) {
        ...
        ORDERED_HASHMAP_FOREACH(subvolume, p->subvolumes) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to make '%s' the default subvolume: %m", subvolume->path);
        }
