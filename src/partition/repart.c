...
/* Hard maximum of 4M on verity signature partition size */
#define VERITY_SIG_MAX_SIZE 4*1024*1024
...
static uint64_t partition_min_size(const Context *context, const Partition *p) {
        ...
        return MAX(round_up_size(p->size_min != UINT64_MAX ? p->size_min : p->verity == VERITY_SIG ? HARD_MIN_SIZE : DEFAULT_MIN_SIZE, context->grain_size), sz);
}
...
static uint64_t partition_max_size(const Context *context, const Partition *p) {
        ...
        if (p->verity == VERITY_SIG)
                sm = VERITY_SIG_MAX_SIZE;
        else if (p->size_max == UINT64_MAX)
                sm = UINT64_MAX;
        else
...
static int sign_verity_roothash(const char *roothash, uint8_t **ret_signature, size_t *ret_signature_size) {
        ...
        _cleanup_(BIO_freep) BIO *cb = NULL, *kb = NULL, *rb = NULL, *pb = NULL;
        ...
        cb = BIO_new_mem_buf(arg_certificate, arg_certificate_size);
        if (!cb)
                return log_oom();
