...
static int partition_populate_directory(Partition *p, char **ret_root, char **ret_tmp_root) {
        ...
        /* When generating squashfs, we need the directory root to be available when we call squashfs.
         * Because we might have multiple source trees, we build a temporary source tree beforehand where we
         * merge all our inputs. We then use this merged source tree to create the squashfs filesystem. */
        ...
        r = fsync_path_at(AT_FDCWD, root);
        if (r < 0)
                return log_error_errno(r, "Failed to synchronize temporary directory: %m");
