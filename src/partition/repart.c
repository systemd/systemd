...
static Context *context_free(Context *context) {
        ...
        safe_close(context->workspace_fd);
        rm_rf_physical_and_free(context->workspace);
...
static int context_split(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                fdt = openat(context->workspace_fd, fname, O_WRONLY|O_NOCTTY|O_CLOEXEC|O_NOFOLLOW|O_CREAT|O_EXCL, 0666);
                if (fdt < 0)
...
static int find_root(Context *context, char **ret, int *ret_fd) {
        ...
        if (arg_node) {
                if (arg_empty == EMPTY_CREATE) {
                        ...
                        s = path_join(context->workspace, f);
                        if (!s)
                                return log_oom();

                        fd = openat(context->workspace_fd, f, O_RDONLY|O_CREAT|O_EXCL|O_CLOEXEC|O_NOFOLLOW, 0666);
                        if (fd < 0)
...
static int make_workspace(Context *context) {
        _cleanup_free_ char *p = NULL;
        _cleanup_close_ int fd = -1;
        int r;
        ...
        context->workspace = TAKE_PTR(p);
        context->workspace_fd = TAKE_FD(fd);

        return 0;
}

static int empty_workspace(Context *context) {
        ...
        r = readdir_all(context->workspace_fd, RECURSE_DIR_SORT, &de);
        if (r < 0)
                return log_error_errno(r, "Failed to read workspace directory entries: %m");

        for (size_t i = 0; i < de->n_entries; i++) {
                const char *name = de->entries[i]->d_name;

                r = renameat(context->workspace_fd, name, dir_fd, name);
                if (r < 0)
                        return log_error_errno(r, "Failed to move %s/%s to %s: %m", context->workspace, name, name);
        }
