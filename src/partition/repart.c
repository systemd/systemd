...
static int partition_target_prepare(
        ...
        if (S_ISBLK(st.st_mode) || (p->format && !mkfs_supports_root_option(p->format))) {
                ...
                *t = (PartitionTarget) {
                ...
                fd = open_temporary(tmp, &path);
                if (fd < 0)
                        return log_error_errno(fd, "Failed to create temporary file in %s", tmp);

                if (ftruncate(fd, size) < 0)
                        return log_error_errno(r, "Failed to truncate temporary file to %s: %m",
                                               FORMAT_BYTES(size));
