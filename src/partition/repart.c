...
static uint64_t arg_logical_sector_size = 0;
static uint64_t arg_physical_sector_size = 0;
...
static int context_load_partition_table(Context *context) {
        ...
        r = fdisk_save_user_sector_size(c, arg_physical_sector_size, arg_logical_sector_size);
        if (r < 0)
        ...
        if (r == -EINVAL && arg_size_auto) {
                ...
                if (S_ISREG(st.st_mode) && st.st_size == 0) {
                        ...
                        context->sector_size = arg_logical_sector_size ?: 512;
...
static int help(void) {
        ...
        printf("%s [OPTIONS...] [DEVICE]\n"
               ...
               "     --logical-sector-size=SIZE\n"
               "                          Set the logical sector size for the image\n"
               "     --physical-sector-size=SIZE\n"
               "                          Set the physical sector size for the image\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        enum {
                ...
                ARG_LOGICAL_SECTOR_SIZE,
                ARG_PHYSICAL_SECTOR_SIZE,
        };
        ...
        static const struct option options[] = {
                ...
                { "logical-sector-size",  required_argument, NULL, ARG_LOGICAL_SECTOR_SIZE  },
                { "physical-sector-size", required_argument, NULL, ARG_PHYSICAL_SECTOR_SIZE },
                {}
        };
        ...
        while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0)
                ...
                case ARG_LOGICAL_SECTOR_SIZE:
                        r = parse_sector_size(optarg, &arg_logical_sector_size);
                        if (r < 0)
                                return r;

                        break;

                case ARG_PHYSICAL_SECTOR_SIZE:
                        r = parse_sector_size(optarg, &arg_physical_sector_size);
                        if (r < 0)
                                return r;
