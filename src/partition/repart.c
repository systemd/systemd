...
static DEFINE_CONFIG_PARSE_ENUM_WITH_DEFAULT(config_parse_verity, verity_mode, VerityMode, VERITY_DATA, "Invalid verity mode");
...
static int partition_read_definition(Partition *p, const char *path, const char *const *conf_file_dirs) {
        ...
        if (p->verity != VERITY_DATA && !p->copy_files && !p->copy_blocks_path && !p->copy_blocks_auto)
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                  "At least one of CopyFiles=/CopyBlocks= must be used when Verity=%s is used",
                                  verity_mode_to_string(p->verity));

        if (p->verity != VERITY_DATA && p->encrypt != ENCRYPT_OFF)
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                  "Verity hash or data partitions cannot be encrypted");
...
static Partition* find_verity_data_partition(Context *context, Partition *p) {
        assert(context);
        assert(p);
        assert(p->copy_files || p->copy_blocks_path);
        assert(p->verity != VERITY_DATA);

        /* Try to find a matching verity data partition for a verity hash partition. A data partition matches
         * a verity hash partition if its content configuration matches that of the verity hash partition. */

        LIST_FOREACH(partitions, q, context->partitions) {
                bool used = false;

                /* Check that the partition is configured to be a verity data partition. */
                if (q->verity != VERITY_DATA)
                        continue;

                /* The verity data partition must have the same CopyFiles= or CopyBlocks= setting. */
                if (p->copy_files && !strv_equal(p->copy_files, q->copy_files))
                        continue;
                if (p->copy_blocks_path && !streq(p->copy_blocks_path, q->copy_blocks_path))
                        continue;

                /* If a filesystem type is set for the verity hash partition, it must match the data
                 * partition's filesystem type. */
                if (p->format && !streq(p->format, q->format))
                        continue;

                /* Ditto for MakeDirectories=. */
                if (p->make_directories && !strv_equal(p->make_directories, q->make_directories))
                        continue;

                /* Only use data partitions that haven't been assigned to another verity hash partition. */

                LIST_FOREACH(partitions, r, context->partitions)
                        if (r->data_partition == q) {
                                used = true;
                                break;
                        }
...
static int context_read_definitions(
        ...
        /* Check that each configured verity hash partition has a matching data partition. */

        LIST_FOREACH(partitions, p, context->partitions) {
                Partition *dp;

                if (p->verity == VERITY_DATA)
                        continue;

                dp = find_verity_data_partition(context, p);
                if (!dp)
                        return log_syntax(NULL, LOG_ERR, p->definition_path, 1, SYNTHETIC_ERRNO(EINVAL),
                                          "Missing verity data partition for verity partition from %s",
                                          p->definition_path);

                p->data_partition = dp;

                if (dp->encrypt != ENCRYPT_OFF)
                        return log_syntax(NULL, LOG_ERR, dp->definition_path, 1, SYNTHETIC_ERRNO(EINVAL),
                                          "A verity data partition cannot be encrypted");
        }
...
static int context_dump_partitions(Context *context, const char *node) {
        ...
        bool no_roothash = true, no_dropin_files = true;
        ...
        if (!DEBUG_LOGGING) {
                if (arg_json_format_flags & JSON_FORMAT_OFF)
                        (void) table_set_display(t, (size_t) 0, (size_t) 1, (size_t) 2, (size_t) 3, (size_t) 4,
                                                    (size_t) 8, (size_t) 11, (size_t) roothash_col, dropin_files_col);
                else
                        (void) table_set_display(t, (size_t) 0, (size_t) 1, (size_t) 2, (size_t) 3, (size_t) 4,
                                                    (size_t) 5, (size_t) 6, (size_t) 7, (size_t) 9, (size_t) 10,
                                                    (size_t) 12, (size_t) roothash_col, dropin_files_col);
        }
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                no_roothash = no_roothash && isempty(rh);
...
static int context_verity(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                Partition *dp;
                _cleanup_(loop_device_unrefp) LoopDevice *hash_device = NULL;
                LoopDevice *data_device;
                _cleanup_free_ uint8_t *rh = NULL;
                size_t rhs;
                ...
                assert(rhs % sizeof(sd_id128_t) == 0);
