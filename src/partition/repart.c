...
static int context_dump_partitions(Context *context, const char *node) {
        ...
        int r;
        const size_t roothash_col = 13, roothash_sig_col = 14, dropin_files_col = 15;
        bool has_roothash = false, has_roothash_sig = false, has_dropin_files = false;
        ...
        t = table_new("type", "label", "uuid", "file", "node", "offset", "old size", "raw size", "size", "old padding", "raw padding", "padding", "activity", "roothash", "signature", "drop-in files");
        if (!t)
                return log_oom();
        ...
        if (!DEBUG_LOGGING) {
                if (arg_json_format_flags & JSON_FORMAT_OFF)
                        (void) table_set_display(t, (size_t) 0, (size_t) 1, (size_t) 2, (size_t) 3, (size_t) 4,
                                                    (size_t) 8, (size_t) 11, roothash_col, roothash_sig_col,
                                                    dropin_files_col);
                else
                        (void) table_set_display(t, (size_t) 0, (size_t) 1, (size_t) 2, (size_t) 3, (size_t) 4,
                                                    (size_t) 5, (size_t) 6, (size_t) 7, (size_t) 9, (size_t) 10,
                                                    (size_t) 12, roothash_col, roothash_sig_col,
                                                    dropin_files_col);
        }
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                const char *label, *activity = NULL, *rh = NULL, *sig = NULL;
                ...
                has_roothash_sig = has_roothash_sig || !isempty(sig);
        ...
        if (!has_roothash_sig) {
                r = table_hide_column_from_display(t, roothash_sig_col);
                if (r < 0)
                        return log_error_errno(r, "Failed to set columns to display: %m");
        }
...
static int context_verity_sig(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                _cleanup_(json_variant_unrefp) JsonVariant *v = NULL;
                _cleanup_free_ uint8_t *sig = NULL;
                _cleanup_free_ char *text = NULL, *hex = NULL;
                Partition *hp;
                size_t sigsz, padsz;
                ...
                r = sign_verity_roothash(hp->roothash, &sig, &sigsz);
                if (r < 0)
                        return r;

                r = json_build(&v,
                               JSON_BUILD_OBJECT(
                                        JSON_BUILD_PAIR("rootHash", JSON_BUILD_STRING(hp->roothash)),
                                        JSON_BUILD_PAIR(
                ...
                hex = hexmem(sig, sigsz);
                if (!hex)
                        return log_oom();

                p->roothash_sig = TAKE_PTR(hex);
        }
