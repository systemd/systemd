...
static int partition_target_fd(PartitionTarget *t) {
        assert(t);
        assert(t->loop || t->file);
        return t->loop ? t->loop->fd : fileno(t->file);
}
...
static int partition_target_prepare(
        ...
        if (S_ISBLK(st.st_mode) || (p->format && !mkfs_supports_root_option(p->format))) {
                ...
                fd = fcntl(d->fd, F_DUPFD_CLOEXEC, 3);
                if (fd < 0)
                        return -errno;
                ...
                *t = (PartitionTarget) {
                ...
                r = fopen_temporary(tmp, &f, &path);
                if (r < 0)
                        return log_error_errno(r, "Failed to create temporary file in %s", tmp);

                if (ftruncate(fileno(f), p->new_size) < 0)
...
static int partition_target_grow(PartitionTarget *t, uint64_t size) {
        ...
        if (t->loop) {
                ...
                if (fallocate(fileno(t->file), 0, 0, arg_size) < 0) {
                        if (!ERRNO_IS_NOT_SUPPORTED(errno))
                                return log_error_errno(errno, "Failed to grow '%s' to %s by allocation: %m",
                                                       t->path, FORMAT_BYTES(arg_size));
                ...
                if (ftruncate(fileno(t->file), arg_size) < 0)
...
static int partition_target_sync(Context *context, Partition *p, PartitionTarget *t) {
        ...
        if (t->loop) {
                ...
                r = copy_bytes(fileno(t->file), whole_fd, UINT64_MAX, COPY_REFLINK|COPY_HOLES|COPY_FSYNC);
                if (r < 0)
...
static int partition_encrypt(Context *context, Partition *p, const char *node) {
        ...
        size_t volume_key_size = 256 / 8, passphrase_size;
        sd_id128_t uuid;
        int r;
...
static int partition_format_verity_hash(
        ...
        r = partition_target_prepare(context, p, p->new_size, &t);
        if (r < 0)
                return r;
        ...
        r = sym_crypt_format(
                        ...
                        &(struct crypt_params_verity){
                                .data_device = partition_target_path(t),
