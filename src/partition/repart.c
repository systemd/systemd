...
static int config_parse_copy_blocks(
        ...
        if (!path_is_absolute(d) || !path_is_normalized(d)) {
                log_syntax(unit, LOG_WARNING, filename, line, 0,
                           "Invalid path name in CopyBlocks= path, ignoring: %s", d);
                return 0;
        }
...
static int context_discard_partition(Context *context, Partition *p) {
        ...
        if (r == -EBUSY) {
                ...
                log_info("Block device busy; can't discard block device while it is mounted.");
                return 0;
        }
        if (r == 0) {
...
static int resolve_copy_blocks_auto_candidate(
        ...
        if (r < 0)
                return log_error_errno(
                                r,
                                "Unable to determine whole block device of block device %u:%u: %m",
        ...
        if (restrict_devno != (dev_t) -1 &&
            restrict_devno != whole_devno)
                return log_error_errno(
                                ...
                                "Partition block device %u:%u is located outside of whole block device %u:%u, refusing.",
                                major(partition_devno), minor(partition_devno),
...
static int context_open_copy_block_paths(
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                if (p->copy_blocks_path) {
                        ...
                        if (source_fd < 0)
                                return log_error_errno(r, "Failed to open '%s': %m", p->copy_blocks_path);
...
static int resize_backing_fd(
        ...
        if (S_ISBLK(st.st_mode)) {
                if (!backing_file)
                        return log_error_errno(EBADF, "Cannot resize block device '%s'.", node);

                assert(loop_device);

                if (ioctl(*fd, BLKGETSIZE64, &current_size) < 0)
                        return log_error_errno(errno, "Failed to determine size of block device: %m");
        } else {
        ...
        if (S_ISBLK(st.st_mode)) {
                ...
                /* This is a loopbck device. We can't really grow those directly, but we can grow the backing
                 * file, hence let's do that. */
...
static int run(int argc, char *argv[]) {
        ...
        if (arg_image) {
                ...
                /* Strictly open this in read-only fashion: we shall modify the partition table, not the file systems */
