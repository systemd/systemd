...
static int make_copy_files_denylist(Context *context, Partition *p, Set **ret) {
        ...
        LIST_FOREACH(partitions, q, context->partitions) {
                ...
                NULSTR_FOREACH(s, sources) {
                        ...
                        dfd = chase_symlinks_and_open(s, arg_root, CHASE_PREFIX_ROOT, O_RDONLY|O_CLOEXEC|O_DIRECTORY, NULL);
                        if (dfd == -ENOENT)
                                continue;
                        if (dfd < 0)
                                return log_error_errno(r, "Failed to stat source '%s%s': %m", strempty(arg_root), s);
                        ...
                        if (r < 0)
                                return log_error_errno(r, "Failed to read '%s%s' contents: %m", strempty(arg_root), s);
