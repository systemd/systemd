...
static int context_dump_partitions(Context *context) {
        ...
        if (context->n_partitions == 0 && !sd_json_enabled(arg_json_format_flags)) {
        ...
        if (!sd_json_enabled(arg_json_format_flags) && (sum_padding > 0 || sum_size > 0)) {
                const char *a, *b;
...
static int context_dump(Context *context, bool late) {
        ...
        if (arg_pretty == 0 && !sd_json_enabled(arg_json_format_flags))
                return 0;

        /* If we're outputting JSON, only dump after doing all operations so we can include the roothashes
         * in the output.  */
        if (!late && sd_json_enabled(arg_json_format_flags))
                return 0;
        ...
        if (late && !sd_json_enabled(arg_json_format_flags) && !context_has_roothash(context))
                return 0;
         ...
         * roothashes. */
        if (!sd_json_enabled(arg_json_format_flags) && !late) {
                putc('\n', stdout);
