...
static int partition_read_definition(Partition *p, const char *path, char **conf_file_dirs) {
        ...
        const char* dropin_dirname = strjoina(filename, ".d");
...
static int context_read_definitions(
        ...
        STRV_FOREACH(f, files) {
                ...
                if (directory)
                        conf_file_dirs = STRV_MAKE(directory);
                else {
                        conf_file_dirs = CONF_PATHS_STRV("repart.d");
                }
                ...
                r = partition_read_definition(p, *f, conf_file_dirs);
                if (r < 0)
                        return r;
