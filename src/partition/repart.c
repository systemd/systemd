        ...
        STRV_FOREACH_PAIR(source, target, p->copy_files) {
                ...
                if (r < 0) {
                        ...
                        if (tfd < 0) {
                                ...
                                r = mkdir_p_root(root, dn, UID_INVALID, GID_INVALID, 0755, p->subvolumes, epoch_or_utime_omit());
                                if (r < 0)
                        ...
                        usec_t ts = epoch_or_utime_omit();
                        if (futimens(pfd, (const struct timespec[2]){ { .tv_nsec = UTIME_OMIT }, { .tv_nsec = ts } }) <
                            0) {
                                return -errno;
                        }
                }
...
static int do_make_directories(Partition *p, const char *root) {
        ...
        STRV_FOREACH(d, p->make_directories) {
                r = mkdir_p_root(root, *d, UID_INVALID, GID_INVALID, 0755, p->subvolumes, epoch_or_utime_omit());
                if (r < 0)
