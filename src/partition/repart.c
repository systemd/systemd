...
static int config_parse_make_symlinks(
        ...
        for (;;) {
                ...
                r = extract_many_words((const char **) &word, ":", EXTRACT_UNQUOTE|EXTRACT_DONT_COALESCE_SEPARATORS, &path, &target);
                if (r < 0) {
...
static int do_make_symlinks(Partition *p, const char *root) {
        ...
        STRV_FOREACH_PAIR(path, target, p->make_symlinks) {
                ...
                r = chase(*path, root, CHASE_PREFIX_ROOT|CHASE_NONEXISTENT, &resolved, NULL);
                if (r < 0)
                        return log_error_errno(r, "Failed to resolve %s in %s", *path, root);

                if (symlink(*target, resolved) < 0)
                        return log_error_errno(errno, "Failed to create symlink at %s to %s: %m", resolved, *target);
        }
