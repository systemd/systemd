...
static int do_copy_fstab(Partition *p, const char *root) {
        _cleanup_free_ char *path = NULL;
        int r;
        ...
        path = path_join(root, arg_generate_fstab);
        if (!path)
                return log_oom();

        r = copy_file(p->fstab, path, 0, 0644, 0);
        if (r < 0)
                return log_error_errno(r, "Failed to copy fstab: %m");
...
static int context_fstab(Context *context) {
        ...
        t = path_join(tmpdir, "repart-fstab-XXXXXX");
        if (!t)
                return log_oom();

        r = fmkostemp_safe(t, "r+", &f);
        if (r < 0)
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                FOREACH_ARRAY(mountpoint, p->mountpoints, p->n_mountpoints) {
                        ...
                        if (t && strcmp(mountpoint->where, "/") == 0) {
                                p->fstab = TAKE_PTR(t);
                        }
