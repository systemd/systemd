...
static int context_dump_partitions(Context *context, const char *node) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                r = table_add_many(
                                ...
                                TABLE_UUID, !sd_id128_is_null(p->new_uuid) || p->new_uuid_is_set ? p->new_uuid : p->current_uuid,
...
static int partition_hint(const Partition *p, const char *node, char **ret) {
        ...
        if (!sd_id128_is_null(p->new_uuid) || p->new_uuid_is_set)
                id = p->new_uuid;
...
static int context_acquire_partition_uuids_and_labels(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                else if (sd_id128_is_null(p->new_uuid) && !p->new_uuid_is_set) {
...
static int context_open_copy_block_paths(
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                if (sd_id128_is_null(p->new_uuid) && !p->new_uuid_is_set && !sd_id128_is_null(uuid))
                        p->new_uuid = uuid;
        }
