...
#define PARTITION_IS_EXISTING(p) (!!(p)->current_partition)
...
static void context_free_free_areas(Context *context) {
        ...
        for (i = 0; i < context->n_free_areas; i++)
...
static int context_drop_one_priority(Context *context) {
        int32_t priority = 0;
        Partition *p;
        bool exists = false;
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                if (p->priority < priority)
                        continue;
                if (p->priority == priority) {
                        exists = exists || PARTITION_IS_EXISTING(p);
                        continue;
                }

                priority = p->priority;
                exists = PARTITION_IS_EXISTING(p);
        }
...
static int context_allocate_partitions(Context *context) {
        Partition *p;
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                if (p->dropped || PARTITION_IS_EXISTING(p))
                        continue;
                ...
                if (!fits)
                        return 0; /* 😢 Oh no! We can't fit this partition into any free area! */
...
overflow_sum:
        return log_error_errno(SYNTHETIC_ERRNO(EOVERFLOW), "Partition weight sum overflow, refusing.");
}
...
static int scale_by_weight(uint64_t value, uint64_t weight, uint64_t weight_sum, uint64_t *ret) {
        ...
        if (value > UINT64_MAX / weight)
                return log_error_errno(SYNTHETIC_ERRNO(EOVERFLOW), "Overflow while scaling by weight, refusing.");
...
static int context_grow_partitions(Context *context) {
        ...
        for (i = 0; i < context->n_free_areas; i++) {
                ...
                for (;;) {
                        ...
                        /* Now let's look at the intended weights and adjust them taking the minimum space
                         * assignments into account. i.e. if a partition has a small weight but a high minimum space
                         * value set it should not get any additional room from the left-overs. Similar, if two
                         * partitions have the same weight they should get the same space if possible, even if one
                         * has a smaller minimum size than the other. */
                        LIST_FOREACH(partitions, p, context->partitions) {
                                ...
                                if (p->new_size == UINT64_MAX) {
                                        ...
                                        /* Calculate how much this space this partition needs if everyone
                                         * would get the weight based share */
                                        ...
                                        if (phase == PHASE_OVERCHARGE && rsz > share) {
                                                /* This partition needs more than its calculated share. Let's
                                                 * assign it that, and take this partition out of all
                                                 * calculations and start again. */
                                ...
                                if (p->new_padding == UINT64_MAX) {
                                        ...
                                        if (charge) {
                                                ...
                                                assert(weight_sum >= p->padding_weight);
                                                weight_sum -= p->padding_weight;
                                        }
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                if (!PARTITION_IS_EXISTING(p) || p->padding_area) {
...
static int context_place_partitions(Context *context) {
        uint64_t partno = 0;
        Partition *p;
        size_t i;
...
static const char *gpt_partition_type_uuid_to_string(sd_id128_t id) {
        ...
        for (i = 0; i < ELEMENTSOF(gpt_partition_type_table); i++)
...
static int context_read_definitions(
        ...
        if (directory)
                ...
                r = conf_files_list_strv(&files, ".conf", root, CONF_FILES_REGULAR|CONF_FILES_FILTER_MASKED, (const char**) CONF_PATHS_STRV("partition.d"));
        if (r < 0)
...
static int context_load_partition_table(Context *context, const char *node) {
        ...
        if (flock(fdisk_get_devfd(c), arg_dry_run ? LOCK_SH : LOCK_EX) < 0)
                return log_error_errno(errno, "Failed top lock block device: %m");
        ...
        for (i = 0; i < n_partitions; i++)  {
                ...
                if (fdisk_partition_has_start(p) <= 0 ||
                    ...
                    fdisk_partition_has_partno(p) <= 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Found partition without a position, size or number.");
                ...
                if (!ids)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Found partition without a UUID.");
...
static int context_dump_partition_bar(Context *context, const char *node) {
        ...
        for (i = 0; i < context->n_partitions; i++) {
...
static int context_wipe_partition(Context *context, Partition *p) {
        ...
        assert(context);
        assert(p);
        assert(!PARTITION_IS_EXISTING(p)); /* Safety check: never wipe existing partitions */
...
static int context_discard_partition(Context *context, Partition *p) {
        ...
        assert(p->offset != UINT64_MAX);
        assert(p->new_size != UINT64_MAX);
        assert(!PARTITION_IS_EXISTING(p)); /* Safety check: never discard existing partitions */
...
static int context_write_partition_table(Context *context, const char *node) {
        ...
        if (!arg_empty && !context_changed(context)) {
                log_info("No changes.");
                return 0;
        }
...
static int context_factory_reset(Context *context) {
        ...
        if (arg_empty) /* Nothing to reset if we start from scratch */
                return 0;
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                if (!p->factory_reset || !PARTITION_IS_EXISTING(p))
                        continue;
...
static int help(void) {
        ...
        printf("%s [OPTIONS...] [DEVICE]\n"
               ...
               "     --empty=BOOL         Whether to ignore existing partitions table and start\n"
               "                          from an empty table\n"
               "     --discard=BOOL       Whether to discard backing blocks for new partitions\n"
               "     --randomize=BOOL     Whether to generate partition UUIDs randomly\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        static const struct option options[] = {
                ...
                { "randomize",         required_argument, NULL, ARG_RANDOMIZE         },
        ...
        while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0) {
                ...
                case ARG_EMPTY:
                        r = parse_boolean(optarg);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --empty= parameter: %s", optarg);
                ...
                case ARG_RANDOMIZE:
                        r = parse_boolean(optarg);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --randomize= parameter: %s", optarg);
        ...
        if (arg_factory_reset > 0 && arg_empty)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "Combination of --factory-reset=yes and --empty=yes is invalid.");
...
static int search_root(char **ret) {
        const char *t;
        int r;
         ...
         * latter we check for cases where / is a tmpfs and only /usr an actual persistent block device
...
static int run(int argc, char *argv[]) {
        ...
        for (;;) {
                r = context_allocate_partitions(context);
                if (r < 0)
                        return r;
                if (r > 0) /* Success! */
                        break;

                r = context_drop_one_priority(context);
                if (r < 0)
                        return r;
                if (r == 0)
        ...
        r = context_place_partitions(context);
        if (r < 0)
                return r;
        ...
        r = context_write_partition_table(context, node);
        if (r < 0)
                return r;
