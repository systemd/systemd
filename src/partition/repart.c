...
static int context_load_partition_table(Context *context) {
        ...
        if (arg_sector_size > 0) {
                ...
                /* If we found the sector size and we're operating on a block the size, use it as the file
                 * system sector size, as we know its the sector size of the actual block device and not just
                 * the offset at which we found the GPT header. */
