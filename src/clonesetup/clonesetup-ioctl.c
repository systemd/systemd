...
static int dm_ioctl_run(const char *name, uint32_t cmd, struct dm_ioctl *data, size_t data_size) {
        ...
        if (ioctl(fd, cmd, dm) < 0) {
                int e = errno;
                if (e == ENXIO && cmd == DM_DEV_REMOVE)
                        return log_debug_errno(e, "DM ioctl failed: %m");
                return log_error_errno(e, "DM ioctl failed: %m");
        }
        return 0;
}
...
static int dm_clone_load_table(const char *name, uint64_t size_sectors, const char *target_params) {
        ...
        dm_size = sizeof(struct dm_ioctl) + ALIGN8(sizeof(struct dm_target_spec) + params_len);
