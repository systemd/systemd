...
static int parse_clone_options(const char *options, uint64_t *ret_region_size) {
        ...
        for (;;) {
                ...
                if ((val = startswith(word, "region_size="))) {
                        ...
                        if (r < 0)
                                log_warning_errno(r, "Invalid region size=value '%s', using default", val);
...
static int clone_device(
        ...
        if (r < 0) {
                (void) dm_clone_remove_device(clone_name);
...
static int verb_add(int argc, char *argv[], uintptr_t data, void *userdata) {
        ...
        const char *src_dev = ASSERT_PTR(argv[2]);
        const char *dst_dev = ASSERT_PTR(argv[3]);
        const char *meta_dev = ASSERT_PTR(argv[4]);
        ...
        return clone_device(name, src_dev, dst_dev, meta_dev, options);
}
...
static int verb_remove(int argc, char *argv[], uintptr_t data, void *userdata) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to remove dm-clone device: %m");
...
static int help(void) {
        ...
        help_cmdline("add NAME SOURCE-DEVICE DEST-DEVICE META-DEVICE [OPTIONS]");
