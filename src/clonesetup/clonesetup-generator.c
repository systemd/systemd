...
static int add_clone_devices(void) {
        ...
        for (;;) {
                ...
                if (!path_is_valid(src) || !path_is_absolute(src) || !path_startswith(src, "/dev/")) {
                        log_error("Invalid src path '%s' in %s:%u, ignoring.", src, fname, clone_line);
                        continue;
                }

                if (!path_is_valid(dst) || !path_is_absolute(dst) || !path_startswith(dst, "/dev/")) {
                        log_error("Invalid dst path '%s' in %s:%u, ignoring.", dst, fname, clone_line);
                        continue;
                }

                if (!path_is_valid(meta) || !path_is_absolute(meta) || !path_startswith(meta, "/dev/")) {
