...
static int add_clone_devices(void) {
        ...
        for (;;) {
                ...
                /* string_is_safe() rejects control characters, backslashes, and quotes by default.
                 * Backslashes must be rejected: The fields from clonetab get written into a unit file
                 * single-quoted. When systemd loads that unit file, it decodes backslash escape sequences
                 * like \x2e\x2e into .. before passing the value to the binary. */
                if (!string_is_safe(name, STRING_FILENAME)) {
                        log_error("Invalid clone name '%s' in %s:%u, ignoring.", name, fname, clone_line);
                        continue;
                }

                /* Reject non-normalized paths (e.g. containing "..") and keep /dev/ confinement.
                 * string_is_safe() blocks backslashes and quotes (see comment on name above). */
