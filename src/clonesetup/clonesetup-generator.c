...
static int generate_clone_units(
        ...
        _cleanup_free_ char *source_unit = NULL, *dest_unit = NULL, *metadata_unit = NULL,
                            *escaped_clone_name = NULL, *escaped_source = NULL, *escaped_dest = NULL,
                            *escaped_metadata = NULL, *escaped_options = NULL, *e = NULL, *unit = NULL,
        ...
        escaped_clone_name = cescape(clone_name_spec_escaped);
        if (!escaped_clone_name)
                return log_oom();
        ...
        escaped_source = cescape(source_spec_escaped);
        if (!escaped_source)
                return log_oom();
        ...
        escaped_dest = cescape(dest_spec_escaped);
        if (!escaped_dest)
                return log_oom();
        ...
        escaped_metadata = cescape(metadata_spec_escaped);
        if (!escaped_metadata)
                return log_oom();
        ...
        if (options) {
                ...
                escaped_options = cescape(options_spec_escaped);
                if (!escaped_options)
                        return log_oom();
        }
        ...
        fprintf(f,
                ...
                e,
                escaped_clone_name, escaped_source, escaped_dest, escaped_metadata, escaped_options ?: "");
