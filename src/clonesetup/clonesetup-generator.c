...
static int validate_fields(const char *fname, unsigned clone_line, const char *name,
                const char *src, const char *dst, const char *meta, const char *options) {
        int r;
        if (!string_is_safe(name, STRING_FILENAME)) {
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                        "Invalid clone name '%s' in %s:%u, ignoring.", name, fname, clone_line);
        }
        ...
        if (options && !string_is_safe(options, 0)) {
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                        "Invalid options '%s' in %s:%u, ignoring.", options, fname, clone_line);
        }
        return 0;
}
