...
static int generate_clone_units(
        ...
        fprintf(f,
                ...
                "BindsTo=%2$s %3$s %4$s\n"
                "After=%2$s %3$s %4$s\n"
                "Before=blockdev@dev-mapper-%5$s.target clonesetup.target shutdown.target\n"
...
static int add_clone_devices(void) {
        ...
        for (;;) {
                ...
                if (!filename_is_valid(name) || strchr(name, '\'')) {
                        log_error("Invalid clone name '%s' in %s:%u, ignoring.", name, fname, clone_line);
                        continue;
                }

                /* Reject non-normalized paths (e.g. containing "..") and single quotes,
                 * to keep /dev/ confinement effective and ExecStart/ExecStop quoting safe. */
                if (!path_is_valid(src) || !path_is_normalized(src) ||
                                !path_is_absolute(src) || !path_startswith(src, "/dev/") || strchr(src, '\'')) {
                        log_error("Invalid src path '%s' in %s:%u, ignoring.", src, fname, clone_line);
                        continue;
                }

                if (!path_is_valid(dst) || !path_is_normalized(dst) ||
                                !path_is_absolute(dst) || !path_startswith(dst, "/dev/") || strchr(dst, '\'')) {
                        log_error("Invalid dst path '%s' in %s:%u, ignoring.", dst, fname, clone_line);
                        continue;
                }

                if (!path_is_valid(meta) || !path_is_normalized(meta) ||
                                !path_is_absolute(meta) || !path_startswith(meta, "/dev/") || strchr(meta, '\'')) {
                        log_error("Invalid meta path '%s' in %s:%u, ignoring.", meta, fname, clone_line);
                        continue;
                }

                /* options should not have single quotes */
