...
int ringbuf_set_memfd(Ringbuf *rb, int memfd) {
        ...
        lower = mmap(address, size, PROT_WRITE, MAP_FIXED|MAP_SHARED, fd, 0);
        if (lower != address)
                return -errno;

        upper = mmap((uint8_t *)address + size, size, PROT_WRITE, MAP_FIXED|MAP_SHARED, fd, 0);
