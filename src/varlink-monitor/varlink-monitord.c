...
/* allowed_uid=
 *   UID_INVALID: capture all varlink traffic
 *   >= 0:        only capture traffic where sender or peer matches this uid */
static int bpf_monitor_new(uid_t allowed_uid, BpfMonitor **ret) {
        ...
        obj->bss->allowed_uid = allowed_uid;
...
static int vl_method_setup(
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                { "authorize", SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_stdbool, 0, 0 },
                {}
        };
        ...
        r = sd_varlink_dispatch(link, parameters, dispatch_table, &authorize);
        if (r != 0)
                return r;
        ...
        privileged = (peer_uid == 0);

        if (authorize && !privileged) {
                r = varlink_verify_polkit_async_full(
                                ...
                                /* ret_admin= */ NULL);
                if (r == 0)
                        return 0; /* Async polkit auth pending, we'll be called again */
                if (r < 0)
                        return r; /* Authorization denied */

                privileged = true;
        }

        /* Security boundary: restrict what the caller is allowed to see */
        r = bpf_monitor_new(privileged ? UID_INVALID : peer_uid, &m);
        if (r < 0)
