...
static int bpf_monitor_set_filters(BpfMonitor *m, sd_json_variant *filters) {
        ...
        JSON_VARIANT_ARRAY_FOREACH(e, filters) {
                ...
                r = sd_json_dispatch_full(e, filter_dispatch, /* bad= */ NULL, /* flags= */ 0, &p, /* bad_field= */ NULL);
                if (r < 0)
                        return r;

                _cleanup_free_ char *path = TAKE_PTR(p.path);

                /* NUL-pad the struct — the BPF path comparison uses
                 * __builtin_memcmp over the full MONITOR_VARLINK_MAX_PATH. */
