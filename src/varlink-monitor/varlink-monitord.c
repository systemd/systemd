...
static int bpf_monitor_new(BpfMonitor **ret) {
        ...
        if (r < 0) {
                /* RHASH may not be supported, downgrade to regular HASH with a lower limit */
                log_debug_errno(r, "Failed to load BPF object, retrying without RHASH: %m");

                obj = monitor_varlink_bpf_free(obj);

                obj = monitor_varlink_bpf__open();
                if (!obj)
                        return log_error_errno(errno, "Failed to open BPF object: %m");

                (void) sym_bpf_map__set_type(obj->maps.varlink_sock_map, BPF_MAP_TYPE_HASH);
                (void) sym_bpf_map__set_max_entries(obj->maps.varlink_sock_map, MONITOR_VARLINK_MAX_SOCKETS_FALLBACK);
...
static int bpf_monitor_set_filters(BpfMonitor *m, struct monitor_varlink_filter *filters, uint32_t n_filters) {
        int r;
        ...
        for (uint32_t i = 0; i < n_filters; i++) {
                r = sym_bpf_map_update_elem(filter_map_fd, &i, &filters[i], BPF_ANY);
                if (r < 0)
                        return log_error_errno(r, "Failed to update filter map: %m");
        }

        m->obj->bss->n_filters = n_filters;
        return 0;
}
...
static bool filters_are_self_only(struct monitor_varlink_filter *filters, size_t n_filters, uid_t peer_uid) {
        if (n_filters == 0)
                return false;

        /* UIDs in these ranges are not reliable identity boundaries — they may be shared across
         * containers, reused dynamically, or map to system services. Don't treat them as "self". */
        if (peer_uid != 0 &&
            (uid_is_system(peer_uid) ||
             uid_is_dynamic(peer_uid) ||
             uid_is_container(peer_uid) ||
             uid_is_foreign(peer_uid) ||
             uid_is_greeter(peer_uid)))
                return false;

        for (size_t i = 0; i < n_filters; i++)
                if (filters[i].uid != peer_uid)
                        return false;

        return true;
}

static int vl_dispatch_filters(
                sd_varlink *link,
                sd_json_variant *v,
                struct monitor_varlink_filter **ret_filters,
                size_t *ret_n_filters) {

        int r;

        assert(link);
        assert(ret_filters);
        assert(ret_n_filters);
        ...
        if (n > MONITOR_VARLINK_MAX_FILTERS)
                return sd_varlink_errorbo(link, "io.systemd.VarlinkMonitor.BadFilter", SD_JSON_BUILD_PAIR_STRING("parameter", "filters"));

        _cleanup_free_ struct monitor_varlink_filter *filters = new0(struct monitor_varlink_filter, n);
        if (!filters)
                return log_oom();
        ...
        JSON_VARIANT_ARRAY_FOREACH(e, v) {
                ...
                static const sd_json_dispatch_field filter_dispatch[] = {
                        { "uid",        _SD_JSON_VARIANT_TYPE_INVALID, sd_json_dispatch_uid_gid, voffsetof(p, uid),       SD_JSON_NULLABLE },
                        { "pid",        _SD_JSON_VARIANT_TYPE_INVALID, json_dispatch_pidref,     voffsetof(p, pid),       SD_JSON_NULLABLE },
                        { "path",       SD_JSON_VARIANT_STRING,        sd_json_dispatch_string,  voffsetof(p, path),      SD_JSON_NULLABLE },
                        {}
                };
                ...
                filters[i] = (struct monitor_varlink_filter) {
                        .uid = p.uid,
                        .pid = p.pid.pid,
                        .pidfd_id = p.pid.fd_id,
                };
                ...
                if (path) {
                        ...
                        if (path_len > MONITOR_VARLINK_MAX_PATH)
                                return sd_varlink_errorbo(link, "io.systemd.VarlinkMonitor.BadFilter", SD_JSON_BUILD_PAIR_STRING("parameter", "path"));

                        filters[i].has_path = true;
                        filters[i].path_len = path_len;
                        memcpy(filters[i].path, path, path_len);
                }

                i++;
        }

        *ret_filters = TAKE_PTR(filters);
        *ret_n_filters = i;
        return 0;
}
...
static int vl_method_acquire(
        ...
        _cleanup_free_ struct monitor_varlink_filter *bpf_filters = NULL;
        size_t n_bpf_filters = 0;

        r = vl_dispatch_filters(link, filters, &bpf_filters, &n_bpf_filters);
        if (r != 0)
                return r;
        ...
        bool self_only = filters_are_self_only(bpf_filters, n_bpf_filters, peer_uid);
        ...
        r = bpf_monitor_set_filters(m, bpf_filters, n_bpf_filters);
        if (r < 0)
                return r;
...
static int run(int argc, char *argv[]) {
        ...
        r = sd_varlink_server_bind_method(varlink_server, "io.systemd.VarlinkMonitor.Acquire", vl_method_acquire);
        if (r < 0)
