...
static int ruleset_load_one(Manager *m, const char *filename) {
        ...
        *ruleset = (OomdRuleset) {
        ...
        /* A threshold of 100% can never be exceeded, so the condition would never fire. */
        if (ruleset->memory_pressure_above == 10000) {
                log_warning("Ruleset '%s' has MemoryPressureAbove=100%% which can never be exceeded, ignoring.", ruleset->name);
                return 0;
        }

        if (ruleset->swap_above == 10000) {
                log_warning("Ruleset '%s' has SwapUsageMax=100%% which can never be exceeded, ignoring.", ruleset->name);
                return 0;
        }
