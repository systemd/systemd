...
static int ruleset_execute_action(
        ...
        if (ruleset->action == OOMD_ACTION_KILL_ALL) {
                ...
                r = oomd_select_by_pgscan_rate(m->monitored_rules_cgroup_contexts_candidates, /* prefix= */ ctx->path, &selected);
                if (r < 0)
                ...
                threshold = m->system_context.swap_total * THRESHOLD_SWAP_USED_PERCENT / 100;
                r = oomd_select_by_swap_usage(m->monitored_rules_cgroup_contexts_candidates, threshold, &selected);
                if (r < 0)
...
static int process_rules_cgroup_context(Manager *m, OomdCGroupContext *ctx, usec_t usec_now) {
        ...
        STRV_FOREACH(rule_name, ctx->rules) {
                ...
                /* Remove start time so LastingSec must be satisfied again before
                 * re-triggering, preventing rapid repeated kills. */
...
static int monitor_rules_contexts_handler(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        /* Update candidate contexts so pgscan rates are computed as proper differentials */
        r = update_monitored_cgroup_contexts_candidates(
