...
static int process_managed_oom_message(Manager *m, uid_t uid, sd_json_variant *parameters) {
        ...
        JSON_VARIANT_ARRAY_FOREACH(c, cgroups) {
                ...
                /* Validate the full path, which means we need to prefix it */
                r = chase(message.path,
                          "/sys/fs/cgroup/",
                          CHASE_PREFIX_ROOT | CHASE_NO_AUTOFS | CHASE_PROHIBIT_SYMLINKS | CHASE_NOFOLLOW | CHASE_MUST_BE_DIRECTORY,
                          &chased,
                          /* ret_fd= */ NULL);
                if (r < 0) {
                        log_debug_errno(r, "Failed to resolve and normalize cgroup path '%s', ignoring: %m", message.path);
                        continue;
                }
                /* And then cut the prefix again, as the rest of the code expects it relative to /sys/fs/cgroup */
                const char *chased_path = path_startswith(chased, "/sys/fs/cgroup");
                if (!chased_path) {
                        log_debug("Resolved cgroup path '%s' is outside of /sys/fs/cgroup, ignoring.", chased);
                        continue;
                }
                ...
                if (uid != 0) {
                        ...
                        r = cg_path_get_owner_uid(chased_path, &cg_uid);
                        if (r < 0) {
                                log_debug_errno(r, "Failed to get cgroup %s owner uid: %m", chased_path);
                                continue;
                        }
                ...
                if (message.mode == MANAGED_OOM_AUTO) {
                        (void) oomd_cgroup_context_unref(hashmap_remove(monitor_hm, empty_to_root(chased_path)));
                        ...
                        if (streq(message.property, "OOMRules")) {
                                ...
                                HASHMAP_FOREACH(ruleset, m->rulesets) {
                                        ...
                                        free(hashmap_remove2(ruleset->start_times, empty_to_root(chased_path), (void **) &key));
                                }
                        }
                ...
                if (streq(message.property, "OOMRules")) {
                        ...
                        ctx = hashmap_get(monitor_hm, empty_to_root(chased_path));
                        if (!ctx) {
                                r = oomd_insert_cgroup_context(NULL, monitor_hm, chased_path);
                                if (r == -ENOMEM)
                                        return r;
                                if (r < 0) {
                        ...
                        if (ctx) {
                                ...
                                STRV_FOREACH(old_rule, ctx->rules) {
                                        ...
                                        if (!dropped)
                                                continue;
                                        _cleanup_free_ char *key = NULL;
                                        free(hashmap_remove2(dropped->start_times, empty_to_root(chased_path), (void**) &key));
                                }
