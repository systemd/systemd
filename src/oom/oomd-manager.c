...
static int monitor_swap_contexts_handler(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        if (oomd_swap_free_below(&m->system_context, 10000 - m->swap_used_limit_permyriad)) {
                ...
                threshold = m->system_context.swap_total * THRESHOLD_SWAP_USED_PERCENT / (uint64_t) 100;
...
static void clear_candidate_hashmap(Manager **m) {
        if (m && *m && (*m)->monitored_mem_pressure_cgroup_contexts_candidates)
...
static int monitor_memory_pressure_contexts_handler(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        _cleanup_(clear_candidate_hashmap) Manager *clear_candidates = userdata;
        ...
        if (r < 0)
                ...
                /* If any monitored cgroup is within 1-2 seconds of hitting their pressure limit, start getting kill
                 * candidates. This saves CPU cycles from doing it every interval.
                ...
                OomdCGroupContext *c;
                usec_t collect_duration = MIN((m->default_mem_pressure_duration_usec - 1 * USEC_PER_SEC),
                                              (m->default_mem_pressure_duration_usec - 2 * USEC_PER_SEC));
                HASHMAP_FOREACH(c, m->monitored_mem_pressure_cgroup_contexts) {
                        if (c->mem_pressure_limit_hit_start == 0 || ((now(CLOCK_MONOTONIC) - c->mem_pressure_limit_hit_start) < collect_duration))
                                continue;
