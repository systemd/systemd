...
static int process_managed_oom_message(Manager *m, uid_t uid, sd_json_variant *parameters) {
        ...
        JSON_VARIANT_ARRAY_FOREACH(c, cgroups) {
                ...
                /* D-Bus shouldn't send duration if its equal to the default value, so duration will be 0 in that case.
                 * For insurance, check to make sure duration is neither 0 (not sent) or default value (USEC_INFINITY) */
                if (streq(message.property, "ManagedOOMMemoryPressure") && message.duration > 0 && message.duration != USEC_INFINITY)
