...
static int monitor_swap_contexts_handler(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        if (oomd_mem_available_below(&m->system_context, 10000 - m->swap_used_limit_permyriad) &&
                ...
                if (r == -ENOMEM)
                        return log_oom();
                if (r < 0)
                        log_notice_errno(r, "Failed to select any cgroups based on swap: %m");
                else {
                        if (selected && r > 0) {
        ...
        if (r < 0)
                ...
                SET_FOREACH(t, targets) {
                        ...
                        r = oomd_select_by_pgscan_rate(m->monitored_mem_pressure_cgroup_contexts_candidates,
                                                       ...
                                                       &selected);
                        if (r < 0)
                                return log_warning_errno(r, "Failed to select any cgroups based on swap, ignoring: %m");
                        if (r == 0) {
                        ...
                        if (r == -ENOMEM)
                                return log_oom();
                        if (r < 0)
                                log_warning_errno(r, "Failed to select any cgroups under %s based on pressure, ignoring: %m", t->path);
                        else {
