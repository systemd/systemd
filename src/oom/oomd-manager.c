...
static int process_managed_oom_message(Manager *m, uid_t uid, sd_json_variant *parameters) {
        ...
        JSON_VARIANT_ARRAY_FOREACH(c, cgroups) {
                ...
                _cleanup_free_ char *chased_path = NULL;
                const char *path;
                ...
                path = empty_to_root(message.path);

                /* First, a superficial and quick check */
                if (!path_is_normalized(path)) {
                        log_debug("Received non-normalized cgroup path '%s', ignoring.", message.path);
                        continue;
                }

                /* Special case: if we are just removing a ref, then the cgroup might not exist, skip chasing */
                if (message.mode != MANAGED_OOM_AUTO) {
                        /* Validate the full path, which means we need to prefix it. An empty path refers to the
                        * root cgroup, so normalize it first. */
                        r = chase(path,
                                "/sys/fs/cgroup/",
                                ...
                                /* ret_fd= */ NULL);
                        if (r < 0) {
                                log_debug_errno(r, "Failed to resolve cgroup path '%s', ignoring: %m", message.path);
                                continue;
                        }

                        /* And then cut the prefix, as the rest expects it relative to /sys/fs/cgroup */
                        path = empty_to_root(path_startswith_full(chased_path, "/sys/fs/cgroup/", PATH_STARTSWITH_RETURN_LEADING_SLASH));
                        /* This was the result of chase() under /sys/fs/cgroup so it cannot be empty */
                        assert(!isempty(path));
                }
