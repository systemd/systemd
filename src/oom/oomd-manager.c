...
static int monitor_swap_contexts_handler(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        if (oomd_mem_available_below(&m->system_context, 10000 - m->swap_used_limit_permyriad) &&
                ...
                r = oomd_kill_by_swap_usage(candidates, threshold, m->dry_run, &selected, m->prekill_enabled, m->prekill_timeout, m->event);
                if (r == -ENOMEM)
                        return log_oom();
                if (r < 0)
        ...
        if (r < 0)
                ...
                SET_FOREACH(t, targets) {
                        ...
                        r = oomd_kill_by_pgscan_rate(m->monitored_mem_pressure_cgroup_contexts_candidates,
                                                     ...
                                                     /* dry_run= */ m->dry_run,
                                                     &selected,
                                                     m->prekill_enabled == ENABLE_PREKILL_HOOK_YES,
                                                     m->prekill_timeout,
                                                     m->event);
                        if (r == -ENOMEM)
                                return log_oom();
