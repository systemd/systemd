...
static int monitor_swap_contexts_handler(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        if (oomd_mem_available_below(&m->system_context, 10000 - m->swap_used_limit_permyriad) &&
                ...
                while (!hashmap_isempty(candidates)) {
                        ...
                        if (r == -ENOMEM)
                                return log_oom();
                        if (r < 0) {
                                log_notice_errno(r, "Failed to mark %s for killing, retrying: %m", selected->path);
                                hashmap_remove(candidates, selected);
                        } else {
                                if (selected && r > 0) {
...
static int monitor_memory_pressure_contexts_handler(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        if (r < 0)
                ...
                SET_FOREACH(t, targets) {
                        ...
                        while (!hashmap_isempty(m->monitored_mem_pressure_cgroup_contexts_candidates)) {
                                ...
                                if (r == -ENOMEM)
                                        return log_oom();
                                if (r < 0) {
                                        log_notice_errno(r, "Failed to select any cgroups under %s based on pressure, retrying: %m", t->path);
                                        hashmap_remove(m->monitored_mem_pressure_cgroup_contexts_candidates, selected);
                                } else {
