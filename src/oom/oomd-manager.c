        ...
        if (r < 0)
                ...
                if ((usec_now - m->last_reclaim_at) <= RECLAIM_DURATION_USEC) {
                        ...
                        SET_FOREACH(t, targets) {
                                ...
                                log_debug("Memory pressure for %s is %lu.%02lu%% > %lu.%02lu%% for > %"PRIu64" seconds "
                                ...
                                if (r < 0)
                                        ...
                                        if (selected != NULL)
                                                log_notice("Killed %s due to memory pressure for %s being %lu.%02lu%% > "
                                                           "%lu.%02lu%% for > %"PRIu64" seconds with reclaim activity",
                                                           selected, t->path,
