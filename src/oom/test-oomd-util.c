...
static int wait_for_killed_signal(sd_bus *client, const char *cgroup_path, const char *reason) {
        int r;
...
TEST(oomd_cgroup_kill_signal_reason) {
        ...
        for (size_t i = 0; i < ELEMENTSOF(reasons); i++) {
                ...
                ASSERT_OK_POSITIVE(oomd_cgroup_kill(&m, &(OomdCGroupContext) { .path = subcgroup }, false, reason));
                ASSERT_OK(wait_for_killed_signal(client, subcgroup, reason));
