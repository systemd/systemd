...
static void test_oomd_get_cgroup_oom_preference(void) {
        _cleanup_(oomd_cgroup_context_freep) OomdCGroupContext *ctx = NULL;
        _cleanup_free_ char *cgroup = NULL;
        ...
        if (test_xattrs) {
                assert_se(oomd_get_cgroup_oom_preference(ctx, NULL) == 0);
        ...
        assert_se(oomd_get_cgroup_oom_preference(ctx, "/herp.slice/derp.scope") == -EINVAL);
        ...
        if (test_xattrs && !empty_or_root(ctx->path)) {
                ...
                assert_se(oomd_get_cgroup_oom_preference(ctx, ctx->path) == 0);
                assert_se(ctx->preference == MANAGED_OOM_PREFERENCE_AVOID);
        }
}
...
int main(void) {
        ...
        test_oomd_get_cgroup_oom_preference();
