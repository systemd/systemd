...
static void sd_oomd_kill_state_unref(struct sd_oomd_kill_state *ks) {
        ...
        if (--ks->n_ref == 0) {
                if (ks->path) {
                        oomd_cgroup_kill(ks->path, ks->recurse, ks->prekill_timeout, ks->e);
...
static int prekill_callback(
        ...
        struct sd_oomd_kill_state *ks = userdata;
...
int oomd_kill_by_pgscan_rate(Hashmap *h, const char *prefix, bool dry_run, char **ret_selected, bool prekill, usec_t prekill_timeout, sd_event *e) {
        ...
        FOREACH_ARRAY(i, sorted, n) {
                ...
                r = oomd_cgroup_kill_mark(c->path, /* recurse= */ true, /* dry_run= */ dry_run, prekill, /* prekill_timeout= */ prekill_timeout, e);
                if (r == -ENOMEM)
