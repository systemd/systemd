...
static int oomd_prekill_hook(const char *path, usec_t prekill_deadline, sd_event *e) {
        ...
        struct sd_oomd_kill_state *ks = malloc(sizeof(*ks));
        if (!ks)
                return log_oom_debug();
        ...
        *ks = (struct sd_oomd_kill_state) {
        ...
        if (!e) {
                log_debug("No event loop, skipping oomd prekill hook.");
                goto do_kill;
        }

        d = opendir(VARLINK_ADDR_PATH_PREKILL);
        if (!d) {
                log_warning_errno(errno, "Failed to open prekill varlink socket directory %s, ignoring: %m",
...
do_kill:
        sd_oomd_kill_state_dec_pending(ks);
