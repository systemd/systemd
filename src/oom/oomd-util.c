...
DEFINE_TRIVIAL_CLEANUP_FUNC_FULL(OomdKillState*, oomd_kill_state_free, NULL);
DEFINE_PRIVATE_TRIVIAL_REF_UNREF_FUNC(OomdKillState, oomd_kill_state, oomd_kill_state_dec);
DEFINE_TRIVIAL_CLEANUP_FUNC_FULL(OomdKillState*, oomd_kill_state_unref, NULL);
...
int oomd_cgroup_kill_mark(Manager *m, OomdCGroupContext *ctx) {
        ...
        _cleanup_(oomd_kill_state_unrefp) OomdKillState *ks = new(OomdKillState, 1);
        if (!ks)
                return log_oom_debug();
