...
static int oomd_prekill_hook(const char *path, struct PrekillHook *hook) {
        ...
        r = sd_varlink_connect_address(&link, hook->path);
        if (r < 0) {
                log_info("Prekill socket at %s not connected, ignoring.", hook->path);
                return 0;
        }
        ...
        (void) sd_varlink_set_relative_timeout(link, hook->timeout_usec);
...
int oomd_kill_by_pgscan_rate(Hashmap *h, const char *prefix, bool dry_run, char **ret_selected, struct PrekillHook *hook) {
        ...
        FOREACH_ARRAY(i, sorted, n) {
                ...
                r = oomd_cgroup_kill(c->path, /* recurse= */ true, /* dry_run= */ dry_run, /* hook= */ hook);
                if (r == -ENOMEM)
