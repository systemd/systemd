 ...
 * in VARLINK_ADDR_PATH_OOMD_PREKILL directory. It returns immediately if no prekill hooks are configured
...
int oomd_cgroup_kill_mark(Manager *m, OomdCGroupContext *ctx) {
        ...
        *ks = (OomdKillState) {
        ...
        set_ensure_put(&m->kill_states, &oomd_kill_state_hash_ops, ks);
