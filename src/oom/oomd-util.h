...
static inline int compare_pgscan_rate_and_memory_usage(OomdCGroupContext * const *c1, OomdCGroupContext * const *c2) {
        ...
        /* If one increased and the other decreased, sort the increased rate first */
        r = CMP((*c2)->pgscan > (*c2)->last_pgscan, (*c1)->pgscan > (*c1)->last_pgscan);
        if (r != 0)
                return r;

        /* If both increased, sort the larger increase first */
        if ((*c2)->pgscan > (*c2)->last_pgscan && (*c1)->pgscan > (*c1)->last_pgscan)
                r = CMP((*c2)->pgscan - (*c2)->last_pgscan, (*c1)->pgscan - (*c1)->last_pgscan);
        /* If both decreased (or had no change), sort the smaller decrease first */
        else
                r = CMP((*c1)->last_pgscan - (*c1)->pgscan, (*c2)->last_pgscan - (*c2)->pgscan);
        if (r != 0)
                return r;
