...
static int config_parse_duration(
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to parse %s= parameter: %m", lvalue);

        if (usec > 0 && usec < 1 * USEC_PER_SEC)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "%s= must be 0 or at least 1s", lvalue);

        if (usec == USEC_INFINITY)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "%s= must be less than infinity", lvalue);

        /* Map zero -> default for backwards compatibility */
        *duration = usec ?: DEFAULT_MEM_PRESSURE_DURATION_USEC;
        return 0;
}
