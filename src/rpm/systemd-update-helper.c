...
static int help(void) {
        printf("%1$s [OPTIONS...] COMMAND [UNITS...]\n\n"
               ...
               "  reload                            Reload manager configuration\n"
               "  restart                           Enqueue marked units\n"
               "  reload-restart                    Reload configuration and enqueue marked units\n",
...
static int reload_restart(int argc, char **argv, RuntimeScope scope) {
        ...
        if (scope == RUNTIME_SCOPE_SYSTEM) {
                ...
                if (strstr(argv[0], "reload") || strstr(argv[0], "reexec")) {
                        _cleanup_(bus_wait_for_jobs_freep) BusWaitForJobs *w = NULL;
...
static int verb_reload_restart(int argc, char **argv, void *userdata) {
        return reload_restart(argc, argv, arg_runtime_scope);
}
...
static int run(int argc, char *argv[]) {
        ...
        static const Verb verbs[] = {
                ...
                { "mark-restart-units",        2, VERB_ANY, 0, verb_mark_restart_units        },
                { "mark-reload-units",         2, VERB_ANY, 0, verb_mark_reload_units         },
                { "reload",                    1, 1,        0, verb_reload_restart            },
                { "restart",                   1, 1,        0, verb_reload_restart            },
                { "reload-restart",            1, 1,        0, verb_reload_restart            },
