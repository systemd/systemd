...
int start_upload(Uploader *u,
        ...
        if (!u->easy) {
                ...
                easy_setopt(curl, CURLOPT_USERAGENT, "systemd-journal-upload " PACKAGE_STRING, LOG_WARNING, );
...
static int open_file_for_upload(Uploader *u, const char *filename) {
        ...
        if (arg_follow) {
                r = sd_event_add_io(u->events, &u->input_event, fd, EPOLLIN, dispatch_fd_input, u);
                if (r < 0) {
...
static int dispatch_sigterm(sd_event_source *event, const struct signalfd_siginfo *si, void *userdata) {
        Uploader *u = userdata;
