...
static int parse_argv(int argc, char *argv[]) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION_LONG("url", "URL", "Read events from systemd-journal-gatewayd at URL"):
                        r = free_and_strdup_warn(&arg_url, state.argument);
                        if (r < 0)
                                return r;
                        break;

                OPTION_LONG("getter", "COMMAND", "Read events from the output of COMMAND"):
                        r = free_and_strdup_warn(&arg_getter, state.argument);
                        if (r < 0)
                                return r;
                        break;

                OPTION_LONG("listen-raw", "ADDR", "Listen for connections at ADDR"):
                        r = free_and_strdup_warn(&arg_listen_raw, state.argument);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("listen-http", "ADDR", "Listen for HTTP connections at ADDR"):
                        ...
                        r = negative_fd(state.argument);
                        if (r >= 0)
                                http_socket = r;
                        else {
                                r = free_and_strdup_warn(&arg_listen_http, state.argument);
                                if (r < 0)
                                        return r;
                        }
                        break;
                ...
                OPTION_LONG("listen-https", "ADDR", "Listen for HTTPS connections at ADDR"):
                        ...
                        if (r >= 0)
                                ...
                                r = free_and_strdup_warn(&arg_listen_https, state.argument);
                                if (r < 0)
                                        return r;
                        }
                        break;
                ...
                OPTION_LONG("key", "FILENAME", "SSL key in PEM format (default: \"" PRIV_KEY_FILE "\")"):
                        r = free_and_strdup_warn(&arg_key, state.argument);
                        if (r < 0)
                                return r;
                        break;

                OPTION_LONG("cert", "FILENAME", "SSL certificate in PEM format (default: \"" CERT_FILE "\")"):
                        r = free_and_strdup_warn(&arg_cert, state.argument);
                        if (r < 0)
                                return r;
                        break;
...
#if HAVE_GNUTLS
                        r = free_and_strdup_warn(&arg_trust, state.argument);
                        if (r < 0)
                                return r;
#else
                ...
                OPTION_LONG("split-mode", "none|host", "How many output files to create"):
                        arg_split_mode = journal_write_split_mode_from_string(state.argument);
                        if (arg_split_mode == _JOURNAL_WRITE_SPLIT_INVALID)
                                return log_error_errno(arg_split_mode, "Invalid split mode: %s", state.argument);
                        break;

                OPTION_LONG_FLAGS(OPTION_OPTIONAL_ARG, "compress", "BOOL",
                                  "Use compression in the output journal (default: yes)"):
                        r = parse_boolean_argument("--compress", state.argument, &arg_compress);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG_FLAGS(OPTION_OPTIONAL_ARG, "seal", "BOOL",
                        ...
                        r = parse_boolean_argument("--seal", state.argument, &arg_seal);
                        if (r < 0)
                                return r;
                        break;
...
#if HAVE_GNUTLS
                        for (const char *p = state.argument;;) {
                ...
                OPTION_LONG("max-use", "BYTES", "Maximum disk space to use"):
                        r = parse_size(state.argument, 1024, &arg_max_use);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --max-use= value: %s", state.argument);
                        break;

                OPTION_LONG("keep-free", "BYTES", "Minimum disk space to keep free"):
                        r = parse_size(state.argument, 1024, &arg_keep_free);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --keep-free= value: %s", state.argument);
                        break;

                OPTION_LONG("max-file-size", "BYTES", "Maximum size of individual journal files"):
                        r = parse_size(state.argument, 1024, &arg_max_size);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --max-file-size= value: %s", state.argument);
                        break;

                OPTION_LONG("max-files", "N", "Maximum number of journal files to keep"):
                        r = safe_atou64(state.argument, &arg_n_max_files);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --max-files= value: %s", state.argument);
                        break;
                }
