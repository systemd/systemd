...
static int get_accept_encoding(char **encoding) {
        size_t count = 0;
        int r;
        float q = 1.0;

        while (arg_compression && arg_compression[count])
                count++;
        ...
        if (count > 0) {
                ...
                for (int i = 0; arg_compression[i] != NULL; i++, q-=step) {
                        _cleanup_free_ const char *c = compression_lowercase_to_string(arg_compression[i]->algorithm);
                        r = strextendf_with_separator(encoding, ",", "%s;q=%.1f", c, q);
                        if (r < 0)
                                return log_oom();
                }
        }
...
static int request_meta(void **connection_cls, int fd, char *hostname) {
        ...
        r = get_accept_encoding(&source->encoding);
        if (r < 0)
                return log_oom();
