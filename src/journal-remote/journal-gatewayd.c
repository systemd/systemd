...
static int request_handler_file(
        ...
        if (MHD_add_response_header(response, "Content-Type", mime_type) == MHD_NO ||
            MHD_add_response_header(response, "Content-Security-Policy", "default-src 'none'; navigate-to 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; connect-src 'self'; img-src 'self';") == MHD_NO)
                return respond_oom(connection);
