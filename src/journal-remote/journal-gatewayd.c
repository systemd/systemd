        ...
        for (int i = 0; i < (int) n_boots; i++) {
                ...
                r = sd_json_variant_append_arrayb(
                                ...
                                SD_JSON_BUILD_OBJECT(
                                                ...
                                                SD_JSON_BUILD_PAIR("boot_id", SD_JSON_BUILD_STRING(boot_id)),
                                                SD_JSON_BUILD_PAIR("first_entry", SD_JSON_BUILD_UNSIGNED(boots[i].first_usec)),
                                                SD_JSON_BUILD_PAIR("last_entry", SD_JSON_BUILD_UNSIGNED(boots[i].last_usec))));
                if (r < 0)
                        return mhd_respondf(connection, r, MHD_HTTP_INTERNAL_SERVER_ERROR, "Failed to append JSON object: %m");
        }

        r = sd_json_variant_format(array, 0, &json);
        if (r < 0) {
