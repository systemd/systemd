...
static int modules_list_append_dup(OrderedSet *module_set, const char *module) {
        ...
        return modules_list_append_take(module_set, &m);
}
...
static int apply_file(FILE *f, const char *filename, OrderedSet *module_set) {
        ...
        for (;;) {
                ...
                r = modules_list_append_take(module_set, &line);
                if (r < 0)
...
static int enqueue_module_to_load(int sock, const char *module) {
        ...
        if (strlen(module) > NAME_MAX) {
                log_error("Filename max length exceeded (%d): %s", NAME_MAX, module);
                return -1;
        }
...
static int dequeue_module_to_load(int sock, char *buffer, size_t buffer_len, bool *terminate) {
        ssize_t bytes;

        assert(sock >= 0);
        assert(buffer);

        bytes = recv(sock, buffer, buffer_len - 1, 0);
        if (bytes == 0) {
                *terminate = true;
                return 0;
        } else if (bytes < 0)
...
static int run_prober(int sock) {
        ...
        for (;;) {
                r = dequeue_module_to_load(sock, buffer, sizeof(buffer), &terminate);
                if (r < 0) {
                        RET_GATHER(ret, log_error_errno(errno, "Failed to receive from module queue: %m"));
                        break;
                } else if (terminate) {
...
static int run(int argc, char *argv[]) {
        ...
        num_threads = determine_num_worker_threads(ordered_set_size(module_set));
