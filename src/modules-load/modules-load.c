...
#define MODULE_NAME_MAX_LEN ((unsigned long) NAME_MAX)
...
static int apply_file(int sock, const char *path, bool ignore_enoent) {
        ...
        for (;;) {
                ...
                if (bytes_received <= 0) {
                        if (bytes_received == 0)
                                ...
                                RET_GATHER(r, log_error_errno(errno, "Failed to receive: %m"));
                        break;
                }
...
static unsigned determine_num_threads(void) {
