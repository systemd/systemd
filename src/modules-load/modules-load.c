...
static int run(int argc, char *argv[]) {
        ...
        pthread_t *threads = malloc(sizeof(pthread_t) * max_tasks);
        if (socketpair(AF_UNIX, SOCK_SEQPACKET, 0, sockets) == -1)
                log_error_errno(errno, "socketpair");

        // Create worker threads
        for (int i = 0; i < max_tasks; ++i)
                if (pthread_create(&threads[i], NULL, worker_thread, &sockets[1]) != 0)
                        log_error_errno(errno, "pthread_create");

        r = 0;
        if (argc > optind) {
                for (int i = optind; i < argc; i++)
                        RET_GATHER(r, apply_file(sockets, argv[i], false));
        } else {
                _cleanup_strv_free_ char **files = NULL;
                STRV_FOREACH(i, arg_proc_cmdline_modules)
                        if (send(sockets[0], *i, strlen(*i), 0))
                                log_error_errno(errno, "send");
                ...
                STRV_FOREACH(fn, files)
                        RET_GATHER(r, apply_file(sockets, *fn, true));
        }
