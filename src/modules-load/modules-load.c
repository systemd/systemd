...
static int apply_file(FILE *f, const char *filename, OrderedSet **module_set) {
        ...
        for (;;) {
                ...
                r = modules_list_append_suffix(module_set, TAKE_PTR(line));
                if (r < 0)
...
static unsigned determine_num_worker_threads(unsigned n_modules) {
        ...
        if (e) {
                ...
                if (r < 0)
                        log_debug(r, "Invalid value in $SYSTEMD_MODULES_LOAD_NUM_THREADS, ignoring: %s", e);
        }
...
static int run(int argc, char *argv[]) {
        ...
        if (argc > optind) {
                ...
                STRV_FOREACH(i, arg_proc_cmdline_modules) {
                        ...
                        if (r < 0)
                                RET_GATHER(ret, log_error_errno(r, "Failed to add module '%s': %m", *i));
                }
