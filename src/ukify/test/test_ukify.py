...
def test_efi_signing(kernel_initrd, tmpdir):
    ...
    cert = ourdir / 'example.signing.crt'
    key = ourdir / 'example.signing.key'
...
def test_pcr_signing(kernel_initrd, tmpdir):
    ...
    ourdir = pathlib.Path(__file__).parent
    pub = ourdir / 'example.tpm2-pcr-public.pem'
    priv = ourdir / 'example.tpm2-pcr-private.pem'
    ...
    opts = ukify.parse_args([
        *kernel_initrd,
        ...
        f'--pcrpkey={pub}',
        f'--pcr-public-key={pub}',
        f'--pcr-private-key={priv}',
    ])
    ...
    assert open(tmpdir / 'out.pcrpkey').read() == pub.read_text()
...
def test_pcr_signing2(kernel_initrd, tmpdir):
    ...
    pub = ourdir / 'example.tpm2-pcr-public.pem'
    priv = ourdir / 'example.tpm2-pcr-private.pem'
    pub2 = ourdir / 'example.tpm2-pcr-public2.pem'
    priv2 = ourdir / 'example.tpm2-pcr-private2.pem'
    ...
    opts = ukify.parse_args([
        *kernel_initrd,
        ...
        f'--pcr-public-key={pub2}',
        f'--pcr-private-key={priv2}',
    ...
    assert open(tmpdir / 'out.pcrpkey').read() == pub2.read_text()
    assert open(tmpdir / 'out.osrel').read() == 'ID=foobar\n'
    assert open(tmpdir / 'out.uname').read() == '1.2.3'
    assert open(tmpdir / 'out.cmdline').read() == 'ARG1 ARG2 ARG3'
    sig = open(tmpdir / 'out.pcrsig').read()
