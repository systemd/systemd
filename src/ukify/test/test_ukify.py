...
def test_resolve_devicetree_path_searches_kernel_install_locations(monkeypatch):
    ...
    monkeypatch.setattr(ukify, 'devicetree_path_is_file', is_file)
...
def test_resolve_devicetree_path_conf_root_replaces_default_locations(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv('KERNEL_INSTALL_CONF_ROOT', os.fspath(root))
    monkeypatch.setattr(
        ukify, 'devicetree_path_is_file', lambda path: path == module_dtb_path(uname, 'DDDDTTTT')
    )

    with pytest.raises(
        FileNotFoundError,
        match=re.escape(f'DeviceTree file {conf_root_dtb_path(root, "DDDDTTTT")} not found'),
    ):
...
def test_resolve_devicetree_path_missing_uses_module_fallback(monkeypatch):
    ...
    monkeypatch.setattr(ukify, 'devicetree_path_is_file', lambda path: False)
