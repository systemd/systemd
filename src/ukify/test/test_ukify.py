...
def test_inspect_json_profiles(tmp_path, capsys):
    # CI provides the addon stub via $EFI_ADDON. Build everything addon-style
    # (no --linux) so only that stub is needed to exercise the JSON output.
    ...
    for i in range(2):
        ...
        build(f'--profile=ID=profile{i}', f'--cmdline=PROFILE{i}ARG', f'--output={profile}')
...
def test_inspect_json_alternative_set_sections(tmp_path, capsys):
    ...
    # Build addon-style with the CI-provided stub.
    stub = os.getenv('EFI_ADDON')
    if not stub:
        pytest.skip('EFI_ADDON not set')
    ...
    build('--cmdline=ARG', *(f'--devicetree-auto={d}' for d in dtbs), f'--output={output}')
    result = inspect(output)

    # No profiles, so no 'profiles' key and single sections stay plain objects (backwards compatible).
    ...
    build(f'--devicetree-auto={dtbs[0]}', f'--output={single}')
