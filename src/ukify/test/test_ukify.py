...
def test_join_pcrsig(capsys, kernel_initrd, tmp_path):
    ...
    ukify.inspect_sections(opts)
    text = capsys.readouterr().out
    assert re.search(fr'\.pcrpkey', text, re.MULTILINE)
    assert re.search(fr'\.pcrsig', text, re.MULTILINE)
    assert not re.search(fr'"sig":', text, re.MULTILINE)
    ...
    opts = ukify.parse_args(['inspect', output_sig])
    ukify.inspect_sections(opts)
    text = capsys.readouterr().out
    assert re.search(fr'\.pcrpkey', text, re.MULTILINE)
    assert re.search(fr'\.pcrsig', text, re.MULTILINE)
    assert re.search(fr'"sig":', text, re.MULTILINE)
