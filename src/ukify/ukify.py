...
def resolve_devicetree_path(path: Path, uname: str, check_exists: bool = False) -> Path:
    ...
    if check_exists and not resolved.exists():
        raise FileNotFoundError(f'DeviceTree file {path} not found at {resolved}')

    return resolved


def resolve_devicetree_options(opts: Union[argparse.Namespace, UkifyConfig], check_exists: bool = False) -> None:
    if opts.uname is None:
        if (opts.devicetree is not None and not opts.devicetree.is_absolute()) or \
                any(not path.is_absolute() for path in opts.devicetree_auto):
            print('Kernel version unknown, cannot resolve relative DeviceTree paths', file=sys.stderr)
        return
