...
def add_section_to_scope(scope: dict[str, Any], name: str, desc: dict[str, Union[int, str]]) -> None:
    ...
    # image is malformed, so warn and keep the first instance rather than silently restructuring the
    # output.
    if name in MULTI_INSTANCE_SECTIONS:
        scope.setdefault(name, []).append(desc)
    elif name in scope:
        print(f'Unexpected duplicate {name!r} section, only the first is reported', file=sys.stderr)
    else:
        scope[name] = desc
...
def inspect_sections(opts: UkifyConfig) -> None:
    ...
    for file in opts.files:
        ...
        # A UKI is a flat list of PE sections, but it has structure: the sections before the first
        # '.profile' section are shared between all profiles and are reported by name at the top
        # level (so e.g. '.cmdline' refers to the base as before). Each '.profile' section starts a
        # new profile whose sections (up to the next '.profile') override the shared ones; those are
        # reported as separate object in the 'profiles' list.
        result: dict[str, Any] = {}
        ...
        for section in pe.sections:
            ...
            if name == '.profile':
                scope = {}
                profiles.append(scope)

            desc = inspect_section(opts, section)
            if desc:
                add_section_to_scope(scope, name, desc)
