...
def maybe_decompress(filename):
    ...
    if start.startswith(b'BZh'):
        raise NotImplementedError('bzip2 decompression not implemented')
...
class Uname:
    ...
    VERSION_PATTERN = r'([a-z0-9._-]+) \((.*)\) (#.*)'

    NOTES_PATTERN = r'^\s+Linux\s+0x[0-9a-f]+\s+OPEN\n\s+description data: ([0-9a-f ]+)\s*$'
    ...
    TEXT_PATTERN = rb'Linux version (\d\.\S+) \('
    ...
    def scrape_x86(cls, filename, opts=None):
        ...
        with open(filename, 'rb') as f:
            f.seek(0x20E)
            offset = ord(f.read(1)) + ord(f.read(1))*256
            f.seek(0x200 + offset)
            text = f.read(128)
    ...
    def scrape_elf(cls, filename, opts=None):
        ...
        text = ''.join(chr(int(c, 16)) for c in m.group(1).split())
