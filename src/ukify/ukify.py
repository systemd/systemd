...
def call_systemd_measure(uki, opts, profile_start=0):
    ...
    to_measure = [s for s in uki.sections[profile_start:] if s.measure]
    ...
    if profile_start != 0:
        for section in uki.sections:
            ...
            if any(section.name == s.name for s in to_measure):
                continue

            to_measure.append(section)
...
def make_uki(opts):
    ...
    if opts.profile and not opts.add_profiles:
    ...
    # Don't add a stab section to profile PE binaries.
    if opts.add_profiles or not opts.profile:
        if linux is not None:
...
"""]
    ...
    if opts.profile and opts.add_profiles:
        uki.add_section(Section.create(".profile", opts.profile, measure=True))
    ...
    for profile in opts.add_profiles:
...
def finalize_options(opts):
    ...
    if opts.add_profiles and not opts.profile:
