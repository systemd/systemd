...
def check_cert_and_keys_nonexistent(opts: UkifyConfig) -> None:
    ...
    paths: Iterator[Union[str, Path, None]] = itertools.chain(
        ...
        *((priv_key, pub_key, cert) for priv_key, pub_key, cert, _, _ in key_path_groups(opts, include_extra=False)),
    )
    for path in paths:
...
CONFIG_ITEMS = [
    ...
    ConfigItem(
        '--sign-initrd-pcrs',
        action=argparse.BooleanOptionalAction,
        help='additionally sign PCR policies that can only be satisified from the initrd',
