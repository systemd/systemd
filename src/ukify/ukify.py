...
def call_systemd_measure(uki: UKI, opts: UkifyConfig, profile_start: int = 0) -> str:
    ...
    if opts.measure or opts.policy_digest:
        ...
        for dtbauto in dtbauto_to_measure:
            ...
            policy_groups: list[tuple[list[str], Optional[str]]] = [
                (list(itertools.chain.from_iterable(opts.phase_path_groups or [])), None),
            ]
            if opts.policy_digest and opts.sign_initrd_pcrs:
                policy_groups += [(['enter-initrd'], 'initrd')]
...
def finalize_options(opts: argparse.Namespace) -> None:
    ...
    if opts.sign_initrd_pcrs and not (n_pcr_priv or opts.policy_digest):
        raise ValueError('--sign-initrd-pcrs requires at least one --pcr-private-key= or --policy-digest')
