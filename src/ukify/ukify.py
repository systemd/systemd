...
"""
    ...
    for profile in opts.join_profiles:
        ...
        if names[0] != '.profile':
            raise ValueError(
                'Expected .profile section as first valid section in '
                f'PE profile binary {profile} but got {names[0]}'
            )
...
def generate_keys(opts):
    ...
    if not work:
        raise ValueError(
            'genkey: --secureboot-private-key=/--secureboot-certificate= or '
            '--pcr-private-key/--pcr-public-key must be specified'
        )
...
CONFIG_ITEMS = [
    ...
    ConfigItem(
        ...
        dest='sb_cert',
        help=(
            'required by --signtool=sbsign. sbsign needs a path to certificate file or engine-specific '
            'designation for SB signing'
        ),
    ...
    ConfigItem(
        ...
        help=(
            'required by --signtool=pesign. Path to nss certificate database directory for PE signing. '
            'Default is /etc/pki/pesign'
        ),
    ...
    ConfigItem(
        ...
        dest='sb_cert_name',
        help=(
            'required by --signtool=pesign. pesign needs a certificate nickname'
            ' of nss certificate database entry to use for PE signing'
        ),
...
def finalize_options(opts):
    ...
    if bool(opts.sb_key) ^ bool(opts.sb_cert):
        ...
        if opts.signtool and opts.signtool != 'sbsign':
            raise ValueError(
                f'Cannot provide --signtool={opts.signtool} with '
    ...
    if opts.sign_kernel and not opts.sb_key and not opts.sb_cert_name:
        raise ValueError(
            '--sign-kernel requires either --secureboot-private-key= and --secureboot-certificate='
            ' (for sbsign) or --secureboot-certificate-name= (for pesign) to be specified'
        )
