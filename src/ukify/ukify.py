...
def pe_add_sections(opts: UkifyConfig, uki: UKI, output: str) -> None:
    ...
    for section in uki.sections:
        ...
        if section.name == '.linux':
            ...
            if not (linux_pe.OPTIONAL_HEADER.DllCharacteristics & pe.DLL_CHARACTERISTICS['IMAGE_DLL_CHARACTERISTICS_NX_COMPAT']):
                pe.OPTIONAL_HEADER.DllCharacteristics &= ~pe.DLL_CHARACTERISTICS['IMAGE_DLL_CHARACTERISTICS_NX_COMPAT']
        else:
