...
class UKI:
    ...
    def add_section(self, section: Section) -> None:
        ...
        multiple_allowed_sectons = ['.dtbauto', 'efifw']
        if any(
            section.name == s.name for s in self.sections[start:] if s.name not in multiple_allowed_sectons
        ):
...
def make_uki(opts: UkifyConfig) -> None:
    ...
    sections = [
        ...
        *(('.dtbauto', dtb, True) for dtb in opts.devicetree_auto),
        ...
        ('.initrd',  initrd,          True),
        *(('.efifw', fw, True) for fw in opts.firmware),
