...
def pack_device(offsets: dict[str, int], devtype: int, name: str,
                compatible: str, chids: set[uuid.UUID]) -> bytes:
    data = b''
...
def parse_hwid_dir(path: Path) -> bytes:
    ...
    for hwid_file in hwid_files:
        ...
        devices[(devstr_to_type[data['devtype']], data['name'], data['compatible'])] |= {uuid.UUID(u) for u in data['hwids']}
