...
def find_tool(name, fallback=None, opts=None):
    ...
    if fallback is None:
        print("Tool %s not installed!" % name)
...
def verify(tool, opts):
    ...
    print('+', shell_join(cmd))
    # just check the output, as pesigcheck returns 1 if PE is not signed
    info = subprocess.run(cmd, stdout=subprocess.PIPE, text=True).stdout
...
def make_uki(opts):
    ...
    if (sign_tool is None or opts.sb_key is None) and opts.sb_cert_nick:
        sign_tool = find_pesign(opts=opts)
        sign = pesign_sign
        verify_tool = pesigcheck
        print("Using PESIGN")
    sign_args_present = opts.sb_key or opts.sb_cert_nick
...
''')
    ...
    p.add_argument('--secureboot-certificate-dir',
                   dest='sb_certdir',
                   default='/etc/pki/pesign',
                   help='path to nss certificate database directory for PE signing. Default is /etc/pki/pesign')
    p.add_argument('--secureboot-certificate-nick',
                   dest='sb_cert_nick',
                   help='nickname of nss certificate database entry to use for PE signing')
    ...
    if bool(opts.sb_key) ^ bool(opts.sb_cert):
        raise ValueError('--secureboot-private-key= and --secureboot-certificate= must be specified together for sbsign')

    if opts.sign_kernel and not opts.sb_key and not opts.sb_cert_nick:
        raise ValueError('--sign-kernel requires either --secureboot-private-key= and --secureboot-certificate= (for sbsign) or --secureboot-certificate-nick (for pesign) to be specified')
    ...
    if opts.output is None:
        ...
        suffix = '.efi' if opts.sb_key or opts.sb_cert_nick else '.unsigned.efi'
