...
def make_uki(opts: UkifyConfig) -> None:
    ...
    for profile in opts.join_profiles:
        ...
        if opts.sign_profiles:
            profile = next(s for s in pe.sections if pe_strip_section_name(s.Name) == '.profile')
            id = read_env_file(profile.get_data(length=profile.Misc_VirtualSize).decode()).get('ID')
