...
def pack_device(offsets: dict[str, int], name: str, compatible: str, chids: set[EFI_GUID]) -> bytes:
    ...
    for (data1, data2, data3, data4) in chids:
        data += struct.pack('<IIIIHH8B', DEVICE_STRUCT_SIZE, offsets[name], offsets[compatible], data1, data2, data3, *data4)
        ...
        for line in content:
            for k in data:
                ...
                if uuid is not None:
                    ...
                    data2 = int(d2, 16)
                    data3 = int(d3, 16)
                    data4 = cast(tuple[int, int, int, int, int, int, int, int], tuple(hex_pairs_list(d4) + hex_pairs_list(d5)))
