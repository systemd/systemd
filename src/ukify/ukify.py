...
def pe_add_sections(uki: UKI, output: str):
    ...
    for section in uki.sections:
        ...
        for i, s in enumerate(pe.sections):
            if s.Name.rstrip(b"\x00").decode() == section.name and new_section.Misc_VirtualSize <= s.SizeOfRawData:
                pe.__data__ = pe.__data__[:s.PointerToRawData] + data +  pe.__data__[pe.sections[i+1].PointerToRawData:]
...
def read_sbat(input: [pathlib.Path]) -> str:
    # We don't care about duplicates, but we do want to preserve the order.
    sbat = set()
    ...
    for f in input:
        ...
        for section in pe.sections:
            if section.Name.rstrip(b"\x00").decode() == ".sbat":
                split = section.get_data(ignore_padding=True).decode().splitlines()
                # Filter out the sbat line, we'll add it back later, there needs to be only one and it
                # needs to be first.
                split = [line for line in split if not line.startswith('sbat,')]
                sbat.update(split)
...
def make_uki(opts):
    ...
    if linux is not None:
        ...
        uki.add_section(Section.create('.sbat', read_sbat([linux, opts.stub]), measure=False))
