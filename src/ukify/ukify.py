    ...
    for f in input:
        ...
        for section in pe.sections:
            if section.Name.rstrip(b"\x00").decode() == ".sbat":
                split = section.get_data(ignore_padding=True).rstrip(b"\x00").decode().splitlines()
                # Filter out the sbat line, we'll add it back later, there needs to be only one and it
                # needs to be first.
                sbat += [line for line in split if not line.startswith('sbat,')]
...
def make_uki(opts):
    ...
    if linux is not None:
        ...
        uki.add_section(Section.create('.sbat', read_sbat([opts.stub, linux]), measure=False))
