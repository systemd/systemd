    ...
    if opts.pcrsig:
        ...
        for i, section in enumerate(pe.sections):
            if pe_strip_section_name(section.Name) == '.pcrsig':
                ...
                for bank, sigs in j.items():
                    for sig in sigs:
                        for input_bank, input_sigs in signatures.items():
                            for input_sig in input_sigs:
                                if input_bank == bank and sig['pol'] == input_sig['pol']:
                                    sig['sig'] = input_sig['sig']

                encoded = bytes(json.dumps(j).encode())
                if len(encoded) > section.SizeOfRawData:
                    raise PEError(
                        f'Not enough space in existing section .pcrsig of size {section.SizeOfRawData} to '
                        'append new data of size {len(encoded)}.'
                    )
...
def make_uki(opts: UkifyConfig) -> None:
    ...
    if not opts.pcrsig and (opts.join_profiles or not opts.profile) and (
