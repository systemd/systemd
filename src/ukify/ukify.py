...
def devicetree_path_is_file(path: Path) -> bool:
    return path.is_file()
...
def validate_relative_devicetree_path(path: Path) -> None:
    if not path.parts or '..' in path.parts:
        raise ValueError(
            f'Relative DeviceTree path {path} must name a file in the kernel-install '
            'DeviceTree search locations'
        )
...
def resolve_devicetree_path(
    ...
    if path.is_absolute():
        ...
        validate_relative_devicetree_path(path)

        search_paths = devicetree_search_paths(uname)
        resolved = search_paths[-1] / path

        for directory in search_paths:
            candidate = directory / path
            if devicetree_path_is_file(candidate):
                resolved = candidate
                break

    if check_exists and not devicetree_path_is_file(resolved):
...
def validate_devicetree_options(opts: UkifyOptions) -> None:
    ...
    for path in [*([] if opts.devicetree is None else [opts.devicetree]), *opts.devicetree_auto]:
...
def finalize_options(opts: argparse.Namespace) -> None:
    ...
    if opts.verb == 'build' and has_relative_devicetree(opts):
        autodetect_uname(opts, opts.linux)

    if opts.verb == 'build' and not opts.uname:
