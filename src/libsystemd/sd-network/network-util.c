...
bool network_is_online(void) {
        _cleanup_free_ char *online_state = NULL, *carrier_state = NULL, *addr_state = NULL;
        LinkOnlineState state;
        int r;

        r = sd_network_get_online_state(&online_state);
        if (r < 0)
                state = LINK_ONLINE_STATE_UNKNOWN;
        else {
        ...
        if (state >= LINK_ONLINE_STATE_PARTIALLY_ONLINE)
                return true;
        else if (state == LINK_ONLINE_STATE_UNKNOWN) {
...
static const char *const link_online_state_table[_LINK_ONLINE_STATE_MAX] = {
        [LINK_ONLINE_STATE_UNKNOWN]          = "unknown",
        [LINK_ONLINE_STATE_OFFLINE]          = "offline",
        [LINK_ONLINE_STATE_PARTIALLY_ONLINE] = "partially-online",
