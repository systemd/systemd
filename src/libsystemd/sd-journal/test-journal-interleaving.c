...
static size_t expected_result(uint64_t needle, const uint64_t *candidates, size_t n, direction_t direction) {
        switch (direction) {
        case DIRECTION_DOWN:
...
static void verify(JournalFile *f, const uint64_t *seqnum, const uint64_t *offset, size_t n) {
        ...
        for (uint64_t i = 0; i < n + 2; i++) {
                ...
                e = expected_result(i, seqnum, n, DIRECTION_DOWN);
                assert_se(r == (e != SIZE_MAX));
                assert_se(p == (e != SIZE_MAX ? offset[e] : 0));

                p = 0;
                r = journal_file_move_to_entry_by_seqnum(f, i, DIRECTION_UP, NULL, &p);
                e = expected_result(i, seqnum, n, DIRECTION_UP);
                assert_se(r == (e != SIZE_MAX));
                assert_se(p == (e != SIZE_MAX ? offset[e] : 0));
        }
        ...
        for (size_t trial = 0; trial < 3 * n; trial++) {
                ...
                e = expected_result(i, offset, n, DIRECTION_DOWN);
                assert_se(r == (e != SIZE_MAX));
                assert_se(p == (e != SIZE_MAX ? offset[e] : 0));
        }
        for (size_t trial = 0; trial < 3 * n; trial++) {
                ...
                e = expected_result(i, offset, n, DIRECTION_UP);
                assert_se(r == (e != SIZE_MAX));
                assert_se(p == (e != SIZE_MAX ? offset[e] : 0));
        }
}
...
static void test_generic_array_bisect_one(size_t n, size_t num_corrupted) {
        ...
        log_debug("/* %s(%zu, %zu) */", __func__, n, num_corrupted);
...
TEST(generic_array_bisect) {
        test_generic_array_bisect_one(5, 2);
        test_generic_array_bisect_one(6, 2);
        test_generic_array_bisect_one(6, 3);
