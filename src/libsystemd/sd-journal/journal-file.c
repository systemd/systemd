...
#define SIZE64_KB UINT64_C(1024)
#define SIZE64_MB (UINT64_C(1024) * SIZE64_KB)
#define SIZE64_GB (UINT64_C(1024) * SIZE64_MB)

/* This is the minimum journal file size */
#define JOURNAL_FILE_SIZE_MIN (512 * SIZE64_KB)          /* 512 KiB */
...
#define MAX_USE_LOWER (1 * SIZE64_MB)                    /* 1 MiB */
#define MAX_USE_UPPER (4 * SIZE64_GB)                    /* 4 GiB */
...
#define MIN_USE_LOW  (1  * SIZE64_MB)                    /* 1 MiB */
#define MIN_USE_HIGH (16 * SIZE64_MB)                    /* 16 MiB */

/* This is the upper bound if we deduce max_size from max_use */
#define MAX_SIZE_UPPER (128 * SIZE64_MB)                 /* 128 MiB */

/* This is the upper bound if we deduce the keep_free value from the file system size */
#define KEEP_FREE_UPPER (4 * SIZE64_GB)                  /* 4 GiB */

/* This is the keep_free value when we can't determine the system size */
#define DEFAULT_KEEP_FREE (1 * SIZE64_MB)                /* 1 MB */
...
#define FILE_SIZE_INCREASE (8 * SIZE64_MB)               /* 8MB */
...
static int journal_file_allocate(JournalFile *f, uint64_t offset, uint64_t size) {
        ...
        if (size > PAGE_ALIGN_DOWN_64(UINT64_MAX) - offset)
                return -EINVAL;
        ...
        if (old_arena_size > PAGE_ALIGN_DOWN_64(UINT64_MAX) - old_header_size)
                return -EBADMSG;

        old_size = old_header_size + old_arena_size;

        new_size = MAX(PAGE_ALIGN_64(offset + size), old_header_size);
...
static void journal_default_metrics(JournalMetrics *m, int fd, bool compact) {
        ...
        if (m->max_use == UINT64_MAX) {
                ...
                if (fs_size > 0)
                        m->max_use = CLAMP(PAGE_ALIGN_64(fs_size / 10), /* 10% of file system size */
                                           MAX_USE_LOWER, MAX_USE_UPPER);
                else
                        m->max_use = MAX_USE_LOWER;
        } else {
                m->max_use = PAGE_ALIGN_64(m->max_use);
        ...
        if (m->min_use == UINT64_MAX) {
                if (fs_size > 0)
                        m->min_use = CLAMP(PAGE_ALIGN_64(fs_size / 50), /* 2% of file system size */
        ...
        if (m->max_size == UINT64_MAX)
                m->max_size = MIN(PAGE_ALIGN_64(m->max_use / 8), /* 8 chunks */
                                  MAX_SIZE_UPPER);
        else
                m->max_size = PAGE_ALIGN_64(m->max_size);
        ...
        if (m->min_size == UINT64_MAX)
                ...
                m->min_size = CLAMP(PAGE_ALIGN_64(m->min_size),
                                    JOURNAL_FILE_SIZE_MIN,
                                    m->max_size ?: UINT64_MAX);

        if (m->keep_free == UINT64_MAX) {
                if (fs_size > 0)
                        m->keep_free = MIN(PAGE_ALIGN_64(fs_size / 20), /* 5% of file system size */
                                           KEEP_FREE_UPPER);
                else
