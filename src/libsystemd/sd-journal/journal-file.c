...
static int journal_file_append_data(
        ...
        compression = maybe_compress_payload(f, o->data.payload, data, size, &rsize);
...
int journal_file_data_payload(
        ...
        return maybe_decompress_payload(f, o->data.payload, size, o->object.flags & OBJECT_COMPRESSION_MASK,
                                        field, field_length, data_threshold, ret_data, ret_size);
}
...
static int journal_file_entry_item_next_inline(
        ...
        for (p = *i; p < offset + sz;) {
                ...
                r = maybe_decompress_payload(f, d, isz, flags & OBJECT_COMPRESSION_MASK, field, field_length,
                                             data_threshold, ret_data, ret_size);
                if (r == -EBADMSG) {
...
static int journal_file_entry_item_next_compact(
        ...
        p = *i == 0 ? le64toh(READ_NOW(e->entry.compact.trie_offset)) : *i;
