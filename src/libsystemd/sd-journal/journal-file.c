...
static bool offset_is_valid(uint64_t offset, uint64_t header_size, uint64_t tail_object_offset) {
        ...
        if (offset != 0 && offset < header_size)
                return false;
        if (tail_object_offset > 0 && offset > tail_object_offset)
                return false;
        return true;
}

static bool hash_is_valid(uint64_t offset, uint64_t size, uint64_t header_size, uint64_t arena_size, uint64_t tail_object_offset) {
        if ((offset == 0) != (size == 0))
                return false;
        if (offset == 0)
                return true;
...
static int journal_file_verify_header(JournalFile *f) {
        ...
        if (!offset_is_valid(tail_object_offset, header_size, 0))
                return -ENODATA;
        ...
        if (!hash_is_valid(le64toh(f->header->data_hash_table_offset),
                           le64toh(f->header->data_hash_table_size),
                           header_size, arena_size, tail_object_offset))
                return -ENODATA;

        if (!hash_is_valid(le64toh(f->header->field_hash_table_offset),
