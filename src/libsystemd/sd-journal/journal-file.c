...
static int journal_file_setup_trie_hash_table(JournalFile *f) {
        ...
        s = DEFAULT_TRIE_HASH_TABLE_SIZE;
        r = journal_file_append_object(f,
...
int journal_file_entry_item_next(
        ...
        if (JOURNAL_HEADER_COMPACT(f->header)) {
                ...
                if (*i >= (offset + offsetof(Object, entry.payload)) &&
                    *i < (offset + le64toh(e->object.size))) {
                        r = journal_file_entry_item_inline_payload(
                                f, e, offset, i, field, field_length, data_threshold, ret_data, ret_size);
                        if (r < 0)
                                return r;
                        ...
                        if (*i == offset + le64toh(e->object.size))
                                *i = UINT64_MAX;
                ...
                p = *i == 0 ? le32toh(e->entry.trie_offset) : *i;
...
uint64_t journal_file_entry_xor_hash(JournalFile *f, Object *o) {
        return JOURNAL_HEADER_COMPACT(f->header) ? le64toh(o->entry.xor_hash) :
                                                   le64toh(o->entry.xor_hash_compact);
}
                        ...
                        if (JOURNAL_FILE_COMPRESS(f) && inlined[i].iov_len >= f->compress_threshold_bytes) {
                                ...
                                if (compression > 0) {
                                        *p++ = compression;
                                        ...
                                        log_debug(
                                                "Compressed data object %" PRIu64 " -> %zu using %s",
                                                inlined[i].iov_len,
                                                rsize,
                                                object_compressed_to_string(compression));
                                } else
        ...
        for (size_t i = 0; i < n_iovec; i++) {
                ...
                if (JOURNAL_HEADER_COMPACT(f->header)) {
                        ...
                        if (r > 0 && FLAGS_SET(o->object.flags, FIELD_INDEXED))
                                dedupped[n_dedupped].indexed = true;
                }
                ...
                if (JOURNAL_HEADER_COMPACT(f->header)) {
                        ...
                        dedupped[n_dedupped].indexed = r > 0 && FLAGS_SET(o->object.flags, FIELD_INDEXED);
                } else
                        dedupped[n_dedupped].indexed = true;
...
int journal_file_open(
        ...
        *f = (JournalFile) {
        ...
        if (f->compact && !f->keyed_hash)
                return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Enabling compact mode without keyed hashing is not supported");
...
int journal_file_copy_entry(JournalFile *from, JournalFile *to, Object *o, uint64_t p) {
        ...
        for (uint64_t i = 0;;) {
                ...
                if (JOURNAL_HEADER_COMPACT(to->header)) {
                        ...
                        dedupped[n_dedupped].indexed = false;
