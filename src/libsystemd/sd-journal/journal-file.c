...
#define DEFAULT_TRIE_HASH_TABLE_SIZE (2047ULL*sizeof(HashItem))
...
static int journal_file_check_object(JournalFile *f, uint64_t offset, Object *o) {
                ...
                if (sz < offsetof(HashTableObject, items) ||
                    ...
                    (sz - offsetof(HashTableObject, items)) / sizeof(HashItem) <= 0)
                        return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
                                               ...
                                               o->object.type == OBJECT_FIELD_HASH_TABLE ? "field" : "trie",
                                               sz, offset);
        ...
        case OBJECT_ENTRY_ARRAY: {
                ...
                if (sz < offsetof(EntryArrayObject, items) ||
                    (sz - offsetof(EntryArrayObject, items)) % item_sz != 0 ||
                    (sz - offsetof(EntryArrayObject, items)) / item_sz <= 0)
                        return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
...
static int journal_file_append_trie_node(
        ...
        o->trie_node.object_offset = htole64(object_offset);
...
int journal_file_entry_item_next(
        ...
        if (JOURNAL_HEADER_COMPACT(f->header)) {
                ...
                if (p == 0) {
                        p = le32toh(READ_NOW(e->entry.trie_offset));
                        if (p == 0 && sz > offsetof(Object, entry.payload))
                ...
                if (p >= (offset + offsetof(Object, entry.payload)) && p < (offset + sz)) {
                ...
                p = le64toh(o->trie_node.parent_offset);
                /* If we're done processing deduplicated items, start processing the inline items if there
                 * are any. */
                if (p == 0)
                        p = sz > offsetof(Object, entry.payload) ? offset + offsetof(Object, entry.payload) :
                                                                   UINT64_MAX;
...
static int journal_file_append_entry_internal(
        ...
        if (JOURNAL_HEADER_COMPACT(f->header)) {
                ...
                o->entry.boot_id_offset = f->header->boot_id_offset;
        } else {
...
int journal_file_append_entry(
        ...
        for (size_t i = 0; i < n_iovec; i++) {
                ...
                if (JOURNAL_HEADER_KEYED_HASH(f->header))
                        items[n_items].xor_hash = jenkins_hash64(iovec[i].iov_base, iovec[i].iov_len);
                else
                        items[n_items].xor_hash = le64toh(o->data.hash);
...
int journal_file_copy_entry(JournalFile *from, JournalFile *to, Object *o, uint64_t p) {
        ...
        for (uint64_t i = 0;;) {
                ...
                if (JOURNAL_HEADER_COMPACT(to->header)) {
                        ...
                        if (r > 0 && FLAGS_SET(f->object.flags, FIELD_UNIQUE)) {
                                struct iovec iovec = {
                ...
                if (JOURNAL_HEADER_KEYED_HASH(to->header))
                        ...
                        items[n_items].xor_hash = le64toh(u->data.hash);

                items[n_items].object_offset = h;
                items[n_items].hash = le64toh(u->data.hash);
                n_items++;
        }
