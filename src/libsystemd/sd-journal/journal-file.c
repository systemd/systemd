...
static int journal_file_entry_item_inline_next(
               JournalFile *f,
               Object *e,
               uint64_t offset,
               uint64_t *i,
        ...
        for (uint64_t isz, end = offset + sz; p < end; p += sizeof(uint8_t) + sizeof(uint32_t) + isz) {
                uint8_t *d;
                uint8_t flags;
                ...
                d = e->entry.payload + *i - offset - offsetof(Object, entry.payload);

                if (p + sizeof(uint8_t) + sizeof(uint32_t) >= end)
                        return -EBADMSG;
                ...
                if (p + sizeof(uint8_t) + sizeof(uint32_t) + isz > end)
                        return -EBADMSG;
                ...
                p += sizeof(uint8_t) + sizeof(uint32_t) + isz;
...
static int journal_file_entry_item_next_compact(
        ...
        if (p < (offset + offsetof(Object, entry.payload)) || p >= (offset + sz)) {
                for (; p != 0; p = le32toh(o->trie_node.parent_offset)) {
                        uint64_t data_p;

                        r = journal_file_move_to_object(f, OBJECT_TRIE_NODE, p, &o);
                        if (r == -EBADMSG) {
                                log_debug("Bad trie node at %"PRIu64", skipping remaining entry items.", p);
                                break;
                        }
                        if (r < 0)
                                return r;
                        ...
                        r = journal_file_data_payload(
                                ...
                                ret_size);
                        if (r == -EBADMSG) {
                                log_debug("Entry item %"PRIu64" data object is bad, skipping over it.", data_p);
                                continue;
                        }
                        if (r < 0)
                                return r;
                        ...
                        /* We've found a match, let's move the iterator to the next trie node offset. */
                        p = le32toh(o->trie_node.parent_offset);

                        /* If we've iterated all the trie nodes, set the iterator to the start of the inline
                         * entry items. */
                        if (p == 0)
        ...
        r = journal_file_entry_item_inline_next(
...
static int journal_file_entry_item_next_non_compact(
        ...
        for (; *i < (sz - offsetof(Object, entry.items)) / sizeof(EntryItem); (*i)++) {
                uint64_t p;

                p = le64toh(e->entry.items[*i].object_offset);

                r = journal_file_data_payload(
                        f, NULL, p, field, field_length, data_threshold, ret_data, ret_size);
                if (r == -EBADMSG) {
                        log_debug("Entry item %"PRIu64" data object is bad, skipping over it.", p);
                        continue;
                }
                if (r < 0)
                        return r;
