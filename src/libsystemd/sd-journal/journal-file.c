...
int journal_file_entry_item_next(
        ...
        if (JOURNAL_HEADER_COMPACT(f->header)) {
                ...
                data_p = le32toh(o->trie_node.object_offset);
...
static int append_field_from_data(
                JournalFile *f,
                const char *data,
                size_t size,
                Object **ret,
...
int journal_file_append_entry(
        ...
        for (size_t i = 0; i < n_iovec; i++) {
                ...
                r = append_field_from_data(f, iovec[i].iov_base, iovec[i].iov_len, &fo, NULL);
                if (r < 0)
                        return r;
                ...
                items[n_items].indexed = !JOURNAL_HEADER_COMPACT(f->header) || FLAGS_SET(fo->object.flags, FIELD_INDEXED);
...
int journal_file_copy_entry(JournalFile *from, JournalFile *to, Object *o, uint64_t p) {
        ...
        for (uint64_t i = 0;;) {
                ...
                r = append_field_from_data(to, data, l, &fo, NULL);
                if (r < 0)
                        goto finish;
                ...
                items[n_items].indexed = !JOURNAL_HEADER_COMPACT(to->header) || FLAGS_SET(fo->object.flags, FIELD_INDEXED);
                n_items++;
        }
