...
static int verify_data(
        ...
        while (i < n) {
                ...
                for (j = 0; i < n && j < m; i++, j++) {
                        ...
                        if (q <= last) {
                                error(p, "Data object's entry array not sorted (%lu <= %lu)", q, last);
                                return -EBADMSG;
                        }
                        last = q;
...
static int verify_hash_table(
        ...
        for (i = 0; i < n; i++) {
                ...
                if (last != le64toh(f->data_hash_table[i].tail_hash_offset)) {
                        error(p,
                              "Tail hash pointer mismatch in hash table (%lu != %lu)",
                              last,
...
int journal_file_verify(
        ...
        for (;;) {
                ...
                if (p > le64toh(f->header->tail_object_offset)) {
                        error(offsetof(Header, tail_object_offset),
                              "Invalid tail object pointer (%lu > %lu)",
                              p,
                ...
                if (!!(o->object.flags & OBJECT_COMPRESSED_XZ) +
                    ...
                    !!(o->object.flags & OBJECT_COMPRESSED_ZSTD) > 1) {
                        error(p, "Object has multiple compression flags set (flags: %u)", o->object.flags);
                        r = -EINVAL;
                        goto fail;
                }
                ...
                case OBJECT_ENTRY:
                        ...
                        if (le64toh(o->entry.realtime) < last_tag_realtime) {
                                error(p,
                                      "Older entry after newer tag (%lu < %lu)",
                                      le64toh(o->entry.realtime),
                                      last_tag_realtime);
                                r = -EBADMSG;
                                goto fail;
                        ...
                        if (!entry_seqnum_set &&
                            le64toh(o->entry.seqnum) != le64toh(f->header->head_entry_seqnum)) {
                                error(p,
                                      "Head entry sequence number incorrect (%lu != %lu)",
                                      le64toh(o->entry.seqnum),
                        ...
                        if (entry_seqnum_set &&
                            entry_seqnum >= le64toh(o->entry.seqnum)) {
                                error(p,
                                      "Entry sequence number out of synchronization (%lu >= %lu)",
                                      entry_seqnum,
                                      le64toh(o->entry.seqnum));
                                r = -EBADMSG;
                                goto fail;
                        ...
                        if (entry_monotonic_set &&
                            ...
                            entry_monotonic > le64toh(o->entry.monotonic)) {
                                error(p,
                                      "Entry timestamp out of synchronization (%lu > %lu)",
                                      entry_monotonic,
                                      le64toh(o->entry.monotonic));
                                r = -EBADMSG;
                                goto fail;
                        ...
                        if (!entry_realtime_set &&
                            le64toh(o->entry.realtime) != le64toh(f->header->head_entry_realtime)) {
                                error(p,
                                      "Head entry realtime timestamp incorrect (%lu != %lu)",
                                      le64toh(o->entry.realtime),
                ...
                case OBJECT_DATA_HASH_TABLE:
                        if (n_data_hash_tables > 1) {
                                error(p, "More than one data hash table: %lu", n_data_hash_tables);
                                r = -EBADMSG;
                                goto fail;
                        }
                        ...
                        if (le64toh(f->header->data_hash_table_offset) != p + offsetof(HashTableObject, items)) {
                                error(p,
                                      "Header offset for data hash table invalid (%lu != %lu)",
                        ...
                        if (le64toh(f->header->data_hash_table_size) != le64toh(o->object.size) - offsetof(HashTableObject, items)) {
                                error(p,
                                      "Header size for data hash table invalid (%lu != %lu)",
                ...
                case OBJECT_FIELD_HASH_TABLE:
                        if (n_field_hash_tables > 1) {
                                error(p, "More than one field hash table: %lu", n_field_hash_tables);
                                r = -EBADMSG;
                                goto fail;
                        }
                        ...
                        if (le64toh(f->header->field_hash_table_offset) != p + offsetof(HashTableObject, items)) {
                                error(p,
                                      "Header offset for field hash table invalid (%lu != %lu)",
                        ...
                        if (le64toh(f->header->field_hash_table_size) != le64toh(o->object.size) - offsetof(HashTableObject, items)) {
                                error(p,
                                      "Header size for field hash table invalid (%lu != %lu)",
                ...
                case OBJECT_TAG:
                        ...
                        if (le64toh(o->tag.seqnum) != n_tags + 1) {
                                error(p,
                                      "Tag sequence number out of synchronization (%lu != %lu)",
                        ...
                        if (le64toh(o->tag.epoch) < last_epoch) {
                                error(p,
                                      "Epoch sequence out of synchronization (%lu < %lu)",
                                      le64toh(o->tag.epoch),
                                      last_epoch);
                                r = -EBADMSG;
                                goto fail;
                        ...
                        if (f->seal) {
                                ...
                                if (entry_realtime_set && entry_realtime >= rt + f->fss_interval_usec) {
                                        error(p,
                                              "tag/entry realtime timestamp out of synchronization (%lu >= %lu)",
                                              entry_realtime,
        ...
        if (JOURNAL_HEADER_CONTAINS(f->header, n_entry_arrays) &&
            n_entry_arrays != le64toh(f->header->n_entry_arrays)) {
                error(offsetof(Header, n_entry_arrays),
                      "Entry array number mismatch (%lu != %lu)",
                      n_entry_arrays,
        ...
        if (entry_seqnum_set &&
            entry_seqnum != le64toh(f->header->tail_entry_seqnum)) {
                error(offsetof(Header, tail_entry_seqnum),
                      "Tail entry sequence number incorrect (%lu != %lu)",
                      entry_seqnum,
        ...
        if (entry_monotonic_set &&
            (sd_id128_equal(entry_boot_id, f->header->boot_id) &&
             entry_monotonic != le64toh(f->header->tail_entry_monotonic))) {
                error(0,
                      "Invalid tail monotonic timestamp (%lu != %lu)",
                      entry_monotonic,
        ...
        if (entry_realtime_set && entry_realtime != le64toh(f->header->tail_entry_realtime)) {
                error(0,
                      "Invalid tail realtime timestamp (%lu != %lu)",
                      entry_realtime,
