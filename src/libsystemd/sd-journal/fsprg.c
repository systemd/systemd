...
static void det_randomize(void *buf, size_t buflen, const void *seed, size_t seedlen, uint32_t idx) {
        ...
        if (gcry_err_code(err) != GPG_ERR_NO_ERROR) /* This shouldn't happen */
                log_debug("gcry_md_open() failed with error code: %d", gcry_err_code(err));
        ...
        for (ctr = 0; buflen; ctr++) {
                ...
                if (gcry_err_code(err) != GPG_ERR_NO_ERROR) /* This shouldn't happen */
                        log_debug("gcry_md_open() failed with error code: %d", gcry_err_code(err));
                assert(gcry_err_code(err) == GPG_ERR_NO_ERROR); /* This shouldn't really fail */
