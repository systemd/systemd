...
static int journal_file_entry_get_machine_id(JournalFile *f, Object *o, sd_id128_t *ret) {
        ...
        for (uint64_t i = 0; i < n; i++) {
                ...
                r = journal_file_data_payload(f, NULL, p, "_MACHINE_ID", STRLEN("_MACHINE_ID"),
                                              SIZE_MAX, &d, &l);
                if (r == 0)
                        continue;
                ...
                if (l < STRLEN("_MACHINE_ID=") + 32)
                        return -EBADMSG;

                /* The data payload is not null-terminated, copy the hex ID to a local buffer. */
                char id_string[SD_ID128_STRING_MAX] = {};
                memcpy(id_string, (const char*) d + STRLEN("_MACHINE_ID="), 32);

                return sd_id128_from_string(id_string, ret);
        }
...
static int journal_file_read_tail_timestamp(sd_journal *j, JournalFile *f) {
        ...
        if (o && o->object.type == OBJECT_ENTRY) {
                sd_id128_t entry_mid;

                r = journal_file_entry_get_machine_id(f, o, &entry_mid);
                if (r >= 0)
                        mid = entry_mid;
                else if (r != -ENOENT)
