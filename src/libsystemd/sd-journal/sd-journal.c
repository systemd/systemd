...
_public_ int sd_journal_get_data(sd_journal *j, const char *field, const void **data, size_t *size) {
        ...
        for (i = 0; i < n; i++) {
                ...
                if (r == -EBADMSG) {
                        log_debug("Entry item %lu data object is bad, skipping over it.", i);
                        continue;
                }
                if (r < 0)
                        return r;
                ...
                if (le_hash != d->data.hash) {
                        log_debug("Entry item %lu hash is bad, skipping over it.", i);
                        continue;
                }
...
_public_ int sd_journal_enumerate_data(sd_journal *j, const void **data, size_t *size) {
        ...
        for (uint64_t n = journal_file_entry_n_items(o); j->current_field < n; j->current_field++) {
                ...
                if (r == -EBADMSG) {
                        log_debug("Entry item %lu data object is bad, skipping over it.", j->current_field);
                        continue;
                }
                if (r < 0)
                        return r;
                ...
                if (le_hash != o->data.hash) {
                        log_debug("Entry item %lu hash is bad, skipping over it.", j->current_field);
                        continue;
                }

                r = return_data(j, f, o, data, size);
                if (r == -EBADMSG) {
                        log_debug("Entry item %lu data payload is bad, skipping over it.", j->current_field);
                        continue;
                }
                if (r < 0)
                        return r;
