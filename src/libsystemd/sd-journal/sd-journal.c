...
static int move_by_boot_for_data(
        ...
        for (;;) {
                ...
                if (r < 0)
                        return r;
                if (r == 0 && found_boot)
                        return -EBADMSG; /* Huh?? We found the boot previously... */
                found_boot = true;

                /* Then, move to the first entry of the next boot (or the last entry of the previous boot with DRECTION_UP). */
                Object *entry;
                r = journal_file_next_entry(f, p, direction, &entry, NULL);
                if (r <= 0) /* r == 0 means that no next (or previous) boot found. That is, HEAD or TAIL now. */
                        return r;
                ...
                r = journal_file_move_to_entry_by_monotonic_for_data(f, data, boot_id,
                                                                     ...
                                                                     ret, offset);
                if (r != 0) /* Here r == 0 is OK, as that means the boot contains no entry matching matching with the data. */
                        return r;
        }
}
...
static int find_location_with_matches(
        ...
        if (j->current_location.monotonic_set) {
                ...
                r = journal_file_move_to_entry_by_monotonic(f, j->current_location.boot_id,
                                                            ...
                                                            direction == DIRECTION_DOWN ? DIRECTION_UP : DIRECTION_DOWN,
                                                            NULL, &p);
                if (r < 0) /* Here r == 0 is OK, as that means this file contains no entry for the boot. */
                        return r;
