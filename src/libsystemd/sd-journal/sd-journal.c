...
static int real_journal_next(sd_journal *j, direction_t direction) {
        ...
        for (;;) {
                ...
                /* Read the clock once per sweep: next_beyond_location() rate-limits its tail
                 * timestamp refresh, which would otherwise be one clock read per file for every
                 * entry. Sampling it here rather than outside the loop keeps a retry sweep, which
                 * we enter after dropping a file below, from reusing a stale timestamp. */
