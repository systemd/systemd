...
static int real_journal_next(sd_journal *j, direction_t direction) {
        ...
        if (new_file &&
            ...
            !sd_id128_equal(new_file->header->seqnum_id, j->current_location.seqnum_id)) {
                ...
                /* Refresh the file cache snapshot as the first loop may have called remove_file_real() on
                 * failing files, freeing them while leaving dangling pointers in the old snapshot. */
                r = iterated_cache_get(j->files_cache, NULL, &files, &n_files);
                if (r < 0)
                        return r;
