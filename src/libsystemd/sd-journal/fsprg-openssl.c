...
static bool secpar_is_valid(uint32_t secpar) {
        return
                secpar % 16 == 0 &&
                secpar >= 16 &&
                secpar <= 16384;
...
static BIGNUM* mpi_import(const struct iovec *iov) {
        assert(iovec_is_set(iov));

        /* Allocate a new BIGNUM. */
        return sym_BN_bin2bn(iov->iov_base, iov->iov_len, /* ret= */ NULL);
}
...
static int generate_prime3mod4(
        ...
        size_t buflen = secpar / 2 / 8;
        uint8_t *buf = newa(uint8_t, buflen);
        ...
        _cleanup_(BN_freep) BIGNUM *p = mpi_import(&iov);
        if (!p)
                return -ENOMEM;
...
static int generate_keys(
        ...
        if (!ret_p && !ret_q && !ret_n)
                return 0;

        _cleanup_(BN_freep) BIGNUM *p = NULL;
        r = generate_prime3mod4(secpar, seed, RND_GEN_P, bn_ctx, &p);
        if (r < 0)
                return r;

        _cleanup_(BN_freep) BIGNUM *q = NULL;
        r = generate_prime3mod4(secpar, seed, RND_GEN_Q, bn_ctx, &q);
        if (r < 0)
                return r;

        _cleanup_(BN_freep) BIGNUM *n = sym_BN_new();
        if (!n)
                return -ENOMEM;
...
static int generate_square(
        ...
        buf[0] &= 0x7f; /* Clear the upper bit so that we are likely to have x < n. */

        _cleanup_(BN_freep) BIGNUM *x = mpi_import(&iov);
        if (!x)
                return -ENOMEM;
...
static int compute_two_pow_mod_phi(
        ...
        _cleanup_(BN_freep) BIGNUM *phi = sym_BN_dup(p);
        if (!phi)
                return -ENOMEM;
        ...
        _cleanup_(BN_freep) BIGNUM *x = sym_BN_new();
        if (!x)
                return -ENOMEM;
...
static int crt_decompose(
        ...
        _cleanup_(BN_freep) BIGNUM *xp = sym_BN_new();
        if (!xp)
                return -ENOMEM;

        _cleanup_(BN_freep) BIGNUM *xq = sym_BN_new();
        if (!xq)
                return -ENOMEM;
...
static int crt_compose(
        ...
        /* Compute p^-1 mod q. */
        _cleanup_(BN_freep) BIGNUM *p_inv = sym_BN_mod_inverse(/* ret= */ NULL, p, q, bn_ctx);
        if (!p_inv)
                return -EIO;
...
static int parse_state(
        ...
        _cleanup_(BN_freep) BIGNUM *modulus = NULL;
        if (ret_modulus) {
                modulus = mpi_import(&IOVEC_MAKE((uint8_t*) state->iov_base + sizeof(uint16_t), secpar / 8));
                if (!modulus)
                        return -ENOMEM;
        }

        _cleanup_(BN_freep) BIGNUM *current = NULL;
        if (ret_current) {
                current = mpi_import(&IOVEC_MAKE((uint8_t*) state->iov_base + sizeof(uint16_t) + secpar / 8, secpar / 8));
                if (!current)
                        return -ENOMEM;
        }
...
int fsprg_generate_state(
        ...
        _cleanup_(BN_CTX_freep) BN_CTX *bn_ctx = sym_BN_CTX_new();
        if (!bn_ctx)
                return -ENOMEM;

        _cleanup_(BN_freep) BIGNUM *p = NULL, *q = NULL, *n = NULL;
        r = generate_keys(secpar, seed, bn_ctx, &p, &q, &n);
        if (r < 0)
                return r;

        _cleanup_(BN_freep) BIGNUM *x = NULL;
