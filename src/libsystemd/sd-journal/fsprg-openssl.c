...
size_t fsprg_state_size(uint16_t secpar) {
        assert(fsprg_secpar_is_valid(secpar));
...
static int generate_prime3mod4(
        ...
        CLEANUP_ERASE_PTR(&buf, buflen);

        r = det_randomize(seed, idx, &IOVEC_MAKE(buf, buflen));
        if (r < 0)
                return r;
        ...
        r = mpi_import(&IOVEC_MAKE(buf, buflen), &p);
        if (r < 0)
                return r;
...
static int generate_square(
        ...
        r = mpi_import(&IOVEC_MAKE(buf, buflen), &x);
        if (r < 0)
                return r;
