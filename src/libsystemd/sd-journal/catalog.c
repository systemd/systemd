...
static int open_mmap(const char *database, int *ret_fd, struct stat *ret_st, void **ret_map) {
        ...
        if (memcmp(h->signature, (const uint8_t[]) CATALOG_SIGNATURE, sizeof(h->signature)) != 0 ||
            ...
            le64toh(h->n_items) <= 0 ||
            !MUL_SAFE(&items_size, le64toh(h->catalog_item_size), le64toh(h->n_items)) ||
            !ADD_SAFE(&total, le64toh(h->header_size), items_size) ||
...
static const char* find_id(const void *p, uint64_t file_size, sd_id128_t id) {
        ...
        if (!MUL_SAFE(&off, le64toh(h->n_items), le64toh(h->catalog_item_size)) ||
            !ADD_SAFE(&off, off, le64toh(h->header_size)) ||
            !ADD_SAFE(&off, off, le64toh(f->offset)) ||
            off >= file_size)
                return NULL;
