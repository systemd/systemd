...
int compress_stream_lz4(int fdf, int fdt, uint64_t max_bytes) {
        ...
        size_t in_allocsize, out_allocsize, n, total_in = 0, total_out, offset = 0, frame_size;
        int r;
        ...
        in_allocsize = frame_size; // TODO: what's the optimal size?
        ...
        for (;;) {
                ...
                k = loop_read(fdf, in_buff, in_allocsize, true);
                if (k < 0)
                        return k;
                if (k == 0)
                        break;
        ...
        n = LZ4F_compressEnd(ctx, out_buff + offset, out_allocsize - offset, &options);
        if (LZ4F_isError(n))
                return -ENOTRECOVERABLE;
