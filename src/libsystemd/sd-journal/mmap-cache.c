...
static int add_mmap(
        ...
        /* PAGE_ALIGN() and friends used below take size_t. If we pass too large value these may provide
         * an unexpected result. */
        if (offset > (uint64_t) SIZE_MAX)
                return -EADDRNOTAVAIL;

        /* overflow check */
        if (size > SIZE_MAX - PAGE_OFFSET(offset))
                return -EADDRNOTAVAIL;

        size = PAGE_ALIGN(size + PAGE_OFFSET(offset));
