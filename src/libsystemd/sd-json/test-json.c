 ...
 * test_float() exercises only INFINITY. Pin that invariant here at compile time. Use __builtin_isinf
 * rather than == so we don't trip -Wfloat-equal. */
assert_cc(__builtin_isinf(__builtin_huge_val()) > 0);
