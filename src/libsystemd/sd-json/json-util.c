...
int json_variant_new_pidref(sd_json_variant **ret, PidRef *pidref) {
         ...
         * identify the process globally, and be good enough to turn back into a PidRef */
...
int json_dispatch_pidref(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
         ...
         * above. If SD_JSON_STRICT is set will acquire a pidfd for the process, and validate that the
         * auxiliary fields match it. Otherwise, will only do a */
        ...
        if (sd_json_variant_is_integer(variant))
                ...
                if (r < 0)
                        return json_log(variant, flags, SYNTHETIC_ERRNO(EINVAL), "JSON field '%s' is not numeric PID", strna(name));
                ...
                return json_log(variant, flags, SYNTHETIC_ERRNO(EINVAL), "JSON field '%s' is not numeric PID or a PID structure.", strna(name));
