...
typedef struct JsonStream JsonStream;
...
int json_stream_new(JsonStream **ret, const JsonStreamParams *params);
JsonStream* json_stream_free(JsonStream *s);
DEFINE_TRIVIAL_CLEANUP_FUNC(JsonStream*, json_stream_free);
...
/* fd ownership */
void json_stream_attach_fds(JsonStream *s, int input_fd, int output_fd);
...
/* Release both fds back to the caller. Always populates *ret_input_fd and *ret_output_fd
 * with the previous fd values (which may be -EBADF) and resets the internal state to
 * -EBADF without closing. The caller then owns the fds. */
void json_stream_take_fds(JsonStream *s, int *ret_input_fd, int *ret_output_fd);
int json_stream_get_input_fd(const JsonStream *s);
int json_stream_get_output_fd(const JsonStream *s);

/* When set, the read path uses MSG_PEEK (or byte-by-byte for non-sockets) to stop at the
 * next message delimiter and avoid consuming bytes past it. Use this whenever the connection
 * may be handed off to a different protocol after the next message — the consumer is
 * responsible for setting and clearing the flag in line with its own protocol semantics. */
void json_stream_set_bounded_reads(JsonStream *s, bool b);
...
void json_stream_set_input_sensitive(JsonStream *s, bool b);
bool json_stream_get_input_sensitive(const JsonStream *s);

bool json_stream_get_allow_fd_passing_input(const JsonStream *s);
bool json_stream_get_allow_fd_passing_output(const JsonStream *s);
