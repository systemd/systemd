 ...
 *
 * Non-static (despite being an internal helper) only so test-json-stream-delimiterless
 * can exercise it directly without pulling in the rest of the JsonStream state machine. */
int json_stream_find_message_end(
                const char *buf,
                size_t size,
                size_t *ret_consumed) {
