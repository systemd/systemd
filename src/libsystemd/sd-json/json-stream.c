...
int json_stream_new(JsonStream **ret, const JsonStreamParams *params) {
        ...
        *s = (JsonStream) {
                .delimiter = params->delimiter, /* NULL → NUL-delimited (varlink) */
...
bool json_stream_flags_set(const JsonStream *s, JsonStreamFlags flags) {
        assert(s);
        assert(FLAGS_SET(JSON_STREAM_BOUNDED_READS|JSON_STREAM_INPUT_SENSITIVE|JSON_STREAM_ALLOW_FD_PASSING_INPUT|JSON_STREAM_ALLOW_FD_PASSING_OUTPUT, flags));
...
void json_stream_set_flags(JsonStream *s, JsonStreamFlags flags, bool b) {
        assert(s);
        assert(FLAGS_SET(JSON_STREAM_BOUNDED_READS|JSON_STREAM_INPUT_SENSITIVE, flags));
