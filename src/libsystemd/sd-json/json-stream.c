...
static int json_stream_format_queue(JsonStream *s) {
        ...
        while (s->output_queue) {
                 ...
                 * JSON into that same buffer: the single sendmsg() below would attach the fds
                 * to the combined bytes and break the message-to-fd boundary. Stop here and
                 * let json_stream_write() drain the buffer first; the next write() call will
                 * pull this item into a clean buffer.
                 *
