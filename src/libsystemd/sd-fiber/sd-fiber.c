...
int sd_fiber_new_full(
        ...
        *f = (sd_fiber) {
        ...
        f->context.uc_stack.ss_sp = (uint8_t*) f->stack + page_size();
        f->context.uc_stack.ss_size = f->stack_size - page_size();
        f->context.uc_link = &main_context;
        makecontext(&f->context, sd_fiber_entry_point, 0);
