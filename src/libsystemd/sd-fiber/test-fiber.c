...
TEST(fiber_priority_ascending) {
        ...
        for (size_t i = 0; i < 5; i++) {
                ...
                ASSERT_OK(sd_fiber_new_full(e, name, priority_fiber, &counter, i, &fibers[i]));
        }
...
TEST(fiber_priority_identical) {
        ...
        for (size_t i = 0; i < 5; i++) {
                _cleanup_free_ char *name = NULL;
                ASSERT_OK(asprintf(&name, "priority-%zu", i));
                ASSERT_OK(sd_fiber_new_full(e, name, priority_fiber, &counter, SD_FIBER_PRIORITY_DEFAULT, &fibers[i]));
        }
        ...
        for (size_t i = 0; i < 5; i++)
                ASSERT_EQ(sd_fiber_result(fibers[i]), (int) 5);
}
