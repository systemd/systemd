...
int sd_fiber_ppoll(struct pollfd *fds, size_t n_fds, const struct timespec *timeout, const sigset_t *sigmask) {
        ...
        /* If every fd was skipped (negative or empty event mask) and no timeout was given, there's
         * nothing that could ever wake the fiber up — same situation as n_fds == 0 && !timeout, just
         * not detectable upfront. Refuse rather than suspend forever. */
        if (n_io_futures == 0 && !timeout)
                return -EINVAL;
