...
int sd_future_group_add(sd_future *f, sd_future *child) {
        ...
        /* If the child was already resolved when added, sd_future_add_callback's inline
         * dispatch fired group_child_resolved → future_group_check above, but n_slots had
         * not yet been bumped so the check iterated stale state. Re-evaluate now that the
         * slot is in place. */
        return future_group_check(f);
}
