...
TEST(future_group_add_rejected_during_finalize) {
        ...
        ASSERT_ERROR(sd_future_group_add_many(group, new_child), ESTALE);
        size_t size;
        ASSERT_OK(sd_future_group_size(group, &size));
        ASSERT_EQ(size, 1U);
...
static int parent_await_driver(void *userdata) {
        ...
        if (s->await)
                ASSERT_ERROR(sd_future_group_await(s->group), ECANCELED);
        else
...
TEST(future_group_parent_await_scope) {
        ...
        FOREACH_ARGUMENT(await, false, true) {
                ...
                if (await)
                        ...
                        ASSERT_ERROR(ASSERT_RETURN_EXPECTED(sd_future_group_await(s.group)), ESRCH);
...
static int peer_await_driver(void *userdata) {
        return sd_future_group_await(userdata);
}
...
TEST(future_group_priority) {
        ...
        ASSERT_OK(sd_future_group_add_many(group, resolved, unsupported, pending));
         ...
         * and children without priority support must not make the group operation fail. */
        ASSERT_OK_POSITIVE(sd_event_run(e, 0));
