...
static int dispatch_slot(sd_future_slot *s) {
         ...
         * invoking it, since the callback may free the slot or itself request exit. */
        RET_GATHER(r, sd_event_source_set_enabled(s->defer_source, SD_EVENT_OFF));
        RET_GATHER(r, sd_event_source_set_enabled(s->exit_source, SD_EVENT_OFF));
        if (r < 0)
                return r;
...
int sd_future_resume_callback(sd_future *f, void *userdata) {
        /* Explicit result delivery, used by timeout scopes to interrupt the fiber with -ETIME.
         * Ordinary waits use a separate callback that only signals completion with 0. */
...
static int fiber_wait(sd_future *target, int *ret_result) {
        ...
        /* Waiting during exit is supported by the slot's exit source. A finished loop can no longer
...
int sd_future_new_defer(sd_event *e, int result, sd_future **ret) {
        ...
        return future_new_defer(e, result, ret);
}
