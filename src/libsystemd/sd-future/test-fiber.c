...
static int await_invalid_fiber(void *userdata) {
        sd_future *other = ASSERT_PTR(userdata);
        ...
        ASSERT_NULL(ASSERT_RETURN_EXPECTED(sd_future_cancel_wait_unref(sd_fiber_get_current())));
        ASSERT_NULL(ASSERT_RETURN_EXPECTED(sd_future_cancel_wait_unref(other)));
...
TEST(fiber_await_invalid) {
        ...
        ASSERT_OK(sd_fiber_new(e, "await-invalid", await_invalid_fiber, target, /* destroy= */ NULL, &waiter));
        ASSERT_OK(sd_event_set_exit_on_idle(e, true));
        ASSERT_OK(sd_event_loop(e));
