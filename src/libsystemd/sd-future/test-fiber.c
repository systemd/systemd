...
TEST(fiber_multiple_yield) {
        ...
        ASSERT_NOT_NULL(fibers = new(sd_future*, 5));
...
TEST(fiber_cancel_one_of_many) {
        ...
        ASSERT_NOT_NULL(fibers = new(sd_future*, 3));
...
TEST(fiber_wait_for_multiple_waiters) {
        ...
        ASSERT_NOT_NULL(waiters = new(sd_future*, 3));
        for (size_t i = 0; i < 3; i++)
...
TEST(fiber_waitgroup_basic) {
        ...
        ASSERT_NOT_NULL(fibers = new(sd_future*, 3));
        for (size_t i = 0; i < 3; i++) {
                ASSERT_OK(sd_future_new_fiber(e, "success", success_fiber, INT_TO_PTR(10 + i), /* destroy= */ NULL, &fibers[i]));
...
TEST(fiber_waitgroup_with_failure) {
        ...
        /* First two succeed */
        ASSERT_OK(sd_future_new_fiber(e, "success", success_fiber, INT_TO_PTR(1), /* destroy= */ NULL, &fibers[0]));
        ASSERT_OK(sd_fiber_waitgroup_add(wg, fibers[0]));

        /* Third fails */
        ASSERT_OK(sd_future_new_fiber(e, "failing", failing_fiber, NULL, /* destroy= */ NULL, &fibers[1]));
        ASSERT_OK(sd_fiber_waitgroup_add(wg, fibers[1]));

        /* Fourth succeeds */
        ASSERT_OK(sd_future_new_fiber(e, "success", success_fiber, INT_TO_PTR(3), /* destroy= */ NULL, &fibers[2]));
        ASSERT_OK(sd_fiber_waitgroup_add(wg, fibers[2]));

        ASSERT_OK(sd_event_loop(e));

        /* Check individual results */
        ASSERT_OK_EQ(sd_future_result(fibers[0]), 1);
        ASSERT_ERROR(sd_future_result(fibers[1]), ENOENT);
        ASSERT_OK_EQ(sd_future_result(fibers[2]), 3);

        /* Waitgroup check should return the first error and the failed fiber */
        _cleanup_(sd_future_unrefp) sd_future *failed = NULL;
...
TEST(fiber_waitgroup_wait) {
        ...
        _cleanup_(sd_future_unrefp) sd_future *waiter = NULL;
        ASSERT_OK(sd_future_new_fiber(e, "waitgroup-waiter", waitgroup_wait_fiber, wg, /* destroy= */ NULL, &waiter));
        ASSERT_OK(sd_future_set_priority(waiter, -1));
...
TEST(fiber_waitgroup_already_completed) {
        ...
        ASSERT_NOT_NULL(fibers = new(sd_future*, 3));
        for (size_t i = 0; i < 3; i++)
                ASSERT_OK(sd_future_new_fiber(e, "simple", simple_fiber, &values[i], /* destroy= */ NULL, &fibers[i]));
...
TEST(fiber_waitgroup_empty) {
        ...
        _cleanup_(sd_future_unrefp) sd_future *f = NULL;
        ASSERT_OK(sd_future_new_fiber(e, "empty-waitgroup", waitgroup_wait_fiber, wg, /* destroy= */ NULL, &f));
...
TEST(fiber_waitgroup_with_cancelled) {
        ...
        ASSERT_OK(sd_future_result(fibers[0]));
        ASSERT_OK(sd_future_result(fibers[2]));

        /* Second should be cancelled */
        ASSERT_ERROR(sd_future_result(fibers[1]), ECANCELED);
