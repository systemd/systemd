...
static int interrupt_count_fiber(void *userdata) {
        _cleanup_(sd_future_unrefp) sd_future *completion = NULL;
        uint64_t before = sd_fiber_interrupt_count();

        ASSERT_EQ(before, 0U);
        ASSERT_OK(sd_future_new_defer(sd_fiber_get_event(), -ECANCELED, &completion));
        ASSERT_ERROR(sd_fiber_await(completion), ECANCELED);
        ASSERT_EQ(sd_fiber_interrupt_count(), before);

        {
                SD_FIBER_TIMEOUT(0);
                ASSERT_ERROR(sd_fiber_suspend(), ETIME);
                ASSERT_EQ(sd_fiber_interrupt_count(), before + 1);
        }

        /* Tell the outer test that the next suspension is waiting for its cancellation. */
        bool *ready = ASSERT_PTR(userdata);
        *ready = true;
        ASSERT_ERROR(sd_fiber_suspend(), ECANCELED);
        ASSERT_EQ(sd_fiber_interrupt_count(), before + 2);
        return 0;
}

TEST(fiber_interrupt_count) {
        _cleanup_(sd_event_unrefp) sd_event *e = NULL;
        _cleanup_(sd_future_unrefp) sd_future *f = NULL;
        bool ready = false;

        ASSERT_EQ(sd_fiber_interrupt_count(), 0U);
        ASSERT_OK(sd_event_new(&e));
        ASSERT_OK(sd_fiber_new(e, "interrupt-count", interrupt_count_fiber, &ready,
                               /* destroy= */ NULL, &f));
        while (!ready)
                ASSERT_OK_POSITIVE(sd_event_run(e, UINT64_MAX));

        ASSERT_OK(sd_future_cancel(f));
        ASSERT_OK(sd_future_cancel(f)); /* A cancellation already queued is idempotent. */
        ASSERT_OK(sd_event_set_exit_on_idle(e, true));
        ASSERT_OK(sd_event_loop(e));
        ASSERT_OK_ZERO(sd_future_result(f));
        ASSERT_EQ(sd_fiber_interrupt_count(), 0U);
