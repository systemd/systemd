...
static void fiber_sigaltstack_trampoline(int sig) {
        fiber_tr_called = 1;
        if (sigsetjmp(fiber_tr_reenter, /* savemask= */ 0) == 0)
...
static int fiber_init_sigaltstack(Fiber *f) {
        ...
        if (sigsetjmp(f->resume_context, 0) == 0) {
                start_switch_stack(&fake_stack_save, &fiber_stack);
                siglongjmp_unchecked(fiber_tr_reenter, 1);
        }
...
static bool fiber_force_sigaltstack(void) {
        static int cached = -1;
        int r;

        if (cached < 0) {
                r = secure_getenv_bool("SYSTEMD_FIBER_FORCE_SIGALTSTACK");
                if (r < 0 && r != -ENXIO)
...
static int fiber_init(Fiber *f) {
        ...
        if (!getcontext || fiber_force_sigaltstack())
