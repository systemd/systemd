...
static int fiber_init_sigaltstack(Fiber *f) {
        ...
        _cleanup_(pthread_mutex_unlock_assertp) pthread_mutex_t *_l = pthread_mutex_lock_assert(&sigurg_mutex);
...
static int fiber_init(Fiber *f) {
         ...
         * where we fall back to the sigaltstack path (qemu's portable pattern from GNU Pth, originally
         * described in Engelschall, "Portable Multithreading", USENIX ATC 2000:
...
int sd_fiber_new(sd_event *e, const char *name, sd_fiber_func_t func, void *userdata, sd_fiber_destroy_t destroy, sd_future **ret) {
        ...
        *fiber = (Fiber) {
                .stack.iov_len = ROUND_UP(rl.rlim_cur, page_size()),
