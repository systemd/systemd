...
static bool oneway_defer_dispatched = false;
static bool oneway_ping_dispatched = false;
static bool oneway_defer_completed = false;
...
TEST(oneway_deferred_parks_connection) {
        ...
        _cleanup_(sd_varlink_server_unrefp) sd_varlink_server *s = NULL;
        ASSERT_OK(sd_varlink_server_new(&s, SD_VARLINK_SERVER_ONEWAY_NEEDS_REPLY));
...
static bool oneway_error_ping_dispatched = false;
static bool oneway_error_completed = false;
...
static int oneway_deferred_error_complete(sd_event_source *s, void *userdata) {
         ...
         * (no client listens for a oneway reply) but must still unpark the connection. Unlike
         * sd_varlink_reply(), sd_varlink_error() keeps returning its negative errno here, so a caller must
         * not propagate it as a failure. */
        ASSERT_TRUE(sd_varlink_error(link, "io.test.SomeError", /* parameters= */ NULL) < 0);
...
static bool oneway_errret_ping_dispatched = false;
...
static bool oneway_fd_ping_dispatched = false;
...
static bool oneway_fiber_ran = false;
static bool oneway_fiber_ping_dispatched = false;
...
static int method_oneway_fiber_defer(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
         ...
         * Setting a sentinel is a no-op for a oneway call and must not disturb this. */
        ASSERT_TRUE(FLAGS_SET(flags, SD_VARLINK_METHOD_ONEWAY));
        ASSERT_OK(sd_varlink_set_sentinel(link, "io.test.SentinelError"));
