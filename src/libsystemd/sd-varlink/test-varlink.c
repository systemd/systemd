...
static void oneway_test_setup(
        ...
        ASSERT_OK(sd_event_add_time_relative(e, &timer, CLOCK_MONOTONIC, 30 * USEC_PER_SEC, 0, oneway_test_timeout, NULL));
...
static int oneway_deferred_error_complete(sd_event_source *s, void *userdata) {
         ...
         * (no client listens for a oneway reply) but must still unpark the connection, returning success
         * like sd_varlink_reply() does. */
        ASSERT_OK_EQ(sd_varlink_error(link, "io.test.SomeError", /* parameters= */ NULL), 1);
