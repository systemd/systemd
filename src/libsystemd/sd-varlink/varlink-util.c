...
static int varlink_finish_idle(Set *s) {
        ...
        SET_FOREACH(vl, s) {
                ...
                if (r <= 0)
                        return r; /* there's one connection that is not idle, tell the caller immediately */
...
int varlink_execute_directory(
        ...
        FOREACH_ARRAY(dp, dentries->entries, dentries->n_entries) {
                ...
                if (set_size(links) >= VARLINK_EXECUTE_SOCKETS_MAX) {
                        log_debug("Too many sockets (%u) in directory, skipping '%s' and other sockets.", set_size(links), j);
                        break;
                }
