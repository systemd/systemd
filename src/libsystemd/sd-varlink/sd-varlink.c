...
static int generic_method_get_info(
        ...
        r = sd_json_variant_set_field_string(&ret, "vendor", link->server->vendor);
        if (r < 0)
                return r;

        r = sd_json_variant_set_field_string(&ret, "product", link->server->product);
        if (r < 0)
                return r;

        r = sd_json_variant_set_field_string(&ret, "version", link->server->version);
        if (r < 0)
                return r;

        r = sd_json_variant_set_field_string(&ret, "url", link->server->url);
        if (r < 0)
                return r;
        ...
        r = sd_json_variant_set_field_strv(&ret, "interfaces", interfaces);
        if (r < 0)
                return r;
...
_public_ int sd_varlink_server_set_info(
        ...
        assert_return(vendor, -EINVAL);
        assert_return(product, -EINVAL);
        assert_return(version, -EINVAL);
        assert_return(url, -EINVAL);
