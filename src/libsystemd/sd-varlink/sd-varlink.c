...
static int varlink_enqueue_json(sd_varlink *v, sd_json_variant *m) {
        ...
        return varlink_enqueue_item(v, q);
}
...
_public_ int sd_varlink_reply(sd_varlink *v, sd_json_variant *parameters) {
        ...
        if (more && v->sentinel) {
                ...
                r = sd_json_variant_set_fieldb(&v->previous->data, "continues", SD_JSON_BUILD_BOOLEAN(true));
                if (r < 0)
                        return r;
...
_public_ int sd_varlink_notify(sd_varlink *v, sd_json_variant *parameters) {
        ...
        if (v->sentinel)
                return varlink_log_errno(v, SYNTHETIC_ERRNO(EINVAL), "Cannot use sd_varlink_notify() on connection with sentinel set");
