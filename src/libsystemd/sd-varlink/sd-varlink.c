...
static int varlink_fiber_entry(void *userdata) {
        ...
        if (r < 0 && IN_SET(v->state, VARLINK_PROCESSING_METHOD_ONEWAY, VARLINK_PENDING_METHOD_ONEWAY))
...
_public_ int sd_varlink_error(sd_varlink *v, const char *error_id, sd_json_variant *parameters) {
         ...
         * machine so a deferred oneway method's completion lets the connection proceed. Return success like
         * sd_varlink_reply() does, so completing a parked oneway call is uniformly detectable. */
        if (varlink_oneway_complete(v))
                return 1;
