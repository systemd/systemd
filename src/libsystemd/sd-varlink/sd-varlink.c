...
static JsonStreamPhase varlink_phase(void *userdata) {
        ...
        if (IN_SET(v->state, VARLINK_PENDING_METHOD, VARLINK_PENDING_METHOD_MORE, VARLINK_PENDING_METHOD_UPGRADE, VARLINK_PENDING_METHOD_ONEWAY))
...
static int varlink_fiber_entry(void *userdata) {
         ...
         * the connection can dispatch the next queued method. */
...
static int varlink_dispatch_method(sd_varlink *v) {
        ...
        /* Set when a oneway method handler ran and returned without replying: the call is then treated as
         * deferred (parked) and must produce a (discarded) reply later to release the connection. */
        ...
        if (callback) {
                ...
                if (!invalid) {
                        ...
                        if (VARLINK_STATE_WANTS_REPLY(v->state)) {
                                 ...
                                 * without replying is treated as deferred: park it below and wait for it to
                                 * signal completion via sd_varlink_reply()/sd_varlink_error() later. */
                                if (r >= 0 && FLAGS_SET(v->server->flags, SD_VARLINK_SERVER_ONEWAY_NEEDS_REPLY))
        ...
        case VARLINK_PROCESSING_METHOD_ONEWAY:
                /* A oneway call never sends anything to the client. In SD_VARLINK_SERVER_ONEWAY_NEEDS_REPLY
                 * mode a handler that returned success without replying is deferred, so park it like a
                 * regular deferred method. Otherwise (default mode, a failed handler, an unknown method or
                 * invalid parameters) there is nothing more to do, so complete it right away. */
                if (oneway_deferred) {
                        varlink_set_state(v, VARLINK_PENDING_METHOD_ONEWAY);
                        break;
                }

                varlink_oneway_complete(v); /* discards any pushed fds, moves us to VARLINK_PROCESSED_METHOD */
                _fallthrough_;
...
static int varlink_dispatch_upgrade(sd_varlink *v) {
        ...
        /* Fail if by this time the caller still hasn't set an upgrade callback. */
...
static int varlink_reply_internal(sd_varlink *v, sd_json_variant *parameters, bool skip_validation) {
        ...
        if (!IN_SET(v->state,
                    ...
                    VARLINK_PENDING_METHOD, VARLINK_PENDING_METHOD_MORE, VARLINK_PENDING_METHOD_UPGRADE, VARLINK_PENDING_METHOD_ONEWAY))
                return varlink_log_errno(v, SYNTHETIC_ERRNO(EBUSY), "Connection busy.");
...
_public_ int sd_varlink_error(sd_varlink *v, const char *error_id, sd_json_variant *parameters) {
         ...
         * machine so a deferred oneway method's completion lets the connection proceed. */
        if (varlink_oneway_complete(v))
...
_public_ int sd_varlink_set_sentinel(sd_varlink *v, const char *error_id) {
        ...
        /* If the caller doesn't want a reply, then don't set a sentinel. This also covers deferred oneway
         * methods (e.g. running in a fiber), which park in VARLINK_PENDING_METHOD_ONEWAY. */
        if (IN_SET(v->state, VARLINK_PROCESSING_METHOD_ONEWAY, VARLINK_PENDING_METHOD_ONEWAY))
                return 0;
