...
_public_ int sd_varlink_connect_exec(sd_varlink **ret, const char *_command, char **_argv) {
        ...
        json_stream_attach_fds(v->stream, conn_fd, conn_fd);
...
static int varlink_connect_ssh_exec(sd_varlink **ret, const char *where) {
        ...
        json_stream_attach_fds(v->stream, TAKE_FD(output_pipe[0]), TAKE_FD(input_pipe[1]));
        v->exec_pidref = TAKE_PIDREF(pidref);
        varlink_set_state(v, VARLINK_IDLE_CLIENT);
...
static void varlink_clear(sd_varlink *v) {
         ...
         * stream's fds, then free the stream — json_stream_free() closes the input/output
        ...
        v->stream = json_stream_free(v->stream);
...
static int varlink_parse_message(sd_varlink *v) {
        ...
        if (json_stream_get_input_sensitive(v->stream)) {
...
_public_ int sd_varlink_get_fd(sd_varlink *v) {
        ...
        int input_fd = json_stream_get_input_fd(v->stream);
        int output_fd = json_stream_get_output_fd(v->stream);
...
static int varlink_handle_upgrade_fds(sd_varlink *v, int *ret_input_fd, int *ret_output_fd) {
        ...
        json_stream_take_fds(v->stream, &input_fd, &output_fd);
...
_public_ int sd_varlink_call_and_upgrade(
        ...
        json_stream_set_bounded_reads(v->stream, true);
        r = varlink_call_internal(v, m);
        if (r < 0) {
                v->protocol_upgrade = false;
                json_stream_set_bounded_reads(v->stream, false);
                return r;
        }
...
_public_ int sd_varlink_push_fd(sd_varlink *v, int fd) {
        ...
        if (!json_stream_get_allow_fd_passing_output(v->stream))
                return -EPERM;
...
_public_ int sd_varlink_peek_fd(sd_varlink *v, size_t i) {
        ...
        if (!json_stream_get_allow_fd_passing_input(v->stream))
                return -EPERM;
...
_public_ int sd_varlink_set_input_sensitive(sd_varlink *v) {
        ...
        json_stream_set_input_sensitive(v->stream, true);
        return 0;
}
...
_public_ int sd_varlink_server_add_connection_pair(
        ...
        json_stream_attach_fds(v->stream, input_fd, output_fd);
        ...
        if (server->event) {
                ...
                if (r < 0) {
                        ...
                        int ifd, ofd;
                        json_stream_take_fds(v->stream, &ifd, &ofd);
                        sd_varlink_close(v);
                        return r;
                }
        }
