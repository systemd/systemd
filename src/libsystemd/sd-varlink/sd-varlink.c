...
static int varlink_dispatch_method(sd_varlink *v) {
        ...
        if (callback) {
                ...
                if (!v->current_method)
                        ...
                        if (IN_SET(r, -EBADE, -EPROTOTYPE)) {
                                varlink_log_errno(v, r, "Method %s() called without '%s' flag, but flag needs to be set.",
                                                  method, r == -EBADE ? "more" : "upgrade");

                                if (v->state == VARLINK_PROCESSING_METHOD) {
                                        r = sd_varlink_error(v, r == -EBADE ? SD_VARLINK_ERROR_EXPECTED_MORE : SD_VARLINK_ERROR_PROTOCOL, NULL);
...
static int varlink_handle_upgrade_fds(sd_varlink *v, int *ret_input_fd, int *ret_output_fd) {
         ...
         * byte-by-byte reading prevents this. On the server side, clients must not pipeline
         * raw data with the upgrade call. */
