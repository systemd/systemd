...
static int varlink_sanitize_outgoing_parameters(sd_json_variant **v) {
        assert(v);
...
_public_ int sd_varlink_get_current_parameters(sd_varlink *v, sd_json_variant **ret) {
        ...
        /* For backward compatibility, return empty object when parameters field is missing */
        if (!p) {
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *empty = NULL;
                int r = sd_json_variant_new_object(&empty, NULL, 0);
                if (r < 0)
                        return r;

                if (ret)
                        *ret = sd_json_variant_ref(empty);
        } else {
                if (ret)
...
_public_ int sd_varlink_send(sd_varlink *v, const char *method, sd_json_variant *parameters) {
        ...
        r = varlink_sanitize_outgoing_parameters(&parameters);
        if (r < 0)
