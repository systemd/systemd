...
static JsonStreamPhase varlink_phase(void *userdata) {
        ...
        if (IN_SET(v->state, VARLINK_PENDING_METHOD, VARLINK_PENDING_METHOD_MORE, VARLINK_PENDING_METHOD_ONEWAY))
...
static bool varlink_oneway_complete(sd_varlink *v) {
        ...
        if (v->state == VARLINK_PENDING_METHOD_ONEWAY) {
                 ...
                 * or fiber). Return to idle ourselves and re-arm deferred processing to pick up the next
                 * queued method. */
                varlink_clear_current(v);
                varlink_set_state(v, VARLINK_IDLE_SERVER);
...
static int varlink_dispatch_method(sd_varlink *v) {
        ...
        /* Complete any oneway method call that the handler did not explicitly defer. There is no reply to
         * send, but we still advance the state machine so the connection can proceed. */
        if (v->state == VARLINK_PROCESSING_METHOD_ONEWAY && !oneway_deferred)
        ...
        case VARLINK_PROCESSING_METHOD_ONEWAY: /* Deferred oneway call, completed (and discarded) later */
