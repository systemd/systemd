...
_public_ int sd_varlink_get_current_parameters(sd_varlink *v, sd_json_variant **ret) {
        ...
        /* For backward compatibility, return NULL when parameters field is missing (empty case) */
        if (ret)
                *ret = p ? sd_json_variant_ref(p) : NULL;
...
_public_ int sd_varlink_send(sd_varlink *v, const char *method, sd_json_variant *parameters) {
        ...
        r = sd_json_buildo(
                        ...
                        SD_JSON_BUILD_PAIR_CONDITION(!sd_json_variant_is_blank_object(parameters), "parameters", SD_JSON_BUILD_VARIANT(parameters)),
...
_public_ int sd_varlink_reply(sd_varlink *v, sd_json_variant *parameters) {
        ...
        r = sd_json_buildo(&m, SD_JSON_BUILD_PAIR_CONDITION(!sd_json_variant_is_blank_object(parameters), "parameters", SD_JSON_BUILD_VARIANT(parameters)));
        if (r < 0)
                return varlink_log_errno(v, r, "Failed to build json message: %m");
