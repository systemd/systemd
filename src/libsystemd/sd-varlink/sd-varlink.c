...
_public_ int sd_varlink_connect_url(sd_varlink **ret, const char *url) {
        ...
        if (p)
                ...
                const char *bridges_dir = secure_getenv("SYSTEMD_VARLINK_BRIDGES_DIR") ?: SYSTEMD_VARLINK_BRIDGES_DIR;
                ...
                if (access(bridge, X_OK) < 0) {
                        if (errno == ENOENT)
                                return log_debug_errno(SYNTHETIC_ERRNO(EPROTONOSUPPORT), "URL scheme not supported (and no auxiliary bridge binary is available).");

                        return log_debug_errno(errno, "Failed to look up varlink bridge binary: %m");
                }
