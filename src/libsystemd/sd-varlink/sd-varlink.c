                ...
                if (!invalid) {
                        ...
                        if (VARLINK_STATE_WANTS_REPLY(v->state)) {
                                if (r < 0) {
                                        ...
                                        if (previous)
                                                r = varlink_enqueue_reply(v, previous, /* more = */ false);
                                        else {
...
_public_ int sd_varlink_notify(sd_varlink *v, sd_json_variant *parameters) {
        ...
        if (v->sentinel)
                return varlink_log_errno(v, SYNTHETIC_ERRNO(EINVAL), "Cannot use sd_varlink_notify() with sentinel.");
        ...
        return varlink_enqueue_reply(v, parameters, /* more= */ true);
}
