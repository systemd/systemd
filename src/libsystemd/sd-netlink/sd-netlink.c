...
int netlink_set_partial_dispatch(sd_netlink *nl, bool enable) {
        ...
        /* Partial dispatch relies on the kernel dumping the routes in trie order, which is only
         * guaranteed when NETLINK_GET_STRICT_CHK is honored. If it could not be enabled, refuse the
         * mode instead of silently changing the semantics of the (then unordered) dump. */
        assert_return(nl->strict_chk, -EOPNOTSUPP);
