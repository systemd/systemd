...
static unsigned netlink_message_get_last_index(sd_netlink_message *m, uint16_t attr_type) {
        struct netlink_attribute *attribute;

        /* For attributes that appear multiple times, non-indexed reads default to the last
         * occurrence (last-wins, matching the kernel's nla_parse() semantics). Indexed reads use
         * explicit parse order (index 0 = first occurrence). */
        assert(m);

        if (m->n_containers >= NETLINK_CONTAINER_DEPTH)
                return 0;
        if (!m->containers[m->n_containers].attributes)
                return 0;
        if (attr_type > m->containers[m->n_containers].attributes->max_attribute)
                return 0;

        attribute = &m->containers[m->n_containers].attributes->attributes[attr_type];
        if (attribute->offsets_count == 0)
                return 0;

        return (unsigned) (attribute->offsets_count - 1);
}
...
static int netlink_message_read_internal(
        ...
        return netlink_message_read_internal_indexed(m, attr_type, ret_data, ret_net_byteorder, index);
}
...
int sd_netlink_message_read_string_indexed(sd_netlink_message *m, uint16_t attr_type, unsigned index, const char **ret) {
        void *attr_data;
        int r;
...
int sd_netlink_message_read_string(sd_netlink_message *m, uint16_t attr_type, const char **ret) {
        ...
        return sd_netlink_message_read_string_indexed(m, attr_type, index, ret);
}
...
int sd_netlink_message_read_strv(sd_netlink_message *m, uint16_t container_type, uint16_t attr_type, char ***ret) {
        ...
        for (unsigned i = 0; i < count; i++) {
                r = sd_netlink_message_read_string_indexed(m, attr_type, i, &str);
                if (r < 0)
                        goto finish;
                r = strv_extend(&s, str);
                if (r < 0)
...
static int netlink_container_parse(
        ...
        for (; RTA_OK(rta, len); rta = RTA_NEXT(rta, len)) {
                ...
                if (!greedy_realloc0((void **) &attributes, n_extra + max_attr + 1, sizeof(struct netlink_attribute)))
                        return -ENOMEM;
                ...
                /* When the same attr_type appears multiple times we keep all offsets, but 'nested' and
                 * 'net_byteorder' only retain the flags of the last instance (and are returned regardless of
                 * the requested index). This is fine because indexed reads are only used for attributes where
                 * these flags are uniform across instances (e.g. strings, which use neither flag). */
