...
typedef struct sd_netlink {
        ...
        /* Whether the NETLINK_GET_STRICT_CHK option could be enabled on this socket. If not, the kernel
         * does not guarantee the dump ordering that partial dispatch mode relies on. */
        bool strict_chk:1;
...
/* Returns -EOPNOTSUPP on sockets where NETLINK_GET_STRICT_CHK is not honored, i.e. where the
 * kernel does not guarantee the dump ordering partial dispatch mode relies on. */
