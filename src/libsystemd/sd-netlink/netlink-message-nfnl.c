...
int sd_nfnl_call_batch(
        ...
        for (size_t i = 0; i < n_messages; i++)
                /* On error, kernel may not send reply for some messages, reading replies with 0 timeout if
                 * we have already received an error. */
                if (r < 0)
                        (void) netlink_read_internal(nfnl, serials[i + 1], /* timeout_timestamp= */ 0, /* ret= */ NULL);
                else
                        RET_GATHER(r, sd_netlink_read(nfnl, serials[i + 1], usec, ret_messages ? replies + i : NULL));
        if (r < 0)
                return r;
