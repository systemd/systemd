...
#define FOREACH_DEVICE_CHILD(device, child)                   \
        for (child = sd_device_get_child_first(device, NULL); \
             child;                                           \
             child = sd_device_get_child_next(device, NULL))

#define FOREACH_DEVICE_CHILD_WITH_NAME(device, child, name)     \
        for (child = sd_device_get_child_first(device, &name);  \
             child;                                             \
             child = sd_device_get_child_next(device, &name))
