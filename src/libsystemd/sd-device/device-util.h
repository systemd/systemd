...
#define FOREACH_DEVICE_PROPERTY(device, key, value) \
        for (key = sd_device_get_property_first(device, &(value)); key; key = sd_device_get_property_next(device, &(value)))

#define FOREACH_DEVICE_TAG(device, tag) for (tag = sd_device_get_tag_first(device); tag; tag = sd_device_get_tag_next(device))

#define FOREACH_DEVICE_SYSATTR(device, attr) \
        for (attr = sd_device_get_sysattr_first(device); attr; attr = sd_device_get_sysattr_next(device))
...
#define FOREACH_SUBSYSTEM(enumerator, device)                                       \
        for (device = sd_device_enumerator_get_subsystem_first(enumerator); device; \
...
#define log_device_full(device, level, error, ...)                                                                                       \
        ({                                                                                                                               \
                ...
                log_object_internal(                                                                                                     \
                        _level, _error, __FILE__, __LINE__, __func__, _sysname ? "DEVICE=" : NULL, _sysname, NULL, NULL, ##__VA_ARGS__); \
        })
