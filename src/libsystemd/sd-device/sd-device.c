...
static int device_cache_sysattr_value_full(sd_device *device, char *key, char *value, size_t size, char *value_stripped, int error, bool ignore_uevent) {
        int r;
...
int device_cache_sysattr_value(sd_device *device, char *key, char *value, int error) {
        char *value_stripped = strdup(value);
        if (!value_stripped)
                return -ENOMEM;

        delete_trailing_chars(value_stripped, NEWLINE);
        return device_cache_sysattr_value_full(device, key, value, strlen(value), value_stripped, error, /* ignore_uevent = */ true);
}
...
cache_result:
        ...
        char *value_stripped = memdup(value, size);
        if (!value_stripped)
                return -ENOMEM;
        delete_trailing_chars(value_stripped, NEWLINE);

        int k = device_cache_sysattr_value_full(device, resolved, value, size, value_stripped, -r, /* ignore_uevent = */ false);
        if (k < 0) {
                if (r < 0)
        ...
        if (ret_value && r >= 0)
                *ret_value = ret_size ? value : value_stripped;
        if (ret_size && r >= 0)
                *ret_size = size;
