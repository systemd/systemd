...
int device_set_syspath(sd_device *device, const char *_syspath, bool verify) {
        ...
        if (verify) {
                ...
                if (r == -ENOENT)
                        ...
                        return log_debug_errno(SYNTHETIC_ERRNO(ENODEV), "sd-device: Failed to chase symlinks in \"%s\".", _syspath);
                if (r < 0)
        ...
        if (devpath[0] != '/')
                return log_debug_errno(SYNTHETIC_ERRNO(ENODEV), "\"/sys\" alone is not a valid device path.");

        r = device_add_property_internal(device, "DEVPATH", devpath);
        if (r < 0)
                return log_debug_errno(r, "Failed to add \"DEVPATH\" property: %m");
