...
_public_ int sd_device_new_from_ifname(sd_device **ret, const char *ifname) {
        ...
        /* This also accepts alternative interface names. Please use
         * sd_device_new_from_subsystem_sysname() if we know the input is the main interface name. */

        r = rtnl_resolve_ifname(NULL, ifname);
        if (r < 0)
                return r;
