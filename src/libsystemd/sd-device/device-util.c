...
static int add_string_field(
        ...
        r = func(device, &s);
        if (r < 0 && r != -ENOENT)
                log_debug_errno(r, "Failed to get device \"%s\" property, ignoring: %m", field);
...
char** device_log_context_fields(sd_device *device) {
        ...
        if (r < 0 && r != -ENOENT)
                log_debug_errno(r, "Failed to get device \"DEVNUM\" property, ignoring: %m");
        ...
        if (r < 0 && r != -ENOENT)
                log_debug_errno(r, "Failed to get device \"IFINDEX\" property, ignoring: %m");
        ...
        if (r < 0 && r != -ENOENT)
                log_debug_errno(r, "Failed to get device \"ACTION\" property, ignoring: %m");
        ...
        if (r < 0 && r != -ENOENT)
                log_debug_errno(r, "Failed to get device \"SEQNUM\" property, ignoring: %m");
        ...
        if (r < 0 && r != -ENOENT)
                log_debug_errno(r, "Failed to get device \"DISKSEQ\" property, ignoring: %m");
