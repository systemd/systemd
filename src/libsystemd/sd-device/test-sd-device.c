...
TEST(sd_device_get_child) {
        ...
        FOREACH_DEVICE(e, dev) {
                const char *syspath, *parent_syspath, *expected_name, *name;
                ...
                assert_se(expected_name = path_startswith(syspath, parent_syspath));

                log_debug("> %s", syspath);

                FOREACH_DEVICE_CHILD_WITH_NAME(parent, child, name) {
                        const char *s;

                        assert_se(child);
                        assert_se(name);

                        if (!streq(name, expected_name))
                                continue;
