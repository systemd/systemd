...
static void test_sd_device_one(sd_device *d) {
        ...
        assert_se(sd_device_new_from_path(&dev, val) >= 0);
        assert_se(sd_device_get_syspath(dev, &val) >= 0);
        assert_se(streq(syspath, val));
        dev = sd_device_unref(dev);
        ...
        if (r >= 0) {
                assert_se(sd_device_new_from_devname(&dev, devname) >= 0);
                assert_se(sd_device_get_syspath(dev, &val) >= 0);
                assert_se(streq(syspath, val));
                dev = sd_device_unref(dev);

                assert_se(sd_device_new_from_path(&dev, devname) >= 0);
                assert_se(sd_device_get_syspath(dev, &val) >= 0);
                assert_se(streq(syspath, val));
                dev = sd_device_unref(dev);
        } else
        ...
        if (r >= 0) {
                ...
                assert_se(sd_device_new_from_devnum(&dev, streq_ptr(subsystem, "block") ? 'b' : 'c', devnum) >= 0);
                assert_se(sd_device_get_syspath(dev, &val) >= 0);
                assert_se(streq(syspath, val));
                dev = sd_device_unref(dev);
        } else
