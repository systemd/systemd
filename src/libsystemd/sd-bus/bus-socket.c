...
static int bus_socket_auth_verify_client(sd_bus *b) {
        ...
        n = 0;
        lines[n] = b->rbuffer;
        l = memmem_safe(b->rbuffer, b->rbuffer_size, "\r\n", 2);
        if (l) {
                lines[++n] = l + 2;
                l = memmem_safe(l + 2, b->rbuffer_size - (l - (char*) b->rbuffer) - 2, "\r\n", 2);
                if (l) {
                        lines[++n] = l + 2;
                        l = memmem_safe(l + 2, b->rbuffer_size - (l - (char*) b->rbuffer) - 2, "\r\n", 2);
                        if (l)
        ...
        /*
         * In case of ANONYMOUS, we expect OK. In case of EXTERNAL, we expect
         * DATA+OK. If FD negotiation was requested, we additionally expect
         * an AGREE_UNIX_FD response.
         */
