...
TEST(chat) {
        ...
        _cleanup_(sd_fiber_unrefp) sd_fiber *f_server = NULL, *f_client1 = NULL, *f_client2 = NULL;
        _cleanup_(sd_bus_flush_close_unrefp) sd_bus *bus = NULL;
        int r;
        ...
        ASSERT_OK(sd_fiber_new_full(e, "client-1", client1, NULL, SD_FIBER_PRIORITY_DEFAULT, &f_client1));
        ASSERT_OK(sd_fiber_new_full(e, "client-2", client2, NULL, SD_FIBER_PRIORITY_DEFAULT, &f_client2));
        ASSERT_OK(sd_fiber_new_full(e, "server", server, bus, SD_FIBER_PRIORITY_DEFAULT, &f_server));

        ASSERT_OK(sd_event_loop(e));

        ASSERT_OK(sd_fiber_result(f_client1));
        ASSERT_OK(sd_fiber_result(f_client2));
        ASSERT_OK(sd_fiber_result(f_server));
}
