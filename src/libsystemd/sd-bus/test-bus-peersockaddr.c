...
TEST(description) {
        ...
        ASSERT_OK(sd_fiber_new_full(e, "server", server, INT_TO_PTR(fd), SD_FIBER_PRIORITY_DEFAULT, &f_server));
        TAKE_FD(fd);

        ASSERT_OK(sd_fiber_new_full(e, "client", client, a, SD_FIBER_PRIORITY_DEFAULT, &f_client));
