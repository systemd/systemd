...
TEST(description) {
        ...
        ASSERT_OK_ERRNO(pthread_create(&s, NULL, server, INT_TO_PTR(fd)));
        TAKE_FD(fd);

        ASSERT_OK_ERRNO(pthread_create(&c, NULL, client, a));

        ASSERT_OK_ERRNO(pthread_join(s, NULL));
        ASSERT_OK_ERRNO(pthread_join(c, NULL));
}
