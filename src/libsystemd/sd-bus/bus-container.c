...
static int bus_container_connect_namespace(sd_bus *b, int pidnsfd, int mntnsfd, int usernsfd, int rootfd) {
        ...
        r = namespace_fork("(sd-buscntrns)", "(sd-buscntr)", FORK_RESET_SIGNALS|FORK_DEATHSIG_SIGKILL|FORK_WAIT,
                           pidnsfd, mntnsfd, /* netns_fd= */ -EBADF, usernsfd, rootfd, &child);
        if (r < 0)
        ...
        return read_errno(errno_pipe_fd[0]);
}
...
int bus_container_connect_socket(sd_bus *b) {
        ...
        if (are_our_namespaces(pidnsfd, mntnsfd, /* netns_fd= */ -EBADF, usernsfd, rootfd))
                r = RET_NERRNO(connect(b->input_fd, &b->sockaddr.sa, b->sockaddr_size));
        else
        ...
        if (r == -EINPROGRESS)
                return 1;
        if (r < 0)
                return log_debug_errno(r, "Failed to connect to D-Bus socket: %m");
