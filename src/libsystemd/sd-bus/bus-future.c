...
static int bus_future_cancel(sd_future *f) {
        BusFuture *bf = sd_future_get_private(ASSERT_PTR(f));
...
int bus_call_suspend(
        ...
        _cleanup_(sd_future_cancel_wait_unrefp) sd_future *bf = NULL;
        r = bus_call_future(bus, m, usec, &bf);
        if (r < 0)
        ...
        if (sd_future_state(bf) != SD_FUTURE_RESOLVED)
                return sd_bus_error_set_errno(reterr_error, r);

        return future_get_bus_reply(bf, reterr_error, ret_reply);
}
