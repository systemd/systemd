...
static int default_request_name_handler(
        ...
        if (sd_bus_message_is_method_error(m, NULL)) {
                ...
                (void) bus_enter_closing(sd_bus_message_get_bus(m), -sd_bus_message_get_errno(m));
                return 1;
        }
        ...
        case BUS_NAME_EXISTS:
                ...
                (void) bus_enter_closing(sd_bus_message_get_bus(m), -EEXIST);
                return 1;
        }

        log_debug("Unexpected response from RequestName(), failing connection.");
        (void) bus_enter_closing(sd_bus_message_get_bus(m), -EPROTO);
        return 1;
}
