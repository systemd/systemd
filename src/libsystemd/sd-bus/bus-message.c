...
static void *message_extend_fields(sd_bus_message *m, size_t align, size_t sz, bool add_offset) {
        ...
        if (new_size < start || new_size > (size_t)((uint32_t) -1))
                goto poison;
...
static int message_append_field_string(sd_bus_message *m, uint64_t h, char type, const char *s, const char **ret) {
...
static int message_append_field_signature(sd_bus_message *m, uint64_t h, const char *s, const char **ret) {
...
int bus_message_from_malloc(sd_bus *bus,
        ...
        r = bus_message_from_header(
                bus,
                buffer,
                length, /* in this case the initial bytes and the final bytes are the same */
                buffer,
                length,
                length,
                fds,
...
_public_ int sd_bus_message_new(sd_bus *bus, sd_bus_message **m, uint8_t type) {
...
_public_ int sd_bus_message_new_signal(
        sd_bus *bus, sd_bus_message **m, const char *path, const char *interface, const char *member) {
        ...
        if (r < 0)
                return r;
        r = message_append_field_string(
                t, BUS_MESSAGE_HEADER_INTERFACE, SD_BUS_TYPE_STRING, interface, &t->interface);
        if (r < 0)
                return r;
...
_public_ int sd_bus_message_new_method_call(sd_bus *bus,
                                            sd_bus_message **m,
        ...
        if (destination) {
                r = message_append_field_string(
                        t, BUS_MESSAGE_HEADER_DESTINATION, SD_BUS_TYPE_STRING, destination, &t->destination);
                if (r < 0)
                        return r;
        }
...
static int message_new_reply(sd_bus_message *call, uint8_t type, sd_bus_message **m) {
        ...
        if (call->sender) {
                r = message_append_field_string(
                        t, BUS_MESSAGE_HEADER_DESTINATION, SD_BUS_TYPE_STRING, call->sender, &t->destination);
                if (r < 0)
                        return r;
        }
...
_public_ int sd_bus_message_new_method_error(sd_bus_message *call, sd_bus_message **m, const sd_bus_error *e) {
        ...
        r = message_append_field_string(
                t, BUS_MESSAGE_HEADER_ERROR_NAME, SD_BUS_TYPE_STRING, e->name, &t->error.name);
        if (r < 0)
                return r;
...
_public_ int sd_bus_message_new_method_errorf(
        sd_bus_message *call, sd_bus_message **m, const char *name, const char *format, ...) {
...
_public_ int sd_bus_message_new_method_errno(sd_bus_message *call,
                                             sd_bus_message **m,
                                             int error,
...
_public_ int sd_bus_message_new_method_errnof(
        sd_bus_message *call, sd_bus_message **m, int error, const char *format, ...) {
...
int bus_message_new_synthetic_error(sd_bus *bus, uint64_t cookie, const sd_bus_error *e, sd_bus_message **m) {
        ...
        if (bus && bus->unique_name) {
                r = message_append_field_string(t,
...
static int part_make_space(struct sd_bus_message *m, struct bus_body_part *part, size_t sz, void **q) {
...
static void *message_extend_body(sd_bus_message *m, size_t align, size_t sz, bool add_offset, bool force_inline) {
        ...
        if (end_body > (size_t)((uint32_t) -1) || end_body < start_body) {
                m->poisoned = true;
                return NULL;
        }
        ...
        if (added > 0) {
                ...
                add_new_part = m->n_body_parts <= 0 || m->body_end->sealed ||
...
_public_ int sd_bus_message_append_string_iovec(sd_bus_message *m,
...
static int bus_message_open_array(sd_bus_message *m,
                                  struct bus_container *c,
                                  const char *contents,
                                  uint32_t **array_size,
                                  size_t *begin,
...
static int bus_message_open_variant(sd_bus_message *m, struct bus_container *c, const char *contents) {
...
static int bus_message_open_struct(
        sd_bus_message *m, struct bus_container *c, const char *contents, size_t *begin, bool *need_offsets) {
...
static int type_stack_push(
        TypeStack *stack, unsigned max, unsigned *i, const char *types, unsigned n_struct, unsigned n_array) {
        assert(stack);
        assert(max > 0);
...
_public_ int sd_bus_message_appendv(sd_bus_message *m, const char *types, va_list ap) {
...
_public_ int sd_bus_message_append_array_iovec(sd_bus_message *m,
                                               char type,
                                               const struct iovec *iov,
                                               unsigned n /* should be size_t, but is API now… 😞 */) {
...
_public_ int sd_bus_message_append_array_memfd(
        sd_bus_message *m, char type, int memfd, uint64_t offset, uint64_t size) {
...
_public_ int sd_bus_message_append_string_memfd(sd_bus_message *m, int memfd, uint64_t offset, uint64_t size) {
...
static int bus_message_close_header(sd_bus_message *m) {
        ...
        if (BUS_MESSAGE_IS_GVARIANT(m)) {
                ...
                sz = bus_gvariant_determine_word_size(
                        sizeof(struct bus_header) + ALIGN8(m->fields_size) + m->body_size + 1 + l + 2, 1);
                ...
                *((uint8_t *) d + 1 + l + 1) = SD_BUS_TYPE_STRUCT_END;
                ...
                bus_gvariant_write_word_le(
                        (uint8_t *) d + 1 + l + 2, sz, sizeof(struct bus_header) + m->fields_size);
...
_public_ int sd_bus_message_seal(sd_bus_message *m, uint64_t cookie, uint64_t timeout_usec) {
        ...
        if (cookie > 0xffffffffULL && !BUS_MESSAGE_IS_GVARIANT(m))
                return -EOPNOTSUPP;

        /* In vtables the return signature of method calls is listed,
         * let's check if they match if this is a response */
        if (m->header->type == SD_BUS_MESSAGE_METHOD_RETURN && m->enforced_reply_signature &&
        ...
        if (!isempty(m->root_container.signature) && !BUS_MESSAGE_IS_GVARIANT(m)) {
                r = message_append_field_signature(
                        m, BUS_MESSAGE_HEADER_SIGNATURE, m->root_container.signature, NULL);
                if (r < 0)
                        return r;
        }
...
static int bus_message_enter_array(sd_bus_message *m,
                                   struct bus_container *c,
                                   const char *contents,
                                   uint32_t **array_size,
                                   size_t *item_size,
...
static int bus_message_enter_variant(sd_bus_message *m,
                                     struct bus_container *c,
                                     const char *contents,
                                     size_t *item_size) {
...
static int build_struct_offsets(sd_bus_message *m,
                                const char *signature,
                                size_t size,
                                size_t *item_size,
                                size_t **offsets,
...
static int enter_struct_or_dict_entry(sd_bus_message *m,
                                      struct bus_container *c,
                                      const char *contents,
                                      size_t *item_size,
                                      size_t **offsets,
...
static int bus_message_enter_struct(sd_bus_message *m,
                                    struct bus_container *c,
                                    const char *contents,
                                    size_t *item_size,
                                    size_t **offsets,
...
static int bus_message_enter_dict_entry(sd_bus_message *m,
                                        struct bus_container *c,
                                        const char *contents,
                                        size_t *item_size,
                                        size_t **offsets,
...
_public_ int sd_bus_message_enter_container(sd_bus_message *m, char type, const char *contents) {
        ...
        /* OK, let's fill it in */
        if (BUS_MESSAGE_IS_GVARIANT(m) && type == SD_BUS_TYPE_STRUCT && isempty(signature))
                end = m->rindex + 0;
        else
...
_public_ int sd_bus_message_peek_type(sd_bus_message *m, char *type, const char **contents) {
        ...
        if (c->signature[c->index] == SD_BUS_TYPE_ARRAY) {
                ...
                if (contents) {
                        ...
                        assert(l >= 1);
                        if (free_and_strndup(&c->peeked_signature, c->signature + c->index + 1, l) < 0)
                                return -ENOMEM;
        ...
        if (IN_SET(c->signature[c->index], SD_BUS_TYPE_STRUCT_BEGIN, SD_BUS_TYPE_DICT_ENTRY_BEGIN)) {
                ...
                if (contents) {
                        ...
                        assert(l >= 3);
                        if (free_and_strndup(&c->peeked_signature, c->signature + c->index + 1, l - 2) < 0)
                                return -ENOMEM;
                ...
                if (type)
                        *type = c->signature[c->index] == SD_BUS_TYPE_STRUCT_BEGIN ? SD_BUS_TYPE_STRUCT :
                                                                                     SD_BUS_TYPE_DICT_ENTRY;
        ...
        if (c->signature[c->index] == SD_BUS_TYPE_VARIANT) {
                if (contents) {
                        ...
                        if (BUS_MESSAGE_IS_GVARIANT(m)) {
                                ...
                                if (free_and_strndup(&c->peeked_signature, (char *) q + 1, k - 1) < 0)
                                        return -ENOMEM;
...
_public_ int sd_bus_message_readv(sd_bus_message *m, const char *types, va_list ap) {
...
_public_ int sd_bus_message_skip(sd_bus_message *m, const char *types) {
        ...
        case SD_BUS_TYPE_DICT_ENTRY_BEGIN: {
...

                {
                        ...
                        r = sd_bus_message_enter_container(m,
                                                           *types == SD_BUS_TYPE_STRUCT_BEGIN ?
                                                                   SD_BUS_TYPE_STRUCT :
                                                                   SD_BUS_TYPE_DICT_ENTRY,
                                                           s);
                        if (r <= 0)
...
static int message_peek_fields(sd_bus_message *m, size_t *rindex, size_t align, size_t nbytes, void **ret) {
...
static int message_peek_field_uint32(sd_bus_message *m, size_t *ri, size_t item_size, uint32_t *ret) {
...
static int message_peek_field_uint64(sd_bus_message *m, size_t *ri, size_t item_size, uint64_t *ret) {
...
static int message_peek_field_string(
        sd_bus_message *m, bool (*validate)(const char *p), size_t *ri, size_t item_size, const char **ret) {
...
static int message_peek_field_signature(sd_bus_message *m, size_t *ri, size_t item_size, const char **ret) {
...
static int message_skip_fields(sd_bus_message *m, size_t *ri, uint32_t array_size, const char **signature) {
        ...
        for (;;) {
                ...
                if (array_size != (uint32_t) -1 && array_size <= *ri - original_index)
                        return 0;
        ...
        if (BUS_MESSAGE_IS_GVARIANT(m)) {
                ...
                for (;;) {
                        ...
                        if (*p == 0) {
                                ...
                                if (l < 2 || p[1] != SD_BUS_TYPE_STRUCT_BEGIN ||
        ...
        while (ri < m->fields_size) {
                ...
                if (BUS_MESSAGE_IS_GVARIANT(m)) {
                        ...
                        if (i == 0)
                                ...
                                ri = ALIGN_TO(
                                        bus_gvariant_read_word_le((uint8_t *) offsets + (i - 1) * sz, sz), 8);
                ...
                case BUS_MESSAGE_HEADER_INTERFACE:
                        ...
                        r = message_peek_field_string(
                                m, interface_name_is_valid, &ri, item_size, &m->interface);
                        break;
                ...
                case BUS_MESSAGE_HEADER_DESTINATION:
                        ...
                        r = message_peek_field_string(
                                m, service_name_is_valid, &ri, item_size, &m->destination);
                        break;
...
_public_ int sd_bus_message_set_destination(sd_bus_message *m, const char *destination) {
        ...
        return message_append_field_string(
                m, BUS_MESSAGE_HEADER_DESTINATION, SD_BUS_TYPE_STRING, destination, &m->destination);
}
...
_public_ int sd_bus_message_set_sender(sd_bus_message *m, const char *sender) {
        ...
        return message_append_field_string(
                m, BUS_MESSAGE_HEADER_SENDER, SD_BUS_TYPE_STRING, sender, &m->sender);
}
...
static int bus_message_get_arg_skip(sd_bus_message *m, unsigned i, char *_type, const char **_contents) {
...
int bus_message_remarshal(sd_bus *bus, sd_bus_message **m) {
        ...
        case SD_BUS_MESSAGE_METHOD_CALL:
                r = sd_bus_message_new_method_call(
                        bus, &n, (*m)->destination, (*m)->path, (*m)->interface, (*m)->member);
                if (r < 0)
                        return r;
        ...
        case SD_BUS_MESSAGE_METHOD_ERROR:
                ...
                if ((*m)->header->type == SD_BUS_MESSAGE_METHOD_ERROR && (*m)->error.name) {
                        r = message_append_field_string(n,
        ...
        if ((*m)->sender && !n->sender) {
                r = message_append_field_string(
                        n, BUS_MESSAGE_HEADER_SENDER, SD_BUS_TYPE_STRING, (*m)->sender, &n->sender);
                if (r < 0)
                        return r;
        }
