...
static int message_parse_fields(sd_bus_message *m, bool fds_might_be_truncated);
...
int bus_message_from_malloc(
        ...
        r = message_parse_fields(m, fds_might_be_truncated);
        if (r < 0)
                return r;
...
static int message_parse_fields(sd_bus_message *m, bool fds_might_be_truncated) {
         ...
         * may have truncated the fd array (e.g., due to LSM denials blocking fd passing). In that case, we
         * accept fewer fds than declared. Any attempt to actually use a truncated fd will fail later when
         * sd_bus_message_read_basic() finds the fd index out of range. */
        if (fds_might_be_truncated) {
