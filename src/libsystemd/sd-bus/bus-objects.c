...
static int bus_fiber_resolved(sd_future *f) {
        ...
        /* set_remove() unhooks the entry but does NOT fire the hash_ops destructor, so we have to
         * drop the ref the set held ourselves. fiber_run() holds an extra ref across the resolve
         * path so f isn't freed mid-resolution even if ours was the last one. */
