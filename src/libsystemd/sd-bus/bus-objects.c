...
static int object_manager_serialize_path(
        ...
        if (!require_fallback && !LIST_IS_EMPTY(n->object_managers)) {
...
static int object_manager_serialize_path_and_fallbacks(
                sd_bus *bus, sd_bus_message *reply, const char *path, sd_bus_error *error) {
...
static int bus_find_parent_object_manager(sd_bus *bus, struct node **out, const char *path, bool* path_has_object_manager) {
        ...
        if (n)
                *path_has_object_manager = !LIST_IS_EMPTY(n->object_managers);
