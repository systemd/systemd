...
int main(int argc, char *argv[]) {
        ...
        ASSERT_OK_EQ_ERRNO(pthread_create(&server, NULL, thread_server, path), 0);
        ASSERT_OK_EQ_ERRNO(pthread_create(&client1, NULL, thread_client1, path), 0);
        ASSERT_OK_EQ_ERRNO(pthread_create(&client2, NULL, thread_client2, path), 0);

        ASSERT_OK_EQ_ERRNO(pthread_join(client1, NULL), 0);
        ASSERT_OK_EQ_ERRNO(pthread_join(client2, NULL), 0);

        request_exit(path);

        ASSERT_OK_EQ_ERRNO(pthread_join(server, NULL), 0);
