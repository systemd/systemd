...
typedef struct RequestExitData {
        const char *path;
        sd_fiber_waitgroup *waitgroup;
} RequestExitData;

static int request_exit(void *userdata) {
        _cleanup_(sd_bus_flush_close_unrefp) sd_bus *bus = NULL;
        RequestExitData *data = ASSERT_PTR(userdata);
        const char *t;

        /* Wait for all client fibers to complete before requesting exit */
        ASSERT_OK(sd_fiber_waitgroup_wait(data->waitgroup));

        ASSERT_OK(sd_bus_new(&bus));

        t = strjoina("unix:path=", data->path);
        ASSERT_OK(sd_bus_set_address(bus, t));
        ASSERT_OK(sd_bus_set_watch_bind(bus, true));
...
int main(int argc, char *argv[]) {
        ...
        _cleanup_(sd_fiber_waitgroup_freep) sd_fiber_waitgroup *waitgroup = NULL;
        _cleanup_(sd_fiber_unrefp) sd_fiber *f_server = NULL, *f_client1 = NULL, *f_client2 = NULL, *f_exit = NULL;
        char *path;
        ...
        ASSERT_OK(sd_fiber_waitgroup_new(&waitgroup));

        ASSERT_OK(sd_fiber_new_full(e, "server", server, path, SD_FIBER_PRIORITY_DEFAULT, &f_server));

        ASSERT_OK(sd_fiber_new_full(e, "client-1", client1, path, SD_FIBER_PRIORITY_DEFAULT, &f_client1));
        ASSERT_OK(sd_fiber_waitgroup_add(waitgroup, f_client1));

        ASSERT_OK(sd_fiber_new_full(e, "client-2", client2, path, SD_FIBER_PRIORITY_DEFAULT, &f_client2));
        ASSERT_OK(sd_fiber_waitgroup_add(waitgroup, f_client2));

        ASSERT_OK(sd_fiber_new_full(e, "request-exit", request_exit, &(RequestExitData) {
                .path = path,
                .waitgroup = waitgroup,
        }, SD_FIBER_PRIORITY_DEFAULT, &f_exit));

        ASSERT_OK(sd_event_loop(e));

        ASSERT_OK(sd_fiber_result(f_client1));
        ASSERT_OK(sd_fiber_result(f_client2));
        ASSERT_OK(sd_fiber_result(f_exit));
