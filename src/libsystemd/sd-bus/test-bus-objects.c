...
static int client(struct context *c) {
        ...
        while (({
                ...
                if (streq(path, "/value/b") || streq(path, "/value/a")) {
        ...
        assert_se(sd_bus_message_read_basic(reply, 'o', &should_be_value_a_x));
        assert_se(streq(should_be_value_a_x, "/value/a/x"));
        assert_se(sd_bus_message_enter_container(reply, SD_BUS_TYPE_ARRAY, "{sa{sv}}") > 0);
        while (({
                       r = sd_bus_message_enter_container(reply, SD_BUS_TYPE_DICT_ENTRY, "sa{sv}");
                       assert_se(r >= 0);
                       r;
               }) > 0) {
                const char* interface_name = NULL;
        ...
        reply = sd_bus_message_unref(reply);
         ...
         r = sd_bus_call_method(bus, "org.freedesktop.systemd.test", "/foo", "org.freedesktop.systemd.test", "EmitObjectWithManagerAdded", &error, NULL, NULL);
        assert_se(r >= 0);
        ...
        assert_se(sd_bus_message_read_basic(reply, 'o', &should_be_value_a));
        ...
        assert_se(true == found_object_manager);
        ...
        while (({
                       r = sd_bus_message_read_basic(reply, 's', &deleted_interface_name);
                       assert_se(r >= 0);
                       r;
               }) > 0) {
