...
static int client(void *userdata) {
        ...
        ASSERT_OK(sd_bus_attach_event(bus, sd_fiber_event(NULL), 0));
...
static int test_one(bool client_negotiate_unix_fds, bool server_negotiate_unix_fds,
        ...
        _cleanup_(sd_fiber_unrefp) sd_fiber *f_server = NULL, *f_client = NULL;
        struct context c;
        int r = 0;
        ...
        ASSERT_OK(sd_fiber_new_full(e, "server", server, &c, SD_FIBER_PRIORITY_DEFAULT, &f_server));
        ASSERT_OK(sd_fiber_new_full(e, "client", client, &c, SD_FIBER_PRIORITY_DEFAULT, &f_client));

        ASSERT_OK(sd_event_loop(e));

        RET_GATHER(r, sd_fiber_result(f_client));
        RET_GATHER(r, sd_fiber_result(f_server));
