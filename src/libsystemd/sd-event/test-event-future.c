...
TEST(future_child_cancel) {
        ...
        /* It also releases sd-event's per-process child slot, so the process can be watched again
         * without waiting for the cancelled future to be freed. */
...
/* Ownership needs to see the process exit and reap it, so it is rejected for waitid() options that
 * report something else or skip the reaping. */
