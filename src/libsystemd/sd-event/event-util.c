...
static int event_forward_signal_callback(sd_event_source *s, const struct signalfd_siginfo *ssi, void *userdata) {
        ...
        siginfo_t si = {
                 ...
                 * kernel. */
                .si_code = (ssi->ssi_code == 0 || ssi->ssi_code & (SI_FLAG_FORWARD|SI_FLAG_POSITIVE)) ? INT_MIN :
                           (ssi->ssi_code > 0 ? -ssi->ssi_code | SI_FLAG_POSITIVE | SI_FLAG_FORWARD : ssi->ssi_code | SI_FLAG_FORWARD),
