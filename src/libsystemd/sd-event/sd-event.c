...
struct sd_event {
        ...
        LIST_HEAD(struct inotify_data, buffered_inotify_datas);
...
static void event_free_inotify_data(sd_event *e, struct inotify_data *d) {
        ...
        if (d->buffer_filled > 0)
                LIST_REMOVE(buffered, e->buffered_inotify_datas, d);
...
static int event_inotify_data_read(sd_event *e, struct inotify_data *d, uint32_t revents, int64_t threshold) {
        ...
        LIST_PREPEND(buffered, e->buffered_inotify_datas, d);
...
static int process_inotify(sd_event *e) {
        ...
        LIST_FOREACH(buffered, d, e->buffered_inotify_datas) {
...
_public_ int sd_event_prepare(sd_event *e) {
        ...
        if (event_next_pending(e) || e->need_process_child || e->buffered_inotify_datas)
                goto pending;
