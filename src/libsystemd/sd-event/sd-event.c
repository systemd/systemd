...
static int event_add_pressure(
        ...
        _cleanup_free_ char *w = NULL, *env_watch = NULL, *env_write = NULL;
        _cleanup_(source_freep) sd_event_source *s = NULL;
        ...
        _cleanup_free_ char *resource_upper = strdup(resource);
        if (!resource_upper)
                return -ENOMEM;
        ...
        env = secure_getenv(env_watch);
        if (env) {
                if (isempty(env) || path_equal(env, "/dev/null"))
                        return log_debug_errno(SYNTHETIC_ERRNO(EHOSTDOWN),
                                               "Pressure logic is explicitly disabled via $%s.", env_watch);

                if (!path_is_absolute(env) || !path_is_normalized(env))
                        return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
                                               "$%s set to invalid path: %s", env_watch, env);

                watch = env;

                env = secure_getenv(env_write);
                if (env) {
                ...
                _cleanup_free_ char *cgroup_file = strjoin(resource, ".pressure");
                if (!cgroup_file)
                        return -ENOMEM;
                ...
                watch_fallback = strjoin("/proc/pressure/", resource);
                if (!watch_fallback)
                        return -ENOMEM;
