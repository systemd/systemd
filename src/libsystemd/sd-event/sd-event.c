...
int sd_event_trim_memory(void) {
        ...
        log_debug("Memory trimming took %s, returned %s to OS.",
                  FORMAT_TIMESPAN(after_timestamp - before_timestamp, 0),
                  FORMAT_BYTES(l));
#else
        log_debug("Memory trimming took %s.",
                  FORMAT_TIMESPAN(after_timestamp - before_timestamp, 0));
#endif
...
_public_ int sd_event_add_memory_pressure(
        ...
        if (S_ISSOCK(st.st_mode)) {
                ...
                /* If this is a regular file we assume it is a procfs or cgroupfs file, where we look for EPOLLPRI */
                events = S_ISREG(st.st_mode) ? EPOLLPRI : EPOLLIN;
