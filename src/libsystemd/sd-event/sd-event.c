...
static int event_source_leave_ratelimit(sd_event_source *s, bool callback) {
        int r;
        ...
        if (callback && s->ratelimit_expire_callback) {
                s->dispatching = true;
...
static int process_timer(
        ...
        for (;;) {
                ...
                if (s->ratelimited) {
                        ...
                        r = event_source_leave_ratelimit(s, true);
                        if (r < 0)
                                return r;
                        else if (r == 1)
...
_public_ int sd_event_source_set_ratelimit(sd_event_source *s, uint64_t interval, unsigned burst) {
         ...
         * non-ratelimited. */
        r = event_source_leave_ratelimit(s, false);
        if (r < 0)
                return r;
