...
DEFINE_PRIVATE_ORIGIN_ID_HELPERS(sd_event);
...
static void source_io_unregister(sd_event_source *s) {
        ...
        if (sd_event_origin_id_changed(s->event))
                return;
...
static int event_make_signal_data(
        ...
        if (sd_event_origin_id_changed(e))
                return -ECHILD;
...
_public_ int sd_event_add_io(
        ...
        assert_return(!sd_event_origin_id_changed(e), -ECHILD);
...
static void event_free_inode_data(
        ...
        if (d->inotify_data) {
                ...
                if (d->wd >= 0) {
                        if (d->inotify_data->fd >= 0 && !sd_event_origin_id_changed(e)) {
...
_public_ int sd_event_source_set_description(sd_event_source *s, const char *description) {
        ...
        assert_return(!sd_event_origin_id_changed(s->event), -ECHILD);
...
_public_ sd_event *sd_event_source_get_event(sd_event_source *s) {
        ...
        assert_return(!sd_event_origin_id_changed(s->event), NULL);
