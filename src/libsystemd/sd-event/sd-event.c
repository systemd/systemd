        ...
        if (event_io_uring_enabled(e)) {
                ...
                if (IN_SET(op, EPOLL_CTL_ADD, EPOLL_CTL_MOD)) {
                        ...
                        if (FLAGS_SET(events, EPOLLET))
                                io_uring_prep_poll_multishot(sqe, fd, events & EPOLL_POLL_COMMON_MASK);
                        else
                                io_uring_prep_poll_add(sqe, fd, events & EPOLL_POLL_COMMON_MASK);
                        io_uring_sqe_set_data(sqe, userdata);

                        /* Pin the source across the kernel's POLL_ADD lifetime: if the user drops
                         * their ref while a CQE is in flight, the source memory must remain valid
                         * until dispatch_cqe() processes that CQE and drops this ref. */
...
static int dispatch_exit(sd_event *e) {
        ...
        if (!p || event_source_is_offline(p)) {
                /* Don't transition to FINISHED until every undispatched slot has had its callback fire;
                 * the exit cascade in sd_event_wait() drives this by waiting for more CQEs. */
...
#if HAVE_LIBURING
/* True for the terminal CQE of a source POLL_ADD: a CQE attached to an event source where the kernel
 * is done with the SQE (IORING_CQE_F_MORE not set, i.e. no further CQEs follow). For EPOLLET
 * (multishot) the kernel sets IORING_CQE_F_MORE on intermediate CQEs and clears it on the terminal
 * one. The matching ref taken in event_attach_fd() must be dropped on every such CQE. */
static bool cqe_is_terminal_source(struct io_uring_cqe *cqe) {
        void *userdata = io_uring_cqe_get_data(cqe);

        return userdata &&
               userdata != INT_TO_PTR(SOURCE_WATCHDOG) &&
               pending_kind(userdata) == WAKEUP_EVENT_SOURCE &&
               !FLAGS_SET(cqe->flags, IORING_CQE_F_MORE);
}
...
static int dispatch_source_cqe(
        ...
        uint32_t events = (uint32_t) cqe->res & EPOLL_POLL_COMMON_MASK;
         ...
         * source_dispatch (which cancels the fd anyway). */
        ...
        case WAKEUP_EVENT_SOURCE: {
                ...
                *min_priority = MIN(*min_priority, s->priority);
                ...
                case SOURCE_IO_PRESSURE:
                        if (s->enabled != SD_EVENT_ONESHOT) {
                                r = event_attach_fd(e, EPOLL_CTL_ADD, s->pressure.fd,
...
static int dispatch_cqe(
        ...
        if (cqe_is_terminal_source(cqe)) {
                sd_event_source *s = io_uring_cqe_get_data(cqe);
                assert(s->io_uring_inflight > 0);
                s->io_uring_inflight--;
                sd_event_source_unref(s);
                /* s may have been freed; do not access. */
        }
 ...
 * FINISHED once every slot has had its terminal CQE delivered — see the exit_requested branch in
 * sd_event_wait() and dispatch_exit(). Idempotent — safe to call repeatedly; only the first call
 * actually submits the cancel SQE. No-op without liburing. */
...
static bool event_has_undispatched_slots(sd_event *e) {
        ...
        /* "Undispatched" covers both slots whose CQE hasn't arrived yet (inflight) and slots whose CQE
         * arrived but whose callback hasn't fired yet (pending). Both must drain before we can FINISH.
         * Without liburing we never create slots, so e->slots is always empty and this returns false. */
        LIST_FOREACH(slots, s, e->slots)
...
_public_ int sd_event_wait(sd_event *e, uint64_t timeout) {
        ...
        if (e->exit_requested) {
                ...
                /* When there are no undispatched slots left, transition straight to PENDING so the
                 * caller sees the FINISHED state on the next dispatch. Otherwise fall through into the
                 * wait+process_io_uring path below to keep draining CQEs. */
                if (!event_has_undispatched_slots(e)) {
