...
static int epoll_wait_usec(
         ...
         * biggie), let's hence rely on glibc's definitions, made weakly referenced so the binary loads on
         * older glibc runtimes too, and fallback to epoll_pwait() when that's missing at runtime. */
#pragma weak epoll_pwait2
