...
TEST(io_uring_signal_stale_cqe_after_unref) {
        ...
        /* The source went away before its signalfd could dequeue SIGUSR2, so the signal is still
         * pending and blocked. Pop it: SIGUSR2 is not queued, so a later sigqueue() of it would be
