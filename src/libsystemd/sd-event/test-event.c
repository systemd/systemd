...
TEST(basic) {
        ...
        ASSERT_GE(sd_event_run(e, UINT64_MAX), 1);
        ASSERT_EQ(got_d, 1U);
        ASSERT_OK_ERRNO(write(d[1], &ch, 1));
        ASSERT_OK_EQ(sd_event_run(e, UINT64_MAX), 1);
        ASSERT_EQ(got_d, 2U);
        ...
        ASSERT_FALSE(got_a && got_b && got_c);
...
TEST(rtqueue) {
        ...
        ASSERT_OK(sigqueue(getpid_cached(), SIGRTMIN+2, (union sigval) { .sival_int = 1 }));
        ASSERT_OK(sigqueue(getpid_cached(), SIGRTMIN+3, (union sigval) { .sival_int = 2 }));
        ASSERT_OK(sigqueue(getpid_cached(), SIGUSR2, (union sigval) { .sival_int = 3 }));
        ASSERT_OK(sigqueue(getpid_cached(), SIGRTMIN+3, (union sigval) { .sival_int = 4 }));
        ASSERT_OK(sigqueue(getpid_cached(), SIGUSR2, (union sigval) { .sival_int = 5 }));
...
TEST(ratelimit) {
        ...
        ASSERT_TRUE(interval == 1 * USEC_PER_SEC && burst == 5);
        ...
        ASSERT_OK(sd_event_source_set_ratelimit(s, 0, 0));
        ASSERT_FALSE(sd_event_source_is_ratelimited(s));
