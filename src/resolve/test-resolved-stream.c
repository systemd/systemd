...
static void receive_and_check_question(int fd, const uint8_t *expected_question,
        ...
        uint8_t actual_question[question_size];
        size_t n_read = 0;
...
static void *tcp_dns_server(void *p) {
        ...
        assert_se((bindfd = socket(AF_INET, SOCK_STREAM | SOCK_CLOEXEC, 0)) != -1);
        ...
        assert_se((acceptfd = accept(bindfd, NULL, NULL)) != -1);
...
static void *tls_dns_server(void *p) {
        ...
        assert_se(inet_ntop(SERVER_ADDRESS.sin_family, &SERVER_ADDRESS.sin_addr,
                            ip_str, sizeof(ip_str)) != NULL);
        sprintf(bind_str, "%s:%d", ip_str, be16toh(SERVER_ADDRESS.sin_port));
        ...
        assert_se((openssl_pid = fork()) >= 0);
        if (openssl_pid == 0) {
                /* If the test fails, terminate the server to release the port */
                assert_se(prctl(PR_SET_PDEATHSIG, SIGTERM) == 0);

                assert_se(dup2(fd_tls, STDIN_FILENO) != -1);
                assert_se(dup2(fd_tls, STDOUT_FILENO) != -1);
                ...
                assert_se(waitpid(openssl_pid, NULL, 0) == openssl_pid);
...
static void send_simple_question(DnsStream *stream, sd_event *event, uint16_t type) {
        _cleanup_(dns_packet_unrefp) DnsPacket *p = NULL;
        _cleanup_(dns_resource_key_unrefp) DnsResourceKey *key = NULL;
        _cleanup_(dns_question_unrefp) DnsQuestion *question = NULL;

        assert_se(dns_packet_new(&p, DNS_PROTOCOL_DNS, 0, DNS_PACKET_SIZE_MAX) == 0);
        assert_se((question = dns_question_new(1)) != NULL);
        assert_se((key = dns_resource_key_new(DNS_CLASS_IN, type, TEST_DOMAIN)) != NULL);
...
static DnsPacket *received_packets[2] = { 0 };
...
static void test_dns_stream(bool tls) {
        ...
        assert_se((clientfd = socket(AF_INET, SOCK_STREAM | SOCK_CLOEXEC, 0)) != -1);
        ...
        assert_se(fcntl(clientfd, F_SETFL, O_NONBLOCK) != -1);
        ...
        send_simple_question(stream, event, DNS_TYPE_A);
        while (n_received_packets != 1)
                assert_se(sd_event_run(event, EVENT_TIMEOUT_USEC) == 1);
        assert_se(DNS_PACKET_DATA(received_packets[0]) != NULL);
        ...
        send_simple_question(stream, event, DNS_TYPE_AAAA);
        while (n_received_packets != 1)
                assert_se(sd_event_run(event, EVENT_TIMEOUT_USEC) == 1);
        assert_se(memcmp(DNS_PACKET_DATA(received_packets[0]),
