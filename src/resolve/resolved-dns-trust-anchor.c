...
static const uint8_t root_digest1[] = { 0x49, 0xAA, 0xC1, 0x1D, 0x7B, 0x6F, 0x64, 0x46, 0x70, 0x2E, 0x54,
...
static const uint8_t root_digest2[] = { 0xE0, 0x6D, 0x44, 0xB8, 0x0B, 0x8F, 0x1D, 0x39, 0xA9, 0x5C, 0x0B,
...
static bool dns_trust_anchor_knows_domain_positive(DnsTrustAnchor *d, const char *name) {
        ...
        return hashmap_contains(d->positive_by_key,
...
static int dns_trust_anchor_add_builtin_positive(DnsTrustAnchor *d) {
        ...
        /* Only add the built-in trust anchor if there's neither a DS nor a DNSKEY defined for the root
         * domain. That way users have an easy way to override the root domain DS/DNSKEY data. */
...
static int dns_trust_anchor_load_positive(DnsTrustAnchor *d, const char *path, unsigned line, const char *s) {
        ...
        if (r < 0)
                return log_warning_errno(
                        r,
                        "Failed to chack validity of domain name '%s', at line %s:%u, ignoring line: %m",
                        domain,
                        path,
                        line);
        if (r == 0) {
        ...
        if (strcaseeq(type, "DS")) {
                ...
                if (r < 0)
                        return log_warning_errno(
                                r, "Failed to parse DS key tag %s on line %s:%u: %m", key_tag, path, line);
                ...
                if (r < 0)
                        return log_warning_errno(
                                r, "Failed to parse DNSKEY parameters on line %s:%u: %m", path, line);
                if (r != 3) {
                ...
                if (r < 0)
                        return log_warning_errno(
                                r, "Failed to parse DNSKEY flags field %s on line %s:%u", flags, path, line);
                ...
                if (r < 0)
                        return log_warning_errno(
                                r, "Failed to parse DNSKEY key data %s on line %s:%u", p, path, line);
...
int dns_trust_anchor_lookup_negative(DnsTrustAnchor *d, const char *name) {
        ...
        for (;;) {
                ...
                /* If the domain isn't listed as NTA, but is listed as positive trust anchor, then that
                 * counts. See RFC 7646, section 1.1 */
                if (hashmap_contains(d->positive_by_key,
...
static int dns_trust_anchor_check_revoked_one(DnsTrustAnchor *d, DnsResourceRecord *revoked_dnskey) {
        ...
        if (a) {
                ...
                DNS_ANSWER_FOREACH(anchor, a) {
                        ...
                        if (((anchor->dnskey.flags ^ revoked_dnskey->dnskey.flags) | DNSKEY_FLAG_REVOKE) !=
                            DNSKEY_FLAG_REVOKE)
                                continue;

                        if (memcmp(anchor->dnskey.key, revoked_dnskey->dnskey.key, anchor->dnskey.key_size) !=
                            0)
                                continue;
        ...
        a = hashmap_get(d->positive_by_key,
                        &DNS_RESOURCE_KEY_CONST(revoked_dnskey->key->class,
                                                DNS_TYPE_DS,
