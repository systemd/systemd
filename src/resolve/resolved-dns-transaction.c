...
static int dns_delegation_type(DnsTransaction *t) {
        ...
        r = dns_answer_match_key(t->answer, dns_transaction_key(t), NULL);
        if (r)
                /* This is either an error or a secure delegation (1) */
                return r;
        ...
        r = dnssec_nsec_test(t->answer, ns, &nsec_result, NULL, NULL);
        if (r < 0)
                return r;

        if (nsec_result == DNSSEC_NSEC_FOUND)
                return 2;
