...
int dns_transaction_pick_server(DnsTransaction *t) {
        ...
        if (dns_scope_get_dns_server_policy(t->scope) == DNS_SERVER_POLICY_SEQUENTIAL) {
                ...
                if (t->server && t->server->linked) {
                        ...
                        /* First pick, or we've reached the end of the server list and
                         * are wrapping around. In the wrap-around case, the caller
                         * (dns_transaction_limited_retry) is responsible for stopping
                         * iteration via the n_picked_servers >= n_dns_servers check. */
