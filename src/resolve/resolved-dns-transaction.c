...
static void dns_transaction_close_connection(
        ...
        /* If we have an UDP socket where we sent a packet, rbut never received one, then add it to the socket
...
static bool dns_transaction_limited_retry(DnsTransaction *t) {
        ...
        /* If we haven't tried all different servers yet, let's trry again with a different server */
...
static void dns_transaction_randomize_answer(DnsTransaction *t) {
        ...
        if (r < 0) /* If this fails, just don't randomize, this is non-essential stuff after all */
                return (void) log_debug_errno(r, "Failed to clone answer record, ignoring: %m");
