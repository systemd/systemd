...
static size_t dns_transaction_curl_write_callback(void *contents, size_t size, size_t nmemb, void *userdata) {
        ...
        if (!t->payload){
                log_debug("Failed to copy HTTP payload to transaction.");
                return 0;
        }
...
static int dns_transaction_curl_recv(DnsTransaction *t, DnsPacket **p) {
        ...
        if (t->payload_size < 1){
                log_debug("Received HTTP payload unexpected size %lu", t->payload_size);
                return -1;
        }
...
static int dns_transaction_curl_make_url(DnsTransaction *t, char **url) {
        ...
        base64_string_len = strlen(base64_string);
        while (base64_string_len > 0 && base64_string[base64_string_len - 1] == '=') {
                base64_string[--base64_string_len] = '\0';
        }

        /* Make the URL to perform the wire format request */
