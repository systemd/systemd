...
static void assert_goodbye_packet(
        ...
        assert_se(n_expected <= ELEMENTSOF(found));
...
TEST(service_type_reference_counting) {
        ...
        ASSERT_OK(dns_scope_add_dnssd_registered_services(&scope));
        ASSERT_EQ(enumeration_record_count(&scope), 2u);
        dns_scope_remove_dnssd_registered_services(&scope, false);
        ASSERT_EQ(enumeration_record_count(&scope), 0u);
        establish_service_records(&scope, statik, dynamic, other);
        ASSERT_OK(dns_scope_add_dnssd_registered_services(&scope));
        ASSERT_EQ(enumeration_record_count(&scope), 2u);
...
TEST(attachment_lifecycle) {
        ...
        dns_scope_remove_dnssd_registered_services(&ipv4, false);
        ASSERT_EQ(hashmap_size(service->attachments), 1u);
        ...
        dns_scope_remove_dnssd_registered_services(&ipv6, false);
        ASSERT_TRUE(hashmap_isempty(service->attachments));
...
TEST(attachment_rollback) {
        DnssdRegisteredService *good, *service;
        ...
        good = add_published_service(&manager, "good", "_http._tcp", RESOLVE_CONFIG_SOURCE_DBUS);
        ASSERT_OK(dns_zone_put(&scope.zone, &scope, good->srv_rr, false));
        ASSERT_EQ(dns_scope_add_dnssd_service(&scope, good), 1);
        ...
        ASSERT_TRUE(hashmap_isempty(service->attachments));
        ASSERT_EQ(hashmap_size(scope.dnssd_services), 1u);
        ASSERT_EQ(hashmap_size(scope.dnssd_service_types), 1u);
        ASSERT_NULL(dns_zone_get(&scope.zone, service->ptr_rr));
        ASSERT_NULL(dns_zone_get(&scope.zone, service->srv_rr));
        ASSERT_NOT_NULL(dns_zone_get(&scope.zone, good->ptr_rr));
        ASSERT_NOT_NULL(dns_zone_get(&scope.zone, good->srv_rr));
        ASSERT_EQ(enumeration_record_count(&scope), 1u);

        dns_scope_remove_dnssd_service(&scope, good, false);
        dns_zone_flush(&scope.zone);
        scope.dnssd_services = hashmap_free(scope.dnssd_services);
        scope.dnssd_service_types = hashmap_free(scope.dnssd_service_types);
        dnssd_registered_service_remove(service, false);
        dnssd_registered_service_remove(good, false);
        manager.dnssd_registered_services = hashmap_free(manager.dnssd_registered_services);
}
