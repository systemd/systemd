...
TEST(mdns_goodbye_rearms_against_current_time) {
        ...
        for (unsigned i = 0;; i++) {
                ...
                /* More margin than the re-arm floor, not a hair's breadth: the equality asserted
                 * below holds only while the expiry stays beyond MDNS_GOODBYE_MIN_INTERVAL of the
                 * callback's own clock reading, so leave room for everything in between rather than
                 * letting a stalled runner turn the floor engaging into a re-arm failure. */
                if (expected_until > usec_add(now(CLOCK_BOOTTIME),
...
TEST(mdns_goodbye_receipt_keeps_the_earlier_deadline) {
         ...
         * The equality below holds only while that expiry stays beyond the re-arm floor of the
         * clock reading inside the arm, so refresh the rewrite until it does rather than assuming a
         * runner fast enough -- past the floor the arm would return the floor, and past the second
         * the prune inside it would empty the cache. */
        put_a(scope, "going.local", 120);
        for (unsigned i = 0;; i++) {
                put_a(scope, "going.local", 1);

                if (dns_cache_next_expiry(&scope->cache) >
                    usec_add(now(CLOCK_BOOTTIME), MDNS_GOODBYE_DELAY - MDNS_GOODBYE_MIN_INTERVAL))
                        break;
...
TEST(mdns_goodbye_rearm_is_floored) {
        ...
        _cleanup_(dns_scope_freep) DnsScope *scope = NULL;
        _cleanup_(sd_event_source_unrefp) sd_event_source *goodbye_source = NULL;
        usec_t stale_deadline, deadline, expiry, t, before, after;
