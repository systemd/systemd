...
TEST(mdns_goodbye_rearms_against_current_time) {
        ...
        /* A DNS scope, not an mDNS one: dns_scope_new() joins the multicast group for the
         * multicast protocols, which asserts on a link and needs a real socket. Only the cache
         * matters here, and the records go in with mDNS semantics below. */
