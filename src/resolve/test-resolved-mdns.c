...
/* The receipt side, where a goodbye whose records left nothing in the cache used to go unnoticed: the
 * arm declined (nothing due), and the direct notification it fell back to was rate limited, so any
 * further goodbye inside the window was dropped outright — no timer, no reconciliation, and the
 * withdrawn service stayed in every browser's list for good. A goodbye now always leaves a timer
 * behind, at the far end of the RFC 6762 § 10.1 window when the cache holds nothing sooner. */
...
TEST(mdns_goodbye_receipt_keeps_the_earlier_deadline) {
        ...
        /* A second goodbye, this one leaving nothing due: the nearer deadline stands. */
        mdns_goodbye_arm_on_receipt(scope);
        ASSERT_OK(sd_event_source_get_time(scope->mdns_goodbye_event_source, &deadline));
