 ...
 * Bounded, because the rewrite only lands on an entry the cache already holds: one that stops
 * landing leaves the expiry the full TTL out, which the far-end check below reports as the
 * fixture's failure rather than letting it surface as a wrong deadline later. Returns the expiry
 * the last rewrite recorded. */
...
TEST(mdns_goodbye_prunes_due_record_and_releases_timer) {
        ...
        /* Pin that the rewrite actually landed: the size alone is satisfied by the 120s put, and
         * the TTL=1 one only takes effect on an entry the cache already holds. Were it to stop
         * landing, the record would expire 120s out, the sleep below would not make it due, and the
         * failure would surface at the prune assertion instead of here. */
