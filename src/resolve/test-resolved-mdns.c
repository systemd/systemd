 ...
 *
 * The decision is now measured against a fresh clock reading inside dns_cache_expiry_in_one_second()
 * itself, so the callback ignores its scheduled-time argument by construction. The tests still hand it a
 * stale deadline to pin exactly that. */
...
TEST(mdns_goodbye_rearms_against_current_time) {
        ...
        ASSERT_TRUE(dns_cache_expiry_in_one_second(&scope->cache, /* ret_until= */ NULL));
        ...
        /* Not just re-armed, but armed for the record's expiry: about a second after the TTL=1 put,
         * and never beyond a second from now. */
        ASSERT_OK(sd_event_source_get_time(scope->mdns_goodbye_event_source, &deadline));
        ASSERT_GE(deadline, usec_add(t0, USEC_PER_SEC));
        ASSERT_LE(deadline, usec_add(now(CLOCK_BOOTTIME), USEC_PER_SEC));
}
