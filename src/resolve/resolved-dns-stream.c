...
DnsPacket *dns_stream_take_read_packet(DnsStream *s) {
         ...
         * dns_stream_update() must be called explicitly. Currently, this is only called from on_packet
         * callback which is called in on_stream_io(), and there dns_stream_update() is called. */
