...
TEST(mdns_announcement_max_sizes_without_a_known_mtu) {
        ...
        /* No MTU known yet: packets are cut to what a host of the family must accept
...
TEST(mdns_announcement_max_sizes_ignores_an_mtu_below_a_packet_header) {
        ...
        /* An MTU that leaves no room for a DNS header past the IP/UDP ones cannot yield a bound at
         * all: below the headers the subtraction underflows, and at them dns_packet_new() would be
         * asked for a packet smaller than the header it asserts room for. So the derivation falls
         * back to the unknown-MTU default. Checked right at the boundary and one byte below the
         * headers alone. A bound that is derivable but too small for any record is a different
         * case, kept deliberately -- see the packetization test for what it does. */
        FOREACH_ARGUMENT(family, AF_INET, AF_INET6) {
                 ...
                 * above are not just restating the default -- and yes, no record fits that bound;
                 * mdns_announcement_packetize_bound_too_small_for_any_record covers where that goes. */
...
TEST(mdns_announcement_packetize_splits_at_max_size) {
        ...
        FOREACH_STRING(name, "small0", "small1", "small2", "small3", "small4") {
                ...
                /* The packing bound below assumes equally sized records; enforce that. */
                if (sz == 0)
                        sz = rr_wire_size(rr);
                else
 ...
 * announcement this whole split exists to prevent. Reaching that requires max_size < fragmented_max
 * (so the retry happens) and a record too large for either (so it fails), which the other
 * skip cases do not have -- they pass the two bounds equal, and the retry never runs. */
 ...
 * fragmented. Pinned so that clamping such a bound up -- fewer datagrams, but several records in
 * each, still fragmented by the link -- cannot be introduced quietly. */
TEST(mdns_announcement_packetize_bound_too_small_for_any_record) {
        ...
        FOREACH_STRING(name, "small0", "small1", "small2") {
                _cleanup_(dns_resource_record_unrefp) DnsResourceRecord *rr = a_rr(name);

                /* The fragmented bound below assumes equally sized records; enforce that. */
                if (sz == 0)
                        sz = rr_wire_size(rr);
                else
                        ASSERT_EQ(rr_wire_size(rr), sz);
 ...
 * item there is nothing accumulated to flush, so control enters the retry with nothing counted yet
 * and a packet that had nothing appended. That is a different path through the same branch than the
 * mid-answer case above, and the one where a count kept outside the packet used to go stale. */
