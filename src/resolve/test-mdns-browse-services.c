...
static int print_txt(JsonVariant *txt_array) {
        JsonVariant *i;
        int r;
        ...
        JSON_VARIANT_ARRAY_FOREACH(i, txt_array) {
                _cleanup_free_ void *ret_str = NULL;

                r = json_variant_unbase64(i, &ret_str, NULL);
                if (r < 0)
                        return r;
...
static int resolve_service_query_reply(JsonVariant *parameters, const char *error_id) {
        ...
        if (srv_data) {
                ...
                printf("   port = [%lu]\n", (unsigned long) json_variant_unsigned(json_variant_by_key(srv_data, "port")));
        }
...
static int service_query_reply(
        ...
        JSON_VARIANT_ARRAY_FOREACH(in, recv_data) {
                ...
                if (resolve_flag && add_flag) {
                        ...
                        r = resolve_service_query_reply(ret_params, ret_err_str);
                        if (r < 0)
                                return r;
                        fflush(stdout);
                }
...
static int run(int argc, char **argv) {
        ...
        r = json_build(&in_params,
                        JSON_BUILD_OBJECT(
                                        ...
                                        JSON_BUILD_PAIR("ifindex", JSON_BUILD_INTEGER(ifindex)),
                                        JSON_BUILD_PAIR("flags", JSON_BUILD_INTEGER(SD_RESOLVED_MDNS | SD_RESOLVED_NO_ZONE | SD_RESOLVED_NO_STALE))));
        if (r < 0)
                goto finish;
