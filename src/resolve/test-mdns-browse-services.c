...
static int service_query_reply(
        ...
        assert(varlink_connect_address(&resolve_link, "/run/systemd/resolve/io.systemd.Resolve") >= 0);

        assert(varlink_set_description(resolve_link, "Resolve client") >= 0);
...
static int stop_signal_handler(sd_event_source *s, const struct signalfd_siginfo *si, void *userdata) {
        ...
        _cleanup_(json_variant_unrefp) JsonVariant* call_params;
        int r;
        ...
        varlink_call(stop_link, "io.systemd.Resolve.StopBrowse", call_params, NULL, NULL, NULL);
        if (r < 0)
                goto finish;
...
static int run(int argc, char **argv) {
        ...
        assert(varlink_connect_address(&start_link, "/run/systemd/resolve/io.systemd.Resolve") >= 0);

        assert(varlink_set_description(start_link, "client") >= 0);

        varlink_set_userdata(start_link, NULL);

        varlink_set_relative_timeout(start_link, USEC_INFINITY);

        assert(sd_event_default(&event) >= 0);

        if (sigemptyset(&ss) < 0 || sigaddset(&ss, SIGINT) < 0) {
        ...
        assert(varlink_attach_event(start_link, event, 0) >= 0);

        assert(sd_event_add_signal(event, NULL, SIGINT, stop_signal_handler, start_link) >= 0);

        assert(varlink_bind_reply(start_link, service_query_reply) >= 0);
        ...
        r = json_build(&in_params,
                        JSON_BUILD_OBJECT(
                                        ...
                                        JSON_BUILD_PAIR("token", JSON_BUILD_UNSIGNED(m_token))));
        if (r < 0) {
                log_error_errno(r, "json_build fail %m\n");
                assert(false);
        }

        assert(varlink_observe(start_link, "io.systemd.Resolve.StartBrowse", in_params) >= 0);

        assert(sd_event_loop(varlink_get_event(start_link)) >= 0);
