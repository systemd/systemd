...
static DnsQueryCandidate* dns_query_candidate_detach(DnsQueryCandidate *c) {
        assert(c);
...
static void dns_query_detach_candidates(DnsQuery *q) {
        assert(q);
        ...
        while (q->candidates)
                ...
                dns_query_candidate_unref(dns_query_candidate_detach(q->candidates));
}
