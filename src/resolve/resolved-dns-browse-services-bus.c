...
int bus_method_browse_services(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        if (!isempty(type) && !dnssd_srv_type_is_valid(type))
                return sd_bus_error_set(error, SD_BUS_ERROR_INVALID_ARGS, "Invalid SRV service type.");
        if (!isempty(domain)) {
                r = dns_name_is_valid(domain);
                if (r < 0)
                        return r;
                if (r == 0)
                        return sd_bus_error_set(error, SD_BUS_ERROR_INVALID_ARGS, "Invalid domain name.");
        }
