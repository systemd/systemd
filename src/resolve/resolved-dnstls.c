...
static char *dnstls_error_string(int ssl_error, char *buf, size_t count) {
        ...
        if (ssl_error == SSL_ERROR_SSL)
                ...
                snprintf(buf, count, "sym_SSL_get_error()=%d", ssl_error);
        return buf;
}
        ...
        if (stream->dnstls_data.buffer_offset < stream->dnstls_data.write_buffer->length) {
                ...
                if (ss < 0) {
                        ...
                        if (stream->dnstls_data.buffer_offset < stream->dnstls_data.write_buffer->length) {
                                ...
                                sym_BIO_ctrl(sym_SSL_get_wbio(stream->dnstls_data.ssl), BIO_CTRL_RESET, 0, NULL);
...
int dnstls_stream_connect_tls(DnsStream *stream, DnsServer *server) {
        ...
        sym_BIO_ctrl(wb, BIO_C_GET_BUF_MEM_PTR, 0, (char *) &stream->dnstls_data.write_buffer);
        stream->dnstls_data.buffer_offset = 0;
        ...
        if (server->server_name) {
                r = sym_SSL_ctrl(s, SSL_CTRL_SET_TLSEXT_HOSTNAME, TLSEXT_NAMETYPE_host_name, (void *) server->server_name);
                if (r <= 0)
...
int dnstls_manager_init(Manager *manager) {
        ...
        r = dlopen_libcrypto();
        if (r < 0)
                return log_warning_errno(r, "Failed to load libcrypto: %m");

        r = dlopen_libssl();
        if (r < 0)
                return log_warning_errno(r, "Failed to load libssl: %m");
        ...
        r = sym_SSL_CTX_ctrl(manager->dnstls_data.ctx, SSL_CTRL_SET_MIN_PROTO_VERSION, TLS1_2_VERSION, NULL);
        if (r == 0)
                return log_warning_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
