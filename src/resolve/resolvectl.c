...
static int show_statistics(int argc, char **argv, void *userdata) {
        ...
        s = json_variant_by_key(reply, "statistics");
        if (!s)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                       "DumpStatistics() response is missing 'statistics' key.");

        if (!json_variant_is_object(s))
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                       "DumpCache() response 'statistics' field not an object");

        if (!FLAGS_SET(arg_json_format_flags, JSON_FORMAT_OFF)) {
                return json_variant_dump(s, arg_json_format_flags, NULL, NULL);
        }

        struct statistics {
                unsigned n_current_transactions;
                ...
                unsigned cache_size;
                unsigned n_cache_hit;
                unsigned n_cache_miss;
                unsigned n_dnssec_secure;
                unsigned n_dnssec_insecure;
                unsigned n_dnssec_bogus;
                unsigned n_dnssec_indeterminate;
        } statistics = { };

        static const JsonDispatch statistics_dispatch_table[] = {
                { "currentTransactions",                JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_current_transactions),                   JSON_MANDATORY },
                { "totalTransactions",                  JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_transactions_total),                     JSON_MANDATORY },
                { "totalTimeouts",                      JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_timeouts_total),                         JSON_MANDATORY},
                { "totalTimeoutsServedStale",           JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_timeouts_served_stale_total),            JSON_MANDATORY },
                { "totalFailedResponses",               JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_failure_responses_total),               JSON_MANDATORY },
                { "totalFailedResponsesServedStale",    JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_failure_responses_served_stale_total),  JSON_MANDATORY },
                { "currentCacheSize",                   JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, cache_size),                               JSON_MANDATORY },
                { "cacheHits",                          JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_cache_hit),                              JSON_MANDATORY },
                { "cacheMisses",                        JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_cache_miss),                             JSON_MANDATORY},
                { "dnssecSecureVerdicts",               JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_dnssec_secure),                          JSON_MANDATORY },
                { "dnssecInsecureVerdicts",             JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_dnssec_insecure),                        JSON_MANDATORY },
                { "dnssecBogusVerdicts",                JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_dnssec_bogus),                           JSON_MANDATORY},
                { "dnssecIndeterminateVerdicts",        JSON_VARIANT_UNSIGNED,    json_dispatch_uint64,  offsetof(struct statistics, n_dnssec_indeterminate),                   JSON_MANDATORY },
                {},
        };

        r = json_dispatch(s, statistics_dispatch_table, NULL, JSON_LOG, &statistics);
        if (r < 0)
                return r;
        ...
        r = table_add_many(table,
                           ...
                           TABLE_UINT64, statistics.n_current_transactions,
                           TABLE_SET_ALIGN_PERCENT, 100,
                           TABLE_FIELD, "Total Transactions",
                           TABLE_UINT64, statistics.n_transactions_total,
                           ...
                           TABLE_SET_ALIGN_PERCENT, 100,
                           TABLE_UINT64, statistics.cache_size,
                           TABLE_FIELD, "Cache Hits",
                           TABLE_UINT64, statistics.n_cache_hit,
                           TABLE_FIELD, "Cache Misses",
                           TABLE_UINT64, statistics.n_cache_miss,
                           TABLE_EMPTY, TABLE_EMPTY,
                           ...
                           TABLE_SET_ALIGN_PERCENT, 100,
                           TABLE_UINT64, statistics.n_timeouts_total,
                           TABLE_FIELD, "Total Timeouts (Stale Data Served)",
                           TABLE_UINT64, statistics.n_timeouts_served_stale_total,
                           TABLE_FIELD, "Total Failure Responses",
                           TABLE_UINT64, statistics.n_failure_responses_total,
                           TABLE_FIELD, "Total Failure Responses (Stale Data Served)",
                           TABLE_UINT64, statistics.n_failure_responses_served_stale_total,
                           TABLE_EMPTY, TABLE_EMPTY,
                           ...
                           TABLE_FIELD, "Secure",
                           TABLE_SET_ALIGN_PERCENT, 100,
                           TABLE_UINT64, statistics.n_dnssec_secure,
                           TABLE_FIELD, "Insecure",
                           TABLE_UINT64, statistics.n_dnssec_insecure,
                           TABLE_FIELD, "Bogus",
                           TABLE_UINT64, statistics.n_dnssec_bogus,
                           TABLE_FIELD, "Indeterminate",
                           TABLE_UINT64, statistics.n_dnssec_indeterminate);
        if (r < 0)
...
static int dump_server_state(JsonVariant *server) {
        ...
        if (asprintf(&buf, "Server: %s ", server_state.server_name) < 0)
                return log_oom();
