...
static int verb_discover(int argc, char *argv[], void *userdata) {
        _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
        _cleanup_(sd_bus_message_unrefp) sd_bus_message *reply = NULL;
        _cleanup_(table_unrefp) Table *table = NULL;
        sd_bus *bus = userdata;
        ...
        r = bus_call_method(bus, bus_resolve_mgr, "Discover", &error, &reply, NULL);
        if (r < 0)
...
static int native_main(int argc, char *argv[], sd_bus *bus) {
        ...
        static const Verb verbs[] = {
                ...
                { "discover",              VERB_ANY, VERB_ANY, 0,            verb_discover         },
                {}
        };
        /* clang-format on */
