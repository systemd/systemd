...
static int varlink_call_link_method(const char *method, sd_json_variant *parameters) {
        ...
        if (streq_ptr(error_id, "io.systemd.Resolve.LinkIsLoopback"))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Link %s is loopback device", FORMAT_IFNAME(arg_ifindex));

        if (streq_ptr(error_id, "io.systemd.Resolve.LinkIsManaged")) {
                /* Link is managed by systemd-networkd, make the call there instead. */
                sd_varlink_unref(vl);
