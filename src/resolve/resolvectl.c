...
static int show_statistics(int argc, char **argv, void *userdata) {
        ...
        JsonVariant *reply = NULL, *stats = NULL, *t = NULL, *c = NULL, *d = NULL;
        ...
        t = json_variant_by_key(stats, "transactions");
        if (!t)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                       "statistics object is missing 'transactions' key.");

        c = json_variant_by_key(stats, "cache");
        if (!c)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                       "statistics object is missing 'cache' key.");

        d = json_variant_by_key(stats, "dnssec");
        if (!d)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                       "statistics object is missing 'dnssec' key.");
        ...
        r = json_dispatch(t, transactions_dispatch_table, NULL, JSON_LOG, &transactions);
        if (r < 0)
                return r;
        ...
        r = json_dispatch(c, cache_dispatch_table, NULL, JSON_LOG, &cache);
        if (r < 0)
                return r;
        ...
        r = json_dispatch(d, dnssec_dispatch_table, NULL, JSON_LOG, &dnsssec);
        if (r < 0)
                return r;
