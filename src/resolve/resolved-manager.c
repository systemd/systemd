...
static int on_mdns_goodbye_retransmit(sd_event_source *s, usec_t usec, void *userdata) {
        ...
        /* The grace second let a pending runtime withdrawal reach its own retransmission on
         * schedule; anything armed for after this point would be lost to the exit below, so flush
         * whatever is left. */
...
void manager_arm_mdns_withdrawal_retransmit(Manager *m) {
        ...
        /* Section 8.3's second is a minimum, so a batch queued while the timer already runs pushes
         * the deadline out rather than riding the pending one: retransmitting it moments after its
         * own first transmission would satisfy the count and not the interval. Pushing out is capped
         * at one further delay, so withdrawals arriving faster than once a second cannot defer the
         * retransmission indefinitely with the queued sets growing all the while; past the cap the
         * whole queue goes out together, some of it earlier than its own second, which the RFC
         * permits far more readily than not going out at all. */
        usec_t n, deadline;
        ...
        /* MAX(n, ...): a cap that has already passed — the loop ran late, or withdrawals kept
         * arriving — must not arm a deadline in the past, which would dispatch on the next
         * iteration and retransmit this batch immediately, the very thing being avoided. */
...
static int manager_dispatch_exit_signal(
         ...
         * withdrawal still waiting for its retransmission would lose it — send it now. Its two
         * transmissions end up closer together than the second RFC 6762 § 8.3 asks for, which is
         * the lesser departure: the alternative is a withdrawal that goes out once. */
