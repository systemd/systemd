...
static void manager_send_mdns_goodbyes(Manager *m) {
        ...
        m->n_mdns_goodbyes_sent++;
}

static int manager_mdns_goodbye_retransmit_handler(sd_event_source *s, uint64_t usec, void *userdata) {
        Manager *m = ASSERT_PTR(userdata);
...
static int manager_dispatch_exit_signal(sd_event_source *s, const struct signalfd_siginfo *si, void *userdata) {
        ...
        r = sd_event_add_time_relative(
                        ...
                        USEC_PER_SEC,
                        /* accuracy= */ 0,
                        manager_mdns_goodbye_retransmit_handler,
                        m);
        if (r < 0) {
