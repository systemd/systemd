...
void send_dns_notification(Manager *m, DnsAnswer *answer, const char *query_name) {
        ...
        if (!set_size(m->varlink_subscription))
                return;
        ...
        DNS_ANSWER_FOREACH_IFINDEX(rr, ifindex, answer) {
                ...
                if (r < 0) {
                        log_debug_errno(r, "Failed to append notification entry to array: %m");
                        return;
                }
        }
        ...
        if (r < 0) {
                log_debug_errno(r, "Failed to normalize query name: %m");
                return;
        }
...
bool manager_monitor_is_enabled(Manager *m) {
        assert(m);
        return m->enable_varlink_notifications;
}
