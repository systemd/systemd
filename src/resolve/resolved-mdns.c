...
static int on_mdns_packet(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        if (dns_packet_validate_reply(p) > 0) {
                 ...
                 * what is left may be an unrelated record hours out. Arming for those is useless at
                 * best, and since an armed timer is left alone, a far deadline would keep the next
                 * goodbye from arming a useful one at all — an empty cache reports USEC_INFINITY. */
                if (goodbye) {
