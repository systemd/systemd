...
static int mdns_goodbye_callback(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        /* Keep going for as long as something expires within the next second — the window the receipt
         * site arms per RFC 6762 section 10.1 — re-arming right at that expiry rather than a flat
         * second out, so the prune that drops the record (and tells the browsers) runs when the record
         * actually expires. Measure against the current time: 'usec' is the deadline this firing was
         * scheduled for, and it lags the clock by up to the timer's accuracy window. */
        until = dns_cache_next_expiry(&scope->cache);
        if (until <= usec_add(now(CLOCK_BOOTTIME), USEC_PER_SEC)) {
                r = sd_event_add_time(
                                scope->manager->event,
