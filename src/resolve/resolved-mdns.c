                        ...
                        if (rr->ttl == 0) {
                                ...
                                if (!scope->mdns_goodbye_event_source) {
                                        ...
                                        if (r < 0)
                                                 ...
                                                 * just above: a negative return from this io handler
                                                 * makes sd-event disable the mDNS socket for good,
                                                 * and this path is reached from untrusted multicast
                                                 * under memory pressure. Without the timer the
                                                 * records still age out on their rewritten TTL. */
                                                log_warning_errno(
                                                        r,
