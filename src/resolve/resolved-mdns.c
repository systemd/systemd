 ...
 * exists to service. USEC_INFINITY when nothing does — including for an empty cache, which is what
 * dns_cache_next_expiry() reports for one. Measured against the current time, not against the
 * deadline a firing was scheduled for: that one lags the clock by up to the timer's accuracy. */
static usec_t mdns_goodbye_next_deadline(DnsScope *scope) {
        usec_t until;
 ...
 * re-arming itself). */
static int mdns_goodbye_arm(DnsScope *scope, usec_t until, bool force_reset) {
        int r;
        ...
        r = event_reset_time(scope->manager->event,
                             ...
                             force_reset);
        if (r < 0)
                return log_warning_errno(r, "mDNS: Failed to arm goodbye timer, ignoring: %m");
...
int mdns_goodbye_callback(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        until = mdns_goodbye_next_deadline(scope);
        if (until == USEC_INFINITY)
                ...
                (void) mdns_goodbye_arm(scope, until, /* force_reset= */ true);
 ...
 * is left may be an unrelated record hours out, which an empty cache reports as USEC_INFINITY. The
 ...
 *
 * A timer already armed for an earlier deadline keeps it: that pass covers this goodbye too, and the
 * callback re-arms for whatever is left. What a goodbye flood — unauthenticated multicast, after all
 * — can ask for is therefore bounded by the distinct expiry instants that fall inside the window and
...
void mdns_goodbye_arm_on_receipt(DnsScope *scope) {
        ...
        until = mdns_goodbye_next_deadline(scope);
        if (until == USEC_INFINITY)
                until = usec_add(now(CLOCK_BOOTTIME), MDNS_GOODBYE_DELAY);

        (void) mdns_goodbye_arm(scope, until, /* force_reset= */ false);
}
