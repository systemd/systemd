...
/* Arm the goodbye timer for the soonest expiry the cache holds, if that falls inside the RFC 6762
 * section 10.1 window this timer exists to service. Returns true when it armed. force_reset picks
 * whether an already armed timer keeps its deadline (the receipt path: first one wins) or takes the
...
static int mdns_goodbye_callback(sd_event_source *s, uint64_t usec, void *userdata) {
         ...
         * the browsers) runs when the record actually expires. The helper measures against the
         * current time: 'usec' is the deadline this firing was scheduled for, and it lags the clock
         * by up to the timer's accuracy window. force_reset, because the source is disabled by the
         * time a real dispatch gets here, but a caller invoking this handler directly still has it
         * armed and the new deadline must win either way. */
        if (!mdns_goodbye_arm(scope, /* force_reset= */ true))
...
static int on_mdns_packet(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        if (dns_packet_validate_reply(p) > 0) {
                ...
                /* A goodbye leaves its records one second from expiry (RFC 6762 § 10.1): look at the
                 * cache when they actually come due and remove them then, which is what keeps service
                 * browsers informed of a removal — there is no other reliable trigger for that. Arm
                 * against the expiry the put just computed rather than a second from the start of this
                 * event loop iteration, which is always a little earlier and would fire a pass that
                 * prunes nothing. An already armed timer is left alone; the callback re-arms itself for
                 * as long as records keep coming due.
                 *
                 * The same window the callback applies gates the arming, because the put need not have
                 * left anything due behind: a goodbye for a record we do not hold is not cached at all
                 * (dns_cache_put_positive() declines TTL<=1 for a record it does not already have), a
                 * cache-flush goodbye drops its record outright, and caching may be off entirely — and
                 * what is left may be an unrelated record hours out. Arming for those would be
                 * pointless, and since an armed timer is left alone, a far deadline would keep the
                 * next goodbye from arming a useful one at all — an empty cache reports
                 * USEC_INFINITY. The subscribers still have to hear about it, though: where the
                 * records went away with the goodbye itself there is nothing to wait for, so tell
                 * the browsers now instead of at a deadline that will never come. */
                /* force_reset= false: a timer already armed for an earlier deadline keeps it —
                 * that pass covers this goodbye too, and the callback re-arms for whatever is left.
                 *
                 * With nothing due there is no timer to coalesce behind, so tell the browsers
                 * directly — but at most once per goodbye window, since the reconciliation walks
                 * every matching scope's cache and the whole discovered-service list, and the
                 * goodbyes driving it are unauthenticated multicast. */
                if (goodbye && !mdns_goodbye_arm(scope, /* force_reset= */ false) &&
                    ratelimit_below(&scope->mdns_goodbye_notify_ratelimit)) {
                        r = mdns_notify_browsers_goodbye(scope);
                        if (r < 0)
                                log_warning_errno(r, "mDNS: Failed to notify service subscribers of goodbyes, ignoring: %m");
                }
