...
static bool mdns_should_reply_using_unicast(DnsPacket *p) {
        ...
        /* The query had a QU bit set, RFC 6762, section 5.4 */
        if (p->wants_unicast_reply)
                return true;
        ...
        switch (p->family) {
        case AF_INET:
                if (!in_addr_equal(AF_INET, &p->destination, (union in_addr_union *) &MDNS_MULTICAST_IPV4_ADDRESS))
                        return true;
                break;
        case AF_INET6:
                if (!in_addr_equal(AF_INET6, &p->destination, (union in_addr_union *) &MDNS_MULTICAST_IPV6_ADDRESS))
                        return true;
                break;
        }
