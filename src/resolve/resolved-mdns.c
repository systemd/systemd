...
/* force_reset picks whether an already armed timer keeps its deadline (the receipt path) or takes
 * the new one (the callback: it has just fired and is re-arming itself).
 *
 * Keeping it is safe, but not because the armed deadline is always the earlier one — the callback's
 * floor below can leave it up to a quarter window later than what a newly arrived goodbye computes.
 * What holds is the bound: both arm sites cap the deadline at now + MDNS_GOODBYE_DELAY, and now only
 * moves forward, so an armed deadline is never past t_receipt + MDNS_GOODBYE_DELAY — exactly the
 * point by which a goodbye received at t_receipt needs its pass. */
static void mdns_goodbye_arm(DnsScope *scope, usec_t until, bool force_reset) {
        ...
        if (r < 0) {
                ...
                /* Release it rather than leaving a disabled source behind: the next goodbye then
                 * arms a fresh one instead of depending on event_reset_time() happening to check
                 * enabledness before anything else. */
                scope->mdns_goodbye_event_source =
...
static int mdns_goodbye_callback(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        if (until == USEC_INFINITY)
                ...
                /* ...but not arbitrarily soon. Each pass prunes and then reconciles every browser
                 * against every matching scope's cache, and the records driving it arrive as
                 * unauthenticated multicast: goodbyes a millisecond apart for distinct records give
                 * expiries a millisecond apart, and this would chain a pass per record. sd-event does
                 * not save us — process_timer() marks a source pending on its deadline alone, and the
                 * flood keeps the loop awake anyway, so the accuracy window only shifts when the
                 * timerfd is armed. A quarter-window floor bounds the passes at four per second, at
                 * the cost of a removal landing up to a quarter window past the § 10.1 second rather
                 * than inside it — 25% over, for records whose expiries fall closer together than
                 * that. Records withdrawn together share one put timestamp and are pruned in one
                 * pass, so the ordinary "withdraw a service" case is unaffected. */
                mdns_goodbye_arm(scope,
                                 MAX(until, usec_add(t, MDNS_GOODBYE_DELAY / 4)),
                                 /* force_reset= */ true);

        return 0;
}

/* A goodbye leaves its records one second from expiry (RFC 6762 § 10.1): look at the cache when they
 * actually come due and remove them then, which is what keeps service browsers informed of a removal
 * — there is no other reliable trigger for that. Arm against the expiry the put just computed rather
 * than a second from the start of this event loop iteration, which is always a little earlier and
 * would fire a pass that prunes nothing.
 *
 * The put need not have left anything due behind, though: a goodbye for a record we do not hold is
 * not cached at all (dns_cache_put_positive() declines TTL<=1 for a record it does not already
 * have), a cache-flush goodbye drops its record outright, and caching may be off entirely — and what
 * is left may be an unrelated record hours out, which the window check rejects just the same. The
 * subscribers still have to hear about it, so fall back to the far end of the window: the
 * reconciliation the callback runs is what tells them, and it has to happen whether or not a record
 * is waiting to expire.
 *
 * An already armed timer keeps its deadline: it is never past this goodbye's own window (see
