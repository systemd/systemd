...
static int mdns_goodbye_callback(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        /* The release path acts on scope->mdns_goodbye_event_source on the assumption that it is
         * this very source. Now that the handler is callable from the test, say so. */
         ...
         * the browsers) runs when the record actually expires. force_reset, because a caller
         * invoking this handler directly still has the source armed; a real dispatch has already
         * disabled it, which would be reset anyway. */
