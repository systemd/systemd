...
void mdns_announcement_max_sizes(
        ...
        if (link_mtu > header_size + DNS_PACKET_HEADER_SIZE)
                ...
                /* No bound to derive: the MTU has not been read yet (0), or is small enough that
                 * subtracting the headers would underflow, or would leave less than the DNS header
                 * dns_packet_new() asserts room for. The section 17
                 ...
                 * same section allows for a single record only. Fall back to the conservative
                 * bound instead: the datagram size every host of the family must be able to accept
                 * (RFC 791 section 3.1 for IPv4, RFC 8200 section 5's minimum link MTU for IPv6,
                 * which additionally guarantees no link fragments one that size). */
                max_size = (family == AF_INET6 ? IPV6_MIN_MTU : MDNS_PACKET_UNKNOWN_MTU_IPV4) - header_size;

        /* An MTU only just above that is kept, even though no record fits the bound it yields: the
         * records then go out one per packet, each larger than the MTU and so fragmented, which is
         * exactly what section 17 asks for -- "MUST NOT contain more than one resource record".
         * Clamping such a bound up to the family minimum would send fewer datagrams, but each would
         * carry several records and still be fragmented by the link, which that sentence forbids. */
