...
DnsServerPolicy dns_scope_get_dns_server_policy(DnsScope *s) {
        ...
        /* Check for per-link/delegate policy first, fall back to manager policy */
        if (s->link) {
                assert(!s->delegate);
                if (s->link->dns_server_policy != _DNS_SERVER_POLICY_INVALID)
                        return s->link->dns_server_policy;
        } else if (s->delegate) {
                if (s->delegate->dns_server_policy != _DNS_SERVER_POLICY_INVALID)
                        return s->delegate->dns_server_policy;
        }
