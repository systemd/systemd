...
static int dns_scope_emit_announcement(DnsScope *scope, DnsAnswer *answer) {
         ...
         * much as the lone oversized-RR fallback below. */
...
int dns_scope_announce(DnsScope *scope, bool goodbye) {
        ...
        /* Calculate answer's size. A goodbye also covers items under re-verification: only initial
         * establishment goes through probing, so a VERIFYING item was announced before, and a
         * re-verification in flight — e.g. right after a configuration reload — must not exempt it
         * from withdrawal. */
        HASHMAP_FOREACH(z, scope->zone.by_key) {
                if (goodbye ? !IN_SET(z->state, DNS_ZONE_ITEM_ESTABLISHED, DNS_ZONE_ITEM_VERIFYING)
                            : z->state != DNS_ZONE_ITEM_ESTABLISHED)
                        continue;
        ...
        HASHMAP_FOREACH(z, scope->zone.by_key)
                LIST_FOREACH (by_key, i, z) {
                        ...
                        if (goodbye ? !IN_SET(i->state, DNS_ZONE_ITEM_ESTABLISHED, DNS_ZONE_ITEM_VERIFYING)
