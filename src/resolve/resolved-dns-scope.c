...
int dns_scope_emit_announcement(DnsScope *scope, DnsAnswer *answer) {
        _cleanup_free_ DnsPacket **packets = NULL;
        size_t n_packets = 0, max_size = DNS_PACKET_SIZE_MAX, fragmented_max;
        int r, ret = 0;
         ...
         * much as the lone oversized-RR fallback. A fragmented IPv6 datagram additionally carries
         * an 8-byte Fragment extension header (RFC 8200 § 4.5) that counts against the ceiling. */
...
static bool dns_zone_item_wants_announce(DnsZoneItem *i, bool goodbye, bool exclude_host_records) {
        assert(i);
         ...
         * unregistration) are not affected. The exclusion is a parameter rather than the manager's
...
bool dns_scope_goodbye_has_content(DnsScope *scope) {
        ...
        HASHMAP_FOREACH(z, scope->zone.by_key)
                LIST_FOREACH(by_key, i, z)
                        if (dns_zone_item_wants_announce(i, /* goodbye= */ true, /* exclude_host_records= */ true))
                                return true;
...
int dns_scope_withdraw_rrs(DnsScope *scope, DnsAnswer *candidates) {
        ...
        DNS_ANSWER_FOREACH(rr, candidates) {
                ...
                if (i && dns_zone_item_wants_announce(i, /* goodbye= */ true, /* exclude_host_records= */ false)) {
...
int dns_scope_announce(DnsScope *scope, bool goodbye) {
        ...
        HASHMAP_FOREACH(z, scope->zone.by_key) {
                if (!dns_zone_item_wants_announce(z, goodbye, dns_scope_mdns_withdrawing(scope)))
                        continue;
                 ...
                 * marks the item WITHDRAWN, which dns_zone_item_wants_announce() then rejects, so
        ...
        HASHMAP_FOREACH(z, scope->zone.by_key)
                LIST_FOREACH (by_key, i, z) {
                        ...
                        if (!dns_zone_item_wants_announce(i, goodbye, dns_scope_mdns_withdrawing(scope)))
                                continue;
