...
void dns_scope_check_conflicts(DnsScope *scope, DnsPacket *p) {
        ...
        /* On the way out, once the mDNS goodbyes went out, the withdrawal is to stand: reacting to
         * a conflicting claim means re-verifying, which flips records back to probing and
...
static int dns_scope_make_announcement_packet(DnsScope *scope, size_t max_size, DnsPacket **ret) {
        ...
        r = dns_packet_new(&p, scope->protocol, /* min_alloc_dsize= */ 0, max_size);
        if (r < 0)
                return r;
...
static int dns_scope_emit_announcement(DnsScope *scope, DnsAnswer *answer) {
         ...
         * it caps the MTU-derived packet size on jumbo-frame links just as much as the lone
         * oversized-RR fallback below. A fragmented IPv6 datagram additionally carries an 8-byte
         * Fragment extension header (RFC 8200 § 4.5) that counts against the ceiling. */
        fragmented_max = MDNS_PACKET_FRAGMENTED_SIZE_MAX - udp_header_size(scope->family) -
                (scope->family == AF_INET6 ? sizeof(struct ip6_frag) : 0);
        ...
        if (scope->link && scope->link->mtu > udp_header_size(scope->family) + DNS_PACKET_HEADER_SIZE)
                ...
                /* No usable MTU — no link on the scope, or IFLA_MTU not read yet. The § 17 ceiling
                 * is the wrong fallback: it is the bound for a datagram deliberately fragmented to
                 * carry one oversized record, and packing an announcement to it would put dozens of
                 * records into a datagram that any ordinary link fragments, which the same section
                 * allows for a single record only. Assume the smallest datagram a host of this
                 * family must accept unfragmented instead. */
                max_size = (scope->family == AF_INET6 ? MDNS_PACKET_UNKNOWN_MTU_IPV6 :
                            MDNS_PACKET_UNKNOWN_MTU_IPV4) - udp_header_size(scope->family);
