...
void dns_scope_check_conflicts(DnsScope *scope, DnsPacket *p) {
         ...
         * (mdns_scope_process_query() answers for them throughout, RFC 6762 section 8.1). mDNS
         * alone, per the helper: LLMNR records are not withdrawn by the goodbyes, so its conflict
...
static int dns_scope_emit_announcement(DnsScope *scope, DnsAnswer *answer) {
         ...
         * packets of the announcement. Each packet takes its own token from the scope's multicast
         * rate limit -- that limit bounds datagrams on the wire, and an announcement that would
         * exhaust its burst is the very flood it exists to cap -- so a packet refused there is
