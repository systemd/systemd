...
int dns_scope_new(
        ...
        *s = (DnsScope) {
                ...
                /* A little over the single-querier sustained tier, so a handful of distinct browse
                 * questions on one link can still each be rescued, while an unbounded fan-out
                 * cannot multiply one received packet into arbitrarily many multicasts. */
                .goodbye_rescue_ratelimit = { 5 * USEC_PER_MINUTE, 12 },
        };
...
DnsScope* dns_scope_free(DnsScope *s) {
        ...
        /* Reconcile the browse subscriptions against the caches that are left — after the unlink
         * above, so the reconciliation never walks this half-destroyed scope. Only an mDNS scope's
         * cache feeds it; a unicast or LLMNR scope going away is none of its business. */
