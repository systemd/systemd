...
static int dns_scope_emit_announcement(DnsScope *scope, DnsAnswer *answer) {
        ...
        if (r < 0)
                return r;
        if (r > 0)
                /* Worth more than the debug line the packetizer leaves: for a withdrawal the caller
                 * drops these records from the zone regardless, so one skipped here is never
                 * withdrawn at all and peers keep it for its full TTL. */
