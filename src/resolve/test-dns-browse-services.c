...
/* Build a PTR under the same browsed type but naming a different instance -- what a goodbye for
 * something this querier never discovered looks like. */
static DnsResourceRecord *new_other_service_rr(uint32_t ttl) {
        DnsResourceRecord *rr;
        ...
        rr->ttl = ttl;
        rr->ptr.name = strdup("Other Service._http._tcp.local");
        ASSERT_NOT_NULL(rr->ptr.name);
...
TEST(mdns_goodbyes_hit_discovered_matches_only_held_instances) {
        ...
        ASSERT_NOT_NULL(other = new_other_service_rr(120));
        ...
        ASSERT_TRUE(mdns_goodbyes_hit_discovered(&sq, hit));
}
