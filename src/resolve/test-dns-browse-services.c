...
static void service_fixture_done(ServiceFixture *f) {
        ...
        /* Services added by dns_add_new_service() are linked into the browser. Unlink them before
         * dropping the reference, so that freeing them does not detach them from it once more. */
        LIST_FOREACH(dns_services, service, f->sb.dns_services) {
                LIST_REMOVE(dns_services, f->sb.dns_services, service);
