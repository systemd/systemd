...
static int dnssec_rrset_verify_sig(
        ...
        default:
                ...
                /* If the signature algorithm is supported by sd-resolved but disabled by host polilcy,
...
int dnssec_verify_dnskey_by_ds(DnsResourceRecord *dnskey, DnsResourceRecord *ds, bool mask_revoke) {
        ...
        /* If the digest is supported by sd-resolved but disabled by host policy, also return -EOPNOTSUPP
         */
        if (EVP_DigestInit_ex(ctx, md_algorithm, NULL) <= 0)
                return -EOPNOTSUPP;
