...
bool mdns_service_contains(mDnsServices *services, DnsResourceRecord *rr, int owner_family) {
        _cleanup_(mdns_service_unrefp) mDnsServices *service = NULL;
...
void mdns_manage_services_answer(mDnsServiceSubscriber *ss, DnsAnswer *answer, int owner_family) {
        _cleanup_(dns_resource_record_unrefp) DnsResourceRecord *i = NULL;
        _cleanup_(mdns_service_unrefp) mDnsServices *service = NULL;
...
static void mdns_browse_service_query_complete(DnsQuery *q) {
        ...
        ss = mdns_service_subscriber_ref(q->manager->dns_service_subscriber);

        if (!ss)
                goto finish;

        r = dns_answer_match_key(q->answer, ss->key, NULL);
        if (r <= 0)
                goto finish;

        mdns_subscriber_lookup_cache(ss, q->answer_family);
...
int mdns_subscribe_browse_service(
        ...
        *ss = (mDnsServiceSubscriber) {
        ...
        r = sd_event_add_time(m->event,
                        &ss->schedule_event,
                        CLOCK_MONOTONIC,
                        now(CLOCK_MONOTONIC) + (ss->delay * USEC_PER_SEC),
                        0,
                        mdns_next_query_schedule,
                        ss);
...
mDnsServiceSubscriber *mdns_service_subscriber_free(mDnsServiceSubscriber *ss) {
        ...
        while (ss->mdns_services)
                mdns_remove_service(ss, ss->mdns_services);
