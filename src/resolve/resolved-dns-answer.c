...
DnsAnswer *dns_answer_new(size_t n) {
        ...
        if (ordered_set_reserve(s, n * 2) < 0)
                return NULL;
...
int dns_answer_reserve(DnsAnswer **a, size_t n_free) {
        ...
        r = ordered_set_reserve((*a)->items, ns);
        if (r < 0)
                return r;
...
int dns_answer_reserve_or_clone(DnsAnswer **a, size_t n_free) {
        ...
        if (!*a || (*a)->n_ref <= 1)
                return dns_answer_reserve(a, n_free);
