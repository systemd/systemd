 ...
 * a compression pointer 11, and reserves 01 and 10: it takes *both* bits to make a pointer, so a
 * reserved type must be rejected rather than followed as one with an offset built from it. */
...
/* The invariant the append-side assert relies on: every mapped offset is pointer-expressible. */
 ...
 * the next append starts at a chosen offset. Takes, and leaves, an empty compression map. */
static void pad_packet_to(DnsPacket *p, size_t target) {
        while (p->size < target) {
                size_t gap = target - p->size;

                /* A filler consumes at least 3 bytes: length octet, one label byte, root byte. No
                 * target used here leaves a smaller gap, and a future one that did would fail in
                 * the arithmetic below (gap == 1 underflows it) or in its own assert rather than
                 * here, where the precondition is stated. */
                ASSERT_GE(gap, 3u);
...
TEST(packet_append_rr_beyond_compression_pointer_range) {
        ...
        while (packet->size <= DNS_COMPRESSION_OFFSET_MAX) {
...
/* The rollback path: a failed append must not leave a compression entry behind that points into
 * truncated space. A packet with room for the labels but not for the root byte reaches it -- with
 * one name, and with two. */
TEST(packet_append_name_rolls_back_on_failure) {
        ...
        /* The same failure -- the root byte, which is the append this commit reroutes -- but with
         * two labels, so more than one compression entry has to come back out: "a" and "b" fill
         * the packet exactly, and the root byte fails with the map holding "a.b" and "b". A name
         * whose *label* append fails would not do: that path took the rollback before this commit
