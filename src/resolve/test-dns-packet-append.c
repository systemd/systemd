...
TEST(packet_append_name_beyond_compression_pointer_range) {
        ...
        for (size_t i = 0; i < 2; i++) {
                ...
                unsigned pointer = (unsigned) DNS_PACKET_DATA(packet)[pointer_at] << 8 |
                        DNS_PACKET_DATA(packet)[pointer_at + 1];
                ASSERT_EQ(pointer, (unsigned) (DNS_PACKET_COMPRESSION_POINTER_FLAG | suffix));
