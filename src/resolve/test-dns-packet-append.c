...
static void pad_packet_to(DnsPacket *p, size_t target) {
        while (p->size < target) {
                ...
                ASSERT_OK(dns_packet_append_name(p, label, /* allow_compression= */ false, /* canonical_candidate= */ false, NULL));
        }
...
TEST(packet_append_name_compression_offset_boundary) {
        ...
        ASSERT_OK(dns_packet_append_name(packet, "edge.example.org", /* allow_compression= */ true, /* canonical_candidate= */ false, &start));
        ...
        size_t before = packet->size;
        ASSERT_OK(dns_packet_append_name(packet, "edge.example.org", /* allow_compression= */ true, /* canonical_candidate= */ false, NULL));
        ASSERT_EQ(packet->size, before + 2);
        ...
        ASSERT_OK(dns_packet_append_name(packet, "past.example.org", /* allow_compression= */ true, /* canonical_candidate= */ false, &start));
        ASSERT_EQ(start, (size_t) DNS_PACKET_COMPRESSION_OFFSET_MAX + 1);
        ASSERT_EQ(hashmap_size(packet->names), 0u);

        before = packet->size;
        ASSERT_OK(dns_packet_append_name(packet, "past.example.org", /* allow_compression= */ true, /* canonical_candidate= */ false, NULL));
