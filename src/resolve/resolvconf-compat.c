...
static int resolvconf_help(void) {
        ...
        printf("%1$s -a INTERFACE < FILE\n"
               ...
               "  -p            Make the interface not used as the default route\n"
...
int resolvconf_parse_argv(int argc, char *argv[]) {
        ...
        if (arg_mode == MODE_SET_LINK) {
                ...
                if (!arg_set_dns || !arg_set_domain || arg_set_default_route < 0) {
                        log_debug("Neither DNS servers nor domains specified. There is nothing we need to do.");
                        return 0;
                }
        }
