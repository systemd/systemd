...
static int dnssd_registered_service_collect_withdraw_rrs(DnssdRegisteredService *s, DnsAnswer **answer) {
        ...
        r = mdns_enumeration_service_ptr_new(service_name, &enumeration_rr);
        if (r < 0)
                return r;

        r = dns_answer_add_extend(answer, enumeration_rr, 0, 0, NULL);
        if (r < 0)
                return r;
        ...
        FOREACH_ARGUMENT(ptr, s->ptr_rr, s->sub_ptr_rr) {
                ...
                r = dns_answer_add_extend(answer, ptr, 0, 0, NULL);
                if (r < 0)
                        return r;
        }

        if (s->srv_rr) {
                r = dns_answer_add_extend(answer, s->srv_rr, 0, 0, NULL);
                if (r < 0)
                        return r;
        }
        ...
        LIST_FOREACH(items, txt_data, s->txt_data_items) {
                ...
                r = dns_answer_add_extend(answer, txt_data->rr, 0, 0, NULL);
                if (r < 0)
                        return r;
        }
...
static void dnssd_withdraw_rrs(Manager *m, DnsAnswer *answer) {
        ...
        HASHMAP_FOREACH(l, m->links)
                FOREACH_ARGUMENT(scope, l->mdns_ipv4_scope, l->mdns_ipv6_scope) {
                        _cleanup_(dns_answer_unrefp) DnsAnswer *emitted = NULL;
                        DnsAnswerItem *item;

                        if (!scope)
                                continue;

                        r = dns_scope_withdraw_rrs(scope, answer, &emitted);
                        if (r < 0)
                                log_warning_errno(r, "Failed to withdraw mDNS records, ignoring: %m");

                        DNS_ANSWER_FOREACH_ITEM(item, emitted) {
                                r = dns_answer_add_extend_full(&m->mdns_pending_withdrawals, item->rr,
                                                               item->ifindex, item->flags, item->rrsig, item->until);
                                if (r < 0) {
                                        log_warning_errno(r, "Failed to queue mDNS withdrawal retransmission, ignoring: %m");
                                        break;
                                }
                        }
                }

        if (!dns_answer_isempty(m->mdns_pending_withdrawals))
                (void) manager_mdns_arm_withdrawal_retransmit(m);
}
...
static int dnssd_withdraw_filtered(Manager *m, DnsAnswer *candidates, DnssdRegisteredService *except) {
        ...
        DNS_ANSWER_FOREACH_ITEM(item, candidates) {
                if (dns_answer_contains(published, item->rr))
                        continue;
...
/* Allocation-free fallback: whatever else fails, the service's records must leave the zones — or
...
/* The reload entry point: withdraw the snapshotted records that did not survive the reload. */
int dnssd_withdraw_stale_rrs(Manager *m, DnsAnswer *old_rrs) {
        assert(m);

        return dnssd_withdraw_filtered(m, old_rrs, /* except= */ NULL);
}
