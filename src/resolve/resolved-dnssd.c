...
static int dnssd_registered_service_make_goodbye(DnssdRegisteredService *service, DnsAnswer **ret) {
        ...
        LIST_FOREACH(items, txt_data, service->txt_data_items) {
                r = dns_answer_add(answer, txt_data->rr, 0, DNS_ANSWER_GOODBYE|DNS_ANSWER_CACHE_FLUSH, NULL);
                if (r < 0)
                        return r;
        }
...
DnssdRegisteredService *dnssd_registered_service_unregister(DnssdRegisteredService *service) {
        ...
        /* Sends goodbye messages for the service and removes its RRs from all mDNS zones, then frees the
         * service. */
