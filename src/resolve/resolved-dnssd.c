...
static int dnssd_registered_service_make_goodbye(DnssdRegisteredService *service, DnsScope *scope, DnsAnswer **ret) {
        ...
        /* Collects those of the service's RRs that are established in the scope's zone, flagged as goodbye
         * (TTL 0), so that only this service is withdrawn, and only where we actually asserted ownership of
         * the records, i.e. not while a record is still being probed for uniqueness, was withdrawn again
         * after a conflict, or was never published in this scope in the first place. */
        ...
        FOREACH_ARRAY(rr, ((DnsResourceRecord*[]) { service->ptr_rr, service->sub_ptr_rr, service->srv_rr }), 3) {
                ...
                if (!i || i->state != DNS_ZONE_ITEM_ESTABLISHED)
                        continue;
...
void dnssd_registered_service_unregister(DnssdRegisteredService *service) {
         ...
         * added back. */
        hashmap_remove(m->dnssd_registered_services, service->id);
