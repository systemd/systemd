...
#define MDNS_CACHE_FLUSH_GRACE_RR_MAX 64U
...
static void dns_cache_trim_key(DnsCache *c, DnsResourceKey *key, unsigned max) {
        DnsCacheItem *first;
        unsigned n = 0;
...
int dns_cache_put(
        ...
        DNS_ANSWER_FOREACH_ITEM(item, answer) {
                ...
                if (protocol == DNS_PROTOCOL_MDNS && !FLAGS_SET(item->flags, DNS_ANSWER_SHARED_OWNER))
                        dns_cache_trim_key(c, item->rr->key, MDNS_CACHE_FLUSH_GRACE_RR_MAX);
        }
