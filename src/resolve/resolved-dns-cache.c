...
static bool dns_cache_remove_by_key_with_mdns_grace(
                DnsCache *c,
                DnsResourceKey *key,
                usec_t timestamp) {

        DnsCacheItem *first, *kept = NULL;
        bool removed = false;
        ...
        LIST_FOREACH(by_key, i, first) {
                if (timestamp < i->received || timestamp - i->received >= MDNS_CACHE_FLUSH_GRACE_USEC) {
                        dns_cache_item_unlink_and_free(c, i);
                        removed = true;
                } else if (!kept)
                        kept = i;
        ...
        if (kept)
                assert_se(hashmap_replace(c->by_key, kept->key, kept) >= 0);
