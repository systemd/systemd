...
int link_set_dns_servers(Link *l, struct in_addr_full **dns, size_t n_dns, ResolveConfigSource source) {
        ...
        if (changed) {
                ...
                if (dns_list_str)
                        log_link_info(l, "%s client set DNS server list to: %s",
                                      source == RESOLVE_CONFIG_SOURCE_DBUS ? "Bus" : "Varlink", dns_list_str);
                else
                        log_link_info(l, "%s client reset DNS server list.",
                                      source == RESOLVE_CONFIG_SOURCE_DBUS ? "Bus" : "Varlink");
        }
...
int link_set_search_domains(Link *l, DomainParameters *domains, size_t n_domains, ResolveConfigSource source) {
        ...
        if (changed) {
                ...
                const char *source_str = source == RESOLVE_CONFIG_SOURCE_VARLINK ? "Varlink" : "Bus";
                if (domain_list)
