...
int dns_server_dump_state_to_json(DnsServer *server, JsonVariant **ret) {
        ...
        return json_build(ret,
                          JSON_BUILD_OBJECT(
                                        ...
                                        JSON_BUILD_PAIR_STRING("verified-feature-level", strna(dns_server_feature_level_to_string(server->verified_feature_level))),
                                        JSON_BUILD_PAIR_STRING("possible-feature-level", strna(dns_server_feature_level_to_string(server->possible_feature_level))),
                                        JSON_BUILD_PAIR_STRING("dnssec-mode", strna(dnssec_mode_to_string(dns_server_get_dnssec_mode(server)))),
                                        JSON_BUILD_PAIR_BOOLEAN("can-do-dnssec", dns_server_dnssec_supported(server)),
                                        JSON_BUILD_PAIR_UNSIGNED("max-udp-fragment-size", server->received_udp_fragment_max),
                                        JSON_BUILD_PAIR_UNSIGNED("failed-udp-attempts", server->n_failed_udp),
                                        JSON_BUILD_PAIR_UNSIGNED("failed-tcp-attempts", server->n_failed_tcp),
                                        JSON_BUILD_PAIR_BOOLEAN("seen-truncated-packet", server->packet_truncated),
                                        JSON_BUILD_PAIR_BOOLEAN("seen-opt-rr-getting-lost", server->packet_bad_opt),
                                        JSON_BUILD_PAIR_BOOLEAN("seen-rrsig-rr-missing", server->packet_rrsig_missing),
                                        JSON_BUILD_PAIR_BOOLEAN("seen-invalid-packet", server->packet_invalid),
                                        JSON_BUILD_PAIR_BOOLEAN("server-dropped-do-flag", server->packet_do_off)));
}
