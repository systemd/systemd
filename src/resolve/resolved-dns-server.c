        ...
        if (s->possible_feature_level < best && dns_server_grace_period_expired(s)) {
                ...
                if (s->n_failed_tcp >= DNS_SERVER_FEATURE_RETRY_ATTEMPTS &&
                    s->possible_feature_level == DNS_SERVER_FEATURE_LEVEL_TCP) {
                        ...
                        /* We lost too many UDP packets in a row, and are on a UDP feature level of UDP. If
                         * the packets are lost, maybe the server cannot parse them, hence downgrading sounds
...
int dns_server_adjust_opt(DnsServer *server, DnsPacket *packet, DnsServerFeatureLevel level) {
        ...
        if (level == DNS_SERVER_FEATURE_LEVEL_LARGE) {
                 ...
                 * reasons) – except we are talking to localhost (where the security considerations don't
