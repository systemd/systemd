...
typedef struct Hook {
        ...
        unsigned filter_labels_min;
        unsigned filter_labels_max;
...
static int dispatch_filter_domains(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        ...
        JSON_VARIANT_ARRAY_FOREACH(i, variant) {
                if (!sd_json_variant_is_string(i))
                        return json_log(variant, flags, SYNTHETIC_ERRNO(EINVAL), "Element of JSON field '%s' is not a string.", strna(name));
...
static int manager_hook_discover(Manager *m) {
         ...
         * hin (and where root ownership hence makes sense). Hence we do not nest the directories, but put
...
static int dispatch_answer(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        ...
        JSON_VARIANT_ARRAY_FOREACH(e, variant) {
                ...
                if (r < 0)
                        return json_log(variant, flags, SYNTHETIC_ERRNO(EINVAL), "JSON field '%s' contains an invalid resource record.", strna(name));
...
static int on_query_reply(
        ...
        HookQuery *q = qc->query; /* save early in case we destroy 'qc' half-way through this function */
        int r;
        ...
        if (p.rcode == DNS_RCODE_SUCCESS)
                ...
                win = qc->query->answer_rcode != DNS_RCODE_SUCCESS ||
                        dns_answer_isempty(qc->query->answer) ||
                        (!dns_answer_isempty(p.answer) &&
                         hook_compare(qc->hook, qc->query->answer_hook) < 0);
        else
                ...
                win = qc->query->answer_rcode < 0 ||
                hook_compare(qc->hook, qc->query->answer_hook) < 0;

        if (win) {
                /* This reply wins over whatever was stored before. Let's track that */
                dns_answer_unref(qc->query->answer);
                qc->query->answer = TAKE_PTR(p.answer);
                qc->query->answer_rcode = p.rcode;
                hook_unref(qc->query->answer_hook);
                qc->query->answer_hook = hook_ref(qc->hook);
        }

        hook_recycle_varlink(qc->hook, qc->link);
        qc->link = sd_varlink_unref(qc->link);

        hook_query_ready(qc->query);
        return 0;
...
static int hook_query_add_candidate(HookQuery *hq, Hook *h) {
        ...
        /* Set a short time-out for hooks. Hooks should not be able to cause the DNS part of the lookup to fail. */
...
int manager_hook_query(
        ...
        HASHMAP_FOREACH(h, m->hooks) {
                ...
                if (r < 0) {
                        log_warning_errno(r, "Failed to test if hook '%s' matches question, ignoring.", h->socket_path);
                        continue;
                }
                if (r == 0) {
