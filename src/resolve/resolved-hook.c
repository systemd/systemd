...
int manager_hook_query(
        ...
        HASHMAP_FOREACH(h, m->hooks) {
                ...
                if (r < 0) {
                        log_warning_errno(r, "Failed to test if hook '%s' matches question (%s), ignoring.", h->socket_path, dns_question_first_name(question_idna));
                        continue;
                }
                if (r == 0) {
                        r = hook_test_filter(h, question_utf8);
                        if (r < 0) {
                                log_warning_errno(r, "Failed to test if hook '%s' matches question (%s), ignoring.", h->socket_path, dns_question_first_name(question_utf8));
                                continue;
                        }
                        if (r == 0) {
