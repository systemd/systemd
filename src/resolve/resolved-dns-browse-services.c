...
/* The ladder is armed against the soonest-expiring instance across both address families, so every
 * revisit brings both back in sync — e.g. a completing maintenance query carries one family's answer,
 * but the other family's records were refreshed in the cache by their own transaction all the same.
 * Best-effort: the callers run where an error must not propagate, and mdns_querier_revisit_cache()
 * logs about all its failures itself, hence errors are simply ignored here. */
...
/* Fire the querier's browse question on the wire once; the answers flow back
 * through the regular completion revisit. */
 ...
 * removal-at-expiry. */
static int mdns_querier_maintenance(sd_event_source *s, usec_t usec, void *userdata) {
        _cleanup_(dns_service_querier_unrefp) DnsServiceQuerier *sq = NULL;
        int r;
...
void mdns_service_update(DnssdDiscoveredService *service, DnsResourceRecord *rr, usec_t until) {
         ...
         * mdns_manage_services_answer(). A bare list entry has no querier attached
         * (the unit tests construct those), hence the guard. */
        if (service->querier)
