...
static usec_t mdns_maintenance_span(DnsResourceRecord *rr) {
        ...
        return MIN(rr->ttl * USEC_PER_SEC, (usec_t) CACHE_TTL_MAX_USEC);
}
...
void dns_service_querier_forget_query(DnsServiceQuerier *sq, DnsQuery *q) {
        ...
        if (sq && sq->maintenance_query == q)
                sq->maintenance_query = NULL;
}
...
static int mdns_querier_send_question(DnsServiceQuerier *sq, bool cache_ok) {
         ...
         * time belongs. A query still in flight is one the cache did not answer, so it is going to
         * the network; a cache hit completes inside dns_query_go() and clears this field, and must
         * not make the next emitter believe a packet just went out. */
        if (sq->maintenance_query)
...
/* RFC 6762 §5.2: successive queries for one question are at least a second apart. The rescue is the
 * one emitter that fires on someone else's schedule -- a goodbye can land milliseconds after a
 * ladder rung or a scheduled query -- so it is the one that has to ask. Nothing is lost by skipping
 * it in that window: the query already in flight is the same question on the same scopes, so a
 * surviving publisher's answer to it refreshes the record well inside the §10.1 grace, which is all
 * the rescue exists to achieve. */
...
static void mdns_querier_schedule_maintenance(DnsServiceQuerier *sq) {
        ...
        usec_t ttl_usec = mdns_maintenance_span(soonest->rr);
        ...
        while (sq->rr_ttl_state < _DNS_RECORD_TTL_STATE_MAX) {
                ...
                sq->rr_ttl_state = mdns_maintenance_next_state(ttl_usec, sq->rr_ttl_state);
        }
...
bool mdns_goodbyes_hit_discovered(DnsServiceQuerier *sq, DnsAnswer *goodbyes) {
        assert(sq);
...
static int dns_service_querier_new(
        ...
        *sq = (DnsServiceQuerier) {
                ...
                .goodbye_rescue_ratelimit = { 5 * USEC_PER_MINUTE, 6 },
        };
...
int dns_subscribe_browse_service(
        ...
        /* Clear the two bits that say nothing about the question itself here, once, so that two
         * otherwise-identical browse questions cannot end up on separate queriers — each with its
         * own wire schedule, ladder and goodbye-rescue budget. For NO_CACHE this is load-bearing
         * rather than cosmetic: whether a query may be cache-served is decided per query by
         * mdns_querier_send_question()'s cache_ok argument, and a NO_CACHE left in sq->flags would
         * override that for the queries meant to be answered from the cache. NO_STALE has no effect
         * on an mDNS browse at all — dns_cache_put() retains nothing past its TTL for anything but
         * unicast DNS, and the reconciliation masks the bit out of its own lookups anyway. The
         * remaining bits do change transaction behaviour and stay part of the identity. */
        flags &= ~(SD_RESOLVED_NO_CACHE | SD_RESOLVED_NO_STALE);
