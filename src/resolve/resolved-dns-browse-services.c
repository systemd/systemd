 ...
 * Best-effort: the callers run where an error must not propagate, and mdns_browser_revisit_cache()
 ...
 * removal-at-expiry. */
static int mdns_browser_maintenance(sd_event_source *s, usec_t usec, void *userdata) {
        _cleanup_(dns_service_browser_unrefp) DnsServiceBrowser *sb = NULL;
...
void mdns_service_update(DnssdDiscoveredService *service, DnsResourceRecord *rr, usec_t until) {
         ...
         * mdns_manage_services_answer(). A bare list entry has no browser attached
