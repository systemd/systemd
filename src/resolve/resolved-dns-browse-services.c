...
static int mdns_maintenance_query(sd_event_source *s, uint64_t usec, void *userdata) {
         ...
         *
         * Also note that none of the failures below are propagated to the event loop: the source is
         * one-shot, so returning an error would both disable it - undoing the re-arm - and take down
         * the manager's event loop over a single service's maintenance query. */
