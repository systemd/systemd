...
int dns_subscribe_browse_service(
        ...
        /* No type means RFC 6763 §9 service-type enumeration. The question for that is the §9
         * meta-name, not a bare PTR on the domain, which no responder answers with the type list —
         * resolved's own responder side implements exactly this name. The replies' rdata are
         * service types rather than instances, which the reconciliation reports as the
         * instance-less events described at browse_service_update_append(). */
        if (!type)
