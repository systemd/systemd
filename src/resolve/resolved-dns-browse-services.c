...
static void mdns_maintenance_query_complete(DnsQuery *q) {
        ...
        if (sb->maintenance_query == query)
                sb->maintenance_query = NULL;
 ...
 * subscription ends, so the browser can be released (the completion handler clears the field). */
void mdns_browser_abort_maintenance_query(DnsServiceBrowser *sb) {
        assert(sb);
...
int dns_service_match_and_update(
        ...
        /* Check whether a discovered service matching the given resource record, owner family, and ifindex
         * exists in the list. Returns 0 if not, 1 if it does (a goodbye's TTL <= 1 leaves it untouched),
         * and 2 if its expiry was refreshed to a later 'until'. */
...
int mdns_manage_services_answer(DnsServiceBrowser *sb, DnsAnswer *answer, int owner_family) {
        ...
        DNS_ANSWER_FOREACH_ITEM(item, answer) {
                ...
                if (r > 0) {
                        /* Refreshed to a later expiry: wind the ladder back (see rr_ttl_state). */
                        if (r > 1)
                                sb->rr_ttl_state = DNS_RECORD_TTL_STATE_80_PERCENT;
                        continue;
                }
...
finish:
        /* The error reply below terminates the subscription — no further event can reach the client,
         * so wind the ladder down rather than re-arming it for a dead link. */
        sb->maintenance_event = sd_event_source_disable_unref(sb->maintenance_event);
