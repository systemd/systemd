 ...
 * subscriber is gone, so the querier can be released (the completion handler clears the field). */
...
finish:
        /* The error replies below terminate every subscription — no further event can reach any
         * client, so wind the ladder down rather than re-arming it for dead links. */
        sq->maintenance_event = sd_event_source_disable_unref(sq->maintenance_event);
