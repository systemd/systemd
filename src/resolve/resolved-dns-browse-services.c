...
static void mdns_maintenance_query_complete(DnsQuery *q) {
        ...
        if (r < 0)
                log_warning_errno(r, "Failed to revisit IPv4 cache for maintenance query, ignoring: %m");

        r = mdns_querier_revisit_cache(sq, AF_INET6);
        if (r < 0)
                log_warning_errno(r, "Failed to revisit IPv6 cache for maintenance query, ignoring: %m");
}
...
static int mdns_querier_maintenance(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        if (sq->rr_ttl_state == DNS_RECORD_TTL_STATE_100_PERCENT) {
                ...
                r = mdns_querier_revisit_cache(sq, AF_INET);
                if (r < 0)
                        log_warning_errno(r, "Failed to revisit IPv4 cache at TTL expiry, ignoring: %m");

                r = mdns_querier_revisit_cache(sq, AF_INET6);
                if (r < 0)
                        log_warning_errno(r, "Failed to revisit IPv6 cache at TTL expiry, ignoring: %m");
