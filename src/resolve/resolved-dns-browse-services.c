...
int dns_service_browser_new(
        ...
        *sb = (DnsServiceBrowser) {
        ...
        r = hashmap_ensure_put(&m->dns_service_browsers, NULL, sb, sb);
        if (r < 0)
...
int dns_subscribe_browse_service(
        ...
        HASHMAP_FOREACH(existing, m->dns_service_browsers)
...
DnsServiceBrowser *dns_service_browser_free(DnsServiceBrowser *sb) {
        ...
        hashmap_remove_value(sb->manager->dns_service_browsers, sb, sb);
