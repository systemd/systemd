 ...
 * Best-effort: the callers run where an error must not propagate, so failures are logged and ignored. */
static void mdns_browser_revisit_cache_both(DnsServiceBrowser *sb) {
        ...
        FOREACH_ARGUMENT(af, AF_INET, AF_INET6) {
                r = mdns_browser_revisit_cache(sb, af);
                if (r < 0)
                        log_warning_errno(r, "Failed to revisit cache for family %s, ignoring: %m", af_to_name(af));
        }
}
