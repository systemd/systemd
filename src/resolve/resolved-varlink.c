...
static int vl_get_link(sd_varlink *vlink, const char *ifname, int ifindex, Link **ret_link) {
        ...
        if (ifname) {
                int resolved_ifindex = rtnl_resolve_interface(/* rtnl= */ NULL, ifname);
                if (resolved_ifindex < 0)
                        return sd_varlink_error_invalid_parameter(vlink, JSON_VARIANT_STRING_CONST("InterfaceName"));

                Link *link_by_name = hashmap_get(m->links, INT_TO_PTR(resolved_ifindex));
                if (!link_by_name)
                        return sd_varlink_error_invalid_parameter(vlink, JSON_VARIANT_STRING_CONST("InterfaceName"));

                if (link && resolved_ifindex != link->ifindex)
...
static int vl_method_link_set_dns(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        r = verify_polkit_full(
                ...
                (const char**) STRV_MAKE("interface", p.ifname ?: FORMAT_IFNAME(p.ifindex)));
        if (r <= 0)
                return r;
...
static int vl_method_link_set_default_route(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        int r;
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                ...
                { "InterfaceName",  SD_JSON_VARIANT_STRING,        sd_json_dispatch_const_string, voffsetof(p, ifname),        0,                },
...
static int vl_method_link_set_dnssec_negative_trust_anchors(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (strv_length(p.ntas) > LINK_NEGATIVE_TRUST_ANCHORS_MAX)
                return sd_varlink_error_errno(link, -E2BIG);
...
static int vl_method_link_revert(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        int r;
...
static int varlink_main_server_init(Manager *m) {
        ...
        r = sd_varlink_server_bind_method_many(
                        ...
                        "io.systemd.Network.Link.SetDNSDefaultRoute",            vl_method_link_set_default_route,
                        ...
                        "io.systemd.Network.Link.RevertDNS",                     vl_method_link_revert,
