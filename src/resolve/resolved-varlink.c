...
static int vl_get_link(sd_varlink *vlink, char *ifname, int ifindex, Link **ret_link) {
        ...
        if (FLAGS_SET(link->flags, IFF_LOOPBACK))
                return sd_varlink_error(vlink, "io.systemd.Resolve.LinkIsLoopback", NULL);
        if (link->is_managed)
                return sd_varlink_error(vlink, "io.systemd.Resolve.LinkIsManaged", NULL);
