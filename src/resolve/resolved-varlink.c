...
finish:
        if (r < 0) {
                log_full_errno(NERRNO_IS_DISCONNECT(r) ? LOG_DEBUG : LOG_ERR, r, "Failed to send hostname reply: %m");
...
finish:
        if (r < 0) {
                log_full_errno(NERRNO_IS_DISCONNECT(r) ? LOG_DEBUG : LOG_ERR, r, "Failed to send address reply: %m");
                r = varlink_error_errno(q->varlink_request, r);
        }
}
