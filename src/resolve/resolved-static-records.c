 ...
 * {/etc,/run,/usr/lib}/systemd/resolve/static.d/. These files are in JSON format, and are RR serializations,
 * that match the usual way we serialize RRs to JSON.
 *
 * Note that this deliberately doesn't user the (probably more user-friendly) classic DNS zone file format,
...
int manager_static_records_lookup(Manager *m, DnsQuestion *q, DnsAnswer **answer) {
        ...
        DnsAnswer *f = hashmap_get(m->static_records, dns_question_first_name(q));
        if (!f)
                return 0;
