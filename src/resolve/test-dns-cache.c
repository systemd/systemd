...
TEST(dns_cache_stale_retention_honored_for_unicast) {
        ...
        /* The entry expires a retention window after its TTL, not at its TTL: it is neither due within
         * a second, nor within a second of an hour from now, but it is an hour and a second out. */
