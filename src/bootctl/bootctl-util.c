...
int get_file_version(int fd, char **ret) {
        ...
        if (r == -EBADMSG)
                return log_debug_errno(SYNTHETIC_ERRNO(ESRCH), "EFI binary is not a valid PE file, assuming no version information: %m");
        if (r < 0)
                return r;
        ...
        if (r == -EBADMSG)
                return log_debug_errno(SYNTHETIC_ERRNO(ESRCH), "Failed to load PE section table, assuming no version information: %m");
        if (r < 0)
                return r;
