...
int boot_entry_make_commit_filename(
         ...
         * image/OS versionm (if non-zero) the profile number, and the number of tries left. */
...
int boot_entry_parse_commit_filename(
        ...
        unsigned entry_commit;
        r = safe_atou_full(entry_commit_string, 10, &entry_commit);
        if (r < 0)
                return r;
        if (!entry_commit_valid(entry_commit))
                return -EBADMSG;
...
int boot_config_find_oldest_commit(
        ...
        if (commit_oldest != commit_blocked)
                ...
                if (commit_2nd_oldest == UINT64_MAX)
                        return log_debug_errno(SYNTHETIC_ERRNO(EBUSY), "Only matching entry found while determine oldest entry is current one, skipping it.");

                commit_picked = commit_2nd_oldest;
        }

        log_debug("Determined commit %" PRIu64 " to be oldest.", commit_2nd_oldest);
