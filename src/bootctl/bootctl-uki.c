...
int verb_kernel_identify(int argc, char *argv[], void *userdata) {
        ...
        r = inspect_kernel(/* kernel_fd= */ -EBADF, AT_FDCWD, argv[1], &t, /* ret_cmdline= */ NULL, /* ret_uname= */ NULL, /* ret_pretty_name= */ NULL, /* ret_osrel= */ NULL);
        if (r < 0)
                return r;
...
int verb_kernel_inspect(int argc, char *argv[], void *userdata) {
        ...
        r = inspect_kernel(/* kernel_fd= */ -EBADF, AT_FDCWD, argv[1], &t, &cmdline, &uname, &pname, /* ret_osrel= */ NULL);
        if (r < 0)
                return r;
