...
static int unref_unlink_file(
        ...
        if (r < 0)
                ...
                if (IN_SET(r, -ENOTEMPTY, -ENOENT))
                        log_debug_errno(r, "Failed to remove '%s', ignoring: %m", parent);
                if (r < 0)
                        log_warning_errno(r, "Failed to remove '%s', ignoring: %m", parent);
                else
...
static int run_unlink(
        ...
        r = 0;
        if (c->esp_path)
                RET_GATHER(r, unlink_entry(&config, c->esp_path, c->esp_fd, BOOT_ENTRY_ESP, ids, dry_run));

        if (c->xbootldr_path && c->xbootldr_devid != c->esp_devid)
                RET_GATHER(r, unlink_entry(&config, c->xbootldr_path, c->xbootldr_fd, BOOT_ENTRY_XBOOTLDR, ids, dry_run));
