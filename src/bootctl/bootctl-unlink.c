...
static void unref_unlink_file(
        ...
        r = chase_and_unlinkat(root_fd, fn, CHASE_AT_RESOLVE_IN_ROOT|CHASE_PROHIBIT_SYMLINKS|CHASE_TRIGGER_AUTOFS|CHASE_MUST_BE_REGULAR, /* unlink_flags= */ 0, &path);
        if (r >= 0)
...
int boot_entry_unlink(
        ...
        unref_unlink_file(&known_files, root_fd, e->kernel, dry_run);
        unref_unlink_file(&known_files, root_fd, e->efi, dry_run);
        unref_unlink_file(&known_files, root_fd, e->uki, dry_run);
        STRV_FOREACH(s, e->initrd)
                unref_unlink_file(&known_files, root_fd, *s, dry_run);
        unref_unlink_file(&known_files, root_fd, e->device_tree, dry_run);
        STRV_FOREACH(s, e->device_tree_overlay)
                unref_unlink_file(&known_files, root_fd, *s, dry_run);
        FOREACH_ARRAY(x, e->local_extras.items, e->local_extras.n_items)
                unref_unlink_file(&known_files, root_fd, x->location, dry_run);
...
static int unlink_entry(
        ...
        return boot_entry_unlink(config->entries + idx, root_fd, known_files, dry_run);
}
...
static int run_unlink(
        ...
        r = 0;
        if (c->esp_path)
                RET_GATHER(r, unlink_entry(&config, c->esp_path, c->esp_fd, BOOT_ENTRY_ESP, id, dry_run));

        if (c->xbootldr_path && c->xbootldr_devid != c->esp_devid)
                RET_GATHER(r, unlink_entry(&config, c->xbootldr_path, c->xbootldr_fd, BOOT_ENTRY_XBOOTLDR, id, dry_run));
...
int verb_unlink(int argc, char *argv[], uintptr_t _data, void *userdata) {
        ...
        return run_unlink(&c, id, arg_dry_run);
}
...
int vl_method_unlink(
        ...
        r = run_unlink(&p.context, p.id, /* dry_run= */ false);
