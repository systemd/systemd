...
int boot_entry_unlink(
        ...
        STRV_FOREACH(s, e->device_tree_overlay)
                unref_unlink_file(&known_files, root_fd, e->root);
...
static int run_unlink(UnlinkContext *c, const char *id) {
        ...
        r = 0;
        if (c->esp_path)
                RET_GATHER(r, unlink_entry(&config, c->esp_path, c->esp_fd, BOOT_ENTRY_ESP, id));

        if (c->xbootldr_path && c->xbootldr_devid != c->esp_devid)
                RET_GATHER(r, unlink_entry(&config, c->xbootldr_path, c->xbootldr_fd, BOOT_ENTRY_XBOOTLDR, id));
