...
static int link_context_add_extras(LinkContext *c, int uki_dir_fd, Set **seen) {
        ...
        FOREACH_ARRAY(i, des->entries, des->n_entries) {
                ...
                FOREACH_ELEMENT(suffix, auto_link_extra_suffixes) {
                        ...
                        bool versioned = endswith(de->d_name, dot_v);
                        if (!versioned && !endswith(de->d_name, *suffix))
                                continue;
                        ...
                        _cleanup_free_ char *key = versioned ? strndup(de->d_name, strlen(de->d_name) - strlen(".v")) : strdup(de->d_name);
                        if (!key)
                                return log_oom();
...
int vl_method_link_auto(
        ...
        if (r == 0) /* Nothing staged for linking. */
                return sd_varlink_replybo(link, SD_JSON_BUILD_PAIR_STRV("ids", NULL));
