...
static int link_context_unlink_oldest(LinkContext *c) {
        ...
        r = boot_entry_unlink(oldest, c->dollar_boot_fd, known_files);
        if (r < 0)
                return r;
...
static int run_link(LinkContext *c) {
        ...
        for (;;) {
                ...
                int q = link_context_unlink_oldest(c);
                if (q < 0) {
                ...
                log_info("Removing oldest boot menu entry suceeded, will retry to create boot loader menu entry.");
                n_removals++;
        }

        log_full(n_removals > 0 ? LOG_INFO : LOG_DEBUG,
                 "Successfully installed boot loader entry, after removing %u old entries.", n_removals);
...
int vl_method_link(
        ...
        //FIXME: return the id just created
