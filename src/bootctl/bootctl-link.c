...
static int link_context_unlink_oldest(LinkContext *c) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to find suitable oldest boot menu enty: %m");
...
static int run_link(LinkContext *c) {
        ...
        log_full(n_removals > 0 ? LOG_INFO : LOG_DEBUG,
                 "Successfully installed boot loader entries \"%s\", after removing %u old entries.", j, n_removals);
