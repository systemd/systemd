...
static int remove_boot_efi(InstallContext *c) {
        ...
        FOREACH_DIRENT(de, d, break) {
                ...
                r = pe_is_native_fd(fd);
                if (r < 0) {
                        log_warning_errno(r, "Failed to detect if \"%s/%s\" is native architecture, ignoring: %m", j, de->d_name);
                        continue;
                }
                if (r == 0)
                        continue;
                ...
                if (unlinkat(dirfd(d), de->d_name, 0) < 0)
                        return log_error_errno(errno, "Failed to remove \"%s/%s\": %m", j, de->d_name);

                log_info("Removed \"%s/%s\".", j, de->d_name);
...
static int unlink_inode(const char *root, int root_fd, const char *path, mode_t type) {
        ...
        log_info("Removed %s\"%s\".", type == S_IFDIR ? "directory " : "", p);
        return 0;
}
