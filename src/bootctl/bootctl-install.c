...
static int copy_one_file(
         ...
         * when its version matches the currently booted version (from LoaderInfo), so a
         * known good binary stays as the fallback. In all other cases, like no fallback yet,
         * LoaderInfo is unavailable, or fallback version differs from what booted, it is
        ...
        if (e && dest_fd >= 0 && !force) {
                ...
                if (r < 0)
                        ...
                        /* Leave the fallback alone if it already holds the currently booted version, so a
                         * known good binary stays as the fallback. If there is no fallback yet, LoaderInfo
                         * is unavailable, or there is a version mismatch, then overwrite it with the current
                         * primary. */
                        ...
                        if (fallback_fd >= 0) {
                                ...
                                if (efi_get_variable_string(EFI_LOADER_VARIABLE_STR("LoaderInfo"), &loader_info) >= 0 &&
                                    get_file_version(fallback_fd, &fallback_version) >= 0)
                                        should_rotate = !streq(loader_info, fallback_version);
                        }
