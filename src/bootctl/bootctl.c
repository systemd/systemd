...
static int parse_argv(int argc, char *argv[], char ***ret_args) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION_LONG("path", "PATH", /* help= */ NULL):  /* Compatibility alias */
                        r = free_and_strdup(&arg_esp_path, state.argument);
                        if (r < 0)
                                return log_oom();
                        break;

                OPTION_LONG("boot-path", "PATH", "Path to the $BOOT partition"):
                        r = free_and_strdup(&arg_xbootldr_path, state.argument);
                        if (r < 0)
                                return log_oom();
                        break;
                ...
                OPTION_LONG("install-source", "SOURCE",
                        ...
                        InstallSource is = install_source_from_string(state.argument);
                        if (is < 0)
                                return log_error_errno(is, "Unexpected parameter for --install-source=: %s", state.argument);
                ...
                OPTION_LONG("variables", "BOOL", "Whether to modify EFI variables"):
                        r = parse_tristate_argument_with_auto("--variables=", state.argument, &arg_touch_variables);
                        if (r < 0)
                                return r;
#if !ENABLE_EFI
                        if (arg_touch_variables > 0)
                                return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP),
                                                       "Compiled without support for EFI, --variables=%s cannot be specified.", state.argument);
#endif
                        break;
                ...
                OPTION_LONG("random-seed", "BOOL", "Whether to create random-seed file during install"):
                        r = parse_boolean_argument("--random-seed=", state.argument, &arg_install_random_seed);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("entry-token", "TOKEN",
                        ...
                        r = parse_boot_entry_token_type(state.argument, &arg_entry_token_type, &arg_entry_token);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("make-machine-id-directory", "BOOL", /* help= */ NULL):  /* Compatibility alias */
                        if (streq(state.argument, "auto"))  /* retained for backwards compatibility */
                                arg_make_entry_directory = -1; /* yes if machine-id is permanent */
                        else {
                                r = parse_boolean_argument("--make-entry-directory=", state.argument, NULL);
                                if (r < 0)
                                        return r;
                ...
                OPTION_LONG("efi-boot-option-description", "DESCRIPTION",
                        ...
                        if (!string_is_safe(state.argument, STRING_ALLOW_BACKSLASHES|STRING_ALLOW_QUOTES|STRING_ALLOW_GLOBS)) {
                                _cleanup_free_ char *escaped = cescape(state.argument);
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Invalid --efi-boot-option-description=: %s", strna(escaped));
                        }
                        if (strlen(state.argument) > EFI_BOOT_OPTION_DESCRIPTION_MAX)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "--efi-boot-option-description= too long: %zu > %zu",
                                                       strlen(state.argument), EFI_BOOT_OPTION_DESCRIPTION_MAX);
                        r = free_and_strdup_warn(&arg_efi_boot_option_description, state.argument);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("efi-boot-option-description-with-device", "BOOL",
                        ...
                        r = parse_boolean_argument("--efi-boot-option-description-with-device=", state.argument,
                ...
                OPTION_LONG("secure-boot-auto-enroll", "BOOL", "Set up secure boot auto-enrollment"):
                        r = parse_boolean_argument("--secure-boot-auto-enroll=", state.argument,
                ...
                OPTION_COMMON_PRIVATE_KEY_SOURCE:
                        r = parse_openssl_key_source_argument(state.argument,
                ...
                OPTION_COMMON_CERTIFICATE_SOURCE:
                        r = parse_openssl_certificate_source_argument(state.argument,
