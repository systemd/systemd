...
static int vl_method_mount_directory(
        ...
        if (mount_fd < 0) {
                if (!ERRNO_IS_NEG_NOT_SUPPORTED(errno))
                        return log_debug_errno(errno, "Failed to issue open_tree_attr() of provided directory '%s': %m", strna(directory_path));

                log_debug_errno(errno, "open_tree_attr() with clearing MOUNT_ATTR_IDMAP is not supported, failling back to classic open_tree()");
