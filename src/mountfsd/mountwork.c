...
static DirectoryOwnership validate_directory_fd(
        ...
        if (xstatx_full(fd,
                        /* path= NULL */ NULL,
                        AT_EMPTY_PATH,
                        /* mandatory_mask= */ STATX_TYPE|STATX_UID|STATX_MNT_ID,
                        /* optional_mask= */ 0,
                        /* mandatory_attributes= */ STATX_ATTR_MOUNT_ROOT,
                        &stx) < 0)
                return log_debug_errno(errno, "Failed to statx() directory fd: %m");
        ...
        for (n_level = 0; n_level < 16; n_level++) {
                ...
                if (xstatx_full(new_parent_fd,
                                /* path= */ NULL,
                                AT_EMPTY_PATH,
                                /* mandatory_mask= */ STATX_UID|STATX_MNT_ID,
                                /* optional_mask= */ 0,
                                /* mandatory_attributes= */ STATX_ATTR_MOUNT_ROOT,
                                &new_stx) < 0)
                        return log_debug_errno(errno, "Failed to statx() parent directory of directory file descriptor: %m");
