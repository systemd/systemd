...
static int on_child(sd_event_source *s, const siginfo_t *si, void *userdata) {
        ...
        if (si->si_code == CLD_EXITED) {
                ...
                /* NB: we propagate success here if the child exited clearnly but never sent us READY=1. We
                 * are not a service manager after all, where this would be a protocol violation. We are just
                 * a shell tool to fork off stuff in the background, where I think it makese sense to allow
...
static int action_fork(char *const *_command) {
        ...
        const int forward_signals[] = { SIGTERM, SIGINT, SIGQUIT, SIGTSTP, SIGCONT, SIGUSR1, SIGUSR2 };
