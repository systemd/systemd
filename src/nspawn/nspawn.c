...
static int setup_nsresourced_socket(const char *directory, uid_t uid_shift, uid_t uid_range) {
        int r;
        ...
        r = mount_custom(
                        ...
                        &(CustomMount) {
                                .type = CUSTOM_MOUNT_BIND,
                                .options = (char*) (uid_is_valid(uid_shift) ? "rootidmap" : NULL),
                                .source = (char*) "/run/systemd/io.systemd.NamespaceResource",
                                .destination = (char*) "/run/systemd/io.systemd.NamespaceResource",
                                .destination_uid = UID_INVALID,
                        },
                        /* n= */ 1,
                        uid_shift,
...
static int outer_child(
        ...
        r = setup_nsresourced_socket(directory, chown_uid, chown_range);
        if (r < 0)
                return r;
