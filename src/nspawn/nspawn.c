...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "+hD:u:abL:M:jS:Z:qi:xp:nUE:P", options, NULL)) >= 0)
                ...
                case ARG_CHDIR: {
                        ...
                        if (r < 0)
                                return log_error_errno(r, "Failed to simplify path %s: %m", wd);
                        ...
                        if (path_below_api_vfs(wd))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Working directory is below API VFS, refsuing: %s", wd);
...
static int inner_child(
        ...
        /* Note, this should be done this later (💣 and not moved earlier! 💣), so that all namespacing
