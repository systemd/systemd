...
static int parse_argv(int argc, char *argv[]) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c) {
                ...
                OPTION_LONG("settings", "BOOLEAN", "Load additional settings from .nspawn file"):
                        ...
                        if (r < 0) {
                                if (streq(state.argument, "trusted")) {
                                        mask_all_settings = false;
                                        mask_no_settings = false;
                                        arg_settings_trusted = true;

                                } else if (streq(state.argument, "override")) {
                                        mask_all_settings = false;
                                        mask_no_settings = true;
                                        arg_settings_trusted = -1;
                                } else
                                        return log_error_errno(r, "Failed to parse --settings= argument: %s", state.argument);
                        } else if (r > 0) {
                                /* yes */
                                mask_all_settings = false;
                                mask_no_settings = false;
                ...
                OPTION('D', "directory", "PATH", "Root directory for the container"):
                        r = parse_path_argument(state.argument, false, &arg_directory);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("template", "PATH",
                        ...
                        r = parse_path_argument(state.argument, false, &arg_template);
                        if (r < 0)
                                return r;
                        arg_settings_mask |= SETTING_DIRECTORY;
                        break;
                ...
                OPTION_LONG("mstack", "PATH", /* help= */ NULL):
                        r = parse_path_argument(state.argument, false, &arg_mstack);
                        if (r < 0)
                                return r;
                        arg_settings_mask |= SETTING_DIRECTORY;
                        break;
                ...
                OPTION_LONG("oci-bundle", "PATH", "OCI bundle directory"):
                        r = parse_path_argument(state.argument, false, &arg_oci_bundle);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG_FLAGS(OPTION_OPTIONAL_ARG, "volatile", "MODE", "Run the system in volatile mode"):
                        ...
                        else if (streq(state.argument, "help"))
                                return DUMP_STRING_TABLE(volatile_mode, VolatileMode, _VOLATILE_MODE_MAX);
                        else {
                                VolatileMode m;

                                m = volatile_mode_from_string(state.argument);
                                if (m < 0)
                                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                               "Failed to parse --volatile= argument: %s", state.argument);
                                else
                                        arg_volatile_mode = m;
                        }
                ...
                OPTION_LONG("root-hash", "HASH", "Specify verity root hash for root disk image"): {
                        ...
                        r = unhexmem(state.argument, &k.iov_base, &k.iov_len);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse root hash: %s", state.argument);
                        if (k.iov_len < sizeof(sd_id128_t))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Root hash must be at least 128-bit long: %s", state.argument);
                ...
                OPTION_LONG("root-hash-sig", "SIG",
                        ...
                        if ((value = startswith(state.argument, "base64:"))) {
                                r = unbase64mem(value, &p.iov_base, &p.iov_len);
                                if (r < 0)
                                        return log_error_errno(r, "Failed to parse root hash signature '%s': %m", state.argument);

                        } else {
                                r = read_full_file(state.argument, (char**) &p.iov_base, &p.iov_len);
                                if (r < 0)
                                        return log_error_errno(r, "Failed to parse root hash signature file '%s': %m", state.argument);
                        }
                ...
                OPTION_LONG("verity-data", "PATH", "Specify hash device for verity"):
                        r = parse_path_argument(state.argument, false, &arg_verity_settings.data_path);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("pivot-root", "PATH[:PATH]",
                        ...
                        r = pivot_root_parse(&arg_pivot_root_new, &arg_pivot_root_old, state.argument);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --pivot-root= argument %s: %m", state.argument);
                ...
                OPTION_LONG("chdir", "PATH", "Set working directory in the container"): {
                        ...
                        if (!path_is_absolute(state.argument))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Working directory %s is not an absolute path.", state.argument);

                        r = path_simplify_alloc(state.argument, &wd);
                        if (r < 0)
                                return log_error_errno(r, "Failed to simplify path %s: %m", state.argument);
                ...
                OPTION('u', "uid", "USER", "Run the command under specified user or UID"):
                        r = free_and_strdup(&arg_user, state.argument);
                        if (r < 0)
                                return log_oom();
                ...
                OPTION_LONG("kill-signal", "SIGNAL", "Select signal to use for shutting down PID 1"):
                        ...
                        arg_kill_signal = signal_from_string(state.argument);
                        if (arg_kill_signal < 0)
                                return log_error_errno(arg_kill_signal, "Cannot parse signal: %s", state.argument);
                ...
                OPTION_LONG("suppress-sync", "BOOLEAN", "Suppress any form of disk data synchronization"):
                        r = parse_boolean_argument("--suppress-sync=", state.argument, &arg_suppress_sync);
                        if (r < 0)
                                return r;
                ...
                OPTION('M', "machine", "NAME", "Set the machine name for the container"):
                        if (!isempty(state.argument) && !hostname_is_valid(state.argument, /* flags= */ 0))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Invalid machine name: %s", state.argument);
                        r = free_and_strdup_warn(&arg_machine, state.argument);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("hostname", "NAME", "Override the hostname for the container"):
                        if (!isempty(state.argument) && !hostname_is_valid(state.argument, /* flags= */ 0))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Invalid hostname: %s", state.argument);
                        r = free_and_strdup_warn(&arg_hostname, state.argument);
                        if (r < 0)
                                return r;
                ...
                OPTION('S', "slice", "SLICE", "Place the container in the specified slice"): {
                        ...
                        r = unit_name_mangle_with_suffix(state.argument, NULL, UNIT_NAME_MANGLE_WARN, ".slice", &mangled);
                        if (r < 0)
                                return log_oom();
                ...
                OPTION_LONG_FLAGS(OPTION_OPTIONAL_ARG, "private-users", "MODE",
                        ...
                        r = parse_private_users(state.argument, &arg_userns_mode, &arg_uid_shift, &arg_uid_range);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("private-users-ownership", "MODE",
                        ...
                        arg_userns_ownership = user_namespace_ownership_from_string(state.argument);
                        if (arg_userns_ownership < 0)
                                return log_error_errno(arg_userns_ownership, "Cannot parse --private-users-ownership= value: %s", state.argument);
                        arg_settings_mask |= SETTING_USERNS;
                        break;
                ...
                OPTION_LONG("private-users-delegate", "N",
                        ...
                        r = safe_atou(state.argument, &arg_delegate_container_ranges);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --private-users-delegate= parameter: %s", state.argument);
                        arg_settings_mask |= SETTING_USERNS;
                        break;
                ...
                OPTION_LONG("network-interface", "HOSTIF[:CONTAINERIF]",
                        ...
                        r = interface_pair_parse(&arg_network_interfaces, state.argument);
                        if (r < 0)
                                return r;
                        arg_private_network = true;
                        arg_settings_mask |= SETTING_NETWORK;
                ...
                OPTION_LONG("network-macvlan", "HOSTIF[:CONTAINERIF]",
                        ...
                        r = macvlan_pair_parse(&arg_network_macvlan, state.argument);
                        if (r < 0)
                                return r;
                        arg_private_network = true;
                        arg_settings_mask |= SETTING_NETWORK;
                ...
                OPTION_LONG("network-ipvlan", "HOSTIF[:CONTAINERIF]",
                        ...
                        r = ipvlan_pair_parse(&arg_network_ipvlan, state.argument);
                        if (r < 0)
                                return r;
                        arg_private_network = true;
                        arg_settings_mask |= SETTING_NETWORK;
                ...
                OPTION_LONG("network-veth-extra", "HOSTIF[:CONTAINERIF]",
                        ...
                        r = veth_extra_parse(&arg_network_veth_extra, state.argument);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --network-veth-extra= parameter: %s", state.argument);
                        arg_private_network = true;
                        arg_settings_mask |= SETTING_NETWORK;
                        break;
                ...
                OPTION_LONG("network-bridge", "INTERFACE",
                        ...
                        if (!ifname_valid(state.argument))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Bridge interface name not valid: %s", state.argument);
                        r = free_and_strdup(&arg_network_bridge, state.argument);
                        if (r < 0)
                                return log_oom();
                        arg_network_veth = true;
                        arg_private_network = true;
                ...
                OPTION_LONG("network-namespace-path", "PATH",
                        ...
                        r = parse_path_argument(state.argument, false, &arg_network_namespace_path);
                        if (r < 0)
                                return r;
                        arg_settings_mask |= SETTING_NETWORK;
                        break;
                ...
                OPTION('p', "port", "[PROTOCOL:]HOSTPORT[:CONTAINERPORT]",
                       "Expose a container IP port on the host"):
                        r = expose_port_parse(&arg_expose_ports, state.argument);
                        if (r == -EEXIST)
                                return log_error_errno(r, "Duplicate port specification: %s", state.argument);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse host port %s: %m", state.argument);
                ...
                OPTION_LONG("drop-capability", "CAP",
                        ...
                        uint64_t m;
                        r = parse_capability_spec(state.argument, &m);
                        if (r <= 0)
                                return r;

                        if (streq(state.current->long_code, "capability"))
                                plus |= m;
                        else
                                minus |= m;
                ...
                OPTION_LONG("no-new-privileges", "BOOL",
                        ...
                        r = parse_boolean_argument("--no-new-privileges=", state.argument, &arg_no_new_privileges);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("system-call-filter", "LIST|~LIST",
                        ...
                        negative = state.argument[0] == '~';
                        items = negative ? state.argument + 1 : state.argument;
                ...
                OPTION('Z', "selinux-context", "SECLABEL",
                       "Set the SELinux security context to be used by processes in the container"):
                        arg_selinux_context = state.argument;
                        break;
                ...
                OPTION('L', "selinux-apifs-context", "SECLABEL",
                       "Set the SELinux security context to be used by API/tmpfs file systems in the container"):
                        arg_selinux_apifs_context = state.argument;
                        break;
                ...
                OPTION_LONG("rlimit", "NAME=LIMIT", "Set a resource limit for the payload"): {
                        ...
                        eq = strchr(state.argument, '=');
                        if (!eq)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "--rlimit= expects an '=' assignment.");

                        name = strndup(state.argument, eq - state.argument);
                        if (!name)
                                return log_oom();
                ...
                OPTION_LONG("oom-score-adjust", "VALUE", "Adjust the OOM score value for the payload"):
                        r = parse_oom_score_adjust(state.argument, &arg_oom_score_adjust);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --oom-score-adjust= parameter: %s", state.argument);
                ...
                OPTION_LONG("cpu-affinity", "CPUS", "Adjust the CPU affinity of the container"): {
                        ...
                        r = parse_cpu_set(state.argument, &cpuset);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse CPU affinity mask %s: %m", state.argument);
                ...
                OPTION_LONG("personality", "ARCH", "Pick personality for this container"):
                        arg_personality = personality_from_string(state.argument);
                        if (arg_personality == PERSONALITY_INVALID)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Unknown or unsupported personality '%s'.", state.argument);
                ...
                OPTION_LONG("resolv-conf", "MODE", "Select mode of /etc/resolv.conf initialization"):
                        ...
                        arg_resolv_conf = resolv_conf_mode_from_string(state.argument);
                        if (arg_resolv_conf < 0)
                                return log_error_errno(arg_resolv_conf,
                                                       "Failed to parse /etc/resolv.conf mode: %s", state.argument);
                ...
                OPTION_LONG("timezone", "MODE", "Select mode of /etc/localtime initialization"):
                        ...
                        arg_timezone = timezone_mode_from_string(state.argument);
                        if (arg_timezone < 0)
                                return log_error_errno(arg_timezone,
                                                       "Failed to parse /etc/localtime mode: %s", state.argument);
                ...
                OPTION_LONG("link-journal", "MODE",
                        ...
                        r = parse_link_journal(state.argument, &arg_link_journal, &arg_link_journal_try);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse link journal mode %s", state.argument);
                ...
                OPTION_LONG("bind-ro", "PATH[:PATH[:OPTIONS]]",
                        ...
                        r = bind_mount_parse(&arg_custom_mounts, &arg_n_custom_mounts, state.argument,
                                             streq(state.current->long_code, "bind-ro"));
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --bind(-ro)= argument %s: %m", state.argument);
                        arg_settings_mask |= SETTING_CUSTOM_MOUNTS;
                        break;

                OPTION_LONG("inaccessible", "PATH",
                            "Over-mount file node with inaccessible node to mask it"):
                        r = inaccessible_mount_parse(&arg_custom_mounts, &arg_n_custom_mounts, state.argument);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --inaccessible= argument %s: %m", state.argument);
                        arg_settings_mask |= SETTING_CUSTOM_MOUNTS;
                        break;

                OPTION_LONG("tmpfs", "PATH:[OPTIONS]",
                            "Mount an empty tmpfs to the specified directory"):
                        r = tmpfs_mount_parse(&arg_custom_mounts, &arg_n_custom_mounts, state.argument);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --tmpfs= argument %s: %m", state.argument);
                        arg_settings_mask |= SETTING_CUSTOM_MOUNTS;
                        break;
                ...
                OPTION_LONG("overlay-ro", "PATH[:PATH...]:PATH",
                        ...
                        r = overlay_mount_parse(&arg_custom_mounts, &arg_n_custom_mounts, state.argument,
                                                streq(state.current->long_code, "overlay-ro"));
                        if (r == -EADDRNOTAVAIL)
                                return log_error_errno(r, "--overlay(-ro)= needs at least two colon-separated directories specified.");
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --overlay(-ro)= argument %s: %m", state.argument);
                        arg_settings_mask |= SETTING_CUSTOM_MOUNTS;
                        break;

                OPTION_LONG("bind-user", "NAME", "Bind user from host to container"):
                        if (!valid_user_group_name(state.argument, 0))
                ...
                OPTION_LONG("console", "MODE",
                        ...
                        if (arg_console_mode < 0)
                                return log_error_errno(arg_console_mode, "Unknown console mode: %s", state.argument);
                ...
                OPTION_LONG_FLAGS(OPTION_OPTIONAL_ARG, "user", "NAME", "Run in the user service manager scope"):
                        if (state.argument)
                                 ...
                                 * with the space-separated form (--user NAME), if the next state.argument does not look
                                 * like an option, interpret it as a user name. */
                                const char *t = option_parser_next_arg(&state);
                                if (t && t[0] != '-') {
                                        state.argument = option_parser_consume_next_arg(&state);
                                        log_warning("--user NAME is deprecated, use --uid=NAME instead.");
                                }
                        }
