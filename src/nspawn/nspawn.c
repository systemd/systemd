...
static void cleanup_propagation_and_export_directories(const char *runtime_dir) {
        ...
        free(p);
        p = strjoin(runtime_dir, "unix-export");
        if (p) {
...
static int run(int argc, char *argv[]) {
        ...
        * the child. Functions like copy_devnodes() change the umask temporarily. */
        ...
        rootdir = path_join(runtime_dir, "root");
        if (!rootdir) {
                r = log_oom();
                goto finish;
        }

        if (mkdir(rootdir, 400) < 0 && errno != EEXIST) {
                r = log_error_errno(errno, "Failed to create '%s': %m", rootdir);
                goto finish;
        }
