...
static int merge_settings(Settings *settings, const char *path) {
        ...
        bool merged_private_network = settings_private_network(settings) && !settings->network_namespace_path;
        ...
        if ((arg_settings_mask & SETTING_NETWORK) == 0 &&
            settings_network_configured(settings)) {
                ...
                if (!arg_settings_trusted)
                        ...
                        if (!merged_private_network) {
                                /* No private network means no inherited link configuration. */
                                ...
                                if (veth != 0 && (settings->network_bridge || settings->network_zone)) {
                                        /* Bridge= and Zone= are alternatives, so specifying either replaces
                                         * both. */
