...
static int run(int argc, char *argv[]) {
        ...
        _cleanup_(rm_rf_physical_and_freep) char *journal_remote_dir = NULL;
        ...
        * the child. Functions like copy_devnodes() change the umask temporarily. */
        ...
        if (arg_forward_journal) {
                ...
                if (asprintf(&subdir, "systemd/nspawn-journal.%" PRIx64, random_u64()) < 0) {
                        r = log_oom();
                        goto finish;
                }

                r = runtime_directory(arg_runtime_scope, subdir, &journal_remote_dir);
                if (r < 0) {
                        log_error_errno(r, "Failed to lookup runtime directory: %m");
                        goto finish;
                }
                if (r > 0) {
                        r = mkdir_p(journal_remote_dir, 0755);
                        if (r < 0) {
                                log_error_errno(r, "Failed to create runtime directory '%s': %m", journal_remote_dir);
                                goto finish;
                        }
                }

                _cleanup_free_ char *socket_path = path_join(journal_remote_dir, "socket");
                if (!socket_path) {
                        r = log_oom();
                        goto finish;
                }
