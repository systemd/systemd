...
static int parse_argv(int argc, char *argv[]) {
        ...
        /* Derive runtime scope from effective UID if not explicitly set via --user/--system */
        if (arg_runtime_scope < 0)
                arg_runtime_scope = getuid() == 0 ? RUNTIME_SCOPE_SYSTEM : RUNTIME_SCOPE_USER;

        /* Apply scope-dependent defaults. These may be further refined by load_settings(). -U sets
         * arg_userns_mode to _USER_NAMESPACE_MODE_INVALID to defer the PICK vs MANAGED resolution to
         * here where arg_runtime_scope has its final value. */
