...
static int setup_nsresourced_socket(const char *directory) {
        ...
        _cleanup_free_ char *dest = path_join(directory, "/run/host/io.systemd.NamespaceResource");
        if (!dest)
                return log_oom();
...
static int setup_mountfsd_socket(const char *directory) {
        ...
        _cleanup_free_ char *dest = path_join(directory, "/run/host/io.systemd.MountFileSystem");
        if (!dest)
                return log_oom();
