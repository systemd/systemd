...
static int outer_child(
        ...
        if (mount_fd >= 0) {
                ...
                /* Set MSTACK_RDONLY if the user explicitly passed --read-only, or if the mstack
                 * has no writable upper layer (rw/ directory). We intentionally do not use
                 * arg_read_only here since volatile modes force it to true to make the base image
                 * immutable, but that should not affect mstack bind mount attributes. */
                mstack_flags = (FLAGS_SET(arg_settings_mask, SETTING_READ_ONLY) || !has_rw_layer) ? MSTACK_RDONLY : 0;
