...
static int get_fuse_version(uint32_t *ret_major, uint32_t *ret_minor) {
        /* Must be called with mount privileges, either via arg_privileged or by being uid=0 in a new
        ...
        _cleanup_(umount_and_rmdir_and_freep) char *dir = NULL;
        ...
        if (fuse_fd < 0) {
                if (ERRNO_IS_NEG_DEVICE_ABSENT(-errno))
                        return log_debug_errno(errno, "FUSE appears to be disabled on the host: %m");
                return log_warning_errno(errno, "Failed to open /dev/fuse: %m");
        }
        if (asprintf(&opts, "fd=%i,rootmode=40000,user_id=0,group_id=0", fuse_fd) < 0)
                return log_oom_warning();
        mnt_fd = make_fsmount(LOG_WARNING, "nspawn-fuse", "fuse.nspawn", 0, opts, -EBADF);
        if (mnt_fd < 0)
                return mnt_fd;
        ...
        if (n < 0)
                return log_warning_errno(errno, "Failed to read /dev/fuse: %m");
        if ((size_t) n < sizeof request.structured.header ||
            (size_t) n < request.structured.header.len)
                return log_warning_errno(SYNTHETIC_ERRNO(EIO), "Failed to read /dev/fuse: Short read");
        ...
        if (request.structured.header.opcode != FUSE_INIT)
                return log_warning_errno(SYNTHETIC_ERRNO(EIO), "Initial request from /dev/fuse should have opcode=%i (FUSE_INIT), but has opcode=%"PRIu32,
                                   FUSE_INIT, request.structured.header.opcode);
        if (request.structured.header.len < sizeof request.structured)
                return log_warning_errno(SYNTHETIC_ERRNO(EIO), "Initial FUSE_INIT request from /dev/fuse is too short");
...
static int outer_child(
        ...
        if (ERRNO_IS_NEG_DEVICE_ABSENT(r))
                ...
                log_debug("Disabling FUSE: Failed to determine FUSE version");
