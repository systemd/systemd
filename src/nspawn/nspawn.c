...
static int apply_deferred_mstack_bind_mounts(MStack *mstack, const char *directory, MStackFlags mstack_flags, MStackBindMountStrategy mstack_bindmount_strategy) {
        int r;
        ...
        if (root_fd < 0)
                return log_debug_errno(errno, "Failed to open container root for deferred mstack mount: %m");
         ...
         * the host filesystem. */
        r = mstack_apply_bind_mounts(mstack, root_fd, directory, mstack_flags, mstack_bindmount_strategy);
        if (r < 0)
                return log_debug_errno(r, "Failed to apply deferred .mstack bind mounts: %m");
...
static int outer_child(
        ...
        if (mount_fd >= 0) {
                ...
                MStackBindMountStrategy mstack_mstack_bindmount_strategy = MSTACK_BINDMOUNT_ALL;

                mstack_flags = arg_read_only ? MSTACK_RDONLY : 0;
                ...
                if (IN_SET(arg_volatile_mode, VOLATILE_YES, VOLATILE_OVERLAY)) {
                        /* Defer all binds. */
                        mstack_mstack_bindmount_strategy = MSTACK_BINDMOUNT_NONE;

                        log_debug("Combination of mstack and volatile flags, deferring all .mstack bind mounts.");
                } else if (arg_volatile_mode == VOLATILE_STATE) {
                        /* Since volatile=state creates only /var overlay
                         * we can just defer only binds under /var. */
                        mstack_mstack_bindmount_strategy = MSTACK_BINDMOUNT_EXCEPT_VAR;

                        log_debug("Combination of mstack and volatile state flags, deferring .mstack bind mounts under /var.");
                }
                ...
                r = mstack_bind_mounts(
                                ...
                                /* where_fd= */ -EBADF,
                                mstack_flags,
                                mstack_mstack_bindmount_strategy,
                                /* ret_root_fd= */ NULL);
                if (r < 0)
         ...
         */
        if (mstack && IN_SET(arg_volatile_mode, VOLATILE_YES, VOLATILE_OVERLAY)) {
                r = apply_deferred_mstack_bind_mounts(mstack, directory, mstack_flags, MSTACK_BINDMOUNT_ALL);
                if (r < 0)
        ...
        if (mstack && arg_volatile_mode == VOLATILE_STATE) {
                r = apply_deferred_mstack_bind_mounts(mstack, directory, mstack_flags, MSTACK_BINDMOUNT_VAR);
                if (r < 0)
                        return log_error_errno(r, "Failed to apply deferred .mstack bind mounts: %m");
        }
