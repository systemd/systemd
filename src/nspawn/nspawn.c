...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "+hD:u:abL:M:jS:Z:qi:xp:nUE:P", options, NULL)) >= 0)
                ...
                case ARG_NETWORK_INTERFACE:
                        r = network_interface_parse_and_test(&arg_network_interfaces, optarg);
                        if (r < 0)
                                return r;
                ...
                case ARG_NETWORK_MACVLAN:
                        r = network_macvlan_parse_and_test(&arg_network_macvlan, optarg);
                        if (r < 0)
                                return r;
                ...
                case ARG_NETWORK_IPVLAN:
                        r = network_ipvlan_parse_and_test(&arg_network_ipvlan, optarg);
                        if (r < 0)
                                return r;
        ...
        *pid = raw_clone(SIGCHLD|CLONE_NEWNS);
        ...
        if (arg_private_network) {
                ...
                if (r == 0) {
                        ...
                        /* revert network interfaces pair list so that interfaces get their initial name back */
