        ...
        case TIMEZONE_SYMLINK: {
                ...
                if (r < 0)
                        ...
                        if (unlink(where) < 0 && errno != ENOENT) {
                                log_full_errno(ERRNO_IS_FS_CANNOT_WRITE(errno) ? LOG_DEBUG : LOG_WARNING, /* Don't complain on read-only images */
                        ...
                        if (symlink(what, where) < 0) {
                                log_full_errno(ERRNO_IS_FS_CANNOT_WRITE(errno) ? LOG_DEBUG : LOG_WARNING,
...
static int setup_resolv_conf(const char *dest) {
        ...
        if (r < 0) {
                ...
                log_full_errno(!IN_SET(RESOLV_CONF_COPY_HOST, RESOLV_CONF_COPY_STATIC, RESOLV_CONF_COPY_UPLINK, RESOLV_CONF_COPY_STUB) &&
                               (r == -ELOOP || ERRNO_IS_NEG_FS_CANNOT_WRITE(r)) ? LOG_DEBUG : LOG_WARNING, r,
