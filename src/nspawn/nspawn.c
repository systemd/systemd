...
static int apply_deferred_mstack_bind_mounts(MStack *mstack, const char *directory, MStackFlags mstack_flags, MStackDeferMode defer_mode) {
        int r;
         ...
         * the host filesystem. */
        r = mstack_apply_bind_mounts(mstack, root_fd, directory, mstack_flags, defer_mode);
        if (r < 0)
...
static int outer_child(
        ...
        /* To support mstack_apply_root_attr() helper. */
        ...
        if (mount_fd >= 0) {
                ...
                MStackDeferMode mstack_defer_mode = MSTACK_DEFER_NONE;
                ...
                if (IN_SET(arg_volatile_mode, VOLATILE_YES, VOLATILE_OVERLAY)) {
                        /* Defer all binds. */
                        mstack_defer_mode = MSTACK_DEFER_ALL;
                        ...
                        mstack_defer_mode = MSTACK_DEFER_VAR;
        ...
        if (mstack && IN_SET(arg_volatile_mode, VOLATILE_YES, VOLATILE_OVERLAY)) {
                r = apply_deferred_mstack_bind_mounts(mstack, directory, mstack_flags, MSTACK_DEFER_NONE);
                if (r < 0)
        ...
        if (mstack && arg_volatile_mode == VOLATILE_STATE) {
                r = apply_deferred_mstack_bind_mounts(mstack, directory, mstack_flags, MSTACK_APPLY_VAR_ONLY);
                if (r < 0)
                        return log_error_errno(r, "Failed to apply deferred .mstack bind mounts: %m");
        }
