        ...
        if (r < 0) {
                ...
                if (FLAGS_SET(flags, ALLOCATE_SCOPE_ALLOW_PIDFD) &&
                    sd_bus_error_has_names(&error, SD_BUS_ERROR_UNKNOWN_PROPERTY, SD_BUS_ERROR_PROPERTY_READ_ONLY)) {
                        SET_FLAG(flags, ALLOCATE_SCOPE_ALLOW_PIDFD, false);
                        return allocate_scope(bus, machine_name, pid, slice, mounts, n_mounts, kill_signal, properties, properties_message, start_mode, flags);
                }
