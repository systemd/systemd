...
static int can_set_coredump_receive(sd_bus *bus) {
        ...
        r = sd_bus_get_property_trivial(
                        ...
                        'b', &b);
        if (r < 0) {
                if (!sd_bus_error_has_names(&error, SD_BUS_ERROR_UNKNOWN_PROPERTY, SD_BUS_ERROR_PROPERTY_READ_ONLY))
                        log_debug_errno(r, "Failed to determine if CoredumpReceive can be set, ignoring: %s",
