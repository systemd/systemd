...
static int create_or_register_machine_ex(
        ...
        r = sd_bus_message_append(m, "(sv)", "name", "s", machine_name);
        if (r < 0)
                return bus_log_create_error(r);

        r = sd_bus_message_append(m, "(sv)", "id", "ay", SD_BUS_MESSAGE_APPEND_ID128(uuid));
        if (r < 0)
                return bus_log_create_error(r);

        r = sd_bus_message_append(m, "(sv)", "service", "s", service);
        if (r < 0)
                return bus_log_create_error(r);

        r = sd_bus_message_append(m, "(sv)", "class", "s", "container");
        if (r < 0)
                return bus_log_create_error(r);

        if (pidref_is_set(pid)) {
                r = sd_bus_message_append(m, "(sv)", "leader", "h", pid->fd);
                if (r < 0)
                        return bus_log_create_error(r);
        }
        ...
        if (local_ifindex > 0) {
                r = sd_bus_message_append(m, "(sv)", "ifIndices", "ai", 1, local_ifindex);
                if (r < 0)
                        return bus_log_create_error(r);
        }
