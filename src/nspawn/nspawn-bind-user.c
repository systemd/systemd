...
static int check_shell_presence(const char *directory, const char *shell) {
        ...
        if (!shell)
                return 1; /* no user shell specified */

        r = chase(shell, directory, CHASE_PREFIX_ROOT, NULL, NULL);
        if (r == -ENOENT)
                return 1; /* shell binary isn't present in container, don't include shell in user record */
        else if (r < 0)
                return log_error_errno(r, "Failed to open user shell (%s) of container: %m", shell);
        else
                return 0; /* shell binary is present in container, include shell in user record */
}
