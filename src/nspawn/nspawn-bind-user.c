...
static int convert_user(
        ...
        if (r > 0)
                return log_error_errno(SYNTHETIC_ERRNO(EBUSY),
                                       "Sorry, the specified user already exists in the container.");
        ...
        if (r > 0)
                return log_error_errno(SYNTHETIC_ERRNO(EBUSY),
                                       "Sorry, the specified group already exists in the container.");
...
int bind_user_setup(
        ...
        for (size_t i = 0; i < c->n_data; i++) {
                ...
                /* Finally write out the main part of hte user record */
