...
static int mount_bind(const char *dest, CustomMount *m, uid_t uid_shift, uid_t uid_range) {
        ...
        /* When user namespaces are not in use (uid_shift is UID_INVALID), we cannot do idmapping.
         * Treat UID_INVALID as 0 for ownership operations and skip idmap remounting. */
        if (idmapping != REMOUNT_IDMAPPING_NONE && !uid_is_valid(uid_shift)) {
                log_debug("Ignoring idmap request for %s because user namespacing is not enabled.", m->source);
