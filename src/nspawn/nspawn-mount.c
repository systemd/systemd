...
/* --volatile= is now handled entirely through mstack: see mstack_merge_volatile() in mstack.c and its
 * call sites in nspawn.c (outer_child()), which merge the requested mode's layers into whichever
 * MStack backs the container's root - whether loaded from a .mstack/ directory or synthesized from an
 * already-mounted --directory=/--image= root via mstack_new_from_root_fd() - instead of layering a
 * separate volatile mount on top of the finished tree afterwards. */
