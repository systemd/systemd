...
static int source_path_parse(const char *p, bool allow_empty, char **ret) {
        assert(p);
        assert(ret);
...
int bind_mount_parse(CustomMount **l, size_t *n, const char *s, bool read_only) {
        ...
        r = source_path_parse(source, /*allow_empty=*/ true, &p);
        if (r < 0)
                return r;

        free_and_replace(source, p);
...
int overlay_mount_parse(CustomMount **l, size_t *n, const char *s, bool read_only) {
        ...
        if (k == 2) {
                ...
                r = source_path_parse(lower[0], /*allow_empty=*/ false, &p);
                if (r < 0)
                        return r;

                free_and_replace(lower[0], p);

                r = source_path_parse(lower[1], /*allow_empty=*/ false, &p);
                if (r < 0)
                        return r;
                ...
                STRV_FOREACH(i, lower) {
                        r = source_path_parse(*i, /*allow_empty=*/ false, &p);
                        if (r < 0)
                                return r;
                 ...
                 * in /var/tmp */
                r = source_path_parse(upper, /*allow_empty=*/ true, &p);
                if (r < 0)
                        return r;
