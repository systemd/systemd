...
static int unit_name_cut_instance(const char *f, char **ret) {
        const char *p, *e;
        char *s;
        size_t a;
...
static int enumerate_dir_d(Hashmap *bottom, const char *toppath, const char *drop) {
        ...
        *c = 0;
        ...
        STRV_FOREACH(file, list) {
                ...
                k = unit_name_cut_instance(basename(path), &tmp);
                if (k < 0 && k != -EINVAL) {
                        free(path);
                        return k;
                }
                key = tmp ? tmp : basename(path);
...
static int enumerate_dir(Hashmap *top, Hashmap *bottom, const char *path, bool dropins) {
        ...
        for (;;) {
                ...
                k = unit_name_cut_instance(basename(p), &tmp);
                if (k < 0 && k != -EINVAL) {
                        free(p);
                        return k;
                }
                key = tmp ? tmp : basename(p);
...
static int process_suffix(const char *suffix, const char *onlyprefix) {
        ...
        HASHMAP_FOREACH_KEY(file, key, top, i) {
                ...
                HASHMAP_FOREACH_KEY(drop, path, drops, j) {
                        ...
                        if (path_exists(path) && (!onlyprefix || startswith(file, onlyprefix))) {
