...
int cryptsetup_token_open(
        ...
        if (r == -ENXIO)
                log_dbg(cd, "No matching TPM2 token data found.");
        if (r)
                return r;
...
int cryptsetup_token_validate(
        ...
        if (r < 0) {
                if (r == -ENOMEM)
                        log_dbg(cd, "Could not parse " TOKEN_NAME " json object (not enough memory).");
                return 1;
        }
        ...
        /*
         * removed check for keyslot. token activation will not proceed unless there's
         * at list single assignment to keyslot paired with default device segment.
         * Multiple keyslots are fine.
         */
