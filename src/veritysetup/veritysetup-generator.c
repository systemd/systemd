...
static int create_device(const char *name, const char *service, char *hash, char *data_what, char *hash_what, char *options) {
        ...
        if (!hash)
                log_error("Verity information incomplete, hash unspecified.");
        if (!data_what)
                log_error("Verity information incomplete, data device unspecified.");
        if (!hash_what)
                log_error("Verity information incomplete, hash device unspecified.");

        if (!hash || !data_what || !hash_what)
                return -EINVAL;

        log_debug("Using %s verity data device %s,\n"
                  "                  hash device %s,\n"
                  "                      options %s,\n"
                  "                and hash %s.", name, data_what, hash_what, options, hash);
...
static int create_root_device(void) {
        return create_device(ROOT_NAME, SYSTEMD_VERITYSETUP_SERVICE_ROOT, arg_root_hash, arg_root_data_what, arg_root_hash_what, arg_root_options);
}

static int create_usr_device(void) {
        return create_device(USR_NAME, SYSTEMD_VERITYSETUP_SERVICE_USR, arg_usr_hash, arg_usr_data_what, arg_usr_hash_what, arg_usr_options);
}
...
static int determine_device(char *hash, char **data_what_p, char **hash_what_p) {
        ...
        if (l < sizeof(sd_id128_t)) {
                log_debug("Hash is shorter than 128 bits (32 characters), ignoring for discovering verity partition.");
                return 0;
        }
        ...
        if (!(*data_what_p)) {
                ...
                *data_what_p = path_join("/dev/disk/by-partuuid", ID128_TO_UUID_STRING(data_uuid));
                if (!(*data_what_p))
                        return log_oom();
        }
        ...
        if (!(*hash_what_p)) {
                ...
                *hash_what_p = path_join("/dev/disk/by-partuuid", ID128_TO_UUID_STRING(verity_uuid));
                if (!(*hash_what_p))
                        return log_oom();
        }
...
static int determine_devices(void) {
        ...
        if (arg_root_hash && !arg_root_data_what && !arg_root_hash_what) {
                r = determine_device(arg_root_hash, &arg_root_data_what, &arg_root_hash_what);
                if (r < 0)
                        return r;
                log_info("Using data device %s and hash device %s for %s.", arg_root_data_what, arg_root_hash_what, ROOT_NAME);
        }

        if (arg_usr_hash && !arg_usr_data_what && !arg_usr_hash_what) {
                r = determine_device(arg_usr_hash, &arg_usr_data_what, &arg_usr_hash_what);
                if (r < 0)
                        return r;
                log_info("Using data device %s and hash device %s for %s.", arg_usr_data_what, arg_usr_hash_what, USR_NAME);
        }
