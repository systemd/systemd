...
#define DEFINE_SET_FUNC(type, entry)                                    \
        int sd_lldp_tx_set_##entry(sd_lldp_tx *lldp_tx, type entry) {   \
                assert_return(lldp_tx, -EINVAL);                        \
                                                                        \
                lldp_tx->entry = entry;                                 \
                return 0;                                               \
...
#define DEFINE_SET_STRING_FUNC(entry)                                   \
        int sd_lldp_tx_set_##entry(sd_lldp_tx *lldp_tx, const char *entry) { \
                assert_return(lldp_tx, -EINVAL);                        \
                                                                        \
                return free_and_strdup(&lldp_tx->entry, empty_to_null(entry)); \
        }

DEFINE_SET_STRING_FUNC(port_description);
DEFINE_SET_STRING_FUNC(hostname);
DEFINE_SET_STRING_FUNC(pretty_hostname);
DEFINE_SET_STRING_FUNC(mud_url);
DEFINE_SET_FUNC(uint16_t, system_capabilities);
DEFINE_SET_FUNC(uint16_t, enabled_capabilities);
...
static int packet_append_tlv_header(uint8_t *packet, size_t packet_size, size_t *offset, uint8_t id, size_t data_len) {
        assert(packet);
        assert(offset);
...
static int packet_append_prefixed_string(
        ...
        r = packet_append_tlv_header(packet, packet_size, offset, id, prefix_len + len);
        if (r < 0)
                return r;

        memcpy(packet + *offset, prefix, prefix_len);
        *offset += prefix_len;
...
static int packet_append_string(
        ...
        return packet_append_prefixed_string(packet, packet_size, offset, id, 0, NULL, str);
}

static int lldp_tx_create_packet(sd_lldp_tx *lldp_tx, size_t *ret_len, uint8_t **ret_packet) {
        ...
        unaligned_write_be16(packet + offset, ttl);
        offset += 2;
        ...
        unaligned_write_be16(packet + offset, lldp_tx->system_capabilities);
        offset += 2;
