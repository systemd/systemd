...
static void send_message(int fd, sd_dhcp_relay_interface *downstream, sd_dhcp_message *m) {
...
static int fake_client_handler(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        case 1:
                ASSERT_EQ(t, DHCP_OFFER);
                break;
        case 2:
                ASSERT_EQ(t, DHCP_ACK);
                break;
        case 3:
                ASSERT_EQ(t, DHCP_NAK);
...
TEST(forwarding) {
        ...
        send_message(downstream_fd[1], downstream, m);
        ...
        send_message(upstream_fd[1], /* downstream= */ NULL, m);
        dhcp_message_remove_option(m, SD_DHCP_OPTION_MESSAGE_TYPE);
