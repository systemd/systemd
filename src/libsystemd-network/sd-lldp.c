...
int sd_lldp_save(sd_lldp *lldp, const char *lldp_file) {
        ...
        HASHMAP_FOREACH(c, lldp->neighbour_mib, i) {
                LIST_FOREACH(port, p, c->ports) {
                        ...
                        r = sd_lldp_tlv_packet_read_chassis_id(p->packet, &type, &mac, &length);
                        if (r < 0)
                                continue;
                        ...
                        r = sd_lldp_tlv_packet_read_port_id(p->packet, &type, &port_id, &length);
                        if (r < 0)
                                continue;
                        ...
                        r = sd_lldp_tlv_packet_read_system_name(p->packet, &k, &length);
                        if (r < 0)
                        ...
                        (void) sd_lldp_tlv_packet_read_system_capability(p->packet, &data);
...
int sd_lldp_get_tlv_packets(sd_lldp *lldp, sd_lldp_tlv_packet ***tlvs, unsigned max) {
        ...
        *tlvs = new(sd_lldp_tlv_packet *, count);
        if (!*tlvs)
                return -ENOMEM;
        ...
        HASHMAP_FOREACH(c, lldp->neighbour_mib, iter) {
                LIST_FOREACH(port, p, c->ports) {
                        if (i >= count)
                                return count;
                        (*tlvs)[i++] = sd_lldp_tlv_packet_ref(p->packet);
                }
        }
