...
static uint64_t checksum_buffer(uint64_t sum, const struct iovec *iov) {
        assert(iov);

        for (struct iovec i = *iov; iovec_is_set(&i); iovec_shift(&i, sizeof(uint64_t), &i)) {
                uint64_t t = 0;
...
uint16_t ip_checksum(const void *buf, size_t len) {
        /* See RFC1071 */
        return checksum_finalize(checksum_buffer(0, &IOVEC_MAKE(buf, len)));
}
...
static uint16_t udphdr_checksum(
        ...
        uint64_t sum = 0;
        sum = checksum_buffer(sum, &IOVEC_MAKE(&pseudo, sizeof(struct udp_pseudo_header)));
        sum = checksum_buffer(sum, &IOVEC_MAKE(udp, sizeof(struct udphdr)));
        ...
        if (payload) {
                ...
                FOREACH_ARRAY(i, payload->iovec, payload->count) {
                        ...
                        if (odd) {
                                ...
                                sum = checksum_buffer(sum, &IOVEC_MAKE(buf, 2));
                                iovec_shift(&v, 1, &v);
                        }
