 ...
 * come back out.
 *
 * On systemd without the SvcParams length fix, ndisc_option_build_encrypted_dns() writes
 * (poff - off) into the 2-byte SvcParams length field, which includes the length field
 * itself. The emitted length is therefore 2 too large, the option fails to parse with
 * -EBADMSG, and ndisc_parse_options() silently drops it.
 *
 * The 'import' : ['test-ndisc-ra'] entry in meson.build pulls in icmp6-test-util.c, which
 * reduces icmp6_send() to a plain writev() so the test can run over a socketpair rather
 * than a real ICMPv6 socket. */
 ...
 * none.
 *
 * Note that ndisc_option_get_by_type() is of no use here: options that may legitimately
 * appear multiple times in one message — DNR among them — are hashed and compared by
 * address, so a lookup with a stack-allocated key never matches. Walk the set instead, and
 * assert on the way that we did not emit or parse more than one DNR option, as that would
 * make the option picked up below arbitrary. */
...
static void verify_round_trip(
        ...
        sd_ndisc_option *p; /* owned by options */
        union in_addr_union expect_addr;

        ASSERT_GE(in_addr_from_string(AF_INET6, addr, &expect_addr), 0);

        /* This is the assertion that fails on unpatched systemd: the option we emitted
         * is rejected by our own parser with -EBADMSG and dropped, so we get -ENOENT. */
 ...
 *
 * The emitted option is 56 bytes; the SvcParams length field sits at option offset 0x2d
 * and covers the 8 bytes at 0x2f..0x36. Unpatched systemd writes 0x000a there instead of
 * 0x0008, so the parser computes 0x2f + 10 = 57 > 56 and returns -EBADMSG. */
 ...
 * strictly increasing key order per RFC 9460 § 2.2. The length is wrong by a constant 2
 * regardless of how many SvcParams are emitted. */
