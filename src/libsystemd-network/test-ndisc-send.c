...
static int parse_argv(int argc, char *argv[]) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION('i', "interface", "INTERFACE", "Network interface"):
                        r = rtnl_resolve_interface_or_warn(&rtnl, state.argument);
                        if (r < 0)
                                return r;
                        arg_ifindex = r;
                        break;
                ...
                OPTION('t', "type", "TYPE", "ICMPv6 message type"):
                        r = parse_icmp6_type(state.argument);
                        if (r < 0)
                ...
                OPTION_LONG("preference", "PREF", "Preference"):
                        r = parse_preference(state.argument);
                        if (r < 0)
                ...
                OPTION_LONG("target-address", "ADDRESS", "Target address"):
                        r = in_addr_from_string(AF_INET6, state.argument, &arg_target_address);
                        if (r < 0)
                ...
                OPTION_LONG("redirect-destination", "ADDRESS", "Redirect destination address"):
                        r = in_addr_from_string(AF_INET6, state.argument, &arg_redirect_destination);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse destination address: %m");
                        break;
                ...
                OPTION_LONG("target-ll-address", "ADDRESS", "Target link-layer address"):
                        r = parse_ether_addr(state.argument, &arg_target_mac);
                        if (r < 0)
                ...
                OPTION_LONG("redirected-header", "BASE64", "Redirected header (base64)"): {
                        ...
                        r = unbase64mem(state.argument, &p, &len);
                        if (r < 0)
                ...
                OPTION_LONG("mtu", "MTU", "MTU"):
                        r = safe_atou32(state.argument, &arg_mtu);
                        if (r < 0)
