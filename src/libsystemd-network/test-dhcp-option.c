...
static struct option_desc option_tests[] = {
        ...
        },
        {
                        ...
                        {},
                        0,
                        { 42, 5, 65, 66, 67, 68, 69, 0, 0, SD_DHCP_OPTION_MESSAGE_TYPE, 1, DHCP_ACK },
                        12,
                        true,
        },
        {
        ...
        },
        {
                        ...
                        0,
                        {
                                        0x35, 0x01, 0x05, 0x36, 0x04, 0x01, 0x00, 0xa8, 0xc0, 0x33,
                                        0x04, 0x00, 0x01, 0x51, 0x80, 0x01, 0x04, 0xff, 0xff, 0xff,
                                        0x00, 0x03, 0x04, 0xc0, 0xa8, 0x00, 0x01, 0x06, 0x04, 0xc0,
                                        0xa8, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        },
                        40,
                        true,
        },
        {
                        ...
                        {},
                        0,
                        { SD_DHCP_OPTION_MESSAGE_TYPE, 1, DHCP_OFFER, 42, 3, 0, 0, 0 },
                        8,
                        true,
        },
        {
...

        {
                        {},
                        0,
                        { 222, 3, 1, 2, 3, SD_DHCP_OPTION_MESSAGE_TYPE, 1, DHCP_NAK },
                        8,
                        { SD_DHCP_OPTION_OVERLOAD, 1, DHCP_OVERLOAD_FILE },
                        3,
                        true,
        },

        {
                        { 1, 4, 1, 2, 3, 4, SD_DHCP_OPTION_MESSAGE_TYPE, 1, DHCP_ACK },
                        9,
                        { 222, 3, 1, 2, 3 },
                        5,
                        { SD_DHCP_OPTION_OVERLOAD, 1, DHCP_OVERLOAD_FILE | DHCP_OVERLOAD_SNAME },
                        3,
                        true,
        },
};
...
static int test_options_cb(uint8_t code, uint8_t len, const void *option, void *userdata) {
        ...
        if (verbose)
                printf("DHCP code %2d(%2d) len %2d(%2d) ", code, optcode, len, optlen);
        ...
        for (unsigned i = 0; i < len; i++) {
                if (verbose)
                        printf("0x%02x(0x%02x) ", ((uint8_t *) option)[i], descoption[*descpos + 2 + i]);
...
static void test_options(struct option_desc *desc) {
        ...
        message = create_message(options, optlen, file, filelen, sname, snamelen);
...
static void test_option_removal(struct option_desc *desc) {
        ...
        assert_se((desc->len = dhcp_option_remove_option(
                                   message->options, desc->len, SD_DHCP_OPTION_MESSAGE_TYPE)) >= 0);
        assert_se(dhcp_option_parse(message, sizeof(DHCPMessage) + desc->len, NULL, NULL, NULL) < 0);
}

static uint8_t the_options[64] = { 'A',  'B',  'C',  'D',  160,  2,    0x11, 0x12, 0,    31,   8,    0x31,
...
static void test_option_set(void) {
        ...
        assert_se(dhcp_option_append(result, 0, &offset, 0, SD_DHCP_OPTION_PAD, 0, NULL) == -ENOBUFS);
        assert_se(offset == 0);

        offset = 4;
        assert_se(dhcp_option_append(result, 5, &offset, 0, SD_DHCP_OPTION_PAD, 0, NULL) == -ENOBUFS);
        assert_se(offset == 4);
        assert_se(dhcp_option_append(result, 6, &offset, 0, SD_DHCP_OPTION_PAD, 0, NULL) >= 0);
        assert_se(offset == 5);

        offset = pos = 4;
        len = 11;
        while (pos < len && the_options[pos] != SD_DHCP_OPTION_END) {
                assert_se(dhcp_option_append(
                                          result,
                                          len,
                                          &offset,
                                          DHCP_OVERLOAD_SNAME,
