...
static int ovsdb_client_dispatch(void *userdata) {
        ...
        for (;;) {
                ...
                static const sd_json_dispatch_field echo_dispatch[] = {
                ...
                if (sd_json_dispatch(message, echo_dispatch, SD_JSON_ALLOW_EXTENSIONS, &echo) >= 0 &&
                    streq_ptr(echo.method, "echo") && echo.id && !sd_json_variant_is_null(echo.id)) {
                        ...
                        sd_json_variant *params_v = echo.params;
...
void ovsdb_client_bind_state_change(OVSDBClient *c, ovsdb_state_cb_t cb) {
        assert(c);

        c->state_cb = cb;
}

void ovsdb_client_bind_notify(OVSDBClient *c, ovsdb_notify_cb_t cb) {
        assert(c);

        c->notify_cb = cb;
}

void ovsdb_client_bind_update(OVSDBClient *c, ovsdb_update_cb_t cb) {
        assert(c);
                                        ...
                                        SD_JSON_BUILD_PAIR_ARRAY("Port",
                                                SD_JSON_BUILD_OBJECT(
                                                        SD_JSON_BUILD_PAIR_ARRAY("columns",
                                                                ...
                                                                JSON_BUILD_CONST_STRING("trunks"),
                                                                JSON_BUILD_CONST_STRING("external_ids")))),
...
int ovsdb_client_transact(
        ...
        /* elements[] holds borrowed refs into `ops`; sd_json_variant_new_array() takes its
         * own ref on each, so the _cleanup_free_ frees only the pointer array, not the variants. */
