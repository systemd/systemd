...
static int ovsdb_stream_parse(JsonStream *s, sd_json_variant **ret) {
        ...
        if (sensitive) {
                msg_sensitive = strndup(begin, s->input_buffer_size);
                msg = msg_sensitive;
        } else {
                msg_plain = strndup(begin, s->input_buffer_size);
                msg = msg_plain;
        }
        if (!msg)
                return -ENOMEM;
        ...
        if (IN_SET(r, -EINVAL, -ENODATA)) {
                /* Truncation OR malformed — sd_json_parse_continue() does not
                 * distinguish. Treat as "need more data" while the buffer can still
                 * grow; once the buffer is at max and parsing still fails, the data
                 * is genuinely malformed. */
                if (s->input_buffer_size >= s->buffer_max - 1) {
