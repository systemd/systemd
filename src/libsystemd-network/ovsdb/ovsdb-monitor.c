...
typedef struct OVSDBTableCache {
        Hashmap *rows; /* sd_id128_t* uuid -> sd_json_variant* */
} OVSDBTableCache;
...
int ovsdb_monitor_apply_initial(OVSDBMonitor *m, sd_json_variant *reply) {
        ...
        JSON_VARIANT_OBJECT_FOREACH(table_name, table_data, reply) {
                ...
                JSON_VARIANT_OBJECT_FOREACH(uuid, row_wrapper, table_data) {
                        ...
                        _cleanup_free_ sd_id128_t *uuid_copy = newdup(sd_id128_t, &parsed, 1);
                        if (!uuid_copy)
                                return -ENOMEM;

                        _cleanup_(sd_json_variant_unrefp) sd_json_variant *new_ref = sd_json_variant_ref(row);

                        /* Insert-or-replace atomically so the old entry is never lost on OOM */
                        sd_id128_t *old_key = NULL;
                        sd_json_variant *old_val = hashmap_get2(tc->rows, &parsed, (void**) &old_key);
                        ...
                        if (r < 0)
                                 ...
                                 * new_ref is unrefed by _cleanup_. */
                                return r;
        ...
        JSON_VARIANT_OBJECT_FOREACH(table_name, table_data, updates) {
                ...
                JSON_VARIANT_OBJECT_FOREACH(uuid, delta, table_data) {
                        ...
                        if (initial_val || insert_val) {
                                ...
                                _cleanup_(sd_json_variant_unrefp) sd_json_variant *new_ref = sd_json_variant_ref(val);
                                ...
                                /* Always unref old_val: either it's a different object
                                 * (free it) or it's the same pointer with a bumped refcount
                                 * from sd_json_variant_ref(existing) above (balance it). */
                                sd_json_variant_unref(old_val);
                                free(old_key);
                                TAKE_PTR(uuid_copy);
