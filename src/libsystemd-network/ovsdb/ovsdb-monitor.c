...
int ovsdb_monitor_apply_initial(OVSDBMonitor *m, sd_json_variant *reply) {
        /* Per RFC 7047 §4.1.14 the initial monitor_cond reply uses the same update2
         * notation as later notifications, with each row wrapped in "initial" — which
         * ovsdb_monitor_apply_update2() handles like "insert". The caller installs a
         * fresh empty cache before applying, so stray modify/delete entries (which a
         * conforming server never puts in an initial reply) are no-ops. */
        return ovsdb_monitor_apply_update2(m, reply);
}
...
/* True if `v` is a two-element OVSDB tagged value ["<tag>", <body>] with the
 * given tag and body type. */
static bool variant_is_tagged(sd_json_variant *v, const char *tag, sd_json_variant_type_t body_type) {
        assert(tag);

        if (!v || !sd_json_variant_is_array(v) || sd_json_variant_elements(v) != 2)
                return false;

        sd_json_variant *t = sd_json_variant_by_index(v, 0);
        sd_json_variant *body = sd_json_variant_by_index(v, 1);

        return t && body &&
               sd_json_variant_is_string(t) &&
               sd_json_variant_has_type(body, body_type) &&
               streq(sd_json_variant_string(t), tag);
}

static bool variant_is_uuid_atom(sd_json_variant *v) {
        return variant_is_tagged(v, "uuid", SD_JSON_VARIANT_STRING);
}

static bool variant_is_set_wrapper(sd_json_variant *v) {
        return variant_is_tagged(v, "set", SD_JSON_VARIANT_ARRAY);
}

static bool variant_is_map_wrapper(sd_json_variant *v) {
        return variant_is_tagged(v, "map", SD_JSON_VARIANT_ARRAY);
}
...
static int extract_uuid_set(sd_json_variant *v, Set **ret) {
        ...
        /* NULL means empty — set_put_strdup() allocates the Set on first use, and all
         * consumers (SET_FOREACH, set_contains, encode_uuid_set) are NULL-safe. */
        if (!v) {
                *ret = NULL;
                return 0;
        }
        ...
        if (variant_is_uuid_atom(v)) {
                r = set_put_strdup(&out, sd_json_variant_string(sd_json_variant_by_index(v, 1)));
                if (r < 0)
                        return r;
                ...
                JSON_VARIANT_ARRAY_FOREACH(atom, body) {
                        if (!variant_is_uuid_atom(atom))
                                return -EINVAL;
                        r = set_put_strdup(&out, sd_json_variant_string(sd_json_variant_by_index(atom, 1)));
                        if (r < 0)
                                return r;
                }
        } else
 ...
 * callers like manager_ovs_get_bridge_ports() expect. */
...
/* Apply a set-XOR diff to a uuid-set column, storing the re-encoded result in `merged`.
 * An unparseable cached value or diff payload leaves the column untouched (returns 0). */
static int apply_uuid_set_diff(
                const char *table,
                const char *column,
                sd_json_variant *existing,
                sd_json_variant *diff,
                sd_json_variant **merged) {
        ...
        r = extract_uuid_set(sd_json_variant_by_key(existing, column), &old_set);
        if (r < 0) {
        ...
        r = extract_uuid_set(diff, &diff_set);
        if (r < 0) {
        ...
        return sd_json_variant_set_field(merged, column, encoded);
}
...
static int apply_string_map_diff(
                const char *table,
                const char *column,
                sd_json_variant *existing,
                sd_json_variant *diff,
...
static int apply_modify_diff(const char *table, sd_json_variant *existing,
        ...
        JSON_VARIANT_OBJECT_FOREACH(column, new_val, modify) {
                if (is_uuid_set_column(table, column))
                        r = apply_uuid_set_diff(table, column, existing, new_val, merged);
                else if (variant_is_map_wrapper(new_val))
                        r = apply_string_map_diff(table, column, existing, new_val, merged);
                else
...
sd_json_variant* ovsdb_monitor_get(OVSDBMonitor *m, const char *table, sd_id128_t uuid) {
        ...
        OVSDBTableCache *tc = hashmap_get(m->tables, table);
        if (!tc)
                return NULL;
