 ...
 * ovsdb_rpc_cancel_all() drains in-flight callbacks with a synthetic
 * NULL error. Callbacks must tolerate this and not touch client state that
 * may already be torn down. */
typedef int (*ovsdb_reply_callback_t)(
                OVSDBClient *client,
...
typedef int (*ovsdb_notify_callback_t)(
                OVSDBClient *client,
                const char *method,
...
typedef void (*ovsdb_update_callback_t)(OVSDBClient *client, void *userdata);
