...
typedef int (*ovsdb_state_callback_t)(OVSDBClient *client, OVSDBClientState old_state, OVSDBClientState new_state, void *userdata);

/* Create from pre-connected fd (for tests). The fd is consumed unconditionally —
 * closed on every error path — so callers can always assign `fd = -EBADF` after
 * the call without a leak. */
...
OVSDBClient* ovsdb_client_ref(OVSDBClient *p);
OVSDBClient* ovsdb_client_unref(OVSDBClient *p);
