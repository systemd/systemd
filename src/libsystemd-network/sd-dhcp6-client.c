...
int sd_dhcp6_client_set_duid(sd_dhcp6_client *client, sd_dhcp_duid *duid) {
        assert_return(client, -EINVAL);
        assert_return(!sd_dhcp6_client_is_running(client), -EBUSY);
...
int sd_dhcp6_client_get_duid(sd_dhcp6_client *client, sd_dhcp_duid **ret) {
        assert_return(client, -EINVAL);
...
int sd_dhcp6_client_duid_as_string(
        ...
        t = hexmem(&client->duid.duid.raw.data, client->duid.size - offsetof(struct duid, raw.data));
        if (!t)
                return -ENOMEM;
