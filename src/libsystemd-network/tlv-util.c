...
int tlv_append(TLV *tlv, uint32_t tag, size_t length, const void *data) {
        ...
        if (FLAGS_SET(tlv->flags, TLV_MERGE)) {
                 ...
                 *
                 * Note, if tlv_append_impl() is failed in the below, we do not rollback the entries, hence
                 * the caller of this function needs to discard the entire data in that case. */
...
int tlv_parse(TLV *tlv, const struct iovec *iov) {
        ...
        for (struct iovec i = *iov; iovec_is_set(&i); ) {
                ...
                if (FLAGS_SET(tlv->flags, TLV_PAD) && tag == SD_DHCP_OPTION_PAD)
                        continue;
                if (FLAGS_SET(tlv->flags, TLV_END) && tag == SD_DHCP_OPTION_END)
                        break;
...
size_t tlv_size(TLV *tlv) {
        ...
        if (FLAGS_SET(tlv->flags, TLV_APPEND_END)) {
                /* When TLV_APPEND_END is set, we currently assume the tag size is uint8_t. */
                assert(FLAGS_SET(tlv->flags, TLV_TAG_U8));
                sz++;
        }
...
int tlv_build(TLV *tlv, struct iovec *ret) {
        ...
        size_t n;
        r = hashmap_dump_keys_sorted(tlv->entries, (void***) &sorted, &n);
        if (r < 0)
                return r;
        ...
        FOREACH_ARRAY(tagp, sorted, n) {
                ...
                if ((FLAGS_SET(tlv->flags, TLV_PAD) && tag == SD_DHCP_OPTION_PAD) ||
                    (FLAGS_SET(tlv->flags, TLV_END) && tag == SD_DHCP_OPTION_END))
                        return -EINVAL;
