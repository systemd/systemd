...
int sd_radv_pref64_prefix_set_prefix(
        ...
        pref64_lifetime &= 0xfff8;
        pref64_lifetime |= prefixlen_code;
        pref64_lifetime = htobe16(pref64_lifetime);

        memcpy(&p->opt.lifetime_prefixlen, &pref64_lifetime, 2);
        memcpy(&p->opt.prefix, in6_addr->s6_addr32, sizeof(uint32_t) * 2);
