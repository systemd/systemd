...
int sd_radv_pref64_prefix_set_prefix(
        ...
        if (prefixlen > 96)
                log_radv(NULL, "Unusual PREF64 prefix length %u greater than 64", prefixlen);
        ...
        memcpy(&p->opt.prefix, in6_addr, 12);
        p->opt.lifetime_prefixlen = (htons(valid_lifetime / USEC_PER_SEC + 7) & 0xFFF8) | (prefixlen_code & 0x7);
