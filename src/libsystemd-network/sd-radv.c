...
int sd_radv_pref64_prefix_set_prefix(
        ...
        pref64_lifetime = (valid_lifetime_usec / USEC_PER_SEC) < (UINT16_MAX - 7) ? (valid_lifetime_usec / USEC_PER_SEC) + 7 : UINT16_MAX;
        p->opt.lifetime_prefixlen = htobe16((0xfff8 & pref64_lifetime) | (0x7 & prefixlen_code));
        p->opt.prefix[0] = in6_addr->s6_addr32[0];
        p->opt.prefix[1] = in6_addr->s6_addr32[1] & htobe32(m1);
        p->opt.prefix[2] = in6_addr->s6_addr32[2] & htobe32(m2);
