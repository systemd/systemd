...
static usec_t radv_compute_timeout(usec_t min, usec_t max) {
        ...
        return min + random_u64_range(max - min);
}
...
_public_ int sd_radv_set_router_lifetime(sd_radv *ra, uint64_t lifetime_usec) {
        ...
        if (lifetime_usec == 0) {
                 ...
                 * to (00) by the sender..." */
                if ((ra->flags & (0x3 << 3)) != (SD_NDISC_PREFERENCE_MEDIUM << 3))
                        return -EINVAL;
        } else {
                /* RFC 4861 section 6.2.1.
                 * MUST be either zero or between MaxRtrAdvInterval (no less than 4 seconds) and 9000 seconds.*/
                if (lifetime_usec < SD_RADV_MIN_ROUTER_LIFETIME_USEC ||
                    lifetime_usec > SD_RADV_MAX_ROUTER_LIFETIME_USEC)
                        return -EINVAL;
        }
...
_public_ int sd_radv_add_prefix(sd_radv *ra, sd_radv_prefix *p) {
        ...
        LIST_FOREACH(prefix, cur, ra->prefixes) {
                ...
                if (cur->opt.prefixlen == p->opt.prefixlen) {
                        /* p and cur may be equivalent. First inrement the counter. */
