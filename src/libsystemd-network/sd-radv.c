...
int sd_radv_pref64_prefix_set_prefix(
        ...
        uint16_t pref64_lifetime = valid_lifetime < (UINT16_MAX - 7) ? valid_lifetime + 7 : UINT16_MAX;
        ...
        p->opt.lifetime_prefixlen = htons((0xfff8 & pref64_lifetime) |  (0x7 & prefixlen_code));
        p->opt.addr[0] = in6_addr->s6_addr32[0];
        p->opt.addr[1] = in6_addr->s6_addr32[1] & htonl(m1);
        p->opt.addr[2] = in6_addr->s6_addr32[2] & htonl(m2);
