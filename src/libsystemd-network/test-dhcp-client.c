...
static void verify_reply(sd_dhcp_client *client, DHCPState state) {
        ...
        if (client->bootp) {
        ...
        } else if (state == DHCP_STATE_BOUND) {
                usec_t t;
                ASSERT_OK(sd_dhcp_lease_get_lifetime(lease, &t));
                ...
                ASSERT_ERROR(sd_dhcp_lease_get_lifetime(lease, /* ret= */ NULL), ENODATA);
                ASSERT_ERROR(sd_dhcp_lease_get_t1(lease, /* ret= */ NULL), ENODATA);
                ASSERT_ERROR(sd_dhcp_lease_get_t2(lease, /* ret= */ NULL), ENODATA);
        }
}
