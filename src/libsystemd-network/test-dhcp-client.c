...
static void test_dhcp_identifier_set_iaid(void) {
        ...
        assert_se(dhcp_identifier_set_iaid(
                                  42,
                                  mac_addr,
                                  sizeof(mac_addr),
                                  /* legacy = */ true,
...
static int check_options(uint8_t code, uint8_t len, const void *option, void *userdata) {
        ...
        case SD_DHCP_OPTION_CLIENT_IDENTIFIER: {
                ...
                assert_se(dhcp_identifier_set_iaid(
                                          42, mac_addr, ETH_ALEN, true, /* use_mac = */ true, &iaid) >= 0);
...
static int test_addr_acq_acquired(sd_dhcp_client *client, int event, void *userdata) {
        ...
        assert_se(memcmp(&addrs[0].s_addr, &test_addr_acq_ack[308], sizeof(addrs[0].s_addr)) == 0);
