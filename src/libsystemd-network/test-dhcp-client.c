...
static uint8_t test_addr_bootp_reply[] = {
        ...
        0x01, 0x34, 0x2b, 0x91, 0x02, 0x01, 0x06, 0x00,
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x63, 0x82, 0x53, 0x63, 0x36, 0x04, 0x0a, 0x00,
        0x00, 0x02, 0x01, 0x04, 0xff, 0x00, 0x00, 0x00,
...
static int test_bootp_acquired(sd_dhcp_client *client, int event,
        ...
        assert_se(sd_dhcp_lease_get_netmask(lease, &addr) >= 0);
        assert_se(memcmp(&addr.s_addr, &test_addr_bootp_reply[276],
                      sizeof(addr.s_addr)) == 0);

        if (verbose)
                printf("  BOOTP address acquired\n");
...
static int test_bootp_recv_request(size_t size, DHCPMessage *request) {
        ...
        if (verbose)
                printf("  recv BOOTP Request  0x%08x\n", be32toh(xid));
