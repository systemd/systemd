...
static void setup(sd_event_io_handler_t io_handler, sd_dhcp_client_callback_t client_handler, sd_dhcp_client **ret) {
        ...
        _cleanup_(tlv_unrefp) TLV *vendor_specific = ASSERT_NOT_NULL(tlv_new(TLV_DHCP4_VENDOR_SPECIFIC_INFORMATION));
        uint8_t c = 0;
...
static void verify_request(sd_dhcp_message *m, uint8_t type) {
        ...
        ASSERT_OK(dhcp_message_get_option_sub_tlv(m, SD_DHCP_OPTION_VENDOR_SPECIFIC, TLV_DHCP4_VENDOR_SPECIFIC_INFORMATION, &vendor_specific));
...
static void verify_anonymized_request(sd_dhcp_message *m, uint8_t type) {
        ...
        ASSERT_ERROR(dhcp_message_get_option_sub_tlv(m, SD_DHCP_OPTION_VENDOR_SPECIFIC, TLV_DHCP4_VENDOR_SPECIFIC_INFORMATION, /* ret= */ NULL), ENODATA);
        ASSERT_ERROR(dhcp_message_get_option_length_prefixed_data(m, SD_DHCP_OPTION_USER_CLASS, /* length_size= */ 1, /* ret= */ NULL), ENODATA);
