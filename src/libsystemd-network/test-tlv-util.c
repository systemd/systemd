...
TEST(tlv) {
        _cleanup_(tlv_done) TLV tlv = { .flags = TLV_DHCP4 };

        _cleanup_(iovec_done) struct iovec data1 = {}, data2a = {}, data2b = {}, data3 = {}, data4 = {};
        ...
        /* tlv_remove() */
        tlv_remove(&tlv, 44);
        ASSERT_EQ(hashmap_size(tlv.entries), 3u);
        tlv_remove(&tlv, 55);
        ASSERT_EQ(hashmap_size(tlv.entries), 3u);

        /* tlv_append_tlv() */
        _cleanup_(tlv_done) TLV tlv_copy = { .flags = TLV_DHCP4 };
        ...
        ASSERT_EQ(hashmap_size(tlv_copy.entries), 3u);
        ...
        ASSERT_EQ(sz, 6 * 2 + data1.iov_len + data2a.iov_len + data2b.iov_len + data3.iov_len + 1);
