...
static int test_client_basic(sd_event *e) {
        ...
        assert_se(sd_dhcp6_client_set_mac(client, (const uint8_t *) &mac_addr, sizeof(mac_addr), ARPHRD_ETHER) >= 0);
...
static int test_option(sd_event *e) {
        ...
        uint8_t result[] = {
                'F',  'O',  'O',  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 'B',  'A',  'R',
        };
        uint16_t optcode;
        size_t optlen;
        ...
        assert_se(dhcp6_option_parse(&buf, &zero, &optcode, &optlen, &optval) == -ENOMSG);
        ...
        assert_se(dhcp6_option_parse(&buf, &buflen, &optcode, &optlen, &optval) >= 0);
        pos += 4 + optlen;
...
static uint8_t msg_advertise[198] = {
        
};

static uint8_t msg_reply[173] = {
        
};

static uint8_t fqdn_wire[16] = { 0x04, 'h', 'o', 's', 't', 0x03, 'l', 'a', 'b', 0x05, 'i', 'n', 't', 'r', 'a', 0x00 };
...
static int test_advertise_option(sd_event *e) {
        ...
        while (pos < len) {
                ...
                case SD_DHCP6_OPTION_DNS_SERVERS:
                        assert_se(optlen == 16);
                        assert_se(dhcp6_lease_set_dns(lease, optval, optlen) >= 0);
                        break;

                case SD_DHCP6_OPTION_DOMAIN_LIST:
                        assert_se(optlen == 11);
                        assert_se(dhcp6_lease_set_domains(lease, optval, optlen) >= 0);
                        break;

                case SD_DHCP6_OPTION_SNTP_SERVERS:
                        assert_se(optlen == 16);
                        assert_se(dhcp6_lease_set_sntp(lease, optval, optlen) >= 0);
                        break;
...
static void test_client_solicit_cb(sd_dhcp6_client *client, int event, void *userdata) {
        sd_event *e = userdata;
...
static int test_client_verify_request(DHCP6Message *request, size_t len) {
        _cleanup_(sd_dhcp6_lease_unrefp) sd_dhcp6_lease *lease = NULL;
        size_t pos = 0;
        bool found_clientid = false, found_iana = false, found_serverid = false, found_elapsed_time = false, found_fqdn = false;
        struct in6_addr addr;
        be32_t val;
        uint32_t lt_pref, lt_valid;
        ...
        while (pos < len) {
                ...
                case SD_DHCP6_OPTION_CLIENTID:
                        ...
                        assert_se(!memcmp(optval, &test_duid, sizeof(test_duid)));
...
static int test_client_verify_solicit(DHCP6Message *solicit, size_t len) {
        bool found_clientid = false, found_iana = false, found_elapsed_time = false, found_fqdn = false;
        size_t pos = 0;
...
static int test_client_verify_information_request(DHCP6Message *information_request, size_t len) {
...
static int test_client_solicit(sd_event *e) {
        ...
        assert_se(sd_dhcp6_client_set_callback(client, test_client_information_cb, e) >= 0);

        assert_se(sd_event_add_time(e, &hangcheck, clock_boottime_or_monotonic(), time_now + 2 * USEC_PER_SEC, 0, test_hangcheck, NULL) >= 0);
