...
static int dump_dhcp_option_vendor_specific_information(sd_dhcp_message *m, DumpDHCPMessageFlag flags) {
        ...
        r = dhcp_message_get_option_sub_tlv(m, SD_DHCP_OPTION_VENDOR_SPECIFIC, TLV_DHCP4_VENDOR_SPECIFIC_INFORMATION, &tlv);
        if (r < 0)
                return log_error_errno(r, "Failed to read DHCP option %i: %m", SD_DHCP_OPTION_VENDOR_SPECIFIC);
...
static int dump_dhcp_option_vendor_identifying_vendor_class(sd_dhcp_message *m, DumpDHCPMessageFlag flags) {
        ...
        _cleanup_(tlv_unrefp) TLV *tlv = NULL;
        r = dhcp_message_get_option_sub_tlv(m, SD_DHCP_OPTION_VENDOR_CLASS, TLV_DHCP4_VENDOR_IDENTIFYING_OPTION, &tlv);
        if (r < 0)
                return log_error_errno(r, "Failed to read DHCP option %i: %m", SD_DHCP_OPTION_VENDOR_CLASS);
...
static int dump_dhcp_option_vendor_identifying_vendor_specific_information(sd_dhcp_message *m, DumpDHCPMessageFlag flags) {
        ...
        _cleanup_(tlv_unrefp) TLV *tlv = NULL;
        r = dhcp_message_get_option_sub_tlv(m, SD_DHCP_OPTION_VENDOR_SPECIFIC_INFORMATION, TLV_DHCP4_VENDOR_IDENTIFYING_OPTION, &tlv);
        if (r < 0)
        ...
        HASHMAP_FOREACH_KEY(value, key, tlv->entries) {
                ...
                _cleanup_(tlv_done) TLV sub_tlv = TLV_INIT(TLV_DHCP4_VENDOR_IDENTIFYING_VENDOR_SPECIFIC_INFORMATION_SUB_OPTION);
