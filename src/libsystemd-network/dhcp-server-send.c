...
int dhcp_server_send_message(
         ...
         * in ’giaddr’.
         *
         * Note, when we are in the relay mode, giaddr is our address. Do not send the message to us. */
...
static int dhcp_server_new_reply(
        ...
        r = dhcp_message_append_option_sub_tlv(message, SD_DHCP_OPTION_VENDOR_SPECIFIC, server->vendor_options);
        if (r < 0)
                return r;
