...
int dhcp6_lease_get_vendor_option(sd_dhcp6_lease *lease) {
        int r;
        sd_dhcp6_option **options = NULL;
        size_t n_vendor_options;

        assert(lease);

        r = set_dump_sorted(lease->vendor_options, (void***)&options, &n_vendor_options);
        if (r < 0)
                return r;
...
static int dhcp6_lease_insert_vendor_option(
        ...
        *option = (sd_dhcp6_option) {
        ...
        r = set_ensure_put(&lease->vendor_options, &dhcp6_option_hash_ops, TAKE_PTR(option));
        if (r < 0)
                return r;

        sd_dhcp6_option_ref(TAKE_PTR(option));

        return 1;
}

int dhcp6_lease_add_vendor_option(sd_dhcp6_lease *lease, const uint8_t *optval, size_t optlen) {
        int r;
