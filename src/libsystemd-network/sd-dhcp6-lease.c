...
static void dhcp6_lease_set_lifetime(sd_dhcp6_lease *lease) {
        ...
        if (lease->ia_na) {
                t1 = MIN(t1, be32_sec_to_usec(lease->ia_na->header.lifetime_t1));
                t2 = MIN(t2, be32_sec_to_usec(lease->ia_na->header.lifetime_t2));

                LIST_FOREACH(addresses, a, lease->ia_na->addresses)
                        min_valid_lt = MIN(min_valid_lt, be32_sec_to_usec(a->iaaddr.lifetime_valid));
        }

        if (lease->ia_pd) {
                t1 = MIN(t1, be32_sec_to_usec(lease->ia_pd->header.lifetime_t1));
                t2 = MIN(t2, be32_sec_to_usec(lease->ia_pd->header.lifetime_t2));

                LIST_FOREACH(addresses, a, lease->ia_pd->addresses)
                        min_valid_lt = MIN(min_valid_lt, be32_sec_to_usec(a->iapdprefix.lifetime_valid));
        }
