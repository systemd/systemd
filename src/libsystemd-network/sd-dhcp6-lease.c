...
int dhcp6_lease_set_captive_portal(sd_dhcp6_lease *lease, const uint8_t *optval, size_t optlen) {
        char *captive_portal = NULL;

        assert(lease);
        assert(optval || optlen == 0);

        if (optlen > 0) {
                captive_portal = malloc(optlen);
                if (!captive_portal)
                        return -ENOMEM;
                memcpy_safe(&captive_portal, optval, optlen);
        }

        return free_and_replace(lease->captive_portal, captive_portal);
}
