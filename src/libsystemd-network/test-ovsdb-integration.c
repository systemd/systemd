...
static int on_transact(
        ...
        TransactResult *t = ASSERT_PTR(userdata);
...
/* Send a transact and drive the event loop until the reply arrives, asserting success. */
static void transact_and_wait(OVSDBClient *c, sd_event *e, sd_json_variant *ops) {
        TransactResult tr = {};

        ASSERT_OK(ovsdb_client_transact(c, ops, on_transact, &tr));

        for (int i = 0; i < 500 && !tr.done; i++)
...
/* Connect a fresh client to `socket_path` and drive the event loop until READY. */
static void connect_client_ready(const char *socket_path, sd_event **ret_event, OVSDBClient **ret_client) {
...
/* Build an OVSDB ["named-uuid", "<name>"] reference. */
static void build_named_uuid_ref(const char *name, sd_json_variant **ret) {
        ASSERT_OK(sd_json_build(ret,
                SD_JSON_BUILD_ARRAY(
                        SD_JSON_BUILD_STRING("named-uuid"),
                        SD_JSON_BUILD_STRING(name))));
}

/* Build an OVSDB where clause [["name", "==", "<name>"]]. */
static void build_where_name(const char *name, sd_json_variant **ret) {
        ASSERT_OK(sd_json_build(ret,
                SD_JSON_BUILD_ARRAY(
                        SD_JSON_BUILD_ARRAY(
                                SD_JSON_BUILD_STRING("name"),
...
typedef struct SelectUuidResult {
        bool done;
        char *uuid;
} SelectUuidResult;
...
static int on_select_bridge_uuid(
        ...
        SelectUuidResult *s = ASSERT_PTR(userdata);
        ...
        s->uuid = strdup(sd_json_variant_string(uuid_str));
        if (!s->uuid)
                return log_oom();
        return 0;
}
...
TEST(ovsdb_integration_create_bridge) {
        ...
        /* Create client, connect, and drive the event loop until READY */
        _cleanup_(sd_event_unrefp) sd_event *e = NULL;
        _cleanup_(ovsdb_client_unrefp) OVSDBClient *c = NULL;
        connect_client_ready(socket_path, &e, &c);
        ...
        build_named_uuid_ref("iface_testbr", &iface_ref);
        ...
        build_named_uuid_ref("port_testbr", &port_ref);
        ...
        /* Send transact and drive event loop until it completes */
        transact_and_wait(c, e, ops);
...
TEST(ovsdb_integration_system_port_attachment) {
        ...
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *iface_ref = NULL;
        build_named_uuid_ref("iface_dummy0", &iface_ref);
        ASSERT_OK(sd_json_buildo(&port_row,
                SD_JSON_BUILD_PAIR_STRING("name", "dummy0"),
                SD_JSON_BUILD_PAIR("interfaces", SD_JSON_BUILD_VARIANT(iface_ref)),
        ...
        build_where_name("testbr", &br_where);

        _cleanup_(sd_json_variant_unrefp) sd_json_variant *port_ref = NULL;
        build_named_uuid_ref("port_dummy0", &port_ref);
...
TEST(ovsdb_integration_update_existing) {
        ...
        /* UPDATE Port where name="dummy0": set tag=20 */
        {
                ...
                build_where_name("dummy0", &port_where);
        ...
        log_info("Verifying update...");
        {
                ...
                transact_and_wait(c, e, verify_ops);
        }
...
TEST(ovsdb_integration_bond_port) {
        ...
        build_where_name("testbr", &br_where);
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *port_ref = NULL;
        build_named_uuid_ref("port_bond0", &port_ref);
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *port_set_ref = NULL;
        ASSERT_OK(sd_json_build(&port_set_ref,
                SD_JSON_BUILD_ARRAY(
                        SD_JSON_BUILD_STRING("set"),
...
TEST(ovsdb_integration_port_type_transition) {
        ...
        log_info("Creating patch port 'patch0'...");
        {
                ...
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *iface_ref = NULL;
                build_named_uuid_ref("iface_patch0", &iface_ref);
                ASSERT_OK(sd_json_buildo(&port_row,
                        SD_JSON_BUILD_PAIR_STRING("name", "patch0"),
                        SD_JSON_BUILD_PAIR("interfaces", SD_JSON_BUILD_VARIANT(iface_ref))));
                ...
                build_where_name("testbr", &br_where);
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *port_ref = NULL;
                build_named_uuid_ref("port_patch0", &port_ref);
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *port_set_ref = NULL;
                ASSERT_OK(sd_json_build(&port_set_ref,
                        SD_JSON_BUILD_ARRAY(
                                SD_JSON_BUILD_STRING("set"),
        ...
        log_info("Transitioning patch0 from patch to internal...");
        {
                ...
                build_where_name("patch0", &iface_where);
...
TEST(ovsdb_integration_tunnel_type_transition) {
        ...
        log_info("Creating vxlan tunnel 'tun0'...");
        {
                ...
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *iface_ref = NULL;
                build_named_uuid_ref("iface_tun0", &iface_ref);
                ASSERT_OK(sd_json_buildo(&port_row,
                        SD_JSON_BUILD_PAIR_STRING("name", "tun0"),
                        SD_JSON_BUILD_PAIR("interfaces", SD_JSON_BUILD_VARIANT(iface_ref))));
                ...
                build_where_name("testbr", &br_where);
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *port_ref = NULL;
                build_named_uuid_ref("port_tun0", &port_ref);
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *port_set_ref = NULL;
                ASSERT_OK(sd_json_build(&port_set_ref,
                        SD_JSON_BUILD_ARRAY(
                                SD_JSON_BUILD_STRING("set"),
        ...
        log_info("Transitioning tun0 from vxlan to gre...");
        {
                ...
                build_where_name("tun0", &iface_where);
                ASSERT_OK(ovsdb_op_update("Interface", iface_where, update_row, &update_op));
                ASSERT_OK(sd_json_build(&ops,
                        SD_JSON_BUILD_ARRAY(SD_JSON_BUILD_VARIANT(update_op))));
...
TEST(ovsdb_integration_delete_bridge) {
        ...
        /* Step 1: Create bridge "testbr2" with external_ids={networkd-managed=true} */
        {
                ...
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *iface_ref = NULL;
                build_named_uuid_ref("iface_testbr2", &iface_ref);
                ASSERT_OK(sd_json_buildo(&port_row,
                        SD_JSON_BUILD_PAIR_STRING("name", "testbr2"),
                        SD_JSON_BUILD_PAIR("interfaces", SD_JSON_BUILD_VARIANT(iface_ref))));
                ...
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *port_ref = NULL;
                build_named_uuid_ref("port_testbr2", &port_ref);
        ...
        log_info("Selecting bridge 'testbr2' UUID...");
        {
                ...
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *br_where = NULL;
                build_where_name("testbr2", &br_where);
                ASSERT_OK(ovsdb_op_select("Bridge", br_where, /* columns= */ NULL, &op_select));
                ...
                ASSERT_OK(ovsdb_client_transact(c, ops, on_select_bridge_uuid, &sel));

                for (int i = 0; i < 500 && !sel.done; i++)
                        ASSERT_OK(sd_event_run(e, 10 * USEC_PER_MSEC));

                ASSERT_TRUE(sel.done);
                ASSERT_NOT_NULL(sel.uuid);
                log_info("Bridge 'testbr2' UUID: %s", sel.uuid);
        }
        ...
        log_info("Deleting bridge 'testbr2'...");
        {
                ...
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *iface_where = NULL;
                build_where_name("testbr2", &iface_where);
                ASSERT_OK(ovsdb_op_delete("Interface", iface_where, &op_del_iface));

                _cleanup_(sd_json_variant_unrefp) sd_json_variant *port_where = NULL;
                build_where_name("testbr2", &port_where);
