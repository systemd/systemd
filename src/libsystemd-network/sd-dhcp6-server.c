...
static int64_t pool_address_offset(const struct in6_addr *base, const struct in6_addr *address) {
        ...
        for (int i = 15; i >= 0; i--) {
                ...
                if (i <= 7) /* Only the lower 8 bytes fit in uint64_t */
...
static int dhcp6_server_build_reply(
        ...
        reply->transaction_id |= htobe32((uint32_t) type << 24);
        ...
        if (req->has_ia_na && assigned_address) {
                ...
                uint32_t t1 = default_lt_sec / 2;
                uint32_t t2 = (default_lt_sec * 4) / 5;
                ...
                struct iaaddr addr_opt = {
                        ...
                        .lifetime_preferred = htobe32(default_lt_sec),
...
static int dhcp6_server_handle_request(sd_dhcp6_server *server,
        ...
        if (req->has_ia_na) {
                ...
                if (existing) {
                        ...
                        /* Refresh expiration */
                        existing->expiration = usec_add(now(CLOCK_MONOTONIC), server->default_lease_time);
                } else {
                        ...
                        if (r < 0) {
                                ...
                                *lease = (DHCP6ServerLease) {
                                ...
                                lease->expiration = usec_add(now(CLOCK_MONOTONIC), server->default_lease_time);
