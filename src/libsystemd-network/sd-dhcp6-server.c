...
int sd_dhcp6_server_configure_pool(sd_dhcp6_server *server, const struct in6_addr *address,
        ...
        /* Allocate bitmap for tracking used addresses */
...
static void dhcp6_server_cleanup_expired_leases(sd_dhcp6_server *server) {
        ...
        (void) sd_event_now(server->event, CLOCK_BOOTTIME, &now_usec);
...
static int dhcp6_server_build_reply(
        ...
        if (req->has_ia_na && assigned_address) {
                uint32_t default_lt_sec = DIV_ROUND_UP(server->default_lease_time, USEC_PER_SEC);
                uint32_t max_lt_sec = DIV_ROUND_UP(server->max_lease_time, USEC_PER_SEC);
                uint32_t preferred_lt_sec = MIN(default_lt_sec, max_lt_sec); /* RFC 8415: preferred <= valid */
                uint32_t t1 = preferred_lt_sec / 2;
                uint32_t t2 = (preferred_lt_sec * 4) / 5;
...
static int dhcp6_server_handle_solicit(sd_dhcp6_server *server,
        ...
        if (req->has_ia_na) {
                ...
                if (existing) {
                        ...
                        if (r < 0) {
                                ...
                                /* Send ADVERTISE with NoAddrsAvail status */
                                have_address = false;
                        } else
                                have_address = true;
                }
        ...
        if (req->rapid_commit && server->rapid_commit && have_address) {
                ...
                DHCP6ServerLease *existing = dhcp6_server_find_lease(server, req->client_id, req->client_id_len);
                if (existing) {
                        existing->expiration = usec_add(now_usec, server->default_lease_time);
                } else {
                        /* Create a new lease */
                        ...
                        *lease = (DHCP6ServerLease) {
                        ...
                        lease->expiration = usec_add(now_usec, server->default_lease_time);
...
static int dhcp6_server_handle_renew_rebind(sd_dhcp6_server *server,
        ...
        /* For RENEW, Server ID must match (RFC 8415, Section 16.5) */
        ...
        DHCP6ServerLease *existing = dhcp6_server_find_lease(server, req->client_id, req->client_id_len);
        if (!existing)
                return log_dhcp6_server_errno(server, SYNTHETIC_ERRNO(ENOENT),
                                              "No lease found for RENEW/REBIND client, ignoring.");
...
static int dhcp6_server_handle_confirm(sd_dhcp6_server *server,
        ...
        /* For CONFIRM, just reply with Success - we don't strictly validate the address is on-link,
         * since our pool configuration implicitly means the addresses are on-link */
...
static int dhcp6_server_handle_message(sd_dhcp6_server *server,
        ...
        case DHCP6_MESSAGE_DECLINE: {
                ...
                /* Client is reporting address conflict - release the address */
                log_dhcp6_server(server, "Received DECLINE from client.");
                if (req.has_ia_na) {
                        DHCP6ServerLease *existing = dhcp6_server_find_lease(server, req.client_id, req.client_id_len);
                        if (existing) {
                                pool_release_address(server, &existing->address);
                ...
                _cleanup_free_ uint8_t *decline_reply = NULL;
                size_t decline_reply_len;
                r = dhcp6_server_build_reply(server, DHCP6_MESSAGE_REPLY,
                                             message->transaction_id, &req,
                                             NULL, &decline_reply, &decline_reply_len);
                if (r < 0)
                        return r;

                return dhcp6_server_send_udp(server, &req.client_address, decline_reply, decline_reply_len);
        }
