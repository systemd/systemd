...
static void verify_sub_tlv(sd_dhcp_message *m, TLV *expected) {
        ...
        ASSERT_OK(dhcp_message_get_option_sub_tlv(m, SD_DHCP_OPTION_VENDOR_SPECIFIC, TLV_DHCP4_VENDOR_SPECIFIC_INFORMATION, &tlv));
...
TEST(dhcp_message) {
        ...
        /* vendor specific */
        ASSERT_OK(dhcp_message_append_option_sub_tlv(m, SD_DHCP_OPTION_VENDOR_SPECIFIC, &vendor));
        ASSERT_ERROR(dhcp_message_append_option_sub_tlv(m, SD_DHCP_OPTION_VENDOR_SPECIFIC, &vendor), EEXIST);
