...
TEST(dhcp_message) {
        ...
        /* u16 */
        uint16_t u16;
        ASSERT_OK(dhcp_message_append_option_u16(m, SD_DHCP_OPTION_MAXIMUM_MESSAGE_SIZE, 512u));
        ASSERT_OK(dhcp_message_get_option_u16(m, SD_DHCP_OPTION_MAXIMUM_MESSAGE_SIZE, &u16));
        ASSERT_EQ(u16, 512u);
        ASSERT_OK(dhcp_message_append_option_u8(m, SD_DHCP_OPTION_MAXIMUM_MESSAGE_SIZE, 32u));
        ASSERT_OK(dhcp_message_get_option_u16(m, SD_DHCP_OPTION_MAXIMUM_MESSAGE_SIZE, &u16));
        ASSERT_EQ(u16, 512u); /* the first option is used, hence the invalid later option is ignored. */
        ASSERT_ERROR(dhcp_message_get_option_u8(m, SD_DHCP_OPTION_MAXIMUM_MESSAGE_SIZE, NULL), EBADMSG); /* size mismatch */
