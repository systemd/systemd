...
int ndisc_router_parse(sd_ndisc *nd, sd_ndisc_router *rt) {
        ...
        for (;;) {
                ...
                case SD_NDISC_OPTION_ROUTE_INFORMATION:
                         ...
                         */
                        if (!IN_SET((p[3] >> 3) & 3, SD_NDISC_PREFERENCE_LOW, SD_NDISC_PREFERENCE_HIGH))
                                log_ndisc(nd, "Reserved (10) received in Route Information Option's Prf field, ignoring route information.");
...
static int get_route_info(sd_ndisc_router *rt, uint8_t **ret) {
        ...
        if (ri[2] > 128)
          ...
          * If the Reserved (10) value is received, the Route Information Option MUST be ignored.
          */
        if (!IN_SET((ri[3] >> 3) & 3, SD_NDISC_PREFERENCE_LOW, SD_NDISC_PREFERENCE_HIGH))
                return -EBADMSG;
