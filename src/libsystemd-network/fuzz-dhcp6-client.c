...
static void fuzz_client(sd_dhcp6_client *client, const uint8_t *data, size_t size, DHCP6State state) {
        ...
        if (size >= sizeof(DHCP6Message))
                assert_se(dhcp6_client_set_transaction_id(client, htobe32(0x00ffffff) & ((const DHCP6Message *) data)->transaction_id) == 0);

        /* Disable all timers for safety. */
        assert_se(event_source_disable(client->timeout_resend) >= 0);
        assert_se(event_source_disable(client->timeout_expire) >= 0);
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
        ...
        fuzz_client(client, data, size, DHCP6_STATE_RENEW);
        fuzz_client(client, data, size, DHCP6_STATE_REBIND);
