...
/* RFC 9686 Section 4.6.1: a refresh is triggered when the lifetime changes by more than 1%. */
#define DHCP6_ADDR_REG_LIFETIME_TOLERANCE_PERCENT 1
...
typedef struct DHCP6AddrRegistration {
        ...
        usec_t lifetime_valid_usec;     /* absolute time, or 0 for de-registration, or USEC_INFINITY */
        usec_t lifetime_preferred_usec; /* absolute time, or 0, or USEC_INFINITY */
...
static int dhcp6_addr_reg_send(DHCP6AddrRegistration *reg) {
        ...
        iaaddr = (struct iaaddr) {
                .address = reg->address,
                .lifetime_valid = htobe32(addr_reg_lifetime_to_sec(reg->lifetime_valid_usec, now)),
                .lifetime_preferred = htobe32(addr_reg_lifetime_to_sec(reg->lifetime_preferred_usec, now)),
        };
...
static int dhcp6_addr_reg_timeout_resend(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        if (reg->retransmit_count >= DHCP6_ADDR_REG_MRC) {
                log_dhcp6_client(
                                reg->client,
                                "Address registration: no ADDR-REG-REPLY received for %s",
                                IN6_ADDR_TO_STRING(&reg->address));

                if (reg->lifetime_valid_usec == 0)
...
static int dhcp6_addr_reg_start_transaction(DHCP6AddrRegistration *reg) {
        sd_dhcp6_client *client;
...
static usec_t addr_reg_refresh_interval(DHCP6AddrRegistration *reg, usec_t now) {
        if (reg->lifetime_valid_usec == USEC_INFINITY)
                return DHCP6_ADDR_REG_STATIC_REFRESH_USEC;

        if (reg->lifetime_valid_usec <= now)
                return 0;

        return (usec_t) ((double) usec_sub_unsigned(reg->lifetime_valid_usec, now) *
...
static int dhcp6_addr_reg_timeout_refresh(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        r = dhcp6_addr_reg_start_transaction(reg);
        if (r < 0) {
                log_dhcp6_client_errno(
                                client,
                                r,
...
static int dhcp6_addr_reg_update_refresh_time(DHCP6AddrRegistration *reg) {
        ...
        refresh_interval = addr_reg_refresh_interval(reg, now);
        reg->next_refresh_time = usec_add(now, refresh_interval);

        if (reg->lifetime_valid_usec == USEC_INFINITY) {
                /* Static address, keep refreshing */
                return event_reset_time_relative(
                                client->event,
                                &reg->timeout_refresh,
                                CLOCK_BOOTTIME,
        ...
        /* Dynamic address, do not refresh automatically. Refresh only when the lifetime changes */
...
/* RFC 9686 Section 4.6.1: "whenever the network changes the Valid Lifetime of an existing address by
 * more than 1%".*/
static bool addr_reg_lifetime_changed_with_tolerance(usec_t old_usec, usec_t new_usec, usec_t now) {
        usec_t diff;
        ...
        return diff * 100 > usec_sub_unsigned(old_usec, now) * DHCP6_ADDR_REG_LIFETIME_TOLERANCE_PERCENT;
}

static int dhcp6_addr_reg_update_lifetime(
                DHCP6AddrRegistration *reg, usec_t lifetime_valid_usec, usec_t lifetime_preferred_usec) {
        sd_dhcp6_client *client;
        bool changed;
        usec_t refresh_time;
        usec_t now;
        ...
        changed = addr_reg_lifetime_changed_with_tolerance(reg->lifetime_valid_usec, lifetime_valid_usec, now);

        reg->lifetime_valid_usec = lifetime_valid_usec;
        reg->lifetime_preferred_usec = lifetime_preferred_usec;
        ...
        if (lifetime_valid_usec == 0) {
                log_dhcp6_client(
                                ...
                                IN6_ADDR_TO_STRING(&reg->address));
                return dhcp6_addr_reg_start_transaction(reg);
        }
...
int dhcp6_client_register_address(
        ...
        if (reg)
                return dhcp6_addr_reg_update_lifetime(reg, lifetime_valid_usec, lifetime_preferred_usec);
...
void dhcp6_client_addr_reg_flush(sd_dhcp6_client *client) {
        ...
        while ((reg = hashmap_first(client->addr_registrations))) {
                if (reg->lifetime_valid_usec != 0) {
                        reg->lifetime_valid_usec = 0;
                        reg->lifetime_preferred_usec = 0;
                        (void) dhcp6_addr_reg_send(reg);
                }
                dhcp6_addr_reg_free(reg);
        }
