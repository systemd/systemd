...
static int dhcp6_addr_reg_send(DHCP6AddrRegistration *reg) {
        ...
        iaaddr = (struct iaaddr) {
                .address = reg->address,
                .lifetime_valid = htobe32(addr_reg_lifetime_to_sec(reg->valid_until_usec, now)),
                .lifetime_preferred = htobe32(addr_reg_lifetime_to_sec(reg->preferred_until_usec, now)),
        };
        ...
        elapsed_usec = MIN(usec_sub_unsigned(now, reg->transaction_start) / USEC_PER_MSEC / 10, (usec_t) UINT16_MAX);
