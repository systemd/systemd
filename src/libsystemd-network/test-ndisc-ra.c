...
static struct ether_addr mac_addr = { .ether_addr_octet = { 0x78, 0x2b, 0xcb, 0xb3, 0x6d, 0x53 } };
...
static const struct in6_addr test_rdnss = {
        { { 0x20, 0x01, 0x0d, 0xb8, 0xde, 0xad, 0xbe, 0xef, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01 } }
};
static const char *test_dnssl[] = { "lab.intra", NULL };
...
TEST(ra) {
        ...
        for (i = 0; i < ELEMENTSOF(prefix); i++) {
                ...
                assert_se(sd_radv_prefix_set_prefix(p, &prefix[i].address, prefix[i].prefixlen) >= 0);
                if (prefix[i].valid > 0)
                        assert_se(sd_radv_prefix_set_valid_lifetime(
                                                  p, prefix[i].valid * USEC_PER_SEC, USEC_INFINITY) >= 0);
                if (prefix[i].preferred > 0)
                        assert_se(sd_radv_prefix_set_preferred_lifetime(
                                                  p, prefix[i].preferred * USEC_PER_SEC, USEC_INFINITY) >= 0);
