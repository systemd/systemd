...
int sd_dhcp6_relay_set_ifname(sd_dhcp6_relay *relay, const char *ifname) {
        ...
        if (ifname && !ifname_valid_full(ifname, IFNAME_VALID_ALTERNATIVE))
                return -EINVAL;
...
int sd_dhcp6_relay_set_link_local_address(sd_dhcp6_relay *relay, const struct in6_addr *address) {
        ...
        relay->link_local_address = *address;
        return 0;
}
...
static int dhcp6_relay_forward_client_message(
        ...
        memcpy(buf + offset, &relay->link_local_address, sizeof(struct in6_addr)); /* link-address */
...
static int dhcp6_relay_forward_relay_message(
        ...
        memcpy(buf + offset, &relay->link_local_address, sizeof(struct in6_addr));
        offset += sizeof(struct in6_addr);
...
static int dhcp6_relay_handle_reply(sd_dhcp6_relay *relay, const uint8_t *message, size_t len) {
        ...
        if (!in6_addr_equal(&link_address, &relay->link_local_address) &&
            !in6_addr_is_null(&link_address)) {
        ...
        log_dhcp6_relay(relay, "Forwarding reply (type %u, %zu bytes) to client", inner_message[0], inner_message_len);

        /* Send inner message back to the client at peer-address on the client port (546) */
        return dhcp6_relay_send_message(relay, &peer_address, DHCP6_PORT_CLIENT, inner_message, inner_message_len);
}

static bool dhcp6_message_type_is_client_message(uint8_t type) {
        return IN_SET(type,
                      DHCP6_MESSAGE_SOLICIT,
                      DHCP6_MESSAGE_REQUEST,
                      DHCP6_MESSAGE_CONFIRM,
                      DHCP6_MESSAGE_RENEW,
                      DHCP6_MESSAGE_REBIND,
                      DHCP6_MESSAGE_RELEASE,
                      DHCP6_MESSAGE_DECLINE,
...
static int dhcp6_relay_receive_message(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        if (ERRNO_IS_NEG_TRANSIENT(datagram_size))
                return 0;
        if (datagram_size < 0) {
        ...
        if (dhcp6_message_type_is_client_message(msg_type)) {
        ...
        } else if (msg_type == DHCP6_MESSAGE_RELAY_FORWARD) {
                ...
                log_dhcp6_relay(relay, "Received unexpected message type %u, ignoring.", msg_type);
