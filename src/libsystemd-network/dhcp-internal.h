...
void dhcp_packet_append_ip_headers(
        DHCPPacket *packet, be32_t source_addr, uint16_t source, be32_t destination_addr, uint16_t destination, uint16_t len);
...
#define log_dhcp_client_errno(client, error, fmt, ...) \
        log_internal(LOG_DEBUG, error, __FILE__, __LINE__, __func__, "DHCP CLIENT (0x%x): " fmt, client->xid, ##__VA_ARGS__)
