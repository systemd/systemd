...
int dhcp_network_bind_raw_socket(int ifindex,
...
int dhcp_option_parse(
        DHCPMessage *message, size_t len, dhcp_option_callback_t cb, void *userdata, char **error_message);
...
#define DHCP_CLIENT_DONT_DESTROY(client) \
        _cleanup_(sd_dhcp_client_unrefp) \
                _unused_ sd_dhcp_client *_dont_destroy_##client = sd_dhcp_client_ref(client)

#define log_dhcp_client_errno(client, error, fmt, ...) \
        log_internal(                                  \
                LOG_DEBUG, error, __FILE__, __LINE__, __func__, "DHCP CLIENT (0x%x): " fmt, client->xid, ##__VA_ARGS__)
