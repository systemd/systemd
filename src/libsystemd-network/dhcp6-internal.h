...
#define log_dhcp6_client_errno(p, error, fmt, ...) \
        log_internal(LOG_DEBUG, error, __FILE__, __LINE__, __func__, "DHCPv6 CLIENT: " fmt, ##__VA_ARGS__)
#define log_dhcp6_client(p, fmt, ...) log_dhcp6_client_errno(p, 0, fmt, ##__VA_ARGS__)

int dhcp6_option_append(uint8_t **buf, size_t *buflen, uint16_t code, size_t optlen, const void *optval);
...
int dhcp6_network_send_udp_socket(int s, struct in6_addr *address, const void *packet, size_t len);
