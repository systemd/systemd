...
static int dhcp_message_append_option_classless_static_routes(sd_dhcp_message *message, uint8_t code, size_t n_routes, const sd_dhcp_route *routes) {
        ...
        FOREACH_ARRAY(route, routes, n_routes) {
                ...
                *p++ = route->dst_prefixlen;
                p = mempcpy(p, &route->dst_addr, DIV_ROUND_UP(route->dst_prefixlen, 8));
...
int dhcp_message_append_option_6rd(
        ...
        if (message_has_option(message, SD_DHCP_OPTION_6RD))
                return -EEXIST;
        ...
        if (n_br_addresses > (SIZE_MAX - 2 - sizeof(struct in6_addr)) / sizeof(struct in_addr))
                return -ENOBUFS;

        size_t buflen = 2 + sizeof(struct in6_addr) + n_br_addresses * sizeof(struct in_addr);
...
int dhcp_message_append_option_user_class(sd_dhcp_message *message, const struct iovec_wrapper *user_class) {
        ...
        size_t len = iovw_size(user_class) + user_class->count;
        _cleanup_free_ uint8_t *buf = new(uint8_t, len);
        if (!buf)
                return -ENOMEM;
...
int dhcp_message_get_option_6rd(
        ...
        _cleanup_free_ uint8_t *buf = NULL;
        size_t len;
        r = dhcp_message_get_option_alloc(message, SD_DHCP_OPTION_6RD, &len, (void**) &buf);
        if (r < 0)
                return r;
        ...
        memcpy(&prefix, buf + 2, sizeof(struct in6_addr));
        ...
        size_t n_br_addresses = (len - 2 - sizeof(struct in6_addr)) / sizeof(struct in_addr);
        _cleanup_free_ struct in_addr *br_addresses = newdup(struct in_addr, buf + 2 + sizeof(struct in6_addr), n_br_addresses);
        if (!br_addresses)
                return -ENOMEM;
        ...
        if (ret_br_addresses)
                *ret_br_addresses = TAKE_PTR(br_addresses);
        return 0;
}
...
static int dhcp_message_get_option_fqdn(sd_dhcp_message *message, uint8_t *ret_flags, char **ret_fqdn) {
        int r;
...
int dhcp_message_get_option_hostname(sd_dhcp_message *message, char **ret) {
        ...
        if (dhcp_message_get_option_fqdn(message, /* ret_flags= */ NULL, ret) >= 0)
                return 0;
...
int dhcp_message_get_option_search_domains(sd_dhcp_message *message, char ***ret) {
        ...
        _cleanup_free_ uint8_t *buf = NULL;
        size_t len;
        r = dhcp_message_get_option_alloc(message, SD_DHCP_OPTION_DOMAIN_SEARCH, &len, (void**) &buf);
        if (r < 0)
                return r;
...
int dhcp_message_build_json(sd_dhcp_message *message, sd_json_variant **ret) {
        ...
        r = dhcp_options_build_json(message->options, &w);
        if (r < 0)
                return r;
...
int dhcp_message_parse_json(sd_json_variant *v, sd_dhcp_message **ret) {
        ...
        message->options = TAKE_PTR(p.options);
