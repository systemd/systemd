...
int dhcp_message_get_option_fqdn(sd_dhcp_message *message, uint8_t *ret_flags, char **ret_fqdn) {
        ...
        r = dns_name_from_wire_format((const uint8_t**) &i.iov_base, &i.iov_len, &name);
        if (r < 0)
                return r;
        ...
        if (!string_is_safe(name, STRING_ALLOW_EMPTY | STRING_ALLOW_BACKSLASHES | STRING_ALLOW_QUOTES | STRING_ALLOW_GLOBS))
                return -EBADMSG;
...
int dhcp_message_parse_json(sd_json_variant *v, sd_dhcp_message **ret) {
        ...
        if (p.options)
                message->options.entries = TAKE_PTR(p.options->entries);
