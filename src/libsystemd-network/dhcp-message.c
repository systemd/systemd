...
int dhcp_message_init_header(
        ...
        message->header = (DHCPMessageHeader) {
                .op = op,
                .htype = arp_type,
                .hlen = arp_type == ARPHRD_INFINIBAND ? 0 : hlen,
                .xid = htobe32(xid),
...
int dhcp_message_append_option(sd_dhcp_message *message, uint8_t code, size_t length, const void *data) {
        ...
        while (length > UINT8_MAX) {
                ...
                r = dhcp_message_append_option_impl(message, code, UINT8_MAX, data);
                if (r < 0)
                        return r;
        ...
        return dhcp_message_append_option_impl(message, code, length, data);
}
