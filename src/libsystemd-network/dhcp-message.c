...
int dhcp_message_append_option_vendor_specific(sd_dhcp_message *message, Hashmap *options) {
        ...
        size_t len = dhcp_options_size(options);
        _cleanup_free_ void *buf = malloc(len);
        if (!buf)
                return -ENOMEM;

        r = dhcp_options_build(options, &IOVEC_MAKE(buf, len));
        if (r < 0)
                return r;
...
int dhcp_message_new(const uint8_t *buf, size_t len, sd_dhcp_message **ret) {
        ...
        r = dhcp_options_extract(&message->options, buf + sizeof(DHCPMessageHeader), len - sizeof(DHCPMessageHeader));
        if (r < 0)
                return r;
        ...
        if (FLAGS_SET(overload, DHCP_OVERLOAD_FILE)) {
                r = dhcp_options_extract(&message->options, message->header.file, sizeof(message->header.file));
                if (r < 0)
                        return r;
        }

        if (FLAGS_SET(overload, DHCP_OVERLOAD_SNAME)) {
                r = dhcp_options_extract(&message->options, message->header.sname, sizeof(message->header.sname));
                if (r < 0)
                        return r;
        }
...
int dhcp_message_build(const sd_dhcp_message *message, struct iovec *ret) {
        ...
        r = dhcp_options_build(message->options, &IOVEC_MAKE(p, size - sizeof(DHCPMessageHeader)));
        if (r < 0)
                return r;
