...
static int client_new_message(sd_dhcp_client *client, uint8_t type, sd_dhcp_message **ret) {
        ...
        assert(client->lease || type == DHCP_DISCOVER);
        assert(ret);
        ...
        /* We do not append no vendor options on BOOTP mode. */
        if (client->bootp) {
        ...
        if (client->anonymize && type == DHCP_DISCOVER) {
                 ...
                 *
                 * RFC7844 section 3.6:
                 * The client intending to protect its privacy SHOULD only request a minimal number of
                 * options in the PRL and SHOULD also randomly shuffle the ordering of option codes in the
                 * PRL. If this random ordering cannot be implemented, the client MAY order the option codes
                 * in the PRL by option code number (lowest to highest).
                 *
        ...
        case DHCP_REQUEST:
                ...
                case DHCP_STATE_REBINDING:
                        ...
                        message->header.ciaddr = client->lease->address;
                        break;
         ...
         * RFC7844 section 3:
         * SHOULD NOT contain any other option (when running on anonymize mode). */
        if (IN_SET(type, DHCP_RELEASE, DHCP_DECLINE) || client->anonymize) {
                *ret = TAKE_PTR(message);
                return 0;
        }
        ...
        r = dhcp_message_append_option_u16(
                        ...
                        MIN(client->mtu, (uint32_t) UINT16_MAX));
        if (r < 0)
                return r;
        ...
        sd_dhcp_option *i;
        ORDERED_HASHMAP_FOREACH(i, client->extra_options) {
                r = dhcp_message_append_option(message, i->option, i->length, i->data);
                if (r < 0)
                        return r;
        }
...
int dhcp_client_send_message(sd_dhcp_client *client, uint8_t type) {
        ...
        if ((type == DHCP_REQUEST && client->state == DHCP_STATE_RENEWING) || type == DHCP_RELEASE)
