...
int sd_dhcp_server_set_boot_server(sd_dhcp_server *server, const struct in_addr *address) {
        assert_return(server, -EINVAL);

        if (address)
                server->boot_server = *address;
        else
                server->boot_server = (struct in_addr) {};
...
static int server_send_offer_or_ack(
        ...
        packet->dhcp.siaddr = server->boot_server.s_addr;
