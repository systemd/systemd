...
static int dhcp_server_append_static_hostname(
        ...
        if (strchr(static_lease->hostname, '.')) {
                ...
                if (r > DHCP_MAX_FQDN_LENGTH)
                        return log_dhcp_server_errno(
                                        server, SYNTHETIC_ERRNO(EINVAL), "FQDN for static lease too long");

                return dhcp_option_append(
                                &packet->dhcp, max_optlen, offset, 0, SD_DHCP_OPTION_FQDN, 3 + r, buffer);
        } else {
                /* Option 12: Simple hostname (max 63 chars for single label) */
                size_t hostname_len = strlen(static_lease->hostname);

                if (hostname_len > 63)
                        return log_dhcp_server_errno(
                                        server,
                                        SYNTHETIC_ERRNO(EINVAL),
                                        "Simple hostname for static lease too long (max 63 characters)");
...
static int server_send_offer_or_ack(
        ...
        sd_dhcp_server_lease *static_lease = dhcp_server_get_static_lease(server, req);
        r = dhcp_server_append_static_hostname(server, packet, req->max_optlen, &offset, static_lease);
        if (r < 0)
                return r;
