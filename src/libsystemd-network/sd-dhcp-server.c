...
int sd_dhcp_server_add_static_lease(
        ...
        assert_return(client_id_size == ETH_ALEN, -EINVAL);
        assert_return(!sd_dhcp_server_is_running(server), -EBUSY);
...
int sd_dhcp_server_remove_static_lease(sd_dhcp_server *server, uint8_t *client_id, size_t client_id_size) {
        ...
        c.data = memdup(client_id, client_id_size);
        if (!c.data)
                return -ENOMEM;
