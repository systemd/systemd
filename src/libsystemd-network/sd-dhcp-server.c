...
static int dhcp_server_append_static_hostname(
                sd_dhcp_server *server, DHCPPacket *packet, size_t max_optlen, size_t *offset, DHCPRequest *req) {
...
static int server_send_offer_or_ack(
        ...
        r = dhcp_server_append_static_hostname(server, packet, req->max_optlen, &offset, req);
        if (r < 0)
                return r;
