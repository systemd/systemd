...
static sd_dhcp_server *dhcp_server_free(sd_dhcp_server *server) {
        ...
        tlv_done(&server->relay_agent_information);
...
int sd_dhcp_server_new(sd_dhcp_server **ret, int ifindex) {
        ...
        *server = (sd_dhcp_server) {
                ...
                .rapid_commit = true,
                .lease_dir_fd = -EBADF,
                .relay_agent_information = TLV_INIT(TLV_DHCP4_RELAY_AGENT_INFORMATION),
        };
...
int sd_dhcp_server_set_relay_agent_information(
        ...
        /* This does not rollback on failure. The caller needs to set the previous values on failure if necessary. */

        tlv_remove(&server->relay_agent_information, SD_DHCP_RELAY_AGENT_CIRCUIT_ID);
        tlv_remove(&server->relay_agent_information, SD_DHCP_RELAY_AGENT_REMOTE_ID);

        if (!isempty(agent_circuit_id)) {
                r = tlv_append(&server->relay_agent_information,
                               SD_DHCP_RELAY_AGENT_CIRCUIT_ID,
                               strlen(agent_circuit_id),
                               agent_circuit_id);
                if (r < 0)
                        return r;
        }

        if (!isempty(agent_remote_id)) {
                r = tlv_append(&server->relay_agent_information,
                               SD_DHCP_RELAY_AGENT_REMOTE_ID,
                               strlen(agent_remote_id),
                               agent_remote_id);
                if (r < 0)
                        return r;
        }
