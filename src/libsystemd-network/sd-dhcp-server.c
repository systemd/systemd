...
static bool static_leases_have_address(sd_dhcp_server *server, be32_t requestedip) {
        DHCPLease *s;
                ...
                if (static_lease)
                        ...
                        for (i = 0; i < server->pool_size; i++) {
                                if (!server->bound_leases[next_offer]) {
                                        ...
                                        if(!static_leases_have_address(server, address))
                                                break;
                                }
...
int sd_dhcp_server_set_static_lease(
        ...
        * the server removes any static lease with the specified mac address. */
        if(address == NULL || address->s_addr == 0) {
                c = (DHCPClientId) {
