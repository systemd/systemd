...
static bool ndisc_option_equal(const sd_ndisc_option *a, const sd_ndisc_option *b) {
        ASSERT_NOT_NULL(a);
        ASSERT_NOT_NULL(b);
...
static void ndisc_round_trip(Set *options) {
        ...
        ASSERT_OK(socketpair(AF_UNIX, SOCK_DGRAM | SOCK_CLOEXEC | SOCK_NONBLOCK, /* protocol= */ 0, pair));
...
static void append_prefix(
        ...
        ASSERT_OK(ndisc_option_set_prefix(options, flags, prefixlen, &a.in6,
