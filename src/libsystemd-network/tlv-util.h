...
typedef enum TLVFlag {
         ...
         * handle it. Also note that the END tag is optional, hence we handle it on parse, but let's not
         * append it on build. */
        TLV_DHCP4_VENDOR_SPECIFIC_INFORMATION
                         = TLV_TAG_U8 | TLV_LENGTH_U8 | TLV_PAD | TLV_END,
        /* DHCPv4 Relay Agent Information option (82).
         * In RFC 3046, it is explicitly mentioned that PAD and END are not supported. */
