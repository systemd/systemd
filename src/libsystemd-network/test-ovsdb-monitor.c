...
/* Look up a key in a cached ["map", [["k","v"], ...]] column, via the library's
 * own accessor — the tests here exercise apply_update2, not map decoding. */
static const char* test_map_get(sd_json_variant *row, const char *column, const char *key) {
        return ovsdb_map_get(sd_json_variant_by_key(row, column), key);
}
