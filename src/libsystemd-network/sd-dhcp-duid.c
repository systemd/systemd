...
int sd_dhcp_duid_is_set(sd_dhcp_duid *duid) {
        ...
        return duid->size >= MIN_DUID_LEN && duid->size <= MAX_DUID_LEN;
}

int sd_dhcp_duid_get(sd_dhcp_duid *duid, uint16_t *ret_type, const void **ret_data, size_t *ret_size) {
        ...
        *ret_type = duid->duid.type;
        *ret_data = duid->duid.raw.data;
        *ret_size = duid->size - sizeof(be16_t);
        return 0;
}

int sd_dhcp_duid_get_raw(sd_dhcp_duid *duid, const void **ret_data, size_t *ret_size) {
        assert_return(sd_dhcp_duid_is_set(duid), -EINVAL);
        assert_return(ret_data, -EINVAL);
        assert_return(ret_size, -EINVAL);
...
int sd_dhcp_duid_set(
        ...
        assert_return(data_size >= MIN_DUID_DATA_LEN && data_size <= MAX_DUID_DATA_LEN, -EINVAL);

        unaligned_write_be16(&duid->duid.type, duid_type);
        memcpy(duid->duid.raw.data, data, data_size);

        duid->size = offsetof(struct duid, raw.data) + data_size;
        return 0;
}
...
int sd_dhcp_duid_set_raw(
        ...
        assert_return(duid, -EINVAL);
        assert_return(data, -EINVAL);
        assert_return(data_size >= MIN_DUID_LEN && data_size <= MAX_DUID_LEN, -EINVAL);

        /* Unlike sd_dhcp_duid_set(), this takes whole DUID including its type. */

        memcpy(&duid->duid, data, data_size);
...
int sd_dhcp_duid_to_string(sd_dhcp_duid *duid, char **ret) {
        ...
        x = hexmem(&duid->duid.raw.data, duid->size - offsetof(struct duid, raw.data));
        if (!x)
                return -ENOMEM;
