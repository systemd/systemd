...
int sd_dhcp_duid_is_set(sd_dhcp_duid *duid) {
        ...
        return duid->size >= sizeof(be16_t) && duid->size <= MAX_DUID_LEN;
}
...
int sd_dhcp_duid_set(
        ...
        assert_return(data_size <= MAX_DUID_DATA_LEN, -EINVAL);

        unaligned_write_be16(&duid->duid.type, duid_type);
        memcpy_safe(duid->duid.raw.data, data, data_size);
...
int sd_dhcp_duid_set_raw(
        ...
        assert_return(data_size >= sizeof(be16_t) && data_size <= MAX_DUID_LEN, -EINVAL);
