...
static int ndisc_option_build_redirected_header(const sd_ndisc_option *option, uint8_t **ret) {
        ...
        buf[0] = SD_NDISC_OPTION_REDIRECTED_HEADER;
        buf[1] = len;

        uint8_t *p = buf + 2;
        p = mempset(p, 0, sizeof(struct nd_opt_rd_hdr) - 2);
        p = mempcpy(p, &option->hdr, sizeof(struct ip6_hdr));
        memset(p, 0, len * 8 - sizeof(struct nd_opt_rd_hdr) + sizeof(struct ip6_hdr));
...
static int ndisc_option_parse_mtu(size_t offset, size_t len, const uint8_t *opt, Set **options) {
        ...
        return ndisc_option_add_mtu(options, offset, mtu);
}
...
int ndisc_option_add_dnssl(Set **options, size_t offset, char * const *domains, usec_t lifetime) {
        ...
        _cleanup_strv_free_ char **dup = strv_copy(domains);
        if (!dup)
                return -ENOMEM;
...
int ndisc_option_add_captive_portal(Set **options, size_t offset, const char *portal) {
        ...
        _cleanup_free_ char *dup = strdup(portal);
        if (!dup)
                return -ENOMEM;
...
static int ndisc_option_build_captive_portal(const sd_ndisc_option *option, uint8_t **ret) {
        ...
        buf[1] = len;
        memset(mempcpy(buf + 2, option->captive_portal, len_portal), 0, len * 8 - 2 - len_portal);
...
int ndisc_build_iov(const struct icmp6_hdr *hdr, Set *options, struct iovec **ret_iovec, size_t *ret_n_iovec) {
        ...
        iovec = new(struct iovec, 1 + set_size(options));
        if (!iovec)
                return -ENOMEM;
        ...
        _cleanup_free_ uint8_t *dup = newdup(uint8_t, hdr, hdr_size);
        if (!dup)
                return -ENOMEM;

        iovec[n_iovec++] = IOVEC_MAKE(TAKE_PTR(dup), hdr_size);
        ...
        SET_FOREACH(option, options) {
                ...
                iovec[n_iovec++] = IOVEC_MAKE(buf, buf[1] * 8);
                TAKE_PTR(buf);
        }

        *ret_iovec = TAKE_PTR(iovec);
        *ret_n_iovec = n_iovec;
        return 0;
}
