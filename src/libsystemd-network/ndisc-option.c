...
static int ndisc_option_add_redirected_header(Set **options, size_t offset, const struct ip6_hdr *hdr) {
        assert(options);
        assert(hdr);
...
static int ndisc_option_add_flags_extension(Set **options, size_t offset, uint64_t flags) {
        assert(options);
...
static int ndisc_option_add_captive_portal(Set **options, size_t offset, const char *portal) {
        assert(options);
