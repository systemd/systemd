...
int lldp_mib_update_objects(lldp_chassis *c, tlv_packet *tlv) {
        ...
        r = sd_lldp_tlv_read_port_id(tlv, &type, &length, &data);
        if (r < 0)
                return r;
        ...
        LIST_FOREACH(port, p, c->ports) {
                ...
                if ((p->type == type && p->length == length && !memcmp(p->data, data, p->length))) {
                        ...
                        r = sd_lldp_tlv_read_ttl(tlv, &ttl);
                        if (r < 0)
                                return r;

                        p->until = ttl * USEC_PER_SEC + now(clock_boottime_or_monotonic());

                        sd_lldp_tlv_unref(p->packet);
                        p->packet = tlv;
...
int lldp_mib_add_objects(Prioq *by_expiry,
        ...
        r = sd_lldp_tlv_read_chassis_id(tlv, &subtype, &length, &data);
        if (r < 0)
                goto drop;
...
int lldp_chassis_new(tlv_packet *tlv,
        ...
        r = sd_lldp_tlv_read_chassis_id(tlv, &type, &length, &data);
        if (r < 0)
                return r;
