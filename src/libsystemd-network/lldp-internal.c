...
int lldp_mib_update_objects(lldp_chassis *c, tlv_packet *tlv) {
        ...
        r = sd_lldp_tlv_read_port_id(tlv, &type, &data, &length);
        if (r < 0)
                return r;
...
int lldp_mib_add_objects(Prioq *by_expiry,
        ...
        r = sd_lldp_tlv_read_chassis_id(tlv, &subtype, &data, &length);
        if (r < 0)
                goto drop;
...
int lldp_chassis_new(tlv_packet *tlv,
        ...
        r = sd_lldp_tlv_read_chassis_id(tlv, &type, &data, &length);
        if (r < 0)
                return r;
