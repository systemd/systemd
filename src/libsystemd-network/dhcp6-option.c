...
int dhcp6_option_parse(
        ...
        if (*offset + offsetof(DHCP6Option, data) >= buflen)
                return -ENOMSG;

        option = (const DHCP6Option*) (buf + *offset);
        len = be16toh(option->len);

        if (*offset + offsetof(DHCP6Option, data) + len > buflen)
                return -ENOMSG;
...
int dhcp6_option_parse_status(const uint8_t *data, size_t data_len, char **ret_status_message) {
        ...
        if (ret_status_message) {
                ...
                msg = memdup_suffix0(data + sizeof(uint16_t), data_len - sizeof(uint16_t));
                if (!msg)
                        return -ENOMEM;
        ...
        return be16toh(*((const be16_t*) data));
}
...
static int dhcp6_option_parse_ia_options(sd_dhcp6_client *client, const uint8_t *buf, size_t buflen) {
        ...
        for(size_t offset = 0; offset < buflen;) {
                ...
                default:
                        log_dhcp6_client(client, "IA option contains unknown sub option, ignoring");
                }
        }
...
int dhcp6_option_parse_ia(
        ...
        case SD_DHCP6_OPTION_IA_NA:
                ...
                if (((const struct ia_na*) option_data)->id != iaid)
        ...
        case SD_DHCP6_OPTION_IA_PD:
                ...
                /* According to RFC8415, IAs which do not match the client's IAID should be ignored,
                 * but not necessary to ignore or refuse the whole message. */
                if (((const struct ia_pd*) option_data)->id != iaid)
                        /* ENOANO indicates the option should be ignored. */
                        return log_dhcp6_client_errno(client, SYNTHETIC_ERRNO(ENOANO),
        ...
        case SD_DHCP6_OPTION_IA_TA:
                ...
                /* According to RFC8415, IAs which do not match the client's IAID should be ignored,
                 * but not necessary to ignore or refuse the whole message. */
                if (((const struct ia_ta*) option_data)->id != iaid)
                        /* ENOANO indicates the option should be ignored. */
                        return log_dhcp6_client_errno(client, SYNTHETIC_ERRNO(ENOANO),
        ...
        for (; offset < option_data_len;) {
                ...
                case SD_DHCP6_OPTION_IAADDR: {
                        ...
                        if (!IN_SET(option_code, SD_DHCP6_OPTION_IA_NA, SD_DHCP6_OPTION_IA_TA))
                                return log_dhcp6_client_errno(client, SYNTHETIC_ERRNO(EINVAL),
                                                              "Received an IA_PD option with an IA address.");
                ...
                case SD_DHCP6_OPTION_IA_PD_PREFIX: {
                        ...
                        if (option_code != SD_DHCP6_OPTION_IA_PD)
                                return log_dhcp6_client_errno(client, SYNTHETIC_ERRNO(EINVAL),
                                                              "Received an IA_NA or IA_TA option with an PD prefix.");
                ...
                default:
                        log_dhcp6_client(client, "Received an IA option with an unknown sub-option %d, ignoring", subopt);
                }
        }

        if (lt_t1 == 0 && lt_t2 == 0 && lt_min != UINT32_MAX) {
                lt_t1 = lt_min / 2;
                lt_t2 = lt_min / 10 * 8;

                log_dhcp6_client(client, "Received an IA option with both T1 and T2 are zero. "
                                 "Adjusting them with the valid lifetime of IA addresses or PD prefixes: "
