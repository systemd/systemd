        ...
        for (size_t offset = sizeof(struct nd_neighbor_advert), length; offset < na->packet->raw_size; offset += length) {
                ...
                case SD_NDISC_OPTION_TARGET_LL_ADDRESS:
                        if (length != sizeof(struct ether_addr) + 2) {
                                log_ndisc(nd, "Neighbor advertisement target link-layer address option with invalid length, ignoring the option.");
                                continue;
                        }

                        if (!ether_addr_is_null(&na->target_mac)) {
                                log_ndisc(nd, "Neighbor advertisement has multiple target link-layer address options, ignoring the option.");
                                continue;
                        }

                        memcpy(&na->target_mac, p + 2, sizeof(struct ether_addr));
                        break;
                }
        }
