...
DEFINE_STRING_TABLE_LOOKUP(duid_type, DUIDType);
...
int dhcp_identifier_set_duid_en(bool test_mode, struct duid *ret_duid, size_t *ret_len) {
        ...
        if (!test_mode) {
                ...
                /* For tests, especially for fuzzers, it is important the reproducibility.
                 * Hence, use the static and constant machine id.
