...
int ndisc_redirect_parse(sd_ndisc *nd, sd_ndisc_redirect *rd) {
        ...
        if (in6_addr_is_null(&ra->target_address) || in6_addr_is_multicast(&ra->target_address))
                return log_ndisc_errno(nd, SYNTHETIC_ERRNO(EBADMSG),
                                       "Received Redirect message with an invalid target address, ignoring datagram: %m");

        if (in6_addr_is_null(&ra->destination_address) || in6_addr_is_multicast(&ra->destination_address))
                return log_ndisc_errno(nd, SYNTHETIC_ERRNO(EBADMSG),
