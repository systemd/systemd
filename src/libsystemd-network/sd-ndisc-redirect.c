...
int ndisc_redirect_parse(sd_ndisc *nd, sd_ndisc_redirect *rd) {
        ...
        for (size_t offset = sizeof(struct nd_redirect), length; offset < rd->packet->raw_size; offset += length) {
                ...
                if (r < 0)
                        return log_ndisc_errno(nd, r, "Failed to parse NDisc option header, ignoring datagram: %m");

                switch (type) {

                case SD_NDISC_OPTION_TARGET_LL_ADDRESS:
                        if (length != sizeof(struct ether_addr) + 2) {
                                log_ndisc(nd, "Redirect message target link-layer address option with invalid length, ignoring the option.");
                                continue;
                        }

                        if (!ether_addr_is_null(&rd->target_mac)) {
                                log_ndisc(nd, "Redirect message has multiple target link-layer address options, ignoring the option.");
                                continue;
                        }
                ...
                case SD_NDISC_OPTION_REDIRECTED_HEADER:
                        if (length < sizeof(struct nd_opt_rd_hdr) + sizeof(struct ip6_hdr)) {
                                log_ndisc(nd, "Redirected header option with invalid length, ignoring the option.");
                                continue;
                        }

                        if (rd->redirected_header.ip6_vfc != 0) {
                                log_ndisc(nd, "Redirect message has multiple redirected header options, ignoring the option.");
                                continue;
                        }
