...
int dhcp_lease_save(sd_dhcp_lease *lease, const char *lease_file) {
        ...
        if (r >= 0) {
                ...
                client_id_hex = hexmem(&client_id->id, client_id->size);
                if (!client_id_hex)
                        return -ENOMEM;
...
int dhcp_lease_load(sd_dhcp_lease **ret, const char *lease_file) {
        ...
        if (client_id_hex) {
                ...
                r = unhexmem(client_id_hex, SIZE_MAX, &data, &len);
                if (r < 0)
                        log_debug_errno(r, "Failed to parse client ID %s, ignoring: %m", client_id_hex);

                if (len <= 0 || len > MAX_CLIENT_ID_LEN)
                        log_debug("Invalid client ID %s, ignoring: %m", client_id_hex);;

                memcpy(&lease->client_id.id, data, len);
                lease->client_id.size = len;
        }
...
int dhcp_lease_set_client_id(sd_dhcp_lease *lease, sd_dhcp_client_id *client_id) {
        assert(lease);
        assert(client_id);
