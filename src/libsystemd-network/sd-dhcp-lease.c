...
static int lease_parse_be32_seconds(const uint8_t *option, size_t len, usec_t *ret) {
        assert(option);
        assert(ret);
        ...
        *ret = unaligned_be32_sec_to_usec(option, /* max_as_infinity = */ true);
        return 0;
}
...
int dhcp_lease_parse_options(uint8_t code, uint8_t len, const void *option, void *userdata) {
        ...
        case SD_DHCP_OPTION_IP_ADDRESS_LEASE_TIME:
                r = lease_parse_be32_seconds(option, len, &lease->lifetime);
                if (r < 0)
        ...
        case SD_DHCP_OPTION_RENEWAL_TIME:
                r = lease_parse_be32_seconds(option, len, &lease->t1);
                if (r < 0)
                        log_debug_errno(r, "Failed to parse T1 time, ignoring: %m");
                break;

        case SD_DHCP_OPTION_REBINDING_TIME:
                r = lease_parse_be32_seconds(option, len, &lease->t2);
                if (r < 0)
