...
static int dhcp_request_set_client_id(sd_dhcp_request *req) {
        ...
        size_t len;
        if (dhcp_message_get_option_alloc(req->message, SD_DHCP_OPTION_CLIENT_IDENTIFIER, &len, &buf) >= 0 &&
            client_id_size_is_valid(len))
                return sd_dhcp_client_id_set_raw(&req->client_id, buf, len);

        /* If not set, fall back to use chaddr. */
        if (!client_id_data_size_is_valid(req->message->header.hlen))
                return -EBADMSG;

        return sd_dhcp_client_id_set(
                        &req->client_id,
...
static int dhcp_request_set_server_identifier(sd_dhcp_request *req, sd_dhcp_server *server) {
        int r;
        ...
        if (!sd_dhcp_server_is_in_relay_mode(server) && a != server->address)
                return -EBADMSG;
...
static int dhcp_request_set_sender_hardware_address(sd_dhcp_request *req) {
        int r;
        ...
        req->hw_addr.length = req->message->header.hlen;
        memcpy_safe(req->hw_addr.bytes, req->message->header.chaddr, req->message->header.hlen);

        if (hw_addr_is_valid(&req->hw_addr, req->message->header.htype))
                return 0;
        ...
        if (type <= 0 || type >= UINT8_MAX || type != req->message->header.htype)
                return -EBADMSG;

        if (size > HW_ADDR_MAX_SIZE)
                return -EBADMSG;

        req->hw_addr.length = size;
        memcpy_safe(req->hw_addr.bytes, data, size);

        if (!hw_addr_is_valid(&req->hw_addr, type))
                return -EBADMSG;
...
static int dhcp_request_set_maximum_message_size(sd_dhcp_request *req) {
        ...
        if (sz < 576) /* 576: minimum IP packet size */
                return -EBADMSG;
...
static int dhcp_request_set_lifetime(sd_dhcp_request *req, sd_dhcp_server *server) {
        ...
        be32_t sec;
        if (dhcp_message_get_option_be32(req->message, SD_DHCP_OPTION_IP_ADDRESS_LEASE_TIME, &sec) >= 0)
                req->lifetime = be32_sec_to_usec(sec, /* max_as_infinity= */ true);
        if (req->lifetime <= 0)
...
static int dhcp_server_parse_message(sd_dhcp_server *server, const struct iovec *iov, sd_dhcp_request **ret) {
        ...
        *req = (sd_dhcp_request) {
        ...
        /* Server Identifier */
        r = dhcp_request_set_server_identifier(req, server);
        if (r < 0)
                return r;

        /* client hardware address */
        r = dhcp_request_set_sender_hardware_address(req);
        if (r < 0)
                return r;
