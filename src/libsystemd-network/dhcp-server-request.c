...
int dhcp_server_relay_message(sd_dhcp_server *server, const struct iovec *iov) {
        ...
        case BOOTREQUEST:
                 ...
                 * Relay agents configured to add a Relay Agent option which receive a client DHCP packet
                 * with a nonzero giaddr SHALL discard the packet if the giaddr spoofs a giaddr address
                 * implemented by the local agent itself. */
                if (req->message->header.giaddr == server->address)
                        return -EBADMSG;

                /* RFC 3046 section 2.1.1:
                 * the relay agent SHALL forward any received DHCP packet with a valid non-zero giaddr
                 * WITHOUT adding any relay agent options. Per RFC 2131, it shall also NOT modify the giaddr
                 * value.
                 *
                 * Hence, here, set giaddr and the relay agent information option only when the giaddr in the
                ...
                if (req->message->header.giaddr == INADDR_ANY) {
                        ...
                        if (dhcp_message_has_option(req->message, SD_DHCP_OPTION_RELAY_AGENT_INFORMATION))
                                return -EBADMSG;

                        r = dhcp_message_append_option_sub_tlv(
                                        req->message,
                                        SD_DHCP_OPTION_RELAY_AGENT_INFORMATION,
                                        &server->relay_agent_information);
                        if (r < 0)
                                return r;
                }
        ...
        case BOOTREPLY: {
                 ...
                 *
                 * Here, we do not check the contents of the option, and unconditionally removes the option. */
