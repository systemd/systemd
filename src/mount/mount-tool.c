...
static int umount_by_device(sd_bus *bus, const char *what) {
        ...
        if (sd_device_get_property_value(d, "SYSTEMD_MOUNT_WHERE", &v) >= 0 && !isempty(v))
...
static int acquire_mount_type(sd_device *d) {
        ...
        if (sd_device_get_property_value(d, "ID_FS_TYPE", &v) < 0 || isempty(v))
                return 0;
...
static int acquire_mount_options(sd_device *d) {
        ...
        if (sd_device_get_property_value(d, "SYSTEMD_MOUNT_OPTIONS", &v) < 0 || isempty(v))
                return 0;
...
static int acquire_mount_where(sd_device *d) {
        ...
        if (sd_device_get_property_value(d, "SYSTEMD_MOUNT_WHERE", &v) < 0 || isempty(v)) {
                ...
                if (!name) {
                        ...
                        if (sd_device_get_devname(d, &dn) < 0 || isempty(dn))
                                return 0;
...
static int acquire_removable(sd_device *d) {
        ...
        for (;;) {
                if (sd_device_get_sysattr_value(d, "removable", &v) > 0 && !isempty(v))
                        break;

                if (sd_device_get_parent(d, &d) < 0)
                        return 0;

                if (sd_device_get_subsystem(d, &v) < 0 || !streq_ptr(v, "block"))
                        return 0;
        }
...
static int discover_loop_backing_file(void) {
        ...
        if (sd_device_get_property_value(d, "ID_FS_USAGE", &v) < 0 || !streq_ptr(v, "filesystem")) {
