...
static int property_get_hardware_property(
        ...
        assert(reply);
        assert(c);
        assert(IN_SET(prop, PROP_VENDOR, PROP_MODEL, PROP_SERIAL));
        assert(getter);
...
static int property_get_hardware_serial(
        ...
        return property_get_hardware_property(reply, userdata, PROP_SERIAL, get_hardware_serial);
}
...
static int method_describe(sd_bus_message *m, void *userdata, sd_bus_error *error) {
        ...
        r = json_build(&v, JSON_BUILD_OBJECT(
                                       ...
                                       JSON_BUILD_PAIR("HardwareSerial", JSON_BUILD_STRING(serial ?: c->data[PROP_SERIAL])),
