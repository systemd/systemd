...
static int get_chassis_asset_tag(Context *c, char **ret) {
        return get_dmi_properties(c, STRV_MAKE_CONST("ID_MODEL_FROM_DATABASE", "ID_CHASSIS_ASSET_TAG"), ret);
}
...
static int build_describe_response(Context *c, bool privileged, sd_json_variant **ret) {
        ...
        r = sd_json_buildo(
                        ...
                        SD_JSON_BUILD_PAIR("ChassisAssetTag", SD_JSON_BUILD_STRING(chassis_asset_tag)));
        if (r < 0)
