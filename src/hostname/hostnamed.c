...
static int get_hardware_sku(Context *c, char **ret) {
        ...
        return strdup_to(ret, sku);
}

static int get_hardware_version(Context *c, char **ret) {
        _cleanup_free_ char *model = NULL, *sku = NULL, *version = NULL;
        int r;

        r = get_dmi_property(c, "ID_HWVERSION", &version);
        if (r < 0)
                return r;
        ...
        if (r < 0) {
                if (r != -ENOENT)
                        return r;
        } else if (streq_ptr(version, model))
                return -ENOENT;

        /* Suppress reporting the version field, if it's the same string as the
         * SKU field, which it appears to be on various systems */
        r = get_dmi_property(c, "ID_SKU", &sku);
        if (r < 0) {
                if (r != -ENOENT)
                        return r;
        } else if (streq_ptr(version, sku))
                return -ENOENT;

        return strdup_to(ret, version);
}
