...
static char *wrap_ansi_seq(const char *ansi_seq) {
        _cleanup_free_ char *ansi_color = NULL;
        int r;

        r = asprintf(&ansi_color, "\x1B[%sm", ansi_seq);
        if (r > 0)
                return TAKE_PTR(ansi_color);
        ...
        if (!isempty(i->hostname) &&
            !streq_ptr(i->hostname, i->static_hostname)) {
                r = table_add_many(table,
                                   ...
                                   TABLE_SET_COLOR, wrap_ansi_seq(i->ansi_color));
                if (r < 0)
