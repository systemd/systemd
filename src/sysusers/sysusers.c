...
static int write_temporary_passwd(
        ...
        r = fopen_temporary_at_label(AT_FDCWD, "/etc/passwd", passwd_path, &passwd, &passwd_tmp, c->label_context);
        if (r < 0)
...
static int write_temporary_shadow(
        ...
        r = fopen_temporary_at_label(AT_FDCWD, "/etc/shadow", shadow_path, &shadow, &shadow_tmp, c->label_context);
        if (r < 0)
...
static int write_temporary_group(
        ...
        r = fopen_temporary_at_label(AT_FDCWD, "/etc/group", group_path, &group, &group_tmp, c->label_context);
        if (r < 0)
...
static int write_temporary_gshadow(
        ...
        r = fopen_temporary_at_label(AT_FDCWD, "/etc/gshadow", gshadow_path, &gshadow, &gshadow_tmp, c->label_context);
        if (r < 0)
...
static int write_files(Context *c) {
        ...
        if (group) {
                r = make_backup("/etc/group", group_path, c->label_context);
                if (r < 0)
                        return log_error_errno(r, "Failed to backup %s: %m", group_path);
        }
        if (gshadow) {
                r = make_backup("/etc/gshadow", gshadow_path, c->label_context);
                if (r < 0)
                        return log_error_errno(r, "Failed to backup %s: %m", gshadow_path);
        }

        if (passwd) {
                r = make_backup("/etc/passwd", passwd_path, c->label_context);
                if (r < 0)
                        return log_error_errno(r, "Failed to backup %s: %m", passwd_path);
        }
        if (shadow) {
                r = make_backup("/etc/shadow", shadow_path, c->label_context);
                if (r < 0)
