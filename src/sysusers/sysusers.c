...
static void log_audit_accounts(Context *c, bool users) {
#if HAVE_AUDIT
        assert(c);
        ...
        Item *i;
        int type = users ? AUDIT_ADD_USER : AUDIT_ADD_GROUP;
        const char *op = users ? "adding-user" : "adding-group";
        ...
        ORDERED_HASHMAP_FOREACH(i, users ? ASSERT_PTR(c)->todo_uids : ASSERT_PTR(c)->todo_gids)
...
static int write_files(Context *c) {
        ...
        log_audit_accounts(c, /* users= */ false);
        if (gshadow) {
        ...
        log_audit_accounts(c, /* users= */ true);
        if (shadow) {
