...
static void context_destroy(Context *c) {
        ...
        c->groups = ordered_hashmap_free(c->groups);
        c->users = ordered_hashmap_free(c->users);
        c->members = ordered_hashmap_free(c->members);
        c->todo_uids = ordered_hashmap_free(c->todo_uids);
        c->todo_gids = ordered_hashmap_free(c->todo_gids);

        c->database_by_uid = hashmap_free(c->database_by_uid);
        c->database_by_username = hashmap_free(c->database_by_username);
        c->database_by_gid = hashmap_free(c->database_by_gid);
        c->database_by_groupname = hashmap_free(c->database_by_groupname);

        c->names = set_free_free(c->names);
        c->uid_range = uid_range_free(c->uid_range);
}
...
static int run(int argc, char *argv[]) {
        ...
        _cleanup_(context_destroy) Context c = {
