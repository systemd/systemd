...
#define devlink_log_internal(key, level, error, file, line, func, fmt, ...)        \
        ({                                                                         \
                const DevlinkKey *_key = (key);                                    \
                int _level = (level), _devlink_error = (error);                    \
                char _buf[LINE_MAX], *_pos = _buf;                                 \
                int _len = sizeof(_buf);                                           \
                                                                                   \
                BUFFER_APPEND(_pos, _len, "%s", devlink_kind_to_string(_key->kind)); \
                devlink_match_log_prefix(&_pos, &_len, &_key->match, _key->matchset); \
                BUFFER_APPEND(_pos, _len, ": ");                                   \
                (void) snprintf(_pos, _len, fmt, ##__VA_ARGS__);                   \
                                                                                   \
                log_internal(_level, _devlink_error, file, line, func, "%s", _buf); \
        })
