...
static void devlink_port_cache_done(Devlink *devlink) {
        DevlinkPortCache *port_cache = DEVLINK_PORT_CACHE(devlink);

        if (port_cache->ifindex != 0)
                hashmap_remove(devlink->manager->port_cache_by_ifindex, &port_cache->ifindex);
}
