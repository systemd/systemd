...
static int devlink_port_cache_genl_cmd_new_msg_process(
        ...
        if (r < 0)
                return r;
        if (r == 1)
                log_devlink_debug(devlink, "Added ifindex \"%lu\"\n", ifindex);
...
static int devlink_port_cache_genl_cmd_del_msg_process(
        ...
        log_devlink_debug(devlink, "Removed ifindex \"%lu\"\n", port_cache->ifindex);
...
int devlink_port_cache_query_by_ifindex(Manager *m, uint64_t ifindex, DevlinkKey *key) {
        ...
        log_debug("port cache: query ifindex %lu\n", ifindex);
        devlink = hashmap_get(m->port_cache_by_ifindex, &ifindex);
        if (!devlink)
                return -ENOENT;
        devlink_key_copy_subkey(key, &devlink->key, DEVLINK_MATCH_BIT_DEV | DEVLINK_MATCH_BIT_PORT_INDEX);
        return 0;
}
