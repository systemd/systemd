...
static bool devlink_match_dev_check(const DevlinkMatch *match, bool is_explicit) {
...
static int devlink_match_dev_genl_read(
                ...
                Manager *m,
                DevlinkMatch *match) {
        _cleanup_free_ char *bus_name = NULL, *dev_name = NULL;
        DevlinkMatchDev *dev = &match->dev;
        int r;
        ...
        r = sd_netlink_message_read_string_strdup(message, DEVLINK_ATTR_BUS_NAME, &bus_name);
        if (r < 0)
                return log_debug_errno(r, "Netlink message without valid bus name: %m");

        r = sd_netlink_message_read_string_strdup(message, DEVLINK_ATTR_DEV_NAME, &dev_name);
        if (r < 0)
                return log_debug_errno(r, "Netlink message without valid device name: %m");

        dev->bus_name = TAKE_PTR(bus_name);
        dev->dev_name = TAKE_PTR(dev_name);
