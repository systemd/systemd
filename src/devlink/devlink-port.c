...
int config_parse_devlink_port_split_count(CONFIG_PARSER_ARGUMENTS) {
        return config_parse_uint32_bounded(
                        unit, filename, line, section, section_line, lvalue, rvalue,
                        2, _DEVLINK_PORT_SPLIT_COUNT_INVALID - 1, /* ignoring= */ true,
                        data);
}
...
static const DevlinkMonitorCommand devlink_port_commands[] = {
        { .cmd = DEVLINK_CMD_PORT_GET, .enumerate = true, .cmd_override = DEVLINK_CMD_PORT_NEW },
        { .cmd = DEVLINK_CMD_PORT_NEW, .msg_process = devlink_port_genl_cmd_new_msg_process },
        { .cmd = DEVLINK_CMD_PORT_DEL, .msg_process = devlink_port_genl_cmd_del_msg_process },
};
