...
static DevlinkIfnameTrackerItem *devlink_ifname_tracker_item_free(DevlinkIfnameTrackerItem *item) {
        ...
        log_debug("ifname tracker: \"%s\" removed", item->ifname);
...
/* by_ifindex is a non-owning lookup index; the item is owned via by_ifname. */
...
static DevlinkIfnameTrackerItem *devlink_ifname_tracker_item_alloc(Manager *m, const char *ifname) {
        _cleanup_(devlink_ifname_tracker_item_unrefp) DevlinkIfnameTrackerItem *item = NULL;
        int r;
        ...
        *item = (DevlinkIfnameTrackerItem) {
        ...
        log_debug("ifname tracker: \"%s\" added", ifname);
...
void devlink_ifname_tracker_del(Devlink *devlink) {
        ...
        if (!item->ifname_tracker) {
                /* Last tracked devlink gone: drop the by_ifname entry so the item is freed. */
                hashmap_remove(devlink->manager->ifname_tracker_by_ifname, item->ifname);
                item->in_hashmap_by_ifname = false;
                devlink_ifname_tracker_item_unref(item);
        }
}
...
int devlink_ifname_tracker_query(Manager *m, uint64_t ifindex, char **ifname) {
        ...
        log_debug("ifname tracker: query ifindex %" PRIu64, ifindex);
...
static int devlink_ifname_tracker_ifindex_update_one(Manager *m, uint64_t ifindex, const char *ifname, const char **found_ifname) {
        ...
        if (item && *found_ifname) {
                log_warning("Multiple netdevice names for the same port match is not supported in config, \"%s\" will be ignored (\"%s\" is already tracked)", ifname, *found_ifname);
                return 0;
        } else if (!item) {
                return 0;
        }
        ...
        *found_ifname = item->ifname;
        ...
        if (orig_item) {
                assert(orig_item->in_hashmap_by_ifindex);
                hashmap_remove(m->ifname_tracker_by_ifindex, &orig_item->ifindex);
                orig_item->in_hashmap_by_ifindex = false;
        }

        if (item->in_hashmap_by_ifindex) {
                hashmap_remove(m->ifname_tracker_by_ifindex, &item->ifindex);
                item->in_hashmap_by_ifindex = false;
        }
        ...
        log_debug("ifname tracker: \"%s\" updated to ifindex \"%" PRIu64 "\"", item->ifname, ifindex);

        _cleanup_free_ Devlink **snapshot = NULL;
        size_t n_snapshot = 0;

        /* manager_enumerate_by_key() dispatches a synchronous netlink reply inline, which can
         * re-enter this function and mutate item->ifname_tracker mid-walk; snapshot with refs. */
        LIST_FOREACH(ifname_tracker, devlink, item->ifname_tracker) {
                if (!GREEDY_REALLOC(snapshot, n_snapshot + 1))
                        return -ENOMEM;
                snapshot[n_snapshot++] = devlink_ref(devlink);
        }

        FOREACH_ARRAY(i, snapshot, n_snapshot) {
                DevlinkKey key = {};

                if (devlink_ifname_tracker_enumerate_key_get(m, &key, *i, ifindex) >= 0)
...
int devlink_ifname_tracker_ifindex_update(Manager *m, uint64_t ifindex, sd_netlink_message *message) {
        ...
        if (r >= 0) {
                ...
                for (unsigned i = 0; i < count; i++) {
                        r = sd_netlink_message_read_string_indexed(message, IFLA_ALT_IFNAME, i, &ifname);
                        if (r >= 0) {
                                r = devlink_ifname_tracker_ifindex_update_one(m, ifindex, ifname, &found_ifname);
                                if (r < 0)
                                        goto finish;
...
finish:
        ...
        /* None of the interface's current names are tracked (e.g. it was renamed away from a
         * configured NetdevName=): drop any stale ifindex->item mapping so it stops matching. */
        if (!found_ifname)
...
void devlink_ifname_tracker_ifindex_remove(Manager *m, uint64_t ifindex) {
        ...
        item = hashmap_remove(m->ifname_tracker_by_ifindex, &ifindex);
        if (item)
                item->in_hashmap_by_ifindex = false;
}
