...
static int devlink_dev_genl_cmd_new_msg_process(
        ...
        if (dev->eswitch_mode != _DEVLINK_DEV_ESWITCH_MODE_INVALID) {
                r = devlink_dev_genl_eswitch_get(devlink, lookup_key, &current_eswitch_mode);
                if (r < 0)
                        return r;
...
static const DevlinkMonitorCommand devlink_dev_commands[] = {
        { .cmd = DEVLINK_CMD_GET, .enumerate = true },
        { .cmd = DEVLINK_CMD_NEW, .msg_process = devlink_dev_genl_cmd_new_msg_process },
};
