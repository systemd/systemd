...
static void devlink_nested_done(Devlink *devlink) {
        DevlinkNested *nested = DEVLINK_NESTED(devlink);

        nested->nested_in = devlink_unref(nested->nested_in);
}
...
static const DevlinkMonitorCommand devlink_nested_commands[] = {
        { .cmd = DEVLINK_CMD_NEW, .msg_process = devlink_nested_genl_cmd_new_msg_process },
        { .cmd = DEVLINK_CMD_DEL, .msg_process = devlink_nested_genl_cmd_del_msg_process },
};
