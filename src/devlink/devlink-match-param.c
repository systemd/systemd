...
static int devlink_match_param_genl_read(
        ...
        r = sd_netlink_message_read_string_strdup(message, DEVLINK_ATTR_PARAM_NAME, &common->name);
...
static int devlink_match_param_genl_append(sd_netlink_message *message, const DevlinkMatch *match) {
        ...
        r = sd_netlink_message_append_string(message, DEVLINK_ATTR_PARAM_NAME, common->name);
        if (r < 0)
...
const DevlinkMatchVTable devlink_match_param_vtable = {
        ...
        .duplicate_func = devlink_match_common_name_duplicate_func,
