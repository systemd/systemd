...
static int devlink_param_genl_process_string(
        ...
        if (streq(current_val, param->value))
                return 0;
...
static int devlink_param_genl_process_bool(
        ...
        r = sd_netlink_message_has_flag(message, DEVLINK_ATTR_PARAM_VALUE_DATA);
        if (r < 0)
                return log_devlink_debug_errno(devlink, r, "Failed to read current param flag value: %m");
        current_val = r;
...
static int devlink_param_genl_cmd_new_msg_process_common(
                Devlink *devlink,
                DevlinkKey *lookup_key,
                sd_netlink_message *message,
                uint8_t set_cmd) {
        ...
        if (r < 0) {
                r = log_devlink_debug_errno(devlink, r, "Netlink message without param type value: %m");
                goto exit_param;
        }

        r = sd_netlink_message_enter_container(message, DEVLINK_ATTR_PARAM_VALUES_LIST);
        if (r < 0) {
                r = log_devlink_debug_errno(devlink, r, "devlink netlink: Message without param values list nest: %m");
                goto exit_param;
        }
        ...
        for (unsigned i = 0; i < count; i++) {
                ...
                if (r < 0) {
                        r = log_debug_errno(r, "Netlink message without valid param values list item nest: %m");
                        goto exit_values_list;
                }

                r = sd_netlink_message_read_u8(message, DEVLINK_ATTR_PARAM_VALUE_CMODE, &cmode);
                if (r < 0) {
                        r = log_devlink_debug_errno(devlink, r, "devlink netlink: Message without param cmode value: %m");
        ...
        r = DEVLINK_MONITOR_COMMAND_RETVAL_OK;
...
static int devlink_param_genl_cmd_new_msg_process(
                ...
                DevlinkKey *lookup_key,
                sd_netlink_message *message) {
        return devlink_param_genl_cmd_new_msg_process_common(devlink, lookup_key, message, DEVLINK_CMD_PARAM_SET);
}
...
static int devlink_port_param_genl_cmd_new_msg_process(
                ...
                DevlinkKey *lookup_key,
                sd_netlink_message *message) {
        return devlink_param_genl_cmd_new_msg_process_common(devlink, lookup_key, message, DEVLINK_CMD_PORT_PARAM_SET);
}

static const DevlinkMatchSet devlink_param_matchsets[] = {
        DEVLINK_MATCH_BIT_PORT_IFNAME | DEVLINK_MATCH_BIT_PARAM_NAME,
        DEVLINK_MATCH_BIT_DEV | DEVLINK_MATCH_BIT_COMMON_INDEX | DEVLINK_MATCH_BIT_PORT_SPLIT | DEVLINK_MATCH_BIT_PARAM_NAME,
        DEVLINK_MATCH_BIT_DEV | DEVLINK_MATCH_BIT_PARAM_NAME,
        0,
};

static const DevlinkMonitorCommand devlink_param_commands[] = {
        { .cmd = DEVLINK_CMD_PARAM_GET, .enumerate = true, .cmd_override = DEVLINK_CMD_PARAM_NEW }, /* Buggy kernel sends "get" cmd for do/dump reply of "get" cmd */
        { .cmd = DEVLINK_CMD_PARAM_NEW, .msg_process = devlink_param_genl_cmd_new_msg_process },
        { .cmd = DEVLINK_CMD_PORT_PARAM_GET, .enumerate = true, .cmd_override = DEVLINK_CMD_PORT_PARAM_NEW }, /* Buggy kernel sends "get" cmd for do/dump reply of "get" cmd */
        { .cmd = DEVLINK_CMD_PORT_PARAM_NEW, .msg_process = devlink_port_param_genl_cmd_new_msg_process },
};
