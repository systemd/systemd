...
typedef enum DevlinkMatchBitPosition {
        ...
        _DEVLINK_MATCH_BIT_POSITION_INVALID = -EINVAL,
...
typedef struct DevlinkMatchVTable {
        ...
        bool (*check)(const DevlinkMatch *match, bool is_explicit);
...
bool devlink_match_common_index_check(const DevlinkMatch *match, bool is_explicit);
...
void devlink_match_name_copy_func(char **dst, const char *src);
