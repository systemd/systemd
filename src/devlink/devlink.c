...
int devlink_log_internal(
        ...
        BUFFER_APPEND(pos, len, "%s", devlink_kind_to_string(key->kind));
        devlink_match_log_prefix(&pos, &len, &key->match, key->matchset);
        ...
        return log_internal(level, error, file, line, func, "%s", (char *) buf);
}
