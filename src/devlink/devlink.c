...
Devlink *devlink_get_may_create_filtered(Manager *m, DevlinkKey *key, DevlinkMatchSet matchset) {
        ...
        /* Check if the current matchset is subset of the one in the key. */
...
static int devlink_load_one(Manager *m, const char *filename, Devlink **ret) {
        ...
        if (!_DEVLINK_VTABLE(kind)->sections) {
                log_warning("Devlink kind '%s' is internal and cannot be configured in %s. Ignoring",
                            devlink_kind_to_string(kind), filename);
                return -EINVAL;
        }
...
static void devlink_config_sweep_stale(Manager *m, unsigned generation) {
        _cleanup_free_ Devlink **stale = NULL;
        size_t n_stale = 0;
        Devlink *devlink;

        /* Remove config-derived objects not seen in this reload: their .devlink file was
         * removed, or its [Match] keys changed so the object now lives under a different key. */
        HASHMAP_FOREACH(devlink, m->devlink_objs) {
                if (!devlink->filename)
                        continue;
                if (devlink->generation == generation)
                        continue;
                if (!GREEDY_REALLOC(stale, n_stale + 1)) {
                        log_oom();
                        return;
                }
                stale[n_stale++] = devlink;
        }

        FOREACH_ARRAY(i, stale, n_stale) {
                devlink = *i;

                log_devlink_debug(devlink, "Removing object for removed or changed .devlink file");
...
static int devlink_config_load_internal(Manager *m, bool reload) {
        ...
        generation = ++m->config_generation;
        ...
        STRV_FOREACH(f, files) {
                ...
                if (r <= 0)
                        continue;
                devlink->generation = generation;
                TAKE_PTR(devlink);
        }

        if (reload)
                devlink_config_sweep_stale(m, generation);
