...
static int devlink_health_reporter_auto_dump_update(
                ...
                bool *set_needed) {
        DevlinkHealthReporter *health_reporter = DEVLINK_HEALTH_REPORTER(devlink);
        bool current_val, val = health_reporter->auto_dump;
        uint8_t tmp;
        int r;
...
static const DevlinkMatchSet devlink_health_reporter_matchsets[] = {
        DEVLINK_MATCH_BIT_PORT_IFNAME | DEVLINK_MATCH_BIT_HEALTH_REPORTER_NAME,
        DEVLINK_MATCH_BIT_DEV | DEVLINK_MATCH_BIT_COMMON_INDEX | DEVLINK_MATCH_BIT_PORT_SPLIT | DEVLINK_MATCH_BIT_HEALTH_REPORTER_NAME,
        DEVLINK_MATCH_BIT_DEV | DEVLINK_MATCH_BIT_HEALTH_REPORTER_NAME,
        0,
};

static const DevlinkMonitorCommand devlink_health_reporter_commands[] = {
        { .cmd = DEVLINK_CMD_HEALTH_REPORTER_GET, .msg_process = devlink_health_reporter_genl_cmd_get_msg_process, .enumerate = true },
};
