...
static bool devlink_match_port_split_check(const DevlinkMatch *match, bool is_explicit) {
        const DevlinkMatchPort *port = &match->port;

        return is_explicit && !port->split_explicit ? false : true;
}
...
static void devlink_match_port_split_copy_func(DevlinkMatch *dst, const DevlinkMatch *src) {
        ...
        dstport->split_explicit = srcport->split_explicit;
}
...
static bool devlink_match_port_ifname_check(const DevlinkMatch *match, bool is_explicit) {
        const DevlinkMatchPort *port = &match->port;
...
static int devlink_match_port_ifname_genl_read(
         ...
         * 2) Port messages contain ifname but if ifname changes, message
...
static void devlink_match_port_ifname_copy_func(DevlinkMatch *dst, const DevlinkMatch *src) {
        dst->port.ifname = src->port.ifname;
}

static int devlink_match_port_ifname_duplicate_func(DevlinkMatch *dst, const DevlinkMatch *src) {
        const DevlinkMatchPort *srcport = &src->port;
        DevlinkMatchPort *dstport = &dst->port;

        assert(srcport->ifname);

        dstport->ifname = strdup(srcport->ifname);
        if (!dstport->ifname)
                return -ENOMEM;
        return 0;
}
...
const DevlinkMatchVTable devlink_match_port_ifname_vtable = {
        ...
        .copy_func = devlink_match_port_ifname_copy_func,
        .duplicate_func = devlink_match_port_ifname_duplicate_func,
