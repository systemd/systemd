...
int path_simplify_and_warn(
        ...
        if (FLAGS_SET(flags, PATH_CHECK_NON_API_VFS | PATH_CHECK_NON_API_VFS_DEV_OK) && path_below_api_vfs(path))
                if (!(PATH_CHECK_NON_API_VFS_DEV_OK && path_startswith(path, "/dev")))
                        return log_syntax(unit, level, filename, line, SYNTHETIC_ERRNO(EINVAL),
