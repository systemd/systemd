...
int socket_forward_new(
        ...
        _cleanup_close_ int server_write_fd = -EBADF, client_write_fd = -EBADF;
        ...
        return socket_forward_new_pair(
                        event,
                        server_fd, TAKE_FD(server_write_fd),
                        client_fd, TAKE_FD(client_write_fd),
