...
int luo_parse_serialization(sd_json_variant **ret, int **ret_fds, size_t *ret_n_fds) {
        ...
        JSON_VARIANT_OBJECT_FOREACH(unit_id, unit_entries, root) {
                ...
                JSON_VARIANT_ARRAY_FOREACH(entry, unit_entries) {
                        sd_json_variant *fd_json = sd_json_variant_by_key(entry, "fd");
...
int luo_preserve_fd_stores(sd_json_variant *serialization, int *ret_session_fd) {
        ...
        JSON_VARIANT_OBJECT_FOREACH(unit_id, entries, serialization) {
                ...
                JSON_VARIANT_ARRAY_FOREACH(entry, entries) {
                        sd_json_variant *name_json, *type_json;
                        ...
                        if (!type_json || !sd_json_variant_is_string(type_json) ||
                            !name_json || !sd_json_variant_is_string(name_json)) {
                                log_warning("LUO serialization entry for unit '%s' missing 'type' or 'name' field, skipping.", unit_id);
                                continue;
                        }
                        ...
                        if (streq(type, "fd")) {
                                ...
                                if (!fd_json || !sd_json_variant_is_integer(fd_json)) {
                                        log_warning("LUO mapping for unit '%s' fd '%s': missing or invalid fd, skipping.", unit_id, name);
                                        continue;
                                }

                                fd = (int) sd_json_variant_integer(fd_json);
                                if (fd < 0) {
                                        log_warning("LUO mapping for unit '%s' fd '%s': negative fd %i, skipping.", unit_id, name, fd);
                                        continue;
                                }

                                /* Regular fd, preserve in the systemd session */
                                r = luo_session_preserve_fd(session_fd, fd, token);
                                if (r < 0) {
                                        log_warning_errno(r, "Failed to preserve LUO fd %i (name '%s') with token %" PRIu64 ", will be lost across kexec: %m", fd, name, token);
                                        continue;
                                }
                                ...
                                sd_json_variant *sname_json = sd_json_variant_by_key(entry, "sessionName");
                                if (!sname_json || !sd_json_variant_is_string(sname_json)) {
                                        log_warning("LUO mapping for unit '%s' fd '%s': missing or invalid sessionName, skipping.", unit_id, name);
                                        continue;
                                }
                                ...
                                r = sd_json_variant_append_arraybo(
                                                ...
                                                SD_JSON_BUILD_PAIR_STRING("name", name),
                                                SD_JSON_BUILD_PAIR_STRING("sessionName", sd_json_variant_string(sname_json)));
                                if (r < 0)
                                ...
                                log_warning("Unknown fd type '%s' for unit '%s' fd '%s', skipping.", type, unit_id, name);
                }
...
int fd_is_luo_session(int fd) {
        ...
        r = fd_get_luo_session_path(fd, /* ret= */ NULL);
        if (r == -EMEDIUMTYPE)
                return false;
        if (r < 0)
                return r;
