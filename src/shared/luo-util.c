        ...
        JSON_VARIANT_OBJECT_FOREACH(cgroup_path, entries, serialization) {
                ...
                JSON_VARIANT_ARRAY_FOREACH(entry, entries) {
                        ...
                        if (streq(type, "fd")) {
                                ...
                                if (!fd_json || !sd_json_variant_is_integer(fd_json)) {
                                        log_warning("LUO mapping for cgroup '%s' fd '%s': missing or invalid fd, skipping.", cgroup_path, name);
                                        continue;
                                }

                                fd = (int) sd_json_variant_integer(fd_json);
                                if (fd < 0) {
                                        log_warning("LUO mapping for cgroup '%s' fd '%s': negative fd %i, skipping.", cgroup_path, name, fd);
                                        continue;
                                }
