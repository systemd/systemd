...
static SD_VARLINK_DEFINE_METHOD(
                ...
                SD_VARLINK_FIELD_COMMENT("The target UID inside the user namespace. If not specified defaults to 0. If identity is true, must be 0 or unset in which case the peer UID is mapped to itself."),
                ...
                SD_VARLINK_FIELD_COMMENT("If true, create a user namespace that maps the peer UID/GID instead of allocating a transient UID range. Defaults to false."),
                SD_VARLINK_DEFINE_INPUT(identity, SD_VARLINK_BOOL, SD_VARLINK_NULLABLE),
                SD_VARLINK_FIELD_COMMENT("Number of delegated UID/GID ranges to allocate in addition to the primary range. These are mapped 1:1 into the user namespace and can be used by nested user namespaces. Must be between 0 and 16."),
                SD_VARLINK_DEFINE_INPUT(delegateCount, SD_VARLINK_INT, SD_VARLINK_NULLABLE),
