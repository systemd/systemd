...
static int osc_context_intro(char **ret_seq, sd_id128_t *ret_context_id) {
        ...
        if (asprintf(&seq, ANSI_OSC "3008;S" SD_ID128_FORMAT_STR, SD_ID128_FORMAT_VAL(id)) < 0)
                return -ENOMEM;
...
int osc_context_close(sd_id128_t id, char **ret_seq) {
        ...
        _cleanup_free_ char *seq = NULL;
        if (asprintf(&seq, ANSI_OSC "3008;X" SD_ID128_FORMAT_STR ANSI_ST, SD_ID128_FORMAT_VAL(id)) < 0)
                return -ENOMEM;
