...
int store_battery_capacity(Hashmap *current_capacity) {
        ...
        FOREACH_DEVICE(e, dev) { /* For AC, the TYPE is mains */
                ...
                r = sd_device_get_property_value(dev, "POWER_SUPPLY_NAME", &x);
                if (r == -ENOENT)
                        return log_debug_errno(r, "battery capacity is unavailable. Assuming no battery exists: %m");
                if (r < 0)
                        return log_debug_errno(r, "Failed to read battery capacity: %m");

                r = hashmap_put(current_capacity, x, INT_TO_PTR(battery_capacity));
                if (r < 0) {
...
static int get_battery_identifier(sd_device *dev, const char *property, struct siphash *ret) {
        ...
        if (r == -ENOENT)
               log_debug_errno(r, "battery device property %s is unavailable: %m", property);
        else if (r < 0)
               return log_debug_errno(r, "Failed to read battery device property %s: %m", property);
        else if (isempty(x))
...
int put_battery_discharge_rate(int estimated_battery_discharge_rate, uint64_t system_hash_id) {
        int r;
...
int estimate_battery_discharge_rate_per_hour(
        ...
        FOREACH_DEVICE(e, dev) {
                ...
                battery_last_capacity = PTR_TO_INT(hashmap_get(last_capacity, x));
                battery_current_capacity = PTR_TO_INT(hashmap_get(current_capacity, x));
