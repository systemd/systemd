...
static int get_system_battery_identifier_hash(uint8_t ret[static SHA256_DIGEST_SIZE]) {
        ...
        else if (strlen(bat_manufacturer) == 0)
               log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "/sys/class/power_supply/BAT0/manufacturer is empty: %m");
        else
        ...
        else if (strlen(bat_model_name) == 0)
               log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "/sys/class/power_supply/BAT0/model_name is empty: %m");
        else
        ...
        else if (strlen(bat_serial_number) == 0)
               log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "/sys/class/power_supply/BAT0/serial_number is empty: %m");
        else
...
int get_battery_discharge_rate(void) {
        _cleanup_free_ char *hash_id_discharge_rate = NULL, *h = NULL, *stored_hash_id = NULL, *stored_discharge_rate = NULL;
        const char *p;
        ...
        if (!streq(h, stored_hash_id))
               return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Current dentifier does not match stored identifier: %m");
