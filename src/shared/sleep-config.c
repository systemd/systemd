...
int parse_sleep_config(SleepConfig **ret_sleep_config) {
        ...
        *sc = (SleepConfig) {
        ...
        const ConfigTableItem items[] = {
                ...
                { "Sleep", "DefaultSuspendIntervalSec", config_parse_sec,      0, &sc->suspend_interval_usec      },
                {}
        };
        ...
        if (sc->suspend_interval_usec == 0)
                sc->suspend_interval_usec = 2 * USEC_PER_HOUR;
...
static int read_battery_capacity_percentage(sd_device *dev) {
        ...
        if (r < 0)
                return log_device_debug_errno(dev, r, "Failed to read/parse battery capacity: %m");
...
static int siphash24_compress_id128(int (*getter)(sd_id128_t*), const char *name, struct siphash *state) {
        ...
        r = getter(&id);
        if (r < 0)
                return log_debug_errno(r, "Failed to get %s ID, ignoring: %m", name);
