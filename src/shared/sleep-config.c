 ...
 *
 * Beware:
 *  Never use a device or file as location that hasn't been either explicitly specified by the root user (for example via
 *  /sys/power/resume) or that isn't an already active swap area!
 *  Otherwise various security attacks might become possible, for example an attacker could silently attach such a device
 *  and circumvent full disk encryption when it would be automatically used for hibernation.
 *
 * Returns:
