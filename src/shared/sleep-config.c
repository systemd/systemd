...
int fetch_batteries_name_with_capacity(Hashmap **ret) {
        ...
        FOREACH_DEVICE(e, dev) {
                _cleanup_free_ const char *battery_name_copy = NULL;
                ...
                r = hashmap_put(current_capacity, battery_name_copy, INT_TO_PTR(battery_capacity));
                if (r < 0)
...
static int put_battery_discharge_rate(int estimated_battery_discharge_rate, uint64_t system_hash_id, bool truncate) {
        ...
        r = write_string_filef(
                ...
                WRITE_STRING_FILE_CREATE | WRITE_STRING_FILE_MKDIR_0755 | (truncate ? WRITE_STRING_FILE_TRUNCATE : 0),
                "%"PRIu64" %d",
                system_hash_id,
