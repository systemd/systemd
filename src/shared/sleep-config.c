...
int get_battery_discharge_rate(void) {
        ...
        dirpath = path_join("/var/lib/systemd/sleep", SD_ID128_TO_STRING(machine_id), "battery_discharge_rate");
        if (!dirpath)
               return log_oom();
...
int put_battery_discharge_rate(int estimated_battery_discharge_rate) {
        const char *discharge_rate_to_store;
        _cleanup_free_ char *dirpath = NULL;
        sd_id128_t machine_id = SD_ID128_NULL;
        int r;
        ...
        discharge_rate_to_store = FORMAT_TIMESPAN(estimated_battery_discharge_rate, USEC_PER_HOUR);
        ...
        r = write_string_file(dirpath, discharge_rate_to_store, WRITE_STRING_FILE_CREATE|WRITE_STRING_FILE_MKDIR_0755);
        if (r < 0)
