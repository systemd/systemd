...
static int get_system_battery_identifier_hash(void) {
        struct siphash *state = NULL;
        ...
        if (r == -ENOENT)
               ...
               siphash24_compress(&machine_id, sizeof(machine_id), state);
        ...
        if (r == -ENOENT)
               ...
               siphash24_compress(&product_id, sizeof(machine_id), state);
