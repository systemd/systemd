...
int battery_is_low(void) {
        ...
        if (r > 0 && r <= 5)
               return 1;/*If battery current capacity percentage is equal to or less than 5%. We have not used battery capacity_level since value is set to full or Normal in case acpi is not working properly.*/

        return 0;/*Not low if greater than 5%. The error will be handled in read_battery_capacity_percentage which returns value from 0-100 only*/
}
...
int read_battery_capacity_percentage(void) {
        ...
        if (r == -ENOENT)
               return r; /*Handling case when no battery is present. Exception here file is manually deleted / corrupted*/
        else if (r < 0)
...
static int battery_discharge_rate_in_range(int battery_discharge_rate) {
        if (battery_discharge_rate > 0 && battery_discharge_rate < 200)
               return 1;/*battery discharge rate is in sane range*/
