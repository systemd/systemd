...
static int get_sd_device_battery(sd_device_enumerator **ret) {
        int r;
...
static int read_battery_capacity_percentage(sd_device *dev) {
        ...
        if (r == -ENOENT)
                return log_device_debug_errno(dev, r, "battery capacity is unavailable. Assuming no battery exists: %m");
        if (r < 0)
...
int battery_is_low(void) {
        ...
        r = get_sd_device_battery(&e);
        if (r < 0)
                return log_error_errno(r, "Failed to get available battery: %m");
        ...
        /* if all batteries are below 5 then c will remain false */
        if (!c)
                return 1;
...
/* get sum of all batteries current charge level */
int get_total_battery_capacity(void) {
        _cleanup_(sd_device_enumerator_unrefp) sd_device_enumerator *e = NULL;
        sd_device *dev;
        int total_battery_capacity, r;
        ...
        FOREACH_DEVICE(e, dev) {
                ...
                total_battery_capacity = total_battery_capacity + r;
        }
...
int hashmap_battery_name_with_battery_capacity(Hashmap **ret_current_capacity) {
        _cleanup_(sd_device_enumerator_unrefp) sd_device_enumerator *e = NULL;
        ...
        FOREACH_DEVICE(e, dev) { /* For AC, the TYPE is mains */
                ...
                r = hashmap_put(current_capacity, battery_name, INT_TO_PTR(battery_capacity));
                if (r < 0) {
                        log_error_errno(r, "Failed to add to hashmap last battery capacity: %m");
                        continue;
                }
        }
        *ret_current_capacity = TAKE_PTR(current_capacity);

        return 0;
}

/* Read file path and return hash of value in that file */
static int get_battery_identifier(sd_device *dev, const char *property, struct siphash *ret) {
        ...
        if (r == -ENOENT)
               ...
               log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "battery device property %s is null: %m", property);
        else
               siphash24_compress_string(x, ret);

        return 0;
}

/* Read system and battery identifier from specific location and generate hash of it */
static int get_system_battery_identifier_hash(sd_device *dev, uint64_t *ret_hash) {
...
int get_battery_discharge_rate(void) {
        ...
        int estimated_total_discharge_rate = 0, discharge_rate, r = 0;
        ...
        for (;;) {
                ...
                FOREACH_DEVICE(e, dev) {
                        ...
                        if(current_hash_id != hash_id)
                                /* matching device not found, move to next device */
                                continue;
                        ...
                        estimated_total_discharge_rate = estimated_total_discharge_rate + discharge_rate;
                        break; /* matching device found, exit iteration */
                }
        }

        return estimated_total_discharge_rate;
}

/* Write battery percentage discharge rate per hour along with system and battery identifier hash to file */
static int put_battery_discharge_rate(int estimated_battery_discharge_rate, uint64_t system_hash_id) {
        int r;
...
int estimate_battery_discharge_rate_per_hour(
        ...
        FOREACH_DEVICE(e, dev) {
                ...
                r = put_battery_discharge_rate(battery_discharge_rate, system_hash_id);
                if (r < 0)
