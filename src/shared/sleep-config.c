...
int sleep_state_supported(char **states) {
        ...
        if (DEBUG_LOGGING) {
                ...
                log_debug("None of the configured sleep states is supported by kernel: %s", strnull(joined));
        }
        return false;
}
...
int sleep_mode_supported(char **modes) {
        ...
        if (DEBUG_LOGGING) {
                ...
                log_debug("None of the configured hibernation power modes is supported by kernel: %s", strnull(joined));
        }
        return false;
}
