...
int fetch_batteries_capacity_by_name(Hashmap **ret) {
        ...
        FOREACH_DEVICE(e, dev) {
                ...
                if (r < 0)
                        return log_device_debug_errno(dev, r, "Failed to add store battery capacity: %m");
...
static int get_battery_discharge_rate(sd_device *dev, int *ret) {
        ...
        for (;;) {
                ...
                if (r < 0)
                        return log_debug_errno(r, "Failed to parse discharge rate read from " DISCHARGE_RATE_FILEPATH " location: %m");
