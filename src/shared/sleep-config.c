...
int get_battery_discharge_rate(void) {
        _cleanup_free_ const char *filepath = NULL;
        ...
        log_debug("hash_id_discharge_rate is %s", hash_id_discharge_rate);
        if (r < 0)
        ...
        log_debug("system_battery_identifier is %s", current_hash_id);
        if (r < 0)
