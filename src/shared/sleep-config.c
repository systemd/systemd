...
int read_battery_capacity(void) {
        ...
        log_debug("Current battery charge level: %d%%", battery_capacity);
...
int get_battery_discharge_rate(void) {
        ...
        log_debug("Loaded machine ID %s from /etc/machine-id.", SD_ID128_TO_STRING(machine_id));

        dirpath = path_join("/var/lib/systemd", SD_ID128_TO_STRING(machine_id), "/battery_discharge_rate");
        if (!dirpath)
               return log_oom();
        log_debug("%s is the dirpath", dirpath);

        r = read_one_line_file(dirpath, &dis_rate);
        if (r < 0)
               return log_error_errno(r, "Failed to read discharge rate from %s: %m", dirpath);

        r = safe_atoi(dis_rate, &stored_discharge_rate);
        if (r < 0)
               return log_debug_errno(r, "Failed to convert discharge rate fetched from %s location from char value to int: %m", dirpath);

        return stored_discharge_rate;
}

int put_battery_discharge_rate(const char *estimated_battery_discharge_rate) {
        _cleanup_free_ char *dirpath = NULL;
        sd_id128_t machine_id = SD_ID128_NULL;
        int r;
        ...
        r = write_string_file(dirpath, estimated_battery_discharge_rate, WRITE_STRING_FILE_CREATE|WRITE_STRING_FILE_MKDIR_0755);
        if (r < 0)
                return log_error_errno(r, "Failed to create %s: %m", dirpath);
