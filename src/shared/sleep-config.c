...
static int get_battery_discharge_rate_filepath(char **ret_path) {
        ...
        *ret_path = path_join("/var/lib/systemd/sleep", SD_ID128_TO_STRING(machine_id), "battery_discharge_percentage_rate_per_hour");
        if (!ret_path) {
                return log_debug_errno(SYNTHETIC_ERRNO(EIO), "Failed to parse battery discharge percentage rate file path: %m");
        }
        return 0;
}
