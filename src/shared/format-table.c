...
static const char* table_data_format(
        ...
        case TABLE_TIMESTAMP_DATE: {
                ...
                _cleanup_free_ char *p = new(char,
        ...
        case TABLE_INT:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->int_val) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%i", d->int_val);
                return d->formatted;

        case TABLE_INT8:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->int8) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%" PRIi8, d->int8);
                return d->formatted;

        case TABLE_INT16:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->int16) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%" PRIi16, d->int16);
                return d->formatted;

        case TABLE_INT32:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->int32) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%" PRIi32, d->int32);
                return d->formatted;

        case TABLE_INT64:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->int64) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%" PRIi64, d->int64);
                return d->formatted;

        case TABLE_UINT:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->uint_val) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%u", d->uint_val);
                return d->formatted;

        case TABLE_UINT8:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->uint8) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%" PRIu8, d->uint8);
                return d->formatted;

        case TABLE_UINT16:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->uint16) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%" PRIu16, d->uint16);
                return d->formatted;

        case TABLE_UINT32:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->uint32) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%" PRIu32, d->uint32);
                return d->formatted;

        case TABLE_UINT32_HEX:
                d->formatted = new(char, 8 + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%" PRIx32, d->uint32);
                return d->formatted;

        case TABLE_UINT32_HEX_0x:
                d->formatted = new(char, 2 + 8 + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "0x%" PRIx32, d->uint32);
                return d->formatted;

        case TABLE_UINT64:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->uint64) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%" PRIu64, d->uint64);
                return d->formatted;

        case TABLE_UINT64_HEX:
                d->formatted = new(char, 16 + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%" PRIx64, d->uint64);
                return d->formatted;

        case TABLE_UINT64_HEX_0x:
                d->formatted = new(char, 2 + 16 + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "0x%" PRIx64, d->uint64);
                return d->formatted;

        case TABLE_PERCENT:
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->percent) + 2);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%i%%" , d->percent);
                return d->formatted;
        ...
        case TABLE_UID:
                ...
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->uid) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, UID_FMT, d->uid);
                return d->formatted;
        ...
        case TABLE_GID:
                ...
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->gid) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, GID_FMT, d->gid);
                return d->formatted;
        ...
        case TABLE_PID:
                ...
                d->formatted = new(char, DECIMAL_STR_WIDTH(d->pid) + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, PID_FMT, d->pid);
                return d->formatted;
        ...
        case TABLE_MODE:
                ...
                d->formatted = new(char, 4 + 1);
                if (!d->formatted)
                        return NULL;

                sprintf(d->formatted, "%04o", d->mode & 07777);
                return d->formatted;
        ...
        case TABLE_DEVNUM:
                ...
                (void) asprintf(&d->formatted, DEVNUM_FORMAT_STR, DEVNUM_FORMAT_VAL(d->devnum));
                return d->formatted;
