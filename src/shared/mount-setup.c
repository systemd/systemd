...
static const MountPoint mount_table[] = {
        ...
        { "tmpfs",       "/dev/shm",                  "tmpfs",      "mode=01777",                               MS_NOSUID|MS_NODEV|MS_STRICTATIME,
          NULL,          MNT_FATAL|MNT_IN_CONTAINER|MNT_TMPFS_USRQUOTA },
...
static int mount_one(const MountPoint *p, bool relabel) {
        ...
        if (FLAGS_SET(p->mode, MNT_TMPFS_USRQUOTA)) {
                r = mount_option_supported("tmpfs", "usrquota", /* value= */ NULL);
                if (r < 0)
                        log_warning_errno(r, "Unable to determine whether tmpfs supports 'usrquota' mount option, assuming not: %m");
                else if (r == 0)
                        log_info("Not enabling 'usrquota' on '%s' as kernel lacks support for it: %m", p->where);
                else {
