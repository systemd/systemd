...
typedef struct MountPoint {
        ...
        const char *type;
        const char *options;
        int (*options_fn)(int priority, const char *, char **);
        unsigned long flags;
        MountMode mode;
...
int mount_cgroupfs(const char *path) {
        ...
        _cleanup_free_ char *opts = NULL;
        r = cgroupfs_mount_options(LOG_DEBUG, /* type= */ NULL, &opts);
        if (r < 0)
                return r;

        /* These options shall be kept in sync with those in mount_table below. */
        if (!strextend_with_separator(&opts, ",", "nsdelegate"))
                return -ENOMEM;

        return mount_nofollow_verbose(LOG_ERR, "cgroup2", path, "cgroup2", MS_NOSUID|MS_NOEXEC|MS_NODEV, opts);
}

static int usrquota_mount_options(int priority, const char *type, char **ret) {
...
static const MountPoint mount_table[] = {
        ...
        },
        {
                ...
                .options_fn = usrquota_mount_options,
...
static int mount_one(const MountPoint *p, bool relabel) {
        ...
        if (p->options_fn) {
                ...
                if (!strextend_with_separator(&extend_options, ",", p->options))
                        return log_oom();
