...
#define tpm2_digest_buffer(alg, digest, data, len, extend) tpm2_digest_many(alg, digest, &IOVEC_MAKE((void*) data, len), 1, extend)
int tpm2_digest_many_digests(TPMI_ALG_HASH alg, TPM2B_DIGEST *digest, const TPM2B_DIGEST data[], size_t count, bool extend);
#define tpm2_digest_rehash(alg, digest) tpm2_digest_many(alg, digest, NULL, 0, true)
#define tpm2_digest_init(alg, digest) tpm2_digest_many(alg, digest, NULL, 0, false)
