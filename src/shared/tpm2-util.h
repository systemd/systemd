...
/* The tpm2-tss library has many structs that are simply a combination of an array and size. These macros
 * allow easily initializing or assigning instances of such structs from an existing buffer and size, while
 * also checking the size for safety with the struct buffer size. The the provided buffer is NULL, the
 * resulting struct's buffer will be all 0. If the provided size is larger than the struct's buffer size,
 * these result in assertion failure; to check the size, use one of the TPM2B_*_CHECK_SIZE() macros. */
