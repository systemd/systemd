...
#define CACHED_UDEV_AVAIL_UNSET -1

static int cached_udev_avail = CACHED_UDEV_AVAIL_UNSET;

void reset_cached_udev_availability(void) {
        cached_udev_avail = CACHED_UDEV_AVAIL_UNSET;
}
...
bool udev_available(void) {
        ...
        if (cached_udev_avail >= 0)
                return cached_udev_avail;

        return (cached_udev_avail = (path_is_read_only_fs("/sys/") <= 0));
}
