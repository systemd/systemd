...
static bool battery_is_discharging(sd_device *d) {
        ...
        if (r < 0)
                log_device_debug_errno(d, r, "Failed to read 'present' sysfs attribute, assuming the battery presents: %m");
        else if (r == 0) {
                log_device_debug(d, "The battery does not present, ignoring the power supply.");
                return false;
        }
