...
int create_ephemeral_snapshot(const char *directory, RuntimeScope scope, bool read_only, LockFile *tree_global_lock, LockFile *tree_local_lock, char **ret_new_path) {
        ...
        if (r < 0) {
                return log_error_errno(r, "Failed to determine whether directory %s is mount point: %m", directory);
        }
        if (r > 0)
        ...
        if (r < 0) {
                return log_error_errno(r, "Failed to create snapshot %s from %s: %m", np, directory);
        }
