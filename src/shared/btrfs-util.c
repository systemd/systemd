...
int btrfs_get_block_device_at(int dir_fd, const char *path, uint64_t *ret_devid, char** ret_path, dev_t *ret) {
...
int btrfs_replace(int fdmntpnt, uint64_t device_id, const char *target) {
        ...
        if (lstat(target, &st) < 0)
                return log_error_errno(-errno, "cannot stat %s: %m", target);
        log_notice("Target %s %d", target, S_ISBLK(st.st_mode));

        if (strlen(target) >= sizeof(replace.start.tgtdev_name))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Path the btrfs replace target is too large");
        strncpy((char *)replace.start.tgtdev_name, target, sizeof(replace.start.tgtdev_name));

        r = ioctl(fdmntpnt, BTRFS_IOC_DEV_REPLACE, &replace);
        if (r < 0)
                return log_error_errno(-errno, "ioctl BTRFS_IOC_DEV_REPLACE failed: %m");
        ...
        case BTRFS_IOCTL_DEV_REPLACE_RESULT_ALREADY_STARTED:
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "btrfs replace was already started on this device");
        case BTRFS_IOCTL_DEV_REPLACE_RESULT_SCRUB_INPROGRESS:
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "btrfs scrub is in progress");
        default:
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "An unknown btrfs error status occured");
        }
...
int btrfs_resize_max(int fdmntpnt, uint64_t devid) {
        ...
        r = snprintf(args.name, sizeof(args.name), "%" PRIu64 ":max", devid);
        if (r < 0)
                return r;
        if ((size_t)r >= sizeof(args.name))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Device identifier too long for btrfs resize");

        r = ioctl(fdmntpnt, BTRFS_IOC_RESIZE, &args);
        if (r < 0)
                return log_error_errno(-errno, "Could not resize btrfs target device: %m");
