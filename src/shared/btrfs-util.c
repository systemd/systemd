...
int btrfs_replace(int fdmntpnt, uint64_t device_id, const char *target) {
        ...
        case BTRFS_IOCTL_DEV_REPLACE_RESULT_NOT_STARTED:
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "btrfs replace was not started");
        ...
        default:
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "An unknown btrfs error status occurred");
        }
