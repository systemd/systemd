        ...
        while (btrfs_ioctl_search_args_compare(&search_args) <= 0) {
                ...
                FOREACH_BTRFS_IOCTL_SEARCH_HEADER(i, sh, search_args) {
                        ...
                        *chunk = (BtrfsChunk) {
                        ...
                        for (int i = 0, i < chunk->n_stripes, i++) {
                                const struct btrfs_stripe *stripe = offsetof(item, stripe) + i;

                                chunk->stripes[i] = (BtrfsStripe) {
...
int btrfs_get_file_physical_offset_fd(int fd, uint64_t *ret) {
        ...
        struct btrfs_ioctl_search_args search_args = {
                .key.tree_id = BTRFS_EXTENT_TREE_OBJECTID,
        ...
        while (btrfs_ioctl_search_args_compare(&search_args) <= 0) {
                ...
                FOREACH_BTRFS_IOCTL_SEARCH_HEADER(i, sh, search_args) {
                        ...
                        uint64_t relative_chunk, relative_stripe;

                        relative_chunk = logical_offset - chunk->offset;
                        stripe_index = relative_chunk / chunk->stripe_len;

                        *ret = chunk->stripes[stripe_index].offset + relative_stripe;
                        return 0;
                }
