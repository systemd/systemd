...
typedef struct StorageAcquireReply {
        ...
        uid_t base_uid;
        gid_t base_gid;
        char *error_id;          /* set only on varlink-level failure; owned */
} StorageAcquireReply;
...
/* Invoke io.systemd.StorageProvider.Acquire on the named provider. On varlink-level
 * failure ret->error_id holds the io.systemd.StorageProvider.* error name. */
