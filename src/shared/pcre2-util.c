...
int pcre2_pattern_compile(const char *pattern, PatternCompileCase case_, void **ret) {
#if HAVE_PCRE2
        ...
        if (case_ == PATTERN_COMPILE_CASE_INSENSITIVE)
                ...
                r = pcre2_pattern_compile("[[:upper:]]", PATTERN_COMPILE_CASE_SENSITIVE, (void **) &cs);
                if (r < 0)
                        return r;
...
int pcre2_pattern_matches(void *compiled_pattern, const char *message, size_t size, size_t *ret_ovec) {
#if HAVE_PCRE2
        _cleanup_(sym_pcre2_match_data_freep) pcre2_match_data *md = NULL;
        int r;
        ...
        if (r < 0) {
                ...
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Pattern matching failed: %s", r < 0 ? "unknown error" : (char*) buf);
        }
