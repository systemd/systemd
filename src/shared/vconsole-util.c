...
bool x11_context_is_safe(const X11Context *xc) {
        ...
        return
                (!xc->layout  || string_is_safe(xc->layout, STRING_NON_EMPTY|STRING_NO_QUOTES|STRING_NO_GLOBS))  &&
                (!xc->model   || string_is_safe(xc->model, STRING_NON_EMPTY|STRING_NO_QUOTES|STRING_NO_GLOBS))   &&
                (!xc->variant || string_is_safe(xc->variant, STRING_NON_EMPTY|STRING_NO_QUOTES|STRING_NO_GLOBS)) &&
                (!xc->options || string_is_safe(xc->options, STRING_NON_EMPTY|STRING_NO_QUOTES|STRING_NO_GLOBS));
}
