...
int metrics_method_list(
                ...
                sd_varlink *link, sd_json_variant *parameters,
        ...
        _cleanup_(metric_family_context_done) MetricFamilyContext ctx = {
                .link = link
        };
        for (const MetricFamily *mf = metric_family_table; mf && mf->name; mf++) {
...
static int metric_set_fields(sd_json_variant **v, char **field_pairs) {
        ...
        n_pairs = strv_length(field_pairs);
        if (n_pairs % 2 != 0)
                return log_debug_errno(SYNTHETIC_ERRNO(ERANGE), "Odd number of field pairs: %zu", n_pairs);

        sd_json_variant **array = new(sd_json_variant*, n_pairs * 2);
        if (!array)
                return log_oom();
        ...
        STRV_FOREACH_PAIR(key, value, field_pairs) {
                r = sd_json_variant_new_string(&array[n++], *key);
                if (r < 0)
                        return log_debug_errno(r, "Failed to create key variant: %m");

                r = sd_json_variant_new_string(&array[n++], *value);
                if (r < 0)
        ...
        return sd_json_variant_set_field(ASSERT_PTR(v), "fields", w);
