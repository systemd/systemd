...
int dns_name_apply_idna(const char *name, char **ret) {
        ...
        log_debug("idn2_lookup_u8: %s %s %s", name, special_glyph(SPECIAL_GLYPH_ARROW_RIGHT, /* force_utf8= */ false), t);
        if (r == IDN2_OK) {
                if (!startswith(name, "xn--")) {
                        ...
                        if (!streq_ptr(name, s)) {
                                log_debug("idn2 roundtrip failed: \"%s\" %s \"%s\" %s \"%s\", ignoring.",
                                          name, special_glyph(SPECIAL_GLYPH_ARROW_RIGHT, /* force_utf8= */ false), t,
                                          special_glyph(SPECIAL_GLYPH_ARROW_RIGHT, /* force_utf8= */ false), s);
                                *ret = NULL;
                                return 0;
                        }
                }
