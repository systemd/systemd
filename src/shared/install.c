...
static int path_is_vendor_or_generator(const LookupPaths *lp, const char *path) {
        ...
        /* Not root stripped: path_is_vendor() does that itself, and the generator directories are
         * root prefixed, so stripping here would keep them from ever matching under --root=. */
        return path_is_vendor(lp, path) || path_is_generator(lp, path);
}
