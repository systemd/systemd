...
static int chroot_unit_symlinks_equivalent(
        ...
        if (!path_is_absolute(target_a) || !path_is_absolute(target_b)) {
                dirname = dirname_malloc(src);
                if (!dirname)
                        return log_oom();
        }

        _cleanup_free_ char *a = strjoin(path_is_absolute(target_a) ? root : dirname, "/", target_a);
        _cleanup_free_ char *b = strjoin(path_is_absolute(target_b) ? root : dirname, "/", target_b);
        if (!a || !b)
                return log_oom();
