...
static int path_is_vendor_or_generator(const LookupPaths *lp, const char *path) {
        ...
        return path_is_vendor(lp, path) || path_is_generator(lp, rpath);
}

static bool is_dependency_dir_name(const char *name) {
        assert(name);

        return STRPTR_IN_SET(strrchr(name, '.'), ".wants", ".requires", ".upholds");
}
...
static bool path_is_dependency_dir(const char *path) {
        _cleanup_free_ char *name = NULL;

        assert(path);

        if (path_extract_filename(path, &name) < 0)
                return false;
...
static int remove_marked_symlinks_fd(
        ...
        FOREACH_DIRENT(de, d, return -errno)
                if (de->d_type == DT_DIR) {
                        ...
                        if (path_is_dependency_dir(path)) {
        ...
        STRV_FOREACH(p, lp->search_path) {
                ...
                STRV_FOREACH(symlink, symlinks) {
                        ...
                        if (r > 0 || is_symlink(*symlink) <= 0) {
                                r = set_ensure_consume(&masked, &string_hash_ops_free, TAKE_PTR(key));
                                if (r < 0)
                                        return r;
                        ...
                        r = ordered_hashmap_ensure_put(&found, &string_hash_ops_free_free, key, path);
                        if (r < 0)
                                return r;
                        ...
                        *same_name_link = true;
                        ...
                        if (ignore_destination) {
                                ...
                                if (r > 0 || de->d_type != DT_LNK) {
                                        r = set_ensure_consume(shadowed, &string_hash_ops_free, TAKE_PTR(key));
                                        if (r < 0)
                                                return r;
...
static int find_symlinks_in_scope(
        ...
        STRV_FOREACH(p, lp->search_path)  {
                ...
                r = find_symlinks(lp, info, match_name, ignore_same_name, *p,
                                  &shadowed, &dependency, &same_name_link);
                if (r < 0)
                        return r;
                if (r > 0) {
...
static int install_info_load_rules(
        ...
        r = install_info_discover(ctx, lp, info->name, SEARCH_LOAD|SEARCH_FOLLOW_CONFIG_SYMLINKS,
                                  &loaded, /* changes= */ NULL, /* n_changes= */ NULL);
        if (r < 0)
                return r;
...
static int install_info_collect_vendor_symlinks(
        ...
        if (FLAGS_SET(file_flags, UNIT_FILE_PRESET))
                return 0;
...
static int do_unit_file_enable(
        ...
        RET_GATHER(r, install_context_unmask_dependencies(&ctx, lp, config_path, changes, n_changes));
        return r;
}
...
static int execute_preset(
        ...
        if (mode != UNIT_FILE_PRESET_ENABLE_ONLY) {
                ...
                r = install_context_mark_for_removal(minus, lp, file_flags | UNIT_FILE_PRESET,
                ...
                if (r >= 0)
                        r = install_context_mask_dependencies(minus, lp, config_path, /* dry_run= */ false,
                                                              changes, n_changes);
        } else
                r = 0;
        ...
        if (mode != UNIT_FILE_PRESET_DISABLE_ONLY) {
                ...
                q = install_context_apply(plus, lp,
                                          file_flags | UNIT_FILE_IGNORE_AUXILIARY_FAILURE | UNIT_FILE_PRESET,
                                          config_path,
                                          SEARCH_LOAD, changes, n_changes);
                if (r >= 0) {
                        if (q < 0)
