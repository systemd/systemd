...
/* Symlink cache for unit_file_get_list — pre-scans .wants/.requires/.upholds
 * directories and direct symlinks once, so find_symlinks_in_scope can do
 * in-memory lookups instead of re-reading the filesystem per unit file. */

typedef struct CachedSymlink {
        char *name;           /* symlink d_name */
        char *target;         /* basename of readlinkat result, only for direct links */
} CachedSymlink;

typedef struct CachedSearchPath {
...
static int unit_file_search(
        ...
        bool has_dropins = true, has_template_dropins = !!template;

        if (template)
                dropin_template_dir_name = strjoina(template, ".d");

        if (cache) {
                has_dropins = set_contains(cache->dropin_dirs, dropin_dir_name);
                has_template_dropins = template && set_contains(cache->dropin_dirs, dropin_template_dir_name);
