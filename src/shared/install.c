...
static int create_symlink(
        ...
        if (!dry_run && symlink(old_path, new_path) >= 0) {
        ...
        if (!dry_run && errno != EEXIST)
                return install_changes_add(changes, n_changes, -errno, new_path, NULL);

        /* For dry-run, we need to check if symlink exists. For regular operation, we reach here due to EEXIST */
        if (dry_run)
                r = is_symlink(new_path) > 0 ? readlink_malloc(new_path, &dest) : -ENOENT;
        else
