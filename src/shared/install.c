...
int unit_file_mask(
        ...
        STRV_FOREACH(name, names) {
                ...
                q = do_mac_check(mac_check, *name, userdata);
                if (q < 0)
                        return q;
...
int unit_file_unmask(
        ...
        STRV_FOREACH(name, names) {
                ...
                r = do_mac_check(mac_check, *name, userdata);
                if (r < 0)
                        return r;
...
int unit_file_enable(
        ...
        return do_unit_file_enable(&lp, scope, flags, config_path, names_or_paths, changes, n_changes, mac_check, userdata);
}
...
int unit_file_preset(
        ...
        STRV_FOREACH(name, names) {
                r = preset_prepare_one(scope, &plus, &minus, &lp, *name, &presets, changes, n_changes, mac_check, userdata);
                if (r < 0)
                        return r;
        }
