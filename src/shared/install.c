...
static int install_info_symlink_alias(
        ...
        STRV_FOREACH(s, info->aliases) {
                ...
                q = install_unit_verify_alias(info, dst, &dst_updated, changes, n_changes);
                if (q == -ELOOP)
                        continue;
                if (q < 0) {
                        r = r < 0 ? r : q;
...
static int read_presets(LookupScope scope, const char *root_dir, UnitFilePresets *presets) {
        _cleanup_(install_unit_presets_freep) UnitFilePresets ps = {};
...
int install_unit_query_preset(LookupScope scope, const char *root_dir, const char *name, UnitFilePresets *cached) {
        _cleanup_(install_unit_presets_freep) UnitFilePresets tmp = {};
        int r;
