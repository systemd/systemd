...
#define BPF_ALU64_REG(OP, DST, SRC) \
        ((struct bpf_insn){         \
                .code = BPF_ALU64 | BPF_OP(OP) | BPF_X, .dst_reg = DST, .src_reg = SRC, .off = 0, .imm = 0 })

#define BPF_ALU32_REG(OP, DST, SRC) \
        ((struct bpf_insn){         \
                .code = BPF_ALU | BPF_OP(OP) | BPF_X, .dst_reg = DST, .src_reg = SRC, .off = 0, .imm = 0 })

/* ALU ops on immediates, bpf_add|sub|...: dst_reg += imm32 */

#define BPF_ALU64_IMM(OP, DST, IMM) \
        ((struct bpf_insn){         \
                .code = BPF_ALU64 | BPF_OP(OP) | BPF_K, .dst_reg = DST, .src_reg = 0, .off = 0, .imm = IMM })

#define BPF_ALU32_IMM(OP, DST, IMM) \
        ((struct bpf_insn){         \
                .code = BPF_ALU | BPF_OP(OP) | BPF_K, .dst_reg = DST, .src_reg = 0, .off = 0, .imm = IMM })
...
#define BPF_MOV32_REG(DST, SRC) \
        ((struct bpf_insn){     \
                .code = BPF_ALU | BPF_MOV | BPF_X, .dst_reg = DST, .src_reg = SRC, .off = 0, .imm = 0 })

/* Short form of mov, dst_reg = imm32 */

#define BPF_MOV64_IMM(DST, IMM) \
        ((struct bpf_insn){     \
                .code = BPF_ALU64 | BPF_MOV | BPF_K, .dst_reg = DST, .src_reg = 0, .off = 0, .imm = IMM })

#define BPF_MOV32_IMM(DST, IMM) \
        ((struct bpf_insn){     \
                .code = BPF_ALU | BPF_MOV | BPF_K, .dst_reg = DST, .src_reg = 0, .off = 0, .imm = IMM })
...
#define BPF_LD_ABS(SIZE, IMM) \
        ((struct bpf_insn){   \
                .code = BPF_LD | BPF_SIZE(SIZE) | BPF_ABS, .dst_reg = 0, .src_reg = 0, .off = 0, .imm = IMM })
...
#define BPF_JMP_REG(OP, DST, SRC, OFF) \
        ((struct bpf_insn){            \
                .code = BPF_JMP | BPF_OP(OP) | BPF_X, .dst_reg = DST, .src_reg = SRC, .off = OFF, .imm = 0 })

/* Conditional jumps against immediates, if (dst_reg 'op' imm32) goto pc + off16 */

#define BPF_JMP_IMM(OP, DST, IMM, OFF) \
        ((struct bpf_insn){            \
                .code = BPF_JMP | BPF_OP(OP) | BPF_K, .dst_reg = DST, .src_reg = 0, .off = OFF, .imm = IMM })
