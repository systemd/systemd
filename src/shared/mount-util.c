        ...
        for (;;) {
                ...
                for (;;) {
                        ...
                        if (r < 0) {
                                ...
                                /* OK, so the remount of this entry failed. We'll ultimately ignore this
                                 * (there are simply so many reasons why this can fail, think autofs, NFS,
                                 * FUSE, …), but let's generate useful debug messages at least. */
                                ...
                                if (IN_SET(q, 0, -ENOENT)) {
                                        ...
                                        log_debug_errno(r, "Mount point '%s' to remount is not a mount point anymore, ignoring: %m", x);
                                        continue;
                                }
                                if (q < 0) /* Any other error on this? Just log and continue */
                                        log_debug_errno(q, "Failed to determine whether '%s' is mount point or not, ignoring: %m", x);

                                if ((((flags & flags_mask) ^ new_flags) & ~MS_RELATIME) == 0) { /* ignore MS_RELATIME while comparing */
                                        log_debug_errno(r, "Couldn't remount '%s', but the flags already match what we want, hence ignoring: %m", x);
                                        continue;
                                }

                                if (!path_equal(x, prefix)) {
                                        /* If this is not the top-level mount, then handle this gracefully:
                                         * log but otherwise ignore. With NFS, FUSE, autofs there's just so
                                         * much reason this might fail, let's hence be strict on the
                                         * top-level mount and lenient on the inner ones. */
                                        log_debug_errno(r, "Couldn't remount submount '%s' for unexpected reason, ignoring: %m", x);
                                        continue;
                                }

                                return r; /* Make this fatal if this is the top-level mount */
                        }
...
int bind_remount_one_with_mountinfo(
        ...
        if (r < 0) {
                if ((((flags & flags_mask) ^ new_flags) & ~MS_RELATIME) != 0)
                        return r;
