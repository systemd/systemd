...
int mount_fd_clone(int mount_fd, bool recursive, int *replacement_fd) {
        ...
        int flags = OPEN_TREE_CLONE|OPEN_TREE_CLOEXEC|AT_EMPTY_PATH|(recursive ? AT_RECURSIVE : 0), r;
        ...
        /* Clone a detached mount (that may be owned by a foreign mountns, i.e. mountfsd's). For this to
        ...
        cloned_fd = RET_NERRNO(open_tree(mount_fd, "", flags));
        if (cloned_fd >= 0 || cloned_fd != -EINVAL)
        ...
        if (r == 0) { /* Child */
                ...
                for (int i = 0; i < 1 + !!replacement_fd; i++) {
                        ...
                        cloned_fd = open_tree(mount_fd, "", flags);
                        if (cloned_fd < 0) {
