...
/* Turn an errno from listmount()/statmount() into a return code, latching skip_statmount when the
 * error means the syscalls are unusable on this host rather than that this one call was wrong: not
 * supported (older kernel), not permitted (seccomp filter, LSM), or a rejected request shape
 * (EINVAL). In all those environments /proc/self/mountinfo is still readable, so -EOPNOTSUPP routes
 * the caller to the libmount path. The two non-errno bail-outs, an incomplete statmount mask and
 * the id-list size cap, do not latch: both can be true of one mount table without the syscalls
 * being unusable. */
 ...
 * statmount() splits the two. The fs_subtype > 0 check is what guards against kernels older than
 * 6.15: they echo the (to them unknown) STATMOUNT_FS_SUBTYPE request bit back in the mask while
 * leaving the field zeroed, and offset 0 is never a real subtype, since kernels that fill the
 * field reserve offset 0 for an empty string. */
...
/* Queries one mount id for the given STATMOUNT_* masks. 'param' is required: if the kernel returns
 * less, this fails with -EOPNOTSUPP, since a partial answer would silently under-report and the
 * caller is better served by the libmount fallback. 'param_optional' is requested but not verified,
 * for fields the caller can do without on kernels that predate them; note that kernels without
 * STATMOUNT_SUPPORTED handling (pre-6.15) echo unknown request bits back in the returned mask
 * without filling anything, so the caller must not trust the mask alone for optional fields.
 * Returns -ENOENT if the id vanished (raced with an unmount; the caller skips it). */
static int statmount_one(uint64_t mnt_id, uint64_t param, uint64_t param_optional, struct statmount *buf, size_t bufsz) {
        struct mnt_id_req req = {
                .size = sizeof(req),
                .mnt_id = mnt_id,
        ...
        if (statmount(&req, buf, bufsz, 0) < 0) {
                if (errno == ENOENT)
                        return -ENOENT;
 ...
 * mounts in ascending id order and an overmount always has the higher id.
 *
...
static int mount_flags_by_path_statmount(const char *path, unsigned long *ret) {
        ...
        FOREACH_ARRAY(id, ids, (size_t) n_ids) {
                ...
                r = statmount_one(*id, STATMOUNT_MNT_BASIC|STATMOUNT_MNT_POINT, 0, sm, bufsz);
                if (r == -ENOENT)
                        continue;
                if (r < 0)
                        return r;
