...
int mount_fd_clone(int mount_fd) {
        int r;

        assert(mount_fd >= 0);

        /* Clone a detached mount (that may be owned by a foreign mountns, i.e. mountfsd's), taking
         * possession of it, so that our own mountns owns it. For this we have to jump through quite
         * some hoops, because the kernel currently doesn't allow us to just call open_tree(OPEN_TREE_CLONE)
         * directly to get a clone of a mount that is detached and owned by another mountns. Hence here's what
        ...
        if (r == 0) { /* Child */
                ...
                /* Now open the 2nd new mount namespace. This changes the ownership of the mount we attached above. */
                if (unshare(CLONE_NEWNS) < 0)
                        report_errno_and_exit(errno_pipe_fds[1], -errno);

                /* And now clone the attached mount that is now ours. */
                _cleanup_close_ int cloned_fd = open_tree(AT_FDCWD, "/", OPEN_TREE_CLONE|OPEN_TREE_CLOEXEC);
                if (cloned_fd < 0)
