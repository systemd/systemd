...
int mount_fd_clone(int mount_fd, bool recursive, int *second_clone) {
        int r;
        ...
        if (r == 0) { /* Child */
                ...
                for (int i = 0; i < 1 + !!second_clone; i++) {
                        /* And now clone the attached mount that is now ours. */
                        _cleanup_close_ int fd = open_tree(
                                        mount_fd, "",
        ...
        fd1 = receive_one_fd(transfer_fds[0], 0);
        if (fd1 < 0)
                return fd1;
        ...
        if (second_clone) {
                ...
                close_and_replace(*second_clone, fd2);
        }
