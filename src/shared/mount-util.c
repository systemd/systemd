...
static int mount_snapshot_acquire(FILE *proc_self_mountinfo, MountSnapshot *ret) {
        ...
        r = libmount_parse_kernel(MOUNT_SNAPSHOT_STATMOUNT_MASK, MNT_ITER_FORWARD, &table, &iter);
        if (r < 0) {
                /* Any failure of the fast path falls back, not just the unavailability errnos. The code
                 * this replaces had no way to fail per-mount, so an unexpected errno here must not be the
                 * thing that aborts namespace setup. */
                if (r != -EOPNOTSUPP)
...
static int mount_flags_by_path(FILE *proc_self_mountinfo, const char *path, unsigned long *ret) {
        ...
        r = libmount_parse_kernel(STATMOUNT_MNT_BASIC|STATMOUNT_MNT_POINT, MNT_ITER_FORWARD, &table, &iter);
        if (r < 0) {
                if (r != -EOPNOTSUPP)
                        log_debug_errno(r, "listmount() enumeration failed, falling back to mountinfo: %m");
