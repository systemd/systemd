...
static int mount_snapshot_entry_compare(const MountSnapshotEntry *a, const MountSnapshotEntry *b) {
        ...
        /* path_compare() orders a prefix before what it prefixes, so comparing the two the other
         * way round puts every child ahead of its parent, which is the order
         * umount_recursive_full() unmounts in. Entries with identical paths (overmounts) compare
         * equal, which is fine: the caller unmounts the topmost mount at a path and re-snapshots
         * after every success. */
        return path_compare(b->path, a->path);
}
