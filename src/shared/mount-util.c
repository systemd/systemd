 ...
 * to some high UID range, ans since UID mappings have to be bijective we thus cannot use them at
...
static int make_userns(uid_t uid_shift, uid_t uid_range, RemountIdmapFlags flags) {
         ...
         * in the OS tree before booting into it, and this becomes very easy and straightforward to do that
         * if it can just do it under its own regular UID. Note that in that case the container's runtime
         * uidmap (i.e. the one the container payload processes run in) will leave this UID unmapped, i.e. if
         * we accidentally leave files owned by host root in the already uidmapped tree around they'll show
         * up as owned by 'nobody', which is safe. (Of course, we shouldn't leave such inodes around, but
         * always chown() them to the container's own UID range.) */
