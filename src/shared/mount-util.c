...
int mount_switch_root(const char *path, unsigned long mount_propagation_flags) {
        ...
        if ((mount_propagation_flags & (MS_SHARED|MS_SLAVE|MS_PRIVATE)) == 0)
                return 0;

        if (mount(NULL, ".", NULL, mount_propagation_flags | MS_REC, 0) < 0)
                return log_debug_errno(errno, "Failed to turn new rootfs '%s' into %s mount: %m",
                                       mount_propagation_flags_to_string(mount_propagation_flags), path);
