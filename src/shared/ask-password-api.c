...
static int touch_ask_password_directory(AskPasswordFlags flags) {
        ...
        if (FLAGS_SET(flags, ASK_PASSWORD_USER)) {
                ...
                r = acquire_user_ask_password_directory(&p);
                if (r < 0)
                        return r;
                if (r == 0)
                        return 0;
                ...
                r = touch("/run/systemd/ask-password/");
        if (r < 0)
                return r;
...
int ask_password_tty(
        ...
        if (FLAGS_SET(flags, ASK_PASSWORD_ACCEPT_CACHED) && req && keyring) {
                ...
                if (FLAGS_SET(flags, ASK_PASSWORD_USER)) {
                        r = acquire_user_ask_password_directory(&buffer);
                        if (r < 0)
                                return r;

                        watch_path = buffer;
                } else
                        watch_path = "/run/systemd/ask-password";
...
int ask_password_agent(
        ...
        if (FLAGS_SET(flags, ASK_PASSWORD_USER)) {
                ...
                askpwdir = "/run/systemd/ask-password";
...
int acquire_user_ask_password_directory(char **ret) {
        ...
        const char *e = secure_getenv("XDG_RUNTIME_DIR");
        if (!e) {
                if (ret)
                        *ret = NULL;
                return 0;
        ...
        if (ret) {
                _cleanup_free_ char *j = path_join(e, "systemd/ask-password");
                if (!j)
                        return -ENOMEM;
