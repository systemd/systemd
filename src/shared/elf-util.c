...
static int parse_core(int fd, const char *executable, const char *sysroot, char **ret, sd_json_variant **ret_package_metadata) {
...
#if HAVE_DWFL_SET_SYSROOT
        if (sysroot && sym_dwfl_set_sysroot(c.dwfl, sysroot) < 0)
                return log_warning_errno(SYNTHETIC_ERRNO(EINVAL), "Could not set sysroot directory, dwfl_set_sysroot() failed: %s", sym_dwfl_errmsg(sym_dwfl_errno()));
#endif
...
static int parse_elf(int fd, const char *executable, const char *sysroot, char **ret, sd_json_variant **ret_package_metadata) {
        ...
        if (elf_header.e_type == ET_CORE) {
                ...
                r = parse_core(fd, executable, sysroot, ret ? &out : NULL, &package_metadata);
                if (r < 0)
...
int parse_elf_object(int fd, const char *executable, const char *sysroot, bool fork_disable_dump, char **ret, sd_json_variant **ret_package_metadata) {
        ...
        if (r == 0) {
                ...
                r = parse_elf(fd, executable, sysroot, ret ? &buf : NULL, ret_package_metadata ? &package_metadata : NULL);
                if (r < 0)
                        goto child_fail;
