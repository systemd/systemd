...
static int parse_package_metadata(const char *name, JsonVariant *id_json, Elf *elf, bool *ret_interpreter_found, struct StackContext *c) {
...
static int parse_buildid(Dwfl_Module *mod, Elf *elf, const char *name, struct StackContext *c, JsonVariant **ret_id_json) {
...
static int parse_core(int fd, const char *executable, char **ret, JsonVariant **ret_package_metadata) {
        ...
        if (!c.elf) {
                log_warning("Could not parse core file, elf_begin() failed: %s", sym_elf_errmsg(sym_elf_errno()));
                return -EINVAL;
        }

        c.dwfl = sym_dwfl_begin(&callbacks);
        if (!c.dwfl) {
                log_warning("Could not parse core file, dwfl_begin() failed: %s", sym_dwfl_errmsg(sym_dwfl_errno()));
                return -EINVAL;
        }

        if (sym_dwfl_core_file_report(c.dwfl, c.elf, executable) < 0) {
                log_warning("Could not parse core file, dwfl_core_file_report() failed: %s", sym_dwfl_errmsg(sym_dwfl_errno()));
                return -EINVAL;
        }

        if (sym_dwfl_report_end(c.dwfl, NULL, NULL) != 0) {
                log_warning("Could not parse core file, dwfl_report_end() failed: %s", sym_dwfl_errmsg(sym_dwfl_errno()));
                return -EINVAL;
        }

        if (sym_dwfl_getmodules(c.dwfl, &module_callback, &c, 0) < 0) {
                log_warning("Could not parse core file, dwfl_getmodules() failed: %s", sym_dwfl_errmsg(sym_dwfl_errno()));
                return -EINVAL;
        }

        if (sym_dwfl_core_file_attach(c.dwfl, c.elf) < 0) {
                log_warning("Could not parse core file, dwfl_core_file_attach() failed: %s", sym_dwfl_errmsg(sym_dwfl_errno()));
                return -EINVAL;
        }

        if (sym_dwfl_getthreads(c.dwfl, thread_callback, &c) < 0) {
                log_warning("Could not parse core file, dwfl_getthreads() failed: %s", sym_dwfl_errmsg(sym_dwfl_errno()));
                return -EINVAL;
        }
...
static int parse_elf(int fd, const char *executable, char **ret, JsonVariant **ret_package_metadata) {
        ...
        c.elf = sym_elf_begin(fd, ELF_C_READ_MMAP, NULL);
        if (!c.elf) {
                log_warning("Could not parse ELF file, elf_begin() failed: %s", sym_elf_errmsg(sym_elf_errno()));
                return -EINVAL;
        }

        if (!sym_gelf_getehdr(c.elf, &elf_header)){
                log_warning("Could not parse ELF file, gelf_getehdr() failed: %s", sym_elf_errmsg(sym_elf_errno()));
                return -EINVAL;
        }
