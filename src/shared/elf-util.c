...
static int parse_core(int fd, const char *executable, const char *root, char **ret, sd_json_variant **ret_package_metadata) {
...
#if HAVE_DWFL_SET_SYSROOT
        if (sym_dwfl_set_sysroot && root && sym_dwfl_set_sysroot(c.dwfl, root) < 0)
...
static int parse_elf(int fd, const char *executable, const char *root, char **ret, sd_json_variant **ret_package_metadata) {
        ...
        if (elf_header.e_type == ET_CORE) {
                ...
                r = parse_core(fd, executable, root, ret ? &out : NULL, &package_metadata);
                if (r < 0)
