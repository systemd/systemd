...
static int make_authenticated_variable_data(
        ...
        if (name_size > SIZE_MAX - sizeof(vendor) - sizeof(attributes) - sizeof(*timestamp) ||
            payload->iov_len > SIZE_MAX - name_size - sizeof(vendor) - sizeof(attributes) - sizeof(*timestamp))
                return -EOVERFLOW;
        total_size = name_size + sizeof(vendor) + sizeof(attributes) + sizeof(*timestamp) + payload->iov_len;
