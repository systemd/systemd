...
static int loader_has_tmp2(void) {
        _cleanup_free_ uint32_t *active_pcr_banks = NULL;
        size_t size;
        int r;

        r = efi_get_variable(EFI_LOADER_VARIABLE_STR("LoaderTpm2ActivePcrBanks"), NULL, (void **)&active_pcr_banks, &size);
        if (r < 0) {
                if (r != -ENOENT)
        ...
        if (size != sizeof(uint32_t))
                return log_debug_errno(SYNTHETIC_ERRNO(-EINVAL), "Unexpected size of LoaderTpm2ActivePcrBanks variable: %zu", size);

        assert(active_pcr_banks);
