...
static int inspect_pretty_name(
                FILE *uki,
                struct PeSectionHeader *sections,
                size_t scount,
                char **ret) {
...
static int inspect_uki(
        ...
        if (ret_cmdline) {
                r = inspect_string(uki, sections, scount, name_cmdline, sizeof(name_cmdline), &cmdline);
                if (r < 0)
                        return r;
        }

        if (ret_uname) {
                r = inspect_string(uki, sections, scount, name_uname, sizeof(name_uname), &uname);
                if (r < 0)
                        return r;
        }

        if (ret_pretty_name) {
                r = inspect_pretty_name(uki, sections, scount, &pname);
                if (r < 0)
                        return r;
        }
...
DEFINE_STRING_TABLE_LOOKUP(kernel_type, KernelType);
