...
static SD_VARLINK_DEFINE_METHOD(
                RelinquishVar,
                SD_VARLINK_FIELD_COMMENT("Controls whether to preserve the late-shutdown journal across a kexec via LUO (Live Update Orchestrator), when available (if not, falls back to normal behavior). Disabled by default."),
                SD_VARLINK_DEFINE_INPUT(withLUO, SD_VARLINK_BOOL, SD_VARLINK_NULLABLE));
