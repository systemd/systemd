...
int image_read_metadata(Image *i) {
        ...
        case IMAGE_DIRECTORY: {
                ...
                _cleanup_strv_free_ char **extension_release = NULL, **syscfg_release = NULL;
                ...
                r = load_syscfg_release_pairs(i->path, i->name, /* relax_syscfg_release_check= */ false, &syscfg_release);
                if (r < 0)
                        log_debug_errno(r, "Failed to read syscfg-release in image, ignoring: %m");
                ...
                strv_free_and_replace(i->syscfg_release, syscfg_release);
                break;
        }
