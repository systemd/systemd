...
static int image_make(
        ...
        r = name_to_handle_at_try_unique_mntid_fid(dir_fd, filename, &fh, &on_mount_id, 0);
        if (r < 0) {
                if (is_name_to_handle_at_fatal_error(r))
                        return r;

                r = path_get_unique_mnt_id_at(dir_fd, filename, &on_mount_id);
                if (r < 0) {
                        if (!ERRNO_IS_NEG_NOT_SUPPORTED(r))
                                return r;

                        int on_mount_id_fallback = -1;
                        r = path_get_mnt_id_at(dir_fd, filename, &on_mount_id_fallback);
                        if (r < 0)
                                return r;
