...
static const char* image_class_suffix[_IMAGE_CLASS_MAX] = {
        [IMAGE_SYSEXT]  = ".sysext",
...
static const char *get_image_class_suffix(ImageClass c) {
        if (c < 0)
                return NULL;

        assert(c < _IMAGE_CLASS_MAX);
        return image_class_suffix[c];
}
...
static int image_make(
        ...
        if (S_ISDIR(st->st_mode)) {
                ...
                if (!pretty) {
                        r = extract_pretty(filename, get_image_class_suffix(c), NULL, &pretty_buffer);
                        if (r < 0)
                                return r;
                ...
                if (!pretty) {
                        r = extract_pretty(filename, get_image_class_suffix(c), ".raw", &pretty_buffer);
                        if (r < 0)
                                return r;
...
int image_discover(
        ...
        NULSTR_FOREACH(path, pick_image_search_path(class)) {
                ...
                FOREACH_DIRENT_ALL(de, d, return -errno) {
                        ...
                        if (S_ISREG(st.st_mode))
                                r = extract_pretty(de->d_name, get_image_class_suffix(class), ".raw", &pretty);
                        else if (S_ISDIR(st.st_mode))
                                r = extract_pretty(de->d_name, get_image_class_suffix(class), NULL, &pretty);
                        else if (S_ISBLK(st.st_mode))
