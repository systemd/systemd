...
int image_remove(Image *i, RuntimeScope scope) {
        ...
        case IMAGE_DIRECTORY:
                ...
                if (i->foreign_uid_owned) {
                        _cleanup_close_ int userns_fd = nsresource_allocate_userns(/* name= */ NULL, /* size= */ 0x10000);
                        if (userns_fd < 0)
                        ...
                        if (r < 0)
                                return r;
                        /* For off child that moves into userns and does the copying */
                        r = safe_fork_full(
                                        "rm-tree",
                                        /* stdio_fds= */ NULL,
                        ...
                        if (r == 0) {
                                ...
                                if (r < 0)
                                        log_warning_errno(r, "Failed to empty '%s' directory in foreign UID mode, ignoring: %m", i->path);
...
static int get_pool_directory(
        ...
        _cleanup_free_ char *p = path_join(root, n, fname, suffix);
        if (!p)
                return -ENOMEM;
