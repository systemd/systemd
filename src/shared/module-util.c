...
static int blacklist_modules(const char *p, char ***blacklist) {
        ...
        if (strv_extend_strv(blacklist, k, true) < 0)
                return -ENOMEM;
...
static int parse_proc_cmdline_item(const char *key, const char *value, void *data) {
        ...
        if (proc_cmdline_key_streq(key, "module_blacklist")) {
                ...
                r = blacklist_modules(value, data);
                if (r < 0)
                        return r;
        }
...
int module_load_and_warn(struct kmod_ctx *ctx, const char *module, bool verbose) {
        ...
        _cleanup_strv_free_ char **blacklist = NULL;
        bool blacklist_parsed = false;
        int r;
        ...
        kmod_list_foreach(itr, modlist) {
                ...
                default:
                        ...
                        if (err == 0)
                                ...
                                if (err == -EPERM) {
                                        if (!blacklist_parsed) {
                                                r = proc_cmdline_parse(parse_proc_cmdline_item, &blacklist, 0);
                                                if (r < 0)
                                        ...
                                        if (strv_contains(blacklist, kmod_module_get_name(mod))) {
                                                log_full(verbose ? LOG_INFO : LOG_DEBUG,
