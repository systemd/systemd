...
static int dispatch_in_addr_full(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        static const sd_json_dispatch_field dispatch_table[] = {
                ...
                { "Port",       SD_JSON_VARIANT_UNSIGNED, sd_json_dispatch_uint16,      offsetof(struct in_addr_full, port),        0                 },
                { "ServerName", SD_JSON_VARIANT_STRING,   sd_json_dispatch_string,      offsetof(struct in_addr_full, server_name), 0                 },
                {},
        };
...
int dispatch_link_set_dns_parameters(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        ...
        r = sd_json_dispatch(variant, dispatch_table, SD_JSON_LOG, &p);
        if (r < 0)
                return r;
