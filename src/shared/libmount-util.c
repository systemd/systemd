...
/* Latched once we learn this kernel has no listmount()/statmount(), so we stop paying for the probe
 * syscall and two allocations on every later call. */
static bool kernel_enumeration_unsupported = false;
...
static int statmount_unsupported(int r) {
        ...
        kernel_enumeration_unsupported = true;
        return -EOPNOTSUPP;
}
...
int libmount_parse_kernel(
        ...
        /* libmount probes the syscall inside mnt_new_statmnt() and returns NULL when the kernel
         * cannot do it, which is a fall-back-to-mountinfo answer rather than an error. Probe before
         * allocating anything, and remember an unsupported kernel so the probe syscall is not
         * repeated on every call for the rest of the process. */
        if (kernel_enumeration_unsupported)
                return -EOPNOTSUPP;
