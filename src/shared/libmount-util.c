...
int libmount_parse_kernel(
        ...
        /* Older libmount: no statmount() support to speak of. */
        if (!sym_mnt_new_statmnt || !sym_mnt_statmnt_set_mask || !sym_mnt_table_refer_statmnt ||
            !sym_mnt_table_listmount_set_id || !sym_mnt_table_fetch_listmount)
                return -EOPNOTSUPP;
        ...
        if (r < 0)
                return r; /* -ENOSYS/-ENOTSUP here means the kernel is older than 6.8 */
