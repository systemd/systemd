...
#include <sys/mount.h>   /* LSMT_ROOT; libmount.h requires this include first */
...
int libmount_parse_kernel(
        ...
        iter = sym_mnt_new_iter(direction);
        if (!table || !iter)
                return -ENOMEM;        /* Propagation info is what makes reading /proc/self/mountinfo expensive: the kernel
...
int dlopen_libmount(int log_level) {
         ...
         * the mountinfo parse when an older libmount left them NULL. Resolved on every call on
         * purpose, like the block above: it is idempotent, and it guarantees the pointers are in
         * place by the time any call returns, rather than only after the call that happened to load
