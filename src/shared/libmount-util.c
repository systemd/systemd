...
int path_is_temporary_fs_harder(const char *path) {
         ...
         * mountpoint is checked recursively. This recursive check is done up to 5 times, in case somebody
         * wants to go crazy and nest overlays.
         */
        ...
        for (unsigned tries = 0; tries < 5; tries++) {
                ...
                if (r == 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "Mount \"%s\" is overlay, but upperdir= option not found.", path);
