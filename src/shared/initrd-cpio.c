...
int initrd_cpio_credentials_to_tempfile(
        ...
        r = append_cpio_entry(&buf, &buf_size, S_IFDIR | 0500, ".extra/credentials", NULL, 0, &inode);
        if (r < 0)
                return log_error_errno(r, "Failed to write '.extra/credentials' directory entry to credentials cpio: %m");
