...
const IMAGE_SECTION_HEADER *pe_header_find_section(
        ...
        FOREACH_ARRAY(section, sections, le16toh(pe_header->pe.NumberOfSections))
                if (strncmp((char*) section->Name, name, sizeof(section->Name)) == 0 &&
                    memeqzero(section->Name + n, sizeof(sections[0].Name) - n))
                        return section;
...
int pe_load_headers(
        ...
        PeHeader *pe_header_tmp = realloc(
                        pe_header,
                        MAX(sizeof(PeHeader),
                            offsetof(PeHeader, optional) + le16toh(pe_header->pe.SizeOfOptionalHeader)));
        if (!pe_header_tmp)
                return log_oom_debug();
        ...
        if (offsetof(PeHeader, optional) + le16toh(pe_header->pe.SizeOfOptionalHeader) !=
...
int pe_load_sections(
        ...
        n = pread(fd,
                  ...
                  le32toh(dos_header->e_lfanew) + offsetof(PeHeader, optional) + le16toh(pe_header->pe.SizeOfOptionalHeader));
        if (n < 0)
