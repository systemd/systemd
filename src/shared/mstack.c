...
static int mstack_apply_attr(int dfd, MStackFlags flags) {
        ...
        if (mount_setattr(dfd, "", AT_EMPTY_PATH|AT_RECURSIVE,
                ...
                return log_debug_errno(errno, "Failed to ser mount attributes: %m");
...
int mstack_apply_bind_mounts(
                ...
                const char *where,
                MStackFlags flags,
                MStackBindMountStrategy mstack_bindmount_strategy) {
        int r;
        ...
        if (mstack_bindmount_strategy != MSTACK_BINDMOUNT_NONE) {
                FOREACH_ARRAY(m, mstack->mounts, mstack->n_mounts) {
                        ...
                        if (mstack_bindmount_strategy == MSTACK_BINDMOUNT_EXCEPT_VAR && path_startswith(m->where, "/var"))
                                continue;

                        if (mstack_bindmount_strategy == MSTACK_BINDMOUNT_VAR && !path_startswith(m->where, "/var"))
                                continue;
...
int mstack_bind_mounts(
        ...
        r = mstack_apply_bind_mounts(mstack, root_fd, where, flags, mstack_bindmount_strategy);
        if (r < 0)
                return r;
