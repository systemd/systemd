...
static int mstack_load_one(MStack *mstack, const char *dir, int dir_fd, const char *fname) {
        ...
        if (streq(unsuffixed, "work")) {
                ...
                *m = (MStackMount) {
                ...
                mstack->n_mounts++;
                log_debug("Found mstack work layer");
                return 0;
        }
...
int mstack_make_mounts(
        ...
        FOREACH_ARRAY(m, mstack->mounts, mstack->n_mounts) {
                ...
                log_debug("Acquired mstack DDI layer '%s'", empty_to_root(m->where));
        }
        ...
        if (mstack->has_overlayfs) {
                ...
                /* Kernel expects the stack in reverse order, hence go down */
        ...
        FOREACH_ARRAY(m, mstack->mounts, mstack->n_mounts) {
                ...
                log_debug("Acquired mstack bind '%s' submount.", empty_to_root(m->where));
        }
