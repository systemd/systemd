...
bool mstack_has_writable_layers(MStack *mstack, MStackFlags flags) {
        ...
        FOREACH_MSTACK_MOUNT_TYPE(m, mstack, MSTACK_RW)
                return true;

        return false;
}

/* Four things about the kernel underneath us that this code has to know, and that cannot be derived from
 * a version number. Distributions back-port these partially: the "lowerdir+" quirk below exists precisely
 * because a kernel can report a version that has the feature while the feature does not work, so a
 * uname() check would be wrong on exactly the kernels that need the fallback. Ask the kernel instead,
 * once per process.
 *
...
struct MStackCaps {
        ...
        bool lowerdir_plus;         /* incremental "lowerdir+" actually realizes a superblock (6.5) */
};
...
/* Asks whether overlayfs will take layers as fds, and whether an incrementally built "lowerdir+" stack
 * actually realizes. Both are answered by the same attempt, because the second is only observable at
 * FSCONFIG_CMD_CREATE - on the kernels this exists for, every fsconfig() call succeeds and only the
 * create fails. */
static void mstack_probe_lowerdir(MStackCaps *c) {
        assert(c);

        _cleanup_close_ int lower_fd = mstack_probe_scratch_tmpfs();
        if (lower_fd < 0)
                return (void) log_debug_errno(lower_fd, "mstack probe: no scratch tmpfs, assuming no lowerdir+ support: %m");
        ...
        if (fsconfig(sb_fd, FSCONFIG_SET_FD, "lowerdir+", /* value= */ NULL, lower_fd) >= 0)
                c->layer_fd = true;
        else if (fsconfig(sb_fd, FSCONFIG_SET_STRING, "lowerdir+", FORMAT_PROC_FD_PATH(lower_fd), /* aux= */ 0) < 0)
                return (void) log_debug_errno(errno, "mstack probe: 'lowerdir+' not accepted at all: %m");

        /* An overlay needs at least two lower layers before it will realize with no upper. */
        _cleanup_close_ int lower2_fd = mstack_probe_scratch_tmpfs();
        if (lower2_fd < 0)
                return (void) log_debug_errno(lower2_fd, "mstack probe: no second scratch tmpfs: %m");

        if (c->layer_fd) {
                if (fsconfig(sb_fd, FSCONFIG_SET_FD, "lowerdir+", /* value= */ NULL, lower2_fd) < 0)
                        return (void) log_debug_errno(errno, "mstack probe: second 'lowerdir+' rejected: %m");
        } else if (fsconfig(sb_fd, FSCONFIG_SET_STRING, "lowerdir+", FORMAT_PROC_FD_PATH(lower2_fd), /* aux= */ 0) < 0)
                return (void) log_debug_errno(errno, "mstack probe: second 'lowerdir+' rejected: %m");

        if (fsconfig(sb_fd, FSCONFIG_CMD_CREATE, /* key= */ NULL, /* value= */ NULL, /* aux= */ 0) < 0)
                return (void) log_debug_errno(errno, "mstack probe: 'lowerdir+' accepted but the superblock will not realize, "
                                              "the joined 'lowerdir=' fallback is needed: %m");

        c->lowerdir_plus = true;
}
...
static const MStackCaps* mstack_caps(void) {
        ...
        if (secure_getenv_bool("SYSTEMD_MSTACK_ASSUME_NO_CAPS") > 0)
                ...
                mstack_probe_lowerdir(&cache);
        } else
                log_debug("mstack probe: no new mount API, assuming nothing.");

        log_debug("mstack capabilities: layer_fd=%s lowerdir_plus=%s",
                  yes_no(cache.layer_fd), yes_no(cache.lowerdir_plus));
...
/* Works out the whole strategy up front, touching nothing. Must run before
 * mstack_back_synthetic_rw_layers(), which is what gives a synthetic rw/ layer the mount_fd that
 * needs_tmpfs_backing is looking for the absence of. */
        ...
        *plan = (MStackPlan) {
        ...
        FOREACH_MSTACK_LAYER(m, mstack) {
                ...
                plan->layers[plan->n_layers++] = (MStackLayerPlan) {
                        .mount = m,
                        .role = writable ? MSTACK_LAYER_ROLE_UPPER : MSTACK_LAYER_ROLE_LOWER,
                        .identity = mstack_layer_identity(mstack, uid_is_valid(uid_shift), writable, &identity_uid),
                        .identity_uid = identity_uid,
                        .needs_tmpfs_backing = m->mount_type == MSTACK_RW && m->what_fd < 0 && m->mount_fd < 0,
                };
        }
 ...
 * read-only lower, or as the single writable upper - which the kernel wants as a data/work pair. The three
 * mount types that reach here map onto those two roles, so the caller dispatches on the role rather than
 * on the type. */
 ...
 * type calls for. */
static int mstack_overlay_builder_add(MStack *mstack, MStackMount *m, MStackOverlayBuilder *b) {
        int r;
        ...
        bool rw_writable = lp->role == MSTACK_LAYER_ROLE_UPPER;
        ...
        if (r < 0)
                return log_debug_errno(r, "Failed to attach layer '%s' to '%s': %m",
                                       strna(m->what), b->temp_mount_dir);
...
static int mstack_overlay_assemble(
        ...
        FOREACH_MSTACK_LAYER_TOP_DOWN(m, mstack) {
                r = mstack_overlay_builder_add(mstack, m, &builder);
                if (r < 0)
                        return r;
        }
...
static int mstack_make_overlayfs(
        ...
        if (r == 0) {
                /* child */
                r = mstack_overlay_assemble(mstack, plan, temp_mount_dir, flags, uidmap_userns_fd,
...
int mstack_make_mounts(
        ...
        r = mstack_realize_root(mstack, plan, temp_mount_dir, flags, uid_shift);
        if (r < 0)
                return r;
        ...
        if (plan->extract_usr) {
                ...
                r = mstack_extract_usr(mstack, temp_mount_dir, uid_shift);
                if (r < 0)
                        return r;
        }
