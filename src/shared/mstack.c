...
static int mstack_extract_usr(MStack *mstack, const char *temp_mount_dir, uid_t uid_shift) {
        ...
        /* Detach on every path out, without letting the detach itself clobber the errno we are about
         * to report. */
...
static int mstack_apply_attr(
        ...
        if (mount_setattr(dfd, "", attr_flags, &ma, sizeof(ma)) < 0) {
                ...
                if (rdonly)
                        return log_debug_errno(errno, "Failed to recursively make mount read-only: %m");
...
static int mstack_open_mount_point(int root_fd, const char *where, int *ret_fd) {
        ...
        if (r == -EROFS)
                return log_debug_errno(r, "Failed to create mount point directory '%s': root is read-only. "
