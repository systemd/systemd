...
static int mstack_apply_attr(int dfd, MStackMountType mount_type, MStackFlags flags) {
        bool rdonly = FLAGS_SET(flags, MSTACK_RDONLY) || mount_type == MSTACK_ROBIND;
...
int mstack_apply_bind_mounts(
        ...
        assert(where);
        assert(filter >= 0);
        assert(filter < _MSTACK_BINDMOUNT_FILTER_MAX);
        ...
        FOREACH_ARRAY(m, mstack->mounts, mstack->n_mounts) {
                ...
                r = mstack_apply_attr(m->mount_fd, m->mount_type, flags);
                if (r < 0)
                        return r;
...
int mstack_bind_mounts(
        ...
        r = mstack_apply_attr(root_fd, MSTACK_ROOT, flags);
        if (r < 0)
                return r;
