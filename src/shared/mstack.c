...
static int mstack_load_one(MStack *mstack, const char *dir, int dir_fd, const char *fname) {
        ...
        if (r > 0) {
                ...
                *m = (MStackMount) {
                ...
                mstack->n_mounts++;
                log_debug("Found mstack layer '%s'", m->sort_key);
                return 0;
        }
        ...
        if (streq(unsuffixed, "rw")) {
                ...
                *m = (MStackMount) {
                ...
                mstack->n_mounts++;
                log_debug("Found mstack rw layer");
                return 0;
        }
        ...
        if (bind_type >= 0) {
                ...
                *m = (MStackMount) {
                ...
                mstack->n_mounts++;
                log_debug("Found mstack bind layer '%s'", empty_to_root(m->where));
                return 0;
        }
        ...
        if (streq(unsuffixed, "root")) {
                ...
                *m = (MStackMount) {
                ...
                mstack->n_mounts++;
                log_debug("Found mstack root layer");
                return 0;
        }
...
static int mstack_load_now(MStack *mstack, const char *dir, int dir_fd, MStackFlags flags) {
        ...
        if (dir_fd < 0) {
                ...
                if (dir_fd < 0)
                        return log_error_errno(dir_fd, "Failed to reopen '%s': %m", dir);
        }
...
static int mstack_make_overlayfs(
         ...
         * to overlayfs (see comments below), hence fork off a child with a private mount namespace, so that noone else sees
         * that. */
        r = safe_fork("(layerfd)",
        ...
        if (r == 0) {
                ...
                for (size_t i = mstack->n_mounts; i > 0; i--) {
                        ...
                        /* When working with detached mounts overlayfs currently insists on upperdir being the root
                         * inode of the mount. But that collides with the requirement that upperdir/workdir are on
                         * the same mount and siblings. Bummer. To work around this we'll temporarily attach the
                         * thing, which relaxes the rules sufficiently. */
                        ...
                        case MSTACK_RW: {
                                if (mount_is_ro(m, flags)) {
                                        ...
                                        if (data_fd < 0) {
                                                ...
                                                log_error_errno(errno, "Failed to open 'data' directory below 'rw' layer: %m");
                                                report_errno_and_exit(errno_pipe_fds[1], -errno);
                                        }

                                        /* Downgrade to regular lowerdir if read-only is requested */
                                        if (fsconfig(sb_fd, FSCONFIG_SET_FD, "lowerdir+", NULL, data_fd) < 0) {
                                                log_error_errno(errno, "Failed to set mount layer lowerdir+=%s/data: %m", m->what);
                                                report_errno_and_exit(errno_pipe_fds[1], -errno);
                                        }
                                } else {
                                        ...
                                        if (data_fd < 0) {
                                                log_error_errno(data_fd, "Failed to open 'data' directory below 'rw' layer: %m");
                                                report_errno_and_exit(errno_pipe_fds[1], data_fd);
                                        }

                                        if (fsconfig(sb_fd, FSCONFIG_SET_FD, "upperdir", NULL, data_fd) < 0) {
                                                log_error_errno(errno, "Failed to set mount layer upperdir=%s/data: %m", m->what);
                                                report_errno_and_exit(errno_pipe_fds[1], -errno);
                                        }
                                        ...
                                        if (work_fd < 0) {
                                                log_error_errno(work_fd, "Failed to open 'work' directory below 'rw' layer: %m");
                                        ...
                                        if (dup_fd < 0) {
                                                log_error_errno(errno, "Failed to duplicate work fd: %m");
                                                report_errno_and_exit(errno_pipe_fds[1], -errno);
                                        }
                                        ...
                                        if (r < 0)
                                                log_warning_errno(r, "Failed to empty 'work' directory below 'rw' layer: %m");

                                        if (fsconfig(sb_fd, FSCONFIG_SET_FD, "workdir", NULL, work_fd) < 0) {
                                                log_error_errno(errno, "Failed to set mount layer workdir=%s/work: %m", m->what);
                                                report_errno_and_exit(errno_pipe_fds[1], -errno);
                                        }
                        ...
                        case MSTACK_LAYER:
                                if (fsconfig(sb_fd, FSCONFIG_SET_FD, "lowerdir+", NULL, temp_fd) < 0) {
                                        log_error_errno(errno, "Failed to set mount layer lowerdir+=%s: %m", m->what);
                                        report_errno_and_exit(errno_pipe_fds[1], -errno);
                                }
                ...
                if (!writable && fsconfig(sb_fd, FSCONFIG_SET_FLAG, "ro", NULL, 0) < 0) {
                        log_error_errno(errno, "Failed to set read only mount flag: %m");
                        report_errno_and_exit(errno_pipe_fds[1], -errno);
                }

                if (fsconfig(sb_fd, FSCONFIG_SET_STRING, "source", mstack->path, 0) < 0) {
                        log_error_errno(errno, "Failed to set mount source: %m");
                        report_errno_and_exit(errno_pipe_fds[1], -errno);
                }

                if (fsconfig(sb_fd, FSCONFIG_CMD_CREATE, NULL, NULL, 0) < 0) {
                        log_error_errno(errno, "Failed to realize overlayfs: %m");
                        report_errno_and_exit(errno_pipe_fds[1], -errno);
                }
...
int mstack_apply(
        ...
        if (!temp_mount_dir) {
                r = mkdtemp_malloc("/tmp/mXXXXXX", &t);
                if (r < 0)
                        return r;
