...
int dlopen_bpf_full(int log_level) {
        ...
        r = dlopen_safe("libbpf.so.1", &dl, NULL);
        if (r < 0) {
                ...
                if (r < 0) {
                        if (dle)
                                log_full_errno(in_initrd() ? LOG_DEBUG : log_level, r, "Neither libbpf.so.1 nor libbpf.so.0 are installed, cgroup BPF features disabled: %s", dle);
                        else
                                log_full_errno(in_initrd() ? LOG_DEBUG : log_level, r, "Neither libbpf.so.1 nor libbpf.so.0 are installed, cgroup BPF features disabled: %m");
