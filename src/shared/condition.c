...
static int condition_test_fraction(Condition *c, char **env) {
        ...
        /* Expected syntax: "[TAG ]PERCENT". The percentage is mandatory; the optional leading tag
         * (quote it if it contains whitespace) is used as a hash salt, so that distinct staged
         * rollouts select independent subsets of a fleet. The condition is true for approximately
         * PERCENT of all machines — on a given machine when its derived value falls below the
         * threshold. This is useful for staged rollouts: hand the same unit to a whole fleet and
         * only a fraction of it will be enabled, with each machine deciding locally and stably (no
         * coordination required). */

        const char *p = c->parameter;
        _cleanup_free_ char *first = NULL, *second = NULL;

        r = extract_first_word(&p, &first, /* separators=*/ NULL, EXTRACT_UNQUOTE);
        if (r < 0)
        ...
        r = extract_first_word(&p, &second, /* separators= */ NULL , EXTRACT_UNQUOTE);
        if (r < 0)
                return log_debug_errno(r, "Failed to parse ConditionFraction=%s: %m", c->parameter);
