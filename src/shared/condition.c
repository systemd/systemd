...
static int condition_test_osrelease(Condition *c, char **env) {
        ...
        const char *parameter = c->parameter, *requested_value;
        OrderOperator order;
        int r;
        ...
        r = extract_first_word(&parameter, &key, "!<=>", 0);
        if (r < 0)
        ...
        /* parse_order() needs the string to start with the comparators, so reset the variable */
        parameter = c->parameter + strlen(key);
        requested_value = strstrip((char *)parameter);
        order = parse_order(&requested_value);
        if (order < 0 || isempty(requested_value))
                return log_debug_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "Failed to parse parameter, key/value format expected: %m");
        ...
        if (order == ORDER_EQUAL)
                return streq_ptr(actual_value, requested_value);
        if (order == ORDER_UNEQUAL)
                return !streq_ptr(actual_value, requested_value);

        return test_order(strverscmp_improved(actual_value, requested_value), order);
}
