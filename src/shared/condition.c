...
static int condition_test_psi(Condition *c, char **env) {
        ...
        if (!is_pressure_supported()) {
                log_debug_errno(SYNTHETIC_ERRNO(EOPNOTSUPP), "Pressure Stall Information (PSI) is not supported, skipping.");
                return 1;
        }
        ...
        if (r == 1) {
                ...
                if (r == 0) {
                        log_debug_errno(SYNTHETIC_ERRNO(EOPNOTSUPP), "PSI condition check requires the unified cgroups hierarchy, skipping.");
                        return 1;
                }
                ...
                if (!FLAGS_SET(mask, required_mask)) {
                        log_debug_errno(SYNTHETIC_ERRNO(EOPNOTSUPP),
                                        "Cgroup %s controller not available, skipping PSI condition check.",
                                        pressure_type);
                        return 1;
                }
