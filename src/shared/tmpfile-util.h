...
int fopen_temporary_at_label(int dir_fd, const char *target, const char *path, FILE **f, char **temp_path, LabelContext *label_userdata);
static inline int fopen_temporary_label(const char *target, const char *path, FILE **f, char **temp_path) {
        return fopen_temporary_at_label(AT_FDCWD, target, path, f, temp_path, NULL);
}
