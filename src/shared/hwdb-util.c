...
int hwdb_update(const char *root, const char *hwdb_bin_dir, bool strict, bool compat) {
        ...
        if (root) {
                ...
                root_abs = path_simplify(root_abs);
        }
        ...
        FOREACH_ARRAY(i, files, n_files) {
                ...
                const char *path_in_root = path_startswith_full(c->original_path, empty_to_root(root_abs), PATH_STARTSWITH_RETURN_LEADING_SLASH);
                assert(path_in_root);
