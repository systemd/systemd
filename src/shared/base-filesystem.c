...
typedef enum BaseFilesystemModifier {
        BASE_FILESYSTEM_IGNORE_ON_FAILURE = 1,
        BASE_FILESYSTEM_EMPTY_MARKER      = 2, /* If this is missing, then we are booting on an empty filesystem - see comment below */
        BASE_FILESYSTEM_EMPTY_ONLY        = 3, /* If booting on an empty filesystem create this, otherwise skip it - see comment below */
        _BASE_FILESYSTEM_MODIFIER_MAX,
        _BASE_FILESYSTEM_MODIFIER_INVALID = -EINVAL,
} BaseFilesystemModifier;
...
typedef struct BaseFilesystem {
        ...
        BaseFilesystemModifier modifier; /* various modifiers for behaviour on creation */
} BaseFilesystem;
...
int base_filesystem_create_fd(int fd, const char *root, uid_t uid, gid_t gid) {
        ...
        FOREACH_ELEMENT(i, table) {
                if (i->modifier == BASE_FILESYSTEM_EMPTY_ONLY && !empty_fs)
                        continue;
                ...
                if (r < 0) {
                        bool ignore = IN_SET(r, -EEXIST, -EROFS) || i->modifier == BASE_FILESYSTEM_IGNORE_ON_FAILURE;
                ...
                if (i->modifier == BASE_FILESYSTEM_EMPTY_MARKER)
                        empty_fs = true;
        }
