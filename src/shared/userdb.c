...
static void membership_data_release(struct membership_data *d) {
        d->user_name = mfree(d->user_name);
        d->group_name = mfree(d->group_name);
}
...
static int userdb_on_query_reply(
        ...
        case LOOKUP_MEMBERSHIP: {
                _cleanup_(membership_data_release) struct membership_data membership_data = {};
