...
static int find_sections(
        ...
        if (n < 0)
                return log_warning_errno(errno, "%s: Failed read DOS header: %m", path);
        if (n != sizeof(dos))
        ...
        if (n < 0)
                return log_warning_errno(errno, "%s: Failed to read PE header: %m", path);
        if (n != sizeof(pe))
        ...
        if (n < 0)
                return log_warning_errno(errno, "%s: Failed to read section data: %m", path);
        ...
        for (size_t i = 0; i < n_sections; i++) {
                ...
                if (n < 0)
                        return log_warning_errno(errno, "%s: Failed to read section payload: %m", path);
                if ((size_t) n != size)
                ...
                if (size > 0 && memchr(k, 0, size - 1))
                        return log_warning_errno(SYNTHETIC_ERRNO(EBADMSG), "%s: Section contains embedded NUL byte.", path);
