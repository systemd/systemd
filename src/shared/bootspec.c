...
static int boot_entry_load_unified(
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to extract name/version/sort-key from os-release data from unified kernel image %s, refusing.", path);
...
BootEntry* boot_config_find_oldest(
        ...
        for (size_t j = config->n_entries; j > 0; j--) {
