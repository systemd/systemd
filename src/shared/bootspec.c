...
static int config_check_inode(BootConfig *config, int fd, const char *fname) {
...
static int boot_entries_find(
        ...
        for (size_t i = 0; i < dentries->n_entries; i++) {
                ...
                r = config_check_inode(config, fileno(f), de->d_name);
                if (r < 0)
                        return r;
...
static int boot_entries_find_unified(
        ...
        FOREACH_DIRENT(de, d, return log_error_errno(errno, "Failed to read %s: %m", dir)) {
                ...
                r = config_check_inode(config, fd, de->d_name);
                if (r < 0)
                        return r;
                if (r == 0) /* inode already seen or otherwise not relevant */
                        continue;
