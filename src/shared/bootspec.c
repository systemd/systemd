...
int check_xbootldr_duplicates(bool unprivileged_mode) {
        ...
        if (unprivileged_mode)
                return check_part_type_duplicates_udev(GPT_XBOOTLDR_STR, MBR_XBOOTLDR_STR);
        else
                return check_part_type_duplicates_blkid(GPT_XBOOTLDR_STR, MBR_XBOOTLDR_STR);
}
...
int check_efi_duplicates(bool unprivileged_mode) {
         ...
         * we use udev in that case. */
        if (unprivileged_mode)
                return check_part_type_duplicates_udev(GPT_ESP_STR, MBR_ESP_STR);
        else
                return check_part_type_duplicates_blkid(GPT_ESP_STR, MBR_ESP_STR);
}
