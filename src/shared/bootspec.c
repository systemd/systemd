...
static void boot_entry_addons_free(void *a) {
        BootEntryAddons *addons = (BootEntryAddons *)a;
...
static int boot_entries_find_unified_addons(
        ...
        _cleanup_free_ char *full = NULL;
        _cleanup_(boot_entry_addons_free) BootEntryAddons addons = {};
        int r;
...
static int print_cmdline(
        ...
        if (!strv_isempty(e->options)) {
                ...
                if (indent_embedded_newlines(t, &options))
                        return log_oom();
        ...
        if (indent_embedded_newlines(t2, &combined_cmdline))
                return log_oom();
