...
int boot_entries_load_config(
        ...
        if (is_efi_boot()) {
                r = efi_get_variable_string(EFI_VARIABLE_LoaderEntryOneShot, &config->entry_oneshot);
                ...
                r = efi_get_variable_string(EFI_VARIABLE_LoaderEntryDefault, &config->entry_default);
                if (r < 0 && !IN_SET(r, -ENOENT, -ENODATA)) {
...
int boot_entries_augment_from_loader(
        ...
        STRV_FOREACH(i, found_by_loader) {
                ...
                p = strdup(EFIVAR_PATH(EFI_VARIABLE_LoaderEntries));
                if (!p)
                        return log_oom();
