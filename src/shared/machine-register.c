...
static const char* register_machine_failed_context_string(RuntimeScope scope, bool registered_system, bool registered_user) {
        ...
        if (r < 0) {
                if (graceful)
                        log_notice_errno(r, "Failed to register machine in %s context, ignoring.",
                                         register_machine_failed_context_string(scope, registered_system, registered_user));
                else
                        r = log_error_errno(r, "Failed to register machine in %s context.",
                                            register_machine_failed_context_string(scope, registered_system, registered_user));
        }
