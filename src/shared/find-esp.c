...
static int statx_mount_same(const struct statx *a, const struct statx *b) {
        assert(a);
        assert(b);
...
static int verify_fsroot_dir(
        ...
        STRUCT_STATX_DEFINE(sxa);
        STRUCT_STATX_DEFINE(sxb);
        int r;
        ...
        r = statx_fallback(fd, "", AT_EMPTY_PATH, STATX_TYPE|STATX_INO|STATX_MNT_ID, &sxa);
        if (r < 0)
        ...
        r = statx_fallback(fd, "..", 0, STATX_TYPE|STATX_INO|STATX_MNT_ID, &sxb);
        if (r < 0) {
                if (ERRNO_IS_PRIVILEGE(r)) {
                        ...
                        r = statx_fallback(AT_FDCWD, parent, AT_SYMLINK_NOFOLLOW, STATX_TYPE, &sxb);
                }
        }
        if (r < 0)
                return log_full_errno(unprivileged_mode && ERRNO_IS_PRIVILEGE(r) ? LOG_DEBUG : LOG_ERR, r,
...
success:
        ...
        if (sxa.stx_dev_major == 0) { /* Hmm, maybe a btrfs device, and the caller asked for the backing device? Then let's try to get it. */
                ...
                r = btrfs_get_block_device_fd(real_fd, ret_dev);
                if (r < 0)
                        return r;
        } else
