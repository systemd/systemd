...
static int verify_fsroot_dir(
        ...
        assert(path);
        assert(isempty(path) || filename_is_valid(path));

        r = statx_fallback(dir_fd, path, AT_SYMLINK_NOFOLLOW|(isempty(path) ? AT_EMPTY_PATH : 0),
...
success:
        ...
        if (sxa.sx.stx_dev_major == 0) /* Hmm, maybe a btrfs device, and the caller asked for the backing device? Then let's try to get it. */
                return btrfs_get_block_device_at(dir_fd, path, ret_dev);
...
static int verify_esp(
        ...
        if (!relax_checks) {
                ...
                if (xstatfsat(pfd, strempty(f), &sfs) < 0)
        ...
        r = verify_fsroot_dir(pfd, strempty(f), searching, unprivileged_mode, relax_checks ? NULL : &devid);
        if (r < 0)
                return r;
...
int find_esp_and_warn(
        ...
        r = find_esp_and_warn_at(rfd, path, unprivileged_mode, ret_path ? &p : NULL, ret_part,
...
int find_xbootldr_and_warn(
        ...
        r = find_xbootldr_and_warn_at(rfd, path, unprivileged_mode, ret_path ? &p : NULL, ret_uuid, ret_devid);
        if (r < 0)
                return r;
