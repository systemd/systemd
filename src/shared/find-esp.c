...
int find_esp_and_xbootldr_paths_and_warn(
        ...
        r = find_esp_and_warn(root, esp_path, unprivileged_mode, &esp, /* ret_part= */ NULL, /* ret_pstart= */ NULL, /* ret_psize= */ NULL, ret_esp_uuid, &esp_devid);
        if (r < 0 && r != -ENOKEY) /* ENOKEY means not found */
                return r;

        r = find_xbootldr_and_warn(root, xbootldr_path, unprivileged_mode, &xbootldr, ret_xbootldr_uuid, &xbootldr_devid);
        if (r < 0 && r != -ENOKEY)
                return r;
