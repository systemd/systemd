...
int keymap_directories(char ***ret) {
        ...
        e = getenv("SYSTEMD_KEYMAP_DIRECTORIES");
        if (!e)
                goto use_default;

        r = strv_split_full(&paths, e, NULL, EXTRACT_UNQUOTE);
        if (r == 0) /* empty string? */
                goto use_default;
        if (r < 0) {
                log_debug_errno(r, "Failed to parse $SYSTEMD_KEYMAP_DIRECTORIES environment variable, ignoring: %m");
                goto use_default;
        }

        STRV_FOREACH(p, paths) {
                if (!path_is_absolute(*p)) {
                        log_debug("Path specified in $SYSTEMD_KEYMAP_DIRECTORIES environment variable is not absolute, ignoring: %m");
                        goto use_default;
                }
                if (!path_is_normalized(*p)) {
                        log_debug("Path specified in $SYSTEMD_KEYMAP_DIRECTORIES environment variable is not normalized, ignoring: %m");
                        goto use_default;
                }
        }
...
use_default:
        ...
        paths = strv_new(KBD_KEYMAP_DIRS);
        if (!paths)
