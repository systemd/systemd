...
static int pick_result_compare(const PickResult *a, const PickResult *b, PickFlags flags) {
        int d;
        ...
        if (FLAGS_SET(flags, PICK_TRIES)) {
                ...
                /* Fourth, prefer entries with fewer attempts done so far */
                if (d == 0)
...
static int make_choice(
        ...
        FOREACH_ARRAY(entry, de->entries, de->n_entries) {
                ...
                _cleanup_close_ int object_fd = openat(dir_fd, (*entry)->d_name, O_CLOEXEC|O_PATH);
                if (object_fd < 0)
                        return log_debug_errno(errno, "Failed to open '%s/%s': %m",
                                               empty_to_root(toplevel_path), skip_leading_slash(p));

                struct stat st;
                if (fstat(object_fd, &st) < 0)
                        return log_debug_errno(errno, "Failed to stat '%s/%s': %m",
                                               empty_to_root(toplevel_path), skip_leading_slash(p));

                if (filter->type_mask != 0 && !BIT_SET(filter->type_mask, IFTODT(st.st_mode))) {
                        log_debug("Found entry '%s' with wrong type '%s', ignoring entry.",
                                  (*entry)->d_name, inode_type_to_string(st.st_mode));
                        continue;
                }

                _cleanup_(pick_result_done) PickResult found = {
                        .path = TAKE_PTR(p),
                        .fd = TAKE_FD(object_fd),
                        .st = st,
                        .version = strdup(e),
                        .architecture = a,
                        .tries_left = found_tries_left,
                        .tries_done = found_tries_done,
                };
                if (!found.version)
                        return -ENOMEM;
