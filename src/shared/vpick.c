        ...
        if (search_mode != MODE_INVALID || ret_inode_mode) {
                ...
                if (search_mode != MODE_INVALID &&
                    ((search_mode ^ st.st_mode) & S_IFMT) != 0)
                        return log_debug_errno(SYNTHETIC_ERRNO(S_ISDIR(search_mode) ? -ENOTDIR :
                                                               S_ISDIR(st.st_mode) ? -EISDIR : -EBADFD),
                                               "Inode '%s/%s' has wrong type.", toplevel_path, inode_path);
        }
...
static int make_choice(
        ...
        size_t n_archs, best_arch_index = SIZE_MAX;
        int r;
        ...
        if (search_version && search_architecture >= 0) {
                ...
                r = chase_symlinks_at(toplevel_fd, p, CHASE_AT_RESOLVE_IN_ROOT, &object, &object_fd);
                if (r < 0) {
                        if (r != -ENOENT)
        ...
        if (search_architecture < 0) {
                archs = local_architectures;
                n_archs = ELEMENTSOF(local_architectures);
        } else {
                archs = &search_architecture;
                n_archs = 1;
        }

        for (size_t i = 0; i < de->n_entries; i++) {
                _cleanup_free_ char *chopped = NULL, *chopped2 = NULL;
                size_t j, ai = SIZE_MAX;
                const char *e;

                if (!isempty(search_basename)) {
                        e = startswith(de->entries[i]->d_name, search_basename);
                        if (!e)
                                continue;
                ...
                for (j = 0; j < n_archs; j++) {
                        ...
                        if (archs[j] >= 0) {
                                ...
                                as = ASSERT_PTR(architecture_to_string(archs[j]));
                                ...
                                pa = strrchr(e, '_');
                                if (pa && architecture_from_string(pa+1) >= 0)
                                        continue;
                        }
                ...
                if (j >= n_archs) /* no matching arch found */
                        continue;

                if (!version_is_valid(e)) {
                        log_debug("Version string '%s' of entry '%s' is invalid, ignoring entry.", e, de->entries[i]->d_name);
                        continue;
                }
                ...
                r = free_and_strdup_warn(&best_filename, de->entries[i]->d_name);
                if (r < 0)
                        return r;
        ...
        search_architecture = archs[best_arch_index];
...
int path_pick(const char *path,
        ...
        /* Given a path, resolve .v/ subdir logic (if used!), and returns the choice made. This supports three ways to be called:
         *
         * • with path referring to any kind of directory, but search_basename/search_suffix explicitly specified with
         *   suitable basename + suffix for files to search for inside.
         *
                                ...
                                if (e) {
                                        ...
                                        if (slash_suffix) {
                                                ...
                                                if (search_mode == MODE_INVALID)
                                                        ...
                                                        return log_debug_errno(SYNTHETIC_ERRNO(ENOTDIR),
        ...
        if (pattern) {
                ...
                if (wildcard) {
                        search_suffix = strempty(wildcard + 3);
                        *wildcard = 0;
                }
...
ready:
        return func(/* toplevel_path= */ NULL,
...
int path_pick_update_warn(
        ...
        r = path_pick(*path,
                      ...
                      &a);
        if (r == ENOENT) {
                log_debug_errno(r, "Path '%s' doesn't exist, leaving as is.", *path);
