...
void pick_result_done(PickResult *p) {
        ...
        p->st = (const struct stat) {
                .st_mode = MODE_INVALID,
        };
        p->version = mfree(p->version);
        p->architecture = _ARCHITECTURE_INVALID;
        p->tries_left = UINT_MAX;
        p->tries_done = UINT_MAX;
}
...
static int pin_choice(
        ...
        if (filter->mode != MODE_INVALID &&
            ((filter->mode ^ st.st_mode) & S_IFMT) != 0)
                return log_debug_errno(
                                SYNTHETIC_ERRNO(errno_from_mode(filter->mode, st.st_mode)),
                                "Inode '%s/%s' has wrong type, was looking for '%s', got '%s'.",
                                toplevel_path, inode_path,
                                inode_type_to_string(filter->mode), inode_type_to_string(st.st_mode));
...
static int make_choice(
        ...
        _cleanup_free_ char *best_version = NULL, *best_filename = NULL, *p = NULL, *object = NULL, *j = NULL;
        ...
        if (r >= 0) {
                ...
                r = chaseat(toplevel_fd, p, CHASE_AT_RESOLVE_IN_ROOT, /* ret_path= */ NULL, &object_fd);
                if (r < 0) {
                        if (r != -ENOENT)
...
int path_pick(const char *toplevel_path,
         ...
         *
         * • with path referring a directory of any name, and filter_basename explicitly specified, in which
         *   case we'll use for files "<filter_basename>_*<filter_suffix>".
         *
         * • with no filter_basename explicit specified and a path referring to a directory named in format
         *   "<somestring><filter_suffix>.v" . In this case the filter basename to search for inside the dir
         *   is derived from the directory name. Example: "/foo/bar/baz.suffix.v" → we'll search for
         *   "/foo/bar/baz.v/baz_*.suffix".
         *
         * • with path whose penultimate component ends in .v/. In this case the last component of the path
         *   refers to the pattern. Example: "/foo/bar/baz.v/waldo__.suffix" → we'll search for
         *   "/foo/bar/baz.v/waldo*.suffix".
         */
        ...
        if (e) {
                ...
                *wildcard = 0;
                ...
                if (slash_suffix) {
                        ...
                        if (filter_mode != MODE_INVALID && !S_ISDIR(filter_mode))
                                return log_debug_errno(SYNTHETIC_ERRNO(errno_from_mode(filter_mode, S_IFDIR)),
