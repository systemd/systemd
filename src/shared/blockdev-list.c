...
int blockdev_list(BlockDevListFlags flags) {
        ...
        FOREACH_DEVICE(e, dev) {
                ...
                if (r < 0) {
                        log_warning_errno(r, "Failed to get device node of discovered block device, ignoring.");
                        continue;
                }
                ...
                if (FLAGS_SET(flags, BLOCKDEV_LIST_IGNORE_ZRAM)) {
                        ...
                        if (r < 0) {
                                log_warning_errno(r, "Failed to get device name of discovered block device, ignoring.");
                                continue;
                        }
                ...
                if (FLAGS_SET(flags, BLOCKDEV_LIST_REQUIRE_PARTITION_SCANNING)) {
                        ...
                        if (r < 0) {
                                log_warning_errno(r, "Unable to determine whether %s supports partition scanning, skipping device.", node);
                                continue;
                        }
                        if (r == 0) {
