...
int seccomp_suppress_sync(void) {
        ...
        /* This is mostly identical to SystemCallFilter=~@sync:0, but simpler to use, and seperately manageable. */
        ...
        SECCOMP_FOREACH_LOCAL_ARCH(arch) {
                ...
                NULSTR_FOREACH(c, syscall_filter_sets[SYSCALL_FILTER_SET_SYNC].value) {
                        ...
                        r = seccomp_rule_add_exact(
                                        ...
                                        0);
                        if (r < 0)
                                log_debug_errno(r, "Failed to add filter for system call %s: %m", c);
                }
