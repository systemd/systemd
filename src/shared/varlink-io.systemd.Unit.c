...
static SD_VARLINK_DEFINE_METHOD_FULL(
                EnqueueJob,
                SD_VARLINK_SUPPORTS_MORE,
                SD_VARLINK_FIELD_COMMENT("The unit names to operate on. All jobs are enqueued in a single transaction, i.e. either all of them are enqueued or none. If more than one unit name is passed in, one reply per enqueued job is sent, and the 'more' flag must be set. Since jobs are merged the replies do not necessarily correspond one-to-one to the requested unit names, correlate them via the unit name reported in each job."),
                ...
                SD_VARLINK_FIELD_COMMENT("If true, and the job type is restart or try-restart, enqueue a reload job instead for units that can be reloaded. Defaults to false."),
                SD_VARLINK_DEFINE_INPUT(reloadIfPossible, SD_VARLINK_BOOL, SD_VARLINK_NULLABLE),
                SD_VARLINK_FIELD_COMMENT("Job mode. Defaults to replace."),
                SD_VARLINK_DEFINE_INPUT_BY_TYPE(mode, JobMode, SD_VARLINK_NULLABLE),
                SD_VARLINK_FIELD_COMMENT("If true, stream job state change notifications. Defaults to false. Requires the 'more' flag, and is currently only supported if a single unit name is passed in."),
                SD_VARLINK_DEFINE_INPUT(notifyJobChanges, SD_VARLINK_BOOL, SD_VARLINK_NULLABLE),
                SD_VARLINK_FIELD_COMMENT("If true, stream unit runtime notifications on state changes. Defaults to false. Requires the 'more' flag, and is currently only supported if a single unit name is passed in."),
