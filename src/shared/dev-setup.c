...
int dev_setup(const char *prefix, uid_t uid, gid_t gid) {
        ...
        NULSTR_FOREACH_PAIR(j, k, symlinks) {
                ...
                r = symlink_label(j, n, /* label_userdata= */ NULL);
                if (r < 0)
...
int make_inaccessible_nodes(
        ...
        FOREACH_ELEMENT(m, table) {
                ...
                if (S_ISDIR(inode_type))
                        r = mkdirat_label(inaccessible_fd, fn, 0000, /* label_userdata= */ NULL);
                else
                        r = mknodat_label(inaccessible_fd, fn, inode_type | 0000, makedev(0, 0), /* label_userdata= */ NULL);
                if (r == -EEXIST) {
