...
static int selinux_fix_fd(
        ...
        if (sym_selabel_lookup_raw(hnd, &fcon, lookup_path, st.st_mode) < 0) {
                ...
                if (c)
                        return log_debug_errno(errno, "Unable to lookup intended SELinux security context of %s (looked up as %s): %m", label_path, lookup_path);
...
static int selinux_create_file_prepare_context(int dir_fd, const char *path, mode_t mode, LabelContext *c) {
        ...
        if (r < 0) {
                ...
                return log_debug_errno(errno, "Failed to determine SELinux security context for %s (looked up as %s): %m", path, lookup_path);
        }
...
int mac_selinux_label_context_new(const char *root, LabelContext **ret) {
        ...
        _cleanup_free_ char *canonical_root = NULL;
        r = fd_get_path(rfd, &canonical_root);
        if (r < 0) {
                sym_selabel_close(hnd);
                return log_debug_errno(r, "Failed to get canonical path for root '%s': %m", root);
        }
