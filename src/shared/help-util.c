 ...
 *      mutiple times.) This generally comes first in the output.
 *
 *   2. help_abstract() outputs a brief prose abstract of the command, should carry a single line of text
 *      that gives the user a hint what this tools does. Use only once, right after the last help_cmdline().
 *
...
void help_man_page_reference(const char *page, const char *section) {
        ...
        /* Display --help texts should not fail, hence let's fall back to a simple string in case of OOM. */
