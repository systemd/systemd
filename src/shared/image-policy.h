...
typedef enum PartitionPolicyFlags {
         ...
         * /home/), and so on. */
        PARTITION_POLICY_VERITY               = 1 << 0, /* must exist, use with verity                 (only applies to root/usr partitions) */
        PARTITION_POLICY_SIGNED               = 1 << 1, /* must exist, use with signed verity          (only applies to root/usr partitions) */
        PARTITION_POLICY_ENCRYPTED            = 1 << 2, /* must exist, use with LUKS encryption        (applies to any data partition, but not to verity/signature partitions */
