...
int dns_packet_append_name(
        ...
        while (!dns_name_is_root(name)) {
                ...
                /* Defensive: only pointer-expressible offsets are inserted below; should that ever
                 * break, fall through to plain labels (always valid) instead of a truncated pointer. */
                if (n > 0 && n <= DNS_PACKET_COMPRESSION_OFFSET_MAX) {
                        assert(n < p->size);

                        r = dns_packet_append_uint16(p, DNS_PACKET_COMPRESSION_POINTER_FLAG | n, NULL);
                        if (r < 0)
                                goto fail;
...
int dns_packet_read_name(
        ...
        for (;;) {
                ...
                if (c == 0)
                        ...
                        continue;
                } else if (allow_compression && FLAGS_SET(c, DNS_PACKET_COMPRESSION_POINTER_FLAG >> 8)) {
                        uint16_t ptr;
                        ...
                        ptr = (uint16_t) (c & ~(DNS_PACKET_COMPRESSION_POINTER_FLAG >> 8)) << 8;
                        ptr |= (uint16_t) d;
