...
/* Useful for using with _cleanup_(), destroys a directory on temporary file system. */
static inline const char* rm_rf_tmpfs(const char *p) {
        PROTECT_ERRNO;
...
DEFINE_TRIVIAL_CLEANUP_FUNC(const char*, rm_rf_tmpfs);
