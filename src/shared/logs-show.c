...
static int source_get_base_url(char **ret) {
        ...
        u = strndup(e, strcspn(e, WHITESPACE)); /* cut out first URL, according to debuginfod(8) separator is spaces */
        if (!u)
                return log_oom();
        ...
        if (urlify_enabled()) {
                ...
                if (build_id && shall_print_refuse_nul(build_id, build_id_len, flags) &&
                    ...
                    code_line && shall_print_refuse_nul(code_line, code_line_len, flags)) {
                        _cleanup_free_ char *source_url = NULL, *base_url = NULL, *urlified = NULL, *normalized = NULL, *text = NULL;
                        const char *z;
                        ...
                        text = strjoin("[", special_glyph(SPECIAL_GLYPH_LIGHT_BULB), "]");
                        if (!text)
                                return log_oom();

                        if (terminal_urlify(source_url, text, &urlified) >= 0) {
                                fputs(urlified, f);
                                fputc(' ', f);
                        }
                }
