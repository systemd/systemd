...
static int output_timestamp_monotonic(FILE *f, sd_journal *j, OutputMode mode, const char *monotonic) {
        ...
        snprintf(buf, sizeof(buf), "[%5"PRI_USEC".%06"PRI_USEC"", t / USEC_PER_SEC, t % USEC_PER_SEC);
        ...
        if (mode == OUTPUT_SHORT_DELTA) {
                ...
                if (r <= 0)
                        ...
                        r = sd_journal_next(j);
                        /* This should actually always succeed because we are coming from this state but to be sure ... */
                        if (r < 0)
                                return r;
                        else if (r == 0)
                        ...
                        if (sd_id128_equal(boot_id, previous_boot_id))
                                delta = usec_sub_unsigned(t, previous_t);
                        else
                                delta = 0;
                }
