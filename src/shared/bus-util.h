...
typedef int (*bus_property_set_t)(
...
{
        BUS_MAP_STRDUP = 1
                << 0, /* If set, each "s" message is duplicated. Thus, each pointer needs to be freed. */
        BUS_MAP_BOOLEAN_AS_BOOL = 1 << 1, /* If set, each "b" message is written to a bool pointer. If not
...
int bus_event_loop_with_idle(
        sd_event *e, sd_bus *bus, const char *name, usec_t timeout, check_idle_t check_idle, void *userdata);
...
int bus_print_all_properties(sd_bus *bus,
                             const char *dest,
                             const char *path,
                             bus_message_print_t func,
                             char **filter,
...
int bus_property_get_bool(sd_bus *bus,
                          const char *path,
                          ...
                          void *userdata,
                          sd_bus_error *error);
int bus_property_set_bool(sd_bus *bus,
                          const char *path,
                          const char *interface,
                          const char *property,
                          ...
                          void *userdata,
                          sd_bus_error *error);
int bus_property_get_id128(sd_bus *bus,
                           const char *path,
                           const char *interface,
                           const char *property,
                           sd_bus_message *reply,
...
#else
int bus_property_get_size(sd_bus *bus,
                          const char *path,
                          const char *interface,
                          const char *property,
                          sd_bus_message *reply,
...
#else
int bus_property_get_long(sd_bus *bus,
                          const char *path,
                          const char *interface,
                          const char *property,
                          sd_bus_message *reply,
                          void *userdata,
                          sd_bus_error *error);
int bus_property_get_ulong(sd_bus *bus,
                           const char *path,
                           const char *interface,
                           const char *property,
                           sd_bus_message *reply,
...
#define BUS_PROPERTY_DUAL_TIMESTAMP(name, offset, flags)                               \
        SD_BUS_PROPERTY(name,                                                          \
                        ...
                        (flags)),                                                      \
                SD_BUS_PROPERTY(name "Monotonic",                                      \
                                "t",                                                   \
                                bus_property_get_usec,                                 \
                                (offset) + offsetof(struct dual_timestamp, monotonic), \
                                (flags))

int bus_path_encode_unique(
        sd_bus *b, const char *prefix, const char *sender_id, const char *external_id, char **ret_path);
int bus_path_decode_unique(const char *path, const char *prefix, char **ret_sender, char **ret_external);

int bus_property_get_rlimit(sd_bus *bus,
                            const char *path,
                            const char *interface,
                            const char *property,
                            sd_bus_message *reply,
