...
static int do_crypt_activate_verity(
        ...
        if (check_signature) {
                ...
                if (k == 0) {
                        ...
                        /* So if we had a signature and where supposed to to exclusively allow signature-based activation, then return the error now */
                        if (!FLAGS_SET(policy_flags, PARTITION_POLICY_VERITY))
                                return r < 0 ? r : -EDESTADDRREQ;

                        log_debug("Activation without signature is permitted by policy. Continuing.");
                } else
                        log_debug("Signature check in userspace worked, activating by root hash.");
