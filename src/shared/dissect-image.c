...
static int dissect_image(
        ...
        if ((flags & DISSECT_IMAGE_NO_BLKID)) {
                ...
                if (!(verity && verity->data_path &&
                                ...
                                (root_fstype & _PARTITION_POLICY_FSTYPE_SET_MASK) &&
                                ((root_fstype - 1) & root_fstype) == 0  && /* It's a power of 2 -> only a single bit is set -> single fstype pinning */
        ...
        if ((!(flags & DISSECT_IMAGE_GPT_ONLY) &&
            ...
            (flags & DISSECT_IMAGE_NO_PARTITION_TABLE)) {
                ...
                if (STRPTR_IN_SET(usage, "filesystem", "crypto")) {
                        ...
                        if ((flags & DISSECT_IMAGE_NO_BLKID)) {
                                mount_node_fd = open(devname, O_RDONLY|O_NONBLOCK|O_CLOEXEC|O_NOCTTY);
                                if (mount_node_fd < 0)
                                        return -errno;
                        } else if (FLAGS_SET(flags, DISSECT_IMAGE_PIN_PARTITION_DEVICES)) {
