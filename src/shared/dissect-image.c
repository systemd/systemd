...
static int mount_partition(
        ...
        if (p) {
                if (m->fsmount_fd >= 0) {
                        bool mount_beneath = FLAGS_SET(flags, DISSECT_IMAGE_MOUNT_REPLACE);
                         ...
                         * this works also in the case where there are submounts down the tree, as long as
                         * there's no propagation. If this is not supported (minimum kernel v6.5), or if
                         * there is no mount on the mountpoint, we get -EINVAl and then we fallback to
                         * normal mounting. */
