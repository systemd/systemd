...
#define LUKS2_FIXED_HDR_SIZE 0x1000UL
...
static int integrity_information(const char *name, sd_json_variant *v, sd_json_dispatch_flags_t flags, void *userdata) {
        static const sd_json_dispatch_field table[] = {
                { "type",     SD_JSON_VARIANT_STRING, sd_json_dispatch_string, offsetof(struct luks_integrity_data, type), SD_JSON_MANDATORY },
                {}
        };
...
static int partition_is_luks2_integrity(int part_fd, uint64_t offset, uint64_t size) {
        ...
        if (be64toh(header.hdr_len) <= LUKS2_FIXED_HDR_SIZE)
        ...
        if (size < LUKS2_FIXED_HDR_SIZE + json_len)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Partition is too small to contain LUKS JSON header.");
        ...
        JSON_VARIANT_OBJECT_FOREACH(key, w, v) {
                ...
                if (!sd_json_variant_is_object(w)) {
                        log_debug("Segment %s information is bogus.", key);
                        return 0;
                }
