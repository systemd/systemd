...
static int validate_signature_userspace(const VeritySettings *verity) {
        ...
        r = conf_files_list(&certs, ".crt", NULL, CONF_FILES_REGULAR|CONF_FILES_FILTER_MASKED, CONF_PATHS_NULSTR("verity.d"));
        if (r < 0)
        ...
        return r;
#else
        log_debug("Not doing client-side certificate validation, OpenSSL support disabled.");
        return 0;
#endif
}

static int do_crypt_activate(
        ...
        check_signature = !!verity->root_hash_sig;
        if (check_signature) {
        ...
        if (check_signature) {
                ...
                check_signature = !r; /* If userspace validation worked, don't let kernel verify a second time */
        }
...
#if HAVE_CRYPT_ACTIVATE_BY_SIGNED_KEY
        return sym_crypt_activate_by_signed_key(
                        cd,
                        name,
                        verity->root_hash,
                        verity->root_hash_size,
...
static int verity_partition(
        ...
        for (unsigned i = 0; i < N_DEVICE_NODE_LIST_ATTEMPTS; i++) {
                ...
                r = do_crypt_activate(cd, name, verity);
...
int dissected_image_load_verity_sig_partition(
        ...
        if (verity->root_hash &&
            memcmp_nn(verity->root_hash, verity->root_hash_size, root_hash, root_hash_size) != 0)
                return log_debug_errno(r, "Root hash of signature JSON data doesn't match configured hash.");
