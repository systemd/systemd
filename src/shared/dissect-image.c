...
int dissect_image(
        ...
        *m = (DissectedImage) {
        ...
        for (int i = 0; i < n_partitions; i++) {
                ...
                if (is_gpt) {
                        ...
                        if (sd_id128_equal(type_id, GPT_HOME)) {
                                ...
                                assert((architecture = gpt_partition_type_uuid_to_arch(type_id)) >= 0);
        ...
        if (m->partitions[PARTITION_ROOT].found) {
                ...
                log_debug(
                        "No root partition found of the native architecture (%s), falling back to a root "
                        "partition of the secondary architecture (%s).",
                        architecture_to_string(native_architecture()),
                        architecture_to_string(ARCHITECTURE_SECONDARY));
                ...
                log_debug(
                        "No root partition found of the native architecture (%s) or the secondary "
                        "architecture (%s), falling back to a root partition of a non-native architecture (%s).",
                        architecture_to_string(native_architecture()),
                        architecture_to_string(ARCHITECTURE_SECONDARY),
        ...
        if (m->partitions[PARTITION_USR].found) {
                ...
                log_debug(
                        "No usr partition found of the native architecture (%s), falling back to a usr "
                        "partition of the secondary architecture (%s).",
                        architecture_to_string(native_architecture()),
                        architecture_to_string(ARCHITECTURE_SECONDARY));
                ...
                log_debug(
                        "No usr partition found of the native architecture (%s) or the secondary "
                        "architecture (%s), falling back to a usr partition of a non-native architecture (%s).",
                        architecture_to_string(native_architecture()),
                        architecture_to_string(ARCHITECTURE_SECONDARY),
                        architecture_to_string(m->partitions[PARTITION_ROOT_OTHER].architecture));
