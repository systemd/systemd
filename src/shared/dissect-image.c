...
static int device_monitor_handler(sd_device_monitor *monitor, sd_device *device, void *userdata) {
        ...
        if (w->diskseq != 0) {
                ...
                if (r < 0) {
                        log_debug("Dropping event because it has no diskseq, but waiting for %" PRIu64, w->diskseq);
                        return 0;
                }
...
static int wait_for_partition_device(
        ...
        /* If we have a disk sequence number we can do exact matching, and
         * given they are monotonic we know if we missed it or if we can still
         * find it. */
        if (diskseq == 0) {
