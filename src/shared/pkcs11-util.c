...
success:
         ...
         * we look it up by CKA_ID. The result is only used for RSA keys. */
        ...
        if (prototype == CK_INVALID_HANDLE)
                log_debug("No public key or certificate found on token to locate matching private key, defaulting to OAEP-SHA1.");
                ...
                log_debug("No matching PKCS#11 private key found for OAEP padding probe, defaulting to OAEP-SHA1.");
...
int pkcs11_acquire_public_key(
        ...
        _cleanup_(pkcs11_acquire_public_key_callback_data_release) struct pkcs11_acquire_public_key_callback_data data = {
                ...
                .rsa_padding = PKCS11_RSA_PADDING_OAEP_SHA1,
        };
        int r;
