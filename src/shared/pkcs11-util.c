...
static int pkcs11_token_ecdh_derive_shared_secret(
        ...
        if (rv != CKR_OK)
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to derive shared secret: %s", sym_p11_kit_strerror(rv));
        ...
        if (rv != CKR_OK) {
                pkcs11_token_destroy_session_object(m, session, shared_secret_handle);
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to retrieve shared secret: %s", sym_p11_kit_strerror(rv));
        }
