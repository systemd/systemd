...
int pkcs11_token_probe_rsa_oaep_padding(
                CK_FUNCTION_LIST *m,
                CK_SESSION_HANDLE session,
                CK_OBJECT_HANDLE private_key,
        ...
        /* The OAEP padding probe is only meaningful for RSA private keys; on EC keys C_DecryptInit with
         * CKM_RSA_PKCS_OAEP would fail unconditionally and produce a misleading log message. Skip cleanly
         * by reporting the padding as not applicable. */
        ...
        if (key_type != CKK_RSA) {
                *ret = _PKCS11_RSA_PADDING_INVALID;
                return 0;
        }

        /* For RSA-OAEP we use SHA-256 or SHA-1 with the matching MGF1 and an empty (zero-length) label.
         * SHA-256 is preferred when the token supports it, but for example SoftHSM doesn't support it, so
         * fall back to SHA-1. If neither is accepted the token doesn't support OAEP at all and we have to
         * fail enrollment up-front rather than producing a cryptic error during key wrapping later. */
...
success:
        ...
        /* Probe the token's private key to determine which OAEP hash variant we may use for encryption at
         * enrollment time. Try OAEP-SHA256 first and fall back to OAEP-SHA1 if the token rejects it. The
         * probe needs the matching private key handle (CKA_DECRYPT cannot be set on public keys), so we
         * look it up by CKA_ID. The probe is a no-op (and leaves the result as _INVALID) for non-RSA
         * keys. If we cannot locate the matching private key (e.g. token holds only the certificate while
         * the private key lives offline) default to SHA256. */
        ...
        if (pkcs11_token_find_related_object(m, session, prototype, CKO_PRIVATE_KEY, &private_key) >= 0) {
                r = pkcs11_token_probe_rsa_oaep_padding(m, session, private_key, &data->rsa_padding);
                if (r < 0)
                        return r;
        } else if (IN_SET(sym_EVP_PKEY_get_id(pkey), EVP_PKEY_RSA, EVP_PKEY_RSA_PSS)) {
                log_debug("No matching PKCS#11 private key found, OAEP padding probe skipped, defaulting RSA enrollment to OAEP-SHA256.");
                data->rsa_padding = PKCS11_RSA_PADDING_OAEP_SHA256;
        } else
                log_debug("No matching PKCS#11 private key found, OAEP padding probe skipped.");
