...
int pkcs11_token_find_related_object(
        ...
        if (attributes[0].ulValueLen != CK_UNAVAILABLE_INFORMATION) {
                ...
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "The prototype does not have CKA_ID or CKA_LABEL");
        }
        ...
        if (rv != CKR_OK)
         ...
         if (n_objects > 1)
                log_warning("Found multiple related objects with the same CKA_CLASS");
