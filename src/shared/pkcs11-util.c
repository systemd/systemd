...
int pkcs11_token_probe_rsa_oaep_padding(
        ...
        *ret = PKCS11_RSA_PADDING_OAEP_SHA1;
        ...
        if (rv == CKR_OK) {
                /* sha256 accepted, cancel the operation. C_Decrypt with a zero-byte input buffer is enough
                 * to reset the session's active operation. */
                CK_BYTE dummy[1];
                CK_ULONG dummy_len = sizeof(dummy);
                (void) m->C_Decrypt(session, dummy, 0, dummy, &dummy_len);
...
success:
        ...
        if (prototype != CK_INVALID_HANDLE &&
                pkcs11_token_find_related_object(m, session, prototype, CKO_PRIVATE_KEY, &private_key) >= 0)
