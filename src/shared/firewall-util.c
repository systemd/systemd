...
static void add_bitwise(struct nft_rule *r, int reg, const void *mask, size_t len) {
        struct nft_rule_expr *expr;
        uint8_t *xor;
...
int fw_add_masquerade(
        ...
        add_cmp(rule, NFT_REG_1, NFT_CMP_EQ, &protocol, sizeof(protocol));
        ...
        if (source) {
                ...
                add_bitwise(rule, NFT_REG_1, &smsk.s_addr, sizeof(struct in_addr));
                add_cmp(rule, NFT_REG_1, NFT_CMP_EQ, &source->in.s_addr, sizeof(struct in_addr));
        }
        ...
        if (destination) {
                ...
                add_bitwise(rule, NFT_REG_1, &dmsk.s_addr, sizeof(struct in_addr));
                add_cmp(rule, NFT_REG_1, NFT_CMP_EQ, &destination->in.s_addr, sizeof(struct in_addr));
        }
        ...
        if (out_interface) {
                ...
                add_cmp(rule, NFT_REG_1, NFT_CMP_EQ, out_interface, strlen(out_interface) + 1);
        }
...
int fw_add_local_dnat(
        ...
        if (in_interface) {
                ...
                nft_rule_expr_set_u32(expr, NFT_EXPR_META_DREG, NFT_REG_1);
                nft_rule_add_expr(rule, expr);
                add_cmp(rule, NFT_REG_1, NFT_CMP_EQ, in_interface, strlen(in_interface) + 1);
        }
        ...
        if (local_port) {
                ...
                add_cmp(rule, NFT_REG_1, NFT_CMP_EQ, &local_port, sizeof(local_port));
        }
        ...
        if (remote) {
                ...
                add_imm(rule, NFT_REG_1, &remote->in.s_addr, sizeof(remote->in.s_addr));
        }
        ...
        if (remote_port) {
                ...
                add_imm(rule, NFT_REG_2, &remote_port, sizeof(remote_port));
        }
