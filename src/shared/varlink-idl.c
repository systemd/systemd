...
static int varlink_idl_format_all_fields(FILE *f, const VarlinkSymbol *symbol, int direction, const char *indent, const char *const colors[static _COLOR_MAX]);
...
static int varlink_idl_format_field(
        ...
        case VARLINK_STRUCT:
                return varlink_idl_format_all_fields(f, ASSERT_PTR(field->symbol), /* direction= */ 0, indent, colors);
...
static int varlink_idl_format_all_fields(
        ...
        for (const VarlinkField *field = symbol->fields; field->field_type != _VARLINK_FIELD_TYPE_END_MARKER; field++) {
                ...
                if (field->direction != filter_direction)
                        continue;
...
static int varlink_idl_format_symbol(
        ...
        case VARLINK_STRUCT_TYPE:
                ...
                r = varlink_idl_format_all_fields(f, symbol, /* direction= */ 0, /* indent= */ NULL, colors);
                break;
        ...
        case VARLINK_METHOD:
                ...
                r = varlink_idl_format_all_fields(f, symbol, /* direction= */ -1, /* indent= */ NULL, colors);
                if (r < 0)
                        return r;
...
int varlink_idl_format(const VarlinkInterface *interface, char **ret) {
        ...
        r = varlink_idl_dump(memstream.f, /* colors= */ false, interface);
        if (r < 0)
                return r;
...
static int varlink_idl_subparse_token(
        ...
        if (l == 0) {
                ...
                *p += ll;
                ...
                l = token_match(*p, allowed_delimiters, allowed_chars);
                if (l == 0)
                        return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG), "Couldn't find token of allowed chars '%s' or allowed delimiters '%s'.", allowed_chars, allowed_delimiters);
        }
...
static int varlink_idl_subparse_struct_or_enum(const char **p, unsigned *line, unsigned *column, VarlinkSymbol **symbol, size_t *n_fields, int direction);
...
int varlink_idl_parse(
        ...
        while (state != STATE_DONE) {
                ...
                case STATE_METHOD:
                        ...
                        r = varlink_idl_subparse_struct_or_enum(&text, line, column, &symbol, &n_fields, /* direction= */ -1);
                        if (r < 0)
                                return r;
                ...
                case STATE_TYPE:
                        ...
                        r = varlink_idl_subparse_struct_or_enum(&text, line, column, &symbol, &n_fields, /* direction= */ 0);
                        if (r < 0)
                                return r;
...
static int varlink_idl_field_consistent(
        ...
        symbol_name = symbol->name ?: "<anomyous>";
        ...
        if (symbol->symbol_type != VARLINK_METHOD) {
                if (field->direction != 0)
                        return log_full_errno(level, SYNTHETIC_ERRNO(EUCLEAN), "Direction of '%s' in non-method symbol '%s' not zero, refusing.", field->name, symbol_name);
        } else {
                if (field->direction == 0)
                        return log_full_errno(level, SYNTHETIC_ERRNO(EUCLEAN), "Direction of '%s' in method symbol '%s' is zero, refusing.", field->name, symbol_name);
        }
...
static int varlink_idl_symbol_consistent(
        ...
        for (const VarlinkField *field = symbol->fields; field->field_type != _VARLINK_FIELD_TYPE_END_MARKER; field++) {
                Set **name_set = field->direction > 0 ? &output_set : &input_set; /* for the method case we need two separate sets, otherwise we use the same */
...
static int varlink_idl_validate_symbol(const VarlinkSymbol *symbol, JsonVariant *v, int direction, const char **bad_field);
...
static int varlink_idl_validate_field_element_type(const VarlinkField *field, JsonVariant *v) {
        ...
        case VARLINK_ENUM:
        case VARLINK_NAMED_TYPE:
                return varlink_idl_validate_symbol(field->symbol, v, /* direction= */ 0, NULL);

        case VARLINK_BOOL:
                if (!json_variant_is_boolean(v))
                        return log_debug_errno(SYNTHETIC_ERRNO(EMEDIUMTYPE), "Field '%s' should be a bool, bus it not, refusing.", strna(field->name));

                break;

        case VARLINK_INT:
                if (!json_variant_is_integer(v) && !json_variant_is_unsigned(v))
                        return log_debug_errno(SYNTHETIC_ERRNO(EMEDIUMTYPE), "Field '%s' should be an int, bus it not, refusing.", strna(field->name));

                break;

        case VARLINK_FLOAT:
                if (!json_variant_is_number(v))
                        return log_debug_errno(SYNTHETIC_ERRNO(EMEDIUMTYPE), "Field '%s' should be a float, bus it not, refusing.", strna(field->name));

                break;

        case VARLINK_STRING:
                if (!json_variant_is_string(v))
                        return log_debug_errno(SYNTHETIC_ERRNO(EMEDIUMTYPE), "Field '%s' should be a string, bus it not, refusing.", strna(field->name));

                break;

        case VARLINK_OBJECT:
                if (!json_variant_is_object(v))
                        return log_debug_errno(SYNTHETIC_ERRNO(EMEDIUMTYPE), "Field '%s' should be an object, bus it not, refusing.", strna(field->name));
...
static int varlink_idl_validate_field(const VarlinkField *field, JsonVariant *v) {
        ...
        if (!v || json_variant_is_null(v)) {
                ...
                if (!json_variant_is_array(v))
                        return log_debug_errno(SYNTHETIC_ERRNO(EMEDIUMTYPE), "Field '%s' should be an array, bus it not, refusing.", strna(field->name));
...
static int varlink_idl_validate_method(const VarlinkSymbol *method, JsonVariant *v, int direction, const char **bad_field) {
        assert(direction != 0);
...
int varlink_idl_validate_method_call(const VarlinkSymbol *method, JsonVariant *v, const char **bad_field) {
        return varlink_idl_validate_method(method, v, /* direction= */ -1, bad_field);
}
...
int varlink_idl_validate_error(const VarlinkSymbol *error, JsonVariant *v, const char **bad_field) {
        ...
        return varlink_idl_validate_symbol(error, v, /* direction= */ 0, bad_field);
}
