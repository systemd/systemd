 ...
 *
 * If successful, returns 0 if there are more available properties, even if the requested count was reached,
 * or 1 if there are no more available properties. This corresponds to the TPM2_GetCapability() "more"
 * parameter; specifically, this returns 0 if "more" is TPM2_YES (true), and 1 if "more" is TPM2_NO
 * (false). */
 ...
 *
 * Returns 1 if the TPM supports the algorithm and the TPMA_ALGORITHM provided, or 0 if the TPM does not
...
static int tpm2_capability_alg(Tpm2Context *c, TPM2_ALG_ID alg, TPMA_ALGORITHM *ret) {
        ...
        TPMS_ALG_PROPERTY property = algorithms.algProperties[0];
        if (property.alg != alg)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                       "Requested alg 0x%02" PRIx16 " but TPM provided 0x%02" PRIx16 ".",
                                       alg, property.alg);

        if (ret)
                *ret = property.algProperties;

        return 1;
}

/* Returns 1 if the TPM supports the alg, 0 if the TPM does not support the alg, or < 0 for any error. */
static inline int tpm2_supports_alg(Tpm2Context *c, TPM2_ALG_ID alg) {
