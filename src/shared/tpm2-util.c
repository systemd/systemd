...
/* The *_from_mask() function is in the header. */
uint32_t tpm2_tpms_pcr_selection_to_mask(const TPMS_PCR_SELECTION *s) {
        ...
        for (unsigned i = 0; i < s->sizeofSelect; i++)
                SET_FLAG(mask, s->pcrSelect[i] << (i * 8), true);
        return mask;
}

/* Add all PCR selections in 'b' to 'a'. Both must have the same hash. */
...
/* Remove all PCR selections in 'b' from 'a'. Both must have the same hash. */
...
/* Move all PCR selections in 'b' to 'a'. Both must have the same hash. */
static void tpm2_tpms_pcr_selection_move(TPMS_PCR_SELECTION *a, TPMS_PCR_SELECTION *b) {
        if (a == b)
                return;

        tpm2_tpms_pcr_selection_add(a, b);
        *b = tpm2_tpms_pcr_selection_from_mask(0, b->hash);
}

/* Count the number of selected PCRs. */
#define tpm2_tpms_pcr_selection_weight(s)                               \
        ((size_t) __builtin_popcount(tpm2_tpms_pcr_selection_to_mask(s)))
...
static char *tpm2_tpms_pcr_selection_to_string(const TPMS_PCR_SELECTION *s) {
        ...
        _cleanup_free_ char *maskstr = tpm2_pcr_mask_to_string(tpm2_tpms_pcr_selection_to_mask(s));
        if (!maskstr)
                return NULL;
...
/* Get a TPMS_PCR_SELECTION from a TPML_PCR_SELECTION for the given hash. Returns NULL if there is no entry
 * for the hash. This guarantees the returned entry contains all the PCR selections for the given hash, which
 * may require modifying the TPML_PCR_SELECTION by removing duplicate entries. */
static TPMS_PCR_SELECTION *tpm2_tpml_pcr_selection_get_tpms_pcr_selection(TPML_PCR_SELECTION *l, TPMI_ALG_HASH hash) {
        ...
        /* Iterate backwards through the entries, removing any other entries for the hash. */
...
static void tpm2_log_debug_tpml_pcr_selection(const TPML_PCR_SELECTION *l, const char *msg) {
        ...
        if (s)
                log_debug("%s: %s", msg ?: "PCR selection", s);
        else
                log_oom();
}
        ...
        while (!tpm2_tpml_pcr_selection_is_empty(&remaining)) {
                ...
                if (DEBUG_LOGGING) {
                        ...
                        FOREACH_PCR_IN_TPML_PCR_SELECTION(pcr, s, current_read) {
                                TPML_PCR_SELECTION l = tpm2_tpml_pcr_selection_from_mask(INDEX_TO_MASK(uint32_t, pcr), s->hash);
                                _cleanup_free_ char *desc = tpm2_tpml_pcr_selection_to_string(&l);
                                if (!desc)
                                        return log_oom();

                                assert(i < current_values->count);
                                tpm2_log_debug_digest(&current_values->digests[i], desc);
                                i++;
                        }
                }
        }
