...
int tpm2_quote(
        ...
        _cleanup_free_ void *quoted_buf = memdup(quoted->attestationData, quoted->size);
        if (!quoted_buf)
                return log_oom_debug();
...
int tpm2_nv_certify(
        ...
        _cleanup_free_ void *certify_info_buf = memdup(certify_info->attestationData, certify_info->size);
        if (!certify_info_buf)
                return log_oom_debug();
...
int tpm2_get_session_audit_digest(
        ...
        _cleanup_free_ void *audit_info_buf = memdup(audit_info->attestationData, audit_info->size);
        if (!audit_info_buf)
                return log_oom_debug();
