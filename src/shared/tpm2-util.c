...
static int tpm2_make_transient_primary(
        ...
        int r = make_lock_file("/run/systemd/tpm2-srk-init", LOCK_EX, &srk_lock);
        if (r < 0)
...
int tpm2_seal(const char *device,
        ...
        r = tpm2_make_transient_primary(c.esys_context, 0, !!ret_srk_buf, &primary_alg, &primary);
        if (r < 0)
                return r;
...
int tpm2_unseal(const char *device,
        ...
        /* old callers without an SRK still need to create a key */
                ...
                r = tpm2_make_transient_primary(c.esys_context, primary_alg,
                                /* use_srk_model =*/ false, /* ret_alg = */ NULL, &primary);
                if (r < 0)
                        return r;
        }
...
finish:
        /* don't flush persistent handles */
        if (!srk_buf)
                primary = tpm2_flush_context_verbose(c.esys_context, primary);
