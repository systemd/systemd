...
int tpm2_extend_bytes(
        ...
        STRV_FOREACH(bank, banks) {
                 ...
                 * wire is unprotected). Hence we instead measure an HMAC signature of a non-secret string
                 * instead. */
                if (secret_size > 0) {
...
#else
        return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP),
                               "TPM2/OPENSSL not supported on this build.");
#endif
}
