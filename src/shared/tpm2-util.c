...
static char *tpm2_tpms_pcr_selection_to_string(const TPMS_PCR_SELECTION *s) {
        ...
        const char *alg = tpm2_hash_alg_to_string(s->hash);
        if (alg) {
...
#define tpm2_log_debug_pcr_selection_digest(s, pcr, digest, ...)        \
        ({                                                              \
                ...
                const char *_bank_ = tpm2_hash_alg_to_string(_hash_);   \
                if (_bank_)                                             \
...
static const TPM2B_DIGEST TPM2_DIGEST_ZERO = { .size = SHA256_DIGEST_SIZE, };
...
#define tpm2_auth_init(auth, data, len)                                 \
        ({                                                              \
                assert(__builtin_types_compatible_p(TPM2B_DIGEST, typeof(auth))); \
...
static int tpm2_policy_authorize(
        ...
        r = tpm2_load_external_key(c, NULL, &pubkey_tpm2, NULL, &pubkey_handle);
        if (r < 0)
                return r;
...
static int tpm2_calculate_policy_duplication_select(
        ...
        /* data/len max size is 4: { buf, object_name, new_parent_name, include_object } */
...
static int tpm2_build_sealing_policy(
        ...
        if (pubkey_pcr_mask != 0) {
                ...
                r = tpm2_policy_authorize(c, session, &pcr_selection, pubkey, pubkey_size, signature_json, ret_policy_digest);
                if (r < 0)
                        return r;
        }
        ...
        if (hash_pcr_mask != 0) {
                ...
                r = tpm2_policy_pcr(c, session, &pcr_selection, ret_policy_digest);
                if (r < 0)
                        return r;
        }

        if (use_pin) {
                r = tpm2_policy_auth_value(c, session, ret_policy_digest);
                if (r < 0)
                        return r;
        }
...
static int tpm2_create_and_duplicate_key(
        ...
        _cleanup_(Esys_Freep) TPM2B_PRIVATE *key_private = NULL;
        _cleanup_(Esys_Freep) TPM2B_PUBLIC *key_public = NULL;
        _cleanup_tpm2_handle_ Tpm2Handle *key_handle = NULL;
        r = tpm2_create_key(
                        c,
                        parent,
                        encryption_session,
        ...
        _cleanup_(Esys_Freep) TPM2B_NAME *key_name = NULL;
        ...
        _cleanup_(Esys_Freep) TPM2B_PRIVATE *dup_private = NULL;
        ...
        if (ret_key_handle)
                *ret_key_handle = TAKE_PTR(key_handle);
        if (ret_dup_private)
