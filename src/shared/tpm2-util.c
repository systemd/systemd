...
static bool has_expected_ek_cert_attributes(TPM2B_NV_PUBLIC *nv_pub) {
...
static int tpm2_get_custom_ek_template(
        ...
        /* Ignore any NV index that doesn't have the expected attributes, to ensure that we don't generate
         * erroneous errors if we encounter some arbitrary owner-created index. */
        if (!has_expected_ek_cert_attributes(template_nvpub))
                return 0;
...
void tpm2_get_default_ek_template(Tpm2EKTemplateProfile profile, TPMT_PUBLIC *ret_template) {
        ...
        case TPM2_ALG_RSA:
                template.parameters.rsaDetail.keyBits = params->params.rsa_key_bits;
                break;
        case TPM2_ALG_ECC:
                template.parameters.eccDetail.curveID = params->params.ecc_curve_id;
