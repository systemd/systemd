...
/* Convert a mask and hash to a TPMS_PCR_SELECTION object. */
...
/* Convert a TPML_PCR_SELECTION object to a mask. Returns -ENOENT if 'hash' is not in the object. */
int tpm2_tpml_pcr_selection_to_mask(const TPML_PCR_SELECTION *l, TPMI_ALG_HASH hash, uint32_t *ret) {
         ...
         * are multiple entries with the requested hash. */
        TPML_PCR_SELECTION lcopy = *l;

        TPMS_PCR_SELECTION *s;
        s = tpm2_tpml_pcr_selection_get_tpms_pcr_selection(&lcopy, hash);
        if (!s)
...
/* Convert a mask and hash to a TPML_PCR_SELECTION object. */
 ...
 * TPMS_PCR_SELECTION entry for the hash if needed. This may modify the TPML_PCR_SELECTION by combining
 * same-hash entries. */
...
/* Remove the PCR selections in 's' from the corresponding hash TPMS_PCR_SELECTION entry in 'l'. This will
 * combine all entries for 's->hash' in 'l'. */
