...
/* Convert a TPML_PCR_SELECTION object to a mask. Returns -ENOENT if the specified hash is not in the
 * object. */
int tpm2_tpml_pcr_selection_to_mask(const TPML_PCR_SELECTION *l, TPMI_ALG_HASH hash, uint32_t *ret) {
        ...
        s = tpm2_tpml_pcr_selection_get_tpms_pcr_selection((TPML_PCR_SELECTION*) l, hash);
        if (!s)
