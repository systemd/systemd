...
int tpm2_unseal(Tpm2Context *c,
         ...
         * indirectly via secure boot). We retry that RETRY_UNSEAL_MAX+1 times before reporting it. */
        ...
        for (unsigned i = RETRY_UNSEAL_MAX;; i--) {
                ...
                for (size_t shard = 0; shard < n_blobs; shard++) {
                        ...
                        if (r == -EUCLEAN) {
                                if (i == 0)
                                        return log_debug_errno(r, "PCR values kept changing while building the TPM2 policy, giving up after %u attempts.", RETRY_UNSEAL_MAX + 1);
                        ...
                        if (rc == TPM2_RC_PCR_CHANGED) {
                                if (i == 0)
                                        return log_debug_errno(SYNTHETIC_ERRNO(EUCLEAN),
                                                               "PCR values kept changing while unsealing, giving up after %u attempts.", RETRY_UNSEAL_MAX + 1);
