...
int tpm2_get_best_pcr_bank(
        ...
        if (r < 0) {
                ...
                if (r == -EOPNOTSUPP) {
                        ...
                        return log_debug_errno(r, "Firmware reports none of the supported PCR banks (%s), cannot operate.", strna(banks));
                }
                if (r < 0)
                        return r;
        ...
        if (hash_with_valid_pcr != 0) {
                ...
                *ret = supported_hash;
                ...
                _cleanup_free_ char *banks = tpm2_pcr_bank_preference_to_string();
                return log_debug_errno(SYNTHETIC_ERRNO(EOPNOTSUPP),
                                       "TPM2 module supports none of the supported PCR banks (%s), cannot operate.", strna(banks));
        }
