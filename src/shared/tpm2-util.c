...
int dlopen_tpm2(void) {
        ...
        r = dlsym_many_or_warn(libtss2_esys_dl, LOG_DEBUG, DLSYM_ARG(Esys_TR_GetTpmHandle));
        if (r < 0)
...
int tpm2_get_location(Tpm2Context *c, const Tpm2Handle *handle, TPM2_HANDLE *ret_location) {
        ...
        if (dlopen_tpm2() >= 0 && sym_Esys_TR_GetTpmHandle == NULL)
...
int tpm2_seal(Tpm2Context *c,
        ...
        if (ret_srk_buf) {
                r = tpm2_get_or_create_srk(c, NULL, NULL, NULL, NULL, &primary_handle);
                if (r < 0)
                        return r;
        } else {
