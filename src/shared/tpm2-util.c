...
static int tpm2_read_ek_cert(
         ...
         * erroneous errors if we encounter some arbitrary owner-created index. These attributes are defined
         * in the "TCG PC Client Platform TPM Profile Specification for TPM 2.0" spec, section 4.6.3.1
         * (Preprovisioned EK Certificates).
         *
        ...
        TPMA_NV expected_set_attrs =
                ...
                TPMA_NV_OWNERREAD |
                TPMA_NV_NO_DA;
        if (((cert_nvpub->nvPublic.attributes & expected_set_attrs) != expected_set_attrs) ||
            ((cert_nvpub->nvPublic.attributes & expected_clear_attrs) != 0))
                return 0;
...
static int tpm2_get_custom_ek_template(
        ...
        if (r <= 0)
                return r; /* An error or no index exists. */

        /* Ignore any NV index that doesn't have the expected attributes, to ensure that we don't generate
         * erroneous errors if we encounter some arbitrary owner-created index. Assume that template or nonce
         * indexes have the same attributes that those for EK certificates have (see tpm2_read_ek_cert). */
        TPMA_NV expected_clear_attrs =
                TPMA_NV_AUTHWRITE |
                TPMA_NV_OWNERWRITE |
                TPMA_NV_GLOBALLOCK |
        ...
        TPMA_NV expected_set_attrs =
                ...
                TPMA_NV_OWNERREAD |
                TPMA_NV_NO_DA;
        if (((template_nvpub->nvPublic.attributes & expected_set_attrs) != expected_set_attrs) ||
            ((template_nvpub->nvPublic.attributes & expected_clear_attrs) != 0))
                return 0;
        ...
        /* There is a custom EK template defined. Now check if there is a nonce. A nonce is only valid
         * with a custom template, which is why we only check it once we have decoded the template. */
        ...
        if (nonce_index != 0) {
                ...
                if (r < 0)
                        return r;
                if (r > 0 && (((nonce_nvpub->nvPublic.attributes & expected_set_attrs) != expected_set_attrs) ||
                              ((nonce_nvpub->nvPublic.attributes & expected_clear_attrs) != 0)))
...
void tpm2_get_default_ek_template(Tpm2EKTemplateProfile profile, TPMT_PUBLIC *ret_template) {
        ...
        static const TPM2B_DIGEST auth_policy_a = {
        ...
        /* High-range EK templates define a policy that permits the privacy administrator to delegate admin
         * usage of the endorsement key via TPM2_PolicyAuthorizeNV. There is one NV index defined per name
         * algorithm. These and their public attributes are specified in secion A.1 of the "TCG EK Credential
         * Profile for TPM Family 2.0" spec. */

        /* This is the SHA-256 policy of:
         *
         * TPM2_PolicySecret(TPM_RH_ENDORSEMENT) OR TPM2_PolicyAuthorizeNV(0x01C07F01). */
        static const TPM2B_DIGEST auth_policy_b_sha256 = {
                .size = TPM2_SHA256_DIGEST_SIZE,
                .buffer = {
        ...
        /* This is the SHA-384 policy of:
         *
         * TPM2_PolicySecret(TPM_RH_ENDORSEMENT) OR TPM2_PolicyAuthorizeNV(0x01C07F02). */
        static const TPM2B_DIGEST auth_policy_b_sha384 = {
        ...
        static const struct {
                ...
                [TPM2_EK_TEMPLATE_RSA_2048_LEGACY] = {
                        ...
                        .attrs = attributes_a,
                        .auth_policy = auth_policy_a,
                        .params.rsa_key_bits = 2048,
                        .unique.rsa.size = 256,
                        .sym_key_bits = 128,
                },
                [TPM2_EK_TEMPLATE_ECC_NIST_P256_LEGACY] = {
                        ...
                        .attrs = attributes_a,
                        .auth_policy = auth_policy_a,
                        .params.ecc_curve_id = TPM2_ECC_NIST_P256,
                ...
                [TPM2_EK_TEMPLATE_RSA_2048] = {
                        ...
                        .name_alg = TPM2_ALG_SHA256,
                        .attrs = attributes_b,
                        .auth_policy = auth_policy_b_sha256,
                        .params.rsa_key_bits = 2048,
                        .unique.rsa.size = 0,
                        .sym_key_bits = 128,
                },
                ...
                [TPM2_EK_TEMPLATE_ECC_NIST_P384] = {
                        ...
                        .attrs = attributes_b,
                        .auth_policy = auth_policy_b_sha384,
                        .params.ecc_curve_id = TPM2_ECC_NIST_P384,
                        .unique.ecc.x.size = 0,
                        .unique.ecc.y.size = 0,
                        .sym_key_bits = 256,
                },
        ...
        TPMT_PUBLIC template = {
                .type = template_params[profile].alg,
                .nameAlg = template_params[profile].name_alg,
                .objectAttributes = template_params[profile].attrs,
                .authPolicy = template_params[profile].auth_policy,
                .parameters.asymDetail = (TPMS_ASYM_PARMS){
                        .symmetric = (TPMT_SYM_DEF_OBJECT){
                                .algorithm = TPM2_ALG_AES,
                                .keyBits.aes = template_params[profile].sym_key_bits,
                                .mode.aes = TPM2_ALG_CFB,
                        },
                        .scheme.scheme = TPM2_ALG_NULL,
                },
                .unique = template_params[profile].unique,
        };

        switch (template.type) {
        case TPM2_ALG_RSA:
                template.parameters.rsaDetail.keyBits = template_params[profile].params.rsa_key_bits;
                break;
        case TPM2_ALG_ECC:
                template.parameters.eccDetail.curveID = template_params[profile].params.ecc_curve_id;
...
int tpm2_get_ek_template(
        ...
        if (r < 0)
                return r;
        if (r > 0)
                return 0 /* Return custom template */;
