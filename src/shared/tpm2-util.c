...
const TPM2B_PUBLIC *tpm2_get_primary_template(bool use_srk_model, bool is_ecc) {

        /*
         * Set up array so adding use_srk_model and is_ecc get us what we
         * want.
         *
         * use_srk_model   is_ecc   result
         *             0        0   RSA old
         *             0        1   ECC old
         *             1        0   RSA SRK
         *             1        1   ECC SRK
         ...
         *
         * XXX What do we want to do here, we should have @Andreas weigh in.
         */
        ...
        /* make sure if someone uses something other than true false we don't explode */
        return &templ[((use_srk_model & 0x1) << 1) + (is_ecc & 0x1)];
}

#define SRK_HANDLE 0x81000001
/*
 * See tpm2-tss bug: https://github.com/tpm2-software/tpm2-tss/issues/2553
 * Keep it simple, just use the raw numerics.
 */
#define TPM2_HR_PERSISTENT_FIRST_GOOD  0x81000000
...
static int tpm2_get_srk(
        ...
        for (UINT32 i=0; i < cap_data->data.handles.count; i++) {
                TPM2_HANDLE handle = cap_data->data.handles.handle[i];
                if (handle != SRK_HANDLE)
                        continue;

                log_debug("Found SRK on TPM.");

                rc = sym_Esys_TR_FromTPMPublic(c,
                        handle,
                        ESYS_TR_NONE,
                        ESYS_TR_NONE,
                        ESYS_TR_NONE,
        ...
        /* Did Not find SRK, indicate this by returning 0 and ESYS_TR_NONE */
        if (primary == ESYS_TR_NONE) {
                *ret_primary = ESYS_TR_NONE;
                return 0;
        }

        /* Found the SRK, make sure the alg is what we were expecting */
        _cleanup_(Esys_Freep) TPM2B_PUBLIC *out_public = NULL;
        _cleanup_(Esys_Freep) TPM2B_NAME *out_name = NULL, *out_qname = NULL;
        ...
        /* Cannot specify the alg and have a mismatch */
        if (alg != 0 && alg != out_public->publicArea.type)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                        "SRK expected alg 0x%x does not match found alg 0x%x",
                                        alg, out_public->publicArea.type);
...
static int tpm2_make_primary(
        ...
        if (use_srk_model) {
                rc = tpm2_get_srk(c, ret_primary, alg, ret_alg);
                if (rc)
                        return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                               "Failed to establish if SRK is present");

                if (*ret_primary != ESYS_TR_NONE) {
                        log_debug("Discovered SRK using that");
                        return 0;
                }
        ...
        if (IN_SET(alg, 0, TPM2_ALG_ECC)) {
                primary_template = tpm2_get_primary_template(use_srk_model, true);
        ...
        if (IN_SET(alg, 0, TPM2_ALG_RSA)) {
                primary_template = tpm2_get_primary_template(use_srk_model, false);
        ...
        if (use_srk_model) {
                rc = sym_Esys_EvictControl(c, ESYS_TR_RH_OWNER, primary, ESYS_TR_PASSWORD, ESYS_TR_NONE, ESYS_TR_NONE, SRK_HANDLE, &primary);
                if (rc != TSS2_RC_SUCCESS)
                        return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
...
int tpm2_seal(const char *device,
              ...
              size_t *ret_primary_buf_size) {

        _cleanup_(tpm2_context_destroy) struct tpm2_context c = {};
        _cleanup_(Esys_Freep) TPM2B_DIGEST *policy_digest = NULL;
        _cleanup_(Esys_Freep) TPM2B_PRIVATE *private = NULL;
        _cleanup_(Esys_Freep) TPM2B_PUBLIC *public = NULL;
        _cleanup_(Esys_Freep) uint8_t *primary_buf = NULL;
        static const TPML_PCR_SELECTION creation_pcr = {};
        ...
        size_t k, blob_size, primary_buf_size;
        usec_t start;
        TSS2_RC rc;
        int r;
        assert(pubkey || pubkey_size == 0);
        ...
        r = tpm2_make_primary(c.esys_context, &primary, 0, !!ret_primary_buf, &primary_alg);
        if (r < 0)
                return r;
         ...
         */
        if (ret_primary_buf) {
                log_debug("Serializing SRK");
                rc = sym_Esys_TR_Serialize(c.esys_context, primary, &primary_buf, &primary_buf_size);
                if (rc) {
                        r = log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
        ...
        *ret_primary_alg = primary_alg;
        ...
        if (ret_primary_buf) {
                 ...
                 * to use for primary_buf in which context a PITA.
                 */
                void *tmp = memdup(primary_buf, primary_buf_size);
                if (!tmp)
                        return log_oom();

                *ret_primary_buf = tmp;
                *ret_primary_buf_size = primary_buf_size;
        }
...
int tpm2_unseal(const char *device,
        ...
        if (primary_buf) {
                ...
                if (rc != TSS2_RC_SUCCESS)
                        return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                               "Failed to unmarshal primary key: %s", sym_Tss2_RC_Decode(rc));
        /* old callers without an SRK still need to create a key */
        } else {
                r = tpm2_make_primary(c.esys_context, &primary, primary_alg, false, NULL);
                if (r < 0)
                        return r;
        }
...
int tpm2_make_luks2_json(
        ...
        r = json_build(&v,
                       JSON_BUILD_OBJECT(
                                       ...
                                       JSON_BUILD_PAIR_CONDITION(primary_buf, "tpm2_srk", JSON_BUILD_BASE64(primary_buf, primary_buf_size))));
        if (r < 0)
                return r;
...
int tpm2_parse_luks2_json(
        ...
        _cleanup_free_ void *blob = NULL, *policy_hash = NULL, *pubkey = NULL, *salt = NULL, *primary_buf = NULL;
        size_t blob_size = 0, policy_hash_size = 0, pubkey_size = 0, salt_size = 0, primary_buf_size = 0;
        ...
        if (w) {
                r = json_variant_unbase64(w, &primary_buf, &primary_buf_size);
                if (r < 0)
                        return log_debug_errno(r, "Invalid base64 data in 'tpm2-blob' field.");
        ...
        if (ret_flags)
                *ret_flags = flags;
        if (ret_primary_buf)
                *ret_primary_buf = TAKE_PTR(primary_buf);
        if (ret_primary_buf_size)
                *ret_primary_buf_size = primary_buf_size;
