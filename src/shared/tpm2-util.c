...
int tpm2_calculate_name(const TPMT_PUBLIC *public, TPM2B_NAME *ret_name) {
        ...
        buf = malloc0(sizeof(*public));
        if (!buf)
                return log_oom();

        rc = sym_Tss2_MU_TPMT_PUBLIC_Marshal(public, buf, sizeof(*public), &size);
        if (rc != TSS2_RC_SUCCESS)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
        ...
        tpm2_digest_hash_buffer(TPM2_ALG_SHA256, &digest, buf, size, false);

        TPMT_HA ha = { .hashAlg = TPM2_ALG_SHA256, };
        assert(digest.size <= sizeof(ha.digest.sha256));
        memcpy(ha.digest.sha256, digest.buffer, digest.size);
...
int tpm2_calculate_policy_pcr(
        ...
        TPM2B_DIGEST hash;
        r = tpm2_digest_init(TPM2_ALG_SHA256, &hash);
        if (r < 0)
                return r;

        tpm2_digest_hash_digests(TPM2_ALG_SHA256, &hash, pcr_values, pcr_values_count, false);
        ...
        tpm2_digest_hash_buffers(TPM2_ALG_SHA256, digest, data, len, 2, true);
...
int tpm2_calculate_policy_authorize(
        ...
        rc = sym_Tss2_MU_TPM2_CC_Marshal(command, buf, sizeof(command), &offset);
        if (rc != TSS2_RC_SUCCESS)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
        ...
        TPM2B_NAME name = {};
        r = tpm2_calculate_name(&pubkey_tpm2.publicArea, &name);
        if (r < 0)
                return r;

        /* PolicyAuthorize requires the hash to be reset to zero. */
        zero(digest->buffer);
        ...
        if (policy_ref)
                tpm2_digest_hash_buffer(TPM2_ALG_SHA256, digest, policy_ref->buffer, policy_ref->size, true);
        else
                tpm2_digest_rehash(TPM2_ALG_SHA256, digest);
...
static int tpm2_policy_authorize(
        ...
        if (signature_json) {
                ...
                tpm2_digest_rehash(TPM2_ALG_SHA256, &signature_hash);
...
int tpm2_calculate_policy_auth_value(TPM2B_DIGEST *digest) {
        ...
        tpm2_digest_hash_buffer(TPM2_ALG_SHA256, digest, buf, offset, true);
...
/* Update the provided digest with the calculated sealing policy. The digest must be an initialized SHA256
 * digest. Its value may be zero. */
...
static int tpm2_build_sealing_policy(
        ...
        if (pubkey_pcr_mask != 0) {
                ...
                r = tpm2_policy_authorize(c, session, &pcr_selection, pubkey, pubkey_size, signature_json, NULL);
                if (r < 0)
                        return r;
        }
