...
int tpm2_get_best_pcr_bank(
        ...
        if (r < 0) {
                ...
                log_debug("Boot loader set LoaderTpm2ActivePcrBanks EFI variable to UINT32_MAX (i.e. GetActivePcrBanks() API is not available in the firmware), we have to guess used PCR banks.");
        else {
...
int tpm2_nvpcr_initialize(
        ...
        *(le32_t*) hmac_buffer = htole32(p.nv_index);
        ...
        if (asprintf(&word, "nvpcr-initialization:%s:0x%x:%s:%s", name, p.nv_index, tpm2_hash_alg_to_string(p.algorithm), h) < 0)
                return log_oom();
        ...
        r = tpm2_pcr_extend_bytes(
                        ...
                        /* secret= */ NULL,
                        TPM2_EVENT_NVPCR_INITIALIZATION,
                        word);
        if (r < 0)
