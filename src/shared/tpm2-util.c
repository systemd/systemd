...
int tpm2_seal(Tpm2Context *c,
        ...
        size_t n_shards = n_policy >= 2 ? n_policy : 1;
...
int tpm2_unseal(Tpm2Context *c,
        ...
        if (n_blobs != n_shards)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Number of provided key blobs (%zu) does not match policy requirements (%zu).", n_blobs, n_shards);
        if (n_known_policy_hash > 0 && n_known_policy_hash != n_shards)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Number of provided known policy hashes (%zu) does not match policy requirements (%zu or 0).", n_known_policy_hash, n_shards);
        ...
        for (unsigned i = RETRY_UNSEAL_MAX;; i--) {
                ...
                for (size_t shard = 0; shard < n_blobs; shard++) {
                        ...
                        r = tpm2_build_sealing_policy(
                                        ...
                                        signature,
                                        !!pin,
                                        (shard == 1 || !iovec_is_set(pubkey)) && pcrlock_policy ? pcrlock_policy : NULL,
                                        &policy_digest);
                        if (r < 0)
                                return r;
...
static int tpm2_make_policy_hash_variant(
        ...
        assert(policy_hash);
        assert(n_policy_hash > 0);
        assert(ret);

        _cleanup_(sd_json_variant_unrefp) sd_json_variant *j = NULL;
        if (n_policy_hash > 1) {
                /* Multiple policy hashes? Then generate an array of hex strings */

                for (size_t i = 0; i < n_policy_hash; i++) {
                        assert(iovec_is_set(policy_hash + i));

                        r = sd_json_variant_append_arrayb(
                                        &j,
                                        JSON_BUILD_IOVEC_HEX(policy_hash + i));
                        if (r < 0)
                                return r;
                }
        } else {
                /* Only one hash? Then create only one hex string, for compatibility with older versions
                 * which didn't support the "sharding" scheme */

                assert(n_policy_hash == 1);
                assert(iovec_is_set(policy_hash + 0));

                r = sd_json_build(&j,
                                  JSON_BUILD_IOVEC_HEX(policy_hash + 0));
                if (r < 0)
                        return r;
        }
...
static int tpm2_make_blob_variant(
                const struct iovec blobs[],
                size_t n_blobs,
                sd_json_variant **ret) {
        ...
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *j = NULL;
        if (n_blobs > 1) {
                /* Multiple blobs? Then generate an array of base64 strings */

                for (size_t i = 0; i < n_blobs; i++) {
                        assert(iovec_is_set(blobs + i));

                        r = sd_json_variant_append_arrayb(
                                        &j,
                                        JSON_BUILD_IOVEC_BASE64(blobs + i));
                        if (r < 0)
                                return r;
                }
        } else {
                /* Only one blob? Then create only one base64 string, for compatibility with older versions
                 * which didn't support the "sharding" scheme */

                assert(n_blobs == 1);
                assert(iovec_is_set(blobs + 0));

                r = sd_json_build(&j,
                                  JSON_BUILD_IOVEC_BASE64(blobs + 0));
                if (r < 0)
                        return r;
        }
...
int tpm2_make_luks2_json(
        ...
        r = tpm2_make_policy_hash_variant(policy_hash, n_policy_hash, &phj);
        if (r < 0)
                return r;

        _cleanup_(sd_json_variant_unrefp) sd_json_variant *bj = NULL;
        r = tpm2_make_blob_variant(blobs, n_blobs, &bj);
        if (r < 0)
                return r;
...
int tpm2_parse_luks2_json(
        ...
        if (sd_json_variant_is_array(w)) {
                if (sd_json_variant_elements(w) == 0)
                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "TPM2 token data contains empty 'tpm2-blob' array.");

                blobs = new0(struct iovec, sd_json_variant_elements(w));
                if (!blobs)
                        return log_oom_debug();

                sd_json_variant *i;
                JSON_VARIANT_ARRAY_FOREACH(i, w) {
                        r = json_variant_unbase64_iovec(i, blobs + n_blobs);
                        if (r < 0)
                ...
                r = json_variant_unbase64_iovec(w, blobs + 0);
                if (r < 0)
                        return log_debug_errno(r, "Invalid base64 data in 'tpm2-blob' field.");
        ...
        if (sd_json_variant_is_array(w)) {
                if (sd_json_variant_elements(w) == 0)
                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "TPM2 token data contains empty 'tpm2-policy-hash' field.");

                policy_hash = new(struct iovec, sd_json_variant_elements(w));
                if (!policy_hash)
                        return log_oom_debug();

                sd_json_variant *i;
                JSON_VARIANT_ARRAY_FOREACH(i, w) {
                        r = json_variant_unhex_iovec(i, policy_hash + n_policy_hash);
                        if (r < 0)
                                return log_debug_errno(r, "Invalid hex data in 'tpm2-policy-hash' field.");

                        n_policy_hash++;
                }
        } else {
                policy_hash = new0(struct iovec, 1);
                if (!policy_hash)
                        return log_oom_debug();

                r = json_variant_unhex_iovec(w, policy_hash + 0);
                if (r < 0)
                        return log_debug_errno(r, "Invalid hex data in 'tpm2-policy-hash' field.");
        }
