...
static int tpm2_get_capability(
        ...
        log_debug("Getting TPM2 capability 0x%04x property 0x%04x count %u.", capability, start, count);
        ...
        if (rc != TSS2_RC_SUCCESS)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                       "Failed to get TPM2 capability 0x%04x property 0x%04x: %s",
                                       capability, start, sym_Tss2_RC_Decode(rc));

        if (capabilities->capability != capability)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                       "TPM provided wrong capability: 0x%04x instead of 0x%04x.",
 ...
 * handle range (i.e. handle & 0xff000000). */
static int tpm2_get_capability_handles(
        ...
        while (more == TPM2_YES && n_handles < max) {
                ...
                memcpy_safe(&handles[n_handles], handle_list.handle, sizeof(TPM2_HANDLE) * handle_list.count);

                n_handles += handle_list.count;

                /* Update current to the handle right after the last handle in the list. */
...
#define tpm2_get_capability_range_handles(c, hr, ret_handles, ret_n_handles) \
        tpm2_get_capability_handles(c, hr, UINT32_C(1) << TPM2_HR_SHIFT, ret_handles, ret_n_handles)
#define tpm2_get_capability_persistent_handles(c, ret_handles, ret_n_handles) \
        tpm2_get_capability_range_handles(c, TPM2_HR_PERSISTENT, ret_handles, ret_n_handles)
#define tpm2_get_capability_transient_handles(c, ret_handles, ret_n_handles) \
        tpm2_get_capability_range_handles(c, TPM2_HR_TRANSIENT, ret_handles, ret_n_handles)
#define tpm2_get_capability_nv_index_handles(c, ret_handles, ret_n_handles) \
        tpm2_get_capability_range_handles(c, TPM2_HR_NV_INDEX, ret_handles, ret_n_handles)
#define tpm2_get_capability_permanent_handles(c, ret_handles, ret_n_handles) \
        tpm2_get_capability_range_handles(c, TPM2_HR_PERMANENT, ret_handles, ret_n_handles)

/* This only checks if the specified handle is populated. Returns true, false, or error. */
static int tpm2_get_capability_handle(Tpm2Context *c, TPM2_HANDLE handle) {
        ...
        return n_handles ? handles[0] == handle : false;
}

static bool tpm2_test_parms(Tpm2Context *c, TPMI_ALG_PUBLIC alg, const TPMU_PUBLIC_PARMS *parms) {
        TSS2_RC rc;
...
int tpm2_context_new(const char *device, Tpm2Context **ret_context) {
        ...
        *context = (Tpm2Context) {
        ...
        if (!tpm2_supports_tpmt_sym_def(context, &context->session_symmetric_template))
...
static int tpm2_get_srk_template(Tpm2Context *c, TPMI_ALG_PUBLIC alg, TPMT_PUBLIC *ret_template) {
        ...
        static const TPMA_OBJECT srk_attributes = (TPMA_OBJECT_DECRYPT |
        ...
        static const TPMT_SYM_DEF_OBJECT srk_symmetric = {
                .algorithm = TPM2_ALG_AES,
                .keyBits.aes = 128,
                .mode.aes = TPM2_ALG_CFB,
        };
        ...
        static const TPMT_PUBLIC srk_ecc = {
                .type = TPM2_ALG_ECC,
                .nameAlg = TPM2_ALG_SHA256,
        ...
        static const TPMT_PUBLIC srk_rsa = {
                .type = TPM2_ALG_RSA,
                .nameAlg = TPM2_ALG_SHA256,
                .objectAttributes = srk_attributes,
                .parameters.rsaDetail = {
...
int tpm2_digest_many(
        ...
        if (extend)
                ...
                *digest = (TPM2B_DIGEST){ .size = SHA256_DIGEST_SIZE, };
                if (count == 0) /* If not extending and no data, return zero hash */
                        return 0;
        }
...
int tpm2_digest_many_digests(
        ...
        iovecs = new0(struct iovec, count);
        if (!iovecs)
                return log_oom();

        for (size_t i = 0; i < count; i++) {
                iovecs[i].iov_base = (void*) data[i].buffer;
                iovecs[i].iov_len = data[i].size;
        }

        return tpm2_digest_many(alg, digest, iovecs, count, extend);
}
 ...
 *
 * Since we (currently) hardcode to always using SHA256 for hashing, this returns error if the public key
...
int tpm2_calculate_name(const TPMT_PUBLIC *public, TPM2B_NAME *ret_name) {
        ...
        buf = (uint8_t*) new0(TPMT_PUBLIC, 1);
        if (!buf)
                return log_oom();
        ...
        TPM2B_DIGEST digest = {};
        r = tpm2_digest_buffer(TPM2_ALG_SHA256, &digest, buf, size, false);
        if (r < 0)
                return r;
...
int tpm2_calculate_policy_pcr(
        ...
        const TPM2_CC command = TPM2_CC_PolicyPCR;
        TSS2_RC rc;
        int r;
        ...
        TPM2B_DIGEST hash = {};
        r = tpm2_digest_many_digests(TPM2_ALG_SHA256, &hash, pcr_values, n_pcr_values, false);
        if (r < 0)
                return r;
        ...
        r = tpm2_digest_many(TPM2_ALG_SHA256, digest, data, ELEMENTSOF(data), true);
        if (r < 0)
                return r;
...
int tpm2_calculate_policy_authorize(
        ...
        const TPM2_CC command = TPM2_CC_PolicyAuthorize;
        TSS2_RC rc;
        int r;
        ...
        if (policy_ref)
                r = tpm2_digest_many_digests(TPM2_ALG_SHA256, digest, policy_ref, 1, true);
        else
...
int tpm2_seal(const char *device,
        ...
        if (pin) {
                r = tpm2_digest_buffer(TPM2_ALG_SHA256, &hmac_sensitive.userAuth, pin, strlen(pin), false);
                if (r < 0)
                        return r;
        }
