...
int tpm2_pcr_extend_bytes(
        ...
        rc = sym_Esys_PCR_Extend(
                        ...
                        &values);
        if (rc != TSS2_RC_SUCCESS) {
                /* Only revert if the TPM itself rejected the command (register untouched). A lower-layer
                 * error is ambiguous, the command may have executed with its reply lost, so keep the marker. */
                if ((rc & TSS2_RC_LAYER_MASK) == TSS2_TPM_RC_LAYER)
...
static int nvpcr_extend_bytes(
        ...
        r = tpm2_extend_nvpcr_nv_index(
                        ...
                        &digest);
        if (r < 0) {
                /* Extend failed, register wasn't advanced, revert the marker. */
                (void) tpm2_userspace_log_clean(log_fd, reset_marker);
                return r;
        }
