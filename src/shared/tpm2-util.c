...
int tpm2_seal(const char *device,
        ...
        _cleanup_(tpm2_handle_releasep) struct tpm2_handle encryption_session = tpm2_handle_init(c),
                primary_handle = tpm2_handle_init(c), secondary_handle = tpm2_handle_init(c);
        ...
        r = tpm2_make_encryption_session(c, primary_handle, HANDLE_NONE, &encryption_session);
        if (r < 0)
                return r;
        ...
        struct tpm2_handle hmac_parent_handle = {};
        if (external_pubkey) {
                _cleanup_(tpm2_handle_releasep) struct tpm2_handle external_handle = tpm2_handle_init(c);
...
int tpm2_unseal(const char *device,
        ...
        if (blob_size > offset) {
                ...
                _cleanup_(tpm2_handle_releasep) struct tpm2_handle import_session = tpm2_handle_init(c);
                r = tpm2_make_encryption_session(c, primary_handle, HANDLE_NONE, &import_session);
                if (r < 0)
                        return r;
