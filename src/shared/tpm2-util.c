 ...
 *
 * Returns true if the TPM supports the algorithm and the TPMA_ALGORITHM is provided, or false if the TPM
...
/* Returns true if the TPM supports the alg, or false if the TPM does not support the alg. */
...
static int tpm2_get_or_create_srk(
        ...
        if (r == 0)
                ...
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE), "Persisted SRK not found.");
