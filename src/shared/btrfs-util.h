...
static inline int btrfs_get_block_device(const char *path, uint64_t *ret_devid, dev_t *ret) {
        return btrfs_get_block_device_at(AT_FDCWD, path, ret_devid, ret);
}
static inline int btrfs_get_block_device_fd(int fd, uint64_t *ret_devid, dev_t *ret) {
        return btrfs_get_block_device_at(fd, "", ret_devid, ret);
}
