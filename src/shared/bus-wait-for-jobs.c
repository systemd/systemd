...
static int check_wait_response(BusWaitForJobs *d, WaitJobsFlags flags, const char* const* extra_args) {
        ...
        static const struct {
                ...
                /* Success */
                { "done",        "Job for %s finished.",    0 },
                { "skipped"      "Job for %s was skipped.", 0 },

                /* Failure */
                { "canceled",    "Job for %s canceled.",                                              -ECANCELED  },
                { "timeout",     "Job for %s timed out.",                                             -ETIME      },
                { "dependency",  "A dependency job for %s failed. See 'journalctl -xe' for details.", -EIO        },
                { "invalid",     "%s is not active, cannot reload.",                                  -ENOEXEC    },
                { "assert",      "Assertion failed on job for %s."                                    -EPROTO     },
                { "unsupported", "Operation on or unit type of %s not supported on this system.",     -EOPNOTSUPP },
                { "collected",   "Queued job for %s was garbage collected.",                          -ECANCELED  },
                { "once",        "Unit %s was already started once and can't be started again.",      -ESTALE     },
        };
        ...
        FOREACH_ARRAY(i, result_table, ELEMENTSOF(result_table)) {
                assert(i->result);
                assert(i->message);
                assert(i->retval <= 0);

                if (!streq(d->result, i->result))
                        continue;

                if (i->retval < 0 && FLAGS_SET(flags, BUS_WAIT_JOBS_LOG_ERROR))
                        log_error_errno(i->retval, i->message, d->name);
                else if (i->retval == 0 && FLAGS_SET(flags, BUS_WAIT_JOBS_LOG_SUCCESS))
                        log_info(i->message, d->name);
