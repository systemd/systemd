...
int machine_id_commit(const char *root) {
        ...
        _cleanup_close_ int etc_machine_id_fd = -EBADF, etc_machine_id_dir_fd = -EBADF;
        _cleanup_free_ char *etc_machine_id = NULL, *etc_machine_id_filename = NULL;
        ...
        if (r == -ENOENT) {
                ...
                if (r == -ENOENT) {
                        log_debug_errno(r, "%s does not exist. Nothing to do: %m", etc_machine_id);
                        return 0;
                }
                if (r < 0)
        ...
        etc_machine_id_dir_fd = chase_and_open_parent("/etc/machine-id", root, CHASE_PREFIX_ROOT,
                                                      &etc_machine_id_filename);
