...
static int acquire_machine_id(
        ...
        /* Then, try reading the D-Bus machine ID, unless it is a symlink. Skipped for --force: unlike
         * the credential and the container/VM UUID, which a provisioning system supplies for this
         * specific instance, this one is by construction the identity we were asked to replace. */
        if (!skip_dbus) {
...
int machine_id_setup(const char *root, sd_id128_t machine_id, MachineIdSetupFlags flags, sd_id128_t *ret) {
        ...
        if (sd_id128_is_null(machine_id) || FLAGS_SET(flags, MACHINE_ID_SETUP_FORCE_FIRMWARE)) {
                ...
                if (FLAGS_SET(flags, MACHINE_ID_SETUP_FORCE_NEW) && existing_r == -ENOPKG) {
                        log_debug("Machine ID is marked 'uninitialized', leaving it as it is.");
