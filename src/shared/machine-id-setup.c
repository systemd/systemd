...
static int acquire_machine_id(
        ...
        /* Then, try reading the D-Bus machine ID, unless it is a symlink. Both this and the
         * /run/machine-id reuse above are skipped for --force: they hold the identity currently in use
         * or the one baked into the image, i.e. exactly what we were asked to replace. The credential
         * and the container/VM UUID are kept: at this point we cannot tell one baked into an image from
         * one a provisioning system supplied for this very instance, and honouring the latter is the
         * documented behaviour — avoiding the former is a matter of preparing the image right, which
         * machine-id(5) covers. */
        if (!force_new) {
...
static int force_new_check_persistent(const char *etc_machine_id, int fd, const char *root, bool writable) {
        ...
        /* A systemd.machine_id= on the kernel command line outranks /etc/machine-id: PID 1 writes its
         * value back on every boot, so the reboot we tell people to perform would restore the old ID and
         * undo the whole operation. Only knowable for the running system, hence checked here.
         *
         * The value matters, not just the presence: systemd.machine_id=firmware leaves arg_machine_id
         * null, so PID 1 reads the file first and keeps whatever we put there, and a valueless
         * systemd.machine_id is discarded outright. Only something parsing as a non-null ID outranks us. */
...
static void update_stale_machine_ids(const char *root, sd_id128_t machine_id, int etc_fd) {
         ...
         * untrusted tree.
         *
         * This is one-directional: a copy that is not there is not created, so the invariant is "if it
         * exists it agrees with /etc/machine-id", not "it exists".
         *
