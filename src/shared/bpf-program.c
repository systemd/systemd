...
DEFINE_HASH_OPS_WITH_VALUE_DESTRUCTOR(bpf_program_hash_ops, void, trivial_hash_func, trivial_compare_func, BPFProgram, bpf_program_unref);
...
int bpf_program_serialize_attachment(
         ...
         * exlicitly do so, it's not done implicitly on close(). Now, since we are serializing here we don't
...
int bpf_program_deserialize_attachment_set(const char *v, FDSet *fds, Set **bpfsetp) {
        ...
        r = set_ensure_consume(bpfsetp, &bpf_program_hash_ops, TAKE_PTR(p));
        if (r < 0)
                return r;
