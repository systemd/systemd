...
int net_get_unique_predictable_bytes(sd_device *device, bool use_sysname, size_t len, uint8_t *ret) {
        ...
        log_device_debug(device, "Using \"%s\" as stable identifying information", name);
        return net_get_unique_predictable_bytes_from_name(
                        name,
...
int net_verify_hardware_address(
        ...
        if (new_hw_addr->length != arphrd_to_hw_addr_len(iftype)) {
                if (warn_invalid)
                        log_link_warning(&link,
                                         "Specified hardware address with invalid length (%zu, expected %zu), refusing.",
                                         new_hw_addr->length, arphrd_to_hw_addr_len(iftype));
                return -EINVAL;
        }

        if (current_hw_addr && current_hw_addr->length != new_hw_addr->length) {
                if (warn_invalid)
                        log_link_warning(&link,
        ...
        case ARPHRD_INFINIBAND:
                ...
                if (current_hw_addr)
                        ...
                        memzero(new_hw_addr->bytes, INFINIBAND_ALEN - 8);
                break;

        default:
                if (warn_invalid)
                        log_link_warning(&link, "Unsupported interface type %s%u to set hardware address, refusing.",
