...
static bool is_survivor_cgroup(pid_t pid) {
        ...
        r = cg_get_xattr_bool(SYSTEMD_CGROUP_CONTROLLER, cgroup_path, "user.survive_system_transition");
        if (r < 0 && !ERRNO_IS_XATTR_ABSENT(r))
                log_debug_errno(r, "Failed to get user.survive_system_transition xattr of %s, ignoring: %m", cgroup_path);
...
static bool ignore_proc(pid_t pid, bool warn_rootfs) {
        ...
        /* Ignore processes that are part of a cgroup marked with the user.survive_system_transition xattr */
