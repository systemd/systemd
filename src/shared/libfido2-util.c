...
int fido2_list_devices(void) {
        ...
        for (size_t i = 0; i < found; i++) {
                ...
                r = table_add_many(
                                ...
                                TABLE_STRING, yes_no(has_rk),
                                TABLE_STRING, yes_no(has_client_pin),
                                TABLE_STRING, yes_no(has_up),
                                TABLE_STRING, yes_no(has_uv));
                if (r < 0) {
...
int fido2_find_device_auto(char **ret) {
        ...
        r = check_device_is_fido2_with_hmac_secret(sym_fido_dev_info_path(entry), NULL, NULL, NULL, NULL);
        if (r < 0)
                goto finish;
        if (!r) {
