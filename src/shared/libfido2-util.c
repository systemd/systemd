...
static int fido2_common_assert_error_handle(int r) {
        ...
        default:
                return log_error_errno(SYNTHETIC_ERRNO(EIO),
                                        "Unknown FIDO2 error: %s", sym_fido_strerr(r));
        }
...
static int fido2_is_cred_in_specific_token(
        ...
        if (r < 0)
                return log_error_errno(r,
                                       "Failed to set FIDO2 assertion basic properties: %m");
...
static int fido2_use_hmac_hash_specific_token(
        ...
        int err = fido2_common_assert_error_handle(r);
        if (err < 0)
                return err;
...
int fido2_use_hmac_hash(
        ...
        for (size_t i = 0; i < found; i++) {
                ...
                if (IN_SET(r, -ENOANO, -ENOLCK, -EOWNERDEAD)) {
                        /* If it's not the last device, try next one */
                        if (i < found - 1)
                                continue;
                        else {
                                /* -EOWNERDEAD is returned when PIN is blocked. Map it to EAGAIN so users can reinsert
                        ...
                        log_error_errno(r, "Failed to determine whether the credential is in the token, try anyway: %m");
