...
static void plymouth_start_interaction(const char *text, bool *ret_displayed) {
        ...
        if (r < 0) {
                if (!ERRNO_IS_NO_PLYMOUTH(r))
                        log_error_errno(r, "Failed to communicate with plymouth, ignoring: %m");
                return;
        }
...
static void plymouth_end_interaction(bool *displayed) {
        ...
        c = asprintf(&plymouth_message,
                     "M\x02%c%c"
                     "a%c", /* hide messages, resume spinner */
                     1, '\x00',
                     '\x00');
        if (c < 0)
                return (void) log_oom();
