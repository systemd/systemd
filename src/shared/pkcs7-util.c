...
int pkcs7_extract_signers(
        ...
        for (int i = 0; i < n; i++) {
                ...
                if (len < 0)
                        return log_debug_errno(SYNTHETIC_ERRNO(EIO), "Failed to convert issuer name to DER format");
                ...
                if (len < 0)
                        return log_debug_errno(SYNTHETIC_ERRNO(EIO), "Failed to convert serial number to DER format");
