...
static int loop_configure(
        ...
        /* Work around a kernel bug, where the chaning offset/size of the loopback device doesn't correctly
        ...
        if (FLAGS_SET(c->info.lo_flags, LO_FLAGS_DIRECT_IO)) {
                ...
                if (ioctl(fd, LOOP_SET_DIRECT_IO, b) < 0)
                        log_debug_errno(errno, "Failed to enable direct IO mode on loopback device, ignoring: %m");
        }
...
static int loop_device_make_internal(
        ...
        if (FLAGS_SET(loop_flags, LO_FLAGS_DIRECT_IO) != FLAGS_SET(f_flags, O_DIRECT)) {
                ...
                if (direct_io_fd < 0) {
                        if (!FLAGS_SET(loop_flags, LO_FLAGS_DIRECT_IO))
                                return log_debug_errno(errno, "Failed to turn off O_DIRECT for file descriptor: %m");
        ...
        if (FLAGS_SET(loop_flags, LO_FLAGS_DIRECT_IO)) {
                ...
                if (!FLAGS_SET(info.lo_flags, LO_FLAGS_DIRECT_IO))
                        log_debug("Unable to enable direct IO mode, proceeding in buffered IO mode.");
        }
...
int loop_device_make_by_path(
        ...
        fd = open(path, O_CLOEXEC|O_NONBLOCK|O_NOCTTY|(open_flags >= 0 ? open_flags : O_RDWR)|(FLAGS_SET(loop_flags, LO_FLAGS_DIRECT_IO) ? O_DIRECT : 0));
        if (fd < 0 && FLAGS_SET(loop_flags, LO_FLAGS_DIRECT_IO)) /* If we had O_DIRECT on, and things failed with that, let's immediately try again without */
        ...
        if (fd < 0) {
                ...
                fd = open(path, O_CLOEXEC|O_NONBLOCK|O_NOCTTY|O_RDONLY|(FLAGS_SET(loop_flags, LO_FLAGS_DIRECT_IO) ? O_DIRECT : 0));
                if (fd < 0 && FLAGS_SET(loop_flags, LO_FLAGS_DIRECT_IO)) /* as above */
