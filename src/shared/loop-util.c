...
static int fd_has_partition_table(int fd) {
         ...
         * instead. Expects an fd suitable for probing, i.e. opened without O_DIRECT (see open_probe_fd()). */

        r = probe_partition_table(fd, &pttype);
        if (r < 0)
                return log_debug_errno(r, "Failed to probe for partition table: %m");
