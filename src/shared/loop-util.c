...
static int loop_device_make_internal(
        ...
        if (S_ISBLK(st.st_mode)) {
                ...
                if (offset == 0 && IN_SET(size, 0, UINT64_MAX)) {
                        ...
                        int copy_dissect_fd = loop_device_fd_dup_for_dissect(copy);
        ...
        int dissect_fd = loop_device_fd_dup_for_dissect(loop_with_fd);
        if (dissect_fd < 0)
                return dissect_fd;
...
int loop_device_open(const char *loop_path, int open_flags, LoopDevice **ret) {
        ...
        int dissect_fd = loop_device_fd_dup_for_dissect(loop_fd);
        if (dissect_fd < 0)
                return dissect_fd;
