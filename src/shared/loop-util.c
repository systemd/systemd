...
static int loop_device_can_shortcut(
        ...
        if (FLAGS_SET(loop_flags, LO_FLAGS_PARTSCAN)) {
                ...
                if (r < 0)
                        return r;
                if (r == 0) {
                        /* Partscan was requested but isn't enabled. A loop device is only needed if there's
                         * actually a partition table to scan, which is not the case for btrfs subvolumes:
                         * https://github.com/systemd/systemd/issues/42520 */
                        uint32_t discard_ssz;
                        r = probe_sector_size_harder(fd, &discard_ssz);
                        if (r != 0) /* Partition table present (or undetermined), need a real loop device with partscan. */
                                return false;
                }
        }
