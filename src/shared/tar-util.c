...
static void open_inode_done(OpenInode *of) {
        ...
        if (of->acl_access)
                acl_free(of->acl_access);
        if (of->acl_default)
                acl_free(of->acl_default);
}
...
static int open_inode_apply_acl(OpenInode *of) {
        ...
        if (of->acl_access) {
                if (acl_set_fd(of->fd, of->acl_access) < 0)
        ...
        if (of->filetype == S_IFDIR && of->acl_default) {
                ...
                if (acl_set_file(FORMAT_PROC_FD_PATH(of->fd), ACL_TYPE_DEFAULT, of->acl_default) < 0)
...
static int make_tmpfs(void) {
        ...
        if (superblock_fd < 0)
                return log_error_errno(errno, "Failed to allocate tmpfs superblock: %m");

        (void) fsconfig(superblock_fd, FSCONFIG_SET_STRING, "source", "hardlink", 0);
        (void) fsconfig(superblock_fd, FSCONFIG_SET_STRING, "mode", "0700", /* aux= */ 0);

        if (fsconfig(superblock_fd, FSCONFIG_CMD_CREATE, NULL, NULL, 0) < 0)
                return log_error_errno(errno, "Failed to finalize superblock: %m");

        _cleanup_close_ int mount_fd = fsmount(superblock_fd, FSMOUNT_CLOEXEC, MS_NODEV|MS_NOEXEC|MS_NOSUID);
        if (mount_fd < 0)
                return log_error_errno(errno, "Failed to turn tmpfs superblock into mount: %m");
...
static int hardlink_lookup(
        ...
        if (d->hardlink_db_fd < 0) {
                ...
                if (d->hardlink_db_fd < 0) {
                        ...
                        d->hardlink_db_fd = mkdtemp_open("/var/tmp/make-tar-XXXXXX", /* flags= */ 0, &d->hardlink_db_path);
                        if (d->hardlink_db_fd < 0)
...
static int archive_generate_sparse(
                struct archive_entry *entry,
                int fd) {
        ...
        for (;;) {
                ...
                if (h < 0) {
                        ...
                        if (e > c && c != 0) {
                                log_debug("final sparse block %li…%li", c, e);
                ...
                if (h > c) {
                        log_debug("inner sparse block %li…%li (%li)", c, h, h - c);
...
static int archive_write_acl(
        ...
        acl_entry_t e;
        r = acl_get_entry(acl, ACL_FIRST_ENTRY, &e);
        for (;;) {
                if (r < 0)
...
static int archive_item(
        ...
        if (inode_type_can_acl(sx->stx_mode)) {
                r = acl_extended_file(FORMAT_PROC_FD_PATH(inode_fd));
                if (r < 0 && !ERRNO_IS_NOT_SUPPORTED(errno))
                        return log_error_errno(errno, "Failed check if '%s' has ACLs: %m", path);
                if (r > 0) {
                        _cleanup_(acl_freep) acl_t acl = NULL;
                        acl = acl_get_file(FORMAT_PROC_FD_PATH(inode_fd), ACL_TYPE_ACCESS);
                        if (!acl)
                        ...
                        if (S_ISDIR(sx->stx_mode)) {
                                ...
                                acl = acl_get_file(FORMAT_PROC_FD_PATH(inode_fd), ACL_TYPE_DEFAULT);
                                if (!acl)
