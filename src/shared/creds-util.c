...
static int pick_up_credential_one(
        ...
        r = copy_file_at(
                        ...
                        /* flags= */ 0);
        if (r < 0)
                return log_warning_errno(r, "Failed to copy credential %s → file %s%s%s: %m",
                                         credential_name, target_dir, endswith(target_dir, "/") ? "" : "/", fn);

        log_info("Installed %s%s%s from credential.", target_dir, endswith(target_dir, "/") ? "" : "/", fn);
        return 1;
}
