...
struct credential_host_secret_format {
         ...
         * gets its own secrent, even people forget to flush out this secret file. Hence we bind it to the
...
int encrypt_credential_and_warn(
        ...
        _cleanup_free_ void *tpm2_blob = NULL, *tpm2_policy_hash = NULL, *iv = NULL, *tag = NULL, *output = NULL;
        ...
        if (try_tpm2 ||
            sd_id128_in_set(with_key, CRED_AES256_GCM_BY_TPM2_HMAC, CRED_AES256_GCM_BY_HOST_AND_TPM2_HMAC)) {
                ...
                if (ERRNO_IS_NOT_SUPPORTED(r) && sd_id128_is_null(with_key))
                        log_debug_errno(r, "TPM2 ot supported, not using.");
                else if (r < 0)
                        return r;
        ...
        assert(cc = EVP_aes_256_gcm());

        ks = EVP_CIPHER_key_length(cc);
        assert(ks == sizeof(md));

        bs = EVP_CIPHER_block_size(cc);
        assert(bs > 0);
        assert((size_t) bs <= CREDENTIAL_FIELD_SIZE_MAX);

        ivs = EVP_CIPHER_iv_length(cc);
        if (ivs > 0) {
                assert((size_t) ivs <= CREDENTIAL_FIELD_SIZE_MAX);

                iv = malloc(ivs);
                if (!iv)
                        return log_oom();

                r = genuine_random_bytes(iv, ivs, RANDOM_BLOCK);
                if (r < 0)
                        return log_error_errno(r, "Failed to acquired randomized IV: %m");
        }

        ts = 16; /* FIXME: On OpenSSL 3 there is EVP_CIPHER_CTX_get_tag_length(), until then let's hardcode this */
        ...
        output_size =
                offsetof(struct encrypted_credential_header, iv) +
                ivs +
                (tpm2_key ? offsetof(struct tpm2_credential_header, policy_hash_and_blob) + tpm2_blob_size + tpm2_policy_hash_size: 0) +
                offsetof(struct metadata_credential_header, name) + strlen_ptr(name) +
                input_size +
                2U * (size_t) bs +
                ts;
        ...
        h->id = id;
        h->block_size = htole32(bs);
        h->key_size = htole32(ks);
        h->tag_size = htole32(ts);
        h->iv_size = htole32(ivs);
        memcpy(h->iv, iv, ivs);

        p = offsetof(struct encrypted_credential_header, iv) + ivs;
        ...
        if (tpm2_key) {
                ...
                p += offsetof(struct tpm2_credential_header, policy_hash_and_blob) + tpm2_blob_size + tpm2_policy_hash_size;
        }

        /* Pass the encrypted + TPM2 header as AAD */
        if (EVP_EncryptUpdate(context, NULL, &added, output, p) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to write AAD data");

        /* Now construct the metadata header */
        ml = strlen_ptr(name);
        m = malloc(offsetof(struct metadata_credential_header, name) + ml);
        if (!m)
                return log_oom();
        ...
        if (EVP_EncryptUpdate(context, (uint8_t*) output + p, &added, (const unsigned char*) m, offsetof(struct metadata_credential_header, name) + ml) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to encrypt metadata header");

        assert(added >= 0);
        assert((size_t) added <= output_size - p);
        p += added;

        /* The encrypt the plaintext */
        if (EVP_EncryptUpdate(context, (uint8_t*) output + p, &added, input, input_size) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to encrypt data");
        ...
        if (EVP_EncryptFinal_ex(context, (uint8_t*) output + p, &added) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to finalize data encryption");

        assert(added >= 0);
        assert((size_t) added <= output_size - p);
        p += added;

        assert(p <= output_size - ts);

        /* Append tag */
        if (EVP_CIPHER_CTX_ctrl(context, EVP_CTRL_GCM_GET_TAG, ts, (uint8_t*) output + p) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to get tag");
...
int decrypt_credential_and_warn(
        ...
        _cleanup_(EVP_CIPHER_CTX_freep) EVP_CIPHER_CTX *context = NULL;
        size_t host_key_size = 0, tpm2_key_size = 0, plaintext_size, p;
        struct encrypted_credential_header *h;
        ...
        /* Ensure we have space for the full header now */
        if (input_size <
            offsetof(struct encrypted_credential_header, iv) +
            le32toh(h->iv_size) +
            (with_tpm2 ? offsetof(struct tpm2_credential_header, policy_hash_and_blob) : 0) +
            offsetof(struct metadata_credential_header, name) +
            le32toh(h->tag_size))
                return log_error_errno(SYNTHETIC_ERRNO(EBADMSG), "Encrypted file too short.");

        p = offsetof(struct encrypted_credential_header, iv) + le32toh(h->iv_size);
        ...
        if (with_tpm2) {
                ...
                if (input_size <
                    offsetof(struct encrypted_credential_header, iv) +
                    le32toh(h->iv_size) +
                    offsetof(struct tpm2_credential_header, policy_hash_and_blob) +
                    le32toh(t->blob_size) +
                ...
                p += offsetof(struct tpm2_credential_header, policy_hash_and_blob) +
                        le32toh(t->blob_size) +
                        le32toh(t->policy_hash_size);
#else
        ...
        if (EVP_CIPHER_CTX_ctrl(context, EVP_CTRL_GCM_SET_IVLEN, le32toh(h->iv_size), NULL) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to set IV size on decryption context.");

        if (EVP_DecryptInit_ex(context, NULL, NULL, md, h->iv) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to set IV and key.");
        ...
        if (EVP_DecryptUpdate(
                ...
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to decrypt data.");
        ...
        if (EVP_CIPHER_CTX_ctrl(context, EVP_CTRL_GCM_SET_TAG, le32toh(h->tag_size), (uint8_t*) input + input_size - le32toh(h->tag_size)) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to set tag.");

        if (EVP_DecryptFinal_ex(context, (uint8_t*) plaintext + plaintext_size, &added) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EBADMSG), "Decryption failed (incorrect key?).");

        plaintext_size += added;

        if (plaintext_size < offsetof(struct metadata_credential_header, name))
        ...
        if (plaintext_size < offsetof(struct metadata_credential_header, name) + le32toh(m->name_size))
                return log_error_errno(SYNTHETIC_ERRNO(EBADMSG), "Metadata header incomplete.");
        ...
        if (ret) {
                ...
                without_metadata = memdup((uint8_t*) plaintext + offsetof(struct metadata_credential_header, name) + le32toh(m->name_size),
                                          plaintext_size - offsetof(struct metadata_credential_header, name) - le32toh(m->name_size));
                if (!without_metadata)
                        return log_oom();
        ...
        if (ret_size)
                *ret_size = plaintext_size - offsetof(struct metadata_credential_header, name) - le32toh(m->name_size);
