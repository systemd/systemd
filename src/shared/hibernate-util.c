...
static SwapEntry* swap_entry_free(SwapEntry *entry) {
        if (!entry)
                return NULL;
...
static int swap_entry_get_resume_config(SwapEntry *swap) {
        ...
        if (r > 0)
                return btrfs_get_file_physical_offset_fd(fd, &swap->offset);
        ...
        swap->offset = fiemap->fm_extents[0].fe_physical / page_size();
        return 0;
}
...
static int read_swap_entries(SwapEntries *ret) {
        ...
        for (;;) {
                ...
                if (k == EOF)
                        break;
                if (k != 5)
                        return log_debug_errno(SYNTHETIC_ERRNO(EIO), "Failed to parse /proc/swaps: %m");

                if (streq(type, "file")) {
                        if (endswith(swap->path, "\\040(deleted)")) {
                                log_debug("Swap file '%s' is deleted, ignoring.", swap->path);
                                continue;
                        }
                        ...
                        if (node && startswith(node, "zram")) {
                                log_debug("Swap partition '%s' is zram device, ignoring.", swap->path);
                                continue;
                        }
                ...
                entries.swaps[entries.n_swaps++] = TAKE_PTR(swap);
        }
...
int find_suitable_hibernate_device_full(HibernateDevice *ret_device, uint64_t *ret_size, uint64_t *ret_used) {
        ...
        if (entries.n_swaps == 0)
                return log_debug_errno(SYNTHETIC_ERRNO(ENOENT), "No swap space available for hibernation found.");

        FOREACH_ARRAY(i, entries.swaps, entries.n_swaps) {
                SwapEntry *swap = ASSERT_PTR(*i);
                ...
                if (resume_config_devno > 0) {
                        if (swap->devno == resume_config_devno &&
                            (!swap->swapfile || swap->offset == resume_config_offset || resume_config_offset == UINT64_MAX)) {
