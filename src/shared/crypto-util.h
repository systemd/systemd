...
int parse_openssl_certificate_source_argument(
                const char *argument, char **certificate_source, CertificateSourceType *certificate_source_type);
...
extern DLSYM_PROTOTYPE(X509_STORE_set_default_paths);

extern DLSYM_PROTOTYPE(TS_REQ_to_TS_VERIFY_CTX);
extern DLSYM_PROTOTYPE(TS_VERIFY_CTX_set0_store);
extern DLSYM_PROTOTYPE(TS_VERIFY_CTX_add_flags);
extern DLSYM_PROTOTYPE(TS_VERIFY_CTX_free);
extern DLSYM_PROTOTYPE(TS_RESP_verify_response);

extern DLSYM_PROTOTYPE(TS_TST_INFO_get_time);
...
DEFINE_TRIVIAL_CLEANUP_FUNC_FULL_RENAME(
                ASN1_OCTET_STRING *, sym_ASN1_OCTET_STRING_free, ASN1_OCTET_STRING_freep, NULL);
...
DEFINE_TRIVIAL_CLEANUP_FUNC_FULL_RENAME(
                PKCS7_SIGNER_INFO *, sym_PKCS7_SIGNER_INFO_free, PKCS7_SIGNER_INFO_freep, NULL);
...
#        define sym_sk_X509_new_null() ((STACK_OF(X509) *) sym_OPENSSL_sk_new_null())
...
static inline int openssl_digest(
                const char *digest_alg, const void *buf, size_t len, void **ret_digest, size_t *ret_digest_size) {
...
static inline int
                openssl_hmac(const char *digest_alg,
                             const void *key,
                             size_t key_size,
                             const void *buf,
                             size_t len,
                             void **ret_digest,
                             size_t *ret_digest_size) {
        return openssl_hmac_many(
                        digest_alg, key, key_size, &IOVEC_MAKE((void *) buf, len), 1, ret_digest, ret_digest_size);
}
...
static inline int
                openssl_cipher(const char *alg,
                               ...
                               void **ret,
                               size_t *ret_size) {
        return openssl_cipher_many(
                        alg, bits, mode, key, key_size, iv, iv_size, &IOVEC_MAKE((void *) buf, len), 1, ret, ret_size);
}
...
#define ARGON2ID_PARAMETERS_DEFAULT                                                \
        (Argon2IdParameters) {                                                     \
                .memcost_bytes = 64ULL * 1024 * 1024, .iterations = 8, .lanes = 4, \
        }
