...
static int journal_auth_setup(JournalAuthContext *c) {
        ...
        c->hmac = sym_EVP_MAC_fetch(NULL, "HMAC", NULL);
        if (!c->hmac)
                return log_openssl_errors(LOG_DEBUG, "EVP_MAC_fetch() failed");

        c->hmac_ctx = sym_EVP_MAC_CTX_new(c->hmac);
        if (!c->hmac_ctx)
        ...
        c->ossl_params = sym_OSSL_PARAM_BLD_to_param(bld);
        if (!c->ossl_params)
...
static int journal_auth_append_tag_maybe(JournalAuthContext *c, JournalFile *f, uint64_t realtime) {
        int r;

        assert(c);
        assert(f);

        if (c->fss_start_usec == 0 || c->fss_interval_usec == 0)
                return -EOPNOTSUPP;

        if (realtime <= 0)
                realtime = now(CLOCK_REALTIME);

        if (realtime < c->fss_start_usec)
                return -ESTALE;

        uint64_t goal = (realtime - c->fss_start_usec) / c->fss_interval_usec;
...
static JournalAuthOps journal_auth_ops = {
