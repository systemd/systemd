...
static int mac_smack_label_pre(int dir_fd, const char *path, mode_t mode, LabelContext *userdata) {
        return 0;
}

static int mac_smack_label_post(int dir_fd, const char *path, bool created, LabelContext *userdata) {
        if (!created)
                return 0;
