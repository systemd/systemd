...
int mac_smack_read_at(int fd, const char *path, SmackAttr attr, char **ret) {
        ...
        return getxattr_at_malloc(fd, path, smack_attr_to_string(attr), ret);
}
...
int mac_smack_apply_at(int fd, const char *path, SmackAttr attr, const char *label) {
        ...
        return xsetxattr(fd, path, /* at_flags = */ 0, smack_attr_to_string(attr), label, SIZE_MAX);
}
...
static int smack_fix_fd(
        ...
        r = xsetxattr(fd, /* path = */ NULL, AT_EMPTY_PATH, "security.SMACK64", label, SIZE_MAX);
