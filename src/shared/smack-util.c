...
static int mac_smack_label_pre(int dir_fd, const char *path, mode_t mode, void *userdata) {
        return 0;
}

static int mac_smack_label_post(int dir_fd, const char *path, bool created, void *userdata) {
        if (!created)
                return 0;
