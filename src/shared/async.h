...
/* This contains various potentially slow operations that are executed asynchronously. These are carefully
 * written to not use pthreads, but via fork() or clone() (without CLONE_VM) so that they do not share any
 * memory with the parent process, and thus cannot possibly interfere with the malloc() synchronization
 * locks.
 *
 * Background: glibc only synchronizes malloc() locks when doing fork(), but not when doing clone()
 * (regardless if through glibc's own wrapper or our own). This means if another thread in the parent has the
