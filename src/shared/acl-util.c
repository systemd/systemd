...
int parse_acl(
                ...
                acl_t *ret_acl_access_cond_exec,
                acl_t *ret_acl_default,
                bool want_mask) {

        _cleanup_free_ char **a = NULL, **e = NULL, **d = NULL; /* strings are not freed */
        ...
        assert(ret_acl_access_cond_exec);
        ...
        STRV_FOREACH(entry, split) {
                ...
                n = strv_split_full(&entry_split, *entry, ":", EXTRACT_RETAIN_SEPARATORS|EXTRACT_DONT_COALESCE_SEPARATORS|EXTRACT_RETAIN_ESCAPE);
                if (n < 0)
                        return -ENOMEM;
                ...
                p = STARTSWITH_SET(entry_join, "default:", "d:");
                if (p)
                        r = strv_push(&d, p);
                else if (!streq(*entry, entry_join))
                        r = strv_push(&e, entry_join);
                else
                        r = strv_push(&a, *entry);
                if (r < 0)
                        return r;
        ...
        *ret_acl_access_cond_exec = TAKE_PTR(e_acl);
