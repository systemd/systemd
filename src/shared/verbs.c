...
const Verb* _verbs_find_command(
        ...
        for (const Verb *verb = verbs; verb < verbs_end; verb++) {
                ...
                if (ret_cmd)
                        *ret_cmd = (const CommandDescription*) ASSERT_PTR(verb->data);
                return verb;
        }
...
int _command_print_help_full(
        ...
        const CommandDescription *cmd;
        const Verb *cmdverb = _verbs_find_command(verbs, verbs_end, name, &cmd);
        if (!cmdverb)
        ...
        const char *a = ansi_highlight(), *b = ansi_add_italics();
        const char *c = a ? strjoina(a, b) : b;

        r = print_wrapped(cmd->abstract, c);
        if (r < 0)
                return r;
