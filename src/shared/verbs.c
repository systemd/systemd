...
int _verbs_get_help_table(
        ...
        const Verb *verb;
        for (verb = verbs; verb < verbs_end; verb++) {
                if (verb == verbs && FLAGS_SET(verb->flags, VERB_COMMAND_MARKER))
                        continue;
...
static int print_man_links(const char *pages) {
        ...
        NULSTR_FOREACH(page, pages) {
                ...
                r = terminal_urlify_man(page, /* suffix= */ NULL, &link);
                if (r < 0)
                        return r;
...
int _command_print_help(
        ...
        if (cmd->argspec)
                ...
                bool have_verbs = verbs_end > cmdverb + 1 && !FLAGS_SET(cmdverb[1].flags, VERB_COMMAND_MARKER);
...
static int verb_build_json(const Verb *verb, sd_json_variant **ret) {
         ...
         * Specification. The "minArguments", "maxArguments", "isDefault", and "isOnlineOnly" fields
         * are extensions not (yet) covered by the specification. Note that the argument counts
