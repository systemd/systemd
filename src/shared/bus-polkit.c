...
static AsyncPolkitQuery *async_polkit_query_free(AsyncPolkitQuery *q) {
         ...
         * calling this is only allowed from async_polkit_queries_create_query() and async_polkit_queries_free(). */
        assert(!q->parent);
...
static int async_polkit_queries_create_query(
...
int bus_verify_polkit_async(
        ...
        r = async_polkit_queries_create_query(qs, call, action, details, &q);
        if (r < 0)
                return r;
