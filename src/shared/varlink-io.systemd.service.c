...
static SD_VARLINK_DEFINE_METHOD(
                ...
                SD_VARLINK_FIELD_COMMENT("The current maximum log level, represented as a string."),
                SD_VARLINK_DEFINE_OUTPUT(levelString, SD_VARLINK_STRING, 0));
...
int varlink_method_get_log_level(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        return sd_varlink_replybo(
                        link,
                        SD_JSON_BUILD_PAIR_INTEGER("level", level),
                        SD_JSON_BUILD_PAIR_STRING("levelString", level_str));
}
