...
DEFINE_HASH_OPS_WITH_VALUE_DESTRUCTOR(config_file_hash_func_fclose,
...
static int config_parse_many_files(
        ...
        STRV_FOREACH(fn, files) {
                ...
                if (fd >= 0) {
                        ...
                        /* Get inodes for all drop-ins. Later we'll verify if main config is a symlink
                         * to one of them. If so, we skip reading main config file directly. */
