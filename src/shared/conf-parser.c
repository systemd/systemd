...
int config_parse_iec_size(
        ...
        if (r >= 0 && (uint64_t) (size_t) v != v)
                r = -ERANGE;
        if (r < 0)
                return LOG_SYNTAX_PARSE_ERROR(r);
...
int config_parse_bool(
        ...
        if (r < 0) {
                LOG_SYNTAX_PARSE_ERROR_FULL(r, fatal);
...
int config_parse_strv(
        ...
        r = strv_extend_strv(sv, strv, /* filter_duplicates = */ true);
        if (r < 0)
                return log_oom();
...
int config_parse_log_facility(
        ...
        if (x < 0)
                return LOG_SYNTAX_PARSE_ERROR(x);
...
int config_parse_personality(
        ...
        if (p == PERSONALITY_INVALID)
                return LOG_SYNTAX_PARSE_ERROR(0);
...
int config_parse_unsigned_bounded(
        ...
        if (r < 0)
                return LOG_SYNTAX_PARSE_ERROR_FULL(r, !ignoring);
