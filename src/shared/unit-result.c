...
int unit_result_query(sd_bus *bus, const char *unit_path, UnitResult *ret, sd_bus_error *reterr_error) {
        assert(bus);
        assert(unit_path);
        assert(ret);

        /* Fills in the UnitResult structure from the properties of the specified unit object, covering all
         * its interfaces. Note that a full GetAll() query is necessary for this, since the various resource
         * accounting properties are not included in PropertiesChanged signals. */

        return bus_map_all_properties(
                        bus,
...
int unit_result_show(const UnitResult *u, FILE *f) {
        ...
        if (u->exit_code > 0) {
                ...
                r = table_add_cell_stringf(
                                ...
                                /* ret_cell= */ NULL,
                                "code=%s, status=%i/%s",
                                sigchld_code_to_string(u->exit_code),
                                u->exit_status,
...
int unit_result_to_exit_status(const UnitResult *u) {
        ...
        * to mean "success". */
         ...
         * state instead. */
        if (isempty(u->result) && streq_ptr(u->active_state, "inactive"))
                return EXIT_SUCCESS;
