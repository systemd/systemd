...
typedef struct AskPasswordUserInterface AskPasswordUserInterface;
...
int openssl_load_key_from_token(KeySourceType private_key_source_type, const char *private_key_source, const char *private_key, AskPasswordUserInterface *ui, EVP_PKEY **ret_private_key);
...
struct AskPasswordUserInterface {
...
static inline AskPasswordUserInterface* ask_password_user_interface_free(AskPasswordUserInterface *ui) {
        if (!ui)
                return NULL;
...
DEFINE_TRIVIAL_CLEANUP_FUNC_FULL(AskPasswordUserInterface*, ask_password_user_interface_free, NULL);
