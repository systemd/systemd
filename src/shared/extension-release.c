...
int extension_release_validate(
        ...
        /* When the arch field is present and not '_any' it must match the host - for now just look at uname but in
         * the future we could check if the kernel also supports 32 bit or binfmt has a translator set up for the arch */
        extension_arch = strv_env_pairs_get(extension_release, "ARCHITECTURE");
        if (!isempty(extension_arch) && !streq(extension_arch, "_any") && !streq(architecture_to_string(uname_architecture()), extension_arch)) {
        ...
        if (!streq(host_os_release_id, extension_release_id) && !streq(extension_release_id, "_any")) {
