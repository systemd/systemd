...
int extension_release_validate_internal(
        ...
        if (host_scope) {
                ...
                scope = strv_env_pairs_get(release_file, syscfg_or_sysext_scope);
                if (scope) {
        ...
        syscfg_or_sysext_architecture = strv_env_pairs_get(release_file, "ARCHITECTURE");
        ...
        syscfg_or_sysext_release_id = strv_env_pairs_get(release_file, "ID");
        ...
        release_syscfg_or_sysext_level = strv_env_pairs_get(release_file, syscfg_or_sysext_level);
        if (!isempty(host_os_release_level) && !isempty(release_syscfg_or_sysext_level)) {
                ...
                syscfg_or_extension_release_version_id = strv_env_pairs_get(release_file, "VERSION_ID");
