...
static void log_setup_nss_internal(void) {
        ...
        if (e) {
                /* NSS plugins are linked statically to all of our libraries so this will only override the
                 * log level for the NSS plugin, and not for the entire systemd binary, since each will have
                 * their own log_level TLS variable. */
