...
int acquire_tpm2_key(
        ...
        if (!(flags & TPM2_FLAGS_USE_PIN)) {
                ...
                if (r == -EUCLEAN) {
                        log_warning_errno(r, "PCR values kept changing while unsealing the TPM2 secret, giving up. Something on this system extends a PCR continuously.");
                        /* Normalize to -EPERM, just like the policy mismatch case below. */
                        return -EPERM;
                }
