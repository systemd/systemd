...
static int unit_freezer_action(UnitFreezer *f, bool freeze) {
        ...
        r = bus_call_method(f->bus, bus_systemd_mgr,
                            ...
                            f->name);
        if (r < 0) {
                if (sd_bus_error_has_names(&error, SD_BUS_ERROR_NOT_SUPPORTED, BUS_ERROR_UNIT_INACTIVE)) {
                        log_debug_errno(r, "%s: %s", f->name, bus_error_message(&error, r));
                        return 0;
                }
        ...
        log_info("Successfully %s unit '%s'.", freeze ? "froze" : "thawed");
        return 1;
}
