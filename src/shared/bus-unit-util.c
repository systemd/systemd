...
void unit_freezer_cancel(UnitFreezer *f) {
        if (!f)
                return;
...
int unit_freezer_freeze(const char *name, UnitFreezer *ret) {
        _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
        _cleanup_(unit_freezer_cancel) UnitFreezer f = UNIT_FREEZER_NULL;
        int r;

        assert(ret);

        r = unit_freezer_restore(name, &f);
        if (r < 0)
                return r;
...
int unit_freezer_thaw(UnitFreezer *_f) {
        _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
        _cleanup_(unit_freezer_cancelp) UnitFreezer *f = TAKE_PTR(_f);
        int r;
