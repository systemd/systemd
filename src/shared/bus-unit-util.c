...
#define DEFINE_BUS_APPEND_PARSE_PTR(sd_type, cast_type, type, parse_func)                            \
        static int bus_append_##parse_func(sd_bus_message *m, const char *field, const char *eq) {   \
                ...
                if (r < 0)                                                                           \
                        return log_error_errno(r, "Failed to parse %s=%s: %m", field, eq);           \
                                                                                                     \
                r = sd_bus_message_append(m, "(sv)", field, sd_type, (cast_type) val);               \
                if (r < 0)                                                                           \
...
#define DEFINE_BUS_APPEND_PARSE(sd_type, parse_func)                                                 \
        static int bus_append_##parse_func(sd_bus_message *m, const char *field, const char *eq) {   \
                int r;                                                                               \
                                                                                                     \
                r = parse_func(eq);                                                                  \
                if (r < 0) {                                                                         \
                        ...
                        return -EINVAL;                                                              \
                                                                                                     ...
                                                                                                     \
                r = sd_bus_message_append(m, "(sv)", field, sd_type, (int32_t) r);                   \
                if (r < 0)                                                                           \
                        return bus_log_create_error(r);                                              \
                                                                                                     \
                return 1;                                                                            \
...
static int bus_append_execute_property(sd_bus_message *m, const char *field, const char *eq) {
        ...
        if (streq(field, "StandardInputText")) {
                ...
                r = sd_bus_message_append_basic(m, 's', "StandardInputData");
                if (r < 0)
                        return bus_log_create_error(r);
        ...
        if (streq(field, "CPUAffinity")) {
                ...
                if (ncpus < 0)
                        return log_error_errno(ncpus, "Failed to parse %s value: %s", field, eq);
        ...
        if (streq(field, "SystemCallFilter")) {
                int whitelist = 1;
        ...
        if (streq(field, "RestrictAddressFamilies")) {
                int whitelist = 1;
                const char *p = eq;
