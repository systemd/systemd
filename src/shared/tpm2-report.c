...
static int tpm2_generate_report_try(
        ...
        _cleanup_(iovec_done) struct iovec quoted = {};
        ...
        components[n_components++] = (Tpm2ReportComponent) {
                ...
                .attest_info = TAKE_STRUCT(quoted),
        ...
        FOREACH_ARRAY(n, nv_pcrs, n_nv_pcrs) {
                ...
                components[n_components++] = (Tpm2ReportComponent) {
                        ...
                        .attest_info = TAKE_STRUCT(certify_info),
                        .signature = *signature,
                };
        }
        ...
        r = tpm2_unmarshal_attestation(audit_info.iov_base, audit_info.iov_len, &audit_info_decoded);
        if (r < 0)
                return r;

        if (audit_info_decoded.type != TPM2_ST_ATTEST_SESSION_AUDIT)
                return log_debug_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                       "Unexpected audit session attestation tag 0x%" PRIx16, audit_info_decoded.type);
        if (audit_info_decoded.attested.sessionAudit.exclusiveSession == TPM2_NO)
        ...
        components[n_components++] = (Tpm2ReportComponent) {
                ...
                .attest_info = TAKE_STRUCT(audit_info),
...
int tpm2_generate_report(
        ...
        _cleanup_(Esys_Freep) TPM2B_PUBLIC *pubkey = NULL;
        if (key) {
                r = tpm2_read_public(c, /* session= */ NULL, key, &pubkey, /* ret_name= */ NULL, /* ret_qname= */ NULL);
                if (r < 0)
                ...
                if ((pubkey->publicArea.objectAttributes & (TPMA_OBJECT_RESTRICTED | TPMA_OBJECT_SIGN_ENCRYPT)) != (TPMA_OBJECT_RESTRICTED | TPMA_OBJECT_SIGN_ENCRYPT))
        ...
        for (int i = MAX_REPORT_GENERATE_RETRIES;; i--) {
                ...
                *report = (Tpm2Report) {
                        ...
                        .public_key = pubkey ? *pubkey : (TPM2B_PUBLIC) {},
