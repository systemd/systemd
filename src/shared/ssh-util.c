...
int generate_ssh_keypair_full(
        ...
        if (sym_ssh_pki_generate(key_type, param, &key) != SSH_OK)
                return log_debug_errno(SYNTHETIC_ERRNO(EOPNOTSUPP), "Failed to generate %s SSH key.", sym_ssh_key_type_to_char(key_type));

        /* returns a full key that can be saved to a file */
        r = sym_ssh_pki_export_privkey_base64(key, /* passphrase= */ NULL, /* auth_fn= */ NULL, /* auth_data= */ NULL, &b64_privkey);
        if (r != SSH_OK)
        ...
        if (sym_ssh_pki_export_pubkey_base64(key, &b64_pubkey) != SSH_OK)
        ...
        full_pubkey = strjoin(sym_ssh_key_type_to_char(key_type), " ", b64_pubkey);
        if (!full_pubkey)
...
int ssh_key_type_from_string(const char *s, SshKeyType *ret) {
        ...
        SshKeyType kt = sym_ssh_key_type_from_name(s);
...
const char* ssh_key_type_to_string(SshKeyType kt) {
        ...
        return sym_ssh_key_type_to_char(kt);
}
