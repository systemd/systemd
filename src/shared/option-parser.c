...
static bool option_takes_arg(const OptionData *opt) {
        return opt->metavar;
}
static bool option_arg_optional(const OptionData *opt) {
        return option_takes_arg(opt) && FLAGS_SET(opt->flags, OPTION_OPTIONAL_ARG);
}
static bool option_arg_required(const OptionData *opt) {
        return option_takes_arg(opt) && !FLAGS_SET(opt->flags, OPTION_OPTIONAL_ARG);
}
...
int option_parse(
                const OptionData options_start[],
                const OptionData options_end[],
                OptionParser *state,
                int argc, char *argv[],
                const OptionData **ret_option,
                const char **ret_arg) {
        ...
        if (ret_option)
                /* Return the matched OptionData structure to allow the caller to "know" what was matched */
                *ret_option = option;
        *ret_arg = optval;
        return option->id;
}
...
int _option_parser_get_help_table(
        ...
        for (const OptionData *opt = options_start; opt < options_end; opt++) {
