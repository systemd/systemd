...
typedef enum OptionFlags {
        OPTION_OPTIONAL_ARG  = 0x1,  /* Same as optional_argument in getopt */
        OPTION_STOPS_PARSING = 0x2,  /* This option acts like "--" */
        OPTION_GROUP_MARKER  = 0x3,  /* Fake option entry to separate groups */
} OptionFlags;
...
#define _OPTION(counter, fl, sc, lc, mv, h)                             \
        ...
        static const OptionData CONCATENATE(option, counter) = {        \
                .id = 0x100 + counter,                                  \
                .flags = fl,                                            \
                .short_code = sc,                                       \
                .long_code = lc,                                        \
 ...
 */
#define OPTION_________________GROUP_________________(gr)               \
...
extern const OptionData __start_getopt[];
extern const OptionData __stop_getopt[];
...
typedef struct OptionParser {
        ...
        int positional_offset;  /* Offset to where positional parameters are. After processing has been finished,
                                 * all options and their args are to the left of this offset. */
...
#define FOREACH_OPTION_FULL(parser, opt, argc, argv, ret_o, ret_a, on_error) \
        for (int opt; (opt = option_parse(ALIGN_PTR(__start_getopt), __stop_getopt, parser, argc, argv, ret_o, ret_a)) != 0; ) \
                if (opt < 0) {                                                  \
                        on_error;                                               \
                        break;                                                  \
                } else
...
#define option_parser_get_help_table_group(group, ret)                   \
        _option_parser_get_help_table(ALIGN_PTR(__start_getopt), __stop_getopt, group, ret)
