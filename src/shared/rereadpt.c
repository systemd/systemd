...
static int process_partition(
        ...
        if (nr < 0)
                return log_debug_errno(errno_or_else(errno), "Failed to read partition number of partition: %m");

        log_debug("Processing partition %i...", nr);

        errno = 0;
        blkid_loff_t start = sym_blkid_partition_get_start(pp);
        if (start < 0)
                return log_debug_errno(errno_or_else(errno), "Failed to read partition start offset of partition %i: %m", nr);
        assert((uint64_t) start < UINT64_MAX / 512U);

        errno = 0;
        blkid_loff_t size = sym_blkid_partition_get_size(pp);
        if (size < 0)
                return log_debug_errno(errno_or_else(errno), "Failed to read partition size of partition %i: %m", nr);
        ...
        for (sd_device *partition = sd_device_enumerator_get_device_first(e);
             ...
             partition = sd_device_enumerator_get_device_next(e)) {
                ...
                if (start_kernel != (uint64_t) start) {
                        ...
                        log_debug("Removing and recreating partition %i...", nr);
                        ...
                        /* And now add it */
                        log_debug("Successfully removed kernel partition '%s' in order to recreate it.", devname);
                        break;
                }

                /* If the start offsize doesn't change we can just resize the partition */
                log_debug("Resizing partition %i...", nr);
                ...
                log_debug("Successfully resized kernel partition '%s' to match partition table.", devname);
                *changed = true;
                return 1;
        }

        log_debug("Adding partition %i...", nr);
        ...
        log_debug("Successfully added kernel partition '%i' to match partition table.", nr);
        *changed = true;
        return 1;
}
...
static int remove_partitions(sd_device *d, int fd, sd_device_enumerator *e, Set *partnos, bool *changed) {
        ...
        for (sd_device *partition = sd_device_enumerator_get_device_first(e);
             ...
             partition = sd_device_enumerator_get_device_next(e)) {
                ...
                log_debug("Kernel knows partition %u which we didn't find, removing.", nr);
...
static int rereadpt_full(sd_device *dev, int fd, RereadPartitionTableFlags flags) {
        ...
        if (!pl)
                return log_device_debug_errno(dev, errno_or_else(EIO), "Unable read partition table of '%s': %m", p);
        ...
        log_debug("Updating/adding kernel partitions...");
        ...
        log_debug("Removing old kernel partitions...");
