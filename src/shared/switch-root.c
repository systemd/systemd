 ...
 * (e.g. sshfs, rclone, gvfs, ...): those are backed by an arbitrary userspace daemon that could hang
 * indefinitely if wedged, dead, or otherwise unresponsive. Note that 'fuseblk' (e.g. ntfs-3g, exfat-fuse)
 * is deliberately *not* excluded here: unlike the above it wraps an actual block device, and carries the
 * same (bounded) sync latency as any other block device backed file system already does here — no worse
 * than what the blanket sync() we're replacing already risked, so there is no good reason to sacrifice its
 * data safety by skipping it.
 *
 * Similarly, '9p' is not excluded either, even though fstype_is_blockdev_backed() (which exists for a
...
static bool fstype_worth_syncing(const char *fstype) {
        ...
        if (streq(fstype, "fuse") || startswith(fstype, "fuse."))
                return false;
...
static void sync_departing_file_systems(const char *new_root) {
        ...
        for (;;) {
                 ...
                 * not be the (possibly departing) one this specific entry refers to. Skip if this entry is
                 * currently shadowed, so that we don't sync the wrong (and possibly already handled, or
                 * not departing at all) superblock under the false impression we synced this entry. This
                 * uses the same check get_sub_mounts() already does for the same reason. */
