...
static bool fstype_worth_syncing(const char *fstype) {
        ...
        bool is_cluster_fs = STR_IN_SET(fstype, "gfs", "gfs2", "ocfs2");
        if (!is_cluster_fs && (fstype_is_api_vfs(fstype) || fstype_is_network(fstype)))
                return false;
...
static int sync_departing_file_systems(const char *new_root) {
        ...
        for (;;) {
                 ...
                 * stacked on top - exactly what fstype_worth_syncing() above is trying to prevent us from
                 * doing.
                 *
                 * Rather than risk either of these, fall back to one global sync() in both cases, and
                 * stop iterating: a global sync() trivially covers this (and every other) file system
                 * correctly, so there's nothing left to do here afterwards. This should be rare in
                 * practice (shadowed departing mounts, or failures determining this, are both unusual),
                 * so this doesn't meaningfully undercut the benefit of the targeted sync in the common
                 * case. */
                r = libmount_fs_id_matches_path(fs, path);
                if (r < 0) {
                        log_debug_errno(r, "Failed to determine whether '%s' is currently shadowed by another mount, falling back to global sync(): %m", path);
                        sync();
                        return 0;
                }
                if (r == 0) {
                        log_debug("'%s' is currently shadowed by another mount, falling back to global sync() so its data isn't silently dropped.", path);
                        sync();
                        return 0;
                }
                ...
                if (r < 0) {
                         ...
                         * replace" guarantee for the (plausibly more common) former case, fall back to a
                         * global sync() here too, and stop iterating, same as for the other cases above. */
                        log_debug_errno(r, "Failed to synchronize file system '%s', falling back to global sync(): %m", path);
                        sync();
                        return 0;
                }
        }
