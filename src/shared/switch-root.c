...
int switch_root(const char *new_root,
         ...
         * are maintained. */
        unsigned long run_mount_flags = MS_BIND|(!FLAGS_SET(flags, SWITCH_ROOT_NON_RECURSIVE_RUN) ? MS_REC : 0);
        struct {
                const char *path;
        ...
        FOREACH_ARRAY(transfer, transfer_table, ELEMENTSOF(transfer_table)) {
                ...
                if (transfer->skip_if_run_is_rec && FLAGS_SET(flags, SWITCH_ROOT_NON_RECURSIVE_RUN))
                        continue;
