...
int nsresource_add_cgroup(int userns_fd, int cgroup_fd) {
        ...
        if (!isempty(error_id))
                return log_debug_errno(SYNTHETIC_ERRNO(ENOANO), "Failed to add cgroup to user namespace: %s", error_id);
...
int nsresource_add_netif(
        ...
        if (!isempty(error_id))
                return log_debug_errno(SYNTHETIC_ERRNO(ENOANO), "Failed to add network to user namespace: %s", error_id);
