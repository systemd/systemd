 ...
 * one per direction. All four fds must be distinct — use dup() for bidirectional sockets.
 *
 ...
 * All four fds must be distinct - use dup()/fnctl(fd, _DUPFD_CLOEXEC, 3) for bidirectional sockets. */
