...
/* The same entries as FOREACH_MSTACK_LAYER, in the order overlayfs wants them handed over: the writable
 * layer first, then the lowers from topmost down, so that root/ - which mstack_normalize() sorts below
 * every layer@ - is added last and ends up the bottommost lowerdir. That is exactly the reverse of the
 * array order, which is stated here once rather than at each call site. */
...
typedef struct MStackLayerPlan {
        ...
        uid_t identity_uid;
        bool needs_tmpfs_backing;  /* a synthetic rw/ layer, conjured by --volatile=overlay rather than
                                    * put on disk by the caller, so it has nothing to hold data/work yet */
} MStackLayerPlan;

/* Every decision, made before anything is mounted. Records derivations only: straight yes/no questions
 * about the kernel are read through 'caps', because copying one in here would just create a second thing
 * that can drift from the first, and observations that need I/O (does this layer's data/ exist?) are left
 * to whoever is standing in front of the filesystem. */
typedef struct MStackCaps MStackCaps; /* what the kernel underneath us can do; private to mstack.c */

typedef struct MStackPlan {
        const MStackCaps *caps;
