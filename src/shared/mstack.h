...
typedef enum MStackBindMountStrategy {
        MSTACK_BINDMOUNT_NONE,          /* Mount only root, defer everything else */
        MSTACK_BINDMOUNT_EXCEPT_VAR,    /* Mount everything EXCEPT paths under /var */
        MSTACK_BINDMOUNT_VAR,           /* Mount ONLY under /var */
        MSTACK_BINDMOUNT_ALL,           /* Mount everything immediately */
        _MSTACK_BINDMOUNT_STRATEGY_MAX,
        _MSTACK_BINDMOUNT_STRATEGY_INVALID = -EINVAL,
} MStackBindMountStrategy;
...
int mstack_apply_bind_mounts(MStack *mstack, int root_fd, const char *where, MStackFlags flags, MStackBindMountStrategy mstack_bindmount_strategy);
int mstack_bind_mounts(MStack *mstack, const char *where, int where_fd, MStackFlags flags, MStackBindMountStrategy mstack_bindmount_strategy, int *ret_root_fd);
