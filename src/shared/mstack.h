...
typedef enum MStackFlags {
        ...
        MSTACK_RDONLY        = 1 << 1,
        /* MSTACK_USER_MOUNTFSD = 1 << 2, /\* When dissecting disk image, go via mountfsd *\/ */
} MStackFlags;
...
typedef enum MStackMountType {
        ...
        MSTACK_RW,       /* "rw" entry that is the upper (writable) layer of an overlayfs stack */
        MSTACK_WORK,     /* "work" entry which is the workdir for the overlayfs stack */
...
int mstack_make_mounts(MStack *mstack, const ImagePolicy *image_policy, const ImageFilter *image_file, MStackFlags flags);
int mstack_bind_mounts(MStack *mstack, const char *where, int where_fd, MStackFlags flags, int *ret_root_fd);

/* The three calls above in one */
int mstack_apply(const char *dir, int dir_fd, const char *where, const ImagePolicy *image_policy, const ImageFilter *image_filter, MStackFlags flags, int *ret_root_fd);

int mstack_is_read_only(MStack *mstack);
int mstack_is_foreign_uid_owned(MStack *mstack);

const char *mstack_mount_type_to_string(MStackMountType t) _const_;
