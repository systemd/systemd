...
int make_filesystem(
        ...
        if (STR_IN_SET(fstype, "ext2", "ext3", "ext4")) {
                argv = strv_new(mkfs,
                                ...
                                "-I", "256",
                                "-m", "0",
                                "-E", (flags & MKFS_DISCARD) ? "discard,lazy_itable_init=1" : "nodiscard,lazy_itable_init=1",
                                "-b", "4096",
                                "-T", "default");
                if (!argv)
                        return log_oom();
                ...
                if ((flags & MKFS_QUIET) && strv_extend(&argv, "-q") < 0)
                        return log_oom();
                ...
                if (!(flags & MKFS_DISCARD) && strv_extend(&argv, "--nodiscard") < 0)
                        return log_oom();

                if (root && strv_extend_many(&argv, "-r", root) < 0)
                        return log_oom();

                if ((flags & MKFS_DISCARD) && strv_extend(&argv, "-q") < 0)
                        return log_oom();
                ...
                argv = strv_new(mkfs,
                                ...
                                "-l", label,
                                "-U", vol_id,
                                "-t", one_zero((flags & MKFS_DISCARD) != 0));
                if (!argv)
                        return log_oom();
                ...
                if (!(flags & MKFS_DISCARD) && strv_extend(&argv, "-K") < 0)
                        return log_oom();
                ...
                if ((flags & MKFS_QUIET) && strv_extend(&argv, "--quiet") < 0)
                        return log_oom();
