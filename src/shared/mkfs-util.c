...
static int do_mcopy(const char *node, const char *root) {
        ...
        rfd = open(root, O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW);
        if (rfd < 0)
                return log_error_errno(errno, "Failed to open %s: %m", root);

        r = readdir_all(rfd, RECURSE_DIR_SORT, &de);
        if (r < 0)
                return log_warning_errno(r, "Failed to read '%s' contents: %m", root);
        ...
        for (size_t i = 0; i < de->n_entries; i++) {
                ...
                r = dirent_ensure_type(rfd, de->entries[i]);
                if (r < 0)
                        return log_error_errno(r, "Failed to ensure dirent type: %m");

                if (de->entries[i]->d_type == DT_LNK) {
