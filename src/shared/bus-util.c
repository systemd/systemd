...
static int pin_project_socket(const char *project, const char *suffix, uid_t *ret_uid, gid_t *ret_gid) {
        ...
        p = path_join("/run/projects", project, suffix);
        if (!p)
                return -ENOMEM;
...
int bus_connect_project_systemd(const char *project, sd_bus **ret_bus) {
        ...
        r = project_name_is_valid(project);
        if (r < 0)
                return r;
        if (r == 0)
                return -EINVAL;

        /* Connects to a project's user bus. We need to do so under the project's UID/GID, otherwise the
         * the service manager might refuse our connection. Hence fake it. */

        inode_fd = pin_project_socket(project, "systemd/private", &uid, &gid);
        if (inode_fd < 0)
                return inode_fd;
...
int bus_set_address_project_bus(sd_bus *bus, const char *project, int *ret_pin_fd) {
        ...
        inode_fd = pin_project_socket(project, "bus", &uid, &gid);
        if (inode_fd < 0)
                return inode_fd;
...
int bus_connect_project_bus(const char *project, sd_bus **ret_bus) {
        ...
        r = bus_set_address_project_bus(bus, project, &inode_fd);
        if (r < 0)
                return r;
