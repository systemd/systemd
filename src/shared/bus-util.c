...
int bus_log_connect_error(int r, BusTransport transport, RuntimeScope scope) {
        ...
        return log_error_errno(r,
                               r == hint_vars ? "Failed to connect to %s session bus via %s transport: $DBUS_SESSION_BUS_ADDRESS and $XDG_RUNTIME_DIR not defined (consider using --machine=<user>@.host --user to connect to bus of other user)" :
                               r == hint_addr ? "Failed to connect to %s session bus via %s transport: Operation not permitted (consider using --machine=<user>@.host --user to connect to bus of other user)" :
                                                "Failed to connect to %s session bus via %s transport: %m", runtime_scope_to_string(scope), bus_transport_to_string(transport));
}
