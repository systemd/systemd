...
int manufacture_swtpm(const char *state_dir, const char *secret) {
        ...
        /* NB: we ignore the exit status of --print-profiles, it's broken. Instead we check if we have received a valid JSON array */
        (void) r;
        ...
        if (isempty(text))
                return log_error_errno(SYNTHETIC_ERRNO(EPROTO), "No list of supported profiles could be acquired from swtpm.");
        ...
        if (v) {
                if (!sd_json_variant_is_array(v))
                        return log_error_errno(r, "'builtin' field is not an array: %m");
        ...
        if (best_profile && strv_extendf(&args, "--profile-name=%s", best_profile))
                return log_oom();
