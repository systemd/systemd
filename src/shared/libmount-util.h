...
/* The statmount()/listmount() API is available since util-linux 2.41. Always redeclare so
...
/* statmount()/listmount(), util-linux 2.41 and newer. Resolved with DLSYM_OPTIONAL, so these stay
 * NULL against an older runtime libmount and libmount_parse_kernel() reports -EOPNOTSUPP. */
...
/* Builds the same table libmount_parse_mountinfo() would, but through listmount()/statmount()
 * instead of /proc/self/mountinfo, asking the kernel only for the fields the caller declares in
 * 'mask'. Returns -EOPNOTSUPP when the runtime libmount or the kernel lacks the syscalls, so the
 * caller can fall back to the mountinfo parse. */
