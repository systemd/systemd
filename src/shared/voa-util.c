...
typedef struct VoaContext {
        const char *root;       /* for messages only, empty for the host root */
        int root_fd;
...
static int voa_collect(
        ...
        r = chaseat(c->root_fd, c->root_fd, dir, CHASE_NONEXISTENT, &physical, /* ret_fd= */ NULL);
        if (IN_SET(r, -ENOTDIR, -ELOOP)) {
                log_full_errno(c->log_level, r, "Ignoring '%s%s': %m", c->root, dir);
                return 0;
        }
        if (r < 0)
                return r;
        if (r == 0) {
                if (path_startswith(skip_leading_slash(physical), "dev/null"))
                        log_full(c->log_level, "Ignoring '%s%s': directories cannot be masked.",
                                 c->root, dir);
                return 0;
        }

        if (!path_startswith_strv(physical, c->hierarchy)) {
                log_full(c->log_level, "Ignoring '%s%s': resolves outside of the VOA hierarchy.",
                         c->root, dir);
                return 0;
        }

        r = readdir_all_at(c->root_fd, skip_leading_slash(physical), RECURSE_DIR_SORT, &de);
        if (r == -ENOTDIR) {
                log_full(c->log_level, "Ignoring '%s%s': not a directory.", c->root, dir);
                return 0;
        }
        if (r < 0)
                return r;
        ...
        FOREACH_ARRAY(i, de->entries, de->n_entries) {
                ...
                r = conf_file_new_at(path, c->root, c->root_fd, c->conf_flags, &f);
                if (r == -ERFKILL) {
                        r = set_put_strdup(masked, name);
                        if (r < 0)
                                return r;

                        log_debug("'%s%s' masks '%s'.", c->root, path, name);
                        continue;
                }
                if (r == -ENOMEM)
                        return r;
                ...
                if (!path_startswith_strv(f->resolved_path, c->hierarchy)) {
                        log_full(c->log_level, "Ignoring '%s%s': points outside of the VOA hierarchy.",
                                 c->root, path);
                        continue;
                }
                ...
                if (r < 0)
                        return r;
                if (!streq(fn, name)) {
                        log_full(c->log_level, "Ignoring '%s%s': target '%s/%s' has a different name.",
                                 c->root, path, c->root, skip_leading_slash(f->resolved_path));
                        continue;
                }
...
static int voa_list_one(
        ...
        FOREACH_ARRAY(f, candidates, n_candidates) {
                ...
                if (set_contains(masked, (*f)->filename)) {
                        log_debug("'%s%s' is masked, ignoring.", c->root, (*f)->original_path);
                        continue;
                }

                /* Every copy counts, but the same file reached twice is one copy */
                FOREACH_ARRAY(g, *files, *n_files)
                        if (stat_inode_same(&(*f)->st, &(*g)->st)) {
                                seen = true;
                                break;
                        }
                if (seen)
...
int voa_list_verifiers(
        ...
        if (lookup->mode < 0 || lookup->mode >= _VOA_MODE_MAX)
                return -EINVAL;
        STRV_FOREACH(context, lookup->contexts)
                if (!voa_identifier_is_valid(*context, /* allow_colon= */ false))
                        return -EINVAL;
        ...
        STRV_FOREACH(os, lookup->os)
                STRV_FOREACH(context, lookup->contexts ?: STRV_MAKE("default")) {
                        r = voa_list_one(&c, *os, purpose, *context, lookup->technology, lookup->suffix,
                                         &files, &n_files);
                        if (r < 0)
                                return r;
                }
