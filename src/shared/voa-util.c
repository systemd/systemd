...
/* No directory on the way may be writable by others, and ownership may pass from root to a user but never on
 * from there */
#define VOA_CHASE_FLAGS(level) \
        (CHASE_MUST_BE_DIRECTORY|CHASE_SAFE|CHASE_MAX_MODE|((level) == LOG_WARNING ? CHASE_WARN : 0))

/* One bad load path, an unsafe transition included, must not hide the others */
static bool voa_chase_error_ignorable(int r) {
        return IN_SET(r, -ENOTDIR, -ELOOP, -ENOLINK, -EACCES, -EPERM);
}
...
static int voa_collect(
         ...
         * resolved path. */
        dfd = chase_and_openat(c->root_fd, c->root_fd, dir, VOA_CHASE_FLAGS(c->log_level),
                               O_DIRECTORY|O_CLOEXEC, &physical);
        if (dfd == -ENOENT)
                return 0;
        if (voa_chase_error_ignorable(dfd)) {
...
int voa_list_verifiers(
         ...
         * too because on some systems such as rpm-ostree /usr/local is a symlink into /var. */
        STRV_FOREACH(lp, voa_load_paths) {
                _cleanup_free_ char *physical = NULL;

                r = chaseat(root_fd, root_fd, *lp, VOA_CHASE_FLAGS(log_level), &physical,
                            /* ret_fd= */ NULL);
                if (r == -ENOENT)
                        continue;
                if (voa_chase_error_ignorable(r)) {
