...
static int voa_collect(
         ...
         * resolved path. Every directory on the way must be root-owned and 0755. */
        dfd = chase_and_openat(c->root_fd, c->root_fd, dir,
                               CHASE_MUST_BE_DIRECTORY|CHASE_SAFE|CHASE_DIR_0755|
                               (c->log_level == LOG_WARNING ? CHASE_WARN : 0),
...
int voa_list_verifiers(
        ...
        STRV_FOREACH(lp, voa_load_paths) {
                ...
                r = chaseat(root_fd, root_fd, *lp,
                            CHASE_MUST_BE_DIRECTORY|CHASE_SAFE|CHASE_DIR_0755|
                            (log_level == LOG_WARNING ? CHASE_WARN : 0),
                            &physical, /* ret_fd= */ NULL);
                if (r == -ENOENT)
                        continue;
                /* Like in voa_collect(), one bad load path must not hide the others */
                if (IN_SET(r, -ENOTDIR, -ELOOP, -ENOLINK, -EACCES, -EPERM)) {
        ...
        VoaContext c = {
                ...
                .conf_flags = CONF_FILES_REGULAR|CONF_FILES_FILTER_MASKED_BY_SYMLINK|CONF_FILES_CHASE_SAFE|
                              CONF_FILES_CHASE_DIR_0755|CONF_FILES_CHASE_FILE_0644|
