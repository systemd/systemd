...
int numa_get_node_from_cpu(unsigned cpu, unsigned *ret) {
        ...
        FOREACH_DIRENT(de, d, break) {
                ...
                r = safe_atou(n, &node);
                if (r < 0) {
                        log_debug_errno(r, "Failed to parse node number %s to unsigned, ignoring", n);
                        continue;
                }
                ...
                r = read_one_line_file(p, &cpulist);
                if (r < 0) {
                        log_debug_errno(r, "Failed to read %s, ignoring", p);
                        continue;
                }

                r = parse_cpu_set(cpulist, &cpus);
                if (r < 0) {
                        log_debug_errno(r, "Failed to parse cpu set %s, ignoring", cpulist);
                        continue;
                }
