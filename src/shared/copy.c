...
static int fd_copy_regular(
        ...
        fdf = xopenat_full(df, from, O_RDONLY|O_NOCTTY|O_NOFOLLOW, XO_REGULAR|XO_EMPTY_PATH, 0); /* from may be NULL */
        if (fdf < 0)
                return fdf;
...
int copy_file_fd_at_full(
        ...
        fdf = xopenat_full(dir_fdf, from, O_RDONLY|O_NOCTTY, XO_REGULAR|XO_EMPTY_PATH, 0); /* from may be NULL */
        if (fdf < 0)
                return fdf;
