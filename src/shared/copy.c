...
static int look_for_signals(CopyFlags copy_flags) {
        ...
        r = pop_pending_signal(copy_flags & COPY_SIGINT ? SIGINT : 0, copy_flags & COPY_SIGTERM ? SIGTERM : 0);
        if (r < 0)
                return r;
        if (r != 0)
                return log_debug_errno(
                                SYNTHETIC_ERRNO(EINTR),
                                "Got %s, cancelling copy operation.",
                                signal_to_string(r));
...
static int
        ...
        if (r < 0) {
                if (FLAGS_SET(copy_flags, COPY_GRACEFUL_WARN) &&
                    (ERRNO_IS_PRIVILEGE(r) || ERRNO_IS_NOT_SUPPORTED(r))) {
                        log_notice_errno(
                                        r,
                                        "Failed to copy symlink%s%s%s, ignoring: %m",
                                        isempty(from) ? "" : " '",
                                        strempty(from),
                                        isempty(from) ? "" : "'");
                        return 0;
...
static int hardlink_context_setup(HardlinkContext *c, int dt, const char *to, CopyFlags copy_flags) {
...
static void hardlink_context_destroy(HardlinkContext *c) {
        ...
        if (c->dir_fd >= 0) {
                ...
                r = rm_rf_children(
                                TAKE_FD(c->dir_fd),
                                REMOVE_PHYSICAL,
                                NULL); /* consumes dir_fd in all cases, even on failure */
                if (r < 0)
                        log_debug_errno(r,
                                        "Failed to remove hardlink store (%s) contents, ignoring: %m",
                                        c->subdir);
                ...
                if (unlinkat(c->parent_fd, c->subdir, AT_REMOVEDIR) < 0)
                        log_debug_errno(errno,
                                        "Failed to remove hardlink store (%s) directory, ignoring: %m",
                                        c->subdir);
        }
...
static int try_hardlink(HardlinkContext *c, const struct stat *st, int dt, const char *to) {
...
static int memorize_hardlink(HardlinkContext *c, const struct stat *st, int dt, const char *to) {
...
static int
        ...
        if (copy_flags & COPY_MAC_CREATE)
                mac_selinux_create_file_clear();
        if (FLAGS_SET(copy_flags, COPY_GRACEFUL_WARN) &&
            (ERRNO_IS_NEG_PRIVILEGE(r) || ERRNO_IS_NEG_NOT_SUPPORTED(r))) {
                log_notice_errno(
                                r,
                                "Failed to copy fifo%s%s%s, ignoring: %m",
                                isempty(from) ? "" : " '",
                                strempty(from),
                                isempty(from) ? "" : "'");
                return 0;
...
static int
        ...
        if (FLAGS_SET(copy_flags, COPY_GRACEFUL_WARN) &&
            (ERRNO_IS_NEG_PRIVILEGE(r) || ERRNO_IS_NEG_NOT_SUPPORTED(r))) {
                log_notice_errno(
                                r,
                                "Failed to copy node%s%s%s, ignoring: %m",
                                isempty(from) ? "" : " '",
                                strempty(from),
                                isempty(from) ? "" : "'");
                return 0;
...
static int fd_copy_directory(
        ...
        if (r < 0 && r != -ENOENT)
                return r;
        if ((r > 0 && !(copy_flags & (COPY_MERGE | COPY_MERGE_EMPTY))) ||
            (r == 0 && !FLAGS_SET(copy_flags, COPY_MERGE)))
                return -EEXIST;

        exists = r >= 0;

        fdt = xopenat_lock_full(
                        dt,
                        to,
                        O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOCTTY | O_NOFOLLOW |
                                        (exists ? 0 : O_CREAT | O_EXCL),
                        (copy_flags & COPY_MAC_CREATE ? XO_LABEL : 0) |
                                        (set_contains(subvolumes, st) ? XO_SUBVOLUME : 0),
                        st->st_mode & 07777,
        ...
        FOREACH_DIRENT_ALL(de, d, return -errno) {
                ...
                if (S_ISDIR(buf.st_mode)) {
                         ...
                         *
                         * • The kernel's overlayfs pseudo file system that overlays multiple real file s
                         * stems propagates the .st_dev field of the file system a file originates from all
                         ...
                         * regular files.
                         *
                         * • The main reason we do this check at all is to protect ourselves from bind mount c
                         * cles, where we really want to avoid descending down in all eternity. However the
...
static int fd_copy_tree_generic(
        ...
        if (t == DENY_INODE) {
                ...
                log_debug("%s is configured to have its contents excluded, but is not a directory",
                          from ?: "file to copy");
        ...
        r = fd_copy_leaf(
                        ...
                        progress_bytes,
                        userdata);
        /* We just tried to copy a leaf node of the tree. If it failed because the node already exists *and*
         * the COPY_REPLACE flag has been provided, we should unlink the node and re-copy. */
...
int copy_tree_at_full(
        ...
        if (S_ISDIR(st.st_mode) && (copy_flags & COPY_SYNCFS)) {
                 ...
                 * case COPY_SYNCFS is set but the top-level inode wasn't actually a directory. We do this so
                 * that COPY_SYNCFS provides reasonable synchronization semantics on any kind of inode: when
                 * the copy operation is done the whole inode — regardless of its type — and all its children
...
int copy_directory_at_full(
        ...
        if (fstatat(dir_fdf, strempty(from), &st, AT_SYMLINK_NOFOLLOW | (isempty(from) ? AT_EMPTY_PATH : 0)) <
            0)
                return -errno;
...
int copy_file_at_full(
        ...
        r = copy_bytes_full(
                        fdf, fdt, UINT64_MAX, copy_flags & ~COPY_LOCK_BSD, NULL, NULL, progress_bytes, userdata);
        if (r < 0)
                goto fail;
...
int copy_xattr(int df, const char *from, int dt, const char *to, CopyFlags copy_flags) {
        ...
        NULSTR_FOREACH(p, names) {
                ...
                RET_GATHER(ret,
                           xsetxattr_full(dt, to, /* at_flags = */ 0, p, value, value_size, /* xattr_flags = */ 0));
        }
