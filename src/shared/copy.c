...
static int fd_copy_directory(
        ...
        if (hashmap_contains(denylist, st) && PTR_TO_INT(hashmap_get(denylist, st)) == DENY_CONTENTS) {
        ...
        FOREACH_DIRENT_ALL(de, d, return -errno) {
                ...
                if (hashmap_contains(denylist, &buf) && PTR_TO_INT(hashmap_get(denylist, &buf)) == DENY_INODE) {
...
static int fd_copy_tree_generic(
        ...
        if (hashmap_contains(denylist, st)) {
                ...
                if (t == DENY_INODE) {
                        ...
                        log_warning("%s is configured to have its contents excluded, but is not a directory", from);
        }
