...
#ifdef FS_IOC_READ_VERITY_METADATA
        int r = fd_verify_regular(fdf);
        if (r < 0)
                return r;
...
#else
        /* If we don't have fs-verity headers then do nothing and report success: we don't want to fail to
         * copy fs-verity state on a system where it's not even possible to query it (and highly unlikely to
         * exist at all).
         */
        return 0;
#endif
