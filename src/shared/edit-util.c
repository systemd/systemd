...
static void edit_file_free_all(EditFile **ef) {
        assert(ef);
...
static void edit_file_unlink_all(EditFile **ef) {
        assert(ef);
...
static void edit_file_free_and_unlink_all(EditFile **ef) {
        assert(ef);

        edit_file_unlink_all(ef);
        edit_file_free_all(ef);
}

void edit_file_context_done(EditFileContext *context) {
        assert(context);

        edit_file_free_and_unlink_all(&context->files);
        context->n_files = 0;
}
...
int edit_files_add(
        ...
        if (EDIT_FILE_CONTAINS(path, context->files))
                return 0;
...
static int run_editor(const EditFileContext *context) {
        ...
        if (r == 0) {
                ...
                if (context->n_files == 1) {
                        ...
                        EDIT_FILE_FOREACH(f, context->files)
                                args[i++] = f->temp;
...
int do_edit_files_and_install(EditFileContext *context) {
        ...
        edit_file_unlink_all(&context->files);
        return r;
}
