...
int run_editor(const EditFile *edit_files) {
        ...
        if (r == 0) { /* child */
                ...
                if (edit_files_count(edit_files) == 1) {
                        ...
                        r = strv_extend(&args, edit_files[0].temp);
                        if (r < 0)
                                goto oom;
                } else
...
int edit_files_add(
        ...
        file = context->files + context->n_files + 1;
        ...
        if (original_path) {
                file->original_path = strdup(original_path);
                if (!file->original_path)
                        return log_oom();
        }

        if (comment_paths) {
                file->comment_paths = strv_copy(comment_paths);
                if (!file->comment_paths)
                        return log_oom();
        }
