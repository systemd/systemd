...
int generator_write_timeouts(
         ...
         *
         * Note that we set *ret_filtered even on failure, so that the calling code can ignore the
         * failure for the snippet if it wishes, but continue with the other options.
         */
        ...
        if (r == -ENOMEM)
                return log_oom();
        if (r < 0) {
                log_warning_errno(r, "%s: failed to parse fstab options, ignoring: %m", what);
                *ret_filtered = NULL;
                return r;
        }
        if (r == 0)
        ...
        if (r < 0)
                return log_warning_errno(r, "%s: failed to parse timeout, ignoring: %s", what, timeout);
        ...
        if (r < 0)
                return log_error_errno(r, "%s: failed to make unit name from path '%s': %m", what, node);
        ...
        r = write_drop_in_format(dir, unit, 50, "device-timeout",
                                 ...
                                 timeout);
        if (r < 0)
                return log_error_errno(r, "%s: failed to write drop-in with device timeout: %m", what);
        return 0;
}
