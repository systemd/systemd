...
int generator_hook_up_quotacheck(
        ...
        r = generator_add_ordering(dir, where_unit, "After", SPECIAL_QUOTACHECK_SERVICE, instance);
        if (r < 0)
                return r;
...
int generator_hook_up_quotaon(
        ...
        if (target) {
                r = generator_add_ordering(dir, where_unit, "Before", SPECIAL_QUOTAON_SERVICE, instance);
                if (r < 0)
                        return r;
        }
