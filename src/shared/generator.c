...
int generator_hook_up_quotaservice(
                const char *dir,
                const char *where,
                const char *target) {

        _cleanup_free_ char *q_check_unit = NULL, *q_on_unit = NULL, *escaped = NULL, *where_unit = NULL, *q_check_unit_file = NULL, *q_on_unit_file =NULL;
        _cleanup_fclose_ FILE *f = NULL;
        int r;
        ...
        r = unit_name_from_path_instance("systemd-quotacheck", where, ".service", &q_check_unit);
        if (r < 0)
                return log_error_errno(r, "Failed to make unit instance name from path \"%s\": %m",
                                       where);
        ...
        q_check_unit_file = path_join(dir, q_check_unit);
        if (!q_check_unit_file)
                return log_oom();

        log_debug("Creating %s", q_check_unit_file);

        f = fopen(q_check_unit_file, "wxe");
        if (!f)
                return log_error_errno(errno, "Failed to create unit file %s: %m",
                                       q_check_unit_file);
        ...
        if (empty_or_root(where)) /* Make sure the fs is ro for quotaon */
                fprintf(f,
        ...
        fprintf(f,
                ...
                "Type=oneshot\n"
                "RemainAfterExit=yes\n"
                "ExecStart="SYSTEMD_QUOTACHECK_PATH " %%f\n"
                "TimeoutSec=0\n");

        r = generator_add_symlink(dir, where_unit, "wants", q_check_unit);
        if (r < 0)
                return r;

        fclose(f);

        r = unit_name_from_path_instance("systemd-quotaon", where, ".service", &q_on_unit);
        if (r < 0)
                return log_error_errno(r, "Failed to make unit instance name from path \"%s\": %m",
                                       where);

        q_on_unit_file = path_join(dir, q_on_unit);
        if (!q_on_unit_file)
                return log_oom();

        log_debug("Creating %s", q_on_unit_file);

        f = fopen(q_on_unit_file, "wxe");
        if (!f)
                return log_error_errno(errno, "Failed to create unit file %s: %m",
                                       q_on_unit_file);
        ...
        fprintf(f,
                "# Automatically generated by %s\n\n"
                "[Unit]\n"
                "Description=Enable File System Quotas on %%f\n"
        ...
        fprintf(f,
                ...
                "Type=oneshot\n"
                "RemainAfterExit=yes\n"
                "ExecStart="QUOTAON_PATH " -ug %%f\n"
                "TimeoutSec=0\n");

        return generator_add_symlink(dir, where_unit, "wants", q_on_unit);
}
