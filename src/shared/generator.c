...
int generator_hook_up_quotacheck(
        ...
        if (path_equal(where, "/"))
                return generator_add_symlink(dir, where_unit, "wants", SYSTEM_DATA_UNIT_DIR "/" SPECIAL_QUOTACHECK_ROOT_SERVICE);
...
int generator_hook_up_quotaon(
        ...
        if (path_equal(where, "/"))
                return generator_add_symlink(dir, SPECIAL_QUOTACHECK_ROOT_SERVICE, "wants", SYSTEM_DATA_UNIT_DIR "/" SPECIAL_QUOTAON_ROOT_SERVICE);
        ...
        if (target) {
                r = generator_add_ordering(dir, target, "Before", SPECIAL_QUOTAON_SERVICE, instance);
                if (r < 0)
                        return r;
        }
