...
void pager_open(PagerFlags flags) {
        ...
        if (r == 0) {
                ...
                /* more(1) does not require MORESECURE to be set to a specific value. Once it exists in the
                 * environment it will enable secure mode. */
                r = set_unset_env("MORESECURE", use_secure_mode ? "1" : NULL, true);
                if (r < 0) {
                        log_error_errno(r, "Failed to adjust environment variable MORESECURE: %m");
                        _exit(EXIT_FAILURE);
                }
