...
typedef struct GatewayAddressKey {
        int ifindex;
        int family;
        union in_addr_union address;
} GatewayAddressKey;

static void gateway_address_hash_func(const GatewayAddressKey *a, struct siphash *state) {
        assert(a);
        assert(state);
...
static int gateway_address_compare_func(const GatewayAddressKey *a, const GatewayAddressKey *b) {
        int r;
...
DEFINE_PRIVATE_HASH_OPS_WITH_KEY_DESTRUCTOR(
        gateway_address_hash_ops,
        GatewayAddressKey,
        gateway_address_hash_func,
        gateway_address_compare_func,
        free);

static void suppress_duplicate_gateways(struct local_address *list, size_t *n_list) {
        ...
        for (size_t i = 0; i < old_size; i++) {
                GatewayAddressKey key = {
                        .ifindex = list[i].ifindex,
                        .family = list[i].family,
                        .address = list[i].address,
                };

                if (seen && set_contains(seen, &key))
                        continue;

                _cleanup_free_ GatewayAddressKey *new_key = new(GatewayAddressKey, 1);
                if (!new_key)
                        return;

                *new_key = key;

                if (set_ensure_consume(&seen, &gateway_address_hash_ops, TAKE_PTR(new_key)) < 0)
                        return;
...
static void gateway_peer_by_link_hash_func(const GatewayPeerByLink *p, struct siphash *state) {
        ...
        in_addr_hash_func(&p->peer, p->family, state);
}
...
static int gateway_peer_by_link_compare_func(const GatewayPeerByLink *a, const GatewayPeerByLink *b) {
        ...
        return memcmp(&a->peer, &b->peer, FAMILY_ADDRESS_SIZE(a->family));
}
...
static int add_gateway_candidate(
        ...
        if (!set_contains(*candidate_links, &link_lookup)) {
...
static int add_gateway_peer_by_link(
        ...
        *new_info = (GatewayPeerByLink) {
        ...
        int r = set_ensure_consume(peers_by_link, &gateway_peer_by_link_hash_ops, TAKE_PTR(new_info));
        if (r < 0)
                return r;
...
static int add_gateway_peer_by_local(
        ...
        GatewayPeerByLocal *info = set_get(*peers_by_local, &lookup);
        if (info) {
        ...
        *new_info = (GatewayPeerByLocal) {
        ...
        int r = set_ensure_consume(peers_by_local, &gateway_peer_by_local_hash_ops, TAKE_PTR(new_info));
        if (r < 0)
                return r;
...
static int append_gateways_from_peer_maps(
        ...
        SET_FOREACH(candidate, candidates) {
                ...
                GatewayPeerByLink *peer;
                const GatewayPeerByLink *unique_peer = NULL;
                bool ambiguous = false;
                SET_FOREACH(peer, peers_by_link) {
                        if (peer->ifindex != candidate->ifindex || peer->family != candidate->family)
                                continue;
                ...
                if (!unique_peer || ambiguous)
                        continue;

                r = add_local_gateway(
                                list, n_list, candidate->ifindex, candidate->priority, candidate->weight,
                                candidate->family, &unique_peer->peer, &candidate->prefsrc);
                if (r < 0)
                        return r;
        }
...
int local_gateways(
        ...
        typesafe_qsort(list, n_list, address_compare);
        suppress_duplicate_gateways(list, &n_list);
        suppress_duplicates(list, &n_list);
