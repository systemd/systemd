...
static int add_gateway_link_info(
        ...
        GatewayLinkInfo lookup = {
                .ifindex = ifindex,
        };
        GatewayLinkInfo *link = set_get(*links, &lookup);
        if (link) {
                link->flags = flags;
                return 0;
        }
        ...
        *new_link = (GatewayLinkInfo) {
        ...
        r = set_ensure_consume(links, &gateway_link_info_hash_ops, TAKE_PTR(new_link));
        if (r < 0)
                return r;
