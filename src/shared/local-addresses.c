...
/* Key for facts that are additionally scoped by the local point-to-point endpoint. */
typedef struct GatewayPeerLocalKey {
        int ifindex;
        int family;
...
DEFINE_PRIVATE_HASH_OPS_FULL(
        gateway_link_key_hash_ops_free_peer_info,
        GatewayLinkKey,
        gateway_link_key_hash_func,
        gateway_link_key_compare_func,
        free,
...
static void gateway_peer_local_key_hash_func(const GatewayPeerLocalKey *k, struct siphash *state) {
        assert(k);
        assert(state);
...
static int gateway_peer_local_key_compare_func(const GatewayPeerLocalKey *a, const GatewayPeerLocalKey *b) {
        int r;
...
DEFINE_PRIVATE_HASH_OPS_FULL(
        gateway_peer_local_key_hash_ops_free_peer_info,
        GatewayPeerLocalKey,
        gateway_peer_local_key_hash_func,
        gateway_peer_local_key_compare_func,
        free,
        GatewayPeerInfo,
        free);
...
static int add_gateway_candidate(
        ...
        GatewayCandidate *candidate = hashmap_get(*candidates, &lookup);
        if (candidate) {
        ...
        if (!hashmap_contains(*candidate_links, &link_lookup)) {
                ...
                *link_key = link_lookup;
                ...
                int r = hashmap_ensure_put(candidate_links, &gateway_link_key_hash_ops, link_key, INT_TO_PTR(1));
                if (r < 0)
                        return r;
        ...
        *new_candidate = (GatewayCandidate) {
        ...
        int r = hashmap_ensure_put(candidates, &gateway_candidate_hash_ops, new_candidate, new_candidate);
        if (r < 0)
                return r;

        TAKE_PTR(new_candidate);
...
static bool have_gateway_candidate_for_link(
        ...
        return hashmap_contains(candidate_links,
                                &(GatewayLinkKey) {
                                        .ifindex = ifindex,
                                        .family = family,
                                });
...
static int get_gateway_link_flags(
        ...
        GatewayLinkInfo *link = hashmap_get(*links, INT_TO_PTR(ifindex));
        if (link) {
                if (link->r < 0)
                        return link->r;
        ...
        *new_link = (GatewayLinkInfo) {
        ...
        int k = hashmap_ensure_put(links, &trivial_hash_ops_value_free, INT_TO_PTR(ifindex), new_link);
        if (k < 0) {
                if (r >= 0) {
                        *ret_flags = flags;
                        return 0;
...
static int add_gateway_peer_by_link(
        ...
        GatewayLinkKey lookup = {
                .ifindex = ifindex,
                .family = family,
        };
        GatewayPeerInfo *info = hashmap_get(*peers_by_link, &lookup);
        if (info) {
                update_gateway_peer_info(info, family, peer);
                return 0;
        }

        _cleanup_free_ GatewayLinkKey *key = new(GatewayLinkKey, 1);
        _cleanup_free_ GatewayPeerInfo *new_info = new(GatewayPeerInfo, 1);
        if (!key || !new_info)
                return -ENOMEM;

        *key = lookup;
        *new_info = (GatewayPeerInfo) {
                .peer = *peer,
                .ambiguous = false,
        };

        int r = hashmap_ensure_put(peers_by_link, &gateway_link_key_hash_ops_free_peer_info, key, new_info);
        if (r < 0)
                return r;
...
static int add_gateway_peer_by_local(
        ...
        GatewayPeerLocalKey lookup = {
                .ifindex = ifindex,
                .family = family,
                .local = *local,
        };
        GatewayPeerInfo *info = hashmap_get(*peers_by_local, &lookup);
        if (info) {
                update_gateway_peer_info(info, family, peer);
                return 0;
        }

        _cleanup_free_ GatewayPeerLocalKey *key = new(GatewayPeerLocalKey, 1);
        _cleanup_free_ GatewayPeerInfo *new_info = new(GatewayPeerInfo, 1);
        if (!key || !new_info)
                return -ENOMEM;
        ...
        *new_info = (GatewayPeerInfo) {
        ...
        int r = hashmap_ensure_put(peers_by_local, &gateway_peer_local_key_hash_ops_free_peer_info, key, new_info);
        if (r < 0)
                return r;
...
static int append_gateways_from_peer_maps(
        ...
        HASHMAP_FOREACH(candidate, candidates) {
                GatewayPeerInfo *peer_info;

                if (in_addr_is_set(candidate->family, &candidate->prefsrc))
                        peer_info = hashmap_get(peers_by_local,
                                                &(GatewayPeerLocalKey) {
                        ...
                        peer_info = hashmap_get(peers_by_link,
                                                &(GatewayLinkKey) {
                                                        .ifindex = candidate->ifindex,
                                                        .family = candidate->family,
                                                });

                if (!peer_info || peer_info->ambiguous)
                        continue;

                r = add_local_gateway(
                                list, n_list, candidate->ifindex, candidate->priority, candidate->weight,
                                candidate->family, &peer_info->peer, &candidate->prefsrc);
                if (r < 0)
                        return r;
        }
...
static int add_gateway_peers(
        ...
        _cleanup_(sd_netlink_message_unrefp) sd_netlink_message *req = NULL, *reply = NULL;
        _cleanup_hashmap_free_ Hashmap *links = NULL, *peers_by_link = NULL, *peers_by_local = NULL;
        int r;
        ...
        for (sd_netlink_message *m = reply; m; m = sd_netlink_message_next(m)) {
                ...
                if (af != AF_UNSPEC && family != af)
                        continue;
                if (!have_gateway_candidate_for_link(candidate_links, ifi, family))
                        continue;
                ...
                unsigned link_flags;
                r = get_gateway_link_flags(rtnl, &links, ifi, &link_flags);
                if (r < 0) {
                        if (ERRNO_IS_NEG_RESOURCE(r))
                                return r;

                        log_debug_errno(r, "Failed to determine whether link %i is point-to-point, ignoring: %m", ifi);
                        continue;
                }
...
int local_gateways(
        ...
        _cleanup_(sd_netlink_unrefp) sd_netlink *rtnl = NULL;
        _cleanup_free_ struct local_address *list = NULL;
        _cleanup_hashmap_free_ Hashmap *candidates = NULL, *candidate_links = NULL;
        size_t n_list = 0;
        int r;
