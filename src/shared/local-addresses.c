...
static int add_gateway_candidate(
        ...
        if (!set_contains(*candidate_links, &link_lookup)) {
                ...
                *link_key = link_lookup;
                ...
                int r = set_ensure_consume(candidate_links, &gateway_link_key_hash_ops, TAKE_PTR(link_key));
                if (r < 0)
                        return r;
        }
        ...
        *new_candidate = (GatewayCandidate) {
        ...
        int r = set_ensure_consume(candidates, &gateway_candidate_hash_ops, TAKE_PTR(new_candidate));
        if (r < 0)
                return r;
...
static int get_gateway_link_flags(
        ...
        *new_link = (GatewayLinkInfo) {
        ...
        int k = set_ensure_consume(links, &gateway_link_info_hash_ops, TAKE_PTR(new_link));
        if (k < 0) {
                if (r >= 0) {
                        *ret_flags = flags;
                        return 0;
...
static int add_gateway_peers(
        ...
        _cleanup_(sd_netlink_message_unrefp) sd_netlink_message *req = NULL, *reply = NULL;
        _cleanup_set_free_ Set *links = NULL, *peers_by_link = NULL, *peers_by_local = NULL;
        int r;
