...
static int local_gateway_from_message(
        ...
        if (dst_len != 0) {
                /* The kernel dumps the routes of the main table in the order of their trie keys,
...
static int local_gateways_ipv4_dump(
        ...
        for (;;) {
                ...
                if (r < 0)
                        return r;
                if (type == NLMSG_DONE) /* The dump has finished, and no (more) default routes were found. */
                        return 0;
                if (type != RTM_NEWROUTE)
                        continue;
...
/* The kernel emits the routes of the IPv4 main table with the default routes first (see
         ...
         * which sd-netlink sets on all sockets it opens. In partial dispatch mode the dump arrives
         * message by message, and we can simply stop reading once we have found the default routes.
         * The remaining messages of the dump would however stay pending in the kernel, and hence
         * disturb the next request on the socket. Also sd-netlink does not expose the network
         * namespace its socket is in. We therefore use a private socket, which we close once we are
         * done, making the kernel cancel the rest of the dump. On kernels that do not honor
         * NETLINK_GET_STRICT_CHK the partial dispatch mode refuses to work, and we fall back to
         * reading the whole dump (see below).
         *
         ...
         *
         * Note that for IPv6 the kernel walks its route tree in post-order, i.e. the default routes are
         * emitted last. Hence we read the whole dump for them (see local_gateways_full_dump()). */
        ...
        if (r == -EOPNOTSUPP) {
                /* The kernel does not honor NETLINK_GET_STRICT_CHK, so the dump does not necessarily
                 * start with the default routes, and we cannot rely on the partial dispatch ordering.
                 * Read the whole dump instead, like we do for IPv6. */
                return local_gateways_full_dump(rtnl, AF_INET, ifindex, allow_via, list, n_list);
        }
        if (r < 0)
                return r;
