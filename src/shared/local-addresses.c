...
static int link_is_point_to_point(sd_netlink *rtnl, int ifindex) {
        _cleanup_(sd_netlink_message_unrefp) sd_netlink_message *req = NULL, *reply = NULL;
        unsigned flags;
        int r;
        ...
        return FLAGS_SET(flags, IFF_POINTOPOINT);
}

static bool gateway_peer_exists(
                const GatewayPeer *peers,
                size_t n_peers,
                int ifindex,
                int family,
...
static int add_gateway_peer(
        ...
        if (gateway_peer_exists(*peers, *n_peers, ifindex, family, local, peer))
                return 0;
...
static int add_gateway_peers(
        ...
        for (sd_netlink_message *m = reply; m; m = sd_netlink_message_next(m)) {
                ...
                r = link_is_point_to_point(rtnl, ifi);
                if (r < 0) {
...
int local_gateways(
        ...
        r = add_gateway_peers(rtnl, ifindex, af, candidates, n_candidates, &list, &n_list);
        if (r < 0)
