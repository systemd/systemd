...
static int on_get_all_properties(sd_bus_message *m, void *userdata, sd_bus_error *reterr_error) {
        ...
        /* The query is complete, drop the slot. Note that on_unit_removed() uses it to tell whether a query
         * is still in flight. */
...
static int on_unit_removed(sd_bus_message *m, void *userdata, sd_bus_error *reterr_error) {
        ...
        if (item->slot_get_all) {
                /* We haven't received the reply to our GetAll() query yet. Since the bus preserves message
                 * ordering, this signal was hence sent before the service manager processed our query, and
                 * refers to an earlier incarnation of the unit, which was garbage collected before we (or the
                 * caller on our behalf) took a reference on it. That's fine, the reply to our query will tell
                 * us about the current incarnation, and we'll be told if that one goes away, too. */
                log_debug("Unit %s has been removed before we retrieved its properties, ignoring.", id);
                return 0;
        }
...
static int on_reloading(sd_bus_message *m, void *userdata, sd_bus_error *reterr_error) {
        ...
        HASHMAP_FOREACH(item, d->items) {
                ...
                if (r < 0) {
                        ...
                        log_debug_errno(r, "Failed to request properties of unit %s: %m", item->name);
                        wait_for_item_fail(item);
                }
        }
