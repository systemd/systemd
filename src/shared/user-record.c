...
const char** user_record_self_modifiable_fields(UserRecord *h) {
        ...
        /* Note: if the self_modifiable_fields field in user-record is NULL we'll apply a default, if we have
         * one. If it is a non-NULL empty strv, we'll report it as explicit empty list. When we the field is
