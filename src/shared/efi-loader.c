...
int efi_measured_os(int log_level) {
        ...
        bool b;
        r = proc_cmdline_get_bool("systemd.tpm2-measured_os", PROC_CMDLINE_VALUE_OPTIONAL, &b);
        if (r < 0)
                log_debug_errno(r, "Failed to parse systemd.tpm2-measured_os= kernel command line argument, ignoring: %m");
        else if (r > 0)
                return (cached = b);
