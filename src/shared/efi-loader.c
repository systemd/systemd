...
int efi_reboot_to_firmware_supported(void) {
        ...
        r = efi_get_variable(EFI_VARIABLE_OsIndicationsSupported, NULL, &v, &s);
        if (r == -ENOENT)
...
static int get_os_indications(uint64_t *ret) {
        ...
        if (stat(EFIVAR_PATH(EFI_VARIABLE_OsIndications), &new_stat) < 0) {
                if (errno != ENOENT)
                        return -errno;
        ...
        r = efi_get_variable(EFI_VARIABLE_OsIndications, NULL, &v, &s);
        if (r == -ENOENT) {
...
int efi_set_reboot_to_firmware(bool value) {
        ...
        if (b != b_new)
                return efi_set_variable(EFI_VARIABLE_OsIndications, &b_new, sizeof(uint64_t));
...
int efi_get_boot_option(
        ...
        char variable[STRLEN(EFI_VENDOR_VARIABLE(EFI_VENDOR_GLOBAL_STR, "Boot")) + 4 + 1];
        ...
        xsprintf(variable, EFI_VENDOR_VARIABLE(EFI_VENDOR_GLOBAL_STR, "Boot%04X"), id);
...
int efi_get_boot_order(uint16_t **order) {
        ...
        r = efi_get_variable(EFI_VARIABLE_BootOrder, NULL, &buf, &l);
        if (r < 0)
                return r;
...
int efi_set_boot_order(uint16_t *order, size_t n) {
        ...
        return efi_set_variable(EFI_VARIABLE_BootOrder, order, n * sizeof(uint16_t));
}
...
int efi_loader_get_boot_usec(usec_t *firmware, usec_t *loader) {
        ...
        r = read_usec(EFI_VARIABLE_LoaderTimeInitUSec, &x);
        if (r < 0)
                return log_debug_errno(r, "Failed to read LoaderTimeInitUSec: %m");

        r = read_usec(EFI_VARIABLE_LoaderTimeExecUSec, &y);
        if (r < 0)
...
int efi_loader_get_device_part_uuid(sd_id128_t *u) {
        ...
        r = efi_get_variable_string(EFI_VARIABLE_LoaderDevicePartUUID, &p);
        if (r < 0)
                return r;
...
int efi_loader_get_entries(char ***ret) {
        ...
        r = efi_get_variable(EFI_VARIABLE_LoaderEntries, NULL, (void**) &entries, &size);
        if (r < 0)
                return r;
...
int efi_loader_get_features(uint64_t *ret) {
        ...
        r = efi_get_variable(EFI_VARIABLE_LoaderFeatures, NULL, &v, &s);
        if (r == -ENOENT) {
                _cleanup_free_ char *info = NULL;

                /* The new (v240+) LoaderFeatures variable is not supported, let's see if it's systemd-boot at all */
                r = efi_get_variable_string(EFI_VARIABLE_LoaderInfo, &info);
                if (r < 0) {
                        if (r != -ENOENT)
                                return r;
...
int efi_loader_get_config_timeout_one_shot(usec_t *ret) {
        ...
        /* stat() the EFI variable, to see if the mtime changed. If it did, we need to cache again. */
        if (stat(EFIVAR_PATH(EFI_VARIABLE_LoaderConfigTimeoutOneShot), &new_stat) < 0)
                return -errno;
        ...
        r = efi_get_variable_string(EFI_VARIABLE_LoaderConfigTimeoutOneShot, &v);
        if (r < 0)
                return r;
...
int efi_loader_update_entry_one_shot_cache(char **cache, struct stat *cache_stat) {
        ...
        /* stat() the EFI variable, to see if the mtime changed. If it did we need to cache again. */
        if (stat(EFIVAR_PATH(EFI_VARIABLE_LoaderEntryOneShot), &new_stat) < 0)
                return -errno;

        if (stat_inode_unmodified(&new_stat, cache_stat))
                return 0;

        r = efi_get_variable_string(EFI_VARIABLE_LoaderEntryOneShot, &v);
        if (r < 0)
                return r;
