...
int nft_set_element_modify_in_addr_open(
        ...
        assert(nft_set);
        assert(address);
        nfproto = nfproto_is_valid(nft_set->nfproto);
        table = ASSERT_PTR(nft_set->table);
        set = ASSERT_PTR(nft_set->set);
...
int nft_set_add(NFTSetContext *s, int nfproto, const char *table, const char *set) {
        ...
        NFTSet *c;
        c = GREEDY_REALLOC(s->sets, s->n_sets + 1);
        if (!c)
                return -ENOMEM;
...
int nft_set_context_dup(const NFTSetContext *src, NFTSetContext *dst) {
        ...
        assert(dst);
        int r;
        _cleanup_(nft_set_context_free) NFTSetContext d = (NFTSetContext) {};

        FOREACH_ARRAY(nft_set, src->sets, src->n_sets) {
                r = nft_set_add(&d, nft_set->nfproto, nft_set->table, nft_set->set);
                if (r < 0)
                        return r;
        }

        dst->n_sets = d.n_sets;
        dst->sets = TAKE_PTR(d.sets);
        d.n_sets = 0;
...
int config_parse_nft_set(
        ...
        if (isempty(rvalue)) {
                nft_set_context_free(nft_set_context);

                return 0;
        }

        for (const char *p = rvalue;;) {
                _cleanup_free_ char *tuple = NULL, *family_str = NULL, *table = NULL, *set = NULL;
                const char *q = NULL;
                ...
                r = nft_set_add(nft_set_context, nfproto, table, set);
                if (r < 0)
                        return r;
        }
