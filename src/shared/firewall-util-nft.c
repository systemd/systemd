...
int nft_set_element_op_in_addr_open(
                FirewallContext **ctx,
                bool add,
        ...
        r = nft_set_element_op_in_addr((*ctx)->nfnl, add, nfproto, af, table, set, address, prefixlen);
...
int fw_nftables_add_masquerade(
        ...
        r = nft_set_element_op_in_addr(ctx->nfnl, add, af, af, NFT_SYSTEMD_TABLE_NAME, NFT_SYSTEMD_MASQ_SET_NAME,
        ...
        return nft_set_element_op_in_addr(ctx->nfnl, add, af, af, NFT_SYSTEMD_TABLE_NAME, NFT_SYSTEMD_MASQ_SET_NAME,
                                          source, source_prefixlen);
}
...
int config_parse_nft_set_context(
        ...
        for (const char *p = rvalue;;) {
                ...
                if (r != 3) {
                        ...
                        esc = cescape(rvalue);
                        log_syntax(unit, LOG_WARNING, filename, line, r, "Failed to parse IPvxNFT set %s, ignoring", strna(esc));
                        return 0;
                }
                ...
                if (nfproto < 0) {
                        ...
                        log_syntax(unit, LOG_WARNING, filename, line, 0, "Unknown NFT protocol family %s, ignoring", strna(esc));
                        return 0;
                }
