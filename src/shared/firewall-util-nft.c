...
static int nft_message_append_setelem_ip4(
                sd_netlink_message *m,
                const union in_addr_union *source) {
        ...
        addr = source->in.s_addr;

        r = sd_netlink_message_open_container(m, NFTA_SET_ELEM_LIST_ELEMENTS);
        if (r < 0)
                return r;

        r = sd_nfnl_nft_message_append_setelem(m, 0, &addr, sizeof(addr), NULL, 0, 0);
        if (r < 0)
                return r;
...
static int nft_message_append_setelem_ip6(
                sd_netlink_message *m,
                const union in_addr_union *source) {
        ...
        r = sd_nfnl_nft_message_append_setelem(m, 0, &source->in6, sizeof(source->in6), NULL, 0, 0);
        if (r < 0)
                return r;
...
int nft_set_element_modify_ip(
        ...
        if (af == AF_INET)
                 r = nft_message_append_setelem_ip4(m, source);
        else
                 r = nft_message_append_setelem_ip6(m, source);
        if (r < 0)
                return r;
...
static const char *const nft_set_source_table[] = {
        [NFT_SET_SOURCE_HOST_ADDRESS] = "hostaddr",
        [NFT_SET_SOURCE_NETWORK] = "network",
...
static int nft_set_add(NFTSetContext *s, NFTSetSource source, int nfproto, const char *table, const char *set) {
        ...
        assert(s);
        assert(IN_SET(source, NFT_SET_SOURCE_HOST_ADDRESS, NFT_SET_SOURCE_NETWORK, NFT_SET_SOURCE_IFINDEX));
        assert(nfproto_is_valid(nfproto));
        assert(table);
        assert(set);
