...
#define DEFINE_HEX_PTR(name, hex)                                       \
        ...
        size_t name##_len = 0;                                          \
        assert_se(unhexmem_full(hex, SIZE_MAX, false, &name, &name##_len) >= 0);
...
_noreturn_ void log_test_failed_internal(const char *file, int line, const char *format, ...) _printf_(3,4);

#define log_test_failed(...) \
        log_test_failed_internal(PROJECT_FILE, __LINE__, ##__VA_ARGS__)
...
#define ASSERT_NOT_NULL(expr)                                                                                           \
        ({                                                                                                              \
                typeof(expr) _result = (expr);                                                                          \
                if (_result == NULL)                                                                                    \
                        log_error("Expected \"%s\" to be not NULL", #expr);                                             \
                _result;                                                                                                \
        })
#endif
