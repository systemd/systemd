...
static inline bool manager_errno_skip_test(int r) {
        return IN_SET(abs(r), EPERM, EACCES, EADDRINUSE, EHOSTDOWN, ENOENT, ENOMEDIUM /* cannot determine cgroup */
        );
}
...
#define REGISTER_TEST(func, ...)                                                                         \
        _Pragma("GCC diagnostic ignored \"-Wattributes\"") _section_("SYSTEMD_TEST_TABLE")               \
                        _alignptr_ _used_ _retain_ _variable_no_sanitize_address_ static const TestFunc  \
                        UNIQ_T(static_test_table_entry,                                                  \
                               UNIQ) = { .f = (union f) & (func),                                        \
                                         .name = STRINGIFY(func),                                        \
                                         .has_ret = __builtin_types_compatible_p(                        \
                                                         typeof((union f){}.int_func), typeof(&(func))), \
                                         ##__VA_ARGS__ }
...
#define DEFINE_TEST_MAIN_WITH_INTRO(log_level, intro) DEFINE_TEST_MAIN_FULL(log_level, intro, NULL)
#define DEFINE_TEST_MAIN(log_level) DEFINE_TEST_MAIN_FULL(log_level, NULL, NULL)
