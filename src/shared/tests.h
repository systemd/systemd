...
#define ASSERT_OK(expr)                                                                                          \
        ({                                                                                                       \
                if ((expr) < 0) {                                                                                \
                        log_error_errno("Assertion failed in file %s at line %d: %s (error: %m)", PROJECT_FILE, __LINE__, #expr); \
                        abort();                                                                                 \
                }                                                                                                \
        })
...
#define ASSERT_EQ(expr1, expr2)                                                                                  \
        ({                                                                                                       \
                ...
                if (_expr1 != _expr2) {                                                                          \
                        log_error("Assertion failed in file %s at line %d: "                                     \
                                  "expected %s == %s, but %d != %d",                                             \
