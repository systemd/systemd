...
/* QMP timestamps are nested objects: {"seconds": N, "microseconds": N}. Convert to usec_t. */
static int json_dispatch_qmp_timestamp(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        usec_t *ret = ASSERT_PTR(userdata);
        uint64_t seconds = 0, microseconds = 0;

        if (!variant || sd_json_variant_is_null(variant)) {
                *ret = 0;
                return 0;
        }

        static const sd_json_dispatch_field table[] = {
                { "seconds",      SD_JSON_VARIANT_UNSIGNED, sd_json_dispatch_uint64, 0,                  0 },
                { "microseconds", SD_JSON_VARIANT_UNSIGNED, sd_json_dispatch_uint64, sizeof(uint64_t),   0 },
                {},
        };

        uint64_t fields[2] = {};
        int r = sd_json_dispatch(variant, table, flags, fields);
        if (r < 0)
                return r;

        seconds = fields[0];
        microseconds = fields[1];
        *ret = seconds * USEC_PER_SEC + microseconds;
        return 0;
}
...
static void qmp_client_dispatch_event(QmpClient *c, sd_json_variant *v) {
        ...
        static const sd_json_dispatch_field table[] = {
                ...
                { "data",      SD_JSON_VARIANT_OBJECT,  sd_json_dispatch_variant,      offsetof(typeof(p), data),           0                 },
                { "timestamp", SD_JSON_VARIANT_OBJECT,  json_dispatch_qmp_timestamp,   offsetof(typeof(p), timestamp_usec), 0                 },
                {},
        };
        ...
        c->event_callback(c, p.event, p.data, p.timestamp_usec, c->event_userdata);
}
...
static void qmp_client_emit_synthetic_shutdown(QmpClient *c) {
        ...
        c->event_callback(c, "SHUTDOWN", data, /* timestamp_usec= */ 0, c->event_userdata);
}
...
int qmp_client_connect_fd(QmpClient **ret, int fd, sd_event *event) {
        ...
        *c = (QmpClient) {
         ...
         * then send qmp_capabilities which handles the response loop. If QEMU dies
