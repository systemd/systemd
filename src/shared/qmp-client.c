...
static int qmp_future_cancel(sd_future *f) {
        QmpFuture *qf = sd_future_get_private(ASSERT_PTR(f));
...
static int qmp_future_callback(
        ...
        return sd_future_resolve(f, (result || desc) ? 0 : error);
}
...
int future_get_qmp_reply(sd_future *f, sd_json_variant **ret_result, char **ret_error_desc) {
        ...
        if (sd_future_result(f) < 0)
                return sd_future_result(f);

        QmpFuture *private = sd_future_get_private(f);

        if (private->error_desc && !ret_error_desc)
                return -EIO;

        if (ret_error_desc && private->error_desc) {
                char *desc = strdup(private->error_desc);
                if (!desc)
                        return -ENOMEM;
                *ret_error_desc = desc;
        ...
        if (ret_result)
                *ret_result = sd_json_variant_ref(private->result);
        return 0;
}
