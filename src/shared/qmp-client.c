...
static int qmp_client_read_line(QmpClient *c, char **ret) {
        ...
        for (;;) {
                ...
                if (r < 0)
                        return r;
                if (r == 0) {
                        /* Non-blocking mode, no data available yet. Wait for data. USEC_INFINITY is
                         * safe here because we only use local socketpairs: if QEMU dies, the kernel
                         * closes its end and read() returns 0 (EOF) immediately — there is no network
                         * timeout scenario where we'd hang indefinitely. */
...
static int qmp_client_io_callback(sd_event_source *source, int fd, uint32_t revents, void *userdata) {
        ...
        /* Process all complete messages in the buffer. This must happen before disconnect handling
