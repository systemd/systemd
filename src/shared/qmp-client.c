...
static const char* qmp_extract_error_description(sd_json_variant *error) {
        assert(error);

        sd_json_variant *desc = sd_json_variant_by_key(error, "desc");
        return desc ? sd_json_variant_string(desc) : "unspecified error";
}
...
static int qmp_client_dispatch_reply(QmpClient *c) {
        ...
        if (id_variant) {
                ...
                const char *error_desc = error ? qmp_extract_error_description(error) : NULL;
...
static int qmp_client_dispatch_handshake(QmpClient *c) {
        ...
        case QMP_CLIENT_HANDSHAKE_CAPABILITIES_SENT: {
                ...
                if (error) {
                        const char *desc = qmp_extract_error_description(error);
                        log_debug("qmp_capabilities failed: %s", desc);
                        qmp_client_clear_current(c);
                        return -EPROTO;
                }
