...
/* This lives separately from pcrextend-util.c so that the libblkid dlopen dependency (pulled in via
 * dlopen_libblkid() below) is only linked into consumers that actually build a file-system identity word.
 * Callers that only need the Varlink measurement helpers (e.g. systemd-cryptsetup) then don't inherit a
 * spurious blkid .note.dlopen for a dlopen they never perform. */
