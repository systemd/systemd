...
static int update_pretimeout(void) {
        ...
        if (watchdog_pretimeout == USEC_INFINITY || watchdog_timeout == USEC_INFINITY)
                ...
                int sec = (int) DIV_ROUND_UP(watchdog_pretimeout, USEC_PER_SEC);
                r = ioctl(watchdog_fd, WDIOC_SETPRETIMEOUT, &sec);
                if (r < 0) {
                        /* EOPNOTSUPP means the watchdog does support pretimeouts */
                        log_full(errno == EOPNOTSUPP ? LOG_DEBUG : LOG_WARNING,
                                 "Failed to set pretimeout to %is: %m", sec);
                        return (errno == EOPNOTSUPP ? 0 : -errno);
                }
