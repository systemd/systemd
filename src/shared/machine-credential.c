...
int machine_credential_set(MachineCredential **credentials, size_t *n_credentials, const char *cred_string) {
        ...
        MachineCredential *creds = *credentials;
        ssize_t l;
        size_t n_creds = *n_credentials;
        int r;
...
int machine_credential_load(MachineCredential **credentials, size_t *n_credentials, const char *cred_path) {
        ...
        MachineCredential *creds = *credentials;
        size_t size, i, n_creds = *n_credentials;
        int r;
        ...
        if (r == 0 || !p)
                return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Missing value for --load-credential=: %s", cred_path);
