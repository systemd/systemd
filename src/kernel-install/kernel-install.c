...
typedef enum KernelSearch {
        ...
        KERNEL_CURRENT,                 /* Will try to find currrently booted kernel image */
        _KERNEL_SEARCH_MAX,
...
static int context_from_cmdline(Context *c, Action action) {
        ...
        if (free_and_strdup_warn(&c->entry_token, arg_entry_token) < 0)
                return log_oom();
...
static int context_set_up_staging_area(Context *c) {
        ...
        if (c->action == ACTION_INSPECT) {
                ...
                /* we'll leave c->staging_aread_fd invalidated, as it is an indication that the staging area
...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] COMMAND ...\n\n"
               ...
               "                               Where to take files from when using --root=/--image=\n"
               "     --extra=PATH              Specify additional file to drop into UKI's .extra.d/ directory\n"
               "\n"
