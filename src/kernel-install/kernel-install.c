...
static int specifier_entry_token(char specifier, const void *data, const char *root, const void *userdata, char **ret) {
        const Context *c = ASSERT_PTR(userdata);
        return strdup_to(ret, c->entry_token);
}

static int specifier_kernel_version(char specifier, const void *data, const char *root, const void *userdata, char **ret) {
        const Context *c = ASSERT_PTR(userdata);
        return strdup_to(ret, c->version ?: "KERNEL_VERSION");
}
...
static int context_resolve_entry_name(Context *c) {
        ...
        const Specifier table[] = {
                { 'e', specifier_entry_token,    NULL },
                { 'k', specifier_kernel_version, NULL },
        ...
        if (!filename_is_valid(resolved))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       ...
                                       c->entry_name_format ?: "%e-%k", resolved);
...
static int context_build_environment(Context *c) {
        ...
        r = strv_env_assign_many(&e,
                                 ...
                                 "KERNEL_INSTALL_ENTRY_NAME_FORMAT",  strempty(c->entry_name_format),
...
static int verb_inspect(int argc, char *argv[], uintptr_t _data, void *userdata) {
        ...
        r = table_add_many(t,
                           ...
                           TABLE_STRING, c.entry_name_format ?: "%e-%k",
