...
typedef enum Layout {
        LAYOUT_AUTO     = 0, /* This must be the first. */
        LAYOUT_UKI,
        LAYOUT_BLS,
        LAYOUT_OTHER,
        _LAYOUT_MAX,
...
static int context_set_layout_from_string(Context *c, char *s, const char *source) {
        _cleanup_free_ char *layout = s;
        Layout t;

        assert(c);
        assert(source);

        /* This takes the ownership of the input string 's'. */

        if (c->layout >= 0 || !layout)
                return 0;
...
static int context_set_machine_id_from_string(Context *c, const char *s, const char *source) {
        int r;
...
static int context_load_environment(Context *c) {
        ...
        (void) context_set_machine_id_from_string(c, getenv("MACHINE_ID"), "environment");
...
static int context_load_install_conf_one(Context *c, const char *path) {
        ...
        (void) context_set_machine_id_from_string(c, machine_id, conf);
        ...
        (void) context_set_layout_from_string(c, TAKE_PTR(layout), conf);
...
static int context_load_machine_info(Context *c) {
        ...
        (void) context_set_machine_id_from_string(c, machine_id, path);
        (void) context_set_layout_from_string(c, TAKE_PTR(layout), path);
        return 0;
}
