...
static int context_load_environment(Context *c) {
        ...
        e = getenv("MACHINE_ID");
        if (e) {
                r = sd_id128_from_string(e, &c->machine_id);
                if (r < 0)
                        log_warning_errno(r, "Failed to parse $MACHINE_ID= from environment, ignoring.");

                if (!sd_id128_is_null(c->machine_id))
                        log_debug("MACHINE_ID=%s set via environment.", SD_ID128_TO_STRING(c->machine_id));
        }

        e = getenv("BOOT_ROOT");
        if (e) {
                if (!path_is_absolute(e) || !path_is_safe(e))
                        log_warning("Invalid path specified in $BOOT_ROOT, ignoring.");
                else {
                        c->boot_root = strdup(e);
                        if (!c->boot_root)
                                return log_oom();

                        log_debug("BOOT_ROOT=%s set via environment.", c->boot_root);
                }
        }

        e = getenv("KERNEL_INSTALL_CONF_ROOT");
        if (e) {
                if (!path_is_absolute(e) || !path_is_safe(e))
                        log_warning("Invalid path specified in $KERNEL_INSTALL_CONF_ROOT, ignoring.");
                else {
                        c->conf_root = strdup(e);
                        if (!c->conf_root)
                                return log_oom();
                }
        }

        e = getenv("KERNEL_INSTALL_PLUGINS");
        if (e) {
                c->plugins = strv_split(e, NULL);
                if (!c->plugins)
                        return log_oom();
        }
...
static int context_load_install_conf_one(Context *c, const char *path) {
        ...
        /* We also read $MACHINE_ID and $BOOT_ROOT from environment, and they have higher priority. */

        if (sd_id128_is_null(c->machine_id) && machine_id) {
                r = sd_id128_from_string(machine_id, &c->machine_id);
                if (r < 0)
                        log_warning_errno(r, "Failed to parse MACHINE_ID= specified in '%s', ignoring: %m", conf);

                if (!sd_id128_is_null(c->machine_id))
                        log_debug("MACHINE_ID=%s set via install.conf.", SD_ID128_TO_STRING(c->machine_id));
        }

        if (!c->boot_root && boot_root) {
                if (!path_is_absolute(boot_root) || !path_is_safe(boot_root))
                        log_warning("Invalid BOOT_ROOT= specified in '%s', ignoring.", conf);
                else {
                        log_debug("BOOT_ROOT=%s set via install.conf.", boot_root);
                        c->boot_root = TAKE_PTR(boot_root);
                }
        }
        ...
        if (layout) {
                ...
                log_debug("%s configures layout=%s.", conf, layout);

                t = layout_from_string(layout);
                if (t >= 0)
                        c->layout = t;
                        ...
                        c->layout_other = TAKE_PTR(layout);
                }
        }

        if (initrd_generator) {
                log_debug("%s configures initrd_generator=%s.", conf, initrd_generator);
                assert(!c->initrd_generator);
                c->initrd_generator = TAKE_PTR(initrd_generator);
        }
...
static int context_load_machine_info(Context *c) {
        ...
        log_debug("Loading /etc/machine-info…");
        ...
        r = sd_id128_from_string(machine_id, &c->machine_id);
        if (r < 0) {
                log_warning_errno(r, "Failed to parse KERNEL_INSTALL_MACHINE_ID= specified in /etc/machine-info, ignoring.");
                return 0;
        }

        if (!sd_id128_is_null(c->machine_id))
                log_debug("MACHINE_ID=%s set via /etc/machine-info.", SD_ID128_TO_STRING(c->machine_id));
...
static int context_load_machine_id(Context *c) {
        ...
        else if (!ERRNO_IS_MACHINE_ID_UNSET(r))
                return log_error_errno(r, "Failed to load machine ID from /etc/machine-id: %m");

        return 0;
}

static int context_generate_machine_id(Context *c) {
        int r;
...
static int context_ensure_machine_id(Context *c) {
        ...
        r = context_generate_machine_id(c);
        if (r < 0)
                return r;
...
static int context_find_boot_root_and_entry_token(Context *c) {
        ...
        STRV_FOREACH(b, boot_root_candidate) {
                ...
                STRV_FOREACH(e, entry_token_candidate) {
                        if (is_dir_full(dfd, *e, /* follow = */ false) <= 0)
                                continue;
                ...
                if (is_dir_full(dfd, "loader/entries", /* follow = */ false) <= 0)
                        continue;
...
static int context_ensure_boot_root(Context *c) {
        ...
        FOREACH_STRING(p, "/efi", "/boot/efi")
                if (path_is_mount_point(p, /* root = */ NULL, /* flags = */ 0) > 0) {
                        log_debug("%s is a mount point, using BOOT_ROOT=%s.", p, p);
        ...
        log_debug("KERNEL_INSTALL_BOOT_ROOT autodetection yielded no candidates, using \"/boot\".");
        return 0;
}
...
static int context_ensure_boot_root_and_entry_token(Context *c) {
        int r;
...
static int context_load_plugins(Context *c) {
        ...
        if (DEBUG_LOGGING) {
                _cleanup_free_ char *joined = strv_join(c->plugins, "\n  ");
                log_debug("Plugin files:%s%s", isempty(joined) ? "" : "\n  ", strempty(joined));
        }
...
static int context_init(Context *c) {
        ...
        r = context_ensure_boot_root_and_entry_token(c);
        if (r < 0)
                return r;
...
static int context_ensure_layout(Context *c) {
        ...
        assert(c);
        assert(c->layout >= 0);
        assert(c->layout < _LAYOUT_MAX);

        if (c->kernel) {
                r = inspect_kernel(c->kernel, &c->kernel_type, NULL, NULL, NULL);
                if (r < 0)
                        return r;
        }

        if (c->layout != LAYOUT_AUTO)
                return 0;
        ...
        if (c->kernel_type == KERNEL_TYPE_UKI) {
                c->layout = LAYOUT_UKI;
                log_debug("Kernel image is %s, using layout=%s.",
                          kernel_type_to_string(c->kernel_type), layout_to_string(c->layout));
                return 0;
        }
...
static int context_build_arguments(Context *c, bool add, char ***ret) {
...
static int context_build_environment(Context *c, char ***ret) {
        ...
        r = strv_extendf(&e, "KERNEL_INSTALL_IMAGE_TYPE=%s", kernel_type_to_string(c->kernel_type));
        if (r < 0)
                return log_oom();
        ...
        r = strv_extendf(&e, "KERNEL_INSTALL_LAYOUT=%s", c->layout_other ?: layout_to_string(c->layout));
        if (r < 0)
                return log_oom();
        ...
        if (DEBUG_LOGGING) {
                _cleanup_free_ char *joined = strv_join(e, "\n  ");
                log_debug("Exporting the following environments:%s%s",
                          isempty(joined) ? "" : "\n  ", strna(joined));
        }
...
static int context_execute(Context *c, bool add) {
        _cleanup_strv_free_ char **e = NULL;
        _cleanup_free_ char **a = NULL;
        int r;
        ...
        r = context_build_arguments(c, add, &a);
        if (r < 0)
                return r;

        r = context_build_environment(c, &e);
        if (r < 0)
                return r;
...
static int verb_add(int argc, char *argv[], void *userdata) {
        ...
        return context_execute(c, /* add = */ true);
}
