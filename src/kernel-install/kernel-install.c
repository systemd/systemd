...
static int context_load_machine_info(Context *c) {
        ...
        r = sd_id128_from_string(machine_id, &c->machine_id);
        if (r < 0) {
                log_warning_errno(r, "Failed to parse KERNEL_INSTALL_MACHINE_ID= specified in '%s', ignoring.", path);
                return 0;
        }
        ...
        log_debug("MACHINE_ID=%s set via '%s'.", SD_ID128_TO_STRING(c->machine_id), path);
        return 1; /* loaded */
}
