...
static int context_set_path_strv(Context *c, char* const* strv, bool needs_absolute, const char *source, const char *name, char ***dest) {
        ...
        STRV_FOREACH(s, strv) {
                ...
                if ((needs_absolute && !path_is_absolute(*s)) || !path_is_safe(*s))
                        return log_warning_errno(SYNTHETIC_ERRNO(EINVAL),
                                                 "Invalid path for %s specified in %s, ignoring.", name, source);
...
static int context_set_plugins(Context *c, const char *s, const char *source) {
        ...
        return context_set_path_strv(c, v, /* needs_absolute = */ false, source, "plugins", &c->plugins);
}

static int context_set_initrds(Context *c, char* const* strv) {
        assert(c);
        return context_set_path_strv(c, strv, /* needs_absolute = */ false, "command line", "initrds", &c->initrds);
}
...
static int strv_extend_path(char ***a, const char *root, const char *path) {
        char *p;

        assert(a);
        assert(path);
        assert(!!root == !path_is_absolute(path));
...
static int context_build_arguments(Context *c) {
        ...
        r = strv_extend_path(&a, arg_root, c->entry_dir);
        if (r < 0)
                return log_oom();

        if (c->action == ACTION_ADD) {
                r = strv_extend_path(&a, arg_root, c->kernel);
                if (r < 0)
                        return log_oom();

                STRV_FOREACH(i, c->initrds) {
                        r = strv_extend_path(&a, arg_root, *i);
                        if (r < 0)
                                return log_oom();
                }
...
static int context_build_environment(Context *c) {
        ...
        _cleanup_free_ char *boot_root = NULL;
        int r;
        ...
        boot_root = path_join(arg_root, c->boot_root);
        if (!boot_root)
                return log_oom();
...
static int context_execute(Context *c) {
        ...
        if (DEBUG_LOGGING) {
                ...
                joined = strv_join(strv_skip(c->args, 1), " ");
