...
static int help(void) {
        ...
        printf("%s [OPTIONS...] COMMAND ...\n\n"
               "%sAdd and remove kernel and initrd images to and from the boot partition.%s\n"
               "\n%sCommands:%s\n",
...
static int parse_argv(int argc, char *argv[], char ***ret_args) {
        ...
        FOREACH_OPTION(&state, c, &arg, /* on_error= */ return c)
                ...
                OPTION_LONG("entry-type", "TYPE",
                        ...
                        if (isempty(arg) || streq(arg, "all")) {
                                arg_boot_entry_type = _BOOT_ENTRY_TYPE_INVALID;
                                break;
                        }

                        BootEntryType e = boot_entry_type_from_string(arg);
                        if (e < 0)
                                return log_error_errno(e, "Invalid entry type: %s", arg);
