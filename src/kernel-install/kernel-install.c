...
static int context_find_boot_root_and_entry_token(Context *c) {
        ...
        STRV_FOREACH(b, boot_root_candidate) {
                ...
                if (dfd < 0) {
                        if (dfd != -ENOENT)
                                return log_error_errno(dfd, "Faield to open \"%s\": %m", *b);
...
static int context_build_arguments(Context *c, bool add, char ***ret) {
        ...
        if (DEBUG_LOGGING) {
                _cleanup_free_ char *joined = strv_join(a + 1, " ");
                log_debug("Executing plugins with the following arguments: %s", strna(joined));
        }
