...
static int add_swap(
        ...
        log_debug("Found swap entry what=%s makefs=%s growfs=%s pcrfs=%s validatefs=%s noauto=%s nofail=%s dmclone=%s",
                  what,
...
static int add_mount(
        ...
        if (flags & MOUNT_DMCLONE) {
                r = generator_hook_up_dmclone(dest, what, where, opts);
                if (r < 0)
                        return r;
        }
...
static MountPointFlags fstab_options_to_flags(const char *options, bool is_swap) {
        ...
        if (!is_swap) {
                ...
                if (fstab_test_yes_no_option(options, "x-systemd.mount-dmclone\0")) {
                        flags |= MOUNT_DMCLONE;
                }
        }
...
static int parse_fstab_one(
        ...
        log_debug("Found entry what=%s where=%s type=%s makefs=%s growfs=%s pcrfs=%s validatefs=%s noauto=%s nofail=%s dmclone=%s",
