...
static int determine_device(char **what_p, char **hash_p, const char *name) {
        ...
        new_what = strjoina("/dev/mapper/", name);
        *what_p = strdup(new_what);

        if (!(*what_p))
                return log_oom();

        log_info("Using verity %s device %s.", name, *what_p);

        return 1;
}

static int determine_root(void) {
        return determine_device(&arg_root_what, &arg_root_hash, ROOT_NAME);
}

static int determine_usr(void) {
        return determine_device(&arg_usr_what, &arg_usr_hash, USR_NAME);
}
