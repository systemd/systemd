...
static int parse_pull_expression(const char *v) {
        ...
        r = sd_json_build(&j, SD_JSON_BUILD_OBJECT(
                                       SD_JSON_BUILD_PAIR("remote", SD_JSON_BUILD_STRING(remote)),
                                       SD_JSON_BUILD_PAIR_CONDITION(!!local, "local", SD_JSON_BUILD_STRING(local)),
                                       SD_JSON_BUILD_PAIR("class", SD_JSON_BUILD_STRING(image_class_to_string(class))),
                                       SD_JSON_BUILD_PAIR("type", SD_JSON_BUILD_STRING(import_type_to_string(type))),
                                       SD_JSON_BUILD_PAIR("readOnly", SD_JSON_BUILD_BOOLEAN(ro)),
                                       SD_JSON_BUILD_PAIR("verify", SD_JSON_BUILD_STRING(import_verify_to_string(verify)))));
        if (r < 0)
