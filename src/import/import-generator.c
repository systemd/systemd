...
static JsonVariant **arg_transfers = NULL;
...
STATIC_ARRAY_DESTRUCTOR_REGISTER(arg_transfers, arg_n_transfers, json_variant_unref_many);
...
static int parse_pull_expression(const char *v) {
        ...
        r = json_build(&j, JSON_BUILD_OBJECT(
                                       JSON_BUILD_PAIR("remote", JSON_BUILD_STRING(remote)),
                                       JSON_BUILD_PAIR_CONDITION(local, "local", JSON_BUILD_STRING(local)),
                                       JSON_BUILD_PAIR("class", JSON_BUILD_STRING(image_class_to_string(class))),
                                       JSON_BUILD_PAIR("type", JSON_BUILD_STRING(import_type_to_string(type))),
                                       JSON_BUILD_PAIR("readOnly", JSON_BUILD_BOOLEAN(ro)),
                                       JSON_BUILD_PAIR("verify", JSON_BUILD_STRING(import_verify_to_string(verify)))));
        if (r < 0)
...
static int transfer_generate(JsonVariant *v, size_t c) {
        int r;
        ...
        const char *remote = json_variant_string(json_variant_by_key(v, "remote"));
        ...
        const char *class = json_variant_string(json_variant_by_key(v, "class"));
        ...
        r = json_variant_format(v, /* flags= */ 0, &formatted);
        if (r < 0)
...
static int generate(void) {
        ...
        FOREACH_ARRAY(i, arg_transfers, arg_n_transfers)
                RET_GATHER(ret, transfer_generate(*i, c++));
