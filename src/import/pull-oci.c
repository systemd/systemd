...
OciPull* oci_pull_unref(OciPull *i) {
        ...
        iovec_done_many(i->layer_digests, i->n_layers);
...
static int oci_pull_process_manifest(OciPull *i, PullJob *j) {
        ...
        ORDERED_SET_FOREACH(q, i->queued_layer_jobs) {
                (void) asprintf(&q->description, "Layer %zu/%zu (%s)", present_layers + k + 1, i->n_layers, q->url);
                k++;
        }
...
static int oci_pull_save_mstack(OciPull *i) {
        ...
        if (!FLAGS_SET(i->flags, IMPORT_READ_ONLY)) {
                if (FLAGS_SET(i->flags, IMPORT_FOREIGN_UID)) {
                        r = mountfsd_make_directory_fd(dir_fd, "rw", /* flags= */ 0, /* ret_directory_fd= */ NULL);
                        if (r < 0)
                                return r;

                        r = mountfsd_make_directory_fd(dir_fd, "work", /* flags= */ 0, /* ret_directory_fd= */ NULL);
                        if (r < 0)
                                return r;
                } else {
                        if (mkdirat(dir_fd, "rw", 0755) < 0)
                                return log_error_errno(errno, "Failed to create 'rw' layer: %m");
                        if (mkdirat(dir_fd, "work", 0755) < 0)
                                return log_error_errno(errno, "Failed to create 'work' layer: %m");
                }
        }
