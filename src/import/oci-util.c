...
bool oci_image_is_valid(const char *n) {
         ...
         *
         * We implement a generalization of this, i.e. do not onsist on the single ., _, __, -, + separator,
         * but allow any number of them. And we refuse leading dots, since if used in the fs this would make
         * the files hidden, and we probably dont want that.
         */
...
int oci_ref_normalize(char **registry, char **image, char **tag) {
         ...
         *
         * If a registry is specified we look for /usr/lib/systemd/ocireg/registry.<registry>.ocireg for
         * registry-specific rules, to enforce on the reference. If no registry is specified, we look for an
         * /usr/lib/systemd/ocireg/image.<name>.ocireg file, which contains image-specific rules instead. If
         * this is not found we load /usr/lib/systemd/ocireg/default.ocireg instead. The files are encoded in
         * JSON.
         *
        ...
        r = search_and_fopen_nulstr(fn, "re", /* root= */ NULL, CONF_PATHS_NULSTR("systemd/ocireg"), &f, &path);
        if (r < 0) {
                if (r != -ENOENT)
                        return r;

                r = search_and_fopen_nulstr("default.ocireg", "re", /* root= */ NULL, CONF_PATHS_NULSTR("systemd/ocireg"), &f, &path);
                if (r < 0 && r != -ENOENT)
                        return r;
        }
        ...
        if (f) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Parse failure at %s:%u:%u: %m", path, line, column);
...
int oci_make_manifest_url(
        ...
        _cleanup_free_ char *url = strjoin("https://", repository, "/v2/", image, "/manifests/", tag);
        if (!url)
                return -ENOMEM;
...
int oci_make_blob_url(const char *repository, const char *image, const struct iovec *digest, char **ret) {
        ...
        _cleanup_free_ char *url = strjoin("https://", repository, "/v2/", image, "/blobs/", d);
        if (!url)
                return -ENOMEM;
