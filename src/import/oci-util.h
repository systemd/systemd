...
int oci_ref_normalize(char **registry, char **image, char **tag);

char *oci_digest_string(const struct iovec *iovec);

int oci_make_manifest_url(const char *repository, const char *image, const char *tag, char **ret);
int oci_make_layer_url(const char *repository, const char *image, const struct iovec *digest, char **ret);
