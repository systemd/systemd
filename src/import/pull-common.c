...
static int verify_gpg(
        ...
        STRV_FOREACH(keyring_source, keyring_sources) {
                r = strv_extendf(&keyring_copies, "%s/keyring-%zu.gpg", gpg_home, (size_t) (keyring_source - keyring_sources));
                if (r < 0) {
                        log_error_errno(r, "Failed to allocate temporary keyring path: %m");
                        goto finish;
                }

                r = copy_file(*keyring_source, strv_last(keyring_copies), 0, 0600, /* copy_flags= */ 0);
                if (r < 0) {
