...
int pull_make_auxiliary_job(
        ...
        job->calc_checksum = IN_SET(verify, IMPORT_VERIFY_CHECKSUM, IMPORT_VERIFY_SIGNATURE, IMPORT_VERIFY_PKCS7);
...
int pull_make_verification_jobs(
        ...
        if (IN_SET(verify, IMPORT_VERIFY_SIGNATURE, IMPORT_VERIFY_PKCS7) && signature_style_from_filename(fn) < 0) {
...
static int verify_pkcs7(
        ...
        if (strv_isempty(certs)) {
                log_debug("No certificates found to verify PKCS#7 for download.");
                return 1;
        }

        const unsigned char *d = signature->iov_base;
        p7 = sym_d2i_PKCS7(NULL, &d, (long) signature->iov_len);
        if (!p7)
                return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to parse PKCS7 DER signature data.");

        s = hexmem(payload->iov_base, payload->iov_len);
        if (!s)
        ...
        STRV_FOREACH(i, certs) {
                ...
                if (sym_X509_cmp_current_time(sym_X509_get0_notBefore(c)) > 0) {
                        log_debug("X509 certificate not yet valid: '%s', ignoring.", *i);
                        continue;
                }

                if (sym_X509_cmp_current_time(X509_get0_notAfter(c)) < 0) {
        ...
        return log_openssl_errors(LOG_DEBUG, "PKCS#7 validation failed");
#else
        return log_error_errno(SYNTHETIC_ERRNO(EBADMSG),
...
int pull_verify(ImportVerify verify,
        ...
        if (verify != IMPORT_VERIFY_SIGNATURE && verify != IMPORT_VERIFY_PKCS7)
                return 0;
...
int pull_job_restart_with_signature(PullJob *j, char **ret) {
        ...
        case SIGNATURE_PKCS7_PER_DIRECTORY: /* Nothing to do anymore */
                *ret = NULL;
                return 0;

        case SIGNATURE_ASC_PER_DIRECTORY: /* Try SHA256SUMS.ps7 next */
                log_debug("Got 404 for '%s', now trying to get SHA256SUMS.ps7 instead.", j->url);
                r = import_url_change_last_component(j->url, "SHA256SUMS.ps7", ret);
                if (r < 0)
