...
static int verify_gpg(
        ...
        _cleanup_strv_free_ char **keyring_sources = NULL;
        _cleanup_strv_free_ char **keyring_copies = NULL;
        ...
        if (keyring_override) {
                ...
                FOREACH_STRING(path,
                               ...
                               VENDOR_KEYRING_PATH,
                               VENDOR_KEYRING_PATH_LEGACY) {
                        r = keyring_add_if_exists(&keyring_sources, path);
                        if (r < 0) {
                                log_error_errno(r, "Failed to collect keyring '%s': %m", path);
                                goto finish;
                        }
                }

                if (strv_isempty(keyring_sources)) {
                        r = strv_extend(&keyring_sources, VENDOR_KEYRING_PATH);
                        if (r < 0) {
                                log_error_errno(r, "Failed to fall back to vendor keyring: %m");
                                goto finish;
                        }
                }
        }
         ...
         * original keyring. */
        size_t keyring_source_idx = 0;
        STRV_FOREACH(keyring_source, keyring_sources) {
                r = strv_extendf(&keyring_copies, "%s/keyring-%zu.gpg", gpg_home, keyring_source_idx);
                if (r < 0) {
                        log_error_errno(r, "Failed to allocate temporary keyring path: %m");
                        goto finish;
                }

                r = copy_file(*keyring_source, keyring_copies[keyring_source_idx], 0, 0600, /* copy_flags= */ 0);
                if (r < 0) {
        ...
        if (strv_isempty(keyring_copies)) {
                r = log_error_errno(SYNTHETIC_ERRNO(ENOENT), "Failed to find any OpenPGP keyring for verification.");
                goto finish;
        }
        ...
        if (r == 0) {
                ...
                if (signature) {
                        r = strv_extend(&cmd, sig_file_path);
                        if (r < 0) {
                                log_error_errno(r, "Failed to build gpg command line: %m");
                                _exit(EXIT_FAILURE);
                        }
