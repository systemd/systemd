...
static int verify_pkcs7(
        ...
        if (payload->iov_len > INT_MAX)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Payload length too large!");
...
#  else
        ...
        sym_X509_STORE_set_purpose(store, X509_PURPOSE_DEFAULT_ANY);
#  endif
