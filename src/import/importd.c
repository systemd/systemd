...
static int make_transfer_json(Transfer *t, sd_json_variant **ret) {
        ...
        r = sd_json_build(ret,
                       SD_JSON_BUILD_OBJECT(
                                       ...
                                       SD_JSON_BUILD_PAIR("type", SD_JSON_BUILD_STRING(transfer_type_to_string(t->type))),
                                       SD_JSON_BUILD_PAIR("remote", SD_JSON_BUILD_STRING(t->remote)),
                                       SD_JSON_BUILD_PAIR("local", SD_JSON_BUILD_STRING(t->local)),
                                       SD_JSON_BUILD_PAIR("class", SD_JSON_BUILD_STRING(image_class_to_string(t->class))),
...
static int vl_method_pull(Varlink *link, sd_json_variant *parameters, VarlinkMethodFlags flags, void *userdata) {
        ...
        if (manager_find(m, tt, p.remote))
                return varlink_errorb(link, "io.systemd.Import.AlreadyInProgress", SD_JSON_BUILD_OBJECT(SD_JSON_BUILD_PAIR_STRING("remote", p.remote)));
        ...
        if (!FLAGS_SET(flags, VARLINK_METHOD_MORE))
                return varlink_replyb(link, SD_JSON_BUILD_OBJECT(SD_JSON_BUILD_PAIR("id", SD_JSON_BUILD_UNSIGNED(t->id))));
        ...
        r = varlink_notifyb(link, SD_JSON_BUILD_OBJECT(SD_JSON_BUILD_PAIR("id", SD_JSON_BUILD_UNSIGNED(t->id))));
        if (r < 0)
                return r;
