...
static int run(int argc, char **argv) {
        ...
        if (streq(argv[1], "-c"))
                ...
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Uknown operation '%s'.", argv[1]);

        r = dlopen_libarchive();
        if (r < 0)
                return r;

        int flags = create ? O_CREAT | O_WRONLY : O_RDONLY;
        ...
        _cleanup_close_ int fd2 = RET_NERRNO(open(argv[3], O_DIRECTORY | O_CLOEXEC, 0666));
        if (fd2 < 0)
