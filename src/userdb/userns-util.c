...
static int userns_load_json(int dir_fd, const char *fn, UserNamespaceInfo **ret) {
        ...
        static const JsonDispatch dispatch_table[] = {
                { "start",  JSON_VARIANT_UNSIGNED, json_dispatch_uid_gid,  offsetof(UserNamespaceInfo, start),        JSON_MANDATORY },
                { "name",   JSON_VARIANT_STRING,   json_dispatch_string,   offsetof(UserNamespaceInfo, name),         JSON_MANDATORY },
                { "size",   JSON_VARIANT_UNSIGNED, json_dispatch_uint32,   offsetof(UserNamespaceInfo, size),         JSON_MANDATORY },
                { "target", JSON_VARIANT_UNSIGNED, json_dispatch_uid_gid,  offsetof(UserNamespaceInfo, target),       JSON_MANDATORY },
        ...
        _cleanup_free_ char *userns_fn = NULL;
        int r;
        ...
        *userns_info = (UserNamespaceInfo) {
        ...
        r = json_dispatch(v, dispatch_table, NULL, 0, userns_info);
        if (r < 0)
                return r;

        if (userns_info->start == 0)
                return -EBADMSG;
        if (userns_info->size == 0 || userns_info->size > UINT32_MAX - userns_info->start)
                return -EBADMSG;
...
int userns_load_json_by_userns_inode(int dir_fd, ino_t inode, UserNamespaceInfo **ret) {
        _cleanup_(userns_info_freep) UserNamespaceInfo *userns_info = NULL;
        _cleanup_free_ char *fn = NULL;
        int r;
        ...
        if (asprintf(&fn, "n" INO_FMT ".userns", inode) < 0)
                return -ENOMEM;
