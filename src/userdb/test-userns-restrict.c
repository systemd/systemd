...
static void test_fails_reg(int parent_fd, const char *fname) {
        errno = 0;
        assert_se(openat(parent_fd, fname, O_RDWR|O_CREAT|O_CLOEXEC) < 0);
...
static int run(int argc, char *argv[]) {
        ...
        _cleanup_close_ int userns_fd = -EBADF, host_fd1 = -EBADF, host_fd2 = -EBADF, host_tmpfs = -EBADF, afd = -EBADF, bfd = -EBADF;
        ...
        r = safe_fork("(test)", FORK_DEATHSIG, &pid);
        assert_se(r >= 0);
        if (r == 0) {
                _cleanup_close_ int private_tmpfs = -EBADF, work_fd = -EBADF;
