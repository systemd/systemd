...
static int vl_method_allocate_user_range(Varlink *link, JsonVariant *parameters, VarlinkMethodFlags flags, void *userdata) {
        ...
        if (asprintf(&reg_fn, "u" UID_FMT ".userns", start) < 0)
                return log_oom_debug();
        if (asprintf(&link_fn, "n" INO_FMT ".userns", st.st_ino) < 0)
                return log_oom_debug();
...
static int vl_method_add_mount_to_user_range(Varlink *link, JsonVariant *parameters, VarlinkMethodFlags flags, void *userdata) {

        static const JsonDispatch parameter_dispatch_table[] = {
                { "userNamespaceFileDescriptor", JSON_VARIANT_UNSIGNED, json_dispatch_uint, offsetof(AddMountParameters, userns_fd_idx), JSON_MANDATORY },
                { "mountFileDescriptor",         JSON_VARIANT_UNSIGNED, json_dispatch_uint, offsetof(AddMountParameters, mount_fd_idx),  JSON_MANDATORY },
                {}
        };
        ...
        _cleanup_(json_variant_unrefp) JsonVariant *def = NULL;
        _cleanup_free_ char *def_fn = NULL;
        ...
        r = json_dispatch(parameters, parameter_dispatch_table, NULL, 0, &p);
        if (r < 0)
                return r;
        ...
        r = uid_range_load_userns_by_fd(&uid_range, userns_fd, UID_RANGE_USERNS_OUTSIDE);
        if (r < 0)
                return r;

        if (uid_range_entries(uid_range) != 1)
                return varlink_error(link, "io.systemd.UserDatabase.UserNamespaceInvalid", NULL);

        r = userns_load_json_by_start_uid(-EBADF, uid_range->entries[0].start, &userns_info);
        if (r < 0)
                return r;

        if (userns_info->size != uid_range->entries[0].nr ||
            userns_info->userns_inode != st.st_ino)
                return varlink_error(link, "io.systemd.UserDatabase.UserNamespaceInvalid", NULL);
        ...
        log_debug("Allowing access to mount %i to user namespace %" PRIu64" ('%s' @ UID " UID_FMT ")",
                  mnt_id, (uint64_t) st.st_ino, userns_info->name, userns_info->start);
...
static int run(int argc, char *argv[]) {
        ...
        r = varlink_server_bind_method_many(
                        ...
                        "io.systemd.UserRegistry.AddMountToUserRange", vl_method_add_mount_to_user_range);
        if (r < 0)
