...
static int verb_ssh_authorized_keys(int argc, char *argv[], uintptr_t _data, void *userdata) {
        ...
        if (arg_chain) {
                /* If --chain is specified, the rest of the command line is the chain command. For
                 * compatibility with older versions which didn't use OPTION_PARSER_STOP_AT_FIRST_NONOPTION,
                 * eat up "--" before the executable path, in case a user used that to allow options to be
                 * passed to the command. The two cases which can occur are [verb, "--", user, argv0, …] and
                 * [verb, user, "--", argv0, …]. If "--" occurs again, it is treated as a normal argument.
                 */
                bool offset = argc > 1 && streq(argv[1], "--");  /* bool is used here to clamp the value to range 0..1 */
                username = argv[1 + offset];
                offset += argc > 2 + offset && streq(argv[2 + offset], "--");
                ...
                if (!path_is_absolute(argv[2 + offset]))
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "Chain invocation of ssh-authorized-keys commands requires an absolute binary path argument (got '%s').",
                                               argv[2 + offset]);

                if (!path_is_normalized(argv[2 + offset]))
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "Chain invocation of ssh-authorized-keys commands requires an normalized binary path argument (got '%s').",
                                               argv[2 + offset]);

                chain_invocation = argv + 2 + offset;
        } else {
...
static int parse_argv(int argc, char *argv[], char ***remaining_args) {
        ...
        FOREACH_OPTION_OR_RETURN(c, &opts)
                ...
                OPTION_LONG("chain", NULL, "Chain another command"):
                        arg_chain = true;
                        /* When --chain is used, stop parsing further switches so they reach the chained
                         * command. */
