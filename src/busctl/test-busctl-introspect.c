...
TEST(introspect_on_path) {
        ...
        assert_se(parse_xml_introspect(
                                  "/org/freedesktop/LogControl1", xml_org_freedesktop_LogControl1, &ops, paths) >=
                  0);
        log_debug("/* parse_xml_introspect(\"/org/freedesktop/network1\") */");
        assert_se(parse_xml_introspect("/org/freedesktop/network1", xml_org_freedesktop_network1, &ops, paths) >=
                  0);
        log_debug("/* parse_xml_introspect(\"/org/freedesktop/network1/network\") */");
        assert_se(parse_xml_introspect(
                                  "/org/freedesktop/network1/network",
                                  xml_org_freedesktop_network1_network,
                                  &ops,
                                  paths) >= 0);
        for (unsigned i = 0; i <= 20; i++) {
                ...
                assert_se(parse_xml_introspect(
                                          path, xml_org_freedesktop_network1_network_unsigned, &ops, paths) >=
                          0);
        }
