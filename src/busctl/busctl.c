...
static int message_append_cmdline(sd_bus_message *m, const char *signature, FDSet **fdset, char ***x) {
        char **p;
        int r;
        ...
        for (;;) {
                ...
                case SD_BUS_TYPE_ARRAY: {
...

                        {
                                ...
                                for (unsigned i = 0; i < n; i++) {
                                        r = message_append_cmdline(m, s, fdset, &p);
                                        if (r < 0)
                                                return r;
                                }
                        }
                ...
                case SD_BUS_TYPE_VARIANT:
                        ...
                        r = message_append_cmdline(m, v, fdset, &p);
                        if (r < 0)
                                return r;
                ...
                case SD_BUS_TYPE_UNIX_FD: {
                        ...
                        r = safe_atoi(v, &fd);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse '%s' as a file descriptor: %m", v);
                        ...
                        if (!fdset_contains(*fdset, fd))
                                return log_error_errno(SYNTHETIC_ERRNO(EBADF), "Failed to find file descriptor '%s' in available file descriptors: %m", v);
...
static int call(int argc, char **argv, void *userdata) {
        ...
        if (!isempty(argv[5])) {
                ...
                r = message_append_cmdline(m, argv[5], &fdset, &p);
                if (r < 0)
                        return r;
...
static int emit_signal(int argc, char **argv, void *userdata) {
        ...
        if (!isempty(argv[4])) {
                ...
                r = message_append_cmdline(m, argv[4], &fdset, &p);
                if (r < 0)
                        return r;
