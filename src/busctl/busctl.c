...
static int message_append_cmdline(sd_bus_message *m, const char *signature, FDSet **passed_fdset, char ***x) {
        ...
        for (;;) {
                ...
                case SD_BUS_TYPE_UNIX_FD: {
                        ...
                        fd = parse_fd(v);
                        if (fd < 0)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to parse '%s' as a file descriptor", v);
                        ...
                        if (!fdset_contains(*passed_fdset, fd))
                                return log_error_errno(SYNTHETIC_ERRNO(EBADF), "Failed to find file descriptor '%s' in available file descriptors", v);
