...
int start_transient_scope(sd_bus *bus, const char *machine_name, bool allow_pidfd, char **ret_scope) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to mangle scope name %s: %m", unmangled);
...
int run_command_bound_to_scope(
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to mangle service name %s: %m", service_name);
...
int attach_command_to_socket_in_scope(sd_bus *bus, const char *scope, const char *unit_name, const char *socket_path,
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to mangle unit name %s: %m", unit_name);

        r = unit_name_change_suffix(socket_name, ".service", &service_name);
        if (r < 0)
                return log_error_errno(r, "Failed to change unit suffix %s: %m", unit_name);
