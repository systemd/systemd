...
int attach_command_to_socket_in_scope(sd_bus *bus, const char *scope, const char *unit_name_prefix, const char *socket_path,
         ...
         *
         * The socket unit listens on `socket_path` for connections of type `socket_type`.
         * When a connection to the socket is initiated the service is started as well and passed the socket's file descriptor as fd 3.
         *
        ...
        socket_name = strjoin(unit_name_prefix, ".socket");
        if (!socket_name)
                return log_oom();

        service_name = strjoin(unit_name_prefix, ".service");
        if (!service_name)
                return log_oom();
        ...
        socket_type_str = socket_address_type_to_string(socket_type);
        if (!socket_type_str)
