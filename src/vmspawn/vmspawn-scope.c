...
int attach_command_to_socket_in_scope(sd_bus *bus, const char *scope, const char *unit_name, const char *socket_path,
                                      int socket_type, char **cmdline, char **cleanup_cmdline) {
        _cleanup_(bus_wait_for_jobs_freep) BusWaitForJobs *w = NULL;
        _cleanup_(sd_bus_error_free) sd_bus_error error = SD_BUS_ERROR_NULL;
        _cleanup_(sd_bus_message_unrefp) sd_bus_message *m = NULL, *reply = NULL;
        _cleanup_free_ char *description = NULL, *service_name = NULL, *socket_name = NULL;
         ...
         * When a connection is started the service is started to and passed its file descriptor as fd 3.
         *
