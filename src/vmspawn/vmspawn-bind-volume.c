...
int vmspawn_bind_volume_acquire(
        ...
        r = storage_acquire_volume(scope, v, /* allow_interactive_auth= */ false, &reply);
        if (r < 0) {
                err = TAKE_PTR(reply.error_id);
                goto fail;
        }
...
int vmspawn_bind_volume_attach_fd(
        ...
        if (S_ISBLK(st.st_mode))
                d->flags |= QMP_DRIVE_BLOCK_DEVICE;
        if (dt == DISK_TYPE_VIRTIO_SCSI_CDROM)
                d->flags |= QMP_DRIVE_READ_ONLY;
        d->flags |= QMP_DRIVE_REMOVABLE;
        d->link = sd_varlink_ref(link);
...
int vmspawn_bind_volume_prepare_boot(
        ...
        if (!GREEDY_REALLOC(drives->drives, drives->n_drives + n_items))
                return log_oom();

        FOREACH_ARRAY(it, items, n_items) {
                BindVolume *v = *it;
