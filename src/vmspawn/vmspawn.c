...
static int run_virtual_machine(void) {
        ...
        /* Apparently, qemu reads floating point numbers, hence let's be precise. We don't use floating %f
         * floating point formatting here since it's locale dependent, and qemu doesn't want that. */
        if (asprintf(&mem, "%zu.%05zuM",
                     arg_qemu_mem / U64_MB,
                     (arg_qemu_mem % U64_MB) * UINT64_C(100000) / U64_MB) < 0)
                return log_oom();
