...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] [ARGUMENTS...]\n\n"
               ...
               "                           (FORMAT: raw, qcow2; DISKTYPE: virtio-blk, virtio-scsi, nvme)\n"
...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        if (need_scsi_controller) {
                if (strv_extend(&cmdline, "-device") < 0)
                        return log_oom();
                if (strv_extend(&cmdline, "virtio-scsi-pci,id=vmspawn_scsi") < 0)
                        return log_oom();
        }
        ...
        if (arg_image) {
                ...
                case DISK_TYPE_VIRTIO_SCSI: {
                        ...
                        r = disk_serial(escaped_image_fn, 30, &serial);
                        if (r < 0)
                                return log_oom();
                        if (strv_extendf(&cmdline, "scsi-hd,bus=vmspawn_scsi.0,drive=vmspawn,bootindex=1,serial=%s", serial) < 0)
                                return log_oom();
                        break;
                }
                case DISK_TYPE_NVME: {
