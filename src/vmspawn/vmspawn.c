...
static Firmware arg_firmware_type = FIRMWARE_AUTO;
...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] [ARGUMENTS...]\n\n"
               ...
               "     --firmware=auto|uefi|bios|none|PATH|list|describe\n"
               "                           Select firmware to use, or a firmware definition\n"
               "                           file (or list/describe available)\n"
        ...
        while ((c = getopt_long(argc, argv, "+hD:i:xM:nqs:G:S:", options, NULL)) >= 0)
                ...
                case ARG_FIRMWARE: {
                        if (isempty(optarg)) {
                                ...
                                arg_firmware_type = FIRMWARE_AUTO;
                        ...
                        Firmware f = firmware_from_string(optarg);
                        if (f >= 0) {
                                arg_firmware = mfree(arg_firmware);
                                arg_firmware_type = f;
                                arg_firmware_describe = false;
...
static int cmdline_add_credentials(char ***cmdline, int smbios_dir_fd, const char *smbios_dir) {
        ...
        FOREACH_ARRAY(cred, arg_credentials.credentials, arg_credentials.n_credentials) {
                ...
                if (smbios_supported()) {
                        ...
                        log_warning("fw_cfg key '%s' exceeds %d character limit, passing credential via kernel command line.", key, QEMU_FW_CFG_MAX_KEY_LEN);
                        ...
                        log_warning("Both SMBIOS and fw_cfg are not supported, passing credential via kernel command line");
                        if (strv_extendf(&arg_kernel_cmdline_extra,
                                         "systemd.set_credential_binary=%s:%s", cred->id, cred_data_b64) < 0)
                                return log_oom();
                } else
        ...
        if (arg_linux_image_type == KERNEL_IMAGE_TYPE_UNKNOWN) {
                if (arg_firmware_type == FIRMWARE_UEFI)
                        return log_error_errno(
                                        ...
                                        arg_linux);
                if (arg_firmware_type == FIRMWARE_AUTO)
