...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        if (ARCHITECTURE_NEEDS_PCIE_ROOT_PORTS) {
                ...
                /* Cap so the multifunction-packed root ports fit in 24 pcie.0 device-numbers
                 * (24 × 8 = 192), leaving generous room on pcie.0 for user-supplied -device
                 * lines (e.g. via $SYSTEMD_VMSPAWN_QEMU_EXTRA). Chassis/slot are uint8_t so
                 * 192 ≤ 255 still satisfies that constraint. */
                if (n_pcie_ports > 192)
                        return log_error_errno(SYNTHETIC_ERRNO(E2BIG),
                                               "Too many PCIe root ports requested (%zu, max 192). "
