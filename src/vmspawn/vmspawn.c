...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
         ...
         * vmspawn_qmp_init() sends them to QEMU via getfd + SCM_RIGHTS. */
        ...
        r = vmspawn_qmp_setup_drives(qmp, qmp_drives.drives, qmp_drives.n);
        if (r < 0)
                return r;
        ...
        r = vmspawn_qmp_setup_virtiofs(qmp, qmp_virtiofs.entries, qmp_virtiofs.n);
        if (r < 0)
                return r;
        ...
        r = vmspawn_qmp_setup(&qmp_ctx, TAKE_PTR(qmp), runtime_dir, arg_runtime_scope, getuid(), &qmp_control_address);
        if (r < 0)
                return r;
