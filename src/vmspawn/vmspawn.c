...
static int resolve_disk_driver(DiskType dt, const char *filename, QmpDriveInfo *info) {
        int r;
...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        _cleanup_(qmp_network_info_done) QmpNetworkInfo qmp_network = { .fd = -EBADF };
        ...
        _cleanup_(qmp_vsock_info_done) QmpVsockInfo qmp_vsock = { .fd = -EBADF };
        if (use_vsock) {
                qmp_vsock.fd = vhost_device_fd;

                if (qmp_vsock.fd < 0) {
                        qmp_vsock.fd = open("/dev/vhost-vsock", O_RDWR|O_CLOEXEC);
                        if (qmp_vsock.fd < 0)
                                return log_error_errno(errno, "Failed to open /dev/vhost-vsock as read/write: %m");
                }

                r = vsock_fix_child_cid(qmp_vsock.fd, &child_cid, arg_machine);
                if (r < 0)
                        return log_error_errno(r, "Failed to fix CID for the guest VSOCK socket: %m");

                qmp_vsock.cid = child_cid;
        }
        ...
        _cleanup_(qmp_virtiofs_infos_done) QmpVirtiofsInfos qmp_virtiofs = {};
        ...
        if (arg_directory) {
                ...
                if (!GREEDY_REALLOC(qmp_virtiofs.entries, qmp_virtiofs.n + 1))
                        return log_oom();

                qmp_virtiofs.entries[qmp_virtiofs.n++] = (QmpVirtiofsInfo) {
                        .id          = strdup("rootdir"),
                        .socket_path = TAKE_PTR(listen_address),
                        .tag         = strdup("root"),
                };

                if (!qmp_virtiofs.entries[qmp_virtiofs.n - 1].id ||
                    !qmp_virtiofs.entries[qmp_virtiofs.n - 1].tag)
                        return log_oom();
        ...
        if (r < 0)
                return r;
        r = qemu_config_keyf(config_file, "fd", "%d", qmp_fds[1]);
        if (r < 0)
                return r;
         ...
         * (if any) live in their respective QmpInfo structs and will be closed after
        ...
        if (arg_image) {
                ...
                QmpDriveInfo *d = &qmp_drives.drives[qmp_drives.n++];
                *d = (QmpDriveInfo) { .fd = -EBADF, .overlay_fd = -EBADF };
        ...
        r = vmspawn_varlink_init(&qmp, TAKE_FD(qmp_fds[0]), event);
        if (r < 0)
                return r;

        /* Device setup via QMP — all before resuming vCPUs */
        r = vmspawn_varlink_setup_drives(qmp, &qmp_drives);
        if (r < 0)
                return r;

        if (qmp_network.type) {
                r = vmspawn_varlink_setup_network(qmp, &qmp_network);
                if (r < 0)
                        return r;
        }

        r = vmspawn_varlink_setup_virtiofs(qmp, &qmp_virtiofs);
        if (r < 0)
                return r;

        r = vmspawn_varlink_setup_rng(qmp);
        if (r < 0)
                return r;

        r = vmspawn_varlink_setup_balloon(qmp);
        if (r < 0)
                return r;

        if (!sd_id128_is_null(vmgenid)) {
                r = vmspawn_varlink_setup_vmgenid(qmp, vmgenid);
                if (r < 0)
                        return r;
        }

        if (qmp_vsock.fd >= 0) {
                r = vmspawn_varlink_setup_vsock(qmp, &qmp_vsock);
                if (r < 0)
                        return r;
        }

        /* Resume vCPUs and switch to async event processing */
        r = vmspawn_varlink_start(qmp);
        if (r < 0)
                return r;

        /* Varlink server for VM control */
        _cleanup_(vmspawn_varlink_context_freep) VmspawnVarlinkContext *qmp_ctx = NULL;
        _cleanup_free_ char *qmp_control_address = NULL;
        r = vmspawn_varlink_setup(&qmp_ctx, TAKE_PTR(qmp), runtime_dir, &qmp_control_address);
        if (r < 0)
                return r;
        ...
        if (arg_register != 0) {
                ...
                const MachineRegistration reg = {
                        ...
                        .control_address      = qmp_control_address,
