...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
         ...
         * sockets, runtime mounts, and SSH key material. If a previous vmspawn instance was killed without 
         * cleanup (e.g. SIGKILL), the directory may already exist with stale contents. This is harmless: 
         * varlink's sockaddr_un_unlink() removes stale sockets before bind(), and other files (QEMU config, 
         * SSH keys) are created fresh. This matches nspawn's approach of not proactively cleaning stale 
         * runtime directories. */
        _cleanup_free_ char *runtime_dir = NULL;
        _cleanup_(rm_rf_physical_and_freep) char *runtime_dir_destroy = NULL;
        r = runtime_directory_make(arg_runtime_scope, subdir, &runtime_dir, &runtime_dir_destroy);
        if (r < 0)
