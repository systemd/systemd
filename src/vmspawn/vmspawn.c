        ...
        while ((c = getopt_long(argc, argv, "+hi:M", options, NULL)) >= 0)
                ...
                case ARG_VSOCK_CID: {
                        ...
                        if (isempty(optarg))
                                ...
                                if (r == -ERANGE)
                                        return log_error_errno(r, "Invalid valid --vsock-cid: %m");
                                if (r < 0)
...
static int run_virtual_machine(void) {
        ...
        r = strv_extend_strv(&cmdline, STRV_MAKE("-cpu", "max"), false);
        if (r < 0)
                return log_oom();
        ...
        if (use_vsock) {
                ...
                if (r == -ENOMEM)
                        return log_oom();
                if (r < 0)
                        log_error_errno(r, "Failed to call getsockname on vsock: %m");
        }
