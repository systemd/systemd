...
static int start_virtiofsd(
        ...
        if (mstack) {
                if (!GREEDY_REALLOC(except_fds, n_except_fds + 3 + mstack->n_mounts))
                        return log_oom();

                except_fds[n_except_fds++] = userns_fd;

                if (mstack->root_mount_fd >= 0)
                        except_fds[n_except_fds++] = mstack->root_mount_fd;
                if (mstack->usr_mount_fd >= 0)
                        except_fds[n_except_fds++] = mstack->usr_mount_fd;

                FOREACH_ARRAY(m, mstack->mounts, mstack->n_mounts) {
                        int fd = m->dissected_image ?
                                m->dissected_image->partitions[PARTITION_ROOT].fsmount_fd :
                                m->mount_fd >= 0 ? m->mount_fd : m->what_fd;
                        if (fd >= 0)
...
static int prepare_mstack(
        ...
        *ret_userns_fd = -EBADF;
        ...
        free_and_replace(arg_mstack, resolved);
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to create temporary .mstack mount directory: %m");
        ...
        MStackFlags mstack_flags = mstack_is_read_only(mstack) ? MSTACK_RDONLY : 0;
...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        bool mstack_read_only = false;
        const char *root_directory = NULL;
        int r;
        ...
        r = prepare_mstack(runtime_dir, &mstack_temp_dir, &mstack, &mstack_userns_fd);
        if (r < 0)
                return r;

        mstack_read_only = mstack && mstack_is_read_only(mstack);
        root_directory = mstack_temp_dir ?: arg_directory;
        ...
        if (root_directory) {
                ...
                r = start_virtiofsd(
                                ...
                                mstack,
                                mstack_read_only ? MSTACK_RDONLY : 0,
                                mstack_userns_fd,
                                runtime_dir,
                                &listen_address,
                                &child);
        ...
        if (arg_register != 0) {
                ...
                const MachineRegistration reg = {
                        ...
                        .class                = "vm",
                        .pidref               = &child_pidref,
                        .root_directory       = mstack ? NULL : arg_directory,
                        .vsock_cid            = child_cid,
