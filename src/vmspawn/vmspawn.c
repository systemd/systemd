...
static int vmspawn_dispatch_notify_fd(sd_event_source *source, int fd, uint32_t revents, void *userdata) {
        ...
        if (r != 0) {
                ...
                /* If we are done or are seeing an error we'll turn of floating mode, which means the even
                 * loop itself won't keep the event source pinned anymore, and since noone else (presumably)
                 * keeps a reference anymore we'll release the whole thing. */
