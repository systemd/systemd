...
static int verify_arguments(void) {
        if (arg_ephemeral && arg_mstack)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                     arg_machine ? "Machine '%s' is a mount stack, which cannot be combined with --ephemeral." :
                                                   "--ephemeral and --mstack= may not be combined.",
                                     arg_machine);
        ...
        if (arg_mstack && !arg_linux)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                     arg_machine ? "Machine '%s' is a mount stack, which requires --linux= to be specified." :
                                                   "--mstack= requires --linux= to be specified.",
                                     arg_machine);
