...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "+hD:i:M:nq", options, NULL)) >= 0)
                ...
                case ARG_PASS_SSH_KEY:
                        r = parse_boolean(optarg);
                        if (r < 0) {
                                log_error("Failed to parse --pass-ssh-key= argument: %s", optarg);
                                return r;
                        }
...
static int generate_ssh_keypair(const char *key_path, const char *key_type) {
        ...
        r = pidref_safe_fork(
                        ssh_keygen,
                        FORK_RESET_SIGNALS|FORK_CLOSE_ALL_FDS|FORK_DEATHSIG_SIGTERM|FORK_LOG|FORK_RLIMIT_NOFILE_SAFE|FORK_REARRANGE_STDIO,
                        &child_pidref);
        if (r < 0)
                return r;
        if (r == 0) {
        ...
        r = pidref_wait_for_terminate(&child_pidref, &si);
        if (r < 0)
                return log_error_errno(r, "Failed to wait for child process ssh-keygen: %m");

        if (si.si_status != 0)
                return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP), "ssh-keygen exited with a non-zero exit code: %d", si.si_status);
...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        if (ssh_public_key_path && ssh_private_key_path) {
                ...
                cred_path = strjoin("ssh.ephemeral-key.all-users:", ssh_public_key_path);
                if (!cred_path)
                        return log_oom();
