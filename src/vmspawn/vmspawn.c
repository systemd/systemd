...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        if (ARCHITECTURE_NEEDS_PCIE_ROOT_PORTS) {
                ...
                /* PCI device numbers are 5 bits (0x00-0x1f). With base slot 0x10 and the
                 * 0x1f device-number unusable (ICH9 LPC sits at 0x1f.0 single-function on
                 * q35, blocking the rest of that slot for multifunction packing), the
                 * usable range is 0x10..0x1e = 15 slots × 8 functions = 120 ports. */
                if (n_pcie_ports > 120)
                        return log_error_errno(SYNTHETIC_ERRNO(E2BIG),
                                               "Too many PCIe root ports requested (%zu, max 120). "
                ...
                for (size_t i = 0; i < n_pcie_ports; i++) {
                         ...
                         * independently hot-pluggable (QEMU docs/pcie.txt §5.1). Base slot
                         * 0x10 sits above the auto-assigned virtio devices and below the
                         * 0x1f LPC reservation on q35. */
                        size_t pci_slot = 0x10 + i / 8;
                        size_t pci_fn   = i % 8;
                        assert(pci_slot <= 0x1e); /* must not collide with ICH9 LPC at 0x1f */
