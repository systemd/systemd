...
static int start_tpm(
         ...
         * added in 0.11. */
        _cleanup_strv_free_ char **argv = strv_new(swtpm_setup, "--tpm-state", state_dir, "--tpm2", "--pcr-banks", "sha256", "--not-overwrite", "--profile-name", "default-v2");
        if (!argv)
                return log_oom();
        ...
        if (r == -EPROTO) {
                 ...
                 * version. */
                argv = strv_remove(argv, "--profile-name");
                if (!argv)
                        return log_oom();
                argv = strv_remove(argv, "default-v2");
                if (!argv)
                        return log_oom();
