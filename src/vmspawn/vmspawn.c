        ...
        while ((c = getopt_long(argc, argv, "+hi:", options, NULL)) >= 0)
                ...
                case ARG_QEMU_VSOCK:
                        if (isempty(optarg))
                                ...
                                r = parse_boolean(optarg);
                                if (r < 0)
                                        return log_error_errno(r, "Failed to parse boolean from --qemu-vsock=%s: %m", optarg);
                                arg_qemu_vsock = r;
                        }
                        break;
                ...
                case ARG_VSOCK_CID: {
                        ...
                        if (isempty(optarg))
                                ...
                                if (r < 0)
                                        return log_error_errno(r, "Failed to parse unsigned int from --vsock-cid=%s: %m", optarg);
                        }
...
static int run_virtual_machine(void) {
        ...
        if (!ovmf_config->supports_sb && arg_secboot)
