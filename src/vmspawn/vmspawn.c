...
static int shutdown_vm_graceful(sd_event_source *s, const struct signalfd_siginfo *si, void *userdata) {
        ...
        if (!userdata) {
                /* if we hit this everything else has failed previously so fall straight through to on_orderly_shutdown */
                goto fallback;
        }
         ...
         * 3. on_orderly_shutdown always available, just SIGKILLs qemu
         */

        r = bus_open_in_machine(&bus, ssh_info->cid, ssh_info->port, ssh_info->private_key_path);
        if (r < 0) {
                log_error_errno(r, "Failed to connect to VM to forward signal: %m");
                goto fallback;
        }
        ...
        log_error_errno(r, "Failed to shutdown VM via logind: %s", bus_error_message(&error, r));
        ...
        log_error_errno(r, "Failed to shutdown VM via systemd: %s", bus_error_message(&error, r));

fallback:
        return on_orderly_shutdown(s, si, userdata);
}
...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        SSHInfo ssh_info;
        ShutdownInfo shutdown_info = (ShutdownInfo) {
                .ssh_info = &ssh_info,
                .pidref = &child_pidref,
        };
        ...
        if (child_cid != VMADDR_CID_ANY && ssh_private_key_path) {
                ...
                (void) sd_event_add_signal(event, NULL, SIGINT | SD_EVENT_SIGNAL_PROCMASK, on_orderly_shutdown, &shutdown_info);
                (void) sd_event_add_signal(event, NULL, SIGTERM | SD_EVENT_SIGNAL_PROCMASK, on_orderly_shutdown, &shutdown_info);
                (void) sd_event_add_signal(event, NULL, (SIGRTMIN+4) | SD_EVENT_SIGNAL_PROCMASK, on_orderly_shutdown, &shutdown_info);
        }
