...
typedef struct BindVolumesContext {
        BindVolume **items;
        size_t n_items;
} BindVolumesContext;
static BindVolumesContext arg_bind_volumes = {};
...
static void bind_volumes_context_done(BindVolumesContext *c) {
        if (!c)
                return;
        FOREACH_ARRAY(v, c->items, c->n_items)
                bind_volume_free(*v);
        c->items = mfree(c->items);
        c->n_items = 0;
}
STATIC_DESTRUCTOR_REGISTER(arg_bind_volumes, bind_volumes_context_done);
...
static int prepare_device_info(const char *runtime_dir, MachineConfig *c) {
        ...
        r = vmspawn_bind_volume_prepare_boot(arg_runtime_scope,
                                             arg_bind_volumes.items, arg_bind_volumes.n_items,
                                             drives);
        if (r < 0)
                return r;
