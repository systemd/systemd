...
static int prepare_primary_drive(DriveInfos *drives) {
        ...
        if (arg_ephemeral && !FLAGS_SET(d->flags, QMP_DRIVE_READ_ONLY)) {
                 ...
                 * room for it. Put it next to the image, where nspawn puts its ephemeral copy too, and
                 * fall back to the directory QEMU picks for its own -snapshot overlays, see
                 * create_tmp_file() in its block.c. A block device has no directory that is any better
                 * than that fallback, and /dev is a tmpfs, so don't look next to it. */
                ...
                FOREACH_ARGUMENT(overlay_dir, image_dir, var_tmp) {
                        ...
                        overlay_fd = open(overlay_dir, O_TMPFILE | O_RDWR | O_CLOEXEC, 0600);
                        if (overlay_fd >= 0)
                                break;

                        log_debug_errno(errno, "Failed to create ephemeral overlay in '%s', trying next directory: %m", overlay_dir);
                }
                if (overlay_fd < 0)
                        return log_error_errno(errno, "Failed to create ephemeral overlay in '%s': %m", var_tmp);
