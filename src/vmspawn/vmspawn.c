...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        _cleanup_(network_info_done) NetworkInfo network = { .fd = -EBADF };
        ...
        if (arg_directory) {
                ...
                if (!virtiofs.entries[virtiofs.n_entries - 1].id ||
                    !virtiofs.entries[virtiofs.n_entries - 1].tag)
                        return log_oom();
        ...
        if (ARCHITECTURE_NEEDS_PCIE_ROOT_PORTS) {
                ...
                size_t n_pcie_ports = 1 +
                                arg_extra_drives.n_drives +   /* drives (+ possible SCSI controller) */
                                1 +                           /* network */
                ...
                for (size_t i = 0; i < n_pcie_ports; i++) {
                        ...
                        if (i < n_pcie_ports - 10)
                                ...
                                xsprintf(id, "vmspawn-hotplug-pci-root-port-%zu", i - (n_pcie_ports - 10));
        ...
        if (arg_image) {
                ...
                *d = (DriveInfo) { .fd = -EBADF, .overlay_fd = -EBADF };
                ...
                int open_flags = ((arg_ephemeral || d->read_only) ? O_RDONLY : O_RDWR) | O_CLOEXEC | O_NOCTTY;
         ...
         * file above. Each PCI device that will be hotplugged via QMP device_add gets a port. */
        if (ARCHITECTURE_NEEDS_PCIE_ROOT_PORTS) {
                unsigned port = 0;
                ...
                FOREACH_ARRAY(d, drives.drives, drives.n_drives) {
                        ...
                        if (asprintf(&d->pcie_port, "vmspawn-pcieport-%u", port++) < 0)
                                return log_oom();
                }
                if (need_scsi)
                        if (asprintf(&drives.scsi_pcie_port, "vmspawn-pcieport-%u", port++) < 0)
                                return log_oom();

                if (network.type)
                        if (asprintf(&network.pcie_port, "vmspawn-pcieport-%u", port++) < 0)
                                return log_oom();

                FOREACH_ARRAY(v, virtiofs.entries, virtiofs.n_entries)
                        if (asprintf(&v->pcie_port, "vmspawn-pcieport-%u", port++) < 0)
                                return log_oom();

                if (vsock.fd >= 0)
                        if (asprintf(&vsock.pcie_port, "vmspawn-pcieport-%u", port++) < 0)
                                return log_oom();
        }
        ...
        r = vmspawn_qmp_setup_drives(bridge, &drives);
        if (r < 0)
                return r;

        if (network.type) {
                r = vmspawn_qmp_setup_network(bridge, &network);
                if (r < 0)
                        return r;
        }
