...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        if (ssh_public_key_path && ssh_private_key_path) {
                ...
                r = machine_credential_set(
                                ...
                                "[Service]\n"
                                "ExecStart=\n"
                                "ExecStart=/usr/sbin/sshd -i -o 'AuthorizedKeysFile=%d/ssh.ephemeral-authorized_keys-all .ssh/authorized_keys'\n"
        ...
        if (arg_register) {
                ...
                r = register_machine(arg_machine, arg_uuid, trans_scope, arg_directory, child_cid, vm_address, ssh_private_key_path);
                if (r < 0)
                        return r;
        }
