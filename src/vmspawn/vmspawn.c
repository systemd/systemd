...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        if (!arg_no_ssh_keygen && arg_ssh_key_type != SSH_KEYTYPE_UNKNOWN) {
                r = generate_ssh_keypair(arg_ssh_key_type, &ssh_private_key, &ssh_public_key);
                if (r == -EOPNOTSUPP)
                        log_warning_errno(r, "Failed to generate an SSH keypair, ignoring: %m");
                else if (r < 0)
                        return log_error_errno(r, "Failed to generate an SSH keypair: %m");
        }

        if (ssh_public_key && ssh_private_key) {
                _cleanup_free_ char *scope_prefix = NULL, *cred = NULL;
                ...
                r = loop_write(fd, ssh_private_key, strlen(ssh_private_key));
                if (r < 0)
