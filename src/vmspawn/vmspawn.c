...
static bool arg_trim_disk = true;
...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] [ARGUMENTS...]\n\n"
               ...
               "     --trim-disk=BOOL      Control whether disk trimming is enabled\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        static const struct option options[] = {
                ...
                { "trim-disk",         required_argument, NULL, ARG_TRIM_DISK         },
        ...
        while ((c = getopt_long(argc, argv, "+hD:i:M:nq", options, NULL)) >= 0)
                ...
                case ARG_TRIM_DISK:
                        r = parse_boolean(optarg);
                        if (r < 0) {
                                log_error("Failed to parse --trim-disk= argument: %s", optarg);
                                return r;
                        }
...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        if (arg_image) {
                ...
                r = strv_extendf(&cmdline, "if=none,id=mkosi,file=%s,format=raw,discard=%s", escaped_image, on_off(arg_trim_disk));
                if (r < 0)
                        return log_oom();
