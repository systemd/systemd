...
#undef FIRMWARE_LOCATIONS
        ...
        FOREACH_STRING(firmware, FIRMWARE_LOCATIONS) {
                if (access(firmware, F_OK) == 0) {
                        log_debug("Couldn't find OVMF firmware blob with secure boot support, "
...
#undef FIRMWARE_LOCATIONS
        return log_debug_errno(SYNTHETIC_ERRNO(ENOENT), "Couldn't find OVMF UEFI firmware blob.");
}
