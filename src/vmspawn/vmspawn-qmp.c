...
static void on_qmp_event(
        ...
        r = sd_json_buildo(
                        ...
                        SD_JSON_BUILD_PAIR_UNSIGNED("timestampUSec", timestamp_usec));
        if (r < 0) {
...
static int qmp_detect_features(QmpClient *qmp, QemuFeatures *ret) {
        ...
        *ret = (QemuFeatures) {};
        ...
        JSON_VARIANT_ARRAY_FOREACH(entry, schema) {
                ...
                if (!name || !streq(sd_json_variant_string(name), "BlockdevAioOptions"))
                        continue;

                sd_json_variant *meta = sd_json_variant_by_key(entry, "meta-type");
                if (!meta || !streq(sd_json_variant_string(meta), "enum"))
                        break;
                ...
                JSON_VARIANT_ARRAY_FOREACH(member, members) {
                        ...
                        if (mname && streq(sd_json_variant_string(mname), "io_uring")) {
                                ret->io_uring = true;
                                break;
                        }
                }
...
static int qmp_build_blockdev_add(const QmpDriveInfo *drive, bool io_uring, sd_json_variant **ret) {
        ...
        /* aio is a member of BlockdevOptionsFile (the protocol-level driver), not of BlockdevOptions
         * (the format-level base). It must be inside the "file" sub-object. discard is in the
         * BlockdevOptions base and correctly stays at the top level. */
