...
static int on_qmp_setup_complete(
        ...
        if (error < 0) {
                log_warning_errno(error, "%s failed: %s", label, strna(error_desc));
 ...
 *
 * Note: any allocation that could fail (path string, JSON args) runs BEFORE the invoke so a
 * late OOM cannot orphan an fdset on QEMU's side. *ret_path is only written on full success
 * (asprintf would otherwise leave undefined contents in *ret_path on its own failure). */
...
static int vmspawn_qmp_probe_io_uring(VmspawnQmpBridge *bridge) {
        ...
        r = qmp_client_call(bridge->qmp, "blockdev-add", QMP_CLIENT_ARGS(args), /* ret_result= */ NULL, &error_desc);
        if (r < 0)
                return r;
        if (error_desc) {
        ...
        r = qmp_client_call(bridge->qmp, "blockdev-del", QMP_CLIENT_ARGS(del_args), /* ret_result= */ NULL, /* ret_error_desc= */ NULL);
        if (r < 0)
