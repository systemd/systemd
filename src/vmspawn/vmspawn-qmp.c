...
static int qmp_build_blockdev_add_file(const QmpFileNodeParams *p, sd_json_variant **ret) {
        ...
        return sd_json_buildo(
                        ...
                        SD_JSON_BUILD_PAIR_CONDITION(FLAGS_SET(p->flags, QMP_DRIVE_IO_URING), "aio", SD_JSON_BUILD_STRING("io_uring")),
...
static int qmp_build_blockdev_add_format(const QmpFormatNodeParams *p, sd_json_variant **ret) {
        ...
        return sd_json_buildo(
                        ...
                        SD_JSON_BUILD_PAIR_CONDITION(FLAGS_SET(p->flags, QMP_DRIVE_READ_ONLY), "read-only", SD_JSON_BUILD_BOOLEAN(true)),
                        SD_JSON_BUILD_PAIR_CONDITION(FLAGS_SET(p->flags, QMP_DRIVE_DISCARD), "discard", SD_JSON_BUILD_STRING("unmap")),
...
static int qmp_add_file_node_async(QmpClient *qmp, const QmpFileNodeParams *p) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *args = NULL;
        int r;
...
typedef struct EphemeralDriveCtx {
        ...
        QmpDriveFlags flags;       /* subset: QMP_DRIVE_DISCARD, QMP_DRIVE_BOOT */
} EphemeralDriveCtx;
...
static int on_ephemeral_create_concluded(QmpClient *qmp, void *userdata) {
        ...
        QmpFormatNodeParams overlay_fmt_params = {
                ...
                .flags          = ctx->flags & QMP_DRIVE_DISCARD,
        };
...
static int qmp_setup_ephemeral_drive(VmspawnQmpBridge *bridge, QmpClient *qmp, DriveInfo *drive) {
        ...
        r = qmp_add_file_node_async(qmp, &base_file_params);
        if (r < 0)
        ...
        *ectx = (EphemeralDriveCtx) {
                ...
                .flags              = drive->flags & (QMP_DRIVE_DISCARD|QMP_DRIVE_BOOT),
        };
...
static int qmp_setup_regular_drive(VmspawnQmpBridge *bridge, QmpClient *qmp, DriveInfo *drive) {
        ...
        r = qmp_add_file_node_async(qmp, &file_params);
        if (r < 0)
...
int vmspawn_qmp_setup_drives(VmspawnQmpBridge *bridge, DriveInfos *drives) {
        ...
        for (size_t i = 0; i < drives->n_drives; i++) {
                r = qmp_setup_drive(bridge, qmp, &drives->drives[i]);
                if (r < 0)
                        return r;
        }
...
int vmspawn_qmp_init(VmspawnQmpBridge **ret, int qmp_fd_consume, sd_event *event) {
        ...
        *bridge = (VmspawnQmpBridge) {};
        ...
        (void) qmp_client_set_description(bridge->qmp, "vmspawn-qmp-client");
