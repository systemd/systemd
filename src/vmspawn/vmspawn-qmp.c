...
static int qmp_fdset_add_async(QmpClient *qmp, int fd, char **ret_path) {
        ...
        r = qmp_client_execute_send_fd(qmp, "add-fd", args, fd, on_add_fd_complete, NULL);
        if (r < 0)
                return r;
...
/* Issue blockdev-add for a file node. io_uring support was probed once during the blocking
 * QMP handshake — the cached result in QmpClient is used here directly, no retry needed. */
static int qmp_add_file_node_async(QmpClient *qmp, const char *node_name, const char *filename,
                                   const char *driver, bool read_only, bool no_flush) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *args = NULL;
        int r;

        r = qmp_build_blockdev_add_file(node_name, filename, driver,
                                        qmp_client_has_feature(qmp, QMP_CLIENT_FEATURE_IO_URING),
                                        read_only, no_flush, &args);
        if (r < 0)
                return r;

        return qmp_client_execute(qmp, "blockdev-add", args, on_blockdev_add_complete, NULL);
}
...
static int on_ephemeral_create_concluded(QmpClient *qmp, void *userdata) {
        ...
        r = qmp_client_execute(qmp, "blockdev-add", fmt_args, on_blockdev_add_complete, NULL);
        if (r < 0)
                return r;
        ...
        r = qmp_client_execute(qmp, "device_add", device_args, on_device_add_complete, NULL);
        if (r < 0)
                return r;
...
static int qmp_setup_one_drive(VmspawnQmpBridge *bridge, QmpClient *qmp, const DriveInfo *drive) {
        int r;
        ...
        if (ephemeral) {
                ...
                r = qmp_client_execute(qmp, "blockdev-add", base_fmt_args, on_blockdev_add_complete, NULL);
                if (r < 0)
                ...
                r = qmp_client_execute(qmp, "blockdev-add", overlay_file_args, on_blockdev_add_complete, NULL);
                if (r < 0)
                        return log_error_errno(r, "Failed to send blockdev-add for overlay file '%s': %m", drive->path);

                /* Step 6: Get base image virtual size and fire blockdev-create */
                ...
                *ectx = (EphemeralDriveCtx) {
                ...
                r = qmp_client_execute(qmp, "blockdev-create", cmd_args, on_blockdev_create_complete, NULL);
                if (r < 0)
                ...
                r = qmp_fdset_add_async(qmp, drive->fd, &fdset_path);
                if (r < 0)
                ...
                r = qmp_add_file_node_async(qmp, file_node_name, fdset_path, driver,
                                            drive->read_only, drive->no_flush);
                if (r < 0)
...
int vmspawn_qmp_setup_network(VmspawnQmpBridge *bridge, NetworkInfo *network) {
        ...
        if (tap_by_fd) {
                ...
                r = qmp_client_execute_send_fd(qmp, "getfd", getfd_args, network->fd,
                                               on_getfd_complete, NULL);
                if (r < 0)
        ...
        r = qmp_client_execute(qmp, "netdev_add", netdev_args, on_netdev_add_complete, NULL);
        if (r < 0)
...
static int vmspawn_qmp_setup_one_virtiofs(QmpClient *qmp, const VirtiofsInfo *vfs) {
        ...
        r = qmp_client_execute(qmp, "chardev-add", chardev_args, on_chardev_add_complete, NULL);
        if (r < 0)
...
int vmspawn_qmp_setup_vsock(VmspawnQmpBridge *bridge, VsockInfo *vsock) {
        ...
        r = qmp_client_execute_send_fd(qmp, "getfd", getfd_args, vsock->fd,
                                       on_getfd_complete, NULL);
        if (r < 0)
...
static int qmp_setup_scsi_controller(QmpClient *qmp, const char *pcie_port) {
        ...
        r = qmp_client_execute(qmp, "device_add", args, on_device_add_complete, NULL);
        if (r < 0)
...
int vmspawn_qmp_setup_drives(VmspawnQmpBridge *bridge, const DriveInfos *drives) {
        ...
        /* io_uring support was probed during the blocking handshake in qmp_client_connect_fd().
         * qmp_client_supports_io_uring() returns the cached result for each file node. */
...
int vmspawn_qmp_start(VmspawnQmpBridge *bridge) {
        ...
        return qmp_client_execute(bridge->qmp, "cont", /* arguments= */ NULL, on_cont_complete, /* userdata= */ NULL);
}
