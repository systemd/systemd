...
static int qmp_setup_ephemeral_drive(VmspawnQmpBridge *bridge, QmpClient *qmp, DriveInfo *drive) {
        ...
        _cleanup_free_ char *qmp_node_name = NULL, *qmp_device_id = NULL;
        _cleanup_free_ char *base_file_node = NULL, *base_fmt_node = NULL, *overlay_file_node = NULL;
        if (asprintf(&qmp_node_name, "vmspawn-%" PRIu64 "-storage", counter) < 0 ||
            asprintf(&qmp_device_id, "vmspawn-%" PRIu64 "-disk", counter) < 0 ||
        ...
        *ectx = (EphemeralDriveCtx) {
                .qmp_node_name      = strdup(qmp_node_name),
                .qmp_device_id      = strdup(qmp_device_id),
