...
static int on_io_uring_probe_add_reply(
        ...
        /* Transport failure: probe never ran. -EIO with error_desc = QEMU rejected the command. */
        ...
        return qmp_client_invoke(c, "blockdev-del", QMP_CLIENT_ARGS_PROBE(del_args),
...
static int probe_io_uring(QmpClient *c, VmspawnQmpBridge *bridge) {
        ...
        return qmp_client_invoke(c, "blockdev-add", QMP_CLIENT_ARGS_PROBE(args),
...
static int probe_schema(QmpClient *c, VmspawnQmpBridge *bridge) {
        ...
        return qmp_client_invoke(c, "query-qmp-schema", QMP_CLIENT_ARGS_PROBE(NULL),
                        on_probe_schema_reply, bridge);
}

static int vmspawn_qmp_on_probe(QmpClient *c, void *userdata) {
        VmspawnQmpBridge *bridge = ASSERT_PTR(userdata);
        int r;

        r = probe_io_uring(c, bridge);
        if (r < 0)
                return r;

        return probe_schema(c, bridge);
}
...
int vmspawn_qmp_init(VmspawnQmpBridge **ret, int qmp_fd_consume, sd_event *event) {
        ...
        qmp_client_bind_probe(bridge->qmp, vmspawn_qmp_on_probe, bridge);
