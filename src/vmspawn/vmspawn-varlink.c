...
static int qmp_execute_varlink_async(
        ...
        r = qmp_client_invoke(ctx->bridge->qmp, command, arguments, callback, link);
        if (r < 0)
                sd_varlink_unref(link);
...
static int vl_method_subscribe_events(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (r < 0) {
                _cleanup_strv_free_ char **f = hashmap_remove(ctx->subscribed, link);
...
static void vl_disconnect(sd_varlink_server *server, sd_varlink *link, void *userdata) {
        ...
        if (!hashmap_contains(ctx->subscribed, link))
                return;

        _cleanup_strv_free_ char **filter = hashmap_remove(ctx->subscribed, link);
        sd_varlink_unref(link);
}
...
static int dispatch_pending_job(VmspawnQmpBridge *bridge, sd_json_variant *data) {
        ...
        if (!streq_ptr(status, "concluded"))
                return 0;

        _cleanup_(pending_job_freep) PendingJob *job = hashmap_remove(bridge->pending_jobs, job_id);
        if (!job)
                return 0;
        ...
        int r = sd_json_buildo(&dismiss_args, SD_JSON_BUILD_PAIR_STRING("id", job_id));
        if (r < 0)
                return r;

        r = qmp_client_invoke(bridge->qmp, "job-dismiss", dismiss_args, on_job_dismiss_complete, NULL);
        if (r < 0)
                return r;

        if (job->on_concluded) {
                int q = job->on_concluded(bridge->qmp, TAKE_PTR(job->userdata));
                if (q < 0) {
                        log_warning_errno(q, "Job continuation failed: %m");
                        return sd_event_exit(qmp_client_get_event(bridge->qmp), q);
                }
        }
...
int vmspawn_varlink_setup(
                ...
                VmspawnQmpBridge *bridge_consume,
                const char *runtime_dir,
                char **ret_control_address) {

        _cleanup_(vmspawn_qmp_bridge_freep) VmspawnQmpBridge *bridge = bridge_consume;
        ...
        *ctx = (VmspawnVarlinkContext) {
        ...
        qmp_client_set_userdata(ctx->bridge->qmp, ctx);
        qmp_client_bind_event(ctx->bridge->qmp, on_qmp_event);
        qmp_client_bind_disconnect(ctx->bridge->qmp, on_qmp_disconnect);
