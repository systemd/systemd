...
static int write_dmesg(PStoreEntry *pe, const char *subdir1, const char *subdir2) {
        ...
        flags = O_CREAT|O_NOFOLLOW|O_NOCTTY|O_CLOEXEC|O_CREAT|O_APPEND|O_WRONLY;
        ofd = open(ofd_path, flags, 0640);
        if (ofd < 0)
...
static void process_dmesg_files(PStoreList *list) {
        ...
        PStoreEntry *pe;
        _cleanup_free_ char *subdir1 = NULL;
         ...
         * original dmesg.
         *
         * The dmesg- filename contains the backend-type and the
         * Common Platform Error Record, CPER, record id, a
         * 64-bit number.
         *
         * Files are processed in reverse lexigraphical order so as
         * to properly reconstruct original dmesg. */

        for (size_t n = list->n_entries; n > 0; n--) {
                _cleanup_free_ char *subdir2 = NULL;
                size_t plen;
                char *p;

                pe = &list->entries[n-1];

                /* Only interested in good dmesg files... */
                if (pe->handled)
                        continue;
                ...
                ++p; /* move beyond dmesg-xxx- */
                plen = strlen(p);
                if (plen < 6)
                        continue;

                if (startswith(pe->dirent.d_name, "dmesg-efi-")) {
                        /* For the EFI backend, the 3 least significant digits
                         * of record id encodes a "count" number, the next 2
                         * least significant digits for the dmesg part (chunk)
                         * number, and the remaining digits as the timestamp.
                         * See linux/drivers/firmware/efi/efi-pstore.c in
                         * efi_pstore_write(). */

                        /* Extract base record id */
                        subdir1 = memdup_suffix0(p, plen-5);
                        if (!subdir1) {
                                log_oom();
                                return;
                        }
                        /* Extract "count" field */
                        subdir2 = memdup_suffix0(&p[plen-3], 3);
                        if (!subdir2) {
                                log_oom();
                                return;
                        }
                        ...
                        move_file(pe, subdir1, subdir2);

                        /* Append to the dmesg */
                        write_dmesg(pe, subdir1, subdir2);

                        /* Cleanup */
                        subdir1 = mfree(subdir1);
                        subdir2 = mfree(subdir2);
                } else

                if (startswith(pe->dirent.d_name, "dmesg-erst-")) {
                        /* For the ERST backend, the record is a monotonically
                         * increasing number, seeded as a timestamp. See linux/
                         * drivers/acpi/apei/erst.c in erst_writer().
                         */
                        uint64_t record_id;
                        char *endptr;

                        record_id = strtoull(p, &endptr, 0);
                        if (*endptr != '\0')
                                continue;
                        if ((last_record_id - 1) != record_id) {
                                /* A discontinuity in the number has been
                                 * detected, this current record id will become
                                 * the directory name for all pieces of the
                                 * dmesg in this series. */
                                subdir1 = mfree(subdir1);
                                subdir1 = memdup_suffix0(p, plen);
                                if (!subdir1) {
                                        log_oom();
                                        return;
                                }
                        ...
                        /* Now move file from pstore to archive storage */
                        move_file(pe, subdir1, NULL);

                        /* Append to the dmesg */
                        write_dmesg(pe, subdir1, NULL);

                        /* Update, but keep subdir1 for next file */
...
static int run(int argc, char *argv[]) {
        ...
        for (size_t n = 0; n < list.n_entries; n++)
                move_file(&list.entries[n], NULL, NULL);
