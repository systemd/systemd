...
static int add_getty_impl(const char *tty, const char *unit, const char *type) {
        int r;
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to escape tty path %s: %m", tty);

        _cleanup_free_ char *unit_path = path_join(SYSTEM_DATA_UNIT_DIR, unit);
        if (!unit_path)
                return log_oom();
...
static int add_serial_getty(const char *path) {
        ...
        return add_getty_impl(skip_dev_prefix(path), "serial-getty@.service", "serial");
}

static int add_container_getty(const char *path) {
        assert(path);

        /* Check if it is actually a pty. */
        const char *pty = path_startswith(skip_dev_prefix(path), "pts/");
        if (isempty(pty)) {
                log_debug("Invalid pty device specified, ignoring: %s", path);
                return 0;
        }

        return add_getty_impl(pty, "container-getty@.service", "container");
}
