...
static int pkcs7_configure(X509 *certificate, EVP_PKEY *private_key, PKCS7 **ret_p7, PKCS7_SIGNER_INFO **ret_si) {
        int r;
        ...
        if (X509_get_signature_info(certificate, &x509_mdnid, &x509_pknid, NULL, NULL) == 0)
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to get X509 digest NID: %s",
                                       ERR_error_string(ERR_get_error(), NULL));

        const EVP_MD *md = EVP_get_digestbynid(x509_mdnid);
        if (!md)
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to get digest algorithm via digest NID");
        ...
        if (private_key) {
                if (PKCS7_SIGNER_INFO_set(si, certificate, private_key, EVP_get_digestbynid(x509_mdnid)) <= 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to configure signer info: %s",
                                               ERR_error_string(ERR_get_error(), NULL));
        } else {
                if (ASN1_INTEGER_set(si->version, 1) == 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to set signer info version: %s",
                                               ERR_error_string(ERR_get_error(), NULL));

                if (X509_NAME_set(&si->issuer_and_serial->issuer, X509_get_issuer_name(certificate)) == 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to set signer info issuer: %s",
                                               ERR_error_string(ERR_get_error(), NULL));
                ...
                if (!si->issuer_and_serial->serial)
                        return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to set signer info serial: %s",
                                               ERR_error_string(ERR_get_error(), NULL));

                if (X509_ALGOR_set0(si->digest_alg, OBJ_nid2obj(x509_mdnid), V_ASN1_NULL, NULL) == 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to set signer info digest algorithm: %s",
                                               ERR_error_string(ERR_get_error(), NULL));

                if (X509_ALGOR_set0(si->digest_enc_alg, OBJ_nid2obj(x509_pknid), V_ASN1_NULL, NULL) == 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to set signer info signing algorithm: %s",
                                               ERR_error_string(ERR_get_error(), NULL));
        }

        if (arg_signed_attributes) {
                si->auth_attr = TAKE_PTR(arg_signed_attributes);
        ...
        if (PKCS7_add_attrib_smimecap(si, sk_X509_ALGOR_new_null()) == 0)
...
static int verb_sign(int argc, char *argv[], void *userdata) {
        ...
        r = pkcs7_configure(certificate, private_key, &p7, &si);
        if (r < 0)
                return r;
