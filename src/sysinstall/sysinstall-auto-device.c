...
static int auto_device_get_node(AutoDeviceContext *c, char **ret) {
        ...
        return strdup_to(ret, c->candidate);
}
...
static int auto_device_add(AutoDeviceContext *c, const BlockDevice *d) {
        ...
        if (c->candidate) {
                ...
                if (path_equal(c->candidate, d->node)) {
                        c->candidate_diskseq = d->diskseq; /* Update with current diskseq */
                        return 0; /* no change */
                }
        ...
        if (r < 0)
                return log_error_errno(r, "Unable to get candidate node to use: %m");
...
static int auto_device_remove(AutoDeviceContext *c, struct sd_device *dev) {
        int r;
        ...
        if (r < 0)
                return log_device_debug_errno(dev, r,
                                              "Failed to acquire device node of block device, ignoring: %m");
...
static int auto_device_on_uevent(sd_device_monitor *monitor, sd_device *dev, void *userdata) {
        ...
        if (r < 0)
                return log_device_debug_errno(dev, r, "Failed to acquire uevent action of block device: %m");
