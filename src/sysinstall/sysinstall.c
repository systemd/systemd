...
static int parse_argv(int argc, char *argv[]) {
        ...
        FOREACH_OPTION_OR_RETURN(c, &opts)
                ...
                OPTION_LONG("device-auto-timeout-usec", "SECS", "How long the list of candidate disks must remain unchanged before a disk is picked"):
                        r = parse_sec(opts.arg, &arg_device_auto_timeout);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --device-auto-timeout-usec= parameter: %s", opts.arg);
                        if (arg_device_auto_timeout == USEC_INFINITY)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "--device-auto-timeout-usec= cannot be infinite, refusing.");
                        break;
