...
static int vl_method_instance_register(
        ...
        *request = (RegisterRequest) {
        ...
        if (request->id && (!unit_prefix_is_valid(request->id) || strchr(request->id, '-') != NULL))
...
static int target_resolve(
        ...
        if (target->conn_fd_idx < 0) {
                ...
                if (conn_fd < 0)
                        return log_debug_errno(conn_fd, "Failed to fetch FD from Varlink connection: %m");
        ...
        if (target_pidref) {
                ...
                if (target->cgroup && (!target_instance || !streq(target->cgroup, target_instance->cg_path)))
                        return sd_varlink_error(link, "io.systemd.AppInstance.ConflictingTarget", NULL);
...
typedef struct AppInstanceNotifyContext {
        sd_event_source *deferred;
        app_instance_notify_finished_t done_cb;
        void *userdata;
} AppInstanceNotifyContext;

static AppInstanceNotifyContext* app_instance_notify_context_free(AppInstanceNotifyContext *ctx) {
        ctx->deferred = sd_event_source_disable_unref(ctx->deferred);
        return mfree(ctx);
}

DEFINE_TRIVIAL_CLEANUP_FUNC(AppInstanceNotifyContext*, app_instance_notify_context_free);
...
static int app_instance_notify_done(void *userdata) {
        _cleanup_(app_instance_notify_context_freep) AppInstanceNotifyContext *ctx = ASSERT_PTR(userdata);
        return ctx->done_cb(ctx->userdata);
}

static int app_instance_notify_context_deferred(sd_event_source *s, void *userdata) {
        return app_instance_notify_done(userdata);
}

static int app_instance_notify_context_queue(sd_event *event, AppInstanceNotifyContext *ctx) {
        int r;
        ...
        r = sd_event_add_defer(event, &ctx->deferred, app_instance_notify_context_deferred, ctx);
        if (r < 0)
                return log_debug_errno(r, "Failed to defer callback: %m");

        return 0;
}

int app_instance_notify(AppInstance *instance, AppInstanceEvent event, app_instance_notify_finished_t cb, void *userdata) {
        int r;

        _cleanup_(app_instance_notify_context_freep) AppInstanceNotifyContext *ctx = NULL;
        varlink_execute_directory_finished_t exec_dir_done_cb = NULL;
        if (cb) {
                ...
                *ctx = (AppInstanceNotifyContext) {
                ...
                exec_dir_done_cb = app_instance_notify_done;
        }
        ...
        if (!instance->ever_queried) {
                ...
                r = app_instance_notify_context_queue(instance->manager->event, ctx);
                if (r < 0)
                        return r;
                TAKE_PTR(ctx);
