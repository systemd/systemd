...
static int set_local_rtc(int argc, char **argv, void *userdata) {
       ...
       if (b == 1) {
               fflush(stdout);
               log_warning("Warning: The system is now being configured to read the RTC time in the local time zone. This mode cannot be fully supported. It will create various problems with time zone changes and DST adjustments. If at all possible, use RTC in UTC.");
       }
