        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                for (VerityMode mode = VERITY_DATA; mode < p->verity; mode++) {
                        ...
                        if (p->copy_blocks_auto && !q->copy_blocks_auto)
                                return log_syntax(NULL, LOG_ERR, p->definition_path, 1, SYNTHETIC_ERRNO(EINVAL),
                                                "CopyBlocks=auto set with Verity=%s but data partition does not set CopyBlocks=auto.",
                                                verity_mode_to_string(p->verity));

                        if (p->copy_blocks_path && !q->copy_blocks_path)
                                return log_syntax(NULL, LOG_ERR, p->definition_path, 1, SYNTHETIC_ERRNO(EINVAL),
                                                "CopyBlocks= set with Verity=%s but data partition does not set CopyBlocks=.",
                                                verity_mode_to_string(p->verity));
                }
        }
