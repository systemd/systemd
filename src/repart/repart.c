...
static int partition_encrypt(Context *context, Partition *p, PartitionTarget *target, bool offline) {
        ...
        if ((arg_discard && p->integrity != INTEGRITY_INLINE)?(p->allow_discards != 0):(p->allow_discards == 1)) {
                uint32_t flags;
        ...
        if (offline) {
                ...
                bool discard = ((p->allow_discards == -1)?arg_discard:p->allow_discards) && p->integrity != INTEGRITY_INLINE;
