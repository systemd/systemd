...
static int context_fstab(Context *context) {
        ...
        if (arg_append_fstab == APPEND_AUTO)
                fputs(AUTOMATIC_FSTAB_HEADER_START "\n", f);
        else
                fputs("# Automatically generated by systemd-repart\n\n", f);
        ...
        case APPEND_AUTO:
                ...
                if (r < 0) {
                        if (r == -ENOENT) {
                                log_warning("File fstab not found in %s", path);
                                break;
                        } else
                                return log_error_errno(r, "Failed to open %s: %m", path);
                }

                if (startswith(c, "# Automatically generated by systemd-repart\n\n")) {
                        log_warning("The fstab was generated by systemd-repart and will not be included in the new one. Ignoring");
                        break;
                } else {
                        char *acs;
                        const char *ace;
                        acs = find_line(c, AUTOMATIC_FSTAB_HEADER_START);
                        ace = find_line(c, AUTOMATIC_FSTAB_HEADER_END);
                        if (acs != NULL)
                                acs[-strlen(AUTOMATIC_FSTAB_HEADER_START) - 1] = '\0';
                        fputs(c, f);
                        if (ace != NULL)
                                fputs(ace, f);
                }
...
static int parse_argv(int argc, char *argv[], X509 **ret_certificate, EVP_PKEY **ret_private_key, OpenSSLAskPasswordUI **ret_ui) {
        ...
        if (arg_append_fstab && !arg_generate_fstab)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "No --generage-fstab= specified for --append-fstab=%s.", append_mode_to_string(arg_append_fstab));
