...
static int context_fstab(Context *context) {
        const char *sd_hdr =  "# Automatically generated by systemd-repart";
        const char *uc_hdr =  "# User provided content";
        ...
        case APPEND_AUTO:
                ...
                if (r >= 0) {
                        if (startswith(c, sd_hdr)) {
                                const char *uc;
                                uc = strstr(c, uc_hdr);
                                if (uc != NULL)
                                        fwrite(uc - 1, 1, l - (uc - c) + 1, f);
                        } else {
                                fprintf(f, "\n%s\n\n", uc_hdr);
                                fwrite(c, 1, l, f);
                        }
        ...
        r = flink_tmpfile(f, t, path, (arg_append_fstab == APPEND_AUTO || arg_append_fstab == APPEND_REPLACE) ? LINK_TMPFILE_REPLACE : 0);
        if (r < 0)
        ...
        while ((c = getopt_long(argc, argv, "hs:SCP", options, NULL)) >= 0)
                ...
                case ARG_APPEND_FSTAB:
                        ...
                        if (arg_append_fstab < 0)
                                return log_error_errno(arg_append_fstab, "Failed to parse --append-fstag parameter: %s", optarg);
                        break;
