...
static int do_copy_files(Context *context, Partition *p, const char *root) {
        ...
        uid_t gid = copy_ownership ? GID_INVALID : getgid();
...
static int check_mkfs(const Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                bool have_root =
                        partition_needs_populate(p) &&
                        (fstype_is_ro(p->format) ||
                         (streq_ptr(p->format, "btrfs") && p->compression));

                /* We don't cover all conditions here. In other words, have_root==true can
                 * be trusted, but have_root==false cannot, so downgrade to -1 which means
                 * "not sure". */
                r = mkfs_find_or_warn(p->format, have_root ? true : -1, /* ret= */ NULL);
                if (r < 0)
                        return r;
        }
