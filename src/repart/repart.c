...
static int context_grow_partitions(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                /* Fixed-offset new partitions are not in any free area; their size is fixed
                 * to the minimum required size since there is no free space to grow into.
                 * Use size_min directly when set — grain-size rounding does not apply to
                 * fixed-offset partitions (their alignment is handled by FixedOffsetBytes=). */
                if (!PARTITION_EXISTS(p) && p->fixed_offset != UINT64_MAX) {
                        if (p->new_size == UINT64_MAX) {
                                if (p->size_min != UINT64_MAX)
                                        p->new_size = p->size_min;
                                else {
                                        /* No explicit size; derive from content or use a hard minimum. */
                                        uint64_t sz = partition_min_size(context, p);
                                        uint64_t xsz = partition_max_size(context, p);
                                        p->new_size = xsz != UINT64_MAX ? MIN(sz, xsz) : sz;
                                }
                        }
                        if (p->new_padding == UINT64_MAX)
                                p->new_padding = 0;
...
static int config_parse_fixed_offset_bytes(
                const char *unit,
                const char *filename,
                unsigned line,
                const char *section,
...
static int partition_read_definition(
        ...
        ConfigTableItem table[] = {
                ...
                { "Partition", "FixedOffsetBytes",          config_parse_fixed_offset_bytes, 0,                                  &p->fixed_offset            },
...
static int context_scan_fixed_partitions(
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                /* Extend fixed_region_end when SizeMinBytes= is known; it is optional but
                 * without it we cannot reserve space against auto-placed partitions. */
                if (p->size_min != UINT64_MAX) {
                        uint64_t end;
                        if (!ADD_SAFE(&end, p->fixed_offset, p->size_min))
                                return log_error_errno(SYNTHETIC_ERRNO(EOVERFLOW),
                                                       "Partition %s FixedOffsetBytes= + SizeMinBytes= overflows, refusing.",
                                                       p->definition_path);
                        fixed_region_end = MAX(fixed_region_end, end);
                } else if (!PARTITION_EXISTS(p))
                        log_warning("Partition %s has FixedOffsetBytes= but no SizeMinBytes=; "
                                    "auto-placed partitions may overlap it.",
                                    p->definition_path);
        }
...
static int context_load_partition_table(Context *context) {
        ...
        if (from_scratch) {
                ...
                if (fixed_region_begin != UINT64_MAX) {
                        ...
                        if (want < cur) {
                                log_debug("Lowering first LBA from %" PRIu64 " to %" PRIu64
                                          " to accommodate fixed-offset partition.",
                                          (uint64_t) cur, (uint64_t) want);
                                (void) sym_fdisk_set_first_lba(c, want);
                        }
                }
...
add_initial_free_area:
        ...
        /* context->start marks where auto-placed partitions begin: at or after the
         * grain-rounded end of any fixed-offset partitions. */
