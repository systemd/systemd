...
static int context_load_partition_table(Context *context) {
         ...
         * beyond the disk end. Only do this when the first usable LBA is at least 1 MiB; smaller
         * values indicate a non-standard disk layout that should keep the default 4 KiB grain. */
        first_lba = fdisk_get_first_lba(c);
        assert(first_lba <= UINT64_MAX / secsz);
        first_lba *= secsz;
        if (first_lba >= 1024 * 1024)
...
add_initial_free_area:
         ...
         * for the next partition without any special-casing. Only do this when the first usable LBA
         * is at least 1 MiB; smaller values indicate a non-standard disk layout that should keep
         * the default 4 KiB grain. */
        if (first_lba >= 1024 * 1024)
                grainsz = MAX(grainsz, first_lba);
