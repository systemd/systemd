...
static int partition_read_definition(
        ...
        if (p->btrfs_replace && arg_offline != 0)
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                  "BtrfsReplace= cannot be used with --offline=yes, refusing.");
        ...
        *t = (PartitionTarget) {
        ...
        if (arg_offline <= 0) {
                ...
                if (r > 0) {
                        ...
                        _cleanup_free_ char* part_node = fdisk_partname(context->node, p->partno+1);
...
static int partition_target_grow(PartitionTarget *t, uint64_t size) {
        ...
        if (t->loop) {
                ...
                if (r < 0)
                        return log_error_errno(errno, "Failed to resize partition %"PRIu64": %m", t->blkpg_partition->nr);
