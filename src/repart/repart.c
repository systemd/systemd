...
static int write_primary_descriptor(int fd, uint32_t root_sector, const struct tm *t, const char *system_id, const char *volume_id, const char *publisher_id) {
        int r;
...
static int write_eltorito(int fd, const struct tm *t, uint32_t load_block, const char *system_id, const char *volume_id, const char *publisher_id) {
        int r;
        ...
        /* block 16+0 */
        r = write_primary_descriptor(fd, root_sector, t, system_id, volume_id, publisher_id);
        if (r < 0)
                return r;
...
static int context_write_partition_table(Context *context) {
        ...
        if (arg_eltorito) {
                ...
                r = write_eltorito(fdisk_get_devfd(context->fdisk_context), &t, esp_offset / 2048, arg_eltorito_system, arg_eltorito_volume, arg_eltorito_publisher);
                if (r < 0)
