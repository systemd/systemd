        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                *replacement = (BtrfsReplacement) {
                ...
                if (replacement->mountpoint_fd < 0)
                        return log_error_errno(replacement->mountpoint_fd, "Failed to open mountpoint for btrfs filesystem: %m");
                ...
                if (r == 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP), "Mountpoint %s is not a btrfs filesystem: %m", p->block_device_replace);
...
static int find_root(Context *context) {
        _cleanup_free_ char *device = NULL;
        int r, open_flags = O_CLOEXEC|(context->dry_run ? O_RDONLY : O_RDWR);
