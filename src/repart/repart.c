...
static int context_scan_fixed_partitions(
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                /* Use SizeMinBytes= if configured, otherwise reserve at least one grain so
                 * the free area for auto-placed partitions starts after the fixed region. */
                uint64_t end;
                uint64_t sz = p->size_min != UINT64_MAX ? p->size_min : grainsz;
