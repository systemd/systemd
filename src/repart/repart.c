...
#define ISO9660_START 16
#define ISO9660_PRIMARY_DESCRIPTOR (ISO9660_START+0)
#define ISO9660_ELTORITO_DESCRIPTOR (ISO9660_START+1)
#define ISO9660_TERMINAL_DESCRIPTOR (ISO9660_START+2)
#define ISO9660_BOOT_CATALOG (ISO9660_START+3)
#define ISO9660_ROOT_SECTOR (ISO9660_START+4)
#define ISO9660_SIZE 5
...
static int write_primary_descriptor(
        ...
        struct iso9660_primary_volume_descriptor desc = {
                ...
                .volume_space_size_little = htole32(ISO9660_SIZE),
                .volume_space_size_big = htobe32(ISO9660_SIZE),
        ...
        set_iso9660_string_constant(desc.header.identifier, sizeof(desc.header.identifier), "CD001", /* allow_a_chars= */ true);

        r = time_to_iso9660_dir_datetime(t, &desc.root_directory_entry.time);
        if (r < 0)
                return r;
        ...
        set_iso9660_string_constant(desc.volume_set_identifier, sizeof(desc.volume_set_identifier), NULL, /* allow_a_chars= */ false);

        r = set_iso9660_string(desc.publisher_identifier, sizeof(desc.publisher_identifier), publisher_id, /* allow_a_chars= */ true);
        if (r < 0)
                return r;
        set_iso9660_string_constant(desc.data_preparer_identifier, sizeof(desc.data_preparer_identifier), NULL, /* allow_a_chars= */ true);

        set_iso9660_string_constant(desc.application_identifier, sizeof(desc.application_identifier), "SYSTEMD-REPART", /* allow_a_chars= */ true);

        set_iso9660_string_constant(desc.copyright_file_identifier, sizeof(desc.copyright_file_identifier), NULL, /* allow_a_chars= */ false);

        set_iso9660_string_constant(desc.abstract_file_identifier, sizeof(desc.abstract_file_identifier), NULL, /* allow_a_chars= */ false);

        set_iso9660_string_constant(desc.bibliographic_file_identifier, sizeof(desc.bibliographic_file_identifier), NULL, /* allow_a_chars= */ false);

        r = time_to_iso9660_datetime(t, &desc.volume_creation_date);
        if (r < 0)
                return r;
        r = time_to_iso9660_datetime(t, &desc.volume_modification_date);
        if (r < 0)
                return r;
...
static uint16_t calculate_validation_entry_checksum(void *p, size_t size) {
        assert(size % 2 == 0);

        uint16_t checksum = 0;

        for (size_t i = 0; i < (size/2); i++)
                checksum -= le16toh(((le16_t*)p)[i]);
...
static int write_boot_catalog(int fd, uint32_t load_block) {
        ...
        assert((size_t) ((uint8_t*) p - sector) <= sizeof(sector));
...
static int write_directories(int fd, const struct tm *t, uint32_t root_sector) {
        ...
        r = time_to_iso9660_dir_datetime(t, &self.time);
        if (r < 0)
                return r;
        ...
        r = time_to_iso9660_dir_datetime(t, &parent.time);
        if (r < 0)
                return r;
...
static int write_eltorito(int fd, const struct tm *t, uint32_t load_block, const char *system_id, const char *volume_id, const char *publisher_id) {
        ...
        if (lseek(fd, ISO9660_START*ISO9660_BLOCK_SIZE, SEEK_SET) < 0)
                return -errno;

        r = write_primary_descriptor(fd, ISO9660_ROOT_SECTOR, t, system_id, volume_id, publisher_id);
        if (r < 0)
                return r;
        ...
        r = write_directories(fd, t, ISO9660_ROOT_SECTOR);
        if (r < 0)
                return r;
...
static int context_verify_eltorito_overlap(Context *context) {
        ...
        /* The first lba is first block where a partition could be. Even if there is no partition
         * there, we should still still not overlap with it since a partition could be added later.
        ...
        if (fdisk_get_first_lba(context->fdisk_context) * context->sector_size < (ISO9660_START+ISO9660_SIZE)*ISO9660_BLOCK_SIZE)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "El Torito is overlappging with the first partition block.");
...
static int context_write_partition_table(Context *context) {
        ...
        if (arg_eltorito) {
                ...
                if (usec != USEC_INFINITY) {
                        ...
                        r = localtime_or_gmtime_usec(usec, /* utc= */ true, &t);
                        if (r < 0)
                ...
                r = write_eltorito(fdisk_get_devfd(context->fdisk_context), &t, esp_offset / ISO9660_BLOCK_SIZE, arg_eltorito_system, arg_eltorito_volume, arg_eltorito_publisher);
                if (r < 0)
