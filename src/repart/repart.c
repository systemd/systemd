...
static int partition_read_definition(
        ...
        if (p->btrfs_replace && (p->format || partition_needs_populate(p)))
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                  "Format=/CopyFiles=/MakeDirectories=/MakeSymlinks= and BtrfsReplace= cannot be combined, refusing.");

        if ((p->copy_blocks_path || p->copy_blocks_auto) && p->btrfs_replace)
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                  "CopyBlocks= and BtrfsReplace= cannot be combined, refusing.");

        if (p->btrfs_replace && arg_offline == 1)
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                  "BtrfsReplace= is incompatible with --offline=yes, refusing.");

        if (p->btrfs_replace) {
                r = path_is_fs_type(p->btrfs_replace, BTRFS_SUPER_MAGIC);
                if (r < 0)
                        return log_syntax(NULL, LOG_ERR, path, 1, r,
                                          "Failed to determine file system type of %s: %m.", p->btrfs_replace);
                else if (r == 0)
                        return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
        ...
        if (!p->format) {
                ...
                if (p->type.designator == PARTITION_SWAP)
                        format = "swap";
                else if (partition_needs_populate(p) || (p->encrypt != ENCRYPT_OFF && !(p->copy_blocks_path || p->copy_blocks_auto || p->btrfs_replace)))
        ...
        *t = (PartitionTarget) {
        ...
        if (arg_offline <= 0) {
                ...
                if (r > 0) {
                         ...
                         */
                        r = block_device_add_partition(whole_fd, part_node, p->partno+1, p->offset, size);
                        if (r < 0)
                        ...
                        *ret = TAKE_PTR(t);
                        ...
                        if ((r != 0) && (r != -ENOTBLK))
                                log_warning_errno(r, "Could not know whether device can be partitioned, assuming it cannot: %m");
...
static int context_btrfs_replace(Context *context) {
        int r;
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                _cleanup_(partition_target_freep) PartitionTarget *t = NULL;
                _cleanup_free_ char *btrfs = NULL, *resize_param = NULL;
                _cleanup_strv_free_ char **argv_replace = NULL, **argv_resize = NULL;
                uint64_t id;
                char id_str[DECIMAL_STR_MAX(uint64_t)];
                ...
                if (p->encrypt != ENCRYPT_OFF) {
                        r = partition_encrypt(context, p, t, /* offline= */ false, /* keep= */ true);
                        if (r < 0)
                ...
                r = find_executable("btrfs", &btrfs);
                if (r == -ENOENT)
                        return log_error_errno(SYNTHETIC_ERRNO(EPROTONOSUPPORT), "btrfs binary not available.");
                if (r < 0)
                        return log_error_errno(r, "Failed to determine whether btrfs binary exists: %m");

                r = btrfs_get_block_device(p->btrfs_replace, &id, NULL);
                if (r < 0)
                ...
                xsprintf(id_str, "%" PRIu64, id);

                argv_replace = strv_new(btrfs, "replace", "start", "-B", id_str, partition_target_path(t), p->btrfs_replace);
                if (!argv_replace)
                        return log_oom();
                ...
                r = pidref_safe_fork_full(
                                ...
                                /* except_fds= */ NULL,
                                /* n_except_fds= */ 0,
                                FORK_RESET_SIGNALS|FORK_RLIMIT_NOFILE_SAFE|FORK_DEATHSIG_SIGTERM|FORK_LOG|FORK_WAIT|
                ...
                if (r == 0) {
                        /* Child */
                        execvp(btrfs, argv_replace);

                        log_error_errno(errno, "Failed to execute %s: %m", btrfs);
                ...
                log_info("Resizing filesystem on partition %" PRIu64 ".", p->partno);
                argv_resize = strv_new(btrfs, "filesystem", "resize", resize_param, p->btrfs_replace);
                if (!argv_resize)
                        return log_oom();
...
static int context_write_partition_table(Context *context) {
         ...
         */
        r = context_btrfs_replace(context);
        if (r < 0)
                goto error;
 ...
 error:
        if (context->needs_rescan) {
                int q = context_partscan(context);
                if (q < 0)
                        log_warning_errno(q, "Failed to rescan partitions after error, ignoring: %m");
        }
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                if (S_ISDIR(st.st_mode)) {
                        ...
                        if (major(st.st_dev) != 0)
                                ...
                                r = btrfs_get_block_device_fd(source_fd, NULL, &devt);
                                if (r == -EUCLEAN)
...
static int acquire_root_devno(
        ...
        if (S_ISBLK(st.st_mode)) {
                ...
                devno = st.st_dev;
                if (major(devno) == 0) {
                        r = btrfs_get_block_device_fd(fd, NULL, &devno);
