...
static int context_grow_partitions(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                /* Fixed-offset new partitions are not in any free area. Size must be at least
                 * large enough to hold the content (CopyBlocks= image, filesystem minimum, etc.)
                 * even when SizeMinBytes= is set explicitly. */
