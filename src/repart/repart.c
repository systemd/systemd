...
static int write_primary_descriptor(
        ...
        if (s < 0)
                log_error_errno(errno, "Failed to write ISO9660 primary descriptor: %m");
        if (s != sizeof(desc))
                log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to fully write ISO9660 primary descriptor");
...
static int write_eltorito_descriptor(int fd, uint32_t catalog_sector) {
        ...
        if (s < 0)
                log_error_errno(errno, "Failed to write ISO9660 El-Torito descriptor: %m");
        if (s != sizeof(desc))
                log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to fully write ISO9660 El-Torito descriptor");
...
static int write_terminal_descriptor(int fd) {
        ...
        if (s < 0)
                log_error_errno(errno, "Failed to write ISO9660 terminal descriptor: %m");
        if (s != sizeof(desc))
                log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to fully write ISO9660 terminal descriptor");
...
static int write_boot_catalog(int fd, uint32_t load_block) {
        ...
        if (s < 0)
                log_error_errno(errno, "Failed to write El-Torito boot catalog: %m");
        if (s != sizeof(sector))
                log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to fully write El-Torito boot catalog");
...
static int write_directories(int fd, const struct tm *t, uint32_t root_sector) {
        ...
        if (s < 0)
                log_error_errno(errno, "Failed to write ISO9660 root directory: %m");
        if (s != sizeof(sector))
                log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to fully write ISO9660 root directory");
...
static int write_eltorito(int fd, const struct tm *t, uint32_t load_block, const char *system_id, const char *volume_id, const char *publisher_id) {
        ...
        r = write_primary_descriptor(fd, ISO9660_ROOT_DIRECTORY, t, system_id, volume_id, publisher_id);
        if (r < 0)
                return r;
        ...
        r = write_directories(fd, t, ISO9660_ROOT_DIRECTORY);
        if (r < 0)
                return r;
