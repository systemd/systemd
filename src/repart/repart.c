...
static int partition_read_definition(
        ...
        ConfigTableItem table[] = {
                ...
                { "Partition", "AllowDiscards",            config_parse_tristate,          0,                                  &p->allow_discards          },
                {}
        };
        ...
        if (p->encrypt == ENCRYPT_OFF && p->allow_discards == 1)
                log_syntax(NULL, LOG_WARNING, path, 1, 0,
                           "AllowDiscards=yes has no effect with Encrypt=off.");

        if (p->encrypt != ENCRYPT_OFF && p->integrity == INTEGRITY_INLINE && p->allow_discards == 1)
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                  "Integrity=inline is incompatible with AllowDiscards=yes.");
...
static int partition_encrypt(Context *context, Partition *p, PartitionTarget *target, bool offline) {
        ...
        bool allow_discards = p->integrity != INTEGRITY_INLINE && (arg_discard ? p->allow_discards != 0 : p->allow_discards == 1);
        if (allow_discards) {
                uint32_t flags;
