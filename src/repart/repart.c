...
static int parse_append_signature(const char *p, Hashmap **verity_settings_map) {
        ...
        if (r == 2) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to read root hash signature file '%s': %m", base);
        } else {
        ...
        r = hashmap_ensure_put(verity_settings_map,
...
static int parse_argv(int argc, char *argv[], X509 **ret_certificate, EVP_PKEY **ret_private_key, OpenSSLAskPasswordUI **ret_ui) {
        ...
        while ((c = getopt_long(argc, argv, "hs:SCP", options, NULL)) >= 0)
                ...
                case ARG_APPEND_SIGNATURE:
                        r = parse_append_signature(optarg, &arg_verity_settings);
                        if (r < 0)
                                return r;
                        break;
