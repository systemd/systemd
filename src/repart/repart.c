...
int write_primary_descriptor(int fd, uint32_t root_sector, const struct tm *t, const char* volume_id, const char* system_id, const char* publisher_id) {
        ...
        r = set_iso9660_string(desc.header.identifier, 5, "CD001", /* allow_d_chard= */ false);
        assert(r == 0);

        time_to_eltorito_datetime(t, &desc.root_directory_entry.time);

        r = set_iso9660_string(desc.system_identifier, 32, system_id, /* allow_d_chard= */ false);
        if (r < 0)
                return r;
        r = set_iso9660_string(desc.volume_identifier, 32, volume_id, /* allow_d_chard= */ true);
        if (r < 0)
                return r;

        r = set_iso9660_string(desc.volume_set_identifier, 128, NULL, /* allow_d_chard= */ true);
        assert(r == 0);

        r = set_iso9660_string(desc.publisher_identifier, 128, publisher_id, /* allow_d_chard= */ false);
        if (r < 0)
                return r;
        r = set_iso9660_string(desc.data_preparer_identifier, 128, NULL, /* allow_d_chard= */ false);
        assert(r == 0);

        r = set_iso9660_string(desc.application_identifier, 128, "SYSTEMD_REPART", /* allow_d_chard= */ false);
        assert(r == 0);

        r = set_iso9660_string(desc.copyright_file_identifier, 37, NULL, /* allow_d_chard= */ true);
        assert(r == 0);

        r = set_iso9660_string(desc.abstract_file_identifier, 37, NULL, /* allow_d_chard= */ true);
        assert(r == 0);

        r = set_iso9660_string(desc.bibliographic_file_identifier, 37, NULL, /* allow_d_chard= */ true);
        assert(r == 0);
...
int write_eltorito_descriptor(int fd, uint32_t catalog_sector) {
        ...
        r = set_iso9660_string(desc.boot_system_identifier, 32, "EL TORITO SPECIFICATION", /* allow_d_chard= */ true);
        assert(r == 0);
