...
static int parse_argv(int argc, char *argv[]) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION_LONG("dry-run", "BOOL",
                        ...
                        r = parse_boolean_argument("--dry-run=", state.argument, &arg_dry_run);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("empty", "MODE",
                        ...
                        arg_empty = empty_mode_from_string(state.argument);
                        if (arg_empty < 0)
                                return log_error_errno(arg_empty, "Failed to parse --empty= parameter: %s", state.argument);

                        break;

                OPTION_LONG("offline", "BOOL",
                            "Whether to build the image offline"):
                        r = parse_tristate_argument_with_auto("--offline=", state.argument, &arg_offline);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("discard", "BOOL",
                        ...
                        r = parse_boolean_argument("--discard=", state.argument, &arg_discard);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("sector-size", "SIZE",
                        ...
                        r = parse_sector_size(state.argument, &arg_sector_size);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("grain-size", "BYTES",
                        ...
                        r = parse_size(state.argument, 1024, &arg_grain_size);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --grain-size= parameter: %s", state.argument);
                ...
                OPTION_LONG("architecture", "ARCH",
                        ...
                        r = architecture_from_string(state.argument);
                        if (r < 0)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid architecture '%s'.", state.argument);
                ...
                OPTION_LONG("size", "BYTES",
                        ...
                        r = parse_size(state.argument, 1024, &parsed);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --size= parameter: %s", state.argument);
                ...
                OPTION_LONG("seed", "UUID",
                        ...
                        if (isempty(state.argument)) {
                                ...
                                arg_randomize = false;
                        } else if (streq(state.argument, "random"))
                                arg_randomize = true;
                        else {
                                r = sd_id128_from_string(state.argument, &arg_seed);
                                if (r < 0)
                                        return log_error_errno(r, "Failed to parse seed: %s", state.argument);
                ...
                OPTION_LONG("split", "BOOL",
                        ...
                        r = parse_boolean_argument("--split=", state.argument, NULL);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("pretty", "BOOL",
                        ...
                        r = parse_boolean_argument("--pretty=", state.argument, NULL);
                        if (r < 0)
                                return r;
                        arg_pretty = r;
                        break;
                ...
                OPTION_LONG("factory-reset", "BOOL",
                        ...
                        r = parse_boolean_argument("--factory-reset=", state.argument, NULL);
                        if (r < 0)
                                return r;
                        arg_factory_reset = r;
                        break;
                ...
                OPTION_LONG("definitions", "DIR",
                        ...
                        _cleanup_free_ char *path = NULL;
                        r = parse_path_argument(state.argument, false, &path);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("join-signature", "HASH:SIG",
                        ...
                        r = parse_join_signature(state.argument, &arg_verity_settings);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("key-file", "PATH",
                        ...
                        r = parse_key_file(state.argument, &arg_key);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("tpm2-device-key", "PATH",
                        ...
                        r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_tpm2_device_key);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("tpm2-seal-key-handle", "HANDLE",
                        ...
                        r = safe_atou32_full(state.argument, 16, &arg_tpm2_seal_key_handle);
                        if (r < 0)
                                return log_error_errno(r, "Could not parse TPM2 seal key handle index '%s': %m", state.argument);

                        break;

                OPTION_LONG("tpm2-pcrs", "PCR1+PCR2+…",
                            "TPM2 PCR indexes to use for TPM2 enrollment"):
                        r = tpm2_parse_pcr_argument_append(state.argument, &arg_tpm2_hash_pcr_values, &arg_tpm2_n_hash_pcr_values);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("tpm2-public-key", "PATH",
                        ...
                        r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_tpm2_public_key);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("tpm2-public-key-pcrs", "PCR1+PCR2+…",
                        ...
                        r = tpm2_parse_pcr_argument_to_mask(state.argument, &arg_tpm2_public_key_pcr_mask);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("tpm2-pcrlock", "PATH",
                        ...
                        r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_tpm2_pcrlock);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("include-partitions", "PART1,PART2…",
                        ...
                        r = parse_partition_types(state.argument, &arg_filter_partitions, &arg_n_filter_partitions);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("defer-partitions", "PART1,PART2…",
                        ...
                        r = parse_partition_types(state.argument, &arg_defer_partitions, &arg_n_defer_partitions);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("defer-partitions-empty", "BOOL",
                        ...
                        r = parse_boolean_argument("--defer-partitions-empty=", state.argument, &arg_defer_partitions_empty);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("defer-partitions-factory-reset", "BOOL",
                        ...
                        r = parse_boolean_argument("--defer-partitions-factory-reset=", state.argument, &arg_defer_partitions_factory_reset);
                        if (r < 0)
                                return r;
                ...
                OPTION('s', "copy-source", "PATH",
                       "Specify the primary source tree to copy files from"):
                        r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_copy_source);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("copy-from", "IMAGE",
                        ...
                        r = parse_path_argument(state.argument, /* suppress_root= */ false, &p);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("make-ddi", "TYPE",
                        ...
                        if (!filename_is_valid(state.argument))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid DDI type: %s", state.argument);

                        r = free_and_strdup_warn(&arg_make_ddi, state.argument);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("append-fstab", "MODE",
                        ...
                        arg_append_fstab = append_mode_from_string(state.argument);
                        if (arg_append_fstab < 0)
                                return log_error_errno(arg_append_fstab, "Failed to parse --append-fstab= parameter: %s", state.argument);
                        break;

                OPTION_LONG("generate-fstab", "PATH",
                            "Write fstab configuration to the given path"):
                        r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_generate_fstab);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("generate-crypttab", "PATH",
                        ...
                        r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_generate_crypttab);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("el-torito", "BOOL",
                        ...
                        r = parse_boolean_argument("--el-torito=", state.argument, &arg_eltorito);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("el-torito-system", "STRING",
                        ...
                        if (!iso9660_system_name_valid(state.argument))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid value '%s' for --el-torito-system=.", state.argument);

                        r = free_and_strdup_warn(&arg_eltorito_system, state.argument);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("el-torito-volume", "STRING",
                        ...
                        if (!iso9660_volume_name_valid(state.argument))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid value '%s' for --el-torito-volume=.", state.argument);

                        r = free_and_strdup_warn(&arg_eltorito_volume, state.argument);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("el-torito-publisher", "STRING",
                        ...
                        if (!iso9660_publisher_name_valid(state.argument))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid value '%s' for --el-torito-publisher=.", state.argument);

                        r = free_and_strdup_warn(&arg_eltorito_publisher, state.argument);
                        if (r < 0)
                                return r;
