...
typedef enum ProgressPhase {
        ...
        PROGRESS_OPENING_BTRFS_FILESYSTEMS,
...
static const char *progress_phase_table[_PROGRESS_PHASE_MAX] = {
        ...
        [PROGRESS_OPENING_BTRFS_FILESYSTEMS]  = "opening-btrfs-filesystems",
...
static int partition_read_definition(
        ...
        if (p->volume_name && strlen(p->volume_name) > 127 /* DM_NAME_LEN-1 */)
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
...
static int context_open_btrfs_filesystems(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                (void) context_notify(context, PROGRESS_OPENING_BTRFS_FILESYSTEMS, p->definition_path, UINT_MAX);
                ...
                *replacement = (BtrfsReplacement) {
                ...
                r = btrfs_get_block_device_fd(replacement->mountpoint_fd, &replacement->devid, &replacement->source_path, /* ret= */NULL);
                if (r < 0)
