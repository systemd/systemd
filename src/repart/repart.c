...
DEFINE_PRIVATE_STRING_TABLE_LOOKUP(encrypt_pbkdf, EncryptPBKDF);
...
static int partition_encrypt(Context *context, Partition *p, PartitionTarget *target, bool offline) {
         ...
         * memory requirement during luksOpen is prohibitive (e.g. during kexec). */
        if (p->encrypt_pbkdf >= 0) {
                struct crypt_pbkdf_type pbkdf = {
                        .type = encrypt_pbkdf_to_string(p->encrypt_pbkdf),
                        .hash = "sha512",
                        .iterations = 1000,
                        .flags = CRYPT_PBKDF_NO_BENCHMARK,
                };

                r = sym_crypt_set_pbkdf_type(cd, &pbkdf);
                if (r < 0)
