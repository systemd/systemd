...
static int determine_auto_size(
        ...
        LIST_FOREACH(partitions, p, c->partitions) {
                ...
                if (PARTITION_EXISTS(p))
                        sum_allocated += p->current_size + p->current_padding;
        ...
        if (sum_allocated != 0)
                ...
                log_full(level,
                         ...
                         FORMAT_BYTES(sum), FORMAT_BYTES(sum_allocated));
        else
...
static int vl_method_run(
        ...
        if (!context->node) {
                ...
                uint64_t size;
                r = determine_auto_size(context, LOG_DEBUG, &size, /* current_size= */ NULL);
                if (r < 0)
                        return r;
        ...
        if (r == -ENOSPC) {
                ...
                /* Check if space issue is caused by the whole disk being too small */
                if (size != UINT64_MAX && current_size != UINT64_MAX && size > context->total)
        ...
        if (p.dry_run) {
                ...
                r = determine_auto_size(context, LOG_DEBUG, &size, &current_size);
                if (r < 0)
                        return r;
...
static int run(int argc, char *argv[]) {
        ...
        if (arg_node_none) {
                (void) determine_auto_size(context, LOG_INFO, /* ret= */ NULL, /* current_size= */ NULL);
                return 0;
        }

        if (arg_size_auto) {
                r = determine_auto_size(context, LOG_INFO, &arg_size, /* current_size= */ NULL);
                if (r < 0)
                        return r;
