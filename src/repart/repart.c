...
static int do_make_validatefs_xattrs(const Partition *p, const char *root) {
        ...
        _cleanup_free_ char *_mp = NULL;
        const char *mp = NULL;
        if (p->n_mountpoints == 1)
                mp = p->mountpoints[0].where;
        else {
                const char *m = gpt_partition_type_mountpoint_nulstr(p->type);
                if (m) {
                        _cleanup_strv_free_ char **d = strv_split_nulstr(m);
                        if (!d)
                                return log_oom();
        ...
        if (mp) {
                r = xsetxattr(fd, /* path= */ NULL, AT_EMPTY_PATH, "user.validatefs.mount_point", mp);
                if (r < 0)
