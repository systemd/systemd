...
static int config_parse_copy_files(
        ...
        for (const char *opts = options;;) {
                ...
                if ((val = startswith(word, "fsverity="))) {
                        ...
                        return log_syntax(unit, LOG_WARNING, filename, line, 0, "Encountered unknown option '%s', ignoring.", word);
        }
...
static int do_copy_files(Context *context, Partition *p, const char *root) {
        ...
        _cleanup_free_ CopyFilesLine *copy_files = newdup(CopyFilesLine, p->copy_files, p->n_copy_files);
        if (copy_files == NULL)
                return log_oom();
