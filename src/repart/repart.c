...
static int partition_read_definition(
        ...
        if (p->btrfs_replace && arg_offline != 0)
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                  "BtrfsReplace= cannot be used with --offline=no, refusing.");

        if (p->btrfs_replace && (path_is_fs_type(p->btrfs_replace, BTRFS_SUPER_MAGIC) <= 0))
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
...
static int context_load_partition_table(Context *context) {
        ...
        if (context->backing_fd < 0) {
                fd = open(context->node, O_RDWR|O_CLOEXEC);
                if (fd < 0)
                        return -errno;
        }
...
static int context_btrfs_replace(Context *context) {
        ...
        LIST_FOREACH(partitions, p, context->partitions) {
                ...
                r = btrfs_get_block_device(p->btrfs_replace, &id, &dev);
                if (r < 0)
                        return log_error_errno(r, "Failed to find device id for btrfs filesystem: %m");
                if (r == 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EPROTONOSUPPORT), "Btrfs filesystem has multiple devices: %m");
