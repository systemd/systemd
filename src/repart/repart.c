...
static int partition_format_verity_sig(Context *context, Partition *p) {
        ...
        if (!sd_id128_is_null(hp->current_uuid) && !sd_id128_is_null(rp->current_uuid)) {
                ...
                memcpy(root_hash_key, rp->current_uuid.bytes, sizeof(sd_id128_t));
                memcpy(root_hash_key + sizeof(sd_id128_t), hp->current_uuid.bytes, sizeof(sd_id128_t));
                VeritySettings key = {
                        .root_hash = &root_hash_key,
                        .root_hash_size = sizeof(root_hash_key),
                };
                verity_settings = set_get(arg_verity_settings, &key);
        }
