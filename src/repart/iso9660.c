...
int time_to_iso9660_datetime(const struct tm *t, struct iso9660_datetime *ret) {
        if (t->tm_year + 1900 >= 10000)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Year has more than 4 digits and is incompatible with ISO9660.");
        if (t->tm_year + 1900 < 0)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Year is negative and is incompatible with ISO9660.");
