...
static int verify_vc_kdmode(int fd) {
        int mode;
...
static int verify_source_vc(char **ret_path, const char *src_vc) {
        ...
        r = verify_vc_kdmode(fd);
        if (r < 0)
                log_notice("Virtual console %s is not in KD_TEXT, font settings won't be applied.", src_vc);
