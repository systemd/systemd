...
static int manager_connect_bus(Manager *m) {
        ...
        r = bus_jobremoved2_signal_available(m->bus);
        if (r < 0)
        ...
        r = bus_match_signal_async(m->bus, NULL, bus_systemd_mgr, new_signal_available ? "JobRemoved2" : "JobRemoved", match_job_removed, NULL, m);
        if (r < 0)
                return log_error_errno(r, "Failed to add match for JobRemoved%s: %m", new_signal_available ? "2" : "");
