...
int clean_pool_read_first_entry(FILE *file, int child_error, sd_bus_error *error) {
        ...
        if (n != sizeof(success)) {
                log_debug_errno(errno_or_else(EIO), "Received unexpected amount of %d bytes: %m", n);
...
int clean_pool_read_next_entry(FILE *file, char **ret_name, uint64_t *ret_usage) {
        ...
        if (n != sizeof(usage))
                return log_debug_errno(errno_or_else(EIO), "Received unexpected amout of bytes: %m");
