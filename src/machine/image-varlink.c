...
int vl_method_update_image(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (p.read_only >= 0) {
                ...
                if (r < 0)
                        log_debug_errno(r, "Failed to toggle image read only, ignoring: %m");
        }

        if (p.limit != UINT64_MAX) {
                r = image_set_limit(image, p.limit);
                if (r < 0)
                        log_debug_errno(r, "Failed to set image limit, ignoring: %m");
        }

        /* We intentionally swallowed errors from image_read_only() and image_set_limit(). Here we return last one to the user if any */
        if (r < 0)
                return r;
