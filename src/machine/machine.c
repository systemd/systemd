...
static bool machine_leader_pid_valid(Machine *m) {
        ...
        if (r == -ESRCH)
                ...
                log_info_errno(r, "Machine unit %s is active, but the leader process "PID_FMT" belongs to a different unit %s",
                                   m->name, m->leader, m->unit);
        else if (unit && !streq(m->unit, unit))
                log_info("Machine unit %s, the leader process of which is "PID_FMT" doesn't match the real unit %s.",
                          m->name, m->leader, m->unit);
