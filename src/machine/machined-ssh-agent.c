 ...
 * have a usable .pub file. Machines without a registered ssh_private_key_path are skipped;
 * machines whose .pub fails to parse are skipped with a debug log (but still skipped, so a
...
static int handle_request_identities(SshAgentConnection *c) {
        ...
        HASHMAP_FOREACH(m, c->manager->machines) {
                ...
                r = openssh_pubkey_load(pub_path, &ktype, &blob, &blob_size, &kcomment);
                if (r < 0) {
                        log_debug_errno(r, "Skipping machine %s: failed to load %s: %m", m->name, pub_path);
                        continue;
                }

                r = append_identity(&body, m, blob, blob_size);
                if (r < 0)
                        return r;
...
static int connection_write(SshAgentConnection *c) {
        ...
        ssize_t n = write(c->fd, c->wbuf.data + c->wbuf_pos, c->wbuf.size - c->wbuf_pos);
        if (n < 0) {
 ...
 *   < 0  → fatal, caller should close the connection
 *   = 0  → success (or transient EAGAIN/EINTR — nothing to do) */
...
static int on_connection_io(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        /* HUP/ERR with no pending reply to flush → just close. */
        if ((revents & (EPOLLHUP|EPOLLERR)) && (c->wbuf.size == 0 || c->wbuf_pos >= c->wbuf.size)) {
                ssh_agent_connection_free(c);
                return 0;
        }

        r = connection_write(c);
        if (r > 0)
                return connection_arm_io(c); /* partial / would-block — wait for EPOLLOUT */
        if (r == 0 && (revents & EPOLLIN))
                r = connection_read(c);
        if (r == 0)
