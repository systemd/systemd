...
/* Synchronous single-machine VM control call. Used by poweroff/terminate as a probe
 * that returns -EOPNOTSUPP for non-VM machines requiring D-Bus fallback. */
static int verb_vm_control_one(const char *machine_name, const char *method) {
        ...
        const char *error_id = NULL;
        r = sd_varlink_callbo(vl, method, /* ret_reply= */ NULL, &error_id,
...
DEFINE_PRIVATE_HASH_OPS_WITH_VALUE_DESTRUCTOR(vm_control_link_hash_ops, void, trivial_hash_func, trivial_compare_func, sd_varlink, sd_varlink_unref);
...
static int on_vm_control_reply(
                sd_varlink *link,
        ...
        VmControlSlot *slot = ASSERT_PTR(userdata);
        ...
        if (error_id &&
            (!streq_ptr(slot->method, "io.systemd.MachineInstance.Terminate") ||
             !STR_IN_SET(error_id, "io.systemd.MachineInstance.NotConnected", SD_VARLINK_ERROR_DISCONNECTED))) {
                ...
                log_error_errno(r, "VM control call for '%s' failed: %s", slot->machine_name, error_id);
                slot->result = r;
        }
...
/* Connect to a VM's varlink control socket and fire an async method call.
...
static int vm_control_connect_and_fire(sd_event *event, Set **links, VmControlSlot *slot) {
        _cleanup_free_ char *address = NULL;
        int r;
        ...
        r = sd_varlink_connect_address(&vl, address);
        if (r < 0)
                return log_error_errno(r, "Failed to connect to VM control socket for '%s': %m", slot->machine_name);
...
static int vm_control_collect_results(sd_event *event, Set *links, VmControlSlot *slots, int n) {
        int ret = 0, r;
...
static int verb_machine_control(
        ...
        if (n == 1) {
                r = verb_vm_control_one(argv[1], method);
                if (r == -EOPNOTSUPP) {
                        if (bus_fallback)
        ...
        _cleanup_free_ VmControlSlot *slots = new0(VmControlSlot, n);
        if (!slots)
                return log_oom();
        ...
        for (int i = 0; i < n; i++) {
                slots[i] = (VmControlSlot) {
                        .machine_name = argv[i + 1],
                        .method = method,
                };

                r = vm_control_connect_and_fire(event, &links, &slots[i]);
                if (r == -EOPNOTSUPP) {
                        if (bus_fallback)
        ...
        /* Process D-Bus fallbacks for containers while VM varlink calls are in flight */
        if (bus_fallback)
                for (int i = 0; i < n; i++) {
        ...
        return vm_control_collect_results(event, links, slots, n);
}
...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] COMMAND ...\n\n"
               ...
               "  poweroff NAME...            Power off one or more VMs/containers\n"
               "  reboot NAME...              Reboot one or more containers\n"
               "  pause NAME...               Pause one or more VMs\n"
               "  resume NAME...              Resume one or more paused VMs\n"
