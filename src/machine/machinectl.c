...
static int get_settings_path(const char *name, char **ret_path) {
        _cleanup_free_ char *file_alloc = NULL;
        const char *file;
        ...
        if (!endswith(name, ".nspawn")) {
                file_alloc = strjoin(name, ".nspawn");
                if (!file_alloc)
                        return log_oom();
                file = file_alloc;
        } else
...
static int edit_settings(int argc, char *argv[], void *userdata) {
        ...
        STRV_FOREACH(machine, strv_skip(argv, 1)) {
                ...
                r = get_settings_path(file, &path, NULL);
                if (r < 0) {
                        log_debug_errno(r,
                                        "No existing settings file for machine '%s' found, creating.",
                                        *machine);
                ...
                } else if (path_startswith(path, "/var/lib/machines")) {
...
static int cat_settings(int argc, char *argv[], void *userdata) {
        ...
        STRV_FOREACH(machine, strv_skip(argv, 1)) {
                ...
                if (q == -ENOENT) {
                        log_error_errno(q, "No settings file found for machine '%s'.", *machine);
                        r = r < 0 ? r : q;
                        continue;
                }
                if (q < 0)
