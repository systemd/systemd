        ...
        if (r < 0) {
                if (reply.error_id)
                        return log_error_errno(r, "Failed to acquire storage volume '%s:%s' from provider: %s",
                                               bv->provider, bv->volume, reply.error_id);
...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] COMMAND ...\n\n"
               ...
               "  bind-volume NAME SPEC       Attach a storage volume to a running machine\n"
               "  unbind-volume NAME STORAGE  Detach a storage volume from a running machine\n"
