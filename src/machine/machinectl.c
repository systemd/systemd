...
static int attempt_chainload_ssh(sd_bus *bus, const char *name, const char *uid, const char *path, char **cmdline) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to extend command line: %m");
        ...
        r = execv(ssh_path, ssh_cmdline);
        if (r < 0)
                return log_error_errno(r, "Failed to execute SSH: %m");
