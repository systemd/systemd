...
static int on_nscd_cache_flush_event(sd_event_source *s, void *userdata) {
        /* Let's ask glibc's nscd daemon to flush its caches. We request this for the three database machines
         * may show up in: the hosts database (for resolvable machine names) and the user and group databases
         * (for the user ns ranges). */
