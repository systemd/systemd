...
TEST(rename_image_and_update_cache_evicts_stale_target) {
        ...
        char *saved_env[ELEMENTSOF(override_env)] = {};

        /* Save the variables we are about to override, so that at the end we can restore them (or unset
         * them again) instead of clobbering or dropping whatever the environment had before. */
        FOREACH_ELEMENT(n, override_env) {
                const char *v = getenv(*n);
                if (v)
                        ASSERT_NOT_NULL(saved_env[n - override_env] = strdup(v));
        }
        ...
        /* Restore the environment we overrode, so we don't leave stale values (pointing at the temp dir
         * we are about to remove) behind for any test that runs after us in this binary. */
        FOREACH_ELEMENT(n, override_env) {
                ASSERT_OK(set_unset_env(*n, saved_env[n - override_env], /* overwrite= */ true));
                free(saved_env[n - override_env]);
        }
}
