...
static int property_get_netif(sd_bus *bus,
                              const char *path,
        ...
        case MACHINE_HOST: {
                ...
                for (a = addresses, i = 0; i < n; a++, i++) {
                        ...
                        r = sd_bus_message_append_array(
                                reply, 'y', &addresses[i].address, FAMILY_ADDRESS_SIZE(addresses[i].family));
                        if (r < 0)
                                return r;
        ...
        case MACHINE_CONTAINER: {
                ...
                if (streq(us, them))
                        return sd_bus_error_setf(error,
                                                 ...
                                                 "Machine %s does not use private networking",
                                                 m->name);
...
int bus_machine_method_open_pty(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        r = bus_verify_polkit_async(message,
                                    CAP_SYS_ADMIN,
                                    m->class == MACHINE_HOST ? "org.freedesktop.machine1.host-open-pty" :
        ...
        case MACHINE_CONTAINER: {
                ...
                if (r == -ENOENT)
                        return sd_bus_error_set_errnof(
                                error, r, "There is no system bus in container %s.", m->name);
                if (r < 0)
                        return r;
...
int bus_machine_method_open_login(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        r = bus_verify_polkit_async(message,
                                    CAP_SYS_ADMIN,
                                    m->class == MACHINE_HOST ? "org.freedesktop.machine1.host-login" :
...
int bus_machine_method_open_shell(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        if (isempty(path)) {
                ...
                if (!path_is_absolute(path))
                        return sd_bus_error_setf(
                                error, SD_BUS_ERROR_INVALID_ARGS, "Specified path '%s' is not absolute", path);
        ...
        r = bus_verify_polkit_async(message,
                                    CAP_SYS_ADMIN,
                                    m->class == MACHINE_HOST ? "org.freedesktop.machine1.host-shell" :
        ...
        r = sd_bus_message_new_method_call(container_bus,
                                           &tm,
                                           "org.freedesktop.systemd1",
                                           "/org/freedesktop/systemd1",
                                           "org.freedesktop.systemd1.Manager",
...
int bus_machine_method_bind_mount(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        bool mount_slave_created = false, mount_slave_mounted = false, mount_tmp_created = false,
             mount_tmp_mounted = false, mount_outside_created = false, mount_outside_mounted = false;
        ...
        if (S_ISLNK(st.st_mode)) /* This shouldn't really happen, given that we just chased the symlinks
                                    above, but let's better be safe… */
                return sd_bus_error_setf(
                        error, SD_BUS_ERROR_NOT_SUPPORTED, "Source directory can't be a symbolic link");
        ...
        if (r < 0) {
                sd_bus_error_set_errnof(
                        error, errno, "Failed to create temporary mount point %s: %m", mount_tmp);
                goto finish;
        }
        ...
        if (read_only)
                if (mount(NULL, mount_tmp, NULL, MS_BIND | MS_REMOUNT | MS_RDONLY, NULL) < 0) {
                        r = sd_bus_error_set_errnof(
                                error, errno, "Failed to remount read-only %s: %m", mount_tmp);
                        goto finish;
                }

        /* Fourth, we move the new bind mount into the propagation directory. This way it will appear there
        ...
        if (r < 0) {
                sd_bus_error_set_errnof(
                        error, errno, "Cannot create propagation file or directory %s: %m", mount_outside);
                goto finish;
        }
        ...
        if (mount(mount_tmp, mount_outside, NULL, MS_MOVE, NULL) < 0) {
                r = sd_bus_error_set_errnof(
                        error, errno, "Failed to move %s to %s: %m", mount_tmp, mount_outside);
                goto finish;
        }
        ...
        if (r == 0) {
                ...
                if (make_file_or_directory) {
                        if (S_ISDIR(st.st_mode))
                                ...
                                safe_close(
                                        open(dest, O_CREAT | O_EXCL | O_WRONLY | O_CLOEXEC | O_NOCTTY, 0600));
                        }
                }
...
int bus_machine_method_copy(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        const char *src, *dest, *host_path, *container_path, *host_basename, *container_basename,
                *container_dirname;
        _cleanup_close_pair_ int errno_pipe_fd[2] = { -1, -1 };
        ...
        if (isempty(dest))
                ...
                return sd_bus_error_setf(
                        error, SD_BUS_ERROR_INVALID_ARGS, "Destination path must be absolute.");
        ...
        if (hostfd < 0)
                return sd_bus_error_set_errnof(
                        error, hostfd, "Failed to open host directory %s: %m", host_path);
        ...
        if (r == 0) {
                ...
                /* Run the actual copy operation. Note that when an UID shift is set we'll either clamp the
                 * UID/GID to 0 or to the actual UID shift depending on the direction we copy. If no UID
                 * shift is set we'll copy the UID/GIDs as they are. */
                if (copy_from)
                        r = copy_tree_at(containerfd,
                                         container_basename,
                                         hostfd,
                                         host_basename,
...
int bus_machine_method_open_root_directory(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        case MACHINE_CONTAINER: {
                ...
                r = namespace_fork("(sd-openrootns)",
                                   "(sd-openroot)",
                                   NULL,
                                   0,
                                   FORK_RESET_SIGNALS | FORK_DEATHSIG,
...
int bus_machine_method_get_uid_shift(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        /* You wonder why this is a method and not a property? Well, properties are not supposed to return
         * errors, but we kinda have to for this. */
        ...
        if (r == -ENXIO)
                return sd_bus_error_setf(error,
                                         SD_BUS_ERROR_NOT_SUPPORTED,
                                         "Machine %s uses a complex UID/GID mapping, cannot determine shift",
                                         m->name);
        if (r < 0)
                return r;
...
const sd_bus_vtable machine_vtable[] = {
        ...
        SD_BUS_PROPERTY(
                "Scope", "s", NULL, offsetof(Machine, unit), SD_BUS_VTABLE_PROPERTY_CONST | SD_BUS_VTABLE_HIDDEN),
        SD_BUS_PROPERTY("Leader", "u", NULL, offsetof(Machine, leader), SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY(
                "Class", "s", property_get_class, offsetof(Machine, class), SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY(
                "RootDirectory", "s", NULL, offsetof(Machine, root_directory), SD_BUS_VTABLE_PROPERTY_CONST),
        ...
        SD_BUS_METHOD(
                "GetAddresses", NULL, "a(iay)", bus_machine_method_get_addresses, SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_METHOD(
                "GetOSRelease", NULL, "a{ss}", bus_machine_method_get_os_release, SD_BUS_VTABLE_UNPRIVILEGED),
        ...
        SD_BUS_METHOD("OpenRootDirectory",
                      NULL,
                      "h",
...
int machine_send_signal(Machine *m, bool new_machine) {
        ...
        return sd_bus_emit_signal(m->manager->bus,
