...
static int list_image_one_and_maybe_read_metadata(sd_varlink *link, Image *image, bool more, bool read_metadata) {
        ...
        if (image->metadata_valid) {
                r = sd_json_variant_merge_objectbo(
                                ...
                                SD_JSON_BUILD_PAIR_CONDITION(!!image->machine_info, "machineInfo", JSON_BUILD_STRV_ENV_PAIR(image->machine_info)),
                                SD_JSON_BUILD_PAIR_CONDITION(!!image->os_release, "OSRelease", JSON_BUILD_STRV_ENV_PAIR(image->os_release)));
                if (r < 0)
                        return r;
        }
