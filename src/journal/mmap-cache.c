...
_pure_ static inline bool window_matches(Window *w, int prot, uint64_t offset, size_t size) {
        ...
        return prot == w->prot && offset >= w->offset && offset + size <= w->offset + w->size;
}
...
static int try_context(MMapCache *m,
...
static int find_mmap(MMapCache *m,
                     MMapFileDescriptor *f,
                     int prot,
                     unsigned context,
                     bool keep_always,
...
static int add_mmap(MMapCache *m,
                    MMapFileDescriptor *f,
                    int prot,
                    unsigned context,
                    bool keep_always,
...
int mmap_cache_get(MMapCache *m,
                   MMapFileDescriptor *f,
                   int prot,
                   unsigned context,
                   bool keep_always,
