...
int config_parse_journal_keep_free(
         ...
         * keep_free_permyriad takes precedence; the effective value is
         * computed on demand by journal_effective_keep_free(), so that
         * a SIGHUP reload that does not reopen the journal files still
         * picks up the current file system size. */
        if (isempty(rvalue)) {
        ...
        if (r >= 0) {
                ...
                /* A configured percentage invalidates any absolute byte
                 * value, except for 0% which explicitly means "keep
                 * nothing free" (keep_free == 0). */
