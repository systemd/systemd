...
static int vl_method_relinquish_var(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                { "withLUO", SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_stdbool, 0, 0 },
                {}
        };
        ...
        r = sd_varlink_dispatch(link, parameters, dispatch_table, &with_luo);
        if (r != 0)
                return r;
        ...
        log_info("Received client request to relinquish %s access%s.",
                 m->system_storage.path, with_luo ? " with LUO-based preservation" : "");
        manager_relinquish_var(m, with_luo);
