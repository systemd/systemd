...
static int client_parse_log_filter_nulstr(const char *nulstr, Set **set) {
        const char *pattern;
        _cleanup_set_free_ Set *_set = NULL;
        int r;
        ...
        NULSTR_FOREACH(pattern, nulstr) {
                ...
                r = set_ensure_consume(&_set, &pcre2_code_hash_ops_free, TAKE_PTR(compiled_pattern));
                if (r < 0)
                        return log_debug_errno(r, "Failed to insert regex into set: %m");
        }

        set_free_and_replace(*set, _set);
...
int client_context_read_log_filter_patterns(ClientContext *c, const char *cgroup) {
        ...
        else if (ERRNO_IS_XATTR_ABSENT(r)) {
         ...
         * following:
         * - No allow list, nor deny list: 0x00, 0xff, 0x00
         * - Allow list, but no deny list: 0xXX, ...., 0x00, 0x00, 0xff, 0x00
         * - No allow list, but deny list: 0x00, 0xff, 0xXX, ...., 0x00, 0x00
