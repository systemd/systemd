...
static int map_all_fields(
        ...
        for (;;) {
                ...
                for (mf = map_fields; mf->audit_field; mf++) {
                        ...
                        if (r < 0)
                                return log_ratelimit_debug_errno(r, LOG_INTERVAL,
...
void server_process_audit_message(
        ...
        if (!sa ||
            ...
            sa->nl.nl_pid != 0) {
                log_ratelimit_debug(LOG_INTERVAL, "Audit netlink message from invalid sender.");
                return;
        }

        if (!ucred || ucred->pid != 0) {
                log_ratelimit_debug(LOG_INTERVAL, "Audit netlink message with invalid credentials.");
                return;
        }

        if (!NLMSG_OK(nl, buffer_size)) {
                log_ratelimit_error(LOG_INTERVAL, "Audit netlink message truncated.");
                return;
        }
