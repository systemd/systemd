...
void server_forward_console(
        ...
        if (fd < 0) {
                log_ratelimit_debug_errno(fd, LOG_INTERVAL, "Failed to open %s for logging: %m", tty);
                return;
        }

        if (writev(fd, iovec, n) < 0)
                log_ratelimit_debug_errno(errno, LOG_INTERVAL, "Failed to write to %s for logging: %m", tty);
}
