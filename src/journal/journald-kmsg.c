...
void dev_kmsg_record(Server *s, char *p, size_t l) {
        ...
        *e = 0;
        ...
        if (LOG_FAC(priority) == LOG_KERN)
                ...
                /* Avoid logging any new messages when we're processing messages
                 * generated by ourselves to avoid infinite loops. */
                if (is_us(identifier, pid) && target != LOG_TARGET_CONSOLE) {
                        c = s->my_context;
