...
static int determine_path_usage(
        ...
        if (!d)
                return log_ratelimit_full_errno(errno == ENOENT ? LOG_DEBUG : LOG_ERR,
                                                errno, LOG_INTERVAL, "Failed to open %s: %m", path);

        if (fstatvfs(dirfd(d), &ss) < 0)
                return log_ratelimit_error_errno(errno, LOG_INTERVAL,
        ...
        *ret_used = 0;
        FOREACH_DIRENT_ALL(de, d, break) {
                ...
                if (fstatat(dirfd(d), de->d_name, &st, AT_SYMLINK_NOFOLLOW) < 0) {
                        log_ratelimit_debug_errno(errno, LOG_INTERVAL,
        ...
        if (!s->runtime_journal &&
            (s->storage != STORAGE_NONE)) {
                ...
                if (s->system_journal && !relinquish_requested) {
                        ...
                        if (r < 0)
                                return log_ratelimit_warning_errno(r, LOG_INTERVAL,
                                                                   "Failed to open runtime journal: %m");
                }
...
static int do_rotate(
        ...
        if (r < 0) {
                if (*f)
                        return log_ratelimit_error_errno(r, LOG_INTERVAL,
...
static int vacuum_offline_user_journals(Server *s) {
        ...
        for (;;) {
                ...
                if (!de) {
                        if (errno != 0)
                                log_ratelimit_warning_errno(errno, LOG_INTERVAL,
                ...
                if (r < 0) {
                        ...
                        if (r < 0)
                                ...
                                log_ratelimit_debug(LOG_INTERVAL,
...
void server_rotate(Server *s) {
        ...
        log_ratelimit_debug(LOG_DEBUG, "Rotating...");
...
void server_sync(Server *s) {
        ...
        if (s->sync_event_source) {
                ...
                if (r < 0)
                        log_ratelimit_error_errno(r, LOG_INTERVAL,
...
void server_vacuum(Server *s, bool verbose) {
        ...
        log_ratelimit_debug(LOG_INTERVAL, "Vacuuming...");
...
static bool shall_try_append_again(JournalFile *f, int r) {
        ...
        case -ENOSPC:          /* Disk full                     */
                log_ratelimit_debug(LOG_INTERVAL, "%s: Allocation limit reached, rotating.", f->path);
                return true;

        case -EIO:             /* I/O error of some kind (mmap) */
                log_ratelimit_warning(LOG_INTERVAL, "%s: IO error, rotating.", f->path);
                return true;

        case -EHOSTDOWN:       /* Other machine                 */
                log_ratelimit_info(LOG_INTERVAL, "%s: Journal file from other machine, rotating.", f->path);
                return true;

        case -EBUSY:           /* Unclean shutdown              */
                log_ratelimit_info(LOG_INTERVAL, "%s: Unclean shutdown, rotating.", f->path);
                return true;

        case -EPROTONOSUPPORT: /* Unsupported feature           */
                log_ratelimit_info(LOG_INTERVAL, "%s: Unsupported feature, rotating.", f->path);
                return true;

        case -EBADMSG:         /* Corrupted                     */
        case -ENODATA:         /* Truncated                     */
        case -ESHUTDOWN:       /* Already archived              */
                log_ratelimit_warning(LOG_INTERVAL, "%s: Journal file corrupted, rotating.", f->path);
                return true;

        case -EIDRM:           /* Journal file has been deleted */
                log_ratelimit_warning(LOG_INTERVAL, "%s: Journal file has been deleted, rotating.", f->path);
                return true;

        case -ETXTBSY:         /* Journal file is from the future */
                log_ratelimit_warning(LOG_INTERVAL, "%s: Journal file is from the future, rotating.", f->path);
                return true;

        case -EAFNOSUPPORT:
                log_ratelimit_warning(LOG_INTERVAL,
...
static void write_to_journal(Server *s, uid_t uid, struct iovec *iovec, size_t n, int priority) {
        ...
        if (ts.realtime < s->last_realtime_clock) {
                ...
                log_ratelimit_info(LOG_INTERVAL, "Time jumped backwards, rotating.");
                rotate = true;
        } else {
                ...
                if (journal_file_rotate_suggested(f->file, s->max_file_usec, LOG_INFO)) {
                        log_ratelimit_info(LOG_INTERVAL,
        ...
        if (vacuumed || !shall_try_append_again(f->file, r)) {
                log_ratelimit_error_errno(r, 1 * USEC_PER_SEC,
        ...
        if (r == -E2BIG)
                ...
                log_ratelimit_info_errno(r, 1 * USEC_PER_SEC,
        ...
        log_ratelimit_debug_errno(r, LOG_INTERVAL, "Retrying write.");
        r = journal_file_append_entry(f->file, &ts, NULL, iovec, n, &s->seqnum, NULL, NULL);
        if (r < 0)
                log_ratelimit_error_errno(r, 1 * USEC_PER_SEC,
...
int server_flush_to_var(Server *s, bool require_flag_file) {
        ...
        log_ratelimit_debug(LOG_INTERVAL, "Flushing to %s...", s->system_storage.path);
        ...
        SD_JOURNAL_FOREACH(j) {
                ...
                if (r < 0) {
                        log_ratelimit_error_errno(r, LOG_INTERVAL, "Can't read entry: %m");
                        goto finish;
                }
                ...
                if (!shall_try_append_again(s->system_journal->file, r)) {
                        log_ratelimit_error_errno(r, LOG_INTERVAL, "Can't write entry: %m");
                        goto finish;
                }

                log_ratelimit_info(LOG_INTERVAL, "Rotating system journal.");

                server_rotate(s);
                server_vacuum(s, false);

                if (!s->system_journal) {
                        log_ratelimit_notice(LOG_INTERVAL,
                                             "Didn't flush runtime journal since rotation of system journal wasn't successful.");
                        r = -EIO;
                        goto finish;
                }

                log_ratelimit_debug(LOG_INTERVAL, "Retrying write.");
                r = journal_file_copy_entry(f, s->system_journal->file, o, f->current_offset);
                if (r < 0) {
                        log_ratelimit_error_errno(r, LOG_INTERVAL, "Can't write entry: %m");
                        goto finish;
...
finish:
        ...
        k = touch(fn);
        if (k < 0)
                log_ratelimit_warning_errno(k, LOG_INTERVAL,
...
static int server_relinquish_var(Server *s) {
        ...
        log_ratelimit_debug(LOG_INTERVAL, "Relinquishing %s...", s->system_storage.path);
...
int server_process_datagram(
        ...
        if (n < 0) {
                ...
                return log_ratelimit_error_errno(n, LOG_INTERVAL, "recvmsg() failed: %m");
        }
