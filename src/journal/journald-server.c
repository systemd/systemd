...
static int find_user_journal(Server *s, uid_t uid, JournalStorage *storage, ManagedJournalFile **ret) {
        ...
        if ((s->persistent_split_mode == SPLIT_NONE && storage == &s->persistent_storage) ||
            (s->volatile_split_mode == SPLIT_NONE && storage == &s->volatile_storage)) {
                *ret = NULL;
                return 0;
        }
...
static int server_archive_offline_user_journals(Server *s, JournalStorage *storage) {
        ...
        for (;;) {
                ...
                r = journal_file_parse_uid(de->d_name, &uid);
                if (r < 0) {
...
static int do_flush(Server *s, sd_journal *j) {
        ...
        SD_JOURNAL_FOREACH(j) {
                ...
                r = journal_file_parse_uid(f->path, &uid);
                if (r < 0 && r != -EREMOTE)
