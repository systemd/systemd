...
static void write_to_journal(Server *s, uid_t uid, struct iovec *iovec, size_t n, int priority) {
        ...
        if (vacuumed || !shall_try_append_again(f->file, r)) {
                log_every_x_usec_errno(60 * USEC_PER_SEC, LOG_ERR,r, "Failed to write entry (%zu items, %zu bytes), ignoring: %m", n, IOVEC_TOTAL_SIZE(iovec, n));
                return;
        }
        ...
        if (r == -E2BIG)
                ...
                log_every_x_usec_errno(60 * USEC_PER_SEC, LOG_INFO, r, "Failed to write entry to %s (%zu items, %zu bytes), rotating before retrying: %m", f->file->path, n, IOVEC_TOTAL_SIZE(iovec, n));
        ...
        r = journal_file_append_entry(f->file, &ts, NULL, iovec, n, &s->seqnum, NULL, NULL);
        if (r < 0)
                log_every_x_usec_errno(60 * USEC_PER_SEC, LOG_ERR, r, "Failed to write entry to %s (%zu items, %zu bytes) despite vacuuming, ignoring: %m", f->file->path, n, IOVEC_TOTAL_SIZE(iovec, n));
        else
                server_schedule_sync(s, priority);
}
