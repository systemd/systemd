...
static int server_reload_journals(Server *s) {
        ...
        if (s->runtime_journal) {
                if (IN_SET(s->storage, STORAGE_PERSISTENT, STORAGE_AUTO)) {
                        /*
                        ...
                        */
                        ...
                        if (r < 0)
                                ...
                                log_warning_errno(r, "Failed to flush runtime journal and will lose the data, ignoring: %m");
                }

                if (IN_SET(s->storage, STORAGE_VOLATILE, STORAGE_AUTO)) {
