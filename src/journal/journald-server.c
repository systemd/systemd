...
static void server_load_credentials(Server *s) {
        ...
        if (r < 0)
                ...
                memcpy(&s->cred_forward_to_socket, &s->forward_to_socket, sizeof(SocketAddress));
        }
        ...
        if (r < 0)
                ...
                if (r < 0) {
                        ...
                        s->storage = r;
                        s->cred_storage = s->storage;
                }
        }
}

static void server_set_defaults(Server *s, bool reload) {
        assert(s);

        if (!reload) {
                /* Set in credentials */
                s->cred_storage = s->namespace ? STORAGE_PERSISTENT : STORAGE_AUTO;
                s->cred_forward_to_socket = (SocketAddress) { .sockaddr.sa.sa_family = AF_UNSPEC };

                server_load_credentials(s);
        }

        s->storage = s->cred_storage;
        ...
        s->forward_to_wall = false;
        memcpy(&s->forward_to_socket, &s->cred_forward_to_socket, sizeof(SocketAddress));
...
static void server_set_config(Server *s, bool reload) {
        int r;

        assert(s);

        server_set_defaults(s, reload);
        server_parse_config_file(s);

        if (!s->namespace && !reload) {
...
static int server_dispatch_reload_signal(sd_event_source *s, const struct signalfd_siginfo *si, void *userdata) {
        ...
        server_set_config(server, /* reload */ true);
...
int server_new(Server **ret, const char *namespace) {
        ...
        *s = (Server) {
        ...
        server_set_config(s, /* reload */ false);
