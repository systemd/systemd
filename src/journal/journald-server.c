...
static int server_system_journal_open(
        ...
        if (!s->system_journal &&
            ...
            !relinquish_requested) {
                ...
                r = server_open_journal(s, true, fn, O_RDWR|O_CREAT, s->seal, &s->system_storage.metrics, &s->system_journal);
                if (r >= 0) {
        ...
        if (!s->runtime_journal &&
            (s->storage != STORAGE_NONE)) {
                ...
                if (s->system_journal && !relinquish_requested) {
                        ...
                        r = server_open_journal(s, false, fn, O_RDWR, false, &s->runtime_storage.metrics, &s->runtime_journal);
                        if (r < 0) {
                                if (r != -ENOENT)
                                        log_ratelimit_warning_errno(r, JOURNAL_LOG_RATELIMIT,
                        ...
                        r = server_open_journal(s, true, fn, O_RDWR|O_CREAT, false, &s->runtime_storage.metrics, &s->runtime_journal);
                        if (r < 0)
...
static int server_find_user_journal(Server *s, uid_t uid, ManagedJournalFile **ret) {
        ...
        r = server_open_journal(s, true, p, O_RDWR|O_CREAT, s->seal, &s->system_storage.metrics, &f);
        if (r < 0)
                return r;
