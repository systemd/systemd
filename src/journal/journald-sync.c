...
static bool sync_req_is_complete(SyncReq *req) {
        ...
        if (req->boottime_prioq_idx != PRIOQ_IDX_NULL) {
                ...
                if (req->server->dev_kmsg_event_source) {
                        ...
                        r = sd_event_source_get_io_revents(req->server->dev_kmsg_event_source, &revents);
                        if (r < 0 && r != -ENODATA)
...
static int sync_req_realtime_compare(const void *a, const void *b) {
        const SyncReq *x = a, *y = b;

        return CMP(x->realtime, y->realtime);
}

static int sync_req_boottime_compare(const void *a, const void *b) {
        const SyncReq *x = a, *y = b;

        return CMP(x->boottime, y->boottime);
}
...
int sync_req_new(Server *s, sd_varlink *link, SyncReq **ret) {
        ...
        *req = (SyncReq) {
        ...
        /* We use four distinct mechanisms to determine when the synchronization request is complete:
         *
         ...
         *
         * 3. For /dev/kmsg we look at the log message timestamps, similar to the AF_UNIX/SOCK_DGRAM case,
         *    and they are in CLOCK_BOOTTIME clock.
         *
         * 4. Finally, as safety net we install an idle handler with a very low priority (lower than the
         ...
         *
         * Note the asymmetry: for AF_UNIX/SOCK_DGRAM + /dev/kmsg we go by timestamp, for AF_UNIX/SOCK_STREAM
         * we count bytes. That's because for SOCK_STREAM we have no timestamps, and for SOCK_DGRAM we have
         * no API to query all pending bytes (as SIOCINQ on SOCK_DGRAM reports size of next datagram, not
         * size of all pending datagrams). Ideally, we'd actually use neither of this, and the kernel would
         * provide us CLOCK_MONOTONIC timestamps...
         *
