...
void manager_process_native_message(
        ...
        if (ucred && pid_is_valid(ucred->pid)) {
                 ...
                 * the cache */
                r = client_context_acquire(m, ucred->pid, ucred, label, label_len, /* unit_id= */ NULL, &context);
                if (r < 0)
...
static size_t entry_size_max_by_ucred(Manager *m, const struct ucred *ucred, const char *label, size_t label_len) {
        ...
        if (pid_is_valid(ucred->pid)) {
                ...
                r = client_context_get(m, ucred->pid, ucred, label, label_len, /* unit_id= */ NULL, &context);
                if (r < 0)
                        log_ratelimit_warning_errno(r, JOURNAL_LOG_RATELIMIT,
                                                    "Failed to retrieve credentials for PID " PID_FMT ", ignoring: %m",
                                                    ucred->pid);
...
int manager_process_native_file(
         ...
         * use. */
        if ((size_t) st.st_size > entry_size_max_by_ucred(m, ucred, label, label_len) / (sealed ? 1 : 2))
