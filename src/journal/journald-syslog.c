...
static void forward_syslog_iovec(
        ...
        if (r < 0) {
                log_ratelimit_debug_errno(r, LOG_INTERVAL,
        ...
        if (errno != ENOENT)
                log_ratelimit_debug_errno(errno, LOG_INTERVAL, "Failed to forward syslog message: %m");
}
...
void server_process_syslog_message(
        ...
        if (ucred && pid_is_valid(ucred->pid)) {
                ...
                if (r < 0)
                        log_ratelimit_warning_errno(r, LOG_INTERVAL,
