...
static pcre2_pattern *arg_compiled_pattern = NULL;
...
STATIC_DESTRUCTOR_REGISTER(arg_compiled_pattern, pcre2_pattern_freep);
...
static int parse_argv(int argc, char *argv[]) {
        ...
        if (arg_pattern) {
                ...
                r = pcre2_pattern_compile(arg_pattern, arg_case, &arg_compiled_pattern);
                if (r < 0)
                        return r;
        }
        ...
        for (;;) {
                while (arg_lines < 0 || n_shown < arg_lines || (arg_follow && !first_line)) {
                        ...
                        if (arg_compiled_pattern) {
                                ...
                                r = pcre2_pattern_matches(arg_compiled_pattern, message, len - strlen("MESSAGE="), highlight);
                                if (r < 0)
                                        goto finish;
                                if (r == 0) {
                                        need_seek = true;
