...
static int list_boots(sd_journal *j) {
        ...
        table = table_new("#", "boot id", "first entry", "last entry");
        if (!table)
                return log_oom();
        ...
        LIST_FOREACH(boot_list, id, all_ids) {
                ...
                /* Right-align the index unless JSON is requested */
                xsprintf(idx, "%*i", OUTPUT_MODE_IS_JSON(arg_output) ? 0 : w, i - count + 1);
