...
static int parse_lines(const char *arg, bool graceful) {
        ...
        if (!arg) {
                arg_lines = 10; /* default to 10 when no argument */
                return 0;
        }
...
static int parse_argv(int argc, char *argv[]) {
        ...
        if (arg_pattern) {
                ...
                if (arg_lines >= 0 && !arg_lines_oldest && !arg_follow)
                        arg_reverse = true;
        }
...
static int run(int argc, char *argv[]) {
        ...
        } else if (arg_until_set && (arg_reverse || (arg_lines >= 0 && !arg_lines_oldest))) {
        ...
        } else if (arg_lines >= 0 && !arg_lines_oldest) {
                r = sd_journal_seek_tail(j);
                if (r < 0)
                        return log_error_errno(r, "Failed to seek to tail: %m");
