...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "hefo:aln::qmb::kD:p:g:c:S:U:t:T:u:NF:xrM:", options, NULL)) >= 0)
...
static int add_exclude_identifier(sd_journal *j) {
        ...
        j->exclude = set_free_free(j->exclude);

        STRV_FOREACH(i, arg_exclude_identifier) {
                r = set_put_strdup_full(&j->exclude, &string_hash_ops, *i);
                if (r < 0)
                        return r;
        }
