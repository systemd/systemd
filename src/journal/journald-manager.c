...
static int manager_open_runtime_journal_memfd(Manager *m, const char *fn) {
        ...
        if (r <= 0)
                return log_ratelimit_warning_errno(r < 0 ? r : SYNTHETIC_ERRNO(ENOTCONN), JOURNAL_LOG_RATELIMIT,
...
static int manager_materialize_runtime_memfd(Manager *m, int memfd) {
        ...
        if (asprintf(&fn, "system@kexec-%016" PRIx64 "-%016" PRIx64 ".journal",
        ...
        log_info("Materialized kexec-preserved runtime journal to %s/%s.", m->runtime_storage.path, fn);
        return 1;
}
        ...
        if (!m->runtime_journal &&
            (m->config.storage != STORAGE_NONE)) {
                ...
                if (!m->system_journal || relinquish_requested) {
                        ...
                        /* If LUO is available and the client requested it, use a memfd, so that we can push
                         * it to the FD store and get it back after kexec. */
                        if (relinquish_requested && with_luo && manager_kexec_preserve_enabled(m)) {
...
static JournalFile* manager_find_journal(Manager *m, uid_t uid) {
        ...
        (void) manager_system_journal_open(m, /* flush_requested= */ false, /* relinquish_requested= */ false, /* with_luo= */ false);
...
int manager_flush_to_var(Manager *m, bool require_flag_file) {
        ...
        (void) manager_system_journal_open(m, /* flush_requested= */ true, /* relinquish_requested= */ false, /* with_luo= */ false);
...
int manager_relinquish_var(Manager *m, bool with_luo) {
        ...
        (void) manager_system_journal_open(m, /* flush_requested= */ false, /* relinquish_requested= */ true, with_luo);
...
int manager_init(Manager *m) {
        ...
        r = manager_system_journal_open(m, /* flush_requested= */ false, /* relinquish_requested= */ false, /* with_luo= */ false);
        if (r < 0)
                return r;
