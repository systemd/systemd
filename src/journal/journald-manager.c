...
static int manager_materialize_runtime_memfd(Manager *m, int memfd) {
        _cleanup_close_ int dir_fd = -EBADF, dst = -EBADF;
        ...
        dir_fd = chase_and_open(m->runtime_storage.path, /* root= */ NULL,
                                CHASE_MKDIR_0755|CHASE_MUST_BE_DIRECTORY,
                                O_DIRECTORY|O_CREAT|O_CLOEXEC, /* ret_path= */ NULL);
        if (dir_fd < 0)
                return log_ratelimit_warning_errno(dir_fd, JOURNAL_LOG_RATELIMIT,
                                                   "Failed to open runtime journal directory %s: %m",
