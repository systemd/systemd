...
static int parse_trusted_field_allow_list(Set **ret) {
        ...
        /* The environment variable is set so allocate an empty set to differentiate from the variable not
         * being set at all. If the variable is not set, we log all trusted fields. If it's set but empty,
         * we log no trusted fields at all. */

        r = set_ensure_allocated(&s, &iovec_hash_ops);
        if (r < 0)
                return r;
