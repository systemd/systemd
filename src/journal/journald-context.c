...
static int client_context_read_log_filter_patterns(ClientContext *c, const char *cgroup) {
        ...
        if (r == -ENODATA) {
                ...
                c->log_filter_allowed_patterns = set_free_with_destructor(c->log_filter_allowed_patterns,
                                                                          pattern_free);
                c->log_filter_denied_patterns = set_free_with_destructor(c->log_filter_denied_patterns,
                                                                         pattern_free);
                return 0;
        }
        if (r < 0)
                return log_debug_errno(r, "Failed to get user.journald_log_filter_patterns xattr for %s: %m",
                                       cgroup);

        const char *pattern;
        NULSTR_FOREACH(pattern, xattr) {
                pcre2_code *regex = NULL;
                Set **s = &c->log_filter_allowed_patterns;

                if (pattern[0] == '~') {
                        s = &c->log_filter_denied_patterns;
                        pattern++;
                }
                ...
                assert(!set_contains(*s, regex));

                r = set_ensure_put(s, NULL, regex);
                if (r < 0) {
