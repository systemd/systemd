...
#if HAVE_PCRE2
        sym_pcre2_match_data_freep(&c->log_match_data);
        sym_pcre2_code_freep(&c->log_include_regex);
        sym_pcre2_code_freep(&c->log_exclude_regex);
#endif
}
...
static int client_context_read_log_regex(
        ...
        if (r < 0 && r != -ENODATA)
                /* -ENODATA should be considered as an error. It means the key
                 * is not defined. */
        ...
        if (pattern) {
                r = pattern_compile(pattern, 0, &new_regex);
                if (r < 0)
                        return r;
        }

        free_and_replace_full(*regex, new_regex, sym_pcre2_code_free);
...
bool is_log_discarded(ClientContext *c, const char *message) {
        ...
        if (c->log_include_regex && pattern_matched(c->log_include_regex, message, &c->log_match_data) == 0)
                return true;

        if (c->log_exclude_regex && pattern_matched(c->log_exclude_regex, message, &c->log_match_data) == 1)
                return true;
#endif
