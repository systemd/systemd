...
static int find_next_free_vt(int fd, int *ret_free_vt, int *ret_original_vt) {
        ...
        if (i >= sizeof(terminal_status.v_state) * 8)
                return log_error_errno(-1, "No free VT found: %m");
...
static int display_emergency_message_fullscreen(char *message) {
        ...
        qr_code_start_column = w.ws_col * 3U / 5U;
        ...
        r = print_positioned_qrcode(stream, "Scan the code for help", (const char *) message, qr_code_start_row, qr_code_start_column);
        if (r < 0)
