...
static int acquire_first_emergency_log_message(char **ret) {
        ...
        if (r == 0) {
                log_warning("No entries in the journal");
                r = -ENOENT;
                goto clean_journal;
        }
        ...
        *ret = strndup((char *) d, l);
        if (! *ret) {
                r = -ENOMEM;
                log_warning_errno(r, "Failed to duplicate journal message: %m");
                goto clean_journal;
        }
...
static int display_emergency_message_fullscreen(char * message) {
        ...
        r = ioctl(fd, VT_GETSTATE, &terminal_status);
        if (r < 0)
                return -errno;
        ...
        snprintf(tty, sizeof(tty), "/dev/tty%zu", i + 1);

        fd = open_terminal(tty, O_RDWR|O_NOCTTY|O_CLOEXEC);
        if (fd < 0)
                return -errno;

        r = ioctl(fd, VT_ACTIVATE, i + 1);
        if (r < 0)
                return -errno;
