...
static int display_emergency_message_fullscreen(const char *message, const char *message_id) {
        ...
        if (message_id) {
                r = loop_write(fd, "\nScan the QR code below, and search for the entry with the ID: --", SIZE_MAX);
                if (r < 0)
                        return log_warning_errno(r, "Failed to write emergency message to terminal: %m");

                r = loop_write(fd, message_id, SIZE_MAX);
                if (r < 0)
                        return log_warning_errno(r, "Failed to write emergency message to terminal: %m");
        }
        ...
        r = print_qrcode_full(stream,
                              "Scan the QR code", "https://cgit.freedesktop.org/systemd/systemd/plain/catalog/systemd.catalog",
...
=======
        int r;
        _cleanup_free_ char *message = NULL, *message_id = NULL;
>>>>>>> 026ecd32e9 (systemd-bsod: Add catalog reference to displayed messages)
