...
static int display_emergency_message_fullscreen(char * message) {
        assert(message);
        int r, free_vt, original_vt, mid_row = 1;
        struct winsize w;
        ...
        mid_row = w.ws_row / 2;
        r = set_terminal_cursor_start_row(fd, mid_row);
        if (r < 0)
        ...
        stream = fdopen(fd, "r");
        if (stream == NULL)
                return log_error_errno(errno, "Failed to optn stream: %m");

        r = read_one_char(stream, NULL, USEC_INFINITY, NULL);
        if (r < 0)
