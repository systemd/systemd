...
static int acquire_first_emergency_log_message(char **ret) {
        ...
        if (r == 0)
                return log_warning_errno(r, "No entries in the journal");
        ...
        *ret = strndup((char *) d, l);
        if (! *ret) {
                r = -ENOMEM;
                return log_error_errno(r, "Failed to duplicate journal message: %m");
        }
...
static int display_emergency_message_fullscreen(char * message) {
        ...
        snprintf(tty, sizeof(tty), "/dev/tty%d", r + 1);
