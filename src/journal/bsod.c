...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...]\n\n"
               "%5$sFilter the journal to fetch the first message from thecurrent boot with an%6$s\n"
               "%5$semergency log level and displays it as a string and a QR code.%6$s\n"
               "\n%3$sOptions:%4$s\n"
...
static int display_emergency_message_fullscreen(const char *message) {
        int r, ret = 0, free_vt = -1, original_vt = -1;
