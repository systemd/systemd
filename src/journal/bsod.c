...
static int display_emergency_message_fullscreen(char * message) {
        assert(message);
        int r, free_vt, original_vt, qr_code_start_row = 1, qr_code_start_column = 1;
        struct winsize w;
        ...
        r = loop_write(fd, ANSI_BACKGROUND_BLUE, strlen(ANSI_BACKGROUND_BLUE), false);
        if (r < 0)
                log_warning_errno(r, "Failed to change terminal color to blue, ignoring: %m");

        r = loop_write(fd, ANSI_HOME_CLEAR, strlen(ANSI_HOME_CLEAR), false);
        if (r < 0)
                log_warning_errno(r, "Failed to clear terminal, ignoring: %m");

        qr_code_start_row = (int) (w.ws_row * 0.75);
        qr_code_start_column = (int) (w.ws_col * 0.75);
        r = loop_write(fd, message, strlen(message), false);
        if (r < 0)
                return log_warning_errno(r, "Failed to write emergency message to terminal: %m");

        r = set_terminal_cursor_position(fd, qr_code_start_row, qr_code_start_column);
        if (r < 0)
        ...
        r = print_qrcode(out, "Scan the code for help", (char*)"https://www.org.freedesktop.org");
        if (r < 0)
