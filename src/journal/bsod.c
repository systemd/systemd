...
static int find_next_free_vt(int fd, int *ret_free_vt, int *ret_original_vt) {
        ...
        if (i >= sizeof(terminal_status.v_state) * 8)
...
static int display_emergency_message_fullscreen(char *message) {
        ...
        _cleanup_close_ int fd = -EBADF, tty1_fd = -EBADF;
        ...
        tty1_fd = open_terminal("/dev/tty1", O_RDWR|O_NOCTTY|O_CLOEXEC);
        if (tty1_fd < 0)
        ...
        r = loop_write(fd, "The current boot has failed!", STRLEN("The current boot has failed!"), /* do_poll = */false);
        if (r < 0)
        ...
        r = print_positioned_qrcode(stream, "Scan the QR code", (const char *) message, qr_code_start_row, qr_code_start_column, w.ws_col, w.ws_row);
        if (r < 0)
        ...
        r = loop_write(fd, "Press any key to exit...", STRLEN("Press any key to exit..."), /* do_poll = */false);
        if (r < 0)
                return log_warning_errno(r, "Failed to write to terminal: %m");
