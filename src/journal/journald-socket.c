...
void server_forward_socket(
        ...
        _cleanup_free_ uint64_t *len_alloc = NULL;
        uint64_t *len = NULL;
        ...
        if (n < ALLOCA_MAX / (sizeof(struct iovec) + sizeof(uint64_t)) / 2) {
                iov = newa(struct iovec, n);
                len = newa(uint64_t, n_iovec);
        } else {
                ...
                len_alloc = new(uint64_t, n_iovec);
                if (!len_alloc) {
                        log_oom();
                        return;
                }
        ...
        FOREACH_ARRAY(i, iovec, n_iovec) {
                if (must_serialise(*i)) {
                        ...
                        iov[iov_idx++] = IOVEC_MAKE(i->iov_base, c - (char*) i->iov_base);
                        iov[iov_idx++] = IOVEC_MAKE_STRING("\n");

                        /* write the length of the value */
                        len[len_idx] = htole64(i->iov_len - (c - (char*) i->iov_base) - 1);
                        iov[iov_idx++] = IOVEC_MAKE(&len[len_idx++], sizeof(uint64_t));

                        /* write the raw binary value */
                        iov[iov_idx++] = IOVEC_MAKE(c + 1, i->iov_len - (c - (char*) i->iov_base) - 1);
                } else
