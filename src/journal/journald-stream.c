...
static int stdout_stream_save(StdoutStream *s) {
        ...
        if (!s->state_file) {
                ...
                r = fstat(s->fd, &st);
                if (r < 0)
                        return log_ratelimit_warning_errno(errno, LOG_INTERVAL,
        ...
        if (!s->fdstore && !s->in_notify_queue) {
                ...
                if (s->server->notify_event_source) {
                        ...
                        if (r < 0)
                                log_ratelimit_warning_errno(r, LOG_INTERVAL, "Failed to enable notify event source: %m");
                }
        }
...
static int stdout_stream_line(StdoutStream *s, char *p, LineBreak line_break) {
        ...
        if (line_break != LINE_BREAK_NEWLINE && s->state != STDOUT_STREAM_RUNNING)
                return log_ratelimit_warning_errno(SYNTHETIC_ERRNO(EINVAL), LOG_INTERVAL,
        ...
        case STDOUT_STREAM_PRIORITY: {
                ...
                if (priority < 0)
                        return log_ratelimit_warning_errno(priority, LOG_INTERVAL,
...
static int stdout_stream_process(sd_event_source *es, int fd, uint32_t revents, void *userdata) {
        ...
        if (l < 0) {
                ...
                log_ratelimit_warning_errno(errno, LOG_INTERVAL, "Failed to read from stream: %m");
                goto terminate;
        }
...
int stdout_stream_install(Server *s, int fd, StdoutStream **ret) {
        ...
        if (r < 0)
                return log_ratelimit_error_errno(r, LOG_INTERVAL, "Failed to generate stream ID: %m");
        ...
        *stream = (StdoutStream) {
        ...
        if (r < 0)
                return log_ratelimit_error_errno(r, LOG_INTERVAL, "Failed to determine peer credentials: %m");
        ...
        if (mac_selinux_use()) {
                ...
                if (r < 0 && r != -EOPNOTSUPP)
                        (void) log_ratelimit_warning_errno(r, LOG_INTERVAL, "Failed to determine peer security context: %m");
        }
        ...
        if (r < 0)
                return log_ratelimit_error_errno(r, LOG_INTERVAL, "Failed to add stream to event loop: %m");

        r = sd_event_source_set_priority(stream->event_source, SD_EVENT_PRIORITY_NORMAL+5);
        if (r < 0)
                return log_ratelimit_error_errno(r, LOG_INTERVAL, "Failed to adjust stdout event source priority: %m");
...
static int stdout_stream_new(sd_event_source *es, int listen_fd, uint32_t revents, void *userdata) {
        ...
        if (fd < 0) {
                ...
                return log_ratelimit_error_errno(errno, LOG_INTERVAL, "Failed to accept stdout connection: %m");
        }
