 ...
 * but the unit name we embedd in the setup protocol might be longer than that. Hence, during the setup phase
 * let's enforce a line length matching the maximum unit name length (255) */
#define STDOUT_STREAM_SETUP_PROTOCOL_LINE_MAX (UNIT_NAME_MAX-1)
...
static size_t stdout_stream_line_max(StdoutStream *s) {
        ...
        /* During the "setup" phase of our protocol, let's ensure we we use a line length where a full unit
         * name can fit in */
        if (s->state != STDOUT_STREAM_RUNNING)
...
static int stdout_stream_process(sd_event_source *es, int fd, uint32_t revents, void *userdata) {
        ...
        limit = MIN(s->allocated - 1, stdout_stream_line_max(s));
