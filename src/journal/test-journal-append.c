...
static int journal_corrupt_and_append(uint64_t start_offset, uint64_t step) {
        ...
        for (int i = 0; i < 10; i++) {
                ...
                assert_se(asprintf(&message, "MESSAGE=Initial message %d", i));
                r = journal_append_message(mj, message);
                if (r < 0)
                        return log_error_errno(r, "Failed to write to the journal: %m");
        }

        start = start_offset == UINT64_MAX ? random_u32() % mj->file->last_stat.st_size : start_offset;
        ...
        log_info("Start offset: %lu, corrupt-step: %lu", start, step);
        fflush(stdout);
        ...
        for (uint64_t offset = start; offset < end; offset += step) {
                ...
                assert_se(asprintf(&message, "MESSAGE=Hello world %lu", offset));
                r = journal_append_message(mj, message);
                if (r < 0) {
...
int main(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "", options, NULL)) >= 0)
                ...
                case ARG_RUN_ONE:
                        r = safe_atou64(optarg, &start_offset);
                        if (r < 0)
                                return log_error_errno(r, "Invalid value for step: %m");
        ...
        for (uint64_t i = 0; i < iterations; i++) {
                ...
                log_info("Iteration #%lu, step: %lu", i, iteration_step);
