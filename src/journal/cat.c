...
static int parse_argv(int argc, char *argv[], char ***ret_args) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION('t', "identifier", "STRING", "Set syslog identifier"):
                        arg_identifier = empty_to_null(state.argument);
                        break;

                OPTION('p', "priority", "PRIORITY", "Set priority value (0..7)"):
                        arg_priority = log_level_from_string(state.argument);
                        if (arg_priority < 0)
                ...
                OPTION_LONG("stderr-priority", "PRIORITY",
                        ...
                        arg_stderr_priority = log_level_from_string(state.argument);
                ...
                OPTION_LONG("level-prefix", "BOOL",
                        ...
                        r = parse_boolean_argument("--level-prefix=", state.argument, &arg_level_prefix);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("namespace", "NAMESPACE",
                        ...
                        arg_namespace = empty_to_null(state.argument);
                        break;
                }
