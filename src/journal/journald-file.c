...
static int journald_file_entry_array_punch_hole(JournalFile *f, uint64_t p, uint64_t n_entries) {
        ...
        if (p == le64toh(f->header->tail_object_offset) && !f->seal) {
                ...
                if (n != sizeof(EntryArrayObject))
                        return log_debug_errno(SYNTHETIC_ERRNO(EIO), "Short pwrite() while modifying entry array object size: %m");
...
static int journald_file_punch_holes(JournalFile *f) {
        ...
        for (uint64_t i = p; i < p + sz && n > 0; i += n) {
                ...
                if ((size_t) n != m)
                        return log_debug_errno(SYNTHETIC_ERRNO(EIO), "Short read while reading hash table items.");
