...
fail:
        ...
        if (sd_varlink_error_errno(link, r) < 0)
...
static int follow_state_setup(sd_varlink *link, sd_journal **j) {
        ...
        *fs = (FollowState) {
        ...
        sd_varlink_set_userdata(link, fs);
        TAKE_PTR(fs);
...
int vl_method_get_entries(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (p.follow) {
                if (n_sent == 0) {
                        /* The backlog matched nothing so move to the current time so the stream picks up
                         * every matching entry from here on. */
                        r = sd_journal_seek_realtime_usec(j, now(CLOCK_REALTIME));
                        if (r < 0)
                                return r;
                }

                return follow_state_setup(link, &j);
        }
