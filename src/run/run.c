...
static void run_context_check_done(RunContext *c) {
        ...
        r = pty_forward_set_flags(c->forward, /* mask = */ PTY_FORWARD_IGNORE_INITIAL_VHANGUP, /* flags = */ 0);
        if (r < 0) {
                log_error_errno(r, "Failed to disable ignore_initial_vhangup flag in PTY forwarder: %m");
                return (void) sd_event_exit(c->event, EXIT_FAILURE);
        }
}
