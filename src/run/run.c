...
static int run_context_update(RunContext *c) {
        ...
        if (r < 0) {
                ...
                if (sd_bus_error_has_names(&error, SD_BUS_ERROR_NO_REPLY, SD_BUS_ERROR_DISCONNECTED, SD_BUS_ERROR_TIMED_OUT, SD_BUS_ERROR_SERVICE_UNKNOWN, SD_BUS_ERROR_NAME_HAS_NO_OWNER)) {
                        log_info("Bus call failed to to connection problems. Trying to reconnect...");
                        r = run_context_reconnect(c);
                        if (r < 0)
                                return r;

                        return 0; /* Not propagating error, because we handled it already, by reconnecting. */
                }
...
static int on_disconnected(sd_bus_message *m, void *userdata, sd_bus_error *error) {
        ...
        r = run_context_reconnect(ASSERT_PTR(userdata));
        if (r < 0)
                return r;
