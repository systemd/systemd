...
static const char *arg_job_mode = "fail";
...
static int make_transient_service_unit(
        ...
        /* Name and mode */
        r = sd_bus_message_append(m, "ss", service, arg_job_mode);
        if (r < 0)
                return bus_log_create_error(r);
...
static int start_transient_scope(sd_bus *bus) {
        ...
        for (;;) {
                ...
                /* Name and Mode */
                r = sd_bus_message_append(m, "ss", scope, arg_job_mode);
                if (r < 0)
                        return bus_log_create_error(r);
...
static int make_transient_trigger_unit(
        ...
        /* Name and Mode */
        r = sd_bus_message_append(m, "ss", trigger, arg_job_mode);
        if (r < 0)
                return bus_log_create_error(r);
