...
static char *arg_verbose_output = NULL;
...
STATIC_DESTRUCTOR_REGISTER(arg_verbose_output, freep);
...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] COMMAND [ARGUMENTS...]\n"
               ...
               "     --verbose-output             Controls formatting of logs, see journalctl\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        static const struct option options[] = {
                ...
                { "verbose-output",     required_argument, NULL, ARG_VERBOSE_OUTPUT     },
        ...
        while ((c = getopt_long(argc, argv, "+hrC:H:M:E:p:tTPqvGdSu:", options, NULL)) >= 0)
                ...
                case ARG_VERBOSE_OUTPUT:
                        r = free_and_strdup_warn(&arg_verbose_output, optarg);
                        if (r < 0)
                                return r;
                        break;
...
static int start_transient_service(sd_bus *bus) {
        ...
        if (arg_verbose)
                (void) journal_fork(arg_runtime_scope, STRV_MAKE(c.unit), arg_verbose_output, &journal_pid);
