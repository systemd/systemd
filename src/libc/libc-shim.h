...
/* For libc helpers that are guaranteed to be present in any libc we care about — the dlsym call here
 * isn't about feature-detection, it's solely to avoid acquiring a hard link-time dependency on a
 * specific symbol version (e.g. fmod was bumped to GLIBC_2.38, exp10 to GLIBC_2.39, but the GLIBC_2.2.5
 * versions are still exported and behaviorally adequate). dlsym returns whatever the runtime's default
 * version happens to be, with no version constraint baked into our binary. */
#define DEFINE_LIBC_PURE_SHIM(func, ret, ...)                                                        \
        ret func##_shim(_SHIM_DECL(__VA_ARGS__)) {                                                   \
                static void *cache = (void *) -1;                                                    \
                void *p = __atomic_load_n(&cache, __ATOMIC_ACQUIRE);                                 \
                if (p == (void *) -1) {                                                              \
                ...
                return ((typeof(&func##_shim)) p)(_SHIM_NAME(__VA_ARGS__));                          \
        }
