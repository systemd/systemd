...
int user_record_add_thin_pool_binding(
        ...
        _cleanup_free_ char *p = NULL, *v = NULL, *u = NULL;
        sd_id128_t mid;
        int r;
        ...
        v = strdup(volume);
        if (volume_uuid)
                u = strdup(volume_uuid);
        if (!p || !v || (volume_uuid && !u))
                return -ENOMEM;
        ...
        r = sd_json_variant_set_field_string(&entry, "thinPool", pool);
        if (r < 0)
                return r;
        r = sd_json_variant_set_field_string(&entry, "thinVolume", volume);
        if (r < 0)
                return r;
        if (volume_uuid) {
                r = sd_json_variant_set_field_string(&entry, "thinVolumeUuid", volume_uuid);
                if (r < 0)
                        return r;
        } else {
                r = sd_json_variant_filter(&entry, STRV_MAKE("thinVolumeUuid"));
                if (r < 0)
                        return r;
        }
        ...
        free_and_replace(h->thin_pool, p);
        free_and_replace(h->thin_volume, v);
        if (u)
                free_and_replace(h->thin_volume_uuid, u);
        else
                h->thin_volume_uuid = mfree(h->thin_volume_uuid);
        h->mask |= USER_RECORD_BINDING;
        return 1;
}
