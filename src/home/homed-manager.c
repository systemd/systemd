...
static int manager_record_protects_thin_intent(
        ...
        return sd_id128_equal(record->uuid, intent->uuid) &&
                streq_ptr(record->thin_pool, intent->thin_pool) &&
                streq_ptr(record->thin_volume, intent->thin_volume) &&
                !isempty(record->thin_volume_uuid);
}
...
static int manager_recover_thin_volume_intents(Manager *m) {
        ...
        FOREACH_DIRENT(de, d, return log_error_errno(errno, "Failed to read record directory: %m")) {
                ...
                if (r == 0) {
                        r = home_thin_volume_remove_incomplete(intent);
                        if (r < 0) {
                                log_error_errno(r,
...
int manager_recover_records(Manager *m) {
        ...
        ret = manager_recover_pending_records(m);
