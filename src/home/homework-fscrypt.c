...
static int fscrypt_unlink_key(UserRecord *h) {
        ...
        if (r < 0)
                return log_debug_errno(r, "Failed to change UID/GID to " UID_FMT "/" GID_FMT ": %m",
                                       h->uid, user_record_gid(h));

        r = keyring_read(KEY_SPEC_USER_KEYRING, (void **) &keyring, &keyring_size);
        if (r < 0)
                return log_debug_errno(r, "Failed to read the keyring of user " UID_FMT ": %m", h->uid);
        ...
        FOREACH_ARRAY(key, ((key_serial_t *) keyring), n_keys) {
                ...
                if (r < 0) {
                        ...
                        return log_debug_errno(r, "Failed to describe key id %d: %m", *key);
                }
...
int home_flush_keyring_fscrypt(UserRecord *h) {
        ...
        r = safe_fork("(sd-delkey)",
                      ...
                      NULL);
        if (r < 0)
                return log_debug_errno(r, "Failed to fork to delete fscrypt key from keyring: %m");
        if (r == 0) {
