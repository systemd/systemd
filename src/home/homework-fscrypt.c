...
static int fscrypt_slot_try_v2(
        ...
        if (sym_EVP_CIPHER_CTX_ctrl(context, EVP_CTRL_GCM_SET_IVLEN, (int) iv->iov_len, NULL) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to set GCM IV length.");

        if (sym_EVP_DecryptInit_ex(context, NULL, NULL, derived, iv->iov_base) != 1)
...
static int fscrypt_slot_try_one(
        ...
        r = make_cstring(body, xattr_size - 3, MAKE_CSTRING_REFUSE_TRAILING_NUL, &body_str);
        if (r < 0)
        ...
        if (ret_decrypted)
                *ret_decrypted = TAKE_PTR(decrypted.iov_base);
        if (ret_decrypted_size)
                *ret_decrypted_size = decrypted.iov_len;
