...
/* RAII rollback for v2 master-key installation. Once armed via fscrypt_v2_key_undo_set(), removes
 ...
 * Disarm on success by clearing image_path; the path is re-opened on demand so the caller does not
 * have to keep the original dir_fd alive. */
...
static void fscrypt_v2_key_undo_done(FscryptV2KeyUndo *u) {
        ...
        _cleanup_close_ int fd = open(u->image_path, O_RDONLY|O_CLOEXEC|O_DIRECTORY|O_NOFOLLOW);
        if (fd < 0) {
                log_debug_errno(errno, "Failed to re-open %s for v2 key rollback, leaving key in keyring: %m",
                                u->image_path);
                return;
        }

        (void) fscrypt_v2_ioctl_remove(fd, u->identifier);
}

static void fscrypt_v2_key_undo_set(
                FscryptV2KeyUndo *u,
                const char *image_path,
                const struct fscrypt_key_specifier *spec) {
        ...
        if (spec->type != FSCRYPT_KEY_SPEC_TYPE_IDENTIFIER)
                return; /* v1: nothing to roll back */
...
int home_setup_fscrypt(
        ...
        _cleanup_(fscrypt_v2_key_undo_done) FscryptV2KeyUndo v2_key_undo = {};
        const char *ip;
        int r;
        ...
        /* Arm v2 master-key rollback in case any of the steps below fails. Disarmed at the very end
         * once setup is complete and the live home owns the key. */
        fscrypt_v2_key_undo_set(&v2_key_undo, ip, &setup->fscrypt_key_spec);
        ...
        v2_key_undo.image_path = NULL; /* setup complete: live home now owns the v2 key */
        return 0;
}
...
int home_create_fscrypt(
        ...
        /* Arm v2 master-key rollback through the temporary path. After rename below the temporary
         * path stops resolving, so we update the cleanup target to ip just before disarming. */
        fscrypt_v2_key_undo_set(&v2_key_undo, temporary, &setup->fscrypt_key_spec);
