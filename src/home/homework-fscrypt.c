...
static int compute_fscrypt_key_descriptor_v1(
                const void *key,
                size_t key_size,
...
static int fscrypt_key_spec_matches(
        ...
        case FSCRYPT_KEY_SPEC_TYPE_DESCRIPTOR: {
                ...
                r = compute_fscrypt_key_descriptor_v1(key, key_size, descriptor);
                if (r < 0)
                        return r;
...
static int fscrypt_create_policy_v1(
        ...
        if (compute_fscrypt_key_descriptor_v1(volume_key, volume_key_size, policy_v1.master_key_descriptor) < 0)
                return -EIO;
