...
static int sync_delete_callback(
        ...
        log_debug("Potentially deleted file: %s", path);
...
static int run(int argc, char *argv[]) {
        _cleanup_close_ int sys = -EBADF, home = -EBADF;
        ...
        /* Tell the service manager that we're deactivating. This makes the service manager
         * start listening for inotify events and queuing startup requests again. Thus, any
         * writes that happen between the time that we drop our locks and the time when this
         * service exits are not lost, and the service will immediately be restarted. */
        sd_notify(false, "STOPPING=1");
        sd_notify_barrier(false, UINT64_MAX); /* ensure systemd gets the signal before we drop locks */
