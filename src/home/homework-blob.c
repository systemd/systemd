...
static int copy_one_blob(
        ...
        if (fstat(src_fd, &st) < 0)
                return log_debug_errno(errno, "Failed to stat fd for blob %s: %m", name);
        if (S_ISDIR(st.st_mode)) {
                log_wraning("Blob %s is a directory. Skipping.", name);
                return 0;
        } else if (!S_ISREG(st.st_mode)) {
                log_warning("Blob %s is not a regular file. Skipping.", name);
                return 0;
        }
        ...
        if (memcmp(hash, known_hash, SHA256_DIGEST_SIZE) != 0) {
                log_wraning("Blob %s has incorrect hash. Skipping.", name);
                return 0;
        }
        ...
        dest = open_tmpfile_linkable_at(dest_dfd, name, O_WRONLY|O_CLOEXEC, &dest_tmpname);
        if (dest < 0)
        ...
        if (lseek(dest, 0, SEEK_SET) < 0)
                return log_debug_errno("Failed to rewind blob %s for rehash: %m", name);
