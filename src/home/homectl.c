...
static int parse_argv(int argc, char *argv[]) {
        ...
        for (;;) {
                ...
                case ARG_BIRTH_DATE:
                        if (isempty(optarg)) {
                                r = drop_from_identity("birthDate", match_identity ?: &arg_identity_extra);
                                if (r < 0)
                                        return r;
                        } else {
                                r = parse_birth_date(optarg, /* ret= */ NULL);
                                if (r < 0)
                                        return log_error_errno(r, "Failed to parse birth date (expected YYYY-MM-DD): %s", optarg);

                                r = parse_string_field(match_identity ?: &arg_identity_extra, "birthDate", optarg);
                                if (r < 0)
                                        return r;
                        }
                        break;
