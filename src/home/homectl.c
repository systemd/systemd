...
static int parse_ssh_authorized_keys(const char *arg) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *v = NULL;
        _cleanup_strv_free_ char **l = NULL, **add = NULL;
        int r;

        if (isempty(arg))
                return drop_from_identity("sshAuthorizedKeys");
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to set %s field: %m", "sshAuthorizedKeys");
...
static int parse_boolean_field(sd_json_variant **identity, const char *field, const char *arg) {
        ...
        r = parse_boolean(arg);
        if (r < 0)
                return log_error_errno(r, "Failed to parse boolean '%s': %m", arg);
...
static int parse_rlimit_field(const char *arg) {
        int r;
        ...
        if (isempty(eq + 1)) {
                ...
                r = strv_extend(&arg_identity_filter_rlimits, rlimit_to_string(limit));
                if (r < 0)
                        return r;

                r = sd_json_variant_filter(&arg_identity_extra_rlimits, STRV_MAKE(s));
                if (r < 0)
                        return log_error_errno(r, "Failed to filter JSON identity data: %m");
                return 0;
        }
        ...
        _cleanup_free_ char *field = strjoin("RLIMIT_", rlimit_to_string(limit));
        if (!field)
                return log_oom();

        r = sd_json_variant_set_fieldbo(
                        &arg_identity_extra_rlimits, field,
...
static int parse_weight_field(sd_json_variant **identity, const char *field, const char *arg) {
        ...
        if (!CGROUP_WEIGHT_IS_OK(u))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "Weight %" PRIu64 " is out of valid range.", u);
...
static int parse_argv(int argc, char *argv[]) {
        ...
        for (;;) {
                ...
                case ARG_SSH_AUTHORIZED_KEYS:
                        r = parse_ssh_authorized_keys(optarg);
                        if (r < 0)
                                return r;
