...
static int acquire_merged_blob_dir(UserRecord *hr, bool existing, Hashmap **ret) {
        ...
        const char *src_blob_path;
        int r;

        assert(ret);

        if (!hashmap_isempty(arg_blob_files)) {
                blobs = hashmap_copy(arg_blob_files);
                if (!blobs)
                        return log_oom();
        }
        ...
        FOREACH_DIRENT_ALL(de, d, return log_error_errno(errno, "Failed to read %s: %m", src_blob_path)) {
                ...
                if (de->d_type != DT_REG) {
                        log_warning("Entry %s in blob directory %s is not a regular file. Skipping.", de->d_name, src_blob_path);
                        continue;
                }
