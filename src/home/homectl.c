...
static void finalize_recovery_key(const char *recovery_key, bool write_file) {
        int r;

        if (!recovery_key)
                return;

        if (write_file && arg_recovery_key_file) {
                r = recovery_key_file_write(arg_recovery_key_file, recovery_key);
                if (r < 0)
                        log_warning_errno(r,
                                          "Failed to write recovery key file '%s', ignoring: %m",
                                          arg_recovery_key_file);
        }
...
static int create_home_common(sd_json_variant *input, bool show_enforce_password_policy_hint) {
        ...
        if (arg_dry_run) {
                ...
                finalize_recovery_key(recovery_key, /* write_file= */ false);
                return 0;
        }
        ...
        finalize_recovery_key(recovery_key, /* write_file= */ true);
        return 0;
}
