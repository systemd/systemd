...
static void finalize_recovery_key(const char *recovery_key, RecoveryKeyFile *recovery_key_file) {
        int r;

        if (!recovery_key)
                return;

        if (recovery_key_file && recovery_key_file->fd >= 0) {
                r = recovery_key_file_write(recovery_key_file, recovery_key);
                if (r < 0)
                        log_warning_errno(r, "Failed to write recovery key file '%s', ignoring: %m", recovery_key_file->path);
        }
...
static int create_home_common(sd_json_variant *input, bool show_enforce_password_policy_hint) {
        ...
        _cleanup_hashmap_free_ Hashmap *blobs = NULL;
        _cleanup_(erase_and_freep) char *recovery_key = NULL;
        _cleanup_(recovery_key_file_done) RecoveryKeyFile recovery_key_file = RECOVERY_KEY_FILE_NULL;
        int r;
        ...
        if (arg_recovery_key_file) {
                r = recovery_key_file_prepare(&recovery_key_file, arg_recovery_key_file);
                if (r < 0)
                        return log_error_errno(r, "Failed to create recovery key file '%s': %m", arg_recovery_key_file);
        }
        ...
        finalize_recovery_key(recovery_key, &recovery_key_file);
        return 0;
}
