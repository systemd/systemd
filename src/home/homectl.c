...
DEFINE_PRIVATE_HASH_OPS_FULL(bulk_hash_ops,
...
static int apply_bulk_dir_changes(JsonVariant **v, bool new, char **ret_tmpdir) {
        ...
        if (tmp_dfd < 0)
                return log_debug_errno(r, "Failed to open bulk tmpdir: %m");
...
static int parse_argv(int argc, char *argv[]) {
        ...
        for (;;) {
                ...
                case ARG_LOGIN_BACKGROUND: {
                        ...
                        TAKE_FD(fd); /* hashmap takes ownership of the FD */
