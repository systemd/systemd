...
static int home_start_work(
        ...
        r = json_variant_set_field_non_null(&v, HOMEWORK_BLOB_FDMAP_FIELD, fdmap);
        if (r < 0)
                return r;
        ...
        if (r == 0) {
                ...
                HASHMAP_FOREACH(fd_ptr, blobs) {
                        r = fd_cloexec(PTR_TO_FD(fd_ptr), false);
                        if (r < 0) {
                                log_error_errno(r, "Failed to disable CLOEXEC on blob fd: %m");
                                _exit(EXIT_FAILURE);
                        }
                }
