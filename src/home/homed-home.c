...
void home_process_notify(Home *h, char **l, int fd) {
        ...
        if (e) {
                r = parse_boolean(e);
                if (r < 0) {
                        log_debug_errno(r, "Failed to parse SYSTEMD_LUKS_LOCK_FD value: %m");
                        return;
                }
                if (r > 0) {
                        if (taken_fd < 0) {
                                log_debug("Got notify message with SYSTEMD_LUKS_LOCK_FD=1 but no fd passed, ignoring: %m");
                                return;
                        }
                        ...
                        if (taken_fd >= 0) {
                                log_debug("Got notify message with SYSTEMD_LUKS_LOCK_FD=0 but fd passed, ignoring: %m");
                                return;
                        }
        ...
        if (!e) {
                log_debug("Got notify message lacking both ERRNO= and SYSTEMD_LUKS_LOCK_FD= field, ignoring.");
                return;
        }
