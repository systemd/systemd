...
static int home_test_image_path(Home *h) {
        ...
        if (r == USER_TEST_ABSENT && h->record->thin_pool)
...
static int home_get_disk_status_luks(
        ...
        if (h->record->thin_pool && h->record->disk_size != UINT64_MAX)
                disk_ceiling = disk_size = h->record->disk_size;

        if (state != HOME_ABSENT && (!h->record->thin_pool || HOME_STATE_IS_ACTIVE(state))) {
                const char *ip;
