...
#if HAVE_PWQUALITY || HAVE_PASSWDQC
...
int user_record_check_password_quality(
        ...
        STRV_FOREACH(pp, secret->password) {
                ...
                r = check_password_quality(*pp, NULL, hr->user_name, &auxerror);
                if (r <= 0)
                        goto error;
        }
        return 1;
...
error:
        ...
        return sd_bus_error_set_errnof(error, r, "Failed to check password quality: %m");
}
