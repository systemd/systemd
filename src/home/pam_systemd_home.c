...
static int handle_generic_user_record_error(
        ...
        if (sd_bus_error_has_name(error, BUS_ERROR_HOME_ABSENT)) {
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL,
                                  N_("Home of user %s is currently absent, please plug in the necessary storage device or backing file system."), user_name);
                ...
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Too frequent login attempts for user %s, try again later."), user_name);
                ...
                if (strv_isempty(secret->password))
                        r = pam_prompt(handle, PAM_PROMPT_ECHO_OFF, &newp, N_("Password: "));
                else {
                        (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Password incorrect or not sufficient for authentication of user %s."), user_name);
                        r = pam_prompt(handle, PAM_PROMPT_ECHO_OFF, &newp, N_("Sorry, try again: "));
                }
                if (r != PAM_SUCCESS)
                ...
                if (strv_isempty(secret->password))
                        r = pam_prompt(handle, PAM_PROMPT_ECHO_OFF, &newp, N_("Recovery key: "));
                else {
                        (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Password/recovery key incorrect or not sufficient for authentication of user %s."), user_name);
                        r = pam_prompt(handle, PAM_PROMPT_ECHO_OFF, &newp, N_("Sorry, reenter recovery key: "));
                }
                if (r != PAM_SUCCESS)
                        return PAM_CONV_ERR; /* no logging here */
                ...
                if (strv_isempty(secret->password)) {
                        (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Security token of user %s not inserted."), user_name);
                        r = pam_prompt(handle, PAM_PROMPT_ECHO_OFF, &newp, N_("Try again with password: "));
                } else {
                        (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Password incorrect or not sufficient, and configured security token of user %s not inserted."), user_name);
                        r = pam_prompt(handle, PAM_PROMPT_ECHO_OFF, &newp, N_("Try again with password: "));
                }
                if (r != PAM_SUCCESS)
                        return PAM_CONV_ERR; /* no logging here */
                ...
                r = pam_prompt(handle, PAM_PROMPT_ECHO_OFF, &newp, N_("Security token PIN: "));
                if (r != PAM_SUCCESS)
                        return PAM_CONV_ERR; /* no logging here */
                ...
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Please authenticate physically on security token of user %s."), user_name);
                ...
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Please confirm presence on security token of user %s."), user_name);
                ...
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Please verify user on security token of user %s."), user_name);
                ...
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Security token PIN is locked, please unlock it first. (Hint: Removal and re-insertion might suffice.)"));
                return PAM_SERVICE_ERR;
                ...
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Security token PIN incorrect for user %s."), user_name);
                r = pam_prompt(handle, PAM_PROMPT_ECHO_OFF, &newp, N_("Sorry, retry security token PIN: "));
                if (r != PAM_SUCCESS)
                        return PAM_CONV_ERR; /* no logging here */
                ...
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Security token PIN of user %s incorrect (only a few tries left!)"), user_name);
                r = pam_prompt(handle, PAM_PROMPT_ECHO_OFF, &newp, N_("Sorry, retry security token PIN: "));
                if (r != PAM_SUCCESS)
                        return PAM_CONV_ERR; /* no logging here */
                ...
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Security token PIN of user %s incorrect (only one try left!)"), user_name);
                r = pam_prompt(handle, PAM_PROMPT_ECHO_OFF, &newp, N_("Sorry, retry security token PIN: "));
                if (r != PAM_SUCCESS)
                        return PAM_CONV_ERR; /* no logging here */
...
static int acquire_home(
        ...
        for (;;) {
                ...
                if (r < 0) {
                        ...
                        if (sd_bus_error_has_name(&error, BUS_ERROR_HOME_NOT_ACTIVE))
                                ...
                                if (r == PAM_CONV_ERR) {
                                        ...
                                        if (home_not_active)
                                                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Home of user %s is currently not active, please log in locally first."), ur->user_name);
                                        if (home_locked)
                                                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Home of user %s is currently locked, please unlock locally first."), ur->user_name);
                ...
                if (++n_attempts >= 5) {
                        (void) pam_prompt(handle, PAM_ERROR_MSG, NULL,
                                          N_("Too many unsuccessful login attempts for user %s, refusing."), ur->user_name);
                        return pam_syslog_pam_error(handle, LOG_ERR, PAM_MAXTRIES,
...
_public_ PAM_EXTERN int pam_sm_acct_mgmt(
        ...
        case -ENOLCK:
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("User record is blocked, prohibiting access."));
                return PAM_ACCT_EXPIRED;

        case -EL2HLT:
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("User record is not valid yet, prohibiting access."));
                return PAM_ACCT_EXPIRED;

        case -EL3HLT:
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("User record is not valid anymore, prohibiting access."));
                return PAM_ACCT_EXPIRED;

        default:
                if (r < 0) {
                        (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("User record not valid, prohibiting access."));
                        return PAM_ACCT_EXPIRED;
                }
        ...
        if (t != USEC_INFINITY) {
                ...
                if (t > n) {
                        (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Too many logins, try again in %s."),
        ...
        case -EKEYREVOKED:
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Password change required."));
                return PAM_NEW_AUTHTOK_REQD;

        case -EOWNERDEAD:
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Password expired, change required."));
                return PAM_NEW_AUTHTOK_REQD;
        ...
        case -EKEYREJECTED:
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Password is expired, but can't change, refusing login."));
                return PAM_AUTHTOK_EXPIRED;

        case -EKEYEXPIRED:
                (void) pam_prompt(handle, PAM_ERROR_MSG, NULL, N_("Password will expire soon, please change."));
                break;
