...
int home_passwd_luks(
        ...
        /* This does not account for the old slots that transiently coexist with the new ones during the
         * two-pass rotation below: if the header is (near) full the first add will fail with ENOSPC
         * mid-rotation and will be rolled back with the old slots left intact. */
