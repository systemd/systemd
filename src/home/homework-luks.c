...
static uint64_t home_max_reserve(uint64_t capacity) {
        return MAX(HOME_MIN_FREE, DISK_SIZE_ROUND_UP(capacity / 100U * HOME_MAX_FREE_PERCENT));
}
...
int home_setup_luks(
        ...
        if (FLAGS_SET(flags, HOME_SETUP_ALREADY_ACTIVATED)) {
                ...
                /* Metadata operations may proceed with a sparse image when space is tight. Activation and
                 * resize deliberately don't set this flag: they must preserve the allocation guarantees
                 * implied by luksDiscard=no and keep resize operations safe. */
                ...
                setup->offline_fallocate_best_effort = FLAGS_SET(flags, HOME_SETUP_DONT_FALLOCATE);
        }
...
static int calculate_initial_image_size(UserRecord *h, int image_fd, const char *fstype, uint64_t *ret) {
        ...
        if (user_record_disk_size_is_fill(h))
                *ret = DISK_SIZE_ROUND_DOWN(MIN(
                                LESS_BY(upper_boundary, home_max_reserve(upper_boundary)),
                                USER_DISK_SIZE_MAX));
...
static int get_largest_image_size(int fd, const struct stat *st, bool reserve_free, uint64_t *ret) {
        ...
        if (reserve_free)
                sum = LESS_BY(sum, home_max_reserve(sum));
...
int home_resize_luks(
        ...
        if (S_ISREG(st.st_mode)) {
                ...
                r = get_largest_image_size(
                                setup->image_fd,
                                &st,
                                user_record_disk_size_is_fill(h),
                                &largest_size);
                if (r < 0)
                        return r;
                ...
                if (FLAGS_SET(flags, HOME_SETUP_RESIZE_MINIMIZE)) {
                        ...
                        if ((h->disk_size == UINT64_MAX || user_record_disk_size_is_fill(h)) && partition) {
                                r = get_maximum_partition_size(image_fd, partition, &new_partition_size_rounded);
                                if (r < 0)
                                        return r;
                        } else if (user_record_disk_size_is_fill(h))
                                new_partition_size_rounded = old_image_size;
                        else if (h->disk_size == UINT64_MAX)
                        ...
                        if (setup->partition_size >= new_partition_size_rounded &&
                            setup->partition_size <= (user_record_disk_size_is_fill(h) ? new_partition_size_rounded : h->disk_size)) {
