...
int home_resize_luks(
        ...
        if (S_ISBLK(st.st_mode)) {
                ...
                if (FLAGS_SET(flags, HOME_SETUP_RESIZE_MINIMIZE)) {
                        ...
                        /* Determine the actual target below, after accounting for the free space reserve. */
                        new_image_size = old_image_size;
                } else {
        ...
        if (S_ISREG(st.st_mode)) {
                ...
                if (r < 0)
                        return r;
                if (user_record_disk_size_is_fill(h) &&
                    !FLAGS_SET(flags, HOME_SETUP_RESIZE_MINIMIZE)) {
                        ...
                        intention = CMP(new_image_size, old_image_size);
                } else if (new_image_size > largest_size)
                        new_image_size = largest_size;
