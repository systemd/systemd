...
static int get_smallest_fs_size(int fd, uint64_t *ret) {
        ...
        if (minsz == UINT64_MAX)
                return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP), "Don't know minimum file system size of home directory.");

        if (minsz < USER_DISK_SIZE_MIN)
                minsz = USER_DISK_SIZE_MIN;

        if (sfs.f_bfree > sfs.f_blocks)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "More disk space free than file system is large. Refusing.");
        ...
        for (uint64_t lower_boundary = new_fs_size, upper_boundary = old_fs_size, try_fs_size = new_fs_size;;) {
                ...
                if (resize_type == CAN_RESIZE_ONLINE) {
                        ...
                        if (r < 0) {
                                ...
                                log_debug("Resizing from %s to %s worked successfully.", FORMAT_BYTES(current_fs_size), FORMAT_BYTES(try_fs_size));
                        ...
                        r = ext4_offline_resize_fs(setup, try_fs_size, user_record_luks_discard(h), user_record_mount_flags(h));
                        if (r < 0)
                                return r;
                         ...
                         * multiple programs it's a big too cumbersome to my taste. */
...
static int resize_image_loop(
        ...
        /* A bisection loop trying to find the closest size to what the user asked for. (Well, we bisect like
         * this only when we *grow* the image — if we shrin the image then there's no need to bisect.) */
        ...
        for (uint64_t lower_boundary = old_image_size, upper_boundary = new_image_size, try_image_size = new_image_size;;) {
                ...
                /* If we are growing then let's adjust our bisection boundries and try again */
                if (worked)
