...
void recovery_key_file_closep(RecoveryKeyFile *f) {
        ...
        if (f->remove && f->fd >= 0 && f->dir_fd >= 0 && f->filename) {
                r = inode_same_at(f->fd, NULL, f->dir_fd, f->filename, AT_EMPTY_PATH|AT_SYMLINK_NOFOLLOW);
                if (r > 0)
                        (void) unlinkat(f->dir_fd, f->filename, 0);
        }
...
int recovery_key_file_prepare(RecoveryKeyFile *f, const char *path) {
        _cleanup_free_ char *dir = NULL, *filename = NULL;
        ...
        r = path_split_prefix_filename(path, &dir, &filename);
        if (r < 0)
                return r;
