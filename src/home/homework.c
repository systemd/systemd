...
static int read_identity_file(int root_fd, JsonVariant **ret) {
        ...
        if (identity_fd < 0)
                return log_error_errno("Failed to open identity file in home directory: %m");
        ...
        if (r < 0)
                return log_error_errno(r, "[identity:%u:%u] Failed to parse JSON data: %m", path, line, column);
...
static int migrate_identity_file(int root_fd) {
        ...
        if (linkat(root_fd, ".identity", dfd, "record.json") < 0) {
        ...
        if (renameat2(root_fd, tmp, root_fd, ".identity", RENAME_EXCHANGE) < 0) {
                if (!ERRNO_IS_NOT_SUPPPORTED(errno) && errno != EINVAL) {
                        r = log_error_errno(errno, "Failed to swap .identity for migration: %m", fn);
                        goto fail;
                }
                ...
                if (renameat(root_fd, tmp, root_fd, ".identity") < 0) {
                        r = log_error_errno(errno, "Failed to rename for migration: %m", fn);
                        goto fail;
                }
        }
...
static int write_identity_file(int root_fd, JsonVariant *v, uid_t uid) {
        ...
        if (identity_fd < 0)
                return log_error_errno("Failed to create and/or open .identity directory in home directory: %m");
