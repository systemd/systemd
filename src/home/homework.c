...
static int home_update(UserRecord *h, Hashmap *blobs, uint64_t dbus_flags, UserRecord **ret) {
...
static int run(int argc, char *argv[]) {
        ...
        const char *json_path = NULL, *blob_filename, *dbus_flags_env;
        FILE *json_file;
        usec_t start;
        JsonVariant *fdmap, *blob_fd_variant;
        uint64_t dbus_flags = 0;
        int r;
        ...
        JSON_VARIANT_OBJECT_FOREACH(blob_filename, blob_fd_variant, fdmap) {
                ...
                assert(json_variant_integer(blob_fd_variant) <= INT_MAX);
                fd = (int) json_variant_integer(blob_fd_variant);
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to re-enable CLOEXEC on blob fd: %m");

                r = hashmap_ensure_put(&blobs, &blob_fd_hash_ops, filename, FD_TO_PTR(fd));
                if (r < 0)
                        return log_error_errno(r, "Failed to insert blob fd into map: %m");
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to strip internal homework fields from JSON: %m");
        ...
        dbus_flags_env = getenv("SYSTEMD_HOMEWORK_DBUS_FLAGS");
        if (dbus_flags_env) {
                r = safe_atou64(dbus_flags_env, &dbus_flags);
                if (r < 0)
                        return log_error_errno(r, "Failed to parse $SYSTEMD_HOMEWORK_DBUS_FLAGS: %m");
        }
        ...
        if (streq(argv[1], "activate"))
                ...
                r = home_update(home, blobs, dbus_flags, &new_home);
