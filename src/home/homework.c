...
int user_record_authenticate(
        ...
        if (cache && cache->volume_key && !FLAGS_SET(h->mask, USER_RECORD_SECRET)) {
...
static int home_deactivate(UserRecord *h, bool force) {
        ...
        if (r == USER_TEST_MOUNTED) {
                ...
                if (user_record_storage(h) == USER_LUKS) {
                         ...
                         * keys to the device. */
                        (void) password_cache_load_keyring(h, &cache);
...
static int home_update(UserRecord *h, Hashmap *blobs, uint64_t call_flags, UserRecord **ret) {
        ...
        if (!FLAGS_SET(call_flags, SD_HOMED_UPDATE_OFFLINE)) {
                r = password_cache_load_from_keyring(h, &cache);
                if (r < 0 && r != -ENOKEY)
                        return r;
