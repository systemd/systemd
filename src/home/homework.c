...
static int home_create(UserRecord *h, Hashmap *blobs, UserRecord **ret_home) {
        ...
        if (thin_pool &&
            ...
            !h->image_path) {
                _cleanup_free_ char *created_name = NULL, *created_path = NULL, *thin_name = NULL, *thin_uuid = NULL;
                sd_id128_t creation_id;
                uint64_t backing_size = UINT64_MAX, size;
                ...
                if (h->disk_size == UINT64_MAX) {
                        r = home_thin_pool_default_size(thin_pool, &size, &backing_size);
                        if (r < 0)
                                return r;

                        r = user_record_set_disk_size(h, size);
                        if (r < 0)
                                return log_error_errno(r, "Failed to record derived thin-volume size: %m");

                        log_info("Selected default thin-volume size %s from %s of currently backed capacity in %s.",
                                 FORMAT_BYTES(size), FORMAT_BYTES(backing_size), thin_pool);
                } else
                        size = h->disk_size;
                ...
                r = user_record_add_thin_pool_binding(h, thin_pool, thin_name, /* volume_uuid= */ NULL);
                if (r < 0)
                ...
                r = user_record_add_thin_pool_binding(h, thin_pool, thin_name, thin_uuid);
                if (r < 0)
                        return log_error_errno(r, "Failed to record thin-volume binding: %m");
        }
        ...
        if (setup.undo_thin_volume) {
                 ...
                 * can recover it. Only then is the LV no longer owned by the rollback cleanup. */
                assert(new_home);
