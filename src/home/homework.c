...
static int copy_skel(UserRecord *h, int root_fd, const char *skel) {
        ...
        r = chaseat(AT_FDCWD, skel, CHASE_MUST_BE_DIRECTORY, NULL, &skel_fd);
        if (r == -ENOENT) {
                log_info("Skeleton directory %s missing, ignoring.", skel);
                return 0;
        }

        r = copy_tree_at(skel_fd, NULL, root_fd, ".", h->uid, user_record_gid(h), COPY_MERGE|COPY_REPLACE, /* denylist= */ NULL, /* subvolumes= */ NULL);
        if (r < 0)
