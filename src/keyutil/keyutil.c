...
static int help(int argc, char *argv[], void *userdata) {
        ...
        printf("%1$s  [OPTIONS...] COMMAND ...\n"
               ...
               "  pkcs7                  Convert a PKCS1 signature to a PKCS7 signature\n"
               "\n%3$sOptions:%4$s\n"
               ...
               "     --signature=PATH    PKCS1 signature to convert to PKCS7\n"
...
static int verb_pkcs7(int argc, char *argv[], void *userdata) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to read PKCS1 file %s: %m", arg_signature);
        if (pkcs1_len == 0)
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "PKCS1 file %s is empty", arg_signature);
        ...
        if (PKCS7_set_type(pkcs7, NID_pkcs7_signed) == 0)
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to set PKCS7 type: %s",
                                       ERR_error_string(ERR_get_error(), NULL));

        if (PKCS7_content_new(pkcs7, NID_pkcs7_data) == 0)
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to set PKCS7 content: %s",
                                       ERR_error_string(ERR_get_error(), NULL));

        if (PKCS7_add_certificate(pkcs7, certificate) == 0)
                return log_error_errno(SYNTHETIC_ERRNO(EIO), "Failed to set PKCS7 certificate: %s",
                                       ERR_error_string(ERR_get_error(), NULL));
