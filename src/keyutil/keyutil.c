...
static int verb_extract_certificate(int argc, char *argv[], void *userdata) {
        ...
        _cleanup_(X509_freep) X509 *extracted = NULL;
        r = openssl_extract_certificate(certificate, &extracted);
        if (r < 0)
                return log_error_errno(r, "Failed to extract certificate from %s: %m", arg_certificate);

        if (PEM_write_X509(stdout, extracted) == 0)
