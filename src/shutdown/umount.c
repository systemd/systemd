...
static void log_mount_blockers(int umount_log_level, const char *mnt) {
        /* Logging proccesses that block umount could get very verbose. So only log when we are at the later
        ...
        FOREACH_DIRENT_ALL(de, dir, break) {
                ...
                const char *fd_path = procfs_file_alloca(pid, "fd");
                _cleanup_closedir_ DIR *fd_dir = opendir(fd_path);
                if (!fd_dir)
                        continue;

                FOREACH_DIRENT(fd_de, fd_dir, break) {
                        _cleanup_free_ char *fdname = NULL, *comm = NULL;

                        if (readlinkat_malloc(dirfd(fd_dir), fd_de->d_name, &fdname) < 0)
                                continue;

                        if (!path_startswith(fdname, mnt))
                                continue;

                        if (PATH_STARTSWITH_SET(fdname, "/dev", "/sys", "/proc"))
                                continue;

                        if (get_process_comm(pid, &comm) < 0)
                                continue;

                        const char *comm_pid = strjoina(comm, "(", de->d_name, ")");
                        strextend_with_separator(&blockers, ", ", comm_pid);
...
static int umount_with_timeout(MountPoint *m, int umount_log_level) {
        ...
        if (r == 0) {
                ...
                if (r == -EBUSY)
                        log_mount_blockers(umount_log_level, m->path);
