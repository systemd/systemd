...
static void init_watchdog(void) {
        ...
        if (s) {
                ...
                if (r < 0)
                        ...
                        log_warning("WATCHDOG_PID set, but not to 1, skipping watchdog logic.");
                        return;
                }
        }
...
static void sleep_until_minimum_uptime(void) {
        uint64_t minimum_uptime_usec = UINT64_MAX;
        int r;
        ...
        if (!isempty(e)) {
                ...
                else if (minimum_uptime_usec == UINT64_MAX)
                        log_warning("Ignoring invalid minimum uptime specification: %s", e);
        }

        if (minimum_uptime_usec <= 0) /* turned off? */
                return;
        if (minimum_uptime_usec == UINT64_MAX) /* pick default */
                minimum_uptime_usec = DEFAULT_MINIMUM_UPTIME_USEC;
        ...
        for (;;) {
                ...
                log_notice("Delaying shutdown for %s, in order to reach minimum uptime of %s.", FORMAT_TIMESPAN(m, USEC_PER_SEC), FORMAT_TIMESPAN(minimum_uptime_usec, USEC_PER_SEC));
