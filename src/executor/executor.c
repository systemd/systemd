...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0)
                ...
                case ARG_MANAGER_SOCKET: {
                        ...
                        fd = parse_fd(optarg);
                        if (fd < 0)
                                return log_error_errno(fd, "Failed to parse manager socket \"%s\": %m", optarg);
...
int main(int argc, char *argv[]) {
        ...
        int exit_status = EXIT_SUCCESS, r, *fds_array = NULL;
        ...
        r = exec_deserialize(serialization, fdset, fds_array, n_fds_array, &unit, &command, &params, &runtime);
        if (r < 0) {
