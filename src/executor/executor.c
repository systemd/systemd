...
int main(int argc, char *argv[]) {
        ...
        _cleanup_(exec_params_clear) ExecParameters params = {
                .stdin_fd         = -EBADF,
                .stdout_fd        = -EBADF,
                .stderr_fd        = -EBADF,
                .exec_fd          = -EBADF,
        ...
        r = exec_context_deserialize(unit_get_exec_context(unit), arg_serialization, fdset);
        if (r < 0)
                return log_error_errno(r, "Failed to deserialize ExecContext: %m");

        r = exec_cgroup_context_deserialize(unit_get_cgroup_context(unit), arg_serialization);
        if (r < 0)
