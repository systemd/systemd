...
static int apply_mount_namespace(
        ...
        if (exec_context_has_credentials(context) &&
            ...
            FLAGS_SET(params->flags, EXEC_WRITE_CREDENTIALS)) {
                creds_path = path_join(params->prefix[EXEC_DIRECTORY_RUNTIME], "credentials", params->id);
                if (!creds_path)
                        return -ENOMEM;
        }
...
static int exec_invoke(
        ...
        if (FLAGS_SET(params->flags, EXEC_WRITE_CREDENTIALS)) {
                r = setup_credentials(context, params, params->id, uid, gid, &creds_path, &creds_fd);
                if (r < 0) {
        ...
        if (needs_mount_namespace) {
                ...
                r = apply_mount_namespace(command->flags, context, params, runtime, memory_pressure_path, creds_path, creds_fd, &error_path);
                if (r < 0) {
                        *exit_status = EXIT_NAMESPACE;
        ...
        if (creds_fd >= 0) {
                ...
                if (move_mount(creds_fd, "", AT_FDCWD, creds_path, MOVE_MOUNT_F_EMPTY_PATH) < 0) {
                        *exit_status = EXIT_CREDENTIALS;
                        return log_exec_error_errno(context, params, errno, "Failed to mount credentials directory on %s: %m", creds_path);
                }
        }
        ...
        exec_log_command_line(context, params, "Executing", executable, final_argv);
...
int main(int argc, char *argv[]) {
        ...
        clearenv();
        STRV_FOREACH(e, context.manager_environment) {
                r = putenv(*e);
                if (r < 0)
                        return r;
        }
