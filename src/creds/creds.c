...
static int parse_argv(int argc, char *argv[], char ***ret_args) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION_LONG("transcode", "METHOD",
                        ...
                        if (parse_boolean(state.argument) == 0) /* If specified as "false", turn transcoding off */
                                arg_transcode = TRANSCODE_OFF;
                        else {
                                TranscodeMode m;

                                m = transcode_mode_from_string(state.argument);
                                if (m < 0)
                ...
                OPTION_LONG("newline", "auto|yes|no", "Suffix output with newline"):
                        r = parse_tristate_argument_with_auto("--newline=", state.argument, &arg_newline);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("name", "NAME",
                        ...
                        if (!credential_name_valid(state.argument))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid credential name: %s", state.argument);

                        arg_name = state.argument;
                        arg_name_any = false;
                        break;

                OPTION_LONG("timestamp", "TIME",
                            "Include specified timestamp in encrypted credential"):
                        r = parse_timestamp(state.argument, &arg_timestamp);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse timestamp: %s", state.argument);
                        break;

                OPTION_LONG("not-after", "TIME",
                            "Include specified invalidation time in encrypted credential"):
                        r = parse_timestamp(state.argument, &arg_not_after);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --not-after= timestamp: %s", state.argument);
                        break;
                ...
                OPTION_LONG("with-key", "KEY",
                        ...
                        if (isempty(state.argument))
                                ...
                                CredKeyType t = cred_key_type_from_string(state.argument);
                                if (t < 0)
                ...
                OPTION_LONG("tpm2-device", "PATH", "Pick TPM2 device"):
                        ...
                        arg_tpm2_device = streq(state.argument, "auto") ? NULL : state.argument;
                        break;

                OPTION_LONG("tpm2-pcrs", "PCR1+PCR2+PCR3+…",
                            "Specify TPM2 PCRs to seal against (fixed hash)"):
                        /* For fixed hash PCR policies only */
                        r = tpm2_parse_pcr_argument_to_mask(state.argument, &arg_tpm2_pcr_mask);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("tpm2-signature", "PATH",
                        ...
                        r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_tpm2_signature);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("uid", "UID", "Select user for scoped credentials"):
                        if (isempty(state.argument))
                                ...
                                const char *name = state.argument;
                                ...
                                if (r < 0)
                                        return log_error_errno(r, "Failed to resolve user '%s': %s",
                                                               state.argument, STRERROR_USER(r));
                        }
                        break;
