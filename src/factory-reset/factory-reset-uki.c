...
static int delete_dir(const char *root, const char *path) {
        int r;

        _cleanup_free_ char *canonical = NULL;
        struct stat st;
        r = chase_and_stat(path, root, CHASE_NOFOLLOW|CHASE_PREFIX_ROOT, &canonical, &st);
        if (r < 0)
...
static int help(void) {
        ...
        r = terminal_urlify_man("systemd-factory-reset-uki.service", "8", &link);
        if (r < 0)
                return log_oom();
...
static int run(int argc, char *argv[]) {
        ...
        FOREACH_ARRAY(entry, bc.entries, bc.n_entries) {
                ...
                r = delete_dir(NULL, dropin_dir);
                if (r == -ENOENT) {
                        log_debug("UKI %s doesn't have a drop-in dir, skipping", entry->id);
                        continue;
                }
                if (r < 0)
                        log_error_errno(r, "Failed to delete drop-in dir %s, ignoring: %m", dropin_dir);
        }
        ...
        if (found_foreign_uki) {
                log_warning("Found another installed OS that uses UKIs. Keeping global resources");
                return 0;
        }
        ...
        struct {
                ...
                { "addons", "/loader/addons" },
                { "credentials", "/loader/credentials" },
                { "extensions", "/loader/extensions" },
        };
        FOREACH_ELEMENT(resource, resources) {
                r = delete_dir(esp_path, resource->path);
                if (r == -ENOENT)
                        log_debug("No global %s dir found on ESP, skipping", resource->name);
                else if (r < 0)
                        log_error_errno(r, "Failed to delete global %s dir from ESP, ignoring: %m", resource->name);

                if (!xbootldr_path)
                        continue;

                r = delete_dir(xbootldr_path, resource->path);
                if (r == -ENOENT)
                        log_debug("No global %s dir found on XBOOTLDR, skipping", resource->name);
                else if (r < 0)
                        log_error_errno(r, "Failed to delete global %s dir from XBOOTLDR, ignoring: %m", resource->name);
        }
