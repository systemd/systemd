...
const ImagePolicy image_policy_untrusted = {
        .n_policies = 2,
        .policies = {
...
static int allowlist_mount(int userns_fd, int mount_fd) {
        ...
        r = varlink_callb(vl,
                          "io.systemd.UserRegistry.AddMountToUserRange",
                          NULL,
                          &error_id,
                          /* ret_flags= */ NULL,
                          JSON_BUILD_OBJECT(
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to call AddMountToUserRange() varlink call.");
        if (!isempty(error_id))
...
static int vl_method_mount_image(
        ...
        } else if (peer_uid != 0) /* Without userns, refuse mounting from unprivileged clients */
        ...
        for (PartitionDesignator d = 0; d < _PARTITION_DESIGNATOR_MAX; d++) {
                ...
                r = allowlist_mount(userns_fd, di->partitions[d].fsmount_fd);
                if (r < 0)
                        return r;
