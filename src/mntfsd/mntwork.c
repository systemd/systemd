...
static int determine_image_policy(
        ...
        /* An image is considered "trusted" if client is privileged, or located in trusted path */
...
static int vl_method_mount_image(
        ...
        bool client_is_privileged = false;
        uid_t peer_uid;
        int r;
        ...
        client_is_privileged = peer_uid == 0;
        ...
        for (;;) {
                ...
                r = determine_image_policy(image_fd, client_is_privileged, p.image_policy, &use_policy);
                if (r < 0)
                        return r;
                ...
                if (r == -ERFKILL) {
                        ...
                        if (!client_is_privileged) {
                                log_debug("Denied by image policy, trying to acquire polkit authentication.");
                                r = varlink_verify_polkit_async(
                                                link,
                                                /* bus= */ NULL,
                                                "io.systemd.mount-file-system.mount-image",
                                ...
                                if (r <= 0 && !ERRNO_IS_NEG_PRIVILEGE(r))
                                        return r;
                                if (r > 0) {
                                        /* Try again, now that we know the client is privileged */
                                        client_is_privileged = true;
