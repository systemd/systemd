...
static void *fake_pressure_thread(void *p) {
        struct fake_pressure_context *c = ASSERT_PTR(p);
...
TEST(fake_pressure) {
        ...
        assert_se(pthread_create(&th, NULL, fake_pressure_thread, &(struct fake_pressure_context) {
                                .fifo_fd = fifo_fd,
                                .socket_fd = socket_fd,
                        }) == 0);
...
static void real_pressure_eat_memory(int pipe_fd) {
        ...
        for (;;) {
                ...
                memset(p, random_u64() & 0xFF, MMAP_SIZE);
                ate += MMAP_SIZE;
...
TEST(real_pressure) {
        ...
        if (r < 0) {
                log_notice_errno(r, "Can't allocate memor pressure fd, skipping test: %m");
                return;
        }
