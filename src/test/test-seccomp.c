...
TEST(lock_personality) {
        ...
        if (pid == 0) {
                ...
                /* Mask the top three bytes from the value that personality() returns, since they are used
                 * for flags that emulate certain historical or architectural quirks and, most importantly,
                 * opinionated_personality() doesn't return them, causing spurious test fails on some
                 * "exotic" architectures/configurations. */
                current = safe_personality(current_opinionated);
                assert_se((current & 0xFF) == current_opinionated);
