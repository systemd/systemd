...
static int nftw_cb(
        ...
        case FTW_F:
                ...
                assert(strv_extend(&list_nftw, fpath) >= 0);
                break;

        case FTW_SL:
                log_debug("ftw found symlink %s", fpath);
                assert(strv_extendf(&list_nftw, "%s→", fpath) >= 0);
                break;
...
static int recurse_dir_callback(
        ...
        case RECURSE_DIR_ENTRY:
                assert(!IN_SET(de->d_type, DT_UNKNOWN, DT_DIR));

                log_debug("found %s", path);

                if (de->d_type == DT_LNK)
                        assert(strv_extendf(l, "%s→", path) >= 0);
                else
                        assert(strv_extend(l, path) >= 0);
                break;
...
int main(int argc, char *argv[]) {
        ...
        if (argc > 1)
                ...
                p = "/usr/share/man"; /* something hopefully reasonably stable and limited in size while we run */
        ...
        assert(nftw(p, nftw_cb, 64, FTW_PHYS|FTW_MOUNT) >= 0);

        strv_sort(list_nftw);

        for (size_t i = 0;; i++) {

                if (!streq_ptr(list_nftw[i], list_recurse_dir[i])) {
                        log_error("entry %zu different: %s vs %s", i, strna(list_nftw[i]), strna(list_recurse_dir[i]));
                        break;
                }
