...
TEST(kdf_hkdf_derive) {
        ...
        /* Exercise the salt=NULL / salt_size=0 path in kdf_hkdf_derive directly. DEFINE_HEX_PTR(NULL)
         * allocates a 1-byte buffer of size 0, so check_hkdf_derive() above does not hit it. Use the
         * same IKM/info/expected output as the third vector above: per RFC 5869 §2.2 an absent salt
         * is identical to HashLen zero bytes, which is what OpenSSL also substitutes for an
         * empty-octet-string salt, so the derived output must match byte-for-byte. */
        {
