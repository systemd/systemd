...
TEST(qmp_client_basic) {
        ...
        /* Connect then attach to event loop — handshake completes asynchronously */
...
TEST(qmp_client_eof) {
        ...
        /* Wait for async handshake — it may succeed (server sent greeting + capabilities
         * reply) or fail (server closed immediately after). Either way, after this the
         * client is in IDLE or DISCONNECTED state. */
        while (qmp_client_is_handshaking(client)) {
                r = qmp_client_process(client);
                if (r < 0)
                        break;
