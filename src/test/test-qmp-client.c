...
static _noreturn_ void mock_qmp_server_fd_first(int fd) {
        ...
        sd_json_variant *cap_id = sd_json_variant_by_key(cap_cmd, "id");
        ASSERT_NOT_NULL(cap_id);
        ASSERT_OK(sd_json_buildo(
                        &cap_reply,
        ...
        sd_json_variant *addfd_id = sd_json_variant_by_key(addfd_cmd, "id");
        ASSERT_NOT_NULL(addfd_id);
        ASSERT_OK(sd_json_buildo(
                        &addfd_return,
...
TEST(qmp_client_first_invoke_with_fd) {
        ...
        r = qmp_client_invoke(client, "add-fd",
...
TEST(qmp_client_invoke_failure_closes_fds) {
         ...
         * should have been closed by qmp_client_pending_fds_done(). fcntl on the
         * old int returns EBADF only if the slot is genuinely free. */
        ASSERT_EQ(fd_to_pass, -EBADF);
        ASSERT_EQ(fcntl(saved_fd_value, F_GETFD), -1);
        ASSERT_EQ(errno, EBADF);
}

/* Mock server: greeting → qmp_capabilities reply → accept N probes → accept query-status → close. */
static _noreturn_ void mock_qmp_server_probes(int fd, const char * const *probes, size_t n_probes) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *status_return = NULL;
        ...
        for (size_t i = 0; i < n_probes; i++)
                mock_qmp_expect_and_reply(fd, probes[i], NULL);
...
typedef struct {
        unsigned probe_cb_fired;
        unsigned probe_reply_fired;
        int probe_reply_error;
} QmpProbeTrace;

static int on_probe_reply(
                QmpClient *c,
                sd_json_variant *result,
                const char *error_desc,
                int error,
        ...
        QmpProbeTrace *tr = ASSERT_PTR(userdata);
        tr->probe_reply_fired++;
        tr->probe_reply_error = error;
        return 0;
}

static int on_probe_single(QmpClient *c, void *userdata) {
        QmpProbeTrace *tr = ASSERT_PTR(userdata);
        tr->probe_cb_fired++;
        return qmp_client_invoke(c, "probe-one", QMP_CLIENT_ARGS_PROBE(NULL), on_probe_reply, userdata);
}

TEST(qmp_client_probe_basic) {
        _cleanup_(qmp_client_unrefp) QmpClient *client = NULL;
        _cleanup_(sd_event_unrefp) sd_event *event = NULL;
        _cleanup_(pidref_done) PidRef pid = PIDREF_NULL;
        QmpProbeTrace tr = {};
        ...
        r = pidref_safe_fork("(mock-qmp-probe-basic)", FORK_DEATHSIG_SIGKILL|FORK_LOG, &pid);
        ASSERT_OK(r);
        if (r == 0) {
                safe_close(qmp_fds[0]);
                const char *probes[] = { "probe-one" };
                mock_qmp_server_probes(qmp_fds[1], probes, ELEMENTSOF(probes));
        }
        safe_close(qmp_fds[1]);

        ASSERT_OK(qmp_client_connect_fd(&client, qmp_fds[0]));
        qmp_client_bind_probe(client, on_probe_single, &tr);
        ASSERT_OK(qmp_client_attach_event(client, event, SD_EVENT_PRIORITY_NORMAL));

        /* First user invoke drives handshake + probe to completion via ensure_running(). */
        ASSERT_OK(qmp_client_invoke(client, "query-status", NULL, on_test_result, &t));
        qmp_test_wait(event, &t);
        ASSERT_EQ(t.error, 0);
        ASSERT_EQ(tr.probe_cb_fired, 1u);
        ASSERT_EQ(tr.probe_reply_fired, 1u);
        ASSERT_EQ(tr.probe_reply_error, 0);
        qmp_test_result_done(&t);
...
static int on_probe_chain_second_reply(
                QmpClient *c,
                sd_json_variant *result,
                const char *error_desc,
                int error,
...
static int on_probe_chain_first_reply(
                QmpClient *c,
                sd_json_variant *result,
                const char *error_desc,
                int error,
        ...
        QmpProbeTrace *tr = ASSERT_PTR(userdata);
        tr->probe_reply_fired++;
        return qmp_client_invoke(c, "probe-two", QMP_CLIENT_ARGS_PROBE(NULL), on_probe_chain_second_reply, tr);
}

static int on_probe_chain_first(QmpClient *c, void *userdata) {
        QmpProbeTrace *tr = ASSERT_PTR(userdata);
        tr->probe_cb_fired++;
        return qmp_client_invoke(c, "probe-one", QMP_CLIENT_ARGS_PROBE(NULL), on_probe_chain_first_reply, tr);
}

TEST(qmp_client_probe_chained) {
        _cleanup_(qmp_client_unrefp) QmpClient *client = NULL;
        _cleanup_(sd_event_unrefp) sd_event *event = NULL;
        _cleanup_(pidref_done) PidRef pid = PIDREF_NULL;
        QmpProbeTrace tr = {};
        ...
        r = pidref_safe_fork("(mock-qmp-probe-chain)", FORK_DEATHSIG_SIGKILL|FORK_LOG, &pid);
        ASSERT_OK(r);
        if (r == 0) {
                safe_close(qmp_fds[0]);
                const char *probes[] = { "probe-one", "probe-two" };
                mock_qmp_server_probes(qmp_fds[1], probes, ELEMENTSOF(probes));
        }
        safe_close(qmp_fds[1]);

        ASSERT_OK(qmp_client_connect_fd(&client, qmp_fds[0]));
        qmp_client_bind_probe(client, on_probe_chain_first, &tr);
        ASSERT_OK(qmp_client_attach_event(client, event, SD_EVENT_PRIORITY_NORMAL));
        ...
        ASSERT_EQ(t.error, 0);
        ASSERT_EQ(tr.probe_cb_fired, 1u);
        ASSERT_EQ(tr.probe_reply_fired, 2u);
        qmp_test_result_done(&t);
...
static int on_probe_returns_error(QmpClient *c, void *userdata) {
        unsigned *fired = ASSERT_PTR(userdata);
        (*fired)++;
        return -EINVAL;
}

TEST(qmp_client_probe_callback_fails) {
        _cleanup_(qmp_client_unrefp) QmpClient *client = NULL;
        _cleanup_(sd_event_unrefp) sd_event *event = NULL;
        _cleanup_(pidref_done) PidRef pid = PIDREF_NULL;
        QmpTestResult t = {};
        ...
        r = pidref_safe_fork("(mock-qmp-probe-fail)", FORK_DEATHSIG_SIGKILL|FORK_LOG, &pid);
        ASSERT_OK(r);
        if (r == 0) {
                safe_close(qmp_fds[0]);
                /* No probes expected — we disconnect at the probe callback. */
                mock_qmp_server_probes(qmp_fds[1], NULL, 0);
        }
        safe_close(qmp_fds[1]);

        ASSERT_OK(qmp_client_connect_fd(&client, qmp_fds[0]));
        qmp_client_bind_probe(client, on_probe_returns_error, &fired);
        ASSERT_OK(qmp_client_attach_event(client, event, SD_EVENT_PRIORITY_NORMAL));

        /* User invoke should fail because the handshake aborts on the probe callback error. */
        r = qmp_client_invoke(client, "query-status", NULL, on_test_result, &t);
        if (r >= 0) {
                qmp_test_wait(event, &t);
                ASSERT_TRUE(t.error < 0);
                qmp_test_result_done(&t);
        } else
                ASSERT_TRUE(r < 0);
        ASSERT_EQ(fired, 1u);

        /* Subsequent invoke hits -ENOTCONN because the client is DISCONNECTED. */
        QmpTestResult t2 = {};
        r = qmp_client_invoke(client, "query-status", NULL, on_test_result, &t2);
        ASSERT_EQ(r, -ENOTCONN);

        /* Release the client fd so the mock child's read() returns EOF and it exits. */
...
static int on_probe_misuses_invoke(QmpClient *c, void *userdata) {
        int *observed_r = ASSERT_PTR(userdata);
        /* Wrongly use qmp_client_invoke() instead of qmp_client_probe_invoke(). Must not
         * recurse into ensure_running(); returns -EBUSY via dispatch_depth guard. */
        *observed_r = qmp_client_invoke(c, "wrong-call", NULL, on_probe_reply, NULL);
        return *observed_r;
}

TEST(qmp_client_probe_invoke_from_probe_rejected) {
        _cleanup_(qmp_client_unrefp) QmpClient *client = NULL;
        _cleanup_(sd_event_unrefp) sd_event *event = NULL;
        _cleanup_(pidref_done) PidRef pid = PIDREF_NULL;
        QmpTestResult t = {};
        ...
        r = pidref_safe_fork("(mock-qmp-probe-recurse)", FORK_DEATHSIG_SIGKILL|FORK_LOG, &pid);
        ASSERT_OK(r);
        if (r == 0) {
                safe_close(qmp_fds[0]);
                mock_qmp_server_probes(qmp_fds[1], NULL, 0);
        ...
        ASSERT_OK(qmp_client_connect_fd(&client, qmp_fds[0]));
        qmp_client_bind_probe(client, on_probe_misuses_invoke, &observed_r);
        ASSERT_OK(qmp_client_attach_event(client, event, SD_EVENT_PRIORITY_NORMAL));
        ...
        /* The inner invoke must have been rejected with -EBUSY, not recursed. */
        ASSERT_EQ(observed_r, -EBUSY);
...
TEST(qmp_client_probe_invoke_outside_probing) {
        _cleanup_(qmp_client_unrefp) QmpClient *client = NULL;
        _cleanup_(sd_event_unrefp) sd_event *event = NULL;
        _cleanup_(pidref_done) PidRef pid = PIDREF_NULL;
        QmpTestResult t = {};
        ...
        r = pidref_safe_fork("(mock-qmp-probe-outside)", FORK_DEATHSIG_SIGKILL|FORK_LOG, &pid);
        ASSERT_OK(r);
        if (r == 0) {
                safe_close(qmp_fds[0]);
                mock_qmp_server_probes(qmp_fds[1], NULL, 0);
        ...
        /* No probe registered: client transitions straight to RUNNING at handshake end. */
        ASSERT_OK(qmp_client_invoke(client, "query-status", NULL, on_test_result, &t));
        qmp_test_wait(event, &t);
        ASSERT_EQ(t.error, 0);
        qmp_test_result_done(&t);

        /* probe after RUNNING is a user error and must be rejected. */
        r = qmp_client_invoke(client, "too-late", QMP_CLIENT_ARGS_PROBE(NULL), on_probe_reply, NULL);
        ASSERT_EQ(r, -EBUSY);
