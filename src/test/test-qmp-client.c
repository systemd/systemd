...
static _noreturn_ void mock_qmp_server(int fd) {
        ...
        /* Accept io_uring probe (blockdev-add + blockdev-del) from handshake */
        mock_qmp_expect_and_reply(fd, "blockdev-add", NULL);
        mock_qmp_expect_and_reply(fd, "blockdev-del", NULL);
...
TEST(qmp_client_basic) {
        ...
        /* Connect — attaches to event loop and performs handshake via sd_event_run() */
