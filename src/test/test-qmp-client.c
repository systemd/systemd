...
static void mock_qmp_server(int fd) {
...
TEST(qmp_client_basic) {
        ...
        r = qmp_client_execute(client, "query-status", NULL, &result, &error_class);
        assert_se(r >= 0);
        assert_se(result);
        ...
        /* Execute stop */
        r = qmp_client_execute(client, "stop", NULL, &result, &error_class);
        assert_se(r >= 0);

        result = sd_json_variant_unref(result);

        /* Execute cont -- the STOP event should be dispatched inline */
        r = qmp_client_execute(client, "cont", NULL, &result, &error_class);
        assert_se(r >= 0);
...
TEST(qmp_client_eof) {
        ...
        r = qmp_client_execute(client, "query-status", NULL, NULL, NULL);
