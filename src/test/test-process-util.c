...
static void test_get_process_cmdline_one(pid_t pid) {
        ...
        r = get_process_cmdline(pid, SIZE_MAX, PROCESS_CMDLINE_QUOTE_POSIX, &e);
        log_info("      %s", r >= 0 ? e : errno_to_name(r));

        r = get_process_cmdline(pid, SIZE_MAX, PROCESS_CMDLINE_QUOTE_POSIX | PROCESS_CMDLINE_COMM_FALLBACK, &f);
        log_info("      %s", r >= 0 ? f : errno_to_name(r));
}
