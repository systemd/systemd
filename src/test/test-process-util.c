...
static void test_get_process_cmdline_harder(void) {
        ...
        assert_se(get_process_cmdline(0, SIZE_MAX-1, PROCESS_CMDLINE_COMM_FALLBACK, &line) >= 0);
        log_info("'%s'", line);
...
#define EXPECT1p  "foo $'\\'bar\\'' $'\"bar$\"' $'x y z' $'!``' ''"
        ...
        assert_se(get_process_cmdline(0, SIZE_MAX, PROCESS_CMDLINE_SHELL_QUOTE_POSIX, &line) >= 0);
        log_info("got: ==%s==", line);
        log_info("exp: ==%s==", EXPECT1p);
