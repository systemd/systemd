...
TEST(clone_hack) {
        ...
        /* FORK_NEW_MOUNTNS forces safe_fork() to go through clone_hack() */
        r = safe_fork("(test-nschild)", FORK_NEW_MOUNTNS|FORK_RESET_SIGNALS|FORK_CLOSE_ALL_FDS|FORK_DEATHSIG|FORK_REARRANGE_STDIO|FORK_REOPEN_LOG, &pid);
        assert_se(r >= 0);
        if (r == 0) {
                /* child */
