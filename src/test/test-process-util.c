...
TEST(pid_get_cmdline_harder) {
        ...
        r = ASSERT_OK(pidref_safe_fork("(cmdline)", FORK_WAIT|FORK_LOG|FORK_DEATHSIG_SIGKILL, NULL));
        if (r == 0) {
...
TEST(getpid_cached) {
        ...
        r = ASSERT_OK(pidref_safe_fork("(getpid)", FORK_WAIT|FORK_LOG|FORK_DEATHSIG_SIGKILL, NULL));
