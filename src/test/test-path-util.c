...
static void test_path_extract_filename_one(const char *input, const char *output, int ret) {
        ...
        /* Extra safety check: make sure that path_extract_directory_and_filename() behaves the same */
        r = path_extract_directory_and_filename(input, NULL, &k2);
        if (r >= 0) {
                ASSERT_STREQ(k2, k);
                assert_se(r == ret);
        }
...
static void test_path_extract_directory_one(const char *input, const char *output, int ret) {
         ...
         * path_extract_directory_and_filename() must also fail. */
        r = path_extract_directory_and_filename(input, &k2, NULL);
        ASSERT_STREQ(k2, k);
        assert_se(!(ret < 0) || r < 0);
        ...
        if (r >= 0) {
                ...
                /* And the same, but for path_extract_directory_and_filename() */
                r = path_extract_directory_and_filename(input, &k3, &f2);
                if (r >= 0) {
                        ASSERT_STREQ(k3, k);
                        ASSERT_STREQ(f2, f);
                }
