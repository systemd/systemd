...
static void test_find_executable_full(void) {
        ...
        char *test_file_name;
        int fd;
        _cleanup_strv_free_ char **exec_search_paths = strv_new("/doesnotexist", "/tmp", "/bin");
        ...
        assert_se(fd = mkostemp_safe(fn));
        (void) fchmod(fd, 0755);

        test_file_name = strchr(fn, delim);

        assert_se(find_executable_full(test_file_name, NULL, exec_search_paths, false, &p, NULL) == 0);
        puts(p);
        assert_se(streq(p, fn));
        free(p);

        unlink(fn);
        assert_se(find_executable_full(test_file_name, NULL, exec_search_paths, false, &p, NULL) == -ENOENT);
}
