...
static void test_find_executable_full(void) {
        ...
        assert_se(find_executable_full(test_file_name, NULL, "/doesnotexist:/tmp:/bin", false, &p, NULL) == 0);
        puts(p);
        assert_se(streq(p, fn));
        free(p);

        (void) unlink(fn);
        assert_se(find_executable_full(test_file_name, NULL, "/doesnotexist:/tmp:/bin", false, &p, NULL) == -ENOENT);
}
