...
TEST(path_pick) {
        ...
        dfd = ASSERT_OK(mkdtemp_open(/* template= */ NULL, O_DIRECTORY|O_CLOEXEC, &p));
        ...
        if (IN_SET(native_architecture(), ARCHITECTURE_X86, ARCHITECTURE_X86_64)) {
                ASSERT_OK_POSITIVE(path_pick(/* root_path= */ NULL, AT_FDCWD, AT_FDCWD, pp, &filter, /* n_filters= */ 1, PICK_ARCHITECTURE|PICK_TRIES, &result));
        ...
        ASSERT_OK_ZERO(path_pick(/* root_path= */ NULL, AT_FDCWD, AT_FDCWD, pp, &filter, /* n_filters= */ 1, PICK_ARCHITECTURE|PICK_TRIES, &result));
...
TEST(pick_filter_image_any) {
        ...
        _cleanup_close_ int dfd = ASSERT_OK(mkdtemp_open(/* template= */ NULL, O_DIRECTORY|O_CLOEXEC, &p));
        ...
        ASSERT_OK_POSITIVE(path_pick(/* root_path= */ NULL, AT_FDCWD, AT_FDCWD, pp, pick_filter_image_any, ELEMENTSOF(pick_filter_image_any), PICK_ARCHITECTURE, &result));
        ...
        ASSERT_OK_POSITIVE(path_pick(/* root_path= */ NULL, AT_FDCWD, AT_FDCWD, pp, pick_filter_image_raw, ELEMENTSOF(pick_filter_image_raw), PICK_ARCHITECTURE, &result));
        ASSERT_TRUE(S_ISREG(result.st.st_mode));
        ASSERT_STREQ(result.version, "3");
        ASSERT_TRUE(endswith(result.path, "/test_3.raw"));
        pick_result_done(&result);
        ...
        ASSERT_OK_ZERO(path_pick(/* root_path= */ NULL, AT_FDCWD, AT_FDCWD, pp, pick_filter_image_any, ELEMENTSOF(pick_filter_image_any), PICK_ARCHITECTURE, &result));
        ...
        ASSERT_OK_POSITIVE(path_pick(/* root_path= */ NULL, AT_FDCWD, AT_FDCWD, pp, pick_filter_image_dir, ELEMENTSOF(pick_filter_image_dir), PICK_ARCHITECTURE, &result));
        ASSERT_TRUE(S_ISDIR(result.st.st_mode));
        ASSERT_STREQ(result.version, "2");
        ...
        ASSERT_OK_ZERO(path_pick(/* root_path= */ NULL, AT_FDCWD, AT_FDCWD, pp, pick_filter_image_dir, ELEMENTSOF(pick_filter_image_dir), PICK_ARCHITECTURE, &result));
}
...
TEST(path_pick_resolve) {
        ...
        ASSERT_OK_POSITIVE(path_pick(/* root_path= */ NULL, AT_FDCWD, AT_FDCWD, pp, pick_filter_image_any, ELEMENTSOF(pick_filter_image_any), PICK_ARCHITECTURE|PICK_RESOLVE, &result));
        ASSERT_STREQ(result.version, "2");
        ...
        ASSERT_OK_POSITIVE(path_pick(/* root_path= */ NULL, AT_FDCWD, AT_FDCWD, pp, pick_filter_image_dir, ELEMENTSOF(pick_filter_image_dir), PICK_ARCHITECTURE|PICK_RESOLVE, &result));
        ASSERT_TRUE(S_ISDIR(result.st.st_mode));
        ASSERT_STREQ(result.version, "1");
        ...
        ASSERT_OK_POSITIVE(path_pick(/* root_path= */ NULL, AT_FDCWD, AT_FDCWD, pp, pick_filter_image_raw, ELEMENTSOF(pick_filter_image_raw), PICK_ARCHITECTURE|PICK_RESOLVE, &result));
        ASSERT_STREQ(result.version, "3");
...
TEST(pick_result_compare) {
        ...
        a.path = (char*) "/a";
        b.path = (char*) "/b";
        ASSERT_LT(pick_result_compare(&a, &b, /* flags= */ 0), 0);
        ASSERT_GT(pick_result_compare(&b, &a, /* flags= */ 0), 0);
        ASSERT_EQ(pick_result_compare(&a, &a, /* flags= */ 0), 0);
        ...
        a.tries_left = 0;
        b.tries_left = 1;
        ASSERT_LT(pick_result_compare(&a, &b, /* flags= */ 0), 0); /* Without PICK_TRIES, paths are compared */
        ...
        if (native_architecture() != ARCHITECTURE_ALPHA) {
                ASSERT_LT(pick_result_compare(&a, &b, /* flags= */ 0), 0); /* Without PICK_ARCHITECTURE, paths are compared */
