...
static void *collect_thread(void *arg) {
        _cleanup_(varlink_flush_close_unrefp) Varlink *c = NULL;
        _cleanup_(json_variant_unrefp) JsonVariant *i = NULL, *o = NULL;
        JsonVariant *k = NULL;
        const char *error_id;
        ...
        assert_se(varlink_collect(c, "io.test.DoSomethingMore", i, &o, &error_id, &flags) >= 0);

        assert_se(!error_id);
        assert_se(!flags);
        assert_se(json_variant_is_array(o) && !json_variant_is_blank_array(o));
