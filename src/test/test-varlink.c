...
static void *upgrade_pipelining_thread(void *arg) {
        ...
        assert_se(fd >= 0);
        int r = sockaddr_un_set_path(&sa.un, arg);
        assert_se(r >= 0);
        assert_se(connect(fd, &sa.sa, offsetof(struct sockaddr_un, sun_path) + strlen(sa.un.sun_path) + 1) >= 0);
        ...
        /* Read the server's upgrade reply (JSON + \0) */
        char reply_buf[256] = {};
        ssize_t n;
        ASSERT_OK_ERRNO(n = recv(fd, reply_buf, sizeof(reply_buf) - 1, 0));
        ASSERT_GT(n, 0);

        /* After the reply \0, the rest is the reversed raw payload */
        char *delim = memchr(reply_buf, 0, n);
        ASSERT_NOT_NULL(delim);

        size_t raw_offset = (size_t)(delim - reply_buf) + 1;
        ASSERT_GE((size_t) n, raw_offset + strlen(raw_payload));
        ASSERT_STREQ(strndupa_safe(reply_buf + raw_offset, strlen(raw_payload)), "!denilepiP");
