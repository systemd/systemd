...
static int method_something(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (!a) {
                ASSERT_ERROR(r = sd_varlink_error(link, "io.test.BadParameters", NULL), EBADR);
                return r;
        }
...
static int method_passfd(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (!a) {
                ASSERT_ERROR(r = sd_varlink_error_invalid_parameter_name(link, "fd"), EINVAL);
                return r;
        }
        ...
        ASSERT_OK_ZERO(sd_varlink_push_fd(link, vv));
...
static void *thread(void *arg) {
        ...
        ASSERT_TRUE(sd_json_variant_is_array(j) && !sd_json_variant_is_blank_array(j));
        ...
        ASSERT_OK_ZERO(sd_varlink_push_fd(c, fd1));
...
TEST(chat) {
        ...
        ASSERT_OK_ZERO_ERRNO(pthread_create(&t, NULL, thread, (void*) sp));
