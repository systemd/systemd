...
static int method_passfd(Varlink *link, JsonVariant *parameters, VarlinkMethodFlags flags, void *userdata) {
        ...
        assert_se(varlink_push_fd(link, ww) >= 0);
...
static void *thread(void *arg) {
        ...
        _cleanup_close_ int fd1 = acquire_data_fd("foo", 3, 0);
        _cleanup_close_ int fd2 = acquire_data_fd("bar", 3, 0);
        _cleanup_close_ int fd3 = acquire_data_fd("quux", 4, 0);
        ...
        assert_se(varlink_push_fd(c, fd2) >= 0);
        assert_se(varlink_push_fd(c, fd3) >= 0);
        ...
        assert_se(varlink_drop_fd(c, 2) >= 0);
