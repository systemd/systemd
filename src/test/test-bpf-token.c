...
static int intro(void) {
        ...
        if (fs_fd < 0)
                return log_tests_skipped_errno(errno, "Failed to fsopen bpffs");
        ...
        FOREACH_STRING(s, "delegate_cmds", "delegate_maps", "delegate_progs", "delegate_attachs") {
                ...
                if (r < 0)
                        return log_tests_skipped_errno(errno, "Failed to FSCONFIG_SET_STRING (%s)", s);
        }

        r = fsconfig(fs_fd, FSCONFIG_CMD_CREATE, /* key = */ NULL, /* value = */ NULL, /* aux = */ 0);
        if (r < 0)
                return log_tests_skipped_errno(errno, "Failed to FSCONFIG_CMD_CREATE");
