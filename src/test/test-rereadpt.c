...
static void sfdisk(LoopDevice *loop, const char *definition) {
        int r;
        ...
        if (r == 0) {
                /* child */
                execlp("sfdisk", "fdisk", "--no-tell-kernel", "--no-reread", loop->node, NULL);
                _exit(EXIT_FAILURE);
        }
...
TEST(rereadpt) {
        ...
        /* No change */
        ASSERT_OK(rereadpt_fd(loop->fd, /* flags= */ 0));
        ASSERT_ERROR_ERRNO(access(p, F_OK), ENOENT);
        ...
        ASSERT_OK(rereadpt_fd(loop->fd, /* flags= */ REREADPT_FORCE_UEVENT));
        ASSERT_OK_ZERO_ERRNO(access(p, F_OK));
        ...
        ASSERT_OK_ZERO_ERRNO(access(p, F_OK));
        ASSERT_ERROR(rereadpt_fd(loop->fd, /* flags= */ 0), EBUSY);
        ASSERT_OK_ZERO_ERRNO(access(p, F_OK));
