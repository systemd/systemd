...
TEST(RET_GATHER) {
        ...
        ASSERT_TRUE(RET_GATHER(x, 5) == 0);
        ASSERT_TRUE(RET_GATHER(x, -5) == -5);
        ASSERT_TRUE(RET_GATHER(x, -1) == -5);

        ASSERT_TRUE(RET_GATHER(x, y++) == -5);
        ASSERT_TRUE(y == 3);
}

TEST(ERRNO_IS_TRANSIENT) {
        ASSERT_TRUE(ERRNO_IS_NEG_TRANSIENT(-EINTR));
        ASSERT_OK_POSITIVE(!ERRNO_IS_NEG_TRANSIENT(EINTR));
        ...
        ASSERT_OK_POSITIVE(!ERRNO_IS_NEG_TRANSIENT(r));

        ASSERT_OK_POSITIVE(!ERRNO_IS_NEG_TRANSIENT(INT_MAX));
        ASSERT_OK_POSITIVE(!ERRNO_IS_NEG_TRANSIENT(INT_MIN));
        ASSERT_OK_POSITIVE(!ERRNO_IS_NEG_TRANSIENT(INTMAX_MAX));
        ASSERT_OK_POSITIVE(!ERRNO_IS_NEG_TRANSIENT(INTMAX_MIN));
}
