...
TEST(test_order) {
        ...
        assert_se(test_order(5, COMPARE_STRING_EQUAL) == -EINVAL); /* not an order operator */
}

TEST(version_or_fnmatch_compare) {
        CompareOperator OTHERS;
        ...
        assert_se(version_or_fnmatch_compare(OTHERS, "local512", "local256") == -EINVAL);
}
