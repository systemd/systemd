...
#define _THIS_IS_CONSTANT(x) ({ x; })

TEST_RET(bits_constant) {
        /* Verify constant folding works (unless optimization is disabled). */
        if (!__builtin_constant_p(_THIS_IS_CONSTANT(1)))
                return log_tests_skipped("Optimisation is disabled, can't test constant folding.");

        assert(__builtin_constant_p(INDEX_TO_MASK(uint8_t, 0) == 1));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint8_t, 4) == 0x10));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint8_t, 7) == 0x80));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint16_t, 0) == 1));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint16_t, 10) == 0x400));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint16_t, 15) == 0x8000));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint32_t, 0) == 1));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint32_t, 21) == 0x200000));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint32_t, 31) == 0x80000000));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint64_t, 0) == 1));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint64_t, 56) == 0x100000000000000));
        assert(__builtin_constant_p(INDEX_TO_MASK(uint64_t, 63) == 0x8000000000000000));
