...
static void test_create_symlinks_from_tuples(void) {
        _cleanup_free_ char *result = NULL;
        ...
        assert_se(mkdtemp(temp));
        assert_se(touch_file(strjoina(temp, "/foo/bar/bar"), true, USEC_INFINITY, UID_INVALID, GID_INVALID, 0644) >= 0);
        assert_se(touch_file(strjoina(temp, "/foobar"), true, USEC_INFINITY, UID_INVALID, GID_INVALID, 0644) >= 0);

        assert_se(create_symlinks_from_tuples(temp, STRV_MAKE("/foo/bar/bar", "/foo/bar/baz", "foobar", "foobaz"), mkdir_parents) == 0);

        assert_se(chase_symlinks(strjoina(temp, "/foo/bar/baz"), temp, 0, &result, NULL) > 0);
        assert_se(path_equal(result, strjoina(temp, "/foo/bar/bar")));
        result = mfree(result);
        assert_se(chase_symlinks(strjoina(temp, "/foobaz"), temp, 0, &result, NULL) > 0);
        assert_se(path_equal(result, strjoina(temp, "/foobar")));
...
static void test_create_many_symlinks(void) {
        ...
        assert_se(create_many_symlinks(temp, "/foo/bar/bar", STRV_MAKE("/foo/baz/baz", "foobar", "foobaz"), mkdir_parents) == 0);

        assert_se(chase_symlinks(strjoina(temp, "/foo/baz/baz"), temp, 0, &result, NULL) > 0);
        assert_se(path_equal(result, strjoina(temp, "/foo/bar/bar")));
        result = mfree(result);
