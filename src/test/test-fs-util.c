...
TEST(xopenat_auto_rw_ro) {
        ...
        assert_se(path_fd >= 0);
        fd = xopenat_full(path_fd, "", O_RDONLY, XO_AUTO_RW_RO, 0);
        assert_se(fd >= 0);
        ASSERT_OK_ERRNO(fl = fcntl(fd, F_GETFL));
        assert_se((fl & O_ACCMODE) == O_RDWR);
        fd = safe_close(fd);
        ...
        if (have_effective_cap(CAP_DAC_OVERRIDE) <= 0) {
                ...
                fd = xopenat_full(tfd, "ro", O_RDONLY, XO_AUTO_RW_RO, 0);
                assert_se(fd >= 0);
                ASSERT_OK_ERRNO(fl = fcntl(fd, F_GETFL));
                assert_se((fl & O_ACCMODE) == O_RDONLY);
                fd = safe_close(fd);
                ...
                fd = xopenat_full(tfd, "ro", O_RDONLY, XO_AUTO_RW_RO|XO_REGULAR, 0);
                assert_se(fd >= 0);
                ASSERT_OK_ERRNO(fl = fcntl(fd, F_GETFL));
                assert_se((fl & O_ACCMODE) == O_RDONLY);
                fd = safe_close(fd);
                ...
                fd = xopenat_full(ro_path_fd, "", O_RDONLY, XO_AUTO_RW_RO, 0);
                assert_se(fd >= 0);
                ASSERT_OK_ERRNO(fl = fcntl(fd, F_GETFL));
                assert_se((fl & O_ACCMODE) == O_RDONLY);
                fd = safe_close(fd);
