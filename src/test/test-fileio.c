...
TEST(search_and_fopen) {
        ...
        r = path_extract_filename(name, &bn);
        assert_se(r >= 0);
        r = search_and_fopen(bn, "re", NULL, (const char**) dirs, &f, &p);
        assert_se(r >= 0);
        ...
        r = search_and_fopen(bn, NULL, NULL, (const char**) dirs, NULL, &p);
        assert_se(r >= 0);
        assert_se(e = path_startswith(p, "/tmp/"));
        ASSERT_STREQ(bn, e);
        p = mfree(p);
        ...
        r = search_and_fopen(bn, "re", "/", (const char**) dirs, &f, &p);
        assert_se(r >= 0);
        assert_se(e = path_startswith(p, "/tmp/"));
        ASSERT_STREQ(bn, e);
        f = safe_fclose(f);
        ...
        r = search_and_fopen(bn, NULL, "/", (const char**) dirs, NULL, &p);
        assert_se(r >= 0);
        assert_se(e = path_startswith(p, "/tmp/"));
        ASSERT_STREQ(bn, e);
        p = mfree(p);
...
TEST(search_and_fopen_nulstr) {
        ...
        r = path_extract_filename(name, &bn);
        r = search_and_fopen_nulstr(bn, "re", NULL, dirs, &f, &p);
        assert_se(r >= 0);
        assert_se(e = path_startswith(p, "/tmp/"));
        ASSERT_STREQ(bn, e);
        f = safe_fclose(f);
