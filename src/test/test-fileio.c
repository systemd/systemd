...
static void test_status_field(void) {
        ...
        assert_se(get_proc_field("/proc/self/status", "Threads", NULL, &t) == 0);
        puts(t);
        assert_se(streq(t, "1"));

        r = get_proc_field("/proc/meminfo", "MemTotal", NULL, &p);
        if (r != -ENOENT) {
                assert_se(r == 0);
                puts(p);
        ...
        r = get_proc_field("/proc/meminfo", "Buffers", NULL, &s);
        if (r != -ENOENT) {
                assert_se(r == 0);
                puts(s);
        ...
        r = get_proc_field("/proc/meminfo", "Seccomp", NULL, &z);
        if (r != -ENOENT) {
                assert_se(r == 0);
                puts(z);
