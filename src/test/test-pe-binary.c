...
typedef struct SectionSpec {
        char name[8];           /* zero-padded; must fit in 8 bytes */
...
static int build_pe_file(
        ...
        for (size_t i = 0; i < n_sections; i++)
                if (specs[i].size_of_raw_data > 0) {
                        uint64_t end = (uint64_t) specs[i].pointer_to_raw_data + specs[i].size_of_raw_data;
                        if (end > needed)
                                needed = end;
                }

        size_t file_size = override_file_size > 0 ? override_file_size : needed;
        _cleanup_free_ uint8_t *buf = calloc(1, file_size > base ? file_size : base);
        assert_se(buf);

        memcpy(buf, HEADER_SKELETON, sizeof(HEADER_SKELETON));
        write_le16(buf + IMAGE_FILE_HEADER_NOS_OFFSET, (uint16_t) n_sections);

        /* Section table at 0x148. */
        for (size_t i = 0; i < n_sections; i++) {
                uint8_t *sh = buf + SECTION_TABLE_OFFSET + i * IMAGE_SECTION_HEADER_BYTES;
                memset(sh, 0, IMAGE_SECTION_HEADER_BYTES);
                size_t nlen = strnlen(specs[i].name, 8);
                memcpy(sh, specs[i].name, nlen);
                write_le32(sh + 8,  specs[i].virtual_size);
                write_le32(sh + 12, specs[i].virtual_address);
                write_le32(sh + 16, specs[i].size_of_raw_data);
                write_le32(sh + 20, specs[i].pointer_to_raw_data);
                /* Characteristics, relocs, etc., left zero. */
        }

        int fd = memfd_new("test-pe-binary");
        assert_se(fd >= 0);

        /* Write only file_size bytes — that lets the caller construct a
         * PE whose section table claims data past EOF. */
        assert_se(write(fd, buf, file_size) == (ssize_t) file_size);
...
TEST(pe_load_sections_valid_in_bounds) {
        ...
        _cleanup_close_ int fd = build_pe_file(&s, 1, /* override = */ 4096);
        _cleanup_free_ IMAGE_DOS_HEADER *dos = NULL;
        _cleanup_free_ PeHeader *pe = NULL;
        _cleanup_free_ IMAGE_SECTION_HEADER *sections = NULL;

        assert_se(load_headers_and_sections(fd, &dos, &pe, &sections) >= 0);
}
...
TEST(pe_load_sections_section_exactly_fills_file) {
        ...
        _cleanup_close_ int fd = build_pe_file(&s, 1, exact_size);
        _cleanup_free_ IMAGE_DOS_HEADER *dos = NULL;
        _cleanup_free_ PeHeader *pe = NULL;
        _cleanup_free_ IMAGE_SECTION_HEADER *sections = NULL;
...
TEST(pe_load_sections_section_past_eof_by_one) {
        ...
        _cleanup_close_ int fd = build_pe_file(&s, 1, short_size);
        _cleanup_free_ IMAGE_DOS_HEADER *dos = NULL;
        _cleanup_free_ PeHeader *pe = NULL;
        _cleanup_free_ IMAGE_SECTION_HEADER *sections = NULL;
...
TEST(pe_load_sections_pointer_way_past_eof) {
        ...
        assert_se(load_headers_and_sections(fd, &dos, &pe, &sections) == -EBADMSG);
}
...
TEST(pe_load_sections_size_zero_with_huge_pointer) {
        ...
        _cleanup_close_ int fd = build_pe_file(&s, 1, /* override = */ 0);
        _cleanup_free_ IMAGE_DOS_HEADER *dos = NULL;
        _cleanup_free_ PeHeader *pe = NULL;
        _cleanup_free_ IMAGE_SECTION_HEADER *sections = NULL;
...
TEST(pe_load_sections_zero_sections) {
        _cleanup_close_ int fd = build_pe_file(NULL, 0, /* override = */ 0);
        _cleanup_free_ IMAGE_DOS_HEADER *dos = NULL;
        _cleanup_free_ PeHeader *pe = NULL;
        _cleanup_free_ IMAGE_SECTION_HEADER *sections = NULL;
...
TEST(pe_load_sections_multi_all_valid) {
        ...
        _cleanup_close_ int fd = build_pe_file(specs, 3, /* override = */ 0);
        _cleanup_free_ IMAGE_DOS_HEADER *dos = NULL;
        _cleanup_free_ PeHeader *pe = NULL;
        _cleanup_free_ IMAGE_SECTION_HEADER *sections = NULL;
...
TEST(pe_load_sections_multi_one_past_eof) {
        ...
        _cleanup_close_ int fd = build_pe_file(specs, 3, /* override = */ 4096);
        _cleanup_free_ IMAGE_DOS_HEADER *dos = NULL;
        _cleanup_free_ PeHeader *pe = NULL;
        _cleanup_free_ IMAGE_SECTION_HEADER *sections = NULL;
...
static int call_uki_hash_with(SectionSpec spec) {
        _cleanup_close_ int fd = build_pe_file(&spec, 1, /* override = */ 0);
        _cleanup_free_ IMAGE_DOS_HEADER *dos = NULL;
        _cleanup_free_ PeHeader *pe = NULL;
        _cleanup_free_ IMAGE_SECTION_HEADER *sections = NULL;
...
TEST_RET(uki_hash_no_padding) {
        ...
        assert_se(call_uki_hash_with(s) >= 0);
        return EXIT_SUCCESS;
}
 ...
 * is ~100–200 ms on a modern CPU; comfortably within test timeouts. */
...
TEST_RET(uki_hash_one_over_cap_rejected) {
        ...
        assert_se(call_uki_hash_with(s) == -EBADMSG);
        return EXIT_SUCCESS;
}
...
TEST_RET(uki_hash_one_bad_among_many_rejected) {
        ...
        _cleanup_close_ int fd = build_pe_file(specs, 2, /* override = */ 0);
        _cleanup_free_ IMAGE_DOS_HEADER *dos = NULL;
        _cleanup_free_ PeHeader *pe = NULL;
        _cleanup_free_ IMAGE_SECTION_HEADER *sections = NULL;
        ...
        void *hashes[_UNIFIED_SECTION_MAX] = {};
        size_t hash_size = 0;
        int r = uki_hash(fd, sym_EVP_sha256(), hashes, &hash_size);
        for (size_t i = 0; i < _UNIFIED_SECTION_MAX; i++)
                free(hashes[i]);
        assert_se(r == -EBADMSG);
        return EXIT_SUCCESS;
...
TEST_RET(uki_hash_empty_pe_does_not_reject) {
        ...
        for (size_t i = 0; i < _UNIFIED_SECTION_MAX; i++)
                free(hashes[i]);
        /* Should succeed (no sections to hash) and not return -EBADMSG. */
        assert_se(r != -EBADMSG);
        return EXIT_SUCCESS;
}
