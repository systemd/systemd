...
static int call_uki_hash_with(SectionSpec spec) {
        ...
        size_t hash_size = 0;
        r = uki_hash(fd, EVP_sha256(), hashes, &hash_size);
...
TEST_RET(uki_hash_one_bad_among_many_rejected) {
        ...
        void *hashes[_UNIFIED_SECTION_MAX] = {};
        size_t hash_size = 0;
        ASSERT_ERROR(uki_hash(fd, EVP_sha256(), hashes, &hash_size), EBADMSG);
        free_many(hashes, _UNIFIED_SECTION_MAX);
        return EXIT_SUCCESS;
}
...
TEST_RET(uki_hash_empty_pe_does_not_reject) {
        ...
        ASSERT_OK(uki_hash(fd, EVP_sha256(), hashes, &hash_size));
        free_many(hashes, _UNIFIED_SECTION_MAX);
        return EXIT_SUCCESS;
}
