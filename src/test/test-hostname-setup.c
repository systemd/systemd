...
TEST(hostname_substitute_wildcards_words) {
        ...
        /* Each token must come from its positional list, not just be a valid hostname. */
        _cleanup_strv_free_ char **tokens = strv_split(a, "-");
        ASSERT_NOT_NULL(tokens);
        ASSERT_EQ(strv_length(tokens), 2u);
        ASSERT_TRUE(STR_IN_SET(tokens[0], "happy", "sad", "jolly"));
        ASSERT_TRUE(STR_IN_SET(tokens[1], "octopus", "falcon", "badger"));
