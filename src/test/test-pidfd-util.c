...
TEST(pidfd_info_mask_is_supported) {
        ...
        if (r > 0)
                ...
                log_info("ioctl(PIDFD_GET_INFO) is not supported. Maybe the kernel is older then v6.13.");
        }

        if (ASSERT_OK_OR(pidfd_info_mask_is_supported(PIDFD_INFO_EXIT), -EOPNOTSUPP) > 0)
                log_info("PIDFD_INFO_EXIT is supported.");
        else
                log_info("PIDFD_INFO_EXIT is not supported. Maybe the kernel is older then v6.15.");

        if (ASSERT_OK_OR(pidfd_info_mask_is_supported(PIDFD_INFO_COREDUMP), -EOPNOTSUPP) > 0)
                log_info("PIDFD_INFO_COREDUMP is supported.");
        else
                log_info("PIDFD_INFO_COREDUMP is not supported. Maybe the kernel is older then v6.16.");
        ...
        if (ASSERT_OK_OR(pidfd_info_mask_is_supported(PIDFD_INFO_SUPPORTED_MASK), -EOPNOTSUPP) > 0)
                ...
                log_info("PIDFD_INFO_SUPPORTED_MASK is not supported. Maybe the kernel is older then v6.19.");
        ...
        if (ASSERT_OK_OR(pidfd_info_mask_is_supported(PIDFD_INFO_COREDUMP_SIGNAL), -EOPNOTSUPP) > 0)
                ...
                log_info("PIDFD_INFO_COREDUMP_SIGNAL is not supported. Maybe the kernel is older then v6.19.");
        ...
        if (ASSERT_OK_OR(pidfd_info_mask_is_supported(PIDFD_INFO_COREDUMP_CODE), -EOPNOTSUPP) > 0)
                ...
                log_info("PIDFD_INFO_COREDUMP_CODE is not supported. Maybe the kernel is older then v7.1.");
}
