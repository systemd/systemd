...
static void test_option_parse_one(
        ...
        for (const Entry *e = entries; e && (e->long_code || e->short_code); e++)
                n_entries++;
...
TEST(option_stops_parsing) {
        ...
        /* "--" after --exec: "--" is still consumed as end-of-options marker.
         * This is needed for backwards compatibility, systemd-dissect implemented
         * this behaviour. But also, it make sense — "--" we're unlikely to ever
         * want to specify "--" as the first argument of whatever sequence, but
         * the user may want to specify it for clarity. */
        test_option_parse_one(STRV_MAKE("arg0",
                                        "--exec",
                                        "--",
                                        "--help"),
...
TEST(option_group_marker) {
        static const Option options[] = {
                { 1, .short_code = 'h', .long_code = "help" },
                { 2, .long_code = "version" },
                { 0, .long_code = "Advanced", .flags = OPTION_GROUP_MARKER },
                { 3, .long_code = "debug" },
                {}
        };
        ...
        /* Group marker name is not a valid option */
        test_option_parse_one(STRV_MAKE("arg0",
                                        "--debug",
                                        "--version"),
                              options,
