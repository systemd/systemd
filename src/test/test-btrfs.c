...
int main(int argc, char *argv[]) {
        ...
        r = xopenat_lock(AT_FDCWD, "/xxxtest4", 0, 0, 0, LOCK_BSD, LOCK_EX|LOCK_NB);
        if (r >= 0)
                log_error_errno(r, "Unexpectedly got lock on /xxxtest4 snapshot");
        if (r != -EAGAIN)
                log_error_errno(r, "Failed to get lock on /xxxtest4 snapshot: %m");
