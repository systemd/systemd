...
TEST(subvol) {
        ...
        _cleanup_close_ int locked_fd = ASSERT_OK(btrfs_subvol_snapshot_at(dir_fd, "test1", dir_fd, "test4", BTRFS_SNAPSHOT_LOCK_BSD));
...
TEST(physical_offset) {
        ...
        ASSERT_OK(pidref_wait_for_terminate_and_check("(btrfs-inspect)", &inspect, 0));
