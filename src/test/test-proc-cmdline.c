...
TEST(proc_cmdline_filter_pid1_args) {
        ...
        test_proc_cmdline_filter_pid1_args_one("systemd hoge  -x foo \t   --aaa var", "hoge foo var");
        test_proc_cmdline_filter_pid1_args_one("systemd -a systemd.log-level=debug -b -c --quu --hoge 3", "systemd.log-level=debug 3");
