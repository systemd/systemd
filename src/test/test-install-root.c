...
static void write_vendor_file(const char *rel, const char *contents) {
        const char *p = strjoina(root, "/usr/lib/systemd/", rel);
...
static void write_vendor_symlink(const char *rel, const char *target) {
        const char *p = strjoina(root, "/usr/lib/systemd/system/", rel);
...
static void do_enable(UnitFileFlags flags, char * const *names) {
        ...
        CLEANUP_ARRAY(changes, n_changes, install_changes_free);
        ASSERT_OK(unit_file_enable(RUNTIME_SCOPE_SYSTEM, flags, root, names, &changes, &n_changes));
}
...
static void do_disable(UnitFileFlags flags, char * const *names) {
        ...
        CLEANUP_ARRAY(changes, n_changes, install_changes_free);
        ASSERT_OK(unit_file_disable(RUNTIME_SCOPE_SYSTEM, flags, root, names, &changes, &n_changes));
}
...
static void do_preset(UnitFileFlags flags, char * const *names) {
        ...
        CLEANUP_ARRAY(changes, n_changes, install_changes_free);
        ASSERT_OK(unit_file_preset(RUNTIME_SCOPE_SYSTEM, flags, root, names, UNIT_FILE_PRESET_FULL,
                                   &changes, &n_changes));
}
...
static void do_preset_all(UnitFileFlags flags) {
        ...
        CLEANUP_ARRAY(changes, n_changes, install_changes_free);
        ASSERT_OK(unit_file_preset_all(RUNTIME_SCOPE_SYSTEM, flags, root, UNIT_FILE_PRESET_FULL,
                                       &changes, &n_changes));
}
...
TEST(vendor_enable) {
        ...
        vendor_link = strjoina(root, VENDOR_WANTS"vendor-enabled.service");
        mask = strjoina(root, ETC_WANTS"vendor-enabled.service");
...
TEST(vendor_preset) {
        ...
        vendor_link = strjoina(root, VENDOR_WANTS"vendor-preset.service");
        mask = strjoina(root, ETC_WANTS"vendor-preset.service");
        ASSERT_TRUE(symlink_exists(vendor_link));
...
TEST(vendor_preset_disable_masks_in_etc) {
        ...
        vendor_link = strjoina(root, VENDOR_WANTS"vendor-preset-off.service");
        mask = strjoina(root, ETC_WANTS"vendor-preset-off.service");
        ASSERT_TRUE(is_dependency_mask(mask));
        ASSERT_TRUE(symlink_exists(vendor_link));
...
TEST(vendor_not_enableable_is_not_masked) {
        ...
        FOREACH_STRING(name, "vendor-static.service", "vendor-alias-only.service") {
                const char *mask = strjoina(root, ETC_WANTS, name);
        ...
        ASSERT_TRUE(symlink_exists(strjoina(root, VENDOR_WANTS"vendor-tmpl-static@one.service")));

        InstallChange *changes = NULL;
        size_t n_changes = 0;
        CLEANUP_ARRAY(changes, n_changes, install_changes_free);
        (void) unit_file_disable(RUNTIME_SCOPE_SYSTEM, UNIT_FILE_VENDOR, root,
                                 STRV_MAKE("vendor-gone.service"), &changes, &n_changes);
        ASSERT_FALSE(symlink_exists(strjoina(root, VENDOR_WANTS"vendor-gone.service")));
}
...
TEST(vendor_preset_all) {
        ...
        mask = strjoina(root, ETC_WANTS"vendor-all.service");
        ASSERT_TRUE(is_dependency_mask(mask));
...
TEST(vendor_template_instance) {
        ...
        mask = strjoina(root, VENDOR_WANTS"vendor-tmpl@inst.service");
        mask = strjoina(root, ETC_WANTS"vendor-tmpl@inst.service");
        ASSERT_TRUE(is_dependency_mask(mask));
...
TEST(vendor_mask_shadows_lower_priority_dir) {
        ...
        mask = strjoina(root, ETC_WANTS"vendor-shadow.service");
        ASSERT_FALSE(symlink_exists(mask));
}
...
TEST(vendor_multiple_units) {
        ...
        FOREACH_STRING(name, "vendor-multi-a.service", "vendor-multi-b.service", "vendor-multi-c.service")
                FOREACH_STRING(target, "graphical.target", "sockets.target")
                        ASSERT_TRUE(is_dependency_mask(strjoina(root, SYSTEM_CONFIG_UNIT_DIR"/", target,
                                                                ".wants/", name)));
        ...
        FOREACH_STRING(name, "vendor-multi-a.service", "vendor-multi-b.service", "vendor-multi-c.service")
                FOREACH_STRING(target, "graphical.target", "sockets.target")
                        ASSERT_FALSE(is_dependency_mask(strjoina(root, SYSTEM_CONFIG_UNIT_DIR"/", target,
                                                                 ".wants/", name)));
}
...
TEST(vendor_entry_kinds) {
        ...
        entry = strjoina(root, ETC_WANTS"vendor-kinds.service");
        ...
        ASSERT_OK(write_string_file(strjoina(root, "/etc/empty"), "",
...
TEST(vendor_dry_run_writes_nothing) {
        ...
        ASSERT_OK(unit_file_disable(RUNTIME_SCOPE_SYSTEM, UNIT_FILE_DRY_RUN, root,
                                    STRV_MAKE("vendor-dry.service"), &changes, &n_changes));
        ASSERT_EQ(n_changes, 1u);
        ASSERT_EQ(changes[0].type, INSTALL_CHANGE_MASK_DEPENDENCY);
        ASSERT_STREQ(changes[0].source, strjoina(root, VENDOR_WANTS"vendor-dry.service"));

        ASSERT_FALSE(symlink_exists(strjoina(root, ETC_WANTS"vendor-dry.service")));
...
TEST(vendor_no_pointless_mask_below_the_vendor_directory) {
        ...
        const char *high = strjoina(root, "/usr/local/lib/systemd/system/multi-user.target.wants/vendor-local.service");
        ...
        ASSERT_FALSE(symlink_exists(strjoina(root, VENDOR_WANTS"vendor-local.service")));
}
...
TEST(vendor_keeps_symlinks_it_was_not_asked_for) {
        ...
        wiring = strjoina(root, VENDOR_WANTS"vendor-named.service");
        ...
        ASSERT_TRUE(symlink_exists(strjoina(root, "/usr/lib/systemd/system/vendor-unnamed-compat.service")));
}
...
TEST(vendor_preset_leaves_etc_alone) {
        ...
        vendor_link = strjoina(root, VENDOR_WANTS"vendor-redundant.service");
        etc_link = strjoina(root, ETC_WANTS"vendor-redundant.service");
...
TEST(vendor_preset_still_writes_what_is_needed) {
        ...
        const char *runtime_link = strjoina(root, "/run/systemd/system/sockets.target.wants/vendor-partial.service");
        ...
        const char *vendor_mask = strjoina(root, "/usr/local/lib/systemd/system/timers.target.wants/vendor-partial.service");
        ...
        ASSERT_FALSE(symlink_exists(strjoina(root, ETC_WANTS"vendor-partial.service")));

        FOREACH_STRING(target, "graphical.target", "sockets.target", "timers.target") {
                const char *p = strjoina(root, SYSTEM_CONFIG_UNIT_DIR"/", target, ".wants/vendor-partial.service");
...
TEST(vendor_preset_skips_redundant_alias) {
         ...
         * reboot. */
        const char *runtime_alias = strjoina(root, "/run/systemd/system/vendor-aliased-transient.service");
        ...
        ASSERT_FALSE(symlink_exists(strjoina(root, SYSTEM_CONFIG_UNIT_DIR"/vendor-aliased-name.service")));
        ASSERT_TRUE(symlink_exists(strjoina(root, SYSTEM_CONFIG_UNIT_DIR"/vendor-aliased-taken.service")));
        ASSERT_TRUE(symlink_exists(strjoina(root, SYSTEM_CONFIG_UNIT_DIR"/vendor-aliased-transient.service")));

        /* An explicit enable records itself as always. */
        do_enable(0, STRV_MAKE("vendor-aliased.service"));
        ASSERT_TRUE(symlink_exists(strjoina(root, SYSTEM_CONFIG_UNIT_DIR"/vendor-aliased-name.service")));
}
