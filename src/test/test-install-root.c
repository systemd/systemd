...
TEST(basic_mask_and_enable) {
        ...
        assert_se(changes[0].change_or_error == INSTALL_CHANGE_SYMLINK);
        ...
        assert_se(n_changes == 1);
        assert_se(changes[0].change_or_error == INSTALL_CHANGE_UNLINK);
        p = strjoina(root, SYSTEM_CONFIG_UNIT_DIR"/a.service");
        assert_se(streq(changes[0].path, p));
        install_changes_free(changes, n_changes);
        changes = NULL; n_changes = 0;
        ...
        p = strjoina(root, SYSTEM_CONFIG_UNIT_DIR"/multi-user.target.wants/a.service");
        assert_se(streq(changes[0].path, p));
        assert_se(changes[1].change_or_error == INSTALL_CHANGE_SYMLINK);
        ...
        assert_se(streq(changes[0].path, p));
        assert_se(changes[1].change_or_error == INSTALL_CHANGE_DESTINATION_NOT_PRESENT);
        p = strjoina(root, "/usr/lib/systemd/system/f.service");
...
TEST(linked_units) {
        ...
        for (i = 0 ; i < n_changes; i++) {
                assert_se(changes[i].change_or_error == INSTALL_CHANGE_SYMLINK);
        ...
        q = strjoina(root, SYSTEM_CONFIG_UNIT_DIR"/linked.service");
        for (i = 0; i < n_changes; i++) {
                assert_se(changes[i].change_or_error == INSTALL_CHANGE_UNLINK);
...
TEST(default) {
        ...
        assert_se(n_changes == 1);
        assert_se(changes[0].change_or_error == -ENOENT);
...
TEST(preset_and_list) {
        ...
        for (i = 0; i < n_changes; i++) {
                ...
                if (changes[i].change_or_error == INSTALL_CHANGE_SYMLINK) {
...
TEST(revert) {
        ...
        assert_se(changes[1].change_or_error == INSTALL_CHANGE_UNLINK);
        assert_se(streq(changes[1].path, p));
        install_changes_free(changes, n_changes);
        changes = NULL; n_changes = 0;
}
