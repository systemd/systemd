...
/* Map native_architecture() to the QEMU system binary suffix. This mirrors
 * the architecture_to_qemu_table[] in vmspawn-util.c — only the native
 * architecture is searched so we test the same binary vmspawn would use.
 *
 * TODO: consider moving find_qemu_binary() from vmspawn-util.c to shared/ so
 * both vmspawn and this test can use a single implementation. */
static const char* native_qemu_suffix(void) {
        switch (native_architecture()) {
        case ARCHITECTURE_X86_64:   return "x86_64";
        case ARCHITECTURE_X86:      return "i386";
        case ARCHITECTURE_ARM64:    return "aarch64";
        case ARCHITECTURE_ARM:      return "arm";
        case ARCHITECTURE_S390X:    return "s390x";
        case ARCHITECTURE_PPC64_LE: return "ppc";
        case ARCHITECTURE_PPC64:    return "ppc";
        case ARCHITECTURE_PPC:      return "ppc";
        case ARCHITECTURE_RISCV64:  return "riscv64";
        case ARCHITECTURE_RISCV32:  return "riscv32";
        default:                    return NULL;
        }
}
...
static int find_qemu(char **ret) {
        ...
        /* Try generic names first, then the architecture-specific binary.
         * Matches vmspawn's find_qemu_binary() search order. */
        ...
        const char *suffix = native_qemu_suffix();
        if (!suffix)
                return -EOPNOTSUPP;

        _cleanup_free_ char *name = strjoin("qemu-system-", suffix);
        if (!name)
                return -ENOMEM;

        return find_executable(name, ret);
}
