...
TEST(qmp_client_qemu_handshake_and_schema) {
        ...
        if (r < 0) {
                log_tests_skipped_errno(r, "Failed to attach event");
                (void) pidref_kill(&pidref, SIGKILL);
                (void) pidref_wait_for_terminate(&pidref, NULL);
                return;
        }

        /* Wait for async handshake */
        while (qmp_client_is_handshaking(client)) {
                r = qmp_client_process(client);
                if (r < 0) {
                ...
                if (r > 0)
                        continue;
                r = qmp_client_wait(client, USEC_INFINITY);
                if (r < 0) {
                        log_tests_skipped_errno(r, "QMP handshake wait failed");
                        (void) pidref_kill(&pidref, SIGKILL);
                        (void) pidref_wait_for_terminate(&pidref, NULL);
                        return;
                }
        ...
        /* Clean shutdown */
        r = qmp_client_invoke(client, "quit", NULL, on_test_result, &t);
        ASSERT_OK(r);
        qmp_test_wait(event, &t);
        ASSERT_EQ(t.error, 0);
        qmp_test_result_done(&t);
