...
TEST(qmp_client_qemu_handshake_and_schema) {
        ...
        r = qmp_client_invoke(client, /* ret_slot= */ NULL, "query-qmp-schema", /* args= */ NULL, on_test_result, &t);
        if (r < 0) {
        ...
        /* Clean shutdown */
        ASSERT_OK(qmp_client_invoke(client, /* ret_slot= */ NULL, "quit", /* args= */ NULL, on_test_result, &t));
        qmp_test_wait(event, &t);
        ASSERT_EQ(t.error, 0);
        qmp_test_result_done(&t);
...
TEST(qmp_client_qemu_query_status) {
        ...
        r = qmp_client_invoke(client, /* ret_slot= */ NULL, "query-status", /* args= */ NULL, on_test_result, &t);
        if (r < 0) {
                log_tests_skipped_errno(r, "QMP invoke failed (handshake or send)");
                return;
        }
        ...
        ASSERT_OK(qmp_client_invoke(client, /* ret_slot= */ NULL, "stop", /* args= */ NULL, on_test_result, &t));
        qmp_test_wait(event, &t);
        ASSERT_EQ(t.error, 0);
        qmp_test_result_done(&t);

        /* Verify status changed */
        ASSERT_OK(qmp_client_invoke(client, /* ret_slot= */ NULL, "query-status", /* args= */ NULL, on_test_result, &t));
        qmp_test_wait(event, &t);
        ASSERT_EQ(t.error, 0);
        ASSERT_NOT_NULL(t.result);
        ...
        ASSERT_OK(qmp_client_invoke(client, /* ret_slot= */ NULL, "cont", /* args= */ NULL, on_test_result, &t));
        qmp_test_wait(event, &t);
        ASSERT_EQ(t.error, 0);
        qmp_test_result_done(&t);
