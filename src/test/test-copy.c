...
TEST_RET(copy_holes_with_gaps) {
        ...
        assert_se(r >= 0);
        assert_se(lseek(fd, blksz, SEEK_CUR) >= 0);
        for (i=0, xfer_count=0; i < 2 * blksz / sizeof(data); i++)
                xfer_count += write(fd, data, sizeof(data));
        assert_se(xfer_count == (uint64_t) 2 * blksz);
        assert_se(lseek(fd, 2 * blksz, SEEK_CUR) >= 0);
        for (i=0, xfer_count=0; i < blksz / sizeof(data); i++)
                xfer_count += write(fd, data, sizeof(data));
        assert_se(xfer_count == (uint64_t) blksz);
        assert_se(lseek(fd, 0, SEEK_SET) >= 0);
        ...
        assert_se(copy_bytes(fd, fd_copy, UINT64_MAX, COPY_HOLES) >= 0);
        assert_se(fstat(fd_copy, &stat) >= 0);
        assert_se(stat.st_size == 6 * blksz);
