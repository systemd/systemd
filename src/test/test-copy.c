...
static bool enable_fsverity(int dirfd, const char *name, int algo, const char *salt, size_t salt_size) {
        ...
        ASSERT_TRUE(errno == ENOTSUP || errno == ENOTTY);
        return false;
}
...
TEST_RET(copy_with_verity) {
        ...
        ASSERT_OK((badsrc = mkdtemp_open("/tmp/test-copy_file-src.XXXXXX", 0, &badsrcp)) >= 0);
        ASSERT_OK((baddst = mkdtemp_open("/tmp/test-copy_file-src.XXXXXX", 0, &baddstp)) >= 0);

        /* Populate the source, same as before */
        FOREACH_ELEMENT(file, files)
                ASSERT_OK(write_string_file_at(badsrc, *file, "src file", WRITE_STRING_FILE_CREATE) >= 0);
