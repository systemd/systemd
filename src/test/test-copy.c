...
TEST(copy_tree_replace_file) {
        ...
        assert_se(copy_tree(src, dst, UID_INVALID, GID_INVALID, 0, NULL, NULL) == -EEXIST);
...
TEST(copy_tree_replace_dirs) {
        ...
        assert_se(copy_tree_at(src, ".", dst, ".", UID_INVALID, GID_INVALID, 0, NULL, NULL) == -EEXIST);
...
TEST(copy_file_fd) {
        ...
        assert_se(copy_file_fd("/a/file/which/does/not/exist/i/guess", out_fd, 0) < 0);
        assert_se(copy_file_fd(in_fn, out_fd, 0) >= 0);
...
TEST(copy_tree) {
        ...
        assert_se(copy_tree(original_dir, copy_dir, UID_INVALID, GID_INVALID, 0, denylist, NULL) < 0);
        assert_se(copy_tree("/tmp/inexistent/foo/bar/fsdoi", copy_dir, UID_INVALID, GID_INVALID, 0, denylist, NULL) < 0);
...
static void test_copy_bytes_regular_file_one(const char *src, uint64_t max_bytes) {
        ...
        r = copy_bytes(fd, fd2, max_bytes, 0);
...
TEST(copy_atomic) {
        ...
        r = copy_file_atomic("/etc/fstab", q, 0644, 0);
        if (r == -ENOENT || ERRNO_IS_PRIVILEGE(r))
                return;

        assert_se(copy_file_atomic("/etc/fstab", q, 0644, 0) == -EEXIST);
