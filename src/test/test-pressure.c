...
static void test_fake_pressure(
        ...
        _cleanup_free_ char *j = NULL, *k = NULL,
                *env_watch = NULL, *env_write = NULL, *resource_upper = NULL;
        ...
        resource_upper = strdup(resource);
        ASSERT_NOT_NULL(resource_upper);
        ascii_strupper(resource_upper);

        env_watch = strjoin(resource_upper, "_PRESSURE_WATCH");
        ASSERT_NOT_NULL(env_watch);
        env_write = strjoin(resource_upper, "_PRESSURE_WRITE");
        ASSERT_NOT_NULL(env_write);

        ASSERT_OK(sd_event_default(&e));

        ASSERT_OK(mkdtemp_malloc(NULL, &tmp));

        ASSERT_NOT_NULL(j = path_join(tmp, "fifo"));
        ASSERT_OK_ERRNO(mkfifo(j, 0600));
        fifo_fd = open(j, O_CLOEXEC|O_RDWR|O_NONBLOCK);
        ASSERT_OK_ERRNO(fifo_fd);

        ASSERT_NOT_NULL(k = path_join(tmp, "sock"));
        ...
        *fp = (struct fake_pressure_context) {
        ...
        ASSERT_OK(add_pressure(e, &es, NULL, &value));
        ...
        ASSERT_OK(add_pressure(e, &ef, NULL, &value));
...
static int fake_pressure_wrapper(sd_event *e, sd_event_source **ret, sd_event_handler_t callback, void *userdata) {
        /* Wrapper that substitutes our custom callback regardless of what the caller passes,
         * since the add_pressure functions use a default callback when NULL is passed */
        return sd_event_add_memory_pressure(e, ret, fake_pressure_callback, userdata);
}
...
static int fake_cpu_pressure_wrapper(sd_event *e, sd_event_source **ret, sd_event_handler_t callback, void *userdata) {
        return sd_event_add_cpu_pressure(e, ret, fake_pressure_callback, userdata);
}
...
static int fake_io_pressure_wrapper(sd_event *e, sd_event_source **ret, sd_event_handler_t callback, void *userdata) {
        return sd_event_add_io_pressure(e, ret, fake_pressure_callback, userdata);
}
