...
_unused_ static void test_compress_decompress(
        ...
        r = compress(data, data_len, compressed, sizeof(compressed), &csize, -1);
        if (r == -ENOBUFS) {
...
#define BUFSIZE_2 20000
        ...
        r = compress(data, data_len, compressed, BUFSIZE_1, &csize, -1);
        if (r == -ENOBUFS) {
                log_info_errno(r, "compression failed: %m");
                assert_se(may_fail);
                ...
                r = compress(data, data_len, compressed, BUFSIZE_2, &csize, -1);
        }
        assert_se(r >= 0);
...
#define TEXT "HUGE=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
        ...
        r = compress(TEXT, sizeof TEXT, buf, sizeof buf, &csize, -1);
        assert_se(r >= 0);
...
static void test_lz4_decompress_partial(void) {
        ...
        r = sym_LZ4_compress_fast(huge, buf, HUGE_SIZE, buf_size, 1);
        assert_se(r >= 0);
        compressed = r;
