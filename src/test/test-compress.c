...
TEST(decompress_blob_zstd_unknown_size) {
        ...
        size_t input_size = 64 * 1024; /* large enough to require several output buffer growths */
        ...
        _cleanup_(unlink_tempfilep) char pattern[] = "/tmp/systemd-test.zstd-unknown.XXXXXX";
        struct stat st;

        ASSERT_OK(src = memfd_new("test-zstd-unknown"));
        ...
        _cleanup_(unlink_tempfilep) char short_pattern[] = "/tmp/systemd-test.zstd-short.XXXXXX";
        static const char short_text[] = "hi";
        struct stat short_st;

        ASSERT_OK(short_src = memfd_new("test-zstd-short"));
