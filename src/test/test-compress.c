...
TEST(decompress_blob_zstd_frame_at_buffer_boundary) {
         ...
         *
         * The first output buffer is greedy_realloc(ZSTD_DStreamOutSize == 131072), so sizing the
         * content to that buffer's exact usable size makes the frame finish precisely when it fills. */
        ...
        ASSERT_NOT_NULL(greedy_realloc(&probe, 131072, 1));
