...
_unused_ static void test_compress_decompress(
        ...
        r = compress(data, data_len, compressed, sizeof(compressed), &csize, 0);
        if (r == -ENOBUFS) {
...
#define BUFSIZE_2 20000
        ...
        r = compress(data, data_len, compressed, BUFSIZE_1, &csize, 0);
        if (r == -ENOBUFS) {
                log_info_errno(r, "compression failed: %m");
                assert_se(may_fail);
                ...
                r = compress(data, data_len, compressed, BUFSIZE_2, &csize, 0);
        }
        assert_se(r >= 0);
...
#define TEXT "HUGE=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
        ...
        r = compress(TEXT, sizeof TEXT, buf, sizeof buf, &csize, 0);
        assert_se(r >= 0);
