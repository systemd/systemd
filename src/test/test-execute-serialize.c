...
static void roundtrip_quotes_check(RoundtripState *s) {
        ...
        for (size_t pos = 0; pos < ELEMENTSOF(quotes_strs); pos++) {
                ...
                assert(streq(expected, s->exec_context->bind_mounts[bind_mount_base].source));
                assert(streq(expected, s->exec_context->bind_mounts[bind_mount_base].destination));

                assert(s->exec_context->bind_mounts[bind_mount_base + 1].read_only);
                assert(streq(expected, s->exec_context->bind_mounts[bind_mount_base + 1].source));
                assert(streq(expected, s->exec_context->bind_mounts[bind_mount_base + 1].destination));

                assert(streq(expected, s->exec_context->mount_images[pos].source));
                assert(streq(expected, s->exec_context->mount_images[pos].destination));

                assert(streq(expected, s->exec_context->extension_images[pos].source));
        }
}
...
static void roundtrip_spaces_check(RoundtripState *s) {
        ...
        assert(streq(spaces_str, s->exec_context->mount_images[0].source));
        assert(streq(spaces_str, s->exec_context->mount_images[0].destination));

        assert(streq(spaces_str, s->exec_context->extension_images[0].source));
}
