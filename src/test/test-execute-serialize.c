...
static void roundtrip_quotes_check(RoundtripState *s) {
        ...
        for (size_t pos = 0; pos < ELEMENTSOF(quotes_strs); pos += 1) {
                ...
                assert_se(strcmp(expected, s->exec_context->bind_mounts[bind_mount_base].source) == 0);
                assert_se(strcmp(expected, s->exec_context->bind_mounts[bind_mount_base].destination) == 0);

                assert_se(s->exec_context->bind_mounts[bind_mount_base + 1].read_only);
                assert_se(strcmp(expected, s->exec_context->bind_mounts[bind_mount_base + 1].source) == 0);
                assert_se(strcmp(expected, s->exec_context->bind_mounts[bind_mount_base + 1].destination) == 0);

                assert_se(strcmp(expected, s->exec_context->mount_images[pos].source) == 0);
                assert_se(strcmp(expected, s->exec_context->mount_images[pos].destination) == 0);

                assert_se(strcmp(expected, s->exec_context->extension_images[pos].source) == 0);
        }
}
...
static void roundtrip_spaces_check(RoundtripState *s) {
        ...
        assert_se(strcmp(spaces_str, s->exec_context->mount_images[0].source) == 0);
        assert_se(strcmp(spaces_str, s->exec_context->mount_images[0].destination) == 0);

        assert_se(strcmp(spaces_str, s->exec_context->extension_images[0].source) == 0);
}
