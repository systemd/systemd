...
static void test_roundtrip_by_calling(RoundtripHook *setup, RoundtripHook *check) {
        ...
        _cleanup_fdset_free_ FDSet *fds = fdset_new();
...
static void roundtrip_quotes_setup(RoundtripState *s) {
        FOREACH_ELEMENT(str, quotes_strs) {
                bind_mount_add(&s->exec_context->bind_mounts, &s->exec_context->n_bind_mounts, &(BindMount) {
                        .source = (char *) *str,
...
static void roundtrip_quotes_check(RoundtripState *s) {
        assert(s->exec_context->n_bind_mounts == 2 * ELEMENTSOF(quotes_strs));
        assert(s->exec_context->n_mount_images == ELEMENTSOF(quotes_strs));
        assert(s->exec_context->n_extension_images == ELEMENTSOF(quotes_strs));

        for (size_t pos = 0; pos < ELEMENTSOF(quotes_strs); pos += 1) {
                size_t bind_mount_base = 2 * pos;

                const char *expected = quotes_strs[pos];

                assert(!s->exec_context->bind_mounts[bind_mount_base].read_only);
                assert(strcmp(expected, s->exec_context->bind_mounts[bind_mount_base].source) == 0);
                assert(strcmp(expected, s->exec_context->bind_mounts[bind_mount_base].destination) == 0);

                assert(s->exec_context->bind_mounts[bind_mount_base + 1].read_only);
                assert(strcmp(expected, s->exec_context->bind_mounts[bind_mount_base + 1].source) == 0);
                assert(strcmp(expected, s->exec_context->bind_mounts[bind_mount_base + 1].destination) == 0);

                assert(strcmp(expected, s->exec_context->mount_images[pos].source) == 0);
                assert(strcmp(expected, s->exec_context->mount_images[pos].destination) == 0);

                assert(strcmp(expected, s->exec_context->extension_images[pos].source) == 0);
        }
}
...
static void roundtrip_spaces_setup(RoundtripState *s) {
        mount_image_add(&s->exec_context->mount_images, &s->exec_context->n_mount_images, &(MountImage) {
                .source = strdup(spaces_str),
                .destination = strdup(spaces_str),
                0,
        });
