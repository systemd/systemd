...
TEST(utf8_is_printable) {
        ...
        assert_se(utf8_is_printable("", 0));

        /* DEL and the C1 range */
        assert_se(!utf8_is_printable("\177", 1));
        assert_se(!utf8_is_printable("\302\233", 2));
        ...
        assert_se(utf8_is_printable("abcdefgh", 8));
        assert_se(utf8_is_printable("abcdefg", 7));
        assert_se(utf8_is_printable("aaaaaaa\342\204\242aaaaa", 15));
        assert_se(!utf8_is_printable("aaaaaaa\342\204", 9));
        ...
        FOREACH_ELEMENT(c, interesting)
                for (size_t i = 0; i < 16; i++) {
                        ...
                        assert_se(utf8_is_printable(buf, sizeof(buf)) == IN_SET(*c, '\t', '\n'));
                        assert_se(utf8_is_printable_newline(buf, sizeof(buf), false) == (*c == '\t'));
                }
}
