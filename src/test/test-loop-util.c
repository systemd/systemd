...
TEST(partscan_required_for_nested_partition_table) {
#if HAVE_BLKID
        _cleanup_(dissected_image_unrefp) DissectedImage *dissected = NULL;
         ...
         * where a partition carries a GPT the kernel won't scan, as partition devices don't support
         * partition scanning. We write and read it back through the same partition node, so the buffer cache
         * stays coherent. */
        cmd = mfree(cmd);
