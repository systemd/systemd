...
TEST(image_filter_test_verity_fallback) {
        ...
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR, "250"));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR, "259"));
        ASSERT_FALSE(image_filter_test(f, PARTITION_USR, "260"));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR_VERITY, "250"));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR_VERITY, "259"));
        ASSERT_FALSE(image_filter_test(f, PARTITION_USR_VERITY, "260"));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR_VERITY_SIG, "250"));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR_VERITY_SIG, "259"));
        ASSERT_FALSE(image_filter_test(f, PARTITION_USR_VERITY_SIG, "260"));
}

TEST(image_filter_test_verity_explicit) {
        _cleanup_(image_filter_freep) ImageFilter *f = NULL;

        /* Explicit usr-verity= pattern wins over the usr= fallback */
        ASSERT_OK(image_filter_parse("usr=250:usr-verity=999", &f));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR, "250"));
        ASSERT_FALSE(image_filter_test(f, PARTITION_USR, "999"));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR_VERITY, "999"));
        ASSERT_FALSE(image_filter_test(f, PARTITION_USR_VERITY, "250"));
        /* usr-verity-sig still falls back to usr= since no explicit usr-verity-sig= was given */
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR_VERITY_SIG, "250"));
        ASSERT_FALSE(image_filter_test(f, PARTITION_USR_VERITY_SIG, "999"));

        f = image_filter_free(f);

        /* Explicit usr-verity-sig= pattern wins over the usr= fallback */
        ASSERT_OK(image_filter_parse("usr=250:usr-verity-sig=777", &f));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR, "250"));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR_VERITY_SIG, "777"));
        ASSERT_FALSE(image_filter_test(f, PARTITION_USR_VERITY_SIG, "250"));
        /* usr-verity still falls back to usr= since no explicit usr-verity= was given */
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR_VERITY, "250"));
        ASSERT_FALSE(image_filter_test(f, PARTITION_USR_VERITY, "777"));

        f = image_filter_free(f);

        /* Fallback does not apply in reverse: usr-verity= must not constrain usr */
        ASSERT_OK(image_filter_parse("usr-verity=999", &f));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR_VERITY, "999"));
        ASSERT_FALSE(image_filter_test(f, PARTITION_USR_VERITY, "250"));
        ASSERT_TRUE(image_filter_test(f, PARTITION_USR, "anything"));
}
