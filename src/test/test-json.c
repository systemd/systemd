...
static void test_zeroes(JsonVariant *v) {
        ...
        for (size_t i = 0; i < json_variant_elements(v); i++) {
                ...
                assert_se(iszero_relaxed(json_variant_real(w)));
...
static void test_float_match(JsonVariant *v) {
        ...
        assert_se(fabs((double) 1.0 - ((double) DBL_MIN / json_variant_real(json_variant_by_index(v, 0)))) <= delta);
        assert_se(fabs((double) 1.0 - ((double) DBL_MAX / json_variant_real(json_variant_by_index(v, 1)))) <= delta);
        ...
        assert_se(json_variant_is_null(json_variant_by_index(v, 5)) ||
                  fabs((double) 1.0 - ((double) HUGE_VAL / json_variant_real(json_variant_by_index(v, 5)))) <= delta); /* HUGE_VAL might be +inf, but might also be something else */
