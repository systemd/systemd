...
TEST(mount_table_kernel_dedup_direction) {
         ...
         * last entry for a path win; other consumers keep the first entry of a backwards walk.
         * Both conventions must pick the same mount for every overmounted path. One snapshot,
         * walked both ways, so there is no second table to race against. */
