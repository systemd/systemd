...
TEST(path_is_network_fs_harder) {
        ...
        ASSERT_ERROR(path_is_network_fs_harder("foo"), EINVAL);

        if (RET_NERRNO(access("/this/path/should/not/exist/for/test-mount-util/", F_OK)) == -ENOENT)
                ASSERT_ERROR(path_is_network_fs_harder("/this/path/should/not/exist/for/test-mount-util/"), ENOENT);

        FOREACH_STRING(s, "/", "/dev/", "/proc/", "/run/", "/sys/", "/tmp/", "/usr/", "/var/tmp/") {
