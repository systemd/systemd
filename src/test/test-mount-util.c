...
TEST(statmount_libmount_parity) {
        ...
        /* Walk the ids in descending order, so that the entry stored for a path is the topmost
         * mount on it, matching the MNT_ITER_BACKWARD walk of the libmount snapshot below. */
        ...
        for (;;) {
                ...
                if (!sm_v) /* Mounted or unmounted between the two snapshots */
                        continue;

                assert_se(asprintf(&expected, "%s %lx", type, fl) >= 0);
                ASSERT_STREQ(sm_v, expected);
                n_compared++;
        }
