...
static void test_mount_option_mangle(void) {
        ...
        assert_se(mount_option_mangle("mode=1777,size=10%,nr_inodes=400k,uid=496107520,gid=496107520,context=\"sys.id:sys.role:systemd.nspawn.container.fs:s0,s1\"", 0, &f, &opts) == 0);
        assert_se(f == 0);
        log_info("%s", opts);
        assert_se(streq(opts, "mode=1777,size=10%,nr_inodes=400k,uid=496107520,gid=496107520,context=\"sys.id:sys.role:systemd.nspawn.container.fs:s0,s1\""));
        opts = mfree(opts);
}
