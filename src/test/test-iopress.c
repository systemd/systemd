...
static int fake_pressure_callback(sd_event_source *s, void *userdata) {
        ...
        *value *= d[0];
        ...
        log_notice("io pressure event: %s", d);
...
TEST(fake_pressure) {
        ...
        *fp = (struct fake_pressure_context) {
        ...
        ASSERT_OK_ERRNO(setenv("IO_PRESSURE_WATCH", j, /* override= */ true));
        ASSERT_OK_ERRNO(unsetenv("IO_PRESSURE_WRITE"));

        ASSERT_OK(sd_event_add_io_pressure(e, &es, fake_pressure_callback, &value));
        ASSERT_OK(sd_event_source_set_description(es, "fifo event source"));

        ASSERT_OK_ERRNO(setenv("IO_PRESSURE_WATCH", k, /* override= */ true));
        ASSERT_OK_ERRNO(setenv("IO_PRESSURE_WRITE", "aGVsbG8K", /* override= */ true));

        ASSERT_OK(sd_event_add_io_pressure(e, &ef, fake_pressure_callback, &value));
...
TEST(real_pressure) {
        ...
        r = sd_event_add_io_pressure(e, &es, real_pressure_callback, &context);
        if (r < 0)
