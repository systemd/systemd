...
static const Test single_prog[] = {
        {
                ...
                .bpffs_path = "/sys/fs/bpf/trivial-skb",
        },
};
...
static const Test path_split_test[] = {
        {
                ...
                .prog_type = BPF_PROG_TYPE_CGROUP_SKB,
                .attach_type = BPF_CGROUP_INET_INGRESS,
                .bpffs_path = "/sys/fs/bpf/path:split:test",
        },
};

static const Test invalid_attach_type[] = {
        {
                .option_name = "BPFProgram",
                .prog_type = BPF_PROG_TYPE_CGROUP_SKB,
                .attach_type = 100500,
...
static const Test same_prog_same_hook[] = {
        {
                ...
                .bpffs_path = "/sys/fs/bpf/trivial-sock",
        },
        {
                .option_name = "BPFProgram",
                .prog_type = BPF_PROG_TYPE_CGROUP_SOCK,
...
static const Test multi_prog_same_hook[] = {
        {
                ...
                .prog_type = BPF_PROG_TYPE_CGROUP_SOCK,
                .attach_type = BPF_CGROUP_INET_SOCK_CREATE,
                .bpffs_path = "/sys/fs/bpf/trivial-sock-0",
        },
        {
                .option_name = "BPFProgram",
                .prog_type = BPF_PROG_TYPE_CGROUP_SOCK,
                .attach_type = BPF_CGROUP_INET_SOCK_CREATE,
                .bpffs_path = "/sys/fs/bpf/trivial-sock-1",
        }
};
...
static int pin_programs(Unit *u, CGroupContext *cc, const Test *test_suite, size_t test_suite_size, char ***paths_ret) {
        ...
        for (size_t i = 0; i < test_suite_size; i++) {
                ...
                r = bpf_foreign_program_to_string(test_suite[i].attach_type, test_suite[i].bpffs_path, &str);
                if (r < 0)
                ...
                dup = strdup(test_suite[i].bpffs_path);
                if (!dup)
                        return log_oom();

                r = strv_consume(&bpffs_paths, dup);
                if (r < 0)
...
static int test_bpf_cgroup_programs(Manager *m, const char *unit_name, const Test *test_suite, size_t test_suite_size) {
        ...
        for (size_t i = 0; i < test_suite_size; i++) {
                ...
                if (streq(test_suite[i].option_name, "BPFProgram")) {
                        r = bpf_foreign_program_to_string(test_suite[i].attach_type, test_suite[i].bpffs_path, &option);
                        if (r < 0)
                        ...
                        void *paths = NULL;
                        option = strdup(test_suite[i].bpffs_path);
                        if (!option)
                                return log_oom();
                        ...
                        r = config_parse_ip_filter_bpf_progs(u->id, "filename", 1, "Service", 1, test_suite[i].option_name, 0, option, paths, u);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse option string '%s': %m", option);
                }
        ...
        r = config_parse_exec(u->id, "filename", 1, "Service", 1, "ExecStart", SERVICE_EXEC_START, "-/bin/ping -c 5 127.0.0.1 -W 1", SERVICE(u)->exec_command, u);
        if (r < 0)
        ...
        if (cld_code != CLD_EXITED) {
                ...
                log_error_errno(r, "ExecStart didn't exited, code='%s'", sigchld_code_to_string(cld_code));
        }

        if (SERVICE(u)->state != SERVICE_DEAD) {
                r = -SYNTHETIC_ERRNO(EBUSY);
                log_error_errno(r, "Service is not dead");
        }
...
int main(int argc, char *argv[]) {
        ...
        r = cg_all_unified();
        if (r <= 0)
                return log_tests_skipped("Unified hierarchy is required: %m");
        ...
        assert_se(test_bpf_cgroup_programs(m,
                                "invalid_attach_type.service",
                                invalid_attach_type,
                                ELEMENTSOF(invalid_attach_type)) >= 0);
        return 0;
}
