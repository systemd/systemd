...
#define BPFFS_PATH(prog_suffix) ("/sys/fs/bpf/bpf-foreing-" # prog_suffix)
...
static int bpf_foreign_test_to_string(enum bpf_attach_type attach_type, const char *bpffs_path, char **ret_str) {
        ...
        assert_se(p = strjoin(s, ":", bpffs_path));
...
int main(int argc, char *argv[]) {
        ...
        r = cg_all_unified();
        if (r <= 0)
                return log_tests_skipped_errno("Unified hierarchy is required: %m");
