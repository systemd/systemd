...
TEST(namespace_open_by_id) {
        ...
        /* namespace_open_by_id() refuses with -EHOSTDOWN outside the initial user namespace, since
         * the kernel restricts open_by_handle_at() on nsfs to the initial userns and pidns. */
        _cleanup_close_ int opened = namespace_open_by_id(userns_fd, ns_id);
        if (opened == -EHOSTDOWN)
                return (void) log_tests_skipped("not in initial user namespace");
        ...
        ASSERT_ERROR(namespace_open_by_id(userns_fd, 0), EINVAL);
        ...
        opened = ASSERT_OK(namespace_open_by_id(userns_fd, transient_id));
        ...
        ASSERT_ERROR(namespace_open_by_id(userns_fd, transient_id), ESTALE);
}
