...
TEST(mkdir_p_safe) {
        ...
        ASSERT_OK_ERRNO(mkdir_p_safe(tmp, p, 0755, UID_INVALID, GID_INVALID, 0));
        ASSERT_OK_POSITIVE(is_dir(p, false));
        ASSERT_OK_POSITIVE(is_dir(p, true));

        p = mfree(p);
        ASSERT_NOT_NULL(p = path_join(tmp, "var/run"));
        ASSERT_OK_ERRNO(mkdir_parents_safe(tmp, p, 0755, UID_INVALID, GID_INVALID, 0));
        ...
        ASSERT_OK_ERRNO(mkdir_safe(p, 0755, UID_INVALID, GID_INVALID, MKDIR_IGNORE_EXISTING));
        ASSERT_OK_ERRNO(mkdir_safe(p, 0755, UID_INVALID, GID_INVALID, MKDIR_FOLLOW_SYMLINK));
        ASSERT_OK_ZERO(is_dir(p, false));
        ASSERT_OK_POSITIVE(is_dir(p, true));
        ...
        ASSERT_OK_ERRNO(mkdir_p_safe(tmp, q, 0755, UID_INVALID, GID_INVALID, 0));
        ...
        ASSERT_OK_ERRNO(mkdir_parents_safe(tmp, p, 0000, UID_INVALID, GID_INVALID, 0));
...
TEST(mkdir_p_root) {
        ...
        ASSERT_NOT_NULL(p = path_join(tmp, "run/aaa/bbb"));
        ASSERT_OK_ERRNO(mkdir_p_root(tmp, "/run/aaa/bbb", UID_INVALID, GID_INVALID, 0755));
        ASSERT_OK_POSITIVE(is_dir(p, false));
        ASSERT_OK_POSITIVE(is_dir(p, true));
        ...
        p = mfree(p);
        ASSERT_NOT_NULL(p = path_join(tmp, "var/run/hoge/foo/baz"));
        ASSERT_OK_ERRNO(mkdir_p_root(tmp, "/var/run/hoge/foo/baz", UID_INVALID, GID_INVALID, 0755));
        ASSERT_OK_POSITIVE(is_dir(p, false));
        ASSERT_OK_POSITIVE(is_dir(p, true));
