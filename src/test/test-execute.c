...
static int prepare_ns(const char *process_name) {
        ...
        if (r == 0) {
                ...
                FOREACH_STRING(p, "/dev/shm", "/tmp", "/var/tmp", "/var/lib", PRIVATE_UNIT_DIR)
                ...
                /* Overmount /root with tmpfs after we copied out the necessary test files */
                assert_se(mount_nofollow_verbose(LOG_DEBUG, "tmpfs", "/root", "tmpfs", MS_NOSUID|MS_NODEV, NULL) >= 0);
