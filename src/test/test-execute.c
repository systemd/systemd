...
static bool can_use_unprivileged_userns(void) {
        ...
        r = safe_fork("(sd-test-check-unpriv-userns)",
                      FORK_RESET_SIGNALS |
                      FORK_CLOSE_ALL_FDS |
        ...
        assert(r >= 0);
        if (r == 0) {
                r = capability_bounding_set_drop(0, /* right_now = */ true);
                if (r < 0) {
                ...
                if (r < 0 && !ERRNO_IS_NEG_PRIVILEGE(r))
                        log_debug_errno(r, "Failed to create unprivileged user namespace: %m");

                _exit(r >= 0 ? EXIT_SUCCESS : ERRNO_IS_NEG_PRIVILEGE(r) ? EXIT_FAILURE : 2);
        }

        /* The exit code records the result of the check:
         *  EXIT_SUCCESS => we can use unprivileged user namespaces
         *  EXIT_FAILURE => we can NOT use unprivileged user namespaces
         *  2            => some other error occurred */
        r = wait_for_terminate_and_check("(sd-test-check-unpriv-userns)", pid, 0);
        if (!IN_SET(r, EXIT_SUCCESS, EXIT_FAILURE))
                log_debug("Failed to check if unprivileged user namespaces can be used, assuming not.");
