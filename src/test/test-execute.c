...
static char *test_unit_dir = NULL;
static char *unit_dir = NULL;
static char *user_runtime_unit_dir = NULL;
static bool can_unshare;

STATIC_DESTRUCTOR_REGISTER(test_unit_dir, freep);
STATIC_DESTRUCTOR_REGISTER(unit_dir, rm_rf_physical_and_freep);
...
static int prepare_ns(const char *process_name) {
        ...
        if (r == 0) {
                ...
                /* Also mount tmpfs on the following directories to make not StateDirectory= or friends disturb the host. */
                FOREACH_STRING(p, "/var/lib", "/root")
                        assert_se(mount_nofollow_verbose(LOG_DEBUG, "tmpfs", p, "tmpfs", 0, NULL) >= 0);
        }
...
static int intro(void) {
        ...
        /* Copy unit files to make them accessible when unprivileged. */
        assert_se(get_testdata_dir("test-execute/", &test_unit_dir) >= 0);
        assert_se(mkdtemp_malloc("/run/test-execute.XXXXXXX", &unit_dir) >= 0);
        assert_se(copy_directory(test_unit_dir, unit_dir, COPY_MERGE_EMPTY) >= 0);
