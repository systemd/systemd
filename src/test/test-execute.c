...
static void test_exec_privatetmp(Manager *m) {
        ...
        if (MANAGER_IS_SYSTEM(m)) {
                /* Prepare /var/ without /var/tmp/, to simulate early boot of separated /var/ system. */
                ASSERT_OK(mount_nofollow_verbose(LOG_DEBUG, "tmpfs", "/var/", "tmpfs", MS_NOSUID|MS_NODEV, NULL));
                ASSERT_OK(touch("/var/service_cannot_see_this_file"));
                test(m, "exec-privatetmp-disconnected.service", can_unshare ? 0 : MANAGER_IS_SYSTEM(m) ? EXIT_FAILURE : EXIT_NAMESPACE, CLD_EXITED);
                /* Check if /var/ is not poluted by the service. */
                ASSERT_OK_ERRNO(unlink("/var/service_cannot_see_this_file"));
