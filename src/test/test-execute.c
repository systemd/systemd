...
STATIC_DESTRUCTOR_REGISTER(unit_dir, freep);
...
static int prepare_ns(const char *process_name) {
        ...
        if (r == 0) {
                ...
                /* Also mount /var/lib to make not StateDirectory= disturb the host. */
                assert_se(mount_nofollow_verbose(LOG_DEBUG, "tmpfs", "/var/lib", "tmpfs", 0, NULL) >= 0);

                /* Also mount /root to make not the unprivileged tests disturb the host. */
                assert_se(mount_nofollow_verbose(LOG_DEBUG, "tmpfs", "/root", "tmpfs", 0, NULL) >= 0);
        }
