...
static void write_etc_os_release(const char *root, const char *contents) {
        _cleanup_free_ char *p = ASSERT_NOT_NULL(path_join(root, "/etc/os-release"));
...
TEST(voa_os_identifiers) {
        ...
        /* /etc/os-release wins over /usr/lib/os-release, the initrd's own file decides there */
        write_etc_os_release(root, "ID=etcos\n");
        ASSERT_OK_ZERO(voa_os_identifiers(root_fd, &l));
        ASSERT_TRUE(strv_equal(l, STRV_MAKE("etcos")));
        l = strv_free(l);
        _cleanup_free_ char *etc = ASSERT_NOT_NULL(path_join(root, "/etc/os-release"));
        ASSERT_OK_ERRNO(unlink(etc));
...
static void set_mode(const char *root, const char *path, mode_t mode) {
        _cleanup_free_ char *p = NULL;
        ASSERT_NOT_NULL(p = path_join(root, path));
        ASSERT_OK_ERRNO(chmod(p, mode));
}

TEST(voa_list_verifiers_mode) {
        _cleanup_(rm_rf_physical_and_freep) char *root = NULL;
        _cleanup_strv_free_ char **l = NULL;
        _cleanup_close_ int root_fd = -EBADF;
        ConfFile **files = NULL;
        ...
        add_file(root, "/etc/voa/testos" X509_DIR "a-certificate.pem");
        add_file(root, "/run/voa/testos" X509_DIR "b-certificate.pem");
        add_file(root, "/usr/share/voa/testos" X509_DIR "c-certificate.pem");

        ASSERT_OK(voa_list_verifiers(root_fd, &lookup, VOA_WARN, &files, &n));
        l = file_paths(files, n);
        ASSERT_TRUE(strv_equal(l, STRV_MAKE("/etc/voa/testos" X509_DIR "a-certificate.pem",
                                            "/run/voa/testos" X509_DIR "b-certificate.pem",
                                            "/usr/share/voa/testos" X509_DIR "c-certificate.pem")));
        l = strv_free(l);
        files = files_free(files, &n);

        /* A file writable by others is dropped, such a directory hides its load path only */
        FOREACH_STRING(p,
                       "/run/voa/testos" X509_DIR "b-certificate.pem",
                       "/run/voa/testos" X509_DIR,
                       "/run/voa/testos/image",
                       "/run/voa") {
                set_mode(root, p, endswith(p, ".pem") ? 0666 : 0775);
                ASSERT_OK(voa_list_verifiers(root_fd, &lookup, VOA_WARN, &files, &n));
                l = file_paths(files, n);
                ASSERT_TRUE(strv_equal(l, STRV_MAKE("/etc/voa/testos" X509_DIR "a-certificate.pem",
                                                    "/usr/share/voa/testos" X509_DIR "c-certificate.pem")));
                l = strv_free(l);
                files = files_free(files, &n);
                set_mode(root, p, endswith(p, ".pem") ? 0644 : 0755);
        }

        /* An executable certificate is dropped as well */
        set_mode(root, "/run/voa/testos" X509_DIR "b-certificate.pem", 0755);
        ASSERT_OK(voa_list_verifiers(root_fd, &lookup, VOA_WARN, &files, &n));
        l = file_paths(files, n);
        ASSERT_TRUE(strv_equal(l, STRV_MAKE("/etc/voa/testos" X509_DIR "a-certificate.pem",
                                            "/usr/share/voa/testos" X509_DIR "c-certificate.pem")));
        ...
        /* Stricter modes and the special bits are fine */
        set_mode(root, "/run/voa/testos" X509_DIR "b-certificate.pem", 0600);
        set_mode(root, "/run/voa/testos" X509_DIR, 0700);
        set_mode(root, "/run/voa/testos/image", 02755);
        ASSERT_OK(voa_list_verifiers(root_fd, &lookup, VOA_WARN, &files, &n));
        ASSERT_EQ(n, 3u);
        files = files_free(files, &n);
}
...
static int intro(void) {
        /* The hierarchy is expected to be 0755 directories and 0644 files */
        umask(0022);
        return EXIT_SUCCESS;
}
