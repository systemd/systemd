...
TEST(iovw_merge_and_iovec_split) {
        ...
        *p++ = 0;
        ...
        /* truncated */
        ASSERT_OK(iovw_merge(&iovw, sizeof(uint8_t), &v));
        ASSERT_ERROR(iovec_split(&IOVEC_MAKE(v.iov_base, v.iov_len - 1), sizeof(uint8_t), &iovw2), EINVAL);
        ...
        ASSERT_ERROR(iovw_merge(&(struct iovec_wrapper) { .iovec = &large, .count = 1, }, sizeof(uint8_t), &v), EINVAL);
        ...
        ASSERT_OK(iovec_split(&IOVEC_MAKE_STRING(""), sizeof(uint8_t), &iovw2));
        ASSERT_TRUE(iovw_isempty(&iovw2));
