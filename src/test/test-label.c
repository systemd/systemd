...
static int pre_labelling_func(int dir_fd, const char *path, mode_t mode) {
        ...
        pre_fd = RET_NERRNO(openat(dir_fd, path, O_CLOEXEC|O_CREAT, 0644));
        if (pre_fd < 0)
                return log_error_errno(r, "Error occured while opening file in directory =>: %m");
...
static int labelling_op(int dir_fd, const char *text, const char *path, mode_t mode) {
        ...
        write_fd = RET_NERRNO(openat(dir_fd, path, O_CLOEXEC|O_WRONLY|O_TRUNC, 0644));
        if (write_fd < 0)
...
TEST(label_ops_pre) {
        ...
        fd = get_dir_fd("label_test_dir", 0755);

        assert_se(label_ops_pre(fd, "file1.txt", 0644) == 0);
        safe_close(pfd);
        assert_se(label_ops_pre(fd, "file2.txt", 0644) == 0);
        safe_close(pfd);
...
TEST(label_ops_post) {
        ...
        label_ops_pre(fd, "file1.txt", 0644);
        ...
        label_ops_pre(fd, "file2.txt", 0644);
        labelling_op(fd, text2, "file2.txt", 0644);
        ...
        safe_close(pfd); /* Close the file safely */
}
