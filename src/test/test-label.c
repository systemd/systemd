...
static int pre_labelling_func(int dir_fd, const char *path, mode_t mode, LabelContext *label_userdata) {
        int r;
...
static int post_labelling_func(int dir_fd, const char *path, bool created, LabelContext *label_userdata) {
       int r;
...
TEST(label_ops_pre) {
        ...
        assert_se(label_ops_pre(fd, "file1.txt", 0644, /* userdata= */ NULL) == 0);
        assert_se(label_ops_pre(fd, "/restricted_directory", 0644, /* userdata= */ NULL) == -EACCES);
        assert_se(label_ops_pre(fd, "", 0700, /* userdata= */ NULL) == -EINVAL);
        assert_se(label_ops_pre(fd, "/tmp", 0700, /* userdata= */ NULL) == 0);
        assert_se(label_ops_pre(fd, "wekrgoierhgoierhqgherhgwklegnlweehgorwfkryrit", 0644, /* userdata= */ NULL) == -ENAMETOOLONG);
}
...
TEST(label_ops_post) {
        ...
        assert_se(label_ops_post(fd, "file1.txt", true, /* userdata= */ NULL) == 0);
        assert_se(strlen(text1) == (size_t)buf.st_size);
        text2 = "Add text2 data to file2\n";

        assert_se(labelling_op(fd, text2, "file2.txt", 0644) == 0);
        assert_se(label_ops_post(fd, "file2.txt", true, /* userdata= */ NULL) == 0);
        assert_se(strlen(text2) == (size_t)buf.st_size);
        assert_se(label_ops_post(fd, "file3.txt", true, /* userdata= */ NULL) == -ENOENT);
        assert_se(label_ops_post(fd, "/abcd", true, /* userdata= */ NULL) == -ENOENT);
        assert_se(label_ops_post(fd, "/restricted_directory", true, /* userdata= */ NULL) == -EACCES);
        assert_se(label_ops_post(fd, "", true, /* userdata= */ NULL) == -EINVAL);
}
