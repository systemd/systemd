...
TEST(mstack_plan_after_merge_volatile) {
         ...
         * backing on disk yet - which is exactly what needs_tmpfs_backing is there to remember. */
        {
                _cleanup_(mstack_freep) MStack *mstack = NULL;
                ASSERT_OK(mstack_load(t, tfd, &mstack));
                ...
                const MStackLayerPlan *upper = ASSERT_PTR(plan_find(plan, MSTACK_RW));
                ASSERT_EQ(upper->role, MSTACK_LAYER_ROLE_UPPER);
                ASSERT_TRUE(upper->needs_tmpfs_backing);
                ...
                ASSERT_FALSE(lower->needs_tmpfs_backing);
        }
