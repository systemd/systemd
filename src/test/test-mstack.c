...
TEST(mstack_merge_volatile) {
...

        {
                ...
                /* Unlike the other privileged blocks this one runs in the test process's own mount
                 * namespace, so the root and its /usr/ submount would otherwise stay behind: rmdir_and_freep()
                 * would fail with EBUSY and the mounts would leak into whatever runs next. */
                ASSERT_OK(umount_recursive(w, /* flags= */ 0));
        }
