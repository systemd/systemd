...
static int pin_programs(Unit *u, CGroupContext *cc, const Test *test_suite, size_t test_suite_size, char ***paths_ret) {
        ...
        for (size_t i = 0; i < test_suite_size; i++) {
                ...
                r = bpffs_program_to_string(test_suite[i].attach_type, test_suite[i].bpffs_path, &str);
                if (r < 0)
...
static int test_bpf_cgroup_programs(Manager *m, const char *unit_name, const Test *test_suite, size_t test_suite_size) {
        ...
        for (size_t i = 0; i < test_suite_size; i++) {
                ...
                if (streq(test_suite[i].option_name, "BPFProgram")) {
                        r = bpffs_program_to_string(test_suite[i].attach_type, test_suite[i].bpffs_path, &option);
                        if (r < 0)
                ...
                r = config_parse_bpffs_programs(u->id, "filename", 1, "Service", 1, test_suite[i].option_name, 0, option, cc, u);
                if (r < 0)
