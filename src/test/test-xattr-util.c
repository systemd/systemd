...
static void xattr_symlink_test_one(int fd, const char *path) {
        ...
        ASSERT_OK_EQ(getxattr_at_malloc(fd, path, "trusted.test", 0, &value), STRLEN("schaffen"));
        ASSERT_STREQ(value, "schaffen");

        ASSERT_OK_EQ(listxattr_at_malloc(fd, path, 0, &list), sizeof("schaffen"));
        ASSERT_STREQ(list, "schaffen");

        ASSERT_OK(xremovexattr(fd, path, 0, "trusted.test"));
        ASSERT_ERROR(getxattr_at_malloc(dfd, "symlink", "trusted.test", 0, &value), ENODATA);
}
