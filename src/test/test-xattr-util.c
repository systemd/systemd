...
TEST(getxattr_at_malloc) {
        ...
        ASSERT_OK(getxattr_at_malloc(fd, "test", "user.foo", /* at_flags= */ 0, &value, /* ret_size= */ NULL));
        ASSERT_EQ(memcmp(value, "bar", 3), 0);
        value = mfree(value);

        ASSERT_OK(getxattr_at_malloc(AT_FDCWD, x, "user.foo", /* at_flags= */ 0, &value, /* ret_size= */ NULL));
        ASSERT_EQ(memcmp(value, "bar", 3), 0);
        value = mfree(value);

        safe_close(fd);
        ASSERT_OK_ERRNO(fd = open("/", O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOCTTY));
        r = getxattr_at_malloc(fd, "usr", "user.idontexist", /* at_flags= */ 0, &value, /* ret_size= */ NULL);
        ...
        r = getxattr_at_malloc(fd, "this-path-does-not-exist-XXXXXX", "user.idontexist",
                               /* at_flags= */ 0, &value, /* ret_size= */ NULL);
        ASSERT_TRUE(ERRNO_IS_NEG_XATTR_ABSENT(r));

        safe_close(fd);
        ASSERT_OK_ERRNO(fd = open(x, O_PATH|O_CLOEXEC));
        ASSERT_OK(getxattr_at_malloc(fd, /* path= */ NULL, "user.foo", /* at_flags= */ 0, &value, /* ret_size= */ NULL));
        ASSERT_STREQ(value, "bar");
        value = mfree(value);

        ASSERT_OK_ERRNO(setxattr(x, "user.foozu", "bar\0qux\0wal\0\0", 13, /* flags= */ 0));
        ASSERT_ERROR(getxattr_at_malloc(fd, /* path= */ NULL, "user.foozu", /* at_flags= */ 0, &value, /* ret_size= */ NULL), EBADMSG);
        size_t value_size;
        ASSERT_OK(getxattr_at_malloc(fd, /* path= */ NULL, "user.foozu", /* at_flags= */ 0, &value, &value_size));
...
TEST(getcrtime) {
        ...
        ASSERT_OK(fd = mkdtemp_open("/var/tmp/test-xattrtestXXXXXX", /* flags= */ 0, &t));
...
static void verify_xattr(int dfd, const char *expected) {
        ...
        ASSERT_OK(getxattr_at_malloc(dfd, "test", "user.foo", /* at_flags= */ 0, &value, /* ret_size= */ NULL));
        ASSERT_STREQ(value, expected);
        value = mfree(value);

        size_t size;
        ASSERT_OK(getxattr_at_malloc(dfd, "test", "user.foo", /* at_flags= */ 0, &value, &size));
...
static void xattr_symlink_test_one(int fd, const char *path) {
        ...
        ASSERT_ERROR(xsetxattr_full(fd, path, /* at_flags= */ 0, "trusted.bar", "bogus", SIZE_MAX, XATTR_CREATE), EEXIST);

        ASSERT_OK(xsetxattr(fd, path, /* at_flags= */ 0, "trusted.test", "schaffen"));
        ASSERT_OK(getxattr_at_malloc(fd, path, "trusted.test", /* at_flags= */ 0, &value, /* ret_size= */ NULL));
        ASSERT_STREQ(value, "schaffen");
        value = mfree(value);
        size_t size;
        ASSERT_OK(getxattr_at_malloc(fd, path, "trusted.test", /* at_flags= */ 0, &value, &size));
        ASSERT_EQ(size, strlen(value));
        ASSERT_STREQ(value, "schaffen");

        r = listxattr_at_malloc(fd, path, /* at_flags= */ 0, &list);
        ASSERT_OK(r);
        ...
        ASSERT_OK(xremovexattr(fd, path, /* at_flags= */ 0, "trusted.test"));
        ASSERT_ERROR(getxattr_at_malloc(fd, path, "trusted.test", /* at_flags= */ 0, &value, /* ret_size= */ NULL), ENODATA);
}
...
TEST(xsetxattr) {
        ...
        /* by full path */
        r = xsetxattr(AT_FDCWD, x, /* at_flags= */ 0, "user.foo", "fullpath");
        ...
        /* by dirfd */
        ASSERT_ERROR(xsetxattr_full(dfd, "test", /* at_flags= */ 0, "user.foo", "dirfd", SIZE_MAX, XATTR_CREATE), EEXIST);
        verify_xattr(dfd, "fullpath");

        ASSERT_OK(xsetxattr_full(dfd, "test", /* at_flags= */ 0, "user.foo", "dirfd", SIZE_MAX, XATTR_REPLACE));
        verify_xattr(dfd, "dirfd");

        /* by fd (O_PATH) */
        ASSERT_OK_ERRNO(fd = openat(dfd, "test", O_PATH|O_CLOEXEC));

        ASSERT_OK(xremovexattr(fd, "", /* at_flags= */ 0, "user.foo"));

        ASSERT_OK(xsetxattr_full(fd, /* path= */ NULL, AT_EMPTY_PATH, "user.foo", "fd_opath", SIZE_MAX, XATTR_CREATE));
        verify_xattr(dfd, "fd_opath");

        ASSERT_OK(xsetxattr(fd, "", /* at_flags= */ 0, "user.foo", "fd_opath_empty"));
        ...
        ASSERT_OK(xsetxattr_full(fd, /* path= */ NULL, /* at_flags= */ 0, "user.foo", "fd_regular", SIZE_MAX, XATTR_REPLACE));
        verify_xattr(dfd, "fd_regular");

        ASSERT_OK(xsetxattr(fd, "", /* at_flags= */ 0, "user.foo", "fd_regular_empty"));
        ...
        r = xsetxattr_full(dfd, "symlink", /* at_flags= */ 0, "trusted.bar", "baz", SIZE_MAX, XATTR_CREATE);
        if (ERRNO_IS_NEG_PRIVILEGE(r))
                return (void) log_tests_skipped_errno(r, "Unable to set trusted.* xattr");
        ASSERT_OK(r);

        xattr_symlink_test_one(dfd, "symlink");
        xattr_symlink_test_one(fd, /* path= */ NULL);
