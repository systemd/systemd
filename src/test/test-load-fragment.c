...
static void check_execcommand(ExecCommand *c,
        ...
        ASSERT_EQ(!!(c->flags & EXEC_COMMAND_IGNORE_FAILURE), ignore);
}
...
TEST(install_printf, .sd_booted = true) {
        ...
        ASSERT_OK_ERRNO(asprintf(&gid, UID_FMT, getgid()));
        ASSERT_NOT_NULL(user = uid_to_name(getuid()));
        ASSERT_OK_ERRNO(asprintf(&uid, UID_FMT, getuid()));
