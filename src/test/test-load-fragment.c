...
TEST(config_parse_open_file) {
        ...
        assert_se(tail->flags == O_RDWR);

        r = config_parse_open_file(NULL, "fake", 1, "section", 1,
                                   "OpenFile", 0, "/path/%n.conf:host-mount-namespace:ro",
                                   &head, u);
        assert_se(r >= 0);
        LIST_FIND_TAIL(open_files, head, tail);
        assert_se(tail != NULL);
        assert_se(streq(tail->path, "/path/foobar.service.conf"));
        assert_se(streq(tail->fdname, "host-mount-namespace"));
        assert_se(tail->flags == O_RDONLY);
