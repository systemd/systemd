...
TEST(local_addresses) {
        ...
        ASSERT_OK(n = local_addresses(/* context= */ NULL, /* ifindex= */ 0, AF_INET, &a));
        log_debug("/* Local Addresses(ifindex:0, AF_INET) */");
        print_local_addresses(a, n);
        a = mfree(a);

        ASSERT_OK(n = local_addresses(/* context= */ NULL, /* ifindex= */ 0, AF_INET6, &a));
        log_debug("/* Local Addresses(ifindex:0, AF_INET6) */");
        print_local_addresses(a, n);
        a = mfree(a);

        ASSERT_OK(n = local_addresses(/* context= */ NULL, /* ifindex= */ 0, AF_UNSPEC, &a));
        log_debug("/* Local Addresses(ifindex:0, AF_UNSPEC) */");
        print_local_addresses(a, n);
        a = mfree(a);

        ASSERT_OK(n = local_addresses(/* context= */ NULL, 1, AF_INET, &a));
        log_debug("/* Local Addresses(ifindex:1, AF_INET) */");
        print_local_addresses(a, n);
        a = mfree(a);

        ASSERT_OK(n = local_addresses(/* context= */ NULL, 1, AF_INET6, &a));
        log_debug("/* Local Addresses(ifindex:1, AF_INET6) */");
        print_local_addresses(a, n);
        a = mfree(a);

        ASSERT_OK(n = local_addresses(/* context= */ NULL, 1, AF_UNSPEC, &a));
        log_debug("/* Local Addresses(ifindex:1, AF_UNSPEC) */");
        print_local_addresses(a, n);
        a = mfree(a);

        ASSERT_OK(n = local_gateways(/* context= */ NULL, /* ifindex= */ 0, AF_UNSPEC, &a));
        log_debug("/* Local Gateways */");
        print_local_addresses(a, n);
        a = mfree(a);

        ASSERT_OK(n = local_outbounds(/* context= */ NULL, /* ifindex= */ 0, AF_UNSPEC, &a));
...
TEST(local_addresses_with_dummy) {
        ...
        ASSERT_OK(sd_netlink_message_append_string(message, IFLA_IFNAME, "test-local-addr"));
        ASSERT_OK(sd_netlink_call(rtnl, message, /* timeout= */ 0, &reply));
        ...
        ASSERT_OK(sysctl_write_ip_property_boolean(AF_INET6, "test-local-addr", "disable_ipv6", /* value= */ false, /* shadow= */ NULL));
        ...
        ASSERT_OK(sd_netlink_call(rtnl, message, /* timeout= */ 0, /* ret= */ NULL));
        message = sd_netlink_message_unref(message);
        ...
        /* Check */
        check_local_addresses(rtnl, ifindex, /* request_ifindex= */ 0, AF_UNSPEC);
        check_local_addresses(rtnl, ifindex, /* request_ifindex= */ 0, AF_INET);
        check_local_addresses(rtnl, ifindex, /* request_ifindex= */ 0, AF_INET6);
        ...
        check_local_gateways(rtnl, ifindex, /* request_ifindex= */ 0, AF_UNSPEC);
        check_local_gateways(rtnl, ifindex, /* request_ifindex= */ 0, AF_INET);
        check_local_gateways(rtnl, ifindex, /* request_ifindex= */ 0, AF_INET6);
        ...
        check_local_outbounds(rtnl, ifindex, /* request_ifindex= */ 0, AF_UNSPEC, "2001:db8:1:123::123");
        check_local_outbounds(rtnl, ifindex, /* request_ifindex= */ 0, AF_INET, "2001:db8:1:123::123");
        check_local_outbounds(rtnl, ifindex, /* request_ifindex= */ 0, AF_INET6, "2001:db8:1:123::123");
        ...
        /* Check again. */
        check_local_outbounds(rtnl, ifindex, /* request_ifindex= */ 0, AF_UNSPEC, "2001:db8:1:123::124");
        check_local_outbounds(rtnl, ifindex, /* request_ifindex= */ 0, AF_INET, "2001:db8:1:123::124");
        check_local_outbounds(rtnl, ifindex, /* request_ifindex= */ 0, AF_INET6, "2001:db8:1:123::124");
