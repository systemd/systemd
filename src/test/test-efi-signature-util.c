...
static void* make_authenticated_update(const void *payload, size_t payload_size, size_t *ret_size) {
        ...
        unaligned_write_le16(data + offsetof(EFI_VARIABLE_AUTHENTICATION_2, AuthInfo.Hdr.wRevision), 0x0200);
        unaligned_write_le16(data + offsetof(EFI_VARIABLE_AUTHENTICATION_2, AuthInfo.Hdr.wCertificateType), 0x0ef1);
...
TEST(signature_list_validation) {
        ...
        ASSERT_OK(efi_signature_list_new(&sha256, &owner, digest, sizeof(digest), &generated));
        ASSERT_OK(efi_signature_list_validate(generated.iov_base, generated.iov_len));
        ASSERT_OK_EQ(efi_signature_list_next(generated.iov_base, generated.iov_len, &offset, &view), 1);
        ASSERT_EQ(view.entries_size, sha256_entry_size);
        ASSERT_EQ(memcmp(view.entries, &owner, sizeof(owner)), 0);
        ASSERT_EQ(memcmp(view.entries + offsetof(EFI_SIGNATURE_DATA, SignatureData), digest, sizeof(digest)), 0);

        esl = make_esl(sha256, /* header_size= */ 0, sha256_entry_size, /* n_entries= */ 2, &esl_size);
        offset = 0;
        ASSERT_OK(efi_signature_list_validate(esl, esl_size));
        ASSERT_OK_EQ(efi_signature_list_next(esl, esl_size, &offset, &view), 1);
        ASSERT_EQ(view.header.signature_size, sha256_entry_size);
        ASSERT_EQ(view.entries_size, 2U * sha256_entry_size);
        ASSERT_OK_EQ(efi_signature_list_next(esl, esl_size, &offset, &view), 0);
        ASSERT_OK(efi_signature_list_validate(NULL, 0));

        ASSERT_ERROR(efi_signature_list_validate(esl, offsetof(EFI_SIGNATURE_LIST, Signatures) - 1), EBADMSG);
        ...
        ASSERT_ERROR(efi_signature_list_validate(bad, esl_size), EBADMSG);
        ...
        _cleanup_free_ void *short_sha256 = make_esl(
                        ...
                        /* n_entries= */ 1,
                        &offset);
        ASSERT_ERROR(efi_signature_list_validate(short_sha256, offset), EBADMSG);
        ...
        ASSERT_ERROR(efi_signature_list_validate(x509_esl, x509_esl_size), EBADMSG);
}
...
TEST(signature_database) {
        ...
        database = make_esl(sha256, /* header_size= */ 0, signature_size, /* n_entries= */ 2, &database_size);
        subset = make_esl(sha256, /* header_size= */ 0, signature_size, /* n_entries= */ 1, &subset_size);

        ASSERT_OK(efi_signature_database_index_new(database, database_size, &index, &n_index));
        ASSERT_EQ(n_index, 2U);
        ASSERT_OK_EQ(efi_signature_list_next(subset, subset_size, &offset, &view), 1);
        ASSERT_TRUE(efi_signature_database_index_contains(
                            index, n_index, &view.header.type, view.header.signature_size, view.entries));
        ASSERT_NOT_NULL(efi_signature_database_index_find_by_data(
                                index, n_index, view.header.signature_size, view.entries));

        ASSERT_OK_EQ(efi_signature_database_is_superset(database, database_size, subset, subset_size), 1);
        ASSERT_OK_EQ(efi_signature_database_is_superset(subset, subset_size, database, database_size), 0);
        ASSERT_OK_EQ(efi_signature_database_is_superset(database, database_size, NULL, 0), 1);
