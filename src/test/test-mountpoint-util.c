...
TEST(fd_is_mount_point) {
        ...
        assert_se(fd_is_mount_point(fd, "", AT_EMPTY_PATH) > 0);
        assert_se(fd_is_mount_point(fd, "", 0) == -EINVAL);
        ...
        assert_se(r == 0 || r == -ENOTDIR); /* on old kernels we can't determine if regular files are mount points if we have no directory fd */
        r = fd_is_mount_point(fd, "", AT_EMPTY_PATH);
        assert_se(r == 0 || r == -ENOTDIR);
        assert_se(fd_is_mount_point(fd, "", 0) == -EINVAL);
}
