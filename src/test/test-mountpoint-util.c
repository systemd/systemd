...
TEST(mount_option_supported) {
        ...
        log_info("tmpfs supports size=64M: %s", r < 0 ? "dont know" : yes_no(r));
        assert_se(r > 0 || (r < 0 && ERRNO_IS_PRIVILEGE(r)));

        r = mount_option_supported("ext4", "discard", NULL);
        log_info("ext4 supports discard: %s", r < 0 ? "dont know" : yes_no(r));
        assert_se(r > 0 || (r < 0 && ERRNO_IS_PRIVILEGE(r)));

        r = mount_option_supported("tmpfs", "idontexist", "64M");
        log_info("tmpfs supports idontexist: %s", r < 0 ? "dont know" : yes_no(r));
        assert_se(r == 0 || (r < 0 && ERRNO_IS_PRIVILEGE(r)));

        r = mount_option_supported("tmpfs", "ialsodontexist", NULL);
        log_info("tmpfs supports ialsodontexist: %s", r < 0 ? "dont know" : yes_no(r));
        assert_se(r == 0 || (r < 0 && ERRNO_IS_PRIVILEGE(r)));

        r = mount_option_supported("proc", "hidepid", "1");
        log_info("proc supports hidepid=1: %s", r < 0 ? "dont know" : yes_no(r));
        assert_se(r >= 0 || (r < 0 && ERRNO_IS_PRIVILEGE(r)));
}
