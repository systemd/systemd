...
TEST(initrd_cpio_credentials_basic) {
        struct stat st;
        size_t foo_size, bar_size;
        _cleanup_free_ char *cmd = NULL, *creds_path = NULL, *foo_path = NULL, *bar_path = NULL, *foo_content = NULL, *bar_content = NULL;
        ...
        ASSERT_NOT_NULL(creds_path = path_join(extract_dir, ".extra/credentials"));
        ASSERT_OK_ERRNO(stat(creds_path, &st));
        assert_se(S_ISDIR(st.st_mode));
        ASSERT_EQ((mode_t)(st.st_mode & 07777), (mode_t)0500);

        ASSERT_NOT_NULL(foo_path = path_join(extract_dir, ".extra/credentials/foo.cred"));
        ...
        assert_se(memcmp(foo_content, "hello", 5) == 0);

        ASSERT_NOT_NULL(bar_path = path_join(extract_dir, ".extra/credentials/bar.cred"));
        ASSERT_OK_ERRNO(stat(bar_path, &st));
        assert_se(S_ISREG(st.st_mode));
        ASSERT_EQ((mode_t)(st.st_mode & 07777), (mode_t)0400);
        ASSERT_OK(read_full_file(bar_path, &bar_content, &bar_size));
        ASSERT_EQ(bar_size, 7U);
        assert_se(memcmp(bar_content, "abc\0def", 7) == 0);
}
