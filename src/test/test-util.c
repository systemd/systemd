...
static void test_unhexmem(void) {
        ...
        assert_se(unhexmem(&mem, &len, hex, strlen(hex)) == 0);
        assert_se(unhexmem(&mem, &len, hex, strlen(hex) + 1) == -EINVAL);
        assert_se(unhexmem(&mem, &len, hex_invalid, strlen(hex_invalid)) == -EINVAL);
        ...
        assert_se(unhexmem(&mem, &len, hex, strlen(hex) - 1) == 0);
        assert_se((hex2 = hexmem(mem, len)));
...
static void test_unbase64mem(void) {
        ...
        assert_se(unbase64mem(&mem, &len, "", strlen("")) == 0);
        assert_se(streq(strndupa(mem, len), ""));
        free(mem);

        assert_se(unbase64mem(&mem, &len, "Zg==", strlen("Zg==")) == 0);
        assert_se(streq(strndupa(mem, len), "f"));
        free(mem);

        assert_se(unbase64mem(&mem, &len, "Zm8=", strlen("Zm8=")) == 0);
        assert_se(streq(strndupa(mem, len), "fo"));
        free(mem);

        assert_se(unbase64mem(&mem, &len, "Zm9v", strlen("Zm9v")) == 0);
        assert_se(streq(strndupa(mem, len), "foo"));
        free(mem);

        assert_se(unbase64mem(&mem, &len, "Zm9vYg==", strlen("Zm9vYg==")) == 0);
        assert_se(streq(strndupa(mem, len), "foob"));
        free(mem);

        assert_se(unbase64mem(&mem, &len, "Zm9vYmE=", strlen("Zm9vYmE=")) == 0);
        assert_se(streq(strndupa(mem, len), "fooba"));
        free(mem);

        assert_se(unbase64mem(&mem, &len, "Zm9vYmFy", strlen("Zm9vYmFy")) == 0);
        assert_se(streq(strndupa(mem, len), "foobar"));
        free(mem);

        assert_se(unbase64mem(&mem, &len, "A", strlen("A")) == -EINVAL);
        assert_se(unbase64mem(&mem, &len, "A====", strlen("A====")) == -EINVAL);
        assert_se(unbase64mem(&mem, &len, "AAB==", strlen("AAB==")) == -EINVAL);
        assert_se(unbase64mem(&mem, &len, "AAAB=", strlen("AAAB=")) == -EINVAL);
}
