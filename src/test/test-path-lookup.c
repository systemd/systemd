...
static void test_paths_empty_components_one(RuntimeScope scope) {
        ...
        assert_se(setenv("SYSTEMD_UNIT_PATH", "", 1) == 0);
...
static void test_generator_binary_paths_one(RuntimeScope scope) {
        ...
        assert_se(setenv("SYSTEMD_GENERATOR_PATH", strjoina(":", systemd_generator_path), 1) == 0);
        assert_se(setenv("SYSTEMD_ENVIRONMENT_GENERATOR_PATH", strjoina(":", systemd_env_generator_path), 1) == 0);
