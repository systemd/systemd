...
TEST(iszero_safe) {
        ...
        /* On 32bit x86, -mfpmath=387 is the default, and floating point values are calculated in 80bit, and
         * truncated to the length of the used type (double in this case). Hence, DBL_MAX * 2 is temporary
         * calculated as a normal value, and goes to zero after divided with INFINITY. See issue #25044. */
        log_debug("i386 architecture is detected. Skipping \"assert_se(!iszero_safe(DBL_MAX * 2 / INFINITY))\".");
#else
