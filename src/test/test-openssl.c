...
TEST(rsa_encrypt_oaep) {
        ...
        assert_se(rsa_pkey_new(2048, &pkey) >= 0);
        ...
        if (r > 0) {
                ...
                assert_se(plain2);
                assert_se(EVP_PKEY_decrypt(ctx, plain2, &plain2_size, cipher, cipher_size) > 0);
                assert_se(plain2_size != sizeof(plain) || memcmp(plain, plain2, sizeof(plain)) != 0);
        } else
                assert_se(r <= 0);
}
