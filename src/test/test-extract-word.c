...
TEST(extract_first_word) {
        ...
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, /* flags= */ 0) > 0);
        ...
        p = original = "\"foobar\" \'waldo\'";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE) > 0);
        ASSERT_STREQ(t, "foobar");
        free(t);
        assert_se(p == original + 9);
        ...
        p = original = "\"";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, /* flags= */ 0) == 1);
        ASSERT_STREQ(t, "\"");
        free(t);
        assert_se(isempty(p));

        p = original = "\"";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE) == -EINVAL);
        ...
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE) == 1);
        ...
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_RETAIN_ESCAPE) == 1);
        ASSERT_STREQ(t, "KEY=val");
        free(t);
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_RETAIN_ESCAPE) == 1);
        ...
        p = original = "\'fooo";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE|EXTRACT_RELAX) > 0);
        ASSERT_STREQ(t, "fooo");
        free(t);
        assert_se(isempty(p));
        ...
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_CUNESCAPE) > 0);
        ...
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE|EXTRACT_CUNESCAPE) > 0);
        ASSERT_STREQ(t, "pi\360\237\222\251le");
        free(t);
        assert_se(isempty(p));

        p = original = "fooo\\";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_RELAX) > 0);
        ASSERT_STREQ(t, "fooo");
        free(t);
        assert_se(isempty(p));

        p = original = "fooo\\";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNESCAPE_RELAX) > 0);
        ASSERT_STREQ(t, "fooo\\");
        free(t);
        assert_se(isempty(p));

        p = original = "fooo\\";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNESCAPE_RELAX|EXTRACT_RELAX) > 0);
        ASSERT_STREQ(t, "fooo\\");
        free(t);
        assert_se(isempty(p));

        p = original = "fooo\\";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_CUNESCAPE|EXTRACT_UNESCAPE_RELAX) > 0);
        ASSERT_STREQ(t, "fooo\\");
        free(t);
        assert_se(isempty(p));

        p = original = "\"foo\\";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, /* flags= */ 0) == -EINVAL);
        assert_se(p == original + 5);
        ...
        p = original = "foo::bar";
        assert_se(extract_first_word(&p, &t, ":", /* flags= */ 0) == 1);
        ASSERT_STREQ(t, "foo");
        free(t);
        assert_se(p == original + 5);
        ...
        assert_se(extract_first_word(&p, &t, ":", /* flags= */ 0) == 0);
        assert_se(!t);
        assert_se(isempty(p));
        ...
        p = original = "\"foo\\";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE|EXTRACT_UNESCAPE_RELAX) == -EINVAL);
        assert_se(p == original + 5);

        p = original = "\"foo\\";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE|EXTRACT_UNESCAPE_RELAX|EXTRACT_RELAX) > 0);
        ASSERT_STREQ(t, "foo\\");
        free(t);
        assert_se(isempty(p));

        p = original = "\"foo\\";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE|EXTRACT_CUNESCAPE|EXTRACT_UNESCAPE_RELAX|EXTRACT_RELAX) > 0);
        ASSERT_STREQ(t, "foo\\");
        free(t);
        assert_se(isempty(p));
        ...
        p = original = "fooo\\ bar quux";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_CUNESCAPE) == -EINVAL);
        assert_se(p == original + 5);
        ...
        p = "foo\\xbar";
        assert_se(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_RETAIN_ESCAPE) > 0);
        ...
        ASSERT_OK_POSITIVE(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE));
        ...
        p = "test1@foo\\x2dbar\\x2dbaz.service test2@aaa\\x2dbbb\\x2dccc.service test3@escaped-path-like-data.service test4@/pure/path/like/data.service";
        ASSERT_OK_POSITIVE(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE | EXTRACT_RETAIN_ESCAPE));
        ...
        p = "test1@foo\\x2dbar\\x2dbaz.service test2@aaa\\x2dbbb\\x2dccc.service test3@escaped-path-like-data.service test4@/pure/path/like/data.service";
        ASSERT_OK_POSITIVE(extract_first_word(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE | EXTRACT_CUNESCAPE));
...
TEST(extract_first_word_and_warn) {
        ...
        p = original = "foobar waldo";
        assert_se(extract_first_word_and_warn(&p, &t, /* separators= */ NULL, /* flags= */ 0, /* unit= */ NULL, "fake", 1, original) > 0);
        ASSERT_STREQ(t, "foobar");
        free(t);
        assert_se(p == original + 7);
        ...
        p = original = "\"foobar\" \'waldo\'";
        assert_se(extract_first_word_and_warn(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE, /* unit= */ NULL, "fake", 1, original) > 0);
        ASSERT_STREQ(t, "foobar");
        free(t);
        assert_se(p == original + 9);
        ...
        p = original = "\"";
        assert_se(extract_first_word_and_warn(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE, /* unit= */ NULL, "fake", 1, original) == -EINVAL);
        assert_se(p == original + 1);
        ...
        p = original = "\'fooo";
        assert_se(extract_first_word_and_warn(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE|EXTRACT_RELAX, /* unit= */ NULL, "fake", 1, original) > 0);
        ASSERT_STREQ(t, "fooo");
        free(t);
        assert_se(isempty(p));

        p = original = " foo\\ba\\x6ar ";
        assert_se(extract_first_word_and_warn(&p, &t, /* separators= */ NULL, EXTRACT_CUNESCAPE, /* unit= */ NULL, "fake", 1, original) > 0);
        ASSERT_STREQ(t, "foo\ba\x6ar");
        free(t);
        assert_se(isempty(p));
        ...
        assert_se(extract_first_word_and_warn(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE|EXTRACT_CUNESCAPE, /* unit= */ NULL, "fake", 1, original) > 0);
        ASSERT_STREQ(t, "pi\360\237\222\251le");
        free(t);
        assert_se(isempty(p));

        p = original = "fooo\\";
        assert_se(extract_first_word_and_warn(&p, &t, /* separators= */ NULL, EXTRACT_RELAX, /* unit= */ NULL, "fake", 1, original) > 0);
        ASSERT_STREQ(t, "fooo");
        free(t);
        assert_se(isempty(p));
        ...
        p = original = "\"foo\\";
        assert_se(extract_first_word_and_warn(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE|EXTRACT_CUNESCAPE, /* unit= */ NULL, "fake", 1, original) == -EINVAL);
        assert_se(p == original + 5);

        p = original = "\"foo\\";
        assert_se(extract_first_word_and_warn(&p, &t, /* separators= */ NULL, EXTRACT_UNQUOTE|EXTRACT_CUNESCAPE|EXTRACT_RELAX, /* unit= */ NULL, "fake", 1, original) > 0);
        ASSERT_STREQ(t, "foo");
        free(t);
        assert_se(isempty(p));
