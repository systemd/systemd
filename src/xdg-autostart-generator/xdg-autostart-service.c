        ...
        if (service->gnome_autostart_phase) {
                ...
                if (strv_contains(only_show_in, "GNOME")) {
                        ...
                        if (strv_isempty(only_show_in)) {
                                log_debug("%s: Gnome startup phases are handled separately on GNOME. Skipping", service->path);
                                return 0;
                        }
                }
                log_debug("%s: Gnome startup phases are handled seprately on GNOME, marking as NotShowIn=GNOME", service->path);
                strv_extend(&not_show_in, "GNOME");
        }
