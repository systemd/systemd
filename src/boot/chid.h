...
static inline const char* device_get_name(const void *base, const Device *device, uint32_t descriptor) {
        if (descriptor == DEVICE_DESCRIPTOR_DEVICETREE) {
                return device->devicetree.name_offset == 0 ? NULL : (const char *) ((const uint8_t *) base + device->devicetree.name_offset);
        } else if (descriptor == DEVICE_DESCRIPTOR_FW) {
                return device->fw.name_offset == 0 ? NULL : (const char *) ((const uint8_t *) base + device->fw.name_offset);
        } else {
                return NULL;
        }
}

static inline const char* device_get_compatible(const void *base, const Device *device, uint32_t descriptor) {
        if (descriptor == DEVICE_DESCRIPTOR_DEVICETREE) {
                return device->devicetree.compatible_offset == 0 ? NULL : (const char *) ((const uint8_t *) base + device->devicetree.compatible_offset);
        } else if (descriptor == DEVICE_DESCRIPTOR_FW) {
                return device->fw.compatible_offset == 0 ? NULL : (const char *) ((const uint8_t *) base + device->fw.compatible_offset);
        } else {
                return NULL;
        }
}
