...
TEST(pe_memory_locate) {
        FOREACH_ELEMENT(test, tests) {
                ...
                /* Locate and check */
                ASSERT_EQ(pe_memory_locate_sections(_binary_src_boot_pe_efi_start, unified_sections, sections, PE_LOCATE_BY_OFFSET_FILE), EFI_SUCCESS);
                ...
                const char *cmdline = pe_section_vector_data(_binary_src_boot_pe_efi_start, &sections[UNIFIED_SECTION_CMDLINE], PE_LOCATE_BY_OFFSET_FILE, NULL);
                ASSERT_STREQ(cmdline, "test");
                const char *osrel = pe_section_vector_data(_binary_src_boot_pe_efi_start, &sections[UNIFIED_SECTION_OSREL], PE_LOCATE_BY_OFFSET_FILE, NULL);
                ASSERT_STREQ(osrel, "v7");

                if (PE_SECTION_VECTOR_IS_SET(&sections[UNIFIED_SECTION_DTBAUTO])) {
                        size_t dtb_length = 0;
                        const uint8_t *dtb = pe_section_vector_data(_binary_src_boot_pe_efi_start, &sections[UNIFIED_SECTION_DTBAUTO], PE_LOCATE_BY_OFFSET_FILE, &dtb_length);
