...
static int pe_sections(FILE *uki, struct PeSectionHeader **ret, uint16_t *ret_n) {
        ...
        *ret_n = 0;
        ...
        bytes = fread(&dos, 1, sizeof(dos), uki);
        if (bytes != sizeof(dos))
        ...
        bytes = fread(&pe, 1, sizeof(pe), uki);
        if (bytes != sizeof(pe))
...
int verb_kernel_identify(int argc, char *argv[], void *userdata) {
        ...
        if (sections) {
                if (find_pe_section(sections, scount, name_osrel, sizeof(name_osrel), NULL) &&
                    find_pe_section(sections, scount, name_osrel, sizeof(name_linux), NULL) &&
                    find_pe_section(sections, scount, name_osrel, sizeof(name_initrd), NULL)) {
                        puts("uki");
                        return EXIT_SUCCESS;
                }
                puts("pe");
