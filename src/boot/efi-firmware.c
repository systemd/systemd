...
static bool efifw_validate_header(
        ...
        if (header_len >= offsetof(EfiFwHeader, chid) + sizeof(EFI_GUID)) {
                ...
                if (!efi_guid_equal(&fw_header->chid, &zero))
                        chid = &fw_header->chid;
        }
...
EFI_STATUS efi_firmware_get_capsule(
        ...
        if (!efifw_validate_header(blob, blob_len, NULL, ret_chid, ret_payload, ret_payload_len))
                return EFI_INVALID_PARAMETER;
...
EFI_STATUS efi_firmware_apply_capsule(const void *payload, size_t payload_len) {
        ...
        Pages capsule_pages = xmalloc_pages(
                        AllocateAnyPages,
        ...
        if (max_size != 0 && header.CapsuleImageSize > max_size)
                return log_error_status(EFI_UNSUPPORTED,
                                        "Capsule (%u bytes) exceeds firmware maximum (%llu bytes).",
                                        header.CapsuleImageSize, (unsigned long long) max_size);

        EFI_PHYSICAL_ADDRESS sgl = 0;
        if (persist) {
                Pages descriptor_pages = xmalloc_pages(
                                AllocateAnyPages,
                                EfiRuntimeServicesData,
