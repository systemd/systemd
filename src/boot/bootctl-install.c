...
static int install_variables(
        ...
        if (r < 0) {
                if (graceful) {
                        log_full_errno(arg_quiet ? LOG_DEBUG : LOG_INFO, r,
                                       r == -ENOENT ?
                                       "Failed to access EFI variables, skipping. Is the \"efivarfs\" filesystem mounted?" :
                                       "Failed to determine current boot order, skipping: %m");
                        return 0;
                }
        ...
        if (first || r == 0) {
                ...
                if (r < 0) {
                        if (graceful) {
                                log_full_errno(arg_quiet ? LOG_DEBUG : LOG_INFO, r,
                                               "Failed to create EFI Boot variable entry, skipping: %m");
                                return 0;
                        }
