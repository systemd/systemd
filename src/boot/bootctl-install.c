...
static int install_secure_boot_auto_enroll(const char *esp, X509 *certificate, EVP_PKEY *private_key) {
        ...
        _cleanup_free_ EFI_SIGNATURE_LIST *siglist = malloc(siglistsz);
        if (!siglist)
                return log_oom();
        ...
        *siglist = (EFI_SIGNATURE_LIST) {
        ...
        FOREACH_STRING(db, "PK", "KEK", "db") {
                ...
                if (BIO_write(bio, db16, char16_strsize(db16)) < 0)
                ...
                *auth = (EFI_VARIABLE_AUTHENTICATION_2) {
                        ...
                        .AuthInfo = {
                                .Hdr = {
                                        .dwLength = offsetof(EFI_VARIABLE_AUTHENTICATION_2, AuthInfo.CertData) + sigsz,
                                        .wRevision = 0x0200,
