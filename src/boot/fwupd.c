...
static EFI_DEVICE_PATH *device_path_find_type(EFI_DEVICE_PATH **dp, uint8_t type) {
...
static EFI_STATUS device_path_find_index_of_type(EFI_DEVICE_PATH *dp, size_t *ret_index) {
        size_t index = 0;
...
static EFI_STATUS find_capsule(const char16_t *name, EFI_DEVICE_PATH **file_dp, EFI_HANDLE *ret_device_handle) {
        ...
        err = device_path_find_index_of_type(dp, &index);
        if (err != EFI_SUCCESS)
