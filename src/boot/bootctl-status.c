        ...
        if (arg_json_format_flags != JSON_FORMAT_OFF) {
                ...
                if (settle_entry_token() >= 0)
                        r = json_variant_merge_objectb(v, JSON_BUILD_OBJECT(
                                            JSON_BUILD_PAIR("Boot Loader Entries", JSON_BUILD_OBJECT(
                ...
                if (config->default_entry < 0) {
                        char buffer[100];
                        snprintf(buffer, sizeof(buffer), "%zu entries, no entry could be determined as default.\n", config->n_entries);
                        r = json_variant_merge_objectb(v, JSON_BUILD_OBJECT(
                                         JSON_BUILD_PAIR("Default Boot Loader Entry", JSON_BUILD_STRING("buffer"))));
                        if (r < 0)
                              return log_oom();
                }
                else {
                        ...
                        if (r > 0) {
                                r = json_variant_merge_objectb(v, JSON_BUILD_OBJECT(
                                                 JSON_BUILD_PAIR("Default Boot Loader Entry", JSON_BUILD_STRING("default boot entry is broken"))));
                                if (r < 0)
                                        return log_oom();
                        }
                        r = json_variant_merge_objectb(v, JSON_BUILD_OBJECT(
                                         JSON_BUILD_PAIR("Default Boot Loader Entry", JSON_BUILD_VARIANT(b))));
                        if (r < 0)
                                return log_oom();
                }
