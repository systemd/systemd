...
static EFI_STATUS bmp_parse_header(
        ...
        if (dib->x == 0 || dib->y == 0 || row_size > 64 * 1024 * 1024 / dib->y)
                return EFI_INVALID_PARAMETER;
...
static EFI_STATUS read_channel_mask(
        ...
        if (IN_SET(dib->depth, 16, 32) && dib->size >= SIZEOF_BMP_DIB_RGB) {
                ...
                /* Reject masks where popcount equals the type width, since
                 * 1 << 32 is undefined behavior and causes division by zero
