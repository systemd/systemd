...
/* The shim lock protocol is for pre-v16 shim, where it was not hooked up to the BS->LoadImage system table
 * and friends, and it has to be checked manually via the shim_validate() helper. If the shim image loader
 * protocol is available (shim v16 and newer), then it will have overridden BS->LoadImage and friends in the
 * system table, so no specific helper is needed, and the standard BS->LoadImage and friends can be called
 * instead. */
