...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "hpx", options, NULL)) >= 0)
                ...
                case ARG_EFI_LABEL:
                        ...
                        if (r < 0)
                                return log_oom();
                        if (isempty(arg_efi_label) || !string_is_safe(arg_efi_label))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Invalid --efi-label=: %s", arg_efi_label);
                        if (strlen(arg_efi_label) > 1024)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "--efi-label= too long: %ld > 1024", strlen(arg_efi_label));
                        break;
