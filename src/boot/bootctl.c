...
static int install_variables(const char *esp_path,
        ...
        if (first || r == 0) {
                r = efi_add_boot_option(slot, arg_efi_label ? arg_efi_label : "Linux Boot Manager",
                                        part, pstart, psize,
                                        uuid, path);
                if (r < 0)
                        return log_error_errno(r, "Failed to create EFI Boot variable entry: %m");

                log_info("Created EFI boot entry \"%s\".", arg_efi_label ? arg_efi_label : "Linux Boot Manager");
        }
