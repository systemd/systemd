...
static int settle_entry_token(void) {
        ...
        case ARG_ENTRY_TOKEN_AUTO: {
                ...
                if (!isempty(buf)) {
                        ...
                        arg_entry_token_type = ARG_ENTRY_TOKEN_EXPLICIT;
...
static int settle_make_entry_directory(void) {
        ...
        bool layout_type1 = !arg_install_layout || streq(arg_install_layout, "bls");
        ...
        if (arg_make_entry_directory > 0 && !layout_type1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "KERNEL_INSTALL_LAYOUT=%s is configured, but Boot Loader Specification Type #1 directory creation was requested.",
                                       arg_install_layout);
...
static int help(int argc, char *argv[], void *userdata) {
        ...
        printf("%1$s  [OPTIONS...] COMMAND ...\n"
               ...
               "     --entry-token=machine-id|os-id|os-image-id|auto|explicit:…\n"
        ...
        while ((c = getopt_long(argc, argv, "hpx", options, NULL)) >= 0)
                ...
                case ARG_ENTRY_TOKEN: {
                        ...
                        if (streq(optarg, "machine-id")) {
                                ...
                                arg_entry_token_type = ARG_ENTRY_TOKEN_OS_ID;
                                arg_entry_token = mfree(arg_entry_token);
                        } else if ((e = startswith(optarg, "explicit:"))) {
                                arg_entry_token_type = ARG_ENTRY_TOKEN_EXPLICIT;
