...
static OpensslKeySourceType arg_private_key_source_type = OPENSSL_KEY_SOURCE_FILE;
...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "hjc", options, NULL)) >= 0)
                ...
                case ARG_PRIVATE_KEY_SOURCE:
                        r = parse_openssl_key_source(optarg, &arg_private_key_source, &arg_private_key_source_type);
                        if (r < 0)
                                return r;
