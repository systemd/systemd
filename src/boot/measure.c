...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0)
                ...
                case ARG_BANK: {
                        ...
                        if (strv_extend(&arg_banks, EVP_MD_get0_name(implementation)) < 0)
                                return log_oom();
...
static int pcr_state_extend(PcrState *pcr_state, const void *data, size_t sz) {
        ...
        if (EVP_DigestInit_ex(mc, pcr_state->md, NULL) != 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to initialize %s context.", EVP_MD_get0_name(pcr_state->md));
        ...
        for (UnifiedSection c = 0; c < _UNIFIED_SECTION_MAX; c++) {
                ...
                for (size_t i = 0; i < n; i++) {
                        ...
                        if (EVP_DigestInit_ex(mdctx[i], pcr_states[i].md, NULL) != 1)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to initialize data %s context.", EVP_MD_get0_name(pcr_states[i].md));
                }
                ...
                for (size_t i = 0; i < n; i++) {
                        ...
                        if (EVP_Digest(unified_sections[c], strlen(unified_sections[c]) + 1, data_hash, &data_hash_size, pcr_states[i].md, NULL) != 1)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to hash section name with %s.", EVP_MD_get0_name(pcr_states[i].md));
...
static int verb_calculate(int argc, char *argv[], void *userdata) {
        ...
        STRV_FOREACH(d, arg_banks) {
                ...
                sz = EVP_MD_get_size(implementation);
                if (sz <= 0 || sz >= INT_MAX)
        ...
        for (size_t i = 0; i < n; i++) {
                ...
                b = strdup(EVP_MD_get0_name(pcr_states[i].md));
                if (!b)
                        return log_oom();
