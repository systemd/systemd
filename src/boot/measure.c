...
static int parse_argv(int argc, char *argv[]) {
        ...
        if (arg_signing_engine && !arg_certificate)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "When using --signing-engine=, --certificate= must be specified.");
...
static int verb_sign(int argc, char *argv[], void *userdata) {
        ...
        if (!arg_signing_engine) {
                ...
                r = openssl_load_key_from_token(arg_signing_engine, arg_private_key, &privkey);
                if (r < 0)
                        return log_error_errno(
                                        r,
