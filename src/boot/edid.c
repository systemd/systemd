...
EFI_STATUS edid_get_panel_id(char16_t **ret_panel) {
        ...
        uint16_t manufacturer_id = be16toh(edid_header->manufacturer_id);

        /* 3-letter Manufacturer ID + 16-bit product code */
        _cleanup_free_ char16_t *panel_id = xnew0(char16_t, 5);

        for (size_t i = 0; i < 3; i++) {
                uint8_t letter = (manufacturer_id >> (5 * i)) & 0b11111;
                if (letter > 0b11010) {
                        log_error("Invalid letter in the EDID manufacturer ID: 0x%x", letter);
                        return EFI_INVALID_PARAMETER;
                }
                panel_id[2 - i] = letter + 'A' - 1;
        }

        log_debug("Panel manufacturer: %ls (0x%x), product: 0x%x", panel_id, manufacturer_id, edid_header->manufacturer_product_code);
