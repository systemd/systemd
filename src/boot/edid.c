...
typedef struct EdidHeader {
        ...
        uint16_t manufacturer_id;             /* big-endian */
        uint16_t manufacturer_product_code;   /* big-endian */
        uint32_t serial_number;               /* little-endian */
        uint8_t week_of_manufacture;          /* week or 0xFF model year flag */
        uint8_t year_of_manufacture;          /* year of manufacture or model if flag is set (0 is 1990) */
...
EFI_STATUS edid_get_panel_id(char16_t **ret_panel) {
        ...
        uint16_t product_code = bswap_16(edid_header->manufacturer_product_code);
...
#ifdef EFI_DEBUG
        log_info("Panel manufacturer: %ls (0x%x), product: 0x%x", panel_id, manufacturer_id, product_code);
#endif

        panel_id[3] = product_code;
        *ret_panel = panel_id;
