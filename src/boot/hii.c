...
char16_t *hii_query_keyboard_layout_language(void) {
        ...
        _cleanup_free_ uint8_t *buf = xmalloc(length);
        err = hii_db->GetKeyboardLayout(hii_db, /* KeyGuid= */ NULL, &length, buf);
        if (err != EFI_SUCCESS) {
                log_debug_status(err, "Failed to retrieve current keyboard layout, ignoring: %m");
                return NULL;
        }

        const EFI_HII_KEYBOARD_LAYOUT *layout = (const EFI_HII_KEYBOARD_LAYOUT *) buf;
        ...
        const EFI_DESCRIPTION_STRING_BUNDLE *bundle = (const EFI_DESCRIPTION_STRING_BUNDLE *) (buf + off);
