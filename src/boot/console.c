...
void cmdline_append_console(char16_t **cmdline) {
        ...
        for (size_t i = 0; i < n_console_args; i++) {
                log_debug("Appending %ls to kernel command line.", console_args[i]);

                _cleanup_free_ char16_t *old = TAKE_PTR(*cmdline);
                if (isempty(old))
                        *cmdline = xstrdup16(console_args[i]);
                else
                        *cmdline = xasprintf("%ls %ls", old, console_args[i]);
        }
}
