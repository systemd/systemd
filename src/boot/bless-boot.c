...
static int verb_status(int argc, char *argv[], void *userdata) {
        ...
        STRV_FOREACH(p, arg_path) {
                ...
                if (faccessat(fd, skip_slash_or_dot(path), F_OK, 0) >= 0) {
                        puts("indeterminate");
                        return 0;
                }
                if (errno != ENOENT)
                ...
                if (faccessat(fd, skip_slash_or_dot(good), F_OK, 0) >= 0) {
                        puts("good");
                        return 0;
                }
                ...
                if (faccessat(fd, skip_slash_or_dot(bad), F_OK, 0) >= 0) {
                        puts("bad");
                        return 0;
                }
                if (errno != ENOENT)
...
static int verb_set(int argc, char *argv[], void *userdata) {
        ...
        STRV_FOREACH(p, arg_path) {
                ...
                r = rename_noreplace(fd, skip_slash_or_dot(source1), fd, skip_slash_or_dot(target));
                if (r == -EEXIST)
                        goto exists;
                if (r == -ENOENT) {

                        r = rename_noreplace(fd, skip_slash_or_dot(source2), fd, skip_slash_or_dot(target));
                        if (r == -EEXIST)
                                goto exists;
                        if (r == -ENOENT) {

                                if (faccessat(fd, skip_slash_or_dot(target), F_OK, 0) >= 0) /* Hmm, if we can't find either source file, maybe the destination already exists? */
                                        goto exists;
                ...
                r = fsync_parent_at(fd, skip_slash_or_dot(target));
                if (r < 0)
