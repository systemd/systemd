...
static bool has_virtio_console_pci_device(void) {
        ...
        EFI_STATUS err = BS->LocateHandleBuffer(
                        ...
                        &handles);
        if (err != EFI_SUCCESS) {
                log_debug("Failed to locate PCI I/O protocol handles, assuming no VirtIO console: %m");
                return false;
        }
        ...
        if (n_virtio_console > 1) {
                log_debug("Found %zu VirtIO console PCI devices, cannot determine which one is the console.", n_virtio_console);
                return false;
        }
...
static bool device_path_has_uart(EFI_DEVICE_PATH *dp) {
...
static bool has_graphics_output(void) {
        ...
        if (err != EFI_SUCCESS) {
                log_debug("No EFI Graphics Output Protocol found: %m");
                return false;
        }
...
static const char16_t *serial_console_arg(void) {
        ...
        return u"console=ttysclp0";
#elif defined(__powerpc__) || defined(__powerpc64__)
        return u"console=hvc0";
#else
...
static void cmdline_append_console(char16_t **cmdline) {
        ...
        if (*cmdline && efi_fnmatch(u"*console=*", *cmdline)) {
