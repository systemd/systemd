...
static bool efifw_validate_header(
                const void *blob,
                size_t blob_len,
                const char **ret_fwid_off,
                const char **ret_payload_off) {
        ...
        const char *fwid_offset    = (const char*)blob + header_len;
        const char *payload_offset = fwid_offset + fwid_len;

        /* check that fwid_offset has a terminating NUL character */
        if (memchr(fwid_offset, 0, fwid_len) != fwid_offset + fwid_len - 1)
                return false;

        if (ret_fwid_off)
                *ret_fwid_off = fwid_offset;

        if (ret_payload_off)
                *ret_payload_off = payload_offset;
        return true;
}
