 ...
 * passed to the OS, in an initrd file inaccessible to unpriviled userspace. To make things a bit more robust
 * while hashing two more pieces of information are mixed in: an random "mixin" that is stored in the ESP and
 * is supposed to ensure that the passed boot secrets are distinct for each disk used on the system; moreover
 * an OS identifier derived from the UKI's .osrel field (ideally IMAGE_ID=, but if not defined ID= will do,
 * with a final fallback to "linux"). Note that these two additions are not supposed to enhance the
 ...
 *
 * The boot secret to the OS can be used to to protect resources during OS runtime, from earliest boot phases
 * on, as a fallback for the usual TPM based protections.
 *
 ...
 * correctly implements boot-time-only EFI variable meachnism. (But then again, this is what mok/shim's
...
static EFI_STATUS random_seed_find_table(struct linux_efi_random_seed **ret) {
        ...
        if (!seed_table)
                return log_debug_status(EFI_NOT_FOUND, "No random seed available, not making boot secret.");
...
static EFI_STATUS read_efivar_secret(uint8_t ret_secret[static BOOT_SECRET_SIZE]) {
        ...
        if (err != EFI_SUCCESS)
                return log_debug_status(err, "Failed to read BootSecret EFI variable: %m");

        if (size != BOOT_SECRET_SIZE)
                return log_debug_status(EFI_PROTOCOL_ERROR, "Unexpected size of BootSecret EFI variable, ignoring.");

        if ((attributes & (EFI_VARIABLE_NON_VOLATILE|EFI_VARIABLE_BOOTSERVICE_ACCESS|EFI_VARIABLE_RUNTIME_ACCESS)) !=
            (EFI_VARIABLE_NON_VOLATILE|EFI_VARIABLE_BOOTSERVICE_ACCESS))
                return log_debug_status(EFI_PROTOCOL_ERROR, "Unexpected attributes of BootSecret EFI variable, ignoring.");
...
static EFI_STATUS setup_efivar_secret(
        ...
        /* Set the variabale with the EFI_VARIABLE_RUNTIME_ACCESS flag off (!), so that it's invisible after
         * ExitBootServices()! */
        err = RT->SetVariable(
                        (char16_t*) u"LoaderBootSecret",
                        MAKE_GUID_PTR(LOADER),
                        EFI_VARIABLE_NON_VOLATILE|EFI_VARIABLE_BOOTSERVICE_ACCESS, /* ← No EFI_VARIABLE_BOOTSERVICE_ACCESS here */
                        sizeof(secret),
                        secret);
        if (err != EFI_SUCCESS)
...
static EFI_STATUS acquire_efifvar_secret(
                struct linux_efi_random_seed *seed_table,
                uint8_t ret_secret[static BOOT_SECRET_SIZE]) {
...
static EFI_STATUS setup_secret_mixin(
        ...
        size_t wsize = sizeof(secret);
        err = handle->Write(handle, &wsize, secret);
        if (err != EFI_SUCCESS)
                return log_debug_status(err, "Failed to write secret mixin file: %m");
        if (wsize != sizeof(secret))
...
static EFI_STATUS acquire_secret_mixin(
        ...
        err = root_dir->Open(
                        root_dir,
                        &handle,
                        (char16_t *) u"\\loader\\secret-mixin",
        ...
        if (err == EFI_WRITE_PROTECTED) {
                err = root_dir->Open(
                                ...
                                /* Attributes= */ 0);
                if (err != EFI_SUCCESS)
                        return log_debug_status(err, "Failed to read the secret mixin file: %m");

                writable = false;
        } else  if (err != EFI_SUCCESS)
                return log_debug_status(err, "Failed to access the secret mixin file: %m");
        else
                writable = true;
        ...
        if (err != EFI_SUCCESS)
                return log_debug_status(err, "Failed to get secret mixin file info: %m");
        ...
        if (info->FileSize < BOOT_SECRET_SIZE)
                return log_debug_status(EFI_PROTOCOL_ERROR, "Secret mixin is too short %" PRIu64 " < %u", info->FileSize, BOOT_SECRET_SIZE);

        uint8_t mixin[BOOT_SECRET_SIZE];
        size_t rsize = sizeof(mixin);
        err = handle->Read(handle, &rsize, &mixin);
        if (err != EFI_SUCCESS)
                return log_debug_status(err, "Failed to read secret mixin: %m");
...
static char *pick_id(const char *_osrel, size_t osrel_size) {
        ...
        char *line, *key, *value, *os_id = NULL;
        size_t pos = 0;
...
EFI_STATUS prepare_boot_secret(
        ...
        err = acquire_efifvar_secret(seed_table, efivar_secret);
        if (err != EFI_SUCCESS)
                return err;
