...
static EFI_STATUS store_interactive_credentials(Config *config) {
        ...
        uint64_t maximum_variable_storage_size, remaining_variable_storage_size, maximum_variable_size;
        EFI_STATUS err;
        ...
        if (maximum_variable_size < variable_name_size ||
...
static bool menu_run(
        ...
        while (IN_SET(action, ACTION_CONTINUE, ACTION_FIRMWARE_SETUP, ACTION_RUN)) {
                if (action == ACTION_RUN) {
                        if (config->entries[idx_highlight]->type == LOADER_CLEAR_CREDENTIALS) {
                                ...
                                if (LOADER_TYPE_SUPPORTS_CREDENTIALS(config->entries[idx_highlight]->type))
                                        err = store_interactive_credentials(config);
                                else {
                                        err = efivar_unset(MAKE_GUID_PTR(LOADER), u"LoaderInteractiveCredentials", 0);
                ...
                case KEYPRESS(0, 0, 'c'): {
                        if (!config->editor) {
                                status = xstrdup16(u"Interactive credential input is disabled.");
                                break;
                        }
                                        ...
                                        if (line_edit(&cred_value, x_max - 2, y_status)) {
                                                if (isempty(cred_value))
                                                        ...
                                                        credentials[n_credentials] = (InteractiveCredential) {
                                                                .name = TAKE_PTR(cred_key),
                                                                .value = TAKE_PTR(cred_value),
                                                        };
        ...
        /* A console read error may leave the non-bootable clear entry selected. */
