        ...
        while (i != config->n_entries) {
                ...
                if (j - i < 3) {
                        *entries++ = config->entries[i];
                        ...
                        /* If the default entry is contained in this group,
                         * use it as top entry */
                        if (config->idx_default_efivar >= i && config->idx_default_efivar < j) {
                                *entries++ = config->entries[config->idx_default_efivar];
                                *idx_default_efivar = *n_entries;
                        /* Otherwise use the first one we find, which is the newest / latest */
                        } else {
                                *entries++ = config->entries[i];
                        }
                        (*n_entries)++;

                        /* Allocate for the submenu entry, it will be freed by reset_grouped_entries() */
...
static bool menu_run(
        ...
        entries = xmalloc(sizeof(void *) * config->n_entries);
        top_entries(config, entries, &n_entries, &idx_default_efivar);
        ...
        while (IN_SET(action, ACTION_CONTINUE, ACTION_FIRMWARE_SETUP)) {
                ...
                case KEYPRESS(0, SCAN_SUSPEND, 0): /* Handle phones/tablets with only a power key + volume up/down rocker (and otherwise just touchscreen input) */
                        if (entries[idx_highlight]->grouped) {
                                /* The "> More..." placeholder always follows a real config entry
                                 * sharing the group's sort key, owned by config and hence
                ...
                case KEYPRESS(0, 0, 'd'):
                        ...
                        if (idx_default_efivar != idx_highlight) {
                                ...
                                for (size_t i = 0; i != config->n_entries; i++) {
                                        if (strcmp16(config->entries[i]->id, entries[idx_highlight]->id) == 0) {
