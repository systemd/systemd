...
static bool menu_run(
        ...
        while (IN_SET(action, ACTION_CONTINUE, ACTION_FIRMWARE_SETUP)) {
                ...
                case KEYPRESS(0, 0, 'd'):
                case KEYPRESS(0, 0, 'D'):
                        if (config->default_from_preferred) {
...
static size_t config_find_entry(Config *config, const char16_t *pattern, const bool check_assessment) {
        ...
        for (size_t i = 0; i < config->n_entries; i++)
                if (efi_fnmatch(pattern, config->entries[i]->id)) {
                        if (!check_assessment || config->entries[i]->tries_left != 0)
                                return i;
                }
...
static void config_select_default_entry(Config *config) {
        ...
        if (config->sysfail_occurred) {
                i = config_find_entry(config, config->entry_sysfail, false);
                if (i != IDX_INVALID) {
        ...
        i = config_find_entry(config, config->entry_oneshot, false);
        if (i != IDX_INVALID) {
                config->idx_default = i;
                return;
        }
        ...
        i = config_find_entry(config, config->use_saved_entry_preferred_efivar ? config->entry_saved : config->entry_preferred_efivar, true);
        if (i != IDX_INVALID) {
                config->idx_default = i;
                config->idx_default_efivar = i;
                config->default_from_preferred = true;
                return;
        }

        i = config_find_entry(config, config->entry_preferred_config, true);
        if (i != IDX_INVALID) {
                config->idx_default = i;
                config->default_from_preferred = true;
                return;
        ...
        if (config->use_saved_entry_preferred)
                ...
                i = config->use_saved_entry_preferred_efivar ? IDX_INVALID : config_find_entry(config, config->entry_saved, true);
        else
                i = config_find_entry(config, config->entry_preferred_config, true);
        if (i != IDX_INVALID) {
                config->idx_default = i;
        ...
        i = config_find_entry(config, config->use_saved_entry_efivar ? config->entry_saved : config->entry_default_efivar, false);
        if (i != IDX_INVALID) {
                config->idx_default = i;
                config->idx_default_efivar = i;
                return;
        ...
        if (config->use_saved_entry)
                /* No need to do the same thing twice. */
                i = config->use_saved_entry_efivar ? IDX_INVALID : config_find_entry(config, config->entry_saved, false);
        else
                i = config_find_entry(config, config->entry_default_config, false);
        if (i != IDX_INVALID) {
                config->idx_default = i;
                return;
        }
