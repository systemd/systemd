...
static void calculate_top_entries(
        ...
        *idx_default_efivar = IDX_INVALID;
        ...
        while (i != config->n_entries) {
                ...
                if (!sort_key) {
                        append_entry(config, entries, n_entries, i++, idx_default, idx_default_efivar);
                        continue;
                }
                ...
                if (j - i < 3) {
                        append_entry(config, entries, n_entries, i, idx_default, idx_default_efivar);
                        if (j - i == 2)
                                append_entry(config, entries, n_entries, i + 1, idx_default, idx_default_efivar);
                } else {
                        /* Collapse this group behind a single representative entry plus a "More >"
                         * submenu placeholder. Prefer the efivar default as the representative, so a
                         * preference set inside the submenu stays visible (and marked) in the top menu;
                         * otherwise fall back to the config default, then the first (newest) one. At a
                         * fresh boot the two indices coincide, so this does not change what boots on
                         * timeout. */
                        if (config->idx_default_efivar >= i && config->idx_default_efivar < j)
                                rep = config->idx_default_efivar;
                        else if (config->idx_default >= i && config->idx_default < j)
                                rep = config->idx_default;
                        else
                                rep = i;

                        size_t rep_pos = *n_entries;
                        append_entry(config, entries, n_entries, rep, idx_default, idx_default_efivar);

                        /* If the config default was collapsed into this group but isn't the
                         * representative (e.g. the efivar default was preferred above), point the
                         * highlight at the representative row so *idx_default never stays IDX_INVALID. */
                        if (idx_default && config->idx_default >= i && config->idx_default < j)
                                *idx_default = rep_pos;
        ...
        *n_entries = 0;
        ...
        while (i < config->n_entries && strcmp16(config->entries[i]->sort_key, sort_key) == 0) {
                if (config->idx_default_efivar == i)
                        *idx_default_efivar = *n_entries;
                entries[(*n_entries)++] = config->entries[i];
                i++;
        }
}
...
static bool menu_run(
        ...
        calculate_top_entries(config, entries, &n_entries, &idx_highlight, &idx_default_efivar);
        ...
        while (IN_SET(action, ACTION_CONTINUE, ACTION_FIRMWARE_SETUP)) {
                ...
                case KEYPRESS(0, SCAN_LEFT, 0):
                        if (in_submenu) {
                                 ...
                                 * config and hence still valid after calculate_top_entries() rebuilds the list. */
                                sort_key = entries[0]->sort_key;
                                calculate_top_entries(config, entries, &n_entries, NULL, &idx_default_efivar);
                                for (size_t i = 0; i < n_entries; i++) {
