...
static void config_remove_last_entry(Config *config, BootEntry *entry);
...
static char16_t* menu_prompt_interactive_credential(
        ...
        _cleanup_free_ char16_t *status = xasprintf("name: %ls value: %ls", name, value);
        ...
        while (IN_SET(action, ACTION_CONTINUE, ACTION_FIRMWARE_SETUP, ACTION_RUN)) {
                if (action == ACTION_RUN) {
                        if (config->entries[idx_highlight]->type == LOADER_CLEAR_CREDENTIALS) {
                                ...
                                config_remove_last_entry(config, config->entries[idx_highlight]);
                ...
                if (err != EFI_SUCCESS) {
                        ...
                        if (err != EFI_SUCCESS)
                                log_error_status(
                                                err,
                                                "Error updating LoaderInteractiveCredentials, booting anyway: %m");
...
static void config_remove_last_entry(Config *config, BootEntry *entry) {
        ...
        assert(config->entries[config->n_entries - 1] == entry);

        config->n_entries--;
        config->entries[config->n_entries] = boot_entry_free(entry);
}
