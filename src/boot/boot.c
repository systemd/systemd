...
static char16_t* update_timeout_efivar(Config *config, bool inc) {
        ...
        default:
                return xasprintf("Menu timeout set to %"PRIu64" sec.", config->timeout_sec_efivar);
        }
}
...
static void print_timeout_status(const char *label, uint64_t t) {
        ...
        default:
                return (void) printf("%s: %"PRIu64" sec\n", label, t);
        }
}
...
static void print_status(Config *config, char16_t *loaded_image_path) {
        ...
        if (config->secure_boot_enroll_timeout_sec == ENROLL_TIMEOUT_OFF)
                ...
                printf("secure-boot-enroll-timeout-sec: %"PRIu64" sec\n", config->secure_boot_enroll_timeout_sec);
...
static bool menu_run(
        ...
        uint64_t timeout_remain = config->timeout_sec == TIMEOUT_MENU_FORCE ? 0 : config->timeout_sec;
        ...
        while (IN_SET(action, ACTION_CONTINUE, ACTION_FIRMWARE_SETUP)) {
                ...
                if (timeout_remain > 0) {
                        free(status);
                        status = xasprintf("Boot in %"PRIu64" sec.", timeout_remain);
                }
