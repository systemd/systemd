...
typedef struct {
        ...
        uint64_t secure_boot_enroll_timeout;
        bool force_menu;
...
static void print_status(Config *config, char16_t *loaded_image_path) {
        ...
        printf("secure-boot-enroll-timeout: %" PRIu64 "\n", config->secure_boot_enroll_timeout);
                ...
                if (streq8(key, "timeout")) {
                        ...
                        if (streq8(value, "reboot"))
                                ...
                                log_error("Error parsing 'secure-boot-enroll-action' config option, ignoring: %s",
                                          value);
                } else if (streq8(key, "secure-boot-enroll-timeout")) {
                        if (streq8(value, "hidden"))
                                config->timeout_sec_config = ENROLL_TIMEOUT_HIDDEN;
                        else {
                                uint64_t u;
                                if (!parse_number8(value, &u, NULL) || u > ENROLL_TIMEOUT_TYPE_MAX) {
                                        log_error("Error parsing 'secure-boot-enroll-timeout' config option, ignoring: %s",
                                                  value);
                                        continue;
                                }
                                config->secure_boot_enroll_timeout = u;
                        }
...
static void config_load_defaults(Config *config, EFI_FILE *root_dir) {
        ...
        *config = (Config) {
                ...
                .secure_boot_enroll_timeout = ENROLL_TIMEOUT_DEFAULT,
                ...
                if (IN_SET(config->secure_boot_enroll, ENROLL_IF_SAFE, ENROLL_FORCE) &&
                    strcaseeq16(dirent->FileName, u"auto"))
                        ...
                        secure_boot_enroll_at(root_dir, entry->directory, config->secure_boot_enroll == ENROLL_FORCE,
                                              config->secure_boot_enroll_action, config->secure_boot_enroll_timeout);
        }
