...
static void smbios_info_populate(SmbiosInfo *ret_info) {
        ...
        EFI_STATUS status = edid_get_discovered_panel_id(&ret_info->smbios_fields[CHID_EDID_PANEL]);
#if SD_BOOT
        if (EFI_STATUS_IS_ERROR(status))
                log_warning_status(status, "Failed to get EDID Panel ID: %m");
#else
        /* Silence the compiler warning about this variable being unused */
        (void) status;
#endif
}
