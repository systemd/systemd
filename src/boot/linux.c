...
EFI_STATUS linux_exec(
        ...
        for (size_t i = 0; i < n_headers; i++) {
                if (headers[i].PointerToRelocations != 0)
                        return log_error_status(EFI_LOAD_ERROR, "Relocations unexpected");
                if (headers[i].SizeOfRawData) {
                        if ((headers[i].VirtualAddress < image_base)
                            || (headers[i].VirtualAddress - image_base + headers[i].SizeOfRawData > kernel_size_in_memory))
                                return log_error_status(EFI_LOAD_ERROR, "Section would write outside of memory");
                        memcpy(loaded_kernel + headers[i].VirtualAddress - image_base,
                               (const uint8_t*)kernel->iov_base + headers[i].PointerToRawData,
                               headers[i].SizeOfRawData);
                        if (headers[i].VirtualSize > headers[i].SizeOfRawData) {
                                memset(loaded_kernel + headers[i].VirtualAddress + headers[i].SizeOfRawData, 0,
                                       headers[i].VirtualSize - headers[i].SizeOfRawData);
                        }
                }
        }

        _cleanup_free_ EFI_LOADED_IMAGE_PROTOCOL* loaded_image = (EFI_LOADED_IMAGE_PROTOCOL*)xmalloc(sizeof(EFI_LOADED_IMAGE_PROTOCOL));
        if (!loaded_image) {
