...
EFI_STATUS linux_exec(
        ...
        *kernel_file_path = (KERNEL_FILE_PATH) {
        ...
        /* Honor the inner kernel's PE SectionAlignment. arm64 sets this to SZ_64K (= MIN_KIMG_ALIGN);
         * the kernel's EFI stub asserts on _text alignment and refuses to boot otherwise.
         * xmalloc_aligned_pages() degenerates to a plain AllocatePages() when alignment <= EFI_PAGE_SIZE
         * (e.g. x86_64, where SectionAlignment is typically 0x1000), so this is free elsewhere. */
