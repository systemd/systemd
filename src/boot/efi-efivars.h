...
EFI_STATUS efivar_get_raw_full(const EFI_GUID *vendor, const char16_t *name, void **ret_data, size_t *ret_size, uint32_t *ret_flags);
static inline EFI_STATUS efivar_get_raw(const EFI_GUID *vendor, const char16_t *name, void **ret_data, size_t *ret_size) {
        return efivar_get_raw_full(vendor, name, ret_data, ret_size, /* ret_flags= */ NULL);
}
