...
static EFI_STATUS devicetree_open_compatible(struct devicetree_state *state, EFI_FILE **handle, EFI_FILE_INFO **info) {
        EFI_FILE *dtbdir = *handle;
        EFI_FILE *file = NULL;
        EFI_STATUS err;

        const char *compatible = devicetree_get_compatible(state->orig);
        if (!compatible)
                return EFI_UNSUPPORTED;

        _cleanup_free_ char16_t *filename = xasprintf("%s.dtb", compatible);

        err = dtbdir->Open(dtbdir, &file, filename, EFI_FILE_MODE_READ, 0ULL);
        if (err != EFI_SUCCESS)
                return err;
        ...
        *handle = file;
        ...
        return get_file_info(file, info, NULL);
}
...
EFI_STATUS devicetree_install(struct devicetree_state *state, EFI_FILE *root_dir, char16_t *name) {
        ...
        err = root_dir->Open(root_dir, &handle, name, EFI_FILE_MODE_READ, 0ULL);
        if (err != EFI_SUCCESS)
                return err;
        ...
        if (FLAGS_SET(info->Attribute, EFI_FILE_DIRECTORY)) {
                err = devicetree_open_compatible(state, &handle, &info);
                if (err != EFI_SUCCESS)
                        return err;
        }
