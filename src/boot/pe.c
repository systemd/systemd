...
static EFI_STATUS pe_size(const DosFileHeader *dos, const PeFileHeader *pe, size_t *ret) {
        ...
        size_t size = pe->OptionalHeader.SizeOfHeaders;
        section_table = (const PeSectionHeader*) ((const uint8_t*) dos + section_table_offset(dos, pe));
...
EFI_STATUS pe_kernel_info(const void *base, uint32_t *ret_compat_address, size_t *ret_size_in_memory, size_t *ret_size_on_disk) {
        ...
        if (ret_size_on_disk) {
                err = pe_size(dos, pe, ret_size_on_disk);
                if (err != EFI_SUCCESS)
                        return err;
        }
