...
static const char* smbios_get_string(const SmbiosHeader *header, uint16_t string, uint64_t left) {
        ...
        for (uint16_t index = 1, const char *p = s; index <= string && p < s + left; index++) {
                if (index == string) {
                    return s;
                }
...
EFI_STATUS smbios_populate_raw_info(struct raw_smbios_info *info) {
        ...
        info->ProductName = smbios_get_string(&type1->header, type1->product_name, left);
        info->ProductSku = smbios_get_string(&type1->header, type1->sku_number, left);
        ...
        info->BaseboardManufacturer = smbios_get_string(&type2->header, type2->manufacturer, left);
        info->BaseboardProduct = smbios_get_string(&type2->header, type2->product_name, left);
