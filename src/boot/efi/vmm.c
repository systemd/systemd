...
static SmbiosHeader *get_smbios_table(void *smbios, uint64_t size, uint8_t type) {
        uint8_t *p = ASSERT_PTR(smbios);
...
static bool smbios_in_hypervisor(void) {
        uint64_t size = 0;
        void *p = find_smbios_configuration_table(&size);
        if (!p)
                return false;

        /* Look up BIOS Information (Type 0). */
        SmbiosTableType0 *type0 = (SmbiosTableType0 *) get_smbios_table(p, size, 0);
