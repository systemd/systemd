...
static EFI_STATUS get_cmdline_mule(
        ...
        for (size_t i = 0; i < n_items; i++) {
                ...
                if (!shim_validate(NULL, content, contentsize)) {
                        log_error("Failed to validate %ls through Shim, ignoring.", items[i]);
                        continue;
                }
                ...
                buffer = xasprintf("%ls%ls%ls", strempty(tmp), tmp ? u" " : u"", extra16);
        }
