...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *sys_table) {
        ...
        if (efivar_get_uint64_le(LOADER_GUID, L"LoaderFeatures", &loader_features) != EFI_SUCCESS ||
            !FLAGS_SET(loader_features, EFI_LOADER_FEATURE_RANDOM_SEED)) {
                ...
                EFI_HANDLE esp_device;

                err = partition_open(ESP_GUID, loaded_image->DeviceHandle, &esp_device, &esp_dir);
                if (err != EFI_SUCCESS)
                        return log_error_status_stall(err, L"Unable to find ESP directory: %r", err);
