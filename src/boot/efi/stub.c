...
static inline void iovec_array_append(struct iovec **arr, size_t *n_arr, struct iovec elem) {
        *arr = xrealloc(*arr, *n_arr * sizeof(struct iovec), (*n_arr + 1)  * sizeof(struct iovec));
        (*arr)[(*n_arr)++] = elem;
}

static void measure_ucode_addons_and_create_final_list(
        ...
        /* Populate all_initrds by first adding ucode addons, and then copying all the static initrds */
        ...
        for (ssize_t i = n_ucode_addons - 1; i >= 0; i--) {
                ...
                iovec_array_append(all_initrds, n_all_initrds, ucode_addons[i].blob);
        }

        /* Now append the other initrds from the static list */
        FOREACH_ARRAY(i, initrds, _INITRD_MAX) {
                if (iovec_is_set(i))
                        iovec_array_append(all_initrds, n_all_initrds, *i);
        }
}
...
static EFI_STATUS run(EFI_HANDLE image) {
        ...
        NamedAddon *ucode_addons = NULL;
        size_t n_ucode_addons = 0;
        struct iovec *all_initrds = NULL;
        ...
        /* Measures ucode addons and populates all_initrds with the final order of all initrds */
        measure_ucode_addons_and_create_final_list(initrds, &all_initrds, &n_all_initrds, ucode_addons, n_ucode_addons, &sections_measured);

        log_error("just after return. elem[1] size %lu, base %p", all_initrds[1].iov_len, all_initrds[1].iov_base);
