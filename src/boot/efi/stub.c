...
static EFI_STATUS cmdline_append_and_measure_addons(
        ...
        for (size_t i = 0; i < n_items; i++) {
                ...
                /* Also enforce that, in case they are specified, .osrel and .uname match */
                if (osrel && szs[UNIFIED_SECTION_OSREL] > 0 &&
                                !strneq8(osrel,
                                         (char *)loaded_addon->ImageBase + addrs[UNIFIED_SECTION_OSREL],
                                         szs[UNIFIED_SECTION_OSREL])) {
                        log_error(".osrel mismatch between %ls and UKI, ignoring", items[i]);
                        continue;
                }
        ...
        if (!isempty(buffer)) {
                ...
                *ret_parameters_measured = *ret_parameters_measured < 0 ? m : (*ret_parameters_measured && m);
...
static EFI_STATUS run(EFI_HANDLE image) {
        ...
        EFI_LOADED_IMAGE_PROTOCOL *loaded_image;
        size_t addrs[_UNIFIED_SECTION_MAX] = {}, szs[_UNIFIED_SECTION_MAX] = {};
        _cleanup_free_ char *osrel = NULL, *uname = NULL;
        ...
        if (szs[UNIFIED_SECTION_OSREL] > 0)
                osrel = xstrndup8((char *)loaded_image->ImageBase + addrs[UNIFIED_SECTION_OSREL],
                                  szs[UNIFIED_SECTION_OSREL]);
        ...
        err = cmdline_append_and_measure_addons(image,
                                                loaded_image,
                                                u"\\loader\\addons",
                                                osrel,
                                                uname,
