...
static EFI_STATUS cmdline_append_and_measure_addons(
        ...
        for (size_t i = 0; i < n_items; i++) {
                ...
                /* Also enforce that, in case it is specified, .osrel matches */
...
static EFI_STATUS run(EFI_HANDLE image) {
        ...
        err = cmdline_append_and_measure_addons(image, loaded_image, u"\\loader\\addons", osrel, &cmdline);
        if (err != EFI_SUCCESS)
        ...
        err = cmdline_append_and_measure_addons(image, loaded_image, dropin_dir, osrel, &cmdline);
        if (err != EFI_SUCCESS)
                log_error_status(err, "Error loading addons, ignoring: %m");
