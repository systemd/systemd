 ...
 *
 *    - teach systemd-pcrlock to be able to predict this
 *    - teach userspace bootspec API to handle this, too
 *    - make sure ordering in sd-boot works
 *    - documentation
...
static EFI_STATUS find_sections(
        ...
        err = pe_memory_locate_sections_for_profile(loaded_image->ImageBase, unified_sections, UINT_MAX, sections);
        if (err != EFI_SUCCESS)
                return log_error_status(err, "Unable to locate embedded base PE sections: %m");

        if (profile != UINT_MAX) {
                /* And the override them with the per-profile sections of the selected profile */
                err = pe_memory_locate_sections_for_profile(loaded_image->ImageBase, unified_sections, profile, sections);
