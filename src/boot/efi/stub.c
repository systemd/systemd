...
static EFI_STATUS load_addons(
        ...
        _cleanup_free_ void **dt_bases = NULL;
        ...
        for (size_t i = 0; i < n_items; i++) {
                ...
                if (err != EFI_SUCCESS) {
                        log_error_status(err, "Error making device path for %ls: %m", addon_spath);
                        goto fail;
                }
                ...
                err = BS->HandleProtocol(addon,
                                         ...
                                         (void **) &loaded_addon);
                if (err != EFI_SUCCESS) {
                        log_error_status(err, "Failed to find protocol in %ls: %m", items[i]);
                        goto fail;
                }
                ...
                if (ret_dt_bases && szs[UNIFIED_SECTION_DTB] > 0) {
                        ...
                        dt_bases = xrealloc(dt_bases,
                                            n_dt * sizeof(EFI_PHYSICAL_ADDRESS),
                                            (n_dt + 1) * sizeof(EFI_PHYSICAL_ADDRESS));
                        dt_bases[n_dt] = xmalloc(dt_sizes[n_dt]);
                        memcpy(dt_bases[n_dt],
                               PHYSICAL_ADDRESS_TO_POINTER(POINTER_TO_PHYSICAL_ADDRESS(loaded_addon->ImageBase) + addrs[UNIFIED_SECTION_DTB]),
                               dt_sizes[n_dt]);
...
static EFI_STATUS run(EFI_HANDLE image) {
        ...
        _cleanup_free_ void **dt_bases_addons_global = NULL, **dt_bases_addons_uki = NULL;
        ...
        cmdline_append_and_measure_addons(cmdline_addons_global, &m, &cmdline);
        parameters_measured = parameters_measured < 0 ? m : (parameters_measured && m);

        cmdline_append_and_measure_addons(cmdline_addons_uki, &m, &cmdline);
        parameters_measured = parameters_measured < 0 ? m : (parameters_measured && m);
        ...
        for (size_t i = 0; i < n_dts_addons_global; ++i) {
                err = devicetree_install_from_memory(
                                &dt_state,
                                dt_bases_addons_global[i],
                                dt_sizes_addons_global[i]);
                if (err != EFI_SUCCESS)
                        log_error_status(err, "Error loading global addon devicetree: %m");
                dt_bases_addons_global[i] = mfree(dt_bases_addons_global[i]);
        }

        for (size_t i = 0; i < n_dts_addons_uki; ++i) {
                err = devicetree_install_from_memory(
                                &dt_state,
                                dt_bases_addons_uki[i],
                                dt_sizes_addons_uki[i]);
                if (err != EFI_SUCCESS)
                        log_error_status(err, "Error loading local addon devicetree: %m");
                dt_bases_addons_uki[i] = mfree(dt_bases_addons_uki[i]);
        }
