...
static EFI_STATUS load_addons_from_efi(EFI_LOADED_IMAGE_PROTOCOL *image, struct addon_entry **addons, size_t *n_items, size_t
                                       *n_allocated)
{
        EFI_STATUS err;
        ...
        const EFI_DEVICE_PATH **addon_paths_split = split_multiple_file_device_path(addon_paths);
        while (*addon_paths_split) {
                char16_t *spath = NULL;
                /* If we increment this pointer instead of addon_paths_split, we will arrive on a end node
                 * marker */
                const EFI_DEVICE_PATH *addon_dpath = *addon_paths_split;
