...
static EFI_STATUS combine_initrds(
        ...
        log_error("Called initrd concat with %zu initrds", n_initrds);
        log_error("Initrds:");
        for (size_t i = 0; i < n_initrds; i++) {
                log_error(" %zu: Base %p, Size %zu", i, initrds[i], initrd_sizes[i]);
        }
...
static EFI_STATUS ucode_merge_and_measure_addons(
        ...
        log_error("Merging ucode sections. # global addons: %zu, # uki addon size: %zu, embedded section: %zu", n_ucode_addons_global, n_ucode_addons_uki, ucode_section_size);
        ...
        log_error("Sizes: globalA %zu, ukiA %zu, section %zu", global_addons_combined_size, uki_addons_combined_size, ucode_section_size);
