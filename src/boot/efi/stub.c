...
static EFI_STATUS get_cmdline_mule(
        ...
        err = open_directory(root, u"\\loader\\mules", &extra_dir); // TODO: bikeshedding
        ...
        for (;;) {
                ...
                if (FLAGS_SET(dirent->Attribute, EFI_FILE_DIRECTORY))
                        continue;
                if (!endswith_no_case(dirent->FileName, u".mule.cmdline.efi")) // TODO: bikeshedding
                        continue;
        ...
        for (size_t i = 0; i < n_items; i++) {
                ...
                /* Load using file offset rather than memory offset - we are reading off a file, not a
                 * loaded image. */
...
static EFI_STATUS run(EFI_HANDLE image) {
        ...
        /* If we have any extra command line to add via PE mules, load them now and append, and
