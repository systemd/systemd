...
static EFI_STATUS load_addons(
        ...
        for (size_t i = 0; i < n_items; i++) {
                ...
                err = pe_memory_locate_sections(loaded_addon->ImageBase, &sections, &nsections);
                if (err != EFI_SUCCESS ||
                    (pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_CMDLINE]) == NULL &&
                        pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_DTB]))) {
                        if (err == EFI_SUCCESS)
                                err = EFI_NOT_FOUND;
                        log_error_status(err,
                ...
                if (pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_LINUX]) != NULL) {
                ...
                if (uname) {
                        struct PeSectionDescriptor *section_uname = pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_UNAME]);
                        if (section_uname != NULL && !strneq8(uname,
                                                        (char *)loaded_addon->ImageBase + section_uname->offset,
                                                        section_uname->size)) {
                ...
                if (ret_cmdline) {
                        struct PeSectionDescriptor *section_cmdline = pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_CMDLINE]);
                ...
                if (ret_dt_bases) {
                        struct PeSectionDescriptor *section_dtb = pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_DTB]);
                        if (section_dtb != NULL) {
...
static EFI_STATUS run(EFI_HANDLE image) {
        ...
        size_t linux_size = 0, initrd_size = 0, n_dts_addons_global = 0, n_dts_addons_uki = 0, nsections = 0;
        EFI_PHYSICAL_ADDRESS linux_base = 0, initrd_base = 0;
        ...
        err = pe_memory_locate_sections(loaded_image->ImageBase, &sections, &nsections);
        if (err != EFI_SUCCESS || pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_LINUX]) == NULL) {
                if (err == EFI_SUCCESS)
                        err = EFI_NOT_FOUND;
        ...
        for (UnifiedSection unified_section = 0; unified_section < _UNIFIED_SECTION_MAX; unified_section++) {
                ...
                struct PeSectionDescriptor *section = pe_bsearch_section(sections, nsections, unified_sections[unified_section]);
                if (section != NULL)  {
                        m = false;
        ...
        struct PeSectionDescriptor *section_splash = pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_SPLASH]);
        if (section_splash != NULL)
                graphics_splash((const uint8_t*) loaded_image->ImageBase + section_splash->offset, section_splash->size);

        struct PeSectionDescriptor *section_uname = pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_UNAME]);
        if (section_uname != NULL)
                uname = xstrndup8((char *)loaded_image->ImageBase + section_uname->offset,
                                  section_uname->size);

        struct PeSectionDescriptor *section_cmdlien = pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_CMDLINE]);
        if (use_load_options(image, loaded_image, section_cmdlien != NULL, &cmdline)) {
        ...
        } else if (section_cmdlien != NULL) {
                cmdline = xstrn8_to_16(
                                (char *) loaded_image->ImageBase + section_cmdlien->offset,
                                section_cmdlien->size);
        ...
        struct PeSectionDescriptor *section_pcrsig = pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_PCRSIG]);
        if (section_pcrsig != NULL)
         ...
         * cpio. */
        struct PeSectionDescriptor *section_pcrpkey = pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_PCRPKEY]);
        if (section_pcrpkey != NULL)
                (void) pack_cpio_literal(
        ...
        struct PeSectionDescriptor *section_linux = ASSERT_PTR(pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_LINUX]));
        linux_size = section_linux->size;
        linux_base = POINTER_TO_PHYSICAL_ADDRESS(loaded_image->ImageBase) + section_linux->offset;

        struct PeSectionDescriptor *section_initrd = pe_bsearch_section(sections, nsections, unified_sections[UNIFIED_SECTION_INITRD]);
        if (section_initrd != NULL) {
