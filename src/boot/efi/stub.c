...
static EFI_STATUS run(EFI_HANDLE image) {
        ...
        const char *extra = smbios_find_credential("boot.kernel-cmdline-extra");
        if (extra) {
                _cleanup_free_ char16_t *tmp = cmdline, *extra16 = xstr8_to_16(extra);
                cmdline = xasprintf("%ls %ls", cmdline, extra16);
        }
