...
EFI_STATUS devicetree_match_by_compatible(const void *dtb_buffer, size_t dtb_length, const char *compat) {
        if ((uintptr_t) dtb_buffer % alignof(FdtHeader) != 0)
                return EFI_INVALID_PARAMETER;

        const FdtHeader *dt_header = ASSERT_PTR(dtb_buffer);

        if (dtb_length < sizeof(FdtHeader) ||
            dtb_length < be32toh(dt_header->total_size))
                return EFI_INVALID_PARAMETER;

        if (!compat)
                return EFI_INVALID_PARAMETER;

        const char *dt_compat = devicetree_get_compatible(dtb_buffer);
        if (!dt_compat)
                return EFI_INVALID_PARAMETER;
