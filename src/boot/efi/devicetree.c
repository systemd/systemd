...
static VOID *devicetree_ptr(const struct devicetree_state *state) {
        ...
        return (VOID *)(unsigned long)state->addr;
}
...
static EFI_STATUS devicetree_fixup(struct devicetree_state *state, UINTN len) {
        ...
        if (err == EFI_BUFFER_TOO_SMALL) {
                ...
                CopyMem(devicetree_ptr(state), (VOID *)(unsigned long)oldaddr, len);
...
EFI_STATUS devicetree_install(struct devicetree_state *state,
        ...
        err = LibGetSystemConfigurationTable(&(EFI_GUID)EFI_DTB_TABLE_GUID, &state->orig);
        if (EFI_ERROR(err))
        ...
        return uefi_call_wrapper(BS->InstallConfigurationTable, 2, &(EFI_GUID)EFI_DTB_TABLE_GUID,
...
void devicetree_cleanup(struct devicetree_state *state) {
        ...
        err = uefi_call_wrapper(BS->InstallConfigurationTable, 2, &(EFI_GUID)EFI_DTB_TABLE_GUID,
                                state->orig);
