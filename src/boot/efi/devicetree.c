...
static EFI_STATUS devicetree_get(struct devicetree_state *state) {
        ...
        for (; entry < end; entry++) {
                if (CompareGuid(&entry->VendorGuid, &(EFI_GUID)DEVICE_TREE_GUID) == 0) {
...
static EFI_STATUS devicetree_fixup(struct devicetree_state *state, UINTN len) {
        ...
        if (err == EFI_BUFFER_TOO_SMALL) {
                ...
                CopyMem(devicetree_ptr(state), (VOID *)oldaddr, len);
...
EFI_STATUS devicetree_install(struct devicetree_state *state,
        ...
        return uefi_call_wrapper(BS->InstallConfigurationTable, 2,
                        &(EFI_GUID)DEVICE_TREE_GUID, devicetree_ptr(state));
}
...
void devicetree_cleanup(struct devicetree_state *state) {
        ...
        err = uefi_call_wrapper(BS->InstallConfigurationTable, 2,
                        &(EFI_GUID)DEVICE_TREE_GUID, state->orig);
