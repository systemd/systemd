...
typedef enum NumberFormatFlags {
        NUMBER_FORMAT_DEFAULT    = 1 << 0,
        NUMBER_FORMAT_HEX        = 1 << 1,
        NUMBER_FORMAT_HEX_PREFIX = 1 << 2,
        NUMBER_FORMAT_LOWER      = 1 << 3,
        NUMBER_FORMAT_SIGNED     = 1 << 4,

        NUMBER_FORMAT_POINTER = NUMBER_FORMAT_HEX | NUMBER_FORMAT_HEX_PREFIX | NUMBER_FORMAT_LOWER,
} NumberFormatFlags;
...
static void format_number(
        ...
        size_t n = 0;
        int base = FLAGS_SET(flags, NUMBER_FORMAT_HEX) ? 16 : 10;
        char16_t tmp[SCRATCH_BUF_SIZE];
        const char16_t *digits = FLAGS_SET(flags, NUMBER_FORMAT_LOWER) ? u"0123456789abcdef" :
                                                                         u"0123456789ABCDEF";

        if (FLAGS_SET(flags, NUMBER_FORMAT_SIGNED) && (int64_t) u < 0) {
                *buf++ = '-';
        ...
        if (FLAGS_SET(flags, NUMBER_FORMAT_HEX_PREFIX)) {
                *buf++ = '0';
                *buf++ = FLAGS_SET(flags, NUMBER_FORMAT_LOWER) ? 'x' : 'X';
        }
...
static const char16_t * const warn_table[] = {
        u"Success",
        u"Unknown glyph",
        u"Delete failure",
        u"Write failure",
...
static const char16_t * const err_table[] = {
        u"Error",
        u"Load error",
        u"Invalid parameter",
        u"Unsupported",
        ...
        u"TFTP error",
        u"Protocol error",
        u"Incompatible version",
        u"Security violation",
        u"CRC error",
        u"End of media",
        u"Reserved (29)",
        u"Reserved (30)",
        ...
        u"Invalid language",
        u"Compromised data",
        u"IP address conflict",
        u"HTTP error",
};

#ifndef EFI_ERROR_MASK
#  define EFI_ERROR_MASK ((uintptr_t) 1 << (sizeof(uintptr_t) * CHAR_BIT - 1))
#endif

enum {
        EFI_WARN_MAX = 7,
        EFI_ERROR_MIN = 0 | EFI_ERROR_MASK,
        EFI_ERROR_MAX = 35 | EFI_ERROR_MASK,
};

static void format_status(char16_t tmp[SCRATCH_BUF_SIZE], EFI_STATUS status) {
        if (status <= EFI_WARN_MAX) {
                strcpy16(tmp, warn_table[status]);
                return;
        }

        if (status >= EFI_ERROR_MIN && status <= EFI_ERROR_MAX) {
                strcpy16(tmp, err_table[status - EFI_ERROR_MIN]);
                return;
        }

        format_number(tmp, 6, NUMBER_FORMAT_POINTER, status);
}
...
static const char *parse_format_specifier(
        ...
        for (;; format++)
                ...
                case 'X': {
                        ...
                        NumberFormatFlags flags = NUMBER_FORMAT_DEFAULT;
                        SET_FLAG(flags, NUMBER_FORMAT_HEX, IN_SET(*format, 'x', 'X'));
                        SET_FLAG(flags, NUMBER_FORMAT_HEX_PREFIX, alternative_form);
                        SET_FLAG(flags, NUMBER_FORMAT_SIGNED, IN_SET(*format, 'd', 'i'));
                        SET_FLAG(flags, NUMBER_FORMAT_LOWER, IN_SET(*format, 'x'));

                        switch (field_length) {
                        case sizeof(int32_t):
                                if (FLAGS_SET(flags, NUMBER_FORMAT_SIGNED))
                        ...
                        *ret_precision = MAX(0, *ret_precision);
                        ...
                        if (FLAGS_SET(flags, NUMBER_FORMAT_HEX | NUMBER_FORMAT_HEX_PREFIX) && *ret_pad_zero) {
                                *ret_pad_zero = false;
                                *ret_precision = MAX(*ret_field_width, *ret_precision) - 2;
                        }

                        format_number(tmp, *ret_precision, flags, n.u);
                        *ret_str16 = tmp;
                ...
                case 'p': {
                        ...
                        if (ptr) {
                                format_number(tmp, 8, NUMBER_FORMAT_POINTER, (uintptr_t) ptr);
                                *ret_str16 = tmp;
                        }
 ...
 *
 * Supported flags: #, 0
 * Supported lengths: h, hh, l, ll, z
 * Supported specifiers: %, c, s, u, i, d, x, X, p, m
 *
 * Strings and numbers also support field precision and width either within the format specifier or as an
 * additional arg using *.
 *
 * Notable differences:
 *  - Passing NULL to %s is permitted and will print "(null)".
 *  - %p will also use "(null)".
 *  - If buf is too small, -1 is returned instead of needed buf size (printf/xasprintf will simply grow the
 *    buffer by a fixed amount).
