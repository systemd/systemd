...
static bool push_str(FormatContext *ctx, SpecifierContext *sp) {
        ...
        grow_buf(ctx, sp->field_width + sp->precision);

        if (!sp->align_left)
                push_padding(ctx, ' ', sp->field_width);

        /* In userspace unit tests we cannot just memcpy() the wide string. */
        if (sp->wstr && sizeof(wchar_t) == sizeof(char16_t)) {
                memcpy(ctx->buf + ctx->n, sp->wstr, sp->precision * sizeof(*sp->wstr));
                ctx->n += sp->precision;
        } else
                for (size_t i = 0; i < sp->precision; i++)
...
static bool handle_format_specifier(FormatContext *ctx, SpecifierContext *sp) {
        ...
        case 's':
                if (sp->long_arg) {
                        ...
                        sp->precision = wcsnlen(sp->wstr, sp->precision);
                } else {
                        sp->str = va_arg(ctx->ap, const char *) ?: "(null)";
                        sp->precision = strnlen8(sp->str, sp->precision);
                }
                return push_str(ctx, sp);
        ...
        while (*ctx.format != '\0') {
                ...
                default:
                        ...
                        while (!IN_SET(*ctx.format, '%', '\n', '\0'))
                                ctx.format++;
                        sp.precision = ctx.format - sp.str;
                        push_str(&ctx, &sp);
                }
        }
