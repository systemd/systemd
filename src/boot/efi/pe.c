...
#  define TARGET_MACHINE_TYPE_COMPAT EFI_IMAGE_MACHINE_X64
...
#ifndef TARGET_MACHINE_TYPE_COMPAT
#  define TARGET_MACHINE_TYPE_COMPAT 0
#endif
...
static inline BOOLEAN verify_pe(const struct PeFileHeader *pe, BOOLEAN allow_compat) {
        assert(pe);
        return CompareMem(pe->Magic, PE_FILE_MAGIC, STRLEN(PE_FILE_MAGIC)) == 0 &&
               (pe->FileHeader.Machine == TARGET_MACHINE_TYPE ||
                (allow_compat && pe->FileHeader.Machine == TARGET_MACHINE_TYPE_COMPAT)) &&
...
EFI_STATUS pe_alignment_info(
        ...
        if (!verify_pe(pe, /* allow_compat= */ TRUE))
                return EFI_LOAD_ERROR;
...
EFI_STATUS pe_memory_locate_sections(
        ...
        if (!verify_pe(pe, /* allow_compat= */ FALSE))
                return EFI_LOAD_ERROR;
...
EFI_STATUS pe_file_locate_sections(
        ...
        if (EFI_ERROR(err))
                return err;
        if (len != sizeof(pe) || !verify_pe(&pe, /* allow_compat= */ FALSE))
                return EFI_LOAD_ERROR;
