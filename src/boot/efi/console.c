...
static EFI_STATUS change_mode(UINT32 mode) {
        EFI_STATUS err;
        UINT32 old_mode = ST->ConOut->Mode->Mode;
        ...
        uefi_call_wrapper(ST->ConOut->SetMode, 2, ST->ConOut, CONSOLE_MODE_FIRMWARE_RANGE_MIN);
        return err;
}

static UINT32 get_auto_mode(void) {
...
EFI_STATUS console_set_mode(UINT32 mode) {
        ...
        case CONSOLE_MODE_KEEP:
                ...
                if ((UINT32)ST->ConOut->Mode->Mode == CONSOLE_MODE_INVALID)
                        return change_mode(CONSOLE_MODE_FIRMWARE_RANGE_MIN);
                return EFI_SUCCESS;

        case CONSOLE_MODE_NEXT:
                if (ST->ConOut->Mode->MaxMode <= CONSOLE_MODE_FIRMWARE_RANGE_MIN)
                        return EFI_UNSUPPORTED;

                /* If mode is invalid (-1) here we'll end up switching to mode 0, which is desired. */
                mode = (UINT32)ST->ConOut->Mode->Mode;
                do {
                ...
                } while (mode > CONSOLE_MODE_FIRMWARE_RANGE_MIN);
        ...
        case CONSOLE_MODE_FIRMWARE_IMPLEMENTED_MAX:
                /* Note: MaxMode is the number of modes, not the last mode. */
                if (ST->ConOut->Mode->MaxMode > 0)
                        mode = ST->ConOut->Mode->MaxMode - 1;
                else
                        mode = CONSOLE_MODE_FIRMWARE_RANGE_MIN;
                _fallthrough_;
