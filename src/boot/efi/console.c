...
EFI_STATUS console_key_read(uint64_t *key, uint64_t timeout_usec) {
        ...
        if (!checked) {
                ...
                err = BS->LocateProtocol(&MAKE_GUID(EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL), NULL, (void **) &extraInEx);
                ...
                err = BS->HandleProtocol(ST->ConsoleInHandle, &MAKE_GUID(EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL), (void **) &conInEx);
...
EFI_STATUS query_screen_resolution(uint32_t *ret_w, uint32_t *ret_h) {
        ...
        err = BS->LocateProtocol(&MAKE_GUID(EFI_GRAPHICS_OUTPUT_PROTOCOL), NULL, (void **) &go);
        if (err != EFI_SUCCESS)
                return err;
