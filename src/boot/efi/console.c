...
static EFI_STATUS change_mode(UINT32 mode) {
        ...
        uefi_call_wrapper(ST->ConOut->SetMode, 2, ST->ConOut, CONSOLE_MODE_MIN);
        return err;
}
...
static UINT32 get_auto_mode(void) {
        ...
        if (ST->ConOut->Mode->MaxMode > CONSOLE_MODE_IMPL_FIRST)
                return CONSOLE_MODE_IMPL_FIRST;
...
EFI_STATUS console_set_mode(UINT32 mode) {
        ...
        case CONSOLE_MODE_KEEP:
                ...
                if ((UINT32)ST->ConOut->Mode->Mode == CONSOLE_MODE_INVALID)
                        return change_mode(CONSOLE_MODE_MIN);
                return EFI_SUCCESS;

        case CONSOLE_MODE_NEXT:
                if (ST->ConOut->Mode->MaxMode <= CONSOLE_MODE_MIN)
        ...
        case CONSOLE_MODE_AUTO_MAX:
                mode = MAX(CONSOLE_MODE_MIN, ST->ConOut->Mode->MaxMode - 1);
                _fallthrough_;
