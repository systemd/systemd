...
static inline void *array_element(const void *array, size_t stride, size_t i) {
...
void sort_array(
        ...
        for (size_t i = 1; i < n_members; i++) {
                size_t k;
                void *entry = array_element(array, stride, i);

                for (k = i; k > 0; k--) {
                        if (compare(array_element(array, stride, k - 1), entry) <= 0)
                                break;
                ...
                memcpy(array_element(array, stride, k), entry, stride);
        }
}
...
void *bsearch_array(
        ...
        while (left_idx <= right_idx) {
                ...
                int ret = key_compare(key, mid_elem);
                if (ret < 0)
                        left_idx = mid_idx + 1;
                else if (ret > 0)
                        right_idx = mid_idx - 1;
