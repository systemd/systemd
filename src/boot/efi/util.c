...
#define QEMU_KERNEL_LOADER_FS_MEDIA_GUID \
        &(const EFI_GUID) { 0x1428f772, 0xb64a, 0x441e, {0xb8, 0xc3, 0x9e, 0xbd, 0xd7, 0xf8, 0x93, 0xc7 }}

#define QEMU_BOOT_ORDER_GUID \
        &(const EFI_GUID) { 0x668f4529, 0x63d0, 0x4bb5, {0xb6, 0x5d, 0x6f, 0xbb, 0x9d, 0x36, 0xa4, 0x4a }}
...
bool is_qemu_direct_boot(EFI_HANDLE device) {
        ...
        if (dp->Header.Type == MEDIA_DEVICE_PATH &&
            ...
            memcmp(&dp->Guid, QEMU_KERNEL_LOADER_FS_MEDIA_GUID, sizeof(EFI_GUID)) == 0)
                return true;
