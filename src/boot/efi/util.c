...
EFI_STATUS chunked_read(EFI_FILE *file, size_t *size, void *buf) {
        ...
        uint64_t offset;
        err = file->GetPosition(file, &offset);
        if (err != EFI_SUCCESS)
                return err;

        /* Try reading the whole file first. */
        size_t chunk = *size;
        err = file->Read(file, &chunk, buf);
        if (err == EFI_SUCCESS) { /* Yay! */
                *size = chunk;
                return EFI_SUCCESS;
        }

        /* Just to make sure, we reset the offset before we try reading in small chunks. */
        err = file->SetPosition(file, offset);
        if (err != EFI_SUCCESS)
                return err;

        /* The firmware is bugged and cannot handle large reads. We'll try reading in 1MB chunks instead. */
