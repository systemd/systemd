...
EFI_STATUS initrd_register(
         ...
         */
        err = BS->LocateDevicePath(&MAKE_GUID(EFI_LOAD_FILE2_PROTOCOL), &dp, &handle);
        ...
        *loader = (struct initrd_loader) {
        ...
        err = BS->InstallMultipleProtocolInterfaces(
                        ret_initrd_handle,
                        &MAKE_GUID(EFI_DEVICE_PATH_PROTOCOL), &efi_initrd_device_path,
                        &MAKE_GUID(EFI_LOAD_FILE2_PROTOCOL), loader,
                        NULL);
        if (err != EFI_SUCCESS)
                free(loader);
...
EFI_STATUS initrd_unregister(EFI_HANDLE initrd_handle) {
        ...
        err = BS->OpenProtocol(
                        initrd_handle, &MAKE_GUID(EFI_LOAD_FILE2_PROTOCOL), (void **) &loader,
                        NULL, NULL, EFI_OPEN_PROTOCOL_GET_PROTOCOL);
        if (err != EFI_SUCCESS)
                return err;

        /* close the handle */
        (void) BS->CloseProtocol(initrd_handle, &MAKE_GUID(EFI_LOAD_FILE2_PROTOCOL), NULL, NULL);
