...
#define LINUX_EFI_RANDOM_SEED_TABLE_GUID \
        &(EFI_GUID) { 0x1ce1e5bc, 0x7ceb, 0x42f2,  { 0x81, 0xe5, 0x8a, 0xad, 0xf1, 0x80, 0xf5, 0x7b } }
...
static EFI_STATUS acquire_rng(void *out, UINTN size) {
        ...
        err = rng->GetRNG(rng, NULL, size, out);
        if (err != EFI_SUCCESS)
...
EFI_STATUS process_random_seed(EFI_FILE *root_dir, RandomSeedMode mode) {
        unsigned char random_bytes[DESIRED_SEED_SIZE], hash_key[HASH_VALUE_SIZE];
        struct linux_efi_random_seed *prior_seed_table = NULL, *new_seed_table;
        _cleanup_free_ void *seed = NULL, *system_token = NULL;
        _cleanup_(file_closep) EFI_FILE *handle = NULL;
        _cleanup_free_ EFI_FILE_INFO *info = NULL;
        uint64_t uefi_monotonic_counter = 0;
        UINTN size, rsize, wsize;
        unsigned char usage_idx;
        struct sha256_ctx hash;
        bool got_rng_bytes;
        EFI_STATUS err;

        assert(root_dir);
        assert(DESIRED_SEED_SIZE == HASH_VALUE_SIZE);
        ...
        err = LibGetSystemConfigurationTable(LINUX_EFI_RANDOM_SEED_TABLE_GUID, (void **) &prior_seed_table);
        if (err != EFI_SUCCESS) {
                size = 0;
                sha256_process_bytes(&size, sizeof(size), &hash);
        } else {
                size = prior_seed_table->size;
                sha256_process_bytes(&size, sizeof(size), &hash);
                sha256_process_bytes(prior_seed_table->seed, size, &hash);

                /* Zero and free the old seed table only at the end after we've managed to install a new one,
                 * so that in case this function fails or aborts, Linux still receives whatever the previous
                 * bootloader chain set. So, the next line of this block is not an explicit_bzero() call. */

                /* XXX: We should probably have a _cleanup_empty_ annotation on hash, since we don't want to
                 * leave around residue of this if this function fails. */
        }
        ...
        explicit_bzero_safe(random_bytes, size);
        ...
        if (mode != RANDOM_SEED_ALWAYS && err != EFI_SUCCESS && !got_rng_bytes)
                return err;
        sha256_process_bytes(&size, sizeof(size), &hash);
        if (system_token) {
        ...
        if (size < info->FileSize) {
                info->FileSize = size;
                err = handle->SetInfo(handle, &GenericFileInfo, sizeof(*info), &info->Size);
                if (err != EFI_SUCCESS)
        ...
        explicit_bzero_safe(hash_key, sizeof(hash_key));
        explicit_bzero_safe(&hash, sizeof(hash));

        err = BS->AllocatePool(EFI_ACPI_RECLAIM_MEMORY, sizeof(*new_seed_table) + sizeof(random_bytes),
                               (void **) &new_seed_table);
        if (err != EFI_SUCCESS)
                return log_error_status_stall(err, L"Failed to allocate EFI table for random seed: %r", err);
        new_seed_table->size = sizeof(random_bytes);
        memcpy(new_seed_table->seed, random_bytes, new_seed_table->size);
        explicit_bzero_safe(random_bytes, sizeof(random_bytes));

        err = BS->InstallConfigurationTable(LINUX_EFI_RANDOM_SEED_TABLE_GUID, new_seed_table);
        if (err != EFI_SUCCESS)
        ...
        if (prior_seed_table) {
                ...
                explicit_bzero_safe(&prior_seed_table->seed, prior_seed_table->size);
                explicit_bzero_safe(prior_seed_table, sizeof(*prior_seed_table));
                BS->FreePool(prior_seed_table);
        }
