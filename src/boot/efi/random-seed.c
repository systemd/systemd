...
static EFI_STATUS acquire_rng(UINTN size, VOID **ret) {
        ...
        if (EFI_ERROR(err))
                return log_error_status_stall(err, L"Failed to acquire RNG data: %r");
...
static EFI_STATUS acquire_system_token(VOID **ret, UINTN *ret_size) {
        ...
        if (EFI_ERROR(err)) {
                if (err != EFI_NOT_FOUND)
                        log_error_status_stall(err, L"Failed to read LoaderSystemToken EFI variable: %r");
                return err;
        }
...
EFI_STATUS process_random_seed(EFI_FILE *root_dir, RandomSeedMode mode) {
        ...
        if (EFI_ERROR(err)) {
                if (err != EFI_NOT_FOUND && err != EFI_WRITE_PROTECTED)
                        log_error_status_stall(err, L"Failed to open random seed file: %r");
                return err;
        }
        ...
        if (EFI_ERROR(err))
                return log_error_status_stall(err, L"Failed to read random seed file: %r");
        if (rsize != size)
                return log_error_status_stall(EFI_PROTOCOL_ERROR, L"Short read on random seed file.");

        err = uefi_call_wrapper(handle->SetPosition, 2, handle, 0);
        if (EFI_ERROR(err))
                return log_error_status_stall(err, L"Failed to seek to beginning of random seed file: %r");
        ...
        if (EFI_ERROR(err))
                return log_error_status_stall(err, L"Failed to write random seed file: %r");
        if (wsize != size)
                return log_error_status_stall(EFI_PROTOCOL_ERROR, L"Short write on random seed file.");

        err = uefi_call_wrapper(handle->Flush, 1, handle);
        if (EFI_ERROR(err))
                return log_error_status_stall(err, L"Failed to flush random seed file: %r");

        /* We are good to go */
        err = efivar_set_raw(LOADER_GUID, L"LoaderRandomSeed", for_kernel, size, 0);
        if (EFI_ERROR(err))
                log_error_status_stall(err, L"Failed to write random seed to EFI variable: %r");
