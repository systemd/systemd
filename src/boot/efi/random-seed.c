...
EFI_STATUS process_random_seed(EFI_FILE *root_dir, RandomSeedMode mode) {
        ...
        /* hash = LABEL || sizeof(input1) || input1 || ... || sizeof(inputN) || inputN || uefi_monotonic || time */
        ...
        for (i = 0; i < ST->NumberOfTableEntries; ++i)
                if (!memcmp(&(const EFI_GUID)LINUX_EFI_RANDOM_SEED_TABLE_GUID,
                            &ST->ConfigurationTable[i].VendorGuid, sizeof(EFI_GUID))) {
        ...
        if (err != EFI_SUCCESS)
                return log_error_status_stall(err, L"Failed to acquire timestamp: %r", err);
        sha256_process_bytes(&now, sizeof(now), &hash);
