...
EFI_STATUS linux_exec(
        ...
        err = pe_get_header_fields(linux_buffer, &header);
        if (EFI_ERROR(err))
                return err;

        /* Allocate kernel in aligned memory, and leave space of the BSS section */
        {
                kernel.num = EFI_SIZE_TO_PAGES(ALIGN_TO(header.image_size, header.alignment) + header.alignment);
                ...
                if (EFI_ERROR(err))
                        return EFI_OUT_OF_RESOURCES;
                new_buffer = PHYSICAL_ADDRESS_TO_POINTER(ALIGN_TO(kernel.addr, header.alignment));
        ...
        err = loaded_image_register(cmdline, cmdline_len, new_buffer, header.image_size, &loaded_image_handle);
        if (EFI_ERROR(err))
                return err;
