...
EFI_STATUS linux_exec(
        ...
        UINTN kernel_alignment, kernel_size_of_image;
        ...
        err = pe_alignment_info(linux_buffer, &kernel_entry, &kernel_size_of_image, &kernel_alignment);
        if (EFI_ERROR(err))
                return err;
        /* sanity check */
        ...
        */
        ...
        kernel_entry = (EFI_IMAGE_ENTRY_POINT) ((const UINT8 *)kernel_entry + ((const UINT8 *)new_buffer - (const UINT8 *)linux_buffer));
