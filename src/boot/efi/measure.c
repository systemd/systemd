...
typedef EFI_STATUS (EFIAPI *EFI_TCG2_GET_RESULT_OF_SET_ACTIVE_PCR_BANKS) (
                IN  EFI_TCG2_PROTOCOL  *This,
...
static EFI_STATUS
tpm1_measure_to_pcr_and_event_log (EFI_TCG *tcg, UINT32 pcrindex,
                                   EFI_PHYSICAL_ADDRESS buffer, UINTN buffer_size,
                                   CHAR16 *description)
{
        EFI_STATUS status;
        ...
        UINTN desc_len = (StrLen(description)+1) * sizeof(CHAR16);
...
static EFI_STATUS
tpm2_measure_to_pcr_and_event_log (EFI_TCG2 *tcg, UINT32 pcrindex,
                                   EFI_PHYSICAL_ADDRESS buffer, UINT64 buffer_size,
                                   CHAR16 *description)
{
        EFI_STATUS status;
        EFI_TCG2_EVENT *tcg_event;

        UINTN desc_len = StrLen(description) * sizeof(CHAR16);
        ...
        status = uefi_call_wrapper(tcg->HashLogExtendEvent, 5,
                                   tcg,
                                   0,
                                   buffer,
                                   buffer_size,
