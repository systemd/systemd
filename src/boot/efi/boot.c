...
static INTN config_entry_find(Config *config, CHAR16 *needle, BOOLEAN is_pattern) {
        assert(config);
        ...
        for (UINTN i = 0; i < config->entry_count; i++)
                if ((is_pattern && MetaiMatch(config->entries[i]->id, needle)) ||
                    (!is_pattern && streq(config->entries[i]->id, needle)))
                        return (INTN) i;
...
static void config_default_entry_select(Config *config) {
        ...
        i = config_entry_find(config, config->entry_oneshot, FALSE);
        if (i >= 0) {
