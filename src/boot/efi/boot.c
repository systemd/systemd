...
static void print_status(Config *config, CHAR16 *loaded_image_path) {
        ...
        ps_string(L"          architecture: %a\n",      EFI_MACHINE_TYPE_NAME);
            Print(L"    UEFI specification: %u.%02u\n", ST->Hdr.Revision >> 16, (UINT32) (ST->Hdr.Revision & 0xffff));
        ps_string(L"       firmware vendor: %s\n",      ST->FirmwareVendor);
            Print(L"      firmware version: %u.%02u\n", ST->FirmwareRevision >> 16, (UINT32) (ST->FirmwareRevision & 0xffff));
          ...
          ps_bool(L"                  shim: %s\n",      shim_loaded());
          ps_bool(L"                   TPM: %s\n",      tpm_present());
            Print(L"          console mode: %d/%ld (%lux%lu @%ux%u)\n", ST->ConOut->Mode->Mode, ST->ConOut->Mode->MaxMode - INT64_C(1), (UINT64) x_max, (UINT64) y_max, screen_width, screen_height);
        ...
        for (UINTN i = 0; i < config->entry_count; i++) {
                ConfigEntry *entry = config->entries[i];
                    ...
                    Print(L"  config entry: %lu/%lu\n", (UINT64) i + 1, (UINT64) config->entry_count);
                ps_string(L"            id: %s\n", entry->id);
                ps_string(L"         title: %s\n", entry->title);
                ...
                if (entry->tries_left != UINTN_MAX) {
                    Print(L"         tries: %lu done, %lu left\n", (UINT64) entry->tries_done, (UINT64) entry->tries_left);
        ...
        while (!exit) {
                ...
                case KEYPRESS(0, 0, 'v'):
                        status = xpool_print(L"systemd-boot " GIT_VERSION " (" EFI_MACHINE_TYPE_NAME "), "
                                             ...
                                             ST->Hdr.Revision >> 16,
                                             (UINT32) (ST->Hdr.Revision & 0xffff),
                                             ST->FirmwareVendor,
                                             ST->FirmwareRevision >> 16,
                                             (UINT32) (ST->FirmwareRevision & 0xffff));
                        break;
...
good:
        ...
        entry->next_name = xpool_print(L"%s+%lu-%lu%s", prefix, (UINT64) next_left, (UINT64) next_done, strempty(suffix));
}
...
static void export_variables(
        ...
        infostr = xpool_print(L"%s %u.%02u", ST->FirmwareVendor, ST->FirmwareRevision >> 16, (UINT32) (ST->FirmwareRevision & 0xffff));
        efivar_set(LOADER_GUID, L"LoaderFirmwareInfo", infostr, 0);

        typestr = xpool_print(L"UEFI %u.%02u", ST->Hdr.Revision >> 16, (UINT32) (ST->Hdr.Revision & 0xffff));
