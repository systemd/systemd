...
static BOOLEAN menu_run(
        ...
        if (EFI_ERROR(err)) {
                uefi_call_wrapper(ST->ConOut->ClearScreen, 1, ST->ConOut);
                log_error_stall(L"Error switching console mode to %lu: %r", config->console_mode, err);
        }
        ...
        while (!exit) {
                ...
                case KEYPRESS(0, 0, 'r'):
                        if (config->console_mode_efivar < (UINT64)ST->ConOut->Mode->MaxMode &&
                            ST->ConOut->Mode->Mode == ST->ConOut->Mode->MaxMode - 1) {
                                ...
                                efivar_set_uint_string(LOADER_GUID,
                                                       L"LoaderConfigConsoleMode",
                                                       config->console_mode_efivar,
                                                       EFI_VARIABLE_NON_VOLATILE);
                                status = PoolPrint(L"Console mode changed to %lu.", config->console_mode_efivar);
                                new_mode = TRUE;
                        }
                        break;
        ...
        while ((line = line_get_key_value(content, (CHAR8 *)" \t", &pos, &key, &value))) {
                ...
                if (strcmpa((CHAR8 *)"console-mode", key) == 0) {
                        if (strcmpa((CHAR8 *)"auto", value) == 0)
                                ...
                                config->console_mode = CONSOLE_MODE_MAX;
...
static VOID config_load_defaults(Config *config, EFI_FILE *root_dir) {
        ...
        *config = (Config) {
        ...
        efivar_get_uint_string(LOADER_GUID, L"LoaderConfigConsoleMode", &config->console_mode_efivar);
}
