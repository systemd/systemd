...
static void print_status(Config *config, char16_t *loaded_image_path) {
        ...
        printf("         auto-shutdown: %ls\n", yes_no(config->auto_shutdown));
...
static void config_defaults_load_from_file(Config *config, char *content) {
        ...
        while ((line = line_get_key_value(content, " \t", &pos, &key, &value))) {
                ...
                if (streq8(key, "auto-shutdown")) {
                        err = parse_boolean(value, &config->auto_shutdown);
                        if (err != EFI_SUCCESS)
                                log_error("Error parsing 'auto-shutdown' config option: %s", value);
                        continue;
                }
                ...
                if (streq8(key, "auto-reboot")) {
                        ...
                        if (err != EFI_SUCCESS)
                                log_error("Error parsing 'auto-reboot' config option: %s", value);
                        continue;
                }
