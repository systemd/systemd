...
static VOID print_status(Config *config, CHAR16 *loaded_image_path) {
        ...
        Print(L"console mode:           %d/%ld\n", ST->ConOut->Mode->Mode, ST->ConOut->Mode->MaxMode - 1L);
...
static BOOLEAN menu_run(
        ...
        INT64 console_mode_efivar = config->console_mode_efivar;
        ...
        while (!exit) {
                ...
                case KEYPRESS(0, 0, 'r'):
                case KEYPRESS(0, 0, 'R'):
                        if (config->console_mode_efivar < ST->ConOut->Mode->MaxMode &&
                            ST->ConOut->Mode->Mode == ST->ConOut->Mode->MaxMode - 1L) {
                                config->console_mode_efivar = CONSOLE_MODE_KEEP;
                                status = StrDuplicate(L"Console mode defined by firmware default or configuration file on next boot.");
                        } else {
        ...
        *chosen_entry = config->entries[idx_highlight];
        ...
        if (console_mode_efivar != config->console_mode_efivar) {
        ...
        while ((line = line_get_key_value(content, (CHAR8 *)" \t", &pos, &key, &value))) {
                ...
                if (strcmpa((CHAR8 *)"console-mode", key) == 0) {
                        if (strcmpa((CHAR8 *)"auto", value) == 0)
                                ...
                                config->console_mode = CONSOLE_MODE_FIRMWARE_IMPLEMENTED_MAX;
                                ...
                                s = stra_to_str(value);
                                config->console_mode = MIN(Atoi(s), CONSOLE_MODE_RANGE_MAX);
                        }
