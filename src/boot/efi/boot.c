 ...
 *
 * Note: all the values below are ABI, so they are not allowed to change.
 * The bootctl tool sets the numerical value of TIMEOUT_MENU_FORCE and
 * TIMEOUT_MENU_HIDDEN, instead of the string for compatibility reasons.
 *
 * The other values may be set by systemd-boot itself and changing those
 * will lead to funtional regression when new version of systemd-boot is
 * installed.
 *
 * All the 64bit values are not ABI and will never be written to an efi
 * variable.
 */
enum {
        TIMEOUT_MIN           = 1,
...
static void print_status(Config *config, char16_t *loaded_image_path) {
        ...
        print_timeout_status("     timeout (EFI var)", config->timeout_sec_config);
