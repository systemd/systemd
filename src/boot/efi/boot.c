...
static VOID print_status(Config *config, CHAR16 *loaded_image_path) {
        ...
        if (efivar_get_int(GLOBAL_GUID, L"OsIndicationsSupported", &indvar) == EFI_SUCCESS)
...
static EFI_STATUS reboot_into_firmware(VOID) {
        ...
        err = efivar_get_int(GLOBAL_GUID, L"OsIndications", &old);
        if (!EFI_ERROR(err))
                new |= old;
...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *sys_table) {
        ...
        if (config.auto_firmware && efivar_get_int(GLOBAL_GUID, L"OsIndicationsSupported", &osind) == EFI_SUCCESS) {
