...
static EFI_STATUS image_start(
        ...
        if (EFI_ERROR(err))
                return log_error_status_stall(err, L"Error loading %s: %r", entry->loader);
        ...
        if (options) {
                ...
                if (EFI_ERROR(err)) {
                        log_error_status_stall(err, L"Error getting LoadedImageProtocol handle: %r");
                        goto out_unload;
                }
                ...
                if (EFI_ERROR(err))
                        log_error_status_stall(err, L"Unable to add image options measurement: %r");
#endif
        }
...
static EFI_STATUS reboot_into_firmware(VOID) {
        ...
        return log_error_status_stall(err, L"Error calling ResetSystem: %r");
}
...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *sys_table) {
        ...
        if (EFI_ERROR(err))
                return log_error_status_stall(err, L"Error getting a LoadedImageProtocol handle: %r");
        ...
        if (secure_boot_enabled() && shim_loaded()) {
                ...
                if (EFI_ERROR(err))
                        return log_error_status_stall(err, L"Error installing security policy: %r");
        }
