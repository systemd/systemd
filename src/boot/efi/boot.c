...
static void config_load_type2_entries(
        ...
        for (;;) {
                ...
                struct PeSectionDescriptor *osrel_section = pe_bsearch_section(sections, nsections, ".osrel");
                if (osrel_section == NULL)
                        continue;
                err = file_read(linux_dir, f->FileName, osrel_section->offset, osrel_section->size, &osrel_content, NULL);
                if (err != EFI_SUCCESS)
                        continue;
                ...
                *entry = (BootEntry) {
                ...
                struct PeSectionDescriptor *cmdline_section = pe_bsearch_section(sections, nsections, ".cmdline");
                if (cmdline_section == NULL)
                        continue;

                /* read the embedded cmdline file */
                size_t cmdline_len;
                err = file_read(linux_dir, f->FileName, cmdline_section->offset, cmdline_section->size, &cmdline_content, &cmdline_len);
                if (err == EFI_SUCCESS) {
