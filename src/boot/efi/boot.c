...
typedef struct Config {
        ...
        BOOLEAN auto_enroll_keys;
        BOOLEAN allow_unsafe_enrollement;
        BOOLEAN force_menu;
...
static void print_status(Config *config, CHAR16 *loaded_image_path) {
        ...
        ps_string(L"             saved entry: %s\n", config->entry_saved);
          ...
          ps_bool(L"        auto-enroll-keys: %s\n", config->auto_enroll_keys);
        Print(L"*** DANGER ***\n");
        Print(L"Setting this option to true might soft-brick your machine.\n");
          ps_bool(L"allow-unsafe-enrollement: %s\n", config->allow_unsafe_enrollement);
        Print(L"*** DANGER ***\n");
...
static void config_defaults_load_from_file(Config *config, CHAR8 *content) {
        ...
        while ((line = line_get_key_value(content, (CHAR8 *)" \t", &pos, &key, &value))) {
                ...
                if (strcmpa((CHAR8 *)"auto-enroll-keys", key) == 0) {
                        err = parse_boolean(value, &config->auto_enroll_keys);
                        if (EFI_ERROR(err))
                                log_error_stall(L"Error parsing 'auto-enroll-keys' config option: %a", value);
                        continue;
                }

                if (strcmpa((CHAR8 *)"i-might-brick-my-machine-please-auto-enroll-keys", key) == 0) {
                        err = parse_boolean(value, &config->allow_unsafe_enrollement);
                        if (EFI_ERROR(err))
                                log_error_stall(L"Error parsing 'i-might-brick-my-machine-please-auto-enroll-keys' config option: %a", value);
                        continue;
                }
...
static void config_load_defaults(Config *config, EFI_FILE *root_dir) {
        ...
        *config = (Config) {
                ...
                .auto_enroll_keys = FALSE,
                .allow_unsafe_enrollement = FALSE,
...
static EFI_STATUS secure_boot_discover_keys(Config *config, EFI_FILE *root_dir) {
        ...
        for (;;) {
                ...
                if (config->auto_enroll_keys && config->allow_unsafe_enrollement && strcaseeq(dirent->FileName, L"auto")) {
