...
static VOID print_status(Config *config, CHAR16 *loaded_image_path) {
        ...
        case CONSOLE_MODE_SET:
                ...
                SPrint(console_mode_value, ELEMENTSOF(console_mode_value), L"%d", config->console_mode);
                break;
        default:
                assert(false);
        }
...
static BOOLEAN menu_run(
        ...
        while (!exit) {
                ...
                case KEYPRESS(0, 0, 'R'):
                        ...
                        console_set_mode(config->console_mode, config->console_mode_change);
...
static VOID config_load_defaults(Config *config, EFI_FILE *root_dir) {
        ...
        *config = (Config) {
        ...
        err = efivar_get_int_string(LOADER_GUID, L"LoaderConfigTimeoutOneShot", &value);
        if (!EFI_ERROR(err)) {
