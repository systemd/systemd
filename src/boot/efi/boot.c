...
static BOOLEAN menu_run(
        ...
        BOOLEAN update_console_mode_efivar = FALSE;
        UINTN line_width = 0;
        ...
        if (EFI_ERROR(err)) {
                clear_screen(COLOR_NORMAL);
                log_error_stall(L"Error switching console mode to %u: %r", config->console_mode, err);
        }
        ...
        while (!exit) {
                ...
                case KEYPRESS(0, 0, 'r'):
                        if ((INT64)config->console_mode_efivar < ST->ConOut->Mode->MaxMode &&
                            ST->ConOut->Mode->Mode == ST->ConOut->Mode->MaxMode - 1) {
                                ...
                                status = PoolPrint(L"Console mode changed to %u.", config->console_mode_efivar);
                                new_mode = TRUE;
                        }
                        update_console_mode_efivar = TRUE;
                        break;
