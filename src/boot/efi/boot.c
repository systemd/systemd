...
static size_t entries_count(Config *config, BootEntry *visible_group) {
        assert(config);
...
static bool menu_run(
        ...
        while (IN_SET(action, ACTION_CONTINUE, ACTION_FIRMWARE_SETUP)) {
                ...
                case KEYPRESS(0, 0, 'E'):
                        ...
                        if (!config->editor ||
                            !IN_SET(config->entries[entry_index]->type, LOADER_EFI, LOADER_LINUX, LOADER_UNIFIED_LINUX)) {
...
static void config_copy_entries(Config *config, BootEntry **entries, size_t start_index, size_t end_index, bool has_group_entry) {
        assert(config);
        assert(entries);
        ...
        if (has_group_entry){
                ...
                *group_entry = (BootEntry){ .title = xstrdup16(u"More..."), .group_entry = NULL, .type = LOADER_MORE, .id = xstrdup16(u"More...") };
