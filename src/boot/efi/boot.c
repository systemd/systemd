...
EFI_STATUS discover_root_dir(EFI_LOADED_IMAGE_PROTOCOL *loaded_image, EFI_FILE **ret_dir) {
        EFI_STATUS err;

        if (is_direct_boot(loaded_image->DeviceHandle))
                err = qemu_open(&loaded_image->DeviceHandle, ret_dir);
        else
                err = open_volume(loaded_image->DeviceHandle, ret_dir);
        return err;
}
