...
enum {
        ...
        CONSOLE_MODE_RANGE_MIN = 0L,
        CONSOLE_MODE_RANGE_MAX = INT32_MAX, /* This is just the theoretical limit. */
        CONSOLE_MODE_INVALID = -1L,         /* UEFI uses -1 if the device is not in a valid text mode. */

        CONSOLE_MODE_80_25 = 0L,            /* 80x25 is required by UEFI spec. */
        CONSOLE_MODE_80_50 = 1L,            /* 80x50 may be supported. */
        CONSOLE_MODE_FIRMWARE_FIRST = 2L,   /* First custom mode, if supported. */

        /* These are our own mode values that map to concrete values at runtime. */
        CONSOLE_MODE_KEEP = CONSOLE_MODE_RANGE_MAX + 1L,
        CONSOLE_MODE_NEXT,
        CONSOLE_MODE_AUTO,
        CONSOLE_MODE_FIRMWARE_IMPLEMENTED_MAX, /* 'max' in config. */
};
