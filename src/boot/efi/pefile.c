...
EFI_STATUS pe_locate_sections(CHAR8 *base, CHAR8 **sections, UINTN *addrs, UINTN *offsets, UINTN *sizes) {
...
EFI_STATUS pefile_locate_sections(EFI_FILE *dir, CHAR16 *path, CHAR8 **sections, UINTN *addrs, UINTN *offsets, UINTN *sizes) {
        ...
        err = pe_locate_sections(header, sections, addrs, offsets, sizes);
out:
        if (header)
                FreePool(header);
