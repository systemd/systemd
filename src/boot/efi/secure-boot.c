...
EFI_STATUS secure_boot_enroll(EFI_FILE *root_dir, Config *config, ConfigEntry *entry) {
        ...
        for(;;) {
                ...
                /* user aborted, we return EFI_SUCCESS so that the user can return to the boot menu */
                return EFI_SUCCESS;
        }

        _cleanup_(file_closep) EFI_FILE *dir = NULL;
        err = open_directory(root_dir, entry->path, &dir);
        if (EFI_ERROR(err))
                return log_error_status_stall(err, L"Failed opening keys directory %s: %r", entry->path, err);
        ...
        for (UINTN i = 0; i < ELEMENTSOF(sb_vars); i++) {
                ...
                if (EFI_ERROR(err)) {
                        log_error_stall(L"Failed reading file \\%s\\%s: %r", entry->path, sb_vars[i].filename, err);
                        goto out_deallocate;
                }
        }
