 ...
 *
 * Enabling custom mode worky only if a user is physically present.  Note that OVMF has a dummy
...
EFI_STATUS secure_boot_enroll_at(EFI_FILE *root_dir, const char16_t *path, bool force) {
        ...
        for (size_t i = 0; i < ELEMENTSOF(sb_vars); i++) {
                ...
                if (strcmp(sb_vars[i].name, u"PK") == 0 && sb_vars[i].size > 20) {
        ...
        if (need_custom_mode && !custom_mode_enabled()) {
                err = set_custom_mode(true);
                if (err == EFI_SUCCESS) {
                        printf("Custom mode enabled.\n");
                } else {
                        log_error_status(err, "Enable custom mode: %m");
                        goto out_deallocate;
                }
        }
