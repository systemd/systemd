...
EFI_STATUS tpm_log_load_options(const void *load_options, size_t size, const char16_t *description, bool *ret_measured);
...
static inline EFI_STATUS tpm_log_load_options(const void *load_options, size_t size, const char16_t *description, bool *ret_measured) {
        if (ret_measured)
                *ret_measured = false;
        return EFI_SUCCESS;
}
