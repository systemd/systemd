...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
        ...
        PRINTF_ONE(i->status, "%*.*s", i->field_width, MIN((int) len, i->precision), i->str);
        if (len >= sizeof(wchar_t))
                PRINTF_ONE(i->status,
                           "%*.*ls",
                           i->field_width,
                           MIN((int) (len / sizeof(wchar_t)), i->precision),
                           (const wchar_t *) i->str);
        ...
        PRINTF_ONE(i->status, "%*.*hhx", i->field_width, i->precision, i->uchar);
        PRINTF_ONE(i->status, "%*.*hx", i->field_width, i->precision, i->ushort);
        PRINTF_ONE(i->status, "%*.*x", i->field_width, i->precision, i->uint);
        PRINTF_ONE(i->status, "%*.*lx", i->field_width, i->precision, i->ulong);
        ...
        PRINTF_ONE(i->status, "%0*.*li", i->field_width, i->precision, i->slong);
        PRINTF_ONE(i->status, "%*.*lli", i->field_width, i->precision, i->slonglong);

        PRINTF_ONE(i->status, "%#-*.*hhX", i->field_width, i->precision, i->uchar);
        PRINTF_ONE(i->status, "%#0*.*hX", i->field_width, i->precision, i->ushort);
        PRINTF_ONE(i->status, "%#-*.*X", i->field_width, i->precision, i->uint);
        PRINTF_ONE(i->status, "%#0*.*lX", i->field_width, i->precision, i->ulong);
        PRINTF_ONE(i->status, "%*.*llX", i->field_width, i->precision, i->ulonglong);

        PRINTF_ONE(i->status, "%-*.*zx", i->field_width, i->precision, i->size);
        PRINTF_ONE(i->status, "% *.*zi", i->field_width, i->precision, i->ssize);
        PRINTF_ONE(i->status, "%0*.*ji", i->field_width, i->precision, i->intmax);
        PRINTF_ONE(i->status, "%#0*.*jX", i->field_width, i->precision, i->uintmax);
        PRINTF_ONE(i->status, "%*.*lx", i->field_width, i->precision, i->ptrdiff);
