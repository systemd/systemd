...
EFI_STATUS pack_cpio(
        ...
        /* EFI only supports vfat, so the error will be "unsupported" if the
         * image was loaded from a non-vfat partition, which might happen if,
         * say, UEFI loaded grub from an ESP, but then grub loaded the image
         * from a different, non-vfat partition. Do not stall in that case.
         */
