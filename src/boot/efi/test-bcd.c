...
TEST(bad_bcd) {
        ...
        Key *root = get_key(hbins, len, offset, "\0");
        assert_se(root);
        ...
        Key *kv_bad_offset = get_key(hbins, len, offset, "\0KeyValuesBadOffset\0");
        assert_se(kv_bad_offset);
        assert_se(!get_key_value(hbins, len, kv_bad_offset, "Dummy"));

        Key *kv_bad_n_key_values = get_key(hbins, len, offset, "\0KeyValuesBadNKeyValues\0");
        assert_se(kv_bad_n_key_values);
        assert_se(!get_key_value(hbins, len, kv_bad_n_key_values, "Dummy"));

        Key *kv = get_key(hbins, len, offset, "\0KeyValues\0");
        assert_se(kv);
