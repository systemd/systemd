 ...
 */
static EFI_STATUS hash_strings_sha1(EFI_SHA1_HASH2 *hash, const void *seed, size_t seed_len, int count, ...) {
        ...
        if (seed) {
                status = prot->HashUpdate(prot, seed, seed_len);
                if (EFI_STATUS_IS_ERROR(status))
                        goto exit;
        }
...
static EFI_STATUS get_chid(SmbiosInfo *info, unsigned id, EFI_GUID *ret_chid) {
        ...
        assert_cc(sizeof(hash) >= sizeof(*chid));
        memcpy(ret_chid, hash, sizeof(*ret_chid));

        /* Convert the resulting CHID back to little-endian: */
        chid->Data1 = bswap32(chid->Data1);
        chid->Data2 = bswap16(chid->Data2);
        chid->Data3 = bswap16(chid->Data3);
...
typedef struct Device {
        char16_t name[128];   /* Arbitrary size, shoule be big enough to fit any device name */
        char compatible[128]; /* Linux kernel limit compatible to 128 characters */
        EFI_GUID hwids[32];   /* Arbitrary size, should be at least 15 to fit all IDs from `fwupdtool hwids` */
} Device;

EFI_STATUS hwid_match(const void *hwids_buffer, size_t hwids_length, const char **ret_compatible) {
        EFI_STATUS status;
        ...
        if (cached_devices == devices) {
                ...
                *ret_compatible = cached_devices[cached_index].compatible;
                return EFI_SUCCESS;
        }
