...
#        define UINTN size_t
#        define strncasecmpa(a, b, n) strncasecmp((a), (b), (n))

#        define HOSTESD_STATIC static
#else
#        include <efi.h>
#        include "macro-fundamental.h"
#        include "util.h"
#        define HOSTESD_STATIC
#endif
...
static Key *get_key(const UINT8 *bcd, UINT32 bcd_len, UINT32 offset, const CHAR8 *name) {
        ...
        if (*name) {
                if (strncasecmpa(name, key->key_name, key->key_name_len) == 0 && !name[key->key_name_len])
...
static KeyValue *get_key_value(const UINT8 *bcd, UINT32 bcd_len, const Key *key, const CHAR8 *name) {
        ...
        for (UINT32 i = 0; i < key->n_key_values; i++) {
                ...
                if (strncasecmpa(name, kv->name, kv->name_len) == 0 && !name[kv->name_len])
                        return kv;
        }
...
HOSTESD_STATIC CHAR16 *get_bcd_title(UINT8 *bcd, UINTN bcd_len) {
        assert(bcd);
