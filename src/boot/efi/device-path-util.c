                                          ...
                                          **ret_dp)
{
        EFI_STATUS err;
        EFI_DEVICE_PATH *dp;
        EFI_DEVICE_PATH *original_device_path;
        size_t total_file_sizes = 0, n_files = 0;
        ...
        /* We need to allocate the original device path for each file
         * + the file path name size for each file
         * + the actual filepath device path structure for each file
         * + the efi device path structure for each file (the end marker) */
        *ret_dp = xmalloc(total_file_sizes + (o_dp_size + sizeof(FILEPATH_DEVICE_PATH) +
                                                        sizeof(EFI_DEVICE_PATH)) * n_files);

        dp = *ret_dp;
        STRV_FOREACH(file, files) {
                /* Prepend the original device path */
                dp = mempcpy(dp, original_device_path, o_dp_size);

                size_t file_size = strsize16(*file);
                const char16_t **peek = file;
                peek++;
                bool is_last_file = (*peek == NULL || isempty(*peek));
                ...
                *dp = is_last_file ? DEVICE_PATH_END_NODE : DEVICE_PATH_END_INSTANCE;
                dp = device_path_next_node(dp);
        }
...
EFI_STATUS make_file_device_path(EFI_HANDLE device, const char16_t *file, EFI_DEVICE_PATH **ret_dp) {
        const char16_t *singleton[2] = {file, NULL};
        return make_multiple_file_device_path(device, singleton, ret_dp);
}
