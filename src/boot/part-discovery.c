...
static EFI_STATUS try_gpt(
        ...
        err = read_gpt_entries(disk_io, media_id, block_size, lba, ret_backup_lba, &gpt, &entries);
        if (err != EFI_SUCCESS)
                return err;
...
static char16_t* disk_get_part_uuid_cdrom(const EFI_DEVICE_PATH *dp) {
        ...
        for (size_t i = 0; i < gpt.NumberOfPartitionEntries; i++) {
                ...
                uint64_t entry_start, cdrom_start;
                if (MUL_SAFE(&entry_start, entry->StartingLBA, sector_size) &&
                    MUL_SAFE(&cdrom_start, cdrom->PartitionStart, block_size) &&
