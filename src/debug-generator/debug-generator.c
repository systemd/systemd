...
static int process_unit_credentials(const char *credentials_dir) {
        ...
        FOREACH_ARRAY(i, des->entries, des->n_entries) {
                ...
                if ((in_initrd() && (e = startswith(de->d_name, "rd.systemd.unit-dropin."))) ||
                    (!in_initrd() && (e = startswith(de->d_name, "systemd.unit-dropin.")))) {
                        _cleanup_free_ void *d = NULL;

                        if (!unit_name_is_valid(e, UNIT_NAME_ANY)) {
                        ...
                        if (r < 0) {
                                log_warning_errno(r, "Failed to write drop-in for unit '%s' from credential '%s', ignoring: %m", e, de->d_name);
                                continue;
                        }
                        ...
                        if (r < 0) {
                                log_warning_errno(r, "Failed to write unit file '%s' from credential '%s', ignoring: %m", e, de->d_name);
                                continue;
                        }
