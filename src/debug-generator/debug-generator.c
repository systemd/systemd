...
static int process_unit_credentials(const char *credentials_dir) {
        ...
        if (IN_SET(fd, -ENXIO, -ENOENT)) /* Credential env var not set, or dir doesn't exist. */
                return 0;
        if (fd < 0)
                return log_error_errno(fd, "Failed to open credentials directory '%s': %m", credentials_dir);
        ...
        FOREACH_ARRAY(i, des->entries, des->n_entries) {
                ...
                if ((in_initrd() && (e = startswith(de->d_name, "systemd.initrd.unit-dropin."))) ||
                ...
                } else if ((in_initrd() && (e = startswith(de->d_name, "systemd.initrd.unit."))) ||
                           (!in_initrd() && (e = startswith(de->d_name, "systemd.unit.")))) {
                        _cleanup_free_ void *d = NULL;
                        _cleanup_free_ char *p = NULL;
