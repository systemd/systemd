...
static int parse_breakpoint_from_string(const char *s, uint32_t *ret_breakpoints) {
        ...
        if (isempty(s)) {
                if (in_initrd()) {
                        for (size_t i = 0; i < ELEMENTSOF(breakpoint_info_table); i++)
                                if (breakpoint_info_table[i].validity & BREAKPOINT_DEFAULT) {
                                        breakpoints |= 1 << breakpoint_info_table[i].type;
                                        break;
                                }
                } else
                ...
                for (;;) {
                        ...
                        if (in_initrd() && !(breakpoint_info_table[tt].validity & BREAKPOINT_IN_INITRD))
                                log_warning("Breakpoint '%s' not valid in the initrd, ignoring.", t);
                        else if (!in_initrd() && !(breakpoint_info_table[tt].validity & BREAKPOINT_ON_HOST))
