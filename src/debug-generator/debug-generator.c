...
typedef enum BootBreakpointType {
        BOOT_BREAKPOINT_PRE_UDEV,
        BOOT_BREAKPOINT_PRE_SYSROOT_MOUNT,
        BOOT_BREAKPOINT_PRE_SWITCH_ROOT,
        _BOOT_BREAKPOINT_TYPE_MAX,
        _BOOT_BREAKPOINT_TYPE_INVALID = -EINVAL,
} BootBreakpointType;
...
static BootBreakpointType arg_boot_breakpoint = _BOOT_BREAKPOINT_TYPE_INVALID;
...
static const char* const boot_breakpoint_type_table[_BOOT_BREAKPOINT_TYPE_MAX] = {
        [BOOT_BREAKPOINT_PRE_UDEV] = "pre-udev",
        [BOOT_BREAKPOINT_PRE_SYSROOT_MOUNT] = "pre-mount",
        [BOOT_BREAKPOINT_PRE_SWITCH_ROOT] = "pre-switch-root",
};

BootBreakpointType boot_breakpoint_type_from_string(const char *s);
const char* boot_breakpoint_type_to_string(BootBreakpointType t);

DEFINE_STRING_TABLE_LOOKUP(boot_breakpoint_type, BootBreakpointType);
...
static int parse_proc_cmdline_item(const char *key, const char *value, void *data) {
        ...
        } else if (proc_cmdline_key_streq(key, "systemd.boot_breakpoint")) {

                if (value) {
                        arg_boot_breakpoint = boot_breakpoint_type_from_string(value);
                        if (arg_boot_breakpoint < 0)
                                return arg_boot_breakpoint;
                        else if (!in_initrd() && IN_SET(arg_boot_breakpoint,
                                                        BOOT_BREAKPOINT_PRE_SYSROOT_MOUNT,
                                                        BOOT_BREAKPOINT_PRE_SWITCH_ROOT)) {
                                log_warning("Breakpoint '%s' only valid in the initrd, ignoring.", value);
                                arg_boot_breakpoint = _BOOT_BREAKPOINT_TYPE_INVALID;
                        }
                } else if (in_initrd())
                        arg_boot_breakpoint = BOOT_BREAKPOINT_PRE_SWITCH_ROOT;

                arg_debug_shell = arg_boot_breakpoint != _BOOT_BREAKPOINT_TYPE_INVALID;
...
static int install_debug_shell_dropin(void) {
        ...
        if (arg_boot_breakpoint != _BOOT_BREAKPOINT_TYPE_INVALID) {
                switch (arg_boot_breakpoint) {
                case BOOT_BREAKPOINT_PRE_UDEV:
                        r = write_drop_in_format(arg_dest, "debug-shell.service", 60, "breakpoint",
                                                 ...
                                                 "Type=oneshot\n"
                                                 "Restart=no\n",
                                                 boot_breakpoint_type_to_string(arg_boot_breakpoint));
                        break;

                case BOOT_BREAKPOINT_PRE_SYSROOT_MOUNT:
                        assert(in_initrd());
                        r = write_drop_in_format(arg_dest, "debug-shell.service", 60, "breakpoint",
                                                 "# Automatically generated by systemd-debug-generator\n\n"
                                                 "[Unit]\n"
                ...
                case BOOT_BREAKPOINT_PRE_SWITCH_ROOT:
                        assert(in_initrd());
                        r = write_drop_in_format(arg_dest, "debug-shell.service", 60, "breakpoint",
                                                 "# Automatically generated by systemd-debug-generator\n\n"
                                                 "[Unit]\n"
