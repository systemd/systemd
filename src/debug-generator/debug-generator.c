...
static const struct breakpoint_info *arg_breakpoint = NULL;
...
static const struct breakpoint_info breakpoint_type_table[_BREAKPOINT_TYPE_MAX] = {
        ...
        { BREAKPOINT_PRE_SHUTDOWN,      "pre-shutdown",    false, true  },
};

static int parse_breakpoint_from_string(const char *s, struct breakpoint_info const **ret_breakpoint) {
        /* Empty value? set default breakpoint */
        if (!s) {
                if (in_initrd())
                        *ret_breakpoint = &breakpoint_type_table[BREAKPOINT_PRE_SWITCH_ROOT];
                else {
                        *ret_breakpoint = NULL;
        ...
        for (size_t i = 0; i < ELEMENTSOF(breakpoint_type_table); i++)
                if (streq(breakpoint_type_table[i].name, s)) {
                        if (in_initrd() && !breakpoint_type_table[i].valid_in_initrd)
                                ...
                                *ret_breakpoint = &breakpoint_type_table[i];
...
static int parse_proc_cmdline_item(const char *key, const char *value, void *data) {
        ...
        if (streq(key, "systemd.mask")) {
                ...
                r = parse_breakpoint_from_string(value, &arg_breakpoint);
                if (r < 0)
                        return log_warning_errno(r, "Invalid breakpoint value '%s'", value);

                arg_debug_shell = arg_breakpoint != NULL;
