...
static int verb_show(int argc, char **argv, void *userdata) {
        ...
        if (strv_isempty(argv))
                for (const GptPartitionType *e = gpt_partition_type_table; e->name; e++) {
                        ...
                        /* The DPS says that the UUID for /var/ should be keyed with machine-id, so
                         * as a special case also show "var-uuid" after "var". */
                ...
                STRV_FOREACH(p, argv) {
                        ...
                        if (have_uuid)
                                ...
                                /* The DPS says that the UUID for /var/ should be keyed with machine-id, so
                                 * as a special case show that if we are asked for "var-uuid". */
                                if (streq(*p, "var-uuid")) {
                                        r = sd_id128_get_machine_app_specific(SD_GPT_VAR, &uuid);
                                        if (r < 0)
                                                return log_error_errno(r, "Failed to generate machine-specific UUID: %m");
