...
static int method_set_vc_keyboard(sd_bus_message *m, void *userdata, sd_bus_error *error) {
        ...
        if (convert) {
                r = x11_write_data(c);
                if (r < 0) {
                        log_error_errno(r, "Failed to write X11 keyboard layout: %m");
                        return sd_bus_error_set_errnof(error, r, "Failed to write X11 keyboard layout: %m");
                }
        }
...
static int method_set_x11_keyboard(sd_bus_message *m, void *userdata, sd_bus_error *error) {
        ...
        r = vconsole_write_data(c);
        if (r < 0) {
                log_error_errno(r, "Failed to update vconsole.conf: %m");
                return sd_bus_error_set_errnof(error, r, "Failed to update vconsole.conf: %m");
        }
