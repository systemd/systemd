...
void x11_context_empty_to_null(X11Context *xc) {
        ...
        xc->layout  = (char*) empty_to_null(xc->layout);
        xc->model   = (char*) empty_to_null(xc->model);
        xc->variant = (char*) empty_to_null(xc->variant);
        xc->options = (char*) empty_to_null(xc->options);
}
        ...
        if (isempty(c->vc_keymap)) {
                ...
                for (unsigned n = 0;;) {
                        ...
                        r = x11_context_copy(xc,
                                             &(X11Context) {
                                                     .layout  = (char*) empty_or_dash_to_null(a[1]),
                                                     .model   = (char*) empty_or_dash_to_null(a[2]),
                                                     .variant = (char*) empty_or_dash_to_null(a[3]),
                                                     .options = (char*) empty_or_dash_to_null(a[4]),
                                             });
                        if (r < 0)
                                return r;
                        modified = r > 0;
