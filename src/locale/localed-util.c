...
int vconsole_read_data(Context *c, sd_bus_message *m) {
        ...
        if (vc_context_verify(&vc) < 0 || x11_context_verify(&xc) < 0) {
                /* Do not fail when the .conf file contains an invalid setting, but discards stored settings
                 * and save the stat to avoid future reading. */
                c->vc_stat = st;
                vc_context_clear(&c->vc);
                x11_context_clear(&c->x11_from_vc);
                return 0;
