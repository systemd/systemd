...
static int list_x11_keymaps(int argc, char **argv, void *userdata) {
        ...
        f = fopen(xkb_base, "re");
        if (!f)
                return log_error_errno(errno, "Failed to open keyboard mapping list %s: %m", xkb_base);
        ...
        for (;;) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to read keyboard mapping list %s: %m", xkb_base);
                if (r == 0)
                        break;
        ...
        if (strv_isempty(list))
                return log_error_errno(SYNTHETIC_ERRNO(ENOENT),
                                       "Couldn't find any entries in keyboard mapping list %s.", xkb_base);
