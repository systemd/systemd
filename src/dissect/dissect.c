...
static int action_list_or_mtree_or_copy(DissectedImage *m, LoopDevice *d) {
        ...
        if (arg_action == ACTION_COPY_FROM) {
                ...
                assert(IN_SET(arg_action, ACTION_LIST, ACTION_MTREE));
                ...
                r = recurse_dir(dfd,
                                arg_action == ACTION_MTREE ? "." : NULL,
                                arg_action == ACTION_MTREE ? STATX_TYPE|STATX_MODE|STATX_UID|STATX_GID|STATX_SIZE : 0,
                                UINT_MAX,
                                RECURSE_DIR_SORT | (arg_action == ACTION_MTREE ? RECURSE_DIR_INODE_FD|RECURSE_DIR_TOPLEVEL : 0),
                                arg_action == ACTION_MTREE ? mtree_print_item : list_print_item,
                                NULL);
                if (r < 0)
