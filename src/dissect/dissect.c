...
static int action_list_or_copy(DissectedImage *m, LoopDevice *d) {
        ...
        if (arg_action == ACTION_COPY_FROM) {
                ...
                size_t offset = strlen(mounted_dir);
                ...
                r = recurse_dir_at(AT_FDCWD, mounted_dir, 0, UINT_MAX, RECURSE_DIR_SORT|RECURSE_DIR_ENSURE_TYPE|RECURSE_DIR_SAME_MOUNT, list_print_item, &list_dir);
                if (r < 0)
                        return log_error_errno(r, "Failed to list temporary mount directory: %m");
        }
