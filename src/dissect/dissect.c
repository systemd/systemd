...
static int get_sysext_scopes(DissectedImage *m, char ***ret_scopes) {
        ...
        /* If there's no extension-release file its not a system extension. Otherwise the SYSEXT_SCOPES field
...
static int action_dissect(DissectedImage *m, LoopDevice *d) {
        ...
        if (r == -EMEDIUMTYPE)
                ...
                printf("            %s portable service\n",
                       COLOR_MARK_BOOL(strv_env_pairs_get(m->os_release, "PORTABLE_MATCHES")));

                r = get_sysext_scopes(m, &sysext_scopes);
                if (r < 0)
                        return log_error_errno(r, "Failed to parse SYSEXT_SCOPES: %m");
