...
static int action_with(DissectedImage *m, LoopDevice *d) {
        ...
        if (chdir(mounted_dir) < 0)
                return log_error_errno(errno, "Failed to change to '%s' directory: %m", mounted_dir);
        ...
        /* Move out of the image again */
        if (chdir("/") < 0)
                log_warning_errno(errno, "Failed to change working director to root file system, ignoring: %m");
