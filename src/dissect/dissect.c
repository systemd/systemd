...
static int parse_argv(int argc, char *argv[]) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION_LONG("discard", "MODE", "Choose discard mode (disabled, loop, all, crypto)"): {
                        ...
                        if (streq(state.argument, "disabled"))
                                flags = 0;
                        else if (streq(state.argument, "loop"))
                                flags = DISSECT_IMAGE_DISCARD_ON_LOOP;
                        else if (streq(state.argument, "all"))
                                flags = DISSECT_IMAGE_DISCARD_ON_LOOP | DISSECT_IMAGE_DISCARD;
                        else if (streq(state.argument, "crypt"))
                                flags = DISSECT_IMAGE_DISCARD_ANY;
                        else if (streq(state.argument, "list")) {
                                puts("disabled\n"
                                     "all\n"
                                     "crypt\n"
                                     "loop");
                                ...
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Unknown --discard= parameter: %s", state.argument);
                ...
                OPTION_LONG("usr-hash", "HASH", "Same, but for the usr partition"): {
                        ...
                        PartitionDesignator d = streq(state.current->long_code, "root-hash") ? PARTITION_ROOT : PARTITION_USR;
                        ...
                        r = unhexmem(state.argument, &roothash.iov_base, &roothash.iov_len);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse root hash '%s': %m", state.argument);
                        if (roothash.iov_len < sizeof(sd_id128_t))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Root hash must be at least 128-bit long: %s", state.argument);
                ...
                OPTION_LONG("usr-hash-sig", "SIG", "Same, but for the usr partition"): {
                        ...
                        PartitionDesignator d = streq(state.current->long_code, "root-hash-sig") ? PARTITION_ROOT : PARTITION_USR;
                        if (arg_verity_settings.designator >= 0 &&
                            arg_verity_settings.designator != d)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "Cannot combine --root-hash=/--root-hash-sig= and --usr-hash=/--usr-hash-sig= options.");
                        ...
                        if ((value = startswith(state.argument, "base64:"))) {
                                ...
                                r = read_full_file(state.argument, (char**) &sig.iov_base, &sig.iov_len);
                                if (r < 0)
                                        return log_error_errno(r, "Failed to read root hash signature file '%s': %m", state.argument);
                        }
                ...
                OPTION_LONG("fsck", "BOOL", "Run fsck before mounting"):
                        ...
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --fsck= parameter: %s", state.argument);
                ...
                OPTION_LONG("growfs", "BOOL", "Grow file system to partition size, if marked"):
                        r = parse_boolean(state.argument);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --growfs= parameter: %s", state.argument);
                ...
                OPTION_LONG("loop-ref", "NAME", "Set reference string for loopback device"):
                        ...
                        if (strlen(state.argument) >= sizeof_field(struct loop_info64, lo_file_name))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Loop device ref string '%s' is too long.", state.argument);

                        r = free_and_strdup_warn(&arg_loop_ref, state.argument);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("mtree-hash", "BOOL", "Whether to include SHA256 hash in the mtree output"):
                        r = parse_boolean_argument("--mtree-hash=", state.argument, &arg_mtree_hash);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("image-filter", "FILTER", "Specify image dissection filter"): {
                        ...
                        r = image_filter_parse(state.argument, &f);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse image filter expression: %s", state.argument);
                ...
                OPTION_LONG("copy-ownership", "BOOL", "Whether to copy ownership when copying files"):
                        r = parse_tristate_argument_with_auto("--copy-ownership=", state.argument, &arg_copy_ownership);
                        if (r < 0)
                                return r;
                        break;
