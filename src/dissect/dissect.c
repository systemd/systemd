...
static int action_umount(const char *path) {
        ...
        r = loop_device_open(devname, 0, /*relinquish=*/ false, &d);
        if (r < 0)
        ...
        if (arg_rmdir && rmdir(canonical) < 0)
                return log_error_errno(errno, "Failed to remove mount directory '%s': %m", canonical);

        /* Before the loop_device_unrefp() kicks in, let's explicitly remove all the partition subdevices of
         * the loop device. We do this to ensure that all traces of the loop device are gone by the time this
         * command exits. */
        r = block_device_remove_all_partitions(d->fd);
        if (r < 0)
                return log_error_errno(r, "Failed to remove loop device '%s' partitions: %m", devname);
