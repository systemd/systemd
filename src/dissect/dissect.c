...
static int action_umount(const char *path) {
        ...
        /* Don't immediately relinquish the loop device so we don't try to remove the loop device if we fail
         * to lock it or fail to unmount the mounted partitions. */
        r = loop_device_open(devname, 0, /*relinquish=*/ true, &d);
        if (r < 0)
        ...
        (void) loop_device_unrelinquish(d);
