...
static int manifest_print_item(
        ...
        /* We ignore this to avoid input-as-output cycles */
        ...
        if (FLAGS_SET(sx->stx_mask, STATX_NLINK|STATX_TYPE) && !S_ISDIR(sx->stx_mode) && sx->stx_nlink > 1 && inode_fd >= 0) {
                ...
                if (hd->db_fd < 0) {
                        ...
                        hd->db_fd = mkdtemp_open(template, O_NONBLOCK, &hd->db_path);
                        if (hd->db_fd < 0)
