...
static int run(int argc, char *argv[]) {
        ...
        if (arg_image) {
                ...
                if (!arg_via_service) {
                        ...
                        if (r < 0) {
                                if ((r != -ENOENT && !ERRNO_IS_PRIVILEGE(r)) || !IN_SET(arg_action, ACTION_MOUNT, ACTION_UMOUNT, ACTION_WITH, ACTION_DISSECT, ACTION_LIST, ACTION_MTREE, ACTION_COPY_FROM, ACTION_COPY_TO, ACTION_SHIFT))
