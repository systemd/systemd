...
typedef struct ManifestData {
        ...
        /* Hardlink database fields */
        int db_fd;
        char *db_path;
        uint64_t counter;
} ManifestData;

static void manifest_data_done(ManifestData *md) {
        assert(md);

        if (md->db_fd >= 0)
                (void) rm_rf_children(TAKE_FD(md->db_fd), REMOVE_PHYSICAL|REMOVE_REWIND, /* root_dev= */ NULL);

        md->db_path = rmdir_and_free(md->db_path);
}
...
static int hardlink_data_lookup(
        ...
        if (md->db_fd < 0) {
                ...
                md->db_fd = mkdtemp_open(template, /* flags= */ 0, &md->db_path);
                if (md->db_fd < 0)
                        return log_error_errno(md->db_fd, "Failed to create temporary directory: %m");
        ...
        if (r < 0) {
                ...
                if (symlinkat(slt, md->db_fd, fhs) < 0)
...
static int manifest_print_item(
        ...
        ManifestData *md = ASSERT_PTR(userdata);
        int r;
        ...
        r = hardlink_data_lookup(md, path, inode_fd, sx, &hardlink_counter, hash, &have_hash);
        if (r < 0)
                return r;
        ...
        r = sd_json_variant_new_string(&jp, path);
        if (r == -EUCLEAN) {
                _cleanup_free_ char *e = cescape(path);
                if (!e)
                        return log_oom();

                /* This is not a string we can express in an UTF-8 string, and hence in JSON. Let's continue,
                 * but count it and report the failure in the end */
                log_warning_errno(r, "Encountered path '%s' which cannot be represented in a JSON string, skipping.", e);
                md->n_skipped++;
                return RECURSE_DIR_CONTINUE;
        }
        if (r < 0)
                return log_error_errno(r, "Failed to build JSON object for path: %m");

        _cleanup_(sd_json_variant_unrefp) sd_json_variant *j = NULL;
        r = sd_json_buildo(
                        &j,
                        SD_JSON_BUILD_PAIR_CONDITION(is_root, "mediaType", JSON_BUILD_CONST_STRING("application/vnd.uapi.16.manifest")),
                        SD_JSON_BUILD_PAIR_CONDITION(!is_root, "name", SD_JSON_BUILD_VARIANT(jp)),
...
static int action_list_or_mtree_or_manifest_or_copy_or_make_archive(DissectedImage *m, LoopDevice *d, int userns_fd) {
        ...
        case ACTION_MANIFEST: {
                ...
                if (arg_action == ACTION_LIST)
                        ...
                        _cleanup_(manifest_data_done) ManifestData md = {
                                .db_fd = -EBADF,
                        };
                        r = recurse_dir(dfd,
                                        /* path= */ NULL,
                                        ...
                                        manifest_print_item,
                                        &md);
                        if (r >= 0 && md.n_skipped > 0) {
                                log_error("%" PRIu64 " entries skipped, as they were not representable in JSON.", md.n_skipped);
                                return -EUCLEAN;
                        }
                } else
                        assert_not_reached();
