...
int verb_enable(int argc, char *argv[], void *userdata) {
        ...
        if (arg_now && STR_IN_SET(argv[0], "enable", "disable", "mask")) {
                ...
                if (strv_extend(&new_args, streq(argv[0], "enable") ? "start" : "stop") < 0)
                        return log_oom();
                ...
                STRV_FOREACH(name, names) {
                        ...
                        if (r < 0)
                                return log_error_errno("Failed to extract filename of '%s': %m", *name);

                        /* We can safely assume the unit names are valid here (i.e. no need for unit_name_is_valid) */
                        t = strrchr(fn, '@');
                        if (t && IN_SET(*(t + 1), '\0', '.')) {
