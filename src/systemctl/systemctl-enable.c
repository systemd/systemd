...
int verb_enable(int argc, char *argv[], void *userdata) {
        ...
        if (arg_now) {
                ...
                if (STR_IN_SET(verb, "enable", "unmask"))
                        start_verb = "start";
                ...
                bool is_enable = STR_IN_SET(verb, "enable", "reenable");
                ...
                STRV_FOREACH(name, names) {
                        ...
                        if (unit_name_is_valid(unit_name, UNIT_NAME_TEMPLATE)) {
                                ...
                                if (is_enable) {
                                        /* Skip template units when (re)enabling. Globbing doesn't make sense
                                         * since the semantics would be altered (we're operating on DefaultInstance=
                                         * when enabling), and starting template unit is not supported. */
                                        log_warning("Template unit '%s' specified along with (re)enable --now, skipping.");
                                        continue;
                                }
