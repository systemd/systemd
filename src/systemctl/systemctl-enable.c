...
int verb_enable(int argc, char *argv[], void *userdata) {
        ...
        if (arg_now) {
                ...
                STRV_FOREACH(name, names) {
                        ...
                        if (STR_IN_SET(verb, "enable", "reenable")) {
                        ...
                        if (unit_name_is_valid(unit_name, UNIT_NAME_TEMPLATE)) {
                                ...
                                if (STR_IN_SET(verb, "enable", "reenable", "unmask")) {
                                        /* Skip template units when (re)enabling/unmasking. Globbing doesn't
                                         * make sense for the former since the semantics would be altered
                                         * (we're operating on DefaultInstance= when enabling), and starting
                                         * template unit is not supported anyway. */
