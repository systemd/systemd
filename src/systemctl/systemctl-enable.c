...
int verb_enable(int argc, char *argv[], void *userdata) {
        ...
        if (arg_now) {
                ...
                STRV_FOREACH(name, names) {
                        ...
                        /* Always pass unit names as-is if enabling, as starting globbed unit seldomly
                         * makes sense and actually changes the semantic (we're operating on
                         * DefaultInstance= when enabling. */
                        if (!streq(verb, "enable") && unit_name_is_valid(fn, UNIT_NAME_TEMPLATE)) {
                                char *globbed;

                                r = unit_name_replace_instance_full(fn, "*", /* accept_glob = */ true, &globbed);
                                if (r < 0)
                                        return log_error_errno(r, "Failed to glob unit name '%s': %m", fn);
