...
static int add_memory_limit(Table *table, const char *label, uint64_t val, uint64_t effective) {
        int r;
...
static int add_memory_protection(Table *table, const char *label, uint64_t val) {
        int r;
...
static int show_limits_for_unit(sd_bus *bus, const char *name) {
        ...
        /* Memory protections */
        r = add_memory_protection(table, "Memory Min", info.memory_min);
        if (r < 0)
                return r;

        r = add_memory_protection(table, "Memory Low", info.memory_low);
        if (r < 0)
                return r;

        r = add_memory_protection(table, "Startup Memory Low", info.startup_memory_low);
        if (r < 0)
                return r;

        /* Memory limits */
        r = add_memory_limit(table, "Memory High", info.memory_high, info.effective_memory_high);
        if (r < 0)
                return r;

        r = add_memory_limit(table, "Startup Memory High", info.startup_memory_high, CGROUP_LIMIT_MAX);
        if (r < 0)
                return r;

        r = add_memory_limit(table, "Memory Max", info.memory_max, info.effective_memory_max);
        if (r < 0)
                return r;

        r = add_memory_limit(table, "Startup Memory Max", info.startup_memory_max, CGROUP_LIMIT_MAX);
        if (r < 0)
                return r;

        r = add_memory_limit(table, "Memory Swap Max", info.memory_swap_max, CGROUP_LIMIT_MAX);
        if (r < 0)
                return r;

        r = add_memory_limit(table, "Startup Memory Swap Max", info.startup_memory_swap_max, CGROUP_LIMIT_MAX);
        if (r < 0)
                return r;

        r = add_memory_limit(table, "Memory ZSwap Max", info.memory_zswap_max, CGROUP_LIMIT_MAX);
        if (r < 0)
                return r;

        r = add_memory_limit(table, "Startup Memory ZSwap Max", info.startup_memory_zswap_max, CGROUP_LIMIT_MAX);
        if (r < 0)
                return r;
        ...
        if (info.tasks_max != CGROUP_LIMIT_MAX) {
                ...
                if (info.effective_tasks_max != CGROUP_LIMIT_MAX && info.effective_tasks_max != info.tasks_max)
                        ...
                        r = table_add_cell_stringf(table, NULL, "%" PRIu64, info.tasks_max);
                if (r < 0)
                        return table_log_add_error(r);
