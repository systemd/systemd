...
static int add_io_device_limits(
        ...
        while ((r = sd_bus_message_read(reply, "(st)", &dev, &value)) > 0) {
                r = table_add_cell(table, /* ret_cell= */ NULL, TABLE_FIELD, label);
                if (r < 0)
                ...
                case IO_DEVICE_FORMAT_IOPS:
                        r = table_add_cell_stringf(table, /* ret_cell= */  NULL, "%s %" PRIu64, dev, value);
                        break;

                case IO_DEVICE_FORMAT_BYTES:
                        r = table_add_cell_stringf(table, /* ret_cell= */  NULL, "%s %s", dev, FORMAT_BYTES(value));
                        break;

                case IO_DEVICE_FORMAT_USEC:
                        r = table_add_cell_stringf(table, /* ret_cell= */  NULL, "%s %s", dev, FORMAT_TIMESPAN(value, USEC_PER_MSEC));
                        break;
                ...
                r = table_add_cell(table, /* ret_cell= */  NULL, TABLE_EMPTY, NULL);
                if (r < 0)
                        return table_log_add_error(r);
        }
        if (r < 0)
...
static int add_memory_limit(Table *table, const char *label, uint64_t val, uint64_t effective, uint64_t current) {
        ...
        if (val != CGROUP_LIMIT_MAX) {
                ...
                if (show_effective)
                        r = table_add_cell_stringf_full(table, /* ret_cell= */  NULL, TABLE_STRING_WITH_ANSI,
                                                        "%s %s(effective: %s)%s",
                                                        FORMAT_BYTES(val),
                                                        ansi_grey(), FORMAT_BYTES(effective), ansi_normal());
                else
                        r = table_add_cell(table, /* ret_cell= */  NULL, TABLE_STRING, FORMAT_BYTES(val));
        ...
        if (current != UINT64_MAX)
                r = table_add_cell(table, /* ret_cell= */  NULL, TABLE_SIZE, &current);
        else
                r = table_add_cell(table, /* ret_cell= */  NULL, TABLE_EMPTY, NULL);
        if (r < 0)
                return table_log_add_error(r);
...
static int show_limits_for_unit(sd_bus *bus, const char *name) {
        ...
        if (info.cpu_quota_per_sec_usec != USEC_INFINITY) {
                r = table_add_cell(table, /* ret_cell= */  NULL, TABLE_FIELD, "CPU Quota");
                if (r < 0)
                        return table_log_add_error(r);

                r = table_add_cell_stringf(table, /* ret_cell= */  NULL, "%g%%",
        ...
        if (info.tasks_max != CGROUP_LIMIT_MAX ||
            ...
            arg_all) {
                ...
                r = table_add_cell(table, /* ret_cell= */  NULL, TABLE_FIELD, "Tasks Max");
                if (r < 0)
                        return table_log_add_error(r);
                ...
                if (info.tasks_max != CGROUP_LIMIT_MAX && show_effective)
                        ...
                        r = table_add_cell(table, /* ret_cell= */  NULL, TABLE_STRING, "infinity");
                if (r < 0)
                        return table_log_add_error(r);
