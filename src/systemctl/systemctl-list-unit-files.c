...
static int compare_unit_file_list(const UnitFileList *a, const UnitFileList *b) {
        ...
        /* sorting callbacks are not allowed to fail, so if we can't extract basename, use entire path. */
        (void) path_extract_filename(a->path, &a_id);
        (void) path_extract_filename(b->path, &b_id);

        return strcasecmp(a_id ? a_id : a->path, b_id ? b_id : b->path);
}
