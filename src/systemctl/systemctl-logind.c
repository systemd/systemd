...
int logind_reboot(enum action a) {
        ...
        if (getuid() == 0 && sd_bus_error_has_name(&error, SD_BUS_ERROR_INTERACTIVE_AUTHORIZATION_REQUIRED))
                return log_error_errno(r,
                                       "The current polkit policy does not allow root to ignore inhibitors in order to %s.\n"
                                       "To allow this action, a new polkit rule is needed.\n"
                                       "See /usr/share/polkit-1/rules.d/10-systemd-logind-root-ignore-inhibitors.rules.example.",
                                       action_table[a].verb);
