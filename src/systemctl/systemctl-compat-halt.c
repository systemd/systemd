...
int halt_parse_argv(int argc, char *argv[]) {
        ...
        /* called in sysvinit system as last command in
         * shutdown/reboot so this is always forceful */
...
int halt_main(void) {
        ...
        if (!arg_force) {
                ...
                if (r >= 0)
                        return r;
                if (IN_SET(r, -EACCES, -EOPNOTSUPP, -EINPROGRESS) && !arg_force)
