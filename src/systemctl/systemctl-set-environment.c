...
static int json_transform_message(sd_bus_message *m, JsonVariant **ret) {
        ...
        while ((r = sd_bus_message_read_basic(m, SD_BUS_TYPE_STRING, &text)) > 0) {
                ...
                sep = strchr(text, '=');
                if (!sep) {
                        r = log_error_errno(SYNTHETIC_ERRNO(EUCLEAN),
                                            "Invalid environment block");
                        goto finish;
                }

                key = strndupa_safe(text, (size_t)(sep - text));
                if (!key) {
                        r = log_oom();
                        goto finish;
                }
                ...
                r = json_variant_new_string(elements + n_elements, key);
                if (r < 0)
                        goto finish;

                n_elements++;

                r = json_variant_new_string(elements + n_elements, value);
                if (r < 0)
                        goto finish;
