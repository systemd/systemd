...
const char *arg_kexec_cmdline = NULL;
...
STATIC_DESTRUCTOR_REGISTER(arg_kexec_cmdline, unsetp);
...
static int systemctl_help(void) {
        ...
        printf("%1$s [OPTIONS...] COMMAND ...\n\n"
               ...
               "     --kexec-cmdline=CMDLINE\n"
               "                         Append to the kernel command line when loading the\n"
               "                             kexec kernel from the booted boot loader entry\n"
...
static int systemctl_parse_argv(int argc, char *argv[]) {
        ...
        static const struct option options[] = {
                ...
                { "kexec-cmdline",       required_argument, NULL, ARG_KEXEC_CMDLINE       },
        ...
        while ((c = getopt_long(argc, argv, "hC:t:p:P:alqvfs:H:M:n:o:iTr.::", options, NULL)) >= 0)
                ...
                case ARG_KEXEC_CMDLINE:
                        if (!utf8_is_valid(optarg))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "--kexec-cmdline= argument is not valid UTF-8: %s", optarg);
                        if (string_has_cc(optarg, NULL))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "--kexec-cmdline= argument contains control characters: %s", optarg);

                        arg_kexec_cmdline = optarg;
                        break;
