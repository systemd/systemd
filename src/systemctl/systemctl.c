...
static int systemctl_parse_argv(int argc, char *argv[]) {
        ...
        static const struct option options[] = {
                ...
                { "needing",             no_argument,       NULL, ARG_NEEDING             },
                {}
        };
        ...
        if (arg_needing) {
                if (!do_reload_or_restart)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "--needing may only be used with 'reload-or-restart'.");
                if (optind + 1 < argc)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "No additional arguments allowed with 'reload-or-restart --needing'.");
                if (arg_wait)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
