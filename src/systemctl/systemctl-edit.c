...
static int find_paths_to_edit(
        ...
        _cleanup_free_ char *dropin = NULL, *suffix = NULL;
        _cleanup_(hashmap_freep) Hashmap *cached_name_map = NULL, *cached_id_map = NULL;
        ...
        if (!arg_dropin || streq(arg_dropin, ""))
                dropin = strdup("override.conf");
        else if (!endswith(arg_dropin, ".conf"))
                dropin = strjoin(arg_dropin, ".conf");
        else
                dropin = strdup(arg_dropin);
        if (!dropin)
                return log_oom();
