...
int verb_cat(int argc, char *argv[], void *userdata) {
        ...
        STRV_FOREACH(name, names) {
                ...
                /* Check if the unit is masked (symlinked to /dev/null or empty) */
                if (fragment_path) {
                        r = null_or_empty_path(fragment_path);
                        if (r < 0)
                                return log_error_errno(r, "Failed to check if '%s' is masked: %m", *name);
                        if (r > 0) {
                                printf("%s# Unit %s is masked%s.\n",
                                       ansi_highlight_magenta(),
                                       *name,
        ...
        STRV_FOREACH(name, names) {
                ...
                if (path) {
                        ...
                        if (r < 0)
                                return log_error_errno(r, "Failed to check if '%s' is masked: %m", *name);
                        if (r > 0)
                                return log_error_errno(SYNTHETIC_ERRNO(ERFKILL), "Unit '%s' is masked, cannot edit.", *name);
                }
