...
static int deduplicate_kernel_cmdline(char **cmdline) {
        ...
        /* Split, reverse, sort, reverse again: last options win, and result is deduplicated */
...
static int load_kexec_kernel(void) {
        ...
        if (kexec_loaded()) {
                if (arg_kernel_cmdline)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "--kernel-cmdline=/--reuse-kernel-cmdline specified but kexec kernel already loaded");
        ...
        if (!isempty(arg_kernel_cmdline)) {
                if (!strextend_with_separator(&options, " ", arg_kernel_cmdline))
                        return log_oom();
