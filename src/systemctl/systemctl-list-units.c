...
static int collect_automount_info(sd_bus* bus, const UnitInfo* info, struct automount_info *ret_info) {
        ...
        _cleanup_(sd_bus_message_unrefp) sd_bus_message* reply = NULL;
        _cleanup_strv_free_ char** triggered = NULL;
        _cleanup_free_ UnitInfo* triggered_infos = NULL;
        _cleanup_free_ char* where = NULL, *what = NULL, *state = NULL;
        usec_t timeout_idle_usec;
        BusLocator locator;
        int r, n;
        ...
        r = get_triggered_units(bus, info->unit_path, &triggered);
        if (r < 0)
                return r;

        n = get_unit_list(bus, NULL, triggered, &triggered_infos, 0, &reply);
        if (n < 0)
                return n;

        for (const UnitInfo* u = triggered_infos; u < triggered_infos + n; u++) {
                if (!endswith(u->id, ".mount"))
                        continue;

                locator.path = u->unit_path;
...
static int output_automounts_list(struct automount_info *infos, unsigned n_infos) {
        _cleanup_(table_unrefp) Table *table = NULL;
        int r;
        ...
        if (arg_legend != 0) {
                ...
                on = n_infos > 0 ? ansi_highlight() : ansi_highlight_red();
                off = ansi_normal();

                printf("\n%s%u automounts listed.%s\n", on, n_infos, off);
                if (!arg_all)
                        printf("Pass --all to see loaded but inactive automounts, too.\n");
        }
...
int verb_list_automounts(int argc, char *argv[], void *userdata) {
        ...
        if (argc == 1 || automounts) {
                ...
                for (const UnitInfo *u = unit_infos; u < unit_infos + n; u++) {
                        ...
                        if (!GREEDY_REALLOC(automount_infos, cs + 1)) {
                                r = log_oom();
                                goto cleanup;
                        }

                        r = collect_automount_info(bus, u, &automount_infos[cs]);
                        if (r < 0)
                                goto cleanup;
                ...
                typesafe_qsort(automount_infos, cs, automount_info_compare);
        }

        output_automounts_list(automount_infos, cs);

 cleanup:
        assert(cs == 0 || automount_infos);
        for (struct automount_info *info = automount_infos; info < automount_infos + cs; info++) {
                free(info->what);
                free(info->where);
        }
