...
static void path_infos_free(struct path_infos *ps) {
        if (!ps)
                return;
...
static int get_paths(sd_bus *bus, const char *unit_path, char ***ret_conditions, char ***ret_paths) {
        ...
        while ((r = sd_bus_message_read(reply, "(ss)", &condition, &path)) > 0) {
                r = strv_extend(&conditions, condition);
                if (r < 0)
                        return log_oom();
...
static int output_paths_list(struct path_infos *ps) {
        ...
        for (struct path_info *p = ps->items; p < ps->items + ps->count; p++) {
                ...
                if (strv_isempty(p->triggered))
                        r = table_add_cell(table, NULL, TABLE_EMPTY, NULL);
                else if (strv_length(p->triggered) == 1)
                        r = table_add_cell(table, NULL, TABLE_STRING, p->triggered[0]);
                else
                        /* This should never happen, currently our path units can only trigger a
                         * single unit. But let's handle this anyway, who knows what the future
                         * brings? */
                        r = table_add_cell(table, NULL, TABLE_STRV, p->triggered);
                if (r < 0)
                        return table_log_add_error(r);
        }
...
int verb_list_paths(int argc, char *argv[], void *userdata) {
        ...
        _cleanup_free_ UnitInfo *unit_infos = NULL;
        _cleanup_(path_infos_free) struct path_infos path_infos = {};
        int r, n;
        sd_bus *bus;
        ...
        if (argc == 1 || units) {
                ...
                for (const UnitInfo *u = unit_infos; u < unit_infos + n; u++) {
                        ...
                        for (int i = 0; i < c; i++)
                                path_infos.items[path_infos.count + i] = (struct path_info) {
                                        .machine = u->machine,
                                        .id = u->id,
