                ...
                for (const UnitInfo *u = unit_infos; u < unit_infos + n; u++) {
                        ...
                        for (int i = 0; i < c; i++)
                                path_infos.items[path_infos.count + i] = (struct path_info) {
                                        ...
                                        .path = paths[i],
                                        .triggered = strv_copy(triggered),
                                };

                        /* The content of these strvs is now owned by path_infos. We must delete them
                         * without deleting their content (as _cleanup_strv_free_ would do). */
