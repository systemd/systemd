...
int acquire_bus(BusFocus focus, sd_bus **ret) {
        ...
        if ((arg_runtime_scope == RUNTIME_SCOPE_SYSTEM && geteuid() != 0) || getenv_bool("SYSTEMCTL_FORCE_BUS") > 0)
                /* Only talk to the system private manager bus if we're root and not explicitly
