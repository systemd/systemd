...
typedef struct ExecStatusInfo {
        ...
        LIST_FIELDS(struct ExecStatusInfo, exec_status_infos);
} ExecStatusInfo;
...
typedef struct UnitStatusInfo {
        ...
        LIST_HEAD(ExecStatusInfo, exec_status_infos);
} UnitStatusInfo;
...
static void unit_status_info_free(UnitStatusInfo *info) {
        ...
        while ((p = info->exec_status_infos)) {
                LIST_REMOVE(exec_status_infos, info->exec_status_infos, p);
...
static void print_status_info(
        ...
        LIST_FOREACH(exec_status_infos, p, i->exec_status_infos) {
...
static int map_exec(sd_bus *bus, const char *member, sd_bus_message *m, sd_bus_error *error, void *userdata) {
        ...
        LIST_FIND_TAIL(exec_status_infos, i->exec_status_infos, last);
        ...
        while ((r = exec_status_info_deserialize(m, info, is_ex_prop)) > 0) {
                ...
                LIST_INSERT_AFTER(exec_status_infos, i->exec_status_infos, last, info);
                last = info;
