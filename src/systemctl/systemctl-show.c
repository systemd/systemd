...
static int print_property(const char *name, const char *expected_value, sd_bus_message *m, BusPrintPropertyFlags flags) {
        ...
        case SD_BUS_TYPE_ARRAY:
                ...
                if (contents[0] == SD_BUS_TYPE_STRUCT_BEGIN && streq(name, "EnvironmentFiles")) {
                        ...
                        r = sd_bus_message_enter_container(m, SD_BUS_TYPE_ARRAY, "(ssx)");
                        if (r < 0)
                                return bus_log_parse_error(r);

                        while ((r = sd_bus_message_read(m, "(ssx)", &path, &fdname, &options)) > 0) {
                                bus_print_property_valuef(name, expected_value, flags, "%s:%s:%lld", path, fdname, options);
                        }
                        if (r < 0)
                                return bus_log_parse_error(r);
