...
static int map_quota(sd_bus *bus, const char *member, sd_bus_message *m, sd_bus_error *error, void *userdata) {
        ...
        int quota_enforce, quota_accounting;
        uint64_t quota_usage, quota_limit;
        const char *flags;
        _cleanup_free_ char *enforce_flag = NULL, *accounting_flag = NULL;

        r = sd_bus_message_read(m, "(stt)", &flags, &quota_usage, &quota_limit);
        if (r < 0)
                return r;

        r = extract_many_words(&flags, " ", 0, &enforce_flag, &accounting_flag);
        if (r < 0)
                return r;
        ...
        quota_accounting = parse_boolean(accounting_flag);
        if (quota_accounting < 0)
                return quota_accounting;
...
static int print_property(const char *name, const char *expected_value, sd_bus_message *m, BusPrintPropertyFlags flags) {
        ...
        case SD_BUS_TYPE_STRUCT:
                ...
                if (contents[0] == SD_BUS_TYPE_UINT32 && streq(name, "Job")) {
                        ...
                        if (!parse_boolean(quota_enforce))
                                bus_print_property_value(name, expected_value, flags, "[not set]");
...
static int show_one(
        ...
        static const struct bus_properties_map property_map[] = {
                ...
                { "StateDirectoryQuotaUsage",       "(stt)",          map_quota,      offsetof(UnitStatusInfo, exec_directories_quota[EXEC_DIRECTORY_STATE])   },
                { "CacheDirectoryQuotaUsage",       "(stt)",          map_quota,      offsetof(UnitStatusInfo, exec_directories_quota[EXEC_DIRECTORY_CACHE])   },
                { "LogsDirectoryQuotaUsage",        "(stt)",          map_quota,      offsetof(UnitStatusInfo, exec_directories_quota[EXEC_DIRECTORY_LOGS])    },
