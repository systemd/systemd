...
static int load_seed_file(
        ...
        if (r < 0)
                ...
                r = random_write_entropy(urandom_fd, &mid, sizeof(mid), false);
                if (r < 0)
...
static int save_seed_file(
        ...
        k = RET_NERRNO(getrandom(buf, seed_size, GRND_NONBLOCK));
        if (k == -EAGAIN && synchronous) {
                log_notice("Kernel entropy pool is not initialized yet, waiting until it is.");
                k = RET_NERRNO(getrandom(buf, seed_size, 0)); /* retry synchronously */
        }
        if (k < 0)
