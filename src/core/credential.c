...
static int setup_credentials_internal(
        ...
        if (workspace_mounted) {
                ...
                if (install) {
                        ...
                        if (ret_mount_fd) {
                                r = RET_NERRNO(open_tree(AT_FDCWD, workspace, OPEN_TREE_CLONE | OPEN_TREE_CLOEXEC));
                                if (r >= 0) {
                                        *ret_mount_fd = r;
                                        return 0;
                                }
...
int setup_credentials(
        ...
        if (r < 0) {
                ...
                if (mount_fd >= 0) {
                        r = send_one_fd(socket_pair[1], mount_fd, MSG_DONTWAIT);
                        if (r < 0)
                                goto child_fail;
                }
        ...
        child_fail:
                ...
                mount_fd = receive_one_fd(socket_pair[0], MSG_DONTWAIT);
                if (mount_fd >= 0)
                        *ret_mount_fd = mount_fd;
                else
                        *ret_mount_fd = -EBADF;
        }
