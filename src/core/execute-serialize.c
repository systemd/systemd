...
static int exec_parameters_serialize(const ExecParameters *p, FILE *f, FDSet *fds) {
        ...
        r = serialize_item(f, "exec-parameters-id", p->id);
        if (r < 0)
                return r;
...
static int exec_parameters_deserialize(ExecParameters *p, FILE *f, FDSet *fds) {
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "exec-parameters-runtime-scope="))) {
                        ...
                        r = free_and_strdup(&p->default_smack_process_label, val);
                        if (r < 0)
                                return r;
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-parameters-id="))) {
                        r = free_and_strdup(&p->id, val);
                        if (r < 0)
                                return r;
                        ...
                        strcpy(p->invocation_id_string, val);

                        r = sd_id128_from_string(p->invocation_id_string, &p->invocation_id);
                        if (r < 0)
                                return r;
                }
        }
