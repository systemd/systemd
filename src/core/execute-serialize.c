...
int exec_cgroup_context_serialize(const CGroupContext *c, FILE *f) {
        ...
        const char *prefix = "exec-cgroup-context-";
        ...
        fprintf(f,
                ...
                "%sCPUQuotaPerSecUsec=" USEC_FMT "\n"
                "%sCPUQuotaPeriodUsec=" USEC_FMT "\n"
                "%sAllowedCPUs=%s\n"
                ...
                "%sMemoryMin=%" PRIu64 "\n"
                "%sMemoryLow=%" PRIu64 "\n"
                "%sStartupMemoryLow=%" PRIu64 "\n"
                "%sMemoryHigh=%" PRIu64 "\n"
                "%sStartupMemoryHigh=%" PRIu64 "\n"
                "%sMemoryMax=%" PRIu64 "\n"
                "%sStartupMemoryMax=%" PRIu64 "\n"
                "%sMemorySwapMax=%" PRIu64 "\n"
                "%sStartupMemorySwapMax=%" PRIu64 "\n"
                "%sMemoryZSwapMax=%" PRIu64 "\n"
                "%sStartupMemoryZSwapMax=%" PRIu64 "\n"
                "%sMemoryLimit=%" PRIu64 "\n"
                "%sTasksMaxValue=%" PRIu64 "\n"
                "%sTasksMaxScale=%" PRIu64 "\n"
                "%sdefault-memory-min-set=%s\n"
                "%sdefault-memory-low-set=%s\n"
                "%sdefault-startup-memory-low-set=%s\n"
                "%smemory-min-set=%s\n"
                "%smemory-low-set=%s\n"
                "%sstartup-memory-low-set=%s\n"
                "%sstartup-memory-high-set=%s\n"
                "%sstartup-memory-max-set=%s\n"
                "%sstartup-memory-swap-max-set=%s\n"
                "%sstartup-memory-zswap-max-set=%s\n"
                "%sDevicePolicy=%s\n"
                ...
                "%sManagedOOMMemoryPressureLimit=%" PRIu32 "\n"
                ...
                prefix, yes_no(c->memory_oom_group),
                prefix, c->cpu_weight,
                prefix, c->startup_cpu_weight,
                prefix, c->cpu_shares,
                prefix, c->startup_cpu_shares,
                prefix, c->cpu_quota_per_sec_usec,
                prefix, c->cpu_quota_period_usec,
                ...
                prefix, c->memory_min,
                prefix, c->memory_low,
                prefix, c->startup_memory_low,
                prefix, c->memory_high,
                prefix, c->startup_memory_high,
                prefix, c->memory_max,
                prefix, c->startup_memory_max,
                prefix, c->memory_swap_max,
                prefix, c->startup_memory_swap_max,
                prefix, c->memory_zswap_max,
                prefix, c->startup_memory_zswap_max,
                prefix, c->memory_limit,
                prefix, c->tasks_max.value,
                prefix, c->tasks_max.scale,
                prefix, yes_no(c->default_memory_min_set),
                prefix, yes_no(c->default_memory_low_set),
                prefix, yes_no(c->default_startup_memory_low_set),
                prefix, yes_no(c->memory_min_set),
                prefix, yes_no(c->memory_low_set),
                prefix, yes_no(c->startup_memory_low_set),
                prefix, yes_no(c->startup_memory_high_set),
                prefix, yes_no(c->startup_memory_max_set),
                prefix, yes_no(c->startup_memory_swap_max_set),
                prefix, yes_no(c->startup_memory_zswap_max_set),
        ...
        if (c->memory_pressure_threshold_usec != USEC_INFINITY)
                fprintf(f, "%sMemoryPressureThresholdUsec=" USEC_FMT "\n",
                        prefix, c->memory_pressure_threshold_usec);
        ...
        LIST_FOREACH(device_latencies, l, c->io_device_latencies)
                fprintf(f,
                        "%sIODeviceLatencyTargetUsec=%s " USEC_FMT "\n",
                        prefix,
                        l->path,
                        l->target_usec);
        ...
        LIST_FOREACH(device_bandwidths, b, c->blockio_device_bandwidths) {
                if (b->rbps != CGROUP_LIMIT_MAX)
                        fprintf(f,
                                "%sBlockIOReadBandwidth=%s %" PRIu64 "\n",
                                prefix,
                                b->path,
                                b->rbps);
                if (b->wbps != CGROUP_LIMIT_MAX)
                        fprintf(f,
                                "%sBlockIOWriteBandwidth=%s %" PRIu64 "\n",
                                prefix,
                                b->path,
                                b->wbps);
        }
        ...
        r = serialize_bool(f, "exec-cgroup-context-ip-address-allow-reduced", c->ip_address_allow_reduced);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-cgroup-context-ip-address-deny-reduced", c->ip_address_deny_reduced);
        if (r < 0)
                return r;

        r = serialize_strv(f, "exec-cgroup-context-IPIngressFilterPath=", c->ip_filters_ingress);
        if (r < 0)
                return r;

        r = serialize_strv(f, "exec-cgroup-context-IPEgressFilterPath=", c->ip_filters_egress);
        if (r < 0)
                return r;

        LIST_FOREACH(programs, p, c->bpf_foreign_programs)
                fprintf(f, "%sBPFProgram=%" PRIu32 " %s", prefix, p->attach_type, p->bpffs_path);
...
int exec_cgroup_context_deserialize(CGroupContext *c, FILE *f) {
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "exec-cgroup-context-CPUAccounting="))) {
                        r = parse_boolean(val);
                        if (r < 0)
                                return r;
                        c->cpu_accounting = r;
                } else if ((val = startswith(l, "exec-cgroup-context-IOAccounting="))) {
                        r = parse_boolean(val);
                        if (r < 0)
                                return r;
                        c->io_accounting = r;
                } else if ((val = startswith(l, "exec-cgroup-context-BlockIOAccounting="))) {
                        r = parse_boolean(val);
                        if (r < 0)
                                return r;
                        c->blockio_accounting = r;
                } else if ((val = startswith(l, "exec-cgroup-context-MemoryAccounting="))) {
                        r = parse_boolean(val);
                        if (r < 0)
                                return r;
                        c->memory_accounting = r;
                } else if ((val = startswith(l, "exec-cgroup-context-TasksAccounting="))) {
                        r = parse_boolean(val);
                        if (r < 0)
                                return r;
                        c->tasks_accounting = r;
                } else if ((val = startswith(l, "exec-cgroup-context-IPAccounting="))) {
                        r = parse_boolean(val);
                        if (r < 0)
                                return r;
                        c->ip_accounting = r;
                ...
                } else if ((val = startswith(l, "exec-cgroup-context-CPUWeight="))) {
                        r = safe_atou64(val, &c->cpu_weight);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupCPUWeight="))) {
                        r = safe_atou64(val, &c->startup_cpu_weight);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-CPUShares="))) {
                        r = safe_atou64(val, &c->cpu_shares);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupCPUShares="))) {
                        r = safe_atou64(val, &c->startup_cpu_shares);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-CPUQuotaPerSecUsec="))) {
                        r = deserialize_usec(val, &c->cpu_quota_per_sec_usec);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-CPUQuotaPeriodUsec="))) {
                        r = deserialize_usec(val, &c->cpu_quota_period_usec);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-AllowedCPUs="))) {
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupAllowedCPUs="))) {
                        r = parse_cpu_set_full(
                                        val,
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-AllowedMemoryNodes="))) {
                        r = parse_cpu_set_full(
                                        val,
                                        &c->cpuset_mems,
                                        /* warn= */ false,
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupAllowedMemoryNodes="))) {
                        r = parse_cpu_set_full(
                                        val,
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-IOWeight="))) {
                        r = safe_atou64(val, &c->io_weight);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupIOWeight="))) {
                        r = safe_atou64(val, &c->startup_io_weight);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-BlockIOWeight="))) {
                        r = safe_atou64(val, &c->blockio_weight);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupBlockIOWeight="))) {
                        r = safe_atou64(val, &c->startup_blockio_weight);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-DefaultMemoryMin="))) {
                        r = safe_atou64(val, &c->default_memory_min);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-DefaultMemoryLow="))) {
                        r = safe_atou64(val, &c->default_memory_low);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-MemoryMin="))) {
                        r = safe_atou64(val, &c->memory_min);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-MemoryLow="))) {
                        r = safe_atou64(val, &c->memory_low);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupMemoryLow="))) {
                        r = safe_atou64(val, &c->startup_memory_low);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-MemoryHigh="))) {
                        r = safe_atou64(val, &c->memory_high);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupMemoryHigh="))) {
                        r = safe_atou64(val, &c->startup_memory_high);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-MemoryMax="))) {
                        r = safe_atou64(val, &c->memory_max);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupMemoryMax="))) {
                        r = safe_atou64(val, &c->startup_memory_max);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-MemorySwapMax="))) {
                        r = safe_atou64(val, &c->memory_swap_max);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupMemorySwapMax="))) {
                        r = safe_atou64(val, &c->startup_memory_swap_max);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-MemoryZSwapMax="))) {
                        r = safe_atou64(val, &c->memory_zswap_max);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-StartupMemoryZSwapMax="))) {
                        r = safe_atou64(val, &c->startup_memory_zswap_max);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-MemoryLimit="))) {
                        r = safe_atou64(val, &c->memory_limit);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-TasksMaxValue="))) {
                        r = safe_atou64(val, &c->tasks_max.value);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-TasksMaxScale="))) {
                ...
                } else if ((val = startswith(l, "exec-cgroup-context-DevicePolicy="))) {
                        c->device_policy = cgroup_device_policy_from_string(val);
                        if (c->device_policy < 0)
                                return -EINVAL;
                } else if ((val = startswith(l, "exec-cgroup-context-DisableControllers="))) {
                        r = cg_mask_from_string(val, &c->disable_controllers);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-DelegateControllers="))) {
                        ...
                        if (r < 0)
                                return r;
                        c->delegate = r;
                } else if ((val = startswith(l, "exec-cgroup-context-ManagedOOMSwap="))) {
                        c->moom_swap = managed_oom_mode_from_string(val);
                        if (c->moom_swap < 0)
                                return -EINVAL;
                } else if ((val = startswith(l, "exec-cgroup-context-ManagedOOMMemoryPressure="))) {
                        c->moom_mem_pressure = managed_oom_mode_from_string(val);
                        if (c->moom_mem_pressure < 0)
                                return -EINVAL;
                } else if ((val = startswith(l, "exec-cgroup-context-ManagedOOMMemoryPressureLimit="))) {
                        ...
                        if (c->moom_preference < 0)
                                return -EINVAL;
                } else if ((val = startswith(l, "exec-cgroup-context-MemoryPressureWatch="))) {
                        c->memory_pressure_watch = cgroup_pressure_watch_from_string(val);
                        if (c->memory_pressure_watch < 0)
                                return -EINVAL;
                } else if ((val = startswith(l, "exec-cgroup-context-DelegateSubgroup="))) {
                        c->delegate_subgroup = strdup(val);
                        if (!c->delegate_subgroup)
                                return -ENOMEM;
                } else if ((val = startswith(l, "exec-cgroup-context-MemoryPressureThresholdUsec="))) {
                        r = deserialize_usec(val, &c->memory_pressure_threshold_usec);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-DeviceAllow="))) {
                ...
                } else if ((val = startswith(l, "exec-cgroup-context-IODeviceWeight="))) {
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-IODeviceLatencyTargetUsec="))) {
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-BlockIODeviceWeight="))) {
                        _cleanup_free_ char *path = NULL, *weight = NULL;
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-BlockIOReadBandwidth="))) {
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-BlockIOWriteBandwidth="))) {
                        _cleanup_free_ char *path = NULL, *bw = NULL;
                        CGroupBlockIODeviceBandwidth *a = NULL;
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-IPAddressAllow="))) {
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-IPAddressDeny="))) {
                        struct in_addr_prefix a;
                ...
                } else if ((val = startswith(l, "exec-cgroup-context-IPIngressFilterPath="))) {
                        r = deserialize_strv(&c->ip_filters_ingress, val);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-IPEgressFilterPath="))) {
                        r = deserialize_strv(&c->ip_filters_egress, val);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-BPFProgram="))) {
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-SocketBindAllow="))) {
                ...
                } else if ((val = startswith(l, "exec-cgroup-context-SocketBindDeny="))) {
                        CGroupSocketBindItem *item;
                        uint16_t nr_ports, port_min;
                        int af, ip_protocol;
                ...
                } else if ((val = startswith(l, "exec-cgroup-context-RestrictNetworkInterfaces="))) {
...
int exec_runtime_serialize(const ExecRuntime *rt, FILE *f, FDSet *fds) {
        int r;
...
int exec_runtime_deserialize(ExecRuntime *rt, FILE *f, FDSet *fds) {
        int r;
                        ...
                        for (size_t i = 0; i < 2; ++i) {
                                ...
                                if ((fd = parse_fd(w)) < 0 || !fdset_contains(fds, fd))
                                        ...
                                        if (fdset_remove(fds, fd) < 0) {
                                                log_debug("Failed to remove %s value=%d from fdset, ignoring", l, fd);
                                                continue;
                                        }
...
int exec_parameters_serialize(const ExecParameters *p, FILE *f, FDSet *fds) {
        ...
        r = serialize_bool(f, "exec-parameters-selinux-context-net", p->selinux_context_net);
        if (r < 0)
                return r;
        ...
        r = serialize_bool(f, "exec-parameters-shall-confirm-spawn", p->shall_confirm_spawn);
        if (r < 0)
                return r;

        r = serialize_item_format(f, "exec-parameters-watchdog-usec", USEC_FMT, p->watchdog_usec);
        if (r < 0)
                return r;
...
int exec_parameters_deserialize(ExecParameters *p, FILE *f, FDSet *fds) {
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "exec-parameters-environment="))) {
                        ...
                        r = safe_atolu(val, &p->n_socket_fds);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-parameters-n-storage-fds="))) {
                        r = safe_atolu(val, &p->n_storage_fds);
                        if (r < 0)
                                return r;
                        ...
                        p->received_credentials_directory = strdup(val);
                        if (!p->received_credentials_directory)
                                return log_oom();
                } else if ((val = startswith(l, "exec-parameters-received-encrypted-credentials-directory="))) {
                        p->received_encrypted_credentials_directory = strdup(val);
                        if (!p->received_encrypted_credentials_directory)
                                return log_oom();
                } else if ((val = startswith(l, "exec-parameters-confirm-spawn="))) {
                        p->confirm_spawn = strdup(val);
                        if (!p->confirm_spawn)
                                return log_oom();
                        ...
                        p->default_smack_process_label = strdup(val);
                        if (!p->default_smack_process_label)
                                return log_oom();
...
int exec_context_serialize(const ExecContext *c, FILE *f, FDSet *fds) {
        ...
        r = serialize_bool(f, "exec-context-non-blocking", c->non_blocking);
        if (r < 0)
                return r;
        ...
        r = serialize_bool(f, "exec-context-private-tmp", c->private_tmp);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-private-devices", c->private_devices);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-protect-kernel-tunables", c->protect_kernel_tunables);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-protect-kernel-modules", c->protect_kernel_modules);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-protect-kernel-logs", c->protect_kernel_logs);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-protect-clock", c->protect_clock);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-protect-control-groups", c->protect_control_groups);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-private-network", c->private_network);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-private-users", c->private_users);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-private-ipc", c->private_ipc);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-remove-ipc", c->remove_ipc);
        if (r < 0)
                return r;
        ...
        r = serialize_bool(f, "exec-context-mount-api-vfs", exec_context_get_effective_mount_apivfs(c));
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-mount-api-vfs-set", c->mount_apivfs_set);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-same-pgrp", c->same_pgrp);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-cpu-sched-reset-on-fork", c->cpu_sched_reset_on_fork);
        if (r < 0)
                return r;
        ...
        r = serialize_bool(f, "exec-context-memory-deny-write-execute", c->memory_deny_write_execute);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-restrict-realtime", c->restrict_realtime);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-restrict-suid-sgid", c->restrict_suid_sgid);
        if (r < 0)
                return r;
        ...
        r = serialize_bool(f, "exec-context-protect-hostname", c->protect_hostname);
        if (r < 0)
                return r;
        ...
        for (ExecDirectoryType dt = 0; dt < _EXEC_DIRECTORY_TYPE_MAX; dt++) {
                ...
                for (size_t i = 0; i < c->directories[dt].n_items; i++) {
                        if (!strextend(&value, " ", c->directories[dt].items[i].path))
                                return log_oom_debug();
        ...
        r = serialize_bool(f, "exec-context-working-directory-missing-ok", c->working_directory_missing_ok);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-working-directory-home", c->working_directory_home);
        if (r < 0)
                return r;
        ...
        for (unsigned i = 0; i < RLIM_NLIMITS; i++)
                if (c->rlimit[i]) {
                        ...
                        r = serialize_item_format(f, key, RLIM_FMT, c->rlimit[i]->rlim_max);
                        if (r < 0)
                                return r;

                        key = mfree(key);
                        key = strjoin("exec-context-limit-soft-", rlimit_to_string(i));
                        if (!key)
                                return log_oom_debug();

                        r = serialize_item_format(f, key, RLIM_FMT, c->rlimit[i]->rlim_cur);
                        if (r < 0)
                                return r;
                }
        ...
        if (c->cpu_sched_set) {
                ...
                r = serialize_bool(f, "exec-context-cpu-scheduling-reset-on-fork", c->cpu_sched_reset_on_fork);
                if (r < 0)
                        return r;
        }
        ...
        r = serialize_bool(f, "exec-context-cpu-affinity-from-numa", c->cpu_affinity_from_numa);
        if (r < 0)
                return r;
        ...
        r = serialize_bool(f, "exec-context-stdio-as-fds", c->stdio_as_fds);
        if (r < 0)
                return r;
        ...
        if (c->tty_path) {
                ...
                r = serialize_bool(f, "exec-context-tty-reset", c->tty_reset);
                if (r < 0)
                        return r;

                r = serialize_bool(f, "exec-context-tty-vhangup", c->tty_vhangup);
                if (r < 0)
                        return r;

                r = serialize_bool(f, "exec-context-tty-vt-disallocate", c->tty_vt_disallocate);
                if (r < 0)
                        return r;
        ...
        r = serialize_bool(f, "exec-context-dynamic-user", c->dynamic_user);
        if (r < 0)
                return r;
        ...
        if (!strv_isempty(c->exec_search_path)) {
        ...
        r = serialize_bool(f, "exec-context-no-new-privileges", c->no_new_privileges);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-selinux-context-ignore", c->selinux_context_ignore);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-apparmor-profile-ignore", c->apparmor_profile_ignore);
        if (r < 0)
                return r;

        r = serialize_bool(f, "exec-context-smack-process-label-ignore", c->smack_process_label_ignore);
        if (r < 0)
                return r;
        ...
        r = serialize_bool(f, "exec-context-lock-personality", c->lock_personality);
        if (r < 0)
                return r;
        ...
        r = serialize_bool(f, "exec-context-syscall-allow-list", c->syscall_allow_list);
        if (r < 0)
                return r;
        ...
        r = serialize_bool(f, "exec-context-syscall-log-allow-list", c->syscall_log_allow_list);
        if (r < 0)
                return r;
#endif
        ...
        r = serialize_bool(f, "exec-context-restrict-filesystems-allow-list", c->restrict_filesystems_allow_list);
        if (r < 0)
                return r;
#endif
        ...
        r = serialize_bool(f, "exec-context-address-families-allow-list", c->address_families_allow_list);
        if (r < 0)
                return r;
        ...
        r = serialize_bool(f, "exec-context-is-system", c->context_is_system);
        if (r < 0)
                return r;
...
int exec_context_deserialize(ExecContext *c, FILE *f, FDSet *fds) {
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "exec-context-environment="))) {
                        ...
                        _cleanup_free_ char *type = NULL, *mode = NULL, *value = NULL, *symlinks = NULL;
                        ...
                        for (;;) {
                                ...
                                r = extract_first_word(&val, &tuple, WHITESPACE, 0);
                                if (r < 0)
                                        return r;
                                if (r == 0)
                                        break;
                                ...
                                p = tuple;
                                r = extract_many_words(&p, ":", 0, &path, &only_create, NULL);
                                if (r < 0)
                                        return r;
                                if (r < 2)
                                        continue;
                                ...
                                c->directories[dt].items[c->directories[dt].n_items - 1].symlinks = strv_split(p, ":");
                                if (!c->directories[dt].items[c->directories[dt].n_items - 1].symlinks)
                                        return log_oom();
                        }
                        ...
                        r = rlimit_parse_one(type, val, &rlim);
                        if (r < 0)
                                return r;
                        ...
                        if (soft)
                                c->rlimit[type]->rlim_cur = rlim;
                        else
                                c->rlimit[type]->rlim_max = rlim;
                        ...
                        c->stdio_fdname[STDIN_FILENO] = strdup(val);
                        if (!c->stdio_fdname[STDIN_FILENO])
                                return log_oom();
                } else if ((val = startswith(l, "exec-context-std-input-fd-name="))) {
                        c->stdio_fdname[STDIN_FILENO] = strdup(val);
                        if (!c->stdio_fdname[STDIN_FILENO])
                        ...
                        c->stdio_fdname[STDOUT_FILENO] = strdup(val);
                        if (!c->stdio_fdname[STDOUT_FILENO])
                                return log_oom();
                } else if ((val = startswith(l, "exec-context-std-error-fd-name="))) {
                        c->stdio_fdname[STDERR_FILENO] = strdup(val);
                        if (!c->stdio_fdname[STDERR_FILENO])
                                return log_oom();
                } else if ((val = startswith(l, "exec-context-std-input-file="))) {
                        c->stdio_file[STDIN_FILENO] = strdup(val);
                        if (!c->stdio_file[STDIN_FILENO])
                                return log_oom();
                } else if ((val = startswith(l, "exec-context-std-output-file="))) {
                        c->stdio_file[STDOUT_FILENO] = strdup(val);
                        if (!c->stdio_file[STDOUT_FILENO])
                                return log_oom();
                        ...
                        c->stdio_file[STDERR_FILENO] = strdup(val);
                        if (!c->stdio_file[STDERR_FILENO])
                                return log_oom();
                        ...
                        c->syslog_identifier = strdup(val);
                        if (!c->syslog_identifier)
                                return log_oom();
                        ...
                        r = extract_first_word(&val, &word, NULL, EXTRACT_UNQUOTE);
                        if (r < 0)
                                return r;
                        if (r == 0)
                                return 0;
                        ...
                        c->smack_process_label = strdup(val);
                        if (!c->smack_process_label)
                                return log_oom();
                        ...
                        c->network_namespace_path = strdup(val);
                        if (!c->network_namespace_path)
                                return log_oom();
                } else if ((val = startswith(l, "exec-context-ipc-namespace-path="))) {
                        c->ipc_namespace_path = strdup(val);
                        if (!c->ipc_namespace_path)
                                return log_oom();
                        ...
                        *lc = (ExecLoadCredential) {
                        ...
                        TAKE_PTR(lc);
                } else if ((val = startswith(l, "exec-context-is-system="))) {
                        r = parse_boolean(val);
                        if (r < 0)
                                return r;
                        c->context_is_system = r;
