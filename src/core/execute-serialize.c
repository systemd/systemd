...
static int exec_runtime_serialize(const ExecRuntime *rt, FILE *f, FDSet *fds, bool serialize_fd_index) {
        ...
        if (rt->shared) {
                ...
                if (rt->shared->netns_storage_socket[0] >= 0 && rt->shared->netns_storage_socket[1] >= 0) {
                        ...
                        if (serialize_fd_index)
                                a = fdset_put_indexed(fds, rt->shared->netns_storage_socket[0]);
                        else
                        ...
                        if (serialize_fd_index)
                                b = fdset_put_indexed(fds, rt->shared->netns_storage_socket[1]);
                        else
                ...
                if (rt->shared->ipcns_storage_socket[0] >= 0 && rt->shared->ipcns_storage_socket[1] >= 0) {
                        ...
                        if (serialize_fd_index)
                                a = fdset_put_indexed(fds, rt->shared->ipcns_storage_socket[0]);
                        else
                        ...
                        if (serialize_fd_index)
                                b = fdset_put_indexed(fds, rt->shared->ipcns_storage_socket[1]);
                        else
        ...
        if (rt->ephemeral_storage_socket[0] >= 0 && rt->ephemeral_storage_socket[1] >= 0) {
                ...
                if (serialize_fd_index)
                        a = fdset_put_indexed(fds, rt->ephemeral_storage_socket[0]);
                else
                ...
                if (serialize_fd_index)
                        b = fdset_put_indexed(fds, rt->ephemeral_storage_socket[1]);
                else
                ...
                if ((val = startswith(l, "exec-runtime-id="))) {
                        ...
                        for (size_t i = 0; i < 2; ++i) {
                                ...
                                if (startswith(l, "exec-runtime-netns-storage-socket-by-fd-index="))
                                        fd = deserialize_fd_from_array(w, fds_array, n_fds_array);
                                else
                                        fd = deserialize_fd_from_set(w, fds);
                                if (fd < 0) {
...
static int exec_parameters_serialize(const ExecParameters *p, FILE *f, FDSet *fds, bool serialize_fd_index) {
        ...
        if (p->n_socket_fds + p->n_storage_fds > 0) {
                ...
                for (size_t i = 0; i < p->n_socket_fds + p->n_storage_fds; ++i) {
                        ...
                        if (p->fds[i] >= 0) {
                                if (serialize_fd_index)
                                        copy = fdset_put_indexed(fds, p->fds[i]);
                                else
        ...
        for (ExecDirectoryType dt = 0; dt < _EXEC_DIRECTORY_TYPE_MAX; dt++) {
                ...
                r = serialize_item(f, key, strempty(p->prefix[dt]));
                if (r < 0)
                        return r;
        }
        ...
        if (p->idle_pipe) {
                ...
                for (size_t i = 0; i < 4; ++i) {
                        ...
                        if (p->idle_pipe[i] >= 0) {
                                if (serialize_fd_index)
                                        copy = fdset_put_indexed(fds, p->idle_pipe[i]);
                                else
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "exec-parameters-runtime-scope="))) {
                        ...
                        if (startswith(l, "exec-parameters-stdin-fd-by-fd-index="))
                                fd = deserialize_fd_from_array(val, fds_array, n_fds_array);
                        else
                                fd = deserialize_fd_from_set(val, fds);
                        if (fd < 0) {
...
static int exec_context_serialize(const ExecContext *c, FILE *f) {
        ...
        r = serialize_item_format_tristate(f, "exec-context-memory-ksm", c->memory_ksm);
        if (r < 0)
                return r;
