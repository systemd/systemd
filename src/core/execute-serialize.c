...
static int exec_context_serialize(const ExecContext *c, FILE *f) {
        ...
        if (c->bpf_delegate_commands) {
                char s[20];
                bpf_delegate_to_string(c->bpf_delegate_commands, s);
                r = serialize_item(f, "exec-context-bpf-delegate-commands", s);
                if (r < 0)
                        return r;
        }
...
static int exec_context_deserialize(ExecContext *c, FILE *f) {
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "exec-context-environment="))) {
                        ...
                        (void) bpf_delegate_from_string(val, &c->bpf_delegate_commands);
