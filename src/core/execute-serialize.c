...
static int exec_cgroup_context_serialize(const CGroupContext *c, FILE *f) {
        ...
        r = serialize_item(f, "exec-cgroup-context-memory-pressure-watch", cgroup_pressure_watch_to_string(c->pressure_watch[PRESSURE_MEMORY]));
        if (r < 0)
                return r;

        r = serialize_item(f, "exec-cgroup-context-cpu-pressure-watch", cgroup_pressure_watch_to_string(c->pressure_watch[PRESSURE_CPU]));
        if (r < 0)
                return r;

        r = serialize_item(f, "exec-cgroup-context-io-pressure-watch", cgroup_pressure_watch_to_string(c->pressure_watch[PRESSURE_IO]));
        if (r < 0)
                return r;
        ...
        if (c->pressure_threshold_usec[PRESSURE_MEMORY] != USEC_INFINITY) {
                r = serialize_usec(f, "exec-cgroup-context-memory-pressure-threshold-usec", c->pressure_threshold_usec[PRESSURE_MEMORY]);
                if (r < 0)
                        return r;
        }

        if (c->pressure_threshold_usec[PRESSURE_CPU] != USEC_INFINITY) {
                r = serialize_usec(f, "exec-cgroup-context-cpu-pressure-threshold-usec", c->pressure_threshold_usec[PRESSURE_CPU]);
                if (r < 0)
                        return r;
        }

        if (c->pressure_threshold_usec[PRESSURE_IO] != USEC_INFINITY) {
                r = serialize_usec(f, "exec-cgroup-context-io-pressure-threshold-usec", c->pressure_threshold_usec[PRESSURE_IO]);
                if (r < 0)
                        return r;
        }
...
static int exec_cgroup_context_deserialize(CGroupContext *c, FILE *f) {
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "exec-cgroup-context-io-accounting="))) {
                        ...
                        c->pressure_watch[PRESSURE_MEMORY] = cgroup_pressure_watch_from_string(val);
                        if (c->pressure_watch[PRESSURE_MEMORY] < 0)
                                return -EINVAL;
                } else if ((val = startswith(l, "exec-cgroup-context-cpu-pressure-watch="))) {
                        c->pressure_watch[PRESSURE_CPU] = cgroup_pressure_watch_from_string(val);
                        if (c->pressure_watch[PRESSURE_CPU] < 0)
                                return -EINVAL;
                } else if ((val = startswith(l, "exec-cgroup-context-io-pressure-watch="))) {
                        c->pressure_watch[PRESSURE_IO] = cgroup_pressure_watch_from_string(val);
                        if (c->pressure_watch[PRESSURE_IO] < 0)
                                return -EINVAL;
                        ...
                        r = deserialize_usec(val, &c->pressure_threshold_usec[PRESSURE_MEMORY]);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-cpu-pressure-threshold-usec="))) {
                        r = deserialize_usec(val, &c->pressure_threshold_usec[PRESSURE_CPU]);
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-cgroup-context-io-pressure-threshold-usec="))) {
                        r = deserialize_usec(val, &c->pressure_threshold_usec[PRESSURE_IO]);
                        if (r < 0)
                                return r;
