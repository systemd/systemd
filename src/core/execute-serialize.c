...
static int exec_parameters_serialize(const ExecParameters *p, FILE *f, FDSet *fds) {
        ...
        if (!isempty(p->default_smack_process_label)) {
...
static int exec_context_serialize(const ExecContext *c, FILE *f, FDSet *fds) {
        ...
        r = serialize_item_format(f, "exec-context-scope", "%d", c->scope);
        if (r < 0)
                return r;
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "exec-context-environment="))) {
                        ...
                        *lc = (ExecLoadCredential) {
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-context-scope="))) {
                        r = safe_atoi(val, &c->scope);
                        if (r < 0)
                                return r;
                }
        }
...
int exec_deserialize(FILE *f,
        ...
        r = exec_context_deserialize(unit_get_exec_context(*ret_unit), f, fds);
        if (r < 0)
                return r;
