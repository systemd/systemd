...
static int exec_context_serialize(const ExecContext *c, FILE *f) {
        ...
        r = serialize_item_tristate(f, "exec-context-memory-thp-madvise", c->memory_thp_disable);
        if (r < 0)
                return r;
...
static int exec_context_deserialize(ExecContext *c, FILE *f) {
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "exec-context-environment="))) {
                        ...
                        if (r < 0)
                                return r;
                } else if ((val = startswith(l, "exec-context-memory-thp-madvise="))) {
                        r = safe_atou(val, &c->memory_thp_disable);
                        if (r < 0)
                                return r;
