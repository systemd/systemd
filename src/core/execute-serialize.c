                ...
                if ((val = startswith(l, "exec-parameters-environment="))) {
                        ...
                        for (size_t i = 0; i < p->n_socket_fds + p->n_storage_fds; ++i) {
                                ...
                                if (r < 0)
                                        return r;
                                if (r == 0)
                                        return -EINVAL; /* Expect p->n_socket_fds + p->n_storage_fds FDs */

                                if ((fd = parse_fd(w)) < 0 || !fdset_contains(fds, fd))
                                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to parse %s value: %s.", l, w);

                                if (fdset_remove(fds, fd) < 0)
                                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to remove %s value=%d from fdset", l, fd);
