...
static int exec_runtime_serialize(const ExecRuntime *rt, FILE *f, FDSet *fds, int *index) {
        ...
        if (rt->shared) {
                ...
                if (rt->shared->netns_storage_socket[0] >= 0 && rt->shared->netns_storage_socket[1] >= 0) {
                        ...
                        if (index)
                                a = fdset_put_dup_indexed(fds, rt->shared->netns_storage_socket[0], (*index)++);
                        else
                        ...
                        if (index)
                                b = fdset_put_dup_indexed(fds, rt->shared->netns_storage_socket[1], (*index)++);
                        else
                        ...
                        r = serialize_item_format(
                                        f,
                                        index ? "exec-runtime-netns-storage-socket-by-fd-index" : "exec-runtime-netns-storage-socket",
                                        "%d %d",
                                        a,
                                        b);
                        if (r < 0)
                ...
                if (rt->shared->ipcns_storage_socket[0] >= 0 && rt->shared->ipcns_storage_socket[1] >= 0) {
                        ...
                        if (index)
                                a = fdset_put_dup_indexed(fds, rt->shared->ipcns_storage_socket[0], (*index)++);
                        else
                        ...
                        if (index)
                                b = fdset_put_dup_indexed(fds, rt->shared->ipcns_storage_socket[1], (*index)++);
                        else
                        ...
                        r = serialize_item_format(
                                        f,
                                        index ? "exec-runtime-ipcns-storage-socket-by-fd-index" : "exec-runtime-ipcns-storage-socket",
                                        "%d %d",
                                        a,
                                        b);
                        if (r < 0)
        ...
        if (rt->dynamic_creds) {
                r = dynamic_user_serialize_one(
                                ...
                                index ? "exec-runtime-dynamic-creds-user-by-fd-index" : "exec-runtime-dynamic-creds-user",
                                f,
                                fds,
                                index);
                if (r < 0)
        ...
        if (rt->dynamic_creds && rt->dynamic_creds->group && rt->dynamic_creds->group == rt->dynamic_creds->user) {
                ...
                r = dynamic_user_serialize_one(
                                ...
                                index ? "exec-runtime-dynamic-creds-group-by-fd-index" : "exec-runtime-dynamic-creds-group",
                                f,
                                fds,
                                index);
                if (r < 0)
        ...
        if (rt->ephemeral_storage_socket[0] >= 0 && rt->ephemeral_storage_socket[1] >= 0) {
                ...
                if (index)
                        a = fdset_put_dup_indexed(fds, rt->ephemeral_storage_socket[0], (*index)++);
                else
                ...
                if (index)
                        b = fdset_put_dup_indexed(fds, rt->ephemeral_storage_socket[1], (*index)++);
                else
                ...
                r = serialize_item_format(
                                f,
                                index ? "exec-runtime-ephemeral-storage-socket-by-fd-index" : "exec-runtime-ephemeral-storage-socket",
                                "%d %d",
                                a,
                                b);
                if (r < 0)
...
static int exec_parameters_serialize(const ExecParameters *p, FILE *f, FDSet *fds, int *index) {
        ...
        if (p->idle_pipe) {
                ...
                for (size_t i = 0; i < 4; ++i) {
                        ...
                        if (p->idle_pipe[i] >= 0) {
                                if (index)
                                        copy = fdset_put_dup_indexed(fds, p->idle_pipe[i], (*index)++);
                                else
