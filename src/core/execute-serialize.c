...
static int exec_parameters_serialize(const ExecParameters *p, const ExecContext *c, FILE *f, FDSet *fds) {
        ...
        r = serialize_item(f, "exec-parameters-virtualization", virtualization_to_string(p->virtualization));
        if (r < 0)
                return r;
...
static int exec_parameters_deserialize(ExecParameters *p, FILE *f, FDSet *fds) {
        ...
        for (;;) {
                ...
                } else if ((val = startswith(l, "exec-parameters-virtualization="))) {
                        p->virtualization = virtualization_from_string(val);
                        if (p->virtualization < 0)
                                return p->virtualization;
