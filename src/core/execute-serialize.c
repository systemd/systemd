...
static int exec_unit_serialize(const Unit *u, FILE *f) {
        ...
        r = serialize_item(f, "exec-unit-id-field", u->id_field);
        if (r < 0)
                return r;

        r = serialize_item(f, "exec-unit-id-field-format", u->id_field_format);
        if (r < 0)
                return r;
        ...
        r = serialize_item(f, "exec-unit-invocation-id-field", u->invocation_id_field);
        if (r < 0)
                return r;

        r = serialize_item(f, "exec-unit-invocation-id-field-format", u->invocation_id_field_format);
        if (r < 0)
                return r;
...
static int exec_unit_deserialize(Unit **ret_unit, FILE *f) {
        ...
        _cleanup_free_ char *id = NULL, *id_field = NULL, *id_field_format = NULL,
                            *invocation_id_field = NULL, *invocation_id_field_format = NULL;
        ...
        for (;;) {
                ...
                } else if ((val = startswith(l, "exec-unit-id-field="))) {
                        r = free_and_strdup(&id_field, val);
                        if (r < 0)
                                return log_oom_debug();
                } else if ((val = startswith(l, "exec-unit-id-field-format="))) {
                        r = free_and_strdup(&id_field_format, val);
                        if (r < 0)
                                return log_oom_debug();
                ...
                } else if ((val = startswith(l, "exec-unit-invocation-id-field="))) {
                        r = free_and_strdup(&invocation_id_field, val);
                        if (r < 0)
                                return log_oom_debug();
                } else if ((val = startswith(l, "exec-unit-invocation-id-field-format="))) {
                        r = free_and_strdup(&invocation_id_field_format, val);
                        if (r < 0)
                                return log_oom_debug();
        ...
        if (id_field) {
                if (strlen(id_field) > sizeof(unit->invocation_id_field) - 1)
                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "ID field name too long: '%s'",
                                               id_field);
                strcpy(unit->id_field, id_field);
        }
        if (id_field_format) {
                if (strlen(id_field_format) > sizeof(unit->id_field_format) - 1)
                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "ID field format too long: '%s'",
                                               id_field_format);
                strcpy(unit->id_field_format, id_field_format);
        }
        if (invocation_id_field) {
                if (strlen(invocation_id_field) > sizeof(unit->invocation_id_field) - 1)
                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "Invocation ID field name too long: '%s'",
                                               invocation_id_field);
                strcpy(unit->invocation_id_field, invocation_id_field);
        }
        if (invocation_id_field_format) {
                if (strlen(invocation_id_field_format) > sizeof(unit->invocation_id_field_format) - 1)
                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "Invocation ID field format too long: '%s'",
                                               invocation_id_field_format);
                strcpy(unit->invocation_id_field_format, invocation_id_field_format);
        }
...
static int exec_parameters_serialize(const ExecParameters *p, FILE *f, FDSet *fds) {
        ...
        r = serialize_item_format(f, "exec-parameters-runtime-scope", "%d", p->runtime_scope);
        if (r < 0)
                return r;
...
static int exec_parameters_deserialize(ExecParameters *p, FILE *f, FDSet *fds) {
        int r, rlimit_nofiles;

        assert(p);
        assert(f);
        assert(fds);

        rlimit_nofiles = read_nr_open();
        if (rlimit_nofiles < 3)
                rlimit_nofiles = HIGH_RLIMIT_NOFILE;
        assert(rlimit_nofiles > 0); /* For compilers/static analyzers */
        ...
        for (;;) {
                ...
                if ((val = startswith(l, "exec-parameters-runtime-scope="))) {
                        r = safe_atoi(val, &p->runtime_scope);
                        if (r < 0)
                                return r;
                        ...
                        if (p->n_socket_fds > (size_t) rlimit_nofiles)
                        ...
                        if (p->n_storage_fds > (size_t) rlimit_nofiles)
                                return -EINVAL; /* too many, someone is playing games with us */
                        ...
                        if (p->n_socket_fds + p->n_storage_fds > (size_t) rlimit_nofiles)
                                return -EINVAL; /* too many, someone is playing games with us */
