...
int dynamic_user_serialize_one(DynamicUser *d, const char *key, FILE *f, FDSet *fds, bool store_index) {
        ...
        if (store_index)
                copy0 = fdset_put_indexed(fds, d->storage_socket[0]);
        else
        ...
        if (store_index)
                copy1 = fdset_put_indexed(fds, d->storage_socket[1]);
        else
...
void dynamic_user_deserialize_one(
        ...
        if (store_index)
                fd0 = deserialize_fd_from_array(s0, fds_array, n_fds_array);
        else
                fd0 = deserialize_fd_from_set(s0, fds);
        if (fd0 < 0) {
        ...
        if (store_index)
                fd1 = deserialize_fd_from_array(s1, fds_array, n_fds_array);
        else
                fd1 = deserialize_fd_from_set(s1, fds);
        if (fd1 < 0) {
