...
_noreturn_ static void crash(int sig, siginfo_t *siginfo, void *context) {
        ...
        if (getpid_cached() != 1)
                ...
                if (pid < 0) {
                        _cleanup_free_ char* message;
                        asprintf(&message, "Caught <%s>, cannot fork for core dump: %m", signal_to_string(sig));
                        ...
                        if (r < 0) {
                                _cleanup_free_ char* message;
                                asprintf(&message, "Caught <%s>, waitpid() failed: %m", signal_to_string(sig));
        ...
        if (arg_crash_shell) {
                ...
                pid = raw_clone(SIGCHLD);
                if (pid < 0)
                        log_emergency_errno(errno, SD_MESSAGE_CRASH_SHELL_FORK_FAILED_STR, "Failed to fork off crash shell: %m");
                else if (pid == 0) {
                        (void) setsid();
                        ...
                        log_emergency_errno(errno, SD_MESSAGE_CRASH_EXECLE_FAILED_STR, "execle() failed: %m");
                        _exit(EXIT_EXCEPTION);
                } else {
