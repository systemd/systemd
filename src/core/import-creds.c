...
static int promote_firstboot_credentials(bool first_boot, const char *received_creds_dir) {
         ...
         *
         * Note the systemd.import_credentials=no opt-out is honored transitively: @firstboot is only ever
         * staged by import_credentials_boot(), which itself respects the opt-out, so when it is set nothing
         * is staged and the open() below fails with ENOENT. */
