...
static int import_credentials_boot(void) {
         ...
         *
         *  - /.extra/system_credentials/ and /.extra/global_system_credentials/ — placed
         *    by host-side producers that take responsibility for the trust (e.g. systemd-vmspawn
         *    when its cpio is covered by the SEV-SNP launch measurement or when the host is
         *    the trust root in non-confidential setups). Routed into the @system bucket where
         *    consumers can use them directly via LoadCredential=. */
        ...
        FOREACH_STRING(p, "/.extra/system_credentials/") {
                r = import_credentials_from_initrd_path(
                        &system_ctx, p,
        ...
        if (system_ctx.n_credentials > 0) {
                log_debug("Imported %u trusted credentials from boot loader.", system_ctx.n_credentials);
