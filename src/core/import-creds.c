...
static int import_credentials_boot(void) {
        ...
        /* systemd-stub will wrap sidecard *.cred files from the UEFI kernel image directory into initrd
...
static int proc_cmdline_callback(const char *key, const char *value, void *data) {
        ...
        if (!proc_cmdline_key_streq(key, "systemd.set-credential"))
                return 0;
...
int import_credentials(void) {
        ...
        if (envvar_set) {
                ...
                r = proc_cmdline_get_key("systemd.import_credentials", 0, &v);
                if (r < 0)
