...
static int parse_smbios_strings(ImportCredentialContext *c, const char *data, size_t size) {
        ...
        /* Unpacks a packed series of SMBIOS OEM vendor strings. These are a series of NUL termianted strings, one after the other. */
        ...
        for (p = data, left = size; left > 0; p += skip, left -= skip) {
                ...
                if (unbase64) {
                        ...
                        if (r < 0) {
                                log_warning("Failed to base64 decode credential '%s': %m", cn);
                                continue;
                        }
