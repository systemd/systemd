        ...
        for (slash = strchr(s->path, '/'); ; slash = strchr(slash+1, '/')) {
                ...
                if (slash) {
                        ...
                        *cut = '\0';
                        ...
                        SET_FLAG(flags, IN_DELETE, p->deactivation_toggle);
                } else {
                        cut = NULL;
                ...
                if (oldslash) {
                        ...
                        *cut2 = '\0';
                        ...
                        flags = IN_MOVE_SELF;
                        SET_FLAG(flags, IN_CREATE | IN_DELETE, p->deactivation_toggle);
...
static void path_enter_waiting(Path *p, bool initial, bool from_trigger_notify) {
        ...
        if (trigger && !UNIT_IS_INACTIVE_OR_DEACTIVATING(unit_active_state(trigger))) {
                ...
                if (p->deactivation_toggle && p->state == PATH_RUNNING &&
                    !path_check_good(p, true, from_trigger_notify, &trigger_path)) {
        ...
        if (path_check_good(p, initial, from_trigger_notify, &trigger_path)) {
                ...
                /* If the path was created before it was watched, watch it here */
                if (p->deactivation_toggle && !IN_SET(p->state, PATH_WAITING, PATH_RUNNING)) {
                        r = path_watch(p);
                        if (r < 0) {
                                log_unit_warning_errno(UNIT(p), r, "Failed to watch path: %m");
                                path_enter_dead(p, PATH_FAILURE_RESOURCES);
                                return;
                        }
                }
