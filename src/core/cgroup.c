...
void cgroup_oomd_xattr_apply(Unit *u, const char *cgroup_path) {
        ...
        r = cg_remove_xattr(SYSTEMD_CGROUP_CONTROLLER, cgroup_path, "trusted.oomd_avoid");
        if (r != -ENODATA)
                log_unit_debug_errno(u, r, "Failed to remove oomd_avoid flag on control group %s, ignoring: %m", cgroup_path);

        r = cg_remove_xattr(SYSTEMD_CGROUP_CONTROLLER, cgroup_path, "trusted.oomd_omit");
        if (r != -ENODATA)
        ...
        if (c->moom_preference == MANAGED_OOM_PREFERENCE_AVOID) {
                r = cg_set_xattr(SYSTEMD_CGROUP_CONTROLLER, cgroup_path, "trusted.oomd_avoid", "1", 1, 0);
                if (r < 0)
                ...
                r = cg_set_xattr(SYSTEMD_CGROUP_CONTROLLER, cgroup_path, "trusted.oomd_omit", "1", 1, 0);
                if (r < 0)
