...
static int unit_realize_cgroup_now(Unit *u, ManagerState state) {
        ...
        if (unit_has_mask_realized(u, target_mask, enable_mask)) {
                /* The cgroup is up to date, but its xattrs might not be: they follow settings like
                 * LogLevelMax= or LogFilterPatterns= that the realization masks know nothing
                 * about. Re-apply them to converge. */
                cgroup_xattr_apply(u);
                return 0;
        }
