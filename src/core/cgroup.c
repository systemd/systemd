...
static int unit_check_cgroup_events(Unit *u) {
        ...
        if (values[1] && IN_SET(u->freezer_state, FREEZER_FREEZING, FREEZER_PARENT_FREEZING, FREEZER_THAWING)) {
                if (streq(values[1], "0"))
                        unit_thawed(u);
                else
                        unit_frozen(u);
...
int unit_cgroup_freezer_action(Unit *u, FreezerAction action) {
        _cleanup_free_ char *path = NULL;
        FreezerState target, current = _FREEZER_STATE_INVALID, next;
        int r;
        ...
        if (current == target)
        ...
        else if (IN_SET(next, FREEZER_FROZEN, FREEZER_PARENT_FROZEN, FREEZER_RUNNING)) {
                ...
                log_unit_warning(u, "Unit wants to transition to %s freezer state but cgroup is unexpectedly %s, fixing up.",
                                 freezer_state_to_string(next), freezer_state_to_string(current));

                if (next == FREEZER_FROZEN)
                        next = FREEZER_FREEZING;
                else if (next == FREEZER_PARENT_FROZEN)
