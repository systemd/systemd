...
static bool cgroup_cpuset_partition_invalid(const char *partition) {
        _cleanup_free_ char *_partition = NULL, *invalid = NULL;
        int r;

        assert(partition);

        /* An invalid line looks like <partition> invalid (<reason>) */
        r = extract_many_words(&partition, NULL, 0, &_partition, &invalid);
        ...
        return strcmp(invalid, "invalid") == 0;
}

static void cgroup_apply_cpuset_partition(Unit *u, const char *partition, const char *name) {
        _cleanup_free_ char *buf = NULL;
        CGroupRuntime *crt;
        int r;
        ...
        else if (cgroup_cpuset_partition_invalid(buf))
...
static void cgroup_context_apply(
        ...
        if ((apply_mask & CGROUP_MASK_CPUSET) && !is_local_root) {
                ...
                if (c->cpuset_partition >= 0)
                        cgroup_apply_cpuset_partition(u, cpuset_partition_to_string(c->cpuset_partition), "cpuset.cpus.partition");
        }
