...
void cgroup_log_xattr_apply(Unit *u, const char *cgroup_path) {
        ...
        if (c->log_filter_regex)
                unit_set_xattr_graceful(u, cgroup_path, "user.journald_log_filter_regex",
                                        c->log_filter_regex, strlen(c->log_filter_regex));
        else
                unit_remove_xattr_graceful(u, cgroup_path, "user.journald_log_filter_regex");
}
