...
int cgroup_log_xattr_apply(Unit *u, const char *cgroup_path) {
        ...
        if (!c)
                return log_debug_errno(-EINVAL, "Failed to get ExecContext from Unit: %m");
        ...
        if (r < 0)
                return log_debug_errno(r, "Failed to push to strv: %m");
        ...
        if (r < 0)
                return log_debug_errno(r, "Failed to get nulstr from strv: %m");
