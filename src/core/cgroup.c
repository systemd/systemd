...
bool unit_has_startup_cgroup_constraints(Unit *u) {
        ...
        return c->startup_cpu_shares != CGROUP_CPU_SHARES_INVALID ||
               ...
               c->startup_memory_high != CGROUP_LIMIT_MAX ||
               c->startup_memory_max != CGROUP_LIMIT_MAX ||
               c->startup_memory_swap_max != CGROUP_LIMIT_MAX ||
               c->startup_memory_zswap_max != CGROUP_LIMIT_MAX ||
...
static bool unit_has_unified_memory_config(Unit *u) {
        ...
        return unit_get_ancestor_memory_min(u) > 0 ||
               ...
               c->memory_high != CGROUP_LIMIT_MAX || c->startup_memory_high != CGROUP_LIMIT_MAX ||
               c->memory_max != CGROUP_LIMIT_MAX || c->startup_memory_max != CGROUP_LIMIT_MAX ||
               c->memory_swap_max != CGROUP_LIMIT_MAX || c->startup_memory_swap_max != CGROUP_LIMIT_MAX ||
               c->memory_zswap_max != CGROUP_LIMIT_MAX || c->startup_memory_zswap_max != CGROUP_LIMIT_MAX;
}
                        ...
                        if (unit_has_unified_memory_config(u)) {
                                if (IN_SET(state, MANAGER_STARTING, MANAGER_INITIALIZING, MANAGER_STOPPING) &&
                                    c->startup_memory_high_set)
                                        high = c->startup_memory_high;
                                else
                                        high = c->memory_high;

                                if (IN_SET(state, MANAGER_STARTING, MANAGER_INITIALIZING, MANAGER_STOPPING) &&
                                    c->startup_memory_max_set)
                                        max = c->startup_memory_max;
                                else
                                        max = c->memory_max;

                                if (IN_SET(state, MANAGER_STARTING, MANAGER_INITIALIZING, MANAGER_STOPPING) &&
                                    c->startup_memory_swap_max_set)
                                        swap_max = c->startup_memory_swap_max;
                                else
                                        swap_max = c->memory_swap_max;

                                if (IN_SET(state, MANAGER_STARTING, MANAGER_INITIALIZING, MANAGER_STOPPING) &&
                                    c->startup_memory_zswap_max_set)
                                        zswap_max = c->startup_memory_zswap_max;
                                else
