...
static int set_bfq_weight(Unit *u, const char *controller, dev_t dev, uint64_t io_weight) {
         ...
         * 795fe54c2a82 ("bfq: Add per-device weight") v5.4-rc1~148^2~20
         * are not interesting anymore. Old kernels will fail with EINVAL, while new kernels won't return
         * EINVAL on properly formatted input by us. Treat EINVAL specially. */
        if (r < 0 && r == -EINVAL && major(dev) > 0) {
               if (!warned) {
...
static void cgroup_apply_io_device_weight(Unit *u, const char *dev_path, uint64_t io_weight) {
        ...
        if (r1 == -EOPNOTSUPP && r2 == -EOPNOTSUPP)
                r = -EOPNOTSUPP; /* unconfigured device */
        else if (r1 == -EOPNOTSUPP)
                r = r2;
        else if (r2 == -EOPNOTSUPP)
                r = r1;
        else /* both fail, prefer latter errno */
                r = r2;
