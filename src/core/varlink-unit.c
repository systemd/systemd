...
static int transient_scope_apply_properties(Scope *s, TransientScopeParameters *sp, const char **reterr_field, sd_bus_error *bus_error) {
        Unit *u = UNIT(ASSERT_PTR(s));
        int r;
        ...
        FOREACH_ARRAY(p, sp->pids, sp->n_pids) {
                r = unit_pid_attachable(u, p, bus_error);
                if (r >= 0)
