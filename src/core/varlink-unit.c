...
static void varlink_job_broadcast_change(Job *j, bool finished) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *plain = NULL, *with_runtime = NULL;
        ...
        HASHMAP_FOREACH_KEY(flags, link, m->varlink_job_subscribers) {
                ...
                bool wr = finished && PTR_TO_UINT(flags) == VARLINK_JOB_SUBSCRIBER_WITH_RUNTIME;
                sd_json_variant **v = wr ? &with_runtime : &plain;
                if (!*v) {
                        r = job_notification_build(j, wr, v);
                        if (r < 0) {
...
int vl_method_subscribe_jobs(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        static const sd_json_dispatch_field dispatch_table[] = {
                { "withRuntime", SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_tristate, 0, 0 },
                {}
        };
        ...
        unsigned f = with_runtime > 0 ? VARLINK_JOB_SUBSCRIBER_WITH_RUNTIME : VARLINK_JOB_SUBSCRIBER_PLAIN;
        r = hashmap_ensure_put(&m->varlink_job_subscribers, NULL, link, UINT_TO_PTR(f));
        if (r < 0)
                return r;
