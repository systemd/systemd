...
static void transient_scope_parameters_done(TransientScopeParameters *p) {
        ...
        pidref_array_free(p->pids, p->n_pids);
}
...
static int dispatch_pidref_array(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        ...
        CLEANUP_ARRAY(pids, n, pidref_array_free);
        ...
        for (; n < n_pids; n++) {
                ...
                r = json_dispatch_pidref(NULL, sd_json_variant_by_index(variant, n), flags, &pids[n]);
                if (r < 0)
                        return r;
        }
...
static int dispatch_transient_scope(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        static const sd_json_dispatch_field scope_dispatch[] = {
                ...
                { "PIDs",                       SD_JSON_VARIANT_ARRAY,         dispatch_pidref_array,   0,                                                           SD_JSON_STRICT },
                {}
        };
...
static int transient_scope_apply_properties(Scope *s, TransientScopeParameters *sp, const char **reterr_field, sd_bus_error *reterr_bus_error) {
        Unit *u = UNIT(ASSERT_PTR(s));
        int r;
        ...
        FOREACH_ARRAY(p, sp->pids, sp->n_pids) {
                r = unit_pid_attachable(u, p, reterr_bus_error);
                if (r >= 0)
...
int vl_method_start_transient_unit(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (scope) {
                bad_field = NULL;
                r = transient_scope_apply_properties(scope, &p.context.scope, &bad_field, &bus_error);
                if (r == -EINVAL)
