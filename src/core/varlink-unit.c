...
static int cgroup_io_device_limits_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        ...
        if (type < 0)
                return log_debug_errno(type, "Failed to get CGroup IO limit type for '%s': %m", name);
...
static int cgroup_context_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        ...
        return sd_json_buildo(ret,
                        ...
                        JSON_BUILD_PAIR_CALLBACK_NON_NULL("DelegateControllers", cgroup_mask_build_json, &c->delegate_controllers),
                        JSON_BUILD_PAIR_CALLBACK_NON_NULL("DisableControllers", cgroup_mask_build_json, &c->disable_controllers),
