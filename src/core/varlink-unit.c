...
/* The file based stdio types require a companion path (StandardInputFile=, StandardOutputFile=, …), which
 * the Varlink interface has no way to pass yet. Without it the executor cannot set them up (and would
 * assert), hence refuse them for now. */
static bool exec_input_refused(ExecInput i) {
        return i == EXEC_INPUT_FILE;
}

static bool exec_output_refused(ExecOutput o) {
...
#define DEFINE_APPLY_EXEC_ENUM(field, JsonName, to_string_fn, refused_fn) \
        static int apply_exec_##field(Unit *u, ExecContext *c, TransientExecContextParameters *p) { \
                ...
                if (refused_fn(p->field))                               \
                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), JsonName "=%s is not supported via Varlink.", to_string_fn(p->field)); \
                c->field = p->field;                                    \
                unit_write_settingf(u, UNIT_RUNTIME|UNIT_PRIVATE, JsonName, \
...
DEFINE_APPLY_EXEC_ENUM(std_error,  "StandardError",  exec_output_to_string, exec_output_refused);
DEFINE_APPLY_EXEC_ENUM(std_input,  "StandardInput",  exec_input_to_string,  exec_input_refused);
DEFINE_APPLY_EXEC_ENUM(std_output, "StandardOutput", exec_output_to_string, exec_output_refused);
...
static int transient_service_apply_properties(Service *s, TransientServiceParameters *sp, const char **reterr_field) {
        ...
        FOREACH_ARRAY(item, sp->exec_start, sp->n_exec_start) {
                ...
                if (item->privileged && item->no_setuid) {
                        ...
                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "ExecStart 'privileged' and 'noSetuid' flags may not be combined.");
                }
        ...
        if (sp->n_exec_start > 0)
                LIST_FOREACH(command, c, s->exec_command[SERVICE_EXEC_START]) {
                        ...
                        if (via_shell || streq(c->path, c->argv[0]))
                                unit_write_settingf(u, UNIT_RUNTIME|UNIT_PRIVATE, "ExecStart", "ExecStart=%s%s%s",
                                                    exec_chars, via_shell && c->argv[0][0] == '-' ? "@" : "", a);
                        else {
                                ...
                                unit_write_settingf(u, UNIT_RUNTIME|UNIT_PRIVATE, "ExecStart", "ExecStart=%s@%s %s", exec_chars, p, a);
                        }
                }
