...
int vl_method_start_transient_unit(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (c) {
                ...
                if (r == -EINVAL)
                        return sd_varlink_error_invalid_parameter_name(link, bad_field ?: "context.Exec");
                if (r < 0)
                        return sd_varlink_error_errno(link, r);
        ...
        if (s) {
                ...
                if (r == -EINVAL)
                        return sd_varlink_error_invalid_parameter_name(link, bad_field ?: "context.Service");
                if (r < 0)
                        return sd_varlink_error_errno(link, r);
