...
static int varlink_unit_queue_job_one(
        ...
        if (r == -ENOENT)
                return varlink_error_no_such_unit(link, "name");
        if (r == -EUNATCH)
                return sd_varlink_errorb(link, VARLINK_ERROR_UNIT_ONLY_BY_DEPENDENCY);
        if (r == -ESHUTDOWN)
                return sd_varlink_errorb(link, VARLINK_ERROR_UNIT_BUS_SHUTTING_DOWN);
        if (r < 0)
                return r;
        ...
        HASHMAP_FOREACH_KEY(u, k, manager->units) {
                ...
                if (r >= 0)
                        r = varlink_unit_queue_job_one(
                                        ...
                                        JOB_TRY_RESTART,
                                        JOB_FAIL,
                                        /* reload_if_possible= */ BIT_SET(u->markers, UNIT_MARKER_NEEDS_RELOAD),
                                        &job_id);
