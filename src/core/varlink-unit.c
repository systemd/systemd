...
static int apply_exec_log_level_max(Unit *u, ExecContext *c, TransientExecContextParameters *p) {
        ...
        /* Validate here, not in the dispatcher, so -EINVAL maps to the precise Exec.LogLevelMax field. */
        l = log_level_from_string(p->log_level_max);
        if (l < 0)
                return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid LogLevelMax= value: %s", p->log_level_max);

        c->log_level_max = l;

        unit_write_settingf(u, UNIT_RUNTIME|UNIT_PRIVATE, "LogLevelMax", "LogLevelMax=%s", p->log_level_max);
        return 0;
}
...
static const TransientExecProperty exec_properties[] = {
        ...
        EXEC_PROPERTY_STRING       ("LogLevelMax",            log_level_max, sd_json_dispatch_const_string, 0),
