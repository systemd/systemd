...
static int can_clean_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        ...
        if (r < 0)
                return log_debug_errno(r, "Failed to unit_can_clean(): %m");
...
static int unit_runtime_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        Unit *u = ASSERT_PTR(userdata);
        Unit *following = unit_following(u);

        return sd_json_buildo(ASSERT_PTR(ret),
                        JSON_BUILD_PAIR_STRING_NON_EMPTY("Following", following ? following->id : NULL),
...
static int lookup_unit_by_name(sd_varlink *link, Manager *manager, const char *name, Unit **ret_unit) {
        assert(link);
        assert(manager);
        assert(ret_unit);
        ...
        Unit *u = manager_get_unit(manager, name);
        if (!u)
                return -ESRCH;
...
int vl_method_list_units(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        static const sd_json_dispatch_field dispatch_table[] = {
                { "name", SD_JSON_VARIANT_STRING,        sd_json_dispatch_const_string, offsetof(UnitLookupParameters, name),   0                                        }, // XXX name or id?
        ...
        if (p.name) {
                r = lookup_unit_by_name(link, manager, p.name, &unit);
                if (r == -EINVAL)
        ...
        HASHMAP_FOREACH_KEY(u, k, manager->units) {
                ...
                /* TODO: we need some kind of filtering to show only necessary units.
                 * However, I'm leaving this to a follow up commits to reduce amount of changes
