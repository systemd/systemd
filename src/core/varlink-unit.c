...
static int varlink_unit_enqueue_job_and_reply(
                sd_varlink *link,
                sd_varlink_method_flags_t flags,
                const EnqueueJobParameters *p,
                void *userdata) {
        ...
        r = load_unit_and_check(link, manager, p->name, &u);
        if (r < 0)
                return r;
        ...
        if (p->job_type == JOB_STOP && u->refuse_manual_stop)
                return sd_varlink_error(link, VARLINK_ERROR_UNIT_ONLY_BY_DEPENDENCY, NULL);
        if (p->job_type != JOB_STOP && u->refuse_manual_start)
                return sd_varlink_error(link, VARLINK_ERROR_UNIT_ONLY_BY_DEPENDENCY, NULL);
