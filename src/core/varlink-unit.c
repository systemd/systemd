...
typedef struct TransientServiceParameters {
        ...
        /* Indices of stdio fds attached to the method call (SCM_RIGHTS), -1 if unset. Resolved into the
         * owned fds below by transient_service_resolve_stdio_fds(). */
        int stdin_fd_index;
        int stdout_fd_index;
        int stderr_fd_index;
        int stdin_fd;
...
static int dispatch_transient_service(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        static const sd_json_dispatch_field service_dispatch[] = {
                ...
                { "StandardInputFileDescriptorIndex",  _SD_JSON_VARIANT_TYPE_INVALID, sd_json_dispatch_int,            offsetof(TransientServiceParameters, stdin_fd_index),    0 },
                { "StandardOutputFileDescriptorIndex", _SD_JSON_VARIANT_TYPE_INVALID, sd_json_dispatch_int,            offsetof(TransientServiceParameters, stdout_fd_index),   0 },
                { "StandardErrorFileDescriptorIndex",  _SD_JSON_VARIANT_TYPE_INVALID, sd_json_dispatch_int,            offsetof(TransientServiceParameters, stderr_fd_index),   0 },
                {}
        };
...
static int transient_service_resolve_stdio_fds(sd_varlink *link, TransientServiceParameters *sp, const char **reterr_field) {
        ...
        if (r < 0) {
                if (reterr_field)
                        *reterr_field = "Service.StandardInputFileDescriptorIndex";
                return r;
        }
        ...
        if (r < 0) {
                if (reterr_field)
                        *reterr_field = "Service.StandardOutputFileDescriptorIndex";
                return r;
        }
        ...
        if (r < 0) {
                if (reterr_field)
                        *reterr_field = "Service.StandardErrorFileDescriptorIndex";
                return r;
        }
