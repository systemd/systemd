...
static int varlink_reply_unit_job_one(sd_varlink *link, Unit *u, Job *j) {
        assert(link);
        assert(u);
        assert(j);
...
static int varlink_reply_unit_job(
        ...
        if (!FLAGS_SET(flags, SD_VARLINK_METHOD_MORE) || (!notify_job_changes && !notify_unit_changes))
                return varlink_reply_unit_job_one(link, u, j);
         ...
         * so reject the second one instead of clobbering the first. Note that callers are expected to
         * refuse an already watched unit before enqueuing a job, so that they can do so without side
         * effects; if we reject here it means the job we got already existed, i.e. the request merged
         * into it and nothing new was enqueued either.
         *
...
static int json_dispatch_unit_names(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
...
int vl_method_enqueue_unit_job(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        static const sd_json_dispatch_field dispatch_table[] = {
                { "names",             SD_JSON_VARIANT_ARRAY,   json_dispatch_unit_names,  offsetof(EnqueueJobParameters, names),               SD_JSON_MANDATORY },
        ...
        if (n == 1) {
                ...
                return varlink_reply_unit_job(link, flags, first, j, p.notify_job_changes > 0, p.notify_unit_changes > 0);
        }
        ...
        SET_FOREACH(j, jobs) {
                ...
                r = varlink_reply_unit_job_one(link, j->unit, j);
                if (r < 0)
                        return r;
        }
