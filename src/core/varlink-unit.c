...
int vl_method_start_transient_unit(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
         ...
         * and runtime, plus a minimal job object so the caller can correlate with later state.
         * The job has just been enqueued and is typically in JOB_WAITING; no Result yet.
         * TODO: unify with varlink_job_send_{change,removed}_signal once a single job_build_json()
         * helper lands. That needs the explicit "finished" state from #41811 and the invalidatable
         * Result from #41809, after which Result can be included unconditionally with suppression. */
