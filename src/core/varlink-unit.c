...
static int parse_unit_markers(UnitSetPropertiesParameters *p, sd_json_variant *variant, sd_json_dispatch_flags_t flags) {
        ...
        JSON_VARIANT_ARRAY_FOREACH(e, variant) {
                ...
                if (r < 0)
                        return json_log(variant, flags, SYNTHETIC_ERRNO(EINVAL), "Failed to parse marker '%s'.", word);
                if (r > 0)
...
static int unit_dispatch_properties(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        ...
        JSON_VARIANT_OBJECT_FOREACH(k, v, variant) {
                if (streq(k, "markers")) {
                        r = parse_unit_markers(p, v, flags);
                        if (r < 0)
                                return r;
                ...
                return json_log(variant, flags, SYNTHETIC_ERRNO(EINVAL), "Unknown property '%s' in 'properties' object.", k);
        }
