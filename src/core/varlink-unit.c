...
static int dispatch_transient_working_directory(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        ...
        static const sd_json_dispatch_field wd_dispatch[] = {
                { "path",      SD_JSON_VARIANT_STRING,  sd_json_dispatch_const_string, offsetof(TransientWorkingDirectory, path),       0 },
...
static int dispatch_transient_set_credential_array(
        ...
        CLEANUP_ARRAY(items, n_items, transient_set_credential_array_free);
        ...
        items = new0(TransientSetCredential, n);
        if (!items)
                return -ENOMEM;

        for (n_items = 0; n_items < n; n_items++) {
                r = sd_json_dispatch(sd_json_variant_by_index(variant, n_items), item_dispatch, /* flags= */ 0, &items[n_items]);
                if (r < 0)
                        return r;
        }
