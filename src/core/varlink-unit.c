...
static void unit_set_properties_parameters_done(UnitSetPropertiesParameters *p) {
        assert(p);
        p->unsupported_property = mfree(p->unsupported_property);
}
...
int vl_method_set_unit_properties(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        static const sd_json_dispatch_field dispatch_table[] = {
         ...
         _cleanup_(unit_set_properties_parameters_done) UnitSetPropertiesParameters p = {};
        Manager *manager = ASSERT_PTR(userdata);
        const char *bad_field = NULL;
        Unit *unit;
        int r;
        ...
        if (r < 0) {
                ...
                if (streq_ptr(bad_field, "properties") && r == -EADDRNOTAVAIL)
                        return sd_varlink_errorbo(
                                ...
                                SD_JSON_BUILD_PAIR_STRING("property", strna(p.unsupported_property)));
                if (bad_field)
