...
/* The file based stdio types require a companion path (StandardInputFile=, StandardOutputFile=, …), and
 * "data" requires the payload from StandardInputData=, none of which the Varlink interface has a way to
 * pass yet. Without their companion field these types are unusable (the executor would silently degrade
 * "data" to "null", and the reply would then misrepresent the effective configuration), hence refuse them
 * for now. */
static bool exec_input_refused(ExecInput i) {
        return exec_input_needs_file(i) || i == EXEC_INPUT_DATA;
}

static bool exec_output_refused(ExecOutput o) {
        return exec_output_needs_file(o);
}
...
static int transient_service_apply_properties(Service *s, TransientServiceParameters *sp, const char **reterr_field) {
        ...
        LIST_FOREACH(command, c, s->exec_command[SERVICE_EXEC_START]) {
                ...
                r = exec_command_to_exec_setting(c, &a);
                if (r < 0)
                        return r;
