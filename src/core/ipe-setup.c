...
int ipe_setup(void) {
        ...
        bool found = false;
        NULSTR_FOREACH(p, CONF_PATHS_NULSTR("ipe"))
                if (dir_is_empty(p, /* ignore_hidden_or_backup= */ true) == 0) {
                        found = true;
                        break;
                }
        if (!found) {
                log_debug("No IPE policy files found, ignoring.");
                return 0;
        }
        ...
        STRV_FOREACH(policy, policies) {
                ...
                suffix = endswith(file_name, ".p7b");
                ASSERT_PTR(suffix); /* Filtered by conf_files_list_nulstr() */
