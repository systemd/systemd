...
static int aux_scope_from_message(Manager *m, sd_bus_message *message, Unit **u, sd_bus_error *error) {
        _cleanup_(sd_bus_creds_unrefp) sd_bus_creds *creds = NULL;
        ...
        for (unsigned i = 0; i < n_pids; i++) {
                r = unit_pid_attachable(scope, pids[i], error);
                if (r < 0)
                        return r;

                r = unit_watch_pid(scope, pids[i], false);
                if (r < 0 && r != -EEXIST)
                        return r;
        }
