...
static int send_changed_signal_strv(sd_bus *bus, void *userdata) {
...
void bus_manager_send_change_signal_strv(Manager *m, char **properties) {
        int r;

        assert(m);
        assert(!strv_isempty(properties));

        r = bus_foreach_bus(m, NULL, send_changed_signal_strv, properties);
        if (r < 0)
