...
static int dump_impl(
        ...
        if (patterns)
                r = manager_get_dump_string_by_patterns(m, patterns, &dump);
        else
...
static int method_dump_patterns_by_fd(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        _cleanup_strv_free_ char **patterns = NULL;
        int r;
        ...
        return dump_impl(message, userdata, error, patterns, reply_dump_by_fd);
}
...
const sd_bus_vtable bus_manager_vtable[] = {
        ...
        SD_BUS_METHOD_WITH_ARGS("DumpByFileDescriptor",
                                ...
                                method_dump_by_fd,
                                SD_BUS_VTABLE_UNPRIVILEGED),
        SD_BUS_METHOD_WITH_NAMES("DumpPatternsByFileDescriptor",
                                 "as",
                                 SD_BUS_PARAM(patterns),
                                 "h",
                                 SD_BUS_PARAM(fd),
                                 method_dump_patterns_by_fd,
                                 SD_BUS_VTABLE_UNPRIVILEGED),
