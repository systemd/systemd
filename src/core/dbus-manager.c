...
static int aux_scope_from_message(Manager *m, sd_bus_message *message, Unit **ret_scope, sd_bus_error *error) {
        ...
        for (;;) {
                ...
                if (r < 0) {
                        log_error_errno(r, "Failed to convert PIDFD to PID, ignoring: %m");
                        continue;
                }

                r = cg_pidfd_get_unit(fd, &unit);
                if (r < 0) {
                        log_error_errno(r, "Failed to get unit from PIDFD, ingoring: %m");
                        continue;
                }
                ...
                if (main == q) {
                        log_error("MainPID can not be migrated into auxiliary scope.");
                        continue;
                }
...
const sd_bus_vtable bus_manager_vtable[] = {
        ...
        SD_BUS_METHOD_WITH_ARGS("StartAuxiliaryScope",
                                SD_BUS_ARGS("s", name, "ah", pidfds, "t", flags),
                                SD_BUS_RESULT("o", job),
