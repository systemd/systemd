...
static int dump_impl(
         ...
         * non-destructive/modifying operations, and can cause PID1 to stall. So it seems similar enough in
         * terms of security considerations and impact, and thus use the same access check for dumps which,
         * given the large amount of data to fetch, can stall PID1 for quite some time. */
        r = mac_selinux_access_check(message, "reload", error);
        if (r < 0)
