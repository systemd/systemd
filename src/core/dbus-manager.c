...
static int aux_scope_from_message(Manager *m, sd_bus_message *message, Unit **ret_scope, sd_bus_error *error) {
        ...
        for (;;) {
                ...
                if (r < 0) {
                        log_unit_error_errno(from, r, "Failed to create process reference from PIDFD, ignoring: %m");
                        continue;
                }

                unit = manager_get_unit_by_pidref(m, &p);
                if (!unit) {
                        log_unit_error_errno(from, SYNTHETIC_ERRNO(ENOENT), "Failed to get unit from PIDFD, ingoring: %m");
                        continue;
                }

                if (!streq(unit->id, from->id)) {
                        log_unit_error(from, "PID " PID_FMT " is not running in the same service as the calling process, ignoring.", p.pid);
                        continue;
                }

                if (main_pid->pid == p.pid) {
                        log_unit_error(from, "Main PID cannot be migrated into auxiliary scope, ignoring.");
                        continue;
                }
        ...
        if (r < 0) {
                log_unit_warning_errno(from, r, "Failed to initialize cgroup context of the auxiliary scope, continuing with default context: %m");
        ...
        FOREACH_ARRAY(p, pidrefs, n_pids) {
                ...
                r = unit_watch_pidref(scope, p, false);
                if (r < 0 && r != -EEXIST)
                        return r;
        }
