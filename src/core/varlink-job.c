...
static int job_context_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        Job *j = ASSERT_PTR(userdata);
...
static int job_runtime_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        Job *j = ASSERT_PTR(userdata);
        ...
        return sd_json_buildo(
                        ...
                        SD_JSON_BUILD_PAIR_CALLBACK("ActivationDetails", activation_details_build_json, j));
}

static int list_job_one(sd_varlink *link, Job *job) {
        return sd_varlink_replybo(
                        link,
                        SD_JSON_BUILD_PAIR_CALLBACK("context", job_context_build_json, job),
                        SD_JSON_BUILD_PAIR_CALLBACK("runtime", job_runtime_build_json, job));
}
...
static int lookup_job_by_parameters(
        ...
        if (p->unit) {
                ...
                if (job && u->job != job) {
                        log_debug("Job lookup by parameters id=%u unit='%s' resulted in different jobs.",
                                  p->id, strnull(p->unit));
