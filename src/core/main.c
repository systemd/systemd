...
static usec_t arg_minimum_uptime_sec;
...
static int parse_proc_cmdline_item(const char *key, const char *value, void *data) {
        ...
        if (STR_IN_SET(key, "systemd.unit", "rd.systemd.unit")) {
                ...
                r = parse_sec(value, &arg_minimum_uptime_sec);
                if (r < 0) {
                        log_warning_errno(r, "Failed to parse systemd.minimum_uptime_usec= argument '%s', ignoring: %m", value);
                        return 0;
                }
...
static int parse_config_file(void) {
        const ConfigTableItem items[] = {
                ...
                { "Manager", "MinimumUptimeSec",             config_parse_sec,                   0,                        &arg_minimum_uptime_sec           },
#if ENABLE_SMACK
...
static int become_shutdown(int objective, int retval) {
        ...
        if (arg_minimum_uptime_sec != USEC_INFINITY)
                (void) strv_extendf(&env_block, "MINIMUM_UPTIME_USEC=" USEC_FMT, arg_minimum_uptime_sec);
...
static void reset_arguments(void) {
        ...
        arg_minimum_uptime_sec = USEC_INFINITY;
}
