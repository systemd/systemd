...
static void bump_file_max_and_nr_open(void) {
        ...
        r = sysctl_write("fs/file-max", LONG_MAX_STR, NULL);
        if (r < 0)
        ...
        for (;;) {
                ...
                r = sysctl_writef("fs/nr_open", NULL, "%i", v);
                if (r == -EINVAL) {
...
static int bump_unix_max_dgram_qlen(void) {
        ...
        r = sysctl_write("net/unix/max_dgram_qlen", STRINGIFY(DEFAULT_UNIX_MAX_DGRAM_QLEN), NULL);
        if (r < 0)
