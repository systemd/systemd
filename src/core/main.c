...
static int setup_os_release(RuntimeScope scope) {
        ...
        if (r < 0 && r != -EEXIST)
                return log_error_errno(r, "Failed to create os-release propagate directory, ignoring: %m");

        r = copy_file(os_release_src, os_release_dst, /* open_flags= */ 0, 0644, COPY_MAC_CREATE);
        if (r < 0)
                return log_error_errno(r, "Failed to copy os-release to propagate directory, ignoring: %m");
