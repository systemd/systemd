...
static void timer_enter_waiting(Timer *t, bool time_change) {
        ...
        LIST_FOREACH(value, v, t->values) {
                ...
                if (v->base == TIMER_CALENDAR) {
                         ...
                         * the job based on the last time the trigger
                         * entered inactivity. If that is not set,
                         * just start from when timer exited inactive state
                         * or realtime. */

                        if (t->always_realtime) {
                                if (dual_timestamp_is_set(&trigger->inactive_enter_timestamp))
                                        b = trigger->inactive_enter_timestamp.realtime;
                                else if (dual_timestamp_is_set(&UNIT(t)->inactive_exit_timestamp))
                                        b = UNIT(t)->inactive_exit_timestamp.realtime;
                                else
                                        b = ts.realtime;
                        /* Otherwise, if we know the last time this was
                         * triggered, schedule the job based relative
                         * to that. If we don't, just start from
                         * the activation time or realtime. */
