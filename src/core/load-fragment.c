...
int unit_load_fragment(Unit *u) {
        ...
        if (fragment) {
                ...
                /* No fragment found on disk. Fall back to a built-in copy if we have one embedded for
                 * this unit. This way the manager can reach a usable state even if none of these unit
                 * files are installed. On-disk files always take precedence (including masks), since we
                 * only get here when nothing was found in the lookup paths. */
