...
int config_parse_bpf_foreign_program(
        ...
        if (attach_type < 0) {
                log_syntax(unit, LOG_WARNING, filename, line, r, "Unknown BPF attach type=%s, ignoring: %s", word, rvalue);
                return 0;
        }
        ...
        r = cgroup_add_bpf_foreign_program(c, (uint32_t) attach_type, resolved);
        if (r < 0)
