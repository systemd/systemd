...
DEFINE_CONFIG_PARSE_ENUM(config_parse_collect_mode,
                         collect_mode,
                         CollectMode,
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_exec_keyring_mode,
                         exec_keyring_mode,
                         ExecKeyringMode,
                         "Failed to parse keyring mode");
DEFINE_CONFIG_PARSE_ENUM(config_parse_exec_utmp_mode,
                         exec_utmp_mode,
                         ExecUtmpMode,
                         "Failed to parse utmp mode");
DEFINE_CONFIG_PARSE_ENUM(config_parse_job_mode, job_mode, JobMode, "Failed to parse job mode");
DEFINE_CONFIG_PARSE_ENUM(config_parse_kill_mode, kill_mode, KillMode, "Failed to parse kill mode");
DEFINE_CONFIG_PARSE_ENUM(config_parse_notify_access,
                         notify_access,
                         NotifyAccess,
                         "Failed to parse notify access specifier");
DEFINE_CONFIG_PARSE_ENUM(config_parse_protect_home,
                         protect_home,
                         ProtectHome,
                         "Failed to parse protect home value");
DEFINE_CONFIG_PARSE_ENUM(config_parse_protect_system,
                         protect_system,
                         ProtectSystem,
...
DEFINE_CONFIG_PARSE_ENUM(config_parse_service_type,
                         service_type,
                         ServiceType,
                         "Failed to parse service type");
DEFINE_CONFIG_PARSE_ENUM(config_parse_service_restart,
                         service_restart,
                         ServiceRestart,
...
DEFINE_CONFIG_PARSE_PTR(config_parse_blockio_weight,
                        cg_blkio_weight_parse,
                        uint64_t,
                        "Invalid block IO weight");
        ...
        for (;;) {
                ...
                if (r < 0)
                        log_syntax(unit,
                                   ...
                                   line,
                                   r,
                                   "Failed to add dependency on %s, ignoring: %m",
                                   k);
        }
...
int config_parse_obsolete_unit_deps(const char *unit,
                                    const char *filename,
                                    unsigned line,
                                    const char *section,
                                    unsigned section_line,
...
int config_parse_unit_string_printf(const char *unit,
        ...
        return config_parse_string(
                unit, filename, line, section, section_line, lvalue, ltype, k, data, userdata);
}
...
int config_parse_unit_path_printf(const char *unit,
        ...
        if (strlen(rvalue) >= PATH_MAX) {
                log_syntax(
                        unit, LOG_ERR, filename, line, 0, "%s value too long%s.", lvalue, fatal ? "" : ", ignoring");
...
int config_parse_unit_path_strv_printf(const char *unit,
                                       const char *filename,
                                       unsigned line,
                                       const char *section,
                                       unsigned section_line,
        ...
        if (ltype != SOCKET_SOCKET) {
                ...
                if (socket_address_family(&p->address) != AF_LOCAL && p->address.type == SOCK_SEQPACKET) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   0,
                                   "Address family not supported, ignoring: %s",
                                   rvalue);
                        return 0;
                }
        ...
        if (r < 0) {
                if (r == -ERANGE)
                        log_syntax(unit,
                                   ...
                                   line,
                                   r,
                                   "Nice priority out of range, ignoring: %s",
                                   rvalue);
                else
                        log_syntax(unit,
                                   LOG_ERR,
...
int config_parse_exec(const char *unit,
        ...
        do {
                ...
                r = extract_first_word_and_warn(
                        &p, &firstword, NULL, EXTRACT_QUOTES | EXTRACT_CUNESCAPE, unit, filename, line, rvalue);
                if (r <= 0)
                        return 0;

                f = firstword;
                for (;;) {
                        /* We accept an absolute path as first argument.  If it's prefixed with - and the
                         * path doesn't exist, we ignore it instead of erroring out; if it's prefixed with @,
                         * we allow overriding of argv[0]; if it's prefixed with +, it will be run with full
                         * privileges and no sandboxing; if it's prefixed with '!' we apply sandboxing, but
                         * do not change user/group credentials; if it's prefixed with '!!', then we apply
                         * user/group credentials if the kernel supports ambient capabilities -- if it
                         * doesn't we don't apply the credentials themselves, but do apply most other
                         ...
                         * systemd's UID/GID dropping if the kernel supports ambient creds, but provide an
                         * automatic fallback to privilege dropping within the daemon if the kernel does not
                         * offer that. */
                        ...
                        if (*f == '-' && !(flags & EXEC_COMMAND_IGNORE_FAILURE)) {
                                ...
                                separate_argv0 = true;
                                 ...
                                 !(flags &
                                   (EXEC_COMMAND_FULLY_PRIVILEGED | EXEC_COMMAND_NO_SETUID |
                                ...
                                flags |= EXEC_COMMAND_NO_SETUID;
                                 ...
                                 !(flags & (EXEC_COMMAND_FULLY_PRIVILEGED | EXEC_COMMAND_AMBIENT_MAGIC))) {
                                flags &= ~EXEC_COMMAND_NO_SETUID;
                ...
                while (!isempty(p)) {
                        ...
                        r = extract_first_word_and_warn(
                                &p, &word, NULL, EXTRACT_QUOTES | EXTRACT_CUNESCAPE, unit, filename, line, rvalue);
                        if (r == 0)
                                break;
                        if (r < 0)
                                return ignore ? 0 : -ENOEXEC;
...
int config_parse_exec_input(const char *unit,
        ...
        if (n) {
                ...
                if (isempty(resolved))
                        ...
                        log_syntax(
                                unit, LOG_ERR, filename, line, 0, "Invalid file descriptor name: %s", resolved);
                        return -ENOEXEC;
                }
                ...
                r = path_simplify_and_warn(
                        resolved, PATH_CHECK_ABSOLUTE | PATH_CHECK_FATAL, unit, filename, line, lvalue);
                if (r < 0)
                        return -ENOEXEC;
                ...
                if (ei < 0) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   0,
                                   "Failed to parse input specifier, ignoring: %s",
                                   rvalue);
                        return 0;
                }
        }
...
int config_parse_exec_input_text(const char *unit,
                                 const char *filename,
                                 unsigned line,
                                 const char *section,
                                 unsigned section_line,
        ...
        if (r < 0)
                return log_syntax(
                        unit, LOG_ERR, filename, line, r, "Failed to decode C escaped text '%s': %m", rvalue);
...
int config_parse_exec_input_data(const char *unit,
                                 const char *filename,
                                 unsigned line,
                                 const char *section,
                                 unsigned section_line,
        ...
        if (r < 0)
                return log_syntax(
                        unit, LOG_ERR, filename, line, r, "Failed to decode base64 data, ignoring: %s", rvalue);
...
int config_parse_exec_io_class(const char *unit,
        ...
        if (x < 0) {
                log_syntax(unit,
                           ...
                           line,
                           0,
                           "Failed to parse IO scheduling class, ignoring: %s",
                           rvalue);
                return 0;
        }
...
int config_parse_exec_io_priority(const char *unit,
        ...
        if (r < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, r, "Failed to parse IO priority, ignoring: %s", rvalue);
                return 0;
        }
...
int config_parse_exec_cpu_sched_policy(const char *unit,
        ...
        c->cpu_sched_priority = CLAMP(
                c->cpu_sched_priority, sched_get_priority_min(x), sched_get_priority_max(x));
...
int config_parse_capability_set(const char *unit,
                                const char *filename,
                                unsigned line,
                                const char *section,
                                unsigned section_line,
        ...
        if (r < 0) {
                log_syntax(unit,
                           ...
                           line,
                           r,
                           "Failed to parse %s= specifier '%s', ignoring: %m",
                           lvalue,
                           rvalue);
                return 0;
        }
...
int config_parse_exec_selinux_context(const char *unit,
                                      const char *filename,
                                      unsigned line,
                                      const char *section,
                                      unsigned section_line,
...
int config_parse_exec_apparmor_profile(const char *unit,
                                       const char *filename,
                                       unsigned line,
                                       const char *section,
                                       unsigned section_line,
...
int config_parse_exec_smack_process_label(const char *unit,
                                          const char *filename,
                                          unsigned line,
                                          const char *section,
                                          unsigned section_line,
...
int config_parse_timer(const char *unit,
        ...
        if (b < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, 0, "Failed to parse timer base, ignoring: %s", lvalue);
                return 0;
        }
...
int config_parse_trigger_unit(const char *unit,
                              const char *filename,
                              unsigned line,
                              const char *section,
                              unsigned section_line,
        ...
        if (!hashmap_isempty(u->dependencies[UNIT_TRIGGERS])) {
                log_syntax(unit,
                           ...
                           line,
                           0,
                           "Multiple units to trigger specified, ignoring: %s",
                           rvalue);
                return 0;
        }
        ...
        if (unit_has_name(u, p)) {
                log_syntax(
                        unit, LOG_ERR, filename, line, 0, "Units cannot trigger themselves, ignoring: %s", rvalue);
                return 0;
        }
...
int config_parse_socket_service(const char *unit,
        ...
        if (r < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, r, "Failed to resolve unit specifiers in %s: %m", rvalue);
                return -ENOEXEC;
        }
...
int config_parse_fdname(const char *unit,
                        const char *filename,
                        unsigned line,
                        const char *section,
                        unsigned section_line,
        ...
        for (;;) {
                ...
                if (r < 0) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   r,
                                   "Trailing garbage in sockets, ignoring: %s",
                                   rvalue);
                        break;
                }
                ...
                if (!endswith(k, ".socket")) {
                        log_syntax(
                                unit, LOG_ERR, filename, line, 0, "Unit must be of type socket, ignoring: %s", k);
                        continue;
                }

                r = unit_add_two_dependencies_by_name(
                        UNIT(s), UNIT_WANTS, UNIT_AFTER, k, true, UNIT_DEPENDENCY_FILE);
                if (r < 0)
                        log_syntax(unit,
                                   LOG_ERR,
                                   filename,
...
int config_parse_bus_name(const char *unit,
                          const char *filename,
                          unsigned line,
                          const char *section,
                          unsigned section_line,
...
int config_parse_service_timeout(const char *unit,
        ...
        /* Traditionally, these options accepted 0 to disable the timeouts. However, a timeout of 0 suggests
         * it happens immediately, hence fix this to become USEC_INFINITY instead. This is in-line with how
         * we internally handle all other timeouts. */
        ...
        if (r < 0) {
                log_syntax(unit,
                           ...
                           line,
                           r,
                           "Failed to parse %s= parameter, ignoring: %s",
                           lvalue,
                           rvalue);
                return 0;
        }
...
int config_parse_sec_fix_0(const char *unit,
                           const char *filename,
                           unsigned line,
                           const char *section,
                           unsigned section_line,
         ...
         * compatibility with older versions of systemd where 0 instead of infinity was used as indicator to
         * turn off a timeout. */
...
int config_parse_user_group(const char *unit,
                            const char *filename,
                            unsigned line,
                            const char *section,
                            unsigned section_line,
...
int config_parse_user_group_strv(const char *unit,
                                 const char *filename,
                                 unsigned line,
                                 const char *section,
                                 unsigned section_line,
        ...
        for (;;) {
                ...
                if (!valid_user_group_name_or_id(k)) {
                        log_syntax(
                                unit, LOG_ERR, filename, line, 0, "Invalid user/group name or numeric ID: %s", k);
                        return -ENOEXEC;
                }
...
int config_parse_working_directory(const char *unit,
                                   const char *filename,
                                   unsigned line,
                                   const char *section,
                                   unsigned section_line,
        ...
        if (streq(rvalue, "~")) {
                ...
                r = path_simplify_and_warn(k,
                                           PATH_CHECK_ABSOLUTE | (missing_ok ? 0 : PATH_CHECK_FATAL),
                                           unit,
                                           filename,
                                           line,
                                           lvalue);
        ...
        for (p = rvalue;;) {
                ...
                if (!env_assignment_is_valid(k)) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   0,
                                   "Invalid environment assignment, ignoring: %s",
                                   k);
                        continue;
                }
        ...
        for (;;) {
                ...
                if (r < 0) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   r,
                                   "Trailing garbage in %s, ignoring: %s",
                                   lvalue,
                                   rvalue);
                        break;
                }
                ...
                if (!env_name_is_valid(k)) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   0,
                                   "Invalid environment name for %s, ignoring: %s",
                                   lvalue,
                                   k);
                        continue;
                }
...
int config_parse_unset_environ(const char *unit,
                               const char *filename,
                               unsigned line,
                               const char *section,
                               unsigned section_line,
...
int config_parse_log_extra_fields(const char *unit,
                                  const char *filename,
                                  unsigned line,
                                  const char *section,
                                  unsigned section_line,
        ...
        for (;;) {
                ...
                if (!eq) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   0,
                                   "Log field lacks '=' character, ignoring: %s",
                                   k);
                        continue;
                }
                ...
                if (!journal_field_valid(k, eq - k, false)) {
                        log_syntax(
                                unit, LOG_ERR, filename, line, 0, "Log field name is invalid, ignoring: %s", k);
                        continue;
                }
...
int config_parse_documentation(const char *unit,
        ...
        r = config_parse_unit_strv_printf(
                unit, filename, line, section, section_line, lvalue, ltype, rvalue, data, userdata);
        if (r < 0)
                return r;
        ...
        if (!c->syscall_filter) {
                ...
                if (invert)
                        ...
                        r = seccomp_parse_syscall_filter(
                                "@default", -1, c->syscall_filter, SECCOMP_PARSE_WHITELIST);
                        if (r < 0)
                                return r;
                }
        }
        ...
        for (;;) {
                ...
                if (r < 0) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   r,
                                   "Failed to parse syscall:errno, ignoring: %s",
                                   word);
                        continue;
                }
                ...
                r = seccomp_parse_syscall_filter_full(
                        ...
                        num,
                        c->syscall_filter,
                        SECCOMP_PARSE_LOG | SECCOMP_PARSE_PERMISSIVE | (invert ? SECCOMP_PARSE_INVERT : 0) |
...
int config_parse_syscall_archs(const char *unit,
                               const char *filename,
                               unsigned line,
                               const char *section,
                               unsigned section_line,
...
int config_parse_syscall_errno(const char *unit,
                               const char *filename,
                               unsigned line,
                               const char *section,
                               unsigned section_line,
        ...
        if (e <= 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, 0, "Failed to parse error number, ignoring: %s", rvalue);
                return 0;
        }
...
int config_parse_address_families(const char *unit,
                                  const char *filename,
                                  unsigned line,
                                  const char *section,
                                  unsigned section_line,
        ...
        for (p = rvalue;;) {
                ...
                if (af < 0) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   af,
                                   "Failed to parse address family, ignoring: %s",
                                   word);
                        continue;
                }
...
int config_parse_restrict_namespaces(const char *unit,
                                     const char *filename,
                                     unsigned line,
                                     const char *section,
                                     unsigned section_line,
...
int config_parse_unit_slice(const char *unit,
                            const char *filename,
                            unsigned line,
                            const char *section,
                            unsigned section_line,
        ...
        if (r < 0) {
                log_syntax(unit,
                           ...
                           line,
                           r,
                           "Failed to load slice unit %s, ignoring: %s",
                           k,
                           bus_error_message(&error, r));
                return 0;
        }
...
int config_parse_cpu_quota(const char *unit,
                           const char *filename,
                           unsigned line,
                           const char *section,
                           unsigned section_line,
...
int config_parse_memory_limit(const char *unit,
                              const char *filename,
                              unsigned line,
                              const char *section,
                              unsigned section_line,
        ...
        if (!isempty(rvalue) && !streq(rvalue, "infinity")) {
                ...
                if (r < 0) {
                        ...
                        if (r < 0) {
                                log_syntax(unit,
                                           ...
                                           line,
                                           r,
                                           "Invalid memory limit '%s', ignoring: %m",
                                           rvalue);
                                return 0;
                        }
                } else
                ...
                if (bytes >= UINT64_MAX || (bytes <= 0 && !streq(lvalue, "MemorySwapMax"))) {
                        log_syntax(unit,
                                   LOG_ERR,
                                   filename,
                                   line,
                                   0,
                                   "Memory limit '%s' out of range, ignoring.",
                                   rvalue);
                        return 0;
                }
        }
...
int config_parse_tasks_max(const char *unit,
                           const char *filename,
                           unsigned line,
                           const char *section,
                           unsigned section_line,
        ...
        if (v <= 0 || v >= UINT64_MAX) {
                log_syntax(unit,
                           ...
                           line,
                           0,
                           "Maximum tasks value '%s' out of range, ignoring.",
                           rvalue);
                return 0;
        }
...
int config_parse_delegate(const char *unit,
                          const char *filename,
                          unsigned line,
                          const char *section,
                          unsigned section_line,
         ...
         * controllers to the mask to delegate. */
                ...
                for (;;) {
                        ...
                        if (cc < 0) {
                                log_syntax(unit,
                                           ...
                                           line,
                                           r,
                                           "Invalid controller name '%s', ignoring",
                                           word);
                                continue;
                        }
...
int config_parse_device_allow(const char *unit,
                              const char *filename,
                              unsigned line,
                              const char *section,
                              unsigned section_line,
        ...
        if (!STARTSWITH_SET(resolved, "block-", "char-")) {
                ...
                if (!valid_device_node_path(resolved)) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   0,
                                   "Invalid device node path '%s', ignoring.",
                                   resolved);
                        return 0;
                }
        }
...
int config_parse_io_device_weight(const char *unit,
                                  const char *filename,
                                  unsigned line,
                                  const char *section,
                                  unsigned section_line,
...
int config_parse_io_device_latency(const char *unit,
                                   const char *filename,
                                   unsigned line,
                                   const char *section,
                                   unsigned section_line,
...
int config_parse_io_limit(const char *unit,
                          const char *filename,
                          unsigned line,
                          const char *section,
                          unsigned section_line,
...
int config_parse_blockio_device_weight(const char *unit,
                                       const char *filename,
                                       unsigned line,
                                       const char *section,
                                       unsigned section_line,
...
int config_parse_blockio_bandwidth(const char *unit,
                                   const char *filename,
                                   unsigned line,
                                   const char *section,
                                   unsigned section_line,
...
int config_parse_job_mode_isolate(const char *unit,
                                  const char *filename,
                                  unsigned line,
                                  const char *section,
                                  unsigned section_line,
...
int config_parse_exec_directories(const char *unit,
                                  const char *filename,
                                  unsigned line,
                                  const char *section,
                                  unsigned section_line,
        ...
        for (p = rvalue;;) {
                ...
                if (path_startswith(k, "private")) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   0,
                                   "%s= path can't be 'private', ingoring assignment: %s",
                                   lvalue,
                                   word);
                        continue;
                }
                ...
                if (r < 0) {
                        ...
                        if (val <= 0) {
                                log_syntax(unit,
                                           ...
                                           line,
                                           0,
                                           "Failed to parse value, ignoring: %s",
                                           word);
                                continue;
                        }
                        ...
                        if (val < 0 || val > 255) {
                                log_syntax(unit,
                                           ...
                                           line,
                                           0,
                                           "Value %d is outside range 0-255, ignoring",
                                           val);
                                continue;
                        }
...
int config_parse_namespace_path_strv(const char *unit,
                                     const char *filename,
                                     unsigned line,
                                     const char *section,
                                     unsigned section_line,
        ...
        for (;;) {
                ...
                if (r < 0) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   r,
                                   "Failed to extract first word, ignoring: %s",
                                   rvalue);
                        return 0;
                }
                ...
                joined = strjoin(ignore_enoent ? "-" : "", shall_prefix ? "+" : "", resolved);
...
int config_parse_temporary_filesystems(const char *unit,
                                       const char *filename,
                                       unsigned line,
                                       const char *section,
                                       unsigned section_line,
        ...
        for (;;) {
                ...
                r = temporary_filesystem_add(
                        &c->temporary_filesystems, &c->n_temporary_filesystems, resolved, w);
                if (r < 0)
                        return log_oom();
        }
}

int config_parse_bind_paths(const char *unit,
                            const char *filename,
                            unsigned line,
                            const char *section,
                            unsigned section_line,
        ...
        for (;;) {
                ...
                r = extract_first_word(
                        &p, &source, ":" WHITESPACE, EXTRACT_QUOTES | EXTRACT_DONT_COALESCE_SEPARATORS);
                if (r == 0)
                        break;
                if (r == -ENOMEM)
                        return log_oom();
                ...
                if (p && p[-1] == ':') {
                        r = extract_first_word(&p,
                                               &destination,
                                               ":" WHITESPACE,
                                               EXTRACT_QUOTES | EXTRACT_DONT_COALESCE_SEPARATORS);
                        if (r == -ENOMEM)
                                return log_oom();
                        if (r < 0) {
                                log_syntax(unit,
                        ...
                        if (r == 0) {
                                log_syntax(unit,
                                           ...
                                           line,
                                           0,
                                           "Missing argument after ':', ignoring: %s",
                                           s);
                                continue;
                        }
                        ...
                        r = path_simplify_and_warn(
                                dresolved, PATH_CHECK_ABSOLUTE, unit, filename, line, lvalue);
                        if (r < 0)
                                continue;
...
int config_parse_job_timeout_sec(const char *unit,
                                 const char *filename,
                                 unsigned line,
                                 const char *section,
                                 unsigned section_line,
        ...
        /* If the user explicitly changed JobTimeoutSec= also change JobRunningTimeoutSec=, for compatibility
         * with old versions. If JobRunningTimeoutSec= was explicitly set, avoid this however as whatever the
         * user picked should count. */
...
int config_parse_job_running_timeout_sec(const char *unit,
                                         const char *filename,
                                         unsigned line,
                                         const char *section,
                                         unsigned section_line,
...
int config_parse_emergency_action(const char *unit,
                                  const char *filename,
                                  unsigned line,
                                  const char *section,
                                  unsigned section_line,
        ...
        if (r < 0) {
                ...
                if (r == -EOPNOTSUPP)
                        log_syntax(unit,
                                   ...
                                   lvalue,
                                   MANAGER_IS_SYSTEM(m) ? "user" : "system",
                                   rvalue);
                else
                        log_syntax(unit,
                                   LOG_ERR,
...
int config_parse_pid_file(const char *unit,
                          const char *filename,
                          unsigned line,
                          const char *section,
                          unsigned section_line,
        ...
        if (e) {
                ...
                log_syntax(unit,
                           ...
                           line,
                           0,
                           "PIDFile= references path below legacy directory /var/run/, updating %s → %s; please update the unit file accordingly.",
                           n,
                           z);
...
int config_parse_exit_status(const char *unit,
                             const char *filename,
                             unsigned line,
                             const char *section,
                             unsigned section_line,
        ...
        if (r < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, r, "Failed to parse exit status '%s', ignoring: %m", rvalue);
                return 0;
        }
...
int config_parse_disable_controllers(const char *unit,
                                     const char *filename,
                                     unsigned line,
                                     const char *section,
                                     unsigned section_line,
...
static int load_from_path(Unit *u, const char *path) {
        ...
        if (path_is_absolute(path)) {
                ...
                STRV_FOREACH (p, u->manager->lookup_paths.search_path) {
                        ...
                        if (u->manager->unit_path_cache && !set_get(u->manager->unit_path_cache, filename))
                                r = -ENOENT;
                        else
                                r = open_follow(&filename, &f, symlink_names, &id);
                        if (r >= 0)
...
int unit_load_fragment(Unit *u) {
        ...
        if (u->load_state == UNIT_STUB && u->instance) {
                ...
                if (r < 0) {
                        if (r == -ENOEXEC)
                                log_unit_notice(
                                        u, "Unit configuration has fatal error, unit will not be started.");
                        return r;
                }
