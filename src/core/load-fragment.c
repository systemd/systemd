...
int config_parse_exec(
        ...
        do {
                ...
                for (;; f++) {
                        ...
                        if (*f == '-' && !(flags & EXEC_COMMAND_IGNORE_FAILURE))
                        ...
                        else if (*f == '!' && !(flags & (EXEC_COMMAND_FULLY_PRIVILEGED) && !ambient_hack)) {
                                ...
                                log_syntax(unit, LOG_NOTICE, filename, line, 0, "!! modifier for %s= lines is no longer supported and is now ignored. Please update your unit files and remove the modifier.", lvalue);
                        } else
                                break;
                }
