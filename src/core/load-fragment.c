...
int config_parse_namespace_flags(
        ...
        if (r > 0) {
                /* RestrictNamespaces= value gets stored into a reverse field retain_namespaces, so
                 * RestrictNamespaces=true means we retain no access to any namespaces and vice-versa. */
