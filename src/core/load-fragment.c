...
int config_parse_managed_oom_mem_pressure_duration_sec(
        ...
        if (usec < 1 * USEC_PER_SEC) {
                log_syntax(unit, LOG_WARNING, filename, line, 0, "%s= must be at least 1s, ignoring: %s", lvalue, rvalue);
                return 0;
        }

        if (usec == USEC_INFINITY) {
                log_syntax(unit, LOG_WARNING, filename, line, 0, "%s= must be less than infinity, ignoring: %s", lvalue, rvalue);
                return 0;
        }
