...
int config_parse_exec_directories(
        ...
        for (const char *p = rvalue;;) {
                ...
                if (!isempty(dest)) {
                        ...
                        if (!STR_IN_SET(lvalue, "StateDirectory", "RuntimeDirectory")) {
                                log_syntax(unit, LOG_WARNING, filename, line, 0,
                                           "Destination parameter is only supported for StateDirectory and RuntimeDirectory, ignoring: %s", tuple);
                                continue;
                        }
                ...
                r = exec_directory_add(&ed->items, &ed->n_items,
                                       &(ExecDirectoryItem) {
                                            .path = sresolved,
                                            .symlinks = symlinks,
                                       });
