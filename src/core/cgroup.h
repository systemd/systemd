...
typedef struct CGroupContext {
        ...
        /* Pressure logic */
        CGroupPressureWatch pressure_watch[_CGROUP_PRESSURE_RESOURCE_MAX];
        usec_t pressure_threshold_usec[_CGROUP_PRESSURE_RESOURCE_MAX];
...
static inline bool cgroup_context_want_pressure(const CGroupContext *c, CGroupPressureResource t) {
        assert(c);
        assert(t >= 0 && t < _CGROUP_PRESSURE_RESOURCE_MAX);

        if (c->pressure_watch[t] == CGROUP_PRESSURE_WATCH_YES)
                return true;

        if (c->pressure_watch[t] != CGROUP_PRESSURE_WATCH_AUTO)
                return false;

        switch (t) {

        case CGROUP_PRESSURE_MEMORY:
                return c->memory_accounting;

        case CGROUP_PRESSURE_CPU:
...
DECLARE_STRING_TABLE_LOOKUP(cgroup_pressure_resource, CGroupPressureResource);
