...
static int manager_deserialize_one_unit(
        ...
        if (!streq(u->id, name) &&
            ...
            set_contains(serialized_units, u->id)) {
                 ...
                 *
                 * This means the unit in the state file is now a stale alias that wasn't an alias when the
                 * state was serialized. Deserializing it would overwrite the canonical unit's own serialized
                 ...
                 * manager_serialize(), so they never appear in the state file.
                 *
