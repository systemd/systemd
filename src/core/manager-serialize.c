...
static int manager_synthesize_orphaned_unit(
        ...
        _cleanup_free_ char *source_path = strdup(original_name);
        if (!source_path)
                return log_oom();
        _cleanup_free_ char *description = strdup("Orphaned resources adopted from aliased unit");
        if (!description)
                return log_oom();
        ...
        free_and_replace(orphan->source_path, source_path);
