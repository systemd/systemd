...
int manager_serialize(
        ...
        if (m->restrict_exec > RESTRICT_EXEC_NO) {
...
static void deserialize_restrict_exec(Manager *m, const char *l, FDSet *fds) {
        ...
        for (size_t i = 0; i < ELEMENTSOF(restrict_exec_link_names); i++) {
                val = startswith(l, restrict_exec_link_names[i]);
                if (!val)
                        continue;
                val = startswith(val, "=");
                if (!val)
                ...
                if (fd >= 0)
                        close_and_replace(m->restrict_exec_link_fds[i], fd);
                return;
        }
