...
static ManagerTimestamp map_timestamp_serialization(ManagerObjective previous_objective, ManagerTimestamp t) {
        ...
        if (previous_objective != MANAGER_SOFT_REBOOT)
                return t; /* Reload/reexec/switch-root: the timestamps still describe the current boot. */

        /* Across a soft-reboot the old instance's shutdown timestamps describe the boot that is going away,
         * so move them into the previous-shutdown-* fields. The phases the new instance runs through again
         * are dropped so they get re-measured; the firmware/loader/kernel/initrd ones are kept. */
        switch (t) {
        ...
        case MANAGER_TIMESTAMP_USERSPACE:
        case MANAGER_TIMESTAMP_FINISH:
        case MANAGER_TIMESTAMP_SECURITY_START:
        case MANAGER_TIMESTAMP_SECURITY_FINISH:
        case MANAGER_TIMESTAMP_GENERATORS_START:
        case MANAGER_TIMESTAMP_GENERATORS_FINISH:
        case MANAGER_TIMESTAMP_UNITS_LOAD_START:
        case MANAGER_TIMESTAMP_UNITS_LOAD_FINISH:
