...
void lock_down_efi_variables(void) {
        ...
        if (r < 0)
                log_warning_errno(r, "Failed to drop FS_IMMUTABLE_FL from EFI system token, ignoring: %m");
        if (fchmod(fd, 0600) < 0)
                log_warning_errno(errno, "Failed to reduce access mode of EFI system token, ignoring: %m");
}
