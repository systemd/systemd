...
static void exec_fuzz_one(FILE *f, FDSet *fdset, int **fds_array, size_t *n_fds_array) {
        ...
        (void) exec_deserialize_invocation(f, fdset, *fds_array, *n_fds_array, &exec_context, &command, &params, &runtime, &cgroup_context);
        (void) exec_serialize_invocation(f, fdset, fds_array, n_fds_array, &exec_context, &command, &params, &runtime, &cgroup_context);
