...
static void exec_fuzz_one(FILE *f, FDSet *fdset, bool store_index) {
        ...
        (void) exec_deserialize_invocation(f, fdset, &context, &command, &params, &runtime, &cgroup_context);
        ...
        for (size_t i = 0; params.fds && i < params.n_socket_fds + params.n_storage_fds; i++)
                params.fds[i] = -EBADF;
        shared.netns_storage_socket[0] = -EBADF;
        shared.netns_storage_socket[1] = -EBADF;
        shared.ipcns_storage_socket[0] = -EBADF;
        shared.ipcns_storage_socket[1] = -EBADF;
