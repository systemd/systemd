...
static bool bpf_can_link_lsm_program(struct bpf_program *prog) {
        ...
        /* If bpf_program__attach_lsm fails the resulting value stores libbpf
         * error code instead of memory pointer. That is the case when the
         * helper is called on architectures where BPF trampoline (hence
