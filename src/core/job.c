...
static void job_emit_done_message(Unit *u, uint32_t job_id, JobType t, JobResult result) {
        ...
        if (!console_only) {  /* Skip printing if output goes to the console, and job_print_status_message()
                ...
                c = t == JOB_START && result == JOB_DONE && !u->condition_result ?
                        unit_find_failed_condition(u) : NULL;
                if (c)
                        ...
                        if (c->trigger)
                                log_unit_struct(
                                        ...
                                        "MESSAGE=%s was skipped because all trigger conditions failed.",
                                        ident,
                                        "JOB_ID=%" PRIu32, job_id,
                                        "JOB_TYPE=%s", job_type_to_string(t),
                                ...
                                log_unit_struct(
                                        u,
                                        job_done_messages[result].log_level,
                                        "MESSAGE=%s was skipped because of a failed condition check (%s=%s%s%s).",
                                        ident,
