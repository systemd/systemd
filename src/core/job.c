...
int job_finish_and_invalidate(Job *j, JobResult result, bool recursive, bool already) {
        ...
        else if (result == JOB_DONE && u->activation_event_info == NULL && IN_SET(t, JOB_START, JOB_RESTART))
                u->activation_event_info = TAKE_PTR(j->activation_event_info); /* Older event wins */
        else if (result == JOB_DONE && t == JOB_STOP)
                u->activation_event_info = activation_event_info_unref(u->activation_event_info);
