...
static char* job_done_message_format(Unit *u, JobType t, JobResult result) {
        ...
        switch (t) {
        case JOB_START:
                return strdup(generic_finished_start_job[result]);
        case JOB_STOP:
        case JOB_RESTART:
                return strdup(generic_finished_stop_job[result]);
        case JOB_RELOAD:
                return strdup(generic_finished_reload_job[result]);
        case JOB_VERIFY_ACTIVE:
                return strdup(generic_finished_verify_active_job[result]);
        default:
                return NULL;
        }
}
