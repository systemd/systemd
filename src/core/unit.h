...
#define log_unit_full_errno_zerook(unit, level, error, ...)             \
        ({                                                              \
                const Unit *_u = (unit);                                \
                (log_get_max_level() < LOG_PRI(level) || (_u && !unit_log_level_test(_u, level))) ? -ERRNO_VALUE(error) : \
...
#define log_unit_struct_errno(unit, level, error, ...)                  \
        ({                                                              \
                const Unit *_u = (unit);                                \
                unit_log_level_test(_u, level) ?                        \
                        log_struct_errno(level, error, __VA_ARGS__, LOG_UNIT_ID(_u), LOG_UNIT_INVOCATION_ID(_u)) : \
                        -ERRNO_VALUE(error);                            \
        })
