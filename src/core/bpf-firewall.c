...
int bpf_firewall_install(Unit *u) {
        ...
        if (FLAGS_SET(flags, BPF_F_ALLOW_MULTI)) {
                ...
                /* If we don't have BPF_F_ALLOW_MULTI then unref the old BPF program (which will implicitly
                 * detach it) right before attaching the new program, to minimize the time window when we
