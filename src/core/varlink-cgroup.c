...
static int memory_accounting_metric_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        ...
        if (metric < 0)
                return log_debug_errno(type, "Failed to get CGroup memory accounting metric type for '%s': %m", name);
        ...
        return sd_json_buildo(ret, SD_JSON_BUILD_UNSIGNED(value));
}
...
static int effective_limit_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        ...
        if (type < 0)
                return log_debug_errno(type, "Failed to get CGroup effective limit type for '%s': %m", name);
...
static int cpu_usage_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        ...
        if (value == NSEC_INFINITY)
                return 0;

        return sd_json_buildo(ret, SD_JSON_BUILD_UNSIGNED(ns)); // TODO
}
...
static int tasks_current_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        ...
        return sd_json_buildo(ret, SD_JSON_BUILD_UNSIGNED(cn));
}
...
static int get_ip_counter_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        ...
        if (metric < 0)
                return log_debug_errno(metric, "Failed to get CGroup IP accounting metric type for '%s': %m", name);
...
static int get_io_counter_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        ...
        if (metric < 0)
                return log_debug_errno(metric, "Failed to get CGroup IO accounting metric type for '%s': %m", name);
...
int unit_cgroup_runtime_build_json(sd_json_variant **ret, const char *name, void *userdata) {
    Unit *u = ASSERT_PTR(userdata); /* unit is necessary for Slice field */
    ...
    return sd_json_buildo(
            ...
            /* CPU */
            //JSON_BUILD_PAIR_CALLBACK_NON_NULL("CPUUsageNSec", cpu_usage_build_json, u), TODO
