...
static int build_managed_oom_json_array_element(Unit *u, const char *property, sd_json_variant **ret_v) {
        bool use_limit_and_duration = false;
        CGroupContext *c;
        const char *mode;
        ...
        return sd_json_buildo(ret_v,
                              ...
                              SD_JSON_BUILD_PAIR_CONDITION(use_limit_and_duration, "limit", SD_JSON_BUILD_UNSIGNED(c->moom_mem_pressure_limit)),
                              SD_JSON_BUILD_PAIR_CONDITION(use_limit_and_duration, "duration", SD_JSON_BUILD_UNSIGNED(c->moom_mem_pressure_duration_usec)));
}
