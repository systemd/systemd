...
int exec_invoke(
        ...
        /* Only adjust ownership for TTYs we acquired via StandardInput=tty*. If stdin is passed as an
         * fd, its ownership is managed by the provider of the fd, see exec_context_revert_tty(). */
        if (uid_is_valid(uid) &&
