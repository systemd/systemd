...
static int set_exec_storage_quota(int fd, uint32_t proj_id, const QuotaLimit *ql) {
        ...
        if (ql->quota_absolute == 0 || ql->quota_scale == 0)
                ...
                int r;
                _cleanup_close_ int fd_rel = -EBADF;
                ...
                fd_rel = openat(fd, "..", O_PATH|O_CLOEXEC|O_DIRECTORY);
                if (fd_rel < 0)
                        return -errno;

                unsigned xattr_flags = 0;
                r = read_xattr_flags_fd(fd_rel, &xattr_flags);
                if (r < 0)
                        return r;
