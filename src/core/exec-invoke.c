...
int exec_invoke(
        ...
        size_t n_fds, /* fds to pass to the child */
               n_image_fds = 0, /* fds of pre-loaded images */
        ...
        for (PartitionDesignator i = 0; i < _PARTITION_DESIGNATOR_MAX; i++)
                if (context->root_image_fsmount_fds[i] >= 0)
                        ++n_image_fds;
        ...
        int keep_fds[n_fds + n_image_fds + 4];
        ...
        for (PartitionDesignator i = 0; i < _PARTITION_DESIGNATOR_MAX; i++) {
                r = add_shifted_fd(keep_fds, ELEMENTSOF(keep_fds), &n_keep_fds, &context->root_image_fsmount_fds[i]);
                if (r < 0) {
                        *exit_status = EXIT_FDS;
                        return log_error_errno(r, "Failed to collect shifted fd: %m");
                }
        ...
        FOREACH_ARRAY(mount, context->extension_images, context->n_extension_images)
                for (PartitionDesignator i = 0; i < _PARTITION_DESIGNATOR_MAX; i++) {
                        r = add_shifted_fd(keep_fds, ELEMENTSOF(keep_fds), &n_keep_fds, &mount->fsmount_fds[i]);
                        if (r < 0) {
                                *exit_status = EXIT_FDS;
                                return log_error_errno(r, "Failed to collect shifted fd: %m");
                        }
