...
static int build_environment(
        ...
        for (CGroupPressureResource t = 0; t < _CGROUP_PRESSURE_RESOURCE_MAX; t++) {
                ...
                _cleanup_free_ char *upper = strdup(cgroup_pressure_resource_to_string(t));
                if (!upper)
                        return -ENOMEM;

                ascii_strupper(upper);

                _cleanup_free_ char *env_watch = strjoin(upper, "_PRESSURE_WATCH=");
                if (!env_watch)
                        return -ENOMEM;
                ...
                if (!path_equal(pressure_path[t], "/dev/null")) {
                        ...
                        if (asprintf(&b, "%s " USEC_FMT " " USEC_FMT,
                                     ...
                                     cgroup_context->pressure_threshold_usec[t] == USEC_INFINITY ? PRESSURE_DEFAULT_THRESHOLD_USEC :
                                     CLAMP(cgroup_context->pressure_threshold_usec[t], 1U, PRESSURE_DEFAULT_WINDOW_USEC),
                        ...
                        _cleanup_free_ char *env_write = strjoin(upper, "_PRESSURE_WRITE=");
                        if (!env_write)
                                return -ENOMEM;
...
int exec_invoke(
        ...
        _cleanup_free_ char *pressure_path[_CGROUP_PRESSURE_RESOURCE_MAX] = {};
                ...
                if (is_pressure_supported() > 0) {
                        for (CGroupPressureResource t = 0; t < _CGROUP_PRESSURE_RESOURCE_MAX; t++) {
                                if (cgroup_context_want_pressure(cgroup_context, t)) {
                                        _cleanup_free_ char *pressure_file = strjoin(cgroup_pressure_resource_to_string(t), ".pressure");
                                        if (!pressure_file) {
                                                *exit_status = EXIT_MEMORY;
                                                return log_oom();
                                        }
                                ...
                                } else if (cgroup_context->pressure_watch[t] == CGROUP_PRESSURE_WATCH_NO) {
