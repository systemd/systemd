...
_noreturn_ static void bpffs_helper(int parent_fd,
        ...
        if (delegate_cmds != ~0ULL) {
                sprintf(number, "0x%lx", delegate_cmds);
                s = number;
        } else
                s = "any";

        r = fsconfig(fs_fd, FSCONFIG_SET_STRING, "delegate_cmds", s, 0);
        if (r < 0)
                _exit(EXIT_FAILURE);
...
int exec_invoke(
        ...
        if (context->private_bpf) {
                ...
                r = safe_fork("(bpffs)", FORK_RESET_SIGNALS|FORK_DEATHSIG_SIGKILL, &bpftoken_pid);
                if (r < 0)
                        return r;
                if (r == 0)
        ...
        if (context->private_bpf) {
                ...
                r = wait_for_terminate_and_check("(bpffs)", TAKE_PID(bpftoken_pid), 0);
                if (r < 0)
                        return r;
                if (r != EXIT_SUCCESS) /* If something strange happened with the child, let's consider this fatal, too */
                        return -EIO;
