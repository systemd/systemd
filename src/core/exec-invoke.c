...
static int apply_console_backing_term(char ***env, const char *tty_path) {
        ...
        /* For a service on /dev/console, prefer an explicit per-tty setting over inheriting the terminal
         * type from PID 1. The precedence, highest first, is: a global TERM= on the kernel command line,
         * then systemd.tty.term.<backing-device>= for the device backing the console (e.g. ttyS0), then a
         * generic systemd.tty.term.console=. The backing-device key is the more specific one and wins over
         * the console key: this lets an interactive console service use a richer terminal type than PID 1
         * does, e.g. console=dumb for a fast boot (PID 1 skips the ANSI reset) while the login shell on
         * ttyS0 still gets TERM=linux. PID 1 and its console services may therefore intentionally differ.
         * We skip all of this outside /dev/console and inside containers, where the container manager owns
         * $TERM and /proc/ and /sys/ refer to the host rather than the payload. Returns 1 if $TERM was set,
         * 0 otherwise, negative on OOM. */
        ...
        /* The device backing the console (e.g. ttyS0) wins over the generic console key. */
        _cleanup_free_ char *resolved = NULL;
        r = resolve_dev_console(&resolved);
        if (r < 0)
                log_debug_errno(r, "Failed to resolve /dev/console, ignoring: %m");
        ...
        /* Otherwise fall back to an explicit systemd.tty.term.console=. */
        _cleanup_free_ char *console_term = NULL;
        r = proc_cmdline_tty_term(tty_path, &console_term);
        if (r < 0)
                return r;
        if (r > 0)
                return strv_env_assign(env, "TERM", console_term);
