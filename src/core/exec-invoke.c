...
static int can_mount_proc(const ExecContext *c, ExecParameters *p) {
         ...
         * where unprivileged user namespaces cannot mount a less restrictive instance of /proc. However, if a manager
         * has CAP_SYS_ADMIN, we can skip this check.
         */
        r = have_effective_cap(CAP_SYS_ADMIN);
        if (r < 0)
                return log_exec_debug_errno(c, p, r, "Failed to check if we have enough privileges for mounting proc: %m");
        if (r > 0)
                return 1;
