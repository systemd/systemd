...
static int setup_pam(
        ...
        _cleanup_close_ int parent_pidfd = -EBADF;
        ...
        parent_pidfd = pidfd_open(getpid_cached(), 0);
        if (parent_pidfd < 0) {
                r = -errno;
                goto fail;
        }
        ...
        if (r == 0) {
                ...
                for (;;) {
                        r = fd_wait_for_event(parent_pidfd, POLLIN, USEC_INFINITY);
                        if (r == -EINTR)
                                continue;
                        if (r < 0) {
        ...
        parent_pidfd = safe_close(parent_pidfd);
