...
static int bpffs_prepare(
        ...
        if (r == 0) {
                ...
                if (fs_fd < 0) {
                        log_error_errno(fs_fd, "Failed to receive_one_fd from parent: %m");
                        _exit(EXIT_FAILURE);
                }

                r = fsconfig(fs_fd, FSCONFIG_SET_STRING, "delegate_cmds", BPF_DELEGATE_TO_STRING(delegate_cmds), 0);
                if (r < 0) {
                        log_error_errno(r, "Failed to FSCONFIG_SET_STRING: %m");
                        _exit(EXIT_FAILURE);
                }
                ...
                if (mnt_fd < 0) {
                        log_error_errno(errno, "Failed to fsmount: %m");
                        _exit(EXIT_FAILURE);
                }

                r = send_one_fd(pipe_fds[0], mnt_fd, 0);
                if (r < 0) {
                        log_error_errno(r, "Failed to send_one_fd to child: %m");
                        _exit(EXIT_FAILURE);
                }
...
static int bpffs_finalize(int pipe_fd, PidRef *pid) {
        ...
        fs_fd = fsopen("bpf", 0);
        if (fs_fd < 0)
                return log_error_errno(errno, "Failed to fsopen: %m");

        r = send_one_fd(pipe_fd, fs_fd, /* flags = */ 0);
        if (r < 0)
                return log_error_errno(r, "Failed to send_one_fd to child: %m");

        r = pidref_wait_for_terminate_and_check("(sd-bpffs)", pid, /* flags = */ 0);
        if (r < 0)
                return log_error_errno(r, "Failed to pidref_wait_for_terminate_and_check: %m");
        if (r != EXIT_SUCCESS) {
                /* If something strange happened with the child, let's consider this fatal, too */
                log_error("Bogus return code from child: %d", r);
                return -EIO;
        }

        mnt_fd = receive_one_fd(pipe_fd, /* flags = */ 0);
        if (mnt_fd < 0)
                return log_error_errno(mnt_fd, "Failed to receive_one_fd from child: %m");

        r = move_mount(mnt_fd, "", AT_FDCWD, "/sys/fs/bpf", MOVE_MOUNT_F_EMPTY_PATH);
        if (r < 0)
                return log_error_errno(errno, "Failed to move_mount: %m");
