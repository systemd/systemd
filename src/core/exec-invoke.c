...
int exec_invoke(
        ...
        /* As documented in systemd.exec(5), UnsetEnvironment= takes effect as the final step when the
         * environment block passed to the executed process is compiled, i.e. after the command line has
         * been expanded. Hence, the variables it removes are still available for expansion. */
