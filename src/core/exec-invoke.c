...
static int set_exec_storage_quota(int fd, uint32_t proj_id, const QuotaLimit *ql) {
        ...
        if (quotactl_fd(fd, QCMD_FIXED(Q_SETQUOTA, PRJQUOTA), proj_id, &req) < 0)
                return -errno;
...
static int apply_exec_quotas(const char *target_dir, const char *cgroup_path, ExecDirectoryType type, const QuotaLimit *ql, uint32_t *exec_dt_proj_id, bool *already_enforced) {
        _cleanup_close_ int fd = -EBADF;
        ...
        if (ERRNO_IS_NEG_IOCTL_NOT_SUPPORTED(r)) {
                log_debug("Not applying storage quotas. Ioctl not supported for %s", target_dir);
                return 0;
        }
        if (r < 0)
        ...
        /* Check if enforcement should be disabled */
        if (!ql->quota_enforce) {
                if (proj_id_exists) {
        ...
        /* Check if projec ID has already been generated for the exec directory */
        ...
        if (proj_id > 0) {
                ...
                proj_id = PROJ_ID_CLAMP_INTO_QUOTA_RANGE(siphash24(proj_id_plain, strlen(proj_id_plain), k.bytes));
...
#define MAX_PROJ_ID_RETRIES 10
                for (unsigned attempt = 0;; attempt++) {
                        ...
                        if (!FLAGS_SET(req.dqb_valid, QIF_BLIMITS) || req.dqb_bhardlimit <= 0) {
                                int set_proj_id = quota_proj_id_set_recursive(fd, proj_id, true);
                                if (set_proj_id < 0)
                                        return log_debug_errno(set_proj_id, "Exec quotas: Failed to set project ID for %s: %m", target_dir);
                                if (set_proj_id) {
                                        *exec_dt_proj_id = proj_id;
                                        log_debug("Exec quotas: Project ID %u generated for %s", proj_id, target_dir);
                                        break;
                                }
                        }
        ...
        if (ql->quota_enforce && !(*already_enforced)) {
                ...
                /* Use target_dir's mountpoint when setting quotas. If a FD for target_dir has been previously
                 ...
                 * associated with the mountpoint, so that total blocks is accurate */
                _cleanup_free_ char *chased_target, *mountpoint = NULL;
                r = chase(target_dir, NULL, CHASE_STEP|CHASE_NONEXISTENT, &chased_target, NULL);
                if (r < 0)
                        return log_debug_errno(fd, "Exec quotas: Failed to chase %s: %m", target_dir);

                fd = open(find_mountpoint(chased_target), O_PATH, O_CLOEXEC);
                if (fd < 0)
                        return log_debug_errno(fd, "Exec quotas: Failed to find mountpoint for %s: %m", target_dir);

                r = set_exec_storage_quota(fd, proj_id, ql);
                if (r < 0)
                        return log_debug_errno(r, "Exec quotas: Failed to set storage quotas for %s: %m", target_dir);
...
static int setup_exec_directory(
        ...
        FOREACH_ARRAY(i, context->directories[type].items, context->directories[type].n_items) {
                ...
                r = apply_exec_quotas(target_dir, params->cgroup_path, type, ql, &exec_dt_proj_id, &quota_already_enforced);
                if (r < 0) {
                        log_debug_errno(r, "Unable to set storage limits for %s: %m", target_dir);
                        goto fail;
                }
        }
