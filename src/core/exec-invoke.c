...
static int setup_private_users(
                ...
                size_t n_self_supplementary_gids,
                        ...
                        if (n_effective_gids < 0) {
                                *exit_status = EXIT_GROUP;
                                return log_error_errno(
                                                n_effective_gids,
                                                "Failed to obtain effective supplementary groups: %m");
                        }
                }

                if (needs_setuid) {
                        _cleanup_free_ gid_t *enforced_and_effective_gids = NULL;

                        int n_enforced_and_effective_gids = merge_gid_lists(
                                        gids_to_enforce,
                                        ngids_to_enforce,
                                        effective_gids,
                                        n_effective_gids,
                                        &enforced_and_effective_gids);
                        if (n_enforced_and_effective_gids < 0) {
                                *exit_status = EXIT_GROUP;
                                return log_error_errno(
                                                n_enforced_and_effective_gids,
                                                "Failed to merge groups to map: %m");
                        }
