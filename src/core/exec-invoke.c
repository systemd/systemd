...
static int can_mount_proc(const ExecContext *c, ExecParameters *p) {
         ...
         *
         * While it would be convenient to check if we have effective capability CAP_SYS_ADMIN and return early if so,
         * we can't do that because the kernel checks if we have CAP_SYS_ADMIN within the mount namespace corresponding
         * to the user namespace. If we're running unprivileged or in a system without CAP_SYS_ADMIN, then we can have
         * CAP_SYS_ADMIN in the current user namespace but not once we unshare a mount namespace. */
...
int exec_invoke(
        ...
        if (needs_sandboxing && exec_needs_pid_namespace(context)) {
                ...
                if (r > 0) {
                        ...
                        *exit_status = EXIT_NAMESPACE;
                        return log_exec_error_errno(context, params, SYNTHETIC_ERRNO(EPERM), "/proc/ cannot be re-mounted due to lack of privileges");
                } else {
                        *exit_status = EXIT_NAMESPACE;
