...
static int apply_exec_quotas(
                ...
                const QuotaLimit *ql,
                uint32_t *exec_dt_proj_id,
                bool *already_enforced) {
        _cleanup_close_ int fd = -EBADF;
...
#define MAX_PROJ_ID_RETRIES 10
                for (unsigned attempt = 0;; attempt++) {
                        ...
                        if (!quota_dqblk_is_populated(&req)) {
                                ...
                                if (set_proj_id < 0)
                                        return log_debug_errno(set_proj_id, "Failed to set project ID for %s: %m", target_dir);
                                if (set_proj_id) {
                                        *exec_dt_proj_id = proj_id;
        ...
        if (ql->quota_enforce && !(*already_enforced)) {
                ...
                _cleanup_free_ char *chased_target = NULL, *mountpoint = NULL;
                r = chase(target_dir, NULL, CHASE_STEP|CHASE_NONEXISTENT, &chased_target, NULL);
                if (r < 0)
                        return log_debug_errno(r, "Failed to chase %s: %m", target_dir);

                mountpoint = find_mountpoint(chased_target);
                if (!mountpoint)
                        return log_debug_errno(SYNTHETIC_ERRNO(ENOENT), "Failed to find mountpoint for %s: %m", target_dir);

                fd = open(mountpoint, O_PATH, O_CLOEXEC);
                if (fd < 0)
                        return log_debug_errno(errno, "Failed to open mountpoint for %s: %m", target_dir);
