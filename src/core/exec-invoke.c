...
static int can_mount_proc(const ExecContext *c, ExecParameters *p) {
        ...
        r = is_dir("/dev/shm/", /* follow = */ true);
        if (r < 0)
                return log_exec_debug_errno(c, p, r, "Failed to stat /dev/shm/: %m");
        if (r == 0)
                return log_exec_debug_errno(c, p, SYNTHETIC_ERRNO(ENOENT), "/dev/shm/ is not a directory");
...
static int setup_private_pids(const ExecContext *c, ExecParameters *p) {
        ...
        r = pidref_safe_fork("(sd-pidfd-child)", FORK_NEW_PIDNS, &pidref);
        if (r < 0)
        ...
        if (n != sizeof(r))
                return log_exec_debug_errno(c, p, SYNTHETIC_ERRNO(EIO), "Failed to read %lu bytes from pipe with parent process", sizeof(r));
        if (r < 0)
