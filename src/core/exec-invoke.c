...
int exec_invoke(
        ...
        if (params->cgroup_path) {
                ...
                /* We cannot spawn the main service process into the subcgroup as it needs to unshare the cgroup
                 ...
                 * of cgroupv2. */
                const char *cgtarget = exec_params_needs_control_subcgroup(params) ? p : params->cgroup_path;

                r = cg_attach_everywhere(params->cgroup_supported, cgtarget, 0);
                if (r == -EUCLEAN) {
                        *exit_status = EXIT_CGROUP;
                        return log_exec_error_errno(context, params, r,
