...
static int bpffs_prepare(
        ...
        if (r == 0) {
                ...
                if (r < 0) {
                        log_debug_errno(r, "Failed to FSCONFIG_CMD_CREATE: %m");
                        _exit(EXIT_FAILURE);
                }

                mnt_fd = fsmount(fs_fd, 0, 0);
                if (mnt_fd < 0) {
                        log_debug_errno(mnt_fd, "Failed to receive_one_fd: %m");
                        _exit(EXIT_FAILURE);
                }

                r = send_one_fd(pipe_fds[0], mnt_fd, 0);
                if (r < 0) {
                        log_debug_errno(errno, "Failed to send_one_fd: %m");
                        _exit(EXIT_FAILURE);
                }
