...
static int api_bus_instance_id_reply(sd_bus_message *reply, void *userdata, sd_bus_error *reterr_error) {
         ...
         * If no instance ID was recorded, this is not a reconnection, hence nothing to compare. */
        if (subscribed_as_strv &&
            (sd_id128_is_null(deserialized_bus_id) || sd_id128_equal(m->bus_id, deserialized_bus_id)))
                (void) bus_track_coldplug(sd_bus_message_get_bus(reply), &m->subscribed, subscribed_as_strv);
...
int bus_init_api(Manager *m) {
         ...
         * connection was torn down) may be coldplugged onto this connection. A synchronous call is not OK
         * here: this is reached on every (re-)connection attempt, and if the D-Bus socket unit is
         ...
         * this connection, hence reset it: only ever populated from the reply, so that a reexec racing the
         * reply serializes no ID rather than a stale one. */
