...
static void destroy_bus(Manager *m, sd_bus **bus) {
        ...
        if (m->subscribed && sd_bus_track_get_bus(m->subscribed) == *bus) {
                ...
                r = strv_extend_strv_consume(&m->subscribed_as_strv, TAKE_PTR(subscribed), /* filter_duplicates = */ false);
                if (r < 0)
                        log_warning_errno(r, "Failed to record api subscribers, ignoring: %m");
