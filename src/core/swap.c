...
static int swap_arm_timer(Swap *s, usec_t usec) {
        ...
        r = sd_event_add_time(
                UNIT(s)->manager->event, &s->timer_event_source, CLOCK_MONOTONIC, usec, 0, swap_dispatch_timer, s);
        if (r < 0)
                return r;
...
static int swap_add_device_dependencies(Swap *s) {
        ...
        if (is_device_path(s->what))
                return unit_add_node_dependency(UNIT(s),
                                                s->what,
                ...
                return unit_add_dependency_by_name(
                        UNIT(s), UNIT_AFTER, SPECIAL_REMOUNT_FS_SERVICE, true, UNIT_DEPENDENCY_FILE);
}
...
static int swap_add_default_dependencies(Swap *s) {
        ...
        r = unit_add_dependency_by_name(
                UNIT(s), UNIT_BEFORE, SPECIAL_SWAP_TARGET, true, UNIT_DEPENDENCY_DEFAULT);
        if (r < 0)
                return r;

        return unit_add_two_dependencies_by_name(
                UNIT(s), UNIT_BEFORE, UNIT_CONFLICTS, SPECIAL_UMOUNT_TARGET, true, UNIT_DEPENDENCY_DEFAULT);
}
...
static int swap_load_devnode(Swap *s) {
        ...
        if (r < 0) {
                log_unit_full(UNIT(s),
                              r == -ENOENT ? LOG_DEBUG : LOG_WARNING,
                              r,
                              "Failed to allocate device for swap %s: %m",
                              s->what);
                return 0;
        }
...
static int swap_setup_unit(
        Manager *m, const char *what, const char *what_proc_swaps, int priority, bool set_flags) {
        ...
        if (u && SWAP(u)->from_proc_swaps && !path_equal(SWAP(u)->parameters_proc_swaps.what, what_proc_swaps))
                return log_error_errno(SYNTHETIC_ERRNO(EEXIST),
                                       "Swap %s appeared twice with different device paths %s and %s",
                                       e,
                                       SWAP(u)->parameters_proc_swaps.what,
                                       what_proc_swaps);
        ...
        /* The unit is definitely around now, mark it as loaded if it was previously referenced but could not
         * be loaded. After all we can load it now, from the data in /proc/swaps. */
...
static int swap_coldplug(Unit *u) {
        ...
        if (s->control_pid > 0 && pid_is_unwaited(s->control_pid) && SWAP_STATE_WITH_PROCESS(new_state)) {
...
static void swap_dump(Unit *u, FILE *f, const char *prefix) {
        ...
        fprintf(f,
                ...
                "%sFrom fragment: %s\n",
                prefix,
                swap_state_to_string(s->state),
                prefix,
                swap_result_to_string(s->result),
                prefix,
                s->what,
                prefix,
                yes_no(s->from_proc_swaps),
                prefix,
                yes_no(s->from_fragment));
...
static int swap_spawn(Swap *s, ExecCommand *c, pid_t *_pid) {
        ...
        r = exec_spawn(UNIT(s), c, &s->exec_context, &exec_params, s->exec_runtime, &s->dynamic_creds, &pid);
        if (r < 0)
                goto fail;
...
static void swap_enter_dead(Swap *s, SwapResult f) {
        ...
        exec_context_destroy_runtime_directory(&s->exec_context,
                                               UNIT(s)->manager->prefix[EXEC_DIRECTORY_RUNTIME]);
...
static void swap_enter_activating(Swap *s) {
        ...
        if (s->from_fragment) {
                ...
                if (r < 0)
                        log_warning_errno(r,
                                          "Failed to parse swap priority \"%s\", ignoring: %m",
        ...
        if (s->parameters_fragment.options || opts) {
                r = exec_command_append(s->control_command, "-o", opts ?: s->parameters_fragment.options, NULL);
                if (r < 0)
                        goto fail;
        }
...
static int swap_start(Unit *u) {
        ...
        if (IN_SET(s->state, SWAP_DEACTIVATING, SWAP_DEACTIVATING_SIGTERM, SWAP_DEACTIVATING_SIGKILL))
                return -EAGAIN;
...
static void swap_enumerate(Manager *m) {
        ...
        if (!m->proc_swaps) {
                ...
                r = sd_event_add_io(
                        m->event, &m->swap_event_source, fileno(m->proc_swaps), EPOLLPRI, swap_dispatch_io, m);
                if (r < 0) {
