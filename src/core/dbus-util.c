...
int bus_read_mount_options(
        ...
        while ((r = sd_bus_message_read(message, "(ss)", &partition, &mount_options)) > 0) {
                ...
                if (!isempty(escaped)) {
                        ...
                        free_and_strdup(&options->options[partition_designator], mount_options);
                } else if (options)
        ...
        if (options) {
                if (in_out_format_str) {
                        if (!strextend_with_separator(in_out_format_str, separator, format_str))
                                return -ENOMEM;
                }
