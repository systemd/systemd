...
int setup_namespace(const NamespaceParameters *p, char **reterr_path) {
        ...
        if (p->rootfs && p->rootfs->directory_fd >= 0) {
                assert(root_mount_fd >= 0);
        ...
        } else if (p->rootfs && p->rootfs->image_fd >= 0) {
                assert(dissected_image);
        ...
        } else if (p->rootfs && p->rootfs->mstack) {
                assert(p->rootfs->mstack);
