...
int refresh_extensions_in_namespace(
        ...
        if (r == 0) {
                ...
                r = safe_fork("(sd-ns-refresh-exts-grandchild)", FORK_DEATHSIG_SIGKILL, &grandchild_pid);
                if (r < 0)
                        _exit(EXIT_FAILURE);
                if (r == 0) {
                ...
                r = wait_for_terminate_and_check("(sd-ns-refresh-exts-grandchild)", grandchild_pid, 0);
                if (r < 0) {
                        log_debug_errno(r, "Target namespace fork did not succeed: %m");
                        _exit(EXIT_FAILURE);
                }
