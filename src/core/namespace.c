...
static int mount_path_compare(const MountEntry *a, const MountEntry *b) {
        ...
        if ((a->mode == EXTENSION_IMAGES) != (b->mode == EXTENSION_IMAGES)) {
                if ((a->mode == EXTENSION_IMAGES))
                        return -1;
                else
                        return 1;
        }
...
static void drop_outside_root(const char *root_directory, MountEntry *m, size_t *n) {
        ...
        for (f = m, t = m; f < m + *n; f++) {
                ...
                /* ExtensionImages bases are opened in the propagate directory intentionally */
...
static int append_extension_images(
        ...
        STRV_FOREACH(hierarchy, hierarchies) {
                ...
                strv_consume(&overlays, TAKE_PTR(prefixed_hierarchy));
        }

        /* On systemd --user instance we don't have a propagate dir. */
        if (!propagate_dir)
                return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to create temp directory for overlays, no propagate directory available");
        ...
        if (r < 0)
                return log_debug_errno(r, "Failed to create temp directory under %s: %m", propagate_dir);
        ...
        for (size_t i = 0; i < n; i++) {
                ...
                mount_point = path_join(mount_point_parent, basename(m->source));
                if (!mount_point)
                        return -ENOMEM;

                STRV_FOREACH_INDEX(hierarchy, hierarchies, j) {
                        _cleanup_free_ char *prefixed_hierarchy = NULL, *escaped = NULL, *lowerdir = NULL;

                        prefixed_hierarchy = path_join(mount_point, *hierarchy);
                        if (!prefixed_hierarchy)
                                return -ENOMEM;
        ...
        STRV_FOREACH_INDEX(hierarchy, hierarchies, j) {
                ...
                *((*p)++) = (MountEntry) {
                        ...
                        .options_malloc = TAKE_PTR(overlays[j]),
                        .mode = OVERLAY_MOUNT,
                        .has_prefix = true,
...
int setup_namespace(
        ...
        if (n_mounts > 0) {
                _cleanup_strv_free_ char **lowerdirs_usr = NULL, **lowerdirs_opt = NULL;
                ...
                r = append_extension_images(&m, root, propagate_dir, hierarchies, extension_images, n_extension_images);
                if (r < 0)
                        goto finish;
