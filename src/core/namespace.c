...
static int mount_private_fs(const char *fstype, const char *entry_path, const char *bind_source, const char *opts) {
        int r;
...
static int mount_private_sysfs(const MountEntry *m) {
        return mount_private_fs("sysfs", mount_entry_path(ASSERT_PTR(m)), "/sys", NULL);
}
...
static int mount_procfs(const MountEntry *m, const NamespaceParameters *p) {
        ...
        return mount_private_fs("proc", mount_entry_path(ASSERT_PTR(m)), "/proc", opts);
}
