...
static int mount_procfs(const MountEntry *m, const NamespaceInfo *ns_info) {
        ...
        if (r == -EPERM) {
                ...
                if (r == 0) {
                        r = path_is_mount_point("/proc", NULL, 0);
                        if (r < 0)
                                return log_debug_errno(r, "Unable to determine whether /proc is already mounted: %m");
                        if (r == 0)
                                /* /proc is not mounted, neither at the target nor on the host. Propagate
...
int setup_namespace(
        ...
        if (r == -EINVAL && root_directory) {
                 ...
                 * MS_MOVE will fail as we don't have permission to change it (of course with EINVAL rather
                 * than EPERM). Attempt to bind-mount it over itself (like we do above if it's not already
                 * a mount point) and try again. */
                r = mount_nofollow_verbose(LOG_DEBUG, root, root, NULL, MS_BIND|MS_REC, NULL);
                if (r < 0)
                        goto finish;
                r = mount_move_root(root);
