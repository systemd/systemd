...
int setup_namespace(
        ...
        _cleanup_strv_free_ char **hierarchies = NULL, **symlinks = NULL;
         ...
         * and readers will never get a half-written version. */
        if (host_os_release_stage) {
                symlinks = strv_copy(exec_dir_symlinks);
                if (!symlinks) {
                        r = -ENOMEM;
                        goto finish;
                }
        ...
        r = apply_mounts(root, mount_image_policy, extension_image_policy, ns_info, mounts, &n_mounts, symlinks ?: exec_dir_symlinks, error_path);
        if (r < 0)
                goto finish;
