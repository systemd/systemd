...
static int mount_bpffs(const MountEntry *m, int socket_fd) {
        ...
        if (bpffs_fd < 0)
                return log_debug_errno(bpffs_fd, "Failed to receive bpffs fd from child: %m");
...
char * bpf_delegate_to_string(uint64_t u, const char * (*parser)(uint64_t) _const_ ) {
        ...
        _cleanup_free_ char *buf = strdup("");
        if (!buf)
                return NULL;

        BIT_FOREACH(i, u)
                if (strextendf_with_separator(&buf, ",", "%s", parser(i)) < 0)
                        return NULL;
...
int bpf_delegate_from_string(const char *s, uint64_t *ret, uint64_t (*parser)(const char *)) {
        ...
        for (;;) {
                ...
                r = parser(word);
                if (r < 0)
                        log_warning_errno(r, "Invalid BPF delegate: %s", word);
                else
                        mask |= UINT64_C(1) << r;
        }
