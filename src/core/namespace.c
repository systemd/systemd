...
static int is_extension_overlay(const char *path, int fd) {
        ...
        if (fd < 0) {
                dfd = chase_and_open(path, /* root= */ NULL, CHASE_TRAIL_SLASH,  /* open_flags= */ 0, /* ret_path= */ NULL);
                if (dfd < 0)
                        return dfd;
                fd = dfd;
        }
...
static int unpeel_get_fd(const char *mount_path, int *ret_fd) {
        ...
        if (r == 0) {
                ...
                r = is_extension_overlay(mount_path, dir_fd);
                if (r < 0) {
                        log_debug_errno(r, "Unable to determine whether '%s' is an extension overlay after mounting: %m", mount_path);
                        _exit(EXIT_FAILURE);
                }
                if (r > 0) {
                        log_debug_errno(r, "'%s' is still an extension overlay after umounting: %m", mount_path);
                        _exit(EXIT_FAILURE);
                }
...
static int refresh_grandchild_proc(
        ...
        FOREACH_ARRAY(m, ml->mounts, ml->n_mounts) {
                ...
                /* path_startswith() truncates the output so use startswith() */
                ...
                if (IN_SET(f->mode, MOUNT_EXTENSION_DIRECTORY, MOUNT_EXTENSION_IMAGE)) {
                        ...
                        if (r == 0) {
                                FOREACH_ARRAY(m, ml->mounts, ml->n_mounts) {
                                        ...
                                        STRV_FOREACH(ol, m->overlay_layers)
                                                if (startswith(*ol, mount_entry_path(f))) {
                                                        r = strv_extend(&to_remove, *ol);
                                                        if (r < 0)
                                                                return log_oom_debug();
                                                }
                                        STRV_FOREACH(ol, to_remove)
                                                m->overlay_layers = strv_remove(m->overlay_layers, *ol);
                                }
                                mount_entry_done(f);
                                continue;
                        }
