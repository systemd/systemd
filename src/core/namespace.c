...
typedef struct MountEntry {
        ...
        ImageClass required_class; /* Used for live updates to skip unapplicable images */
        bool idmapped;
        uid_t idmap_uid;
        gid_t idmap_gid;
} MountEntry;
...
static int mount_image(
        ...
        if (m->mode == MOUNT_EXTENSION_IMAGE) {
                ...
                required_class = m->required_class;
        }
...
static int apply_one_mount(
        ...
        case MOUNT_EXTENSION_DIRECTORY: {
                ...
                r = load_extension_release_pairs(
                                mount_entry_source(m),
                                m->required_class >= 0 ? m->required_class : IMAGE_SYSEXT,
                                extension_name,
                                /* relax_extension_release_check= */ false,
                                &extension_release);
                if (r == -ENOENT) {
                        if (m->required_class >= 0)
                                return 0; /* Nothing to do, wrong class */
...
static int is_extension_overlay(const char *path) {
        ...
        r = path_is_fs_type(path, OVERLAYFS_SUPER_MAGIC);
        if (r < 0)
        ...
        r = path_get_mount_info(path, /* ret_fstype= */ NULL, /* ret_options= */ NULL, &source);
        if (r < 0)
...
static int unpeel_get_fd(const char *mount_path, int *ret_fd) {
        ...
        if (r == 0) {
                ...
                r = is_extension_overlay(mount_path);
                if (r < 0) {
...
static int refresh_apply_and_prune(const NamespaceParameters *p, MountList *ml) {
        ...
        for (f = ml->mounts, t = ml->mounts; f < ml->mounts + ml->n_mounts; f++) {
                if (IN_SET(f->mode, MOUNT_EXTENSION_DIRECTORY, MOUNT_EXTENSION_IMAGE)) {
                        f->required_class = IMAGE_CONFEXT;
                                        ...
                                        if (m->mode == MOUNT_OVERLAY)
                                                STRV_FOREACH(ol, m->overlay_layers)
                                                        if (startswith(*ol, mount_entry_path(f)))
                                                                strv_remove(m->overlay_layers, *ol);
                                mount_entry_done(f);
                                continue;
                        }
                }
