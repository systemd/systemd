...
static int mount_image(
        ...
        if (r == -ESTALE && host_os_release_id)
                return log_error_errno(r,
                                       ...
                                       host_os_release_level ? image_class_info[class].level_env : "",
...
int setup_namespace(
        ...
        if (n_extension_images > 0 || !strv_isempty(extension_directories)) {
                // r = parse_env_extension_hierarchies(&hierarchies, image_class_info[dissected_image->image_class].name_env);
                log_error("\n\nThe class of the dissected image is: %s\n\n", image_class_to_string(dissected_image->image_class));
                log_error("\n\nThe hierarchy env name is: %s\n\n", image_class_info[dissected_image->image_class].name_env);
