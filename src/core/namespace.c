...
static int mount_private_dev(const MountEntry *m, const NamespaceParameters *p) {
        ...
        r = label_fix_full(AT_FDCWD, dev, "/dev", /* flags= */ 0, /* label_userdata= */ NULL);
        if (r < 0)
...
static int mount_tmpfs(const MountEntry *m) {
        ...
        r = label_fix_full(AT_FDCWD, entry_path, inner_path, /* flags= */ 0, /* label_userdata= */ NULL);
        if (r < 0)
...
static int apply_one_mount(
        ...
        case MOUNT_BIND_RECURSIVE: {
                ...
                if (m->create_source_dir) {
                        ...
                        r = label_fix_full(AT_FDCWD, mount_entry_source(m), mount_entry_unprefixed_path(m), /* flags= */ 0, /* label_userdata= */ NULL);
                        if (r < 0)
...
int setup_tmp_dir_one(const char *id, const char *prefix, char **ret_path) {
        ...
        if (rw) {
                ...
                r = label_fix_full(AT_FDCWD, inner_dir, prefix, /* flags= */ 0, /* label_userdata= */ NULL);
                if (r < 0) {
