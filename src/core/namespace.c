...
int setup_namespace(
         ...
         * When working out ExtensionImages this will be needed already, just like
         * the root directory is needed by other mounts, so set it up immediately. */
        if (setup_propagate)
                (void) mkdir_p(propagate_dir, 0600);

        if (n_extension_images > 0) {
                r = getenv_path_list("SYSTEMD_SYSEXT_HIERARCHIES", &hierarchies);
                if (r < 0)
