...
static int unpeel_get_fd(const char *mount_path, int *ret_fd) {
        ...
        if (r == 0) {
                ...
                if (path_is_fs_type(mount_path, OVERLAYFS_SUPER_MAGIC)) {
                        r = umount_recursive(mount_path, MNT_DETACH);
                        if (r == 0) { /* no umounts done */
                                log_debug("No overlay layer unmountable from %s, refusing to proceed", mount_path);
                                _exit(EXIT_FAILURE);
                        } else if (r < 0) {
...
int refresh_extensions_in_namespace(
        ...
        if (r == 0) {
                ...
                if (r == 0) {
                        ...
                        r = pidref_namespace_open(target, &pidns_fd, &mntns_fd, NULL, NULL, &root_fd);
                        if (r < 0) {
                        ...
                        if (r < 0) {
                                log_debug_errno(r, "Failed to check if target namespace is separate: %m");
                                _exit(EXIT_FAILURE);
                        }
                        if (r > 0) {
                                log_debug("Target namespace is not separate, cannot reload extensions");
                                _exit(EXIT_FAILURE);
                        }
                        ...
                        FOREACH_ARRAY(m, ml.mounts, ml.n_mounts) {
                                ...
                                /* If there are no extensions mounted for this overlay layer, instead of
                                 * setting everything up, the correct behavior would be to unmount the
                                 * existing overlay in the target namespace to expose the original files.
                                 */
                                ...
                                if (r < 0) {
                                        ...
                                        /* In case parent is waiting, send FD 0 to signal an error (nonsense here) */
                                        (void) send_one_fd(pair[1], 0, 0);
                                        _exit(EXIT_FAILURE);
                                }
                        }
                        _exit(EXIT_SUCCESS);
                ...
                FOREACH_ARRAY(m, ml.mounts, ml.n_mounts) {
                        ...
                        if (r < 0) {
                                log_debug_errno(r, "Failed to move mount for %s: %m", mount_path);
                                _exit(EXIT_FAILURE);
                        }
