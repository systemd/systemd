...
static int append_extensions(
        ...
        _cleanup_free_ VeritySettings *verities = NULL;
        char ***overlays = NULL;
        size_t n_overlays = 0, n_verities = 0;
        int r;
         ...
         * drop duplicates. Overlayfs rejects duplicate lowerdir= even if they are mounted on different
         * paths, as it resolves each mount to its source device, rather than using the lowerdir. */
        verities = new0(VeritySettings, n);
        if (!verities)
                return -ENOMEM;

        CLEANUP_ARRAY(verities, n_verities, verities_done_many);
        ...
        for (size_t i = 0; i < n; i++) {
                ...
                if (verity.root_hash_size > 0) {
                        ...
                        FOREACH_ARRAY(v, verities, n_verities)
                                if (v->root_hash_size == verity.root_hash_size &&
                                    memcmp_safe(v->root_hash, verity.root_hash, v->root_hash_size) == 0) {
                                        dup = true;
                                        break;
                                }

                        if (dup) {
                                log_debug("Skipping duplicate extension image %s", result.path);
                                continue;
                        }

                        if (!GREEDY_REALLOC(verities, n+1))
                                return -ENOMEM;
                        verities[n_verities++] = verity;
                        verity = (VeritySettings) VERITY_SETTINGS_DEFAULT;
                }
