...
static int verities_find_duplicates(
        ...
        if (verities_find_by_roothash(verities, n_verities, &verity)) {
...
static int mount_image(
        ...
        if (m->mode == MOUNT_EXTENSION_IMAGE) {
                 ...
                 * explained above. We check twice, first time here, as for single filesystem images we can
                 * detect duplicates by simply looking for the sidecar roothash file, which is cheap and can
                 * be done without changing the state of the system (opening verity device in the kernel). */
        ...
        if (m->mode == MOUNT_EXTENSION_IMAGE && verity.designator != _PARTITION_DESIGNATOR_INVALID) {
                /* Second duplicate check: for DDIs we cannot check the roothash until after the GPT image
                 * has been fully mounted, so we have to do it again here. */
                if (verities_find_by_roothash(*extension_verities, *n_extension_verities, &verity)) {
                        (void) umount_recursive(mount_entry_path(m), 0);
                        log_debug("Skipping duplicate extension image %s", mount_entry_source(m));
                        return 0;
                }
