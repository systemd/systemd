 ...
 * 1. Sends underlying mount directory (e.g., /etc) fd to parent
 * 2. Creates and sends new overlay fs fd to parent
...
static int unpeel_mount_and_setup_overlay(int pair_fd, const char *mount_path) {
        _cleanup_close_ int dir_unpeeled_fd = -1, overlay_fs_fd = -1, mount_fd = -1;
        int r;
        ...
        /* Opportunistically unmount the directory to peel back any overlays */
        r = umount2(mount_path, MNT_DETACH|UMOUNT_NOFOLLOW);
        if (r < 0 && errno != EINVAL && errno != ENOENT)
                return log_debug_errno(errno, "Failed to unmount %s: %m", mount_path);

        /* Open the now peeled-back directory without any overlays */
        dir_unpeeled_fd = open_tree(-EBADF, mount_path, AT_SYMLINK_NOFOLLOW|OPEN_TREE_CLONE|OPEN_TREE_CLOEXEC);
        if (dir_unpeeled_fd < 0)
                return log_debug_errno(errno, "Failed to open %s: %m", mount_path);
        ...
        r = mount_exchange_graceful(mount_fd, mount_path, MS_MOVE);
        if (r < 0)
...
int refresh_extensions_in_namespace(
        ...
        if (!mount_new_api_supported()) {
                log_info("Skipping extension refresh in namespace, as the kernel does not support the new mount API");
                return 0;
        }
        ...
        if (r == 0) {
                ...
                /* Create a grandchild process to handle the unmounting and reopening of the hierarchy directory */
                ...
                FOREACH_ARRAY(m, ml.mounts, ml.n_mounts) {
                        ...
                        _cleanup_close_ int hierarchy_path_fd = -1, overlay_fs_fd = -1;
