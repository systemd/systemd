...
int setup_shareable_ns(int ns_storage_socket[static 2], unsigned long nsflag) {
        ...
        r = unposix_lock(ns_storage_socket[0], LOCK_EX);
        if (r < 0)
                return r;

        CLEANUP_UNPOSIX_UNLOCK(ns_storage_socket[0]);

        ns = receive_one_fd(ns_storage_socket[0], MSG_PEEK|MSG_DONTWAIT);
        if (ns >= 0)
                /* Yay, found something, so let's join the namespace */
                return RET_NERRNO(setns(ns, nsflag));
        ...
        return send_one_fd(ns_storage_socket[1], ns, MSG_DONTWAIT);
}
