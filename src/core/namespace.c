...
static int mount_image(
        ...
        if (m->mode == MOUNT_EXTENSION_IMAGE && verity_settings_set(&verity)) {
...
static int apply_mounts(
         ...
         * resolves each mount to its source filesystem.  This only covers non-DDI verity images. */
        if (p->n_extension_images > 0) {
