...
static void service_enter_reload_post(Service *s) {
        ...
        /* Service notified us of reload failure and is to be terminated (RELOADING=-1 + STOPPING=1)? */
        if (s->reload_result != SERVICE_SUCCESS)
                return service_reload_finish(s, s->reload_result);
        ...
        if (s->control_command) {
                ...
                return service_set_state(s, SERVICE_RELOAD_POST);
        } else
...
static void service_notify_message_process_state(Service *s, char * const *tags) {
        ...
        const char *reloading = strv_find_prefix(tags, "RELOADING=");
        if (reloading) {
                int v;
        ...
        if (strv_contains(tags, "STOPPING=1")) {
                ...
                if (reload_status == RELOAD_ERROR)
                        s->reload_result = SERVICE_FAILED_RESOURCES;
        ...
        if (strv_contains(tags, "READY=1")) {
                ...
                if (s->state == SERVICE_RELOAD_NOTIFY) {
                        if (reload_status == RELOAD_ERROR)
                                service_reload_finish(s, SERVICE_FAILED_RESOURCES);
                        else
                                service_enter_reload_post(s);
                }
