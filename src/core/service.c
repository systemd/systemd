...
static usec_t service_coldplug_timeout(Service *s) {
        ...
        case SERVICE_AUTO_RESTART:
                return usec_add(UNIT(s)->inactive_enter_timestamp.monotonic,
                                usec_add(service_restart_usec_next(s), s->restart_randomized_delay_chosen_usec));
...
static void service_enter_dead(Service *s, ServiceResult f, bool allow_restart) {
        ...
        if (allow_restart) {
                ...
                /* Do the randomized restart delay once and remember it so that its stable accross daemon-reload */
                ...
                restart_usec_next = usec_add(service_restart_usec_next(s), s->restart_randomized_delay_chosen_usec);
