...
static void service_enter_reload_notify(Service *s);
...
static void service_enter_reload_notify(Service *s) {
        ...
        /* service_enter_reload_notify is never called during a reload, thus no loops are possible. */
...
static void service_notify_message(
        ...
        if (strv_contains(tags, "STOPPING=1")) {
                ...
                if (s->state == SERVICE_RELOAD_SIGNAL &&
                    monotonic_usec != USEC_INFINITY &&
                    monotonic_usec >= s->reload_begin_usec)
                        /* Note, we don't call service_enter_reload_notify() here, because we
