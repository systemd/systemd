...
static int service_check_reload_signal_handler(Service *s, const char *log_suffix) {
        int r;

        assert(s);
        assert(log_suffix);

        if (!pidref_is_set(&s->main_pid) || s->reload_signal == SIGKILL)
                return 0;
        ...
        if (r < 0) {
                if (r != -ESRCH)
                        log_unit_warning_errno(
                                        UNIT(s), r,
                                        "Failed to check for reload signal handler: %m%s",
                                        log_suffix);
                return r;
        }
        ...
        if (r == 0) {
                ...
                if (r < 0) {
                        if (r != -ESRCH)
                                log_unit_warning_errno(
                                                UNIT(s), r,
                                                "Failed to check for blocked reload signal: %m%s",
                                                log_suffix);
                        return r;
                }
        }
...
static void service_enter_reload_signal(Service *s) {
        ...
        if (pidref_is_set(&s->main_pid)) {
                ...
                (void) service_check_reload_signal_handler(s, ", sending reload signal anyway");
...
static void service_notify_message_process_state(Service *s, char * const *tags) {
        ...
        if (strv_contains(tags, "READY=1")) {
                ...
                if (s->type == SERVICE_NOTIFY_RELOAD && s->state == SERVICE_START) {
                        r = service_check_reload_signal_handler(s, ", refusing to reload");
                        if (r < 0) {
                                service_enter_signal(s, SERVICE_STOP_SIGTERM, SERVICE_FAILURE_PROTOCOL);
                                return;
                        }
