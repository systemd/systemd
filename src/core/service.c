...
static void service_notify_message_process_state(Service *s, char * const *tags) {
        ...
        if (strv_contains(tags, "READY=1")) {
                ...
                /* Only check for a reload handler before leaving SERVICE_START. Use the cached flag above
                 * since we may already be in START_POST/RUNNING by the time we get here. */
