...
static int service_load_pid_file(Service *s, bool may_warn) {
        ...
        if (r == 0) {
                ...
                if (st.st_uid != getuid())
                        return log_unit_error_errno(UNIT(s), SYNTHETIC_ERRNO(EPERM),
                                                    "New main PID "PID_FMT" from PID file does not belong to service, and PID file is owned by "PID_FMT" (must be owned by "PID_FMT"). Refusing.",
                                                    pidref.pid, st.st_uid, getuid());

                log_unit_debug(UNIT(s), "New main PID "PID_FMT" does not belong to service, accepting anyway since PID file is owned by "PID_FMT".",
