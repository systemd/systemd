...
static int service_build_luo_session_name(Service *s, const char *name, char **ret) {
        ...
        /* The LUO session name lives in a global, flat kernel namespace, and it's very short. Derive it
         * from the unit id and the configured name, formatted as a hex string, so it is deterministic.
         * Prefix it with a (possibly truncated) unit id for debugging purposes. */
        assert_cc(SD_ID128_STRING_MAX < LIVEUPDATE_SESSION_NAME_LENGTH);
        size_t prefix_max = LIVEUPDATE_SESSION_NAME_LENGTH - SD_ID128_STRING_MAX - 1; /* leave room for '-' */

        if (asprintf(&result, "%.*s-%s",
                     (int) prefix_max, UNIT(s)->id,
                     SD_ID128_TO_STRING(id128_digest(full, SIZE_MAX))) < 0)
                return -ENOMEM;
