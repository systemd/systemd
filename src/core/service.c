...
static void service_notify_pidref(Unit *u, PidRef *parent_pidref, PidRef *child_pidref) {
        ...
        if (pidref_equal(&s->main_pid, parent_pidref)) {
                ...
                if (r < 0) {
                        log_unit_warning_errno(u, r, "Failed to set new main pid: %m");
                        return;
                }
                ...
                if (r < 0)
                        log_unit_warning_errno(u, r, "Failed to watch new main PID: %m");
                ...
                if (r < 0)
                        log_unit_warning_errno(u, r, "Failed to watch new control PID: %m");
        } else
                return;
