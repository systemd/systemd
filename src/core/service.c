...
int service_add_fd_store(Service *s, int fd_in, const char *name, bool do_poll, bool propagate_upstream) {
        ...
        *fs = (ServiceFDStore) {
        ...
        if (propagate_upstream && s->fd_store_preserve_mode == EXEC_PRESERVE_YES) {
                /* The combined "unit-id|fdname" tag must remain a valid fdname for the upstream
                 * supervisor to accept it, otherwise the fd would be stored under the default name
                 * ("stored") and we'd be unable to route it back to this unit on restore. */
                _cleanup_free_ char *combined = strjoin(UNIT(s)->id, "|", fs->fdname);
                if (!combined)
                        log_oom_debug();
                else if (!fdname_is_valid_full(combined, /* accept_propagation= */ true))
                        log_unit_debug(UNIT(s),
                                       "Combined upstream tag '%s' is invalid, not propagating.",
                                       combined);
                else {
                        r = notify_push_fd(fs->fd, combined);
                        if (r >= 0)
...
static void service_notify_message(
        ...
        if (strv_contains(tags, "FDSTOREREMOVE=1")) {
                ...
                if (!name || !fdname_is_valid_full(name, /* accept_propagation= */ true))
