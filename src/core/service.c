...
static int service_notify_pidref(Unit *u, PidRef *parent_pidref, PidRef *child_pidref) {
        Service *s = ASSERT_PTR(SERVICE(u));
        int r;
        ...
        if (pidref_equal(&s->main_pid, parent_pidref)) {
                ...
                if (r < 0)
                        return log_unit_warning_errno(u, r, "Failed to set new main pid: %m");
                ...
                log_unit_debug(u, "Parent process " PID_FMT " does not match main or control processes", parent_pidref->pid);
                return 0;
        }
