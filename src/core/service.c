...
static Service *service_get_triggering_service(Service *s) {
        ...
        UNIT_FOREACH_DEPENDENCY(other, UNIT(s), UNIT_ATOM_PROPAGATE_EXIT_STATUS) {
                ...
                if (unit_has_dependency(other, UNIT_ATOM_ON_SUCCESS, UNIT(s)) ||
                    unit_has_dependency(other, UNIT_ATOM_ON_FAILURE, UNIT(s))) {
                    if (candidate) {
        ...
        if (unit_has_dependency(candidate, UNIT_ATOM_ON_SUCCESS, UNIT(s)) &&
            SERVICE(candidate)->result == SERVICE_SUCCESS &&
            unit_active_state(candidate) != UNIT_FAILED)
                return SERVICE(candidate);
        else if (unit_has_dependency(candidate, UNIT_ATOM_ON_FAILURE, UNIT(s)) &&
                 !IN_SET(SERVICE(candidate)->result, SERVICE_SUCCESS, SERVICE_SKIP_CONDITION) &&
                 unit_active_state(candidate) == UNIT_FAILED)
                return SERVICE(candidate);
