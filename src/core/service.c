...
static int service_dispatch_exec_io(sd_event_source *source, int fd, uint32_t events, void *userdata) {
        ...
        for (;;) {
                ...
                if (x == 0) {
                        /* A single zero byte was read, which means the exec fd logic is now off.
                        ...
                        /* Check n just in case somehow an old executor before this logic was introduced
                         * sent the notification with x=1. */
                        if (n == sizeof(x) && s->state == SERVICE_START) {
