...
static int service_reload_extensions(Unit *u) {
        _cleanup_free_ char *root_dir = NULL, *propagate_dir = NULL, *private_namespace_dir = NULL,
                *incoming_dir = NULL, *hierarchy_env = NULL;
        ...
        hierarchy_env = strdup("SYSTEMD_CONFEXT_HIERARCHIES");
        if (!hierarchy_env)
                return -ENOMEM;
        ...
        return refresh_extensions_in_namespace(unit_main_pid(u), hierarchy_env, &p);
}
