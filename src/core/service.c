...
static int service_live_mount(Unit *u,
        ...
        if (!c)
                return log_unit_warning_errno(u, SYNTHETIC_ERRNO(EINVAL), "Cannot access unit execution context");
...
static bool service_can_live_mount(Unit *u, const char *dst) {
        assert(u);
        assert(dst);
        ...
        if (!c) {
                log_unit_debug(u, "Cannot access unit execution context");
                return false;
        }
        ...
        if (!exec_needs_mount_namespace(c, NULL, unit_get_exec_runtime(u))) {
                log_unit_debug(u, "Unit not running in private mount namespace, cannot activate bind mount");
                return false;
        }
