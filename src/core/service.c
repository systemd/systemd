...
static usec_t service_restart_usec(Service *s) {
        ...
        unit = pow(usec_sub_unsigned(s->restart_usec_max, s->restart_usec), 1.0 / s->restart_steps);

        return (usec_t) (s->restart_usec + pow(unit, s->n_restarts));
}
        ...
        case SERVICE_AUTO_RESTART:
                if (s->restart_usec > 0)
                        log_unit_debug(UNIT(s),
                                       "Service restart interval %u expired, scheduling restart.",
