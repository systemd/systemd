...
static Service *service_get_triggering_service(Service *s) {
        ...
        UNIT_FOREACH_DEPENDENCY(other, UNIT(s), UNIT_ATOM_PROPAGATE_EXIT_STATUS) {
                if (other->type != UNIT_SERVICE)
                        continue;

                if (unit_has_dependency(other, UNIT_ATOM_ON_SUCCESS, UNIT(s))) {
                        if (SERVICE(other)->result == SERVICE_SUCCESS &&
                            unit_active_state(other) != UNIT_FAILED) {
                                if (candidate != NULL) {
                ...
                if (unit_has_dependency(other, UNIT_ATOM_ON_FAILURE, UNIT(s))) {
                        if (!IN_SET(SERVICE(other)->result, SERVICE_SUCCESS, SERVICE_SKIP_CONDITION) &&
                            unit_active_state(other) == UNIT_FAILED) {
                                if (candidate != NULL) {
                                        log_unit_debug(UNIT(s), "multiple trigger source candidates for exit status propagation");
                                        return NULL;
