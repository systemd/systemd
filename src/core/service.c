...
static int service_spawn(
        ...
        if ((flags & EXEC_SETENV_RESULT) ^ (flags & EXEC_SETENV_MONITOR_RESULT)) {
                ...
                static const char *monitor_prefix = "";
                Service *env_source;
                ...
                if (env_source) {
                        if (asprintf(our_env + n_env++, "%sSERVICE_RESULT=%s", monitor_prefix,
                        ...
                        if (env_source->main_exec_status.pid > 0 &&
                            dual_timestamp_is_set(&env_source->main_exec_status.exit_timestamp)) {
                                if (asprintf(our_env + n_env++, "%sEXIT_CODE=%s", monitor_prefix,
                                             sigchld_code_to_string(env_source->main_exec_status.code)) < 0)
                                        return -ENOMEM;

                                if (env_source->main_exec_status.code == CLD_EXITED)
                                        r = asprintf(our_env + n_env++, "%sEXIT_STATUS=%i", monitor_prefix,
                                                     env_source->main_exec_status.status);
                                else
                                        r = asprintf(our_env + n_env++, "%sEXIT_STATUS=%s", monitor_prefix,
