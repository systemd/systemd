...
static int connect_unix_probe(Service *s, OpenFile *of, int ofd) {
...
static int get_open_file_fd(Service *s, OpenFile *of) {
        struct stat st;
        _cleanup_close_ int fd = -1;
...
static int service_collect_fds(
        ...
        if (s->open_files) {
                ...
                LIST_FOREACH(open_files, f, s->open_files) {
                        ...
                        if (fd < 0) {
                                if (FLAGS_SET(f->flags, OPENFILE_IGNORE))
                                        continue;
