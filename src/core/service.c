...
static int connect_unix_harder(Service *s, const OpenFile *of, int ofd) {
        ...
        return log_unit_error_errno(
                        UNIT(s), SYNTHETIC_ERRNO(EINVAL), "Failed to connect socket for \"%s\".", of->path);
}
...
static int get_open_file_fd(Service *s, const OpenFile *of) {
        ...
        if (S_ISSOCK(st.st_mode)) {
                fd = connect_unix_harder(s, of, ofd);
                if (fd < 0)
                        return fd;
