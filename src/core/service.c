...
static void service_enter_running(Service *s, ServiceResult f) {
        ...
        if (s->result != SERVICE_SUCCESS)
                ...
                log_unit_debug(UNIT(s), "D-Bus name %s not currently owned, deferring stop for grace period.", s->bus_name);

                if (s->state != SERVICE_RUNNING) {
                        service_set_state(s, SERVICE_RUNNING);

                        /* We might be coming from a non-running state here, in which case the timer is still
                         * armed for that state's timeout. Re-arm it so a stale deadline can't fire mid-grace
                         * and stop us, and so RuntimeMaxSec= still applies. */
                        r = service_arm_timer(s, /* relative= */ false, service_running_timeout(s));
                        if (r < 0) {
                                log_unit_warning_errno(UNIT(s), r, "Failed to install timer: %m");
