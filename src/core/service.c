...
static void service_dump(Unit *u, FILE *f, const char *prefix) {
        ...
        if (s->open_files) {
                LIST_FOREACH(open_files, of, s->open_files) {
                        _cleanup_free_ const char *of_str = open_file_to_string(of);
...
static int connect_to_socket(Service *s, OpenFile *f, int o_path_fd, int *ret) {
        _cleanup_close_ int fd = -1;
        union sockaddr_union addr;
        socklen_t sa_len;
        int r;

        r = sockaddr_un_set_path(&addr.un, FORMAT_PROC_FD_PATH(o_path_fd));
        if (r < 0) {
                return log_unit_error_errno(UNIT(s), r, "Failed to fill sockaddr for %s: %m", f->path);
        }
        ...
        if (fd < 0) {
                return log_unit_error_errno(UNIT(s), errno, "Failed to create socket for %s: %m", f->path);
        }

        if (connect(fd, &addr.sa, sa_len) >= 0) {
                *ret = TAKE_FD(fd);
                return 0;
        }

        log_unit_debug_errno(UNIT(s), errno, "Failed to connect to socket sun_path %s sun_family %d using SOCK_DGRAM socket type: %m", addr.un.sun_path, addr.un.sun_family);
        ...
        log_unit_debug_errno(UNIT(s), errno, "Failed to connect to socket sun_path %s sun_family %d using SOCK_STREAM socket type: %m", addr.un.sun_path, addr.un.sun_family);
        ...
        log_unit_debug_errno(UNIT(s), errno, "Failed to connect to socket sun_path %s sun_family %d using SOCK_SEQPACKET socket type: %m", addr.un.sun_path, addr.un.sun_family);
...
static int get_open_file_fd(Service *s, OpenFile *f) {
        struct stat st;
        _cleanup_close_ int fd = -1;
        int r;

        int o_path_fd = open(f->path, O_PATH);
        if (fstat(o_path_fd, &st) < 0)
                return log_error_errno(errno, "Failed to stat %s: %m", f->path);

        if (S_ISSOCK(st.st_mode)) {
                r = connect_to_socket(s, f, o_path_fd, &fd);
                if (r < 0)
                        return r;

                log_unit_debug(UNIT(s), "socket %s opened (fd=%d)", f->path, fd);
        } else {
                fd = fd_reopen(o_path_fd, (FLAGS_SET(f->flags, OPENFILE_RDONLY) ? O_RDONLY : O_RDWR)|O_CLOEXEC);
                if (fd < 0)
                        return log_unit_error_errno(UNIT(s), errno, "Failed to open file %s: %m", f->path);

                log_unit_debug(UNIT(s), "file %s opened (fd=%d)", f->path, fd);
        }
        return TAKE_FD(fd);
}
