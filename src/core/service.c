...
static int service_verify(Service *s) {
        ...
        if (s->refresh_on_reload_set && FLAGS_SET(s->refresh_on_reload_flags, SERVICE_RELOAD_EXTENSIONS))
                service_can_reload_extensions(s, /* warn = */ true);

        if (s->refresh_on_reload_set && FLAGS_SET(s->refresh_on_reload_flags, SERVICE_RELOAD_CREDENTIALS) &&
