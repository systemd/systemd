...
static int service_can_live_mount(Unit *u, const char *src, const char *dst, sd_bus_error *error) {
        ...
        if (mount_point_is_credentials(u->manager->prefix[EXEC_DIRECTORY_RUNTIME], dst)) {
                log_unit_debug(u, "Refusing to live mount over credential mount '%s'", dst);
                return sd_bus_error_setf(
                                error,
                                SD_BUS_ERROR_INVALID_ARGS,
                                "Live mounting '%s' on '%s' for unit '%s' cannot be queued: cannot mount over credential mount",
                                src,
                                dst,
                                u->id);
        }
        ...
        if (path_startswith_strv(dst, c->inaccessible_paths)) {
                log_unit_debug(u, "%s is not accessible to this unit, cannot live mount", dst);
                return sd_bus_error_setf(
                                error,
                                SD_BUS_ERROR_INVALID_ARGS,
                                "Live mounting '%s' on '%s' for unit '%s' cannot be queued: destination is not accessible to this unit",
                                src,
                                dst,
                                u->id);
        }
        ...
        if (!exec_needs_mount_namespace(c, NULL, unit_get_exec_runtime(u))) {
                ...
                return sd_bus_error_setf(
                                error,
                                SD_BUS_ERROR_INVALID_ARGS,
                                "Live mounting '%s' on '%s' for unit '%s' cannot be queued: unit not running in private mount namespace",
                                src,
                                dst,
                                u->id);
        }
