...
static int service_spawn(
        ...
        if (flags & EXEC_SETENV_RESULT) {
                r = asprintf(our_env + n_env, "SERVICE_RESULT=%s", service_result_to_string(s->result));
                if (r < 0)
                        return -ENOMEM;
                ...
                if (s->main_exec_status.pid > 0 && dual_timestamp_is_set(&s->main_exec_status.exit_timestamp)) {
                        r = asprintf(our_env + n_env, "EXIT_CODE=%s", sigchld_code_to_string(s->main_exec_status.code));
                        if (r < 0)
                                return -ENOMEM;
                }
