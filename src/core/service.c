...
static ServiceFDStore* service_fd_store_unlink_full(ServiceFDStore *fs, bool propagate_upstream) {
        ...
        if (fs->service) {
                ...
                if (propagate_upstream && fs->index > 0) {
                        (void) notify_remove_fd_warnf("%" PRIu64, fs->index);
                        fs->index = 0;
...
int service_add_fd_store(Service *s, int fd_in, const char *name, bool do_poll, bool propagate_upstream) {
        ...
        *fs = (ServiceFDStore) {
        ...
        if (propagate_upstream && s->fd_store_preserve_mode == EXEC_PRESERVE_YES) {
                ...
                char idx_str[DECIMAL_STR_MAX(uint64_t)];

                xsprintf(idx_str, "%" PRIu64, idx);
...
int service_propagate_fd_store_mapping_upstream(Manager *m) {
        ...
        if (!root) {
                 ...
                 * one in the past, otherwise we might inadvertently remove a mapping that was just
                 * handed back to us via LISTEN_FDS during a fresh manager startup. */
                if (m->fd_store_upstream_mapping_pushed) {
                        (void) notify_remove_fd_warn(SERVICE_FDSTORE_MAPPING_FDNAME);
                        m->fd_store_upstream_mapping_pushed = false;
                }
                return 0;
        }
        ...
        m->fd_store_upstream_mapping_pushed = true;
        return 0;
}
