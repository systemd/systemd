...
const sd_bus_vtable bus_automount_vtable[] = {
        ...
        SD_BUS_PROPERTY("Options", "s", NULL, offsetof(Automount, options), SD_BUS_VTABLE_PROPERTY_CONST),
...
static int bus_automount_set_transient_property(
        ...
        if (streq(name, "Options"))
                return bus_set_transient_string(u, name, &a->options, message, flags, error);
