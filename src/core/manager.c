...
static int manager_dispatch_pidref_transport_fd(sd_event_source *source, int fd, uint32_t revents, void *userdata) {
        ...
        if (child_pidfd < 0) {
                log_warning("Received pidref message without valid child PIDFD. Ignoring.");
                return 0;
        }
        ...
        if (child_pidref.pid != child_pid) {
                log_warning("Got SCM_PIDFD for child process " PID_FMT " but PID in IOV message is " PID_FMT ". Ignoring.",
        ...
        if (!u) {
                log_debug("Got pidref for process " PID_FMT " we are not interested in, ignoring.", parent_pidref.pid);
                return 0;
        }
