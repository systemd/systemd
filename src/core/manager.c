...
int manager_spawn_workers(Manager *m) {
        ...
        if (IN_SET(manager_state(m), MANAGER_STARTING, MANAGER_INITIALIZING))
                n_workers = 50;
        else
                n_workers = 10;
        ...
        for (size_t i = 0; i < n_workers; ++i) {
                ...
                r = posix_spawn_wrapper(m->executor_path,
                                STRV_MAKE(m->executor_path,
                                        ...
                                        "--log-target", log_target_to_string(manager_journal_is_running(m) ? log_get_target() : LOG_TARGET_KMSG)),
                                environ,
                                &pid);
                if (r < 0)
