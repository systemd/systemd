...
int manager_new(RuntimeScope runtime_scope, ManagerTestRunFlags test_run_flags, Manager **ret) {
        ...
        *m = (Manager) {
        ...
        /* sd_event_add_defer() creates an enabled one-shot source. Keep it armed so the first dispatch
         * establishes the initial system state baseline. */
...
void manager_reset_failed(Manager *m) {
        ...
        if (!set_isempty(m->transactions_with_cycle))
