...
static int manager_distribute_listen_fds_named(Manager *m, Hashmap *named_listen_fds) {
         ...
         * unit's fd store. The hashmap is built and owned by main.c's collect_fds(); we steal
         * entries here so any leftover (skipped) entries are still cleaned up by the hashmap's
         * destructor on the caller side. */

        if (MANAGER_IS_TEST_RUN(m))
                return 0;

        for (;;) {
                _cleanup_free_ char *name = NULL, *unit_id = NULL;
                _cleanup_close_ int fd = -EBADF;
                void *key;
                const char *p;
                int r;

                name = hashmap_steal_first_key_and_value(named_listen_fds, &key);
                if (!name)
                        break;
                ...
                (void) manager_dispatch_external_fd_to_unit(m, unit_id, p, TAKE_FD(fd), "LISTEN_FDS");
        }
