...
int manager_dispatch_external_fd_to_unit(
        ...
        if (r <= 0 && u->load_state == UNIT_NOT_FOUND)
                /* The fd was not actually stored (failure or duplicate), roll back */
                s->n_fd_store_max--;
        if (r < 0)
...
static void manager_distribute_listen_fds_named(Manager *m, FDSet *fds) {
         ...
         * form "unit-id|fdname" is meant to be restored into a specific unit's fd store, mirroring what we
        ...
        if (!e)
                return; /* no names and no tagged fds: nothing to do here */

        r = strv_split_full(&names, e, ":", EXTRACT_DONT_COALESCE_SEPARATORS);
        if (r < 0 || strv_length(names) != (size_t) n_fds)
                return;
