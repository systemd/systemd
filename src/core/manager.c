...
Manager* manager_free(Manager *m) {
        ...
        for (UnitMountDependencyType t = 0; t < _UNIT_MOUNT_DEPENDENCY_TYPE_MAX; ++t) {
                assert(hashmap_isempty(m->units_needing_mounts_for[t]));
                hashmap_free(m->units_needing_mounts_for[t]);
        }
...
Set* manager_get_units_needing_mounts_for(Manager *m, const char *path, UnitMountDependencyType t) {
        assert(m);
        assert(path);
        assert(t >= UNIT_MOUNT_WANTS && t < _UNIT_MOUNT_DEPENDENCY_TYPE_MAX);
