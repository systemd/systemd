...
static int manager_setup_notify_pam_pid(Manager *m) {
        ...
        if (m->notify_pam_pid_fds[0] < 0) {
                ...
                r = fd_increase_rxbuf(socket[0], NOTIFY_RCVBUF_SIZE);
                if (r < 0)
                        log_debug_errno(r, "Failed to increase receive buffer size for notifying PAM pid: %m");
...
static int manager_dispatch_notify_pam_pid_fd(sd_event_source *source, int fd, uint32_t revents, void *userdata) {
         ...
         * duplicate units make sure we only invoke each unit's handler once. */
        if (u1) {
                unit_watch_pam_pidref(u1, &parent_pidref, &pam_pidref);
                found = true;
        }
        if (u2) {
                unit_watch_pam_pidref(u2, &parent_pidref, &pam_pidref);
                found = true;
        }
        if (array_copy)
                for (size_t i = 0; array_copy[i]; i++) {
                        unit_watch_pam_pidref(array_copy[i], &parent_pidref, &pam_pidref);
                        found = true;
                }

        if (!found)
                log_warning("Cannot find unit for notifying PAM pid message of PID "PID_FMT", ignoring.", ucred->pid);
