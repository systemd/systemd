...
static int manager_run_generators(Manager *m) {
         ...
         * we are the user manager, let's just execute the generators directly. We might now have the
         * necessary privileges, and the system manager has already mounted /tmp/ and everything else for us.
         */
        if (MANAGER_IS_USER(m)) {
                r = manager_execute_generators(m, paths, false);
                goto finish;
        }
        ...
        r = safe_fork("(sd-gens)",
                      ...
                      NULL);
        if (r == 0) {
                r = manager_execute_generators(m, paths, true);
