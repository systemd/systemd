...
static void manager_ready(Manager *m) {
        ...
        (void) touch_file("/run/systemd/systemd-units-load-start", false, m->units_reloading_start_monotonic_timestamp == 0
                ? m->units_reloading_start_monotonic_timestamp
                : now(CLOCK_REALTIME), UID_INVALID, GID_INVALID, 0444);
...
Manager* manager_reloading_start(Manager *m) {
        m->n_reloading++;
        m->units_reloading_start_monotonic_timestamp = now(CLOCK_REALTIME);
        return m;
}
