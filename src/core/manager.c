...
int manager_new(RuntimeScope runtime_scope, ManagerTestRunFlags test_run_flags, Manager **ret) {
        ...
        *m = (Manager) {
                ...
                .restrict_fs_prog_fd = -EBADF,
                .restrict_fs_link_fd = -EBADF,
...
static int manager_setup_restrict_fs_bpf(Manager *m) {
        ...
        /* Release the previous instance of the LSM BPF prog, if any. */
        m->restrict_fs_map_fd = safe_close(m->restrict_fs_map_fd);
        m->restrict_fs_link_fd = safe_close(m->restrict_fs_link_fd);
        m->restrict_fs_prog_fd = safe_close(m->restrict_fs_prog_fd);
#endif
        return 0;
}
