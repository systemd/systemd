...
static int property_get_exec_dir(
        ...
        FOREACH_ARRAY(i, d->items, d->n_items) {
                /* Returned by property_get_exec_dir_symlink() to represent flags too */
                if ((i->flags & _EXEC_DIRECTORY_PUBLIC) != 0)
                        continue;
...
static int property_get_exec_dir_symlink(
        ...
        FOREACH_ARRAY(i, d->items, d->n_items) {
                 ...
                 * destination */
                if ((i->flags & _EXEC_DIRECTORY_PUBLIC) != 0 && !strv_isempty(i->symlinks)) {
                        r = sd_bus_message_append(reply, "(sst)", i->path, "", (uint64_t) (i->flags & _EXEC_DIRECTORY_PUBLIC));
                        if (r < 0)
                                return r;
                }
                STRV_FOREACH(dst, i->symlinks) {
                        r = sd_bus_message_append(reply, "(sst)", i->path, *dst, (uint64_t) (i->flags & _EXEC_DIRECTORY_PUBLIC));
                        if (r < 0)
                                return r;
                }
        }
...
int bus_exec_context_set_transient_property(
        ...
        if (STR_IN_SET(name, "CPUAffinity", "NUMAMask")) {
                ...
                while ((r = sd_bus_message_read(message, "(sst)", &source, &destination, &symlink_flags)) > 0) {
                        if ((symlink_flags & ~_EXEC_DIRECTORY_PUBLIC) != 0)
                        ...
                        if (!path_is_normalized(source))
                                return sd_bus_error_setf(error, SD_BUS_ERROR_INVALID_ARGS, "Source path %s is not normalized.", source);
                        /* The message will not have a 'destination' field when we want to specify a
                         * read-only exec directory, as the old D-Bus method does not support flags */
                        if (!FLAGS_SET(symlink_flags, EXEC_DIRECTORY_READ_ONLY) || !isempty(destination)) {
                        ...
                        if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                                ...
                                r = exec_directory_add(
                                                directory,
                                                source,
                                                isempty(destination) ? NULL : destination,
                                                FLAGS_SET(symlink_flags, EXEC_DIRECTORY_READ_ONLY));
                                if (r < 0)
                                        return r;
