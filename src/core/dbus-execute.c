...
static int property_get_protect_hostname_ex(
        ...
        s = strjoin(protect_hostname_to_string(c->protect_hostname), c->private_hostname ? ":" : NULL, c->private_hostname);
        if (!s)
                return -ENOMEM;
...
const sd_bus_vtable bus_exec_vtable[] = {
        ...
        SD_BUS_PROPERTY("ProtectHostnameEx", "s", property_get_protect_hostname_ex, 0, SD_BUS_VTABLE_PROPERTY_CONST),
...
int bus_exec_context_set_transient_property(
        ...
        if (streq(name, "ProtectHostnameEx")) {
                ...
                ProtectHostname t = protect_hostname_from_string(p ?: s);
                if (t < 0 || (t == PROTECT_HOSTNAME_NO && h))
                        return sd_bus_error_setf(error, SD_BUS_ERROR_INVALID_ARGS, "Invalid %s setting: %s", name, s);

                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                        c->protect_hostname = t;
                        r = free_and_strdup(&c->private_hostname, h);
                        if (r < 0)
                                return r;
