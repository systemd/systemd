...
static int property_get_log_filter_patterns(
        ...
        r = sd_bus_message_append_log_filter_patterns(reply, c->log_filter_allowed_patterns, true);
        if (r < 0)
                return r;

        r = sd_bus_message_append_log_filter_patterns(reply, c->log_filter_denied_patterns, false);
        if (r < 0)
                return r;
...
int bus_exec_context_set_transient_property(
        ...
        if (streq(name, "LogFilterPatterns")) {
                _cleanup_strv_free_ char **allow_list = NULL, **deny_list = NULL;
                ...
                while ((r = sd_bus_message_read(message, "(bs)", &is_allowlist, &pattern)) > 0) {
                        ...
                        r = strv_push(is_allowlist ? &allow_list : &deny_list, pattern);
                        if (r < 0)
                                return r;
                }
                if (r < 0)
