...
const sd_bus_vtable bus_exec_vtable[] = {
        ...
        SD_BUS_PROPERTY("ExecSearchPath", "s", NULL, offsetof(ExecContext, exec_search_path), SD_BUS_VTABLE_PROPERTY_CONST),
...
int bus_exec_context_set_transient_property(
        ...
        if (STR_IN_SET(name, "CPUAffinity", "NUMAMask")) {
                ...
                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                        r = free_and_strdup(&c->exec_search_path, empty_to_null(s));
                        if (r < 0)
                                return r;
                        unit_write_settingf(u, flags|UNIT_ESCAPE_SPECIFIERS, name, "ExecSearchPath=%s%s", "", s);
                }
