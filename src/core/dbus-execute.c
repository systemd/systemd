...
static int bus_property_get_exec_dir_symlink(
        ...
        for (size_t i = 0; i < d->n_items; i++)
                STRV_FOREACH(dst, d->items[i].symlinks) {
                        r = sd_bus_message_append(reply, "(sst)", d->items[i].path, *dst, /* flags, unused for now */0);
                        if (r < 0)
                                return r;
                }
...
int bus_exec_context_set_transient_property(
        ...
        if (STR_IN_SET(name, "CPUAffinity", "NUMAMask")) {
                ...
                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                        ...
                        if (strv_isempty(l)) {
                                ...
                                STRV_FOREACH(source, l) {
                                        r = exec_directory_add(&d->items, &d->n_items,
                                                               &(ExecDirectoryItem) {
                                                                        .path = *source,
                                                               });
                                        if (r < 0)
                ...
                while ((r = sd_bus_message_read(message, "(sst)", &source, &destination, &symlink_flags)) > 0) {
                        ...
                        if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                                _cleanup_free_ char *destination_escaped = NULL, *source_escaped = NULL, *d = NULL;

                                for (size_t j = 0; j < directory->n_items; ++j)
                                        if (path_equal(source, directory->items[j].path))
                                                item = &directory->items[j];
                                if (!item)
                                        return sd_bus_error_setf(error, SD_BUS_ERROR_INVALID_ARGS, "Source path %s is not configured as a %s.", source, exec_directory_type_to_string(i));

                                d = strdup(destination);
                                if (!d)
                                        return -ENOMEM;

                                r = strv_consume(&item->symlinks, TAKE_PTR(d));
                                if (r < 0)
                                        return r;
