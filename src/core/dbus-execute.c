...
static int property_get_extension_images(
        ...
        r = sd_bus_message_open_container(reply, 'a', "(sbba(ss))");
        if (r < 0)
                return r;
        ...
        for (size_t i = 0; i < c->n_extension_images; i++) {
                ...
                r = sd_bus_message_open_container(reply, SD_BUS_TYPE_STRUCT, "sbba(ss)");
                if (r < 0)
                        return r;
                r = sd_bus_message_append(
                                reply, "sbb",
                                c->extension_images[i].source,
                                c->extension_images[i].type == MOUNT_IMAGE_SHALLOW_OVERLAY,
...
const sd_bus_vtable bus_exec_vtable[] = {
        ...
        SD_BUS_PROPERTY("ExtensionImages", "a(sbba(ss))", property_get_extension_images, 0, SD_BUS_VTABLE_PROPERTY_CONST),
...
int bus_exec_context_set_transient_property(
        ...
        if (STR_IN_SET(name, "CPUAffinity", "NUMAMask")) {
                ...
                r = sd_bus_message_enter_container(message, 'a', "(sbba(ss))");
                if (r < 0)
                        return r;
                ...
                for (;;) {
                        ...
                        char *source, *tuple;
                        int permissive, shallow;

                        r = sd_bus_message_enter_container(message, 'r', "sbba(ss)");
                        if (r < 0)
                                return r;

                        r = sd_bus_message_read(message, "sbb", &source, &permissive, &shallow);
                        if (r <= 0)
                                break;
                        ...
                        r = mount_image_add(&extension_images, &n_extension_images,
                                            &(MountImage) {
                                                    ...
                                                    .mount_options = options,
                                                    .ignore_enoent = permissive,
                                                    .type = shallow ? MOUNT_IMAGE_SHALLOW_OVERLAY : MOUNT_IMAGE_OVERLAY,
                                            });
                        if (r < 0)
                                return r;
                }
