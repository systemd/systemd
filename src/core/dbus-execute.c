...
static int property_get_bpf_delegate(
                sd_bus *bus,
                const char *path,
                const char *interface,
                const char *property,
        ...
        uint64_t *p = ASSERT_PTR(userdata);
        ...
        return sd_bus_message_append_basic(reply, 's', "any");
}
...
const sd_bus_vtable bus_exec_vtable[] = {
        ...
        SD_BUS_PROPERTY("BPFDelegateCommands", "s", property_get_bpf_delegate, offsetof(ExecContext, bpf_delegate_commands), SD_BUS_VTABLE_PROPERTY_CONST),
...
static BUS_DEFINE_SET_TRANSIENT_PARSE(bpf_delegate_commands, uint64_t, bpf_delegate_from_string);
