...
static int exec_context_tty_size(const ExecContext *context, unsigned *ret_rows, unsigned *ret_cols) {
        ...
        if (context->tty_rows == UINT_MAX) {
                ...
                key = strjoin("systemd.tty.", tty, ".rows");
                if (!key)
                        return -ENOMEM;

                r = proc_cmdline_get_key(key, 0, &value);
                if (r < 0)
                        log_debug_errno(r, "Failed to read %s from kernel cmdline, ignoring: %m", key);
                if (r > 0) {
                        r = safe_atou(value, &rows);
                        if (r == -EINVAL)
                                log_debug_errno(r, "Failed to parse %s=%s, ignoring", key, value);
                        else if (r < 0)
                                return r;
                }
        } else
        ...
        if (context->tty_cols == UINT_MAX) {
                ...
                key = strjoin("systemd.tty.", tty, ".columns");
                if (!key)
                        return -ENOMEM;
...
#define N_ENV_VARS 19
        ...
        if (exec_context_needs_term(c)) {
                ...
                if (path_equal_ptr(tty_path, "/dev/console") && getppid() == 1)
                        ...
                        key = strjoin("systemd.tty.", skip_dev_prefix(tty_path), ".term");
                        if (!key)
                                return -ENOMEM;
