...
#define N_ENV_VARS 19
        ...
        if (memory_pressure_path) {
                ...
                if (cgroup_context && !path_equal(memory_pressure_path, "/dev/null")) {
                        ...
                        x = strjoin("MEMORY_PRESSURE_WATCH=", e);
                        if (!x)
                                return -ENOMEM;
...
static int exec_child(
        ...
        if (params->cgroup_path && uid_is_valid(uid)) {
                ...
                if (cgroup_context) {
                        if (cgroup_context_want_memory_pressure(cgroup_context)) {
                                memory_pressure_path = path_join(params->cgroup_path, "memory.pressure");
                                if (!memory_pressure_path) {
                                        *exit_status = EXIT_MEMORY;
                                        return log_oom();
                                }

                                r = cg_set_access("memory", memory_pressure_path, uid, gid);
                                if (r < 0) {
