...
static int setup_exec_directory(
        ...
        for (size_t i = 0; i < context->directories[type].n_items; i++) {
                ...
                if (IN_SET(type, EXEC_DIRECTORY_STATE, EXEC_DIRECTORY_LOGS) && params->runtime_scope == RUNTIME_SCOPE_USER) {
                         ...
                         * situtation. Hence, create a compatibility symlink, so that all expectations are
                         * met.
                         *
                        ...
                        if (r == -ENOENT) {
                                ...
                                if (r >= 0) {
                                        ...
                                        log_unit_warning_errno(u, r, "Unable to detect whether unit configuration directory '%s' exists, ignoring: %m", q);

                        } else if (r < 0)
                                log_unit_warning_errno(u, r, "Unable to detect whether unit state directory '%s' exists, ignoring: %m", p);
                }
