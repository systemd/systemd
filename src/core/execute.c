...
#define N_ENV_VARS 17
        ...
        for (ExecDirectoryType t = 0; t < _EXEC_DIRECTORY_TYPE_MAX; t++) {
                ...
                for (size_t i = 0; i < c->directories[t].n_items; i++)
                        if (!strextend_with_separator(&joined, ":", pre, c->directories[t].items[i], NULL))
                                return -ENOMEM;
        ...
        for (ExecDirectoryType dt = 0; dt < _EXEC_DIRECTORY_TYPE_MAX; dt++) {
                for (size_t i = 0; i < context->directories[dt].n_items; i++) {
                        ...
                        STRV_FOREACH(symlink, context->directories[dt].items[i].symlinks) {
                                ...
                                symlink_abs = path_join(params->prefix[dt], *symlink);
                                if (!symlink_abs)
                                        return -ENOMEM;
                                r = strv_consume(&symlinks, TAKE_PTR(symlink_abs));
                                if (r < 0)
                                        return r;
                        }
                        ...
                        p = path_join(params->prefix[dt], context->directories[dt].items[i].path);
                        if (!p)
                                return -ENOMEM;

                        pp = path_join(params->prefix[dt], "private", context->directories[dt].items[i].path);
                        if (!pp)
                                return -ENOMEM;
...
void exec_context_done(ExecContext *c) {
        ...
        for (ExecDirectoryType t = 0; t < _EXEC_DIRECTORY_TYPE_MAX; t++)
                exec_directory_free(&c->directories[t]);
...
int exec_directory_add(ExecDirectoryItem **d, size_t *n, const ExecDirectoryItem *item) {
        ...
        if (item->symlinks) {
                symlinks = strv_copy(item->symlinks);
                if (!symlinks)
                        return -ENOMEM;
        }
        ...
        *d = c;
        ...
        c[(*n) ++] = (ExecDirectoryItem) {
                .path = TAKE_PTR(path),
                .symlinks = TAKE_PTR(symlinks),
        };
