        ...
        if (exec_needs_network_namespace(context) && runtime && runtime->shared && runtime->shared->netns_storage_socket[0] >= 0) {
                ...
                if (ns_type_supported(NAMESPACE_NET) && have_effective_cap(CAP_NET_ADMIN) != 0) {
                        ...
                        if (r < 0) {
                                if (ERRNO_IS_PRIVILEGE(r))
                                        log_unit_notice_errno(unit, r,
                                                               "PrivateNetwork=yes is configured, but network namespace setup not permitted, proceeding wthout: %m");
                                else {
                                        *exit_status = EXIT_NETWORK;
                        ...
                        *exit_status = EXIT_NETWORK;
                        ...
                        log_unit_notice(unit, "PrivateNetwork=yes is configured, but the kernel or available privileges do not support or permit network namespaces, proceeding without.");
        }
