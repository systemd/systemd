...
const char* exec_get_effective_notify_socket_path(const ExecContext *context, const ExecParameters *params, bool needs_sandboxing) {
        return params->notify_socket && needs_sandboxing && exec_context_get_effective_mount_apivfs(context) && (context->root_directory || context->root_image) ? "/run/host/notify" : NULL;
}
