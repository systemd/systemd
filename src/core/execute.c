...
int exec_spawn(Unit *unit,
        ...
        if (params->cgroup_path) {
                ...
                if (r > 0) { /* We are using a child cgroup */
                        ...
                        r = cgroup_log_xattr_apply(unit, subcgroup_path);
                        if (r != 0)
                                (void) log_unit_warning_errno(unit, r, "Failed to set user.journald_log_filter_patterns cgroup xattr, ignoring: %m");
                }
        }
