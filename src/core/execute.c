...
int exec_context_apply_tty_size(
        ...
        /* If we got nothing and we are talking to a physical device, and tty reset is one, then let's query
         * things from the ANSI driver */
...
void exec_context_tty_reset(const ExecContext *context, const ExecParameters *p) {
         ...
         * potentially invalidate the TTY here through hangups or VT disallowcations, and hence do not keep a
         * continous fd open. */
