...
void exec_context_dump(const ExecContext *c, FILE* f, const char *prefix) {
        ...
        for (size_t i = 0; i < c->n_extension_images; i++) {
                ...
                fprintf(f, "%sExtensionImages: %s%s:%s%s", prefix,
                        c->extension_images[i].ignore_enoent ? "-": "",
                        c->extension_images[i].source,
                        c->extension_images[i].type == MOUNT_IMAGE_OVERLAY ? "overlay" : "shallow",
                        LIST_IS_EMPTY(c->extension_images[i].mount_options) ? "": ":");
