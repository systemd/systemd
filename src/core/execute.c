...
static int apply_mount_namespace(
        ...
        if (context->scope == RUNTIME_SCOPE_SYSTEM) {
...
int exec_child(
        ...
        if (setenv("SYSTEMD_ACTIVATION_UNIT", unit->id, true) != 0 ||
            setenv("SYSTEMD_ACTIVATION_SCOPE", runtime_scope_to_string(context->scope), true) != 0) {
                *exit_status = EXIT_MEMORY;
...
int exec_spawn(Unit *unit,
        ...
        cb_arg.executor_path = unit->manager->executor_path;
        /* If journald is not available tell sd-executor to go to kmsg, as it might be starting journald */
        cb_arg.log_target = log_target_to_string(manager_journal_is_running(unit->manager) ? log_get_target() : LOG_TARGET_KMSG);
        cb_arg.log_level = log_level;

        pid = clone_vm_vfork(clone_cb, SIGCHLD, &cb_arg);
        if (pid < 0)
                return log_unit_error_errno(unit, pid, "Failed to clone: %m");
