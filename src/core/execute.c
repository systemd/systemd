                ...
                if (IN_SET(type, EXEC_DIRECTORY_STATE, EXEC_DIRECTORY_LOGS) && params->runtime_scope == RUNTIME_SCOPE_USER) {
                        ...
                        if (r == -ENOENT) {
                                ...
                                if (r >= 0) {
                                        ...
                                        log_warning_errno(r, "Unable to detect whether unit configuration directory '%s' exists, ignoring: %m", q);

                        } else if (r < 0)
                                log_warning_errno(r, "Unable to detect whether unit state directory '%s' exists, ignoring: %m", p);
                }
