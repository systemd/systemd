...
int exec_child(
        ...
        r = close_remaining_fds(params, runtime, params->user_lookup_fd, socket_fd, keep_fds, n_keep_fds);
        if (r < 0) {
                *exit_status = EXIT_FDS;
        ...
        if (setenv("SYSTEMD_ACTIVATION_UNIT", unit->id, true) != 0 ||
            setenv("SYSTEMD_ACTIVATION_SCOPE", runtime_scope_to_string(context->context_is_system ? RUNTIME_SCOPE_SYSTEM : RUNTIME_SCOPE_USER), true) != 0) {
                *exit_status = EXIT_MEMORY;
        ...
        safe_close(params->user_lookup_fd);
...
int exec_spawn(Unit *unit,
        ...
        _cleanup_close_ int executor_fd = -EBADF, serialization_fd = -EBADF;
        ...
        if (stack_size == 0) {
                ...
                if (r < 0)
                        return log_unit_error_errno(unit, errno, "Failed to get RLIMIT_STACK: %m");

                stack_size = MIN(rl.rlim_cur, page_size());
        }
        ...
        stack = mmap(NULL,
                     ...
                     /* offset= */ 0);
        if (stack == MAP_FAILED)
                return log_unit_error_errno(unit, errno, "Failed to allocate stack: %m");

        pid = clone(clone_cb, stack + stack_size, CLONE_VFORK|CLONE_VM|SIGCHLD, &cb_arg);
        if (pid < 0)
