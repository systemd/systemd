...
int exec_spawn(
        ...
        /* For delegated units, a previous run of the service may have enabled domain controllers
         * (e.g. "pids") in the service cgroup's cgroup.subtree_control via a cgroup namespace root
         * write. When the process exits the sub-cgroup stays and the stale controllers remain. On
         * restart, clone3(CLONE_INTO_CGROUP) targeting the service cgroup then fails with EBUSY because
         * the cgroup would become an "internal" node with a domain controller enabled. Proactively clear
         * all controllers from subtree_control before spawning the main process. Only do this for the
         * main process spawn (not control process spawns), because control processes are already placed
         * into a fresh subcgroup. */
        if (cgtarget && FLAGS_SET(params->flags, EXEC_CGROUP_DELEGATE) && !exec_params_needs_control_subcgroup(params)) {
                r = cg_enable(unit->manager->cgroup_supported, /* mask= */ 0, cgtarget, /* ret_result_mask= */ NULL);
                if (r < 0)
                        log_unit_debug_errno(unit, r, "Failed to clear subtree_control on %s before spawn, ignoring: %m", cgtarget);
        }
