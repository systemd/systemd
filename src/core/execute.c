...
static void exec_context_tty_reset(const ExecContext *context, const ExecParameters *p) {
         ...
         * We open a new fd that will be closed automatically, and we operate on the new fd for convenience.
         */

        if (p && p->stdin_fd >= 0)
                fd = lock_generic_new_fd(p->stdin_fd, LOCK_BSD, LOCK_EX);
        else if (path) {
        ...
        if (context->tty_vhangup && fd >= 0)
                (void) terminal_vhangup_fd(fd);

        if (context->tty_reset && fd >= 0)
