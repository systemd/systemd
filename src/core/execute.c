...
static int connect_unix_harder(Unit *u, const OpenFile *of, int ofd) {
        ...
        return log_unit_error_errno(u, SYNTHETIC_ERRNO(EINVAL), "Failed to connect socket for \"%s\".", of->path);
}
...
static int collect_open_file_fds(
        ...
        if (open_files) {
                LIST_FOREACH(open_files, f, open_files) {
                        _cleanup_close_ int fd = -EBADF;

                        fd = get_open_file_fd(u, f);
                        if (fd < 0) {
                        ...
                        r = strv_extend(fdnames, f->fdname);
                        if (r < 0)
                                return r;
