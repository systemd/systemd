...
static int load_cred_recurse_dir_cb(
        ...
        _cleanup_free_ char *cred = NULL, *credname = NULL, *sub_id = NULL;
        ...
        /* create the credential ID by taking the path relative to the parent
         * directory, so e.g. if we pass foo:dir/ which contains bar/baz, a
         * credential called `foo_bar_baz` will be created */
        r = path_make_relative(args->parent_lc->path, path, &cred);
        if (r < 0)
                return r;

        credname = strreplace(cred, "/", "_");
        if (!credname)
                return -ENOMEM;

        sub_id = strjoin(args->parent_lc->id, "_", credname);
        if (!sub_id)
                return -ENOMEM;
        ...
        r = load_credential(args->context, args->params,
                &(ExecLoadCredential) {
                        .id = sub_id,
                        .path = (char *) de->d_name,
                        .encrypted = args->parent_lc->encrypted,
...
static int acquire_credentials(
        ...
        seen_creds = set_new(&string_hash_ops);
        if (!seen_creds)
                return -ENOMEM;
