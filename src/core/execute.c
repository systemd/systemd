...
int exec_spawn(Unit *unit,
        ...
        if (fds_array) {
                ...
                if (r <= 0) {
                        ...
                        if (socketpair(AF_UNIX, SOCK_DGRAM|SOCK_CLOEXEC, 0, unit->manager->executors_pool_socket) < 0)
                        ...
                        if (ftruncate(fileno(f), 0) < 0)
                                return log_unit_error_errno(unit, errno, "Failed to truncate serialization stream: %m");
