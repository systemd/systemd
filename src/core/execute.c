...
void exec_context_revert_tty(ExecContext *c) {
        ...
        if (fd < 0) {
                log_full_errno(errno == ENOENT ? LOG_DEBUG : LOG_WARNING, errno,
                               "Failed to open TTY inode of '%s' for adusting ownership/access mode, ignoring: %m", path);
                return;
        }

        if (fstat(fd, &st) < 0) {
                log_warning_errno(errno, "Failed to stat TTY '%s', ignoring: %m", path);
                return;
        }
         ...
         * use isatty(), but we'd rather not do that ince opening TTYs comes with all kinds of side-effects
        ...
        if (!S_ISCHR(st.st_mode)) {
                log_warning("Configured TTY '%s' is not actually a character device, ignoring.", path);
                return;
        }
