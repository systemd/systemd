...
static int exec_parameters_get_cgroup_path(Unit *u, const ExecParameters *params, char **ret) {
        ...
        if (cg_unified_controller(SYSTEMD_CGROUP_CONTROLLER) < 1)
                subcgroup = NULL;
        else if (!FLAGS_SET(params->flags, EXEC_CONTROL_CGROUP|EXEC_CGROUP_DELEGATE))
                subcgroup = NULL;
        else if (FLAGS_SET(params->flags, EXEC_IS_CONTROL))
                subcgroup = c->delegate_path_control ?: DELEGATE_CGROUP_CONTROL;
        else
                subcgroup = c->delegate_path_payload ?: DELEGATE_CGROUP_PAYLOAD;
        ...
        if (params->cgroup_path) {
                ...
                if (r > 0) { /* We are using a child cgroup */
                        ...
                        if (FLAGS_SET(params->flags, EXEC_IS_CONTROL)) {
                                ...
                                /* Payload wrap inherits all controllers configured for unit's cgroup.
                                 * Supports only one level of payload wrap. */
                                r = cg_enable_everywhere(unit->manager->cgroup_supported,
                                                         unit->cgroup_realized_mask,
                                                         params->cgroup_path, NULL);
                                if (r < 0)
                                        return log_unit_error_errno(unit, r, "Failed to enable cgroup controllers in '%s': %m",
                                                                    subcgroup_path);
                        }
