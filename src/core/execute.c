...
static int exec_shared_runtime_make(
        ...
        r = setup_tmp_dirs(id,
                           c->private_tmp == PRIVATE_TMP_CONNECTED && !prefixed_path_strv_contains(c->inaccessible_paths, "/tmp") ? &tmp_dir : NULL,
                           c->private_var_tmp == PRIVATE_TMP_CONNECTED &&
                           !prefixed_path_strv_contains(c->inaccessible_paths, "/var/tmp") &&
                           !prefixed_path_strv_contains(c->inaccessible_paths, "/var") ? &var_tmp_dir : NULL);
        if (r < 0)
                return r;
