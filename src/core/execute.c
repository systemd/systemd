...
static int exec_context_tty_size(const ExecContext *context, unsigned *ret_rows, unsigned *ret_cols) {
        ...
        rowskey = strjoin("systemd.tty.", tty, ".rows");
        if (!rowskey)
                return -ENOMEM;

        colskey = strjoin("systemd.tty.", tty, ".columns");
        if (!colskey)
                return -ENOMEM;

        r = proc_cmdline_get_key_many(0,
                                      rowskey, &rowsvalue,
                                      colskey, &colsvalue);
        if (r < 0)
                log_debug_errno(r, "Failed to read %s size from kernel cmdline, ignoring: %m", tty);
