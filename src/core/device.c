...
static int device_coldplug(Unit *u) {
        ...
        if (d->deserialized_state < 0 ||
            (d->deserialized_state == d->state && d->deserialized_found == d->found))
                return 0;
...
static int device_add_udev_wants(Unit *u, sd_device *dev) {
        ...
        for (;;) {
                ...
                if (r < 0)
                        return log_unit_error_errno(
                                u, r, "Failed to parse property %s with value %s: %m", property, wants);

                if (unit_name_is_valid(word, UNIT_NAME_TEMPLATE) && d->sysfs) {
                        _cleanup_free_ char *escaped = NULL;

                        /* If the unit name is specified as template, then automatically fill in the sysfs
                         * path of the device as instance name, properly escaped. */
                        ...
                        if (r < 0)
                                return log_unit_error_errno(
                                        u, r, "Failed to build %s instance of template %s: %m", escaped, word);
                } else {
        ...
        if (d->state != DEVICE_DEAD) {
                ...
                /* So here's a special hack, to compensate for the fact that the udev database's reload
                 * cycles are not synchronized with our own reload cycles: when we detect that the
                 * SYSTEMD_WANTS property of a device changes while the device unit is already up, let's
                 * manually trigger any new units listed in it not seen before. This typically appens during
                 * the boot-time switch root transition, as udev devices will generally already be up in the
                 * initrd, but SYSTEMD_WANTS properties get then added through udev rules only available on
                 * the host system, and thus only when the initial udev coldplug trigger runs.
                 *
                 * We do this only if the device has been up already when we parse this, as otherwise the
                 * usual dependency logic that is run from the dead → plugged transition will trigger these
                 * deps. */
                ...
                STRV_FOREACH (i, added) {
                        ...
                        if (r < 0)
                                log_unit_warning_errno(u,
                                                       r,
                                                       "Failed to enqueue SYSTEMD_WANTS= job, ignoring: %s",
        ...
        if (sd_device_get_property_value(dev, "SYSTEMD_MOUNT_DEVICE_BOUND", &bound_by) >= 0) {
                ...
                if (r < 0)
                        log_device_warning_errno(
                                dev,
                                r,
                                "Failed to parse SYSTEMD_MOUNT_DEVICE_BOUND='%s' udev property, ignoring: %m",
                                bound_by);
...
static int device_setup_unit(Manager *m, sd_device *dev, const char *path, bool main) {
        ...
        if (u) {
                /* The device unit can still be present even if the device was unplugged: a mount unit can
                 * reference it hence preventing the GC to have garbaged it. That's desired since the device
                 * unit may have a dependency on the mount unit which was added during the loading of the
                 * later. When the device is plugged the sysfs might not be initialized yet, as we serialize
                 * the device's state but do not serialize the sysfs path across reloads/reexecs. Hence, when
                 * coming back from a reload/restart we might have the state valid, but not the sysfs path.
                 * Hence, let's filter out conflicting devices, but let's accept devices in any state with no
                 * sysfs path set. */
                ...
                /* Let's remove all dependencies generated due to udev properties. We'll readd whatever is
                 * configured now below. */
...
static int device_process_new(Manager *m, sd_device *dev) {
        ...
        if (sd_device_get_devnum(dev, &devnum) >= 0) {
                ...
                FOREACH_DEVICE_DEVLINK(dev, p) {
                        ...
                        if (stat(p, &st) >= 0 &&
                            ((!S_ISBLK(st.st_mode) && !S_ISCHR(st.st_mode)) || st.st_rdev != devnum))
                                continue;
        ...
        for (;;) {
                ...
                if (r < 0)
                        return log_device_warning_errno(
                                dev, r, "Failed to add parse SYSTEMD_ALIAS property: %m");
...
static void device_found_changed(Device *d, DeviceFound previous, DeviceFound now) {
        ...
        if (FLAGS_SET(now, DEVICE_FOUND_UDEV))
                ...
                /* If the device has not been seen by udev yet, but is now referenced by the kernel, then we
                 * assume the kernel knows it now, and udev might soon too. */
...
static void device_update_found_one(Device *d, DeviceFound found, DeviceFound mask) {
        ...
        if (MANAGER_IS_RUNNING(UNIT(d)->manager)) {
                ...
                /* We aren't running yet, let's apply the new mask to the shadow variable instead, which
                 * we'll apply as soon as we catch-up with the state. */
...
static int device_dispatch_io(sd_device_monitor *monitor, sd_device *dev, void *userdata) {
        ...
        if (streq(action, "remove")) {
                ...
                if (r < 0)
                        log_device_warning_errno(
                                dev, r, "Failed to process swap device remove event, ignoring: %m");

                /* If we get notified that a device was removed by
                 * udev, then it's completely gone, hence unset all
                 * found bits */
                device_update_found_by_sysfs(
                        m, sysfs, 0, DEVICE_FOUND_UDEV | DEVICE_FOUND_MOUNT | DEVICE_FOUND_SWAP);
                ...
                if (r < 0)
                        log_device_warning_errno(
                                dev, r, "Failed to process swap device new event, ignoring: %m");
                ...
                if (r == -ENOENT) {
                        ...
                        *ret = NULL;
                        ...
                        return log_error_errno(r,
                                               "Failed to get udev device from devnum %u:%u: %m",
                                               major(st.st_rdev),
                                               minor(st.st_rdev));
...
void device_found_node(Manager *m, const char *node, DeviceFound found, DeviceFound mask) {
         ...
         *
         * This takes a pair of DeviceFound flags parameters. The 'mask' parameter is a bit mask that
         * indicates which bits of 'found' to copy into the per-device DeviceFound flags field. Thus, this
         * function may be used to set and unset individual bits in a single call, while merging partially
         * with previous state. */

        if ((found & mask) != 0) {
                _cleanup_(sd_device_unrefp) sd_device *dev = NULL;

                /* If the device is known in the kernel and newly appeared, then we'll create a device unit
                 * for it, under the name referenced in /proc/swaps or /proc/self/mountinfo. But first, let's
                 * validate if everything is alright with the device node. */
