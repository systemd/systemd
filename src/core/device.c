...
static void device_update_found_one(Device *d, DeviceFound found, DeviceFound mask, bool from_catchup) {
        assert(d);
        ...
        if (MANAGER_IS_RUNNING(UNIT(d)->manager)) {
                ...
                /* Normally, if the resulting found mask is unchanged there is nothing to do. However, there
                 * is one important exception: on switch-root, device_coldplug() intentionally leaves a device
                 * in the inconsistent combination of found == DEVICE_NOT_FOUND and state == DEVICE_TENTATIVE,
                 * to keep a previously plugged device in the 'activating' state until udevd reprocesses it,
                 * thus avoiding a spurious active → dead → active transition (see #12953 and #23208). While the
                 * device sits in that state its found mask is already empty, so a later uevent that clears the
                 * udev bit — e.g. the 'move' event of a renamed interface, whose old sysfs name is dropped via
                 * device_remove_old_on_move(), or a 'remove' event — computes n == d->found == DEVICE_NOT_FOUND
                 * and would be skipped here, leaving the unit stuck in DEVICE_TENTATIVE forever (see #43767,
                 * e.g. a virtio interface renamed eth0 → enp0s4 after switch-root). Hence, when the resulting
                 * mask is empty we must not short-cut, but re-evaluate the state so the device can finally
                 * settle to DEVICE_DEAD. The only caller that legitimately wants to preserve the
                 * empty-found/tentative combination is device_catchup(), which performs the switch-root hold
                 * described above; it passes from_catchup=true to opt out of this re-evaluation. */
                if (n == d->found && (from_catchup || n != DEVICE_NOT_FOUND))
                        return;
...
static void device_update_found_by_sysfs(Manager *m, const char *sysfs, DeviceFound found, DeviceFound mask) {
        ...
        LIST_FOREACH(same_sysfs, d, l)
                device_update_found_one(d, found, mask, /* from_catchup= */ false);
}
...
static void device_update_found_by_name(Manager *m, const char *path, DeviceFound found, DeviceFound mask) {
        ...
        device_update_found_one(DEVICE(u), found, mask, /* from_catchup= */ false);
}
...
static void device_catchup(Unit *u) {
        ...
        device_update_found_one(d, d->enumerated_found, _DEVICE_FOUND_MASK, /* from_catchup= */ true);
}
...
static void device_enumerate(Manager *m) {
        ...
        FOREACH_DEVICE(e, dev) {
                ...
                SET_FOREACH(d, ready_units) {
                        device_update_found_one(d, DEVICE_FOUND_UDEV, DEVICE_FOUND_UDEV, /* from_catchup= */ false);
                ...
                SET_FOREACH(d, not_ready_units)
                        device_update_found_one(d, DEVICE_NOT_FOUND, DEVICE_FOUND_UDEV, /* from_catchup= */ false);
        }
