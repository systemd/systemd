...
static void device_process_new(Manager *m, sd_device *dev, const char *sysfs) {
        ...
        if (r < 0) {
                if (r != -ENOENT)
                        log_device_debug_errno(dev, r, "Failed to get SYSTEMD_ALIAS property, ignoring: %m");
                return;
        }
...
static bool device_is_ready(sd_device *dev) {
        ...
        if (r < 0)
                log_device_debug_errno(dev, r, "Failed to check if device is renaming: %m");
        if (r == 0)
        ...
        if (r < 0)
                log_device_debug_errno(dev, r, "Failed to check if device has \"systemd\" tag: %m");
        if (r == 0)
                log_device_debug(dev, "Device busy: not tagged with \"systemd\"");
        if (r <= 0)
                return false;

        r = device_get_property_bool(dev, "SYSTEMD_READY");
        if (r < 0)
                log_device_debug_errno(dev, r, "Failed to get device SYSTEMD_READY property: %m");
        if (r == 0)
...
void device_found_node(Manager *m, const char *node, DeviceFound found, DeviceFound mask) {
        ...
        if ((found & mask) != 0) {
                ...
                if (r == -ENODEV)
                        ...
                        return (void) log_debug_errno(r, "Failed to open %s device, ignoring: %m", node);
