...
int unit_can_live_mount(Unit *u, const char *src, const char *dst, sd_bus_error *error) {
        ...
        if (!UNIT_VTABLE(u)->can_live_mount || !UNIT_VTABLE(u)->live_mount) {
                log_unit_debug(u, "Live mounting not supported for unit type '%s'", unit_type_to_string(u->type));
                return sd_bus_error_setf(
                                error,
                                SD_BUS_ERROR_INVALID_ARGS,
                                "Live mounting '%s' on '%s' for unit '%s' cannot be queued: live mounting not supported for unit type '%s'",
                                src,
                                dst,
                                u->id,
        ...
        if (u->load_state != UNIT_LOADED) {
                ...
                return sd_bus_error_setf(
                                ...
                                "Live mounting '%s' on '%s' for unit '%s' cannot be queued: unit not loaded",
                                src,
                                dst,
                                u->id);
        }
        ...
        if (!UNIT_IS_ACTIVE_OR_RELOADING(unit_active_state(u))) {
                ...
                return sd_bus_error_setf(
                                ...
                                "Live mounting '%s' on '%s' for unit '%s' cannot be queued: unit not active",
                                src,
                                dst,
                                u->id);
        }
        ...
        if (unit_active_state(u) == UNIT_REFRESHING) {
                ...
                return sd_bus_error_setf(
                                error,
                                BUS_ERROR_UNIT_BUSY,
                                "Live mounting '%s' on '%s' for unit '%s' cannot be queued: another operation in progress",
                                src,
                                dst,
                                u->id);
        }

        return UNIT_VTABLE(u)->can_live_mount(u, src, dst, error);
}
