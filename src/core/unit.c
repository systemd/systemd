...
Unit* unit_free(Unit *u) {
        ...
        activation_event_info_unref(u->activation_event_info);
...
int unit_merge(Unit *u, Unit *other) {
        ...
        if (!u->activation_event_info)
                u->activation_event_info = activation_event_info_ref(other->activation_event_info);
...
int activation_event_info_append_env(ActivationEventInfo *info, char ***strv) {
        ...
        if (!isempty(info->trigger_unit_name)) {
                char *s = strjoin("TRIGGER_UNIT=", info->trigger_unit_name);
                if (!s)
                        return -ENOMEM;
        ...
        if (ACTIVATION_EVENT_INFO_VTABLE(info)->append_env) {
                r = ACTIVATION_EVENT_INFO_VTABLE(info)->append_env(info, strv);
                if (r < 0)
                        return r;
        }

        return r + !isempty(info->trigger_unit_name) ? 1 : 0; /* Return the number of variables added to the env block */
}
...
int activation_event_info_append_pair(ActivationEventInfo *info, char ***strv) {
        ...
        return r + !isempty(info->trigger_unit_name) ? 1 : 0; /* Return the number of pairs added to the strv */
}
