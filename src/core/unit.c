                ...
                if (crt && crt->cgroup_path) {
                        ...
                        if (r < 0 && !IN_SET(r, -ESRCH, -ENOENT)) {
                                if (ret >= 0)
                                        sd_bus_error_set_errnof(
                                                        ret_error, r,
                                                        "Failed to send signal SIG%s to auxiliary processes in unit cgroup '%s': %m",
                                                        signal_to_string(signo), p);

                                RET_GATHER(ret, log_unit_warning_errno(
                                                        u, r,
                                                        "Failed to send signal SIG%s to auxiliary processes in unit cgroup '%s' on client request: %m",
                                                        signal_to_string(signo), p));
                        }
                        killed = killed || r > 0;
                }
