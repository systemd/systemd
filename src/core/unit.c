...
void unit_next_freezer_state(Unit *u, FreezerAction action, FreezerState *ret, FreezerState *ret_target) {
        ...
        assert(u);
        assert(IN_SET(action, FREEZER_FREEZE, FREEZER_FREEZE_BY_PARENT,
                              FREEZER_THAW, FREEZER_THAW_BY_PARENT));
        assert(ret);
        assert(ret_target);
        ...
        if (action == FREEZER_FREEZE) {
                ...
                if (IN_SET(curr, FREEZER_FROZEN, FREEZER_FROZEN_PARENT))
                        next = FREEZER_FROZEN;
                else
                ...
                if (IN_SET(curr, FREEZER_RUNNING, FREEZER_THAWING, FREEZER_FREEZING_PARENT, FREEZER_FROZEN_PARENT))
                        next = curr;
                else if (curr == FREEZER_FREEZING) {
                        if (IN_SET(parent, FREEZER_RUNNING, FREEZER_THAWING))
                                next = FREEZER_THAWING;
                        else
                                next = FREEZER_FREEZING_PARENT;
                } else {
                        assert(curr == FREEZER_FROZEN);
                        if (IN_SET(parent, FREEZER_RUNNING, FREEZER_THAWING))
                                next = FREEZER_THAWING;
                        else
                                next = FREEZER_FROZEN_PARENT;
                }
        } else if (action == FREEZER_FREEZE_BY_PARENT) {
                /* We need to avoid accidentally demoting units frozen manually */
                if (IN_SET(curr, FREEZER_FREEZING, FREEZER_FROZEN, FREEZER_FROZEN_PARENT))
                        next = curr;
                else
                        next = FREEZER_FREEZING_PARENT;
        } else if (action == FREEZER_THAW_BY_PARENT) {
...
int unit_freezer_action(Unit *u, FreezerAction action) {
        ...
        if (action == FREEZER_FREEZE && IN_SET(u->freezer_state, FREEZER_FREEZING, FREEZER_FREEZING_PARENT))
                return -EALREADY;
        if (action == FREEZER_THAW && u->freezer_state == FREEZER_THAWING)
                return -EALREADY;
        if (action == FREEZER_THAW && IN_SET(u->freezer_state, FREEZER_FREEZING_PARENT, FREEZER_FROZEN_PARENT))
                return -ECHILD;
        ...
        assert(IN_SET(u->freezer_state, FREEZER_FREEZING, FREEZER_FREEZING_PARENT, FREEZER_THAWING));
        return 1;
}
