...
Unit* unit_new(Manager *m, size_t size) {
        ...
        u->manager = m;
        strcpy(u->id_field, (m && m->runtime_scope == RUNTIME_SCOPE_USER) ? "USER_UNIT=" : "UNIT=");
        strcpy(u->id_field_format,
               (m && m->runtime_scope == RUNTIME_SCOPE_USER) ? "USER_UNIT=%s" : "UNIT=%s");
        strcpy(u->invocation_id_field,
               (m && m->runtime_scope == RUNTIME_SCOPE_USER) ? "USER_INVOCATION_ID=" : "INVOCATION_ID=");
        strcpy(u->invocation_id_field_format,
               (m && m->runtime_scope == RUNTIME_SCOPE_USER) ? "USER_INVOCATION_ID=%s" : "INVOCATION_ID=%s");
...
Unit* unit_free(Unit *u) {
        ...
        if (u->on_console && u->manager) /* Might be called in sd-executor with no manager object */
...
static int unit_log_resources(Unit *u) {
        ...
        t = strjoina(u->id_field, u->id);
        iovec[n_iovec + 2] = IOVEC_MAKE_STRING(t);

        t = strjoina(u->invocation_id_field, u->invocation_id_string);
