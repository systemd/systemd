                ...
                if (dual_timestamp_is_set(&u->inactive_exit_timestamp) &&
                    dual_timestamp_is_set(&u->inactive_enter_timestamp)) {
                        ...
                        if (timestamp_is_set(wall_clock_usec)) {
                                uint64_t c;
                                if (MUL_SAFE(&c, cpu_usec, 10000U))
                                        c = (c + wall_clock_usec / 2) / wall_clock_usec;
                                else
                                        c = (cpu_usec / wall_clock_usec) * 10000U;

                                if (c >= 1000U) {
                                        uint64_t pct = (c + 50U) / 100U;
                                        if (strextendf(&message, " (%" PRIu64 "%% CPU)", pct) < 0)
                                                return log_oom();
                                } else if (c > 0) {
                                        if (strextendf(&message,
                                                       " (%" PRIu64 ".%02" PRIu64 "%% CPU)",
                                                       PERMYRIAD_AS_PERCENT_FORMAT_VAL(c)) < 0)
                                                return log_oom();
                                }
                        }
