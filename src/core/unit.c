...
static PrivateTmp unit_get_private_tmp(const Unit *u, const ExecContext *c) {
         ...
         * above. For /var/tmp/, we need to disable the setting, because we don't want to
         * create the /var/tmp/ directory if /var/ is a mount point. We don't have this
         * problem with /tmp/ because there is no nesting. */
