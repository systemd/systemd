...
int unit_start(Unit *u, ActivationDetails *details) {
        ...
        if (u->activation_details == NULL) /* Older details object wins */
...
int activation_details_append_env(ActivationDetails *details, char ***strv) {
        ...
        return r + (!isempty(details->trigger_unit_name) ? 1 : 0); /* Return the number of variables added to the env block */
}
...
int activation_details_append_pair(ActivationDetails *details, char ***strv) {
        ...
        if (!isempty(details->trigger_unit_name)) {
                ...
                if (strv_extend(strv, details->trigger_unit_name) < 0)
                        return -ENOMEM;
        }
        ...
        return r + (!isempty(details->trigger_unit_name) ? 1 : 0); /* Return the number of pairs added to the strv */
}
