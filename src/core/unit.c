...
static int unit_add_dependency_impl(
        ...
        if (u_info.data != u_info_old.data) {
                if (u_info_old.data)
                        r = hashmap_update(u_deps, other, u_info.data);
                else
                        r = hashmap_put(u_deps, other, u_info.data);
                if (r < 0)
                        return r;
        ...
        if (other_info.data != other_info_old.data) {
                if (other_info_old.data)
                        r = hashmap_update(other_deps, u, other_info.data);
                else
                        r = hashmap_put(other_deps, u, other_info.data);
                if (r < 0) {
                        if (u_info.data != u_info_old.data) {
