...
static int unit_dispatch_transient_owner(sd_event_source *source, int fd, uint32_t revents, void *userdata) {
        ...
        bool stop = owner->stop_on_exit && owner->unit->n_bound_transient_owners <= 1;

        log_unit_debug(owner->unit, "Owner of transient unit exited%s: " PID_FMT,
                       stop ? ", enqueuing stop job" : "", owner->pidref.pid);
...
int unit_transient_add_owner(Unit *u, PidRef pidref_consume, bool stop_on_exit) {
