...
bool slice_test_concurrency_soft_max(Slice *slice, Unit *ignore) {
        assert(slice);
        ...
        if (parent)
                return slice_test_concurrency_soft_max(SLICE(parent), ignore);

        return false;
}

bool slice_test_concurrency_hard_max(Slice *slice, Unit *ignore) {
        assert(slice);
        ...
        Unit *parent = UNIT_GET_SLICE(UNIT(slice));
        if (parent)
                return slice_test_concurrency_hard_max(SLICE(parent), ignore);
