...
static void exec_import_credential_hash_func(const ExecImportCredential *lc, struct siphash *state) {
        assert(lc);
        assert(state);

        siphash24_compress_string(lc->glob, state);
        if (lc->rename)
                siphash24_compress_string(lc->rename, state);
}
...
static int exec_import_credential_compare_func(const ExecImportCredential *a, const ExecImportCredential *b) {
        ...
        return streq(a->glob, b->glob) && streq_ptr(a->rename, b->rename);
}
