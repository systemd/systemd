...
static int credentials_dir_finalize_permissions(int dfd, uid_t uid, bool ownership_ok) {
        _cleanup_close_ int fd_close = -EBADF;
        int r;

        assert(dfd >= 0);

        dfd = fd_reopen_condition(dfd, O_DIRECTORY|O_CLOEXEC, O_PATH, &fd_close);
        if (dfd < 0)
                return dfd;
...
finalize:
        if (r >= 0) {
                r = credentials_dir_finalize_permissions(dfd, uid, ownership_ok);
                if (r < 0)
...
static int setup_credentials_internal(
        ...
        (void) label_fix_full(mfd, /* inode_path = */ NULL, cred_dir, /* flags = */ 0);

        r = acquire_credentials(context, cgroup_context, params, unit, mfd, uid, gid, /* ownership_ok = */ true);
        if (r < 0)
                return r;

        r = credentials_dir_finalize_permissions(mfd, uid, ownership_ok);
        if (r < 0)
                return log_debug_errno(r, "Failed to adjust ACLs of credentials dir: %m");;

        _cleanup_close_ int fs_fd = fspick(mfd, "", FSPICK_EMPTY_PATH|FSPICK_CLOEXEC);
        if (fs_fd < 0)
                return -errno;
