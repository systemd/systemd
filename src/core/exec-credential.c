...
typedef SetupCredentialsContext {
        RuntimeScope scope;
...
typedef LoadCredentialArguments {
...
int exec_setup_credentials(
        ...
        r = setup_credentials_internal(&ctx, /* fresh = */ FLAGS_SET(params->flags, EXEC_SETUP_CREDENTIALS_FRESH), p);
        if (r < 0)
                (void) rmdir(p);
