...
static int bus_unit_method_freezer_generic(sd_bus_message *message, void *userdata, sd_bus_error *error, FreezerAction action) {
        ...
        if (r == -EOPNOTSUPP)
                return sd_bus_error_setf(error, SD_BUS_ERROR_NOT_SUPPORTED, "Unit does not support freeze/thaw.", u->id);
        if (r == -EBUSY)
        ...
        if (r == -ECHILD)
                return sd_bus_error_setf(error, SD_BUS_ERROR_FAILED, "Unit is frozen by a parent slice.", u->id);
        if (r < 0)
                return r;
        if (r == 0)
                reply_no_delay = true;
