...
static int bus_unit_set_transient_property(
        ...
        if (streq(name, "Documentation")) {
                ...
                if (unit_has_name(u, SPECIAL_INIT_SCOPE) || unit_has_name(u, SPECIAL_INIT_WORKERS_SCOPE) || unit_has_name(u, SPECIAL_INIT_SLICE))
                        return sd_bus_error_set(error, SD_BUS_ERROR_INVALID_ARGS, "Cannot set slice for init.slice, init.scope and init-workers.scope");
