...
static int bus_verify_manage_units_async_full(
        ...
        _cleanup_strv_free_ char **details = NULL;

        details = strv_new("unit", u->id,
                           "verb", verb,
                           NULL);
        if (!details)
                return -ENOMEM;
        ...
        if (polkit_message) {
                ...
                r = strv_extend_pair(&details, "polkit.message", polkit_message);
                if (r < 0)
                        return r;

                r = strv_extend_pair(&details, "polkit.gettext_domain", GETTEXT_PACKAGE);
                if (r < 0)
                        return r;
        }
