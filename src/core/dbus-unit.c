...
static int bus_unit_set_live_property(
        ...
        if (streq(name, "DebugWait")) {
                ...
                if (SERVICE(u)->exec_command[SERVICE_EXEC_START_PRE])
                        d = false;

                if (SERVICE(u)->exec_command[SERVICE_EXEC_START_POST])
                        d = false;
                ...
                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                        ...
                        unit_write_settingf(u, flags|UNIT_ESCAPE_SPECIFIERS, name, "DebugWait=%d", d);
                }
