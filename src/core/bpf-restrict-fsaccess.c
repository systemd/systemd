...
/* On failure sets *ret_load_failed to true if the verifier rejected the object, which the caller may
...
int bpf_restrict_fsaccess_prepare(struct restrict_fsaccess_bpf **ret) {
        ...
        bool have_real_data_inode = bpf_kernel_has_kfunc("bpf_real_data_inode");
        ...
        for (;;) {
                ...
                r = restrict_fsaccess_load_variant(/* compat= */ false, have_real_data_inode, &load_failed, ret);
                if (r >= 0) {
                ...
                r = restrict_fsaccess_load_variant(/* compat= */ true, have_real_data_inode, &load_failed, ret);
                if (r >= 0) {
