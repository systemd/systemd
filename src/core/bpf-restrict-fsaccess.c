...
static int prepare_restrict_fsaccess_bpf(struct restrict_fsaccess_bpf **ret) {
        ...
        if (r < 0)
                return log_error_errno(errno, "bpf-restrict-fsaccess: Failed to size hash table: %m");
...
static bool restrict_fsaccess_have_deserialized_fds(Manager *m) {
        size_t count = 0;
        ...
        if (count > 0 && count < ELEMENTSOF(m->restrict_fsaccess_link_fds))
                log_error("bpf-restrict-fsaccess: Only %zu of %zu link FDs deserialized, enforcement may be incomplete.",
...
static int restrict_fsaccess_validate_deserialized_fds(Manager *m) {
        ...
        FOREACH_ELEMENT(fd, m->restrict_fsaccess_link_fds) {
                ...
                if (r < 0)
                        return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                               "bpf-restrict-fsaccess: Deserialized FD for %s is not a valid BPF link, aborting.",
                                               restrict_fsaccess_link_names[fd - m->restrict_fsaccess_link_fds]);
        }
...
int bpf_restrict_fsaccess_setup(Manager *m) {
        ...
        if (restrict_fsaccess_have_deserialized_fds(m)) {
