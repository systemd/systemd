...
static int restrict_fsaccess_load_variant(bool compat, bool have_real_data_inode, struct restrict_fsaccess_bpf **ret) {
...
int bpf_restrict_fsaccess_prepare(struct restrict_fsaccess_bpf **ret) {
        ...
        bool have_kfunc = bpf_kernel_has_kfunc("bpf_real_data_inode");
        ...
        for (;;) {
                r = restrict_fsaccess_load_variant(/* compat= */ false, have_kfunc, ret);
                if (r >= 0) {
                        log_debug("bpf-restrict-fsaccess: Loaded with full const void * access%s.",
                                  have_kfunc ? " and union filesystem resolution" : "");
                        return 0;
                }

                log_debug_errno(r, "bpf-restrict-fsaccess: Full version failed to load (%m), trying compat variant.");

                r = restrict_fsaccess_load_variant(/* compat= */ true, have_kfunc, ret);
                if (r >= 0) {
