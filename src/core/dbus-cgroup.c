        ...
        if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                *p = v;
                ...
                if (v == CGROUP_WEIGHT_INVALID)
                        ...
                        unit_write_settingf(u, flags, name, "%s=%s", name, "idle");
                else
...
static int bus_cgroup_set_cpu_shares(
                Unit *u,
                const char *name,
                uint64_t *p,
                sd_bus_message *message,
        ...
        if (r < 0)
                return r;
        if (!CGROUP_CPU_SHARES_IS_OK(v) && v != CGROUP_CPU_SHARES_IDLE)
                return sd_bus_error_setf(
                                error, SD_BUS_ERROR_INVALID_ARGS, "Value specified in %s is out of range", name);
        if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                *p = v;
                unit_invalidate_cgroup(u, CGROUP_MASK_CPU);
                if (v == CGROUP_CPU_SHARES_INVALID)
                        unit_write_settingf(u, flags, name, "%s=", name);
                else if (v == CGROUP_CPU_SHARES_IDLE)
                        unit_write_settingf(u, flags, name, "%s=%s", name, "idle");
                else
                        unit_write_settingf(u, flags, name, "%s=%" PRIu64, name, v);
        }
