...
int bus_cgroup_set_property(
        ...
        if (streq(name, "ManagedOOMMemoryPressureDurationUSec")) {
                ...
                if (!managed_oom_mem_pressure_duration_is_valid(t))
                        return sd_bus_error_setf(error, SD_BUS_ERROR_INVALID_ARGS, "%s= must be at least 1s, got %" PRIu64, name, t);

                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                        c->memory_pressure_threshold_usec = t;
                        if (c->memory_pressure_threshold_usec == USEC_INFINITY)
                                unit_write_setting(u, flags, name, "ManagedOOMMemoryPressureDurationUSec=");
                        else
                                unit_write_settingf(u, flags, name, "ManagedOOMMemoryPressureDurationUSec=%" PRIu64, t);
                }
