...
static int property_get_socket_bind(
        ...
        r = sd_bus_message_open_container(reply, 'a', "(iuu)");
        if (r < 0)
                return r;

        LIST_FOREACH(socket_bind_items, i, *items) {
                r = sd_bus_message_append(reply, "(iuu)", i->address_family, i->nr_ports, i->port_min);
                if (r < 0)
                        return r;
        }
...
const sd_bus_vtable bus_cgroup_vtable[] = {
        ...
        SD_BUS_PROPERTY("SocketBindAllow", "a(iuu)", property_get_socket_bind, offsetof(CGroupContext, socket_bind_allow), 0),
        SD_BUS_PROPERTY("SocketBindDeny", "a(iuu)", property_get_socket_bind, offsetof(CGroupContext, socket_bind_deny), 0),
        SD_BUS_VTABLE_END
};
...
int bus_cgroup_set_property(
        ...
        if (STR_IN_SET(name, "SocketBindAllow", "SocketBindDeny")) {
                ...
                r = sd_bus_message_enter_container(message, 'a', "(iuu)");
                if (r < 0)
                        return r;

                while ((r = sd_bus_message_read(message, "(iuu)", &family, &nr_ports, &port_min)) > 0) {
