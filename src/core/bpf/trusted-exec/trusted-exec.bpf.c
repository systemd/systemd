...
int BPF_PROG(trusted_exec_bdev_setintegrity, struct block_device *bdev,
...
void BPF_PROG(trusted_exec_bdev_free, struct block_device *bdev)
{
        __u32 dev = bdev->bd_dev;
...
int BPF_PROG(trusted_exec_bprm_check, struct linux_binprm *bprm)
{
        struct file *file;
...
SEC("lsm/mmap_file")
int BPF_PROG(trusted_exec_mmap_file, struct file *file, unsigned long reqprot,
...
int BPF_PROG(trusted_exec_file_mprotect, struct vm_area_struct *vma,
...
int BPF_PROG(trusted_exec_ptrace_guard, struct task_struct *child,
             unsigned int mode)
{
...
SEC("lsm/bpf_map")
int BPF_PROG(trusted_exec_bpf_map_guard, struct bpf_map *map,
             unsigned int fmode)
{
        __u32 id;
...
SEC("lsm/bpf_prog")
int BPF_PROG(trusted_exec_bpf_prog_guard, struct bpf_prog *prog)
{
        __u32 id;
...
SEC("lsm/bpf")
int BPF_PROG(trusted_exec_bpf_guard, int cmd, union bpf_attr *attr,
             unsigned int size)
{
        __u32 id;
