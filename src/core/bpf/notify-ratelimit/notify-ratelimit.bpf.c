...
int sd_notify_ratelimit(struct bpf_sock_addr *ctx) {
        ...
        sun = bpf_core_cast(sa_kern->uaddr, struct sockaddr_un);
        bpf_probe_read_kernel(&path, sizeof(path), sun->sun_path);

        if (bpf_strncmp(path, sizeof(PID1_NOTIFY_SOCKET), PID1_NOTIFY_SOCKET) != 0)
                return 1;
