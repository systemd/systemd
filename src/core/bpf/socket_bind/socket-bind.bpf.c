...
static __always_inline bool match_port(
...
static __always_inline bool match(
        ...
        return match_af(address_family, r) && match_port(port, r);
}
...
static __always_inline int socket_bind_impl(
        ...
        for (__u32 i = 0; i < socket_bind_max_rules; ++i) {
                const __u32 key = i;
                const struct socket_bind_rule *r = bpf_map_lookup_elem(&rules, &key);

                if (!r)
                        break;

                if (match(ctx->user_family, port, r))
                        return r->action;
        }
