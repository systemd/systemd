...
typedef struct Manager {
        ...
        /* Whether we have ever pushed a non-empty fdstore-mapping memfd to our upstream
         * supervisor. Used to decide if we need to send FDSTOREREMOVE for the mapping when
         * the mapping becomes empty (avoids removing a freshly-handed-back mapping memfd
         * during a brand-new manager startup with no propagated fds yet). */
        bool fd_store_upstream_mapping_pushed;
