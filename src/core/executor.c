...
static int parse_argv(int argc, char *argv[]) {
        ...
        while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0)
                ...
                case ARG_MANAGER_SOCKET: {
                        ...
                        if (r < 0)
                                return log_error_errno(r,
                                                "Failed to set CLOEXEC on manager socket %d: %m",
                                                fd);
...
static int send_our_pid(int socket) {
        ...
        if (pidfd < 0) {
                ...
                xsprintf(iov_buffer, PID_FMT, getpid_cached());
                iov = IOVEC_MAKE_STRING(iov_buffer);
                r = send_one_fd_iov(socket, /* fd= */ -EBADF, &iov, 1, /* flags= */ 0);
                if (r < 0)
                        return log_error_errno(r, "Failed to send pid: %m");
        } else {
                r = send_one_fd(socket, pidfd, /* flags= */ 0);
                if (r < 0)
...
int main(int argc, char *argv[]) {
        ...
        if (arg_serialization) {
                ...
                /* The array always contains at the very least the exec FD */
                if (n_fds_array == 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "Received no FDs from manager socket.");

                r = fdset_new_array(&fdset, fds_array, n_fds_array);
                if (r < 0)
                        return log_error_errno(r, "Failed to create fd set: %m");
                ...
                r = send_our_pid(manager_socket);
                if (r < 0)
                        return r;

                /* The memfd with all the data is always the first one */
                r = fdopen_independent(fds_array[0], "r", &serialization);
                if (r < 0)
                        return log_error_errno(r, "Failed to open serialization fd: %m");
        } else
                assert_not_reached();

        r = exec_deserialize_invocation(serialization,
                                        fdset,
                                        /* deserialize_fd_index= */ arg_manager_socket >= 0,
                                        &context,
                                        &command,
                                        &params,
                                        &runtime,
