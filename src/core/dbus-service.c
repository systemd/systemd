...
static int property_get_open_files(
        ...
        LIST_FOREACH(open_files, open_file, s->open_files) {
                _cleanup_free_ const char *open_file_str = open_file_to_string(open_file);

                r = sd_bus_message_append(reply, "s", open_file_str);
                if (r < 0)
                        return r;
        }
...
const sd_bus_vtable bus_service_vtable[] = {
        ...
        SD_BUS_PROPERTY("OpenFile", "a(s)", property_get_open_files, 0, SD_BUS_VTABLE_PROPERTY_CONST),
...
static int bus_service_set_transient_property(
        ...
        if (streq(name, "OpenFile")) {
                const char *open_file_str;

                r = sd_bus_message_enter_container(message, 'a', "s");
                if (r < 0)
                        return r;

                while ((r = sd_bus_message_read(message, "s", &open_file_str)) > 0) {
                        _cleanup_free_ OpenFile *open_file = NULL;
                        r = parse_open_file(open_file_str, open_file);
                        if (r < 0)
                                return r;

                        if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                                LIST_APPEND(open_files, s->open_files, TAKE_PTR(open_file));
                                unit_write_settingf(u, flags|UNIT_ESCAPE_SPECIFIERS, name, "OpenFile=%s", open_file_str);
                        }
                }
