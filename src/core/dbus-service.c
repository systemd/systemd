...
static int bus_service_set_transient_property(
        ...
        if (streq(name, "RefreshOnReload")) {
                ...
                const char *s;
                bool invert;
                while ((r = sd_bus_message_read(message, "(bs)", &invert, &s)) > 0) {
                        ServiceRefreshOnReload f;

                        f = service_refresh_on_reload_flag_from_string(s);
                        if (f < 0)
                                return sd_bus_error_setf(reterr_error, SD_BUS_ERROR_INVALID_ARGS, "Invalid RefreshOnReload= value: %s", s);
