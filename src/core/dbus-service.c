...
static int property_get_open_files(
        ...
        LIST_FOREACH(open_files, open_file, s->open_files) {
                r = sd_bus_message_append(reply, "(sst)", open_file->path, open_file->fdname, open_file->flags);
                if (r < 0)
                        return r;
        }
...
static int bus_service_set_transient_property(
        ...
        if (streq(name, "OpenFile")) {
                ...
                while ((r = sd_bus_message_read(message, "(sst)", &path, &fdname, &options)) > 0) {
                        if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                                OpenFile *of = new(OpenFile, 1);
                                if (!of)
                                        return -ENOMEM;
                                ...
                                *of = (OpenFile) {
                                ...
                                LIST_APPEND(open_files, s->open_files, of);
                                unit_write_settingf(u, flags|UNIT_ESCAPE_SPECIFIERS, name, "OpenFile=%s", open_file_to_string(of));
                        }
                }
