...
static int bus_service_set_transient_property(
        ...
        if (streq(name, "LUOSession")) {
                ...
                STRV_FOREACH(p, l) {
                        if (!string_is_safe(*p, /* flags= */ 0) || strpbrk(*p, WHITESPACE))
                                return sd_bus_error_setf(reterr_error, SD_BUS_ERROR_INVALID_ARGS,
                                                         "Invalid LUO session name: %s", *p);

                        if (strlen(*p) >= LIVEUPDATE_SESSION_NAME_LENGTH)
                                return sd_bus_error_setf(reterr_error, SD_BUS_ERROR_INVALID_ARGS,
                                                         "LUO session name too long (max %zu): %s",
                                                         (size_t) LIVEUPDATE_SESSION_NAME_LENGTH - 1, *p);
                }
                ...
                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                        if (strv_isempty(l)) {
                                ...
                                joined = strv_join(s->luo_sessions, " ");
                                if (!joined)
                                        return -ENOMEM;
