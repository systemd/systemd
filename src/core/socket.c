...
static int socket_coldplug(Unit *u) {
        ...
        /* Patch "deferred" back to "listening" and let service_enter_running() figure out what to do.
...
static bool socket_stop_notify(Unit *u) {
        ...
        r = manager_add_job(u->manager, JOB_START, UNIT_DEREF(s->service), JOB_FAIL, &error, /* ret = */ NULL);
        if (r >= 0) { /* Yay! */
...
static void socket_enter_deferred(Socket *s) {
         ...
         * even after all existing jobs have completed. */
...
static void socket_enter_running(Socket *s, int cfd_in) {
        ...
        if (cfd < 0) { /* Accept=no case */
                ...
                if (!pending) {
                        ...
                        if (s->defer_trigger) {
                                r = manager_add_job(UNIT(s)->manager, JOB_START, UNIT_DEREF(s->service), JOB_FAIL, &error, /* ret = */ NULL);
                                if (r == -EDEADLK && !hashmap_isempty(UNIT(s)->manager->jobs))
