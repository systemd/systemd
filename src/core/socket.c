...
static int socket_address_listen_in_cgroup(
        ...
        if (s->exec_context.network_namespace_path &&
            ...
            s->exec_runtime->netns_storage_socket[0] >= 0) {
                r = open_ns_path(s->exec_runtime->netns_storage_socket, s->exec_context.network_namespace_path, CLONE_NEWNET);
                if (r < 0)
        ...
        if (s->exec_context.ipc_namespace_path &&
            ...
            s->exec_runtime->ipcns_storage_socket[0] >= 0) {
                r = open_ns_path(s->exec_runtime->netns_storage_socket, s->exec_context.network_namespace_path, CLONE_NEWIPC);
                if (r < 0)
                ...
                if ((s->exec_context.private_network || s->exec_context.network_namespace_path) &&
                    ...
                    s->exec_runtime->netns_storage_socket[0] >= 0) {
                        ...
                        if (ns_type_supported(NAMESPACE_NET)) {
                                r = setup_ns(s->exec_runtime->netns_storage_socket, CLONE_NEWNET);
                                if (r < 0) {
