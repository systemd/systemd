...
static int automount_coldplug(Unit *u) {
        ...
        if (IN_SET(a->deserialized_state, AUTOMOUNT_WAITING, AUTOMOUNT_RUNNING)) {
                ...
                if (u->load_state != UNIT_LOADED) {
                        /* This automount unit's file vanished or got masked while we were reloading. Nobody can
                         * serve the autofs mount point anymore, but the kernel keeps blocking every process that
                         * is trying to access it while the mount exists. */
                        log_unit_warning(u, "Unit file is not loaded anymore, tearing down automount point %s.", a->where);
                        unmount_autofs(a, /* keep_mount_point= */ false);
                        return 0;
                }
