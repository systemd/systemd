...
#define BUS_DEFINE_SET_TRANSIENT(function, bus_type, type, cast_type, fmt)                                                     \
        int bus_set_transient_##function(                                                                                      \
                Unit *u, const char *name, cast_type *p, sd_bus_message *message, UnitWriteFlags flags, sd_bus_error *error) { \
                                                                                                                               \
                type v;                                                                                                        \
                int r;                                                                                                         \
                                                                                                                               \
                ...
                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {                                                                           \
                        *p = (cast_type) v;                                                                                    \
                        unit_write_settingf(u, flags, name, "%s=" fmt, name, v);                                               \
                }                                                                                                              \
                                                                                                                               \
                return 1;                                                                                                      \
        }
        ...
        int bus_set_transient_##function(                                                                                      \
                ...
                if (r < 0)                                                                                                     \
                        return r;                                                                                              \
                                                                                                                               \
                if (!check(v))                                                                                                 \
                        return sd_bus_error_setf(error, SD_BUS_ERROR_INVALID_ARGS, "Invalid %s setting: " fmt, name, v);       \
                                                                                                                               \
                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {                                                                           \
                        *p = (cast_type) v;                                                                                    \
                        unit_write_settingf(u, flags, name, "%s=" fmt, name, v);                                               \
...
#define BUS_DEFINE_SET_TRANSIENT_PARSE(function, type, parse)                                                             \
        int bus_set_transient_##function(                                                                                 \
                Unit *u, const char *name, type *p, sd_bus_message *message, UnitWriteFlags flags, sd_bus_error *error) { \
                                                                                                                          \
                const char *s;                                                                                            \
                type v;                                                                                                   \
                int r;                                                                                                    \
...
#define BUS_DEFINE_SET_TRANSIENT_PARSE_PTR(function, type, parse)                                                         \
        int bus_set_transient_##function(                                                                                 \
                ...
                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {                                                                      \
                        *p = v;                                                                                           \
                        unit_write_settingf(u, flags, name, "%s=%s", name, strempty(s));                                  \
                }                                                                                                         \
                                                                                                                          \
                return 1;                                                                                                 \
        }
...
#define BUS_DEFINE_SET_TRANSIENT_STRING_WITH_CHECK(function, check)                                                        \
        int bus_set_transient_##function(                                                                                  \
                Unit *u, const char *name, char **p, sd_bus_message *message, UnitWriteFlags flags, sd_bus_error *error) { \
                                                                                                                           \
                const char *v;                                                                                             \
                int r;                                                                                                     \
                                                                                                                           \
                ...
                if (!isempty(v) && !check(v))                                                                              \
                        return sd_bus_error_setf(error, SD_BUS_ERROR_INVALID_ARGS, "Invalid %s setting: %s", name, v);     \
                                                                                                                           \
                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {                                                                       \
                        ...
                        if (r < 0)                                                                                         \
                                return r;                                                                                  \
                                                                                                                           \
                        unit_write_settingf(u, flags | UNIT_ESCAPE_SPECIFIERS, name, "%s=%s", name, strempty(v));          \
                }                                                                                                          \
                                                                                                                           \
                return 1;                                                                                                  \
        }
...
static inline int
        bus_set_transient_usec(Unit *u, const char *name, usec_t *p, sd_bus_message *message, UnitWriteFlags flags, sd_bus_error *error) {
        return bus_set_transient_usec_internal(u, name, p, false, message, flags, error);
}
static inline int bus_set_transient_usec_fix_0(
        Unit *u, const char *name, usec_t *p, sd_bus_message *message, UnitWriteFlags flags, sd_bus_error *error) {
