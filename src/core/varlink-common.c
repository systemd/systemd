...
int exec_command_status_build_json(sd_json_variant **ret, const char *name, void *userdata) {
        ...
        return sd_json_buildo(
                        ret,
                        SD_JSON_BUILD_PAIR_UNSIGNED("PID", status->pid),
                        ...
                        SD_JSON_BUILD_PAIR_CONDITION(status->code == CLD_EXITED, "ExitStatus", SD_JSON_BUILD_INTEGER(status->status)),
                        SD_JSON_BUILD_PAIR_CONDITION(!IN_SET(status->code, 0, CLD_EXITED), "ExitSignal", SD_JSON_BUILD_STRING(signal_to_string(status->status))));
}
