...
static bool dont_stop_on_isolate(Transaction *tr, Unit *u) {
        assert(u);
...
int transaction_add_isolate_jobs(Transaction *tr, Manager *m) {
        ...
        HASHMAP_FOREACH_KEY(u, k, m->units) {
                ...
                if (dont_stop_on_isolate(tr, u))
                        continue;

                /* Don't stop units that are triggered by units we want to keep around. */
                bool keep = false;
                UNIT_FOREACH_DEPENDENCY(o, u, UNIT_ATOM_TRIGGERED_BY)
                        if (dont_stop_on_isolate(tr, o))
                                keep = true;
                if (keep)
                        continue;
