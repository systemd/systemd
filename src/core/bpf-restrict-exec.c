...
/* This is a static map, we should switch to a dynamic map if we need more. */
...
static int prepare_restrict_exec_bpf(struct restrict_exec_bpf **ret) {
        ...
        log_info("bpf-restrict-exec: Loaded with compat bdev_setintegrity (kernel lacks 1271a40eeafa).");
        *ret = TAKE_PTR(obj);
        return 0;
}
...
static int restrict_exec_validate_deserialized_fds(Manager *m) {
        ...
        if (r < 0)
                return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                       "bpf-restrict-exec: Failed to load libbpf for FD validation, skipping.");

        FOREACH_ELEMENT(fd, m->restrict_exec_link_fds) {
                r = bpf_get_link_ids(*fd, &id, NULL);
                if (r < 0)
                        return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                              "bpf-restrict-exec: Deserialized FD for %s is not a valid BPF link, closing.",
        ...
        if (m->restrict_exec_bss_map_fd >= 0) {
                ...
                if (r < 0)
                        return log_error_errno(SYNTHETIC_ERRNO(ENOTRECOVERABLE),
                                               "bpf-restrict-exec: Deserialized FD for .bss map is not a valid BPF map, closing.");
        }
