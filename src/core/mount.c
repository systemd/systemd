...
static void mount_enter_mounting(Mount *m) {
         ...
         * us anymore. */
        r = chase(m->where, /* root= */ NULL, CHASE_PARENT|CHASE_EXTRACT_FILENAME|CHASE_PROHIBIT_SYMLINKS|CHASE_MKDIR_0755, &fn, &fd);
        if (r == -EREMCHG) {
                r = unit_log_noncanonical(UNIT(m), m->where);
                goto fail;
        }
        if (r < 0) {
                log_unit_error_errno(UNIT(m), r, "Failed to resolve parent of mount point '%s', ignoring: %m", m->where);
                goto fail;
        }
