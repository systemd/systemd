...
static int mount_setup_existing_unit(
        ...
        /* when we have both info from /etc/fstab and from /proc/self/mountinfo, and the mount generated by
         * fstab is in a failed state then remove any BindsTo= deps generated due to fstab, and instead
         * snthesize BindsTo= on the block device listed in /proc/self/mountinfo. */
        if (u->fragment_path && IN_SET(MOUNT(u)->state, MOUNT_DEAD, MOUNT_FAILED))
                unit_remove_dependencies(u, UNIT_DEPENDENCY_FILE);
...
static int mount_process_proc_self_mountinfo(Manager *m) {
        ...
        bool is_from_proc_self_mountinfo;
        const char *what;
        Iterator i;
        Unit *u;
        int r;
        ...
        LIST_FOREACH(units_by_type, u, m->units_by_type[UNIT_MOUNT]) {
                ...
                if (!mount_is_mounted(mount)) {
                        ...
                        is_from_proc_self_mountinfo = mount->from_proc_self_mountinfo;
                        ...
                        /* When we have both info from /etc/fstab and /proc/self/mountinfo, we discard the dep info
                         * from /etc/fstab and add the dep info from /proc/self/mountinfo; when we lose the info
                         * from /proc/self/mountinfo again (because the device is unmounted), the we need to add the
                         * dep info generated by /etc/fstab. */
                        if (UNIT(mount)->fragment_path && is_from_proc_self_mountinfo)
                                mount_add_device_dependencies(mount);
