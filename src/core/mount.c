...
void mount_remount_context_clear(RemountContext *ctx) {
        ...
        ctx->request = sd_bus_message_unref(ctx->request);
        ctx->options = mfree(ctx->options);
        ctx->flags = 0;
}
...
static void mount_done(Unit *u) {
        ...
        mount_remount_context_clear(&m->remount_context);
...
static bool mount_can_reload(Unit *u) {
        ...
        return get_mount_parameters_fragment(m) || m->remount_context.request;
}
