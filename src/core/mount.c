...
static int mount_add_device_dependencies(Mount *m) {
        ...
        if (r < 0)
                return r;
        if (r > 0)
                log_unit_trace("Added %s dependency on %s", unit_dependency_to_string(dep), p->what);
        ...
        if (mount_propagate_stop(m)) {
                ...
                if (r < 0)
                        return r;
                if (r > 0)
                        log_unit_trace("Added %s dependency on %s",
                                       unit_dependency_to_string(UNIT_STOP_PROPAGATED_FROM), p->what);
        }

        r = unit_add_blockdev_dependency(UNIT(m), p->what, mask);
        if (r > 0)
                log_unit_trace("Added %s dependency on %s", unit_dependency_to_string(UNIT_AFTER), p->what);
