...
static void mount_enter_dead(Mount *m, MountResult f, bool flush_failure);
...
static void mount_catchup(Unit *u) {
        ...
        if (m->from_proc_self_mountinfo)
                ...
                case MOUNT_MOUNTED:
                        assert(!pidref_is_set(&m->control_pid));
                        mount_enter_dead(m, MOUNT_SUCCESS, /* flush_failure = */ false);
                        break;
                default:
                        break;
                }
...
static void mount_enter_dead(Mount *m, MountResult f, bool flush_failure) {
        ...
        if (m->result == MOUNT_SUCCESS || flush_failure)
                m->result = f;
...
static void mount_enter_dead_or_mounted(Mount *m, MountResult f, bool flush_failure) {
        assert(m);
         ...
         *
         * Note that flush_failure only applies to mount_enter_dead(), since that's when the result gets
         * turned into unit end state. */

        if (m->from_proc_self_mountinfo)
                mount_enter_mounted(m, f);
        else
                mount_enter_dead(m, f, flush_failure);
}
...
static void mount_enter_signal(Mount *m, MountState state, MountResult f) {
        ...
        if (r > 0) {
                ...
                mount_enter_dead_or_mounted(m, MOUNT_SUCCESS, /* flush_failure = */ false);

        return;

fail:
        mount_enter_dead_or_mounted(m, MOUNT_FAILURE_RESOURCES, /* flush_failure = */ false);
}
...
static void mount_sigchld_event(Unit *u, pid_t pid, int code, int status) {
        ...
        case MOUNT_MOUNTING:
                ...
                mount_enter_dead(m, f, /* flush_failure = */ false);
                break;
        ...
        case MOUNT_UNMOUNTING:
                if (f == MOUNT_SUCCESS && m->from_proc_self_mountinfo) {
                        ...
                        mount_enter_dead(m, MOUNT_SUCCESS, /* flush_failure = */ true);
                } else
                         ...
                         * end state. */
                        mount_enter_dead_or_mounted(m, f, /* flush_failure = */ true);

                break;

        case MOUNT_UNMOUNTING_SIGTERM:
        case MOUNT_UNMOUNTING_SIGKILL:
                mount_enter_dead_or_mounted(m, f, /* flush_failure = */ false);
                break;
...
static int mount_dispatch_timer(sd_event_source *source, usec_t usec, void *userdata) {
        ...
        case MOUNT_UNMOUNTING_SIGTERM:
                if (m->kill_context.send_sigkill) {
                        ...
                        mount_enter_dead_or_mounted(m, MOUNT_FAILURE_TIMEOUT, /* flush_failure = */ false);
                }
                break;
        ...
        LIST_FOREACH(units_by_type, u, m->units_by_type[UNIT_MOUNT]) {
                ...
                if (!mount_is_mounted(mount)) {
                        ...
                        case MOUNT_MOUNTED:
                                ...
                                mount_enter_dead(mount, MOUNT_SUCCESS, /* flush_failure = */ true);
                                break;
...
static int mount_can_start(Unit *u) {
        ...
        if (r < 0) {
                mount_enter_dead(m, MOUNT_FAILURE_START_LIMIT_HIT, /* flush_failure = */ false);
                return r;
        }
