        ...
        if (streq(name, "GracefulOptions")) {
                ...
                if (!UNIT_WRITE_FLAGS_NOOP(flags)) {
                        ...
                        if (strv_isempty(add)) {
                                ...
                                _cleanup_strv_free_ char **joined = strv_copy(m->graceful_options);
                                if (!joined)
                                        return -ENOMEM;

                                r = strv_extend_strv(&joined, add, /* filter_duplicates= */ false);
                                if (r < 0)
                                        return r;

                                strv_free_and_replace(m->graceful_options, joined);
