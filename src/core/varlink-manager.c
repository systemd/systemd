        ...
        HASHMAP_FOREACH_KEY(u, k, manager->units) {
                ...
                if (r < 0) {
                        const char *error_id = varlink_error_id_from_bus_error(&bus_error),
                                   *error_msg = bus_error.message ?: error_id ? NULL : STRERROR(r);

                        r = sd_varlink_notifybo(link,
                                                SD_JSON_BUILD_PAIR_STRING("unitID", u->id),
                                                JSON_BUILD_PAIR_STRING_NON_EMPTY("jobError", error_id),
                                                JSON_BUILD_PAIR_STRING_NON_EMPTY("jobErrorMessage", error_msg);
                } else
                        r = sd_varlink_notifybo(link,
                                                SD_JSON_BUILD_PAIR_STRING("unitID", u->id),
