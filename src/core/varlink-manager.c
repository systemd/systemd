...
static void varlink_log_caller(sd_varlink *link, Manager *manager, const char *method) {
        ...
        /* We need at least the pidref, otherwise there's nothing to log about. */
        if (varlink_get_peer_pidref(link, &pidref) < 0)
                return;

        manager_log_caller(manager, &pidref, method);
}

static int manager_do_set_objective(sd_varlink *link, sd_json_variant *parameters, ManagerObjective objective, const char *selinux_permission) {
...
int vl_method_soft_reboot_manager(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        /* keep the checks in sync with dbus-manager.c:method_soft_reboot() */
        r = mac_selinux_access_check_varlink(link, "reboot");
        if (r < 0)
                return r;

        /* See comment in manager_do_set_objective() why we use varlink_check_privileged_peer() here. */
        r = varlink_check_privileged_peer(link);
        if (r < 0)
                return r;
        ...
        varlink_log_caller(link, m, "Soft reboot");
