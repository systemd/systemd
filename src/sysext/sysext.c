...
static const char *directory_name(void) {
        return syscfg_setting ? "systemd-syscfg" : "systemd-sysext";
}
...
static int verb_status(int argc, char **argv, void *userdata) {
        ...
        STRV_FOREACH(p, arg_hierarchies) {
                ...
                f = path_join(*p, dot_directory_name(), "extensions");
                if (!f)
                        return log_oom();
...
static int merge_hierarchy(
         ...
         * recognizable. */
        f = path_join(meta_path, dot_directory_name(), "extensions");
        if (!f)
                return log_oom();
...
static int verb_help(int argc, char **argv, void *userdata) {
        ...
        r = terminal_urlify_man(directory_name(), "1", &link);
        if (r < 0)
                return log_oom();
...
static int run(int argc, char *argv[]) {
         ...
         * switch. */
        r = parse_env_extension_hierarchies(&arg_hierarchies, syscfg_setting);
        if (r < 0)
