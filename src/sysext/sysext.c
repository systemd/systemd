...
typedef struct Context {
        ...
        /* The policy is usually borrowed from arg_image_policy, but owned when parsed from config. */
...
static int context_copy_cmdline_common(Context *c, ImageClass image_class) {
        assert(c);
...
static int context_from_cmdline(Context *ret) {
        ...
        r = context_copy_cmdline_common(&c, arg_image_class);
        if (r < 0)
                return r;

        c.hierarchies = strv_copy(arg_hierarchies);
        if (!c.hierarchies && arg_hierarchies)
                return log_oom();

        c.mutable = arg_mutable;
        c.mutable_set = arg_mutable_set;
        c.image_policy = arg_image_policy;
        c.image_policy_set = arg_image_policy_set;
        c.overlayfs_mount_options = arg_overlayfs_mount_options;
...
static int context_from_cmdline_for_image_class(Context *ret, ImageClass image_class) {
        _cleanup_(context_done) Context c = CONTEXT_NULL;
        int r;

        assert(ret);

        r = context_copy_cmdline_common(&c, image_class);
        if (r < 0)
                return r;
...
static int parse_config_file_from_cmdline(void) {
        _cleanup_(context_done) Context c = CONTEXT_NULL;
        int r;

        c.image_class = arg_image_class;
        if (arg_root && strdup_to(&c.root, arg_root) < 0)
                return log_oom();
        ...
        arg_mutable = c.mutable;
        arg_mutable_set = c.mutable_set;
        if (c.image_policy_owned) {
                assert(!arg_image_policy_set);
                arg_image_policy = TAKE_PTR(c.image_policy);
                c.image_policy_owned = false;
        }
        arg_image_policy_set = c.image_policy_set;
...
static int parse_env(void) {
        ...
        arg_mutable = c.mutable;
        arg_mutable_set = c.mutable_set;
        arg_overlayfs_mount_options = c.overlayfs_mount_options;
...
static int unmerge(const Context *c) {
        ...
        r = get_extension_release_metadata(c->image_class, c->hierarchies, c->no_reload, &need_to_reload, &units_to_restart, &units_to_reload_or_restart);
        if (r < 0)
                return r;
...
static int vl_method_merge(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        r = context_from_cmdline_for_image_class(&c, image_class);
        if (r < 0)
                return r;

        c.image_class = image_class;
        if (hierarchies)
                strv_free_and_replace(c.hierarchies, hierarchies);
        if (p.force >= 0)
                c.force = p.force;
        if (p.no_reload >= 0)
...
static int run(int argc, char *argv[]) {
        ...
        r = parse_config_file_from_cmdline();
        if (r < 0)
