...
static int unmerge_hierarchy(const char *p) {
        ...
        for (;;) {
                /* We only unmount /usr/ if it is a mount point and and really one of ours, in order not to
                 * break systems where /usr/ is a mount point of its own already. */
...
static int merge_hierarchy(
        ...
        STRV_FOREACH(p, paths) {
                ...
                if (r > 0) {
                        log_debug("Hierarchy '%s' im extension '%s' is empty, not merging.", hierarchy, *p);
                        continue;
                }
...
static int merge_subprocess(Hashmap *images, const char *workspace) {
        ...
        /* Mark the whole of /run as MS_SLAVE, so that we can mount stuff below it that doesn't show
         * up on the host otherwise. */
         ...
         * but let the kernel do that entirely automatically, once our namespace dies. Note that th*/
        ...
        HASHMAP_FOREACH(img, images) {
                 ...
                 * the place one in /etc/os-release, i.e. where things don't matter, as they aren't
                 * merged.) */
...
static int run(int argc, char *argv[]) {
        ...
        case ACTION_REFRESH:
                ...
                if (r == 0) /* No images found? Then unmerge. The goal of --refresh is after all that after
                             * having called it the merge status matches the installed extensions. */
                        r = unmerge();
                break;
