...
static void parse_env_image_class_config(Context *c) {
        ...
        r = secure_getenv_bool("SYSTEMD_SYSEXT_MOUNT_BENEATH");
        if (r >= 0)
                c->mount_beneath = r;
        else if (r != -ENXIO)
                log_warning_errno(r, "Failed to parse $SYSTEMD_SYSEXT_MOUNT_BENEATH, ignoring: %m");
}
...
static int context_from_cmdline(Context *ret, ImageClass image_class) {
        ...
        /* Each hierarchy is processed once, also if listed repeatedly in the environment variable */
...
static int install_hierarchy(const Context *c, const char *hierarchy, HierarchyTree *tree) {
        ...
        if (old_detached && old_work_dir) {
                ...
                if (k < 0)
                        RET_GATHER(r, log_error_errno(k, "Failed to remove '%s': %m", old_work_dir));
        }
