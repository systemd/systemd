...
static int unmerge_hierarchy(
        ...
        for (;;) {
                _cleanup_free_ char *escaped_work_dir = NULL, *work_dir = NULL;
                ...
                r = read_one_line_file(work_dir_info_file, &escaped_work_dir);
                if (r < 0) {
                        if (r != -ENOENT)
                        ...
                        l = cunescape_length(escaped_work_dir, r, 0, &work_dir);
                        if (l < 0)
...
static char *hierarchy_as_single_path_component(const char *hierarchy) {
        ...
        if (!dir_name)
                return NULL;
        dir_name = delete_trailing_chars(dir_name, "/");
        dir_name = string_replace_char(dir_name, '/', '.');
...
static int merge_hierarchy(
        ...
        if (work_dir) {
                 ...
                 * line file. */
                escaped_work_dir = cescape(work_dir);
                if (!escaped_work_dir)
                        return log_oom();
                r = write_string_file(f, escaped_work_dir, WRITE_STRING_FILE_CREATE);
                if (r < 0)
                        return log_error_errno(r, "Failed to write '%s': %m", f);
        }
