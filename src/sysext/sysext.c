...
static int consume_lower_dir(OverlayFSPaths *op, char *layer) {
        int r;

        assert(op);
        assert(layer);

        r = strv_consume_with_size(&op->lower_dirs, &op->lower_dirs_size, layer);
        if (r < 0)
                return log_oom();
...
static int append_lower_dir(OverlayFSPaths *op, const char *layer) {
        int r;

        assert(op);
        assert(layer);

        r = strv_extend_with_size(&op->lower_dirs, &op->lower_dirs_size, layer);
        if (r < 0)
                return log_oom();
...
static int determine_top_lower_dirs(OverlayFSPaths *op, const char *meta_path) {
        ...
        r = append_lower_dir(op, meta_path);
        if (r < 0)
                return r;
        ...
        if (arg_mutable == MUTABLE_IMPORT && op->resolved_mutable_directory) {
                r = append_lower_dir(op, op->resolved_mutable_directory);
                if (r < 0)
                        return r;
        }
...
static int determine_middle_lower_dirs(OverlayFSPaths *op, char **paths, size_t *extensions_used) {
        ...
        STRV_FOREACH(p, paths) {
                ...
                r = consume_lower_dir(op, TAKE_PTR(resolved));
                if (r < 0)
                        return r;
                ++n;
        }
...
static int determine_bottom_lower_dirs(OverlayFSPaths *op) {
        ...
        if (r < 0)
                return r;
        if (!r) {
                r = append_lower_dir(op, op->resolved_hierarchy);
                if (r < 0)
                        return r;
        }
