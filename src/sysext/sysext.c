...
static int move_submounts(const char *src, const char *dst) {
        ...
        FOREACH_ARRAY(m, submounts, n_submounts) {
                ...
                r = umount_verbose(LOG_WARNING, m->path, MNT_DETACH);
                if (r < 0) {
                        /* If we fail to unmount the original path, get rid of the bind mount as well, as
                         * we'll leave the nested mount in place. */
                        r = umount_verbose(LOG_ERR, t, MNT_DETACH);
                        if (r < 0)
                                return r;
                }
        }
