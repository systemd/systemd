...
static int validate_version(
        ...
        r = extension_release_validate_internal(
                        ...
                        image_class_info[arg_image_class].scope_env);
        if (r < 0)
                return log_error_errno(r, "Failed to validate release file information: %m");
...
static int verb_help(int argc, char **argv, void *userdata) {
        ...
        int r;
         ...
         r = terminal_urlify_man(image_class_info[arg_image_class].directory_name, "1", &link);
        if (r < 0)
                return log_oom();
