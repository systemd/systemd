...
static int vl_method_unmerge(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        r = varlink_verify_polkit_async(
                        link,
                        /* bus= */ NULL,
                        image_class == IMAGE_SYSEXT ? "io.systemd.sysext.manage" : "io.systemd.confext.manage",
                        (const char**) STRV_MAKE(
                                "verb", "unmerge",
                                "noReload", yes_no(no_reload)),
                        polkit_registry);
        if (r <= 0)
                return r;
...
static int vl_method_list(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        r = varlink_verify_polkit_async(
                        link,
                        /* bus= */ NULL,
                        image_class == IMAGE_SYSEXT ? "io.systemd.sysext.read" : "io.systemd.confext.read",
