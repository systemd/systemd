...
int acquire_fido2_key(
        ...
        for (;;) {
                ...
                /* Always make an attempt at using the current key slot before asking for PIN.
                 * fido2_use_hmac_hash will perform a pre-flight check for whether the credential for
                 * the current key slot can be found on one of the connected devices. This way, we can
                 * avoid prompting the user for a PIN when we are sure that no device can be used for
...
int acquire_fido2_key_auto(
        ...
        if (r < 0) {
                if (r == -EAGAIN) /* fido2 device does not exist, or UV is blocked (reinsertion needed) */
