...
_public_ int cryptsetup_token_open(
        ...
        r = acquire_luks2_key(
                        ...
                        &decrypted_key_size);
        if (r < 0) {
                crypt_log_err(cd, "Failed to acquire LUKS2 key from tpm.");
                return r;
        }
        ...
        if (r < 0) {
                crypt_log_err(cd, "Not enough memory.");
                return -ENOMEM;
        }
...
_public_ int cryptsetup_token_validate(
        ...
        if (r < 0) {
                if (r == -ENOMEM)
                        crypt_log_dbg(cd, "Could not parse " TOKEN_NAME " json object (not enough memory).");
                return 1;
        }
