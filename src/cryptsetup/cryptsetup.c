...
static int measure_keyslot(
        ...
        r = tpm2_nvpcr_extend_bytes(c, arg_tpm2_measure_keyslot_nvpcr, &IOVEC_MAKE_STRING(s), /* secret= */ NULL, TPM2_EVENT_KEYSLOT, s);
        if (IN_SET(r, -ENOENT, -ENETDOWN)) {
                bool need_allocation = r == -ENOENT;

                /* NvPCR is not allocated or not initialized yet. Do so now. */
                ...
                if (need_allocation) {
                        r = tpm2_nvpcr_allocate(c, /* session= */ NULL, arg_tpm2_measure_keyslot_nvpcr, /* sync_secondary= */ false);
                        if (r < 0)
                                return log_error_errno(r, "Failed to allocate NvPCR index '%s': %m", name);
                }

                r = tpm2_nvpcr_initialize(c, /* session= */ NULL, arg_tpm2_measure_keyslot_nvpcr, &anchor_secret, /* sync_secondary= */ false);
                if (r < 0)
...
static int attach_luks_or_plain_or_bitlk_by_key_data(
        ...
        if (pass_volume_key)
                r = measured_crypt_activate_by_volume_key(cd, name, /* mechanism= */ NULL, /* keyslot= */ -1, key_data, key_data_size, flags);
        else
                r = measured_crypt_activate_by_passphrase(cd, name, /* mechanism= */ NULL, arg_key_slot, key_data, key_data_size, flags);
        if (r == -EPERM) {
