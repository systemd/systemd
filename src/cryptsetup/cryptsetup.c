...
static const char* passphrase_type_to_string(PassphraseType t);
...
static char *make_bindname(const char *volume, TokenType token_type) {
        const char *token_type_name = token_type_to_string(token_type);
        char *bindname;
        int r;

        r = asprintf(&bindname,
                     "@%" PRIx64"/cryptsetup%s%s/%s",
                     random_u64(),
                     token_type_name ? "-" : "",
...
/* since cryptsetup 2.3.0 (Feb 2020) */
                ...
                for (tries = 0; arg_tries == 0 || tries < arg_tries; tries++) {
                        ...
                        if (token_type < 0 && !key_file && !iovec_is_set(&key_data) && !passwords) {
