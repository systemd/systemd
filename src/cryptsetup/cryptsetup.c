...
static char *arg_key_keyring = NULL;
static char *arg_key_type = NULL;
static char *arg_key_description = NULL;
...
STATIC_DESTRUCTOR_REGISTER(arg_key_keyring, freep);
STATIC_DESTRUCTOR_REGISTER(arg_key_type, freep);
STATIC_DESTRUCTOR_REGISTER(arg_key_description, freep);
...
/* since cryptsetup 2.3.0 (Feb 2020) */
        ...
        } else if ((val = startswith(option, "link-volume-key-to-keyring="))) {
#ifdef HAVE_CRYPT_SET_KEYRING_TO_LINK
                const char *sep, *c;
                _cleanup_free_ char *keyring = NULL, *key_type = NULL, *key_description = NULL, *tmp = NULL;
                 ...
                 */
                if (!(sep = strstr(val, "::")))
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Failed to parse link-volume-key-to-keyring= option value: %m");
                ...
                arg_key_keyring = TAKE_PTR(keyring);
                arg_key_type = TAKE_PTR(key_type);
                arg_key_description = TAKE_PTR(key_description);
#else
                log_error("Build lacks libcryptsetup support for linking volume keys in user specificied keyrings upon device activation, ignoring: %s", option);
#endif
...
/* since cryptsetup 2.7.0 (Jan 2024) */
                        ...
                        if (arg_key_description && streq(crypt_get_type(cd), CRYPT_LUKS2)) {
                                r = crypt_set_keyring_to_link(cd, arg_key_description, NULL, arg_key_type, arg_key_keyring);
                                if (r < 0)
                                        return log_error_errno(r, "Failed to set keyring or key description to link volume key in: %m");
                        }
#endif
