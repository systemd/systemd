...
static int parse_one_option(const char *option) {
        ...
        if ((val = startswith(option, "cipher="))) {
                ...
                if (streq(val, "read-only")) {
                        ...
                        SET_FLAG(arg_ask_password_flags, ASK_PASSWORD_ACCEPT_CACHED, r);
                        SET_FLAG(arg_ask_password_flags, ASK_PASSWORD_PUSH_CACHE, r);
                }
...
static int parse_crypt_config(const char *options) {
        ...
        /* This prevents future backward-compatibility issues if we decide to implement caching for PKCS#11 */
        if (FLAGS_SET(arg_ask_password_flags, ASK_PASSWORD_ACCEPT_CACHED) && (arg_pkcs11_uri || arg_pkcs11_uri_auto))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                        ...
                        if (!key_file && !key_data && !arg_pkcs11_uri && !arg_pkcs11_uri_auto && !arg_fido2_device && !arg_fido2_device_auto && !arg_tpm2_device && !arg_tpm2_device_auto) {
                                ...
                                if (arg_try_empty_password) {
                                        ...
                                        r = get_password(
                                                        ...
                                                        source,
                                                        until,
                                                        /* ignore_cached= */ tries > 0 || arg_verify,
                                                        passphrase_type,
                                                        &passwords);
                                        if (r == -EAGAIN)
                                                continue;
