...
/* since cryptsetup 2.3.0 (Feb 2020) */
                ...
                if (streq(val, "auto")) {
                        /* link volume key in @u keyring, key type 'user' (default),
                         * key description derived from uuid */
                        keyring = strdup("@u");
                        if (!keyring)
                                return log_oom();

                } else if (streq(val, "auto-logon")) {
                        /* link volume key in @u keyring, key type 'logon',
                         * key description derived from uuid */
                        ...
                        key_type = strdup("logon");
                        if (!key_type)
                                return log_oom();

                } else if ((sep = strstr(val, "::"))) {
