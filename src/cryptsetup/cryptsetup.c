...
static PasswordRegisteredType check_registered_passwords(struct crypt_device *cd) {
        ...
        for (int slot = 0; slot < slot_max; slot++) {
                ...
                slots[slot] = IN_SET(status, CRYPT_SLOT_ACTIVE, CRYPT_SLOT_ACTIVE_LAST);
        }
        ...
        for (int token = 0; token < sym_crypt_token_max(CRYPT_LUKS2); token++) {
                ...
                if (STR_IN_SET(type, "systemd-recovery", "systemd-pkcs11", "systemd-fido2", "systemd-tpm2")) {
                        ...
                        if (pass_reg_type == _PASSWORD_REGISTERED_TYPE_INVALID && streq(type, "systemd-recovery"))
                                pass_reg_type = PASSWORD_RECOVERY_KEY;
        ...
        for (int slot = 0; slot < slot_max; slot++)
                if (slots[slot])
                        return (pass_reg_type == PASSWORD_RECOVERY_KEY) ? PASSWORD_PASSPHRASE_AND_RECOVERY_KEY : PASSWORD_PASSPHRASE;
