...
static int parse_one_option(const char *option) {
        ...
        if ((val = startswith(option, "cipher="))) {
                ...
                if (arg_ask_password_flags) {
                        log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                        "Duplicate password-echo= option \"%s\", ignoring.", val);
                        return 0;
                }

                if (streq(val, "off"))
                        arg_ask_password_flags = ASK_PASSWORD_SILENT;
                else if (streq(val, "on"))
                        arg_ask_password_flags = ASK_PASSWORD_ECHO;
                else if (streq(val, "masked")) {
                        /* default, do nothing */
                } else {
                        log_error_errno(SYNTHETIC_ERRNO(EINVAL),
