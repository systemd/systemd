...
_public_ int cryptsetup_token_open_pin(
        ...
        /* only char string are supported */
        if (pin && ((strnlen(pin, pin_size) != pin_size) || pin[pin_size] != '\0'))
                return crypt_log_error_errno(cd, EINVAL, "PIN must be character string.");

        return acquire_luks2_key(cd, json, usrptr, pin, password, password_len);
}
