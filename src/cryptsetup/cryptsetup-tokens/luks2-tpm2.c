...
int acquire_luks2_key(
        ...
        if (FLAGS_SET(flags, TPM2_FLAGS_USE_PIN) && !pin)
                return -ENOANO;

        if (FLAGS_SET(flags, TPM2_FLAGS_USE_FIDO2) && FLAGS_SET(fido2_flags, FIDO2ENROLL_PIN) && !pin)
                return -ENOANO;
        ...
        r = tpm2_unseal(tpm2_context,
                        ...
                        pubkey_pcr_mask,
                        signature_json,
                        FLAGS_SET(flags, TPM2_FLAGS_USE_PIN) ? pin : NULL,
                        b64_fido2_secret,
        ...
        if (r == -EILSEQ) {
                /* When there is no separate TPM2 PIN to blame (a wrong FIDO2 token or client PIN would have
                 * failed earlier, and any PIN here is the token's already-accepted client PIN), don't return
                 * -ENOANO: re-prompting for a PIN cannot fix an authValue mismatch caused by inconsistent
                 * metadata or tampering. */
                if (FLAGS_SET(flags, TPM2_FLAGS_USE_FIDO2) &&
