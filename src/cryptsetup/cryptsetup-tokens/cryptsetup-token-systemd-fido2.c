...
_public_ int cryptsetup_token_open_pin(
        ...
        if (pin) {
                if (strnlen(pin, pin_size) != pin_size ||
                    pin[pin_size] != '\0') {
                        crypt_log_error(cd, "PIN must be character string.");
                        return -EINVAL;
                }
...
_public_ void cryptsetup_token_dump(
        ...
        if (crypt_dump_buffer_to_hex_string(cid, cid_size, &cid_str) < 0) {
                crypt_log_debug_errno(cd, r, "Can not dump " TOKEN_NAME " content: %m");
                return;
        }

        if (crypt_dump_buffer_to_hex_string(salt, salt_size, &salt_str) < 0) {
                crypt_log_debug_errno(cd, r, "Can not dump " TOKEN_NAME " content: %m");
                return;
        }
