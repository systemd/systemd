...
_public_ void cryptsetup_token_dump(
        ...
        for (size_t i = 0; i < n_policy_hash; i++)  {
                _cleanup_free_ char *policy_hash_str = NULL;

                r = crypt_dump_buffer_to_hex_string(policy_hash[i].iov_base, policy_hash[i].iov_len, &policy_hash_str);
                if (r < 0)
                        return (void) crypt_log_debug_errno(cd, r, "Cannot dump " TOKEN_NAME " content: %m");
        ...
        for (size_t i = 0; i < n_blobs; i++) {
                ...
                r = crypt_dump_buffer_to_hex_string(blobs[i].iov_base, blobs[i].iov_len, &blob_str);
                if (r < 0)
                        return (void) crypt_log_debug_errno(cd, r, "Cannot dump " TOKEN_NAME " content: %m");
