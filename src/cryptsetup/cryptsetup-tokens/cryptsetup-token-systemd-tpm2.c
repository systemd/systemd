...
static int log_debug_open_error(struct crypt_device *cd, int token, int r) {
        ...
        if (r == -ENOMEDIUM)
                return crypt_log_debug_errno(cd, r, "FIDO2 token not found.");
        if (r == -ENOSTR) {
...
_public_ int cryptsetup_token_open_pin(
        ...
        r = acquire_luks2_key(
                        ...
                        &fido2_cid,
                        &fido2_salt,
                        /* Trust the relying party recorded in the token over the crypttab fido2-rp= option,
                         * which is scoped to standalone systemd-fido2 slots. */
                        fido2_rp ?: params.fido2_rp,
                        fido2_flags,
                        &decrypted_key);
        if (r < 0)
                return log_debug_open_error(cd, token, r);
...
_public_ int cryptsetup_token_validate(
        ...
        r = sd_json_dispatch(v, dispatch_table, SD_JSON_LOG|SD_JSON_DEBUG|SD_JSON_ALLOW_EXTENSIONS, NULL);
        if (r < 0)
