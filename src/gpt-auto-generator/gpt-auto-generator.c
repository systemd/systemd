...
static int add_root_cryptsetup(void) {
        ...
        const char *bdev = arg_auto_root == GPT_AUTO_ROOT_FORCE ?
                "/dev/gpt-auto-root-luks-nofr" : "/dev/gpt-auto-root-luks";
...
static int add_root_mount(void) {
         ...
         *
         * There are two distinct names: the /dev/gpt-auto-root-nofr symlink is always created for the root
         * partition, and the /dev/gpt-auto-root symlink is only created if factory reset mode is either not
         * enabled or already complete. In scenarios where the root disk is reset during factory reset the
         * latter is the link to use, otherwise the former. */

        const char *bdev = arg_auto_root == GPT_AUTO_ROOT_FORCE ?
                "/dev/gpt-auto-root-nofr" : "/dev/gpt-auto-root";
