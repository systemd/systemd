...
static int add_partition_root_growfs(DissectedPartition *p) {
        ...
        (void) generator_hook_up_growfs(arg_dest, "/", SPECIAL_LOCAL_FS_TARGET);
...
static int add_partition_root_flags(DissectedPartition *p) {
        ...
        r = add_partition_root_growfs(p);
        if (r < 0)
                return r;

        r = add_partition_root_rw(p);
        if (r < 0)
                return r;
...
static int add_root_mount(void) {
         ...
         * systemd-fstab-generator will pull it in for us, and otherwise add_partition_root_options() will do
