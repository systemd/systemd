...
static int add_cryptsetup(
        ...
        _cleanup_free_ char *e = NULL, *n = NULL, *d = NULL, *options = NULL, *escaped_path, *failure_target = NULL;
        ...
        r = unit_name_path_escape(what, &escaped_path);
        if (r < 0)
                return log_error_errno(r, "Failed to escape path: %m");
        ...
        if (!arg_interactive_cryptsetup_recovery) {
                ...
                if (r < 0)
                        log_warning_errno(r, "Failed to generate failure target unit name: %m");
                else if (r == 0)
                        fprintf(f,
                                "OnFailure=%s\n",
                                failure_target);
