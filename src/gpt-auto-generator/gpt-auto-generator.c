...
static int add_root_cryptsetup(void) {
        ...
        const char *bdev = arg_auto_root == GPT_AUTO_ROOT_FORCE ?
                "/dev/gpt-auto-root-luks-ignore-factory-reset" : "/dev/gpt-auto-root-luks";
...
static int add_root_mount(void) {
         ...
         *
         * There are two distinct names: the /dev/gpt-auto-root-ignore-factory-reset symlink is always
         * created for the root partition (regardless of the factory reset state), and the /dev/gpt-auto-root
         * symlink is only created if factory reset mode is off or already complete (thus taking factry reset
         * state into account). In scenarios where the root disk is partially reformatted during factory
         * reset the latter is the link to use, otherwise the former (so that we don't accidentally mount a
         * root partition too early that is about to be wiped and replaced by another one). */

        const char *bdev = arg_auto_root == GPT_AUTO_ROOT_FORCE ?
                "/dev/gpt-auto-root-ignore-factory-reset" : "/dev/gpt-auto-root";
