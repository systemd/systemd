...
static int add_partition_esp(DissectedPartition *p, bool has_xbootldr) {
        ...
        /* If /boot is present but unused and empty, we'll take that.
         * Otherwise, if /efi is present but unused, we'll take that.
        ...
        if (!esp_path) {
                if (access("/efi", F_OK) < 0) {
                        ...
                        log_warning_errno(errno,
                                          "/efi does not exist, no place to mount ESP configured or detected.");
                        return 0;
                } else {
