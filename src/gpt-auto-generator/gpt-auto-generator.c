...
static int enumerate_partitions(dev_t devnum) {
        ...
        xbootldr_ok = m->partitions[PARTITION_XBOOTLDR].found;
        ...
        if (is_efi_boot()) {
                ...
                if (r < 0) {
                        ...
                        if (r == -ENOENT)
                                ...
                                log_error_errno(r, "Failed to read loader partition UUID: %m");
                }
                if (r >= 0) {
                        if (xbootldr_ok && sd_id128_equal(m->partitions[PARTITION_XBOOTLDR].uuid, loader_uuid)) {
                        ...
                        if (esp_state == ESP_NEEDS_CHECK && !sd_id128_equal(m->partitions[PARTITION_ESP].uuid, loader_uuid)) {
                                esp_state = ESP_DONT_MOUNT;
