...
static int add_mount(const char *id,
                     const char *what,
                     const char *where,
                     const char *fstype,
                     bool rw,
        ...
        /* Note that we don't apply specifier escaping on the input strings here, since we know they are not
         * configured externally, but all originate from our own sources here, and hence we know they contain
         * no % characters that could potentially be understood as specifiers. */
...
static int add_partition_mount(DissectedPartition *p, const char *id, const char *where, const char *description) {
        ...
        return add_mount(id, p->node, where, p->fstype, p->rw, NULL, description, SPECIAL_LOCAL_FS_TARGET);
}
...
#if ENABLE_EFI
static int add_automount(const char *id,
                         const char *what,
                         const char *where,
                         const char *fstype,
                         bool rw,
...
static int add_esp(DissectedPartition *p) {
        ...
        if (is_efi_boot()) {
                ...
                if (!sd_id128_equal(p->uuid, loader_uuid)) {
                        log_debug("Partition for %s does not appear to be the partition we are booted from.",
                                  esp);
                        return 0;
                }
        } else
...
static int add_root_rw(DissectedPartition *p) {
        ...
        if (arg_root_rw >= 0) {
                log_debug(
                        "Parameter ro/rw specified on kernel command line, not generating drop-in for systemd-remount-fs.service.");
                return 0;
        }

        if (!p->rw) {
                log_debug(
                        "Root partition marked read-only in GPT partition table, not generating drop-in for systemd-remount-fs.service.");
                return 0;
        }
...
static int enumerate_partitions(dev_t devnum) {
        ...
        if (m->partitions[PARTITION_SRV].found) {
                k = add_partition_mount(
                        m->partitions + PARTITION_SRV, "srv", "/srv", "Server Data Partition");
                if (k < 0)
                        r = k;
        }
...
static int add_root_cryptsetup(void) {
        ...
        /* If a device /dev/gpt-auto-root-luks appears, then make it pull in systemd-cryptsetup-root.service,
         * which sets it up, and causes /dev/gpt-auto-root to appear which is all we are looking for. */
