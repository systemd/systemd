...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] /path/to/mountpoint\n"
               "\n%5$sCheck file system validation constraints.%6$s\n"
               "\n%3$sOptions:%4$s\n"
...
static void validate_fields_done(struct ValidateFields *f) {
        assert(f);
...
static int validate_fields_read(int fd, ValidateFields *ret) {
        ...
        r = getxattr_at_malloc(fd, /* path= */ NULL, "user.validatefs.gpt_type_uuid", AT_EMPTY_PATH, &t);
        if (r < 0) {
        ...
        r = getxattr_at_malloc(fd, /* path= */ NULL, "user.validatefs.gpt_label", AT_EMPTY_PATH, &f.gpt_label);
        if (r < 0) {
                if (r != -ENODATA && !ERRNO_IS_NOT_SUPPORTED(r))
                ...
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Extended attribute 'user.validatefs.gpt_label' contains invalid characters, refusing.");

        r = getxattr_at_malloc(fd, /* path= */ NULL, "user.validatefs.mount_point", AT_EMPTY_PATH, &f.mount_point);
        if (r < 0) {
                if (r != -ENODATA && !ERRNO_IS_NOT_SUPPORTED(r))
                        return log_error_errno(r, "Failed to read 'user.validatefs.mount_point' xattr: %m");
        } else if (!utf8_is_valid(f.mount_point) || string_has_cc(f.mount_point, /* ok= */ NULL) || !path_is_valid(f.mount_point) || !path_is_absolute(f.mount_point))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Extended attribute 'user.validatefs.mount_point' is not a valid absolute path or contains invalid characters, refusing.");

        r = !sd_id128_is_null(f.gpt_type_uuid) || f.gpt_label || f.mount_point;
        *ret = TAKE_STRUCT(f);
        return r;
}
...
static int validate_fields_check(int fd, const char *path, const ValidateFields *f) {
        ...
        if (f->mount_point) {
                _cleanup_free_ char *j = arg_root ? path_join(arg_root, f->mount_point) : strdup(f->mount_point);
                if (!j)
                        return log_oom();

                if (!path_equal(path, j))
                        return log_error_errno(SYNTHETIC_ERRNO(EPERM), "File system is supposed to be mounted to '%s' only, but is mounted to '%s', refusing.", f->mount_point, path);
        }
        ...
        if (f->gpt_label || !sd_id128_is_null(f->gpt_type_uuid)) {
                ...
                if (r == _BLKID_SAFEPROBE_AMBIGUOUS)
                        return log_error_errno(SYNTHETIC_ERRNO(ENOPKG), "Found multiple file system labels on block device '%s': %m", path);
                if (r == _BLKID_SAFEPROBE_NOT_FOUND)
                        return log_error_errno(SYNTHETIC_ERRNO(ENOPKG), "Found no file system label on block device '%s': %m", path);
                ...
                if (f->gpt_label) {
                        ...
                        if (!streq_ptr(f->gpt_label, strempty(v)))
                                return log_error_errno(SYNTHETIC_ERRNO(EPERM), "File system is supposed to be placed in a partition with label '%s' only, but is placed in one labelled '%s', refusing.", f->gpt_label, strempty(v));
                }
                ...
                if (!sd_id128_is_null(f->gpt_type_uuid)) {
                        ...
                        if (!v || sd_id128_from_string(v, &id) < 0)
                                return log_error_errno(SYNTHETIC_ERRNO(EPERM), "File system is supposed to be placed in a partition of type UUID '%s' only, but has no type, refusing.", SD_ID128_TO_UUID_STRING(f->gpt_type_uuid));
                        if (!sd_id128_equal(f->gpt_type_uuid, id))
                                return log_error_errno(SYNTHETIC_ERRNO(EPERM), "File system is supposed to be placed in a partition of type UUID '%s' only, but has type '%s', refusing.", SD_ID128_TO_UUID_STRING(f->gpt_type_uuid), SD_ID128_TO_UUID_STRING(id));
                }
        }
...
static int run(int argc, char *argv[]) {
        ...
        _cleanup_close_ int target_fd = chase_and_open(arg_target, /* root= */ NULL, CHASE_MUST_BE_DIRECTORY, O_DIRECTORY|O_CLOEXEC, &resolved);
        if (r < 0)
                return log_error_errno(r, "Failed to open directory '%s': %m", arg_target);
