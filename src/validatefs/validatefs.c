...
static int validate_fields_read(int fd, ValidateFields *ret) {
        ...
        r = getxattr_at_malloc(fd, /* path= */ NULL, "user.validatefs.mount_point", AT_EMPTY_PATH, &nulstr, &nulstr_size);
        if (r < 0) {
                if (r != -ENODATA && !ERRNO_IS_NOT_SUPPORTED(r))
...
static int validate_fields_check(int fd, const char *path, const ValidateFields *f) {
        ...
        if (!strv_isempty(f->mount_point)) {
                ...
                STRV_FOREACH(i, f->mount_point) {
                        ...
                        if (!strextend_with_separator(&bad, ",", j))
                                return log_oom();
                }
                ...
                if (!good)
                        return log_error_errno(
                                        ...
                                        "File system is supposed to be mounted on one of %s only, but is mounted to %s, refusing.",
                                        bad, path);
        }
