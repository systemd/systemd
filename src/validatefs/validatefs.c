...
static int validate_fields_read(int fd, ValidateFields *ret) {
        ...
        } else if (!utf8_is_valid(f.mount_point) ||
                   string_has_cc(f.mount_point, /* ok= */ NULL) ||
                   !path_is_valid(f.mount_point) ||
                   !path_is_absolute(f.mount_point))
                return log_error_errno(
                                SYNTHETIC_ERRNO(EINVAL),
                                "Extended attribute 'user.validatefs.mount_point' is not a valid absolute path or contains invalid characters, refusing.");
        ...
        if (f->mount_point) {
                ...
                if (!path_equal(path, j))
                        return log_error_errno(
                                        ...
                                        "File system is supposed to be mounted to '%s' only, but is mounted to '%s', refusing.",
                                        f->mount_point, path);
        }
