...
static int nvme_subsystem_unlink(NvmeSubsystem *s) {
        ...
        if (s->nvme_subsystems_fd >= 0 && s->name) {
                if (unlinkat(s->nvme_subsystems_fd, s->name, AT_REMOVEDIR) < 0 && errno != ENOENT)
                        log_warning_errno(errno, "Failed to remove NVME subsystem '%s', ignoring: %m", s->name);

                s->nvme_subsystems_fd = safe_close(s->nvme_subsystems_fd); /* Invalidate the subsystems/ dir fd, to remember we unlinked the thing already */
...
static int nvme_subsystem_add(const char *node, int consumed_fd, NvmeSubsystem **ret) {
        ...
        if (r == 0) {
                /* Set a time out for taking the lock */
                alarm(10);
         ...
         * than "self" symlink. This is because this string is visible to others via configfs, and by
