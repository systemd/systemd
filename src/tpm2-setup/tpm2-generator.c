...
static int generate_swtpm_symlink(Tpm2Support support) {
        ...
        if (FLAGS_SET(support, TPM2_SUPPORT_DRIVER)) {
                log_debug("Not generating software TPM units, as TPM2 device is already present.");
                return 0;
        }
        ...
        if (access(bs, F_OK) < 0) {
                if (errno != ENOENT)
                        return log_error_errno(errno, "Failed to determine if '%s' exists: %m", bs);

                log_warning("TPM software fallback requested but not boot secret available, not pulling in software TPM unit.");
                return 0;
        }

        r = find_executable("swtpm", /* ret= */ NULL);
        if (r == -ENOENT) {
        ...
        if (in_initrd())
                 ...
                 * data. Note that it might be mounted to two different places depending on the existance of
...
static int generate_now(void) {
        ...
        /* Let's shortcut things before we check for TPM2 support if noone cares anyway */
         ...
         * all, i.e. if we have a suitale kernel+userspace. */
