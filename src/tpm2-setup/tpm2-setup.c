...
static void cleanup_one_old_nvpcr_cred(const char *path) {
        int r;

        assert(path);

        r = chase_and_unlink(path, /* root= */ NULL, /* chase_flags= */ 0, /* unlink_flags= */ 0, /* ret_path= */ NULL);
        if (r == -ENOENT)
                return;
        if (r < 0)
                return (void) log_debug_errno(r, "Failed to unlink old NvPCR anchor secret credential '%s': %m.", path);

        log_debug("Removed old NvPCR anchor secret credential '%s'.", path);
}

static void cleanup_old_nvpcr_creds(void) {
        int r;

        if (arg_early)
                return;

        cleanup_one_old_nvpcr_cred("/var/lib/systemd/nvpcr/nvpcr-anchor.cred");
        ...
        if (r < 0)
                return (void) log_debug_errno(r, "Failed to read machine ID, not removing old NvPCR anchor secret boot credential: %m");
        ...
        if (!filename_is_valid(fname))
                return (void) log_debug("'%s' is not a valid file name, not removing old NvPCR anchor secret boot credential.", fname);

        _cleanup_free_ char *joined = path_join(dir, fname);
        if (!joined)
                return (void) log_oom();

        cleanup_one_old_nvpcr_cred(joined);
}
