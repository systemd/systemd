...
static int run(int argc, char *argv[]) {
        ...
        const char *esp;
        if (in_initrd())
                /* The eary ESP support uses only a single mount point, we do not need to search for it. */
                esp = "/sysefi";
        else {
                r = find_esp_and_warn(
                                /* root= */ NULL,
        ...
        if (in_initrd() && unlinkat(state_fd, "tpm2-00.volatilestate", /* flags= */ 0) < 0 && errno != ENOENT)
                return log_error_errno(errno, "Failed to remove 'tpm2-00.volatilestate': %m");
