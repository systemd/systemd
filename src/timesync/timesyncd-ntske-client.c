...
#if HAVE_GNUTLS
static int ntske_acquire_auth_data(Manager *m, NTSKEPacket *packet) {
        ...
        r = crypto_random_bytes(&m->nonce, sizeof(m->nonce));
        if (r < 0)
                return log_error_errno(r, "NTS: Failed to acquire random data to generate nonce: %m");

        r = crypto_random_bytes(&m->uid, sizeof(m->uid));
        if (r < 0)
...
int ntske_tls_receive_response(sd_event_source *source, int fd, uint32_t revents, void *userdata) {
        ...
        if (m->ntske_packet)
                packet = TAKE_PTR(m->ntske_packet);
        else {
        ...
        log_debug("Received '%zu' bytes from NTSKE server", packet->size);
        ...
        {
                ServerName *s;
                r = server_name_new(m, &s, SERVER_SYSTEM,
                                    packet->server ? packet->server : m->current_ntske_server_name->string);
                if (r < 0) {
                        log_error_errno(r, "Failed to add NTP server '%s' received from NTSKE server: %m",
                                        packet->server ?: m->current_ntske_server_name->string);
                        goto error;
                }
        }

#if HAVE_GNUTLS
        r = ntske_acquire_auth_data(m, packet);
        if (r < 0)
                goto error;
#endif
...
int ntske_tls_connect(Manager *m) {
        ...
        if (m->current_ntske_server_name->string) {
                r = gnutls_server_name_set(tls_session, GNUTLS_NAME_DNS,
                                           m->current_ntske_server_name->string,
                                           strlen(m->current_ntske_server_name->string));
                if (r < 0)
                        return r;
        }
...
void ntske_tls_bye(Manager *m) {
        ...
        if (r < 0) {
                if (gnutls_error_is_fatal(r))
                        log_error_errno(r, "GnuTLS Shutdown failed with '%s': %m", gnutls_strerror(r));
        }
