...
int ntske_append_record(NTSKEPacket *packet, uint16_t type, const void *data, size_t data_size, bool critical) {
        ...
        new_data = realloc(packet->data, packet->size + sizeof(NTSKERecord) + data_size);
        if (!new_data)
                return -ENOMEM;
...
int ntske_parse_packet(NTSKEPacket *packet) {
        ...
        for (; !end && ntske_read_record(packet, &type, &data, &data_size, &critical);) {
                ...
                case NTS_KE_RECORD_COOKIE: {
                        ...
                        if (!GREEDY_REALLOC(packet->cookies, n_cookies + 1))
                                return log_oom();
                ...
                case NTS_KE_RECORD_NTPV4_SERVER_NEGOTIATION: {
                        ...
                        if (data_size == 0)
                                return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
                                                       "NTSKE: NTPv4 server negotiation field has wrong size, ignoring packet");
                ...
                case NTS_KE_RECORD_NTPV4_PORT_NEGOTIATION:
                        if (data_size != sizeof(uint16_t))
                                return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
                                                       "NTSKE: NTPv4 port negotiation field has wrong size, ignoring packet");

                        packet->port = be16toh(*p);
                        break;
                default:
                        break;
                }
        ...
        log_debug("NTSKE: Received Next Protocol='%d', AEAD Algorithm='%d', Server='%s', N Cookies='%zu' of size='%zu' each",
                  packet->next_protocol, packet->aead_algorithm, strna(packet->server),
                  n_cookies, packet->cookies[n_cookies-1].size);
