...
static int manager_receive_response(sd_event_source *source, int fd, uint32_t revents, void *userdata) {
        ...
        if (precision > NTP_ACCURACY_SEC) {
                if (offset < -precision) {
                        ...
                        log_debug("Received NTP response with low precision, mitigating rounding clock offset %+.3f sec to %+.3f sec.",
                         ...
                         */
                        log_debug("Received NTP response with low precision, accepting clock offset %+.3f sec.",
                                offset);
                         ...
                         */
                        offset_adjusted = 0;
                        log_debug("Received NTP response with low precision, offset (%+.3f sec) less than precision, ignoring",
                                offset);
