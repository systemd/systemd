...
static int method_set_runtime_servers(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        _cleanup_strv_free_ char **valid_names = NULL;
        ...
        STRV_FOREACH(name, msg_names) {
                ...
                strv_push(&valid_names, strdup(*name));
        }

        STRV_FOREACH(name, valid_names) {
        ...
        if (manager_is_connected(m)) {
                if (m->current_server_address &&
                    ...
                    strv_contains(valid_names, m->current_server_address->name->string)) {
