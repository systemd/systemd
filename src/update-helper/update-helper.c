...
static int read_unit_paths_from_stdin(char ***ret) {
        ...
        for (;;) {
                ...
                if (!PATH_IN_SET(d, SYSTEM_DATA_UNIT_DIR, USER_DATA_UNIT_DIR)) {
                        log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Path '%s' is outside of the systemd unit directories, ignoring", d);
                        continue;
                }
                ...
                struct stat st;
                r = RET_NERRNO(stat(absolute, &st));
                if (r >= 0) {
                        if (arg_removed) {
                                log_debug("--removed was specified and %s still exists on disk, ignoring", absolute);
                                continue;
                        }
                        ...
                        log_debug_errno(r, "Failed to stat '%s', ignoring: %m", absolute);
                        continue;
                }

                if (!unit_name_is_valid(f, UNIT_NAME_ANY)) {
                        log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "'%s' is not a valid unit name, ignoring", f);
                        continue;
                }
