...
static int help(void) {
        printf("%1$s [OPTIONS...] COMMAND [UNITS...]\n\n"
               ...
               "     --removed                      Operate on units that were removed on disk\n"
               "\n%2$sCommands:%3$s\n"
...
static int parse_unit(const char *s, bool fatal, char **ret) {
        ...
        if (!PATH_IN_SET(d, SYSTEM_DATA_UNIT_DIR, USER_DATA_UNIT_DIR))
                return log_full_errno(priority, SYNTHETIC_ERRNO(EINVAL),
                                      "Path '%s' is outside of the systemd unit directories%s", d, suffix);
        ...
        if (r >= 0) {
                if (arg_removed)
                        return log_full_errno(priority, SYNTHETIC_ERRNO(EEXIST),
                                              "--removed was specified and %s still exists on disk%s",
                                              s, suffix);

                if (!S_ISREG(st.st_mode) || S_ISLNK(st.st_mode))
                        return log_full_errno(priority, SYNTHETIC_ERRNO(EINVAL),
                                              "'%s' is not a regular file%s", s, suffix);
        } else if (!arg_removed || r != -ENOENT)
...
static int finalize_units(int argc, char **argv, char ***ret) {
        ...
        if (arg_stdin) {
                for (;;) {
                        ...
                        if (arg_removed && !path_is_absolute(line))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "All arguments must be absolute paths when --removed is used");

                        _cleanup_free_ char *u = NULL;
                        r = parse_unit(line, /* fatal= */ false, &u);
                        if (r < 0)
                                continue;
                ...
                STRV_FOREACH(s, strv_skip(argv, 1)) {
                        ...
                        if (arg_removed && !path_is_absolute(*s))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "All arguments must be absolute paths when --removed is used");

                        r = parse_unit(*s, /* fatal= */ true, &u);
                        if (r < 0)
                                return r;
...
static int remove_units(int argc, char **argv, RuntimeScope scope) {
        ...
        if (scope == RUNTIME_SCOPE_SYSTEM) {
                ...
                r = bus_wait_for_jobs(w, arg_quiet ? BUS_WAIT_JOBS_LOG_ERROR : 0);
                if (r < 0)
                        return r;
        } else {
                ...
                STRV_FOREACH(user, users) {
                        ...
                        bus_wait_for_jobs_set_bus_own(w, true);
