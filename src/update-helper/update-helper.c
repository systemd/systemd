...
static int bus_connect_user_unit(const char *unit, sd_bus **ret) {
        ...
        for (;;) {
                ...
                if (r > 0)
                        continue;
                if (r < 0)
                        return log_full_errno(arg_quiet ? LOG_DEBUG : LOG_WARNING, r,
                                              "%s: Failed to process bus, ignoring: %m", unit);

                if (sd_bus_is_ready(bus))
                        break;
                if (!sd_bus_is_open(bus))
                        return log_full_errno(arg_quiet ? LOG_DEBUG : LOG_WARNING, SYNTHETIC_ERRNO(ENOTCONN),
                                              "%s: Failed to connect to bus, ignoring", unit);

                r = sd_bus_wait(bus, 10 * USEC_PER_SEC);
                if (r == 0)
                        return log_full_errno(arg_quiet ? LOG_DEBUG : LOG_WARNING, SYNTHETIC_ERRNO(ETIMEDOUT),
