...
static int parse_unit(const char *s, bool fatal, char **ret) {
        ...
        if (!PATH_IN_SET(d, SYSTEM_DATA_UNIT_DIR, SYSTEM_CONFIG_UNIT_DIR, USER_DATA_UNIT_DIR, USER_CONFIG_UNIT_DIR))
                return log_full_errno(priority, SYNTHETIC_ERRNO(EINVAL),
                                      "'%s' is outside of the systemd unit directories%s", s, suffix);
