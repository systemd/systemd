...
static int make_overlay(const char *root_path) {
        ...
        escaped_path = shell_escape(root_path, ",:");
        if (!escaped_path) {
                r = log_oom();
                goto finish;
        }
        ...
        r = mount_nofollow_verbose(LOG_ERR, "overlay", root_path, "overlay", 0, options);
