...
static int make_overlay(const char *root, VolatileMode overlay_type) {
        _cleanup_free_ char *escaped_path = NULL, *usr_path = NULL, *mount_path = NULL;
        const char *overlay_name;
        bool overlay_dir_created = false;
        ...
        const char *overlay_dir = strjoina("/run/systemd/", overlay_name);
        const char *upper_dir = strjoina(overlay_dir, "/upper");
        const char *work_dir = strjoina(overlay_dir, "/work");

        /* Mount tmpfs to overlay_dir if it is not a mountpoint already */
        r = path_is_mount_point_full(overlay_dir, /* root= */ NULL, AT_SYMLINK_FOLLOW);
        if (r < 0 && r != -ENOENT)
                return log_error_errno(r, "Couldn't determine whether %s is a mount point: %m", overlay_dir);
        if (r == 0 || r == -ENOENT) {
                if (r == -ENOENT ) {
        ...
        r = mkdir_p(upper_dir, 0755);
        if (r < 0) {
                log_error_errno(r, "Failed to create %s: %m", upper_dir);
                goto finish;
        }

        r = mkdir_p(work_dir, 0755);
        if (r < 0) {
                log_error_errno(r, "Failed to create %s: %m", work_dir);
                goto finish;
        }
        ...
        if (overlay_type == VOLATILE_OVERLAY_USR) {
                ...
                if (r < 0) {
                        log_error_errno(r, "/usr not available in sysroot: %m");
                        goto finish;
                }
        }
        ...
        const char *options = strjoina("lowerdir=", escaped_path, ",upperdir=", upper_dir, ",workdir=", work_dir);
...
static int run(int argc, char *argv[]) {
        ...
        switch (m) {
        case VOLATILE_YES:
                r = make_volatile(root);
                break;
        case VOLATILE_OVERLAY_ROOT:
                r = make_overlay(root, VOLATILE_OVERLAY_ROOT);
                break;
        case VOLATILE_OVERLAY_USR:
                r = make_overlay(root, VOLATILE_OVERLAY_USR);
                break;
        default:
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid volatile mode: %d", m);
        }

        if (r < 0 && symlink_created)
                unlink("/run/systemd/volatile-root");
