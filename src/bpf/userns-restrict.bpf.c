 ...
 * through the map, where the up flag selects which end of the extents is searched. Returns UID_GID_INVALID
 * if the ID is not mapped. */
static uid_t uid_gid_map_translate(struct uid_gid_map *map, uid_t id, bool up) {
        struct uid_gid_extent *array;
        unsigned n;

        n = map->nr_extents;

        /* Small maps carry their extents inline, larger ones keep two arrays instead, each sorted by one
         * end. The kernel bisects the latter, a linear scan arrives at the same answer.
         *
         * Those two pointers share storage with the inline extent array. The verifier resolves the overlap
         * to the latter and then refuses an 8 byte load in the middle of it, so read them out explicitly.
         * Both offsets have to stay compile time constants for that, hence the duplicated branches. */
        if (n <= UID_GID_MAP_MAX_BASE_EXTENTS)
                array = map->extent;
        else if (up) {
                if (bpf_probe_read_kernel(&array, sizeof(array),
                                          (const void*) map + bpf_core_field_offset(struct uid_gid_map, reverse)) != 0)
                        return UID_GID_INVALID;
        } else {
                if (bpf_probe_read_kernel(&array, sizeof(array),
        ...
        for (unsigned i = 0; i < UID_GID_MAP_MAX_EXTENTS; i++) {
                ...
                if (id >= first && id - first < e.count)
                        return (up ? e.first : e.lower_first) + (id - first);
        }
 ...
 * Returns UID_GID_INVALID if the ID does not map, in which case the kernel refuses the operation with
 * EOVERFLOW anyway, see fsuidgid_has_mapping(). */
static uid_t mount_map_id(struct vfsmount *mnt, struct user_namespace *fs_userns, uid_t id, bool gid) {
        ...
        if (m->nr_extents > 0) {
                id = uid_gid_map_translate(m, id, /* up= */ true);
        ...
        return uid_gid_map_translate(gid ? &fs_userns->gid_map : &fs_userns->uid_map, id, /* up= */ false);
}
...
static int validate_mount(struct vfsmount *v, int ret) {
        ...
        uid = mount_map_id(v, fs_userns, cred->fsuid.val, /* gid= */ false);
        gid = mount_map_id(v, fs_userns, (uid_t) cred->fsgid.val, /* gid= */ true);
...
static bool idmap_map_neutralizes_transient(struct uid_gid_map *map) {
        ...
        /* Same inline-vs-pointer-array split as uid_gid_map_translate(); we scan every extent, so either
         * of the two sorted arrays would do, and we read the one used for the "up" direction. */
        if (n <= UID_GID_MAP_MAX_BASE_EXTENTS)
                array = map->extent;
        else if (bpf_probe_read_kernel(&array, sizeof(array),
                                       (const void*) map + bpf_core_field_offset(struct uid_gid_map, reverse)) != 0)
                return false;
        if (!array)
                return false;
...
int BPF_PROG(userns_restrict_sb_kern_mount, struct super_block *sb, int ret) {
        ...
        upper_addr = BPF_CORE_READ(ofs, layers, mnt); /* layers[0] is the upper; .mnt is its first member */
...
int BPF_PROG(userns_restrict_task_fix_setgroups, struct cred *new_cred, const struct cred *old, int ret) {
        ...
        p = new_cred->user_ns;
        for (unsigned i = 0; i < USER_NAMESPACE_DEPTH_MAX; i++) {
                if (!p) /* No nsresourced-managed ancestor found, allow. */
                        return 0;
