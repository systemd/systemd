...
static int query_bpf_progs_and_maps(void) {
        ...
        r = varlink_collect(vl, "io.systemd.Bpf.Describe", i, &o, NULL, NULL);
        if (r < 0)
        ...
        JSON_VARIANT_ARRAY_FOREACH(i, o) {
                ...
                if (streq(json_variant_string(m), "map")) {
                        ...
                        r = table_add_many(map_table,
                                   ...
                                   TABLE_STRING, p.memlock, TABLE_SET_ALIGN_PERCENT, 100);
                } else {
                        static const JsonDispatch dispatch_table[] = {
                                ...
                                { "memlock", JSON_VARIANT_STRING, json_dispatch_string, offsetof(struct BpfProgramData, memlock), JSON_MANDATORY},
                                { "tag", JSON_VARIANT_STRING, json_dispatch_string, offsetof(struct BpfProgramData, tag), JSON_MANDATORY},
                                { "map_names", JSON_VARIANT_STRING, json_dispatch_string, offsetof(struct BpfProgramData, map_names), 0},
                ...
                // table_print(prog_table, NULL);
        }
        table_print(map_table, NULL);
