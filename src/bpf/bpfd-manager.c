...
static int fetch_bpf_maps_and_progs(Varlink *link) {
        ...
        while (sym_bpf_prog_get_next_id(id, &id) >= 0) {
                ...
                r = get_memlock_by_fd(fd, &memlock);
                if (r < 0) {
                        log_debug_errno(r, "Failed to read memlock from /proc/self/fdinfo/%i", fd);
                        continue;
                }
                ...
                if (link) {
                        ...
                        /* Check if this is the final bpf program. If so, send a varlink reply instead of notify */
