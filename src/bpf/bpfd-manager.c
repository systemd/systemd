...
static int get_memlock_by_fd(int fd, char **ret_memlock) {
        _cleanup_free_ char *fdinfo = NULL;
        char *memlock, *memlock_line;
...
static int fetch_bpf_maps_and_progs(Varlink *link) {
        ...
        while (sym_bpf_map_get_next_id(id, &id) >= 0) {
                ...
                if (r < 0) {
                        log_error_errno(r, "Can't get bpf map info for fd %i: %m", fd);
                        continue;
                }
                ...
                if (r < 0) {
                        log_error_errno(r, "Failed to read map memlock from /proc/self/fdinfo/%i: %m", fd);
                        continue;
                }
                ...
                if (link) {
                        ...
                        if (r < 0) {
                                log_error_errno(r, "Failed to varlink notify: %m");
                                continue;
                        }
                } else
        ...
        while (sym_bpf_prog_get_next_id(id, &id) >= 0) {
                _cleanup_close_ int fd = -EBADF;
                _cleanup_free_ char *memlock = NULL, *prog_name = NULL, *prog_map_names = NULL;
                ...
                if (r < 0) {
                        log_error_errno(r, "Can't get bpf prog info for fd %i: %m", fd);
                        continue;
                }
                ...
                if (nr_map_ids > 0) {
                        ...
                        if (!prog_map_ids)
                                return -ENOMEM;
                        prog_map_names = new(char, nr_map_ids * BPF_OBJ_NAME_LEN);
                        if (!prog_map_names)
                                return -ENOMEM;
                        ...
                        FOREACH_ARRAY(j, prog_map_ids, nr_map_ids) {
                                ...
                                if (map_name == NULL) {
                                        log_error_errno(r, "Can't find map in map_name_by_id %u: %m", map_id);
                                        continue;
                                }
                ...
                r = get_memlock_by_fd(fd, &memlock);
                if (r < 0) {
                        log_error_errno(r, "Failed to read memlock from /proc/self/fdinfo/%i", fd);
                        continue;
                }
                ...
                if (strlen(prog_info.name) == 0 || strlen(prog_info.name) >= BPF_OBJ_NAME_LEN - 1) {
                        ...
                        if (prog_info.nr_func_info == 0) {
                                log_error("0 func infos found for prog id %u", id);
                                continue;
                        }
                        ...
                        r = sym_bpf_obj_get_info_by_fd(fd, &prog_info, &prog_info_len);
                        if (r < 0) {
                                log_error_errno(r, "Couldn't get func info for prog id %u: %m", id);
                                continue;
                        }
                        ...
                        if (sym_libbpf_get_error(btf)) {
                                log_error_errno(r, "Failed to load btf for prog fd %i, %m", fd);
                                continue;
                        }
                ...
                if (r < 0) {
                        log_error_errno(r, "Failed to build JSON object, ignoring: %m");
                        continue;
                }
                ...
                if (link) {
                        ...
                        if (sym_bpf_prog_get_next_id(id, &next_id) < 0) {
                                ...
                                if (r < 0) {
                                        log_error_errno(r, "Failed to varlink reply: %m");
                                        continue;
                                }
                        } else {
                                r = varlink_notify(link, v);
                        ...
                        json_variant_append_array(&prog_and_map_list, v);
                        if (sym_bpf_prog_get_next_id(id, &next_id) >= 0)
                                json_variant_dump(prog_and_map_list, JSON_FORMAT_PRETTY, NULL, NULL);
                }
        }
