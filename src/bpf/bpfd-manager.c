...
static void toggle_bpf_stats_enabled(bool enable) {
        ...
        r = write_string_file("/proc/sys/kernel/bpf_stats_enabled", value, WRITE_STRING_FILE_DISABLE_BUFFER);
        if (r < 0)
                log_error_errno(r, "Failed to write '%c' to /proc/sys/kernel/bpf_stats_enabled, ignoring: %m", *value);
}
...
static int get_memlock_by_fd(int fd, uint64_t *ret_memlock) {
        ...
        r = safe_atou64(memlock_line, &v);
        if (r < 0)
                return log_error_errno(r, "Failed to convert memlock string to unsigned integer, ignoring: %m");
...
static int fetch_bpf_maps_and_progs(Varlink *link) {
        ...
        uint32_t id = 0;
        JsonVariant *prog_and_map_list = NULL;
        int r;
        ...
        while (sym_bpf_map_get_next_id(id, &id) >= 0) {
                ...
                if (!map_name)
                        return log_oom();
                r = hashmap_put(map_name_by_id, UINT32_TO_PTR(id), map_name);
                if (r < 0) {
                ...
                if (link) {
                        ...
                        if (r < 0) {
                                log_error_errno(r, "Failed to varlink notify for map, ignoring: %m");
                                continue;
                        }
                } else {
                        r = json_variant_append_array(&prog_and_map_list, v);
                        if (r < 0) {
        ...
        while (sym_bpf_prog_get_next_id(id, &id) >= 0) {
                ...
                uint32_t nr_map_ids = 0, next_id = 0;
                uint64_t memlock = 0;
                _cleanup_close_ int fd = -EBADF;
                ...
                if (r < 0) {
                        log_error_errno(r, "Can't get bpf prog info for fd %i, ignoring: %m", fd);
                        continue;
                }
                ...
                if (nr_map_ids > 0) {
                        ...
                        FOREACH_ARRAY(j, prog_map_ids, nr_map_ids) {
                                ...
                                if (map_name == NULL) {
                                        log_error_errno(r, "Can't find map in map_name_by_id %u, ignoring: %m", map_id);
                                        continue;
                                }
                ...
                if (isempty(prog_info.name) || strlen(prog_info.name) >= BPF_OBJ_NAME_LEN - 1) {
                        ...
                        r = sym_bpf_obj_get_info_by_fd(fd, &prog_info, &prog_info_len);
                        if (r < 0) {
                                log_error_errno(r, "Couldn't get func info for prog id %u, ignoring: %m", id);
                                continue;
                        }
                ...
                if (link) {
                        /* Check if this is the last bpf program. If it is, varlink_reply instead of varlink_notify */
                        if (sym_bpf_prog_get_next_id(id, &next_id) >= 0) {
                                r = varlink_notify(link, v);
                                if (r < 0) {
                                        log_error_errno(r, "Failed to varlink notify for program, ignoring: %m");
                                        continue;
                                }
                        } else {
                                r = varlink_reply(link, v);
                                if (r < 0) {
                                        log_error_errno(r, "Failed to varlink reply for program, ignoring: %m");
                                        continue;
                                }
                        }
                } else {
                        json_variant_append_array(&prog_and_map_list, v);
                        if (r < 0) {
                                log_error_errno(r, "Failed to append to json variant array for program, ignoring: %m");
                                continue;
                        }
                        /* If this is the last bpf program, log everything we've accumulated */
                        if (sym_bpf_prog_get_next_id(id, &next_id) < 0) {
                                _cleanup_(erase_and_freep) char *formatted = NULL;

                                r = json_variant_format(prog_and_map_list, JSON_FORMAT_NEWLINE, &formatted);
                                if (r < 0) {
                                        log_error_errno(r, "Failed to format list of bpf programs and maps, ignoring: %m");
                                        continue;
                                }

                                log_struct(LOG_INFO, "MESSAGE_ID=" SD_MESSAGE_BPFD_LOG_STR, "DATA=%s", formatted);
                        }
                }
        }
...
static int manager_bind_varlink(Manager *m) {
        ...
        r = varlink_server_bind_method(
                        m->varlink_server,
                        "io.systemd.Bpf.Describe", vl_method_describe
        );
        if (r < 0)
