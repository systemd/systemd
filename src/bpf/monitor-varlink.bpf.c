 ...
 *
 * Userspace downgrades this to BPF_MAP_TYPE_HASH with a lower max_entries
 * on kernels that don't support RHASH. */
struct {
        __uint(type, BPF_MAP_TYPE_RHASH);
        __uint(max_entries, MONITOR_VARLINK_MAX_SOCKETS);
...
/* __builtin_memcmp may emit a memcmp call for large sizes and
 * bpf_strncmp requires s2 in a read-only map, but the filter map
 * must be writable for userspace to populate it. */
static __always_inline bool path_equal(const char *a, const char *b, __u8 len) {
        for (int i = 0; i < MONITOR_VARLINK_MAX_PATH && i < len; i++)
                if (a[i] != b[i])
                        return false;
        return true;
}
        ...
        for (__u32 i = 0; i < MONITOR_VARLINK_MAX_FILTERS; i++) {
                ...
                if (f->has_path) {
                        if (!ps || ps->len != f->path_len)
                                continue;
                        if (ps->len > 0 && !path_equal(ps->path, f->path, ps->len))
                                continue;
                }
...
int BPF_PROG(
        ...
        /* Split into fixed-size packets; data beyond MONITOR_VARLINK_MAX_PACKETS *
