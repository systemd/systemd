...
static const char *context_runtime_directory(Context *c) {
        assert(c);
...
static int context_save_ifname(Context *c) {
        ...
        const char *d = context_runtime_directory(c);
        if (!d)
                return 0;
        ...
        if (curl == c->curl_token) {
                ...
                if (status == 404) {
                        ...
                        if (fsetxattr(c->cache_fd, "user.imds.result", "key-not-found", STRLEN("key-not-found"), /* flags= */ 0) < 0)
                                ...
                                context_log(c, LOG_DEBUG, "Cached data not found.");
                        }
...
static CacheResult context_process_cache(Context *c) {
        ...
        const char *e = context_runtime_directory(c);
        if (!e)
...
static int context_find_if(Context *c) {
        ...
        c->ifindex = rtnl_resolve_interface_or_warn(&c->rtnl, ifname);
        if (c->ifindex < 0)
                return c->ifindex;
...
static int setup_address_rrs(void) {
        ...
        /* Creates local RRs (honoured by systemd-resolved) for the IMDS address. */
...
static int vl_method_get(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (c->ifindex > 0) {
                ...
                if (cr == CACHE_RESULT_KEY_NOT_FOUND) {
                        context_fail_full(c, context_log_errno(c, LOG_ERR, SYNTHETIC_ERRNO(ENOENT), "Cache reports: key not found"), "io.systemd.InstanceMetadata.KeyNotFound");
                        return 0;
                }
...
static int vl_method_get_vendor_info(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        /* NB! We allow acces to this call without Polkit */
