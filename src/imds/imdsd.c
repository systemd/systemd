...
#define DIRECT_OVERALL_TIMEOUT_SEC (40U * USEC_PER_SEC) /* a bit shorter than the default D-Bus/Varlink method call time-out) */
#define INDIRECT_OVERALL_TIMEOUT_SEC (DIRECT_OVERALL_TIMEOUT_SEC + 5U * USEC_PER_SEC)
...
static int on_retry(sd_event_source *s, uint64_t usec, void *userdata) {
        ...
        /* Maybe some other instance was successful in the menatime and already found something? */
...
static int context_acquire_http_status(Context *c, CURL *curl, long *ret_status) {
        ...
        if (IN_SET(status,
                   503, /* AWS + GCP */
                   429  /* Azure + GCP */))
                return context_schedule_retry(c);
...
static void curl_glue_on_finished(CurlGlue *g, CURL *curl, CURLcode result) {
        ...
        case CURLE_WRITE_ERROR:
                /* CURLE_WRITE_ERROR we'll see if the data callbacks failed already. But if they did, they
                 * already cancelled the whole operation, and we don't have to do anything here */
                return;
        ...
        if (curl == c->curl_token) {
                ...
                context_log(c, LOG_DEBUG, "Token successfully acquired: %s", c->token_string);
...
static size_t data_write_callback(void *contents, size_t size, size_t nmemb, void *userdata) {
        ...
        if (r == -EADDRNOTAVAIL) {
                context_fail_full(c, r, "io.systemd.InstanceMetadata.NotAvailable");
                return 0;
        }
        if (r < 0) {
                context_fail(c, r);
        ...
        if (c->write_stdout)
                fwrite(contents, sz, nmemb, stdout);
...
static int vl_on_reply(sd_varlink *link, sd_json_variant *m, const char *error_id, sd_varlink_reply_flags_t flags, void *userdata) {
        ...
        if (error_id) {
                ...
                /* We shall no retry immediately. In that case, we give up on the child, and propagate the
...
static int context_spawn_child(Context *c, int ifindex, sd_varlink **ret) {
         ...
         * instances of ourselves, one for earch interface, and collect the results. We communicate with
        ...
        r = sd_varlink_invokebo(
                        ...
                        SD_JSON_BUILD_PAIR_CONDITION(!c->fwmark_set, "firewallMark", SD_JSON_BUILD_NULL)); /* explicitly turn of fwmark, if not set */
        if (r < 0)
...
static int context_spawn_children(Context *c) {
        ...
        /* If we we don't know yet on which interface to query, let's see which interfaces there are and
         * spawwn ourselves, once on each */
...
static int setup_network(void) {
        ...
        if (fchmod(fileno(f), 0644) < 0)
                return log_error_errno(errno, "Failed set access mode for 85-imds-early.network: %m");
...
static int cmdline_run(void) {
        ...
        if (c.ifindex > 0) {
                ...
                r = context_start_overall_timeout(&c, DIRECT_OVERALL_TIMEOUT_SEC);
                if (r < 0)
                        return r;
        } else {
                ...
                r = context_start_overall_timeout(&c, INDIRECT_OVERALL_TIMEOUT_SEC);
                if (r < 0)
                        return r;
        }
...
static int parse_argv(int argc, char *argv[]) {
        ...
        static const struct option options[] = {
                ...
                { "setup-network",       no_argument,       NULL, ARG_SETUP_NETWORK       },

                /* The following all configure endpoint information explicitly */
                { "vendor",              required_argument, NULL, ARG_VENDOR              },
                ...
                { "well-known-key",      required_argument, NULL, ARG_WELL_KNOWN_KEY      },
                {}
        };
        ...
        while ((c = getopt_long(argc, argv, "hi:wK:", options, NULL)) >= 0) {
                ...
                case 'i':
                        ...
                        if (!ifname_valid_full(optarg, IFNAME_VALID_ALTERNATIVE|IFNAME_VALID_NUMERIC))
                ...
                case ARG_WELL_KNOWN_KEY: {
                        ...
                        if (!e)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "--well-known-key= expects colon separate name and key pairs.");

                        _cleanup_free_ char *name = strndup(optarg, e - optarg);
                        if (!name)
                                return log_oom();
        ...
        if (!arg_varlink) {
                ...
                if (arg_setup_network) {
                        ...
                        } else if (argc > optind+1) /* if not, then the additional parameter is optional */
...
static int secure_getenv_ip_address(
        ...
        /* Parses and IP address specified in an environment variable */
