...
static size_t data_write_callback(void *contents, size_t size, size_t nmemb, void *userdata) {
        ...
        /* Before we use acquired data, let's verify the HTTP status, if ther's a failure or we need to
         * restart, abort the write here. Note that the curl_glue_on_finish() call will then check the HTTP
