...
static int help(void) {
        ...
        printf("%s [OPTIONS...] [KEY...]\n"
               ...
               "  -K --well-known=     Select well-known key/base\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        if (IN_SET(arg_action, ACTION_USERDATA, ACTION_IMPORT)) {
                ...
                if (argc == optind)
                        ...
                        else if (!imds_well_known_can_suffix(arg_well_known))
...
static int acquire_imds_key(
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to issue Get(): %m");
        if (error_id) {
                ...
                return log_error_errno(sd_varlink_error_to_errno(error_id, reply), "Failed to issue Get(): %s", error_id);
        }
...
static int run(int argc, char* argv[]) {
        ...
        // FIXME: replace by call to service via AF_UNIX
        _cleanup_free_ char *p = NULL;
        _cleanup_close_ int pin_fd = -EBADF;
        pin_fd = pin_callout_binary("/usr/lib/systemd/systemd-imdsd", &p);
        if (pin_fd < 0)
                return log_error_errno(pin_fd, "Failed to pick up imdsd binary: %m");

        _cleanup_(sd_varlink_unrefp) sd_varlink *link = NULL;
        r = sd_varlink_connect_exec(&link, p, NULL);
        if (r < 0)
