...
static int write_credential(const char *dir, const char *name, const struct iovec *data) {
        ...
        if (faccessat(dfd, name, F_OK, AT_SYMLINK_FOLLOW) < 0) {
                if (errno != ENOENT)
        ...
        r = link_tmpfile_at(dfd, dfd, t, name, /* flags= */ 0);
        if (r < 0)
...
static int import_imds_addresses(sd_varlink *link) {
        int r, ret = 0;
...
static int import_imds_ssh_key(sd_varlink *link) {
        ...
        log_info("Imported hostname as credential 'ssh.authorized_keys.root'.");
        return 0;
}
...
static int action_import(sd_varlink *link) {
        ...
        int ret = import_imds_addresses(link);
