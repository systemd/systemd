...
static int help(void) {
        ...
        printf("%s [OPTIONS...] [KEY...]\n"
               ...
               "  -K --well-known=[base|hostname|region|zone|ipv4-public|ipv6-public|ssh-key|\n"
...
static int action_summary(sd_varlink *link) {
        ...
        r = sd_varlink_call(
                        ...
                        &error_id);
        if (r < 0)
                return log_error_errno(r, "Failed to issue io.systemd.InstanceMetadata.GetStatus(): %m");
        if (error_id)
                return log_error_errno(sd_varlink_error_to_errno(error_id, reply), "Failed to issue io.systemd.InstanceMetadata.GetStatus(): %s", error_id);
...
static int import_credential_one(CredentialData *d) {
        ...
        dfd = open_mkdir(dir, O_CLOEXEC|O_PATH, 0700);
        if (dfd < 0)
                return log_error_errno(dfd, "Failed to open credential directory '%s': %m", dir);

        if (faccessat(dfd, d->name, F_OK, AT_SYMLINK_FOLLOW) < 0) {
                if (errno != ENOENT)
                        return log_error_errno(errno, "Failed to check if '%s' exists in credential directory '%s': %m", d->name, dir);
        } else {
                log_notice("Skipping importing of credential '%s', it already exists locally in '%s'.", d->name, dir);
                return 0;
        }

        _cleanup_free_ char *t = NULL;
        _cleanup_close_ int fd = open_tmpfile_linkable_at(dfd, d->name, O_WRONLY|O_CLOEXEC, &t);
        if (fd < 0)
                return log_error_errno(fd, "Failed to create credential file '%s/%s': %m", dir, d->name);

        CLEANUP_TMPFILE_AT(dfd, t);

        r = loop_write(fd, v->iov_base, v->iov_len);
        if (r < 0)
                return log_error_errno(r, "Failed to write credential file '%s/%s': %m", dir, d->name);

        if (fchmod(fd, 0400) < 0)
                return log_error_errno(errno, "Failed to set access mode on credential file '%s/%s': %m", dir, d->name);

        r = link_tmpfile_at(dfd, dfd, t, d->name, /* flags= */ 0);
        if (r < 0)
                return log_error_errno(r, "Failed to move credential file '%s/%s' into place: %m", dir, d->name);
...
static int import_credentials(const char *text) {
        ...
        r = sd_json_dispatch(j, top_table, SD_JSON_PERMISSIVE|SD_JSON_LOG, &creds);
        if (r < 0)
                return r;
        ...
        if (creds) {
                ...
                JSON_VARIANT_ARRAY_FOREACH(c, creds) {
                        static const sd_json_dispatch_field credential_table[] = {
                                ...
                                { "encrypted", SD_JSON_VARIANT_STRING, json_dispatch_unbase64_iovec,  offsetof(CredentialData, data), 0                 },
                                {},
                        };
                        ...
                        if ((!!d.text + !!iovec_is_set(&d.data) + !!iovec_is_set(&d.encrypted)) != 1) {
                                RET_GATHER(ret, log_warning_errno(SYNTHETIC_ERRNO(EBADMSG), "Exactly one of 'name', 'data', 'encrypted' must be set for credential '%s', refusing.", d.name));
                                continue;
                        }
...
static int get_imds_server_addresses(
        ...
        r = sd_varlink_call(
                        ...
                        &error_id);
        if (r < 0)
                return log_error_errno(r, "Failed to issue GetStatus(): %m");
        if (error_id)
                return log_error_errno(sd_varlink_error_to_errno(error_id, reply), "Failed to issue GetStatus(): %s", error_id);
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                { "addressIPv4", SD_JSON_VARIANT_ARRAY, json_dispatch_in_addr,  voffsetof(p, in4), 0 },
...
static int import_imds_hostnames(sd_varlink *link) {
        ...
        /* Creates local RRs (honoured by systemd-resolved) from the IMDS endpoint address, as well as our
         * public addresses.
         *
         * NB: unlike other IMDS clients we do not actually do sethostname() or an equivalent call here, we
         * expect this is done via DHCP. */
        ...
        r = write_string_file("/run/systemd/resolve/static.d/imds.rr", text, WRITE_STRING_FILE_CREATE|WRITE_STRING_FILE_ATOMIC|WRITE_STRING_FILE_MKDIR_0755);
        if (r < 0)
                return log_error_errno(r, "Failed to write IMDS rr data: %m");

        log_debug("IMDS server addresses written out.");
        return 1;
}
...
static int action_import(sd_varlink *link) {
        ...
        /* Keep a pristince copy of the userdata we actually applied. (Note that this data is typically also
...
static int run(int argc, char* argv[]) {
        ...
        if (r < 0) {
                ...
                _cleanup_close_ int pin_fd =
                        pin_callout_binary("/usr/lib/systemd/systemd-imdsd", &p);
                if (pin_fd < 0)
