...
static int add_dmi_id_dependency(const char *dest, const char *unit) {
        int r;
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to hook DMI id device before %s: %m", unit);
...
static int run(const char *dest, const char *dest_early, const char *dest_late) {
        ...
        if (arg_network_mode != IMDS_NETWORK_OFF) {
                ...
                r = add_dmi_id_dependency(dest_early, "systemd-imds-early-network.service");
                if (r < 0)
                        return r;
        }
        ...
        r = add_dmi_id_dependency(dest_early, "systemd-imdsd@.service");
        if (r < 0)
                return r;
