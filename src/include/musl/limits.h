 ...
 * This works only when GCC's limits.h is included first, but a user override breaks the cycle.
 * Hence, we need to manually achieve the cycle here. */

/* First, get the POSIX defines from glibc/musl's limits.h. When the two macros below are defined, GCC's limits.h
 * includes the next limits.h, that is, one from glibc/musl. */
