...
static int pseudo_tty_send_acquire_reply(PseudoTTY *pty) {
         ...
         * to be started), or delayed from the StartTransient() reply callback (if one does). */

        if (!pty->acquire_link)
                return 0;

        _cleanup_(sd_varlink_unrefp) sd_varlink *link = TAKE_PTR(pty->acquire_link);
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *reply = TAKE_PTR(pty->acquire_reply);

        if (pty->acquire_monitor)
                return sd_varlink_respond_and_upgrade(link, reply);
...
static int start_shell_reply_callback(
        ...
        if (r < 0)
                log_warning_errno(r, "Failed to send AcquirePty() reply to client, ignoring: %m");
