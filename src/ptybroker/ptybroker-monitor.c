...
static int pseudo_tty_monitor_push_ouput(PseudoTTYMonitor *monitor, const void *data, size_t n) {
        assert(monitor);
        assert(data || n == 0);
...
static int pseudo_tty_monitor_flush_subscribe(PseudoTTYMonitor *monitor) {
        ...
        r = osc_winsize_format(OSC_WINSIZE_SUBSCRIBE, monitor->width, monitor->height, &seq);
        if (r < 0)
                return r;

        r = pseudo_tty_monitor_push_ouput(monitor, seq, SIZE_MAX);
        if (r < 0)
                return r;
...
int pseudo_tty_monitor_enqueue_output(PseudoTTYMonitor *monitor, const void *data, size_t n) {
        ...
        if (!monitor->osc_winsize)
                return pseudo_tty_monitor_push_ouput(monitor, data, n);

        const char *q = data;
        FOREACH_ARRAY(c, q, n) {
                r = pseudo_tty_monitor_push_ouput(monitor, c, 1);
                if (r < 0)
                        return r;
                ...
                case ANSI_SEQ_EVENT_END: {
                        ...
                        if (seq && monitor->input_parser.introducer == ']' &&
                            ...
                            ws.type == OSC_WINSIZE_REPORT) { /* Ignore (i.e. pass through) reflected requests, as per spec */
                                ...
                                monitor->width = ws.width;
                                monitor->height = ws.height;
