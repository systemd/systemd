...
int pseudo_tty_monitor_push_pending(PseudoTTYMonitor *monitor) {
        ...
        /* If we we are setting up a new monitor, let's immediately enqueue any incomplete lines still queued in the pty */
        ...
        for (;;) {
                ...
                if (end_of_line_from_char(*(char*) v.iov_base) == 0)
                        break;
