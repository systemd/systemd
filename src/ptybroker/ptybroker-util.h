...
enum EndOfLine {
        ...
        /* The following four are four distinct line separators we recognizes, that sometimes appear in
         * combination and sometimes along */
        EOL_NEWLINE         = 1 << 0,
        EOL_CARRIAGE_RETURN = 1 << 1,
        EOL_FORM_FEED       = 1 << 2,
        EOL_NUL             = 1 << 3,

        /* The following are synthetic line separators, inserted by the tracking logic */
        EOL_TOO_LONG        = 1 << 4,  /* line became too long */
        EOL_FORCE_FLUSH     = 1 << 5,  /* connection ended, and a line flush was requested */

        _EOL_MASK_ALL       = EOL_NEWLINE|EOL_CARRIAGE_RETURN|EOL_FORM_FEED|EOL_NUL|
                              EOL_TOO_LONG|EOL_FORCE_FLUSH,
};
...
int terminal_settings_copy(const TerminalSettings *ts, TerminalSettings *ret);
