...
int pseudo_tty_sync_winsize(PseudoTTY *pty) {
        ...
        LIST_FOREACH(monitors, m, pty->monitors) {
                ...
                if (m->width > 0)
                        columns = columns > 0 ? MIN(columns, m->width) : m->width;
                if (m->height > 0)
                        lines = lines > 0 ? MIN(lines, m->height) : m->height;
        }
...
static int pseudo_tty_process_line(PseudoTTY *pty, bool force_flush) {
        ...
        for (; l > 0;) {
                ...
                if (eol_found == 0) {
                        ...
                        bool line_too_long = e - begin >= (ptrdiff_t) LONG_LINE_MAX,
                                flush_last = force_flush && l <= 1;

                        if (!line_too_long && !flush_last) {
                ...
                if (eol_found == 0 || FLAGS_SET(eol_mask, eol_found)) {
                        ...
                        iovec_reduce(&pty->frontend_read_buffer, (e - (char*) pty->frontend_read_buffer.iov_base) + n_drop_extra);
                        ...
                        pty->eol_mask = eol_mask = eol_found == 0 ? _EOL_MASK_ALL : eol_found;
                        continue;
                }
...
int pseudo_tty_track_buffer_to_json(PseudoTTY *pty, size_t n_lines, sd_json_variant **ret) {
        ...
        for (size_t i = 0; i < n; i++)
                array[i] = strempty(pty->track_buffer[(pty->track_buffer_next_line + pty->track_buffer_allocated_lines - n + i) % pty->track_buffer_allocated_lines]);
        array[n] = NULL;
