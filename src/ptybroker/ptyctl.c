...
COMMAND(
        "ptyctl\0",
        "Interact with the pseudo TTY broker.",
        .man_pages = "ptyctl(1)\0",
);

VERB_COMMON_HELP_AUTO_HIDDEN();

typedef struct ListPtyEntry {
        const char *name;
...
static void list_pty_entry_done(ListPtyEntry *e) {
        assert(e);
...
static int verb_list(int argc, char *argv[], uintptr_t data, void *userdata) {
        ...
        _cleanup_(table_unrefp) Table *table = table_new("name", "description", "tag", "frontend", "backend", "backend path", "uid", "pid");
        if (!table)
                return log_oom();
        ...
        JSON_VARIANT_ARRAY_FOREACH(i, reply) {
                static const sd_json_dispatch_field dispatch_table[] = {
                        { "name",         SD_JSON_VARIANT_STRING,        sd_json_dispatch_const_string, offsetof(ListPtyEntry, name),          0 },
                        { "description",  SD_JSON_VARIANT_STRING,        sd_json_dispatch_const_string, offsetof(ListPtyEntry, description),   0 },
                        { "tag",          SD_JSON_VARIANT_STRING,        sd_json_dispatch_const_string, offsetof(ListPtyEntry, tag),           0 },
                        { "frontendType", SD_JSON_VARIANT_STRING,        sd_json_dispatch_const_string, offsetof(ListPtyEntry, frontend_type), 0 },
                        { "backendType",  SD_JSON_VARIANT_STRING,        sd_json_dispatch_const_string, offsetof(ListPtyEntry, backend_type),  0 },
                        { "backendPath",  SD_JSON_VARIANT_STRING,        sd_json_dispatch_const_string, offsetof(ListPtyEntry, backend_path),  0 },
                        { "owner",        SD_JSON_VARIANT_OBJECT,        json_dispatch_pidref,          offsetof(ListPtyEntry, owner),         0 },
                        { "ownerUID",     _SD_JSON_VARIANT_TYPE_INVALID, sd_json_dispatch_uid_gid,      offsetof(ListPtyEntry, owner_uid),     0 },
                        {}
                };

                _cleanup_(list_pty_entry_done) ListPtyEntry e = {
                        .owner = PIDREF_NULL,
                ...
                r = table_add_many(
                                ...
                                TABLE_UID, e.owner_uid,
                                TABLE_PID, e.owner.pid);
                if (r < 0)
...
static int monitor_broker_pty(const char *name, int *ret_monitor_fd, char ***ret_track_buffer) {
        int r;

        assert(name);
        assert(ret_monitor_fd);
        assert(ret_track_buffer);
        ...
        r = sd_varlink_call_and_upgradebo(
                        ...
                        SD_JSON_BUILD_PAIR_UNSIGNED("trackBufferLines", MAX(5U, lines() / 2U)),
        ...
        _cleanup_(strv_freep) char **tb = NULL;
        static const sd_json_dispatch_field dispatch_table[] = {
                { "trackBuffer", SD_JSON_VARIANT_ARRAY, sd_json_dispatch_strv, 0, 0 },
                {}
        };

        r = sd_json_dispatch(reply, dispatch_table, SD_JSON_ALLOW_EXTENSIONS, &tb);
        if (r < 0)
                return log_error_errno(r, "Failed to parse MonitorPty() reply: %m");
...
static int pty_forward_handler(PTYForward *f, int rcode, void *userdata) {
        ...
        /* rcode == 0: the monitored PTY has been hung up or released. Unlike systemd-pty-forward we manage no
         * child process whose exit would terminate the event loop, hence exit on hangup here. */
        log_debug("Monitored PTY was released, exiting.");
        return sd_event_exit(e, EXIT_SUCCESS);
}

VERB(verb_monitor, "monitor", "NAME\0", 2, 2, 0, "Monitor the specified PTY");
static int verb_monitor(int argc, char *argv[], uintptr_t data, void *userdata) {
        ...
        _cleanup_(strv_freep) char **track_buffer = NULL;
        r = monitor_broker_pty(argv[1], &monitor_fd, &track_buffer);
        if (r < 0)
                return r;
        ...
        STRV_FOREACH(i, track_buffer)
                printf("%s%s%s\n", ansi_grey(), *i, ansi_normal());
        fflush(stdout);
...
static int on_hangup_reply(
        ...
        if (set_isempty(context->connections))
                (void) sd_event_exit(context->event, 0); /* Errors are propagated via context->error instead */

        return 0;
}

VERB(verb_hangup, "hangup", "NAME…\0", 2, VERB_ANY, 0, "Hang up the specified PTY(s)");
