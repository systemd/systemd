...
static void ovs_check_bridge_delete(sd_id128_t uuid, sd_json_variant *row, void *userdata) {
        ...
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *where = NULL, *delete_op = NULL,
                *ovs_where = NULL, *mutations = NULL, *mutate_op = NULL;
...
static void ovs_check_port_delete(sd_id128_t uuid, sd_json_variant *row, void *userdata) {
        struct ovs_delete_ctx *ctx = ASSERT_PTR(userdata);
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *where = NULL, *delete_op = NULL,
                *ovs_where = NULL, *port_uuid_ref = NULL, *mutations = NULL, *mutate_op = NULL;
        sd_json_variant *name_v;
        const char *managed, *name;
        int r;
...
static void ovs_check_name_cb(sd_id128_t uuid, sd_json_variant *row, void *userdata) {
        struct ovs_name_check *check = ASSERT_PTR(userdata);
...
static void ovs_find_port_bridge_cb(sd_id128_t uuid, sd_json_variant *row, void *userdata) {
        ...
        /* Bridge.ports is the string-based uuid-set encoding ["set", [["uuid", "<str>"], ...]];
         * format our real-uuid search key to a string to compare against those atoms. */
        const char *port_uuid_str = SD_ID128_TO_UUID_STRING(ctx->port_uuid);
        ...
        if (streq(sd_json_variant_string(tag), "uuid") && sd_json_variant_is_string(val)) {
                if (streq(sd_json_variant_string(val), port_uuid_str)) {
        ...
        for (size_t i = 0; i < sd_json_variant_elements(val); i++) {
                ...
                if (streq(sd_json_variant_string(puuid), port_uuid_str)) {
                        name = sd_json_variant_by_key(row, "name");
                        if (name && sd_json_variant_is_string(name))
                                ctx->found_bridge_name = sd_json_variant_string(name);
                        return;
...
static int ovs_emit_bridge_port_mutate(
        ...
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *where = NULL, *mutations = NULL,
                *mutate_op = NULL;
        int r;
                                ...
                                SD_JSON_BUILD_ARRAY(
                                        ...
                                        SD_JSON_BUILD_ARRAY(
                                                ...
                                                SD_JSON_BUILD_ARRAY(
                                                        SD_JSON_BUILD_ARRAY(
                                                                JSON_BUILD_CONST_STRING("uuid"),
                                                                SD_JSON_BUILD_STRING(SD_ID128_TO_UUID_STRING(port_uuid))))))));
        if (r < 0)
                return r;
...
int ovs_monitor_get_bridge_ports(Manager *m, const char *bridge_name, char ***ret_ports) {
        ...
        *ret_ports = NULL;
        ...
        if (streq(sd_json_variant_string(tag), "uuid") && sd_json_variant_is_string(val)) {
                sd_id128_t port_uuid;
                if (sd_id128_from_string(sd_json_variant_string(val), &port_uuid) >= 0) {
                        const char *name = ovs_monitor_port_name_by_uuid(mon, port_uuid);
                        if (name) {
        ...
        for (size_t i = 0; i < sd_json_variant_elements(val); i++) {
                ...
                sd_id128_t port_uuid;
                if (sd_id128_from_string(sd_json_variant_string(puuid), &port_uuid) < 0)
                        continue;
                        ...
                        STRV_FOREACH(member, members) {
                                ...
                                if (ifcheck.found) {
                                        ...
                                        r = sd_json_build(&iface_ref,
                                                        SD_JSON_BUILD_ARRAY(
                                                                JSON_BUILD_CONST_STRING("uuid"),
                                                                SD_JSON_BUILD_STRING(SD_ID128_TO_UUID_STRING(ifcheck.uuid))));
                                        if (r < 0)
                                                return r;
                                } else {
 ...
 * if the client was unrefed (caller may want to skip further work on m->ovsdb). Safe to
...
static bool ovs_drain_pending_teardown(Manager *m, const char *log_msg) {
        ...
        /* Null out m->ovsdb BEFORE unref so cancel_all callbacks see NULL. */
...
static int ovs_clear_done(
        ...
        if (!result) {
                if (!ovs_drain_pending_teardown(m, "OVS clear transact aborted; draining pending teardown"))
...
int manager_ovs_maybe_start(Manager *m) {
        ...
        /* We reach here only when no client exists yet (first start, or after a prior
         * FAILED transition cleared m->ovsdb). A reconnect timer may still be armed from
         * that failure; disable it so it cannot later fire and re-enter maybe_start() on
         * the client we are about to create. */
        m->ovs_reconnect_timer = sd_event_source_disable_unref(m->ovs_reconnect_timer);
        ...
        if (r < 0) {
                 ...
                 * whose state callback ran manager_ovs_schedule_reconnect() synchronously
                 * and cleared m->ovsdb. Only the earlier connect()/attach_event() failures
                 * return here with the client still live and no reconnect scheduled yet. */
                if (m->ovsdb)
                        manager_ovs_schedule_reconnect(m);
