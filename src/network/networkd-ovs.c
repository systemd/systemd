...
/* Build a where clause matching rows by name: [["name", "==", <name>]]. */
static int ovs_where_name(const char *name, sd_json_variant **ret) {
        assert(name);
        assert(ret);
...
/* Wrap a prebuilt array of atoms in the OVSDB set encoding: ["set", inner]. */
static int ovs_build_set(sd_json_variant *inner, sd_json_variant **ret) {
        assert(ret);
...
/* Append an UPDATE op targeting the row with the given (real, already-committed) UUID. */
static int ovs_emit_update(const char *table, sd_id128_t uuid, sd_json_variant *row, sd_json_variant **ops) {
        ...
        r = ovsdb_op_update(table, where, row, &op);
        if (r < 0)
                return r;

        return sd_json_variant_append_array(ops, op);
}

/* Append an INSERT op for a fresh row, registered under uuid_name for same-transact references. */
static int ovs_emit_insert(const char *table, const char *uuid_name, sd_json_variant *row, sd_json_variant **ops) {
        ...
        r = ovsdb_op_insert(table, uuid_name, row, &op);
        if (r < 0)
                return r;
...
/* Append an Open_vSwitch.bridges mutate op that inserts/deletes bridge_ref (a prebuilt uuid or
 * named-uuid ref). Open_vSwitch is a single-row table, so the where clause matches all rows. */
static int ovs_emit_ovs_bridges_mutate_ref(
                sd_json_variant *bridge_ref,
                const char *op_verb,      /* "insert" or "delete" */
                sd_json_variant **ops) {

        _cleanup_(sd_json_variant_unrefp) sd_json_variant *where = NULL, *mutations = NULL, *mutate_op = NULL;
        int r;
        ...
        /* mutations: [["bridges", <verb>, ["set", [ref]]]] */
...
typedef struct OVSDeleteContext {
        Manager *m;
        sd_json_variant **ops;
        int error;
} OVSDeleteContext;
...
static void ovs_check_bridge_delete(sd_id128_t uuid, sd_json_variant *row, void *userdata) {
        OVSDeleteContext *ctx = ASSERT_PTR(userdata);
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *where = NULL, *delete_op = NULL, *bridge_ref = NULL;
        ...
        if (!streq_ptr(ovs_get_external_id(row, "networkd-managed"), "true"))
                return;
        ...
        r = ovs_emit_ovs_bridges_mutate_ref(bridge_ref, "delete", ctx->ops);
        if (r < 0)
                goto fail;
...
typedef struct OVSDoomedPortsContext {
        Manager *m;
        Set *names;
        int error;
} OVSDoomedPortsContext;

static void ovs_collect_doomed_ports_cb(sd_id128_t uuid, sd_json_variant *row, void *userdata) {
        OVSDoomedPortsContext *ctx = ASSERT_PTR(userdata);
        const char *name;
        int r;
...
/* Append a Bridge.ports mutate op that inserts/deletes port_ref (a prebuilt uuid or named-uuid
 * ref). Shared by the real-uuid and named-uuid wrappers.
 *
...
static int ovs_emit_bridge_ports_mutate_ref(
                const char *bridge_name,  /* NULL = all bridges */
                sd_json_variant *port_ref,
                const char *op_verb,      /* "insert" or "delete" */
                sd_json_variant **ops) {
        ...
        if (bridge_name)
                r = ovs_where_name(bridge_name, &where);
        else
                r = ovsdb_where_all(&where);
        if (r < 0)
                return r;
...
/* Real-uuid variant: relink/unlink an already-inserted Port row by its live UUID. */
static int ovs_emit_bridge_ports_mutate(
        ...
        r = ovs_build_uuid_ref(port_uuid, &port_ref);
        if (r < 0)
                return r;

        return ovs_emit_bridge_ports_mutate_ref(bridge_name, port_ref, op_verb, ops);
}

/* Unlink a Port from whatever bridge currently owns it (strong ref; the all-bridges mutate finds
 * it even if Bridge= changed in the same edit) and delete the Port row — OVSDB cascades the
 * deletion of the referenced Interface row(s). */
static int ovs_emit_port_delete(sd_id128_t uuid, sd_json_variant **ops) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *where = NULL, *delete_op = NULL;
        int r;

        assert(ops);

        r = ovs_emit_bridge_ports_mutate(/* bridge_name= */ NULL, uuid, "delete", ops);
        if (r < 0)
                return r;
        ...
        return sd_json_variant_append_array(ops, delete_op);
}
...
static void ovs_check_port_delete(sd_id128_t uuid, sd_json_variant *row, void *userdata) {
        ...
        r = ovs_emit_port_delete(uuid, ctx->ops);
        if (r < 0)
                ctx->error = r;
}
...
/* Look up a row by name in the monitor cache via the O(1) by-name index. Returns true and
 * stores the row's UUID in *ret_uuid (if non-NULL) when found, false otherwise. */
static bool ovs_lookup_by_name(OVSDBMonitor *mon, const char *table, const char *name, sd_id128_t *ret_uuid) {
        sd_id128_t uuid;
        ...
        if (ovsdb_monitor_get_by_name(mon, table, name, &uuid, NULL) <= 0)
                return false;
...
typedef struct OVSFindPortBridgeContext {
...
static void ovs_find_port_bridge_cb(sd_id128_t uuid, sd_json_variant *row, void *userdata) {
        OVSFindPortBridgeContext *ctx = ASSERT_PTR(userdata);
...
const char* manager_ovs_get_bridge_fail_mode(Manager *m, const char *bridge_name) {
        ...
        if (ovsdb_monitor_get_by_name(mon, "Bridge", bridge_name, /* ret_uuid= */ NULL, &row) <= 0)
                return NULL;
...
typedef struct OVSCollectPortNamesContext {
        OVSDBMonitor *mon;
        char ***ports;
        int error;
} OVSCollectPortNamesContext;

static int ovs_collect_port_name_cb(const char *uuid_str, void *userdata) {
        OVSCollectPortNamesContext *c = ASSERT_PTR(userdata);
        sd_id128_t uuid;
        const char *name;
        int r;
...
int manager_ovs_get_bridge_ports(Manager *m, const char *bridge_name, char ***ret_ports) {
        ...
        *ret_ports = NULL;
        ...
        OVSCollectPortNamesContext cctx = { .mon = mon, .ports = &ports };
...
/* Build a Bridge.protocols value ["set", [<proto>, ...]] from a non-NULL, _OVS_PROTOCOL_INVALID-
 * terminated list. Callers decide the empty case: INSERT omits the column, UPDATE resets it with
 * ovs_build_empty_set(). */
static int ovs_build_protocols_set(const OVSProtocol *protocols, sd_json_variant **ret) {
        ...
        for (const OVSProtocol *p = protocols; *p != _OVS_PROTOCOL_INVALID; p++) {
                r = sd_json_variant_append_arrayb(&inner, SD_JSON_BUILD_STRING(ovs_protocol_to_string(*p)));
                if (r < 0)
                        return r;
        }

        return ovs_build_set(inner, ret);
}
...
/* Named-uuid variant of ovs_emit_bridge_ports_mutate(): link a Port being inserted in the same
 * transaction (referenced by its named-uuid) to its parent bridge, matched by name. */
static int ovs_emit_bridge_port_insert_named(
                const char *bridge_name,
                const char *port_uuid_name,
                sd_json_variant **ops) {
        ...
        return ovs_emit_bridge_ports_mutate_ref(bridge_name, port_ref, "insert", ops);
}

/* Set (or clear) other_config:datapath-id on a Bridge via a map mutate. The Bridge.datapath_id
 * column itself is a read-only status column that ovs-vswitchd overwrites; the configuration
 * knob is the other_config:datapath-id map key. Mutating just that key (instead of updating the
 * whole other_config column) preserves out-of-band keys set by ovs-vsctl, OVN or OpenStack —
 * same policy as the tristate columns in ovs_reconcile_bridge(). A NULL datapath_id emits only
 * the "delete" mutation, resetting the key when the operator removed DatapathID=. */
static int ovs_emit_bridge_datapath_id(const char *bridge_name, const char *datapath_id, sd_json_variant **ops) {
        ...
        /* Delete any existing key first (harmless no-op when absent) — OVSDB map "insert"
         * does not overwrite existing keys, so the delete must come before the insert. */
        r = sd_json_variant_append_arrayb(&mutations,
                        SD_JSON_BUILD_ARRAY(
                                JSON_BUILD_CONST_STRING("other_config"),
                                JSON_BUILD_CONST_STRING("delete"),
                                SD_JSON_BUILD_ARRAY(
                                        JSON_BUILD_CONST_STRING("set"),
                                        SD_JSON_BUILD_ARRAY(
                                                JSON_BUILD_CONST_STRING("datapath-id")))));
        if (r < 0)
                return r;
                                ...
                                SD_JSON_BUILD_ARRAY(
                                        ...
                                        SD_JSON_BUILD_ARRAY(
                                                ...
                                                SD_JSON_BUILD_ARRAY(
                                                        SD_JSON_BUILD_ARRAY(
                                                                JSON_BUILD_CONST_STRING("datapath-id"),
                                                                SD_JSON_BUILD_STRING(datapath_id))))));
                if (r < 0)
                        return r;
        }

        r = ovsdb_op_mutate("Bridge", where, mutations, &op);
        if (r < 0)
                return r;
...
static int ovs_update_bridge(NetDev *netdev, sd_id128_t uuid, sd_json_variant **ops) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *update_row = NULL, *update_ext = NULL,
        ...
        if (b->protocols)
                r = ovs_build_protocols_set(b->protocols, &update_protocols);
        else
        ...
        r = ovs_emit_update("Bridge", uuid, update_row, ops);
        if (r < 0)
                return r;

        /* Unconditional: a NULL datapath_id resets other_config:datapath-id,
         * matching the reset policy the optional columns above use. */
        r = ovs_emit_bridge_datapath_id(netdev->ifname, b->datapath_id, ops);
        if (r < 0)
                return r;

        /* READY transition deferred: bridges are kernel-backed and go READY
         * via set_ifindex on RTM_NEWLINK, or via ovs_reconcile_done if the
         * ifindex is already bound (daemon-reload with existing interface). */
        return 0;
}
...
static int ovs_reconcile_bridge(Manager *m, NetDev *netdev, sd_json_variant **ops) {
        ...
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *external_ids = NULL, *iface_row = NULL,
                *iface_ref = NULL, *port_row = NULL, *port_ref = NULL,
                *bridge_row = NULL, *bridge_ref = NULL;
        ...
        if (mon) {
                ...
                if (ovs_lookup_by_name(mon, "Bridge", netdev->ifname, &uuid))
                        return ovs_update_bridge(netdev, uuid, ops);
        }
        ...
        r = ovs_emit_insert("Interface", iface_uuid_name, iface_row, ops);
        if (r < 0)
                return r;
        ...
        r = ovs_emit_insert("Port", port_uuid_name, port_row, ops);
        if (r < 0)
                return r;
        ...
        /* Build protocols set if present (omitted on INSERT when unset). */
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *protocols_set = NULL;
        if (b->protocols) {
                r = ovs_build_protocols_set(b->protocols, &protocols_set);
                if (r < 0)
                        return r;
        }
        ...
        r = ovs_emit_insert("Bridge", bridge_uuid_name, bridge_row, ops);
        if (r < 0)
                return r;

        /* other_config:datapath-id is a map key, not the (read-only) datapath_id column;
         * mutate the freshly inserted row by name within the same transaction. */
        if (b->datapath_id) {
                r = ovs_emit_bridge_datapath_id(netdev->ifname, b->datapath_id, ops);
                if (r < 0)
                        return r;
        ...
        r = ovs_emit_ovs_bridges_mutate_ref(bridge_ref, "insert", ops);
        if (r < 0)
                return r;
...
static int ovs_build_port_row(const OVSPortColumns *c, sd_json_variant **ret) {
...
/* Emit an Interface INSERT for a bond member (type="" = system) and return the named-uuid ref
 * for linking it into the bond Port's interfaces set. Shared by the bond UPDATE and INSERT paths. */
static int ovs_emit_member_iface_insert(
                const char *member,
                sd_json_variant *external_ids,
                sd_json_variant **ops,
                sd_json_variant **ret_ref) {
        ...
        return ovs_build_named_uuid_ref(iface_uuid_name, ret_ref);
}
...
static int ovs_reconcile_bond_port(
        ...
        if (mon) {
                ...
                if (ovs_lookup_by_name(mon, "Port", netdev->ifname, &uuid)) {
                        ...
                        STRV_FOREACH(member, members) {
                                ...
                                sd_id128_t iface_uuid;
                                bool iface_found = ovs_lookup_by_name(mon, "Interface", *member, &iface_uuid);
                                ...
                                if (iface_found && !set_contains(doomed_ports, *member)) {
                                        /* Reuse existing Interface by UUID */
                                        r = ovs_build_uuid_ref(iface_uuid, &iface_ref);
                                        if (r < 0)
                                                return r;
                                } else {
                                        r = ovs_emit_member_iface_insert(*member, update_ext, ops, &iface_ref);
                                        if (r < 0)
                                                return r;
                                }
                        ...
                        r = ovs_build_set(update_iface_refs, &update_iface_set);
                        if (r < 0)
                                return r;

                        r = ovs_build_port_row(
                                        &(const OVSPortColumns) {
                        ...
                        r = ovs_emit_update("Port", uuid, update_row, ops);
                        if (r < 0)
                                return r;

                        /* If the parent bridge changed, move the Port between bridges */
                        r = ovs_ensure_port_bridge_membership(m, uuid, p->bridge, ops);
                        if (r < 0)
                                return r;

                        /* No READY handling here: bond ports were marked READY already in
                         * ovs_port_create() — no kernel netdev to wait for, and READY does
                         * not wait on the transact ACK. */
                        return 0;
                }
        }
        ...
        STRV_FOREACH(member, members) {
                ...
                r = ovs_emit_member_iface_insert(*member, external_ids, ops, &iface_ref);
                if (r < 0)
                        return r;
        ...
        r = ovs_build_set(iface_refs_inner, &iface_set);
        if (r < 0)
                return r;
        ...
        /* 3. Link the new port to its parent bridge (ports += named-uuid). */
        r = ovs_emit_bridge_port_insert_named(p->bridge, port_uuid_name, ops);
        if (r < 0)
                return r;
...
/* ovs_uuid_set_foreach callback for ovs_port_needs_recreate(): count the Port.interfaces atoms
 * and remember the name of the first Interface that resolves in the monitor cache. */
typedef struct OVSPortIfaceShapeContext {
        OVSDBMonitor *mon;
        size_t n;
        const char *first_name;  /* aliases the monitor cache */
} OVSPortIfaceShapeContext;

static int ovs_port_iface_shape_cb(const char *uuid_str, void *userdata) {
        OVSPortIfaceShapeContext *c = ASSERT_PTR(userdata);
        sd_json_variant *row, *name;
        sd_id128_t uuid;
        ...
        if (c->first_name)
                return 0;  /* only the first name matters, but keep counting */

        if (sd_id128_from_string(uuid_str, &uuid) < 0)
                return 0;

        row = ovsdb_monitor_get(c->mon, "Interface", uuid);
        if (!row)
                return 0;

        name = sd_json_variant_by_key(row, "name");
        if (name && sd_json_variant_is_string(name))
                c->first_name = sd_json_variant_string(name);

        return 0;
}

/* An internal/patch Port created by ovs_reconcile_port() references exactly one Interface whose
 * name equals the Port name. A Port previously created with Type=bond instead references member
 * Interfaces named after the members, and the UPDATE path can never converge it: it leaves
 * Port.interfaces untouched, and the internal<->patch retype block is a no-op because no
 * same-named Interface row exists — so the stale bond members would linger forever while every
 * reconcile reports success. Detect that shape from the cached row so the caller can delete and
 * reinsert. Deliberately conservative: fire only on positive evidence (two or more interfaces,
 * or a single interface whose cached name differs from the Port name). Unresolvable UUIDs or
 * malformed cache shapes never fire, so a normal port is never needlessly recreated. */
static bool ovs_port_needs_recreate(OVSDBMonitor *mon, sd_id128_t port_uuid, const char *ifname) {
        OVSPortIfaceShapeContext ctx;
        sd_json_variant *row;

        assert(mon);
        assert(ifname);

        row = ovsdb_monitor_get(mon, "Port", port_uuid);
        if (!row)
                return false;

        ctx = (OVSPortIfaceShapeContext) { .mon = mon };
        (void) ovs_uuid_set_foreach(sd_json_variant_by_key(row, "interfaces"),
                                    ovs_port_iface_shape_cb, &ctx);

        if (ctx.n >= 2)
                return true;  /* multiple member Interfaces → bond shape */

        return ctx.n == 1 && ctx.first_name && !streq(ctx.first_name, ifname);
}

/* Select the Interface.type and Interface.options values for an internal/patch OVS port. Patch
 * ports require options: {"peer": "<name>"}; internal ports (or patch with missing peer) get an
 * explicit empty options map, so an UPDATE after a Type= change clears a stale patch peer.
 * Shared by the UPDATE and INSERT paths of ovs_reconcile_port(). */
static int ovs_port_build_iface_type_and_options(
                const OVSPort *p,
                const char **ret_type,
                sd_json_variant **ret_options) {
        ...
        case OVS_PORT_TYPE_PATCH:
                /* PeerPort= is mandatory for patch ports, enforced by ovs_port_verify(). */
                assert(p->peer_port);
        ...
        default:
                /* Bond ports are handled by ovs_reconcile_bond_port() and never get here. */
                assert_not_reached();
        }
        if (r < 0)
                return r;
...
static int ovs_update_port(Manager *m, NetDev *netdev, OVSDBMonitor *mon, sd_id128_t uuid, sd_json_variant **ops) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *update_row = NULL, *update_ext = NULL;
        OVSPort *p;
        int r;
        ...
        sd_id128_t iface_uuid;
        if (ovs_lookup_by_name(mon, "Interface", netdev->ifname, &iface_uuid)) {
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *iface_options = NULL, *iface_row_update = NULL;
                const char *iface_type_now;

                r = ovs_port_build_iface_type_and_options(p, &iface_type_now, &iface_options);
                if (r < 0)
                        return r;
                ...
                r = ovs_emit_update("Interface", iface_uuid, iface_row_update, ops);
                if (r < 0)
                        return r;
        }
        ...
        /* READY transition: internal ports go READY via set_ifindex on
         * RTM_NEWLINK (or via ovs_reconcile_done when the ifindex is already
         * bound after a daemon-reload); patch ports were marked READY already
         * in ovs_port_create() and do not wait on the transact ACK. */
        return 0;
}
...
static int ovs_reconcile_port(Manager *m, NetDev *netdev, Set *doomed_ports, sd_json_variant **ops) {
                ...
                if (strv_isempty(bond_members)) {
                        ...
                        if (mon) {
                                ...
                                if (ovs_lookup_by_name(mon, "Port", netdev->ifname, &uuid)) {
                                        ...
                                        /* The all-bridges unlink in ovs_emit_port_delete() matters here:
                                         * if Bridge= changed in the same edit that emptied the bond, the
                                         * port still lives in the old bridge, and a name-matched mutate
                                         * would miss it and leave a dangling strong reference that rolls
                                         * back the whole transact. */
                                        r = ovs_emit_port_delete(uuid, ops);
                                        if (r < 0)
                                                return r;
                                } else
        ...
        if (mon) {
                sd_id128_t uuid;
                bool found = ovs_lookup_by_name(mon, "Port", netdev->ifname, &uuid);

                if (found && ovs_port_needs_recreate(mon, uuid, netdev->ifname)) {
                        /* The cached Port still has the bond shape (member Interfaces instead of a
                         * single same-named one), i.e. Type= changed from bond to internal/patch.
                         * UPDATE cannot converge it, so delete the row (unlink + DELETE, cascading
                         * the member Interface deletes) and fall through to the INSERT path below —
                         * the deletes are ordered before the inserts in this same transact, so
                         * there is no name collision. */
                        log_netdev_debug(netdev,
                                         "OVS port '%s' exists in OVSDB with bond-shaped interfaces (Type= changed from bond), recreating",
                                         strna(netdev->ifname));
                        ...
                        return ovs_update_port(m, netdev, mon, uuid, ops);
        }
        ...
        /* 1. Insert Interface */
        {
                ...
                /* Determine interface type and options from port type */
                r = ovs_port_build_iface_type_and_options(p, &iface_type, &iface_options);
                if (r < 0)
...
/* Append one ["key", "value"] pair to the inner array of an OVSDB map being built. */
static int ovs_map_append(sd_json_variant **inner, const char *key, const char *value) {
        assert(inner);
        assert(key);
        assert(value);
        ...
        return sd_json_variant_append_arrayb(
                        ...
                        SD_JSON_BUILD_ARRAY(
                                ...
                                SD_JSON_BUILD_STRING(value)));
}

/* Like ovs_map_append(), but formats an unsigned integer value as its decimal string —
 * OVSDB Interface.options is a string→string map, so numbers go over the wire as strings. */
static int ovs_map_append_uint64(sd_json_variant **inner, const char *key, uint64_t u) {
        _cleanup_free_ char *str = NULL;

        if (asprintf(&str, "%" PRIu64, u) < 0)
                return log_oom();

        return ovs_map_append(inner, key, str);
}

/* Build the tunnel Interface.options map: ["map", [["remote_ip","x.x.x.x"], ...]]. */
static int ovs_tunnel_build_options(NetDev *netdev, sd_json_variant **ret) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *opts_inner = NULL;
        _cleanup_free_ char *remote_str = NULL, *local_str = NULL;
        OVSTunnel *t;
        int r;
        ...
        r = in_addr_to_string(t->remote.family, &t->remote.address, &remote_str);
        if (r < 0)
                return log_netdev_warning_errno(netdev, r, "Failed to format remote address: %m");

        /* Always have remote_ip */
        r = ovs_map_append(&opts_inner, "remote_ip", remote_str);
        if (r < 0)
                return r;

        if (t->local.family != AF_UNSPEC) {
                r = in_addr_to_string(t->local.family, &t->local.address, &local_str);
                if (r < 0)
                        return log_netdev_warning_errno(netdev, r, "Failed to format local address: %m");

                r = ovs_map_append(&opts_inner, "local_ip", local_str);
                if (r < 0)
                        return r;
        }

        if (t->key_set) {
                r = ovs_map_append_uint64(&opts_inner, "key", t->key);
                if (r < 0)
                        return r;
        }

        if (t->destination_port > 0) {
                r = ovs_map_append_uint64(&opts_inner, "dst_port", t->destination_port);
                if (r < 0)
                        return r;
        }
        ...
        if (t->tos > 0) {
                r = ovs_map_append_uint64(&opts_inner, "tos", t->tos);
                if (r < 0)
                        return r;
        }

        if (t->ttl > 0) {
                r = ovs_map_append_uint64(&opts_inner, "ttl", t->ttl);
                if (r < 0)
                        return r;
        }

        if (t->dont_fragment >= 0) {
                r = ovs_map_append(&opts_inner, "df_default", t->dont_fragment > 0 ? "true" : "false");
                if (r < 0)
                        return r;
        }
...
static int ovs_update_tunnel(
                Manager *m,
                NetDev *netdev,
                OVSDBMonitor *mon,
                sd_id128_t port_uuid,
                sd_json_variant *options,
                sd_json_variant *external_ids,
                sd_json_variant **ops) {
        ...
        if (ovs_lookup_by_name(mon, "Interface", netdev->ifname, &iface_uuid)) {
                ...
                r = ovs_emit_update("Interface", iface_uuid, update_iface, ops);
                if (r < 0)
                        return r;
        } else
        ...
        r = ovs_emit_update("Port", port_uuid, port_update_row, ops);
        if (r < 0)
                return r;

        /* If the parent bridge changed, move the tunnel Port between bridges */
        r = ovs_ensure_port_bridge_membership(m, port_uuid, t->bridge, ops);
        if (r < 0)
                return r;

        /* No READY handling here: tunnels were marked READY already in
         * ovs_tunnel_create() — no same-name kernel netdev exists, and READY
         * does not wait on the transact ACK. */
        return 0;
}
...
static int ovs_reconcile_tunnel(Manager *m, NetDev *netdev, sd_json_variant **ops) {
        ...
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *external_ids = NULL, *options = NULL,
                *iface_row = NULL, *iface_ref = NULL, *port_row = NULL;
        _cleanup_free_ char *iface_uuid_name = NULL, *port_uuid_name = NULL;
        int r;
        ...
        bool tunnel_exists = ovs_lookup_by_name(mon, "Port", netdev->ifname, &tunnel_port_uuid);
        ...
        r = ovs_tunnel_build_options(netdev, &options);
        if (r < 0)
                return r;

        if (tunnel_exists)
                return ovs_update_tunnel(m, netdev, mon, tunnel_port_uuid, options, external_ids, ops);
        ...
        /* 3. Link the new port to its parent bridge (ports += named-uuid). */
        r = ovs_emit_bridge_port_insert_named(t->bridge, port_uuid_name, ops);
        if (r < 0)
                return r;
...
static int ovs_reconcile_network_port_one(Manager *m, Network *network, const char *ifname, sd_json_variant **ops) {
        ...
        /* If a .netdev of kind OVS_BRIDGE, OVS_PORT or OVS_TUNNEL already manages this
         * interface, skip .network attachment to avoid named-uuid collisions in the transact
         * (the bridge/port/tunnel reconcilers emit iface_/port_<ifname> named-uuids that would
         * otherwise clash with the ones Phase 4 builds here, aborting the whole transact). */
        {
                NetDev *existing;
                if (netdev_get(m, ifname, &existing) >= 0 &&
                    IN_SET(existing->kind, NETDEV_KIND_OVS_BRIDGE, NETDEV_KIND_OVS_PORT, NETDEV_KIND_OVS_TUNNEL)) {
                        log_debug("Interface '%s' already managed as OVS bridge/port/tunnel netdev, skipping .network attachment",
                                  ifname);
                        return 0;
                }
        }
        ...
        OVSDBMonitor *mon = ovsdb_client_get_monitor(m->ovsdb);
        if (mon && !ovs_lookup_by_name(mon, "Bridge", network->ovs_bridge_name, /* ret_uuid= */ NULL)) {
        ...
        if (mon) {
                ...
                if (ovs_lookup_by_name(mon, "Port", ifname, &uuid)) {
                        _cleanup_(sd_json_variant_unrefp) sd_json_variant *update_row = NULL, *update_ext = NULL;
                        ...
                        r = ovs_ensure_port_bridge_membership(m, uuid, network->ovs_bridge_name, ops);
                        if (r < 0)
                                return r;
        ...
        /* 3. Link the new port to its parent bridge (ports += named-uuid). */
        r = ovs_emit_bridge_port_insert_named(network->ovs_bridge_name, port_uuid_name, ops);
        if (r < 0)
                return r;
...
static bool manager_ovs_drain_pending_teardown(Manager *m, const char *log_msg) {
        assert(m);
        assert(log_msg);
...
/* An aborted transact: the server never applied the ops. Two paths produce result==NULL:
 * ovsdb_client_unref cancel (NULL/NULL via cancel_all), and runtime FAILED-state cancel (NULL
 * plus synthetic_error, emitted from ovsdb_client_set_state after the state already switched
 * to FAILED). Both are treated identically by the callers — silent log_debug, no warnings, no
 * post-processing; the next reconcile after reconnect will rebuild whatever state was lost.
 *
 * A genuine JSON-RPC error reply also arrives as result==NULL + error!=NULL (the RPC layer
 * normalizes JSON null to C NULL), but only while the client is still live — the dispatch loop
 * never fires reply callbacks in FAILED state. Use the client state to tell it apart from the
 * synthetic cancel error, so real server errors reach the callers' warning branches instead of
 * being swallowed here. */
static bool ovs_transact_aborted(OVSDBClient *client, sd_json_variant *result, sd_json_variant *error) {
        return !result && (!error || ovsdb_client_get_state(client) == OVSDB_CLIENT_FAILED);
}

/* Scan a transact reply's per-op results for errors, logging each one prefixed with `what`.
 * Comment ops return empty objects; abort/malformed replies may be non-objects — skipped. */
static bool ovs_transact_result_has_errors(sd_json_variant *result, const char *what) {
        ...
        for (size_t i = 0; i < sd_json_variant_elements(result); i++) {
                ...
                if (err && !sd_json_variant_is_null(err)) {
                        ...
                        log_warning("%s: %s", what, strna(text));
                        had_errors = true;
                }
        }
...
static int ovs_reconcile_done(
        ...
        if (ovs_transact_aborted(client, result, error)) {
        ...
        if (sd_json_variant_is_array(result)) {
                log_debug("OVS reconciliation completed: %zu operations", sd_json_variant_elements(result));
                ...
                if (!ovs_transact_result_has_errors(result, "OVS transact op error")) {
                        NetDev *netdev;
                        HASHMAP_FOREACH(netdev, m->netdevs) {
                                ...
                                case NETDEV_KIND_OVS_PORT:
                                        /* Only internal ports ever enter CREATING — patch and
                                         * bond ports (and tunnels) are marked READY already in
                                         * their create() callbacks and never reach this loop. */
                                        if (netdev->ifindex <= 0)
                                                continue;  /* wait for RTM_NEWLINK */
                                        break;

                                default:
                                        /* Not an OVS kind that can be in CREATING here (OVS
                                         * tunnels go READY in ovs_tunnel_create()). */
                                        continue;
                                }
...
try_pending:
         ...
         * teardown / reconnect. manager_ovs_reconcile() would otherwise log a confusing
        ...
        (void) manager_ovs_drain_pending_teardown(m, "OVS pending teardown: drained, releasing client");
...
static int ovs_clear_done(
        ...
        /* On an aborted transact, skip the post-clear bookkeeping: the database state is
         * unknown, and the next reconnect will re-attempt the clear via the marker file. */
        ...
        if (ovs_transact_result_has_errors(result, "OVS clear op error, not setting marker")) {
...
finish:
        ...
        (void) manager_ovs_drain_pending_teardown(m, "OVS clear: draining pending teardown after transact completion");
        return 0;
}
...
int manager_ovs_clear_database(Manager *m) {
        ...
        r = ovs_build_empty_set(&empty_set);
        if (r < 0)
                return r;
...
int manager_ovs_reconcile(Manager *m) {
         ...
         * manager_ovs_reconcile() either runs at startup (link_reconfigure_full hook), at
        ...
        if (have_bond) {
                OVSDBMonitor *mon = ovsdb_client_get_monitor(m->ovsdb);
                if (mon) {
                        OVSDoomedPortsContext dctx = { .m = m };
...
/* Trigger a reconcile for a link-driven OVS config change, but only when the OVSDB client is
 * live and READY. Skip if the client is still HANDSHAKING / FAILED — manager_ovs_reconcile() would see
 * an empty monitor cache and emit ops as if nothing existed in OVSDB, causing
 * constraint-violation transacts. The post-handshake snapshot reconcile
 * (manager_ovs_on_monitor_initial) covers this link too. */
void link_ovs_maybe_reconcile(Link *link) {
        ...
        if (!link->manager->ovsdb ||
            ovsdb_client_get_state(link->manager->ovsdb) != OVSDB_CLIENT_READY)
                return;

        r = manager_ovs_reconcile(link->manager);
        if (r < 0)
                log_link_warning_errno(link, r, "OVS reconciliation triggered by link change failed: %m");
}

static void manager_ovs_count_usage(Manager *m) {
        NetDev *netdev;
        Network *network;
...
static int manager_ovs_on_monitor_initial(
        ...
        if (need_clear) {
                ...
                r = manager_ovs_clear_database(m);
                if (r < 0) {
...
static void manager_ovs_on_update(OVSDBClient *client, void *userdata) {
         ...
         * them). If we re-entered manager_ovs_reconcile() here we would re-emit the
        ...
        /* Drain any coalesced pending reconcile (cleared by us; manager_ovs_reconcile
...
static int manager_ovs_state_changed(
        ...
        if (new_state == OVSDB_CLIENT_READY) {
                ...
                if (r < 0) {
                         ...
                         * useless: manager_ovs_reconcile() returns 0 silently because the
...
int manager_ovs_maybe_start(Manager *m) {
        ...
        manager_ovs_count_usage(m);

        if (m->ovs_use_count == 0) {
                /* The last OVS config was removed. If we have a live READY client, run one final
                 * reconcile: with empty desired state manager_ovs_reconcile() emits only DELETE ops, which
                 ...
                 * manager_ovs_reconcile) on the next reconcile that runs with a live connection. We do not
