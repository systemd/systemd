 ...
 * ref). bridge_name NULL targets all bridges. Shared by the real-uuid and named-uuid wrappers. */
...
static int ovs_reconcile_bridge(Manager *m, NetDev *netdev, sd_json_variant **ops) {
        ...
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *external_ids = NULL, *iface_row = NULL,
                ...
                *bridge_row = NULL, *bridge_op = NULL, *bridge_ref = NULL;
...
struct ovs_port_iface_shape_ctx {
        OVSDBMonitor *mon;
        size_t n;
...
static int ovs_port_iface_shape_cb(const char *uuid_str, void *userdata) {
        struct ovs_port_iface_shape_ctx *c = ASSERT_PTR(userdata);
        sd_json_variant *row, *name;
        sd_id128_t uuid;
...
static bool ovs_port_needs_recreate(OVSDBMonitor *mon, sd_id128_t port_uuid, const char *ifname) {
        struct ovs_port_iface_shape_ctx ctx;
        sd_json_variant *row;
        ...
        ctx = (struct ovs_port_iface_shape_ctx) { .mon = mon };
...
static int ovs_reconcile_port(Manager *m, NetDev *netdev, Set *doomed_ports, sd_json_variant **ops) {
        ...
        if (mon) {
                struct ovs_name_check check = ovs_lookup_by_name(mon, "Port", netdev->ifname);
                if (check.found && ovs_port_needs_recreate(mon, check.uuid, netdev->ifname)) {
...
static int ovs_reconcile_done(
         ...
         * and runtime FAILED-state cancel (NULL plus synthetic_error, emitted
         * from ovsdb_client_set_state after the state already switched to
         * FAILED). Treat both identically — silent log_debug, no warnings, no
         * post-processing. The next reconcile after reconnect will rebuild
         * whatever state was lost.
         *
         * A genuine JSON-RPC error reply also arrives as result==NULL +
         * error!=NULL (the RPC layer normalizes JSON null to C NULL), but only
         * while the client is still live — the dispatch loop never fires reply
         * callbacks in FAILED state. Use the client state to tell it apart from
         * the synthetic cancel error, so real server errors reach the warning
         * branch below instead of being swallowed here. */
        if (!result && (!error || ovsdb_client_get_state(client) == OVSDB_CLIENT_FAILED)) {
...
static int ovs_clear_done(
         ...
         * The unref cancel delivers NULL/NULL; the FAILED-state cancel delivers
         * NULL plus a synthetic error, fired only after the client state already
         * switched to FAILED. A genuine JSON-RPC error reply also arrives as
         * result==NULL + error!=NULL (the RPC layer normalizes JSON null to C
         * NULL), but never in FAILED state — so use the client state to route
         * real server errors to the warning-plus-reconcile branch below.
         *
