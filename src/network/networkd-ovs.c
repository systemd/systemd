...
static bool ovs_port_still_configured(Manager *m, const char *name) {
        ...
        /* Check if this is a bridge's own local port (bridge creates a Port with its name) */
        if (netdev_get(m, name, &netdev) >= 0 && netdev->kind == NETDEV_KIND_OVS_BRIDGE)
                return true;

        /* Check if any OVS_PORT or OVS_TUNNEL netdev has this name */
        if (netdev_get(m, name, &netdev) >= 0 &&
            IN_SET(netdev->kind, NETDEV_KIND_OVS_PORT, NETDEV_KIND_OVS_TUNNEL))
                return true;
...
static void ovs_find_port_bridge_cb(const char *uuid, sd_json_variant *row, void *userdata) {
        ...
        if (!tag || !sd_json_variant_is_string(tag) ||
            !streq(sd_json_variant_string(tag), "set") ||
            !val || !sd_json_variant_is_array(val))
                return;
...
int ovs_monitor_get_bridge_ports(Manager *m, const char *bridge_name, char ***ret_ports) {
        ...
        *ret_ports = NULL;
        ...
        /* Bridge.ports has max=unlimited, so it's always encoded as ["set", [...]] */
        if (!streq(sd_json_variant_string(tag), "set") ||
            !val || !sd_json_variant_is_array(val))
                return 0;
...
static int ovs_reconcile_bridge(Manager *m, NetDev *netdev, sd_json_variant **ops) {
        ...
        if (mon) {
                ...
                if (check.found) {
                        ...
                        log_netdev_debug(netdev, "OVS bridge '%s' already exists, updating", netdev->ifname);
...
/* True if no bit is set in the BridgeVLAN-style bitmap. */
static bool ovs_vlan_bitmap_is_empty(const uint32_t *bitmap) {
        assert(bitmap);
        for (size_t i = 0; i < BRIDGE_VLAN_BITMAP_LEN; i++)
                if (bitmap[i] != 0)
                        return false;
        return true;
}
...
static int ovs_reconcile_bond_port(
        ...
        if (mon) {
                ...
                if (check.found) {
                        ...
                        log_netdev_debug(netdev, "OVS bond '%s' already exists, updating", netdev->ifname);

                        r = ovs_build_external_ids(netdev->filename, &update_ext);
                        if (r < 0)
                                return r;

                        if (!ovs_vlan_bitmap_is_empty(p->vlan_bitmap)) {
        ...
        log_netdev_debug(netdev, "OVS bond port '%s' queued for reconciliation (%zu members)",
                         netdev->ifname, strv_length(members));
        return 0;
}
...
static int ovs_reconcile_port(Manager *m, NetDev *netdev, sd_json_variant **ops) {
        ...
        if (mon) {
                ...
                if (check.found) {
                        ...
                        log_netdev_debug(netdev, "OVS port '%s' already exists, updating", netdev->ifname);
...
static int ovs_reconcile_tunnel(Manager *m, NetDev *netdev, sd_json_variant **ops) {
        ...
        if (tunnel_exists) {
                ...
                log_netdev_debug(netdev, "OVS tunnel '%s' already exists, updating", netdev->ifname);
...
static int ovs_reconcile_network_port_one(Manager *m, Network *network, const char *ifname, sd_json_variant **ops) {
        ...
        if (mon) {
                ...
                if (check.found) {
                        ...
                        if (r < 0)
                                return r;
                        if (!ovs_vlan_bitmap_is_empty(network->ovs_port_vlan_bitmap)) {
...
static int ovs_reconcile_done(
        ...
        if (result && sd_json_variant_is_array(result)) {
