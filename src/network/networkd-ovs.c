        ...
        if (mon) {
                ...
                if (ovs_lookup_by_name(mon, "Port", netdev->ifname, &uuid)) {
                        _cleanup_(sd_json_variant_unrefp) sd_json_variant *update_row = NULL,
                                *update_ext = NULL, *update_trunks = NULL,
...
static int ovs_reconcile_port(Manager *m, NetDev *netdev, Set *doomed_ports, sd_json_variant **ops) {
                ...
                if (strv_isempty(bond_members)) {
                        ...
                        if (mon) {
                                ...
                                if (ovs_lookup_by_name(mon, "Port", netdev->ifname, &uuid)) {
                                        ...
                                        r = ovsdb_where_uuid(uuid, &del_where);
                                        if (r < 0)
                                                return r;
        ...
        if (mon) {
                ...
                if (found && ovs_port_needs_recreate(mon, uuid, netdev->ifname)) {
                        _cleanup_(sd_json_variant_unrefp) sd_json_variant *del_where = NULL, *delete_op = NULL;
                         ...
                         * UPDATE cannot converge it, so delete the row and fall through to the
                         * INSERT path below — the deletes are ordered before the inserts in this
                         * same transact, so there is no name collision. */
                        log_netdev_debug(netdev,
                                         "OVS port '%s' exists in OVSDB with bond-shaped interfaces (Type= changed from bond), recreating",
                                         strna(netdev->ifname));

                        /* Unlink from whatever bridge currently owns it (strong ref). The
                         * all-bridges mutate finds it even if Bridge= changed in the same edit. */
                        r = ovs_emit_bridge_ports_mutate(/* bridge_name= */ NULL, uuid, "delete", ops);
                        if (r < 0)
                                return r;

                        /* Delete the Port row — OVSDB cascades the member Interface deletes. */
                        r = ovsdb_where_uuid(uuid, &del_where);
                        if (r < 0)
                                return r;
                        ...
                        r = ovsdb_where_uuid(uuid, &update_where);
                        if (r < 0)
                                return r;
                         ...
                         * Interface state (old type or stale patch peer). */
                        {
                                ...
                                if (ovs_lookup_by_name(mon, "Interface", netdev->ifname, &iface_uuid)) {
                                        ...
                                        r = ovsdb_where_uuid(iface_uuid, &iface_where_update);
                                        if (r < 0)
                                                return r;
