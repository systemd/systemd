...
void link_check_ready(Link *link) {
        ...
        SET_FOREACH(a, link->addresses_ipv4acd)
                if (!set_contains(link->addresses, a))
                        return (void) log_link_debug(link, "%s(): an address "IPV4_ADDRESS_FMT_STR" is ARP probing.",
                                                     __func__, IPV4_ADDRESS_FMT_VAL(a->in_addr.in));
