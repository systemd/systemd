...
static int link_enable_ipv6(Link *link) {
        ...
        state = !link_ipv6_enabled(link);

        p = strjoina("/proc/sys/net/ipv6/conf/", link->ifname, "/disable_ipv6");

        r = write_string_file(p, one_zero(state), WRITE_STRING_FILE_VERIFY_ON_FAILURE);
        if (r < 0)
                log_link_warning_errno(link, r, "Cannot set IPv6 for interface: %m");
        else {
                if (state)
                        log_link_warning(link, "IPv6 disabled for interface: %m");
                else
                        log_link_warning(link, "IPv6 enabled for interface: %m");
        }
...
static int link_up(Link *link) {
        ...
        if (link->network->mtu) {
                ...
                if (link_ipv6_enabled(link) && link->network->mtu < IPV6_MIN_MTU) {
                        ...
                        log_link_warning(link, "Bumping MTU to 1280, as IPv6 is requested and requires a minimum MTU of 1280 bytes: %m");
