...
static int link_reconfigure_impl(Link *link, bool force) {
        ...
        /* The bound_to map depends on .network file, hence it needs to be freed.
         * But, do not free the bound_by map, Moreover, it is not necessary to recreate the map here,
         * as it depends on .network files assigned to other links. */
        link_free_bound_to_list(link);
