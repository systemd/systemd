...
static int link_new(Manager *manager, sd_netlink_message *message, Link **ret) {
        ...
        if (asprintf(&state_file, "/run/systemd/netif/links/%d", link->ifindex) < 0)
                return log_oom_debug();

        if (asprintf(&lease_file, "/run/systemd/netif/leases/%d", link->ifindex) < 0)
                return log_oom_debug();

        if (asprintf(&lldp_file, "/run/systemd/netif/lldp/%d", link->ifindex) < 0)
                return log_oom_debug();
