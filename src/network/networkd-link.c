...
static void link_update_master_operstate(Link *link) {
        Link *master;
...
void link_update_operstate(Link *link, bool also_update_master) {
        ...
        if (also_update_master)
                link_update_master_operstate(link);
}
...
static int link_update_master(Link *link, sd_netlink_message *message) {
        ...
        if (master_ifindex == link->ifindex) {
                log_link_debug(link, "rtnl: received master ifindex is equivalent to the self ifindex, ignoring.");
                return 0;
        }
...
static int link_update_hardware_address(Link *link, sd_netlink_message *message) {
        ...
        link->hw_addr.length = hw_addr.length;
...
static int link_new(Manager *manager, sd_netlink_message *message, Link **ret) {
        ...
        r = sd_netlink_message_get_type(message, &message_type);
        if (r < 0)
                return log_debug_errno(r, "rtnl: failed to read link message type: %m");
        else if (message_type != RTM_NEWLINK)
                return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "rtnl: received invalid link message type.");
