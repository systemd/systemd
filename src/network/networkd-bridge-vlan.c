...
int config_parse_ovs_trunks(
        ...
        log_syntax(unit, LOG_WARNING, filename, line, 0,
                   "OVSPort.Trunks= is deprecated, use OVSPort.VLAN= instead");
        ...
        for (const char *p = rvalue;;) {
                ...
                /* parse_vid_range accepts 0..VLANID_MAX (4094); VID 4095 is rejected
                 * upstream. IEEE 802.1Q reserves VID 0 (priority-tagged) at this level
                 * too — ovs-vswitchd rejects VID 0 as a trunk member;
                 * the encoder clamps to 1..VLANID_MAX. Warn so the operator sees their
                 * config didn't take effect, instead of silently dropping the VID. */
                if (vid == 0) {
                        log_syntax(unit, LOG_WARNING, filename, line, 0,
                                   "VLAN id 0 is reserved (IEEE 802.1Q priority-tagged frames), %s in %s=: %s",
                                   vid_end == 0 ? "ignoring" : "skipping VID 0 in range",
                                   lvalue, word);
                        if (vid_end == 0)
                                continue;
                        vid = 1;
