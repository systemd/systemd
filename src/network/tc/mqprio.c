...
static int mqprio_verify(QDisc *qdisc) {
        ...
        if (mqprio->n_queues > 0 && mqprio->n_queues != mqprio->num_tc)
                return log_warning_errno(SYNTHETIC_ERRNO(EINVAL),
                                         "%s: Number of Queues= entries (%u) must match "
                                         "NumberOfTrafficClasses= (%u). "
                                         "Ignoring [MultiQueuePriorityQDisc] section from line %u.",
                                         qdisc->section->filename, mqprio->n_queues,
                                         mqprio->num_tc, qdisc->section->line);
