...
TEST(config_parse_duid_type) {
        ...
        test_config_parse_duid_type_one(
                        "link-layer-time:2000-01-01 00:00:00 UTC", 0, DUID_TYPE_LLT, (usec_t) 946684800000000);
...
static void test_config_parse_duid_rawdata_one(const char *rvalue, int ret, const DUID *expected) {
        ...
        r = config_parse_duid_rawdata(
                        "network", "filename", 1, "section", 1, "lvalue", 0, rvalue, &actual, NULL);
        d = hexmem(actual.raw_data, actual.raw_data_len);
        log_info_errno(r, "\"%s\" → \"%s\" (%m)", rvalue, strnull(d));
        assert_se(r == ret);
        if (expected) {
...
static void test_config_parse_ether_addrs_one(const char *rvalue, const struct ether_addr *list, size_t n) {
        ...
        assert_se(config_parse_ether_addrs(
                                  "network", "filename", 1, "section", 1, "lvalue", 0, rvalue, &s, NULL) == 0);
...
TEST(config_parse_duid_rawdata) {
        ...
        test_config_parse_duid_rawdata_one(
                        "00:11:22:33:44:55:66:77",
                        0,
                        &(DUID){ 0, 8, { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77 } });
        test_config_parse_duid_rawdata_one("00:11:22:", 0, &(DUID){ 0, 3, { 0x00, 0x11, 0x22 } });
        ...
        test_config_parse_duid_rawdata_one(
                        "0:1:2:3:4:5:6:7", 0, &(DUID){ 0, 8, { 0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7 } });
        test_config_parse_duid_rawdata_one(
                        "11::", 0, &(DUID){ 0, 1, { 0x11 } }); /* FIXME: should this be an error? */
...
TEST(config_parse_ether_addr) {
        ...
        test_config_parse_ether_addrs_one(
                        "123.4567.89ab aa:bb:cc:dd:ee:ff 01-23-45-67-89-ab aa:Bb:CC:dd:ee:ff", t, 2);
        test_config_parse_ether_addrs_one(
                        "123.4567.89ab aa:bb:cc:dd:ee:fx hogehoge 01-23-45-67-89-ab aaaa aa:Bb:CC:dd:ee:ff",
                        t,
                        2);
}

static void test_config_parse_address_one(
                const char *rvalue,
                int family,
        ...
        assert_se(config_parse_match_ifnames(
                                  "network", "filename", 1, "section", 1, "Name", 0, "*", &network->match.ifname, network) ==
                  0);
        assert_se(config_parse_address(
                                  "network", "filename", 1, "section", 1, "Address", 0, rvalue, network, network) ==
                  0);
...
TEST(config_parse_address) {
        ...
        test_config_parse_address_one(
                        "::1", AF_INET6, 1, &(union in_addr_union){ .in6 = IN6ADDR_LOOPBACK_INIT }, 0);
        test_config_parse_address_one(
                        "::1/0", AF_INET6, 1, &(union in_addr_union){ .in6 = IN6ADDR_LOOPBACK_INIT }, 0);
        test_config_parse_address_one(
                        "::1/1", AF_INET6, 1, &(union in_addr_union){ .in6 = IN6ADDR_LOOPBACK_INIT }, 1);
        test_config_parse_address_one(
                        "::1/2", AF_INET6, 1, &(union in_addr_union){ .in6 = IN6ADDR_LOOPBACK_INIT }, 2);
        test_config_parse_address_one(
                        "::1/32", AF_INET6, 1, &(union in_addr_union){ .in6 = IN6ADDR_LOOPBACK_INIT }, 32);
        test_config_parse_address_one(
                        "::1/33", AF_INET6, 1, &(union in_addr_union){ .in6 = IN6ADDR_LOOPBACK_INIT }, 33);
        test_config_parse_address_one(
                        "::1/64", AF_INET6, 1, &(union in_addr_union){ .in6 = IN6ADDR_LOOPBACK_INIT }, 64);
        test_config_parse_address_one(
                        "::1/128", AF_INET6, 1, &(union in_addr_union){ .in6 = IN6ADDR_LOOPBACK_INIT }, 128);
...
TEST(config_parse_match_ifnames) {
        ...
        assert_se(config_parse_match_ifnames(
                                  "network", "filename", 1, "section", 1, "Name", 0, "!hoge hogehoge foo", &names, NULL) ==
                  0);
        assert_se(config_parse_match_ifnames(
                                  "network", "filename", 1, "section", 1, "Name", 0, "!baz", &names, NULL) ==
                  0);
        assert_se(config_parse_match_ifnames(
                                  "network", "filename", 1, "section", 1, "Name", 0, "aaa bbb ccc", &names, NULL) ==
                  0);
...
TEST(config_parse_match_strv) {
        ...
        assert_se(config_parse_match_strv(
                                  "network", "filename", 1, "section", 1, "Name", 0, "!hoge hogehoge foo", &names, NULL) ==
                  0);
        assert_se(config_parse_match_strv(
                                  "network", "filename", 1, "section", 1, "Name", 0, "!baz", &names, NULL) ==
