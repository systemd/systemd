...
static int bearer_get_all_handler(sd_bus_message *message, void *userdata, sd_bus_error *ret_error) {
        ...
        if (e) {
                ...
                log_warning_errno(r, "Could not get properties of bearer \"%s\": %s", b->path, bus_error_message(e, r));
                return 0;
        }
...
static int on_path(const char *path, void *userdata) {
        return set_put_strdup(ASSERT_PTR(userdata), path);
}
...
static int bearer_properties_changed_handler(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        if (bearer_get_all_handler(message, b, error) < 0)
                return 0;
