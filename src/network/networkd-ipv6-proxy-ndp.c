...
static int ipv6_proxy_ndp_address_configure_handler(sd_netlink *rtnl, sd_netlink_message *m, Request *req) {
        Link *link;
        int r;
...
static int ipv6_proxy_ndp_address_configure(Request *req) {
        ...
        if (r < 0)
                return log_link_warning_errno(link, r, "Could not create RTM_NEWNEIGH message: %m");

        r = sd_rtnl_message_neigh_set_flags(m, NTF_PROXY);
        if (r < 0)
                return log_link_warning_errno(link, r, "Could not set neighbor flags: %m");

        r = sd_netlink_message_append_in6_addr(m, NDA_DST, address);
        if (r < 0)
                return log_link_warning_errno(link, r, "Could not append NDA_DST attribute: %m");
...
int link_request_static_ipv6_proxy_ndp_addresses(Link *link) {
        ...
        SET_FOREACH(address, link->network->ipv6_proxy_ndp_addresses) {
                r = link_queue_request_full(link, REQUEST_TYPE_IPV6_PROXY_NDP,
                                            address, NULL,
                                            (hash_func_t) in6_addr_hash_func,
                                            (compare_func_t) in6_addr_compare_func,
                                            request_link_is_ready_to_configure,
