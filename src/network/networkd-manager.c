...
static int signal_reload_callback(sd_event_source *s, const struct signalfd_siginfo *si, void *userdata) {
        ...
        (void) manager_reload(m, /* message= */ NULL, /* varlink_request= */ NULL);
...
int manager_reload(Manager *m, sd_bus_message *message, sd_varlink *varlink_request) {
        Link *link;
        int r;

        assert(m);
        assert(!message || !varlink_request); /* D-Bus and Varlink are mutually exclusive */
        ...
        HASHMAP_FOREACH(link, m->links_by_index)
                (void) link_reconfigure_full(link, /* flags= */ 0, message,
                                             /* counter= */ (message || varlink_request) ? &m->reloading : NULL,
                                             varlink_request);
