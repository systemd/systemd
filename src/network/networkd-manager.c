...
static int persistent_storage_is_ready(void) {
        int r;

        if (access("/run/systemd/netif/persistent-storage-ready", F_OK) < 0) {
                if (errno != ENOENT)
                        return log_debug_errno(errno, "Failed to check if /run/systemd/netif/persistent-storage-ready exists, ignoring: %m");
                return false;
        }
        ...
        if (r == 0)
                return true;
        if (r < 0)
                log_debug_errno(r, "Failed to check if /var/lib/systemd/network/ is read-only, ignoring: %m");
        else
                log_debug_errno(SYNTHETIC_ERRNO(EROFS), "/var/lib/systemd/network/ is read-only.");
