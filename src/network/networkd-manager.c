...
static int manager_save(Manager *m) {
        _cleanup_ordered_set_free_free_ OrderedSet *dns = NULL, *ntp = NULL, *sip = NULL, *pop3 = NULL,
                *smtp = NULL, *log_servers = NULL, *search_domains = NULL, *route_domains = NULL;
        ...
        log_servers = ordered_set_new(&string_hash_ops);
        if (!log_servers)
                return -ENOMEM;
        ...
        HASHMAP_FOREACH(link, m->links, i) {
                ...
                r = sd_dhcp_lease_get_log_servers(link->dhcp_lease, &addresses);
                if (r > 0) {
                        r = ordered_set_put_in4_addrv(log_servers, addresses, r, in4_addr_is_non_local);
                        if (r < 0)
                                return r;
                } else if (r < 0 && r != -ENODATA)
                        return r;
        ...
        ordered_set_print(f, "LOG_SERVERS=", log_servers);
