...
static int manager_schedule_logind_reconnect(Manager *m) {
        ...
        (void) sd_event_source_set_description(m->logind_varlink_retry_event, "networkd-logind-reconnect");
        return 0;
}
...
static int on_logind_event(sd_varlink *link, sd_json_variant *parameters, const char *error_id, sd_varlink_reply_flags_t flags, void *userdata) {
        ...
        log_debug("Coming back from suspend (via Varlink), reconfiguring all connections...");

        Link *link_entry;
        HASHMAP_FOREACH(link_entry, m->links_by_index)
                (void) link_reconfigure(link_entry, LINK_RECONFIGURE_UNCONDITIONALLY);
