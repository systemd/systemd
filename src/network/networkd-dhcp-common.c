...
int link_get_captive_portal(Link *link, const char **ret) {
        ...
        if (dhcp4_cp) {
                ...
                if (ndisc_cp && !streq(dhcp4_cp, ndisc_cp))
                        log_link_debug(link, "IPv6RA captive portal (%s) does not match DHCPv4 (%s), ignorning IPv6RA captive portal.",
                                       ndisc_cp, dhcp4_cp);
        ...
        if (dhcp6_cp) {
                if (ndisc_cp && !streq(dhcp6_cp, ndisc_cp))
                        log_link_debug(link, "IPv6RA captive portal (%s) does not match DHCPv6 (%s), ignorning IPv6RA captive portal.",
                                       ndisc_cp, dhcp6_cp);
