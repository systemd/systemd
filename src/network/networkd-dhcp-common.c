...
int config_parse_dhcp_send_option(
        ...
        dhcpv6 = !(STRPTR_IN_SET(section, "DHCP", "DHCPv4"));
        ...
        if (dhcpv6) {
                ...
                old6 = ordered_hashmap_remove(*options, UINT_TO_PTR(u16));
                r = ordered_hashmap_put(*options, UINT_TO_PTR(u16), opt6);
                if (r < 0) {
                        log_syntax(unit, LOG_ERR, filename, line, r,
                                   "Failed to store DHCP option '%s', ignoring assignment: %m", rvalue);
                        return 0;
                ...
                /* Overwrite existing option */
                old4 = ordered_hashmap_remove(*options, UINT_TO_PTR(u8));
                r = ordered_hashmap_put(*options, UINT_TO_PTR(u8), opt4);
                if (r < 0) {
                        log_syntax(unit, LOG_ERR, filename, line, r,
                                   "Failed to store DHCP option '%s', ignoring assignment: %m", rvalue);
                        return 0;
