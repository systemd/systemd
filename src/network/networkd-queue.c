...
static int remove_request_netlink_handler(sd_netlink *nl, sd_netlink_message *m, RemoveRequest *req) {
        assert(req);
        ...
        if (req->netlink_handler)
                return req->netlink_handler(nl, m, req);
...
int manager_process_remove_requests(Manager *manager) {
        ...
        while ((req = ordered_set_first(manager->remove_request_queue))) {
                ...
                r = netlink_call_async(
                                ...
                                remove_request_netlink_handler,
