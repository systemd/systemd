...
static int request_compare_func(const struct Request *a, const struct Request *b) {
        ...
        assert(a->hash_func == b->hash_func);
        assert(a->compare_func == b->compare_func);
...
static int request_new(
        ...
        *req = (Request) {
                ...
                .compare_func = compare_func ?: trivial_compare_func,
                .is_ready = is_ready,
                .process = process,
                .counter = counter,
...
int netdev_queue_request(
        ...
        return request_new(netdev->manager, NULL, REQUEST_TYPE_NETDEV_INDEPENDENT,
                           netdev_ref(netdev), (mfree_func_t) netdev_unref, NULL, NULL,
...
int link_queue_request_full(
        ...
        return request_new(link->manager, link, type,
                           ...
                           is_ready, process, counter, netlink_handler, ret);
}
...
int manager_process_requests(sd_event_source *s, void *userdata) {
        ...
        for (;;) {
                ...
                ORDERED_SET_FOREACH(req, manager->request_queue) {
                        ...
                        processed = true;
                        request_unref(ordered_set_remove(manager->request_queue, req));
                        continue;
                ...
                /* When at least one request is processed, then another request can be ready now. */
                if (!processed)
                        break;
        }
...
static int request_netlink_handler(sd_netlink *nl, sd_netlink_message *m, Request *req) {
        ...
        if (req->netlink_handler)
                r = req->netlink_handler(nl, m, req);
