...
int manager_serialize(Manager *manager) {
        ...
        HASHMAP_FOREACH(link, manager->links_by_index) {
                ...
                r = addresses_append_json(link->addresses, /* only_managed = */ true, &e);
                if (r < 0)
                        return r;
...
static int link_deserialize_address(Link *link, sd_json_variant *v) {
        ...
        if (p.provider.iov_len != 0 && p.peer.iov_len != FAMILY_ADDRESS_SIZE(p.family))
                return log_link_debug_errno(link, SYNTHETIC_ERRNO(EINVAL),
...
static int manager_deserialize_link(Manager *manager, sd_json_variant *v) {
        static const sd_json_dispatch_field dispatch_table[] = {
                { "Index", _SD_JSON_VARIANT_TYPE_INVALID, json_dispatch_ifindex, 0, SD_JSON_MANDATORY | SD_JSON_REFUSE_NULL },
                {},
        };
        ...
        int ifindex;
        r = sd_json_dispatch(v, dispatch_table, SD_JSON_ALLOW_EXTENSIONS, &ifindex);
        if (r < 0)
                return log_debug_errno(r, "Failed to dispatch ifindex from json variant: %m");

        Link *link;
        r = link_get_by_index(manager, ifindex, &link);
        if (r < 0) {
                log_debug_errno(r, "No interface with deserialized ifindex (%i) found: %m", ifindex);
                return 0; /* Already removed? */
        }

        sd_json_variant *i;
        JSON_VARIANT_ARRAY_FOREACH(i, sd_json_variant_by_key(v, "Addresses"))
