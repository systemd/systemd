...
static int generate_tmp_unit(const char *dest_dir, char **ret_path, FILE **ret_file) {
        _cleanup_free_ char *t = NULL;
        FILE *f;
        int r;

        r = fopen_temporary(dest_dir, &f, &t);
        if (r < 0)
                return r;

        (void) fchmod(fileno(f), 0644);

        fprintf(f, "# Automatically generated by systemd-network-generator\n\n");
...
static int network_save(Network *network, const char *dest_dir) {
        ...
        r = generate_tmp_unit(dest_dir, &tmp_path, &f);
        if (r < 0)
                return r;
        ...
        r = conservative_rename(tmp_path, p);
        if (r < 0)
                return r;
