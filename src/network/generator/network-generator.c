...
static int network_set_dhcp_type(Context *context, const char *ifname, const char *dhcp_type) {
        ...
        if (t < 0)
                return log_error_errno(t, "Invalid DHCP type '%s'", dhcp_type);

        network = network_get(context, ifname);
        if (!network) {
                r = network_new(context, ifname, &network);
                if (r < 0)
                        return log_error_errno(r, "Failed to create network for '%s': %m", ifname);
        }
...
static int network_set_hostname(Context *context, const char *ifname, const char *hostname) {
        ...
        network = network_get(context, ifname);
        if (!network)
                return log_error_errno(SYNTHETIC_ERRNO(-ENODEV), "No network found for '%s'", ifname);
...
static int network_set_mtu(Context *context, const char *ifname, const char *mtu) {
        ...
        if (r < 0)
                return log_error_errno(r, "Invalid MTU '%s' for '%s': %m", mtu, ifname);
...
static int network_set_mac_address(Context *context, const char *ifname, const char *mac) {
        ...
        if (r < 0)
                return log_error_errno(r, "Invalid MAC address '%s' for '%s'", mac, ifname);
...
static int network_set_dns(Context *context, const char *ifname, int family, const char *dns) {
        ...
        if (r < 0)
                return log_error_errno(r, "Invalid DNS address '%s' for '%s'", dns, ifname);
...
static int parse_ip_address_one(int family, const char **value, union in_addr_union *ret) {
        ...
        if (family == AF_INET6) {
                if (p[0] != '[')
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid IPv6 address '%s'", p);
                ...
                q = strchr(p, ':');
                if (!q)
                        log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid IPv4 address '%s'", p);
        ...
        if (r < 0)
                return log_error_errno(r, "Invalid IP address '%s': %m", buf);
...
static int parse_netmask_or_prefixlen(int family, const char **value, unsigned char *ret) {
        ...
        if (r > 0) {
                if (family == AF_INET6)
                        ...
                        return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP), "IPv6 prefix length is not supported yet");
                ...
                *ret = family == AF_INET6 ? 128 : 32;
                ...
                p = strchr(*value, ':');
                if (!p)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid netmask or prefix length '%s'", *value);

                q = strndupa_safe(*value, p - *value);
                r = safe_atou8(q, ret);
                if (r < 0)
                        return log_error_errno(r, "Invalid netmask or prefix length '%s': %m", q);
...
static int parse_ip_dns_address_one(Context *context, const char *ifname, const char **value) {
        ...
        if (p[0] == '[') {
                q = strchr(p + 1, ']');
                if (!q || !IN_SET(q[1], ':', '\0'))
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid IP DNS address '%s'", p);
...
static int parse_cmdline_ip_address(Context *context, int family, const char *value) {
        ...
        p = strchr(value, ':');
        if (!p)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid IP address '%s'", value);

        if (p != value) {
                hostname = strndupa_safe(value, p - value);
                if (!hostname_is_valid(hostname, 0))
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid hostname '%s'", hostname);
        }
...
static int parse_cmdline_ip(Context *context, const char *key, const char *value) {
        ...
        if (proc_cmdline_value_missing(key, value))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Missing value for '%s'", key);
...
static int parse_cmdline_rd_route(Context *context, const char *key, const char *value) {
        ...
        if (value[0] == '[') {
                p = strchr(value, ']');
                if (!p)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid IPv6 address '%s'", value);
                ...
                p = strchr(value, ':');
                if (!p)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid IPv4 address '%s'", value);
...
static int parse_cmdline_rd_peerdns(Context *context, const char *key, const char *value) {
        ...
        if (r < 0)
                return log_error_errno(r, "Invalid boolean value '%s'", value);
...
static int parse_cmdline_vlan(Context *context, const char *key, const char *value) {
        ...
        p = strchr(value, ':');
        if (!p)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid VLAN value '%s'", value);
        ...
        if (!netdev) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to create VLAN device for '%s': %m", name);
        }
...
static int parse_cmdline_bridge(Context *context, const char *key, const char *value) {
        ...
        p = strchr(value, ':');
        if (!p)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid bridge value '%s'", value);
        ...
        if (!netdev) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to create bridge device for '%s': %m", name);
        }

        p++;
        if (isempty(p))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Missing slave interfaces for bridge '%s'", name);
        ...
        for (;;) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to parse slave interfaces for bridge '%s'", name);
                if (r == 0)
                        return 0;
...
static int parse_cmdline_bond(Context *context, const char *key, const char *value) {
        ...
        p = strchr(value, ':');
        if (!p)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid bond value '%s'", value);
        ...
        if (!netdev) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to create bond device for '%s': %m", name);
        }
        ...
        if (isempty(slaves))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Missing slave interfaces for bond '%s'", name);
        ...
        for (const char *q = slaves; ; ) {
                ...
                if (r == 0)
                        break;
                if (r < 0)
                        return log_error_errno(r, "Failed to parse slave interfaces for bond '%s'", name);
...
static int parse_cmdline_ifname(Context *context, const char *key, const char *value) {
        ...
        p = strchr(value, ':');
        if (!p)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid ifname value '%s'", value);

        name = strndupa_safe(value, p - value);

        r = parse_hw_addr(p + 1, &mac);
        if (r < 0)
                return log_error_errno(r, "Invalid MAC address '%s' for '%s'", p + 1, name);

        r = link_new(context, name, &mac, NULL);
        if (r < 0)
                return log_error_errno(r, "Failed to create link for '%s': %m", name);
...
static int parse_cmdline_ifname_policy(Context *context, const char *key, const char *value) {
        ...
        for (const char *q = value; ; ) {
                ...
                if (r == 0)
                        break;
                if (r < 0)
                        return log_error_errno(r, "Failed to parse ifname policy '%s'", value);

                p = name_policy_from_string(word);
                if (p < 0) {
                        r = parse_hw_addr(word, &mac);
                        if (r < 0)
                                return log_error_errno(r, "Invalid MAC address '%s'", word);

                        if (hw_addr_is_null(&mac))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "MAC address is not set");

                        if (!isempty(q))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Unexpected trailing string '%s' in ifname policy '%s'", q, value);
        ...
        if (strv_isempty(policies))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "No ifname policy specified");

        r = link_new(context, NULL, &mac, &link);
        if (r < 0)
                return log_error_errno(r, "Failed to create link: %m");
...
int context_merge_networks(Context *context) {
        ...
        HASHMAP_FOREACH(network, context->networks_by_name) {
                ...
                LIST_FOREACH(routes, route, all->routes) {
                        ...
                        if (r < 0)
                                return log_error_errno(r, "Failed to copy route: %m");
                }
        }
