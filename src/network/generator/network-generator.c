...
static int parse_cmdline_ip_address(Context *context, int family, const char *value) {
         ...
         *
         * Here, only interface and DHCP type are mandatory. */
        ...
        if (parse_cmdline_ip_mtu_mac(context, ifname, p) >= 0)
                return 0;
         ...
         * Note, we accept one trailing ':', as it was eaten by extract_first_word(). */
        if (!isempty(p))
