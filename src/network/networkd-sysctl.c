...
static int sysct_write_event(void *ctx, void *data, size_t data_sz) {
        ...
        if (we->version != 1)
                return log_warning_errno(SYNTHETIC_ERRNO(EINVAL),
                                "Invalid sysctl write event version: %d", we->version);
        ...
        if (!streq(value, we->newvalue))
                log_struct(LOG_WARNING,
                           LOG_MESSAGE("Foreign process '%s' changed sysctl '%s' from '%s' to '%s', conflicting with our setting to '%s'",
                                        we->comm, we->name, we->current, we->newvalue, value),
...
int sysctl_add_monitor(Manager *manager) {
        ...
        if (r < 0)
                return log_warning_errno(r, "Failed to get cgroup path: %m, ignoring.");

        rootcg = cg_path_open(SYSTEMD_CGROUP_CONTROLLER, "/");
        if (rootcg < 0)
                return log_warning_errno(rootcg, "Failed to open cgroup: %m, ignoring.");

        obj = sysctl_monitor_bpf__open_and_load();
        if (!obj) {
                log_info_errno(errno, "Failed to load sysctl monitor BPF program: %m, ignoring.");
                return 0;
        }
        ...
        if (r < 0) {
                log_info_errno(r, "Failed to attach sysctl monitor BPF program to cgroup: %m, ignoring.");
                return 0;
        }
...
int sysctl_clear_link_shadows(Link *link) {
        ...
        HASHMAP_FOREACH_KEY(value, key, link->manager->sysctl_shadow)
                if (path_startswith(key, ipv4) || path_startswith(key, ipv6)) {
                        hashmap_remove_value(link->manager->sysctl_shadow, key, value);
                        free(key);
                        free(value);
                }
