...
static int sysct_write_event(void *ctx, void *data, size_t data_sz) {
        ...
        r = cg_path_get_cgroupid("/sys/fs/cgroup/system.slice/systemd-networkd.service", &cgroup_id);
        if (r < 0)
                return log_warning_errno(r, "Failed to get cgroup id: %m");

        log_error("write event, cgroupids: %lu, %lu", cgroup_id, we->cgroup_id);
        if (we->cgroup_id == cgroup_id)
                return 0;
