...
static int open_rootcg(void) {
        int fd = open(CGROUP_MOUNT_DFLT, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
...
void sysctl_add_monitor(Manager *manager) {
        ...
        r = dlopen_bpf();
        if (r < 0) {
                log_info("sysctl monitor disabled, as BPF support is not available.");
                return;
        }
        ...
        sysctl_link = bpf_program__attach_cgroup(obj->progs.sysctl_monitor, cgroup_fd);
        r = bpf_get_error_translated(sysctl_link);
        if (r < 0) {
                log_warning_errno(r, "Failed to attach sysctl monitor BPF program to cgroup: %m");
                return;
        }

        sysctl_buffer = ring_buffer__new(
                        bpf_map__fd(obj->maps.written_sysctls),
        ...
        r = sd_event_add_io(manager->event, &manager->sysctl_event_source,
                        ring_buffer__epoll_fd(sysctl_buffer), EPOLLIN, on_ringbuf_io, sysctl_buffer);
        if (r < 0) {
