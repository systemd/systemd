...
int sysctl_add_monitor(Manager *manager) {
        ...
        if (rootcg < 0)
                return log_warning_errno(rootcg, "Failed to open cgroup: %m");

        obj = sysctl_monitor_bpf__open_and_load();
        if (!obj)
                return log_warning_errno(errno, "Failed to load sysctl monitor BPF program: %m");

        cgroup_fd = cg_path_open(SYSTEMD_CGROUP_CONTROLLER, "/system.slice/systemd-networkd.service");
        if (cgroup_fd < 0)
...
int sysctl_remove_monitor(Manager *manager) {
        _cleanup_close_ int rootcg = -EBADF;
        int r;
