...
int sysctl_add_monitor(Manager *manager) {
        ...
        r = cg_path_get_cgroupid("/sys/fs/cgroup/system.slice/systemd-networkd.service", &l2);
        if (r < 0)
                return log_warning_errno(r, "Failed to get cgroup id: %m");

        idx = l2;
        if (sym_bpf_map_update_elem(sym_bpf_map__fd(obj->maps.cgroup_map), &idx, &cgroup_id, BPF_ANY))
                return log_error_errno(errno, "Failed to update cgroup map: %m");
