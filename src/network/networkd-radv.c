...
static int pref64_prefix_section_verify(pref64Prefix *p) {
        ...
        if (!IN_SET(p->prefixlen, 32, 40, 48, 56, 64, 96))
                return log_warning_errno(SYNTHETIC_ERRNO(EINVAL),
        ...
        if (p->lifetime == 0)
                return log_warning_errno(SYNTHETIC_ERRNO(EINVAL),
                                         "%s: The lifetime of PREF64 prefix cannot be zero. But it be is used to deconfigure "
                                         "a previously announced prefix immediately. Ignoring [IPv6PREF64Prefix] section from line %u.",
                                         p->section->filename, p->section->line);
        return 0;
}
