...
static int radv_configure(Link *link) {
        ...
        if (link->network->router_retransmit_usec > 0) {
                r = sd_radv_set_retransmit(link->radv, link->network->router_retransmit_usec / USEC_PER_MSEC);
                if (r < 0)
                        return r;
        }
...
int config_parse_router_retransmit(
        ...
        if (r < 0) {
                log_syntax(unit, LOG_WARNING, filename, line, r,
                           "Failed to parse router Neighbor Solicitation retransmit, ignoring assignment: %s", rvalue);
                return 0;
        }
        ...
        if (usec != USEC_INFINITY &&
            DIV_ROUND_UP(usec, USEC_PER_MSEC) > UINT32_MAX) {
                log_syntax(unit, LOG_WARNING, filename, line, 0,
                           "Router Neighbor Solicitation retransmit = must be in the range 0...%"PRIu32"ms, ignoring: %s", UINT32_MAX, rvalue);
                return 0;
        }
