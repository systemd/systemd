...
int radv_add_prefix(
        ...
        r = sd_radv_prefix_set_preferred_lifetime(p, usec_sub_unsigned(lifetime_preferred, now_usec) / USEC_PER_SEC);
        if (r < 0)
                return r;

        r = sd_radv_prefix_set_valid_lifetime(p, usec_sub_unsigned(lifetime_valid, now_usec) / USEC_PER_SEC);
        if (r < 0)
                return r;
