...
int link_request_to_set_addrgen_mode(Link *link) {
        ...
        /* If the link is already up, then changing the mode by netlink does take no effect until the
         * link becomes down. Hence, we need to reset the interface. However, setting the mode by
         * sysctl does not need that. Let's use the sysctl interface when the link is already up.
