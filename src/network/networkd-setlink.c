...
static int link_configure(Request *req) {
        ...
        if (r < 0)
                return log_link_warning_errno(link, r, "Could not allocate netlink message: %m");
        ...
        r = netlink_call_async(link->manager->rtnl, NULL, m,
                               (link_netlink_message_handler_t) req->netlink_handler,
                               link_netlink_destroy_callback, link);
        if (r < 0)
                return log_link_warning_errno(link, r, "Could not send netlink message: %m");

        if (req->type == REQUEST_TYPE_SET_LINK_FLAGS)
                link->set_flags_messages++;
...
static int link_is_ready_to_set_link(Request *req) {
        Link *link;
        int r;
...
static int link_request_set_link(
        ...
        r = link_queue_request_full(link, type, NULL, NULL, NULL, NULL,
                                    link_is_ready_to_set_link, link_configure,
...
static int link_up_or_down(Request *req) {
        _cleanup_(sd_netlink_message_unrefp) sd_netlink_message *m = NULL;
        Link *link;
        bool up;
        int r;
...
static int link_is_ready_to_activate(Request *req) {
        Link *link;
...
static int link_is_ready_to_bring_up_or_down(Request *req) {
        Link *link;
        bool up;
...
int link_request_to_bring_up_or_down(Link *link, bool up) {
        ...
        r = link_queue_request_full(link, REQUEST_TYPE_UP_DOWN,
                                    INT_TO_PTR(up), NULL, NULL, NULL,
                                    link_is_ready_to_bring_up_or_down,
                                    link_up_or_down,
                                    &link->set_flags_messages,
                                    link_up_or_down_handler, NULL);
        if (r < 0)
