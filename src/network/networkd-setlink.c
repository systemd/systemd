 ...
 * (format_ifname), not via networkd's links_by_index hashmap, which may not
...
static bool link_master_is_ovs_system(Link *link) {
        ...
        r = format_ifname(link->master_ifindex, buf);
        if (r < 0) {
                /* -ENXIO/-ENODEV are the common cases: the master ifindex was just freed
                 * by the kernel and not yet observed via RTM_DELLINK. Other errors
                 * (e.g. -ENOMEM) are unexpected and worth surfacing in debug output. */
                if (!IN_SET(r, -EINVAL, -ENXIO, -ENODEV))
                        log_link_debug_errno(link, r, "Failed to resolve master ifindex %i to name, treating as non-OVS: %m",
                                             link->master_ifindex);
                return false;
        }
...
int link_request_to_set_master(Link *link) {
        ...
        if (link->network->keep_master) {
                ...
                if (link_master_is_ovs_system(link)) {
                        if (!link->ovs_master_warning_logged) {
                                ...
                                link->ovs_master_warning_logged = true;
                        } else {
                                log_link_debug(link,
                                               "Cannot move OVS-attached port to kernel master via netlink, skipping (already warned).");
                        }
                        link->master_set = true;
                        return 0;
                }

                /* Reset the OVS-conflict warning so a recurrence (e.g. an automation tool
                 * re-attaches the port to OVS later) is reported again at LOG_WARNING
                 * instead of being silently demoted to LOG_DEBUG. */
                link->ovs_master_warning_logged = false;
                ...
                /* OVS attachment is set up out-of-band: manager_ovs_reconcile() pushes
                 ...
                 *    and let manager_ovs_reconcile() do its thing.
                 ...
                 *    in that case; manager_ovs_reconcile() will run on the next
                 ...
                 * yet — ovs-vswitchd will release the port when manager_ovs_reconcile()
