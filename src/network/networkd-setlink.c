...
static int set_link_handler_internal(
        ...
        if (r < 0) {
                ...
                error_msg = strjoina("Failed to set ", request_type_to_string(type), ignore ? ", ignoring" : "");
        ...
        log_link_debug(link, "%s set.", request_type_to_string(type));
...
static int link_set_addrgen_mode_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        ...
        r = set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_ADDRESS_GENERATION_MODE, /* ignore = */ true, NULL);
        if (r <= 0)
                return r;
...
static int link_set_bond_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_BOND, /* ignore = */ false, NULL);
}

static int link_set_bridge_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_BRIDGE, /* ignore = */ true, NULL);
}

static int link_set_bridge_vlan_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_BRIDGE_VLAN, /* ignore = */ false, NULL);
}

static int link_set_can_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_CAN, /* ignore = */ false, NULL);
}

static int link_set_flags_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_FLAGS, /* ignore = */ false, get_link_update_flag_handler);
}

static int link_set_group_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_GROUP, /* ignore = */ false, NULL);
}

static int link_set_ipoib_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_IPOIB, /* ignore = */ true, NULL);
}

static int link_set_mac_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_MAC, /* ignore = */ true, get_link_default_handler);
}
...
static int link_set_master_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_MASTER, /* ignore = */ false, get_link_master_handler);
}
...
static int link_unset_master_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        ...
        return set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_MASTER, /* ignore = */ true, get_link_master_handler);
}

static int link_set_mtu_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        int r;

        r = set_link_handler_internal(rtnl, m, link, REQUEST_TYPE_SET_LINK_MTU, /* ignore = */ true, get_link_default_handler);
        if (r <= 0)
                return r;
...
static int link_configure(Link *link, RequestType type, void *userdata, link_netlink_message_handler_t callback) {
        ...
        log_link_debug(link, "Setting %s", request_type_to_string(type));

        if (type == REQUEST_TYPE_SET_LINK_BOND)
                r = sd_rtnl_message_new_link(link->manager->rtnl, &m, RTM_NEWLINK, link->master_ifindex);
        else if (IN_SET(type, REQUEST_TYPE_SET_LINK_CAN, REQUEST_TYPE_SET_LINK_IPOIB))
        ...
        r = link_configure_fill_message(link, m, type, userdata);
        if (r < 0)
                return r;
        ...
        if (type == REQUEST_TYPE_SET_LINK_FLAGS)
                link->set_flags_messages++;
...
static int link_is_ready_to_set_link(Link *link, RequestType type, Request *req) {
        int r;
        ...
        case REQUEST_TYPE_SET_LINK_MAC:
                if (req->netlink_handler == (request_netlink_handler_t) link_set_mac_handler) {
...
static int link_process_set_link(Request *req, Link *link, void *userdata) {
        ...
        r = link_is_ready_to_set_link(link, req->type, req);
        if (r <= 0)
                return r;

        r = link_configure(link, req->type, req->userdata, (link_netlink_message_handler_t) req->netlink_handler);
        if (r < 0)
...
static int link_request_set_link(
                Link *link,
                RequestType type,
                link_netlink_message_handler_t handler,
                Request **ret) {
        ...
        r = link_queue_request_full(link, type, NULL, NULL, NULL, NULL,
                                    ...
                                    NULL, (request_netlink_handler_t) handler,
                                    &req);
        if (r < 0)
...
static int link_up_or_down_handler(sd_netlink *rtnl, sd_netlink_message *m, Request *req) {
        bool up, on_activate;
        Link *link;
        int r;
...
static int link_process_activate(Request *req, Link *link, void *userdata) {
...
int link_request_to_bring_up_or_down(Link *link, bool up) {
        ...
        r = link_queue_request_full(link, REQUEST_TYPE_UP_DOWN,
                                    ...
                                    link_up_or_down_handler, NULL);
        if (r < 0)
                return log_link_error_errno(link, r, "Failed to request to bring link %s: %m",
                                            up_or_down(up));
