...
static int set_link_handler_internal(sd_netlink *rtnl, sd_netlink_message *m, Link *link, SetLinkMode mode, bool ignore) {
        ...
        log_link_debug(link, "%s set.", set_link_mode_to_string(mode));
        return 1;
}
...
static bool link_is_ready_to_call_set_link(Request *req) {
        ...
        case SET_LINK_BRIDGE: {
                Request req_set_master = {
                        ...
                        .set_link_mode = SET_LINK_MASTER,
                };

                if (ordered_set_contains(link->manager->request_queue, &req_set_master))
                        return false;
                break;
        }
        case SET_LINK_MASTER: {
...
int request_process_set_link(Request *req) {
        ...
        r = link_configure(req->link, req->set_link_mode, req->userdata, req->netlink_handler);
        if (r < 0)
                return log_link_error_errno(req->link, r, "Failed to set %s: %m",
                                            set_link_mode_to_string(req->set_link_mode));
