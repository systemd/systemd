...
static int manager_update_sr_iov_ifindices(Manager *manager, int phys_port_ifindex, int virt_port_ifindex) {
        ...
        (void) link_get_by_index(manager, phys_port_ifindex, &phys_link);
        (void) link_get_by_index(manager, virt_port_ifindex, &virt_link);
...
static int link_set_sr_iov_virt_ports(Link *link) {
        ...
        FOREACH_DIRENT_ALL(de, dir, break) {
                ...
                r = manager_update_sr_iov_ifindices(link->manager, link->ifindex, r);
                if (r < 0)
                        return r;
        }
