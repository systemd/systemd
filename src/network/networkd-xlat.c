...
static int xlat_create_tun(Link *link) {
        ...
        if (snprintf(tun_name, sizeof(tun_name), "cl-%s", link->ifname) >= (int) sizeof(tun_name))
                return log_link_error_errno(link, SYNTHETIC_ERRNO(ENAMETOOLONG),
                                            "CLAT TUN name 'cl-%s' is too long.", link->ifname);
        ...
        if (link->clat_ifindex <= 0)
                return log_link_error_errno(link, errno,
                                            "Failed to get ifindex for CLAT TUN '%s': %m", ifr.ifr_name);

        int r = sd_event_add_io(link->manager->event, &link->clat_tun_event_source,
...
static int xlat_attach_bpf_filter(Link *link, int fd) {
        ...
        /* Max filter size: next_header(3) + prefix(3*4) + dst(4*4) + accept/reject(2) = ~25 instructions */
...
static int xlat_set_tun_up(Link *link) {
        ...
        if (link->mtu > 20) {
                uint32_t tun_mtu = MIN(link->mtu - 20, CLAT_BUFSIZE - sizeof(struct ip6_hdr));
...
static int xlat_configure_route_on_link(Link *link) {
        ...
        /* Dynamic MTU: link MTU minus header overhead (20 for IPv4→IPv6 + 8 for fragmentation ext hdr).
         * Minimum 1280 (IPv6 minimum MTU) - 28 = 1252. */
        ...
        uint32_t advmss = mtu > 40 ? mtu - 40 : mtu;
...
static int xlat_start_bpf(Link *link) {
        ...
        obj = clat_bpf__open_and_load();
        if (!obj)
                return log_link_debug_errno(link, errno,
                                            "CLAT: failed to load BPF program, will try TUN fallback.");

        /* Configure BPF program via BSS section */
...
int xlat_start(Link *link) {
        ...
        log_link_info(link, "Starting CLAT with PREF64 %s/%u, source %s.",
