...
static int xlat_tun_handler(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        if (n < 0) {
                if (ERRNO_IS_TRANSIENT(errno))
                        return 0;
                return log_link_warning_errno(link, errno, "CLAT: failed to read from TUN: %m");
        }
        if (n == 0)
                return 0;
...
static int xlat_recv_handler(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        if (n < 0) {
                if (ERRNO_IS_TRANSIENT(errno))
                        return 0;
                return log_link_warning_errno(link, errno, "CLAT: failed to read from packet socket: %m");
        }
...
static int xlat_set_tun_up(Link *link) {
        ...
        if (link->mtu > 20) {
                r = sd_netlink_message_append_u32(m, IFLA_MTU, link->mtu - 20);
                if (r < 0)
...
static int xlat_configure_route(Link *link) {
        ...
        log_link_debug(link, "CLAT: added default IPv4 route via TUN (metric %" PRIu32 ").",
                       CLAT_ROUTE_METRIC);
        return 0;
}
