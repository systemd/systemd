...
static int ndisc_router_process_default(Link *link, sd_ndisc_router *rt) {
        ...
        if (link->network->ipv6_accept_ra_use_icmp6_ratelimit) {
                ...
                if (r < 0)
                        log_link_debug(link, "Failed to get default router preference from RA, ignoring: %m");
                else if (timestamp_is_set(icmp6_ratelimit)) {
                        ...
                        if (r < 0)
                                log_link_warning_errno(link, r, "Could not configure icmp6 rate limit, ignoring: %m");
                }
        }
