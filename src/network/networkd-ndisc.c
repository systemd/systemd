...
static int ndisc_process_request(Request *req) {
        Link *link;
        int r;
...
static int ndisc_is_ready_to_configure(Request *req) {
        Link *link;
...
int link_request_ndisc(Link *link) {
        ...
        r = link_queue_request(link, REQUEST_TYPE_NDISC, ndisc_is_ready_to_configure, ndisc_process_request, NULL);
        if (r < 0)
