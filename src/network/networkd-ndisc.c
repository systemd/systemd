...
static void ndisc_check_ready(Link *link);
...
static int ndisc_address_ready_callback(Address *address) {
        ...
        ndisc_check_ready(address->link);
        return 0;
}
...
static void ndisc_check_ready(Link *link) {
        ...
        if (link->ndisc_messages > 0) {
                log_link_debug(link, "%s(): SLAAC address and routes are not set.", __func__);
                return;
        }
...
static void ndisc_mark(Link *link, const struct in6_addr *router) {
        ...
        SET_FOREACH(address, link->addresses) {
                if (address->source != NETWORK_CONFIG_SOURCE_NDISC)
                        continue;
                if (!in6_addr_equal(&address->provider.in6, router))
                        continue;
