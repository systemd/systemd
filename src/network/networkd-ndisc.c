...
static int ndisc_pref64_compare_func(const NDiscPREF64 *a, const NDiscPREF64 *b) {
        ...
        return  memcmp(&a->prefix, &b->prefix, FAMILY_ADDRESS_SIZE(AF_INET6));
}
...
static int ndisc_router_process_pref64(Link *link, sd_ndisc_router *rt) {
        ...
        if (lifetime_usec == 0) {
                free(set_remove(link->ndisc_pref64, &(NDiscPREF64) { .prefix = a, .prefix_len = prefix_len }));
                return 0;
        }

        exist = set_get(link->ndisc_pref64, &(NDiscPREF64) { .prefix = a, .prefix_len = prefix_len });
        if (exist) {
                /* update existing entry */
                exist->router = router;
                exist->lifetime_usec = lifetime_usec;
