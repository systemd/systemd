...
static int ndisc_request_route(Route *route, Link *link, sd_ndisc_router *rt) {
        ...
        if (link->network->ipv6_accept_ra_quickack) {
                r = route_set_metric(route, RTAX_QUICKACK, link->network->ipv6_accept_ra_quickack);
                if (r < 0)
                        return r;
        }
        if (mtu > 0) {
                r = route_set_metric(route, RTAX_MTU, mtu);
                if (r < 0)
                        return r;
        }
        if (hop_limit > 0) {
                r = route_set_metric(route, RTAX_HOPLIMIT, hop_limit);
                if (r < 0)
                        return r;
        }
