        ...
        ORDERED_HASHMAP_FOREACH(static_lease, link->network->dhcp_static_leases_by_section) {
                ...
                * it removes any static lease with the specified mac address. */
                if (static_lease->address.s_addr == 0)
                        r = sd_dhcp_server_remove_static_lease(link->dhcp_server, static_lease->client_id, static_lease->client_id_size);
                else
                        r = sd_dhcp_server_add_static_lease(link->dhcp_server, &static_lease->address, static_lease->client_id, static_lease->client_id_size);
                if (r < 0)
