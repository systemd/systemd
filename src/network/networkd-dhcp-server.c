...
static int link_push_uplink_lpr_to_dhcp_server(Link *link, sd_dhcp_server *s) {
        _cleanup_free_ struct in_addr *addresses = NULL;
        size_t n_addresses = 0, n_allocated = 0;
        char **a;
        ...
        log_link_debug(link, "Copying LPR server information from link");

        STRV_FOREACH(a, link->network->lpr_servers) {
                union in_addr_union ia;
        ...
        if (link->dhcp_lease) {
                ...
                n = sd_dhcp_lease_get_lpr_servers(link->dhcp_lease, &da);
                if (n > 0) {
...
int dhcp4_server_configure(Link *link) {
        ...
        */
        ...
        if (link->network->n_dhcp_server_lpr > 0)
                r = sd_dhcp_server_set_lpr_servers(link->dhcp_server, link->network->dhcp_server_lpr, link->network->n_dhcp_server_lpr);
        else {
                if (!acquired_uplink)
                        uplink = manager_find_uplink(link->manager, link);

                if (!uplink) {
                        log_link_debug(link, "Not emitting LPR server information on link, couldn't find suitable uplink.");
                        r = 0;
                } else
                        r = link_push_uplink_lpr_to_dhcp_server(uplink, link->dhcp_server);
        }
        if (r < 0)
                log_link_warning_errno(link, r, "Failed to set LPR server for DHCP server, ignoring: %m");
...
int config_parse_dhcp_server_lpr_servers(
        ...
        for (;;) {
                ...
                if (r < 0) {
                        log_syntax(unit, LOG_ERR, filename, line, r,
                                   "Failed to parse LPR server address '%s', ignoring: %m", w);
                        continue;
                }

                m = reallocarray(n->dhcp_server_lpr, n->n_dhcp_server_lpr + 1, sizeof(struct in_addr));
                if (!m)
                        return log_oom();

                m[n->n_dhcp_server_lpr++] = a.in;
                n->dhcp_server_lpr = m;
        }
}
