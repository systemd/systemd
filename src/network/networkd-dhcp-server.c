...
static int link_push_uplink_log_to_dhcp_server(Link *link, sd_dhcp_server *s) {
        _cleanup_free_ struct in_addr *addresses = NULL;
        size_t n_addresses = 0, n_allocated = 0;
        char **a;
        ...
        log_link_debug(link, "Copying log server information from link");

        STRV_FOREACH(a, link->network->log_servers) {
                union in_addr_union ia;
        ...
        if (link->dhcp_lease) {
                ...
                n = sd_dhcp_lease_get_log_servers(link->dhcp_lease, &da);
                if (n > 0) {
...
int dhcp4_server_configure(Link *link) {
        ...
        */
        ...
        if (link->network->n_dhcp_server_log > 0)
                r = sd_dhcp_server_set_log_servers(link->dhcp_server, link->network->dhcp_server_log, link->network->n_dhcp_server_log);
        else {
                if (!acquired_uplink)
                        uplink = manager_find_uplink(link->manager, link);

                if (!uplink) {
                        log_link_debug(link, "Not emitting log server information on link, couldn't find suitable uplink.");
                        r = 0;
                } else
                        r = link_push_uplink_log_to_dhcp_server(uplink, link->dhcp_server);
        }
        if (r < 0)
                log_link_warning_errno(link, r, "Failed to set log server for DHCP server, ignoring: %m");
...
int config_parse_dhcp_server_log_servers(
                const char *unit,
                const char *filename,
                unsigned line,
                const char *section,
        ...
        for (;;) {
                ...
                if (r < 0) {
                        log_syntax(unit, LOG_ERR, filename, line, r,
                                   "Failed to parse LOG server address '%s', ignoring: %m", w);
                        continue;
                }

                m = reallocarray(n->dhcp_server_log, n->n_dhcp_server_log + 1, sizeof(struct in_addr));
                if (!m)
                        return log_oom();

                m[n->n_dhcp_server_log++] = a.in;
                n->dhcp_server_log = m;
        }
}
