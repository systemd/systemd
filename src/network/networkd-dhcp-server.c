...
static int dhcp4_server_configure(Link *link) {
        ...
        r = dhcp_server_set_extra_options(link->dhcp_server, link->network->dhcp_server_send_options);
        if (r < 0)
                return log_link_error_errno(link, r, "Failed to set DHCPv4 extra options: %m");

        r = dhcp_server_set_vendor_options(link->dhcp_server, link->network->dhcp_server_send_vendor_options);
        if (r < 0)
