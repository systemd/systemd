...
static int dhcp4_server_configure(Link *link) {
        ...
        if (link->network->dhcp_server_ipv6_only_preferred_usec > 0) {
                r = sd_dhcp_server_set_ipv6_only_preferred_sec(link->dhcp_server,
                                                               DIV_ROUND_UP(link->network->dhcp_server_ipv6_only_preferred_usec, USEC_PER_SEC));
                if (r < 0)
...
int config_parse_dhcp_server_ipv6_only_preferred(
        ...
        if (isempty(rvalue)) {
                network->dhcp_server_ipv6_only_preferred_usec = 0;
                return 0;
        }
        ...
        network->dhcp_server_ipv6_only_preferred_usec = usec;
        return 0;
}
