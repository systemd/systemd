...
static int dump_dhcp_leases(Table *table, const char *prefix, sd_bus *bus, const LinkInfo *link) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to get link description: %s", bus_error_message(&error, r));
        ...
        if (dhcp_server) {
                JSON_VARIANT_ARRAY_FOREACH(lease, json_variant_by_key(dhcp_server, "Leases")) {
                        ...
                        hostname = json_variant_by_key(lease, "Hostname");
                        ...
                        r = strv_extendf(
                                        ...
                                        id,
                                        hostname ? ", \"" : "",
                                        strempty(json_variant_string(hostname)),
                                        hostname ? "\"" : "",
                                        FORMAT_TIMESPAN(usec_sub_unsigned(expiration_usec, now(CLOCK_BOOTTIME)), USEC_PER_SEC));
                        if (r < 0)
                                return r;
                }
        }
