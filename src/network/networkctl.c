...
static int lldp_neighbors_varlink_reply(Varlink *link, JsonVariant *parameters, const char *error_id, VarlinkReplyFlags flags, void *userdata) {
        ...
        if (udata->table) {
                capabilities = lldp_capabilities_to_string(entry.capabilities, /* dots= */ true);
        ...
        if (udata->json) {
                ...
                capabilities = lldp_capabilities_to_string(entry.capabilities, /* dots= */ false);
...
static int link_lldp_status(int argc, char *argv[], void *userdata) {
        ...
        if (arg_json_format_flags == JSON_FORMAT_OFF) {
                ...
                r = json_variant_new_object(&udata.json, NULL, 0);
                if (r < 0)
                        return r;
        }
