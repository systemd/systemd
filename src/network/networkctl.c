...
static int verb_persistent_storage(int argc, char *argv[], void *userdata) {
        ...
        if (ready) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to check if the directory /var/lib/systemd/network/ is writable: %m");
                if (r > 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EROFS), "The directory /var/lib/systemd/network/ is not writable.");
        }
