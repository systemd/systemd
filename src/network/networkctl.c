...
static int dump_dhcp_leases(Table *table, const char *prefix, sd_bus *bus, const LinkInfo *link) {
        ...
        r = bus_call_method(bus, bus_network_mgr, "DescribeLink", &error, &reply, NULL, "i", index);
        if (r < 0)
                return log_error_errno(r, "Failed to get description: %s", bus_error_message(&error, r));
        ...
        if (dhcp_server) {
                JSON_VARIANT_ARRAY_FOREACH(lease, json_variant_by_key(dhcp_server, "Leases")) {
                        JsonVariant *json_hostname = NULL;
                        ...
                        json_hostname = json_variant_by_key(lease, "Hostname");
                        ...
                        r = strv_extendf(
                                        ...
                                        id,
                                        json_hostname ? ", \"" : "",
                                        strempty(json_variant_string(json_hostname)),
                                        json_hostname ? "\"" : "",
                                        FORMAT_TIMESPAN(usec_sub_unsigned(now(CLOCK_BOOTTIME), expiration_usec), USEC_PER_SEC));
                        if (r < 0)
                                return r;
                }
        }
