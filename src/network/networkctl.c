...
static int get_config_files_by_name(const char *name, char **ret_path, char ***ret_dropins) {
                        ...
                        if (access(p, F_OK) < 0) {
                                if (errno != ENOENT)
                                        log_debug_errno(errno,
                                                        "Failed to determine whether '%s' exists, ignoring: %m",
                                                        p);
                                continue;
                        }
        ...
        if (ret_dropins) {
                r = conf_files_list_strv(ret_dropins, ".conf", NULL, 0, (const char* const*) dropin_dirs);
                if (r < 0)
                        return r;
        }
...
static int get_network_files_by_link(
        ...
        if (r == -ENODATA)
                return log_error_errno(SYNTHETIC_ERRNO(ENOENT),
                                       "Link '%s' has no associated network file.",
                                       link);
        if (r < 0)
...
static int parse_link_config(
                const char *link_config,
                char **ret_ifname,
                char **ret_type) {

        _cleanup_strv_free_ char **split = NULL;
        _cleanup_free_ char *ifname = NULL, *type = NULL;
        size_t n;
        ...
        split = strv_split(link_config, ":");
        if (!split)
                return log_oom();

        n = strv_length(split);
        if (n == 0 || isempty(split[0]))
        ...
        ifname = strdup(split[0]);
        type = strdup(n == 2 ? split[1] : "network");
        if (!ifname || !type)
                return log_oom();
...
static int add_config_to_edit(
        ...
        if (r > 0 && !path_startswith(old_dropin, "/usr"))
                ...
                dropin_path = path_join(dropin_dir, arg_drop_in);
                if (!dropin_path)
                        return log_oom();
        }
...
static int verb_edit(int argc, char *argv[], void *userdata) {
        ...
        _cleanup_(sd_netlink_unrefp) sd_netlink *rtnl = NULL;
        bool networkd_reload = false, udevd_trigger = false;
        int r, has_networkd = -1;
        ...
        STRV_FOREACH(name, strv_skip(argv, 1)) {
                ...
                if (link_config) {
                        ...
                        if (streq(type, "network")) {
                                if (has_networkd < 0)
                                        has_networkd = networkd_is_running();
                                if (!has_networkd)
                                        return log_error_errno(SYNTHETIC_ERRNO(ESRCH),
                                                               "Cannot edit network file for link if systemd-networkd is not running.");
        ...
        if (udevd_trigger) {
                ...
                if (r > 0) {
                        ...
                        if (r < 0)
                                return r;
                        if (r == 0) { /* Child */
                                execlp("udevadm", "udevadm", "trigger", "--subsystem-match=net", "--action=add",
                                       "--settle", NULL);
        ...
        STRV_FOREACH(name, strv_skip(argv, 1)) {
                ...
                if (link_config) {
                        ...
                        if (streq(type, "network")) {
                                ...
                                if (!has_networkd)
                                        return log_error_errno(SYNTHETIC_ERRNO(ESRCH),
                                                               "Cannot cat network file for link if systemd-networkd is not running.");
...
static int help(void) {
        ...
        printf("%s [OPTIONS...] COMMAND\n\n"
               ...
               "     --no-reload         Do not reload systemd-networkd or trigger systemd-udevd\n"
