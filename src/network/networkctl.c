...
static int get_config_files_by_name(const char *name, char **ret_path, char ***ret_dropins) {
        ...
        if (!ENDSWITH_SET(name, ".network", ".netdev", ".link"))
                return -EINVAL;
...
static int verb_edit(int argc, char *argv[], void *userdata) {
        ...
        _cleanup_free_ char *drop_in_alloc = NULL;
        const char *drop_in;
        int r;
        ...
        STRV_FOREACH(name, strv_skip(argv, 1)) {
                ...
                if (drop_in) {
                        ...
                        _cleanup_free_ char *old = NULL, *dropin_path = NULL;

                        r = get_dropin_by_name(drop_in, (const char**) dropins, &old);
                        if (r < 0)
                                return log_error_errno(r, "Failed to acquire drop-in '%s': %m", drop_in);
                        if (r > 0 && !path_startswith(old, "/usr"))
                                /* An existing drop-in is found and not in /usr/. Let's edit it directly. */
                                dropin_path = TAKE_PTR(old);
                        else {
                                /* No drop-in was found or an existing drop-inresides in /usr/. Let's
                        ...
                        if (old) {
                                STRV_FOREACH(i, dropins) {
                                        if (path_equal_filename(*i, old))
                                                continue;

                                        r = strv_extend(&comment_paths, *i);
                                        if (r < 0)
                                                return log_oom();
                                }

                                assert(strv_length(comment_paths) == strv_length(dropins) - 1);
                        } else {
                        ...
                        r = edit_files_add(&context, dropin_path, old, comment_paths);
                } else if (new_path)
...
static int verb_cat(int argc, char *argv[], void *userdata) {
        ...
        STRV_FOREACH(name, strv_skip(argv, 1)) {
                ...
                q = get_config_files_by_name(*name, &path, &dropins);
                if (q == -ENOENT) {
                        log_error_errno(q, "Cannot found network config file '%s'.", *name);
                        r = r < 0 ? r : q;
                        continue;
                }
                if (q < 0) {
                        log_error_errno(q, "Failed to get the path of network config '%s': %m", *name);
                        return r < 0 ? r : q;
                }

                q = cat_files(path, dropins, /* flags = */ 0);
                if (q < 0)
                        return r < 0 ? r : q;
        }
...
static int help(void) {
        ...
        printf("%s [OPTIONS...] COMMAND\n\n"
               ...
               "     --no-reload         Do not reload systemd-networkd after editing config\n"
