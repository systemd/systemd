...
int route_section_verify_nexthops(Route *route) {
        ...
        /* For device-only nexthops (no gateway), set the family to the route's family. */
        RouteNextHop *nhop;
        ORDERED_SET_FOREACH(nhop, route->nexthops)
                if (nhop->family == AF_UNSPEC)
                        nhop->family = route->family;
