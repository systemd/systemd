...
#define VXLAN_VNI_MAX 16777215U
...
void network_drop_invalid_bridge_vlan_tunnel_entries(Network *network) {
        ...
        HASHMAP_FOREACH(tunnel, network->bridge_vlan_tunnel_entries_by_section) {
                ...
                if (!vlanid_is_valid(tunnel->vlan_id)) {
                        log_warning("%s: [BridgeVLANTunnel] section without valid VLAN=, ignoring.",
                                    tunnel->section->filename);
                        bridge_vlan_tunnel_free(tunnel);
                        continue;
                }

                if (tunnel->tunnel_id == UINT32_MAX) {
                        log_warning("%s: [BridgeVLANTunnel] section without TunnelID=, ignoring.",
                                    tunnel->section->filename);
                        bridge_vlan_tunnel_free(tunnel);
                        continue;
                }
                ...
                if (tunnel->vlan_id_end != UINT16_MAX && tunnel->vlan_id != tunnel->vlan_id_end) {
                        uint16_t vlan_span = tunnel->vlan_id_end - tunnel->vlan_id;
                        uint32_t tunnel_span = tunnel->tunnel_id_end - tunnel->tunnel_id;

                        if (tunnel->tunnel_id_end == UINT32_MAX || vlan_span != tunnel_span) {
                                log_warning("%s: [BridgeVLANTunnel] VLAN= and TunnelID= ranges must have the same span, ignoring.",
                                            tunnel->section->filename);
                                bridge_vlan_tunnel_free(tunnel);
                                continue;
                        }
