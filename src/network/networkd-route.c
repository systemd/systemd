...
int route_table_from_string_full(Network *network, const char *s, uint32_t *ret) {
        int r;
        ...
        if (r >= 0)
                *ret = (uint32_t) r;
                ...
                if (!hashmap_isempty(network->manager->route_tables)) {
                        uint32_t t = PTR_TO_UINT32(hashmap_get(network->manager->route_tables, s));
                        if (t)
                                *ret = t;
                } else {
                        r = safe_atou32(s, ret);
...
int config_parse_route_table(
        ...
        r = route_table_from_string_full(network, rvalue, &n->table);
        if (r < 0) {
                log_syntax(unit, LOG_WARNING, filename, line, r,
...
int config_parse_route_table_names(
        ...
        if (r < 0)
                return log_syntax(unit, LOG_WARNING, filename, line, r,
                                  "Specified RouteTable= name and value pair is duplicated, ignoring assignment: %s", rvalue);
