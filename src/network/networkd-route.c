...
void route_tables_free(Manager *m) {
        ...
        /* route table names may be shared by the two hashmaps. So, let's first clear all entries in
         * route_table_names_by_number, and then free the two hashmaps. */

        while ((name = hashmap_steal_first(m->route_table_names_by_number))) {
...
int config_parse_route_table_names(
        ...
        for (const char *p = rvalue;;) {
                ...
                *num++ = '\0';
                ...
                k = hashmap_ensure_put(&m->route_table_names_by_number, NULL, UINT32_TO_PTR(table), name);
                if (k == -ENOMEM)
                        return log_oom();
                if (k == -EEXIST) {
                        log_syntax(unit, LOG_WARNING, filename, line, r,
