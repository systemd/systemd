        ...
        case AF_INET6:
                if (existing->protocol != requesting->protocol) {
                        if (DEBUG_LOGGING) {
                                ...
                                log_debug("existing protocol: %s, requesting protocol: %s", ex, req);
                        }

                        return false;
                }
                if (existing->type != requesting->type) {
                        log_debug("existing type: %s, requesting type: %s",
                ...
                if (existing->flags != requesting->flags) {
                        if (DEBUG_LOGGING) {
                                ...
                                log_debug("existing flags: %s, requesting flags: %s", ex, req);
                        }

                        return false;
                }
                if (!in6_addr_equal(&existing->prefsrc.in6, &requesting->prefsrc.in6)) {
                        log_route_debug(existing, "Cannot update route, as the preferred source is different",
                                        existing->manager);
                        return false;
                }
                ...
                if (existing->expiration_managed_by_kernel && requesting->lifetime_usec == USEC_INFINITY) {
                        log_route_debug(existing, "Cannot update route, as the expiration is managed by the kernel and lifetime is infinite",
                                        existing->manager);
                ...
                if (!route_metric_can_update(&existing->metric, &requesting->metric, existing->expiration_managed_by_kernel)) {
                        log_route_debug(existing, "Cannot update route, as the metrics are different",
                                        existing->manager);
                        return false;
                }
                if (existing->nexthop.weight != requesting->nexthop.weight) {
