...
static void log_route_debug(const Route *route, const char *str, const Link *link, const Manager *manager) {
        ...
        if (in_addr_is_set(route->gw_family, &route->gw)) {
                ...
                ORDERED_SET_FOREACH(m, route->multipath_routes) {
                        ...
                        if (m->ifname)
                                (void) strextend_with_separator(&gw_alloc, "@", m->ifname);
                        else if (m->ifindex > 0)
                                (void) strextendf_with_separator(&gw_alloc, "@", "%"PRIu32, m->ifindex);
                        /* See comments in config_parse_multipath_route(). */
                        (void) strextendf_with_separator(&gw_alloc, ":", "%"PRIu32, m->weight + 1);
                }
                gw = gw_alloc;
        }
