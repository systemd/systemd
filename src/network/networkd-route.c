...
int config_parse_route_nexthop_id(
                const char *unit,
                const char *filename,
                unsigned line,
                const char *section,
...
static int route_section_verify(Route *route, Network *network) {
        ...
        if (route->nexthop_id > 0) {
                ...
                if (!found)
                        return log_warning_errno(SYNTHETIC_ERRNO(EINVAL),
                                                 "%s: NextHopId=%"PRIu32" is specified, but no corresponding "
                                                 "[NextHop] section with Id=%"PRIu32" is found. "
                                                 "Ignoring [Route] section from line %u.",
                                                 route->section->filename,
                                                 route->nexthop_id, route->nexthop_id,
                                                 route->section->line);

                if (route->family == AF_INET6 && nh->family == AF_INET)
                        return log_warning_errno(SYNTHETIC_ERRNO(EINVAL),
                                                 "%s: IPv4 nexthop is specified for IPv6 route. "
                                                 "Ignoring [Route] section from line %u.",
                                                 route->section->filename, route->section->line);
