...
static int route_convert(Manager *manager, const Route *route, Route ***ret) {
        ...
        if (route->nexthop_id > 0) {
                ...
                if (hashmap_isempty(nh->group)) {
                        ...
                        r = route_dup(route, &routes[0]);
                        if (r < 0)
                                return r;

                        route_apply_nexthop(routes[0], nh, UINT8_MAX);

                        *ret = TAKE_PTR(routes);
                        return 1;
                }

                p = routes = new(Route*, hashmap_size(nh->group));
                if (!routes)
                        return -ENOMEM;
                ...
                HASHMAP_FOREACH(nhg, nh->group) {
                        ...
                        route_apply_nexthop(*p, h, nhg->weight);

                        p++;
                }

                *ret = TAKE_PTR(routes);
                return hashmap_size(nh->group);

        } else if (!ordered_set_isempty(route->multipath_routes)) {
                MultipathRoute *m;

                p = routes = new(Route*, ordered_set_size(route->multipath_routes));
                if (!routes)
                        return -ENOMEM;
                ...
                ORDERED_SET_FOREACH(m, route->multipath_routes) {
                        ...
                        route_apply_multipath_route(*p, m);

                        p++;
                }

                *ret = TAKE_PTR(routes);
                return ordered_set_size(route->multipath_routes);
...
static int manager_drop_routes(Manager *manager, bool foreign, const Link *except) {
        ...
        HASHMAP_FOREACH(link, manager->links_by_index) {
                ...
                HASHMAP_FOREACH(route, link->network->routes_by_section) {
                        _cleanup_free_ Route **converted = NULL;
                        Route *existing;
                        int n;

                        n = route_convert(manager, route, &converted);
                        if (n < 0)
                                continue;
                        if (n == 0) {
                        ...
                        for (int i = 0; i < n; i++) {
                                if (route_get(manager, NULL, converted[i], &existing) >= 0)
                                        route_unmark(existing);
                                route_free(converted[i]);
                        }
                }
        }
...
int link_drop_foreign_routes(Link *link) {
        ...
        HASHMAP_FOREACH(route, link->network->routes_by_section) {
                ...
                n = route_convert(link->manager, route, &converted);
                if (n < 0)
                        continue;
                if (n == 0) {
                ...
                for (int i = 0; i < n; i++) {
                        if (route_get(NULL, link, converted[i], &existing) >= 0)
                                route_unmark(existing);
                        route_free(converted[i]);
                }
        }
...
static int link_request_static_route(Link *link, Route *route) {
        ...
        log_route_debug(route, "Requesting", link, link->manager);
        r = link_queue_request(link, REQUEST_TYPE_ROUTE, route, false,
        ...
        for (int i = 0; i < n; i++) {
                ...
                if (route_get(link->manager, link, converted[i], &existing) < 0) {
                        r = route_add(link->manager, link, converted[i]);
                        if (r < 0)
                                goto finalize;

                        existing = TAKE_PTR(converted[i]);
                } else {
                        existing->source = converted[i]->source;
                        existing->provider = converted[i]->provider;
                }
...
static int route_is_ready_to_configure(const Route *route, Link *link) {
        ...
        if (route->nexthop_id > 0) {
                ...
                n = route_convert(link->manager, route, &converted);
                if (n == -ENOENT)
                        return false; /* nexthop is not configured yet. */
                if (n < 0)
                        return n;
...
int manager_rtnl_process_route(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        if (r < 0) {
                log_link_warning_errno(link, r, "Failed to convert received route, ignoring: %m");
                return 0;
        }
        if (r == 0)
                process_route_one(m, link, type, TAKE_PTR(tmp));
        else {
                for (int i = 0; i < r; i++)
                        process_route_one(m, link, type, TAKE_PTR(converted[i]));
                free(converted);
        }
