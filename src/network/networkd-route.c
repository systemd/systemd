...
static void route_hash_func(const Route *route, struct siphash *state) {
        ...
        case AF_INET:
                ...
                /* metrics */
                route_metric_hash_func(route, state);
...
static int route_compare_func(const Route *a, const Route *b) {
        ...
        case AF_INET:
                ...
                r = route_metric_compare_func(a, b);
                if (r != 0)
                        return r;
...
static int route_set_netlink_message(const Route *route, sd_netlink_message *m) {
        ...
        /* metric */
        r = route_metric_set_netlink_message(route, m);
        if (r < 0)
                return r;
...
int manager_rtnl_process_route(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        /* metrics */
        if (route_metric_read_netlink_message(tmp, message) < 0)
                return 0;
