...
static int route_setup_timer(Route *route, const struct rta_cacheinfo *cacheinfo) {
        ...
        if (cacheinfo && cacheinfo->rta_expires != 0)
                /* Assume that non-zero rta_expires means kernel handle the route expiration. */
                return 0;

        r = sd_event_add_time(manager->event, &s, clock_boottime_or_monotonic(),
                              route->lifetime, 0, route_expire_handler, route);
        if (r < 0)
                return r;

        sd_event_source_disable_unref(route->expire);
        route->expire = TAKE_PTR(s);
        return 1;
}
...
static int process_route_one(Manager *manager, Link *link, uint16_t type, Route *in, const struct rta_cacheinfo *cacheinfo) {
...
int manager_rtnl_process_route(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        for (size_t i = 0; i < converted->n; i++)
                (void) process_route_one(m, converted->links[i] ?: link, type, TAKE_PTR(converted->routes[i]), has_cacheinfo ? &cacheinfo : NULL);
