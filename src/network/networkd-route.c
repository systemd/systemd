...
bool route_can_update(const Route *existing, const Route *requesting) {
        ...
        case AF_INET6:
                ...
                if (existing->expiration_managed_by_kernel && requesting->lifetime_usec == USEC_INFINITY) {
                        log_route_debug(existing, "Cannot update route, as the expiration is managed by the kernel and requested lifetime is infinite",
                                        existing->manager);
                ...
                if (!route_metric_can_update(&existing->metric, &requesting->metric, existing->expiration_managed_by_kernel)) {
                        log_route_debug(existing, "Cannot update route, as the metrics are different or the expiration is managed by the kernel",
                                        existing->manager);
                        return false;
                }
                if (existing->nexthop.weight != requesting->nexthop.weight) {
