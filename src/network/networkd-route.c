...
static ConvertedRoutes *converted_routes_free(ConvertedRoutes *routes) {
        if (!routes)
                return NULL;

        for (size_t i = 0; i < routes->n; i++)
                route_free(routes->routes[i]);
...
static int converted_routes_new(size_t n, ConvertedRoutes **ret) {
        _cleanup_(converted_routes_freep) ConvertedRoutes *converted_routes = NULL;
        ...
        converted_routes = new(ConvertedRoutes, 1);
        if (!converted_routes)
                return -ENOMEM;

        *converted_routes = (ConvertedRoutes) {
                .n = n,
                .routes = TAKE_PTR(routes),
                .links = TAKE_PTR(links),
        };

        *ret = TAKE_PTR(converted_routes);
        return 0;
}

static int route_convert(Manager *manager, const Route *route, ConvertedRoutes **ret) {
        _cleanup_(converted_routes_freep) ConvertedRoutes *routes = NULL;
        int r;
        ...
        if (route->nexthop_id > 0) {
                ...
                if (hashmap_isempty(nh->group)) {
                        r = converted_routes_new(1, &routes);
                        if (r < 0)
                                return r;

                        r = route_dup(route, &routes->routes[0]);
                        if (r < 0)
                                return r;

                        route_apply_nexthop(routes->routes[0], nh, UINT8_MAX);
                        routes->links[0] = nh->link;

                        *ret = TAKE_PTR(routes);
                        return 1;
                }

                r = converted_routes_new(hashmap_size(nh->group), &routes);
                if (r < 0)
                        return r;
                ...
                HASHMAP_FOREACH(nhg, nh->group) {
                        ...
                        r = route_dup(route, &routes->routes[i]);
                        if (r < 0)
                                return r;

                        route_apply_nexthop(routes->routes[i], h, nhg->weight);
                        routes->links[i] = h->link;
                ...
                *ret = TAKE_PTR(routes);
                ...
                r = converted_routes_new(ordered_set_size(route->multipath_routes), &routes);
                if (r < 0)
                        return r;
                ...
                ORDERED_SET_FOREACH(m, route->multipath_routes) {
                        ...
                        route_apply_multipath_route(routes->routes[i], m);

                        r = multipath_route_get_link(manager, m, &routes->links[i]);
                        if (r < 0)
                                return r;
...
int link_drop_foreign_routes(Link *link) {
        ...
        SET_FOREACH(route, link->routes) {
                ...
                /* TODO: Why the kernel adds this route with protocol RTPROT_BOOT??? We need to investigate that.
...
static int route_setup_timer(Route *route) {
        ...
        return sd_event_add_time(route->link->manager->event, &route->expire, clock_boottime_or_monotonic(),
...
static int route_configure(
        ...
        r = route_convert(link->manager, route, &converted);
        if (r < 0)
                return log_link_error_errno(link, r, "Could not convert route: %m");
        if (r == 0)
                if (route_get(NULL, link, route, &existing) >= 0) {
                        r = route_setup_timer(existing);
                        if (r < 0)
                                return log_link_error_errno(link, r, "Could not setup timer: %m");
                }
...
void route_cancel_request(Route *route) {
        ...
        route_update_state(route, 0, NETWORK_CONFIG_STATE_REQUESTING);
}
...
int link_request_route(
        ...
        r = link_queue_request(link, REQUEST_TYPE_ROUTE, route, consume_object,
...
static int link_request_static_route(Link *link, Route *route) {
        ...
        if (route->nexthop_id == 0 && ordered_set_isempty(route->multipath_routes))
...
static int route_is_ready_to_configure(const Route *route, Link *link) {
        ...
        if (converted)
                for (size_t i = 0; i < converted->n; i++) {
                        ...
                        if (route_get(link->manager, converted->links[i] ?: link, converted->routes[i], &existing) < 0) {
                                r = route_add(link->manager, converted->links[i] ?: link, converted->routes[i]);
                                if (r < 0)
                                        return r;

                                existing = TAKE_PTR(converted->routes[i]);
                        } else {
...
int manager_rtnl_process_route(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        if (r == -ENOENT) {
                log_link_warning_errno(link, r, "rtnl: received route with unknown nexthops or multipath routes on unknown links, ignoring: %m");
                return 0;
        }
        if (r < 0) {
        ...
        for (size_t i = 0; i < converted->n; i++)
                process_route_one(m, converted->links[i] ?: link, type, TAKE_PTR(converted->routes[i]));
