...
static int wireguard_read_default_key_cred(Wireguard *w, const char *filename) {
        _cleanup_(erase_and_freep) char *cred = NULL;
        _cleanup_(erase_and_freep) void *key = NULL;
        size_t len;
        const char *p;
        ...
        r = read_credential(strjoina("network.wireguard.private.", p), (void**) &cred, /* ret_size = */ NULL);
        if (r < 0)
                return log_netdev_error_errno(netdev, r,
...
static int wireguard_verify(NetDev *netdev, const char *filename) {
        ...
        if (eqzero(w->private_key)) {
                r = wireguard_read_default_key_cred(w, filename);
                if (r < 0)
                        return r;
        }
