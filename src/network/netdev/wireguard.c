...
static int wireguard_verify(NetDev *netdev, const char *filename) {
        ...
        if (eqzero(w->private_key)) {
                ...
                p = endswith(filename, ".netdev");
                if (!p)
                        /* Fuzzer run? Then we just bail out. */
                        return log_netdev_error_errno(netdev, SYNTHETIC_ERRNO(EINVAL),
                                                      "%s: Invalid netdev config name, ignoring.",
                                                      filename);

                p = strndupa_safe(filename, p - filename);

                r = read_credential(strjoina("network.wireguard.private.", p), (void**) &cred, &cred_len);
                if (r < 0)
                        return log_netdev_error_errno(netdev, r,
                ...
                if (cred_len != WG_KEY_LEN)
                        return log_netdev_error_errno(netdev, r,
                                                      "%s: No private key specified and default key is invalid. Ignoring network device.",
                                                      filename);

                memcpy(w->private_key, cred, WG_KEY_LEN);
        }
