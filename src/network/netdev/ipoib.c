...
assert_cc((int) IPOIB_CONNECT_MODE_DATAGRAM  == (int) IPOIB_MODE_DATAGRAM);
assert_cc((int) IPOIB_CONNECT_MODE_CONNECTED == (int) IPOIB_MODE_CONNECTED);
...
static void netdev_ipoib_init(NetDev *netdev) {
        ...
        ipoib->mode = _IPOIB_CONNECT_MODE_INVALID;
        ipoib->umcast = -1;
}
...
static const char * const ipoib_mode_table[_IPOIB_CONNECT_MODE_MAX] = {
        [IPOIB_CONNECT_MODE_DATAGRAM]  = "datagram",
        [IPOIB_CONNECT_MODE_CONNECTED] = "connected",
};

DEFINE_PRIVATE_STRING_TABLE_LOOKUP_FROM_STRING(ipoib_mode, IPoIBConnectMode);
DEFINE_CONFIG_PARSE_ENUM(config_parse_ipoib_mode, ipoib_mode, IPoIBConnectMode, "Failed to parse IPoIB connection mode");
