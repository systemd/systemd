...
static int netdev_create_tuntap(NetDev *netdev) {
        ...
        if (t->keep_fd) {
                ...
                state = strjoina("FDSTORE=1\nFDNAME=tuntap-", netdev->ifname);
                r = sd_pid_notify_with_fds(0, /* unset_environment = */ false, state, &t->fd, 1);
                if (r < 0)
                        log_netdev_warning_errno(netdev, r, "Failed to send file descriptor to service manager, ignoring: %m");
        }
