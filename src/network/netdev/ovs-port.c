...
bool ovs_vlan_mode_is_valid(const char *v) {
        return STR_IN_SET(v, "trunk", "access", "native-tagged", "native-untagged", "dot1q-tunnel");
}
...
static int ovs_port_verify(NetDev *netdev, const char *filename) {
        ...
        if (p->vlan_mode && !ovs_vlan_mode_is_valid(p->vlan_mode))
                return log_netdev_warning_errno(netdev, SYNTHETIC_ERRNO(EINVAL),
                                                "%s: invalid VLANMode='%s'. Ignoring.",
                                                filename, p->vlan_mode);

        /* Strip bond-only settings on non-bond ports first, so that subsequent
         * validation only fires for settings that actually apply. Without this
         * ordering 'Type=internal LACP=typo' would be rejected while
         * 'Type=internal LACP=active' would be silently dropped — inconsistent. */
