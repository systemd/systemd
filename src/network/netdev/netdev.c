...
int request_process_create_stacked_netdev(Request *req) {
        ...
        r = netdev_create(req->netdev, req->link, req->netlink_handler);
        if (r < 0)
...
static int link_create_stacked_netdev_handler_internal(sd_netlink *rtnl, sd_netlink_message *m, Link *link, unsigned *counter, bool *flag) {
        int r;
...
static int link_create_stacked_netdev_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return link_create_stacked_netdev_handler_internal(
                        rtnl, m, link,
                        &link->create_stacked_netdev_messages,
                        &link->stacked_netdevs_created);
}
...
static int link_create_stacked_netdev_after_configured_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        return link_create_stacked_netdev_handler_internal(
                        ...
                        &link->stacked_netdevs_after_configured_created);
}
