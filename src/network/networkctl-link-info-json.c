...
static int acquire_link_dhcp_lease(LinkInfo *link) {
        int r;
        ...
        r = dhcp_message_parse_json(v, &message);
        if (r < 0)
                return r;

        return dhcp_lease_new_from_message(message, &link->dhcp_lease);
}
...
int link_info_parse_description(LinkInfo *link, sd_varlink *vl) {
        ...
        (void) acquire_link_dhcp_lease(link);
