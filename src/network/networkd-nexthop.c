...
static int nexthop_get_request(Manager *manager, uint32_t id, Request **ret) {
        Request *req;
...
static int link_find_nexthop_request(Link *link, const NextHop *in, Request **ret) {
        Request *req;
        int ifindex;
        ...
        if (in->id > 0)
                return nexthop_get_request(link->manager, in->id, ret);
...
int nexthop_get(Manager *manager, uint32_t id, NextHop **ret) {
        ...
        nexthop = set_get(manager->nexthops, &(const NextHop) { .id = id });
        if (!nexthop)
                return -ENOENT;
...
static int link_find_nexthop(Link *link, const NextHop *in, NextHop **ret) {
        NextHop *nexthop;
        int ifindex;
        ...
        if (in->id > 0)
                return nexthop_get(link->manager, in->id, ret);
...
static int nexthop_acquire_id(Manager *manager, NextHop *nexthop) {
        ...
        for (uint32_t id = 1; id < UINT32_MAX; id++) {
                if (nexthop_get(manager, id, NULL) >= 0)
                        continue;
                if (nexthop_get_request(manager, id, NULL) >= 0)
                        continue;
...
static int nexthop_remove(NextHop *nexthop) {
        ...
        if (nexthop_get_request(manager, nexthop->id, &req) >= 0)
...
static bool nexthop_is_ready_to_configure(Link *link, const NextHop *nexthop) {
        ...
        if (nexthop->ifindex > 0) {
                assert(link->ifindex == nexthop->ifindex);
        ...
        HASHMAP_FOREACH(nhg, nexthop->group) {
                ...
                if (nexthop_get(link->manager, nhg->id, &g) < 0)
                        return false;
...
static int nexthop_process_request(Request *req, Link *link, NextHop *nexthop) {
        ...
        if (nexthop_get(link->manager, nexthop->id, &existing) >= 0)
...
static int link_request_nexthop(Link *link, const NextHop *nexthop) {
        ...
        if (link_find_nexthop_request(link, nexthop, NULL) >= 0)
        ...
        if (link_find_nexthop(link, nexthop, &existing) < 0) {
...
static void link_mark_nexthops(Link *link, bool foreign) {
        ...
        HASHMAP_FOREACH(other, link->manager->links_by_index) {
                ...
                HASHMAP_FOREACH(nexthop, other->network->nexthops_by_section) {
                        ...
                        if (link_find_nexthop(other, nexthop, &existing) >= 0)
...
int manager_rtnl_process_nexthop(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        (void) nexthop_get(m, id, &nexthop);
        (void) nexthop_get_request(m, id, &req);
