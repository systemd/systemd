...
static int manager_add_nexthop_by_id(Manager *manager, NextHop *nexthop) {
        assert(manager);
        assert(nexthop);
...
static int nexthop_add(Manager *manager, Link *link, NextHop *nexthop) {
        ...
        return manager_add_nexthop_by_id(manager ?: link->manager, nexthop);
}
...
static int nexthop_update_id(NextHop *nexthop, uint32_t id) {
        ...
        if (nexthop->id > 0)
                return manager_add_nexthop_by_id(manager, nexthop);
...
static int link_request_nexthop(
        ...
        if (nexthop_get(link->manager, link, nexthop, &existing) < 0) {
                ...
                r = nexthop_update_id(existing, nexthop->id);
                if (r < 0)
                        return r;
