...
static void log_nexthop_debug(const NextHop *nexthop, uint32_t id, const char *str, const Link *link) {
        ...
        HASHMAP_FOREACH(nhg, nexthop->group)
                (void) strextendf(&group, "%s%"PRIu32",%"PRIu32, group ? "/" : "", nhg->id, nhg->weight+1);
...
int manager_rtnl_process_nexthop(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        if (r < 0 && r != -ENODATA) {
                ...
                if (raw_group_size == 0 || raw_group_size % sizeof(struct nexthop_grp)) {
                        log_link_warning(link, "rtnl: received nexthop message with invalid nexthop group, ignoring: %m");
                        return 0;
                }
                ...
                for (size_t i = 0; i < n_group; i++) {
                        ...
                        if (group[i].id == 0) {
                                log_link_warning(link, "rtnl: received nexthop message with invalid ID in group, ignoring: %m");
                                return 0;
                        }
                        if (group[i].weight > 254) {
                                log_link_warning(link, "rtnl: received nexthop message with invalid weight in group, ignoring: %m");
                                return 0;
                        }
...
int config_parse_nexthop_group(
        ...
        for (const char *p = rvalue;;) {
                ...
                if (r < 0) {
                        log_syntax(unit, LOG_WARNING, filename, line, r,
                                   "Failed to store nexthop group, ignoring assignment: %s%s%s",
                                   word, sep ? "," : "", strempty(sep));
                        continue;
                }
                assert(r > 0);
                TAKE_PTR(nhg);
