...
static int nexthop_update_id(NextHop *nexthop, uint32_t id) {
        ...
        nexthops = nexthop->link ? nexthop->link->nexthops : nexthop->manager->nexthops;
        ...
        if (r <= 0) {
                ...
                nexthop->id = 0;
                assert_se(set_put(nexthops, nexthop) > 0);
                return r == 0 ? -EEXIST : r;
        }
...
static void log_nexthop_debug(const NextHop *nexthop, const char *str, const Link *link) {
        ...
        log_link_debug(link, "%s nexthop: id: %"PRIu32", gw: %s, blackhole: %s, group: %s",
                       str, nexthop->id, strna(gw), yes_no(nexthop->blackhole), strna(group));
}
...
static int nexthop_remove(NextHop *nexthop) {
        ...
        nexthop_update_state(nexthop, NETWORK_CONFIG_STATE_MARKED, NETWORK_CONFIG_STATE_REMOVING);
        return 0;
}
...
static int nexthop_configure(
        ...
        nexthop_update_state(nexthop, 0, NETWORK_CONFIG_STATE_CONFIGURING);
        return 0;
}
...
static int link_request_nexthop(
        ...
        nexthop_update_state(existing, 0, NETWORK_CONFIG_STATE_REQUESTING);
        return 1;
}
...
static int manager_drop_nexthops(Manager *manager, bool foreign, const Link *except) {
        ...
        SET_FOREACH(nexthop, manager->nexthops) {
                ...
                if (!nexthop_should_removed(nexthop))
                        continue;

                nexthop_update_state(nexthop, 0, NETWORK_CONFIG_STATE_MARKED);
        }
        ...
        HASHMAP_FOREACH(link, manager->links_by_index) {
                ...
                HASHMAP_FOREACH(nexthop, link->network->nexthops_by_section) {
                        ...
                        if (nexthop_get(manager, NULL, nexthop, &existing) >= 0)
                                nexthop_update_state(existing, NETWORK_CONFIG_STATE_MARKED, 0);
                }
        }

        /* Finally, remove all marked rules. */
        SET_FOREACH(nexthop, manager->nexthops) {
                if (!FLAGS_SET(nexthop->state, NETWORK_CONFIG_STATE_MARKED))
                        continue;
...
int request_process_nexthop(Request *req) {
        ...
        nexthop_update_state(req->nexthop, NETWORK_CONFIG_STATE_REQUESTING, 0);
...
int manager_rtnl_process_nexthop(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        if (nexthop_get(m, link, tmp, &nexthop) < 0) {
                uint32_t id;
        ...
        case RTM_NEWNEXTHOP:
                if (nexthop) {
                        nexthop_update_state(nexthop, NETWORK_CONFIG_STATE_CONFIGURING, NETWORK_CONFIG_STATE_CONFIGURED);
                        log_nexthop_debug(tmp, "Received remembered", link);

                        r = nexthop_update_id(nexthop, tmp->id);
                        if (r < 0)
                                log_link_warning_errno(link, r, "Could not update nexthop ID, ignoring: %m");
                } else {
                        tmp->source = NETWORK_CONFIG_SOURCE_FOREIGN;
                        nexthop_update_state(tmp, 0, NETWORK_CONFIG_STATE_CONFIGURED);
                        ...
                        r = nexthop_update_id(tmp, tmp->id);
                        if (r < 0)
                                log_link_warning_errno(link, r, "Could not update nexthop ID, ignoring: %m");
        ...
        case RTM_DELNEXTHOP:
                if (nexthop) {
                        nexthop_update_state(nexthop, NETWORK_CONFIG_STATE_CONFIGURED | NETWORK_CONFIG_STATE_REMOVING, 0);
                        if (nexthop->state == 0) {
