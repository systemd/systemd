...
int link_lldp_tx_configure(Link *link) {
        ...
        r = sd_lldp_tx_set_system_capabilities(link->lldp_tx,
        ...
        r = sd_lldp_tx_set_enabled_capabilities(link->lldp_tx,
        ...
        r = sd_lldp_tx_set_mud_url(link->lldp_tx, link->network->lldp_mud);
        if (r < 0)
                return r;
