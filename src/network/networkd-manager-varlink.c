...
static int vl_method_set_link(sd_varlink *vlink, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        r = sd_json_variant_get_boolean(sd_json_variant_by_key(parameters, "Up"), &up);
        if (r < 0)
                return sd_varlink_error_invalid_parameter(vlink, "Up");

        link = manager_get_link_by_index_or_name(manager, info.ifindex, info.ifname);
