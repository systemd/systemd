...
static int link_set_dhcp_routes(Link *link) {
        ...
        for (i = 0; i < n; i++) {
                ...
                route->family = AF_INET;
                route->protocol = RTPROT_DHCP;
                sd_dhcp_route_get_gateway(static_routes[i], &route->gw.in);
                sd_dhcp_route_get_destination(static_routes[i], &route->dst.in);
                sd_dhcp_route_get_destination_prefix_length(static_routes[i], &route->dst_prefixlen);
                route->priority = link->network->dhcp_route_metric;
        ...
        if (link->network->dhcp_routes) {
                ...
                if (n >= 0) {
                        for (i = 0; i < n; i++) {
                                ...
                                if (r >= 0) {
                                        route->family = AF_INET;
                                        sd_dhcp_route_get_gateway(routes[i], &route->gw.in);
                                        sd_dhcp_route_get_destination(routes[i], &route->dst.in);
                                        sd_dhcp_route_get_destination_prefix_length(routes[i], &route->dst_prefixlen);
