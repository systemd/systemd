...
static int dhcp4_handler(sd_dhcp_client *client, int event, void *userdata) {
        ...
        switch (event) {
                ...
                case SD_DHCP_CLIENT_EVENT_SELECTING:
                        ...
                        if (link->network->dhcp_allow_ipv6_only_preferred && dhcp_offer_contains_ipv6_only_preferred_option(link, client))
                                return -ENOMSG;
...
static int dhcp4_configure(Link *link) {
        ...
        if (!link->network->dhcp_anonymize) {
                ...
                if (link->network->dhcp_allow_ipv6_only_preferred) {
