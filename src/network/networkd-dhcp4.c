...
int config_parse_dhcp_label(
        ...
        r = config_parse_string(unit, filename, line, section, section_line, lvalue, ltype, rvalue, &label, userdata);

        if (r < 0)
                return r;

        if (!address_label_valid(label)) {
                log_syntax(unit, LOG_WARNING, filename, line, 0,
                           "Address label is too long or invalid, ignoring assignment: %s", rvalue);
                return 0;
        }

        return free_and_replace(*datap, label);
}
