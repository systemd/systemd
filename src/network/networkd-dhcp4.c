...
static int dhcp_offer_contains_ipv6_only_preferred_option(Link *link, sd_dhcp_client *client) {
        ...
        if (r < 0)
                return log_link_error_errno(link, r, "Failed to get DHCP IPv6-Only Preferred Option from lease: %m");
        ...
        log_link_debug(link, "DHCPv4 lease contains IPv6-Only Preferred Option, ignoring offer.");
...
static int dhcp4_handler(sd_dhcp_client *client, int event, void *userdata) {
        ...
        switch (event) {
                ...
                case SD_DHCP_CLIENT_EVENT_SELECTING:
                        ...
                        if (link->network->dhcp_use_ipv6_only_preferred && dhcp_offer_contains_ipv6_only_preferred_option(link, client))
                                return -ENOMSG;
