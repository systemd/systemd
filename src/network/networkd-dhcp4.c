...
static int dhcp4_get_static_routes(Link *link, sd_dhcp_route ***ret_routes, size_t *ret_num) {
...
static int dhcp4_find_gateway_for_destination(
        ...
        r = dhcp4_get_static_routes(link, &routes, &n_routes);
        if (r < 0 && r != -ENODATA)
                return r;
        is_classless = r > 0;
...
static int dhcp4_request_static_routes(Link *link) {
        _cleanup_free_ sd_dhcp_route **routes = NULL;
        size_t n_routes;
        int r;
...
static int dhcp4_request_default_gateway(Link *link) {
        ...
        /* According to RFC 3442: If the DHCP server returns both a Classless Static Routes option and
         * a Router option, the DHCP client MUST ignore the Router option. */
        if (link->network->dhcp_use_routes && dhcp4_get_static_routes(link, NULL, NULL) > 0)
                return 0;
...
static int dhcp4_request_semi_static_routes(Link *link) {
        ...
        HASHMAP_FOREACH(rt, link->network->routes_by_section) {
                ...
                r = dhcp4_find_gateway_for_destination(link, &route->dst.in, route->dst_prefixlen,
                                                       /* allow_null = */ false, &gw);
                if (IN_SET(r, -EHOSTUNREACH, -ENODATA)) {
                        log_link_debug_errno(link, r, "DHCP: Cannot find suitable gateway for destination %s of semi-static route, ignoring: %m",
                                             IN4_ADDR_PREFIX_TO_STRING(&route->dst.in, route->dst_prefixlen));
                        continue;
                }
                if (r < 0)
                        return r;
...
static int dhcp4_request_routes(Link *link) {
        ...
        r = dhcp4_request_static_routes(link);
        if (r < 0)
