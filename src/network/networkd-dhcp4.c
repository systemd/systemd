...
static int dhcp4_request_address(Link *link, bool announce) {
        ...
        if (!FLAGS_SET(link->network->keep_configuration, KEEP_CONFIGURATION_DHCP)) {
                uint32_t lifetime_u32;

                r = sd_dhcp_lease_get_lifetime(link->dhcp_lease, &lifetime_u32);
                if (r < 0)
                        return log_link_warning_errno(link, r, "DHCP error: no lifetime: %m");

                lifetime = usec_add(lifetime_u32 * USEC_PER_SEC, now(clock_boottime_or_monotonic()));
        } else
        ...
        addr->lifetime_preferred = lifetime;
        addr->lifetime_valid = lifetime;
