...
static int dhcp4_request_prefix_route(Link *link) {
        ...
        if (network->dhcp_add_prefixroute == 0)
                return 0;

        if (network->dhcp_add_prefixroute < 0 && network->dhcp_use_routes == 0)
                return 0;
...
static int dhcp4_request_address(Link *link, bool announce) {
        ...
        SET_FLAG(addr->flags, IFA_F_NOPREFIXROUTE, !link_prefixroute(link) || network->dhcp_add_prefixroute == 0);
