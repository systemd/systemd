...
static int dhcp_offer_contains_has_ipv6_only_preferred_option(Link *link, sd_dhcp_client *client) {
        sd_dhcp_lease *lease;
        uint32_t delay;
        int r;
        ...
        r = sd_dhcp_client_initialize_timeout__ipv6_only_mode(link->dhcp_client, delay);
        if (r < 0)
                return r;
...
static int dhcp4_handler(sd_dhcp_client *client, int event, void *userdata) {
        ...
        switch (event) {
                ...
                case SD_DHCP_CLIENT_EVENT_SELECTING:
                        ...
                        if (link->network->dhcp_allow_ipv6_only_mode && dhcp_offer_contains_has_ipv6_only_preferred_option(link, client))
                                return -ENOMSG;
