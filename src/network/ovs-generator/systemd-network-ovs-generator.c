 ...
 * Also scans matching .conf snippets in *.<suffix>.d drop-in directories. */
static int dir_has_kv_in_suffix(const char *dir, const char *suffix, const char *const (*kvs)[2]) {
        ...
        FOREACH_DIRENT(de, d, return -errno) {
                ...
                /* Direct .netdev / .network file. dirent_is_file_with_suffix() handles
                 * regular files, symlinks (admin tooling / `systemctl enable` workflows
                 * often install configs as symlinks) and DT_UNKNOWN (some filesystems
                 * never fill d_type). */
                ...
                /* Drop-in directory: foo.netdev.d/ or foo.network.d/. Accept symlinked
                 * directories (some packagers ship drop-in dirs as symlinks); DT_UNKNOWN
                 * falls through and opendir() inside dropin_dir_has_kv sorts it out. */
                if (endswith(de->d_name, dropin_suffix) && IN_SET(de->d_type, DT_DIR, DT_LNK, DT_UNKNOWN)) {
...
static int detect_ovs_usage(void) {
        ...
        const char *const *network_dirs = NETWORK_DIRS;
        int r;
