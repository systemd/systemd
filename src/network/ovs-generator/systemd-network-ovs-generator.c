...
static bool line_matches_kv(const char *line, const char *key, const char *value_prefix) {
        ...
        return !!startswith(p, value_prefix);
}
...
static int run(const char *dest, const char *dest_early, const char *dest_late) {
         ...
         * unprivileged "systemd-network" user. Pull in both:
         *  - systemd-networkd-ovsdb-acl.path, which watches for the socket and grants that
         *    user access to it via an ACL each time ovsdb-server (re)creates it; and
         *  - systemd-networkd-ovsdb-acl.service directly, which applies the ACL once at
         *    start if the socket already exists. This covers the case where OVS is already
         *    running when the OVS config is added and networkd is (re)started: PathModified=
         *    is edge-triggered and does not fire for a socket that already exists when the
         *    .path unit starts, so the service (guarded by ConditionPathExists= on the
         *    socket, hence inert when it is absent) closes that gap.
         * Pulling both from here keeps them inert on systems that do not use OVS. */
        r = write_drop_in_format(dest, "systemd-networkd.service", 50, "ovs",
                                 ...
                                 "Wants=systemd-networkd-ovsdb-acl.service\n",
                                 OPENVSWITCH_SERVICE_NAMES,
                                 OPENVSWITCH_SERVICE_NAMES);
        if (r < 0)
