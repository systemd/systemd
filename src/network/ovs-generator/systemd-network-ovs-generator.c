...
static int dir_has_kv_in_suffix(const char *dir, const char *suffix, const char *const (*kvs)[2]) {
        ...
        FOREACH_DIRENT(de, d, return -errno) {
                ...
                /* Direct .netdev / .network file. DT_UNKNOWN falls through (some fs return it
                 * even for regular files); fopen() in file_has_kv will then sort it out. */
                if (endswith(de->d_name, suffix) && IN_SET(de->d_type, DT_REG, DT_UNKNOWN)) {
                        r = file_has_kv(path, kvs);
                        if (r < 0) {
                                log_debug_errno(r, "Failed to scan '%s', ignoring: %m", path);
                                continue;
                ...
                if (endswith(de->d_name, dropin_suffix) && IN_SET(de->d_type, DT_DIR, DT_UNKNOWN)) {
...
static int run(const char *dest, const char *dest_early, const char *dest_late) {
        ...
        r = write_drop_in_format(dest, "systemd-networkd.service", 10, "ovs",
        ...
        r = write_drop_in_format(dest, "systemd-networkd-wait-online.service", 10, "ovs",
                                 "# Automatically generated by systemd-network-ovs-generator\n"
                                 "\n"
                                 "[Unit]\n"
                                 "After=%s\n",
