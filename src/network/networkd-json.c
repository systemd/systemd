...
static int sd_dns_resolver_transports_to_strv(sd_dns_alpn_flags transports, char ***ret) {
        _cleanup_strv_free_ char **ans = NULL;

        assert(ret);

        if (FLAGS_SET(transports, SD_DNS_ALPN_DO53)) {
                if (strv_extend(&ans, "UDP") < 0)
                        return -ENOMEM;
        }
        if (FLAGS_SET(transports, SD_DNS_ALPN_HTTP_2_TLS)) {
                if (strv_extend(&ans, "HTTPS/2") < 0)
                        return -ENOMEM;
        }
        if (FLAGS_SET(transports, SD_DNS_ALPN_HTTP_3)) {
                if (strv_extend(&ans, "HTTPS/3") < 0)
                        return -ENOMEM;
        }
        if (FLAGS_SET(transports, SD_DNS_ALPN_DOT)) {
                if (strv_extend(&ans, "TLS") < 0)
                        return -ENOMEM;
        }
        if (FLAGS_SET(transports, SD_DNS_ALPN_DOQ)) {
                if (strv_extend(&ans, "QUIC") < 0)
                        return -ENOMEM;
        }
