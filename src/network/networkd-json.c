...
static int dhcp6_client_vendor_options_append_json(Link *link, JsonVariant **v) {
        ...
        LIST_FOREACH(options, option, link->dhcp6_lease->vendor_options) {
                char key[STRLEN("VENDOR_SPECIFIC_SUB_OPTION_") + DECIMAL_STR_MAX(uint16_t)];

                xsprintf(key, "VENDOR_SPECIFIC_SUB_OPTION_%" PRIu16, option->option_code);
        ...
        return json_append_one(v, "VendorSpecificOptions", array);
}
