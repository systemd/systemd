...
static int ovs_append_json(Link *link, sd_json_variant **v) {
        ...
        if (netdev && netdev->kind == NETDEV_KIND_OVS_BRIDGE) {
                ...
                ovs_fail_mode = manager_ovs_get_bridge_fail_mode(link->manager, netdev->ifname);
                r = manager_ovs_get_bridge_ports(link->manager, netdev->ifname, &interfaces);
                if (r < 0)
        ...
        return sd_json_variant_merge_objectbo(
                        ...
                        /* The IDL declares these as enums; underscorify ("bond-member" → "bond_member") */
                        JSON_BUILD_PAIR_ENUM_NON_EMPTY("OVSPortType", ovs_port_type),
                        JSON_BUILD_PAIR_ENUM_NON_EMPTY("OVSFailMode", ovs_fail_mode),
