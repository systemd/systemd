...
static int dhcp6_client_pd_append_json(Link *link, JsonVariant **v) {
        ...
        while (sd_dhcp6_lease_get_pd(link->dhcp6_lease, &prefix, &prefix_len, &lifetime_preferred, &lifetime_valid) >= 0) {
                r = json_variant_append_arrayb(&array, JSON_BUILD_OBJECT(
                                               ...
                                               JSON_BUILD_PAIR_FINITE_USEC("PreferredLifetime", sec_to_usec(lifetime_preferred, 0)),
                                               JSON_BUILD_PAIR_FINITE_USEC("ValidLifetime", sec_to_usec(lifetime_valid, 0))));
                if (r < 0)
                        return r;
        }
...
static int dhcp_client_lease_append_json(Link *link, JsonVariant **v) {
        ...
        r = json_build(&w, JSON_BUILD_OBJECT(
                                JSON_BUILD_PAIR_FINITE_USEC("T1", sec_to_usec(t1, 0)),
                                JSON_BUILD_PAIR_FINITE_USEC("T2", sec_to_usec(t2, 0))));
        if (r < 0)
                return r;
...
static int dhcp_client_pd_append_json(Link *link, JsonVariant **v) {
        ...
        for (n = 0; n < n_br_addresses; ++n) {
                r = json_variant_append_arrayb(&addresses, JSON_BUILD_IN4_ADDR(br_addresses + n));
                if (r < 0)
                        return r;
        }
