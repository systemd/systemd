...
static int route_build_json(Route *route, JsonVariant **ret) {
        ...
        r = sd_event_now(manager->event, clock_boottime_or_monotonic(), &usec_now);
        if (r < 0)
                return r;
        ...
        r = json_build(&v, JSON_BUILD_OBJECT(
                                ...
                                JSON_BUILD_PAIR_FINITE_TIMESPAN("LifetimeString", usec_sub_unsigned(route->lifetime_usec, usec_now), USEC_PER_SEC),
...
static int routing_policy_rule_build_json(RoutingPolicyRule *rule, JsonVariant **ret) {
        ...
        r = json_build(&v, JSON_BUILD_OBJECT(
                                ...
                                JSON_BUILD_PAIR_STRING("TypeString", fr_act_type_to_string(rule->type)),
