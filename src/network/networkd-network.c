...
int network_verify(Network *network) {
        ...
        /* OVSBridge=+OVSBond= is already rejected by the check above, so this one need not
         * repeat ovs_bridge_name. */
            ...
            !eqzero(network->ovs_port_vlan_bitmap)) {
                ...
                if (network->ovs_bond_name)
                        ...
                        log_warning("%s: [OVSPort] settings without OVSBridge= or OVSBond=, ignoring.",
                                    network->filename);
                else if (network->ovs_port_tag != VLANID_INVALID &&
                         !eqzero(network->ovs_port_vlan_bitmap) &&
                         network->ovs_port_vlan_mode < 0)
                        /* Mirror the .netdev-side ovs_port_verify() warning: with Tag=/PVID= and
                         * VLAN=/Trunks= both set but VLANMode= unset, OVS makes the port an access
                         * port and silently ignores the trunk VLANs. */
                        log_warning("%s: Both Tag=/PVID= and VLAN=/Trunks= are set, but VLANMode= is not. "
...
int network_load_one(Manager *manager, OrderedHashmap **networks, const char *filename) {
        ...
        *network = (Network) {
                ...
                .ovs_port_vlan_mode = _OVS_PORT_VLAN_MODE_INVALID,
