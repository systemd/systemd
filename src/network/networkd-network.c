...
void network_apply_anonymize_if_set(Network *network) {
        ...
        /* RFC7844 section 3.6.: The client intending to protect its privacy SHOULD only request a
         * minimal number of options in the PRL and SHOULD also randomly shuffle the ordering of option
         * codes in the PRL. If this random ordering cannot be implemented, the client MAY order the
         * option codes in the PRL by option code number (lowest to highest).
         *
         * TODO: Maybe there should be another variable called dhcp_request_mtu (to use the MTU sent by
         * the server but to do not request the option in the PRL)? */
        network->dhcp_use_mtu = false;
        /* NOTE: when Anonymize=yes, the PRL route options are sent by default, but this is needed to
         * use them. */
