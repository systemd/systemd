...
DEFINE_HASH_OPS(network_config_hash_ops,
...
int network_load_one(Manager *manager, const char *filename) {
        ...
        *d = '\0';
        ...
        if (network->mtu > 0 && network->dhcp_use_mtu) {
                log_warning(
                        "MTUBytes= in [Link] section and UseMTU= in [DHCP] section are set in %s. "
                        "Disabling UseMTU=.",
                        filename);
                network->dhcp_use_mtu = false;
        }
        ...
        LIST_FOREACH(routes, route, network->static_routes) {
                if (!route->family) {
                        log_warning(
                                "Route section without Gateway field configured in %s. "
                                "Ignoring",
                                filename);
                        return 0;
                }
        ...
        LIST_FOREACH(addresses, address, network->static_addresses) {
                if (!address->family) {
                        log_warning(
                                "Address section without Address field configured in %s. "
                                "Ignoring",
                                filename);
                        return 0;
                }
...
int network_get(Manager *manager,
                sd_device *device,
                const char *ifname,
        ...
        LIST_FOREACH(networks, network, manager->networks) {
                if (net_match_config(network->match_mac,
                                     network->match_path,
                                     ...
                                     network->match_host,
                                     network->match_virt,
                                     network->match_kernel_cmdline,
                                     network->match_kernel_version,
                                     network->match_arch,
                                     address,
                                     path,
                                     parent_driver,
                        ...
                        if (network->match_name && device) {
                                ...
                                if (name_assign_type == NET_NAME_ENUM)
                                        log_warning(
                                                "%s: found matching network '%s', based on potentially unpredictable ifname",
                                                ifname,
                                                network->filename);
                                else
...
int network_apply(Network *network, Link *link) {
        ...
        if (network->n_dns > 0 || !strv_isempty(network->ntp) || !strv_isempty(network->search_domains) ||
...
int config_parse_netdev(const char *unit,
        ...
        if (r < 0) {
                log_syntax(unit,
                           ...
                           line,
                           r,
                           "%s could not be found, ignoring assignment: %s",
                           lvalue,
                           rvalue);
                return 0;
        }
        ...
        if (netdev->kind != kind) {
                log_syntax(unit,
                           ...
                           line,
                           0,
                           "NetDev is not a %s, ignoring assignment: %s",
                           lvalue,
                           rvalue);
                return 0;
        }
        ...
        case NETDEV_KIND_VCAN:
                ...
                if (r < 0) {
                        log_syntax(
                                unit, LOG_ERR, filename, line, r, "Cannot add NetDev '%s' to network: %m", rvalue);
                        return 0;
                }
                ...
                if (dns_name_is_root(domain) || streq(domain, "*")) {
                        ...
                        if (is_localhost(domain)) {
                                log_syntax(unit,
                                           ...
                                           line,
                                           0,
                                           "'localhost' domain names may not be configure as search or route domains, ignoring assignment: %s",
                                           domain);
                                continue;
                        }
                }
...
int config_parse_tunnel(const char *unit,
        ...
        if (r < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, r, "Tunnel is invalid, ignoring assignment: %s", rvalue);
                return 0;
        }
        ...
        if (!IN_SET(netdev->kind,
                ...
                log_syntax(unit,
                           ...
                           line,
                           0,
                           "NetDev is not a tunnel, ignoring assignment: %s",
                           rvalue);
                return 0;
        }
        ...
        if (r < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, r, "Cannot add VLAN '%s' to network, ignoring: %m", rvalue);
                return 0;
        }
...
int config_parse_ipv4ll(const char *unit,
                        const char *filename,
                        unsigned line,
                        const char *section,
                        unsigned section_line,
...
int config_parse_dhcp(const char *unit,
                      const char *filename,
                      unsigned line,
                      const char *section,
                      unsigned section_line,
        ...
        if (s < 0) {
                ...
                if (streq(rvalue, "none"))
                        ...
                        log_syntax(unit,
                                   ...
                                   line,
                                   0,
                                   "Failed to parse DHCP option, ignoring: %s",
                                   rvalue);
                        return 0;
                }
        }
...
int config_parse_ipv6token(const char *unit,
                           const char *filename,
                           unsigned line,
                           const char *section,
                           unsigned section_line,
        ...
        if (r < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, r, "Failed to parse IPv6 token, ignoring: %s", rvalue);
                return 0;
        }
...
int config_parse_ipv6_privacy_extensions(const char *unit,
                                         const char *filename,
                                         unsigned line,
                                         const char *section,
                                         unsigned section_line,
                ...
                if (s < 0) {
                        ...
                        if (streq(rvalue, "kernel"))
                                ...
                                log_syntax(unit,
                                           ...
                                           line,
                                           0,
                                           "Failed to parse IPv6 privacy extensions option, ignoring: %s",
                                           rvalue);
                                return 0;
                        }
                }
...
int config_parse_hostname(const char *unit,
        ...
        r = config_parse_string(
                unit, filename, line, section, section_line, lvalue, ltype, rvalue, &hn, userdata);
        if (r < 0)
                return r;

        if (!hostname_is_valid(hn, false)) {
                log_syntax(
                        unit, LOG_ERR, filename, line, 0, "Hostname is not valid, ignoring assignment: %s", rvalue);
                return 0;
        }
        ...
        if (r < 0) {
                log_syntax(unit,
                           ...
                           line,
                           r,
                           "Failed to check validity of hostname '%s', ignoring assignment: %m",
                           rvalue);
                return 0;
        }
        if (r == 0) {
...
int config_parse_timezone(const char *unit,
                          const char *filename,
                          unsigned line,
                          const char *section,
                          unsigned section_line,
        ...
        r = config_parse_string(
                unit, filename, line, section, section_line, lvalue, ltype, rvalue, &tz, userdata);
        if (r < 0)
                return r;

        if (!timezone_is_valid(tz, LOG_ERR)) {
                log_syntax(
                        unit, LOG_ERR, filename, line, 0, "Timezone is not valid, ignoring assignment: %s", rvalue);
                return 0;
        }
...
int config_parse_dhcp_server_dns(const char *unit,
                                 const char *filename,
                                 unsigned line,
                                 const char *section,
                                 unsigned section_line,
        ...
        for (;;) {
                ...
                if (r < 0) {
                        log_syntax(
                                unit, LOG_ERR, filename, line, r, "Failed to extract word, ignoring: %s", rvalue);
                        return 0;
                }
                if (r == 0)
                        break;
                ...
                if (inet_pton(AF_INET, w, &a) <= 0) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   0,
                                   "Failed to parse DNS server address, ignoring: %s",
                                   w);
                        continue;
                }
...
int config_parse_radv_dns(const char *unit,
                          const char *filename,
                          unsigned line,
                          const char *section,
                          unsigned section_line,
...
int config_parse_radv_search_domains(const char *unit,
                                     const char *filename,
                                     unsigned line,
                                     const char *section,
                                     unsigned section_line,
...
int config_parse_dhcp_server_ntp(const char *unit,
                                 const char *filename,
                                 unsigned line,
                                 const char *section,
                                 unsigned section_line,
        ...
        for (;;) {
                ...
                if (inet_pton(AF_INET, w, &a) <= 0) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   0,
                                   "Failed to parse NTP server address, ignoring: %s",
                                   w);
                        continue;
                }
...
int config_parse_dnssec_negative_trust_anchors(const char *unit,
                                               const char *filename,
                                               unsigned line,
                                               const char *section,
                                               unsigned section_line,
        ...
        for (;;) {
                ...
                if (r <= 0) {
                        log_syntax(
                                unit, LOG_ERR, filename, line, r, "%s is not a valid domain name, ignoring.", w);
                        continue;
                }
...
int config_parse_ntp(const char *unit,
                     const char *filename,
                     unsigned line,
                     const char *section,
                     unsigned section_line,
...
int config_parse_dhcp_user_class(const char *unit,
                                 const char *filename,
                                 unsigned line,
                                 const char *section,
                                 unsigned section_line,
        ...
        for (;;) {
                ...
                if (strlen(w) > 255) {
                        log_syntax(unit,
                                   ...
                                   line,
                                   r,
                                   "%s length is not in the range 1-255, ignoring.",
                                   w);
                        continue;
                }
...
int config_parse_dhcp_route_table(const char *unit,
        ...
        if (r < 0) {
                log_syntax(unit,
                           ...
                           line,
                           r,
                           "Unable to read RouteTable, ignoring assignment: %s",
                           rvalue);
                return 0;
        }
...
int config_parse_iaid(const char *unit,
        ...
        if (r < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, r, "Unable to read IAID, ignoring assignment: %s", rvalue);
                return 0;
        }
