...
static void test_route_tables_one(Manager *manager, const char *name, uint32_t number) {
        ...
        assert_se(route_table_to_string_full(manager, number, &str) >= 0);
        assert_se(streq(str, expected));

        assert_se(route_table_from_string_full(manager, name, &t) >= 0);
        assert_se(t == number);

        assert_se(asprintf(&num_str, "%" PRIu32, number) >= 0);
        assert_se(route_table_from_string_full(manager, num_str, &t) >= 0);
        assert_se(t == number);
}
