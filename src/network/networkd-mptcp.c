...
static int mp_tcp_new_static(Network *network, const char *filename, unsigned section_line, MPTCP **ret) {
        ...
        mp_tcp = hashmap_get(network->mp_tcp_by_section, n);
        if (mp_tcp) {
                *ret = TAKE_PTR(mp_tcp);
                return 0;
        }
        ...
        r = hashmap_ensure_put(&network->mp_tcp_by_section, &config_section_hash_ops, mp_tcp->section, mp_tcp);
        if (r < 0)
                return r;
...
MPTCP *mp_tcp_free(MPTCP *mp_tcp) {
        ...
        if (mp_tcp->network && mp_tcp->section)
                hashmap_remove(mp_tcp->network->mp_tcp_by_section, mp_tcp->section);

        config_section_free(mp_tcp->section);
...
int mp_tcp_configure_address(Link *link, MPTCP *mp_tcp) {
        ...
        r = sd_genl_message_new(link->manager->genl, MPTCP_PM_NAME, MPTCP_PM_CMD_ADD_ADDR, &req);
        if (r < 0)
...
static int mp_tcp_limit_handler(sd_netlink *rtnl, sd_netlink_message *n, Manager *m) {
        ...
        if (r < 0) {
                log_message_warning_errno(n, r, "Could not set up MPTCP limits");
                return 1;
        }
...
int mp_tcp_configure_limit(Manager *m) {
        ...
        r = sd_genl_message_new(m->genl, MPTCP_PM_NAME, MPTCP_PM_CMD_SET_LIMITS, &req);
        if (r < 0)
...
int link_configure_mp_tcp(Link *link) {
        ...
        HASHMAP_FOREACH(mp_tcp, link->network->mp_tcp_by_section) {
...
int network_drop_invalid_mp_tcp(Network *network) {
        ...
        HASHMAP_FOREACH(mp_tcp, network->mp_tcp_by_section)
