...
static int manager_link_is_online(Manager *m, Link *l, const LinkOperationalStateRange *s) {
        ...
        if (!s || !operational_state_range_is_valid(s))
                s = operational_state_range_is_valid(&m->required_operstate) ?
                        &m->required_operstate : &l->required_operstate;
        assert(operational_state_range_is_valid(s));

        if (!operational_state_is_in_range(l->operational_state, s))
...
bool manager_configured(Manager *m) {
        ...
        HASHMAP_FOREACH(l, m->links_by_index) {
                ...
                r = manager_link_is_online(m, l, NULL);
