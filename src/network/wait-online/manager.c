...
static int manager_link_is_online(Manager *m, Link *l, LinkOperationalStateRange s) {
        ...
        if (l->operational_state >= LINK_OPERSTATE_DEGRADED) {
        ...
        if (l->operational_state >= LINK_OPERSTATE_ROUTABLE) {
...
family_not_ready:
        log_link_debug(l, "Required address family '%s' is not reached", link_address_family_to_string(required_family));
        return 0;
}
