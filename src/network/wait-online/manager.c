...
static int manager_link_is_online(Manager *m, Link *l, LinkOperationalStateRange s) {
        ...
        /* If no state string exists, networkd (and possibly also udevd) has not detected the interface
         * yet, that mean we cannot determine whether the interface is managed or not. Hence, return
         * negative value.
         * If the link is in pending state, then udevd has not processed the link, and networkd has not
         * tried to find .network file for the link. Hence, return negative value. */
