...
int link_new(Manager *m, Link **ret, int ifindex, const char *ifname) {
        ...
        *l = (Link) {
                ...
                .required_family = ADDRESS_FAMILY_NO,
        };
...
int link_update_monitor(Link *l) {
        ...
        if (r < 0)
                ...
                f = link_address_family_from_string(required_family);
                if (f < 0)
                        ret = log_link_debug_errno(l, SYNTHETIC_ERRNO(EINVAL),
                                                   "Failed to parse required address family, ignoring: %m");
                else
                        l->required_family = f;
        }
        ...
        if (r < 0)
                ...
                if (s < 0)
                        ret = log_link_debug_errno(l, SYNTHETIC_ERRNO(EINVAL),
                                                   "Failed to parse IPv4 address state, ignoring: %m");
                else
        ...
        if (r < 0)
                ...
                if (s < 0)
                        ret = log_link_debug_errno(l, SYNTHETIC_ERRNO(EINVAL),
                                                   "Failed to parse IPv6 address state, ignoring: %m");
                else
