...
static int netlabel_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        ...
        log_link_message_debug_errno(link, m, r, "NetLabel operation successful");
...
static int netlabel_command(uint16_t command, const char *label, const Address *address) {
        ...
        r = netlink_call_async(address->link->manager->genl, NULL, m, netlabel_handler, NULL, address->link);
        if (r < 0)
                return r;
