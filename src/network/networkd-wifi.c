...
int wifi_get_info(Link *link) {
        ...
        if (iftype == NL80211_IFTYPE_STATION) {
                ...
                if (r < 0)
                        return r;
                if (r == 0 && !isempty(ssid)) {
                        /* We got ssid, but not bssid. The interface seems to be (de)authenticating.
                         * Let's keep the old information now.
                         * FIXME: If this hits, then BSSID= in [Match] does not work... */
                        log_link_debug(link,
                                       "Obtained SSID '%s' without BSSID. Assuming the interface is (de)authenticating.",
                                       ssid);
                        return 0;
                }
