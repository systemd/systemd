...
int link_varlink_simple_method(int argc, char *argv[], void *userdata) {
        ...
        typedef struct LinkVarlinkAction {
                const char *verb;
                const char *varlink_method;
                const char *error_message;
        } LinkVarlinkAction;

        static const LinkVarlinkAction link_varlink_action_table[] = {
                { "renew",       "io.systemd.Network.RenewLink",       "Failed to renew dynamic configuration of interface"          },
                { "forcerenew",  "io.systemd.Network.ForceRenewLink",  "Failed to forcibly renew dynamic configuration of interface" },
                { "reconfigure", "io.systemd.Network.ReconfigureLink", "Failed to reconfigure network interface"                     },
        };
        ...
        ORDERED_SET_FOREACH(p, indexes) {
                ...
                RET_GATHER(ret, varlink_callbo_and_log(
                                        ...
                                        a->varlink_method,
                                        /* reply= */ NULL,
                                        SD_JSON_BUILD_PAIR_INTEGER("InterfaceIndex", index),
                                        SD_JSON_BUILD_PAIR_BOOLEAN("allowInteractiveAuthentication", arg_ask_password)));
        }
