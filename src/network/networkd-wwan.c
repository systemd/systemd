...
static int bearer_get_by_name(Manager *m, const char *name, Bearer **ret) {
        Bearer *b;

        assert(m);
        assert(name);

        b = hashmap_get(m->bearers_by_name, name);
        if (!b)
                return -ENOENT;
...
int link_get_bearer(Link *link, Bearer **ret) {
        ...
        return bearer_get_by_name(link->manager, link->ifname, ret);
}
...
int link_dhcp_enabled_by_bearer(Link *link, int family) {
        ...
        return family == AF_INET ?
                b->ip4_method == MM_BEARER_IP_METHOD_DHCP :
                b->ip6_method == MM_BEARER_IP_METHOD_DHCP;
}

static int bearer_address_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        int r;

        assert(link);
        assert(link->bearer_messages > 0);

        link->bearer_messages--;
...
static int bearer_route_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        int r;
        ...
        if (r < 0 && r != -EEXIST) {
                log_link_message_warning_errno(link, m, r, "Could not set gateway provided by bearer");
...
int link_apply_bearer(Link *link) {
        ...
        (void) bearer_get_by_name(link->manager, link->ifname, &b);
