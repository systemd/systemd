...
static int routing_policy_rule_get(Manager *m, RoutingPolicyRule *in, RoutingPolicyRule **ret) {
        RoutingPolicyRule *rule;
        ...
        if (in->priority_set) {
                in->priority_set = false;
                rule = set_get(m->rules, in);
                in->priority_set = true;
        } else
                SET_FOREACH(rule, m->rules) {
                        ...
                        equal = routing_policy_rule_equal(rule, in);
...
static int link_request_routing_policy_rule(
        ...
        if (routing_policy_rule_get(link->manager, rule, &existing) < 0) {
                ...
                r = routing_policy_rule_update_priority(existing, rule->priority);
                if (r < 0)
                        return r;
