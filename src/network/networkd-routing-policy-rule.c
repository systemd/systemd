...
DEFINE_PRIVATE_HASH_OPS(routing_policy_rule_hash_ops,
                        RoutingPolicyRule,
...
int routing_policy_rule_add(Manager *m,
        ...
        return routing_policy_rule_add_internal(m,
                                                &m->rules,
                                                family,
                                                from,
                                                from_prefixlen,
...
int routing_policy_rule_configure(RoutingPolicyRule *rule,
                                  Link *link,
                                  link_netlink_message_handler_t callback,
                                  bool update) {
        _cleanup_(sd_netlink_message_unrefp) sd_netlink_message *m = NULL;
        ...
        r = routing_policy_rule_add(link->manager,
                                    rule->family,
                                    &rule->from,
                                    rule->from_prefixlen,
                                    &rule->to,
...
int config_parse_routing_policy_rule_tos(const char *unit,
                                         const char *filename,
                                         unsigned line,
                                         const char *section,
                                         unsigned section_line,
        ...
        if (r < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, r, "Failed to parse RPDB rule tos, ignoring: %s", rvalue);
                return 0;
        }
...
int config_parse_routing_policy_rule_priority(const char *unit,
                                              const char *filename,
                                              unsigned line,
                                              const char *section,
                                              unsigned section_line,
        ...
        if (r < 0) {
                log_syntax(unit,
                           ...
                           line,
                           r,
                           "Failed to parse RPDB rule priority, ignoring: %s",
                           rvalue);
                return 0;
        }
...
int config_parse_routing_policy_rule_table(const char *unit,
                                           const char *filename,
                                           unsigned line,
                                           const char *section,
                                           unsigned section_line,
        ...
        if (r < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, r, "Failed to parse RPDB rule table, ignoring: %s", rvalue);
                return 0;
        }
...
int config_parse_routing_policy_rule_fwmark_mask(const char *unit,
                                                 const char *filename,
                                                 unsigned line,
                                                 const char *section,
                                                 unsigned section_line,
...
int config_parse_routing_policy_rule_prefix(const char *unit,
                                            const char *filename,
                                            unsigned line,
                                            const char *section,
                                            unsigned section_line,
...
int config_parse_routing_policy_rule_device(const char *unit,
                                            const char *filename,
                                            unsigned line,
                                            const char *section,
                                            unsigned section_line,
        ...
        if (!ifname_valid(rvalue)) {
                log_syntax(unit,
                           ...
                           line,
                           r,
                           "Failed to parse '%s' interface name, ignoring: %s",
                           lvalue,
                           rvalue);
                return 0;
        }
...
int config_parse_routing_policy_rule_port_range(const char *unit,
                                                const char *filename,
                                                unsigned line,
                                                const char *section,
                                                unsigned section_line,
...
int config_parse_routing_policy_rule_ip_protocol(const char *unit,
                                                 const char *filename,
                                                 unsigned line,
                                                 const char *section,
                                                 unsigned section_line,
        ...
        if (r < 0) {
                log_syntax(unit,
                           ...
                           line,
                           r,
                           "Failed to parse IP protocol '%s' for routing policy rule, ignoring: %m",
                           rvalue);
                return 0;
        }
...
int config_parse_routing_policy_rule_invert(const char *unit,
                                            const char *filename,
                                            unsigned line,
                                            const char *section,
                                            unsigned section_line,
        ...
        if (r < 0) {
                log_syntax(
                        unit, LOG_ERR, filename, line, r, "Failed to parse RPDB rule invert, ignoring: %s", rvalue);
                return 0;
        }
...
int routing_policy_serialize_rules(Set *rules, FILE *f) {
        ...
        SET_FOREACH(rule, rules, i) {
                ...
                if (!in_addr_is_null(rule->family, &rule->to)) {
                        ...
                        fprintf(f, "%sto=%s/%hhu", space ? " " : "", to_str, rule->to_prefixlen);
                        space = true;
                }
                ...
                if (rule->fwmark != 0) {
                        fprintf(f, "%sfwmark=%" PRIu32 "/%" PRIu32, space ? " " : "", rule->fwmark, rule->fwmask);
                        space = true;
                }
                ...
                if (rule->dport.start != 0 || rule->dport.end != 0) {
                        fprintf(f,
                                "%sdestinationport=%" PRIu16 "-%" PRIu16,
                                space ? " " : "",
                                rule->dport.start,
                                rule->dport.end);
                        space = true;
        ...
        STRV_FOREACH (i, l) {
                ...
                for (;;) {
                        ...
                        if (STR_IN_SET(a, "from", "to")) {
                                ...
                                if (r < 0) {
                                        log_error_errno(
                                                r, "RPDB rule prefix is invalid, ignoring assignment: %s", b);
                                        continue;
                                }
                                ...
                                if (r < 0) {
                                        log_error_errno(
                                                r, "Failed to parse RPDB rule protocol, ignoring: %s", b);
                                        continue;
                                }
                                ...
                                if (r < 0) {
                                        log_error_errno(
                                                r,
                                                "Invalid routing policy rule source port range, ignoring assignment:'%s'",
                                                b);
                                        continue;
                                }
                                ...
                                if (r < 0) {
                                        log_error_errno(
                                                r,
                                                "Invalid routing policy rule destination port range, ignoring assignment:'%s'",
                                                b);
                                        continue;
                                }
