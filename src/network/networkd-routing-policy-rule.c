...
static int routing_policy_rule_set_netlink_message(const RoutingPolicyRule *rule, sd_netlink_message *m) {
        ...
        if (rule->from.prefixlen > 0) {
                ...
                r = sd_rtnl_message_routing_policy_rule_set_fib_src_prefixlen(m, rule->from.prefixlen);
                if (r < 0)
                        return r;
        }
        ...
        if (rule->to.prefixlen > 0) {
                ...
                r = sd_rtnl_message_routing_policy_rule_set_fib_dst_prefixlen(m, rule->to.prefixlen);
                if (r < 0)
                        return r;
        }
...
int manager_rtnl_process_rule(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        if (r < 0 && r != -ENODATA) {
                ...
                r = sd_rtnl_message_routing_policy_rule_get_fib_src_prefixlen(message, &tmp->from.prefixlen);
                if (r < 0) {
        ...
        if (r < 0 && r != -ENODATA) {
                ...
                r = sd_rtnl_message_routing_policy_rule_get_fib_dst_prefixlen(message, &tmp->to.prefixlen);
                if (r < 0) {
