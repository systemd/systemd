...
int manager_rtnl_process_rule(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        if (r >= 0)
                tmp->suppress_ifgroup = (int) suppress_ifgroup;
...
int config_parse_routing_policy_rule_suppress_ifgroup(
        ...
        r = safe_atoi32(rvalue, &n->suppress_ifgroup);
        if (r < 0) {
                n->suppress_ifgroup = -1;
                log_syntax(unit, LOG_WARNING, filename, line, r,
                           "Failed to parse SuppressIfGroup=, ignoring assignment: %s", rvalue);
                return 0;
        }
        if (n->suppress_ifgroup < 0) {
                log_syntax(unit, LOG_WARNING, filename, line, r,
                           "Failed to parse SuppressIfGroup=, value should be in range: [0, 2147483647]");
                return 0;
        }
