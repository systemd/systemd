...
static const char *const address_type_table[_ADDRESS_TYPE_MAX] = {
        [ADDRESS_TYPE_DHCP4_IP]      = "_dhcp4_ip",
        [ADDRESS_TYPE_DHCP4_GATEWAY] = "_dhcp4_gw",
};

DEFINE_PRIVATE_STRING_TABLE_LOOKUP_FROM_STRING(address_type, int);
...
static int routing_policy_rule_new(RoutingPolicyRule **ret) {
        ...
        *rule = (RoutingPolicyRule) {
                ...
                .type = FR_ACT_TO_TBL,
                .from_type = _ADDRESS_TYPE_INVALID,
                .to_type = _ADDRESS_TYPE_INVALID,
        };
...
int config_parse_routing_policy_rule_prefix(
        ...
        if (STR_IN_SET(lvalue, "To", "From") && IN_SET(n->family, AF_UNSPEC, AF_INET)) {
                AddressType a = address_type_from_string(rvalue);
                if (a >= 0) {
                        if (streq(lvalue, "To"))
                                n->to_type = r;
                        else
...
static int routing_policy_rule_section_verify(RoutingPolicyRule *rule) {
        ...
        if ((rule->family == AF_INET6 || FLAGS_SET(rule->address_family, ADDRESS_FAMILY_IPV6)) &&
            (rule->from_type != _ADDRESS_TYPE_INVALID || rule->to_type != _ADDRESS_TYPE_INVALID))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
