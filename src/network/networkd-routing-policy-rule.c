...
int manager_rtnl_process_rule(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        if (r >= 0) {
                ...
                if (suppress_prefixlen > 128) {
                        log_warning("rtnl: received invalid FRA_SUPPRESS_PREFIXLEN attribute value, ignoring: %"PRIu32, suppress_prefixlen);
                        return 0;
                }
        ...
        if (r >= 0) {
                if (suppress_ifgroup > INT32_MAX) {
                        log_warning("rtnl: received invalid FRA_SUPPRESS_IFGROUP attribute value, ignoring: %"PRIu32, suppress_ifgroup);
                        return 0;
                }
