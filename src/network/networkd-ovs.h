...
int manager_ovs_reconcile(Manager *m);
...
int manager_ovs_clear_database(Manager *m);
...
const char* manager_ovs_get_bridge_fail_mode(Manager *m, const char *bridge_name);

/* Look up the OVS Bridge row's ports (Port UUIDs resolved to names).
 * Returns a strv that caller owns, or NULL if not found. */
int manager_ovs_get_bridge_ports(Manager *m, const char *bridge_name, char ***ret_ports);

/* Count loaded OVS netdevs/attachments and, depending on the result, start the
 * OVSDB client (and reconcile) or tear it down. Called on config load/reload. */
int manager_ovs_maybe_start(Manager *m);

/* Trigger a reconcile for a link-driven OVS config change; no-op unless the
 * OVSDB client is connected and READY. */
void link_ovs_maybe_reconcile(Link *link);
