...
int vl_method_link_set_domains(sd_varlink *vlink, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        _cleanup_ordered_set_free_ OrderedSet *route_domains = NULL;
        ...
        FOREACH_ARRAY(d, p.domains, p.n_domains) {
                ...
                if (r < 0)
                        return r;
                if (r == 0)
                        return sd_varlink_error_invalid_parameter(vlink, JSON_VARIANT_STRING_CONST("Domain"));

                if (!d->route_only && dns_name_is_root(d->name))
                        return sd_varlink_error_invalid_parameter(vlink, JSON_VARIANT_STRING_CONST("Domain"));

                _cleanup_free_ char *normalized = NULL;
                r = dns_name_normalize(d->name, 0, &normalized);
                if (r < 0)
                        return sd_varlink_error_invalid_parameter(vlink, JSON_VARIANT_STRING_CONST("Domain"));

                r = ordered_set_consume(d->route_only ? route_domains : search_domains, TAKE_PTR(normalized));
                if (r == -EEXIST)
                        continue;
                if (r < 0)
                        return r;
...
int vl_method_link_set_dns_over_tls(sd_varlink *vlink, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (link->dns_over_tls_mode != mode) {
                link->dns_over_tls_mode = mode;
...
int vl_method_link_set_dnssec(sd_varlink *vlink, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        if (link->dnssec_mode != mode) {
                link->dnssec_mode = mode;
