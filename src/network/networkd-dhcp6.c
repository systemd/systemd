...
int request_process_dhcp6_client(Request *req) {
        ...
        r = dhcp6_start(link);
        if (r < 0)
                return log_link_warning_errno(link, r, "Failed to start DHCP6 client: %m");

        log_link_debug(link, "DHCP6 client is configured%s",
...
int link_request_dhcp6_client(Link *link) {
        ...
        if (r < 0)
                return log_link_warning_errno(link, r, "Failed to request to configure DHCP6 client: %m");

        log_link_debug(link, "Requested to configure DHCP6 client.");
        return 0;
}
