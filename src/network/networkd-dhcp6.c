...
bool dhcp6_address_is_eligible_for_registration(const Address *address) {
        ...
        /* RFC 9686 section 4.2 registers global addresses except those assigned by DHCPv6. */
        return address->family == AF_INET6 &&
               address->scope == RT_SCOPE_UNIVERSE &&
               address->source != NETWORK_CONFIG_SOURCE_DHCP6 &&
               !address->also_requested_by_dhcp6 &&
               !FLAGS_SET(address->flags, IFA_F_DADFAILED) &&
...
void dhcp6_reset_address_registration(Link *link) {
        ...
        /* RFC 9686 section 4.4 makes registration support link-scoped and sticky until disconnection. */
...
static int verify_dhcp6_address(Link *link, const Address *address) {
        ...
        dhcp6_sync_address_registration(link, existing);
        ...
        SET_FOREACH(address, link->addresses) {
                ...
                if (r < 0)
                        log_link_warning_errno(
                                        link, r,
                                        "DHCPv6 CLIENT: Failed to synchronize address registration, "
                                        "ignoring: %m");
        }
