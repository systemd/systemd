...
static void dhcp6_pd_check_ready(Link *link);
...
static void dhcp6_pd_check_ready(Link *link) {
        ...
        if (link->dhcp6_pd_messages > 0) {
                log_link_debug(link, "%s(): DHCPv6PD address and routes are not set.", __func__);
                return;
        }
        ...
        if (dhcp6_pd_remove(link, /* only_marked = */ true) < 0) {
...
static void dhcp6_check_ready(Link *link);
...
static void dhcp6_check_ready(Link *link) {
        ...
        if (link->dhcp6_messages > 0) {
                log_link_debug(link, "%s(): DHCPv6 address and routes are not set.", __func__);
                return;
        }
        ...
        if (dhcp6_remove(link, /* only_marked = */ true) < 0) {
                link_enter_failed(link);
                return;
        }
...
static int dhcp6_lease_ip_acquired(sd_dhcp6_client *client, Link *link) {
        ...
        SET_FOREACH(route, link->addresses)
                if (route->source == NETWORK_CONFIG_SOURCE_DHCP6)
                        route_mark(route);
