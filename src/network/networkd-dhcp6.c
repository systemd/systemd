...
static int dhcp6_pd_remove_old(Link *link, bool force) {
        ...
        if (!force && (!link->dhcp6_pd_address_configured || !link->dhcp6_pd_route_configured))
                return 0;
        ...
        SET_FOREACH(route, link->dhcp6_pd_routes_old, i) {
                ...
                l = hashmap_remove2(link->manager->dhcp6_prefixes, &route->dst.in6, (void **) &a);
...
int dhcp6_pd_remove(Link *link) {
        ...
        r = dhcp6_pd_remove_old(link, true);
        if (r < 0)
                return r;
...
static int dhcp6_set_pd_route(Link *link, const union in_addr_union *addr) {
        ...
        assigned_link = hashmap_get(link->manager->dhcp6_prefixes, &addr->in6);
        if (assigned_link) {
                assert(assigned_link == link);
                return 0;
        }

        a = newdup(struct in6_addr, &addr->in6, 1);
        if (!a)
                return log_oom();
        ...
        r = hashmap_put(link->manager->dhcp6_prefixes, a, link);
        if (r < 0)
...
static int dhcp6_pd_assign_prefix(Link *link, const union in_addr_union *prefix, uint8_t prefix_len,
        ...
        r = dhcp6_set_pd_route(link, prefix);
        if (r < 0)
                return r;
...
static int dhcp6_get_preferred_delegated_prefix(
        ...
        if (link_has_preferred_subnet_id(link)) {
                ...
                Link* assigned_link = dhcp6_prefix_get(link->manager, &prefix.in6);
...
static void dhcp6_pd_prefix_distribute(Link *dhcp6_link,
        ...
        HASHMAP_FOREACH(link, dhcp6_link->manager->links, i) {
                ...
                r = dhcp6_get_preferred_delegated_prefix(link, prefix, pd_prefix_len, &assigned_prefix);
                if (r < 0)
                        continue;

                (void) in_addr_to_string(AF_INET6, &assigned_prefix, &assigned_buf);
                r = dhcp6_pd_assign_prefix(link, &assigned_prefix, 64,
        ...
        HASHMAP_FOREACH(l, link->manager->links, i) {
                ...
                if (enabled != 0) {
                        if (l->dhcp6_lease) {
                                ...
                                (void) dhcp6_pd_prefix_acquired(l);
                        }
                        continue;
                }
