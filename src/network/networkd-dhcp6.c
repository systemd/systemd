...
static int dhcp6_pd_request_route(Link *link, const struct in6_addr *prefix, usec_t lifetime) {
...
static int dhcp6_pd_request_address(
        ...
        SET_FOREACH(a, addresses) {
                ...
                address->lifetime_preferred = lifetime_preferred;
                address->lifetime_valid = lifetime_valid;
...
static int dhcp6_pd_assign_prefix(
        ...
        r = dhcp6_pd_request_route(link, prefix, lifetime_valid);
        if (r < 0)
                return r;
...
static int dhcp6_request_unreachable_route(Link *link, const struct in6_addr *addr, uint8_t prefixlen, usec_t lifetime) {
        _cleanup_(route_freep) Route *route = NULL;
        _cleanup_free_ char *buf = NULL;
        Route *existing;
        int r;
...
static int dhcp6_pd_prefix_acquired(Link *dhcp6_link) {
        ...
        for (sd_dhcp6_lease_reset_pd_prefix_iter(dhcp6_link->dhcp6_lease);;) {
                uint32_t lifetime_preferred_u32, lifetime_valid_u32;
                usec_t lifetime_preferred, lifetime_valid;
                struct in6_addr pd_prefix;
                uint8_t pd_prefix_len;

                r = sd_dhcp6_lease_get_pd(dhcp6_link->dhcp6_lease, &pd_prefix, &pd_prefix_len,
                                          &lifetime_preferred_u32, &lifetime_valid_u32);
                if (r < 0)
                        break;

                lifetime_preferred = usec_add(lifetime_preferred_u32 * USEC_PER_SEC, timestamp_usec);
                lifetime_valid = usec_add(lifetime_valid_u32 * USEC_PER_SEC, timestamp_usec);
                ...
                r = dhcp6_request_unreachable_route(dhcp6_link, &pd_prefix, pd_prefix_len, lifetime_valid);
                if (r < 0)
                        return r;
...
static int dhcp6_request_address(
        ...
        addr->prefixlen = 128;
        addr->lifetime_preferred = lifetime_preferred;
        addr->lifetime_valid = lifetime_valid;
...
static int dhcp6_address_acquired(Link *link) {
        ...
        for (sd_dhcp6_lease_reset_address_iter(link->dhcp6_lease);;) {
                ...
                r = dhcp6_request_address(link, &ip6_addr,
                                          usec_add(lifetime_preferred * USEC_PER_SEC, timestamp_usec),
                                          usec_add(lifetime_valid * USEC_PER_SEC, timestamp_usec));
                if (r < 0)
                        return r;
        }
