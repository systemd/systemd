...
int link_drop_foreign_neighbors(Link *link) {
        ...
        SET_FOREACH(neighbor, link->neighbors) {
                ...
                if (!FLAGS_SET(neighbor->state, NETWORK_CONFIG_STATE_CONFIGURED) ||
                    FLAGS_SET(neighbor->state, NETWORK_CONFIG_STATE_REMOVING))
                        continue;
...
int link_drop_neighbors(Link *link) {
        ...
        SET_FOREACH(neighbor, link->neighbors) {
                ...
                if ((neighbor->state & (NETWORK_CONFIG_STATE_CONFIGURING | NETWORK_CONFIG_STATE_CONFIGURED)) == 0 ||
                    (FLAGS_SET(neighbor->state, NETWORK_CONFIG_STATE_REMOVING) &&
                     !FLAGS_SET(neighbor->state, NETWORK_CONFIG_STATE_CONFIGURING)))
                        continue;
