...
static int link_request_neighbor(Link *link, const Neighbor *neighbor) {
        ...
        if (neighbor->ll_addr.length != link->hw_addr.length) {
                log_link_debug(link,
                               "The link layer address lenght (%zu) for neighbor %s does not match "
...
int manager_rtnl_process_neighbor(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        /* Then, find the managed Neighbor and Request objects correspond to the netlink notification. */
