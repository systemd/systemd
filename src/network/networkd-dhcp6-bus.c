...
static int property_get_dhcp6_client_state(
        ...
        c = l->dhcp6_client;
        if (!c)
                return sd_bus_message_append(reply, "s", "n/a");

        return sd_bus_message_append(reply, "s", dhcp6_state_to_string(sd_dhcp6_client_get_state(c)));
}
...
static int dhcp6_client_emit_changed(Link *link, const char *property, ...) {
        ...
        return sd_bus_emit_properties_changed_strv(
                        link->manager->bus,
                        path,
                        "org.freedesktop.network1.DHCP6Client",
                        l);
}
