...
int ipv4acd_configure(Link *link, Address *address, bool is_static) {
        ...
        r = sd_ipv4acd_new(&a->acd);
        if (r < 0)
                return r;

        r = sd_ipv4acd_attach_event(a->acd, link->manager->event, 0);
        if (r < 0)
                return r;

        r = sd_ipv4acd_set_ifindex(a->acd, link->ifindex);
        if (r < 0)
                return r;

        r = sd_ipv4acd_set_mac(a->acd, &link->hw_addr.ether);
        if (r < 0)
                return r;
        ...
        HASHMAP_FOREACH(l, link->manager->links) {
                ...
                if (ether_addr_is_null(&l->hw_addr.ether))
                        continue;
                r = sd_ipv4acd_add_other_mac(a->acd, &l->hw_addr.ether);
                if (r < 0)
                        return r;
        }

        r = sd_ipv4acd_set_address(a->acd, &a->in_addr.in);
        if (r < 0)
                return r;

        r = sd_ipv4acd_set_callback(a->acd, is_static ? static_address_on_acd : dhcp4_address_on_acd, a);
        if (r < 0)
                return r;

        if (link_has_carrier(link)) {
                r = sd_ipv4acd_start(a->acd, true);
                if (r < 0)
                        return r;
        }
