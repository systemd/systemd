...
int address_new(Address **ret) {
        ...
        *address = (Address) {
                ...
                .lifetime_valid = USEC_INFINITY,
                .lifetime_preferred = USEC_INFINITY,
                .set_broadcast = -1,
...
static struct ifa_cacheinfo *address_set_cinfo_from_lifetime(const Address *a, struct ifa_cacheinfo *cinfo) {
        usec_t now_usec;
        ...
        *cinfo = (struct ifa_cacheinfo) {
                .ifa_valid = MIN(usec_sub_unsigned(a->lifetime_valid, now_usec) / USEC_PER_SEC, UINT32_MAX),
                .ifa_prefered = MIN(usec_sub_unsigned(a->lifetime_preferred, now_usec) / USEC_PER_SEC, UINT32_MAX),
        };

        return cinfo;
}

static void address_set_lifetime_from_cinfo(Address *a, const struct ifa_cacheinfo *cinfo) {
        usec_t now_usec;
        ...
        if (cinfo->ifa_valid == UINT32_MAX)
                a->lifetime_valid = USEC_INFINITY;
        else
                a->lifetime_valid = usec_add(cinfo->ifa_valid * USEC_PER_SEC, now_usec);

        if (cinfo->ifa_prefered == UINT32_MAX)
                a->lifetime_preferred = USEC_INFINITY;
        else
                a->lifetime_preferred = usec_add(cinfo->ifa_prefered * USEC_PER_SEC, now_usec);
}
...
const char* format_lifetime(char *buf, size_t l, usec_t lifetime) {
        assert(buf);
        assert(l > 4);
        ...
        assert_se(format_timespan(buf + 4, l - 4, usec_sub_unsigned(lifetime, now(clock_boottime_or_monotonic())), USEC_PER_SEC));
        return buf;
}
...
static void log_address_debug(const Address *address, const char *str, const Link *link) {
        ...
        log_link_debug(link, "%s %s address (%s): %s%s%s/%u (valid %s, preferred %s), flags: %s",
                       ...
                       FORMAT_LIFETIME(address->lifetime_valid),
                       FORMAT_LIFETIME(address->lifetime_preferred),
                       strna(flags_str));
}
...
bool link_address_is_dynamic(const Link *link, const Address *address) {
        ...
        if (address->lifetime_preferred != USEC_INFINITY)
                return true;
...
static int address_configure(
        ...
        r = sd_netlink_message_append_cache_info(req, IFA_CACHEINFO,
                                                 address_set_cinfo_from_lifetime(address, &(struct ifa_cacheinfo) {}));
        if (r < 0)
...
int link_request_address(
        ...
        if (address_get(link, address, &existing) < 0) {
                ...
                existing->lifetime_valid = address->lifetime_valid;
                existing->lifetime_preferred = address->lifetime_preferred;
        }
...
int manager_rtnl_process_address(sd_netlink *rtnl, sd_netlink_message *message, Manager *m) {
        ...
        case RTM_NEWADDR:
                if (address) {
                        ...
                        address_set_lifetime_from_cinfo(address, &cinfo);
                        address_enter_configured(address);
                        log_address_debug(address, "Received updated", link);
                } else {
                        address_set_lifetime_from_cinfo(tmp, &cinfo);
