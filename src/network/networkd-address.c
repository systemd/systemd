...
int link_request_address(
        ...
        if (a->family == AF_INET &&
            a->duplicate_address_detection & ADDRESS_FAMILY_IPV4 &&
            link->hw_addr.length == ETH_ALEN &&
            !ether_addr_is_null(&link->hw_addr.ether) &&
            address_get(link, a, NULL) < 0) {

                r = ipv4acd_configure(link, a, is_static);
                if (r < 0)
                        return r;

                consume_object = false; /* The object is already managed by Link. */
        } else
                consume_object = true;
...
int link_request_static_address(Link *link, Address *address, bool consume) {
        ...
        r = link_request_address(link, address, consume, /* is_static = */ true,
                                 &link->static_address_messages,
...
static int address_is_ready_to_configure(Link *link, Address *address) {
        int r;
