...
static int vl_server(void) {
        ...
        _cleanup_(cgroup_metrics_context_freep) CGroupMetricsContext *ctx = NULL;
        int r;

        ctx = new0(CGroupMetricsContext, 1);
        if (!ctx)
                return log_oom();
...
static int help(void) {
        ...
        r = terminal_urlify_man("systemd-report-cgroup-metrics", "8", &url);
        if (r < 0)
                return log_oom();

        printf("%s [OPTIONS...]\n"
               "\n%sReport cgroup-based metrics for systemd units.%s\n"
               "\n%sOptions:%s\n"
