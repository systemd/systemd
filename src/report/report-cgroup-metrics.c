...
static void cgroup_metrics_context_flush(CGroupMetricsContext *ctx) {
        assert(ctx);
        unit_cgroup_info_array_free(ctx->units, ctx->n_units);
        ctx->units = NULL;
        ctx->n_units = 0;
...
CGroupMetricsContext *cgroup_metrics_context_free(CGroupMetricsContext *ctx) {
        if (!ctx)
                return NULL;
        cgroup_metrics_context_flush(ctx);
        return mfree(ctx);
}

static int query_units(CGroupMetricsContext *ctx, UnitCGroupInfo ***ret, size_t *ret_n) {
        ...
        ctx->units = TAKE_PTR(infos);
        ctx->n_units = TAKE_GENERIC(n_infos, size_t, 0);
...
static int cpu_usage_build_json(MetricFamilyContext *context, void *userdata) {
        CGroupMetricsContext *ctx = ASSERT_PTR(userdata);
        ...
        r = query_units(ctx, &units, &n_units);
        if (r < 0)
                ...
                if (r >= 0) {
                        ...
                        for (char *p = path_buf;;) {
                                ...
                                if (!slash)
                                        break;
                                if (slash == p)
                                        *p = '\0'; /* Check root cgroup next, then break */
                                else
                                        *slash = '\0';
                        }
...
static int ensure_io_stat_cached(UnitCGroupInfo *info) {
        if (info->io_stat_cached)
                return 0;

        int r = io_stat_parse(info->cgroup_path, &info->io_rbytes, &info->io_rios);
        if (r < 0)
                return r;

        info->io_stat_cached = true;
        return 0;
}
