...
static int geoip_data_parse(const char *text, GeoIPData *ret) {
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                { "ip",           _SD_JSON_VARIANT_TYPE_INVALID, json_dispatch_in_addr_data, offsetof(GeoIPData, ip),           SD_JSON_PERMISSIVE },
...
static size_t write_callback(char *buf, size_t size, size_t nmemb, void *userdata) {
        struct iovec_wrapper *answer = ASSERT_PTR(userdata);
        int r;
        ...
        size_t new_size = size_add(iovw_size(answer), nmemb);
        ...
        r = iovw_extend(answer, buf, nmemb);
        if (r < 0) {
                log_debug_errno(r, "Failed to store server answer (%zu bytes): %m", nmemb);
...
static int geoip_fetch(const char *endpoint, char **ret) {
        _cleanup_(iovw_done_free) struct iovec_wrapper answer = {};
        ...
        _cleanup_free_ char *text = iovw_to_cstring(&answer);
        if (!text)
                return -ENOMEM;
...
static int geoip_data_acquire(GeoIPData *ret) {
        ...
        /* Acquires the geolocation data, preferably from the cache if it is recent enough, otherwise from the
         * network. A stale cache is used as fallback if the network is unavailable. This is a "logging"
         * function: on failure it returns 0 and an empty structure, so that the caller simply reports no
         * metrics rather than failing the whole request. */

        const char *endpoint = arg_endpoint ?: GEOIP_ENDPOINT; /* compile-time default, see -Dgeoip-endpoint= */

        _cleanup_free_ char *cache_path = NULL;
        r = geoip_cache_path(&cache_path);
        if (r < 0)
                return log_error_errno(r, "Failed to determine cache file path: %m");
        ...
        if (r < 0)
                log_warning_errno(r, "Failed to take lock on '%s', proceeding without: %m", cache_path);

        usec_t n = now(CLOCK_REALTIME);

        _cleanup_free_ char *cached = NULL;
        ...
        if (r < 0)
                ...
                age = usec_sub_unsigned(n, mtime); /* A cache from the future counts as brand new. */
        ...
        if (r < 0)
                ...
                if (r < 0)
                        ...
                        r = write_string_file(cache_path, fresh,
        ...
        log_notice("No geolocation data available, reporting nothing.");
        *ret = GEOIP_DATA_INIT;
        return 0;
}
