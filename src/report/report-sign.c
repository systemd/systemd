...
static int signature_reply_failed(SignatureList *sl, int r) {
        assert(sl);
...
static int execute_dir_reply(
        ...
        if (r < 0) {
                ...
                return signature_reply_failed(sl, r);
        }
        ...
        if (array) {
                ...
                JSON_VARIANT_ARRAY_FOREACH(s, array) {
                        if (!GREEDY_REALLOC(sl->signatures, sl->n_signatures + 1))
                                return signature_reply_failed(sl, log_oom());
...
int context_sign_report(
        ...
        ssize_t jobs = varlink_execute_directory(
                        ...
                        /* userdata= */ &sl);
        if (jobs < 0) {
                /* Could not open or enumerate the signer directory. Treat as "no signers present":
                 * best-effort  emits an unsigned report, while require-one and require-all fail below. */
                log_warning_errno(jobs, "Failed to execute signing via '%s': %m",
                                REPORT_SIGN_DIR);
                jobs = 0;
        }
