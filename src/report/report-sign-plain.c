...
#define METRIC_IO_SYSTEMD_SIGN_PLAIN_PREFIX "io.systemd.Report.SignPlain."
...
static int write_key_file(int dir_fd, EVP_PKEY *pkey, bool private_key, const char *name, mode_t mode) {
        ...
        if (r < 0) {
                ...
                return log_error_errno(r, "Failed to move key file '%s/%s' into place: %m", REPORT_SIGN_PLAIN_DIR, name);
        }
...
static int vl_method_sign(
        ...
        /* Signing is privileged: we share the Varlink server (and possibly the socket) with the unprivileged
         * metrics interface, so we cannot rely on a server-wide access check and must gate this method
         * ourselves. Only root (i.e. ourselves) may request signatures. */
...
static int public_key_generate(const MetricFamily *mf, sd_varlink *link, void *userdata) {
        int r;

        assert(mf && mf->name);
        assert(link);

        /* Expose the public key the signer uses also as metrics. This has the nice effect that the signature
         * will cover the public key, in the sense of a self-signed certificate. We acquire it through the
         * very same logic as the Sign() method, i.e. we load it off disk, or generate it on first use if it
         * is still missing. */
        _cleanup_(EVP_PKEY_freep) EVP_PKEY *pkey = NULL;
        r = acquire_key(&pkey);
        if (r < 0)
...
static const MetricFamily metric_family_table[] = {
        {
                METRIC_IO_SYSTEMD_SIGN_PLAIN_PREFIX "PublicKey",
                "PEM-encoded public key used to sign system reports with the local software key",
                METRIC_FAMILY_TYPE_STRING,
                .generate = public_key_generate,
        },
        {}
};

static int vl_method_list_metrics(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        return metrics_method_list(metric_family_table, link, parameters, flags, userdata);
}

static int vl_method_describe_metrics(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
...
static int vl_server(void) {
        ...
        /* No server-wide access restriction: the metrics interface is meant to be reachable unprivileged,
         * while the signing method enforces root access on its own (see vl_method_sign()). */
        r = varlink_server_new(&vs, /* flags= */ 0, /* userdata= */ NULL);
        if (r < 0)
                return log_error_errno(r, "Failed to allocate Varlink server: %m");

        /* We serve two interfaces off the same server: the signing API and a metrics provider that exposes our
         * public key. Either is reachable regardless of which socket the connection arrived on. */
