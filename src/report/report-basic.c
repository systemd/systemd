...
static int cpu_usage_generate(const MetricFamily mf[static 2], sd_varlink *link, void *userdata) {
        ...
        static const size_t family[] = { 0, 0, 0, 1, 1 };
        ...
        for (size_t i = 0; i < ELEMENTSOF(values); i++) {
                ...
                /* Split the conversion so that the multiplication cannot wrap before the result itself
                 * exceeds the 64 bit range. */
                uint64_t nsec = values[i] / ticks_per_second * NSEC_PER_SEC +
                        values[i] % ticks_per_second * NSEC_PER_SEC / ticks_per_second;
...
static int disk_io_generate(const MetricFamily mf[static 2], sd_varlink *link, void *userdata) {
        ...
        /* Everything read below comes from sysfs directly, no udev db entry is needed. */
        ...
        FOREACH_DEVICE(e, dev) {
                const char *name, *syspath, *v;

                r = sd_device_get_sysname(dev, &name);
                if (r < 0) {
                        log_device_warning_errno(dev, r, "Failed to get block device name, ignoring: %m");
                        continue;
                }
                ...
                if (!utf8_is_valid(name)) {
                        log_device_debug(dev, "Block device name is not valid UTF-8, ignoring device.");
                        continue;
                }

                /* Virtual devices, which carry no traffic of their own. */
                if (STARTSWITH_SET(name, "loop", "ram", "zram"))
                        continue;

                r = sd_device_get_syspath(dev, &syspath);
                if (r < 0) {
                        log_device_warning_errno(dev, r, "Failed to get syspath, ignoring device: %m");
                        continue;
                }

                _cleanup_free_ char *slaves = path_join(syspath, "slaves");
                if (!slaves)
                        return -ENOMEM;

                r = dir_is_empty(slaves, /* ignore_hidden_or_backup= */ false);
                if (r == 0) /* Stacked device */
                        continue;
                if (r < 0 && r != -ENOENT) {
                        log_device_full_errno(dev, ERRNO_IS_NEG_DEVICE_ABSENT(r) ? LOG_DEBUG : LOG_WARNING,
                                              r, "Failed to check 'slaves/' directory, ignoring device: %m");
                        continue;
                }
                ...
                if (r < 0) {
                        log_device_full_errno(dev, ERRNO_IS_NEG_DEVICE_ABSENT(r) ? LOG_DEBUG : LOG_WARNING,
                                              r, "Failed to read 'stat' attribute, ignoring device: %m");
                        continue;
                }

                /* Fields 3 and 7: the number of sectors read and written. The "stat" attribute always
                 * counts in 512 byte sectors, regardless of the device's logical block size. The skipped
                 * fields are consumed with %*s, since their values may exceed any fixed integer width. */
                unsigned long long read_sectors, write_sectors;
                if (sscanf(v, "%*s %*s %llu %*s %*s %*s %llu", &read_sectors, &write_sectors) < 2) {
                        log_device_warning(dev, "Failed to parse 'stat' attribute, ignoring device.");
                        continue;
                }

                uint64_t bytes[2];
                if (!MUL_SAFE(&bytes[0], (uint64_t) read_sectors, UINT64_C(512)) ||
                    !MUL_SAFE(&bytes[1], (uint64_t) write_sectors, UINT64_C(512))) {
                        log_device_warning(dev, "Sector counters out of range, ignoring device.");
                        continue;
                }

                for (size_t i = 0; i < ELEMENTSOF(bytes); i++) {
                        r = metric_build_send_unsigned(
                                        mf + i,
                                        link,
                                        /* object= */ name,
...
static const MetricFamily metric_family_table[] = {
         ...
         * so CpuUsage is split into two families of different types. */
        {
                ...
                "Aggregate CPU usage across all CPUs in nanoseconds (type=user|system|idle)",
                METRIC_FAMILY_TYPE_COUNTER,
                .generate = cpu_usage_generate,
        },
        {
                METRIC_IO_SYSTEMD_BASIC_PREFIX "CPUInvoluntaryWait",
                "Aggregate involuntary CPU wait across all CPUs in nanoseconds (type=steal|iowait)",
                METRIC_FAMILY_TYPE_GAUGE,
        },
        {
