...
static int disk_io_generate(const MetricFamily mf[static 2], sd_varlink *link, void *userdata) {
        ...
        FOREACH_ARRAY(d, devices, n_devices) {
                ...
                r = path_extract_filename(d->node, &name);
                if (r < 0)
                        return log_error_errno(r, "%s: failed to extract filename: %m", d->node);
                ...
                r = metric_build_send_unsigned(mf, link, /* object= */ name, d->read_bytes, /* fields= */ NULL);
                if (r < 0)
                        return r;
                r = metric_build_send_unsigned(mf + 1, link, /* object= */ name, d->write_bytes, /* fields= */ NULL);
                if (r < 0)
                        return r;
        }
