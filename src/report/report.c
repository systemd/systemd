...
static int parse_argv(int argc, char *argv[], char ***ret_args) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION_LONG("network-timeout", "SEC", "Specify timeout for network upload operation"):
                        r = parse_sec(state.argument, &arg_network_timeout_usec);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --network-timeout value: %s", state.argument);
                        break;
                ...
                OPTION_LONG("extra-header", "NAME: VALUE",
                        ...
                        if (strv_extend(&arg_extra_headers, state.argument) < 0)
                                return log_oom();
                        break;
                }
