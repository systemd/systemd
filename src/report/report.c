...
STATIC_DESTRUCTOR_REGISTER(arg_process, freep);
...
static int parse_argv(int argc, char *argv[], char ***ret_args) {
        ...
        FOREACH_OPTION(&state, c, &arg, /* on_error= */ return c)
                ...
                OPTION_LONG("process", "PATH",
                        ...
                        r = free_and_strdup_warn(&arg_process, arg);
                        if (r < 0)
                                return r;
                        break;
                }
        ...
        if (arg_url && arg_process)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "--url and --process are mutually exclusive.");
