...
static int metrics_call(Context *context, const char *path) {
        ...
        r = sd_varlink_observe(vl,
                               context->action == ACTION_LIST ? "io.systemd.Metrics.List" : "io.systemd.Metrics.Describe",
                               /* parameters= */ NULL);
        if (r < 0)
...
static int metrics_output_list(Context *context, Table **ret) {
        ...
        FOREACH_ARRAY(m, context->metrics, context->n_metrics) {
                ...
                if (r < 0) {
                        ...
                        r = sd_json_variant_format(*m, /* flags= */ 0, &t);
                        if (r < 0)
...
static int parse_metrics_matches(char **matches) {
        ...
        STRV_FOREACH(i, matches) {
                ...
                if (!r && !varlink_idl_interface_name_is_valid(*i))
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Selector is not a valid family name or prefix: %s", *i);
...
static bool test_service_matches(const char *service) {
         ...
         * there the specified metric families are a prefix of the service */
...
static int verb_metrics(int argc, char *argv[], void *userdata) {
        ...
        assert(STR_IN_SET(argv[0], "metrics", "describe-metrics"));
        ...
        if (!d) {
                ...
                FOREACH_DIRENT(de, d,
                        ...
                        if (!test_service_matches(de->d_name))
                                continue;
...
static int verb_list_sources(int argc, char *argv[], void *userdata) {
        ...
        r = readdir_all_at(AT_FDCWD, sources_path, RECURSE_DIR_IGNORE_DOT|RECURSE_DIR_ENSURE_TYPE, &de);
        if (r < 0) {
                if (r != -ENOENT)
                        return log_error_errno(r, "Failed to open sources directory %s: %m", sources_path);
        } else {
                FOREACH_ARRAY(i, de->entries, de->n_entries) {
                        ...
                        if (r < 0) {
                                log_warning_errno(r, "Failed to resolve '%s' skipping.", k);
                                continue;
                        }
