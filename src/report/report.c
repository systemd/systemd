...
static int metrics_query(void) {
        ...
        _cleanup_strv_free_ char **socket_names = NULL;
        size_t n_sockets = 0;
        int r;
        ...
        /* First pass: collect socket names */
        FOREACH_DIRENT(de, d, return -errno) {
                if (!IN_SET(de->d_type, DT_SOCK, DT_UNKNOWN))
                        continue;

                if (!GREEDY_REALLOC0(socket_names, n_sockets + 2))
                        return log_oom();

                socket_names[n_sockets] = strdup(de->d_name);
                if (!socket_names[n_sockets])
                        return log_oom();
                n_sockets++;
        }

        strv_sort(socket_names);
        ...
        /* Second pass: iterate over sorted socket names */
        STRV_FOREACH(name, socket_names) {
                _cleanup_free_ char *p = NULL;

                p = path_join(metrics_path, *name);
                if (!p)
                        return log_oom();
