...
static int mount_collect(Hashmap **file_systems, struct libmnt_fs *fs) {
        ...
        *f = (FileSystem) {
        ...
        file_system_free(hashmap_remove(*file_systems, &f->devno));
...
static int file_system_get_originating_source(const FileSystem *f, char **ret) {
         ...
         * we want the devnum of the backing device node, and not of the anonymous superblock */
        dev_t devno;
        ...
        if (devno == 0) {
                log_debug("Path '%s' has no backing block device, ignoring.", file_system_path(f));
                *ret = NULL;
                return 0;
        }
        ...
        if (r < 0) {
                log_debug_errno(r, "Failed to get block device " DEVNUM_FORMAT_STR " backing '%s', ignoring: %m",
                                DEVNUM_FORMAT_VAL(devno), file_system_path(f));
                *ret = NULL;
                return 0;
        }
...
static int file_system_send(
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to determine originating source: %m");
        ...
        r = sd_json_buildo(
                        ...
                        SD_JSON_BUILD_PAIR_STRING("fileSystemType", sym_mnt_fs_get_fstype(f->fs)));
        if (r < 0)
...
static const MetricFamily diskspace_metric_family_table[] = {
         ...
         * first of them. */
        {
                ...
                "block device backed file systems (object=mount point, fileSystemType=file system type, "
        ...
        },
        {
                ...
                "systems (object=mount point, fileSystemType=file system type, source=backing device, "
