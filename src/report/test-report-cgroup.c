...
/* The two families report_cgroup_pressure_send() expects, in the order it expects them. */
...
static int method_pressure(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        for (unsigned i = 0; i < data->n_units; i++) {
                ...
                /* Mirror walk_cgroups(), which flushes once per unit so that a large scrape doesn't
                 * accumulate in the Varlink output buffer. */
...
static int reply_pressure(sd_varlink *link, sd_json_variant *parameters, const char *error_id, sd_varlink_reply_flags_t flags, void *userdata) {
        ...
        /* With no metric sent at all the sentinel fires instead of a reply, which is precisely what a
         * PSI-less host must produce: no error from the scrape itself, just no pressure series. */
        if (error_id) {
...
TEST(pressure_garbage) {
        /* A file we can't parse is ignored like a missing one: the other resources still come through. */
        run_pressure_test(1, TEST_FILES_GARBAGE_CPU);
}

TEST(pressure_unsupported) {
        /* No pressure attributes at all: no replies, but the scrape itself must still succeed. */
        run_pressure_test(1, TEST_FILES_NONE);
}

TEST(pressure_large_scrape) {
        /* Many more replies than fit the Varlink output buffer, so this only passes if the per-unit
         * flush drains it, without requiring thousands of live cgroups. */
