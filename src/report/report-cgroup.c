...
static int cpu_stat_parse(const char *cgroup_path, uint64_t ret[static 3]) {
        ...
        memcpy(ret, values, sizeof values);
        return 0;
}
...
static int cpu_usage_send(
        ...
        if (r < 0) {
                if (r != -ENOENT)
                        return log_debug_errno(r, "Failed to read %s/%s: %m", path, "cpu.stat");
                return 0;
        }
...
static int io_read_send(
        ...
        if (r < 0) {
                if (r != -ENOENT)
                        return log_debug_errno(r, "Failed to read %s/%s: %m", path, "io.stat");
                return 0;
        }
...
static int memory_usage_send(
        ...
        bool bad[] = {};
        uint64_t current, limit = UINT64_MAX, peak;
        int r;
        ...
        if (r < 0) {
                if (r != -ENOENT)
                        return log_debug_errno(r, "Failed to read %s/%s: %m", path, "memory.current");
        ...
        if (r < 0) {
                if (r != -ENOENT)
                        return log_debug_errno(r, "Failed to read %s/%s: %m", path, "memory.peak");
                bad[2] = true;
        }
...
static int walk_cgroups(
        ...
        if (r >= 0) {
                (void) cpu_usage_send(mf + 0, link, path, unit);
                (void) io_read_send(mf + 1, link, path, unit);
                (void) memory_usage_send(mf + 2, link, path, unit);
                (void) tasks_current_send(mf + 4, link, path, unit);
