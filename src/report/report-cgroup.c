...
static int walk_cgroups_recursive(const char *path, CGroupInfo ***infos, size_t *n_infos) {
        ...
        /* Only collect service cgroups */
        if (endswith(path, ".service")) {
                ...
                *info = (CGroupInfo) {
                ...
                return 0; /* Service cgroups are leaf nodes for our purposes */
        }
...
static int cpu_usage_build_json(MetricFamilyContext *context, void *userdata) {
        ...
        FOREACH_ARRAY(c, cgroups, n_cgroups) {
                ...
                r = cg_get_keyed_attribute((*c)->path, "cpu.stat", STRV_MAKE("usage_usec"), &val);
                if (r < 0)
                        continue;
...
static int ensure_io_stat_cached(CGroupInfo *info) {
        ...
        int r = io_stat_parse(info->path, &info->io_rbytes, &info->io_rios);
        if (r < 0)
                return r;
