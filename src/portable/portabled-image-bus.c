...
/* Give two PortableChange arrays, return a new array that has all elements of the first that are
...
static int normalize_portable_changes(
        ...
        for (size_t i = 0; i < n_changes_detached; ++i) {
                ...
                for (size_t j = 0; j < n_changes_attached; ++j) {
                        char *attached_name = (char *)basename(changes_detached[i].path),
                                *detached_name = (char *)basename(changes_attached[j].path);

                        if (streq(attached_name, detached_name)) {
                                found = true;
                                break;
                        }
                }
