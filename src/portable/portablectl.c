...
static int attach_reattach_image(int argc, char *argv[], const char *method) {
        ...
        if (!strv_isempty(arg_extension_images)) {
                ...
                STRV_FOREACH(p, arg_extension_images) {
        ...
        if (streq(method, "AttachImageWithExtensions") || streq(method, "ReattachImageWithExtensions")) {
                r = sd_bus_message_append(m, "t", flags);
                if (r < 0)
                        return bus_log_create_error(r);
        }
        ...
        if (streq(method, "AttachImage") || streq(method, "AttachImageWithExtensions"))
...
static int help(int argc, char *argv[], void *userdata) {
        ...
        printf("%s [OPTIONS...] COMMAND ...\n\n"
               ...
               "     --extension=PATH         Extend the image with overlays\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        for (;;) {
                ...
                case ARG_EXTENSION: {
                        _cleanup_free_ char *extension_image = NULL;
                        int r;

                        r = parse_path_argument(optarg, false, &extension_image);
                        if (r < 0)
                                return r;
                        r = strv_consume(&arg_extension_images, TAKE_PTR(extension_image));
                        if (r < 0)
                                return log_oom();
                        break;
                }
