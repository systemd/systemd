...
static int install_image_symlink(
        ...
        SET_FOREACH(ext, extension_images) {
        ...
        if (!image_in_search_path(IMAGE_PORTABLE, NULL, image->path) && set_isempty(extension_images)) {
                _cleanup_free_ char *sl = NULL;
...
int portable_attach(
        ...
        _cleanup_hashmap_free_ Hashmap *unit_files = NULL;
        _cleanup_(lookup_paths_free) LookupPaths paths = {};
        _cleanup_(image_unrefp) Image *image = NULL, *ext = NULL;
        _cleanup_set_free_ Set *extension_images = NULL;
        PortableMetadata *item;
        char **p;
        int r;
        ...
        if (strv_length(extension_image_paths) > 0) {
                extension_images = set_new(&image_hash_ops);
                if (!extension_images)
                        return -ENOMEM;
                ...
                STRV_FOREACH(p, extension_image_paths) {
                        ...
                        r = image_find_harder(IMAGE_PORTABLE, *p, NULL, &ext);
                        if (r < 0)
                                return r;

                        r = set_consume(extension_images, TAKE_PTR(ext));
                        if (r < 0)
                                return r;
                }
        }
