...
static int extract_image_and_extensions(
        ...
        ORDERED_HASHMAP_FOREACH(ext, extension_images) {
                ...
                if (validate_extension) {
                        r = extension_release_validate(ext->path, id, NULL, version_id, sysext_level, "portable", extension_release, IMAGE_SYSEXT);
                        if (r < 0)
                                r = extension_release_validate(ext->path, id, NULL, version_id, confext_level, "portable", extension_release, IMAGE_CONFEXT);
