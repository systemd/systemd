...
static int attach_unit_file(
        ...
        if ((flags & PORTABLE_PREFER_SYMLINK) && m->source) {
                ...
                r = mac_selinux_create_file_prepare_label(path, m->selinux_label);
                if (r < 0)
                        log_debug_errno(r, "Failed to set SELinux security context '%s' for '%s', ignoring: %m", m->selinux_label, path);
