...
int portable_detach(
        ...
        FOREACH_DIRENT(de, d, return log_debug_errno(errno, "Failed to enumerate '%s' directory: %m", where)) {
                ...
                /* "portablectl --enable detach" calls DisableUnitFilesWithFlags DBus method, and the main
                 * unit file is removed, but its drop-ins are not. Hence, here we need to list both main unit
                 * files and drop-in directories (without the main unit files). */
