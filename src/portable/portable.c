...
static int extract_now(
        ...
        if (r == -ENOENT) {
                os_release_file_path = strjoina("/usr/lib/extension-release.d/extension-release.", image_name);
                r = open_extension_release(where, image_name, &os_release_path, &os_release_fd);
        }
        if (r < 0)
                log_debug_errno(r, "Couldn't acquire os-release/extension-release file, ignoring: %m");
        else {
                if (socket_fd >= 0) {
                        r = send_item(socket_fd, os_release_file_path, os_release_fd);
                        if (r < 0)
                                return log_debug_errno(r, "Failed to send os-release file: %m");
                }

                if (ret_os_release) {
                        os_release = portable_metadata_new(os_release_file_path, NULL, os_release_fd);
                        if (!os_release)
                                return -ENOMEM;
...
static int portable_extract_by_path(
        ...
        if (r == -EISDIR) {
                ...
                r = extract_now(path, matches, NULL, -1, &os_release, &unit_files);
                if (r < 0)
                        return r;
                ...
                if (r == 0) {
                        ...
                        r = dissected_image_mount(m, tmpdir, UID_INVALID, UID_INVALID, DISSECT_IMAGE_READ_ONLY|DISSECT_IMAGE_VALIDATE_OS);
                        if (r < 0) {
                                log_debug_errno(r, "Failed to mount dissected image: %m");
                                goto child_finish;
                        }

                        r = extract_now(tmpdir, matches, m->image_name, seq[1], NULL, NULL);
        ...
        if (!os_release)
                return sd_bus_error_setf(error, SD_BUS_ERROR_INVALID_ARGS, "Image '%s' lacks os-release/extension-release data, refusing.", path);
...
int portable_attach(
        ...
        r = portable_extract_by_path(image->path, true, matches, ordered_hashmap_isempty(extension_images) ? NULL : &os_release, &unit_files, error);
        if (r < 0)
                return r;
        ...
        ORDERED_HASHMAP_FOREACH(ext, extension_images) {
                ...
                r = portable_extract_by_path(ext->path, false, matches, &extension_release_meta, &extra_unit_files, error);
                if (r < 0)
                        return r;
