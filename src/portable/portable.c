...
/* 'output=' is always appended to, even if there are ENOMEM errors */
static int append_release_info_to_unit(char **output, const PortableMetadata *release, ImageClass type) {
        static const char *const field_names[_IMAGE_CLASS_MAX][6]= {
                 {}, /* IMAGE_MACHINE is not expected here, the assert below enforces it */
                 { "ID", "VERSION_ID", "IMAGE_VERSION", "IMAGE_ID", "BUILD_ID", NULL },
                 { "SYSEXT_ID", "SYSEXT_VERSION_ID", "SYSEXT_IMAGE_VERSION", "SYSEXT_IMAGE_ID", "SYSEXT_BUILD_ID", NULL },
        };
        ...
        if (type == IMAGE_EXTENSION) {
                ...
                r = path_extract_filename(release->name, &basename);
                if (r < 0)
                        return log_debug_errno(r, "Failed to extract basename from '%s': %m", release->name);

                image_name = startswith(basename, "extension-release.");
                /* The extension-release filename should always start with extension-release. as we enforce
                 * this much earlier, if we get here and it's not set there's a logic error somewhere up the
                 * stack. */
                assert(image_name);

                prefix = strjoina(image_name, ":");
        } else
                prefix = "";
        ...
        STRV_FOREACH_PAIR(key, value, fields) {
                if (!strv_find((char *const *)field_names[type], *key))
                        continue;

                if (!strextend(output, "LogExtraFields=",
                               *key,
                               "=",
                               prefix,
                               *value,
...
static int install_chroot_dropin(
        ...
        if (endswith(m->name, ".service")) {
                ...
                r = append_release_info_to_unit(&text, os_release, IMAGE_PORTABLE);
                if (r < 0)
                        return r;
                ...
                if (m->image_path && !path_equal(m->image_path, image_path))
                        ORDERED_HASHMAP_FOREACH(ext, extension_images) {
                                ...
                                PortableMetadata *extension_release = ordered_hashmap_get(extension_releases,
                                                                                          ext->name);
                                r = append_release_info_to_unit(&text,
                                                                extension_release,
                                                                IMAGE_EXTENSION);
                                if (r < 0)
                                        return r;
