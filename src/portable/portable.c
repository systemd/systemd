...
static int install_image(
        ...
        r = image_storage_path(image_path, flags, &target);
        if (r < 0)
...
int portable_detach(
        ...
        SET_FOREACH(item, markers) {
                ...
                r = image_storage_path(item, flags, &target);
                if (r < 0) {
                ...
                if (lstat(target, &st) < 0) {
                        log_debug_errno(errno, "Failed to stat '%s', ignoring: %m", target);
                        continue;
                }
