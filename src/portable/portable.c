...
static int portable_extract_by_path(
        ...
        /* Note that we don't check here if we found any units, as we might be looking at
         * a base OS layer, before opening the extension images. Let the caller do that
...
int portable_extract(
        ...
        r = portable_extract_by_path(image->path, false, matches, &os_release, &unit_files, error);
        if (r < 0)
                return r;
        ...
        ORDERED_HASHMAP_FOREACH(ext, extension_images) {
                ...
                r = portable_extract_by_path(ext->path, true, matches, NULL, &extra_unit_files, error);
                if (r < 0)
                        return r;
...
int portable_attach(
        ...
        r = portable_extract_by_path(image->path, false, matches, ordered_hashmap_isempty(extension_images) ? NULL : &os_release, &unit_files, error);
        if (r < 0)
                return r;
        ...
        ORDERED_HASHMAP_FOREACH(ext, extension_images) {
                ...
                r = portable_extract_by_path(ext->path, true, matches, &extension_release_meta, &extra_unit_files, error);
                if (r < 0)
                        return r;
