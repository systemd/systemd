...
int portable_detach(
        ...
        FOREACH_DIRENT(de, d, return log_debug_errno(errno, "Failed to enumerate '%s' directory: %m", where)) {
                ...
                if (dot && laccess(unit_name, F_OK) >= 0)
                        continue; /* The main unit file exists, skipping. */
                ...
                if (!dot) {
                        r = unit_file_lookup_state(LOOKUP_SCOPE_SYSTEM, &paths, unit_name, &state);
                        if (r < 0)
                        ...
                        if (!IN_SET(state, UNIT_FILE_STATIC, UNIT_FILE_DISABLED, UNIT_FILE_LINKED, UNIT_FILE_RUNTIME, UNIT_FILE_LINKED_RUNTIME))
                                return sd_bus_error_setf(error, BUS_ERROR_UNIT_EXISTS, "Unit file '%s' is in state '%s', can't detach.", unit_name, unit_file_state_to_string(state));
                }
