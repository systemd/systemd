...
static PortableMetadata *portable_metadata_new(
                ...
                PortableMetatadaSignatureType signature_type,
                int signature_fd,
                int fd) {

        PortableMetadata *m;

        assert(signature_type >= 0);
...
static int extract_now(
        ...
        STRV_FOREACH(i, paths.search_path) {
                ...
                FOREACH_DIRENT(de, d, return log_debug_errno(errno, "Failed to read directory: %m")) {
                        PortableMetatadaSignatureType signature_type = PORTABLE_METADATA_SIGNATURE_TYPE_NONE;
                        ...
                        if (socket_fd >= 0) {
                                struct iovec iov[] = {
                                        ...
                                        IOVEC_MAKE((char *)"\0", sizeof(char)),
                                        IOVEC_MAKE((PortableMetatadaSignatureType *)&signature_type, sizeof(PortableMetatadaSignatureType)),
...
static int portable_extract_by_path(
        ...
        if (r == -EISDIR) {
                ...
                for (;;) {
                        ...
                        if (n > selinux_label + sizeof(PortableMetatadaSignatureType) - (char *)iov.iov_base) {
                                signature_type = *(PortableMetatadaSignatureType *) ++selinux_label;
                                selinux_label += sizeof(PortableMetatadaSignatureType);
