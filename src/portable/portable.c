...
static int install_profile_dropin(
        ...
        if (flags & PORTABLE_PREFER_COPY) {
                ...
                if (r < 0)
                        return log_debug_errno(r, "Failed to copy %s %s %s: %m", from, special_glyph(SPECIAL_GLYPH_ARROW_RIGHT, /* force_utf8= */ false), dropin);
                ...
                if (symlink(from, dropin) < 0)
                        return log_debug_errno(errno, "Failed to link %s %s %s: %m", from, special_glyph(SPECIAL_GLYPH_ARROW_RIGHT, /* force_utf8= */ false), dropin);
...
static int install_image_symlink(
        ...
        if (symlink(image_path, sl) < 0)
                return log_debug_errno(errno, "Failed to link %s %s %s: %m", image_path, special_glyph(SPECIAL_GLYPH_ARROW_RIGHT, /* force_utf8= */ false), sl);
