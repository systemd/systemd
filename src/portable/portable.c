...
static int extract_now(
         ...
         * unit discovery logic. We look for units in the system scope regardless of whether portabled is
         * running as a user or system service, as the units in the portable image will always be in
         * /usr/lib/systemd/system instead of in user specific unit file directories. Note that we force
         * looking inside of /lib/systemd/system/ for units too, as the image might have a legacy split-usr
         * layout. */
        r = lookup_paths_init(&paths, RUNTIME_SCOPE_SYSTEM, LOOKUP_PATHS_SPLIT_USR, /* root_dir= */ NULL);
        if (r < 0)
...
int portable_attach(
        ...
        HASHMAP_FOREACH(item, unit_files) {
                r = attach_unit_file(scope, &paths, image->path, image->type, extension_images, extension_releases,
