...
static int vl_method_enroll(
        ...
        /* Figure out the unlock method from which of the unlock* fields is set; at most one may be given. */
        unsigned n_unlock = 0;
        ...
        case ENROLL_FIDO2:
                ...
                if (strdup_to(&c.fido2_device, p.fido2_device) < 0)
                        return -ENOMEM;
                if (!c.fido2_device) {
        ...
        return sd_varlink_replybo(
                        ...
                        JSON_BUILD_PAIR_STRING_NON_EMPTY("recoveryKey", recovery_key));
}
...
static int vl_method_list_slots(
        ...
        FOREACH_ARRAY(s, slots, n_slots) {
                /* enroll_mechanism_to_string() returns NULL for unrecognized types; conflicts are reported
                ...
                r = sd_varlink_notifybo(
                                ...
                                JSON_BUILD_PAIR_ENUM_NON_EMPTY("type", type));
                if (r < 0)
                        return r;
        }
...
int cryptenroll_varlink_server(void) {
        ...
        r = varlink_server_new(
                        &varlink_server,
                        SD_VARLINK_SERVER_ROOT_ONLY|SD_VARLINK_SERVER_MYSELF_ONLY|SD_VARLINK_SERVER_INPUT_SENSITIVE|SD_VARLINK_SERVER_INHERIT_USERDATA|SD_VARLINK_SERVER_ALLOW_FD_PASSING_INPUT,
                        &polkit_registry);
        if (r < 0)
