...
static bool arg_fido2_token_parameters = true;
...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] [BLOCK-DEVICE]\n\n"
               ...
               "     --fido2-token-parameters=BOOL\n"
               "                       Whether to store FIDO2 parameters in a LUKS2 token\n"
...
static int parse_argv(int argc, char *argv[]) {
        ...
        static const struct option options[] = {
                ...
                { "fido2-token-parameters",        required_argument, NULL, ARG_FIDO2_TOKEN_PARAMETERS },
        ...
        while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0) {
                ...
                case ARG_FIDO2_TOKEN_PARAMETERS:
                        r = parse_boolean_argument("--fido2-token-parameters=", optarg, &arg_fido2_token_parameters);
                        if (r < 0)
                                return r;
        ...
        if (arg_enroll_type == ENROLL_FIDO2) {
                ...
                if (!arg_fido2_token_parameters && !arg_fido2_salt_file)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "FIDO2 device with token parameters disabled was specified, but no salt file provided, refusing.");
...
static int run(int argc, char *argv[]) {
        ...
        case ENROLL_FIDO2:
                slot = enroll_fido2(cd, vk, vks, arg_fido2_device, arg_fido2_lock_with, arg_fido2_cred_alg, arg_fido2_salt_file, arg_fido2_token_parameters);
                break;
