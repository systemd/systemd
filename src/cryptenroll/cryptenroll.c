...
static uint64_t arg_fido2_salt_file_offset = 0;
static bool arg_fido2_manual_mode = false;
...
static int help(void) {
        ...
        printf("%1$s [OPTIONS...] [BLOCK-DEVICE]\n\n"
               ...
               "     --fido2-salt-file-offset=BYTES\n"
               "                       Number of bytes to skip at the start of the salt file\n"
               "     --fido2-manual-mode=BOOL\n"
               "                       Do not store FIDO2 parameters in a LUKS2 token\n"
...
static int parse_argv(int argc, char *argv[]) {
        enum {
                ...
                ARG_FIDO2_DEVICE,
                ARG_FIDO2_SALT_FILE,
                ARG_FIDO2_SALT_FILE_OFFSET,
                ARG_FIDO2_MANUAL_MODE,
                ARG_TPM2_DEVICE,
                ARG_TPM2_DEVICE_KEY,
        ...
        static const struct option options[] = {
                ...
                { "fido2-salt-file-offset",        required_argument, NULL, ARG_FIDO2_SALT_FILE_OFFSET },
                { "fido2-manual-mode",             required_argument, NULL, ARG_FIDO2_MANUAL_MODE      },
        ...
        while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0) {
                ...
                case ARG_FIDO2_SALT_FILE_OFFSET:
                        r = safe_atou64(optarg, &arg_fido2_salt_file_offset);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse FIDO2 salt file offset: %s", optarg);

                        break;

                case ARG_FIDO2_MANUAL_MODE:
                        r = parse_boolean_argument("--fido2-manual-mode=", optarg, &arg_fido2_manual_mode);
                        if (r < 0)
                                return r;
        ...
        if (arg_enroll_type == ENROLL_FIDO2) {
                ...
                if (arg_fido2_manual_mode && !arg_fido2_salt_file)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "FIDO2 device with manual parameters selected, but no salt file specified, refusing.");
...
static int run(int argc, char *argv[]) {
        ...
        case ENROLL_FIDO2:
                slot = enroll_fido2(cd, vk, vks, arg_fido2_device, arg_fido2_lock_with, arg_fido2_cred_alg, arg_fido2_salt_file, arg_fido2_salt_file_offset, arg_fido2_manual_mode);
                break;
