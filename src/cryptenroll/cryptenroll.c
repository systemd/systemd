...
static COSEAlgorithm arg_fido2_cred_alg = COSE_ALG_ES256;
        ...
        while ((c = getopt_long(argc, argv, "h", options, NULL)) >= 0) {
                ...
                case ARG_FIDO2_CRED_ALG: {
                        if (streq(optarg, "ES256"))
                                arg_fido2_cred_alg = COSE_ALG_ES256;
                        else if(streq(optarg, "RS256"))
                                arg_fido2_cred_alg = COSE_ALG_RS256;
                        else if(streq(optarg, "EDDSA"))
                                arg_fido2_cred_alg = COSE_ALG_EDDSA;
                        else
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Unsupported COSE algorithm: %s", optarg);
                        break;
