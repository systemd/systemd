...
static int parse_argv(int argc, char *argv[]) {
        ...
        FOREACH_OPTION_OR_RETURN(c, &opts)
                ...
                OPTION_LONG("tpm2-with-pin", "BOOL|direct",
                        ...
                        bool b;
                        if (streq(opts.arg, "direct")) {
                                arg_tpm2_pin = 2;
                                break;
                        }
                        r = parse_boolean_argument("--tpm2-with-pin=", opts.arg, &b);
                        if (r < 0)
                                return r;
                        arg_tpm2_pin = b ? 1 : 0;
                        break;
...
static void argon2id_parameters_benchmark_iterations(Argon2IdParameters *p, usec_t target_time) {
        ...
        if (elapsed == 0)
                ...
                p->iterations = CLAMP((uint64_t) target_time * 2 / elapsed, 2U, 10000U);
        ...
        log_notice("Argon2id benchmark: using %u iterations for ~%"PRIu64"ms unlock time with %"PRIu64" MiB. "
                   ...
                   p->iterations,
                   (uint64_t) elapsed * p->iterations / 2 / USEC_PER_MSEC,
...
static int run(int argc, char *argv[]) {
        ...
        if (arg_tpm2_pin == 1 && !dlopen_libcrypto_has_argon2id()) {
