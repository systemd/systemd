...
static int parse_argv(int argc, char *argv[]) {
        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION_LONG("wipe-slot", "SLOT1,SLOT2,…",
                        ...
                        r = parse_wipe_slot(state.argument);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("unlock-key-file", "PATH",
                        ...
                        r = parse_path_argument(state.argument, /* suppress_root= */ true, &arg_unlock_keyfile);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("pkcs11-token-uri", "URI|auto|list",
                        ...
                        if (streq(state.argument, "auto")) {
                                ...
                                if (!pkcs11_uri_valid(state.argument))
                                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Not a valid PKCS#11 URI: %s", state.argument);

                                uri = strdup(state.argument);
                                if (!uri)
                                        return log_oom();
                        }
                ...
                OPTION_LONG("fido2-salt-file", "PATH",
                        ...
                        r = parse_path_argument(state.argument, /* suppress_root= */ true, &arg_fido2_salt_file);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("fido2-parameters-in-header", "BOOL",
                        ...
                        r = parse_boolean_argument("--fido2-parameters-in-header=", state.argument, &arg_fido2_parameters_in_header);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("fido2-credential-algorithm", "STRING",
                        ...
                        r = parse_fido2_algorithm(state.argument, &arg_fido2_cred_alg);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse COSE algorithm: %s", state.argument);
                        break;

                OPTION_LONG("fido2-with-client-pin", "BOOL",
                            "Whether to require entering a PIN to unlock the volume"):
                        r = parse_boolean_argument("--fido2-with-client-pin=", state.argument, NULL);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("fido2-with-user-presence", "BOOL",
                        ...
                        r = parse_boolean_argument("--fido2-with-user-presence=", state.argument, NULL);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("fido2-with-user-verification", "BOOL",
                        ...
                        r = parse_boolean_argument("--fido2-with-user-verification=", state.argument, NULL);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("tpm2-with-pin", "BOOL",
                            "Whether to require entering a PIN to unlock the volume"):
                        r = parse_boolean_argument("--tpm2-with-pin=", state.argument, &arg_tpm2_pin);
                        if (r < 0)
                                return r;
                        break;
                }
