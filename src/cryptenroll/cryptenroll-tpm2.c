...
int enroll_tpm2(struct crypt_device *cd,
        ...
        if (use_fido2) {
                r = get_fido2_secret(cd, fido2_device, fido2_lock_with, fido2_cred_alg, fido2_salt_file, &fido2_salt, &fido2_cid, &fido2_secret_str, &fido2_lock_with);
                if (r < 0)
                        return r;

                flags = flags | TPM2_FLAGS_USE_FIDO2;
        }
