...
int enroll_tpm2(const EnrollContext *c,
        ...
        if (c->tpm2_fido2) {
                r = get_fido2_secret(cd, c->fido2_device, c->fido2_pin, c->interactive ? 0 : ASK_PASSWORD_HEADLESS, c->fido2_lock_with, c->fido2_cred_alg, c->fido2_salt_file, &fido2_salt, &fido2_cid, &fido2_secret_str, &fido2_lock_with);
                if (r < 0)
                        return r;
