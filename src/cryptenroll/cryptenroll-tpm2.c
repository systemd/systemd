...
static int search_policy_hash(
        ...
        /* Searches among the already enrolled TPM2 tokens for one that matches the exact set of policies
         * specified.
         *
         * Only used for TPM2 enrollments without FIDO2 tokens, as the policy hash of those can be the same
         * as that of a TPM2+PIN one. Also, because the hash carries no further information about the FIDO2
         * token, it cannot differentiate between tokens. */
        ...
        for (int token = 0; token < sym_crypt_token_max(CRYPT_LUKS2); token++) {
                ...
                if (luks2_token_type_from_json(v) == ENROLL_TPM2_WITH_FIDO2)
                        continue;
...
static int generate_fido2_secret(
        ...
        _cleanup_(erase_and_freep) char *base64_encoded = NULL, *used_pin = NULL;
        ...
        *ret_secret_str = TAKE_PTR(base64_encoded);
        if (ret_used_pin)
                *ret_used_pin = TAKE_PTR(used_pin);
...
int enroll_tpm2(const EnrollContext *c,
        ...
        _cleanup_(erase_and_freep) char *fido2_used_pin = NULL;
        ...
        if (IN_SET(c->tpm2_pin, TPM2_WITH_PIN_YES, TPM2_WITH_PIN_DIRECT)) {
                /* Use a single PIN for the FIDO2 and the TPM2.  Helps with a limitation of the libcryptsetup
                 * API. */
                if (fido2_used_pin) {
                        pin_str = TAKE_PTR(fido2_used_pin);
                        flags |= TPM2_FLAGS_USE_PIN;
                } else {
                        TPM2Flags pin_flags;

                        r = get_pin(&pin_str, &pin_flags);
                        if (r < 0)
                                return r;
        ...
        r = c->tpm2_fido2 ? -ENOENT : search_policy_hash(cd, policy_hash_as_iovec, n_policy_hash);
        if (r == -ENOENT)
