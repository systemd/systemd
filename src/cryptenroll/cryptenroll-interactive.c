...
int cryptenroll_run_interactive(EnrollContext *c, unsigned prompt_suppress_mask) {
        ...
        if (existing_mask == 0) {
                log_debug("No recognized LUKS slot recognized, we're unlikely able to unlock, skipping interactive setup.");
                return 0;
        }
        ...
        if (emoji_enabled())
                printf("%s %sLet's enroll additional disk encryption mechanisms for recovering access to the system.%s\n\n",
                       glyph(GLYPH_COMPUTER_DISK), ansi_highlight(), ansi_normal());
        else
                printf("%sLet's enroll additional disk encryption mechanisms for recovering access to the system.%s\n\n",
