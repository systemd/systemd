...
int enroll_pkcs11(
        ...
        /* The URI provided by a user should point to a certificate, and can explicitly declare this type.
         * To unlock a volume we need an URI that points to a private key, so we adjust the type (if present)
         * accordingly. */
        ...
        r = json_build(&v,
                JSON_BUILD_OBJECT(
                        ...
                        JSON_BUILD_PAIR("pkcs11-uri", JSON_BUILD_STRING((private_uri)? private_uri : uri)),
