...
static void device_broadcast(sd_device_monitor *monitor, sd_device *dev, int result) {
        ...
        if (result != 0) {
                assert(result != EVENT_RESULT_TRY_AGAIN);
                ...
                } else if (result <= EVENT_RESULT_EXIT_STATUS_MAX)
                        (void) device_add_propertyf(dev, "UDEV_WORKER_EXIT_STATUS", "%i", result);
                else {
        ...
        for (;;) {
                ...
                if ((!WIFEXITED(status) || WEXITSTATUS(status) != 0) && worker->event) {
                        ...
                        device_broadcast(manager->monitor, worker->event->dev,
                                         WIFEXITED(status) ? WEXITSTATUS(status) : EVENT_RESULT_SIGNAL_BASE + WTERMSIG(status));
                }
