...
static int manager_parse_udev_config(Manager *manager) {
        int log_val = -1;
        Manager *m = ASSERT_PTR(manager); /* A hack to check the ptr before using it in the table */
        const ConfigTableItem config_table[] = {
                { NULL, "udev_log",       config_parse_log_level,           0, &log_val },
                { NULL, "children_max",   config_parse_unsigned,            0, &m->children_max },
                { NULL, "exec_delay",     config_parse_sec,                 0, &m->exec_delay_usec },
                { NULL, "event_timeout",  config_parse_sec,                 0, &m->timeout_usec },
                { NULL, "resolve_names",  config_parse_resolve_name_timing, 0, &m->resolve_name_timing },
                { NULL, "timeout_signal", config_parse_signal,              0, &m->timeout_signal },
                {}
        };
        int r;
