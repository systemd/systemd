...
typedef enum EventResult {
        ...
        EVENT_RESULT_UNKNOWN          = 257,
        EVENT_RESULT_SIGNAL_BASE      = 258,
...
static int on_sigchld(sd_event_source *s, const siginfo_t *si, void *userdata) {
        ...
        case CLD_TRAPPED:
                log_device_info(dev, "Worker ["PID_FMT"] trapped.", si->si_pid);
                return 0;

        case CLD_STOPPED:
                log_device_info(dev, "Worker ["PID_FMT"] stopped.", si->si_pid);
                return 0;

        case CLD_CONTINUED:
                log_device_info(dev, "Worker ["PID_FMT"] continued.", si->si_pid);
                return 0;

        default:
                log_device_warning(dev, "Worker ["PID_FMT"] failed due to unknown reason (%i).",
                                   si->si_pid, si->si_code);
                result = EVENT_RESULT_UNKNOWN;
        }
