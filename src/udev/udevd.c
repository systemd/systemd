...
static int event_queue_insert(Manager *manager, sd_device *d) {
        ...
        for (;;) {
                ...
                if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
                        if (worker->event) {
                                ...
                                if (r < 0)
                                        log_device_error_errno(worker->event->dev_kernel->device, r, "Failed to send device: %m");
                        }
                }
