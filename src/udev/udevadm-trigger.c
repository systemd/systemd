...
static int exec_list(
        ...
        FOREACH_DEVICE_AND_SUBSYSTEM(e, d) {
                ...
                if (arg_settle) {
                        if (uuid_supported) {
                                _cleanup_free_ sd_id128_t *dup = NULL;

                                dup = newdup(sd_id128_t, &id, 1);
                                if (!dup)
                                        return log_oom();

                                r = set_ensure_consume(&settle_path_or_ids, &id128_hash_ops_free, TAKE_PTR(dup));
                        } else {
                                ...
                                r = set_ensure_consume(&settle_path_or_ids, &path_hash_ops_free, TAKE_PTR(dup));
                        }
                        if (r < 0)
                                return log_oom();
                }
