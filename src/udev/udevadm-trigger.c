...
static int exec_list(
        ...
        FOREACH_DEVICE_AND_SUBSYSTEM(e, d) {
                ...
                r = sd_device_trigger_with_uuid(d, action, arg_uuid || settle_hashmap ? &id : NULL);
                if (r == -EINVAL && !arg_uuid && settle_hashmap) {
...
static int device_monitor_handler(sd_device_monitor *m, sd_device *dev, void *userdata) {
        ...
        if (!sd_id128_is_null(*settle_id)) { /* If this is SD_ID128_NULL then we are on pre-4.13 and have no UUID to check, hence don't */
                ...
                if (r < 0) {
                        log_debug("Got uevent without synthetic UUID for device '%s', ignoring.", syspath);
                        return 0;
                }
