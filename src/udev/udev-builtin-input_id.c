...
static bool test_key(sd_device *dev,
        ...
        for (size_t i = 0; i < BTN_MISC/BITS_PER_LONG; ++i) {
                if (bitmask_key[i]) {
                        found = true;
                        goto done;
                }
        ...
        for (size_t block = 0; block < sizeof(high_key_blocks) / sizeof(struct range); block++)
                for (unsigned i = high_key_blocks[block].start; i < high_key_blocks[block].end; i++)
