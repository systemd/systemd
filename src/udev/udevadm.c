...
UDEVADM_VERB(verb_info_main,    "info",         "[DEVPATH|FILE]",   "Query sysfs or the udev database");
UDEVADM_VERB(verb_trigger_main, "trigger",      "DEVPATH",          "Request events from the kernel");
UDEVADM_VERB(verb_settle_main,  "settle",       NULL,               "Wait for pending udev events");
UDEVADM_VERB(verb_control_main, "control",      "OPTION",           "Control the udev daemon");
UDEVADM_VERB(verb_monitor_main, "monitor",      NULL,               "Listen to kernel and udev events");
UDEVADM_VERB(verb_test_main,    "test",         "DEVPATH",          "Test an event run");
UDEVADM_VERB(verb_builtin_main, "test-builtin", "COMMAND DEVPATH",  "Test a built-in command");
UDEVADM_VERB(verb_verify_main,  "verify",       "[FILE…]",          "Verify udev rules files");
UDEVADM_VERB(verb_cat_main,     "cat",          "[FILE…]",          "Show udev rules files");
UDEVADM_VERB(verb_wait_main,    "wait",         "DEVICE [DEVICE…]", "Wait for device or device symlink");
UDEVADM_VERB(verb_lock_main,    "lock",         "COMMAND",          "Lock a block device");
