...
static int link_config_apply_ethtool_settings(int *ethtool_fd, const link_config *config, sd_device *device) {
        ...
        if (r < 0) {
                ...
                if (config->autonegotiation >= 0)
                        log_device_warning_errno(device, r, "Could not %s auto negotiation, ignoring: %m", enable_disable(config->autonegotiation));
                ...
                if (config->port >= 0)
                        log_device_warning_errno(device, r, "Could not set port to '%s', ignoring: %m", port_to_string(config->port));
