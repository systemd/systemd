...
static const char *const link_dirs[] = { "/etc/systemd/network",
                                         "/run/systemd/network",
...
int link_config_get(link_config_ctx *ctx, sd_device *device, link_config **ret) {
        ...
        LIST_FOREACH(links, link, ctx->links) {
                ...
                if (net_match_config(link->match_mac,
                                     link->match_path,
                                     link->match_driver,
                                     link->match_type,
                                     link->match_name,
                                     link->match_host,
                                     link->match_virt,
                                     link->match_kernel_cmdline,
                                     link->match_kernel_version,
                                     link->match_arch,
                        ...
                        if (link->match_name) {
                                ...
                                if (name_assign_type == NET_NAME_ENUM) {
                                        log_warning(
                                                "Config file %s applies to device based on potentially unpredictable interface name '%s'",
                                                link->filename,
                                                sysname);
                                        *ret = link;
                        ...
                        log_debug("Config file %s applies to device %s", link->filename, sysname);
...
int link_config_apply(link_config_ctx *ctx, link_config *config, sd_device *device, const char **name) {
...
static const char *const mac_policy_table[_MACPOLICY_MAX] = { [MACPOLICY_PERSISTENT] = "persistent",
