...
static int link_get_device_numa_node(Link *link, unsigned *ret) {
        ...
        r = sd_device_get_sysattr_value(link->event->dev, "device/numa_node", &content);
        if (r < 0)
                return r;
...
static int discover_cpu_topology(CPUTopology **ret, size_t *ret_count) {
        ...
        FOREACH_DEVICE_CHILD_WITH_SUFFIX(parent_node, cpu_node, name) {
                ...
                r = safe_atou(n, &cpu);
                if (r < 0) {
                        log_debug_errno(r, "Failed to convert %s to unsigned, skipping", n);
                        continue;
                }
                ...
                if (r < 0) {
                        log_debug_errno(r, "Failed to get NUMA node for CPU %u, ignoring: %m", cpu);
                        cpus[count].numa_node = 0;
                }

                r = device_get_sysattr_unsigned(cpu_node, "topology/physical_package_id", &cpus[count].package_id);
                if (r < 0) {
                        log_device_debug_errno(cpu_node, r, "Failed to get physical_package_id value: %m");
                ...
                if (r < 0) {
                        log_device_debug_errno(cpu_node, r, "Failed to get core_id value: %m");
...
static void equidist_permute(size_t *indices, size_t n_indices) {
        ...
        if (!left || !right) {
                log_debug("Failed to allocate memory, skipping equidistant permutation");
                return;
        }
...
static int link_apply_irq_affinity_spread(Link *link, const CPUSet *allowed_cpus) {
        ...
        if (r < 0) {
                ...
                log_link_debug_errno(link, r, "No MSI IRQs found, skipping IRQ affinity configuration.");
                return 0;
        }
...
static int link_apply_irq_affinity_single(Link *link, const CPUSet *allowed_cpus) {
        ...
        FOREACH_DIRENT(de, dir, return log_link_error_errno(link, errno, "Failed to read directory device/msi_irqs: %m"))
                (void) set_irq_affinity(link, de->d_name, target_cpu);
...
static int link_apply_irq_affinity(Link *link) {
        ...
        if (link->config->irq_affinity_numa != IRQ_AFFINITY_NUMA_UNSET) {
                ...
                if (r < 0) {
                        log_link_warning_errno(link, r,
                                "Failed to get CPUs for NUMA node %u, skipping IRQ affinity configuration.", numa_node);
                        return 0;
                }
