...
static int find_gpt_root(UdevEvent *event, blkid_probe pr, const char *loop_backing_fname) {
        ...
        if (r < 0)
                return log_device_debug_errno(dev, r, "Failed to query devtype from device: %m");

        if (!streq(devtype, "disk")) {
                log_device_debug_errno(dev, r, "Skipping GPT root logic on partition block device.");
                return 0;
        }
