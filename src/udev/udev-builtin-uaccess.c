...
static int builtin_uaccess(UdevEvent *event, int argc, char *argv[]) {
        ...
        if (r > 0) {
                ...
                if (r < 0) {
                        ...
                        if (set_ensure_put(&uids, &trivial_hash_ops, UID_TO_PTR(uid)) < 0)
                                return log_device_error_errno(dev, r, "Failed to construct UID set: %m");
                }
        }
        ...
        if (r > 0) {
                ...
                STRV_FOREACH(s, sessions) {
                        ...
                        if (sd_session_get_state(*s, &state) < 0) {
                                log_device_error_errno(dev, r, "Failed to query state for session %s: %m", *s);
                                continue;
                        }
                        ...
                        if (r < 0) {
                                log_device_error_errno(dev, r, "Failed to query extra device access for session %s: %m", *s);
                                continue;
                        }
                        if (r == 0)
                                continue;
                        if (sd_session_get_uid(*s, &uid) < 0) {
                                log_device_error_errno(dev, r, "Failed to query uid for session %s: %m", *s);
                                continue;
                        }
                        if (set_ensure_put(&uids, &trivial_hash_ops, UID_TO_PTR(uid)) < 0)
                                return log_device_error_errno(dev, r, "Failed to construct UID set: %m");
