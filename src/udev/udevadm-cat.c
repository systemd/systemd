...
int cat_main(int argc, char *argv[], void *userdata) {
        ...
        STRV_FOREACH(s, strv_skip(argv, optind)) {
                ...
                if (path_is_absolute(*s)) {
                        if (!path_is_safe(*s))
                                return log_warning_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid udev rules path: %s", *s);

                        if (!endswith(*s, ".rules"))
                                return log_warning_errno(SYNTHETIC_ERRNO(EINVAL), "File name must end with '.rules': %s", *s);

                        r = cat_files(*s, /* dropins = */ NULL, arg_cat_flags);
                        if (r < 0)
                                return r;
                        ...
                        if (!filename_is_valid(filename))
                                return log_warning_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid udev rules file name: %s", *s);

                        bool found = false;
                        STRV_FOREACH(p, CONF_PATHS_STRV("udev/rules.d")) {
                                _cleanup_free_ char *path = path_join(*p, filename);
                                if (!path)
                                        return log_oom();

                                if (access(path, F_OK) >= 0) {
                                        r = cat_files(path, /* dropins = */ NULL, arg_cat_flags);
                                        if (r < 0)
                                                return r;
                                        found = true;
                                        break;
                        ...
                        if (!found)
                                return log_warning_errno(SYNTHETIC_ERRNO(ENOENT), "Cannout find udev rules file: %s", filename);
                }
        }
