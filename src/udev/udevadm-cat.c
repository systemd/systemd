...
int cat_main(int argc, char *argv[], void *userdata) {
        ...
        if (optind < argc) {
                STRV_FOREACH(s, strv_skip(argv, optind)) {
                        if (!endswith(*s, ".rules")) {
                                log_warning("%s: File name must end with '.rules', ignoring.", *s);
                                continue;
                        }

                        r = strv_extend(&files, *s);
                        if (r < 0)
                                return r;
                }

                files = strv_sort_uniq(files);
                if (strv_isempty(files))
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "No udev rules files specified.");
