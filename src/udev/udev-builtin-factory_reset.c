 ...
 * the current state at moment of probing as an udev property. This can be used to create certain device node
...
static int builtin_factory_reset(UdevEvent *event, int argc, char *argv[]) {
        ...
        udev_builtin_add_property(event, "ID_FACTORY_RESET", factory_reset_mode_to_string(f));
        return 0;
}
