...
static int find_real_nvme_parent(sd_device *dev, sd_device **ret) {
        ...
        /* If the device is belonging to "nvme-subsystem" (instead of "nvme", do not cofuse), which
         * happens when multipath is enabled, then the syspath is something like the following:
         ...
         * The sysname format of nvme subsystem deviceis nvme%d.
