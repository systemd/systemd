...
static int handle_pnp(sd_device *dev, char **path, char **compat_path) {
        ...
        r = sd_device_new_child(&firmware_node, dev, "firmware_node");
        if (r < 0)
                return r;
...
static int builtin_path_id(UdevEvent *event, int argc, char *argv[]) {
        ...
        for (sd_device *parent = dev; parent; ) {
                ...
                if (sd_device_get_sysname(parent, &sysname) < 0) {
                        ...
                        if (!supported_parent && handle_pnp(parent, &path, &compat_path) >= 0)
                                supported_parent = true;
