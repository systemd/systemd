                ...
                case 'N':
                        ...
                        if (arg_resolve_name_timing < 0)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                                       "--resolve-names= must be early or never");
                        /*
...
static int verify_rules_file(UdevRules *rules, const char *fname) {
        ...
        unsigned int issues = udev_check_current_rule_file(rules);
        unsigned int mask = (1U << LOG_ERR) | (1U << LOG_WARNING);
        if (issues & mask)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "%s: udev rules check failed", fname);

        return EXIT_SUCCESS;
}

static int verify_rules(UdevRules *rules, unsigned int num_args, char **args) {
        int rv = EXIT_SUCCESS;

        for (unsigned int i = 0; i < num_args; ++i) {
                int r = verify_rules_file(rules, args[i]);
                if (r < 0 && rv == EXIT_SUCCESS)
                        rv = r;
        }
...
int verify_main(int argc, char *argv[], void *userdata) {
        ...
        return verify_rules(rules, argc - optind, argv + optind);
}
