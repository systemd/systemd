...
static int worker_new(Worker **ret, Manager *manager, sd_device_monitor *worker_monitor, PidRef *pidref) {
        ...
        *worker = (Worker) {
        ...
        r = event_add_child_pidref(manager->event, &s, pidref, WEXITED, on_sigchld, worker);
        if (r < 0)
                return r;

        r = sd_event_source_set_priority(s, EVENT_PRIORITY_WORKER_SIGCHLD);
        if (r < 0)
                return r;
