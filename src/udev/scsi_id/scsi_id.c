...
enum scsi_id_interface arg_scsi_interface = SCSI_ID_INTERFACE_UNKNOWN;
...
static void help(void) {
        printf("Usage: %s [OPTION...] DEVICE\n\n"
               ...
               "  -i --sgio                        Use sg_io for reading VPD pages\n"
               "  -y --sysfs                       Use sysfs for reading VPD pages\n"
...
static int set_options(int argc, char **argv,
        ...
        while ((option = getopt_long(argc, argv, "d:f:gp:uvVxhbs:iy", options, NULL)) >= 0)
                ...
                case 'i':
                        if (arg_scsi_interface != SCSI_ID_INTERFACE_UNKNOWN)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid option: Only one interface --sgio/-i/--sysfs/-y can be specified");
                        arg_scsi_interface = SCSI_ID_INTERFACE_SGIO;
                        break;

                case 'y':
                        if (arg_scsi_interface != SCSI_ID_INTERFACE_UNKNOWN)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid option: Only one interface --sgio/-i/--sysfs/-y can be specified");
                        arg_scsi_interface = SCSI_ID_INTERFACE_SYSFS;
                        break;
