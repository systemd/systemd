...
static int set_options(int argc, char **argv, char *maj_min_dev) {
        ...
        FOREACH_OPTION(&state, c, &arg, /* on_error= */ return c)
                ...
                OPTION_SHORT('V', NULL, /* help= */ NULL): /* don't advertise -V */
                        return version();
...
static int per_dev_options(struct scsi_id_device *dev_scsi, int *good_bad, int *page_code) {
        ...
        *page_code = default_page_code;
        ...
        OptionParser state = { strv_length(newargv), newargv };
        const Option *opt;
        const char *arg;
...
static int run(int argc, char **argv) {
        ...
        if (r < 0)
                return r;
        if (r == 1) {
                r = set_options(strv_length(newargv), newargv, maj_min_dev);
                if (r < 0)
