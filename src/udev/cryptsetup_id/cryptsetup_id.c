...
static int print_attribute(const char *name, const char *devname) {
        ...
        if (!device_in_subsystem(dev, "block"))
                return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Device %s is not a block device", devname);

        r = sd_device_get_devpath(dev, &devpath);
        if (r < 0)
                return log_debug_errno(r, "Could not resolve devpath for device '%s': %m", devname);
...
static int run(int argc, char *argv[]) {
        ...
        r = sd_device_get_sysattr_value(dev, "dm/name", &dm_name);
        if (r < 0)
                return log_device_debug_errno(dev, r, "Failed to get device DM name: %m");
        ...
        if (streq(type, CRYPT_VERITY)) {
                ...
                const char *device_name, *metadata_device_name;
