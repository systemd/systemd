...
static int enumerate_partitions_by_blkid(sd_device *dev, Hashmap **ret) {
        ...
        if (IN_SET(r, _BLKID_SAFEPROBE_AMBIGUOUS, _BLKID_SAFEPROBE_NOT_FOUND)) {
                /* Maybe broken partition table or uninitialized disk. Anyway, no valid partition should
                 * exist. */
                *ret = NULL;
                return 0;
        }
...
static int partitions_synced(sd_device *dev) {
        ...
        if (partitions)
                log_device_debug(dev, "All partitions found by blkid are available on sysfs, triggering 'change' events without requesting the kernel to reread partition table.");
        else
                log_device_debug(dev, "No partition found by blkid and on sysfs, triggering 'change' event for the disk without requesting the kernel to reread partition table.");

        (void) synthesize_change_all(dev, e);
        return 1; /* synced and sent change events */
}

#define SYNC_PARTITION_TIMEOUT_USEC  (5 * USEC_PER_SEC)
#define SYNC_PARTITION_MAX_STEP_USEC (500 * USEC_PER_MSEC)
#define SYNC_PARTITION_MIN_STEP_USEC (250 * USEC_PER_MSEC)
...
static int wait_for_partitions_synced(sd_device *dev) {
        ...
        for (usec_t timeout_usec = usec_add(now(CLOCK_MONOTONIC), SYNC_PARTITION_TIMEOUT_USEC);;) {
                ...
                (void) usleep_safe(MIN(random_u64_range(SYNC_PARTITION_MAX_STEP_USEC - SYNC_PARTITION_MIN_STEP_USEC) + SYNC_PARTITION_MIN_STEP_USEC,
                                       usec_sub_unsigned(timeout_usec, now_usec)));
        }

        log_device_debug(dev, "Partitions on sysfs are not synced in %s, requesting the kernel to reread partition table.",
...
static int synthesize_change_synced(sd_device *dev) {
        ...
        if (r < 0)
                return log_device_debug_errno(dev, r, "Failed to request the kernel to reread partition table: %m");

        /* The kernel should already sent out a "change" event for the disk, and "remove/add" for all
         * partitions. Hence, it is not necessary to do anything here anymore. */
        return 0;
}
