...
static int partition_add(sd_device *dev, blkid_partition pp, Hashmap **partitions) {
        ...
        r = strdup_to(&uuid, blkid_partition_get_uuid(pp));
        if (r < 0)
                return r;
...
static int wait_for_partitions_synced(sd_device *dev) {
        ...
        usec_t timeout_usec = usec_add(now(CLOCK_MONOTONIC), sync_partition_timeout_usec());
        usec_t range = 100 * USEC_PER_MSEC;
        ...
        for (;;) {
                ...
                (void) usleep_safe(MIN(usec_add(random_u64_range(range), 100 * USEC_PER_MSEC),
                                       usec_sub_unsigned(timeout_usec, now_usec)));

                /* exponentially increase the range */
                if (range <= USEC_INFINITY / 2)
                        range *= 2;
        }
