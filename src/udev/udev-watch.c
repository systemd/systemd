...
static int context_check_partition_synced(SyncDeviceContext *c) {
...
static int context_on_event(SyncDeviceContext *c) {
        ...
        r = context_check_partition_synced(c);
        if (r != 0)
...
static int wait_for_partitions_synced(sd_device *dev) {
        ...
        /* Let's anyway check if partitions are already synced at first. */
        r = partitions_synced(dev);
        if (r < 0 && r != -EBUSY)
                return log_device_debug_errno(dev, r, "Failed to check if partitions are synced: %m");
        if (r > 0)
        ...
        /* If not synced, then check again when an event for the device or its children is received.
         * Also, if the device is currently locked, also check after a short delay. */
        ...
        r = sd_event_add_time_relative(c.event, NULL, CLOCK_MONOTONIC, sync_partition_timeout_usec(),
                                       /* accuracy = */ 0, /* handler = */ NULL, INT_TO_PTR(-ETIMEDOUT));
        if (r < 0)
