...
int udev_watch_begin(int inotify_fd, sd_device *dev) {
        ...
        for (unsigned i = 0; i < SAVE_WATCH_HANDLE_MAX_RETRIES; i++) {
                if (i > 0) {
                        ...
                        /* When the same handle is reused for different device node, we may failed to
                         * save the watch handle. Let's consider the case two workers A and B do the
                         * following:
                         *
