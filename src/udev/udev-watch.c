...
static void udev_watch_dump(void) {
        int r;
...
void manager_push_inotify(Manager *manager) {
        ...
        if (r < 0)
                log_warning_errno(r, "Failed to push inotify file descriptor, ignoring: %m");
        else
                log_debug("Pushed inotify file descriptor to service manager.");
...
int manager_init_inotify(Manager *manager, int fd) {
        ...
        if (fd >= 0) {
                ...
                log_debug("Received inotify fd (%i).", fd);
