...
int device_new_from_watch_handle(sd_device **ret, int wd) {
         ...
         * A. No matching symlink exists. This happens when the watch handle is assigned to a device,
         *    but it has not been saved to a symlink yet. Or, the device is already removed and the
         *    corresponding 'remove' uevent is already processed or being processed.
         ...
         *
         * However, there are no possibility that there exist multiple symlinks from IDs of *existing*
         * devices to the watch handle, as udev_watch_end() first removes a symlink, and then removes
         * the watch handle. Hence, in the below, it is OK to return the first device obtained by
