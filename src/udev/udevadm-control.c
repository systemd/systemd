...
static int varlink_send_commands(void) {
        ...
        if (arg_exit) {
                r = varlink_call_and_log(link, "org.systemd.Udev.Exit", /* parameters = */ NULL, /* reply = */ NULL);
                if (r < 0)
                        return r;
        }

        if (arg_log_level >= 0) {
                r = varlink_callbo_and_log(link, "org.systemd.Service.SetLogLevel", /* reply = */ NULL,
        ...
        if (arg_start_exec_queue == true) {
                r = varlink_call_and_log(link, "org.systemd.Udev.StartExecQueue", /* parameters = */ NULL, /* reply = */ NULL);
                if (r < 0)
                        return r;
        }

        if (arg_start_exec_queue == false) {
                r = varlink_call_and_log(link, "org.systemd.Udev.StopExecQueue", /* parameters = */ NULL, /* reply = */ NULL);
                if (r < 0)
                        return r;
        }

        if (arg_reload) {
                r = varlink_call_and_log(link, "org.systemd.Service.Reload", /* parameters = */ NULL, /* reply = */ NULL);
                if (r < 0)
                        return r;
        }

        if (!strv_isempty(arg_env)) {
                r = varlink_callbo_and_log(link, "org.systemd.Udev.SetEnvironment", /* reply = */ NULL,
        ...
        if (arg_max_children >= 0) {
                r = varlink_callbo_and_log(link, "org.systemd.Udev.SetChildrenMax", /* reply = */ NULL,
...
static int ctrl_send_commands(void) {
...
static int send_control_commands(void) {
        ...
        r = varlink_send_commands();
        if (r != 0)
                return r;

        return ctrl_send_commands();
}
