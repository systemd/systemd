...
static int rule_resolve_user(UdevRuleLine *rule_line, const char *name, uid_t *ret) {
        ...
        r = userdb_by_name(name, &match, USERDB_PARSE_NUMERIC, &ur);
        if (r == -ESRCH)
...
static int rule_resolve_group(UdevRuleLine *rule_line, const char *name, gid_t *ret) {
        ...
        r = groupdb_by_name(name, &match, USERDB_PARSE_NUMERIC, &gr);
        if (r == -ESRCH)
...
static int udev_rule_apply_token_to_event(
        ...
        case TK_A_OWNER: {
                ...
                r = userdb_by_name(owner, &match, USERDB_PARSE_NUMERIC, &ur);
                if (r == -ESRCH)
        ...
        case TK_A_GROUP: {
                ...
                r = groupdb_by_name(group, &match, USERDB_PARSE_NUMERIC, &gr);
                if (r == -ESRCH)
