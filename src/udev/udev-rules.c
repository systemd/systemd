...
static int rule_resolve_user(UdevRuleLine *rule_line, const char *name, uid_t *ret) {
        ...
        assert(rule_line);
        assert(rule_line->rule_file);
        assert(rule_line->rule_file->rules);
        assert(name);

        val = hashmap_get(rule_line->rule_file->rules->known_users, name);
        if (val) {
                *ret = PTR_TO_UID(val);
                return 0;
        }
        ...
        r = hashmap_ensure_put(&rule_line->rule_file->rules->known_users, &string_hash_ops, n, UID_TO_PTR(uid));
        if (r < 0)
                return r;
...
static int rule_resolve_group(UdevRuleLine *rule_line, const char *name, gid_t *ret) {
        ...
        val = hashmap_get(rule_line->rule_file->rules->known_groups, name);
        if (val) {
                *ret = PTR_TO_GID(val);
                return 0;
        }
        ...
        r = hashmap_ensure_put(&rule_line->rule_file->rules->known_groups, &string_hash_ops, n, GID_TO_PTR(gid));
        if (r < 0)
                return r;
...
static int parse_token(UdevRuleLine *rule_line, const char *key, char *attr, UdevRuleOperatorType op, char *value) {
        ...
        rules = rule_line->rule_file->rules;
...
static int udev_rule_apply_token_to_event(
        ...
        case TK_M_IMPORT_BUILTIN: {
                ...
                (void) udev_event_apply_format(event, token->value, buf, sizeof(buf), false, &truncated);
                if (truncated) {
                        log_event_debug(dev, token, "The builtin command '%s' is truncated while substituting into '%s', "
                                       "assuming the IMPORT key does not match", buf, token->value);
                        return false;
                }
