...
static int udev_rule_apply_token_to_event(
        ...
        case TK_A_OWNER: {
                ...
                if (IN_SET(r, -ENOENT, -ESRCH))
                        ...
                        log_event_error_errno(event, token, r, "User \"%s\" is not a system user (UID="UID_FMT"), ignoring: %m", owner, uid);
                else {
                        event->uid = uid;
        ...
        case TK_A_GROUP: {
                ...
                if (IN_SET(r, -ENOENT, -ESRCH))
                        ...
                        log_event_error_errno(event, token, r, "Group \"%s\" is not a system group (GID="GID_FMT"), ignoring: %m", group, gid);
                else {
                        event->gid = gid;
