...
static bool token_match_attr(UdevRules *rules, UdevRuleToken *token, sd_device *dev, UdevEvent *event) {
        ...
        case SUBST_TYPE_FORMAT:
                ...
                if (truncated)
                        log_rule_warning(dev, rules, "Result of substitution into %s is truncated.", name);
...
static int udev_rule_apply_token_to_event(
        ...
        case TK_M_SYSCTL: {
                ...
                if (truncated)
                        log_rule_warning(dev, rules, "Result of substitution into %s is truncated.", (const char*) token->data);
        ...
        case TK_M_TEST: {
                ...
                if (truncated)
                        log_rule_warning(dev, rules, "Result of substitution into %s is truncated.", token->value);
