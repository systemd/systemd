...
#define log_event_full_errno_zerook(event, t, level, error, fmt, ...)   \
        ({                                                              \
                UdevEvent *_ee = ASSERT_PTR(event);                     \
                UdevRuleToken *_tt = ASSERT_PTR(t);                     \
                UdevRuleLine *_ll = ASSERT_PTR(_tt->rule_line);         \
                sd_device *_dd = token_is_for_parents(_tt) ?            \
                        _ee->dev_parent : _ee->dev;                     \
                                                                        \
                log_udev_rule_internal(                                 \
                                _dd, _ll->rule_file, _ll->line_number,  \
                                level, error, "%s: " fmt,               \
                                _tt->token, ##__VA_ARGS__);             \
        })
...
#define log_event_trace(event, ...)                                     \
        ({                                                              \
                UdevEvent *_eee = ASSERT_PTR(event);                    \
                                                                        \
                _eee->trace ?                                           \
                        log_event_debug(_eee, ##__VA_ARGS__) :          \
                        0;                                              \
        })
...
#define log_event_truncated(event, token, what, format)                 \
        ({                                                              \
                UdevRuleToken *_ttt = ASSERT_PTR(token);                \
                                                                        \
                _ttt->type < _TK_M_MAX ?                                \
                        log_event_debug(event, _ttt,                    \
...
#define log_event_line(event, line, ...)                                \
        ({                                                              \
                UdevEvent *_ee = ASSERT_PTR(event);                     \
                UdevRuleLine *_ll = ASSERT_PTR(line);                   \
                                                                        \
                _ee->trace ?                                            \
                        log_udev_rule_internal(                         \
                                        _ee->dev,                       \
...
static int udev_rule_apply_token_to_event(
                ...
                for (const char *p = buf;;) {
                        ...
                        if (token->op == OP_REMOVE) {
                                ...
                                if (r < 0)
                                        ...
                                        log_event_debug(event, token, "Dropped device node symlink '%s'", path);
                        } else {
        ...
        case TK_A_RUN_PROGRAM: {
                ...
                log_event_debug(event, token, "Set command: %s", token->value);
                return true;
        }
...
static int udev_rule_apply_parent_token_to_event(UdevRuleToken *head_token, UdevEvent *event) {
        ...
        if (event->trace) {
                _cleanup_free_ char *token_str = NULL;

                LIST_FOREACH(tokens, token, head_token)
                        if (token_is_for_parents(token))
                                (void) strextend_with_separator(&token_str, ", ", token->token);
                        else
                                break;

                log_event_line(event, line, "Checking conditions for parent devices: %s", strna(token_str));
        }
