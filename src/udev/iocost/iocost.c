...
static int query_named_solution(
        ...
        if (r == -ENOENT) {
                *ret_qos = *ret_model = NULL;
                log_device_debug_errno(device, r, "No value found for key %s, skipping iocost logic.", qos_key);
                return 0;
        }
        if (r < 0)
        ...
        if (r == -ENOENT) {
                *ret_qos = *ret_model = NULL;
                log_device_debug_errno(device, r, "No value found for key %s, skipping iocost logic.", model_key);
                return 0;
        }
        if (r < 0)
                return log_device_error_errno(device, r, "Failed to obtain model for iocost solution from device: %m");
