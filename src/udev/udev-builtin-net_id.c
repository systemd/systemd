...
/* So here's the deal: net_id is supposed to be an excercise in providing stable names for network devices.
 * However, we also want to keep updating the naming scheme used in future versions of net_id. These two
 * goals of course are contradictory: on one hand we want things to not change and on the other hand we want
 * them to improve. Our way out of this dilemma is to introduce the "naming scheme" concept: each time we
 * improve the naming logic we define a new flag for it. Then, we keep a list of schemes, each identified by
 * a name associated with the flags it implements. Via a kernel command line and environment variable we then
 * allow the user to pick the scheme they want us to follow: installers could "freeze" the used scheme at the
 * moment of installation this way.
 *
 * Developers: each time you tweak the naming logic here, define a new flag below, and condition the tweak
 * with it. Each time we do a release we'll then add a new scheme entry and include all newly defined flags.
 *
 * Note that this is only half a solution to the problem though: not only udev/net_id gets updated all the
 * time, the kernel gets too. And thus a kernel that previously didn't expose some sysfs attribute we look
 * for might eventually do, and thus affect our naming scheme too. Thus, enforcing a naming scheme will make
 * interfacing more stable across OS versions, but not fully stabilize them. */
typedef enum NamingSchemeFlags
{
        ...
        NAMING_INFINIBAND = 1
                << 2, /* Use "ib" prefix for infiniband, see 938d30aa98df887797c9e05074a562ddacdcdf5e */
        NAMING_ZERO_ACPI_INDEX = 1
                << 3, /* Allow zero acpi_index field, see d81186ef4f6a888a70f20a1e73a812d6acb9e22f */
...
static int dev_pci_slot(sd_device *dev, struct netnames *names) {
        ...
        if (sd_device_get_sysattr_value(dev, "dev_port", &attr) >= 0) {
                ...
                if (dev_port == 0 && sd_device_get_sysattr_value(dev, "type", &attr) >= 0) {
                        unsigned long type;

                        type = strtoul(attr, NULL, 10);
                        if (type == ARPHRD_INFINIBAND && sd_device_get_sysattr_value(dev, "dev_id", &attr) >= 0)
        ...
        while (hotplug_slot_dev) {
                ...
                FOREACH_DIRENT_ALL(dent, dir, break) {
                        ...
                        if (snprintf_ok(str, sizeof str, "%s/%s/address", slots, dent->d_name) &&
                            read_one_line_file(str, &address) >= 0 && startswith(sysname, address)) {
                                hotplug_slot = i;
                                break;
                        }
                }
...
static int names_pci(sd_device *dev, struct netnames *names) {
        ...
        if (sd_device_get_subsystem(parent, &subsystem) >= 0 && streq("pci", subsystem)) {
                names->type = NET_PCI;
                names->pcidev = parent;
        } else {
        ...
        if (naming_scheme_has(NAMING_SR_IOV_V) && get_virtfn_info(dev, names, &vf_info) >= 0) {
...
static int builtin_net_id(sd_device *dev, int argc, char *argv[], bool test) {
        ...
        if (names.type == NET_PCI) {
                ...
                if (names.pci_onboard[0] && snprintf_ok(str, sizeof str, "%s%s", prefix, names.pci_onboard))
                ...
                if (names.pci_path[0] && snprintf_ok(str, sizeof str, "%s%s", prefix, names.pci_path))
                        udev_builtin_add_property(dev, test, "ID_NET_NAME_PATH", str);

                if (names.pci_slot[0] && snprintf_ok(str, sizeof str, "%s%s", prefix, names.pci_slot))
                        udev_builtin_add_property(dev, test, "ID_NET_NAME_SLOT", str);
                return 0;
        }
