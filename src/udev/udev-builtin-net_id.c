...
static int names_pci(UdevEvent *event, const char *prefix) {
        ...
        /* If the netdev's direct parent is an auxiliary device that exposes a stable 'sfnum'
         * attribute (currently mlx5_core sub-functions), peel off the SF identity into a
         * 'S<sfnum>' suffix piece and pick up the aux device's underlying PCI function as
         * 'parent'. The aux device is just a bump on the path; everything below — PF/VF
         * resolution, slot/onboard lookup — proceeds the same way as for any PCI-rooted netdev. */
        if (naming_scheme_has(NAMING_SUBFUNC)) {
                sd_device *aux;

                if (get_matching_parent(dev, STRV_MAKE("auxiliary"),
                                        /* skip_virtio= */ true, &aux) >= 0)
                        (void) get_subfunc_info(aux, &parent_pcidev, &subfunc_suffix);
        }

        /* SF: parent is the aux device's PCI function. Otherwise the netdev's direct parent
         * must itself be a PCI device. */
        if (subfunc_suffix)
