...
static int dev_pci_onboard(sd_device *dev, const LinkInfo *info, NetNames *names) {
        ...
        if (sd_device_get_sysattr_value(names->pcidev, "label", &names->pci_onboard_label) >= 0)
                log_device_debug(dev, "Onboard label: %s", names->pci_onboard_label);
        else
...
static bool is_pci_bridge(sd_device *dev) {
        ...
        if (b)
                log_device_debug(dev, "Device is a bridge.");
        return b;
}
...
static int dev_pci_slot(sd_device *dev, const LinkInfo *info, NetNames *names) {
        ...
        log_device_debug(dev, "Parsing slot information from sysname \"%s\": %s",
                        ...
                        if (snprintf_ok(str, sizeof str, "%s/%s/address", slots, dent->d_name) &&
                            ...
                            startswith(sysname, address)) {
                                ...
                                if (naming_scheme_has(NAMING_BRIDGE_NO_SLOT) && is_pci_bridge(hotplug_slot_dev)) {
                                        log_device_debug(dev, "Not using slot information because device is a PCI bridge.");
                                        return 0;
                                }
