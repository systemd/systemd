...
toolong:
        ...
        dest[size - 12] = urlsafe_base64char((h >> 60) & 63);
        dest[size - 11] = urlsafe_base64char((h >> 54) & 63);
        dest[size - 10] = urlsafe_base64char((h >> 48) & 63);
        dest[size -  9] = urlsafe_base64char((h >> 42) & 63);
        dest[size -  8] = urlsafe_base64char((h >> 36) & 63);
        dest[size -  7] = urlsafe_base64char((h >> 30) & 63);
        dest[size -  6] = urlsafe_base64char((h >> 24) & 63);
        dest[size -  5] = urlsafe_base64char((h >> 18) & 63);
        dest[size -  4] = urlsafe_base64char((h >> 12) & 63);
        dest[size -  3] = urlsafe_base64char((h >>  6) & 63);
        dest[size -  2] = urlsafe_base64char( h        & 63);
        dest[size -  1] = '\0';
...
static int link_update(sd_device *dev, const char *slink_in, bool add) {
        ...
        (void) escape_path(slink_name, name_enc, sizeof(name_enc));
