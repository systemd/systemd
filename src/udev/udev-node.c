...
static int update_stack_dir(sd_device *dev, const char *dirname, bool add) {
        ...
        for (unsigned j = 0; j < CREATE_STACK_LINK_MAX_RETRIES; j++) {
                ...
                if (r == -ENOENT)
                        continue;
                if (r < 0)
                        return log_device_debug_errno(dev, r, "Failed to create the directory %s: %m", dirname);
                ...
                if (utimensat(AT_FDCWD, dirname, NULL, 0) < 0)
                        return log_device_debug_errno(dev, errno, "Failed to update the timestamp of %s: %m", dirname);

                return 0;
        }

        return log_device_debug_errno(dev, SYNTHETIC_ERRNO(ENOENT), "Failed to create symbolic link %s: %m", filename);
}
...
static int link_update(sd_device *dev, const char *slink_in, bool add) {
        ...
        r = update_stack_dir(dev, dirname, add);
        if (r < 0)
                return r;
