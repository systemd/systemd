...
static int stack_directory_open_and_lock(const char *dirname, int *ret_dirfd, int *ret_lockfd) {
        ...
        dirfd = open_mkdir_at(AT_FDCWD, dirname, O_CLOEXEC | O_DIRECTORY | O_NOFOLLOW | O_RDONLY, 0700);
        if (dirfd < 0)
                return dirfd;
...
static int link_update(sd_device *dev, const char *slink, bool add) {
        ...
        r = stack_directory_open_and_lock(dirname, &dirfd, &lockfd);
        if (r < 0)
                return log_device_debug_errno(dev, r, "Failed to lock stack directory '%s': %m", dirname);
