...
static int stack_directory_lock(const char *dirname) {
        ...
        /* Prevent that unprivileged processes may lock the directory.
         * Note, we created the directory with 0700 in the above, but the direcotry may be already
         * created by an old udevd, in that case open_mkdir_at() does not change the access mode.
         * Hence, we need to change the access mode here explicitly. */
        if (fchmod(fd, 0700) < 0)
                return -errno;
