...
static int send_proxy_protocol_v1(Connection *c) {
        ...
        if (getsockname(c->server_fd, (struct sockaddr*) &local_addr, &addr_len) < 0) {
                log_warning_errno(errno, "Failed to get local address (getsockname): %m");
                goto unknown;
        }

        addr_len = sizeof(remote_addr);
        if (getpeername(c->server_fd, (struct sockaddr*) &remote_addr, &addr_len) < 0) {
                log_warning_errno(errno, "Failed to get remote address (getpeername): %m");
                goto unknown;
        }

        /* only TCP on IPv4 and TCP on IPv6 are supported by the protocol v1 */
        switch (remote_addr.sa.sa_family) {
        case AF_INET:
                s_len = asprintf(&s, "PROXY TCP4 %s %s %u %u\r\n",
                ...
                break;
        case AF_INET6:
                s_len = asprintf(&s, "PROXY TCP6 %s %s %u %u\r\n",
        ...
        if (r < 0) {
                log_warning_errno(r, "Failed to write to remote host: %m");
                return r;
        }
