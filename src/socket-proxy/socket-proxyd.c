...
static void send_proxy_protocol_v1(Connection *c) {
        ...
        if (getsockopt(c->server_fd, SOL_SOCKET, SO_TYPE, &type, &type_len) < 0) {
        ...
        if (local_addr.ss_family == AF_INET) {
                ...
                s_len = asprintf(&s, "PROXY TCP4 %s %s %hu %hu\r\n",
                        IN4_ADDR_TO_STRING(&(r->sin_addr)),
                        IN4_ADDR_TO_STRING(&(l->sin_addr)),
                        be16toh(r->sin_port),
                        be16toh(l->sin_port)
                );
                if (s_len < 0)
                    goto fail;
                loop_write(c->client_fd, s, s_len);

                return;
        }

        if (local_addr.ss_family == AF_INET6) {
                struct sockaddr_in6 *l = (struct sockaddr_in6*)&local_addr;
                struct sockaddr_in6 *r = (struct sockaddr_in6*)&remote_addr;

                s_len = asprintf(&s, "PROXY TCP6 %s %s %hu %hu\r\n",
                        IN6_ADDR_TO_STRING(&(r->sin6_addr)),
                        IN6_ADDR_TO_STRING(&(l->sin6_addr)),
                        be16toh(r->sin6_port),
                        be16toh(l->sin6_port)
                );
                if (s_len < 0)
...
fail:
        /* ignore errors - remote can decide to deny UNKNOWN connections */
        loop_write(c->client_fd, "PROXY UNKNOWN\r\n", SIZE_MAX);
        return;
}
