...
static int create_disk(
                const char *name,
                const char *device,
                const char *hmac_key_file,
                char *options) {

        _cleanup_free_ char *n = NULL, *dd = NULL, *e = NULL, *name_escaped = NULL, *hmac_file_escaped = NULL;
        ...
        r = verify_hmac_key_file(hmac_key_file, NULL, NULL);
        if (r < 0)
                return r;

        hmac_file_escaped = specifier_escape(hmac_key_file);
        if (!hmac_file_escaped)
                return log_oom();
        ...
        fprintf(f,
                ...
                name_escaped, device, hmac_file_escaped, (options) ? options: "",
                name_escaped);
...
static int add_integritytab_devices(void) {
        ...
        for (;;) {
                _cleanup_free_ char *line = NULL, *name = NULL, *device_id = NULL, *device_path = NULL, *hmac_key_file = NULL, *options = NULL;
                char *l;
                ...
                r = sscanf(l, "%ms %ms %ms %ms", &name, &device_id, &hmac_key_file, &options);
                if (!IN_SET(r, 3, 4)) {
                ...
                r = create_disk(name, device_path, hmac_key_file, options);
                if (r < 0)
                        return r;
        }
