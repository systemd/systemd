...
static void manager_restore_userns_policy(Manager *m, uint64_t inode) {
        ...
        r = userns_registry_load_by_userns_inode(m->registry_fd, inode, &info);
        if (r < 0)
         ...
         * instead of a transient range, so it is the one whose start UID is its owner. */
        bool deny = info->setgroups_deny >= 0 ? info->setgroups_deny :
                (info->size == 1 && info->start_uid == info->owner);
...
int manager_startup(Manager *m) {
        ...
        SET_FOREACH(p, registry_inodes) {
                ...
                manager_restore_userns_policy(m, inode);
        }
