...
int manager_startup(Manager *m) {
        ...
        SET_FOREACH(p, registry_inodes) {
                ...
                if (r >= 0 || IN_SET(r, -EBUSY, -ENODATA))
                        continue; /* Reaped, still alive, or this entry has no id to probe with. */
                if (r == -EHOSTDOWN) {
                         ...
                         * logging once per entry. */
                        log_debug_errno(r, "Cannot detect stale registry entries, skipping: %m");
                        break;
                }
