...
static void manager_release_userns_by_info(Manager *m, UserNamespaceInfo *info) {
        ...
        userns_registry_release_by_info(m->userns_restrict_bpf, m->registry_fd, info);
}
...
static void manager_release_userns_by_inode(Manager *m, uint64_t inode) {
        ...
        userns_registry_release_by_userns_inode(m->userns_restrict_bpf, m->registry_fd, inode);
}
