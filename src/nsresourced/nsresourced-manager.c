...
int manager_startup(Manager *m) {
        ...
        SET_FOREACH(p, registry_inodes) {
                ...
                if (probe_fd != -ESTALE) {
                        /* -EHOSTDOWN means we're not in the initial user namespace; the sweep
                         * can't proceed for any entry, so bail out rather than logging once per
                         * entry. Anything else is unexpected — log it but skip just this one. */
                        if (probe_fd == -EHOSTDOWN)
                                break;
