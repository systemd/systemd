...
static int validate_userns(Varlink *link, int userns_fd) {
        ...
        if (peer_uid != 0) {
                ...
                if (ioctl(userns_fd, NS_GET_OWNER_UID, &owner_uid) < 0)
                        return log_debug_errno(r, "Failed to get owner UID of user namespace: %m");
...
static int validate_userns_is_empty(Varlink *link, int userns_fd) {
        ...
        r = uid_range_load_userns_by_fd(&range, userns_fd, UID_RANGE_USERNS_OUTSIDE);
        if (r < 0)
        ...
        r = uid_range_load_userns_by_fd(&range, userns_fd, GID_RANGE_USERNS_OUTSIDE);
        if (r < 0)
...
static int validate_userns_is_safe(Varlink *link, int userns_fd) {
        ...
        r = uid_range_load_userns_by_fd(&outside_range, userns_fd, UID_RANGE_USERNS_OUTSIDE);
        if (r < 0)
                return log_debug_errno(r, "Failed to read userns UID range: %m");
        ...
        r = uid_range_load_userns_by_fd(&outside_range_gid, userns_fd, GID_RANGE_USERNS_OUTSIDE);
        if (r < 0)
                return log_debug_errno(r, "Failed to read userns GID range: %m");
        ...
        r = uid_range_load_userns_by_fd(&inside_range, userns_fd, UID_RANGE_USERNS_INSIDE);
        if (r < 0)
        ...
        r = uid_range_load_userns_by_fd(&inside_range_gid, userns_fd, GID_RANGE_USERNS_INSIDE);
        if (r < 0)
                return log_debug_errno(r, "Failed to read userns contents: %m");
