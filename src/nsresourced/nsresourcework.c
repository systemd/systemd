...
static int write_userns_mappings(PidRef *pidref, const char *uidmap, const char *gidmap, bool allow_setgroups) {
        int r;
        ...
        const char *setgroups = procfs_file_alloca(pidref->pid, "setgroups");
        const char *value = allow_setgroups ? "allow" : "deny";
        r = write_string_file(setgroups, value, WRITE_STRING_FILE_DISABLE_BUFFER);
        if (r < 0)
                return log_debug_errno(r, "Failed to write '%s' to 'setgroups' file of user namespace: %m", value);

        const char *gmap = procfs_file_alloca(pidref->pid, "gid_map");
        r = write_string_file(gmap, gidmap, /* flags= */ 0);
        if (r < 0)
                return log_error_errno(r, "Failed to write 'gid_map' file of user namespace: %m");

        const char *umap = procfs_file_alloca(pidref->pid, "uid_map");
        r = write_string_file(umap, uidmap, /* flags= */ 0);
        if (r < 0)
...
static int write_userns(
        ...
        if (map_foreign) {
                ...
                if (strextendf(&uidmap, UID_FMT " " UID_FMT " %" PRIu32 "\n",
                               FOREIGN_UID_MIN, start_uid, 0x10000U) < 0)
                        return log_oom();
        }
        ...
        if (map_foreign) {
                ...
                if (strextendf(&gidmap, GID_FMT " " GID_FMT " %" PRIu32 "\n",
                               FOREIGN_UID_MIN, start_gid, 0x10000U) < 0)
                        return log_oom();
        }
        ...
        if (r > 0)
                return write_userns_mappings(&pidref, uidmap, gidmap, /* allow_setgroups= */ !identity);
        ...
        if (r == 0) {
                ...
                r = write_userns_mappings(&pidref, uidmap, gidmap, /* allow_setgroups= */ !identity);
...
static int validate_target_and_size(sd_varlink *link, uid_t target, uint32_t size, bool identity) {
        ...
        if (identity) {
                /* Identity userns must have size 1 and target must be 0 or unset */
                if (size != 1)
...
static int validate_delegate_counts(sd_varlink *link, uint32_t delegate_container_ranges, uint32_t delegate_dynamic_users) {
        assert(link);

        if (delegate_container_ranges > USER_NAMESPACE_DELEGATIONS_MAX)
                return sd_varlink_error_invalid_parameter_name(link, "delegateContainerRanges");

        if (delegate_dynamic_users > USER_NAMESPACE_DELEGATIONS_MAX)
                return sd_varlink_error_invalid_parameter_name(link, "delegateDynamicUsers");

        if (delegate_container_ranges + delegate_dynamic_users > USER_NAMESPACE_DELEGATIONS_MAX)
...
typedef struct AllocateParameters {
        ...
        uint32_t delegate_dynamic_users;
        bool map_foreign;
} AllocateParameters;
...
static int vl_method_allocate_user_range(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                ...
                { "delegateDynamicUsers",        _SD_JSON_VARIANT_TYPE_INVALID, sd_json_dispatch_uint32,       offsetof(AllocateParameters, delegate_dynamic_users),    0                 },
        ...
        r = validate_delegate_counts(link, p.delegate_container_ranges, p.delegate_dynamic_users);
        if (r != 0)
                return r;
        ...
        if (p.identity) {
                /* The start UID/GID will be mapped to the parent userns in write_userns(). If an identity
        ...
        uint32_t n_delegates = p.delegate_container_ranges + p.delegate_dynamic_users;
        if (n_delegates > 0) {
                userns_info->delegates = new0(DelegatedUserNamespaceInfo, n_delegates);
                if (!userns_info->delegates)
                        return -ENOMEM;

                userns_info->n_delegates = n_delegates;

                size_t i = 0;
                for (uint32_t j = 0; j < p.delegate_container_ranges; j++)
                        userns_info->delegates[i++].size = 0x10000U;
                for (uint32_t j = 0; j < p.delegate_dynamic_users; j++)
                        userns_info->delegates[i++].size = 1U;
        }
        ...
        r = write_userns(userns_fd, parent_userns_fd, userns_info, p.identity, p.map_foreign);
        if (r < 0)
                goto fail;
