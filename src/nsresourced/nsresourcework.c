...
static int registry_range_is_available(
        ...
        if (IN_SET(r, -EBUSY, -ENODATA, -EHOSTDOWN)) {
                /* Owner is alive, or its liveness can't be probed → the range is taken. */
                log_debug("UID " UID_FMT " unavailable: %salready registered in userns registry.",
                          candidate, gid ? "GID " : "");
                return false;
        }
...
static int delegation_range_is_available(
        ...
        for (;;) {
                ...
                if (IN_SET(r, -EBUSY, -ENODATA, -EHOSTDOWN)) {
                        log_debug("UID " UID_FMT " unavailable: %sdelegated to userns inode %" PRIu64 " still in use (parent is %" PRIu64 ").",
                                  candidate, gid ? "GID " : "", delegation.userns_inode, parent_userns_inode);
                        return false;
                }
...
static int uid_is_available(
        ...
        r = registry_range_is_available(registry_dir_fd, bpf, candidate, /* gid= */ false);
        if (r < 0)
                return r;
        if (r == 0)
                return false;

        r = registry_range_is_available(registry_dir_fd, bpf, candidate, /* gid= */ true);
        if (r < 0)
                return r;
        if (r == 0)
                return false;
        ...
        r = delegation_range_is_available(registry_dir_fd, bpf, candidate, parent_userns_inode, /* gid= */ false);
        if (r < 0)
                return r;
        if (r == 0)
                return false;

        r = delegation_range_is_available(registry_dir_fd, bpf, candidate, parent_userns_inode, /* gid= */ true);
        if (r < 0)
                return r;
        if (r == 0)
                return false;
...
static int allocate_one(
        ...
        for (;; idx = random_u32()) {
                ...
                r = uid_is_available(registry_dir_fd, bpf, candidate, parent_userns_fd);
                if (r < 0)
