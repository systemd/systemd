...
static int write_userns(
        ...
        if (r > 0)
                return write_userns_mappings(&pidref, uidmap, gidmap, /* allow_setgroups= */ type != ALLOCATE_USER_RANGE_SELF);
        ...
        if (r == 0) {
                ...
                r = write_userns_mappings(&pidref, uidmap, gidmap, /* allow_setgroups= */ type != ALLOCATE_USER_RANGE_SELF);
...
static int vl_method_allocate_user_range(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        r = write_userns(userns_fd, parent_userns_fd, userns_info, p.type, p.map_foreign);
        if (r < 0)
                goto fail;
