...
static int validate_delegate_count_and_size(sd_varlink *link, uint32_t delegate_count, uint32_t delegate_size) {
        assert(link);

        if (delegate_count > USER_NAMESPACE_DELEGATIONS_MAX)
                return sd_varlink_error(link, "io.systemd.NamespaceResource.TooManyDelegations", NULL);

        if (delegate_count > 0 && !IN_SET(delegate_size, 1U, 0x10000U))
...
static int vl_method_allocate_user_range(sd_varlink *link, sd_json_variant *parameters, sd_varlink_method_flags_t flags, void *userdata) {
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                ...
                { "delegateCount",               _SD_JSON_VARIANT_TYPE_INVALID, sd_json_dispatch_uint32,       offsetof(AllocateParameters, delegate_count),  0                 },
        ...
        r = validate_delegate_count_and_size(link, p.delegate_count, p.delegate_size);
        if (r != 0)
                return r;
        ...
        if (p.delegate_count > 0) {
                userns_info->delegates = new0(DelegatedUserNamespaceInfo, p.delegate_count);
                if (!userns_info->delegates)
                        return -ENOMEM;

                userns_info->n_delegates = p.delegate_count;

                FOREACH_ARRAY(delegate, userns_info->delegates, p.delegate_count)
                        delegate->size = p.delegate_size;
        }
