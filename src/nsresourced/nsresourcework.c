...
static int write_userns(
        ...
        for (size_t i = 0; i < userns_info->n_delegates; i++) {
                r = uid_range_translate(outside_range, inside_range, userns_info->delegates[i].start_uid, &start_uid);
                if (r < 0)
                        return log_debug_errno(r, "Failed to translate UID "UID_FMT" to parent userns: %m", userns_info->start_uid);

                if (start_uid != userns_info->delegates[i].start_uid)
                        return log_debug_errno(
                                SYNTHETIC_ERRNO(ERANGE),
                                "Delegated transient UID range not mapped 1:1 in parent userns ("UID_FMT" -> "UID_FMT")",
                                userns_info->delegates[i].start_uid, start_uid);
        ...
        for (size_t i = 0; i < userns_info->n_delegates; i++) {
                r = uid_range_translate(outside_range, inside_range, userns_info->delegates[i].start_gid, &start_gid);
                if (r < 0)
                        return log_debug_errno(r, "Failed to translate GID "GID_FMT" to parent userns: %m", userns_info->start_gid);

                if (start_gid != userns_info->delegates[i].start_gid)
                        return log_debug_errno(
                                SYNTHETIC_ERRNO(ERANGE),
                                "Delegated transient GID range not mapped 1:1 in parent userns ("GID_FMT" -> "GID_FMT")",
                                userns_info->delegates[i].start_gid, start_gid);
