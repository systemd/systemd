...
static int validate_mount(struct vfsmount *v, int ret) {
        ...
        /* fsuid/fsgid are the UID/GID in the initial user namespace, before any idmapped mounts or idmapping
         * to the user namespace attached to the mount's filesystem have been applied. There is no way (yet)
         * to figure out what the UID/GID that will be written to disk will be after idmapped mounts and
         * filesystem user namespace mapping, hence we have to rely on an allowlist of mounts populated by
         * userspace which tells us if a mount has an appropriate uid mapping in place to translate the
         * transient UID range to something else. For other UIDs/GIDs, there's no need to do these checks as
         * we don't insist on idmapped mounts or such for UIDs/GIDs outside the transient ranges. */
...
int BPF_PROG(userns_restrict_path_chown, struct path *path, unsigned long long uid, unsigned long long gid, int ret) {
        ...
        /* This is a mount foreign to our task's user namespace, refuse any UIDs/GIDs in the transient
         * ranges. Note that we can only do this check in the chown() hook because it receives the UID/GID as
         * it will be written to the filesystem whereas the other hooks only know the UID before any mount
         * idmapping or filesystem user namespace idmapping has been done and hence have to rely on the
         * mount allowlist to ensure the transient fsuid/fsgid will be translated to something else when
         * written to disk. */
