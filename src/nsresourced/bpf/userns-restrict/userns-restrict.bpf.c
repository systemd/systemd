...
static int validate_mount(struct vfsmount *v, uid_t uid, gid_t gid) {
        ...
        /* uid/gid are the UID/GID in the initial user namespace, before any idmapped mounts or idmapping to
         * the user namespace attached to the mount's filesystem have been applied. For transient UIDs/GIDs,
         * we have to check that the mount is owned by the user namespace or allowlisted to make sure that
         * the mount is an idmapped mount that maps the transient user range to something else on disk. For
         * other UIDs/GIDs, there's no need to do these checks as we don't insist on idmapped mounts or such
         * for UIDs/GIDs outside the transient ranges.  */
