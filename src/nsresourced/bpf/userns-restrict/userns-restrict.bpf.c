...
int BPF_PROG(userns_restrict_path_chown, struct path *path, unsigned long long uid, unsigned long long gid, int ret) {
         ...
         * ranges. Note that we can only do this check in the chown() hook because it receives the UID/GID
         * with idmapped mounts already taken into account, unlike the other hooks where we cannot (yet)
         * figure out the UID/GID after idmapped mounts are applied. Hence in the other hooks we have to rely
         * on the mount allowlist to ensure the transient fsuid/fsgid will be translated to something else
         * when written to disk but in the chown() hook we can check the provided UID/GID directly to see if
         * it is transient or not. */
