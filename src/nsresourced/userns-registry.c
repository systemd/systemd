...
UserNamespaceInfo *userns_info_free(UserNamespaceInfo *userns) {
        ...
        FOREACH_ARRAY(d, userns->delegates, userns->n_delegates)
                delegated_userns_info_done(d);
        free(userns->delegates);
...
void delegated_userns_info_done(DelegatedUserNamespaceInfo *info) {
        ...
        info->parent_userns = mfree(info->parent_userns);
        info->n_parent_userns = 0;
}
...
static int dispatch_delegates_array(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        UserNamespaceInfo *info = ASSERT_PTR(userdata);
        _cleanup_free_ DelegatedUserNamespaceInfo *delegates = NULL;
        size_t n = 0;
        int r;

        if (sd_json_variant_is_null(variant)) {
                info->delegates = mfree(info->delegates);
                info->n_delegates = 0;
                return 0;
        }
        ...
        JSON_VARIANT_ARRAY_FOREACH(e, variant) {
                ...
                delegates[n] = (DelegatedUserNamespaceInfo) {
                        .start_uid = UID_INVALID,
                        .start_gid = GID_INVALID,
                };
        ...
        free_and_replace(info->delegates, delegates);
        info->n_delegates = n;

        return 0;
}

static int dispatch_parent_userns_array(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        DelegatedUserNamespaceInfo *info = ASSERT_PTR(userdata);
        _cleanup_free_ uint64_t *parent_userns = NULL;
        size_t n = 0;
        ...
        parent_userns = new(uint64_t, sd_json_variant_elements(variant));
        if (!parent_userns)
                return json_log_oom(variant, flags);
        ...
        JSON_VARIANT_ARRAY_FOREACH(e, variant) {
                ...
                if (v == 0)
                        return json_log(e, flags, SYNTHETIC_ERRNO(EINVAL), "Invalid parent userns inode 0.");

                parent_userns[n++] = v;
        }

        free_and_replace(info->parent_userns, parent_userns);
        info->n_parent_userns = n;
...
int userns_registry_store(int dir_fd, UserNamespaceInfo *info) {
        ...
        for (size_t i = 0; i < info->n_delegates; i++) {
                r = sd_json_variant_append_arraybo(
                                &delegates_array,
                                SD_JSON_BUILD_PAIR_UNSIGNED("userns", info->delegates[i].userns_inode),
                                SD_JSON_BUILD_PAIR_UNSIGNED("start", info->delegates[i].start_uid),
                                SD_JSON_BUILD_PAIR_UNSIGNED("startGid", info->delegates[i].start_gid),
                                SD_JSON_BUILD_PAIR_UNSIGNED("size", info->delegates[i].size));
                if (r < 0)
                        return r;
        }
        ...
        for (size_t i = 0; i < info->n_delegates; i++) {
                _cleanup_(sd_json_variant_unrefp) sd_json_variant *delegate_def = NULL, *parent_array = NULL;
                _cleanup_free_ char *delegate_buf = NULL, *delegate_uid_fn = NULL, *delegate_gid_fn = NULL;

                if (asprintf(&delegate_uid_fn, "u" UID_FMT ".delegate", info->delegates[i].start_uid) < 0) {
                        r = log_oom_debug();
                        goto fail;
                }
                ...
                _cleanup_(delegated_userns_info_done) DelegatedUserNamespaceInfo existing = {
                        .start_uid = UID_INVALID,
                        .start_gid = GID_INVALID,
                };

                r = userns_registry_load_delegation_by_uid(dir_fd, info->delegates[i].start_uid, &existing);
                if (r >= 0) {
                        /* Delegation file exists — append old owner to parent chain */
                        for (size_t j = 0; j < existing.n_parent_userns; j++) {
                                r = sd_json_variant_append_arrayb(
                                                &parent_array,
                                                SD_JSON_BUILD_UNSIGNED(existing.parent_userns[j]));
                                if (r < 0)
                                        goto fail;
                        }
                        ...
                        log_debug_errno(r, "Failed to load existing delegation for UID " UID_FMT ": %m", info->delegates[i].start_uid);
                        goto fail;
                }
                ...
                r = sd_json_buildo(
                                ...
                                SD_JSON_BUILD_PAIR_UNSIGNED("startGid", info->delegates[i].start_gid),
                                SD_JSON_BUILD_PAIR_UNSIGNED("size", info->delegates[i].size),
                                SD_JSON_BUILD_PAIR_CONDITION(!!parent_array, "parentUserns", SD_JSON_BUILD_VARIANT(parent_array)));
                if (r < 0)
                        goto fail;
                ...
                if (asprintf(&delegate_gid_fn, "g" GID_FMT ".delegate", info->delegates[i].start_gid) < 0) {
                        r = log_oom_debug();
                        goto fail;
                }
...
int userns_registry_remove(int dir_fd, UserNamespaceInfo *info) {
        ...
        for (size_t i = 0; i < info->n_delegates; i++) {
                ...
                /* Check if this delegation has parent user namespaces. If so, restore ownership to
                 * the last parent instead of removing the delegation file entirely. */
                _cleanup_(delegated_userns_info_done) DelegatedUserNamespaceInfo existing = {
                        .start_uid = UID_INVALID,
                        .start_gid = GID_INVALID,
                };
                ...
                r = userns_registry_load_delegation_by_uid(dir_fd, info->delegates[i].start_uid, &existing);
                if (r >= 0 && existing.n_parent_userns > 0) {
                        _cleanup_(sd_json_variant_unrefp) sd_json_variant *delegate_def = NULL, *parent_array = NULL;
                        _cleanup_free_ char *delegate_buf = NULL;

                        /* Pop the last parent userns inode to become the new owner */
                        uint64_t new_owner = existing.parent_userns[existing.n_parent_userns - 1];

                        /* Rebuild parent array without the last entry */
                        for (size_t j = 0; j + 1 < existing.n_parent_userns; j++) {
                                r = sd_json_variant_append_arrayb(
                                                &parent_array,
                                                SD_JSON_BUILD_UNSIGNED(existing.parent_userns[j]));
                                if (r < 0) {
                        ...
                        r = sd_json_variant_format(delegate_def, /* flags= */ 0, &delegate_buf);
                        if (r < 0) {
                                RET_GATHER(ret, log_debug_errno(r, "Failed to format delegation JSON object: %m"));
                                continue;
                        }
                ...
                /* No parent chain (or failed to load) — just remove the delegation files */
                RET_GATHER(ret, RET_NERRNO(unlinkat(dir_fd, delegate_uid_fn, 0)));

                if (asprintf(&delegate_gid_fn, "g" GID_FMT ".delegate", info->delegates[i].start_gid) < 0)
                        return log_oom_debug();

                RET_GATHER(ret, RET_NERRNO(unlinkat(dir_fd, delegate_gid_fn, 0)));
        }
...
int userns_registry_load_delegation_by_uid(int dir_fd, uid_t start, DelegatedUserNamespaceInfo *ret) {
        ...
        static const sd_json_dispatch_field dispatch_table[] = {
                ...
                { "size",         SD_JSON_VARIANT_UNSIGNED, sd_json_dispatch_uint32,      offsetof(DelegatedUserNamespaceInfo, size),         SD_JSON_MANDATORY },
                { "parentUserns", SD_JSON_VARIANT_ARRAY,    dispatch_parent_userns_array, 0,                                                  0                 },
                {}
        };
        ...
        r = sd_json_parse_file_at(/* f= */ NULL, dir_fd, fn, /* flags= */ 0, &v, /* reterr_line= */ NULL, /* reterr_column= */ NULL);
        if (r < 0)
                return r;

        DelegatedUserNamespaceInfo data = {
                .start_uid = UID_INVALID,
                .start_gid = GID_INVALID,
        };
