...
static int help(void) {
        ...
        printf("%s [OPTIONS...] COMMAND\n"
               "\n%sEnumerate storage volumes and services%s\n"
               "\n%sCommands:%s\n",
...
static int verb_list(int argc, char *argv[], uintptr_t data, void *userdata) {
        ...
        r = runtime_directory_generic(arg_runtime_scope, "systemd/io.systemd.Volumes", &socket_path);
        if (r < 0)
...
static int run_as_mount_helper(int argc, char *argv[]) {
        ...
        if (optind + 2 != argc)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "Expected a volume specification and target directory as only arguments.");
