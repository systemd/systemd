...
static int open_volumes_dir(void) {
        ...
        _cleanup_close_ int volumes_fd = chase_and_openat(state_fd, "volumes", CHASE_TRIGGER_AUTOFS|CHASE_MUST_BE_DIRECTORY, O_CLOEXEC|O_DIRECTORY, /* ret_path= */ NULL);
        if (volumes_fd == -ENOENT) {
                ...
                if (volumes_fd == -EEXIST)
                        volumes_fd = chase_and_openat(state_fd, "volumes", CHASE_TRIGGER_AUTOFS|CHASE_MUST_BE_DIRECTORY, O_CLOEXEC|O_DIRECTORY, /* ret_path= */ NULL);
        }
        if (volumes_fd < 0)
