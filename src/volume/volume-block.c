...
static int block_device_pick_name(
        ...
        /* We only use paths here that identify the device by hardware properties, not by contents, for security reasons. */

        const char *by_id = NULL, *by_path = NULL;
        _cleanup_strv_free_ char **aliases = NULL;
        STRV_FOREACH(sl, d->symlinks) {
                if (path_startswith(*sl, "/dev/disk/by-id"))
                        by_id = *sl;
                else if (path_startswith(*sl, "/dev/disk/by-path"))
                        by_path = *sl;
                else {
...
static int vl_method_list(
        ...
        FOREACH_ARRAY(d, l, n) {
                ...
                r = sd_varlink_replybo(
                                ...
                                SD_JSON_BUILD_PAIR_STRING("type", "block"),
