...
static int block_device_pick_name(
        ...
        static const char *const prefixes[] = {
                 ...
                 * use identifiers that are not depending on the contents, i.e. that are chose chosen by
                 * whoever created the device or are hardware properties, but not names generated from
                 * superblock meta information or so. */
                "/dev/mapper",
        ...
        static const char* found[ELEMENTSOF(prefixes)] = {};
        ...
        if (best == SIZE_MAX) /* No preferred prefix found, use the kernel device name */
                *ret_name = d->node;
                ...
                /* If there are any less preferred prefies also add them to the aliases array */
...
static bool block_device_match(const BlockDevice *d, const char *match) {
        ...
        if (fnmatch(match, d->node, /* flags= */ 0) == 0)
                return true;

        STRV_FOREACH(sl, d->symlinks)
                if (fnmatch(match, *sl, /* flags= */ 0) == 0)
                        return true;
...
static int vl_method_list_templates(
        ...
        /* This volume provider does not support templates */
        assert(link);
        assert(FLAGS_SET(flags, SD_VARLINK_METHOD_MORE));
...
static int vl_method_acquire(
        ...
        if (!PATH_STARTSWITH_SET(p.name, "/dev", "/sys") || !path_is_absolute(p.name) || !path_is_normalized(p.name))
        ...
        if (p.request_as >= 0 && p.request_as != VOLUME_BLK)
                return sd_varlink_error(link, "io.systemd.Volumes.TypeNotSupported", NULL);
        ...
        r = varlink_verify_polkit_async(
                        link,
                        /* bus= */ NULL,
                        "io.systemd.volumes.block.acquire",
                        details,
                        polkit_registry);
        if (r <= 0)
                return r;
        ...
        r = sd_device_new_from_path(&d, p.name);
        if (ERRNO_IS_NEG_DEVICE_ABSENT(r))
                return sd_varlink_error(link, "io.systemd.Volumes.NoSuchVolume", NULL);
        if (r < 0)
                return r;
        ...
        if (p.read_only <= 0) {
                ...
                if (r < 0)
                        ...
                        if (p.read_only == 0)
                                return sd_varlink_error(link, "io.systemd.Volumes.ReadOnlyVolume", NULL);
        ...
        int open_flags = p.read_only > 0 ? O_RDONLY : O_RDWR;
        fd = sd_device_open(d, open_flags);
        if (fd < 0) {
                if (!ERRNO_IS_NEG_FS_WRITE_REFUSED(fd) || p.read_only >= 0)
                        return log_device_debug_errno(d, fd, "Failed to open device in %s mode: %m", p.read_only > 0 ? "read-only" : "read-write");
        ...
        return sd_varlink_replybo(
                        ...
                        SD_JSON_BUILD_PAIR_STRING("type", "blk"),
                        SD_JSON_BUILD_PAIR_BOOLEAN("readOnly", open_flags == O_RDONLY));
}
...
static int help(void) {
        ...
        r = terminal_urlify_man("systemd-volume-block", "8", &link);
        if (r < 0)
                return log_oom();
        ...
        printf("%s [OPTIONS...]\n"
               "\n%sSimple block device backed storage volume service%s\n"
               "\n%sOptions:%s\n",
