...
static int parse_argv(int argc, char *argv[], char ***ret_args) {
        ...
        FOREACH_OPTION_OR_RETURN(c, &opts)
                ...
                OPTION_LONG("credentials", NULL,
                            "Measure marked boot credentials into NvPCR 'credentials'"):
        ...
        if (r > 0)
                ...
                arg_pcr_mask =
                        ...
                        (arg_product_id || arg_credentials || arg_login) ? 0 :                                     /* → NvPCR */
...
static int measure_credentials(Tpm2UserspaceEventType event) {
        ...
        credentials_fd = open(credentials_directory, O_RDONLY|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW);
        if (credentials_fd < 0)
