...
static bool arg_interactive_credentials = false;
...
COMMAND(
        "systemd-pcrextend\0",
        "Extend a TPM2 PCR with boot phase, machine ID, file system ID, interactive credential or user record.",
        .argspec =
                "WORD\0"
                "--file-system=PATH\0"
                "--machine-id\0"
                "--product-id\0"
                "--interactive-credentials\0"
                "--login=UID|USER\0",
...
static int parse_argv(int argc, char *argv[], char ***ret_args) {
        ...
        FOREACH_OPTION_OR_RETURN(c, &opts)
                ...
                OPTION_LONG("interactive-credentials", NULL,
                            "Measure marked boot credentials into NvPCR 'interactive-credentials'"):
                        arg_interactive_credentials = true;
                        break;
        ...
        if (!!arg_file_system + arg_machine_id + arg_product_id + arg_interactive_credentials + !!arg_login > 1)
                return log_error_errno(
                                SYNTHETIC_ERRNO(EINVAL),
                                "--file-system=, --machine-id, --product-id, --interactive-credentials, "
                                "--login= may not be combined.");

        if (arg_pcr_mask != 0 && arg_nvpcr_name)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "--pcr= and --nvpcr= may not be combined.");
        if (arg_interactive_credentials && arg_pcr_mask != 0)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "--interactive-credentials may not be combined with --pcr=.");
        ...
        if (r > 0)
                ...
                else if (!arg_product_id && !arg_interactive_credentials && !arg_login)
                ...
                r = free_and_strdup_warn(&arg_nvpcr_name,
                                         ...
                                         arg_interactive_credentials ? "interactive-credentials" :
                                         arg_login       ? "login" :
                                                           NULL);
                if (r < 0)
                        return r;
...
static int measure_credentials(Tpm2UserspaceEventType event) {
        ...
        if (markers_fd < 0) {
                ...
                r = readdir_all(markers_fd, RECURSE_DIR_SORT|RECURSE_DIR_MUST_BE_REGULAR, &de);
                if (r < 0)
                        return log_error_errno(r, "Failed to enumerate credentials to measure: %m");
        }

        struct dirent **entries = de ? de->entries : NULL;
        ...
        FOREACH_ARRAY(i, entries, n_credentials) {
...
static int run(int argc, char *argv[]) {
        ...
        } else if (arg_interactive_credentials) {
                if (n_args != 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Expected no argument.");
        ...
        if (arg_interactive_credentials)
                r = measure_credentials(event);
