...
static bool item_type_does_not_follow_symlinks_on_create(ItemType t) {
        return IN_SET(t,
                      CREATE_DIRECTORY,
                      TRUNCATE_DIRECTORY,
                      CREATE_SUBVOLUME,
...
static int tmpfiles_open_root(const char *path, const char **ret_path) {
        ...
        return RET_NERRNO(open(root, O_NOCTTY|O_CLOEXEC|O_DIRECTORY|O_PATH));
}

static int log_mkdir_parent_failure(const Item *i, int r) {
        assert(i);
...
static int mkdir_parents_no_follow(const Item *i) {
        ...
        parent_fd = tmpfiles_open_root(i->path, &p);
        if (parent_fd < 0)
                return log_mkdir_parent_failure(i, parent_fd);
        ...
        for (;;) {
                _cleanup_close_ int child_fd = -EBADF;
                const char *first;
                int n = path_find_first_component(&p, /* accept_dot_dot= */ false, &first);

                if (n < 0)
                        return log_mkdir_parent_failure(i, n);
                if (n == 0 || isempty(p))
                        return 0;

                char component[path_len + 1];
                *mempcpy_typesafe(component, first, n) = 0;

                child_fd = openat(parent_fd, component, O_NOCTTY|O_CLOEXEC|O_DIRECTORY|O_NOFOLLOW|O_PATH);
                if (child_fd < 0) {
                        if (errno != ENOENT)
                                return log_mkdir_parent_failure(i, -errno);

                        WITH_UMASK(0000)
                                r = mkdirat_label(parent_fd, component, 0755, arg_label_context);
                        if (r < 0 && r != -EEXIST)
                                return log_mkdir_parent_failure(i, r);
                ...
                close_and_replace(parent_fd, child_fd);
        }
}

/* If child_mode is non-zero, rm_if_wrong_type_safe will be executed for the last path component. */
static int mkdir_parents_rm_if_wrong_type(mode_t child_mode, const char *path, bool prohibit_symlinks) {
        _cleanup_close_ int parent_fd = -EBADF;
        ...
        if (prohibit_symlinks) {
                parent_fd = tmpfiles_open_root(path, &todo);
                if (parent_fd < 0)
                        return log_error_errno(parent_fd, "Failed to open tmpfiles root: %m");
        } else {
...
static int mkdir_parents_item(Item *i, mode_t child_mode) {
        bool prohibit_symlinks = item_type_does_not_follow_symlinks_on_create(i->type);
        int r;

        if (i->try_replace) {
                r = mkdir_parents_rm_if_wrong_type(child_mode, i->path, prohibit_symlinks);
                if (r < 0 && r != -ENOENT)
                        return r;
        } else if (prohibit_symlinks) {
                r = mkdir_parents_no_follow(i);
                if (r < 0)
                        return r;
        } else
                WITH_UMASK(0000)
