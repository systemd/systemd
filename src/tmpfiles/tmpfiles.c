...
static int parse_acl_cond_exec(
        ...
        _cleanup_(acl_freep) acl_t parsed = NULL, old = NULL;
        acl_entry_t entry, parsed_entry;
...
static int fd_set_acls(
        ...
        if (r == 0 && item->acl_access_exec) {
                r = parse_acl_cond_exec(FORMAT_PROC_FD_PATH(fd), item->acl_access_exec, &access_exec_parsed, st, !item->append_or_force);
                if (r < 0)
