...
static int item_instance_needs_cleanup(
                Item *i,
                const char *instance,
        ...
        *ret = needs_cleanup(atime_nsec, btime_nsec, ctime_nsec, mtime_nsec,
...
static int clean_remove_item_instance_at(
        ...
        bool cleanup_needed;
        r = item_instance_needs_cleanup(i, instance, sx, &cleanup_needed);
        if (r < 0)
                return r;
        if (!cleanup_needed)
                return 0;
        ...
        if (S_ISDIR(sx->stx_mode)) {
                _cleanup_closedir_ DIR *d = xopendirat_nomod(parent_fd, name);
                if (!d) {
        ...
        if (!arg_dry_run &&
            ...
            errno != ENOENT)
                return log_error_errno(errno, "Failed to remove %s: %m", instance);
...
static int clean_recursive_remove_item_instance(
        ...
        bool cleanup_needed;
        r = item_instance_needs_cleanup(i, instance, &dir_sx, &cleanup_needed);
        if (r < 0)
                return r;
