...
static int parse_acls_from_arg(Item *item) {
        ...
        if (r < 0) {
                if (arg_graceful && IN_SET(r, -EINVAL, -ENOENT, -ESRCH)) {
                        log_debug_errno(r, "Failed to parse ACL \"%s\", ignoring: %m", item->argument);
                        return 0;
                }

                return log_warning_errno(r, "Failed to parse ACL \"%s\": %m", item->argument);
        }
#else
...
static int parse_caps_from_arg(Item *item) {
        ...
        if (r < 0) {
                if (arg_graceful) {
                        log_debug_errno(r, "Failed to parse capabilities \"%s\", ignoring: %m", item->argument);
                        return 0;
                }

                return log_warning_errno(r, "Failed to parse capabilities \"%s\": %m", item->argument);
        }
