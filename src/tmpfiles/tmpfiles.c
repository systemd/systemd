...
shortcut:
        ...
        return label_fix_full(fd, /* inode_path= */ NULL, /* label_path= */ path, /* flags= */ 0, /* label_userdata= */ NULL);
}
...
static int create_directory_or_subvolume(
        ...
        if (!subvol || ERRNO_IS_NEG_NOT_SUPPORTED(r)) {
                ...
                if (!arg_dry_run)
                        WITH_UMASK(0000)
                                r = mkdirat_label(pfd, bn, mode, /* label_userdata= */ NULL);
        }
        ...
        for (const char *e = path;;) {
                ...
                *mempcpy_typesafe(t, s, e - s) = 0;
                ...
                if (r == -ENOENT) {
                        if (!arg_dry_run) {
                                WITH_UMASK(0000)
                                        r = mkdirat_label(parent_fd, t, 0755, /* label_userdata= */ NULL);
                                if (r < 0) {
                                        _cleanup_free_ char *parent_name = NULL;
