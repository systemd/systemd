...
static bool needs_cleanup(
        ...
        /* Ignore "ctime" for directories, we change it when deleting. */
        if (!is_dir && FLAGS_SET(age_by, AGE_BY_CTIME) && ctime != NSEC_INFINITY && ctime >= cutoff) {
                char a[FORMAT_TIMESTAMP_MAX];
...
static int parse_line(
        ...
        if (!empty_or_dash(age)) {
                ...
                if (r == -ENOMEM)
                        return log_oom();
                if (r == 0) {
                        /* We found a ":"; parse the age-by argument. */
