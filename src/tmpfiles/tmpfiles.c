...
static int age_by_format(AgeBy age_by, char **ret_age_by_fmt) {
        ...
        p = types;
        if (age_by & AGE_BY_ATIME)
                *p++ = 'a';

        if (age_by & AGE_BY_BTIME)
                *p++ = 'b';

        if (age_by & AGE_BY_CTIME)
                *p++ = 'c';

        if (age_by & AGE_BY_MTIME)
                *p++ = 'm';

        /* Stop and rewind back to the beginning. */
        *p = 0;
        p = types;

        do {
                if (asprintf(&q, "%ctime", *p) < 0)
                        return -ENOMEM;

                if (strv_push(&split, q) < 0)
                        return -ENOMEM;
        } while(*(++p));
        ...
        if (asprintf(ret_age_by_fmt, "(age-by: %s)", tmp) < 0)
                return -ENOMEM;
...
static int clean_item_instance(Item *i, const char* instance) {
        ...
        if (age_by_format(i->age_by, &age_by) < 0)
                return log_oom();
...
static int parse_line(
        ...
        if (!empty_or_dash(age)) {
                ...
                /* Format: "age-by:age"; where age-by is "[abcm]+". */
                if (!strchr(a, ':')) {
                        seconds = strdup(a);
                        if (!seconds)
                                return log_oom();
