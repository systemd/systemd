...
static int rm_if_wrong_type_safe(
        ...
        assert(!follow || parent_st);

        if (!filename_is_valid(name))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "\"%s\" is not a valid filename.", name);

        if (fstatat(parent_fd, name, &st, follow ? 0 : AT_SYMLINK_NOFOLLOW) < 0) {
        ...
        if (follow && parent_st->st_uid != 0 && parent_st->st_uid != st.st_uid) {
                (void) fd_get_path(parent_fd, &parent_name);
                return log_error_errno(SYNTHETIC_ERRNO(ENOLINK), "\"%s\" is not a valid filename.", name);
        }
        ...
        if (follow && fstatat(parent_fd, name, &st, AT_SYMLINK_NOFOLLOW) < 0)
                return log_error_errno(errno,
                                "Failed to stat \"%s\" at \"%s\": %m", name, strna(parent_name));

        /* Do not remove mount points. */
        r = fd_is_mount_point(parent_fd, name, follow ? AT_SYMLINK_FOLLOW : 0);
        if (r < 0)
                return log_error_errno(r, "Failed to check if  \"%s/%s\" is a mountpoint: %m", strna(parent_name), name);
        else if (r > 0)
                return log_error_errno(SYNTHETIC_ERRNO(EBUSY),
                                "Not removing  \"%s/%s\" because it is a mountpoint.", strna(parent_name), name);
        ...
        if (r < 0)
                return log_error_errno(errno, "Failed to remove \"%s\" at \"%s\": %m", name, strna(parent_name));
...
static int mkdir_parents_rm_if_wrong_type(mode_t child_mode, const char *path) {
        ...
        for (;;) {
                ...
                if (next_fd < 0) {
                        (void) fd_get_path(parent_fd, &parent_name);
                        return log_error_errno(errno, "Failed to open \"%s\" at \"%s\": %m", t, parent_name);
                }
                if (fstat(next_fd, &parent_st) < 0) {
                        (void) fd_get_path(parent_fd, &parent_name);
                        return log_error_errno(errno, "Failed to stat \"%s\" at \"%s\": %m", t, parent_name);
                }
