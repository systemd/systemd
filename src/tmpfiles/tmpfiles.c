...
static int parse_acl_cond_exec(
        ...
        if (!S_ISDIR(st->st_mode)) {
                ...
                has_exec = false;
                bool exec_unmasked = false, exec_masked = false, have_mask = false, mask_exec = false;
                     ...
                     r = sym_acl_get_entry(old, ACL_NEXT_ENTRY, &entry)) {
                        ...
                        if (r > 0) {
                                if (IN_SET(tag, ACL_USER_OBJ, ACL_OTHER))
                                        exec_unmasked = true;   /* never bounded by the mask */
                                else
                                        exec_masked = true;
                        }
                }
                if (r < 0)
                        return -errno;
                has_exec = exec_unmasked || (exec_masked && (!have_mask || mask_exec));
