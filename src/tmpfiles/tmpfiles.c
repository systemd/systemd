...
static int rm_if_wrong_type_safe(
        ...
        r = fstatat_harder(parent_fd, name, &st, flags, REMOVE_CHMOD | REMOVE_RESTORE_MODE);
        if (r < 0) {
        ...
        if (follow_links) {
                r = fstatat_harder(parent_fd, name, &st, AT_SYMLINK_NOFOLLOW, REMOVE_CHMOD | REMOVE_RESTORE_MODE);
                if (r < 0)
        ...
        if (r < 0)
                (void) log_warning_errno(r, "Failed to check if  \"%s/%s\" is a mountpoint: %m; Continuing",
        ...
        if ((st.st_mode & S_IFMT) == S_IFDIR) {
                ...
                r = unlinkat_harder(parent_fd, name, AT_REMOVEDIR, REMOVE_CHMOD | REMOVE_RESTORE_MODE);
        } else
                r = unlinkat_harder(parent_fd, name, 0, REMOVE_CHMOD | REMOVE_RESTORE_MODE);
        if (r < 0)
