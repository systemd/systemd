...
static int rm_if_wrong_type_safe(
        ...
        if (fstatat(parent_fd, name, &st, flags) < 0) {
        ...
        if (follow_links && fstatat(parent_fd, name, &st, AT_SYMLINK_NOFOLLOW) < 0)
                return log_error_errno(errno,
                                "Failed to stat \"%s\" at \"%s\": %m", name, strna(parent_name));
...
static int mkdir_parents_rm_if_wrong_type(mode_t child_mode, const char *path) {
        ...
        if (!e)
                ...
                return child_mode ? rm_if_wrong_type_safe(child_mode, AT_FDCWD, NULL, path, AT_SYMLINK_NOFOLLOW) : 0;
        ...
        for (;;) {
                ...
                if (*e == 0)
                        return child_mode ? rm_if_wrong_type_safe(child_mode, parent_fd, &parent_st, t, AT_SYMLINK_NOFOLLOW) : 0;
                else {
