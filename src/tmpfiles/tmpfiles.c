...
static int dir_cleanup(
        ...
        FOREACH_DIRENT_ALL(dent, d, break) {
                ...
                if (S_ISDIR(sx.stx_mode)) {
                        ...
                        if (FLAGS_SET(age_by_dir, AGE_BY_MTIME)) {
                        ...
                        if (FLAGS_SET(age_by_dir, AGE_BY_ATIME)) {
                        ...
                        if (FLAGS_SET(age_by_dir, AGE_BY_BTIME)) {
                        ...
                        if (FLAGS_SET(age_by_file, AGE_BY_MTIME)) {
                                if (mtime_nsec != NSEC_INFINITY && mtime_nsec >= cutoff_nsec) {
                                        char a[FORMAT_TIMESTAMP_MAX];
                        ...
                        if (FLAGS_SET(age_by_file, AGE_BY_ATIME)) {
                                if (atime_nsec != NSEC_INFINITY && atime_nsec >= cutoff_nsec) {
                                        char a[FORMAT_TIMESTAMP_MAX];
                        ...
                        if (FLAGS_SET(age_by_file, AGE_BY_CTIME)) {
                        ...
                        if (FLAGS_SET(age_by_file, AGE_BY_BTIME)) {
                                if (btime_nsec != NSEC_INFINITY && btime_nsec >= cutoff_nsec) {
                                        char a[FORMAT_TIMESTAMP_MAX];
...
static int format_age_by(const Item *item, char **ret_age_by_fmt) {
...
static int clean_item_instance(Item *i, const char* instance) {
        ...
        if (DEBUG_LOGGING) {
                ...
                if (format_age_by(i, &age_by_fmt) < 0)
                        return log_oom();
...
static int parse_age_by_from_arg(const char *age_by_str, Item *item) {
        ...
        /* Note: "ctime" is ignored for directories. */
        item->age_by_file = ab_f > 0 ? ab_f : AGE_BY_DEFAULT_FILE;
        item->age_by_dir = (ab_d > 0 && ab_d != AGE_BY_CTIME) ? ab_d : AGE_BY_DEFAULT_DIR;
