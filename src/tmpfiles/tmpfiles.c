...
static int capability_vfs_from_string(const char *s, FCapsUpdate *ret) {
        ...
        for (const char *p = s;;) {
                ...
                if (sep == '=' && streq(keys, "rootid")) {
                        r = parse_uid(value, &set.rootid);
                        if (r < 0)
                        ...
                        if (STR_IN_SET(keys, "all", ""))
                                ...
                                for (const char *remaining_keys = keys; remaining_keys && *remaining_keys;) {
                                        ...
                                        if (r < 0)
                                                return log_debug_errno(r, "Failed to parse capablity list '%s': %m", keys);
                                        if (r == 0)
                                                break;
                                        if (streq(key, "all"))
                                                caps = all_capabilities();
        ...
        if (append) {
                ...
                if (r == -ENODATA)
                        ...
                        if (xattr_data_len != expected_size)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Size of file capabilities are not matching its type");
