...
shortcut:
        ...
        return label_fix_full(fd, /* inode_path= */ NULL, /* label_path= */ path, 0, NULL);
}
...
static int create_file(
        ...
        WITH_UMASK(0000) {
                mac_selinux_create_file_prepare(path, S_IFREG, /* label_userdata= */ NULL);
...
static int create_directory_or_subvolume(
        ...
        if (!subvol || ERRNO_IS_NEG_NOT_SUPPORTED(r)) {
                ...
                if (!arg_dry_run)
                        WITH_UMASK(0000)
                                r = mkdirat_label(pfd, bn, mode, NULL);
        }
...
static int create_device(
        ...
        WITH_UMASK(0000) {
                mac_selinux_create_file_prepare(i->path, file_type, /* label_userdata= */ NULL);
...
static int create_fifo(Context *c, Item *i) {
        ...
        WITH_UMASK(0000) {
                mac_selinux_create_file_prepare(i->path, S_IFIFO, /* label_userdata= */ NULL);
...
static int create_symlink(Context *c, Item *i) {
        ...
        mac_selinux_create_file_prepare(i->path, S_IFLNK, /* label_userdata= */ NULL);
        ...
        if (!good) {
                ...
                r = symlinkat_atomic_full(i->argument, pfd, bn, SYMLINK_LABEL, NULL);
                if (IN_SET(r, -EISDIR, -EEXIST, -ENOTEMPTY)) {
                        r = rm_rf_child(pfd, bn, REMOVE_PHYSICAL);
                        if (r < 0)
                                return log_error_errno(r, "rm -rf %s failed: %m", i->path);
        ...
        for (const char *e = path;;) {
                ...
                *mempcpy_typesafe(t, s, e - s) = 0;
                ...
                if (r == -ENOENT) {
                        if (!arg_dry_run) {
                                WITH_UMASK(0000)
                                        r = mkdirat_label(parent_fd, t, 0755, NULL);
                                if (r < 0) {
                                        _cleanup_free_ char *parent_name = NULL;
