...
/* Asynchronous cancellation must be awaited from a fiber on the same event loop. */
...
/* The group and all its children must belong to the same event loop. */
...
/* The target must belong to the calling fiber's event loop. */
