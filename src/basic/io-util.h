...
/* Short writes in data runs are retried so that filesystem errors such as ENOSPC are surfaced instead of
 * being converted to EIO. Returns 0 on success, and a negative errno on failure. */
int sparse_write(int fd, const void *p, size_t sz, size_t run_length);
