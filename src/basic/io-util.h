 ...
 * between poll() and epoll() event masks — notably at the io_uring POLL_ADD boundary, whose
 * `poll32_events` input and `cqe->res` output speak poll() values, not epoll() values. Bits
 * outside the mapped set (EPOLLET, EPOLLONESHOT, POLLNVAL, …) are dropped. */
