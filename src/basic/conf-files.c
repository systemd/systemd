...
int conf_file_new_at(const char *path, int rfd, ConfFilesFlags flags, ChaseFlags chase_flags, ConfFile **ret) {
        int r;
        ...
        *c = (ConfFile) {
        ...
        if (r < 0)
                return log_full_errno(log_level, r, "Failed to chase '%s%s': %m", empty_to_root(root), skip_leading_slash(c->original_path));

        if (c->fd >= 0 && fstat(c->fd, &c->st) < 0)
                return log_full_errno(log_level, r, "Failed to stat '%s%s': %m", empty_to_root(root), skip_leading_slash(c->resolved_path));

        *ret = TAKE_PTR(c);
        return 0;
}

int conf_file_new(const char *path, const char *root, ConfFilesFlags flags, ChaseFlags chase_flags, ConfFile **ret) {
        int r;
        ...
        r = conf_file_new_at(path, rfd, flags, chase_flags, &c);
        if (r < 0)
                return r;
...
static int conf_files_list_impl(
        ...
        if (replacement) {
                r = conf_file_new_at(replacement, rfd, flags, CHASE_NONEXISTENT, &c);
                if (r < 0)
                        return r;
        }
                ...
                if (r < 0) {
                        if (r != -ENOENT)
                                log_full_errno(conf_files_log_level(flags), r,
                                               ...
                                               strempty(root), skip_leading_slash(*p));
                        continue;
                }
