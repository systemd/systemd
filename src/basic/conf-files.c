        ...
        *c = (ConfFile) {
        ...
        if (r >= 0) {
                r = chaseat(rfd, dirpath,
                            CHASE_AT_RESOLVE_IN_ROOT | (FLAGS_SET(chase_flags, CHASE_NONEXISTENT) ? 0 : CHASE_MUST_BE_DIRECTORY),
        ...
        if (c->fd >= 0 && fstatat(c->fd, "", &c->st, AT_EMPTY_PATH) < 0)
...
static int files_add(
        ...
        FOREACH_DIRENT(de, dir, return log_debug_errno(errno, "Failed to read directory '%s/%s': %m",
                ...
                struct stat st = {};
                if (fd >= 0 && fstatat(fd, "", &st, AT_EMPTY_PATH) < 0) {
