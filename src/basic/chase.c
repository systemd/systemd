...
int chaseat(int root_fd, int dir_fd, const char *path, ChaseFlags flags, char **ret_path, int *ret_fd) {
        ...
        /* If we get an absolute path (or are supposed to treat a relative path as an absolute path), we
         * ignore dir_fd and resolve relative to the root fd, so we need to return an absolute path. The
         * exception is when we're operating within a non-host root (root_fd != XAT_FDROOT) and dir_fd equals
         * root_fd: in that case we return a relative path so the result works seamlessly when passed to an
         * openat() style function afterwards, which would otherwise ignore the directory fd if an absolute
         * path was passed.
         *
         * - no chroot boundary (XAT_FDROOT) and we treat the path as absolute => absolute result as there's
         *   no need to worry about at() style calls.
         * - root_fd != dir_fd and we treat the path as absolute => absolute result so the caller knows it's
         *   relative to root_fd.
         */
        ...
        struct statx stx;
        const unsigned mask = STATX_TYPE|STATX_UID|STATX_INO;
        r = xstatx_full(fd,
                        /* path= */ NULL,
                        /* statx_flags= */ 0,
                        XSTATX_MNT_ID_BEST,
        ...
        /* Remember stat data of the root, so that we can recognize it later. When root_fd == dir_fd (the
         * usual case from chaseat()) root_stx equals stx; otherwise stat the separate root. */
        struct statx root_stx;
        if (root_fd == dir_fd)
                root_stx = stx;
        else {
                r = xstatx_full(root_fd,
                                /* path= */ NULL,
                                /* statx_flags= */ 0,
                                XSTATX_MNT_ID_BEST,
                                mask,
...
int chase_and_open(
        ...
        r = chase_xopenat(path_fd, strempty(fname), chase_flags, open_flags|O_NOFOLLOW, /* xopen_flags= */ 0);
        if (r < 0)
                return r;
