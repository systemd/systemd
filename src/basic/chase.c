...
int chaseat(int dir_fd, const char *path, ChaseFlags flags, char **ret_path, int *ret_fd) {
        ...
        for (todo = buffer;;) {
                ...
                /* Otherwise let's see pin it by file descriptor, via O_PATH. Sounds pretty obvious to do
                 * this, right? You just do open() with O_PATH, and there you go. But uh, it's not that
