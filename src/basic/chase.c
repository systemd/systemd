...
static int log_unexpected_mode(int fd, mode_t mode, mode_t expected, const char *path, ChaseFlags flags) {
        ...
        return log_warning_errno(SYNTHETIC_ERRNO(ENOLINK),
                                 "Detected mode 0%04o instead of 0%04o on %s during canonicalization of %s.",
 ...
 * and are not checked. Symlinks are never as there mode is irrelevant. The flags must disable the openat2()
 * and open() shortcuts. */
