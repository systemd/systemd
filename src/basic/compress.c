...
static inline void bz_stream_compress_end(bz_stream *s) {
        sym_BZ2_bzCompressEnd(s);
}

static inline void bz_stream_decompress_end(bz_stream *s) {
...
int dlopen_bzip2(void) {
        ...
        SD_ELF_NOTE_DLOPEN(
                        "bzip2",
                        "Support bzip2 decompression in imports",
                        SD_ELF_NOTE_DLOPEN_PRIORITY_SUGGESTED,
                        "libbz2.so.1.0");

        return dlopen_many_sym_or_warn(
                        &bzip2_dl,
                        "libbz2.so.1.0", LOG_DEBUG,
...
static int compress_blob_bzip2(
        ...
        _cleanup_(bz_stream_compress_end) bz_stream s = {};

        r = sym_BZ2_bzCompressInit(&s, level < 0 ? 9 : level, 0, 0);
        if (r != BZ_OK)
                return -ENOMEM;
...
static int decompress_blob_gzip(
        ...
        r = sym_inflateInit2_(&s, 15+16, ZLIB_VERSION, (int) sizeof(s));
        if (r != Z_OK)
                return -ENOMEM;
...
static int decompress_blob_bzip2(
        ...
        _cleanup_(bz_stream_decompress_end) bz_stream s = {};

        r = sym_BZ2_bzDecompressInit(&s, 0, 0);
        if (r != BZ_OK)
                return -ENOMEM;
...
int decompress_zlib_raw(
        ...
        r = sym_inflateInit2_(&s, wbits, ZLIB_VERSION, (int) sizeof(s));
        if (r != Z_OK)
                return -EIO;
...
static int decompress_startswith_bzip2(
        ...
        for (;;) {
                ...
                allocated = MALLOC_SIZEOF_SAFE(*buffer);
                s.next_out = (char*) *buffer + allocated - s.avail_out;
        }
#else
        return -EPROTONOSUPPORT;
#endif
...
int compress_stream(
        ...
        for (;;) {
                uint8_t input[BUFSIZ];
                size_t m = sizeof(input);
                ssize_t n;
...
static int decompressor_new(Decompressor **ret, Compression type) {
        ...
        case COMPRESSION_GZIP:
                ...
                r = sym_inflateInit2_(&c->gzip, 15+16, ZLIB_VERSION, (int) sizeof(c->gzip));
                if (r != Z_OK)
                        return -EIO;
                break;
#endif
        ...
        case COMPRESSION_BZIP2:
                ...
                r = sym_BZ2_bzDecompressInit(&c->bzip2, 0, 0);
                if (r != BZ_OK)
                        return -EIO;
                break;
#endif
...
int decompress_stream_by_filename(const char *filename, int fdf, int fdt, uint64_t max_bytes) {
        ...
        if (endswith(filename, ".xz"))
                return decompress_stream(COMPRESSION_XZ, fdf, fdt, max_bytes);
        if (endswith(filename, ".lz4"))
                return decompress_stream(COMPRESSION_LZ4, fdf, fdt, max_bytes);
        if (endswith(filename, ".zst"))
                return decompress_stream(COMPRESSION_ZSTD, fdf, fdt, max_bytes);
        if (endswith(filename, ".gz"))
                return decompress_stream(COMPRESSION_GZIP, fdf, fdt, max_bytes);
        if (endswith(filename, ".bz2"))
                return decompress_stream(COMPRESSION_BZIP2, fdf, fdt, max_bytes);
...
#define COMPRESSOR_BUFFER_SIZE (128U*1024U)
...
int decompressor_push(Decompressor *c, const void *data, size_t size, DecompressorCallback callback, void *userdata) {
        ...
        case COMPRESSION_XZ:
                ...
                while (c->xz.avail_in > 0) {
                        uint8_t buffer[COMPRESSOR_BUFFER_SIZE];
        ...
        case COMPRESSION_GZIP:
                ...
                while (c->gzip.avail_in > 0) {
                        ...
                        r = sym_inflate(&c->gzip, Z_NO_FLUSH);
        ...
        case COMPRESSION_BZIP2:
                ...
                while (c->bzip2.avail_in > 0) {
                        ...
                        r = sym_BZ2_bzDecompress(&c->bzip2);
                        if (!IN_SET(r, BZ_OK, BZ_STREAM_END))
                                return -EBADMSG;
...
static int enlarge_buffer(void **buffer, size_t *buffer_size, size_t *buffer_allocated) {
        ...
        l = MAX(COMPRESSOR_BUFFER_SIZE, (*buffer_size * 2));
        p = realloc(*buffer, l);
        if (!p)
                return -ENOMEM;
...
int compressor_new(Compressor **ret, Compression type) {
        ...
        case COMPRESSION_XZ: {
                ...
                xzr = sym_lzma_easy_encoder(&c->xz, LZMA_PRESET_DEFAULT, LZMA_CHECK_CRC64);
                if (xzr != LZMA_OK)
                        return -EIO;
        ...
        case COMPRESSION_LZ4: {
                ...
                size_t header_bound = sym_LZ4F_compressBound(0, &preferences);
                c->lz4_header = malloc(header_bound);
                if (!c->lz4_header)
                        return -ENOMEM;

                c->lz4_header_size = sym_LZ4F_compressBegin(c->c_lz4, c->lz4_header, header_bound, &preferences);
        ...
        case COMPRESSION_ZSTD: {
                ...
                z = sym_ZSTD_CCtx_setParameter(c->c_zstd, ZSTD_c_compressionLevel, ZSTD_CLEVEL_DEFAULT);
                if (sym_ZSTD_isError(z))
                        return -EIO;

                z = sym_ZSTD_CCtx_setParameter(c->c_zstd, ZSTD_c_checksumFlag, 1);
                if (sym_ZSTD_isError(z))
        ...
        case COMPRESSION_GZIP:
                ...
                r = sym_deflateInit2_(&c->gzip, Z_DEFAULT_COMPRESSION, Z_DEFLATED, 15 + 16, 8, Z_DEFAULT_STRATEGY, ZLIB_VERSION, (int) sizeof(c->gzip));
                if (r != Z_OK)
                        return -EIO;
        ...
        case COMPRESSION_BZIP2:
                ...
                r = sym_BZ2_bzCompressInit(&c->bzip2, 9, 0, 0);
                if (r != BZ_OK)
                        return -EIO;
        ...
        *buffer_size = 0;
        ...
        case COMPRESSION_LZ4: {
                ...
                if (c->lz4_header_size > 0) {
                        if (*buffer_allocated < c->lz4_header_size) {
                                void *p = realloc(*buffer, c->lz4_header_size);
                                if (!p)
                                        return -ENOMEM;
                                *buffer = p;
                                *buffer_allocated = c->lz4_header_size;
                        }
                ...
                size_t bound = sym_LZ4F_compressBound(size, NULL);
                if (*buffer_allocated < *buffer_size + bound) {
                        void *p = realloc(*buffer, *buffer_size + bound);
                        if (!p)
                                return -ENOMEM;
                        *buffer = p;
                        *buffer_allocated = *buffer_size + bound;
                }
...
#if HAVE_LZ4
        case COMPRESSION_LZ4: {
                size_t bound = sym_LZ4F_compressBound(0, NULL);
                if (*buffer_allocated < bound) {
                        void *p = realloc(*buffer, bound);
                        if (!p)
                                return -ENOMEM;
                        *buffer = p;
                        *buffer_allocated = bound;
                }
