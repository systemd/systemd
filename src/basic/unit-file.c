...
static int add_names(
        ...
        assert(IN_SET(name_type, UNIT_NAME_PLAIN, UNIT_NAME_INSTANCE));
        assert(name_type == UNIT_NAME_PLAIN || instance);

        /* The unit has it's own name. If we're looking at a fragment, the fragment name (possibly with
         * instance inserted), is also always a one of the unit names. */
