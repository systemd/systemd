...
int unit_file_build_name_map(
         ...
         * the former for the case where some unit file alias points is a dangling symlink that points
         * to one of the "original" directories (and can't be followed).
         */
        STRV_FOREACH(dir, lp->search_path) {
                char *resolved_dir;
                ...
                if (strv_consume(&expanded_search_path, resolved_dir) < 0)
                        return log_oom();
        }
