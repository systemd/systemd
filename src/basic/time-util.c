...
int mktime_or_timegm_usec(
        ...
        if (tm->tm_year < 1970) /* early check for negative time_t */
                return -ERANGE;
        if ((usec_t) tm->tm_year > CONST_MIN(USEC_INFINITY / USEC_PER_YEAR, (usec_t) TIME_T_MAX / (365U * 24U * 60U * 60U))) /* early check for possible overrun of usec_t or time_t */
                return -ERANGE;
...
int localtime_or_gmtime_usec(
        ...
        if (t / USEC_PER_SEC > (usec_t) TIME_T_MAX)
                return -ERANGE;

        time_t sec = (time_t) (t / USEC_PER_SEC); /* Round down */
