...
static inline bool iovw_ends_with_newline(struct iovec_wrapper *iovw) {
        if (iovw_isempty(iovw))
                return false;

        struct iovec *last = &iovw->iovec[iovw->count - 1];
        return last->iov_len > 0 && ((char*) last->iov_base)[last->iov_len - 1] == '\n';
}
