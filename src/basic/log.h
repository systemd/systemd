...
extern thread_local LIST_HEAD(LogContext, _log_context);
...
/* Same as log_context_free(), but frees the given fields strv on failure. */
...
#define _LOG_CONTEXT_PUSH_STRV(strv, c) \
        _unused_ _cleanup_(log_context_detachp) LogContext *c = log_context_attach(&(LogContext) { .fields = strv });
...
#define _LOG_CONTEXT_CONSUME_STR(s, c, strv) \
        ...
        if (!strv)                                                                                      \
                free(s);                                                                                \
        _unused_ _cleanup_(log_context_freep) LogContext *c = log_context_consume(TAKE_PTR(strv))
...
#define _LOG_CONTEXT_CONSUME_STRV(strv, c) \
        _unused_ _cleanup_(log_context_freep) LogContext *c = log_context_consume(strv);
