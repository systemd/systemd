...
#define log_ratelimit_error(num_dropped_errors, _level, _error, _file, _line, _func, _format, ...)      \
({                                                                                                      \
        (num_dropped_errors > 0)                                                                        \
                ? log_internal(_level, _error, _file, _line, _func, _format " (Dropped %zu similar message(s))", __VA_ARGS__, num_dropped_errors) \
                : log_internal(_level, _error, _file, _line, _func, _format, __VA_ARGS__);              \
})
...
({                                                                              \
        ...
        if (_error != _log_ratelimit.error || _level != _log_ratelimit.level) { \
                ratelimit_reset(&_log_ratelimit.ratelimit);                     \
                _log_ratelimit.error = _error;                                  \
                _log_ratelimit.level = _level;                                  \
        }                                                                       \
        if (ratelimit_below(&_log_ratelimit.ratelimit)) {                       \
                _log_ratelimit_error = log_ratelimit_error(num_dropped_errors, _level, _error, _file, _line, _func, _format, __VA_ARGS__); \
        }                                                                       \
