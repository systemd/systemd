...
bool string_is_safe(const char *p, StringSafeFlags flags) {
        ...
        if (FLAGS_SET(flags, STRING_NON_EMPTY) ? isempty(p) : !p)
                return false;
        ...
        for (const char *t = p; *t; t++) {
                ...
                if (FLAGS_SET(flags, STRING_NO_BACKSLASHES) && *t == '\\')
                        return false;

                if (FLAGS_SET(flags, STRING_NO_QUOTES) && strchr(QUOTES, *t))
                        return false;

                if (FLAGS_SET(flags, STRING_NO_GLOBS) && strchr(GLOB_CHARS, *t))
                        return false;
