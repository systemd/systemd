...
int strverscmp_improved(const char *a, const char *b) {
        ...
        /* This is based on RPM's rpmvercmp(). But also supports '-', which is used for separating
         * version and release, as we usually want to directly compare strings like
        ...
        for (;;) {
                ...
                /* Handle '~'. Used for pre-release, e.g. 123~rc1, or 4.5~alpha1 */
                if (*a == '~' || *b == '~') {
                ...
                /* Handle '^'. Similar to '~', but its meaning is inverse. */
                if (*a == '^' || *b == '^') {
                        r = CMP(*a == '^', *b == '^');
                        if (r != 0)
                                return r;
                ...
                if (isdigit(*a) || isdigit(*b)) {
                        /* Find the leading numeric segments. One may be an empty string. */
                ...
                /* First, compare the lengths of the segments. Note that this also works fine with the
                 * numeric segments. Eg. 12345 vs 123, longer is newer. */
                r = CMP(aa - a, bb - b);
                if (r != 0)
                        return r;

                /* When their length are equivalent, compare them as strings. Again, this also works
                 * fine with numeric segments. */
                r = strncmp(a, b, aa - a);
                if (r != 0)
                        return r;

                /* The segments are equivalent. Let's compare the next one. */
                a = aa;
                b = bb;
        }
}
