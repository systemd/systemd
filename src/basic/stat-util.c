...
int dir_is_empty_at(int dir_fd, const char *path, bool ignore_hidden_or_backup) {
        ...
        for (;;) {
                 ...
                 * + ".."), and only once we have seen if there's a third we know whether the dir is empty or
                 * not. If 'ignore_hidden_or_backup' is true we'll allocate a bit more, since we might skip
                 * over a bunch of entries that we ignore. */
                DEFINE_DIRENT_BUFFER(buffer, ignore_hidden_or_backup ? 16 : 3);
                struct dirent *de;
                ssize_t n;
