...
int inode_same_at(int fda, const char *filea, int fdb, const char *fileb, int flags) {
        ...
        if (fda >= 0 && fda == fdb && isempty(filea) && isempty(fileb) && FLAGS_SET(flags, AT_EMPTY_PATH|AT_SYMLINK_NOFOLLOW))
                return true;

        _cleanup_close_ int pin_a = -EBADF, pin_b = -EBADF;
        if (!FLAGS_SET(flags, AT_NO_AUTOMOUNT)) {
                /* Let's try to use the name name_to_handle_at() AT_HANDLE_FID API to identify identical
                 * inodes. We have to issue multiple calls on the same file for that (first, to acquire the
                 * FID, and then to check if the st_dev is actually the same). Hence let's pin the
                 * inode in between via O_PATH, unless we already have an fd for it. */
