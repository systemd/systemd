...
static uint64_t msr(uint64_t index) {
        uint64_t ret;
        _cleanup_close_ int fd = open(MSR_DEVICE, O_RDONLY|O_CLOEXEC);
        if (fd < 0) {
                log_debug_errno(errno, "Cannot open MSR device %s, ignoring: %m", MSR_DEVICE);
                return 0;
        }
...
static ConfidentialVirtualization detect_sev(void) {
        ...
        if (msrval & (UINT64_C(1) << 2))
                return CONFIDENTIAL_VIRTUALIZATION_SEV_SNP;
        else if (msrval & (UINT64_C(1) << 1))
                return CONFIDENTIAL_VIRTUALIZATION_SEV_ES;
        else if (msrval & (UINT64_C(1) << 0))
...
static ConfidentialVirtualization detect_tdx(void) {
        ...
        cpuid(&eax, &ebx, &ecx, &edx);
        log_debug("CPUID max function: %" PRIx32 " %" PRIx32 " %" PRIx32 "%" PRIx32, eax, ebx, ecx,edx);
