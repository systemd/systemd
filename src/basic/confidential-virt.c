...
static uint64_t msr(uint64_t index) {
        ...
        if (rv < 0) {
                log_debug_errno(errno, "Cannot read MSR device %s, ignoring: %m", MSR_DEVICE);
                ret = 0;
        } else if (rv != sizeof(ret)) {
                log_debug("Short read %d bytes from MSR device %s, ignoring", rv, MSR_DEVICE);
                ret = 0;
        }
...
static ConfidentialVirtualization detect_sev(void) {
        ...
        if (msrval & (UINT64_C(1) << 2))
                return CONFIDENTIAL_VIRTUALIZATION_SEV_SNP;
        if (msrval & (UINT64_C(1) << 1))
                return CONFIDENTIAL_VIRTUALIZATION_SEV_ES;
        if (msrval & (UINT64_C(1) << 0))
