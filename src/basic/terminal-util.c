        ...
        for (;;) {
                ...
                if (c == '\t') {
                        ...
                        if (IN_SET(cr, COMPLETION_PARTIAL, COMPLETION_ALREADY)) {
                                ...
                                _cleanup_free_ char **filtered = strv_filter_prefix(completions, string);
                                if (!filtered) {
                                        r = -ENOMEM;
                                        goto fail;
                                }
...
static size_t widest_list_element(char **l) {
        size_t w = 0;

        /* Returns the largest console width of all elements in 'l' */

        STRV_FOREACH(i, l) {
                size_t m = utf8_console_width(*i);
...
int show_menu(char **x,
        ...
        if (column_width == SIZE_MAX) {
                ...
                column_width = MIN(widest+1, column_max);
