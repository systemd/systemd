...
/* Determines terminal dimensions by means of ANSI sequences: saves the cursor via DECSC, positions it far
 * to the bottom right (clamped to actual terminal dimensions), reads back via DSR where we ended up, and
 * restores cursor via DECRC. Only needs a single DSR round-trip, and always restores the cursor regardless
...
/* Common setup for ANSI terminal queries: validates the fds, opens a non-blocking input fd, and configures
...
/* Determines terminal dimensions by means of an ANSI CSI 18 sequence.
 *
 * Caller must have already opened a non-blocking input fd and configured termios (echo/icanon off). */
...
int terminal_fix_size(int input_fd, int output_fd) {
        ...
        /* Open a non-blocking input fd once for both query methods, and configure termios once. This avoids
         * a window between the CSI 18 attempt and the DSR fallback where echo would be briefly re-enabled,
         * which could cause late CSI 18 responses to be echoed back to the terminal and interfere with the
         * DSR query. See https://github.com/systemd/systemd/issues/35499 */
        _cleanup_close_ int nonblock_input_fd = -EBADF;
        struct termios old_termios;
        r = terminal_prepare_query(input_fd, output_fd, &nonblock_input_fd, &old_termios);
        if (r < 0)
        ...
        r = terminal_query_size_by_csi18(nonblock_input_fd, output_fd, &rows, &columns);
        if (IN_SET(r, -EOPNOTSUPP, -EINVAL)) {
                ...
                r = terminal_query_size_by_dsr(nonblock_input_fd, output_fd, &rows, &columns);
        }
