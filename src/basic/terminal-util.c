...
int acquire_terminal(
        ...
        assert(name);
        assert(IN_SET(flags & _ACQUIRE_TERMINAL_MODE_MASK,
                      ACQUIRE_TERMINAL_TRY, ACQUIRE_TERMINAL_FORCE, ACQUIRE_TERMINAL_WAIT));
        ...
        if ((flags & _ACQUIRE_TERMINAL_MODE_MASK) == ACQUIRE_TERMINAL_WAIT) {
        ...
        for (;;) {
                ...
                r = RET_NERRNO(ioctl(fd, TIOCSCTTY, (flags & _ACQUIRE_TERMINAL_MODE_MASK) == ACQUIRE_TERMINAL_FORCE));
                ...
                if ((flags & _ACQUIRE_TERMINAL_MODE_MASK) != ACQUIRE_TERMINAL_WAIT) /* If we are in TRY or FORCE mode, then propagate EPERM as EPERM */
                        return r;
                ...
                for (;;) {
                        ...
                        r = ppoll(&pollfd, 1, left == USEC_INFINITY ? NULL : TIMESPEC_STORE(left), &poll_ss);
                        if (r < 0)
                                return -errno;
                        if (r == 0)
                                return -ETIMEDOUT;
