...
int resolve_dev_console(char **ret) {
         ...
         * /sys/calss/tty/tty0/active, but only if /sys/ is actually ours (i.e. not read-only-mounted which
...
int get_default_background_color(double *ret_red, double *ret_green, double *ret_blue) {
        ...
        for (;;) {
                if (buf_full == 0) {
                        ...
                        /* On the first read, read a multiple characters, i.e. the shortest valid
                         * reply. Afterwards read byte-wise, since we don't want to read too much, and drop
...
int terminal_get_size_by_dsr(
        ...
        char buf[6]; /* The shortest valid reply possible, i.e. "\x1B[9;9R" */
        size_t buf_full = 0;
        bool first = true;
        CursorPositionContext context = {};
