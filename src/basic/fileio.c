...
static int write_string_file_atomic_at(
        ...
        if (FLAGS_SET(flags, WRITE_STRING_FILE_LABEL)) {
                r = label_ops_pre(dir_fd, fn, mode, label_userdata);
                if (r < 0)
                        return r;
        ...
        int k = call_label_ops_post ? label_ops_post(f ? fileno(f) : dir_fd, f ? NULL : fn, /* created= */ !!f, label_userdata) : 0;
...
int write_string_file_full_label(
        ...
        if (flags & WRITE_STRING_FILE_ATOMIC) {
                ...
                r = write_string_file_atomic_at(dir_fd, fn, line, flags, ts, label_userdata);
                if (r < 0)
                        goto fail;
        ...
        if (isempty(fn))
                ...
                if (FLAGS_SET(flags, WRITE_STRING_FILE_LABEL|WRITE_STRING_FILE_CREATE)) {
                        r = label_ops_pre(dir_fd, label_fn ?: fn, mode, label_userdata);
                        if (r < 0)
                                goto fail;
                ...
                if (call_label_ops_post)
                        ...
                        RET_GATHER(r, label_ops_post(fd >= 0 ? fd : dir_fd, fd >= 0 ? NULL : fn, made_file, label_userdata));
        }
        if (r < 0)
                goto fail;
