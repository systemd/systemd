...
/* libc's SOMAXCONN is defined to 128 or 4096 on glibc. But actually, the value can be much larger. In our
 * codebase we want to set it to the max usually, since noawaday socket memory is properly tracked by memcg,
 * and hence we don't need ned to enforce extra limits here. Moreover, the kernel caps it to
