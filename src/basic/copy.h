...
typedef enum CopyFlags
{
        ...
        COPY_SAME_MOUNT = 1
                << 3, /* Don't descend recursively into other file systems, across mount point boundaries */
} CopyFlags;
...
int copy_file_fd_full(
        const char *from, int to, CopyFlags copy_flags, copy_progress_bytes_t progress, void *userdata);
...
int copy_file_full(const char *from,
                   ...
                   void *userdata);
static inline int copy_file(const char *from,
                            const char *to,
                            int open_flags,
                            mode_t mode,
                            unsigned chattr_flags,
...
int copy_file_atomic_full(const char *from,
                          ...
                          copy_progress_bytes_t progress,
                          void *userdata);
static inline int copy_file_atomic(
        const char *from, const char *to, mode_t mode, unsigned chattr_flags, CopyFlags copy_flags) {
...
static inline int copy_tree(
        const char *from, const char *to, uid_t override_uid, gid_t override_gid, CopyFlags copy_flags) {
        return copy_tree_at_full(
                AT_FDCWD, from, AT_FDCWD, to, override_uid, override_gid, copy_flags, NULL, NULL, NULL);
}
