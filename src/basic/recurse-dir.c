...
int recurse_dir(
        ...
        /* This is a lot ftw()/nftw(), but a lot more modern, i.e. built around openat()/statx(), and under
         * the assumption that fds are not as 'expensive' as they used to be. */
        ...
        for (int i = 0; i < n; i++) {
                ...
                if (!subdir) {
                        ...
                        if (flags & RECURSE_DIR_INODE_FD) {
                                ...
                                r = statx_fallback(dirfd(subdir), de[i]->d_name, AT_SYMLINK_NOFOLLOW, statx_mask | STATX_TYPE, &sx);
                ...
                if (subdir) {
                        ...
                        if (n_depth_max <= 1) {
                                /* When we reached maxed depth, generate a special event */
