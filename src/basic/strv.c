...
int strv_rebreak_lines(char **l, size_t width, char ***ret) {
        ...
        STRV_FOREACH(i, l) {
                ...
                /* Process rest of the line, except if the input ended in a newline, which we already
                 * flushed out above: a trailing newline terminates the last line, it doesn't add a new,
                 * empty one. */
                assert(start);
                if (after_newline && *start == 0)
                        continue;
