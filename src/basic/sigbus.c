...
static _Atomic(void *) sigbus_queue[SIGBUS_QUEUE_MAX];
static _Atomic(sig_atomic_t) n_sigbus_queue = 0;
...
static void sigbus_push(void *addr) {
        ...
        sig_atomic_t c = __atomic_load_n(&n_sigbus_queue, __ATOMIC_SEQ_CST);
        for (;;) {
