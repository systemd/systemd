...
int path_make_relative(const char *from, const char *to, char **ret) {
        ...
        for (;;) {
                r = path_get_first_component(&from, true, &f);
                if (r < 0)
                        return r;

                s = path_get_first_component(&to, true, &t);
                if (s < 0)
                        return s;
        ...
        for (n_parents = 1;; n_parents++) {
                ...
                r = path_get_first_component(&from, false, &f);
                if (r < 0)
                        return r;
                if (r == 0)
                        break;
...
char *path_startswith_full(const char *path, const char *prefix, bool accept_dot_dot) {
        ...
        for (;;) {
                ...
                r = path_get_first_component(&path, accept_dot_dot, &p);
                if (r < 0)
                        return NULL;

                s = path_get_first_component(&prefix, accept_dot_dot, &q);
                if (s < 0)
                        return NULL;
...
int path_compare(const char *a, const char *b) {
        ...
        for (;;) {
                ...
                j = path_get_first_component(&a, true, &aa);
                k = path_get_first_component(&b, true, &bb);
...
int path_get_last_component(const char *path, bool accept_dot_dot, const char **next, const char **ret) {
        ...
        /* Similar to path_get_first_component(), but search components from the end.
        *
        * Examples
...
int path_extract_filename(const char *path, char **ret) {
        ...
        r = path_get_last_component(path, false, &next, &c);
        if (r < 0)
                return r;
...
bool path_is_valid_full(const char *p, bool accept_dot_dot) {
        ...
        for (const char *e = p;;) {
                ...
                r = path_get_first_component(&e, accept_dot_dot, NULL);
                if (r < 0)
                        return false;
