...
int name_to_handle_at_try_unique_mntid_fid(
        ...
        if (ret_mnt_id && mnt_id >= 0) {
                 ...
                 * and only if this doesn't work we use the mnt_id from above.
                 * The function only sets mnt_id after checking the error code, so omitted here. */
...
int path_get_mnt_id_at_fallback(int dir_fd, const char *path, int *ret) {
        ...
        r = name_to_handle_at_loop(dir_fd, path, NULL, ret, NULL, isempty(path) ? AT_EMPTY_PATH : 0);
        if (r >= 0 || is_name_to_handle_at_fatal_error(r))
                return r;
