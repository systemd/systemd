...
int fd_is_mount_point(int fd, const char *filename, int flags) {
        ...
        /* Note that AT_EMPTY_PATH is only supported if the kernel supports statx(), or if the inode
         * specified via 'fd' refers to a directory. Otherwise, we'll have to fail, because we have no kernel
         * API to query the information we need.
         *
         * Also note that 'filename' being NULL will turn on AT_EMPTY_PATH implicitly. */

        if (FLAGS_SET(flags, AT_EMPTY_PATH))
                filename = empty_to_null(filename); /* Normalize → NULL */
        else if (!filename)
                flags |= AT_EMPTY_PATH;             /* Normalize → enable AT_EMPTY_PATH */
        else if (filename_possibly_with_slash_suffix(filename))
                flags &= ~AT_EMPTY_PATH;            /* Normalize → disable AT_EMPTY_PATH */
        else
        ...
        r = name_to_handle_at_loop(fd, strempty(filename), &h, &mount_id, flags);
        if (r < 0) {
