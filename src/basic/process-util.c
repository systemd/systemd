...
int namespace_fork(
        ...
        r = pidref_safe_fork_full(outer_name,
                                  NULL,
        ...
        if (r == 0) {
                ...
                r = pidref_safe_fork_full(inner_name,
                                          NULL,
                                          except_fds, n_except_fds,
                                          flags & ~(FORK_WAIT|FORK_RESET_SIGNALS|FORK_CLOSE_ALL_FDS|FORK_REARRANGE_STDIO), &pidref);
                if (r < 0)
                        _exit(EXIT_FAILURE);
                if (r == 0) {
                        /* Child */
                ...
                r = pidref_wait_for_terminate_and_check(inner_name, &pidref, FLAGS_SET(flags, FORK_LOG) ? WAIT_LOG : 0);
                if (r < 0)
                        _exit(EXIT_FAILURE);
