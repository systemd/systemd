...
static char *find_line_startswith(const char *buffer, const char *haystack) {
        char *p;

        assert(buffer);
        assert(haystack);

        p = strstr(buffer, haystack);
        if (!p)
                return NULL;

        if (p > buffer && p[-1] != '\n') /* In the middle, need to check beginning of line then */
                return NULL;

        return p + strlen(haystack);
}
