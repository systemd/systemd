...
int safe_fork_full(
        ...
        if (FLAGS_SET(flags, FORK_DETACH) && ret_pid && socketpair(AF_UNIX, SOCK_DGRAM|SOCK_CLOEXEC, 0, pid_transport_fds) < 0)
                return log_full_errno(prio, errno, "Failed to allocate pid transport socketpair: %m");
                ...
                if (!r) {
                        ...
                        if (pid > 0) {
                                ...
                                if (pid_transport_fds[0] >= 0) {
                                        ...
                                        ssize_t l = recv(pid_transport_fds[0], &pid, sizeof(pid), 0);
                                        if (l < 0)
        ...
        if (pid > 0) {
                ...
                if (intermediary) {
                        if (pid_transport_fds[1] >= 0) {
                                ssize_t l = send(pid_transport_fds[1], &pid, sizeof(pid), MSG_NOSIGNAL);
                                if (l < 0) {
