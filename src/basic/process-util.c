...
int posix_spawn_wrapper(
        ...
        if (r == EBUSY && FLAGS_SET(flags, POSIX_SPAWN_SETCGROUP)) {
                 ...
                 * fall back for this spawn only and let the caller move the child via cg_attach(). Note
                 * that cg_attach() may also fail if subtree_control still has controllers at that point
                 * (e.g. if the proactive cg_enable(mask=0) in exec_spawn() was unable to clear them),
                 * in which case the child may run outside the target cgroup. */
                flags &= ~POSIX_SPAWN_SETCGROUP;
                r = posix_spawnattr_setflags(&attr, flags);
                if (r != 0)
                        return -r;
