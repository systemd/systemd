...
static int get_process_cmdline_nulstr(pid_t pid, size_t max_size, ProcessCmdlineFlags flags, char **ret, size_t *ret_size) {
        const char *p;
        char *t;
        size_t k;
        int r;
...
int get_process_cmdline(pid_t pid, size_t max_columns, ProcessCmdlineFlags flags, char **line) {
        ...
        r = get_process_cmdline_nulstr(pid, max_columns, flags, &t, &k);
        if (r < 0)
                return r;
        bool truncated = r == 0;
        ...
        if (flags & PROCESS_CMDLINE_QUOTE) {
                ...
                ans = escape_non_printable_full(t, max_columns, eight_bit, truncated);
                if (!ans)
                        return -ENOMEM;
