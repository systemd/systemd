...
int safe_fork_full(
        ...
        if (FLAGS_SET(flags, FORK_DETACH)) {
                assert_se(!FLAGS_SET(flags, FORK_WAIT));
                assert_se(!ret_pid);
...
int reaper_process(void) {
        ...
        if (prctl(PR_GET_CHILD_SUBREAPER, (unsigned long) &b, 0, 0, 0) < 0)
                return -errno;
...
int make_reaper_process(bool b) {
        ...
        if (prctl(PR_SET_CHILD_SUBREAPER, (unsigned long) b, 0LU, 0LU, 0LU) < 0)
                return -errno;
