        ...
        if (clone_support >= CLONE_CAN_PIDFD) {
                ...
                if ((ERRNO_IS_NOT_SUPPORTED(r) || ERRNO_IS_PRIVILEGE(r) || r == E2BIG) &&
                    FLAGS_SET(flags, POSIX_SPAWN_SETCGROUP)) {
                         ...
                         * need to change the flags to remove the cgroup one, which is what redirects to clone3().
                        ...
                        r = pidfd_spawn(&pidfd, path, NULL, &attr, argv, envp);
                        /* if pidfd_spawn was successful after removing SPAWN_CGROUP, record that so that
                         * we do not retry every time */
                        if (r == 0)
