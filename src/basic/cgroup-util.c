...
int cg_kill_kernel_sigkill(const char *path, size_t *ret_n_pids_killed) {
        _cleanup_free_ char *killfile = NULL;
        size_t n_pids = UINT64_MAX;
        int r;
        ...
        if (ret_n_pids_killed) {
                 ...
                 * if a process starts/stop between this read and the next write, but this is used for
