...
int cg_kill_kernel(const char *path) {
        ...
        r = write_string_file(path, "1", WRITE_STRING_FILE_DISABLE_BUFFER);
        if (r < 0)
                return r;
...
int cg_kill_recursive(
        ...
        if (cg_kill_supported() && sig == SIGKILL &&
            FLAGS_SET(flags, CGROUP_REMOVE) && !FLAGS_SET(flags, CGROUP_IGNORE_SELF)) {
                r = cg_kill_kernel(path);
                if (r < 0)
                        return r;
