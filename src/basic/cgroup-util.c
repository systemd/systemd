...
int cg_kill(
        ...
        if (!set) {
                set = allocated_set = set_new(NULL);
                if (!set)
                        return -ENOMEM;
        }
        ...
        do {
                ...
                for (;;) {
                        ...
                        if (set_contains(set, PID_TO_PTR(pidref.pid)))
                                continue;
                        ...
                        r = set_put(set, PID_TO_PTR(pidref.pid));
                        if (r < 0)
...
int cg_kill_recursive(
        ...
        ret = cg_kill(path, sig, flags, set, log_kill, userdata);
        ...
        for (;;) {
                ...
                r = cg_kill_recursive(p, sig, flags, set, log_kill, userdata);
                if (r < 0)
