 ...
 * userspace prng here, or even do fast key erasure, but rather just the
 * shortest thing that is at least better than libc rand(). */
...
void random_bytes(void *p, size_t n) {
        ...
        if (fd >= 0 && loop_read_exact(fd, p, n, true) == 0)
                return;
...
int crypto_random_bytes(void *p, size_t n) {
        ...
        if (!seen_initialized) {
                ...
                poll_fd = open("/dev/random", O_RDONLY|O_CLOEXEC|O_NOCTTY);
                if (poll_fd < 0)
                        return -errno;
                poller.fd = poll_fd;
