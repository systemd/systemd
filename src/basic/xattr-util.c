...
static int getxattrat_fake_prepare(
        ...
        if (isempty(filename)) {
                ...
                assert_se(snprintf(ret_fn, PROC_FD_PATH_MAX, "/proc/self/fd/%i", dirfd) < (int) PROC_FD_PATH_MAX);
        } else {
                fd = openat(dirfd, filename, O_CLOEXEC|O_PATH|(flags & AT_SYMLINK_NOFOLLOW ? O_NOFOLLOW : 0));
                if (fd < 0)
                        return -errno;

                assert_se(snprintf(ret_fn, PROC_FD_PATH_MAX, "/proc/self/fd/%i", fd) < (int) PROC_FD_PATH_MAX);
        }
