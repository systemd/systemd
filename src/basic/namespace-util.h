...
typedef enum NamespaceEnterFlags {
         ...
         * ensure that retaining its original identity is appropriate in that case. */
        NAMESPACE_ENTER_KEEP_UID_GID_IF_ROOT_UNMAPPED = 1 << 0,
} NamespaceEnterFlags;

int namespace_enter_full(int pidns_fd, int mntns_fd, int netns_fd, int userns_fd, int root_fd, NamespaceEnterFlags flags);
