...
#define ELF_NOTE_DLOPEN_VENDOR "uapi"

/* Add an ".note.uapi.dlopen" ELF note to our binary that declares our weak dlopen() dependency. This
 * information can be read from an ELF file via "readelf -p .note.uapi.dlopen" or an equivalent command. */
#define _ELF_NOTE_DLOPEN(module, variable_name)                         \
        __attribute__((used, section(".note.uapi.dlopen"))) _Alignas(sizeof(uint32_t)) static const struct { \
                struct {                                                \
...
#define ELF_NOTE_DLOPEN(module) \
        _ELF_NOTE_DLOPEN(module, UNIQ_T(s, UNIQ))
