...
#define _DEFINE_HASH_OPS_WITH_KEY_DESTRUCTOR(uq, name, type, hash_func, compare_func, free_func, scope) \
        ...
        _DEFINE_HASH_OPS(uq, name, type, hash_func, compare_func, UNIQ_T(static_free_wrapper, uq), NULL, scope)
...
#define _DEFINE_HASH_OPS_WITH_VALUE_DESTRUCTOR(                                \
        ...
        _DEFINE_HASH_OPS(uq, name, type, hash_func, compare_func, NULL, UNIQ_T(static_free_wrapper, uq), scope)

#define _DEFINE_HASH_OPS_FULL(                                                                      \
        uq, name, type, hash_func, compare_func, free_key_func, type_value, free_value_func, scope) \
        ...
        _DEFINE_HASH_OPS(uq,                                                                        \
                         ...
                         UNIQ_T(static_free_value_wrapper, uq),                                     \
                         scope)
...
#define DEFINE_PRIVATE_HASH_OPS_WITH_VALUE_DESTRUCTOR(              \
        ...
        _DEFINE_HASH_OPS_WITH_VALUE_DESTRUCTOR(                     \
                UNIQ, name, type, hash_func, compare_func, value_type, free_func, static)

#define DEFINE_HASH_OPS_FULL(name, type, hash_func, compare_func, free_key_func, value_type, free_value_func) \
        _DEFINE_HASH_OPS_FULL(                                                                                \
                UNIQ, name, type, hash_func, compare_func, free_key_func, value_type, free_value_func, )

#define DEFINE_PRIVATE_HASH_OPS_FULL(                                                    \
        name, type, hash_func, compare_func, free_key_func, value_type, free_value_func) \
        _DEFINE_HASH_OPS_FULL(                                                           \
                UNIQ, name, type, hash_func, compare_func, free_key_func, value_type, free_value_func, static)
...
/* This will compare the passed pointers directly, and will not dereference them. This is hence not useful
 * for strings or suchlike. */
void trivial_hash_func(const void *p, struct siphash *state);
int trivial_compare_func(const void *a, const void *b) _const_;
extern const struct hash_ops trivial_hash_ops;

/* 32bit values we can always just embed in the pointer itself, but in order to support 32bit archs we need
 * store 64bit values indirectly, since they don't fit in a pointer. */
void uint64_hash_func(const uint64_t *p, struct siphash *state);
int uint64_compare_func(const uint64_t *a, const uint64_t *b) _pure_;
extern const struct hash_ops uint64_hash_ops;

/* On some archs dev_t is 32bit, and on others 64bit. And sometimes it's 64bit on 32bit archs, and sometimes
 * 32bit on 64bit archs. Yuck! */
#if SIZEOF_DEV_T != 8
