...
/* A framework for registering static variables that shall be freed on shutdown of a process. It's a bit like
 * gcc's destructor attribute, but allows us to precisely schedule when we want to free the variables. This
 * is supposed to feel a bit like the gcc cleanup attribute, but for static variables. Note that this does
 * not work for static variables declared in .so's, as the list is private to the same linking unit. But
...
#define _STATIC_DESTRUCTOR_REGISTER(uq, variable, func)                                               \
        ...
        _section_("SYSTEMD_STATIC_DESTRUCT") /* We pick pointer alignment, since that is apparently what gcc does for static variables */                                                                     \
                _alignptr_ /* Make sure this is not dropped from the image because not explicitly referenced */                                                                                                   \
                        _used_ /* Make sure that AddressSanitizer doesn't pad this variable: we want  \
                                  everything in this section packed next to each other so that we can \
                                  enumerate it. */                                                    \
                                _variable_no_sanitize_address_ static const StaticDestructor UNIQ_T(  \
                                        static_destructor_entry, uq) = {                              \
                                        .data = &(variable),                                          \
 ...
 * even if there's not a single destructor is defined in which case the section will be missing. */
