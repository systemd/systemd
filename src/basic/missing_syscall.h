...
static inline ssize_t missing_getxattrat(int fd, const char *path, int at_flags, const char *name, struct xattr_args *args, size_t size) {
#  ifdef __NR_getxattrat
        return syscall(__NR_getxattrat, fd, path, at_flags, name, args, size);
#  else
        errno = ENOSYS;
        return -1;
#  endif
}
#  define getxattrat missing_getxattrat

static inline ssize_t missing_listxattrat(int fd, const char *path, int at_flags, char *list, size_t size) {
#  ifdef __NR_listxattrat
        return syscall(__NR_listxattrat, fd, path, at_flags, list, size);
#  else
        errno = ENOSYS;
        return -1;
#  endif
}
#  define listxattrat missing_listxattrat
