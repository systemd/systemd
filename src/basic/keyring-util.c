...
int keyring_find_by_name(const char *name, uid_t owner, key_serial_t *ret) {
         ...
         * their names are not published, hence /proc/keys is the only way to find them: request_key()
         * searches the keyrings of the caller only. As anybody may create a keyring of any name, only
         * keyrings owned by the specified user are considered, UID_INVALID for any. Returns -ENOKEY if
         * there is none, -ENOTUNIQ if there is more than one, -ERFKILL if /proc/keys is masked. */
