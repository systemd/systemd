...
int touch_file(const char *path, bool parents, usec_t stamp, uid_t uid, gid_t gid, mode_t mode) {
        ...
        if (fd < 0) {
                ...
                fd = xopenat_full(AT_FDCWD, path, O_WRONLY|O_CREAT|O_EXCL, /* xopen_flags= */ 0, IN_SET(mode, 0, MODE_INVALID) ? 0644 : mode);
                if (fd < 0)
                        return fd;
        }
...
int openat_report_new(int dirfd, const char *pathname, int flags, mode_t mode, bool *ret_newly_created) {
        ...
        for (unsigned attempts = 7;;) {
                ...
                /* A dangling symlink in the way? Then retrying won't help, report it like O_NOFOLLOW would. */
                struct stat st;
