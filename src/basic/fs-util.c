...
int chase_symlinks(const char *path, const char *original_root, unsigned flags, char **ret_path, int *ret_fd) {
        ...
        for (;;) {
                _cleanup_free_ char *first = NULL, *joined = NULL;
                _cleanup_close_ int child = -1;
                struct stat st;
                const char *e;

                log_info("done: %s, todo: %s", strna(done), strna(todo));

                r = path_get_first_component_full(&todo, true, &e);
                if (r < 0)
                        return r;
                ...
                if (child < 0) {
                        if (errno == ENOENT &&
                            ...
                            (isempty(todo) || path_is_safe(todo))) {
                                ...
                                joined = path_join(done, first, todo);
                                if (!joined)
                                        return -ENOMEM;

                                free_and_replace(done, joined);
                                exists = false;
                                break;
                        }
                ...
                if (S_ISLNK(st.st_mode) && !((flags & CHASE_NOFOLLOW) && isempty(todo))) {
                        ...
                        free_and_replace(buffer, joined);
                        todo = buffer;
