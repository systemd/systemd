...
int symlinkat_atomic_full_label(const char *target, int atfd, const char *linkpath, SymlinkFlags flags, LabelContext *label_userdata) {
        int r;
        ...
        if (FLAGS_SET(flags, SYMLINK_LABEL)) {
                r = label_ops_pre(atfd, linkpath, S_IFLNK, label_userdata);
                if (r < 0)
                        return r;
        ...
        if (call_label_ops_post)
                RET_GATHER(r, label_ops_post(atfd, t, /* created= */ r >= 0, label_userdata));
        if (r < 0)
                return r;
...
int open_mkdir_at_full_label(int dirfd, const char *path, int flags, XOpenFlags xopen_flags, mode_t mode, LabelContext *label_userdata) {
        ...
        fd = xopenat_full_label(dirfd, path, flags|O_CREAT|O_DIRECTORY|O_NOFOLLOW, xopen_flags, mode, label_userdata);
...
int xopenat_full_label(int dir_fd, const char *path, int open_flags, XOpenFlags xopen_flags, mode_t mode, LabelContext *label_userdata) {
        _cleanup_close_ int fd = -EBADF;
        ...
        if (FLAGS_SET(open_flags, O_CREAT) && FLAGS_SET(xopen_flags, XO_LABEL)) {
                r = label_ops_pre(dir_fd, path, FLAGS_SET(open_flags, O_DIRECTORY) ? S_IFDIR : S_IFREG, label_userdata);
                if (r < 0)
                        return r;
        ...
        if (call_label_ops_post) {
                ...
                r = label_ops_post(fd, /* path= */ NULL, made_file || made_dir, label_userdata);
                if (r < 0)
                        goto error;
        }
...
error:
        if (call_label_ops_post)
                (void) label_ops_post(fd >= 0 ? fd : dir_fd, fd >= 0 ? NULL : path, made_dir || made_file, label_userdata);
...
int xopenat_lock_full_label(
        ...
        for (;;) {
                ...
                fd = xopenat_full_label(dir_fd, path, open_flags, xopen_flags, mode, label_userdata);
                if (fd < 0)
                        return fd;
