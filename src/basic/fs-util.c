...
int open_mkdir_at_full(int dirfd, const char *path, int flags, XOpenFlags xopen_flags, mode_t mode, void *label_userdata) {
        ...
        fd = xopenat_full(dirfd, path, flags|O_CREAT|O_DIRECTORY|O_NOFOLLOW, xopen_flags, mode, label_userdata);
...
int xopenat_lock_full(
        ...
        for (;;) {
                ...
                fd = xopenat_full(dir_fd, path, open_flags, xopen_flags, mode, label_userdata);
                if (fd < 0)
                        return fd;
