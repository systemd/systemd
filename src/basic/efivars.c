...
bool is_efi_secure_boot(void) {
        ...
        if (cache < 0)
                cache = read_flag(EFI_VARIABLE_SecureBoot);
...
bool is_efi_secure_boot_setup_mode(void) {
        ...
        if (cache < 0)
                cache = read_flag(EFI_VARIABLE_SetupMode);
...
static int read_efi_options_variable(char **line) {
        ...
        if (is_efi_secure_boot()) {
                ...
                if (access(EFIVAR_PATH(EFI_VARIABLE_SystemdOptions), F_OK) < 0)
                        return errno == ENOENT ? -ENODATA : -errno;

                return -EPERM;
        }

        r = efi_get_variable_string(EFI_VARIABLE_SystemdOptions, line);
        if (r == -ENOENT)
                return -ENODATA;
        return r;
}
...
int cache_efi_options_variable(void) {
        ...
        return write_string_file(EFIVAR_CACHE_PATH(EFI_VARIABLE_SystemdOptions), line,
...
int systemd_efi_options_variable(char **line) {
        ...
        if (e) {
                ...
                *line = m;
                ...
                r = read_one_line_file(EFIVAR_CACHE_PATH(EFI_VARIABLE_SystemdOptions), line);
                if (r == -ENOENT)
                        return -ENODATA;
                return r;
        }
...
static inline int compare_stat_mtime(const struct stat *a, const struct stat *b) {
        return CMP(a->st_mtim.tv_sec, b->st_mtim.tv_sec) ?:
               CMP(a->st_mtim.tv_nsec, b->st_mtim.tv_nsec);
}
...
int systemd_efi_options_efivarfs_if_newer(char **line) {
        ...
        if (stat(EFIVAR_PATH(EFI_VARIABLE_SystemdOptions), &a) < 0 && errno != ENOENT)
                return log_debug_errno(errno, "Failed to stat EFI variable SystemdOptions: %m");

        if (stat(EFIVAR_CACHE_PATH(EFI_VARIABLE_SystemdOptions), &b) < 0) {
                if (errno != -ENOENT)
                        log_debug_errno(errno, "Failed to stat "EFIVAR_CACHE_PATH(EFI_VARIABLE_SystemdOptions)": %m");
        } else if (compare_stat_mtime(&a, &b) > 0)
                log_debug_errno(errno, "Variable SystemdOptions in evifarfs is newer than in cache.");
        else {
                log_debug_errno(errno, "Variable SystemdOptions in cache is up to date.");
                *line = NULL;
                return 0;
        }
