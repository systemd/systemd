...
static inline int parse_calendar_date(const char *s, usec_t *ret) {
        return parse_calendar_date_full(s, /* for_birth_date= */ false, ret, NULL);
}

#define BIRTH_DATE_UNSET ((const struct tm) {   \
        .tm_sec = -1,                           \
        .tm_min = -1,                           \
        .tm_hour = -1,                          \
        .tm_mday = -1,                          \
...
#define BIRTH_DATE_IS_SET(tm) ((tm).tm_mday >= 1 && (tm).tm_mon >= 0 && (tm).tm_year != INT_MIN)

static inline int parse_birth_date(const char *s, struct tm *ret) {
        return parse_calendar_date_full(s, /* for_birth_date= */ true, NULL, ret);
}
