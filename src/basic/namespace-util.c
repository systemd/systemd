...
int namespace_open_by_id(int nsfs_fd, uint64_t ns_id) {
        _cleanup_close_ int local_fd = -EBADF;
        int r;

        /* Looks up a namespace by its unique boot-stable identifier and returns an O_PATH fd to it.
         * Requires kernel ≥ 6.13. The nsfs_fd parameter is any fd to an nsfs inode, used only to
         * identify the filesystem to look in; if negative, /proc/self/ns/user is opened
         * internally.
         *
         ...
         * with "kernel refused us", we refuse early with -EHOSTDOWN when we aren't in the initial
         * user namespace and let the caller skip the check. */
        ...
        if (nsfs_fd < 0) {
                local_fd = open("/proc/self/ns/user", O_RDONLY|O_CLOEXEC);
                if (local_fd < 0)
                        return -errno;
        ...
        /* nsfs_file_handle layout: { __u64 ns_id; __u32 ns_type; __u32 ns_inum; }. ns_type and
         * ns_inum stay zero so the kernel looks up by id alone. */
        unaligned_write_ne64(fh.file_handle.f_handle, ns_id);

        return RET_NERRNO(open_by_handle_at(nsfs_fd, &fh.file_handle, O_PATH|O_CLOEXEC));
}
