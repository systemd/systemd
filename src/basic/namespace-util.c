...
int namespace_enter(int pidns_fd, int mntns_fd, int netns_fd, int userns_fd, int root_fd) {
        ...
        if (userns_fd >= 0)
                /* If our current user isn't mapped into the user namespace we just entered, try to become
                 * the root user. Otherwise, don't change UID/GID but instead just drop all auxiliary groups. */
                return getuid() == UID_NOBODY && getgid() == GID_NOBODY ? reset_uid_gid() :
                                                                          maybe_setgroups(/* size= */ 0, /* list= */ NULL);
