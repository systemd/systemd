...
int userns_acquire(const char *uid_map, const char *gid_map, uid_t owner_uid, gid_t owner_gid) {
        bool unshare_manually = owner_uid != UID_INVALID || owner_gid != GID_INVALID;
        ...
        /* If we have to set a different owner we have to set the IDs before unsharing the new namespace, otherwise
        ...
        if (r < 0)
                return r;
        if (r == 0) {
                /* Child. Signal after unshare() and then just freeze until somebody kills us. */
                ...
                (void) write(pipe_fd[1], &(const int){0}, sizeof(r));
        ...
        if (unshare_manually) {
                r = fd_wait_for_event(pipe_fd[0], POLLIN, USEC_PER_SEC * 10);
                if (r < 0)
                        return r;
                if (r == 0)
                        return -ETIMEDOUT;
