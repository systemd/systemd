...
int is_idmapping_supported(const char *path) {
        ...
        if (ERRNO_IS_NEG_NOT_SUPPORTED(userns_fd))
                return false;
        if (userns_fd < 0) {
                log_debug_errno(userns_fd, "ID-mapping supported namespace acquire failed for '%s' : %m", path);
                return userns_fd;
        }

        dir_fd = RET_NERRNO(open(path, O_RDONLY | O_CLOEXEC | O_DIRECTORY | O_NOFOLLOW));
        if (ERRNO_IS_NEG_NOT_SUPPORTED(dir_fd) || dir_fd == -EINVAL)
                return false;
        if (dir_fd < 0) {
                log_debug_errno(dir_fd, "ID-mapping supported open failed for '%s' : %m", path);
                return dir_fd;
        }
        ...
        if (ERRNO_IS_NEG_NOT_SUPPORTED(mount_fd) || mount_fd == -EINVAL)
                return false;
        if (mount_fd < 0) {
                log_debug_errno(mount_fd, "ID-mapping supported open tree failed for '%s' : %m", path);
                return mount_fd;
        }
        ...
        if (ERRNO_IS_NEG_NOT_SUPPORTED(r) || r == -EINVAL)
                return false;
        if (r < 0) {
                log_debug_errno(r, "ID-mapping supported setattr failed for '%s' : %m", path);
                return r;
        }
