...
static int write_to_journal(
        ...
        iovec_len = MIN(4 + log_context_num_fields(_log_context) * 2, IOVEC_MAX);
...
int log_struct_internal(
        ...
        if (IN_SET(log_target,
                ...
                if (journal_fd >= 0) {
                        ...
                        iovec_len = MIN(17 + log_context_num_fields(_log_context) * 2, IOVEC_MAX);
                        iovec = newa(struct iovec, iovec_len);
...
int log_struct_iovec_internal(
        ...
        if (IN_SET(log_target, LOG_TARGET_AUTO,
            ...
            journal_fd >= 0) {
                ...
                iovec_len = MIN(1 + n_input_iovec * 2 + log_context_num_fields(_log_context) * 2, IOVEC_MAX);
                iovec = newa(struct iovec, iovec_len);
...
LogContext* log_context_new(char **fields) {
...
LogContext* log_context_free(LogContext *c) {
        ...
        c->fields = strv_free(c->fields);
        log_context_detach(c);
        return mfree(c);
}

size_t log_context_num_fields(LogContext *context) {
        size_t num_fields = 0;

        LIST_FOREACH(ll, c, context)
                STRV_FOREACH(s, c->fields)
                        num_fields++;
