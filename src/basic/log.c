...
#define LOG_CONTEXT_RESERVED 18
...
static int write_to_journal(
        ...
        /* Make sure to check if LOG_CONTEXT_RESERVED is large enough when adding more fields here. */
...
int log_struct_internal(
        ...
        if (IN_SET(log_target,
                ...
                if (journal_fd >= 0) {
                        ...
                        va_start(ap, format);
                        r = log_format_iovec(log_context, LOG_CONTEXT_RESERVED, &n, true, error, format, ap);
                        if (r < 0)
                                fallback = true;
                        else {
                                const struct msghdr msghdr = {
...
void _log_context_push_(const char *field) {
        if (!field)
                return;
...
void _log_context_push_strv_(char *const *fields) {
        if (!fields)
                return;
...
void _log_context_pop_free_(char **field) {
        if (!*field)
                return;
...
void _log_context_pop_strv_free_(char ***fields) {
        if (!*fields)
                return;
