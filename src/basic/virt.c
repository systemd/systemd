...
static int running_in_cgroupns(void) {
        ...
        if (r < 0)
                log_debug_errno(r, "Failed to test if in cgroup root namespace, ignoring: %m");
        else if (r > 0)
                return false;
...
static int running_in_pidns(void) {
        ...
        if (r < 0)
                return log_debug_errno(r, "Failed to test if in PID root namespace, ignoring: %m");
...
int running_in_userns(void) {
        ...
        if (r < 0)
                log_debug_errno(r, "Failed to test if in user root namespace, ignoring: %m");
        else if (r > 0)
                return false;
