...
int path_is_syscfg_tree(const char *path, const char *syscfg, bool relax_syscfg_release_check) {
        ...
        /* We use /etc/syscfg-release.d/syscfg-release[.NAME] as flag for something being a system configuration,
         * and {/etc|/usr/lib}/os-release as a flag for something being an OS (when not an extension). */
        r = open_syscfg_release(path, syscfg, relax_syscfg_release_check, NULL, NULL);
        if (r == -ENOENT) /* We got nothing */
                return 0;
        if (r < 0)
                return r;
...
static int syscfg_release_strict_xattr_value(int syscfg_release_fd, const char *syscfg_release_dir_path, const char *filename) {
        int r;

        assert(syscfg_release_fd >= 0);
        assert(syscfg_release_dir_path);
        assert(filename);

        /* No xattr or cannot parse it? Then skip this. */
        _cleanup_free_ char *syscfg_release_xattr = NULL;
        r = fgetxattr_malloc(syscfg_release_fd, "user.syscfg-release.strict", &syscfg_release_xattr);
        if (r < 0) {
                if (!ERRNO_IS_XATTR_ABSENT(r))
                        return log_debug_errno(r,
                                               "%s/%s: Failed to read 'user.syscfg-release.strict' extended attribute from file, ignoring: %m",
                                               syscfg_release_dir_path, filename);

                return log_debug_errno(r, "%s/%s does not have user.syscfg-release.strict xattr, ignoring.", syscfg_release_dir_path, filename);
        }

        /* Explicitly set to request strict matching? Skip it. */
        r = parse_boolean(syscfg_release_xattr);
        if (r < 0)
                return log_debug_errno(r,
                                       "%s/%s: Failed to parse 'user.syscfg-release.strict' extended attribute from file, ignoring: %m",
                                       syscfg_release_dir_path, filename);
        if (r > 0) {
                log_debug("%s/%s: 'user.syscfg-release.strict' attribute is true, ignoring file.",
                          syscfg_release_dir_path, filename);
                return true;
        }

        log_debug("%s/%s: 'user.syscfg-release.strict' attribute is false%s",
                  syscfg_release_dir_path, filename,
                  special_glyph(SPECIAL_GLYPH_ELLIPSIS));
...
int open_syscfg_release(const char *root, const char *syscfg, bool relax_syscfg_release_check, char **ret_path, int *ret_fd) {
        ...
        if (syscfg) {
                const char *syscfg_full_path;

                if (!image_name_is_valid(syscfg))
                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "The extension name %s is invalid.", syscfg);

                syscfg_full_path = strjoina("/etc/syscfg-release.d/syscfg-release.", syscfg);
                r = chase_symlinks(syscfg_full_path, root, CHASE_PREFIX_ROOT,
                                   ret_path ? &q : NULL,
                                   ret_fd ? &fd : NULL);
                log_full_errno_zerook(LOG_DEBUG, MIN(r, 0), "Checking for %s: %m", syscfg_full_path);

                /* Cannot find the expected syscfg-release file? The image filename might have been
                 * mangled on deployment, so fallback to checking for any file in the syscfg-release.d
                 * directory, and return the first one with a user.syscfg-release xattr instead.
                 * The user.syscfg-release.strict xattr is checked to ensure the author of the image
                 * considers it OK if names do not match. */
                if (r == -ENOENT) {
                        _cleanup_free_ char *syscfg_release_dir_path = NULL;
                        _cleanup_closedir_ DIR *syscfg_release_dir = NULL;

                        r = chase_symlinks_and_opendir("/etc/syscfg-release.d/", root, CHASE_PREFIX_ROOT,
                                                       &syscfg_release_dir_path, &syscfg_release_dir);
                        if (r < 0)
                                return log_debug_errno(r, "Cannot open %s/etc/syscfg-release.d/, ignoring: %m", root);

                        r = -ENOENT;
                        FOREACH_DIRENT(de, syscfg_release_dir, return -errno) {
                                int k;

                                if (!IN_SET(de->d_type, DT_REG, DT_UNKNOWN))
                                        continue;

                                const char *image_name = startswith(de->d_name, "syscfg-release.");
                                if (!image_name)
                                        continue;

                                if (!image_name_is_valid(image_name)) {
                                        log_debug("%s/%s is not a valid syscfg-release file name, ignoring.",
                                                  syscfg_release_dir_path, de->d_name);
                                        continue;
                                }

                                /* We already chased the directory, and checked that
                                 * this is a real file, so we shouldn't fail to open it. */
                                _cleanup_close_ int syscfg_release_fd = openat(dirfd(syscfg_release_dir),
                                                                                  de->d_name,
                                                                                  O_PATH|O_CLOEXEC|O_NOFOLLOW);
                                if (syscfg_release_fd < 0)
                                        return log_debug_errno(errno,
                                                               "Failed to open syscfg-release file %s/%s: %m",
                                                               syscfg_release_dir_path,
                                                               de->d_name);

                                /* Really ensure it is a regular file after we open it. */
                                if (fd_verify_regular(syscfg_release_fd) < 0) {
                                        log_debug("%s/%s is not a regular file, ignoring.", syscfg_release_dir_path, de->d_name);
                                        continue;
                                }

                                if (!relax_syscfg_release_check) {
                                        k = syscfg_release_strict_xattr_value(syscfg_release_fd,
                                                                                 syscfg_release_dir_path,
                                                                                 de->d_name);
                                        if (k != 0)
                                                continue;
                                ...
                                if (ret_fd)
                                        fd = TAKE_FD(syscfg_release_fd);

                                if (ret_path) {
                                        q = path_join(syscfg_release_dir_path, de->d_name);
                                        if (!q)
                                                return -ENOMEM;
                                }
                        }
...
int fopen_syscfg_release(const char *root, const char *syscfg, bool relax_syscfg_release_check, char **ret_path, FILE **ret_file) {
        ...
        if (!ret_file)
                return open_syscfg_release(root, syscfg, relax_syscfg_release_check, ret_path, NULL);

        r = open_syscfg_release(root, syscfg, relax_syscfg_release_check, ret_path ? &p : NULL, &fd);
        if (r < 0)
                return r;
...
static int parse_syscfg_release_internal(const char *root, bool relax_syscfg_release_check, const char *syscfg, va_list ap) {
        _cleanup_fclose_ FILE *f = NULL;
        _cleanup_free_ char *p = NULL;
        int r;

        r = fopen_syscfg_release(root, syscfg, relax_syscfg_release_check, &p, &f);
        if (r < 0)
                return r;
...
int _parse_syscfg_release(const char *root, bool relax_syscfg_release_check, const char *syscfg, ...) {
        va_list ap;
        int r;

        va_start(ap, syscfg);
        r = parse_syscfg_release_internal(root, relax_syscfg_release_check, syscfg, ap);
        va_end(ap);
...
int _parse_syscfg_os_release(const char *root, ...) {
        va_list ap;
        int r;

        va_start(ap, root);
        r = parse_syscfg_release_internal(root, /* relax_extension_release_check= */ false, NULL, ap);
        va_end(ap);
