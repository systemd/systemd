...
static const struct {
        ...
        const char *extension_path_name;
        const char *extension_full_path_name;
        const char *release_file_name;
} image_class_release_info[_IMAGE_CLASS_MAX] = {
        [IMAGE_EXTENSION] = {
                .strict_name = "user.extension-release.strict",
                .extension_path_name = "/usr/lib/extension-release.d/",
                .extension_full_path_name = "/usr/lib/extension-release.d/extension-release.",
                .release_file_name = "extension-release.",
        },
        [IMAGE_SYSCFG] = {
                .strict_name = "user.syscfg-release.strict",
                .extension_path_name = "/etc/syscfg-release.d/",
                .extension_full_path_name = "/etc/syscfg-release.d/syscfg-release.",
                .release_file_name = "syscfg-release.",
        }
};
...
static int extension_release_strict_xattr_value(ImageClass image_class, int extension_release_fd, const char *extension_release_dir_path, const char *filename) {
        ...
        r = fgetxattr_malloc(extension_release_fd, image_class_release_info[image_class].strict_name, &extension_release_xattr);
        if (r < 0) {
                if (!ERRNO_IS_XATTR_ABSENT(r))
                        return log_debug_errno(r,
                                               "%s/%s: Failed to read '%s' extended attribute from file, ignoring: %m",
                                               extension_release_dir_path, filename, image_class_release_info[image_class].strict_name);

                return log_debug_errno(r, "%s/%s does not have %s xattr, ignoring.", extension_release_dir_path, filename, image_class_release_info[image_class].strict_name);
        }
...
int open_extension_release(const char *root, ImageClass image_class, const char *extension, bool relax_extension_release_check, char **ret_path, int *ret_fd) {
        ...
        if (extension) {
                ...
                extension_full_path = strjoina(image_class_release_info[image_class].extension_full_path_name, extension);
                ...
                if (r == -ENOENT) {
                        ...
                        r = chase_symlinks_and_opendir(image_class_release_info[image_class].extension_path_name, root, CHASE_PREFIX_ROOT,
                                                       &extension_release_dir_path, &extension_release_dir);
                        if (r < 0)
                                return log_debug_errno(r, "Cannot open %s%s, ignoring: %m", root, image_class_release_info[image_class].extension_path_name);
                        ...
                        FOREACH_DIRENT(de, extension_release_dir, return -errno) {
                                ...
                                const char *image_name = startswith(de->d_name, image_class_release_info[image_class].release_file_name);
                                if (!image_name)
                                        continue;
