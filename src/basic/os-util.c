...
int open_extension_release(const char *root, const char *extension, char **ret_path, int *ret_fd) {
        ...
        if (extension) {
                ...
                /* An empty string extension means we should check for the unversioned filename, as a
                 * fallback for cases where image names are not stable and predictable. */
                if (!isempty(extension) && !image_name_is_valid(extension))
                        return log_debug_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "The extension name %s is invalid.", extension);

                if (isempty(extension))
                        extension_full_path = "/usr/lib/extension-release.d/extension-release";
                else
