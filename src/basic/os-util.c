        ...
        if (extension) {
                ...
                if (r == -ENOENT) {
                        ...
                        FOREACH_DIRENT(de, extension_release_dir, return -errno) {
                                ...
                                _cleanup_close_ int extension_release_fd = openat(dirfd(extension_release_dir), de->d_name, O_RDONLY|O_CLOEXEC|O_NOCTTY|O_NOFOLLOW);
                                if (extension_release_fd < 0)
                                        return log_debug_errno(errno, "Failed to open extension-release file %s/%s: %m", extension_release_dir_path, de->d_name);
                                ...
                                if (k < 0 && !ERRNO_IS_NOT_SUPPORTED(k) && k != -ENODATA)
                                        log_debug_errno(k, "Failed to read 'user.extension-release.strict' extended attribute from extension-release file %s/%s: %m", extension_release_dir_path, de->d_name);
                                if (k < 0)
                                        continue;
                                ...
                                if (k < 0)
                                        log_debug_errno(k, "Failed to parse 'user.extension-release.strict' extended attribute value from extension-release file %s/%s: %m", extension_release_dir_path, de->d_name);
                                if (k < 0 || k > 0)
                                        continue;
