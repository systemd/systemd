...
int chase_symlinks_at(int dir_fd, const char *path, ChaseSymlinksFlags flags, char **ret_path, int *ret_fd) {
        ...
        if (dir_fd < 0 && !ret_path && !(flags & (CHASE_NONEXISTENT|CHASE_NO_AUTOFS|CHASE_SAFE|CHASE_STEP)) && ret_fd) {
        ...
        rootfd = fd_reopen(MAX(dir_fd, -1), O_CLOEXEC|O_DIRECTORY|O_PATH);
        if (rootfd < 0)
                return rootfd;

        /* This is our iterator file descriptor which we initialize with a copy of the root file descriptor. */
        fd = fd_reopen(rootfd, O_CLOEXEC|O_DIRECTORY|O_PATH);
        if (fd < 0)
                return fd;
        ...
        for (todo = buffer;;) {
                ...
                if (S_ISLNK(st.st_mode) && !((flags & CHASE_NOFOLLOW) && isempty(todo))) {
                        ...
                        if (path_is_absolute(destination)) {
                                ...
                                safe_close(fd);
                                fd = fd_reopen(rootfd, O_CLOEXEC|O_PATH|O_DIRECTORY);
                                if (fd < 0)
                                        return fd;
