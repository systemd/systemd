...
/* This cannot be a static inline because gcc then loses the attributes on the function.
 * See: https://gcc.gnu.org/bugzilla/show_bug.cgi?id=96503 */
void *expand_to_usable(void *p, size_t newsize) _malloc_ _alloc_(2) _returns_nonnull_;

static inline size_t malloc_sizeof_safe(void **xp) {
        if (__builtin_expect(!xp || !*xp, 0))
                return 0;
...
#define MALLOC_SIZEOF_SAFE(x)                                                                               \
        ({                                                                                                  \
                void *nullp = NULL;                                                                         \
                malloc_sizeof_safe((void **) &__builtin_choose_expr(__builtin_constant_p(x), nullp, (x)));  \
        })
