...
static LogContext* log_context_attach(LogContext *c) {
        ...
        if (sd_fiber_current()) {
                sd_fiber_current()->log_context_num_fields += add;
                return LIST_PREPEND(ll, sd_fiber_current()->log_context, c);
        }
...
static LogContext* log_context_detach(LogContext *c) {
        ...
        if (sd_fiber_current()) {
                assert(sd_fiber_current()->log_context_num_fields >= sub);
                sd_fiber_current()->log_context_num_fields -= sub;
                LIST_REMOVE(ll, sd_fiber_current()->log_context, c);
                return NULL;
        }
...
LogContext* log_context_head(void) {
        return sd_fiber_current() ? sd_fiber_current()->log_context : _log_context;
}
...
size_t log_context_num_fields(void) {
        return sd_fiber_current() ? sd_fiber_current()->log_context_num_fields : _log_context_num_fields;
}
