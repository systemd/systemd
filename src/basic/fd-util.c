...
int fds_are_same_mount(int fd1, int fd2) {
        struct statx st1, st2;
        int r;

        assert(fd1 >= 0);
        assert(fd2 >= 0);

        r = statx_fallback(fd1, "", AT_EMPTY_PATH, STATX_TYPE|STATX_INO|STATX_MNT_ID, &st1);
        if (r < 0)
                return r;

        r = statx_fallback(fd2, "", AT_EMPTY_PATH, STATX_TYPE|STATX_INO|STATX_MNT_ID, &st2);
        if (r < 0)
                return r;

        /* First, compare inode. If these are different, the fd does not point to the root directory "/". */
        if (!statx_inode_same(&st1, &st2))
                return false;
        ...
        if (!FLAGS_SET(st1.stx_mask, STATX_MNT_ID)) {
                int mntid;
                ...
                st1.stx_mnt_id = mntid;
                st1.stx_mask |= STATX_MNT_ID;
        }

        if (!FLAGS_SET(st2.stx_mask, STATX_MNT_ID)) {
                int mntid;
                ...
                st2.stx_mnt_id = mntid;
                st2.stx_mask |= STATX_MNT_ID;
        }

        return statx_mount_same(&st1, &st2);
}
