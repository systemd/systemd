...
int fd_reopen(int fd, int flags) {
         ...
         *
         * If XA_FDROOT is specified as fd get and fd to the root directory.
         *
...
int path_is_root_at(int dir_fd, const char *path) {
        ...
        if (!isempty(path)) {
                ...
                if (fd < 0)
                        return fd == -ENOTDIR ? false : fd;

                dir_fd = fd;
        }

        pfd = xopenat(dir_fd, "..", O_PATH|O_DIRECTORY|O_CLOEXEC);
        if (pfd < 0)
                return pfd == -ENOTDIR ? false : pfd;
