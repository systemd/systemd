...
int path_is_root_at(int dir_fd, const char *path) {
        ...
        r = statx_fallback(dir_fd, path, AT_EMPTY_PATH | AT_SYMLINK_NOFOLLOW, STATX_TYPE|STATX_INO|STATX_MNT_ID, &st.sx);
        if (r < 0)
                return r;
        ...
        if (!FLAGS_SET(st.nsx.stx_mask, STATX_MNT_ID)) {
                ...
                r = path_get_mnt_id_at_fallback(dir_fd, path, &mntid);
        ...
        if (!FLAGS_SET(pst.nsx.stx_mask, STATX_MNT_ID)) {
                ...
                r = path_get_mnt_id_at_fallback(AT_FDCWD, "/", &mntid);
                if (ERRNO_IS_NEG_NOT_SUPPORTED(r))
                        return true; /* skip the mount ID check */
                if (r < 0)
                        return r;
