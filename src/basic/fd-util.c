...
int fd_reopen_propagate_append_and_position(int fd, int flags) {
         ...
         * but propagates O_APPEND if set on original fd,
         * and also tries to keep current file position.
         *
         * You should use this if the original fd potentially is O_APPEND,
         ...
         *
         * The "keep position" part is obviously nonsense for the O_APPEND case,
         * but should reduce surprises if someone carefully pre-positioned the
         * passed in original input or non-append output FDs. */
        ...
        if (p > 0) {
                ...
                if (new_p == (off_t)-1)
                        log_warning_errno(errno,
                                        "Failed to propagate file position for re-opened fd %d, "
                                        "ignoring: %m", fd);
                else if (new_p != p)
                        log_warning("Failed to propagate file position for re-opened fd %d (%lld != %lld), "
                                        "ignoring: %m", fd, (long long)new_p, (long long)p);
        }
