...
static inline int path_is_os_tree(const char *path) {
        return path_is_extension_tree(IMAGE_EXTENSION, path, NULL, false);
}
...
static inline int open_os_release(const char *root, char **ret_path, int *ret_fd) {
        return open_extension_release(root, IMAGE_EXTENSION, NULL, false, ret_path, ret_fd);
}
...
static inline int fopen_os_release(const char *root, char **ret_path, FILE **ret_file) {
        return fopen_extension_release(root, IMAGE_EXTENSION, NULL, false, ret_path, ret_file);
}
...
int _parse_os_release(const char *root, ImageClass image_class,...) _sentinel_;
...
#define parse_os_release(root, image_class,...) _parse_os_release(root, image_class, __VA_ARGS__, NULL)
