...
bool hostname_is_valid(const char *s, ValidHostnameFlags flags) {
        ...
        /* Note that LINUX_HOST_NAME_MAX is 64 on Linux, but DNS allows domain names up to 255 characters. */
        if (p-s > LINUX_HOST_NAME_MAX)
                return false;
...
char* hostname_cleanup(char *s) {
        ...
        for (p = s, d = s, dot = hyphen = true; *p && d - s < LINUX_HOST_NAME_MAX; p++)
                if (*p == '.') {
                        if (dot || hyphen)
                                continue;
