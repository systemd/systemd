...
bool unit_name_is_valid(const char *n, UnitNameFlags flags) {
        ...
        if (strlen(n) >= unit_name_max_length(t))
                return false;
...
int unit_name_from_path(const char *path, const char *suffix, char **ret) {
        ...
        if (strlen(s) >= unit_name_max_length(unit_type_from_string(suffix+1))) /* Return a slightly more descriptive error for this specific condition */
                return -ENAMETOOLONG;
