...
static int get_runpath(const char **ret) {
        ...
        /* Iterate through the Phdr structures to find the PT_DYNAMIC section */
        for (unsigned long i = 0; i < phnum; i++) {
                const ElfW(Phdr) *p = (const ElfW(Phdr)*) (phdr + (i * phent));

                if (p->p_type == PT_DYNAMIC)
                        return get_runpath_from_dynamic((const ElfW(Dyn)*) p->p_vaddr, ret);
        }
...
int get_build_exec_dir(char **ret) {
         ...
         * look for shared libraries). If so we know that this is not a regular installed binary, but one
         * which shall acquire its libraries from some specific directory, i.e. a build directory or
         * similar. In that case it typically makes sense to also search for our auxiliary executables we
         * fork() off in a directory close to our main program binary, rather than in the system.
         *
         ...
         *
         * Returns:
         *     -ENOEXEC         → We are not running in an rpath/runpath environment
        ...
        if (runpath_cached == -ERRNO_MAX-1)
                runpath_cached = get_runpath(NULL);
        if (runpath_cached < 0)
...
static int find_build_dir_binary(const char *path, char **ret) {
        int r;
        ...
        if (path_equal(path, np)) /* Same as the input? */
                return -EALREADY;
