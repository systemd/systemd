...
int mac_smack_read(const char *path, SmackAttr attr, const char **label) {
        assert(path);
        assert(attr >= SMACK_ATTR_ACCESS && attr < _SMACK_ATTR_MAX);
        assert(label);
...
int mac_smack_read_fd(int fd, SmackAttr attr, const char **label) {
        assert(fd >= 0);
        assert(attr >= SMACK_ATTR_ACCESS && attr < _SMACK_ATTR_MAX);
        assert(label);
        ...
        return getxattr_malloc_fd(fd, smack_attr_to_string(attr), label);
}
