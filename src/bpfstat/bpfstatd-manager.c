...
static int fetch_bpf_maps(sd_json_variant **ret) {
        uint32_t id = 0;
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *map_list = NULL;
        int r;
        ...
        while (sym_bpf_map_get_next_id(id, &id) >= 0) {
                ...
                r = sd_json_build(&v, SD_JSON_BUILD_OBJECT(
                                        SD_JSON_BUILD_PAIR_STRING("name", map_info.name),
                                        SD_JSON_BUILD_PAIR_UNSIGNED("type", map_info.type),
                                        SD_JSON_BUILD_PAIR_UNSIGNED("memlock", memlock)));
                if (r < 0) {
                        log_error_errno(r, "Failed to build map JSON object, ignoring: %m");
                        continue;
                }

                r = sd_json_variant_append_array(&map_list, v);
                if (r < 0) {
...
static int fetch_bpf_programs(sd_json_variant **ret) {
        _cleanup_hashmap_free_free_ Hashmap *map_name_by_id = NULL;
        uint32_t id = 0;
        sd_json_variant *prog_list = NULL;
        int r;
        ...
        while (sym_bpf_prog_get_next_id(id, &id) >= 0) {
                ...
                r = sd_json_build(&v, SD_JSON_BUILD_OBJECT(
                                        SD_JSON_BUILD_PAIR_STRING("name", prog_name),
                                        SD_JSON_BUILD_PAIR_UNSIGNED("type", prog_info.type),
                                        SD_JSON_BUILD_PAIR_UNSIGNED("memlock", memlock),
                                        SD_JSON_BUILD_PAIR_STRING("tag", prog_tag),
                                        SD_JSON_BUILD_PAIR_CONDITION(!strv_isempty(prog_map_names), "map_names", SD_JSON_BUILD_STRV(prog_map_names)),
                                        SD_JSON_BUILD_PAIR_UNSIGNED("run_time_ns", prog_info.run_time_ns),
                                        SD_JSON_BUILD_PAIR_UNSIGNED("run_count", prog_info.run_cnt)));
                if (r < 0) {
                        log_error_errno(r, "Failed to build JSON object for program, ignoring: %m");
                        continue;
                }

                r = sd_json_variant_append_array(&prog_list, v);
                if (r < 0) {
...
static int log_bpf_maps_and_progs(Manager *m) {
        sd_json_variant *map_list = NULL, *prog_list = NULL;
        ...
        r = sd_json_variant_format(map_list, SD_JSON_FORMAT_NEWLINE, &maps_formatted);
        if (r < 0)
        ...
        r = sd_json_variant_format(prog_list, SD_JSON_FORMAT_NEWLINE, &progs_formatted);
        if (r < 0)
...
static int vl_method_describe_maps(Varlink *link, sd_json_variant *parameters, VarlinkMethodFlags flags, void *userdata) {
        sd_json_variant *v = NULL, *map_list = NULL;
        ...
        length = sd_json_variant_elements(map_list);
...
static int vl_method_describe_programs(Varlink *link, sd_json_variant *parameters, VarlinkMethodFlags flags, void *userdata) {
        sd_json_variant *v = NULL, *prog_list = NULL;
        size_t idx = 0, length = 0;
        int r = 0;
        ...
        length = sd_json_variant_elements(prog_list);
