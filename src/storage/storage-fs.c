        ...
        FOREACH_ARRAY(dp, dentries->entries, dentries->n_entries) {
                ...
                case S_IFBLK: {
                        ...
                        if (r < 0)
                                ...
                                if (r < 0)
                                        log_debug_errno(r, "Failed to get read/only state of '%s', ignoring: %m", d->d_name);
                                else
                                        ro = r > 0;
                        }
...
static int create_volume_dir(
        ...
        r = RET_NERRNO(renameat(storage_fd, tf, storage_fd, filename));
        if (r < 0)
                goto fail;
...
static int vl_method_acquire(
        ...
        case VOLUME_DIR: {
                ...
                _cleanup_close_ int mount_fd = open_tree_attr_with_fallback(
                                ...
                                &(struct mount_attr) {
                                        .attr_set = (p.read_only > 0 ? MS_RDONLY : 0),
                                        .attr_clr = MS_NOSUID|MS_NOEXEC|MS_NODEV,
