...
VERB_NOARG(verb_services, "providers", "List storage providers");
        ...
        if (merror_id) {
                ...
                if (error_id) {
                        if (streq(error_id, "io.systemd.StorageProvider.NoSuchVolume"))
                                return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP), "Volume '%s' not known.", name);
                        if (streq(error_id, "io.systemd.StorageProvider.NoSuchTemplate"))
                                return log_error_errno(SYNTHETIC_ERRNO(EADDRNOTAVAIL), "Template '%s' not known.", template);
                        if (streq(error_id, "io.systemd.StorageProvider.VolumeExists"))
                                return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP), "Volume '%s' exists already.", name);
                        ...
                        if (streq(error_id, "io.systemd.StorageProvider.CreateNotSupported"))
                                return log_error_errno(SYNTHETIC_ERRNO(EADDRNOTAVAIL), "Storage provider does not support creating volumes.");
                        if (streq(error_id, "io.systemd.StorageProvider.CreateSizeRequired"))
                                return log_error_errno(SYNTHETIC_ERRNO(EADDRNOTAVAIL), "Storage provider requires a create size to be provided when creating volumes on-the-fly. Use 'storage.create-size=' mount option.");
                        if (streq(error_id, "io.systemd.StorageProvider.ReadOnlyVolume"))
                                return log_error_errno(SYNTHETIC_ERRNO(EADDRNOTAVAIL), "Volume '%s' is read-only.", name);
                        if (streq(error_id, "io.systemd.StorageProvider.BadTemplate"))
                                return log_error_errno(SYNTHETIC_ERRNO(EADDRNOTAVAIL), "Template '%s' does not apply to this volume type.", template);
        ...
        if (fstype) {
                ...
                if (p.base_uid > UINT32_MAX - 0x10000U ||
                    p.base_gid > UINT32_MAX - 0x10000U)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Returned base UID out of range.");
                ...
                if (st.st_uid < p.base_uid || st.st_uid >= p.base_uid + 0x10000 ||
                    st.st_gid < p.base_gid || st.st_gid >= p.base_gid + 0x10000)
                        return log_error_errno(SYNTHETIC_ERRNO(EPERM), "File descriptor for directory volume is not owned by base UID/GID range, refusing: %m");
