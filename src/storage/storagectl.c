...
static int run_as_mount_helper(int argc, char *argv[]) {
        ...
        r = storage_acquire_volume(arg_runtime_scope, &bv, arg_ask_password, &reply);
        if (r < 0 && fstype &&
            STR_IN_SET(strna(reply.error_id),
                       "io.systemd.StorageProvider.TypeNotSupported",
                       "io.systemd.StorageProvider.WrongType")) {
                /* The provider couldn't give us a regular file — retry as a block device. */
                _cleanup_(storage_acquire_reply_done) StorageAcquireReply retry = STORAGE_ACQUIRE_REPLY_INIT;
                bv.request_as = VOLUME_BLK;
                int rr = storage_acquire_volume(arg_runtime_scope, &bv, arg_ask_password, &retry);
                if (rr >= 0) {
                ...
                /* If both failed we keep the first error_id (matches the user's preferred type). */
        }

        if (r < 0) {
                const char *eid = reply.error_id;
                ...
                if (eid)
                        return log_error_errno(r, "Failed to issue io.systemd.StorageProvider.Acquire() varlink call: %s", eid);
        ...
        if (fstype) {
                ...
                _cleanup_free_ char *gid_line = asprintf_safe(
                                ...
                                reply.base_gid, (gid_t) 0, (gid_t) USERNS_RANGE_SIZE);
