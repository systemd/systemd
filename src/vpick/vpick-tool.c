...
static int help(void) {
        ...
        r = option_parser_get_help_table_group("Lookup Keys", &lookup_keys);
        if (r < 0)
                return r;
        ...
        (void) table_sync_column_width(lookup_keys, 0, output, 0);
        ...
        table_print(lookup_keys, stdout);

        printf("\n%sOutput:%s\n", ansi_underline(), ansi_normal());
        table_print(output, stdout);
