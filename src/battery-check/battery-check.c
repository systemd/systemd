...
static int run(int argc, char *argv[]) {
        ...
        if (r > 0) {
                ...
                if (asprintf(&message, "%s Battery level critically low. Please connect your charger or the system will power off in 10 seconds.", special_glyph(SPECIAL_GLYPH_LOW_BATTERY, true)) < 0)
                        return log_oom();
                ...
                battery_check_send_plymouth_message(message, "shutdown");
                sleep(10);
                ...
                if (asprintf(&message, "A.C. power restored, continuing") < 0)
                        return log_oom();

                log_info("%s", message);
                dprintf(fd, "%s\n", message);
                battery_check_send_plymouth_message(message, "boot-up");
        }
        return r;
}
