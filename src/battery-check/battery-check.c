...
static void battery_check_send_plymouth_message(char *message) {
        ...
        _cleanup_free_ char *plymouth_message = NULL, *plymouth_shutdown = NULL;

        if (asprintf(&plymouth_message, "M\002%c%s", (int) strlen(message), message) < 0) {
                log_oom();
                return;
        }
        if (asprintf(&plymouth_shutdown, "C\002%c%s", (int) strlen("shutdown"), "shutdown") < 0) {
                log_oom();
                return;
        }
        ...
        w = strlen(plymouth_message);
        if (write(fd, plymouth_message, strlen(plymouth_message)) != w)
        ...
        w = strlen(plymouth_shutdown);
        if (write(fd, plymouth_shutdown, strlen(plymouth_shutdown)) != w)
                log_full_errno(IN_SET(errno, EAGAIN, ENOENT) || ERRNO_IS_DISCONNECT(errno) ? LOG_DEBUG : LOG_WARNING, errno, "Failed to write to plymouth: %m");
