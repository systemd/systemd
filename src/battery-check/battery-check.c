...
static int run(int argc, char *argv[]) {
        ...
        if (r > 0) {
                _cleanup_close_ int fd = -EBADF;
                _cleanup_free_ char *message = NULL, *plymouth_message = NULL;
                ...
                if (asprintf(&plymouth_message, "A.C. power restored, continuing") < 0)
                        return log_oom();

                message = plymouth_message;

                log_info("%s", message);
                dprintf(fd, "%s\n", message);
                battery_check_send_plymouth_message(plymouth_message, "boot-up");
        }
        return r;
}
