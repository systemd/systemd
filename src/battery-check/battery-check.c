...
static void battery_check_send_plymouth_shutdown_message(char *message) {
        assert(message);
...
static void battery_check_send_plymouth_boot_up_message(char *message) {
        assert(message);
...
static int run(int argc, char *argv[]) {
        ...
        if (r > 0) {
                ...
                battery_check_send_plymouth_shutdown_message(message);
                sleep(10);
                ...
                log_info("%s", message);
                dprintf(fd, "%s\n", message);
                battery_check_send_plymouth_boot_up_message(message);
        }
        return r;
}
