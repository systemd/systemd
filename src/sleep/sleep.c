...
static int execute_s2h(const SleepConfig *sleep_config) {
        ...
        r = check_wakeup_type();
        if (r < 0)
                log_debug_errno(r, "Failed to fetch wakeup-type bit, ignoring: %m");

        s = battery_trip_point_alarm_exists();
        if (s < 0)
                log_debug_errno(r, "Failed to check acpi_btp support enabled or not, ignoring: %m");
