        ...
        while (battery_is_low() == 0) {
                ...
                if (!woken_by_timer) {
                        if (current_capacity < last_capacity) {
                                ...
                                log_debug("Attempting to estimate battery discharge rate after manual wakeup");
                ...
                if (current_capacity >= last_capacity)
                        ...
                        log_debug("Attempting to estimate battery discharge rate after auto wakeup from %s hours sleep timer",
                                  FORMAT_TIMESPAN(suspend_interval, USEC_PER_HOUR));

                        /* If woken up after alarm time, estimate discharge rate for suspend interval */
                        estimated_discharge_rate = (last_capacity - current_capacity) * USEC_PER_HOUR / suspend_interval;
