...
static int write_efi_hibernate_location(const HibernateLocation *hibernate_location, bool required) {
        ...
        r = json_build(&v, JSON_BUILD_OBJECT(
                               JSON_BUILD_PAIR("uuid", JSON_BUILD_UUID(uuid)),
                               JSON_BUILD_PAIR("offset", JSON_BUILD_UNSIGNED(hibernate_location->offset)),
                               ...
                               JSON_BUILD_PAIR_CONDITION(uts.release, "kernelVersion", JSON_BUILD_STRING(uts.release))));
        if (r < 0)
