...
static int write_efi_hibernate_info(dev_t devno, const char *offset, int log_level) {
        int r = 0;
        ...
        r = devname_from_devnum(S_IFBLK, devno, &device);
        if (r < 0)
                return log_full_errno(log_level,
                                      r,
                                      "Failed to get resume devname from devnum '" DEVNUM_FORMAT_STR "': %m",
...
static int write_hibernate_location_info(const HibernateLocation *hibernate_location, bool efivar_only) {
        ...
        r = write_efi_hibernate_info(hibernate_location->devno, offset_str, efivar_only ? LOG_DEBUG : LOG_WARNING);
