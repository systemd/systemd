...
static int execute_s2h(const SleepConfig *sleep_config) {
        ...
        do {
                ...
                if (r > 0) {
                        ...
                        log_debug_errno(r, "Battery capacity percentage unavailable, System will be suspended for hibernatedelaysec interval: %m");
                        /*_exit(r);*/
                ...
                if (!FLAGS_SET(r, POLLIN))  { /* We woke up before alarm time, estimate discharge rate using suspension duration. */
                        after_timestamp = now(CLOCK_BOOTTIME);
                        time_elapsed = (after_timestamp - before_timestamp) / USEC_PER_SEC;
