...
static int write_efi_hibernate_info(const char *device, const char *offset, int log_level) {
        ...
        char info_name[STRLEN("HibernateInfo-") + SD_ID128_STRING_MAX];
        const char *uuid;
        ...
        if (!sd_id128_is_null(machine_id))
                xsprintf(info_name, "HibernateInfo-" SD_ID128_FORMAT_STR, SD_ID128_FORMAT_VAL(machine_id));
        else
                strcpy(info_name, "HibernateInfo");

        r = efi_set_variable_string(EFI_SYSTEMD_VARIABLE_STR(info_name), info);
        if (r < 0)
