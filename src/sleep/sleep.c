...
static int execute_s2h(const SleepConfig *sleep_config) {
        _cleanup_hashmap_free_ Hashmap *last_capacity = NULL;
        _cleanup_hashmap_free_ Hashmap *current_capacity = NULL;
        int r;
        ...
        while (battery_is_low() == 0) {
                ...
                bool woken_by_timer;
                int total_battery_capacity, previous_discharge_rate;
                ...
                r = hashmap_battery_name_with_battery_capacity(&last_capacity);
                if (r >= 0)
                ...
                r = get_total_battery_capacity();
                if (r < 0)
                        log_full_errno(r == -ENOENT ? LOG_DEBUG : LOG_WARNING, r, "Failed to get discharge rate, ignoring: %m");
                total_battery_capacity = r;

                r = get_battery_discharge_rate();
                if (r < 0)
                        log_full_errno(r == -ENOENT ? LOG_DEBUG : LOG_WARNING, r, "Failed to get discharge rate, ignoring: %m");
                else if (total_battery_capacity * 2 <= r) { /* function to add all current capacity and then compare sum*/
                        ...
                        suspend_interval = usec_sub_unsigned(total_battery_capacity * USEC_PER_HOUR / previous_discharge_rate, 30 * USEC_PER_MINUTE);
                ...
                r = hashmap_battery_name_with_battery_capacity(&current_capacity);
                if (r == -ENOENT) {
