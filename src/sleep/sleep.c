...
static int execute_s2h(const SleepConfig *sleep_config) {
        int r;
        assert(sleep_config);
        usec_t suspend_interval = sleep_config->hibernate_delay_sec, before_timestamp, after_timestamp;

        do {
                _cleanup_close_ int tfd = -1;
                struct itimerspec ts = {};
                int last_capacity, current_capacity, previous_discharge_time;
                double estimated_discharge_rate;
                ...
                previous_discharge_time = get_battery_discharge_rate();
                if (previous_discharge_time < 0)
                        log_error_errno(errno, "Error fetching battery discharge rate: %m");
                else
                ...
                if (!FLAGS_SET(r, POLLIN))  { /* We woke up before the alarm time, calculate battery discharged and estimate rate. */
                        ...
                        estimated_discharge_rate = (last_capacity - current_capacity) / (before_timestamp - after_timestamp);
                        log_debug("Battery will discharge in %s time", FORMAT_TIMESPAN(estimated_discharge_rate, USEC_PER_HOUR));
                        r = put_battery_discharge_rate(FORMAT_TIMESPAN(estimated_discharge_rate, USEC_PER_HOUR));
                        if (r < 0)
                ...
                /* the wiggle val to be removed after testing */
                r = put_battery_discharge_rate(FORMAT_TIMESPAN(estimated_discharge_rate, USEC_PER_HOUR));
                if (r < 0)
                ...
                suspend_interval = ((estimated_discharge_rate * 60) - 30) * USEC_PER_MINUTE;
