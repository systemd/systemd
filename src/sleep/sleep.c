...
static int write_efi_hibernate_info(const char *device, const char *offset, int log_level) {
        ...
        r = efi_set_variable_string(EFI_SYSTEMD_VARIABLE_STR(name), info);
        if (r < 0)
                return log_full_errno(log_level, r, "Failed to set EFI variable '%s': %m", name);

        log_debug("Set EFI variable '%s' to '%s'.", name, info);
#endif
