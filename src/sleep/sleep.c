...
static int write_efi_hibernate_info(const char *device, const char *offset, int log_level) {
        ...
        char info_name[STRLEN("HibernateInfo-") + SD_ID128_STRING_MAX + STRLEN("-" EFI_VENDOR_SYSTEMD_STR)];
        const char *uuid;
        ...
        if (!sd_id128_is_null(machine_id))
                xsprintf(info_name,
                         "HibernateInfo-" SD_ID128_FORMAT_STR "-" EFI_VENDOR_SYSTEMD_STR,
                         SD_ID128_FORMAT_VAL(machine_id));
        else
                strcpy(info_name, "HibernateInfo" "-" EFI_VENDOR_SYSTEMD_STR);

        r = efi_set_variable_string(info_name, info);
        if (r < 0)
                return log_full_errno(log_level, r, "Failed to set EFI variable '%s': %m", info_name);

        log_debug("Set EFI variable '%s' to '%s'.", info_name, info);
#endif
