...
static int write_wakeup_count(void) {
        ...
        r = read_full_file("/run/systemd/system/wakeup_count", &buf, NULL);
        if (r == -ENOENT) {
                r = read_full_virtual_file("/sys/power/wakeup_count", &buf, NULL);
                if (r < 0)
                        return log_debug_errno(r, "Unable to read kernel wakeup count: %m");
        } else if (r < 0) {
                return log_debug_errno(r, "Unable to read systemd wakeup count: %m");
        } else {
                r = unlink("/run/systemd/system/wakeup_count");
                if (r < 0)
                        return log_debug_errno(r, "Unable to unlink wakeup_count: %m");
        }
