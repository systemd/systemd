...
typedef enum CoredumpHandshakeState {
        COREDUMP_HANDSHAKE_WAITING_REQUEST,
        COREDUMP_HANDSHAKE_SENDING_ACK,
        COREDUMP_HANDSHAKE_WAITING_MARKER,
        COREDUMP_HANDSHAKE_DONE,
        _COREDUMP_HANDSHAKE_STATE_MAX,
        _COREDUMP_HANDSHAKE_STATE_INVALID = -EINVAL,
} CoredumpHandshakeState;

typedef struct HandshakeContext {
        CoredumpHandshakeState state;
        struct coredump_req req;
} HandshakeContext;

static int handshake_process_request(HandshakeContext *hc, sd_event_source *s, int fd, uint32_t revents) {
        int r;

        assert(hc);
        assert(hc->state == COREDUMP_HANDSHAKE_WAITING_REQUEST);
        assert(s);
        assert(fd >= 0);
        ...
        hc->state = COREDUMP_HANDSHAKE_SENDING_ACK;
...
static int handshake_send_ack(HandshakeContext *hc, sd_event_source *s, int fd, uint32_t revents) {
        int r;

        assert(hc);
        assert(hc->state == COREDUMP_HANDSHAKE_SENDING_ACK);
        assert(s);
        assert(fd >= 0);
        ...
        struct coredump_ack ack = {
                ...
                .size = MIN(sizeof(struct coredump_ack), hc->req.size_ack),
        ...
        hc->state = COREDUMP_HANDSHAKE_WAITING_MARKER;
...
static int handshake_process_marker(HandshakeContext *hc, sd_event_source *s, int fd, uint32_t revents) {
        assert(hc);
        assert(hc->state == COREDUMP_HANDSHAKE_WAITING_MARKER);
        assert(s);
        assert(fd >= 0);
        assert(FLAGS_SET(revents, EPOLLIN));
        ...
        case COREDUMP_MARK_REQACK:
                ...
                hc->state = COREDUMP_HANDSHAKE_DONE;
                return 1;
...
static int on_handshake_io(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        HandshakeContext *hc = ASSERT_PTR(userdata);
        int r;
        ...
        switch (hc->state) {
        case COREDUMP_HANDSHAKE_WAITING_REQUEST:
                r = handshake_process_request(hc, s, fd, revents);
                break;
        case COREDUMP_HANDSHAKE_SENDING_ACK:
                r = handshake_send_ack(hc, s, fd, revents);
                break;
        case COREDUMP_HANDSHAKE_WAITING_MARKER:
                r = handshake_process_marker(hc, s, fd, revents);
                break;
        default:
                assert_not_reached();
        }
...
static int coredump_handshake(CoredumpContext *context) {
        int r;
        ...
        HandshakeContext hc = {};
        r = sd_event_add_io(e, /* ret= */ NULL, context->input_fd, EPOLLIN, on_handshake_io, &hc);
        if (r < 0)
        ...
        if (hc.state != COREDUMP_HANDSHAKE_DONE) /* Maybe received SIGTERM/SIGINT. */
