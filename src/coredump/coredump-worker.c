...
static int coredump_process_request(CoredumpContext *context) {
        int r;
        ...
        SocketContext sc = {};
        r = sd_event_add_io(e, /* ret= */ NULL, context->input_fd, EPOLLIN, on_coredump_io, &sc);
        if (r < 0)
...
int coredump_worker(const CoredumpConfig *config, int coredump_fd, usec_t timestamp) {
        ...
        r = coredump_process_request(&context);
