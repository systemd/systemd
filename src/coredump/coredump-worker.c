...
static int send_to_new_container(CoredumpContext *context) {
        ...
        return varlink_callbo_and_log(vl, "io.systemd.Coredump.Client.Submit", /* ret_parameters= */ NULL,
...
static int send_to_legacy_container(CoredumpContext *context) {
        ...
        /* FIXME: we cannot get the exit code of the crashed process through PIDFD_GET_INFO, as the process
         * has not been reaped yet. The old coredump client requires that COREDUMP_SIGNAL= field is set.
         * Let's set a fake signal here. */
        ...
        /* The legacy interface does not support the new request protocol. We need to process that here. */
        r = coredump_process_socket(&container_context);
        if (r < 0)
                return r;

        /* FIXME: Skip coredump_process_socket() in coredump_worker() when coredump_send() below failed. */
