...
#define EVENT_PRIORITY_SIGTERM_SIGINT  (SD_EVENT_PRIORITY_NORMAL - 5)
...
static int manager_spawn_worker(Manager *manager, int coredump_fd) {
        ...
        r = pidref_safe_fork_full(
                        ...
                        /* stdio_fds= */ NULL,
                        (int[]) { coredump_fd }, 1,
                        FORK_DEATHSIG_SIGTERM | FORK_CLOSE_ALL_FDS | FORK_REOPEN_LOG,
                        &worker->pidref);
        if (r < 0)
                return r;
        if (r == 0) {
...
static int manager_register_coredump_socket(Manager *manager) {
        ...
        r = sd_varlink_callbo(
                        vl,
                        "io.systemd.Coredump.Register.RegisterSocket",
                        &reply,
                        &error_id,
...
int coredump_manager(int argc, char *argv[]) {
        ...
        /* This requires the following flags to be supported by the kernel. */
        r = pidfd_info_mask_is_supported(PIDFD_INFO_COREDUMP | PIDFD_INFO_COREDUMP_SIGNAL);
        if (r < 0 && !ERRNO_IS_NEG_NOT_SUPPORTED(r)) {
                log_debug_errno(r, "Failed to check if PIDFD_INFO_COREDUMP and PIDFD_INFO_COREDUMP_SIGNAL flags are supported by ioctl(PIDFD_GET_INFO): %m");
                return 0;
        }
        if (r <= 0) {
