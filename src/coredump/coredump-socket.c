...
static int socket_process_request(SocketContext *socket, int fd, uint32_t revents) {
        ...
        size_t l = endoffsetof_field(union coredump_req_union, req.size);
        ssize_t n = recv(fd, &req, l, MSG_PEEK | MSG_WAITALL);
        if (n < 0) {
                if (ERRNO_IS_TRANSIENT(errno))
                        return 0;
                return log_debug_errno(errno, "Failed to receive coredump request header: %m");
        }
        if ((size_t) n != l)
                return log_debug_errno(errno, "Received coredump request header with unexpected size (%zi != %zu): %m", n, l);

        /* Verify the acquired request size. */
        if (req.req.size < COREDUMP_REQ_SIZE_VER0)
                return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
                                       "Acquired coredump request size is too small (%"PRIu32" < %i)",
                                       req.req.size, COREDUMP_REQ_SIZE_VER0);
        if (req.req.size > COREDUMP_REQ_SIZE_MAX)
                return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
                                       "Acquired coredump request size is too large (%"PRIu32" > %u)",
                                       req.req.size, COREDUMP_REQ_SIZE_MAX);

        l = req.req.size;
        n = recv(fd, &req, l, MSG_WAITALL);
        if (n < 0) {
                if (ERRNO_IS_TRANSIENT(errno))
                        return 0;
        ...
        if ((size_t) n != l)
                return log_debug_errno(errno, "Received coredump request with unexpected size (%zi != %zu): %m", n, l);
...
static int socket_process_marker(SocketContext *socket, int fd, uint32_t revents) {
        ...
        ssize_t n = recv(fd, &mark, sizeof(mark), MSG_WAITALL);
        if (n < 0) {
                if (ERRNO_IS_TRANSIENT(errno))
                        return 0;
