...
static int socket_process_request(SocketContext *sc, sd_event_source *s, int fd, uint32_t revents) {
        ...
        if (!FLAGS_SET(req.req.mask, COREDUMP_KERNEL | COREDUMP_WAIT))
                return log_debug_errno(SYNTHETIC_ERRNO(EBADMSG),
...
static int coredump_process_request(int fd) {
        ...
        SocketContext sc = {};
        r = sd_event_add_io(e, NULL, fd, EPOLLIN, on_coredump_io, &sc);
        if (r < 0)
...
int coredump_socket_worker(int coredump_fd) {
        int r;
