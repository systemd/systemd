...
int coredump_process_request(int coredump_fd) {
        ...
        r = sd_event_add_io(e, NULL, coredump_fd, EPOLLIN | EPOLLOUT, on_coredump_io, &socket);
        if (r < 0)
...
static void log_reset_targetp(LogTarget *p) {
        assert(p);

        if (*p >= 0)
                log_set_target_and_open(*p);
}

static LogTarget log_set_safe_target(int coredump_fd) {
        assert(coredump_fd >= 0);

        if (log_on_console())
                return _LOG_TARGET_INVALID;

        _cleanup_(pidref_done) PidRef pidref = PIDREF_NULL;
        if (getpeerpidref(coredump_fd, &pidref) < 0)
                return _LOG_TARGET_INVALID;
        ...
        if (!streq_ptr(unit, SPECIAL_JOURNALD_SERVICE))
                return _LOG_TARGET_INVALID;
...
int coredump_process_socket(
                ...
                int coredump_fd, /* This invalidates the fd even on failure. */
                bool request_mode,
                usec_t timestamp,
                bool forwarded) {
        ...
        _cleanup_(log_reset_targetp) LogTarget t = log_set_safe_target(coredump_fd);
