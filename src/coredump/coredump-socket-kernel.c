...
int coredump_register_socket(int argc, char *argv[]) {
        ...
        if (r == 0) {
                if (unlink(COREDUMP_SOCKET_PATH"-request") < 0 && errno != ENOENT)
                        return log_error_errno(errno, "Failed to remove "COREDUMP_SOCKET_PATH"-request: %m");
        } else {
                r = symlink_idempotent(COREDUMP_SOCKET_PATH, COREDUMP_SOCKET_PATH"-request", /* make_relative= */ true);
                if (r < 0)
                        return log_error_errno(r, "Failed to create symbolic link "COREDUMP_SOCKET_PATH"-request: %m");
        }
