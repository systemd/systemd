...
static const char* const coredump_field_table[_COREDUMP_FIELD_MAX] = {
        ...
        [COREDUMP_FIELD_COREDUMP]         = "COREDUMP",
...
static int coredump_fields_load(sd_journal *j, CoredumpFields *ret) {
        ...
        SD_JOURNAL_FOREACH_DATA(j, d, l) {
                for (CoredumpField i = 0; i < _COREDUMP_FIELD_MAX; i++)
                        RETRIEVE(d, l, coredump_field_table[i], ret->fields[i]);
        }
        ...
        } else if (ret->fields[COREDUMP_FIELD_COREDUMP])
...
static int print_info_json(FILE *file, sd_journal *j) {
        ...
        r = sd_json_build(&v, SD_JSON_BUILD_OBJECT(
                ...
                SD_JSON_BUILD_PAIR_CONDITION(pid_as_int <= 0 && !!f.fields[COREDUMP_FIELD_PID], "PID", SD_JSON_BUILD_STRING(f.fields[COREDUMP_FIELD_PID])),
                SD_JSON_BUILD_PAIR_CONDITION(tid_as_int > 0, "TID", SD_JSON_BUILD_UNSIGNED(tid_as_int)),
                SD_JSON_BUILD_PAIR_CONDITION(tid_as_int <= 0 && !!f.fields[COREDUMP_FIELD_TID], "TID", SD_JSON_BUILD_STRING(f.fields[COREDUMP_FIELD_TID])),
                ...
                SD_JSON_BUILD_PAIR_CONDITION(uid_as_int == UID_INVALID && !!f.fields[COREDUMP_FIELD_UID], "UID", SD_JSON_BUILD_STRING(f.fields[COREDUMP_FIELD_UID])),
                SD_JSON_BUILD_PAIR_CONDITION(gid_as_int != GID_INVALID, "GID", SD_JSON_BUILD_UNSIGNED(gid_as_int)),
                SD_JSON_BUILD_PAIR_CONDITION(gid_as_int == GID_INVALID && !!f.fields[COREDUMP_FIELD_GID], "GID", SD_JSON_BUILD_STRING(f.fields[COREDUMP_FIELD_GID])),
                ...
                SD_JSON_BUILD_PAIR_CONDITION(owner_uid_as_int == UID_INVALID && !!f.fields[COREDUMP_FIELD_OWNER_UID], "OwnerUID", SD_JSON_BUILD_STRING(f.fields[COREDUMP_FIELD_OWNER_UID])),
