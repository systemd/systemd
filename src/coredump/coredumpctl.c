...
static int coredump_fields_load(sd_journal *j, CoredumpFields *ret) {
        ...
        SD_JOURNAL_FOREACH_DATA(j, d, l) {
                ...
                RETRIEVE(d, l, "COREDUMP_TID", ret->tid);
                RETRIEVE(d, l, "COREDUMP_THREAD_NAME", ret->thread_name);
        ...
        if (ret->pkgmeta_json) {
                r = sd_json_parse(ret->pkgmeta_json, SD_JSON_PARSE_MUST_BE_OBJECT, &ret->package_json, NULL, NULL);
                if (r < 0) {
...
static int print_info_json(FILE *file, sd_journal *j) {
        ...
        if (f.tid)
                (void) parse_pid(f.tid, &tid_as_int);
        if (f.uid)
        ...
        r = sd_json_build(&v, SD_JSON_BUILD_OBJECT(
                ...
                SD_JSON_BUILD_PAIR_CONDITION(!!f.thread_name, "ThreadName", SD_JSON_BUILD_STRING(f.thread_name)),
                ...
                SD_JSON_BUILD_PAIR_CONDITION(f.normal_coredump && sig_as_int <= 0 && !!f.sgnl, "Signal", SD_JSON_BUILD_STRING(f.sgnl)),
