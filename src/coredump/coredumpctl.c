...
static int parse_argv(int argc, char *argv[]) {
        ...
        if ((!!arg_image + !!arg_root) > 1)
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Please specify either --root= or --image=, the combination of these options is not supported.");
...
static int save_core(sd_journal *j, FILE *file, char **path, bool *unlink_temp) {
        ...
        if (r == 0) {
                ...
                /* If a journal directory or file is passed we might be looking for a core file on the host,
                 * for example when accessing a core generated by nspawn, so try again without arg_root. */
                r = chase_and_access(filename, arg_root, CHASE_PREFIX_ROOT, F_OK, &resolved);
                if (r == -ENOENT && (arg_directory || arg_file)) {
                        r = chase_and_access(filename, NULL, 0, F_OK, &resolved);
                        if (r < 0)
                                return log_error_errno(r, "Cannot access \"%s\": %m", filename);
                } else if (r < 0)
