...
#define PIDFD_INFO_REQUIRED_FLAGS    \
        (PIDFD_INFO_PID |            \
         PIDFD_INFO_CREDS |          \
         PIDFD_INFO_EXIT |           \
         PIDFD_INFO_COREDUMP |       \
...
int coredump_context_parse_from_peer(CoredumpContext *context) {
        ...
        struct pidfd_info info = {
                .mask = PIDFD_INFO_REQUIRED_FLAGS,
        };
        ...
        if (!FLAGS_SET(info.supported_mask, PIDFD_INFO_REQUIRED_FLAGS))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                       "ioctl(PIDFD_GET_INFO) does not support required flags.");
