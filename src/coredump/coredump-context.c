...
static void coredump_context_read_pidfd_info(CoredumpContext *context) {
        ...
        if (ERRNO_IS_NEG_NOT_SUPPORTED(r)) {
                log_debug_errno(r, "PIDFD_INFO_COREDUMP not supported, ignoring: %m");
                return;
        }

        if (r < 0) {
                log_debug_errno(r, "Failed to get pidfd coredump info, ignoring: %m");
                return;
        }
...
static int coredump_context_parse_from_procfs(CoredumpContext *context) {
        ...
        coredump_context_read_pidfd_info(context);
