...
int coredump_context_update_signal(CoredumpContext *context) {
        ...
        /* Close input_fd to make the crashed process being killed now. Otherwise, ioctl(PIDFD_GET_INFO) does
         * not provide PIDFD_INFO_EXIT and we cannot get signal number. */
        ...
        for (;;) {
                ...
                r = pidfd_get_info(context->pidref.fd, &info);
                if (r >= 0) {
                        /* Both PIDFD_INFO_COREDUMP and PIDFD_INFO_EXIT are since kernel v6.16. The
                         * PIDFD_INFO_EXIT flag may not be set even the kernel is new enough, as the process
                         * may not exit yet, hence here let's check if PIDFD_INFO_COREDUMP flag is set. It
                         * should be always set when we request it and the kernel supports it. Otherwise, we
                ...
                } else if (r != -ESRCH) /* When the process is under terminating, PIDFD_GET_INFO may return
                                         * ESRCH and that's fine. Let's try again later in that case. */
