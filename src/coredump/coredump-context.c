...
static int context_parse_one(CoredumpContext *context, MetadataField meta, bool from_kernel, const char *s) {
        int r;
        ...
        case META_ARGV_TIMESTAMP:
                ...
                r = parse_time(s, &context->timestamp, from_kernel ? USEC_PER_SEC : 1);
                if (r < 0)
        ...
        case META_ARGV_PIDFD: {
                /* We do not forward the of the file descriptor, as it is meaningless, and always set to 1. */
                if (!from_kernel) {
                        if (!streq(s, "1"))
...
int coredump_context_parse_iovw(CoredumpContext *context) {
        ...
        FOREACH_ARRAY(iovec, context->iovw.iovec, context->iovw.count) {
                ...
                for (MetadataField i = 0; i < _META_MAX; i++) {
                        ...
                        r = context_parse_one(context, i, /* from_kernel= */ false, p);
                        if (r < 0)
                                return r;
...
int coredump_context_parse_from_argv(CoredumpContext *context, int argc, char **argv) {
        ...
        for (MetadataField i = 0; i < MIN(argc, _META_ARGV_MAX); i++) {
                r = context_parse_one(context, i, /* from_argv= */ true, argv[i]);
                if (r < 0)
                        return r;
        }
