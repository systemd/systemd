...
int parse_elf_object(int fd, const char *executable, char **ret, JsonVariant **ret_package_metadata) {
        ...
        if (r == 0) {
                 ...
                 * Why not adding a flag or letting the caller decide? Because we want to
                 * get at least one core from systemd-coredump, when the child crashes while
                 * analyzing the original core file, but then break the chain. */
                if (endswith(executable, "systemd-coredump"))
