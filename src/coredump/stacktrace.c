...
static struct StackContext* stack_context_destroy(struct StackContext *c) {
        if (!c)
                return NULL;
...
DEFINE_TRIVIAL_CLEANUP_FUNC(struct StackContext *, stack_context_destroy);
DEFINE_TRIVIAL_CLEANUP_FUNC_FULL(Elf *, elf_end, NULL);

static int frame_callback(Dwfl_Frame *frame, void *userdata) {
        struct StackContext *c = userdata;
...
static int parse_package_metadata(const char *name, JsonVariant *id_json, Elf *elf, struct StackContext *c) {
...
static int parse_core(int fd, const char *executable, char **ret, JsonVariant **ret_package_metadata) {
        ...
        if (!c.elf) {
                log_warning("Could not parse core file, elf_begin() failed: %s", elf_errmsg(elf_errno()));
                return -EINVAL;
        }

        c.dwfl = dwfl_begin(&callbacks);
        if (!c.dwfl) {
                log_warning("Could not parse core file, dwfl_begin() failed: %s", dwfl_errmsg(dwfl_errno()));
                return -EINVAL;
        }

        if (dwfl_core_file_report(c.dwfl, c.elf, executable) < 0) {
                log_warning("Could not parse core file, dwfl_core_file_report() failed: %s", dwfl_errmsg(dwfl_errno()));
                return -EINVAL;
        }

        if (dwfl_report_end(c.dwfl, NULL, NULL) != 0) {
                log_warning("Could not parse core file, dwfl_report_end() failed: %s", dwfl_errmsg(dwfl_errno()));
                return -EINVAL;
        }

        if (dwfl_getmodules(c.dwfl, &module_callback, &c, 0) < 0) {
                log_warning("Could not parse core file, dwfl_getmodules() failed: %s", dwfl_errmsg(dwfl_errno()));
                return -EINVAL;
        }

        if (dwfl_core_file_attach(c.dwfl, c.elf) < 0) {
                log_warning("Could not parse core file, dwfl_core_file_attach() failed: %s", dwfl_errmsg(dwfl_errno()));
                return -EINVAL;
        }

        if (dwfl_getthreads(c.dwfl, thread_callback, &c) < 0) {
                log_warning("Could not parse core file, dwfl_getthreads() failed: %s", dwfl_errmsg(dwfl_errno()));
                return -EINVAL;
        }
