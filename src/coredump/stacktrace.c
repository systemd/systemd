...
static int parse_package_metadata(const char *name, Elf *elf, struct stack_context *c) {
...
static int module_callback(Dwfl_Module *mod, void **userdata, const char *name, Dwarf_Addr start, void *arg) {
        ...
        if (elf)
                return parse_package_metadata(name, elf, c);
        ...
        for (size_t i = 0; i < n_program_headers; ++i) {
                ...
                if (!memelf)
                        continue;
                r = parse_package_metadata(name, memelf, c);
                if (r != DWARF_CB_OK)
                        return r;
        }
