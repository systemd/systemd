...
static int context_parse_iovw(Context *context, struct iovec_wrapper *iovw) {
        ...
        if (pidref_is_set(&context->pidref)) {
                ...
                if (!isempty(context->meta[META_ARGV_PIDFD]))  {
                        ...
                        if (r < 0)
                                return log_error_errno(r, "Failed to initialize pidref from pidfd %s: %m", context->meta[META_ARGV_PIDFD]);

                        if (context->pidref.pid != parsed_pid)
                                return log_error_errno(r, "Passed PID " PID_FMT " does not match PID from PIDFD " PID_FMT ": %m", parsed_pid, context->pidref.pid);
                } else {
