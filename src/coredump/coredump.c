...
static int gather_pid_metadata_from_argv(
        ...
        for (int i = 0; i < MIN(argc, _META_ARGV_MAX); i++) {
                ...
                if (i == META_ARGV_PIDFD) {
                        ...
                        assert(pidref_is_set(&context->pidref) || pidref_is_set(&kernel_pidref));
                        r = pidref_in_same_namespace(/* pid1 = */ NULL,
                                                     pidref_is_set(&context->pidref) ? &context->pidref : &kernel_pidref,
                                                     NAMESPACE_PID);
                        if (r < 0)
