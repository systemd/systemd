...
static int gather_pid_metadata_from_argv(
        ...
        for (int i = 0; i < MIN(argc, _META_ARGV_MAX); i++) {
                ...
                if (i == META_ARGV_PID) {
                        ...
                        r = parse_pid(t, &kernel_pid);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse PID \"%s\": %m", t);

                        r = pidref_set_pid(&kernel_pidref, kernel_pid);
                        if (r < 0)
                                return log_error_errno(r, "Failed to initialize pidref from pid " PID_FMT ": %m", kernel_pid);
                }

                if (i == META_ARGV_PIDFD) {
                        PidRef *target_pidref = &kernel_pidref;
                        ...
                        assert(pidref_is_set(target_pidref));
                        r = pidref_in_same_namespace(/* pid1 = */ NULL, target_pidref, NAMESPACE_PID);
                        if (r < 0)
