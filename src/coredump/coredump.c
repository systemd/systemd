...
static int process_kernel(int argc, char* argv[]) {
        ...
        safe_atoi(context.meta[META_ARGV_SIGNAL], &signo);
        log_info("PID %s (%s) crashed with signal %s/%s, processing coredump.",
                        context.meta[META_ARGV_PID], strna(context.meta[META_EXE]),
                        context.meta[META_ARGV_SIGNAL],
                        safe_atoi(context.meta[META_ARGV_SIGNAL], &signo) >= 0
                        ? strna(signal_to_string(signo)) : "n/a");
