...
static int save_external_coredump(
        ...
        if (arg_compress == COREDUMP_COMPRESS_ON_THE_FLY) {
                ...
                r = compress_core(context, uid, fn, input_fd, max_size, &fn_compressed, &fd_compressed, &compressed_size);
                if (r < 0)
                        return r;
                if (r == 0)
                        goto uncompressed;
...
uncompressed:
        ...
        r = compress_core(context, uid, fn, fd, max_size, &fn_compressed, &fd_compressed, &compressed_size);
        if (r <= 0) {
