        ...
        for (int i = 0; i < MIN(argc, _META_ARGV_MAX); i++) {
                ...
                if (i == META_ARGV_PIDFD) {
                        if (!isempty(t)) {
                                ...
                                /* We don't print the FD number in the journal as it's meaningless, but we
                                 * still record that the parsing was done with a kernel-provided fd as it
                                 * means it's safe from races, which is valuable information to provide in
                                 * the journal record. */
                                t = "1";
                        } else
                                t = "0";
