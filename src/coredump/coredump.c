...
static int get_cgroup_memory_limit(uint64_t *ret_max_size) {
        ...
        uint64_t unit_memory_available;
        int r;
        ...
        r = sd_bus_get_property_trivial(
                        ...
                        "MemoryAvailable",
                        &error,
                        't', &unit_memory_available);
        if (r < 0)
                return log_error_errno(r, "Failed to get memory available to current unit: %s", bus_error_message(&error, r));

        *ret_max_size = unit_memory_available;
...
static int save_external_coredump(
        ...
        if (storage_on_tmpfs && arg_compress) {
                ...
                if (statvfs("/var/lib/systemd/coredump/", &sv) >= 0)
                        max_size = MIN(sv.f_frsize * sv.f_bfree, max_size);
