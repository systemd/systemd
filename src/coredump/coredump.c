...
static int parse_config(void) {
        static const ConfigTableItem items[] = {
                ...
                { "Coredump", "RateLimitInterval", config_parse_sec,              0, &arg_ratelimitinterval },
...
static int process_kernel(int argc, char* argv[]) {
        ...
        if (context.is_journald || context.is_pid1)
                ...
                _cleanup_free_ char *process_name;
                process_name = NULL;
                process_name = filename_escape(context.meta[META_COMM]);
                if (!process_name)
                        log_error_errno(r, "Failed to escape process name. Ignoring coredump ratelimit: %m");
                else {
                        /* per process coredump ratelimit */
                        r = coredump_ratelimit(process_name, arg_ratelimitinterval, arg_ratelimitburst);
                        if (r < 0)
                                log_error_errno(r, "Failed to process coredump ratelimit: %m");

                        if (r > 0) {
                                log_warning("Core dump of %s is ratelimited", process_name);
                                goto finish;
                        }
                }
