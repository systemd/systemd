...
static int get_process_container_parent_cmdline(pid_t pid, char** cmdline) {
        ...
        r = get_process_cmdline(container_pid, SIZE_MAX, PROCESS_CMDLINE_SHELL_QUOTE_POSIX, cmdline);
        if (r < 0)
                return r;
...
static int gather_pid_metadata(struct iovec_wrapper *iovw, Context *context) {
        ...
        if (get_process_cmdline(pid, SIZE_MAX, PROCESS_CMDLINE_SHELL_QUOTE_POSIX, &t) >= 0)
