...
static int parse_line(const char *fname, unsigned line, const char *buffer, bool *invalid_config, void *userdata) {
        ...
        if (arg_install_file) {
                /* When installing the loaded rules, refuse unsafe characters, especially newlines, in keys
                 * and values. Otherwise, the installed file may not be parsed correctly. */
                if (!string_is_safe(key, STRING_ALLOW_BACKSLASHES | STRING_ALLOW_QUOTES | STRING_ALLOW_GLOBS) ||
                    !string_is_safe(value, STRING_ALLOW_EMPTY | STRING_ALLOW_BACKSLASHES | STRING_ALLOW_QUOTES | STRING_ALLOW_GLOBS))
                        return log_syntax(NULL, LOG_WARNING, fname, line, SYNTHETIC_ERRNO(EINVAL),
                                          "Ignoring unsafe key and/or value for '%s'.", key);
        } else {
                /* We can't filter out globs at this point, we'll need to do that later.
                 * Keep them when installing the loaded rules, and filter them after writing. */
...
static int install_sysctl_options(OrderedHashmap *sysctl_options, const char *filename) {
        int r;
...
static int parse_conf_filename(const char *src, const char *option, char **dest) {
        ...
        if (!filename_is_valid(name))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid file name specified for %s: %s", option, name);
...
static int parse_argv(int argc, char *argv[], char ***remaining_args) {
        ...
        FOREACH_OPTION_OR_RETURN(c, &opts)
                ...
                OPTION_LONG("install", "FILENAME",
                            "Write loaded settings to the specified file"):
                        r = parse_conf_filename(opts.arg, "--install=", &arg_install_file);
                        if (r < 0)
                                return r;
                        break;
        ...
        *remaining_args = option_parser_get_args(&opts);
        ...
        if (arg_install_file && strv_isempty(*remaining_args))
                return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
        ...
        if (arg_revert_file) {
                if (!strv_isempty(*remaining_args))
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "Positional arguments are not allowed with --revert=.");

                if (arg_cat_flags != CAT_CONFIG_OFF)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "--cat-config/--tldr are not allowed with --revert=.");

                if (arg_install_file)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                               "--install= is not allowed with --revert=.");
...
static int run(int argc, char *argv[]) {
        ...
        if (!strv_isempty(args)) {
                ...
                if (arg_install_file) {
                        r = install_sysctl_options(sysctl_options, arg_install_file);
                        if (r < 0)
                                return r;
