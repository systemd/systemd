...
static int sysctl_write_or_warn(const char *key, const char *value, bool ignore_failure) {
        ...
        if (r < 0) {
                 ...
                 * cannot write it, then log about the issue, and proceed without failing. Unless if
                 * strict mode (arg_strict = true) is enabled, in which case we should fail. (EROFS is treated
