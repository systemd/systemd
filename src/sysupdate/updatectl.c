...
static int parse_describe(sd_bus_message *reply, Version *ret) {
        ...
        r = json_parse(version_json, 0, &json, NULL, NULL);
        if (r < 0)
                return log_error_errno(r, "Failed to parse JSON: %m");

        assert(json_variant_is_object(json));

        const char *key;
        JsonVariant *value;
        JSON_VARIANT_OBJECT_FOREACH(key, value, json) {
                if (streq(key, "version")) {
                        assert(json_variant_is_string(value));
                        assert_se(v.version = strdup(json_variant_string(value)));
                } else if (streq(key, "newest")) {
                        assert(json_variant_is_boolean(value));
                        if (json_variant_boolean(value))
                        ...
                        JsonVariant *entry;
                        assert(json_variant_is_array(value));
                        JSON_VARIANT_ARRAY_FOREACH(entry, value) {
                                assert(json_variant_is_string(entry));
                                r = strv_extend(&v.changelog, json_variant_string(entry));
                                if (r < 0)
                                        log_error_errno(r, "Failed to parse JSON: %m");
                        }
                } else if (streq(key, "contents")) {
                        r = json_variant_format(value, 0, &v.contents_json);
                        if (r < 0)
                                log_error_errno(r, "Failed to parse JSON: %m");
                } else
                        assert_not_reached();
