...
static int ensure_targets(sd_bus *bus, char **argv, char ***ret_targets) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to parse listed Targets: %m");
...
static int list_targets(sd_bus *bus) {
        ...
        for (size_t i = 0; i < n; i++) {
                ...
                r = sd_bus_call_method(bus, bus_sysupdate_mgr->destination, target_paths[i], SYSUPDATE_TARGET_INTERFACE, "GetVersion", &error, &reply, NULL);
                if (r < 0)
...
static int parse_describe(sd_bus_message *reply, Version *ret) {
        ...
        static const sd_json_dispatch_field version_dispatch_table[] = {
                { "version",        SD_JSON_VARIANT_STRING,  sd_json_dispatch_const_string, offsetof(VersionJSON, version),   0 },
                { "newest",         SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_stdbool,      offsetof(VersionJSON, newest),    0 },
                { "available",      SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_stdbool,      offsetof(VersionJSON, available), 0 },
                { "installed",      SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_stdbool,      offsetof(VersionJSON, installed), 0 },
                { "obsolete",       SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_stdbool,      offsetof(VersionJSON, obsolete),  0 },
                { "protected",      SD_JSON_VARIANT_BOOLEAN, sd_json_dispatch_stdbool,      offsetof(VersionJSON, protected), 0 },
                { "changelog_urls", SD_JSON_VARIANT_ARRAY,   sd_json_dispatch_strv,         offsetof(VersionJSON, changelog), 0 },
                { "contents",       SD_JSON_VARIANT_ARRAY,   sd_json_dispatch_variant,      offsetof(VersionJSON, contents),  0 },
                {},
        };

        VersionJSON deserialize = {};
        r = sd_json_dispatch(json, version_dispatch_table, SD_JSON_ALLOW_EXTENSIONS, &deserialize);
        if (r < 0)
                return log_error_errno(r, "Failed to parse JSON: %m");

        v.version = strdup(deserialize.version);
	if (!v.version)
	        return log_oom();
        SET_FLAG(v.flags, UPDATE_NEWEST, deserialize.newest);
        SET_FLAG(v.flags, UPDATE_AVAILABLE, deserialize.available);
        SET_FLAG(v.flags, UPDATE_INSTALLED, deserialize.installed);
        SET_FLAG(v.flags, UPDATE_OBSOLETE, deserialize.obsolete);
        SET_FLAG(v.flags, UPDATE_PROTECTED, deserialize.protected);
        v.changelog = strv_copy(deserialize.changelog);
	if (!v.changelog)
		return log_oom();
        r = sd_json_variant_format(deserialize.contents, 0, &v.contents_json);
        if (r < 0)
...
static int describe(sd_bus *bus, const char *target_path, const char *version) {
        ...
        STRV_FOREACH(url, v.changelog) {
                ...
                r = strv_consume(&changelog_links, TAKE_PTR(changelog_link));
                if (r < 0)
                        return log_oom();
...
static int check_describe_finished(sd_bus_message *reply, void *userdata, sd_bus_error *ret_error) {
	...
	char *version = NULL;
        ...
        r = table_add_many(table,
                           ...
                           TABLE_STRING,  update,
                           TABLE_SET_URL, strv_isempty(v.changelog) ? NULL : v.changelog);
        if (r < 0)
                return table_log_add_error(r);
...
static int check_finished(sd_bus_message *reply, void *userdata, sd_bus_error *ret_error) {
        ...
        r = sd_bus_call_method_async(op->bus, NULL, bus_sysupdate_mgr->destination, op->target_path, SYSUPDATE_TARGET_INTERFACE, "Describe",
