...
static int update_render_progress(sd_event_source *source, void *userdata) {
        ...
        HASHMAP_FOREACH_KEY(progress, target, map)
        ...
        HASHMAP_FOREACH_KEY(progress, target, map) {
                ...
                if (progress == UPDATE_PROGRESS_FAILED) {
                        fprintf(stdout, "%s%s%s %s\n",
                                ansi_red(), special_glyph(SPECIAL_GLYPH_CROSS_MARK), ansi_normal(),
                                target);
                        total += 100;
                } else if (progress < 0) {
                        ...
                        fprintf(stdout, "%s%s%s %s\n",
                                ansi_green(), special_glyph(SPECIAL_GLYPH_CHECK_MARK), ansi_normal(),
                                target);
                        total += progress;
                } else {
        ...
        if (n > 1) {
                if (colors_enabled())
                        fputs(ANSI_ERASE_TO_END_OF_LINE, stdout);
                if (!exiting) {
                        draw_progress_bar("TOTAL", progress, max_target_len, true);
                        if (!colors_enabled())
                                fputs("\n", stdout);
                }
        }
...
static int update_started(sd_bus_message *reply, void *userdata, sd_bus_error *ret_error) {
        ...
        r = sd_bus_message_read(reply, "uo", &data->job_id, &job_path); // TODO
