...
static Job *job_free(Job *j) {
        ...
        if (j->manager) {
                ...
                if (hashmap_isempty(j->manager->jobs))
                        hashmap_clear(j->manager->targets);
        }
...
static void job_on_ready(Job *j) {
        ...
        if (r < 0)
                log_warning_errno(r, "Failed to reply to method call, ignoring: %m");
}
...
static int job_start(Job *j) {
        ...
        if (IN_SET(j->type, JOB_UPDATE, JOB_VACUUM) && j->target->busy) {
                log_info("Target %s busy, ignoring job.", j->target->name);
                return -EBUSY;
        }

        stdout_fd = memfd_new("sysupdate-stdout");
        if (stdout_fd < 0)
                return log_error_errno(errno, "Failed to create memfd: %m");
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to fork (sd-sysupdate): %m");
        if (r == 0) {
                /* Child */
...
static int target_method_get_version(sd_bus_message *msg, void *userdata, sd_bus_error *error) {
        ...
        sd_json_variant *version_json = sd_json_variant_by_key(v, "current");
        if (!version_json)
                return -EINVAL;
...
static int target_get_appstream(char ***ret, Target *t) {
        _cleanup_free_ char *target_arg;
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *v = NULL;
        int r;
        ...
        sd_json_variant *appstream_url_json = sd_json_variant_by_key(v, "appstream_urls");
...
static int target_method_get_appstream(sd_bus_message *msg, void *userdata, sd_bus_error *error) {
        ...
        r = target_get_appstream(&appstream_urls, t);
        if (r < 0)
                return r;
...
static int method_list_appstream(sd_bus_message *msg, void *userdata, sd_bus_error *error) {
        ...
        HASHMAP_FOREACH(t, m->targets) {
                ...
                r = target_get_appstream(&target_appstream, t);
                if (r < 0)
                        return r;
