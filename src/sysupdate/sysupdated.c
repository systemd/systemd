...
typedef int (*JobComplete)(sd_bus_message *, const Job *, sd_json_variant *, sd_bus_error *);
        ...
        if (j->dbus_msg && j->complete_cb) {
                if (sd_bus_error_is_set(&error)) {
                        ...
                        if (r < 0 || sd_bus_error_is_set(&error)) {
                                log_warning_errno(r, "Error during execution of job callback: %m");
...
static int cache_string_from_json(char **ret, const char *key, sd_json_variant *json) {
        sd_json_variant *v;
        const char *j;
        _cleanup_free_ char *s = NULL;

        v = sd_json_variant_by_key(json, key);
        if (!v)
                return -EINVAL;
...
static int cache_strv_from_json(char ***ret, const char *key, sd_json_variant *json) {
        sd_json_variant *v;
        _cleanup_free_ char **s = NULL;
        int r;
...
static int target_method_check_new_finish(
        ...
        if (sd_json_variant_is_null(json))
                reply = "";
        else
                reply = sd_json_variant_string(json);
        if (!reply)
                return -EINVAL;
