...
static int job_start(Job *j) {
        ...
        if (r < 0)
                return r;
        assert_se(sd_event_source_set_child_process_own(j->child, true) >= 0);
        TAKE_PID(pid); /* Don't kill the child */
...
static int target_method_describe_finish(
        ...
        r = json_variant_format(json, 0, &reply);
        if (r < 0)
                return r;
...
static int target_method_update(sd_bus_message *msg, void *userdata, sd_bus_error *error) {
        ...
        return sd_bus_reply_method_return(msg, "uo", j->id, TAKE_PTR(j)->object_path);
}
...
static int manager_new(Manager **ret) {
        ...
        if (r < 0)
                return r;
        assert_se(sd_event_source_set_io_fd_own(m->notify_event, true) >= 0);
        TAKE_FD(notify_fd); /* Don't close the fd */
...
static int manager_enumerate_components(Manager *m) {
        ...
        r = simple_fork_sysupdate(&v, "components", NULL);
        if (r < 0)
                return r;
