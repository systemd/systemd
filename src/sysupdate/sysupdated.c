...
static int job_new(JobType type, Target *t, sd_bus_message *msg, JobComplete cb, Job **ret) {
        ...
        *j = (Job) {
        ...
        r = hashmap_put(t->manager->jobs, UINT32_TO_PTR(j->id), j);
        if (r < 0)
                return r;
...
static int job_start(Job *j) {
        ...
        if (r == 0) {
                ...
                if (j->target->class == TARGET_HOST)
                        ...
                        r = asprintf(&target_arg, "--component=%s", j->target->name);
...
static int job_cancel(Job *j) {
        ...
        r = kill_and_sigcont(j->pid, j->n_cancelled < 3 ? SIGTERM : SIGKILL);
        if (r < 0)
                return r;
...
static int target_method_update(sd_bus_message *msg, void *userdata, sd_bus_error *error) {
        ...
        return sd_bus_reply_method_return(msg, "o", TAKE_PTR(j)->object_path);
}
...
static int manager_on_notify(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        log_debug("Got percentage from worker " PID_FMT ": %u%%", j->pid, j->progress_percent);
        return 0;
}
...
static int manager_new(Manager **ret) {
        ...
        r = sd_event_add_io(m->event, &m->notify_event_source, m->notify_fd, EPOLLIN,
                            manager_on_notify, m);
        if (r < 0)
                return r;
...
static int manager_discover_images(Manager *m, TargetClass class) {
...
static int manager_enumerate_components(Manager *m) {
        ...
        if (r == 0) {
                /* Child */
                execl(sysupdate_binary_path(), "systemd-sysupdate", "components", NULL);
                log_error_errno(errno, "Failed to execute systemd-sysupdate: %m");
                _exit(EXIT_FAILURE);
        }
...
static int manager_enumerate_targets(Manager *m) {
        const TargetClass discoverable_classes[] = { TARGET_MACHINE, TARGET_PORTABLE,
                                                     TARGET_SYSEXT, TARGET_CONFEXT };
        Target *t;
        int r;
