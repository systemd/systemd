...
static int target_method_vacuum_finish(
        ...
        v = sd_json_variant_by_key(json, "disabled_transfers");
        if (!v)
                return log_sysupdate_bad_json(SYNTHETIC_ERRNO(EPROTO), "vacuum", "Missing key 'disabled_transfers'");
        if (!sd_json_variant_is_unsigned(v))
                return log_sysupdate_bad_json(SYNTHETIC_ERRNO(EPROTO), "vacuum", "Key 'disabled_transfers' should be an unsigned int");
