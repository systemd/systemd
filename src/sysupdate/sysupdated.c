...
static void job_on_ready(Job *j) {
        ...
        r = sd_bus_reply_method_return(msg, "uo", j->id, j->object_path);
        if (r < 0)
...
static int job_start(Job *j) {
        ...
        if (r == 0) {
                ...
                if (j->target->class != TARGET_HOST) {
                        if (j->target->class == TARGET_COMPONENT)
                                target_arg = strjoin("--component=", j->target->name);
                        else if (IN_SET(j->target->image_type, IMAGE_DIRECTORY, IMAGE_SUBVOLUME))
                                target_arg = strjoin("--root=", j->target->path);
                        else if (IN_SET(j->target->image_type, IMAGE_RAW, IMAGE_BLOCK))
                                target_arg = strjoin("--image=", j->target->path);
                        else
                                assert_not_reached();
...
static int simple_fork_sysupdate(JsonVariant **ret, ...) {
...
static int target_property_get_version(
        ...
        if (!t->version) {
                r = simple_fork_sysupdate(&v, "--offline", "list", NULL);
                if (r < 0)
                        return r;

                r = target_cache_version_from_json(t, v);
                if (r < 0)
                        return r;
        }
...
static int target_method_describe_finish(
                ...
                sd_bus_error *error) {
        _cleanup_(sd_bus_message_unrefp) sd_bus_message *reply = NULL;
        _cleanup_free_ char *contents_str;
        const char *version;
        bool newest, available, installed, obsolete, protected;
        JsonVariant *contents;
        JsonDispatch dispatch_table[] = {
                { "version",   JSON_VARIANT_STRING,  json_dispatch_const_string,  PTR_TO_SIZE(&version),   JSON_MANDATORY },
                { "newest",    JSON_VARIANT_BOOLEAN, json_dispatch_boolean,       PTR_TO_SIZE(&newest),    JSON_MANDATORY },
                { "available", JSON_VARIANT_BOOLEAN, json_dispatch_boolean,       PTR_TO_SIZE(&available), JSON_MANDATORY },
                { "installed", JSON_VARIANT_BOOLEAN, json_dispatch_boolean,       PTR_TO_SIZE(&installed), JSON_MANDATORY },
                { "obsolete",  JSON_VARIANT_BOOLEAN, json_dispatch_boolean,       PTR_TO_SIZE(&obsolete),  JSON_MANDATORY },
                { "protected", JSON_VARIANT_BOOLEAN, json_dispatch_boolean,       PTR_TO_SIZE(&protected), JSON_MANDATORY },
                { "contents",  JSON_VARIANT_ARRAY,   json_dispatch_variant_noref, PTR_TO_SIZE(&contents),  JSON_MANDATORY },
                {}
        };
        int r;
        ...
        r = json_dispatch(json, dispatch_table, NULL, 0, NULL);
        if (r < 0)
                return r;
...
static int target_method_update_finished_early(
         ...
         * This happens when there is no candidate, or specified update is already installed. */
...
static const sd_bus_vtable target_vtable[] = {
        ...
        SD_BUS_PROPERTY("Path", "s", NULL, offsetof(Target, path),
                        SD_BUS_VTABLE_PROPERTY_CONST),
        SD_BUS_PROPERTY("Version", "s", target_property_get_version, 0, 0),
        ...
        SD_BUS_METHOD_WITH_ARGS("Update",
                                ...
                                SD_BUS_RESULT("u", job_id, "o", job_path),
...
static int manager_on_notify(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        HASHMAP_FOREACH(j, m->jobs) {
                ...
                r = sd_event_source_get_child_pid(j->child, &pid);
                if (r < 0) {
                        log_warning("Found job without a child process, ignoring.");
                        continue;
                }
...
static int manager_enumerate_components(Manager *m) {
        ...
        r = simple_fork_sysupdate(&json, "components", NULL);
        if (r < 0)
                return r;
