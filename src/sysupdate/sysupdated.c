...
static void manager_became_idle(Manager *m);
...
assert_cc(_IMAGE_CLASS_MAX == _TARGET_CLASS_IS_IMAGE_CLASS_MAX);
...
static int job_on_exit(sd_event_source *s, const siginfo_t *si, void *userdata) {
        ...
        if (j->manager && hashmap_isempty(j->manager->jobs))
                manager_became_idle(j->manager);
...
static int target_get_appstream(Target *t, char ***ret) {
        ...
        if (r < 0) {
                ...
                log_debug("Invalid JSON response from 'systemd-sysupdate list': Expected array of strings for key 'appstream_urls'");
                return -EINVAL;
        }
