...
static int get_sysext_overlay_block(const char *p, dev_t *ret) {
        ...
        if (r == 0)
                return log_debug_errno(SYNTHETIC_ERRNO(ENOTTY), "Backing file system of '%s' is not an overlayfs: %m", j);

        _cleanup_free_ char *buf = NULL;
        r = read_one_line_file_at(fd, "backing", &buf);
        if (r < 0)
                return log_debug_errno(r, "Failed to read contents of '%s/dev': %m", j);

        r = parse_devnum(buf, ret);
        if (r < 0)
                return log_debug_errno(r, "Failed to parse contents of '%s/dev': %m", j);
...
int resource_resolve_path(
        ...
        if (rr->path_auto) {
                ...
                if (r < 0) {
                        ...
                        if (r == 0) {
                                /* Not backed by a block device? Let's see if this is a sysext overlayfs instance */
