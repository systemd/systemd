...
static int resource_load_from_directory_recursive(
        ...
        for (;;) {
                ...
                /* With MatchBasename=yes we descend into any subdirectory when we look for regular files */
                if (rr->match_basename && S_ISDIR(st.st_mode) && S_ISREG(m)) {
                        _cleanup_closedir_ DIR *subdir = NULL;

                        subdir = xopendirat(dirfd(d), de->d_name, 0);
                        if (!subdir) {
                                if (errno != ENOENT)
                                        log_debug_errno(errno, "Failed to open subdirectory '%s', ignoring: %m", de->d_name);
                                continue;
                        }
                ...
                const char *match_str = rr->match_basename ? de_d_name_stripped : rel_joined_for_matching;
                r = pattern_match_many(rr->patterns, match_str, &extracted_fields);
                /* Here we also descend because the match pattern itself uses subdirectories */
...
static int resource_load_from_web(
        ...
        while (left > 0) {
                 ...
                 * absolute paths and any paths containing "." or ".." */
                if (path_is_absolute(fn) || !path_is_normalized(fn))
                ...
                if (r < 0)
                        return r;
                if (r == 0) {
                        /* If this isn't a magic file, then do the pattern matching but
                         * with MatchBasename=yes only for the last component of the path */
                        const char *match_str = fn;
                        if (rr->match_basename) {
...
int resource_load_instances(Resource *rr, bool verify, Hashmap **web_cache) {
        ...
        /* There are various cases where the same version can be provided by different files
         * either due to multiple match patterns defined in a naming scheme transition or due to
         * a subdirectory containing the same file again with MatchBasename=yes or due to a match
         * pattern wildcard like @s or @t being used and files with different values there still
         * report the same version. Since this could lead to unexpected behavior we report this
         * here to the user. */
        for (size_t i = 1; i < rr->n_instances; i++) {
                Instance *a = rr->instances[i - 1], *b = rr->instances[i];

                if (streq(a->metadata.version, b->metadata.version) && !streq(a->path, b->path))
                        log_info("Multiple instances of version '%s' found at '%s' and '%s'.",
                                 a->metadata.version, a->path, b->path);
        }
