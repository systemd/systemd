...
static bool resource_has_basename_pattern(Resource *rr) {
        assert(rr);
...
static int resource_load_from_directory_recursive(
        ...
        for (;;) {
                ...
                /* If any pattern has "@W/" descend into subdirectories when looking for regular files */
                if (S_ISDIR(st.st_mode) && S_ISREG(m) && resource_has_basename_pattern(rr)) {
...
static int resource_load_from_web(
        ...
        while (left > 0) {
                ...
                if (r < 0)
                        return r;
                if (r == 0) {
                        /* Pattern matching against the full manifest entry. A pattern with the "@W/"
                         * prefix matches against the basename only; that's handled by pattern_match_many. */
...
int resource_load_instances(Resource *rr, bool verify, Hashmap **web_cache) {
         ...
         * the same file again when a "@W/" pattern is used, or due to a wildcard like @s or @t being used
         * and the files offer different values there while reporting the same version. Since this could
         * lead to unexpected behavior we report this here to the user. */
