...
static int config_parse_resource_pattern(
        ...
        for (;;) {
                ...
                if (r < 0) {
                        log_syntax(unit, LOG_WARNING, filename, line, r,
                                   "Failed to extract first pattern from Pattern=, ignoring: %s", rvalue);
                        return 0;
                }
                if (r == 0)
                        break;
...
int transfer_read_definition(Transfer *t, const char *path) {
        ...
        if (t->source.path) {
                if (RESOURCE_IS_FILESYSTEM(t->source.type) || t->source.type == RESOURCE_PARTITION)
                        if (!path_is_absolute(t->source.path) || !path_is_normalized(t->source.path))
                                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                                  "Source path is not a normalized, absolute path.");

                if (RESOURCE_IS_HTTP(t->source.type))
                        if (!http_url_is_valid(t->source.path))
                                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                                  "Source path is not a valid HTTP or HTTPS URL.");
        }

        if (strv_isempty(t->source.patterns))
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                  "Source specification lacks Pattern=.");
        ...
        if (t->target.path &&
            (!path_is_absolute(t->target.path) || !path_is_normalized(t->target.path)))
                return log_syntax(NULL, LOG_ERR, path, 1, SYNTHETIC_ERRNO(EINVAL),
                                  "Target path is not a normalized, absolute path.");
        ...
        if (t->instances_max == 0)
                t->instances_max = t->target.type == RESOURCE_PARTITION ? UINT64_MAX : 3;
...
int transfer_resolve_paths(
         ...
         * automatically determine backing block device, so that users can reference block devices by mount
         * point. */
...
int transfer_acquire_instance(Transfer *t, Instance *i) {
        ...
        log_info("%s Acquiring %s %s %s...", special_glyph(SPECIAL_GLYPH_DOWNLOAD), i->path, special_glyph(SPECIAL_GLYPH_ARROW), where);
        ...
        case RESOURCE_REGULAR_FILE:
                ...
                case RESOURCE_REGULAR_FILE:
                        ...
                        if (r == 0) {
                                ...
                                execv(import_binary_path(), (char *const*) cmdline);
                                log_error_errno(errno, "Failed to execute %s tool: %m", import_binary_path());
                                _exit(EXIT_FAILURE);
                        }
                        break;
        ...
        case RESOURCE_SUBVOLUME:
                ...
                if (r == 0) {
                        ...
                        (void) unsetenv("NOTIFY_SOCKET");
                        execv(import_fs_binary_path(), (char *const*) cmdline);
                        log_error_errno(errno, "Failed to execute %s tool: %m", import_fs_binary_path());
                        _exit(EXIT_FAILURE);
                }
                break;
