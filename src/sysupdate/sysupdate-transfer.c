...
static int run_helper(
        ...
        _cleanup_(sd_event_source_unrefp) sd_event_source *notify_source = NULL;
        ...
        r = sd_event_add_child(event, NULL, pid, WEXITED, helper_on_exit, (void*) name);
        if (r < 0)
                return r;

        /* Propagate sd_notify calls */
        userdata->pid = pid;
        r = sd_event_add_io(event, &notify_source, TAKE_FD(fd), EPOLLIN, helper_on_notify, userdata);
        if (r < 0)
                return r;
