...
static PatternElementType pattern_element_type_from_char(char c) {
        ...
        static const char map[_PATTERN_ELEMENT_TYPE_MAX] = {
                [PATTERN_VERSION]         = 'v',
                [PATTERN_PARTITION_UUID]  = 'u',
                [PATTERN_PARTITION_FLAGS] = 'f',
                [PATTERN_MTIME]           = 't',
                [PATTERN_MODE]            = 'm',
                [PATTERN_SIZE]            = 's',
                [PATTERN_TRIES_DONE]      = 'd',
                [PATTERN_TRIES_LEFT]      = 'l',
                [PATTERN_NO_AUTO]         = 'a',
                [PATTERN_READ_ONLY]       = 'r',
                [PATTERN_GROWFS]          = 'g',
                [PATTERN_SHA256SUM]       = 'h',
        ...
        for (PatternElementType t = 0; t < _PATTERN_ELEMENT_TYPE_MAX; t++)
                if (c == map[t])
                        return t;
...
static int pattern_split(
        ...
        bool dollar = false, last_literal = true;
        ...
        for (e = pattern; *e != 0; e++) {
                if (*e == '$') {
                        ...
                        /* Two dollar signs in a sequence, write out one */
                        dollar = false;
                        ...
                        if (t < 0)
                                return log_debug_errno(SYNTHETIC_ERRNO(EINVAL), "Unknown pattern field marker '$%c'.", *e);
...
int pattern_match(const char *pattern, const char *s, InstanceMetadata *ret) {
        ...
        LIST_FOREACH(elements, e, elements) {
                ...
                case PATTERN_VERSION:
                        ...
                        free_and_replace(found.version, t);
                        break;

                case PATTERN_PARTITION_UUID: {
                        sd_id128_t id;

                        /* Check if the uuid fits into the name */
                        if (strlen(t) != SD_ID128_STRING_MAX-1)
                                goto nope;

                        if (sd_id128_from_string(t, &id) < 0)
                                goto nope;

                        /* Field used twice and doesn't match? Refuse */
                        if (found.partition_uuid_set && !sd_id128_equal(found.partition_uuid, id))
                                goto nope;
...
int pattern_format(
        ...
        LIST_FOREACH(elements, e, elements) {
                ...
                default:
                        assert_not_reached("unexpected pattern element");
                }
        }
