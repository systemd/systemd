...
static int context_installdb_acquire_fd(Context *c, bool make) {
        ...
        if (c->installdb_fd == -ENOENT && !make)
                return 0;
        if (c->installdb_fd < 0)
                return log_error_errno(c->installdb_fd, "Failed to open install database: %m");
...
static int installdb_make_names(const char *path, const char *pattern, char **ret_key, char **ret_value) {
        ...
        _cleanup_free_ char *h = hexmem(SHA256_DIRECT(s, strlen(s)), SHA256_DIGEST_SIZE);
        if (!h)
                return log_oom();
...
int context_installdb_record(
        ...
        STRV_FOREACH(i, patterns) {
                ...
                r = installdb_make_names(path, *i, &key, &value);
                if (r < 0)
                        return r;
...
static int context_installdb_process_directory(
        ...
        FOREACH_ARRAY(_d, de->entries, de->n_entries) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to match pattern '%s' against '%s': %m", pattern, p);
                if (r == PATTERN_MATCH_NO) /* No match, do not clean up */
                        continue;
                if (r == PATTERN_MATCH_RETRY) {
                        /* Might match in a subdirectory */

                        keep_installdb = true; /* this inode is referenced as prefix of the pattern, thus keep the installdb entry for it */

                        if (d->d_type != DT_DIR)
                                continue;

                        _cleanup_close_ int subdir_fd = RET_NERRNO(openat(dir_fd, d->d_name, O_DIRECTORY|O_CLOEXEC));
                        if (subdir_fd == -ENOENT)
                                continue;
                        if (subdir_fd < 0) {
                        ...
                        RET_GATHER(ret, context_installdb_process_directory(c, path, p, subdir_fd, subde, pattern));
                        continue;
                }
                ...
                r = rm_rf_child(dir_fd, d->d_name, REMOVE_PHYSICAL);
                if (r < 0) {
                        if (r != -ENOENT)
...
static int context_installdb_process_entry(
        ...
        _cleanup_free_ char *h = hexmem(SHA256_DIRECT(value, strlen(value)), SHA256_DIGEST_SIZE);
        if (!h)
                return log_oom();
        ...
        _cleanup_close_ int dir_fd = chase_and_open(path, arg_root, CHASE_MUST_BE_DIRECTORY|CHASE_PREFIX_ROOT, O_DIRECTORY|O_CLOEXEC, /* ret_path= */ NULL);
        if (dir_fd == -ENOENT) {
