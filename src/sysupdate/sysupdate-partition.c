...
int find_suitable_partition(
        ...
        for (size_t i = 0; i < n_partitions; i++)  {
                ...
                if (smallest.partno != SIZE_MAX_MAX && smallest.size <= pinfo.size) /* already found smaller */
                        continue;
...
int patch_partition(
        ...
        /* Make sure udev doesn't read the device while we make changes */
        if (flock(fd, LOCK_EX) < 0)
                return log_error_errno(r, "Failed to lock block device '%s': %m", device);
