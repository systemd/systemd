        ...
        FOREACH_OPTION(c, &state, /* on_error= */ return c)
                ...
                OPTION('C', "component", "NAME",
                       "Select component to update"):
                        ...
                        r = component_name_valid(state.argument);
                        if (r < 0)
                                return log_error_errno(r, "Failed to determine if component name is valid: %m");
                        if (r == 0)
                                return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Component name invalid: %s", state.argument);

                        r = free_and_strdup_warn(&arg_component, state.argument);
                        if (r < 0)
                                return r;
                ...
                OPTION_LONG("definitions", "DIR",
                        ...
                        r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_definitions);
                        if (r < 0)
                                return r;
                        break;
                ...
                OPTION_LONG("transfer-source", "PATH",
                        ...
                        r = parse_path_argument(state.argument, /* suppress_root= */ false, &arg_transfer_source);
                        if (r < 0)
                                return r;
                ...
                OPTION('m', "instances-max", "INT",
                       "How many instances to maintain"):
                        r = safe_atou64(state.argument, &arg_instances_max);
                        if (r < 0)
                                return log_error_errno(r, "Failed to parse --instances-max= parameter: %s", state.argument);
                ...
                OPTION_LONG("verify", "BOOL",
                        ...
                        r = parse_boolean_argument("--verify=", state.argument, &b);
                        if (r < 0)
                                return r;
