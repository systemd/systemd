...
/* Assumes validation of the component has already been done */
static int context_load_paths_from_component(Context *context, const char *component_name) {
        assert(context);
        assert(component_name);

        assert(!context->component);

        context->component = strdup(component_name);
...
static int target_compare_func(const Target *x, const Target *y) {
        ...
        r = target_identifier_compare_func(&x->identifier, &y->identifier);
        if (r != 0)
                return r;

        r = strcmp(x->current_version, y->current_version);
        if (r != 0)
                return r;

        r = CMP(x->current_version_is_pending, y->current_version_is_pending);
        if (r != 0)
                return r;

        r = strv_compare(x->all_versions, y->all_versions);
        if (r != 0)
                return r;

        return strv_compare(x->appstream_urls, y->appstream_urls);
}
...
static int context_enumerate_components(Context *context, Set **targets) {
        ...
        STRV_FOREACH(component, component_names) {
                ...
                r = context_load_paths_from_component(&component_context, *component);
                if (r < 0)
                        return r;
