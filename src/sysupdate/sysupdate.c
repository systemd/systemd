...
static int read_definitions(
        ...
        _cleanup_strv_free_ char **owned_dirs = NULL, **files = NULL;
        char **dirs = NULL;
        ...
        if (directory)
                ...
                owned_dirs = new0(char*, strv_length(l) + 1);
                if (!owned_dirs)
                        return log_oom();
                ...
                dirs = CONF_PATHS_STRV("sysupdate.d");
        else
                dirs = CONF_PATHS_STRV("sysregistry.d");

        r = conf_files_list_strv(&files, ".conf", root, CONF_FILES_REGULAR|CONF_FILES_FILTER_MASKED, (const char**) dirs);
        if (r < 0)
