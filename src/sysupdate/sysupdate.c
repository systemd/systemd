...
static int context_read_config(Context *c) {
        ...
        /* Command-line settings take precedence over the configuration file. */
        if (!c->source_url)
                c->source_url = TAKE_PTR(source_url);
...
static int parse_argv(int argc, char *argv[], char ***remaining_args) {
        ...
        FOREACH_OPTION_OR_RETURN(c, &opts)
                ...
                OPTION_LONG("source-url", "URL",
                        ...
                        if (!http_url_is_valid(opts.arg) && !file_url_is_valid(opts.arg))
