...
/* Most of these macros rely on __VA_OPT__() to determine if there are variadic args; 'any variadic args'
 * means __VA_OPT__() evaluates to its content, and 'no variadic args' means __VA_OPT__() evaluates to
 * nothing. Note that whitespace is not a preprocessor token, so a single whitespace-only arg is the same as
 * no args.
 *
 * For example these calls all evaluate to 2:
 *   VA_IF_ELSE(1,2)
 *   VA_IF_ELSE(1,2,)
 *   VA_IF_ELSE(1,2, )
 *   #define NONE
...
#define __VA_REPEAT_TOOMANY_0x01ff
...
/* Evaluates to token 1 if both args are non-empty, otherwise evaluates to nothing. */
#define VA_AND(x, y) VA_NOT(VA_NOT(x) VA_NOT(y))

/* Evaluates to token 1 if either arg is non-empty, otherwise evaluates to nothing. */
