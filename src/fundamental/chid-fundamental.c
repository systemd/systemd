...
#  include <uchar.h>
#  include <utf8.h>
#define strsize16(str) char16_strlen(str) * sizeof(char16_t)
#endif
...
static void get_chid(const char16_t *const smbios_fields[static _CHID_SMBIOS_FIELDS_MAX], uint32_t mask, WindowsGuid *ret_chid) {
        assert(mask);
        assert(ret_chid);
        uint8_t hash[SHA1_DIGEST_SIZE] = {};
        const WindowsGuid namespace = { 0x12d8ff70, 0x7f4c, 0x7d4c, {} }; /* Swapped to BE */
...
void chid_calculate(const char16_t *const smbios_fields[static _CHID_SMBIOS_FIELDS_MAX], WindowsGuid ret_chids[static CHID_COUNT_MAX]) {
        for (size_t i = 0; i < _CHID_SMBIOS_FIELDS_MAX; i++)
                if (chid_smbios_table[i] != 0)
                        get_chid(smbios_fields, chid_smbios_table[i], &ret_chids[i]);
                else
                        memzero(&ret_chids[i], sizeof(WindowsGuid));
}
