...
/* Similar to ((t *) (void *) (p)) to cast a pointer. The macro
 * asserts that the pointer has a suitable alignment for type "t".
 * This exists for places where otherwise "-Wcast-align=strict"
 * would issue a warning and if you want to assert that the cast
 * gives a pointer of suitable alignment. */
#define PTRFORCEALIGN(t, p)                                             \
        ({                                                              \
                const void * const _p = (p);                            \
                                                                        \
